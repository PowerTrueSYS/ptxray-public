...
# === BEGIN currency-gate ===
# The HW_GEN table below is the payload the ibm-aix-lifecycle source-registry
# row (data/source-registry.json) attests to. A confident verdict over a row
# that is unloaded, unverified or stale would be a hollow PASS, so refuse with a
# typed NOT_ASSESSED unless the accessor reports CURRENT. currency_source_status
# prints "<STATUS>\t<reason>"; every non-CURRENT status refuses and the inline
# table is never used as a fallback.
CURRENCY_ROW=$(currency_source_status ibm-aix-lifecycle)
CURRENCY_TAB=$(printf '\t')
CURRENCY_STATUS=${CURRENCY_ROW%%"$CURRENCY_TAB"*}
CURRENCY_REASON=${CURRENCY_ROW#*"$CURRENCY_TAB"}
if [ "$CURRENCY_STATUS" != "CURRENT" ]; then
  add lifecycle aix_uak_expiry "AIX Update Access Key" NOT_ASSESSED med \
      "not assessed - ibm-aix-lifecycle source registry row is $CURRENCY_STATUS: $CURRENCY_REASON" \
      "The ibm-aix-lifecycle source-registry row this check scores against is not CURRENT ($CURRENCY_STATUS: $CURRENCY_REASON); the inline HW_GEN lifecycle table cannot be trusted as current, so this finding is refused rather than scored against a stale or unverified reference." \
