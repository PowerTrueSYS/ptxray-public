...
_AIXRAY_SESSION_KEYS=""
  ...
  if [ "$ROOTPRC" -ne 0 ] || [ "$ROOTLRC" -ne 0 ]; then
    add resilience bootlist "Boot device list" NOT_ASSESSED low \
        "not assessed — 'lsvg -p rootvg' rc=$ROOTPRC, 'lsvg -l rootvg' rc=$ROOTLRC" \
        "The rootvg layout probes were run, but did not return a readable layout, so the mirror precondition cannot be verified and the boot device list is not assessed." \
        "run 'lsvg -p rootvg' and 'lsvg -l rootvg' as root and verify the rootvg layout."
  elif [ "$NPV" -ge 2 ] && [ "$MIRR" = "full" ]; then
    ...
    add resilience bootlist "Boot device list" NOT_APPLICABLE low \
        "rootvg not fully mirrored (NPV=$NPV, MIRR=$MIRR)" \
        "The boot device list is only meaningful when rootvg is fully mirrored across at least two physical disks; this box does not meet that precondition." \
        "n/a"
  fi
}
