...
AIXRAY_TOOL=ck-cde-cmsd


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # cde_cmsd — detect whether the CDE calendar daemon (rpc.cmsd) is active in
  # /etc/inetd.conf.  An active cmsd entry exposes a historically exploitable
  # RPC service; CIS requires it unavailable.  Capture: grep for the exact
  # first-field token cmsd followed by whitespace (not a comment, not a
  # later-field match).
  #
  # Verdict logic:
  #   rc=0  AND  every nonblank row starts with exactly "cmsd "  => FAIL
  #   rc=1  AND  empty output                                    => PASS
  #   anything else                                              => NOT_ASSESSED
  #
  typeset CDE_CMSD_RC CDE_CMSD_RAW CDE_CMSD_PARSED CDE_CMSD_SHAPE
  typeset CDE_CMSD_STATUS CDE_CMSD_OBSERVED CDE_CMSD_MEANING
  typeset CDE_CMSD_FIX CDE_CMSD_SEV
  CDE_CMSD_RC=0
  CDE_CMSD_RAW=""
  CDE_CMSD_PARSED="absent"
  CDE_CMSD_SHAPE=1

  CDE_CMSD_RAW=$(aix cde_cmsd grep -E '^[	 ]*cmsd[	 ]' /etc/inetd.conf)
  CDE_CMSD_RC=$?

  if [ "$CDE_CMSD_RC" -eq 0 ] && [ -n "$CDE_CMSD_RAW" ]; then
    CDE_CMSD_PARSED=$(printf '%s\n' "$CDE_CMSD_RAW" | awk '
      /^[ \t]*$/ { next }
      {
        rows++
        line = $0
    ...
    case "$CDE_CMSD_PARSED" in
      single) CDE_CMSD_SHAPE=1 ;;
      multiple) CDE_CMSD_SHAPE=1 ;;
      *) CDE_CMSD_SHAPE=0 ;;
    esac
  fi
  ...
  case "$CDE_CMSD_RC" in
    0)
      if [ "$CDE_CMSD_SHAPE" -eq 1 ] && [ "$CDE_CMSD_PARSED" != "absent" ]; then
        ...
        case "$CDE_CMSD_PARSED" in
          non-cmsd-first)
            CDE_CMSD_OBSERVED="not assessed - cmsd inetd probe captured output contains non-cmsd-first rows" ;;
          malformed)
            CDE_CMSD_OBSERVED="not assessed - cmsd inetd probe captured rows are malformed" ;;
          *)
      ...
      ;;
    1)
      if [ -z "$CDE_CMSD_RAW" ]; then
        CDE_CMSD_STATUS=PASS
        CDE_CMSD_OBSERVED="cmsd not active in /etc/inetd.conf"
        CDE_CMSD_MEANING="The CDE calendar service is not enabled."
        CDE_CMSD_FIX="n/a"
      else
        CDE_CMSD_STATUS=NOT_ASSESSED
  ...
  case "$CDE_CMSD_STATUS" in
    FAIL) CDE_CMSD_SEV=high ;;
    *) CDE_CMSD_SEV=low ;;
  esac

  add security cde_cmsd "CDE calendar daemon (cmsd) in inetd" \
    "$CDE_CMSD_STATUS" "$CDE_CMSD_SEV" \
    "$CDE_CMSD_OBSERVED" \
    "$CDE_CMSD_MEANING" \
    "$CDE_CMSD_FIX" \
    "cis-l1"
