...
_AIXRAY_SESSION_KEYS=""
  ...
  # TCP service; CIS requires it unavailable.  Capture: grep for the exact
  # first-field token dtspc followed by whitespace (not a comment, not a
  # later-field match).
  #
  # Verdict logic:
  #   rc=0  AND  every nonblank row starts with exactly "dtspc "  => FAIL
  #   rc=1  AND  empty output                                    => PASS
  #   anything else                                              => NOT_ASSESSED
  #
  typeset CDE_DTSPC_RC CDE_DTSPC_RAW CDE_DTSPC_PARSED CDE_DTSPC_SHAPE
  typeset CDE_DTSPC_STATUS CDE_DTSPC_OBSERVED CDE_DTSPC_MEANING
  typeset CDE_DTSPC_FIX CDE_DTSPC_SEV
  CDE_DTSPC_RC=0
  CDE_DTSPC_RAW=""
  CDE_DTSPC_PARSED="absent"
  CDE_DTSPC_SHAPE=1

  CDE_DTSPC_RAW=$(aix cde_dtspc grep -E '^[	 ]*dtspc[	 ]' /etc/inetd.conf)
  CDE_DTSPC_RC=$?

  if [ "$CDE_DTSPC_RC" -eq 0 ] && [ -n "$CDE_DTSPC_RAW" ]; then
    CDE_DTSPC_PARSED=$(printf '%s\n' "$CDE_DTSPC_RAW" | awk '
      /^[ \t]*$/ { next }
      {
        rows++
        line = $0
    ...
    case "$CDE_DTSPC_PARSED" in
      single) CDE_DTSPC_SHAPE=1 ;;
      multiple) CDE_DTSPC_SHAPE=1 ;;
      *) CDE_DTSPC_SHAPE=0 ;;
    esac
  fi

  case "$CDE_DTSPC_RC" in
    0)
      if [ "$CDE_DTSPC_SHAPE" -eq 1 ] && [ "$CDE_DTSPC_PARSED" != "absent" ]; then
        ...
        case "$CDE_DTSPC_PARSED" in
          non-dtspc-first)
            CDE_DTSPC_OBSERVED="not assessed - dtspc inetd probe captured output contains non-dtspc-first rows" ;;
          malformed)
            CDE_DTSPC_OBSERVED="not assessed - dtspc inetd probe captured rows are malformed" ;;
          *)
      ...
      ;;
    1)
      if [ -z "$CDE_DTSPC_RAW" ]; then
        CDE_DTSPC_STATUS=PASS
        CDE_DTSPC_OBSERVED="dtspc not active in /etc/inetd.conf"
        CDE_DTSPC_MEANING="The CDE subprocess control service is not enabled."
        CDE_DTSPC_FIX="n/a"
      else
        CDE_DTSPC_STATUS=NOT_ASSESSED
  ...
  case "$CDE_DTSPC_STATUS" in
    FAIL) CDE_DTSPC_SEV=high ;;
    *) CDE_DTSPC_SEV=low ;;
  esac

  add security cde_dtspc "CDE subprocess control (dtspc) in inetd" \
    "$CDE_DTSPC_STATUS" "$CDE_DTSPC_SEV" \
    "$CDE_DTSPC_OBSERVED" \
    "$CDE_DTSPC_MEANING" \
    "$CDE_DTSPC_FIX" \
    "cis-l1"
