...
AIXRAY_TOOL=ck-cde-xservers-access


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # cde_xservers_access — ownership and write-mode of the dt Xservers file.
  ...
  typeset CDE_XSA_FS_RAW CDE_XSA_FS_RC CDE_XSA_FS_STATE
  typeset CDE_XSA_RAW CDE_XSA_RC CDE_XSA_SHAPE CDE_XSA_DETAIL
  typeset CDE_XSA_STATUS CDE_XSA_OBSERVED CDE_XSA_MEANING CDE_XSA_FIX

  if [ "${MYUID:-0}" != "0" ]; then
    CDE_XSA_STATUS=NOT_ASSESSED
    CDE_XSA_OBSERVED="not assessed - CDE configuration probe requires root"
  else
    # --- shared CDE fileset state ---
    if [ "${CDE_FS_RC+x}" = x ]; then
      CDE_XSA_FS_RAW=$CDE_FS_RAW
      CDE_XSA_FS_RC=$CDE_FS_RC
    else
      CDE_XSA_FS_RAW=$(aix cde_filesets lslpp -qcL 'X11.Dt*')
      CDE_XSA_FS_RC=$?
    fi
    if [ "$CDE_XSA_FS_RC" -eq 1 ] && [ -z "$CDE_XSA_FS_RAW" ]; then
      CDE_XSA_FS_STATE=absent
    elif [ "$CDE_XSA_FS_RC" -eq 0 ] && [ -n "$CDE_XSA_FS_RAW" ] &&
        printf '%s\n' "$CDE_XSA_FS_RAW" | awk -F: '/^[ \t]*$/ { next } { if (NF < 6 || $2 !~ /^X11\.Dt/) bad = 1 } END { exit bad }'; then
      CDE_XSA_FS_STATE=installed
    else
      CDE_XSA_FS_STATE=unknown
    fi
    # --- end shared CDE fileset state ---
    if [ "$CDE_XSA_FS_STATE" = "absent" ]; then
      CDE_XSA_STATUS=PASS
      CDE_XSA_OBSERVED="CDE not installed (fileset query empty); no dt Xservers file exists or is consumed"
    elif [ "$CDE_XSA_FS_STATE" = "unknown" ]; then
      CDE_XSA_STATUS=NOT_ASSESSED
      CDE_XSA_OBSERVED="not assessed - CDE fileset query was contradictory or failed, so install state is unproven"
    else
      CDE_XSA_RAW=$(aix cde_xservers_access ls -ld /etc/dt/config/Xservers /usr/dt/config/Xservers)
      CDE_XSA_RC=$?
      if [ -n "$CDE_XSA_RAW" ]; then
        CDE_XSA_SHAPE=$(printf '%s\n' "$CDE_XSA_RAW" | awk '
          /^[ \t]*$/ { next }
          {
            rows++
            mode = $1
            owner = $3
            path = $NF
            if (path !~ /^\/(etc|usr)\/dt\/config\/Xservers$/) { foreign = 1; next }
        ...
        case "$CDE_XSA_SHAPE" in
          foreign)
            CDE_XSA_STATUS=NOT_ASSESSED
            CDE_XSA_OBSERVED="not assessed - file-access probe returned an unexpected path"
            ;;
          malformed)
            CDE_XSA_STATUS=NOT_ASSESSED
            CDE_XSA_OBSERVED="not assessed - file-access probe rows were malformed"
            ;;
          none)
            CDE_XSA_STATUS=NOT_ASSESSED
            CDE_XSA_OBSERVED="not assessed - file-access probe output held no parseable rows"
            ;;
          viol:*)
            CDE_XSA_DETAIL=${CDE_XSA_SHAPE#viol:}
            CDE_XSA_STATUS=FAIL
            CDE_XSA_OBSERVED="${CDE_XSA_DETAIL%%:*} dt Xservers file(s) writable beyond root or not root-owned, first: ${CDE_XSA_DETAIL#*:}"
            ;;
          *)
            CDE_XSA_STATUS=PASS
            CDE_XSA_OBSERVED="${CDE_XSA_SHAPE#ok:} dt Xservers file(s) root-owned with no group/other write"
            ;;
        esac
      elif [ "$CDE_XSA_RC" -ne 0 ]; then
        CDE_XSA_STATUS=NOT_ASSESSED
        CDE_XSA_OBSERVED="not assessed - CDE is installed but no dt Xservers file was found (probe rc=$CDE_XSA_RC); the shipped copy should exist"
      else
        CDE_XSA_STATUS=NOT_ASSESSED
        CDE_XSA_OBSERVED="not assessed - file-access probe returned rc=0 with empty output"
      fi
    fi
  fi

  case "$CDE_XSA_STATUS" in
    PASS)
      CDE_XSA_MEANING="Only root can modify the dt Xservers login-manager configuration, or no such file exists to modify."
      CDE_XSA_FIX="n/a"
      ;;
    FAIL)
  ...
  add security cde_xservers_access "CDE Xservers file access" \
    "$CDE_XSA_STATUS" med "$CDE_XSA_OBSERVED" \
    "$CDE_XSA_MEANING" "$CDE_XSA_FIX" "cis-l1"
}
