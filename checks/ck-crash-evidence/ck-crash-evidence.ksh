...
AIXRAY_TOOL=ck-crash-evidence


function standalone_check {
_AIXRAY_SESSION_KEYS=""

  VM=$(aix find_vmcore_30d find /var/adm/ras -name 'vmcore*' -mtime -30 -print); VMRC=$?
  VM_OK=0; VMWHY=""
  if [ "$VMRC" -ne 0 ]; then
    VMWHY="not assessed — vmcore search failed (rc=$VMRC)"
  elif [ -z "$VM" ]; then
    VM_OK=1
  elif printf '%s\n' "$VM" | awk '
      NF {
        n++
        if (NF != 1 || $0 != $1 || index($1, "/var/adm/ras/") != 1) {
          bad=1
          next
        }
        relative=substr($1, length("/var/adm/ras/") + 1)
        parts=split(relative, component, "/")
        if (relative == "" || parts < 1) {
          bad=1
          next
        }
        for (i=1; i<=parts; i++)
          if (component[i] == "" || component[i] == "." || component[i] == "..")
            bad=1
        if (component[parts] !~ /^vmcore/) bad=1
      }
  ...
  then
    ...
    VMWHY="not assessed — vmcore search output unparseable (rc=0)"
  fi
  ...
  if [ "$VM_OK" -ne 1 ]; then
    ...
    VMN=$(printf '%s\n' "$VM" | awk 'NF{n++} END{print n+0}')
    add errors crash_evidence "Recent crash evidence" WARN high "$VMN dump file(s) in /var/adm/ras" \
