...
AIXRAY_TOOL=ck-default-route


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # default_route — DISA STIG V-215262 (SV-215262r991589_rule, AIX7-00-002063,
  # CCI-000366): an IPv4 system must have a default gateway unless it is a router.
  # Deviation from the literal check-content `netstat -r |grep default`: PTxray matches
  # the destination column of the IPv4 route tree (Protocol Family 2) rather than a bare
  # substring, so "default" appearing in an unrelated column cannot mask a missing
  # gateway and an IPv6-only default route cannot satisfy this IPv4-scoped rule.
  typeset DR_RAW DR_RC DR_COUNT DR_AWK_RC DR_REASON

  DR_RAW=$(aix netstat_r netstat -r)
  DR_RC=$?
  DR_COUNT=""
  DR_REASON=""

  if [ "$DR_RC" -ne 0 ]; then
    DR_REASON="netstat -r exited rc=$DR_RC"
  elif [ -z "$(printf '%s' "$DR_RAW" | tr -d ' \011\012')" ]; then
    DR_REASON="netstat -r produced no output at rc 0 (swallowed error, not absence of a route)"
  else
    DR_COUNT=$(printf '%s\n' "$DR_RAW" | awk '
      BEGIN { in4 = 1; seen = 0; n = 0 }
    ...
    ')
    DR_AWK_RC=$?
    if [ "$DR_AWK_RC" -ne 0 ] || [ -z "$DR_COUNT" ]; then
      DR_REASON="netstat -r output not parseable as a route table"
    fi
  fi

  if [ -n "$DR_REASON" ]; then
    add security default_route "Default route" NOT_ASSESSED med \
        "not assessed - $DR_REASON" \
        "PTxray did not obtain a trustworthy route table, so it cannot say whether an IPv4 default route is defined." \
        "run 'netstat -r', correct the capture problem, and rerun PTxray." \
        "stig:V-215262"
  elif [ "$DR_COUNT" -ge 1 ]; then
    add security default_route "Default route" PASS med \
        "IPv4 default route present in netstat -r route tree" \
        "An IPv4 default gateway is defined, as required by DISA STIG V-215262." \
        "n/a" "stig:V-215262"
  else
    add security default_route "Default route" FAIL med \
        "no IPv4 default route defined in netstat -r; a system operating as a router is a site exception for the operator to adjudicate" \
        "No IPv4 default gateway is defined, so traffic bound for any network other than the directly attached ones has no path off the system." \
