...
AIXRAY_TOOL=ck-diracc-atjobs-group


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # diracc_atjobs_group — CIS L1-aligned /var/spool/cron/atjobs group boundary.
  ...
  typeset DAJG_RAW DAJG_RC DAJG_PARSED DAJG_PARSE_RC DAJG_GID DAJG_REASON DAJG_ABSENT DAJG_DIAG

  DAJG_RAW=$(aixv diracc_atjobs_group_ls ls -ldn /var/spool/cron/atjobs)
  DAJG_RC=$?
  DAJG_PARSED=""
  DAJG_GID=""
  DAJG_REASON=""
  ...
  if [ "$DAJG_RC" -ne 0 ]; then
    DAJG_REASON="absent or unreadable (rc=$DAJG_RC)"
    case "$DAJG_RAW" in
      *"/var/spool/cron/atjobs not found"*) DAJG_ABSENT=1 ;;
    esac
    if [ "$DAJG_ABSENT" -eq 1 ]; then
      DAJG_REASON="absent"
    else
      DAJG_DIAG=$(printf '%s\n' "$DAJG_RAW" | awk 'NF { gsub(/[ \t]+/, " "); sub(/^ /, ""); sub(/ $/, ""); if (length("" $0) > 40) $0 = substr($0, 1, 37) "..."; print; exit }')
      if [ -n "$DAJG_DIAG" ]; then
        DAJG_REASON="absent or unreadable (rc=$DAJG_RC): $DAJG_DIAG"
      fi
    fi
  elif [ -z "$DAJG_RAW" ]; then
    DAJG_REASON="empty"
  else
    DAJG_PARSED=$(printf '%s\n' "$DAJG_RAW" | awk '
      NF {
        records++
    ...
    ')
    DAJG_PARSE_RC=$?
    if [ "$DAJG_PARSE_RC" -ne 0 ] || [ -z "$DAJG_PARSED" ]; then
      DAJG_REASON="unparseable"
    else
      DAJG_GID=$DAJG_PARSED
    fi
  fi

  if [ "$DAJG_REASON" = "absent" ]; then
    add security diracc_atjobs_group "/var/spool/cron/atjobs group" NOT_APPLICABLE med \
        "/var/spool/cron/atjobs absent" \
        "The probe returned the determinate-absence signature ('/var/spool/cron/atjobs not found', exit $DAJG_RC); /var/spool/cron/atjobs need not exist, so the group boundary does not apply." \
        "n/a" "cis-l1"
    ...
    add security diracc_atjobs_group "/var/spool/cron/atjobs group" PASS med \
        ...
        "/var/spool/cron/atjobs has the required group boundary." \
        "n/a" "cis-l1"
  else
