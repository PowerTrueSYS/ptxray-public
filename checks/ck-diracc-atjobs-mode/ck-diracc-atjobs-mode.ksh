...
AIXRAY_TOOL=ck-diracc-atjobs-mode


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # diracc_atjobs_mode — CIS L1-aligned /var/spool/cron/atjobs mode boundary.
  ...
  # TODO-VERIFY: extended ACL marker fails because remediation clears ACLs (README)
  typeset DAJM_RAW DAJM_RC DAJM_PARSED DAJM_PARSE_RC DAJM_MODE DAJM_OK DAJM_REASON DAJM_ABSENT DAJM_DIAG

  DAJM_RAW=$(aixv diracc_atjobs_mode_ls ls -ldn /var/spool/cron/atjobs)
  DAJM_RC=$?
  DAJM_PARSED=""; DAJM_MODE=""; DAJM_OK=""; DAJM_REASON=""; DAJM_ABSENT=0; DAJM_DIAG=""

  if [ "$DAJM_RC" -ne 0 ]; then
    DAJM_REASON="absent or unreadable (rc=$DAJM_RC)"
    case "$DAJM_RAW" in
      *"/var/spool/cron/atjobs not found"*) DAJM_ABSENT=1 ;;
    esac
    if [ "$DAJM_ABSENT" -eq 1 ]; then
      DAJM_REASON="absent"
    else
      DAJM_DIAG=$(printf '%s\n' "$DAJM_RAW" | awk 'NF { gsub(/[ \t]+/, " "); sub(/^ /, ""); sub(/ $/, ""); if (length("" $0) > 40) $0 = substr($0, 1, 37) "..."; print; exit }')
      if [ -n "$DAJM_DIAG" ]; then
        DAJM_REASON="absent or unreadable (rc=$DAJM_RC): $DAJM_DIAG"
      fi
    fi
  elif [ -z "$DAJM_RAW" ]; then
    DAJM_REASON="empty"
  else
    DAJM_PARSED=$(printf '%s
' "$DAJM_RAW" | awk '
      NF {
        records++
        ...
        if (!date_ok) bad=1
        if ($NF != "/var/spool/cron/atjobs") bad=1
      }
      END {
    ...
    ')
    DAJM_PARSE_RC=$?
    if [ "$DAJM_PARSE_RC" -ne 0 ] || [ -z "$DAJM_PARSED" ]; then DAJM_REASON="unparseable"
    else DAJM_MODE=${DAJM_PARSED%%\|*}; DAJM_OK=${DAJM_PARSED#*\|}; fi
  fi
