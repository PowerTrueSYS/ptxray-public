...
AIXRAY_TOOL=ck-diracc-atjobs-owner
...
_AIXRAY_SESSION_KEYS=""
  ...
  typeset DAJO_RAW DAJO_RC DAJO_PARSED DAJO_PARSE_RC DAJO_UID DAJO_REASON DAJO_ABSENT DAJO_DIAG

  DAJO_RAW=$(aixv diracc_atjobs_owner_ls ls -ldn /var/spool/cron/atjobs)
  DAJO_RC=$?
  DAJO_PARSED=""
  DAJO_UID=""
  DAJO_REASON=""
  ...
  if [ "$DAJO_RC" -ne 0 ]; then
    DAJO_REASON="absent or unreadable (rc=$DAJO_RC)"
    case "$DAJO_RAW" in
      *"/var/spool/cron/atjobs not found"*) DAJO_ABSENT=1 ;;
    esac
    if [ "$DAJO_ABSENT" -eq 1 ]; then
      DAJO_REASON="absent"
    else
      DAJO_DIAG=$(printf '%s\n' "$DAJO_RAW" | awk 'NF { gsub(/[ \t]+/, " "); sub(/^ /, ""); sub(/ $/, ""); if (length("" $0) > 40) $0 = substr($0, 1, 37) "..."; print; exit }')
      if [ -n "$DAJO_DIAG" ]; then
        DAJO_REASON="absent or unreadable (rc=$DAJO_RC): $DAJO_DIAG"
      fi
    fi
  elif [ -z "$DAJO_RAW" ]; then
    DAJO_REASON="empty"
  else
    DAJO_PARSED=$(printf '%s\n' "$DAJO_RAW" | awk '
      NF {
        records++
    ...
    ')
    DAJO_PARSE_RC=$?
    if [ "$DAJO_PARSE_RC" -ne 0 ] || [ -z "$DAJO_PARSED" ]; then
      DAJO_REASON="unparseable"
    else
      DAJO_UID=$DAJO_PARSED
    fi
  fi

  if [ "$DAJO_REASON" = "absent" ]; then
    add security diracc_atjobs_owner "/var/spool/cron/atjobs owner" NOT_APPLICABLE med \
        "/var/spool/cron/atjobs absent" \
        "The probe returned the determinate-absence signature ('/var/spool/cron/atjobs not found', exit $DAJO_RC); /var/spool/cron/atjobs need not exist, so the owner boundary does not apply." \
        "n/a" "cis-l1"
    ...
    add security diracc_atjobs_owner "/var/spool/cron/atjobs owner" PASS med \
        ...
        "/var/spool/cron/atjobs has the required owner boundary." \
        "n/a" "cis-l1"
  else
