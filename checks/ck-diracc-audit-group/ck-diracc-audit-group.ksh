...
AIXRAY_TOOL=ck-diracc-audit-group


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # diracc_audit_group — CIS L1-aligned /audit group boundary.
  ...
  typeset DAAG_RAW DAAG_RC DAAG_PARSED DAAG_PARSE_RC DAAG_GID DAAG_REASON DAAG_ABSENT DAAG_DIAG

  DAAG_RAW=$(aixv diracc_audit_group_ls ls -ldn /audit)
  DAAG_RC=$?
  DAAG_PARSED=""
  DAAG_GID=""
  DAAG_REASON=""
  ...
  if [ "$DAAG_RC" -ne 0 ]; then
    DAAG_REASON="absent or unreadable (rc=$DAAG_RC)"
    case "$DAAG_RAW" in
      *"/audit not found"*) DAAG_ABSENT=1 ;;
    esac
    if [ "$DAAG_ABSENT" -eq 1 ]; then
      DAAG_REASON="absent"
    else
      DAAG_DIAG=$(printf '%s\n' "$DAAG_RAW" | awk 'NF { gsub(/[ \t]+/, " "); sub(/^ /, ""); sub(/ $/, ""); if (length("" $0) > 40) $0 = substr($0, 1, 37) "..."; print; exit }')
      if [ -n "$DAAG_DIAG" ]; then
        DAAG_REASON="absent or unreadable (rc=$DAAG_RC): $DAAG_DIAG"
      fi
    fi
  elif [ -z "$DAAG_RAW" ]; then
    DAAG_REASON="empty"
  else
    DAAG_PARSED=$(printf '%s\n' "$DAAG_RAW" | awk '
      NF {
        records++
    ...
    ')
    DAAG_PARSE_RC=$?
    if [ "$DAAG_PARSE_RC" -ne 0 ] || [ -z "$DAAG_PARSED" ]; then
      DAAG_REASON="unparseable"
    else
      DAAG_GID=$DAAG_PARSED
    fi
  fi

  if [ "$DAAG_REASON" = "absent" ]; then
    add security diracc_audit_group "/audit group" NOT_APPLICABLE med \
        "/audit absent" \
        "The probe returned the determinate-absence signature ('/audit not found', exit $DAAG_RC); /audit need not exist, so the group boundary does not apply." \
        "n/a" "cis-l1"
    ...
    add security diracc_audit_group "/audit group" PASS med \
        ...
        "/audit has the required group boundary." \
        "n/a" "cis-l1"
  else
