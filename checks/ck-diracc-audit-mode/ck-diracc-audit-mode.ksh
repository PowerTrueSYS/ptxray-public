...
AIXRAY_TOOL=ck-diracc-audit-mode


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # diracc_audit_mode — CIS L1-aligned /audit mode boundary.
  ...
  typeset DAAM_RAW DAAM_RC DAAM_PARSED DAAM_PARSE_RC DAAM_MODE DAAM_OK DAAM_REASON DAAM_ABSENT DAAM_DIAG

  DAAM_RAW=$(aixv diracc_audit_mode_ls ls -ldn /audit)
  DAAM_RC=$?
  DAAM_PARSED=""; DAAM_MODE=""; DAAM_OK=""; DAAM_REASON=""; DAAM_ABSENT=0; DAAM_DIAG=""

  if [ "$DAAM_RC" -ne 0 ]; then
    DAAM_REASON="absent or unreadable (rc=$DAAM_RC)"
    case "$DAAM_RAW" in
      *"/audit not found"*) DAAM_ABSENT=1 ;;
    esac
    if [ "$DAAM_ABSENT" -eq 1 ]; then
      DAAM_REASON="absent"
    else
      DAAM_DIAG=$(printf '%s\n' "$DAAM_RAW" | awk 'NF { gsub(/[ \t]+/, " "); sub(/^ /, ""); sub(/ $/, ""); if (length("" $0) > 40) $0 = substr($0, 1, 37) "..."; print; exit }')
      if [ -n "$DAAM_DIAG" ]; then
        DAAM_REASON="absent or unreadable (rc=$DAAM_RC): $DAAM_DIAG"
      fi
    fi
  elif [ -z "$DAAM_RAW" ]; then
    DAAM_REASON="empty"
  else
    DAAM_PARSED=$(printf '%s
' "$DAAM_RAW" | awk '
      NF {
        records++
    ...
    ')
    DAAM_PARSE_RC=$?
    if [ "$DAAM_PARSE_RC" -ne 0 ] || [ -z "$DAAM_PARSED" ]; then DAAM_REASON="unparseable"
    else DAAM_MODE=${DAAM_PARSED%%\|*}; DAAM_OK=${DAAM_PARSED#*\|}; fi
  fi
