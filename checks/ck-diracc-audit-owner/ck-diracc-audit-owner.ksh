...
AIXRAY_TOOL=ck-diracc-audit-owner


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # diracc_audit_owner — CIS L1-aligned /audit owner boundary.
  ...
  typeset DAAO_RAW DAAO_RC DAAO_PARSED DAAO_PARSE_RC DAAO_UID DAAO_REASON DAAO_ABSENT DAAO_DIAG

  DAAO_RAW=$(aixv diracc_audit_owner_ls ls -ldn /audit)
  DAAO_RC=$?
  DAAO_PARSED=""
  DAAO_UID=""
  DAAO_REASON=""
  ...
  if [ "$DAAO_RC" -ne 0 ]; then
    DAAO_REASON="absent or unreadable (rc=$DAAO_RC)"
    case "$DAAO_RAW" in
      *"/audit not found"*) DAAO_ABSENT=1 ;;
    esac
    if [ "$DAAO_ABSENT" -eq 1 ]; then
      DAAO_REASON="absent"
    else
      DAAO_DIAG=$(printf '%s\n' "$DAAO_RAW" | awk 'NF { gsub(/[ \t]+/, " "); sub(/^ /, ""); sub(/ $/, ""); if (length("" $0) > 40) $0 = substr($0, 1, 37) "..."; print; exit }')
      if [ -n "$DAAO_DIAG" ]; then
        DAAO_REASON="absent or unreadable (rc=$DAAO_RC): $DAAO_DIAG"
      fi
    fi
  elif [ -z "$DAAO_RAW" ]; then
    DAAO_REASON="empty"
  else
    DAAO_PARSED=$(printf '%s\n' "$DAAO_RAW" | awk '
      NF {
        records++
    ...
    ')
    DAAO_PARSE_RC=$?
    if [ "$DAAO_PARSE_RC" -ne 0 ] || [ -z "$DAAO_PARSED" ]; then
      DAAO_REASON="unparseable"
    else
      DAAO_UID=$DAAO_PARSED
    fi
  fi

  if [ "$DAAO_REASON" = "absent" ]; then
    add security diracc_audit_owner "/audit owner" NOT_APPLICABLE med \
        "/audit absent" \
        "The probe returned the determinate-absence signature ('/audit not found', exit $DAAO_RC); /audit need not exist, so the owner boundary does not apply." \
        "n/a" "cis-l1"
    ...
    add security diracc_audit_owner "/audit owner" PASS med \
        ...
        "/audit has the required owner boundary." \
        "n/a" "cis-l1"
  else
