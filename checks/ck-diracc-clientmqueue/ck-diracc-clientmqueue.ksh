...
AIXRAY_TOOL=ck-diracc-clientmqueue


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # diracc_clientmqueue — CIS 4.6.4.4 /var/spool/clientmqueue directory access.
  # Path must exist, be owned smmsp:smmsp, and carry mode drwxrwx--- (0770).
  # Any divergence in owner, group, or mode bits is a finding. Determinate
  # absence (rc=2, empty stdout) is NOT_APPLICABLE.
  typeset CK_RAW CK_RC CK_REC CK_VALID CK_VALID_RC
  typeset CK_MODE CK_OWNER CK_GROUP CK_REMAIN CK_DIVERGED CK_REMEDIATE

  CK_RAW=$(aix diracc_clientmqueue_ls ls -ld /var/spool/clientmqueue)
  CK_RC=$?

  if [ "$CK_RC" -eq 2 ] && [ -z "$CK_RAW" ]; then
    add security diracc_clientmqueue "/var/spool/clientmqueue access" NOT_APPLICABLE med \
        "path absent" \
        "/var/spool/clientmqueue does not exist, so there is no directory access state to constrain." \
        "n/a" "cis-l1"
    ...
    if [ "$CK_REC" = "drwxrwx--- smmsp smmsp /var/spool/clientmqueue" ]; then
      add security diracc_clientmqueue "/var/spool/clientmqueue access" PASS med \
          "mode=drwxrwx--- owner=smmsp group=smmsp" \
          "/var/spool/clientmqueue has the required directory access boundary." \
          "n/a" "cis-l1"
    else
      CK_VALID=$(printf '%s\n' "$CK_REC" | awk '
        {
          ...
          if (length("" mode) != 10 || substr(mode, 1, 1) != "d") exit 1
          if (length($2) < 1 || length($3) < 1) exit 1
          if ($4 != "/var/spool/clientmqueue") exit 1
          print $1 "|" $2 "|" $3
        }
      ...
      if [ "$CK_VALID_RC" -ne 0 ] || [ -z "$CK_VALID" ]; then
        ...
        CK_GROUP=${CK_REMAIN#*\|}
        CK_DIVERGED=""
        if [ "$CK_MODE" != "drwxrwx---" ]; then CK_DIVERGED="permissions"; fi
        if [ "$CK_OWNER" != "smmsp" ]; then
          if [ -n "$CK_DIVERGED" ]; then CK_DIVERGED="$CK_DIVERGED, owner"; else CK_DIVERGED="owner"; fi
        fi
        if [ "$CK_GROUP" != "smmsp" ]; then
          if [ -n "$CK_DIVERGED" ]; then CK_DIVERGED="$CK_DIVERGED, group"; else CK_DIVERGED="group"; fi
        fi
        CK_REMEDIATE=""
        if [ "$CK_MODE" != "drwxrwx---" ]; then CK_REMEDIATE="chmod 770 /var/spool/clientmqueue"; fi
        if [ "$CK_OWNER" != "smmsp" ] || [ "$CK_GROUP" != "smmsp" ]; then
          if [ -n "$CK_REMEDIATE" ]; then CK_REMEDIATE="$CK_REMEDIATE; "; fi
          CK_REMEDIATE="${CK_REMEDIATE}chown smmsp:smmsp /var/spool/clientmqueue"
        fi
