...
AIXRAY_TOOL=ck-diracc-crontabs-group
...
_AIXRAY_SESSION_KEYS=""
  ...
  typeset DCTG_RAW DCTG_RC DCTG_PARSED DCTG_PARSE_RC DCTG_GID DCTG_REASON DCTG_ABSENT DCTG_DIAG

  DCTG_RAW=$(aixv diracc_crontabs_group_ls ls -ldn /var/spool/cron/crontabs)
  DCTG_RC=$?
  DCTG_PARSED=""
  DCTG_GID=""
  DCTG_REASON=""
  ...
  if [ "$DCTG_RC" -ne 0 ]; then
    DCTG_REASON="absent or unreadable (rc=$DCTG_RC)"
    case "$DCTG_RAW" in
      *"/var/spool/cron/crontabs not found"*) DCTG_ABSENT=1 ;;
    esac
    if [ "$DCTG_ABSENT" -eq 1 ]; then
      DCTG_REASON="absent"
    else
      DCTG_DIAG=$(printf '%s\n' "$DCTG_RAW" | awk 'NF { gsub(/[ \t]+/, " "); sub(/^ /, ""); sub(/ $/, ""); if (length("" $0) > 40) $0 = substr($0, 1, 37) "..."; print; exit }')
      if [ -n "$DCTG_DIAG" ]; then
        DCTG_REASON="absent or unreadable (rc=$DCTG_RC): $DCTG_DIAG"
      fi
    fi
  elif [ -z "$DCTG_RAW" ]; then
    DCTG_REASON="empty"
  else
    DCTG_PARSED=$(printf '%s\n' "$DCTG_RAW" | awk '
      NF {
        records++
    ...
    ')
    DCTG_PARSE_RC=$?
    if [ "$DCTG_PARSE_RC" -ne 0 ] || [ -z "$DCTG_PARSED" ]; then
      DCTG_REASON="unparseable"
    else
      DCTG_GID=$DCTG_PARSED
    fi
  fi

  if [ "$DCTG_REASON" = "absent" ]; then
    add security diracc_crontabs_group "/var/spool/cron/crontabs group" NOT_APPLICABLE med \
        "/var/spool/cron/crontabs absent" \
        "The probe returned the determinate-absence signature ('/var/spool/cron/crontabs not found', exit $DCTG_RC); /var/spool/cron/crontabs need not exist, so the group boundary does not apply." \
        "n/a" "cis-l1"
    ...
    add security diracc_crontabs_group "/var/spool/cron/crontabs group" PASS med \
        ...
        "/var/spool/cron/crontabs has the required group boundary." \
        "n/a" "cis-l1"
  else
