...
AIXRAY_TOOL=ck-diracc-crontabs-mode


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # diracc_crontabs_mode — CIS L1-aligned /var/spool/cron/crontabs mode boundary.
  ...
  # Ceiling 0770 per the CIS IBM AIX 7 v1.2.0 control 4.1.2.8 (L1) audit procedure.
  # Extended ACL fails because the audit procedure tests the extended-ACL bit.
  typeset DCTM_RAW DCTM_RC DCTM_PARSED DCTM_PARSE_RC DCTM_MODE DCTM_OK DCTM_REASON DCTM_ABSENT DCTM_DIAG

  DCTM_RAW=$(aixv diracc_crontabs_mode_ls ls -ldn /var/spool/cron/crontabs)
  DCTM_RC=$?
  DCTM_PARSED=""; DCTM_MODE=""; DCTM_OK=""; DCTM_REASON=""; DCTM_ABSENT=0; DCTM_DIAG=""

  if [ "$DCTM_RC" -ne 0 ]; then
    DCTM_REASON="absent or unreadable (rc=$DCTM_RC)"
    case "$DCTM_RAW" in
      *"/var/spool/cron/crontabs not found"*) DCTM_ABSENT=1 ;;
    esac
    if [ "$DCTM_ABSENT" -eq 1 ]; then
      DCTM_REASON="absent"
    else
      DCTM_DIAG=$(printf '%s\n' "$DCTM_RAW" | awk 'NF { gsub(/[ \t]+/, " "); sub(/^ /, ""); sub(/ $/, ""); if (length("" $0) > 40) $0 = substr($0, 1, 37) "..."; print; exit }')
      if [ -n "$DCTM_DIAG" ]; then
        DCTM_REASON="absent or unreadable (rc=$DCTM_RC): $DCTM_DIAG"
      fi
    fi
  elif [ -z "$DCTM_RAW" ]; then
    DCTM_REASON="empty"
  else
    DCTM_PARSED=$(printf '%s
' "$DCTM_RAW" | awk '
      NF {
        records++
        ...
        if (length("" mode) == 11) {
          ...
          else { acl=1; mode=substr(mode, 1, 10) }
        } else if (length("" mode) != 10) bad=1
        bits=0
        if (length("" mode) == 10) {
          if (substr(mode,1,1) != "d") bad=1
        ...
        date_ok=0; date_field=$8
        if (length("" date_field) == 4 && date_field ~ /^[0-9][0-9][0-9][0-9]$/) date_ok=1
        else if (length("" date_field) == 5 && substr(date_field,3,1) == ":") {
          hour=substr(date_field,1,2); minute=substr(date_field,4,2)
          if (hour ~ /^[0-9][0-9]$/ && minute ~ /^[0-9][0-9]$/ && (hour + 0) <= 23 && (minute + 0) <= 59) date_ok=1
        }
        if (!date_ok) bad=1
      ...
      END {
        ...
        target="0770"; ceiling=0
        for (i=1; i<=length("" target); i++) ceiling=ceiling*8+substr(target,i,1)
        extra=0
        for (power=1; power<=2048; power*=2) if (int(bits/power)%2 == 1 && int(ceiling/power)%2 == 0) extra+=power
        aclsuffix=""
        if (acl) aclsuffix="+acl"
        okflag=0
        if (extra == 0 && acl == 0) okflag=1
        print sprintf("%04o", bits) aclsuffix "|" okflag
      }
    ')
    DCTM_PARSE_RC=$?
    if [ "$DCTM_PARSE_RC" -ne 0 ] || [ -z "$DCTM_PARSED" ]; then DCTM_REASON="unparseable"
    else DCTM_MODE=${DCTM_PARSED%%\|*}; DCTM_OK=${DCTM_PARSED#*\|}; fi
  fi
