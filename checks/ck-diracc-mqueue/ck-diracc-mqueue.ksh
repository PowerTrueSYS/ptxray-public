...
AIXRAY_TOOL=ck-diracc-mqueue


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # diracc_mqueue — /var/spool/mqueue presence, ownership, and mode boundary.
  # CIS L1 mapping 4.6.4.5: the directory must exist, be owned by root:system,
  # and carry mode 0700 (drwx------); any other mode, owner, or group is
  # non-compliant, and absence is non-compliance rather than inapplicability.
  typeset DQM_RAW DQM_RC DQM_SHAPE DQM_DETAIL DQM_MODE DQM_OWNER DQM_GROUP
  typeset DQM_OWNER_REST DQM_STATUS DQM_SEV DQM_OBSERVED DQM_MEANING DQM_FIX DQM_BAD

  DQM_RAW=$(aix var_spool_mqueue_ls ls -ld /var/spool/mqueue)
  DQM_RC=$?
  DQM_SHAPE=""; DQM_STATUS=""; DQM_SEV=""; DQM_OBSERVED=""; DQM_MEANING=""; DQM_FIX=""

  if [ "$DQM_RC" -eq 2 ]; then
    DQM_STATUS=FAIL
    DQM_SEV=high
    DQM_OBSERVED="/var/spool/mqueue is absent (rc=2); the control mandates its existence"
  elif [ "$DQM_RC" -ne 0 ]; then
    DQM_STATUS=NOT_ASSESSED
    DQM_SEV=low
    DQM_OBSERVED="not assessed - probe of /var/spool/mqueue metadata failed (rc=$DQM_RC); the directory state is unknown"
  else
    DQM_SHAPE=$(printf '%s\n' "$DQM_RAW" | awk '
      /^[ \t]*$/ { next }
      {
        ...
        owner = $3
        group = $4
        if ($NF != "/var/spool/mqueue") { shape = "foreign"; next }
        if (length("" mode) == 0) next
        if (mode != "drwx------") { shape = "viol|" mode "|" owner "|" group; next }
        if (owner != "root") { shape = "viol|" mode "|" owner "|" group; next }
        if (group != "system") { shape = "viol|" mode "|" owner "|" group; next }
        shape = "ok"
      }
      END { if (rows != 1 || shape == "") print "malformed"; else print shape }
    ')
    case "$DQM_SHAPE" in
      ok)
        DQM_STATUS=PASS
        DQM_SEV=low
        DQM_OBSERVED="/var/spool/mqueue exists with mode drwx------, owned by root:system"
        ;;
      viol*)
        DQM_DETAIL=${DQM_SHAPE#viol\|}
        DQM_MODE=${DQM_DETAIL%%\|*}
        DQM_OWNER_REST=${DQM_DETAIL#*\|}
        DQM_OWNER=${DQM_OWNER_REST%%\|*}
        DQM_GROUP=${DQM_DETAIL##*\|}
        DQM_STATUS=FAIL
        DQM_SEV=high
        DQM_BAD=""
        if [ "$DQM_MODE" != "drwx------" ]; then DQM_BAD="mode"; fi
        if [ "$DQM_OWNER" != "root" ]; then DQM_BAD="${DQM_BAD:+$DQM_BAD }owner"; fi
        if [ "$DQM_GROUP" != "system" ]; then DQM_BAD="${DQM_BAD:+$DQM_BAD }group"; fi
        DQM_OBSERVED="/var/spool/mqueue mode=$DQM_MODE owner=$DQM_OWNER group=$DQM_GROUP; requires drwx------ root:system; deviates: $DQM_BAD"
        ;;
      foreign)
        DQM_STATUS=NOT_ASSESSED
        DQM_SEV=low
        DQM_OBSERVED="not assessed - metadata probe returned an unexpected path"
        ;;
      malformed)
        DQM_STATUS=NOT_ASSESSED
        DQM_SEV=low
        DQM_OBSERVED="not assessed - metadata row was empty or malformed"
        ;;
      *)
        DQM_STATUS=NOT_ASSESSED
        DQM_SEV=low
        DQM_OBSERVED="not assessed - unexpected parse result for /var/spool/mqueue metadata"
        ;;
    esac
  fi
  ...
  case "$DQM_STATUS" in
    PASS)
      DQM_MEANING="Only the root account can enter /var/spool/mqueue; the queued mail files are not exposed to any other identity."
      DQM_FIX="n/a"
      ;;
    FAIL)
  ...
  add security diracc_mqueue "/var/spool/mqueue mode and ownership" \
    "$DQM_STATUS" "$DQM_SEV" "$DQM_OBSERVED" \
    "$DQM_MEANING" "$DQM_FIX" "cis-l1"
}
