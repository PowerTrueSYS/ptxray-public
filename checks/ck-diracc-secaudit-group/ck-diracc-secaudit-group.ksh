...
AIXRAY_TOOL=ck-diracc-secaudit-group


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # diracc_secaudit_group — CIS L1-aligned /etc/security/audit group boundary.
  ...
  typeset DASG_RAW DASG_RC DASG_PARSED DASG_PARSE_RC DASG_GID DASG_REASON DASG_ABSENT DASG_DIAG

  if [ "${MYUID:-0}" != "0" ]; then
    DASG_RAW=""
    DASG_RC=126
  else
    DASG_RAW=$(aixv diracc_secaudit_group_ls ls -ldn /etc/security/audit)
    DASG_RC=$?
  fi
  DASG_PARSED=""
  DASG_GID=""
  ...
  if [ "$DASG_RC" -eq 126 ]; then
    DASG_REASON="requires root; capture not executed"
  elif [ "$DASG_RC" -ne 0 ]; then
    DASG_REASON="absent or unreadable (rc=$DASG_RC)"
    case "$DASG_RAW" in
      *"/etc/security/audit not found"*) DASG_ABSENT=1 ;;
    esac
    if [ "$DASG_ABSENT" -eq 1 ]; then
      DASG_REASON="absent"
    else
      DASG_DIAG=$(printf '%s\n' "$DASG_RAW" | awk 'NF { gsub(/[ \t]+/, " "); sub(/^ /, ""); sub(/ $/, ""); if (length("" $0) > 40) $0 = substr($0, 1, 37) "..."; print; exit }')
      if [ -n "$DASG_DIAG" ]; then
        DASG_REASON="absent or unreadable (rc=$DASG_RC): $DASG_DIAG"
      fi
    fi
  elif [ -z "$DASG_RAW" ]; then
    DASG_REASON="empty"
  else
    DASG_PARSED=$(printf '%s\n' "$DASG_RAW" | awk '
      NF {
        records++
    ...
    ')
    DASG_PARSE_RC=$?
    if [ "$DASG_PARSE_RC" -ne 0 ] || [ -z "$DASG_PARSED" ]; then
      DASG_REASON="unparseable"
    else
      DASG_GID=$DASG_PARSED
    fi
  fi

  if [ "$DASG_REASON" = "absent" ]; then
    add security diracc_secaudit_group "/etc/security/audit group" NOT_APPLICABLE med \
        "/etc/security/audit absent" \
        "The probe returned the determinate-absence signature ('/etc/security/audit not found', exit $DASG_RC); /etc/security/audit need not exist, so the group boundary does not apply." \
        "n/a" "cis-l1"
    ...
    add security diracc_secaudit_group "/etc/security/audit group" PASS med \
        ...
        "/etc/security/audit has the required group boundary." \
        "n/a" "cis-l1"
  else
