...
AIXRAY_TOOL=ck-diracc-secaudit-mode


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # diracc_secaudit_mode — CIS L1-aligned /etc/security/audit mode boundary.
  ...
  # TODO-VERIFY: unpublished audit predicate; top-level directory only, no setgid requirement (README)
  typeset DASM_RAW DASM_RC DASM_PARSED DASM_PARSE_RC DASM_MODE DASM_OK DASM_REASON DASM_ABSENT DASM_DIAG

  if [ "${MYUID:-0}" != "0" ]; then
    DASM_RAW=""
    DASM_RC=126
  else
    DASM_RAW=$(aixv diracc_secaudit_mode_ls ls -ldn /etc/security/audit)
    DASM_RC=$?
  fi
  DASM_PARSED=""; DASM_MODE=""; DASM_OK=""; DASM_REASON=""; DASM_ABSENT=0; DASM_DIAG=""

  if [ "$DASM_RC" -eq 126 ]; then
    DASM_REASON="requires root; capture not executed"
  elif [ "$DASM_RC" -ne 0 ]; then
    DASM_REASON="absent or unreadable (rc=$DASM_RC)"
    case "$DASM_RAW" in
      *"/etc/security/audit not found"*) DASM_ABSENT=1 ;;
    esac
    if [ "$DASM_ABSENT" -eq 1 ]; then
      DASM_REASON="absent"
    else
      DASM_DIAG=$(printf '%s\n' "$DASM_RAW" | awk 'NF { gsub(/[ \t]+/, " "); sub(/^ /, ""); sub(/ $/, ""); if (length("" $0) > 40) $0 = substr($0, 1, 37) "..."; print; exit }')
      if [ -n "$DASM_DIAG" ]; then
        DASM_REASON="absent or unreadable (rc=$DASM_RC): $DASM_DIAG"
      fi
    fi
  elif [ -z "$DASM_RAW" ]; then
    DASM_REASON="empty"
  else
    DASM_PARSED=$(printf '%s
' "$DASM_RAW" | awk '
      NF {
        records++
        ...
        if (!date_ok) bad=1
        if ($NF != "/etc/security/audit") bad=1
      }
      END {
        if (records != 1 || bad) exit 1
        target="0750"; ceiling=0
    ...
    ')
    DASM_PARSE_RC=$?
    if [ "$DASM_PARSE_RC" -ne 0 ] || [ -z "$DASM_PARSED" ]; then DASM_REASON="unparseable"
    else DASM_MODE=${DASM_PARSED%%\|*}; DASM_OK=${DASM_PARSED#*\|}; fi
  fi
