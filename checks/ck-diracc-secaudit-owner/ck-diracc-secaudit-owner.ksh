...
AIXRAY_TOOL=ck-diracc-secaudit-owner
...
_AIXRAY_SESSION_KEYS=""
  ...
  typeset DASO_RAW DASO_RC DASO_PARSED DASO_PARSE_RC DASO_UID DASO_REASON DASO_ABSENT DASO_DIAG

  if [ "${MYUID:-0}" != "0" ]; then
    DASO_RAW=""
    DASO_RC=126
  else
    DASO_RAW=$(aixv diracc_secaudit_owner_ls ls -ldn /etc/security/audit)
    DASO_RC=$?
  fi
  DASO_PARSED=""
  DASO_UID=""
  ...
  if [ "$DASO_RC" -eq 126 ]; then
    DASO_REASON="requires root; capture not executed"
  elif [ "$DASO_RC" -ne 0 ]; then
    DASO_REASON="absent or unreadable (rc=$DASO_RC)"
    case "$DASO_RAW" in
      *"/etc/security/audit not found"*) DASO_ABSENT=1 ;;
    esac
    if [ "$DASO_ABSENT" -eq 1 ]; then
      DASO_REASON="absent"
    else
      DASO_DIAG=$(printf '%s\n' "$DASO_RAW" | awk 'NF { gsub(/[ \t]+/, " "); sub(/^ /, ""); sub(/ $/, ""); if (length("" $0) > 40) $0 = substr($0, 1, 37) "..."; print; exit }')
      if [ -n "$DASO_DIAG" ]; then
        DASO_REASON="absent or unreadable (rc=$DASO_RC): $DASO_DIAG"
      fi
    fi
  elif [ -z "$DASO_RAW" ]; then
    DASO_REASON="empty"
  else
    DASO_PARSED=$(printf '%s\n' "$DASO_RAW" | awk '
      NF {
        records++
    ...
    ')
    DASO_PARSE_RC=$?
    if [ "$DASO_PARSE_RC" -ne 0 ] || [ -z "$DASO_PARSED" ]; then
      DASO_REASON="unparseable"
    else
      DASO_UID=$DASO_PARSED
    fi
  fi

  if [ "$DASO_REASON" = "absent" ]; then
    add security diracc_secaudit_owner "/etc/security/audit owner" NOT_APPLICABLE med \
        "/etc/security/audit absent" \
        "The probe returned the determinate-absence signature ('/etc/security/audit not found', exit $DASO_RC); /etc/security/audit need not exist, so the owner boundary does not apply." \
        "n/a" "cis-l1"
    ...
    add security diracc_secaudit_owner "/etc/security/audit owner" PASS med \
        ...
        "/etc/security/audit has the required owner boundary." \
        "n/a" "cis-l1"
  else
