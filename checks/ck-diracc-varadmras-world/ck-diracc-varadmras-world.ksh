...
AIXRAY_TOOL=ck-diracc-varadmras-world
...
_AIXRAY_SESSION_KEYS=""
  ...
  # TODO-VERIFY: first-level entries only; symlink rows excluded from the verdict (README)
  typeset DRW_RAW DRW_RC DRW_PARSED DRW_PARSE_RC DRW_BAD DRW_TOTAL DRW_REASON DRW_ABSENT DRW_DIAG

  DRW_RAW=$(aixv diracc_varadmras_world_ls ls -ln /var/adm/ras)
  DRW_RC=$?
  DRW_PARSED=""
  DRW_BAD=""
  DRW_TOTAL=""
  ...
  if [ "$DRW_RC" -ne 0 ]; then
    DRW_REASON="absent or unreadable (rc=$DRW_RC)"
    case "$DRW_RAW" in
      *"/var/adm/ras not found"*) DRW_ABSENT=1 ;;
    esac
    if [ "$DRW_ABSENT" -eq 1 ]; then
      DRW_REASON="absent"
    else
      DRW_DIAG=$(printf '%s\n' "$DRW_RAW" | awk 'NF { gsub(/[ \t]+/, " "); sub(/^ /, ""); sub(/ $/, ""); if (length("" $0) > 40) $0 = substr($0, 1, 37) "..."; print; exit }')
      if [ -n "$DRW_DIAG" ]; then
        DRW_REASON="absent or unreadable (rc=$DRW_RC): $DRW_DIAG"
      fi
    fi
  elif [ -z "$DRW_RAW" ]; then
    DRW_REASON="empty"
    ...
    DRW_PARSED=$(printf '%s\n' "$DRW_RAW" | awk '
      NF {
        records++
        if (records == 1) {
          if (NF != 2 || $1 != "total" || $2 !~ /^[0-9][0-9]*$/) bad=1
          next
        }
        if (NF < 9) { bad=1; next }
        mode=$1
        ...
        if (!date_ok) bad=1
        if (substr(mode,1,1) == "l") { links++; next }
        entries++
        if (substr(mode,8,1) == "r" || substr(mode,9,1) == "w") worldbad++
      }
      END {
        if (records < 1 || bad) exit 1
        print (worldbad + 0) "|" (entries + 0)
      }
    ')
    DRW_PARSE_RC=$?
    if [ "$DRW_PARSE_RC" -ne 0 ] || [ -z "$DRW_PARSED" ]; then
      DRW_REASON="unparseable"
    else
      DRW_BAD=${DRW_PARSED%%\|*}
      DRW_TOTAL=${DRW_PARSED#*\|}
    fi
  fi

  if [ "$DRW_REASON" = "absent" ]; then
    add security diracc_varadmras_world "/var/adm/ras world access" NOT_APPLICABLE med \
        "/var/adm/ras absent" \
        "The probe returned the determinate-absence signature ('/var/adm/ras not found', exit $DRW_RC); /var/adm/ras need not exist, so the world-access boundary does not apply." \
        "n/a" "cis-l1"
    ...
    add security diracc_varadmras_world "/var/adm/ras world access" PASS med \
        "/var/adm/ras: 0 of $DRW_TOTAL entries allow world read or write" \
        "Entries under /var/adm/ras deny world read and write access as required." \
        "n/a" "cis-l1"
  else
