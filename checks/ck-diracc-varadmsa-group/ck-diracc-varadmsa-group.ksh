...
AIXRAY_TOOL=ck-diracc-varadmsa-group


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # diracc_varadmsa_group — CIS L1-aligned /var/adm/sa group boundary.
  ...
  typeset DSAG_RAW DSAG_RC DSAG_PARSED DSAG_PARSE_RC DSAG_GID DSAG_REASON DSAG_ABSENT DSAG_DIAG

  DSAG_RAW=$(aixv diracc_varadmsa_group_ls ls -ldn /var/adm/sa)
  DSAG_RC=$?
  DSAG_PARSED=""
  DSAG_GID=""
  DSAG_REASON=""
  ...
  if [ "$DSAG_RC" -ne 0 ]; then
    DSAG_REASON="absent or unreadable (rc=$DSAG_RC)"
    case "$DSAG_RAW" in
      *"/var/adm/sa not found"*) DSAG_ABSENT=1 ;;
    esac
    if [ "$DSAG_ABSENT" -eq 1 ]; then
      DSAG_REASON="absent"
    else
      DSAG_DIAG=$(printf '%s\n' "$DSAG_RAW" | awk 'NF { gsub(/[ \t]+/, " "); sub(/^ /, ""); sub(/ $/, ""); if (length("" $0) > 40) $0 = substr($0, 1, 37) "..."; print; exit }')
      if [ -n "$DSAG_DIAG" ]; then
        DSAG_REASON="absent or unreadable (rc=$DSAG_RC): $DSAG_DIAG"
      fi
    fi
  elif [ -z "$DSAG_RAW" ]; then
    DSAG_REASON="empty"
  else
    DSAG_PARSED=$(printf '%s\n' "$DSAG_RAW" | awk '
      NF {
        records++
    ...
    ')
    DSAG_PARSE_RC=$?
    if [ "$DSAG_PARSE_RC" -ne 0 ] || [ -z "$DSAG_PARSED" ]; then
      DSAG_REASON="unparseable"
    else
      DSAG_GID=$DSAG_PARSED
    fi
  fi

  if [ "$DSAG_REASON" = "absent" ]; then
    add security diracc_varadmsa_group "/var/adm/sa group" NOT_APPLICABLE med \
        "/var/adm/sa absent" \
        "The probe returned the determinate-absence signature ('/var/adm/sa not found', exit $DSAG_RC); /var/adm/sa need not exist, so the group boundary does not apply." \
        "n/a" "cis-l1"
    ...
    add security diracc_varadmsa_group "/var/adm/sa group" PASS med \
        ...
        "/var/adm/sa has the required group boundary." \
        "n/a" "cis-l1"
  else
