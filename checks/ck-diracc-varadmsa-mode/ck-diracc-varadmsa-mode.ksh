...
AIXRAY_TOOL=ck-diracc-varadmsa-mode


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # diracc_varadmsa_mode — CIS L1-aligned /var/adm/sa mode boundary.
  ...
  # TODO-VERIFY: ceiling 0755 accepts stricter modes; exactness unconfirmed (README)
  typeset DSAM_RAW DSAM_RC DSAM_PARSED DSAM_PARSE_RC DSAM_MODE DSAM_OK DSAM_REASON DSAM_ABSENT DSAM_DIAG

  DSAM_RAW=$(aixv diracc_varadmsa_mode_ls ls -ldn /var/adm/sa)
  DSAM_RC=$?
  DSAM_PARSED=""; DSAM_MODE=""; DSAM_OK=""; DSAM_REASON=""; DSAM_ABSENT=0; DSAM_DIAG=""

  if [ "$DSAM_RC" -ne 0 ]; then
    DSAM_REASON="absent or unreadable (rc=$DSAM_RC)"
    case "$DSAM_RAW" in
      *"/var/adm/sa not found"*) DSAM_ABSENT=1 ;;
    esac
    if [ "$DSAM_ABSENT" -eq 1 ]; then
      DSAM_REASON="absent"
    else
      DSAM_DIAG=$(printf '%s\n' "$DSAM_RAW" | awk 'NF { gsub(/[ \t]+/, " "); sub(/^ /, ""); sub(/ $/, ""); if (length("" $0) > 40) $0 = substr($0, 1, 37) "..."; print; exit }')
      if [ -n "$DSAM_DIAG" ]; then
        DSAM_REASON="absent or unreadable (rc=$DSAM_RC): $DSAM_DIAG"
      fi
    fi
  elif [ -z "$DSAM_RAW" ]; then
    DSAM_REASON="empty"
  else
    DSAM_PARSED=$(printf '%s
' "$DSAM_RAW" | awk '
      NF {
        records++
        ...
        if (!date_ok) bad=1
        if ($NF != "/var/adm/sa") bad=1
      }
      END {
        if (records != 1 || bad) exit 1
        target="0755"; ceiling=0
    ...
    ')
    DSAM_PARSE_RC=$?
    if [ "$DSAM_PARSE_RC" -ne 0 ] || [ -z "$DSAM_PARSED" ]; then DSAM_REASON="unparseable"
    else DSAM_MODE=${DSAM_PARSED%%\|*}; DSAM_OK=${DSAM_PARSED#*\|}; fi
  fi
