...
AIXRAY_TOOL=ck-diracc-varadmsa-owner


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # diracc_varadmsa_owner — CIS L1-aligned /var/adm/sa owner boundary.
  ...
  typeset DSAO_RAW DSAO_RC DSAO_PARSED DSAO_PARSE_RC DSAO_UID DSAO_REASON DSAO_ABSENT DSAO_DIAG

  DSAO_RAW=$(aixv diracc_varadmsa_owner_ls ls -ldn /var/adm/sa)
  DSAO_RC=$?
  DSAO_PARSED=""
  DSAO_UID=""
  DSAO_REASON=""
  ...
  if [ "$DSAO_RC" -ne 0 ]; then
    DSAO_REASON="absent or unreadable (rc=$DSAO_RC)"
    case "$DSAO_RAW" in
      *"/var/adm/sa not found"*) DSAO_ABSENT=1 ;;
    esac
    if [ "$DSAO_ABSENT" -eq 1 ]; then
      DSAO_REASON="absent"
    else
      DSAO_DIAG=$(printf '%s\n' "$DSAO_RAW" | awk 'NF { gsub(/[ \t]+/, " "); sub(/^ /, ""); sub(/ $/, ""); if (length("" $0) > 40) $0 = substr($0, 1, 37) "..."; print; exit }')
      if [ -n "$DSAO_DIAG" ]; then
        DSAO_REASON="absent or unreadable (rc=$DSAO_RC): $DSAO_DIAG"
      fi
    fi
  elif [ -z "$DSAO_RAW" ]; then
    DSAO_REASON="empty"
  else
    DSAO_PARSED=$(printf '%s\n' "$DSAO_RAW" | awk '
      NF {
        records++
    ...
    ')
    DSAO_PARSE_RC=$?
    if [ "$DSAO_PARSE_RC" -ne 0 ] || [ -z "$DSAO_PARSED" ]; then
      DSAO_REASON="unparseable"
    else
      DSAO_UID=$DSAO_PARSED
    fi
  fi

  if [ "$DSAO_REASON" = "absent" ]; then
    add security diracc_varadmsa_owner "/var/adm/sa owner" NOT_APPLICABLE med \
        "/var/adm/sa absent" \
        "The probe returned the determinate-absence signature ('/var/adm/sa not found', exit $DSAO_RC); /var/adm/sa need not exist, so the owner boundary does not apply." \
        "n/a" "cis-l1"
    ...
    add security diracc_varadmsa_owner "/var/adm/sa owner" PASS med \
        ...
        "/var/adm/sa has the required owner boundary." \
        "n/a" "cis-l1"
  else
