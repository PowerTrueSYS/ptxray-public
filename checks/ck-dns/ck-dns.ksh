...
# dns
RAW=$(aix resolv_conf grep nameserver /etc/resolv.conf); RC=$?
DNS=$(printf '%s\n' "$RAW" | awk '
  function valid_ipv4(address, n, octet, i) {
    n = split(address, octet, ".")
    if (n != 4) return 0
    for (i = 1; i <= 4; i++) {
      if (octet[i] !~ /^[0-9][0-9]?[0-9]?$/) return 0
      if (octet[i] ~ /^0[0-9]/) return 0
      if (octet[i] + 0 > 255) return 0
    }
    return 1
  }
  function valid_ipv6(address, chars, colons) {
    if (address !~ /^[0-9A-Fa-f:]+$/) return 0
    chars = gsub(/./, "&", address)
    if (chars > 39) return 0
    colons = gsub(/:/, ":", address)
  ...
  }
  {
    ...
    if (valid_ipv4($2) || valid_ipv6($2)) valid++
    else active_malformed++
  }
  END { printf "%d %d %d", input_lines + 0, valid + 0, active_malformed + 0 }
')
DNS_LINES=${DNS%% *}
DNS_REST=${DNS#* }
DNS_VALID=${DNS_REST%% *}
DNS_MALFORMED=${DNS_REST##* }
if [ "$RC" -ge 2 ]; then
  add config dns "DNS resolver" NOT_ASSESSED med \
      "not assessed - /etc/resolv.conf unreadable (rc $RC)" \
      "PTxray could not read /etc/resolv.conf, so resolver configuration cannot be assessed." \
      "make /etc/resolv.conf readable by the check, then re-run PTxray."
elif [ "$RC" -eq 1 ]; then
...
elif [ "${DNS_VALID:-0}" -ge 1 ]; then
  add config dns "DNS resolver" PASS low "$DNS_VALID nameserver(s)" \
      "At least one DNS resolver is configured." "n/a"
elif [ "${DNS_MALFORMED:-0}" -ge 1 ]; then
  add config dns "DNS resolver" NOT_ASSESSED med \
      "not assessed - /etc/resolv.conf has no parseable nameserver address" \
      "The nameserver entries in /etc/resolv.conf carry no syntactically valid IPv4 or IPv6 address." \
      "correct the nameserver entries in /etc/resolv.conf, then re-run PTxray."
elif [ "${DNS_LINES:-0}" -eq 0 ]; then
  add config dns "DNS resolver" NOT_ASSESSED med \
      "not assessed - /etc/resolv.conf capture is empty (rc 0)" \
      "The nameserver capture produced no output despite a successful probe, so resolver configuration cannot be assessed." \
      "verify the /etc/resolv.conf capture, then re-run PTxray."
else
  add config dns "DNS resolver" WARN low "no nameserver" \
      "No DNS resolver configured — this may be intentional on an isolated LPAR, or a gap." \
      "add a nameserver to /etc/resolv.conf if this box should resolve names."
