...
_AIXRAY_SESSION_KEYS=""
  typeset DT_MATCH DT_MATCH_RC DT_COUNT DT_STATE
    ...
    if [ "$DT_MATCH_RC" -ne 0 ]; then
      ...
      if [ "$DT_COUNT" -gt 1 ]; then
        ...
        add security dt "dt service" PASS low \
            "dt not defined in the complete SRC inventory" \
            "The dt subsystem is absent and therefore is not in use." "n/a" "cis-l1"
      else
        DT_STATE=$DT_MATCH
        if [ "$DT_STATE" = in_use ]; then
          ...
          add security dt "dt service" PASS low \
              "dt inoperative in the SRC inventory" \
              "The dt subsystem is present but not in use." "n/a" "cis-l1"
        fi
      fi
    fi
  fi
