...
AIXRAY_TOOL=ck-errnotify-methods
...
_AIXRAY_SESSION_KEYS=""
  ...
  # errnotify_decode's own comment; believability review finding #7).
  ...
  if [ "$RC" -ne 0 ] || [ "$ERRNOTIFY_OK" -ne 1 ]; then
    ...
    ENDIAG=$(printf '%s' "$ERRNOTIFY_DEC" | awk -F'\t' '{print $3+0}')
    ENLIST=$(printf '%s' "$ERRNOTIFY_DEC" | awk -F'\t' '{print $4}')
    DIAGN=""
