...
AIXRAY_TOOL=ck-fileperm-atallow-group


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_atallow_group — CIS L1-aligned /var/adm/cron/at.allow group boundary.
  ...
  typeset FAAG_RAW FAAG_RC FAAG_PARSED FAAG_PARSE_RC FAAG_GID FAAG_REASON FAAG_ABSENT FAAG_DIAG

  FAAG_RAW=$(aixv fileperm_atallow_group_ls ls -ln /var/adm/cron/at.allow)
  FAAG_RC=$?
  FAAG_PARSED=""
  FAAG_GID=""
  FAAG_REASON=""
  ...
  if [ "$FAAG_RC" -ne 0 ]; then
    FAAG_REASON="absent or unreadable (rc=$FAAG_RC)"
    case "$FAAG_RAW" in
      *"/var/adm/cron/at.allow not found"*) FAAG_ABSENT=1 ;;
    esac
    if [ "$FAAG_ABSENT" -eq 1 ]; then
      FAAG_REASON="absent"
    else
      FAAG_DIAG=$(printf '%s\n' "$FAAG_RAW" | awk 'NF { gsub(/[ \t]+/, " "); sub(/^ /, ""); sub(/ $/, ""); if (length("" $0) > 40) $0 = substr($0, 1, 37) "..."; print; exit }')
      if [ -n "$FAAG_DIAG" ]; then
        FAAG_REASON="absent or unreadable (rc=$FAAG_RC): $FAAG_DIAG"
      fi
    fi
  elif [ -z "$FAAG_RAW" ]; then
    FAAG_REASON="empty"
  else
    FAAG_PARSED=$(printf '%s\n' "$FAAG_RAW" | awk '
      NF {
        records++
    ...
    ')
    FAAG_PARSE_RC=$?
    if [ "$FAAG_PARSE_RC" -ne 0 ] || [ -z "$FAAG_PARSED" ]; then
      FAAG_REASON="unparseable"
    else
      FAAG_GID=$FAAG_PARSED
    fi
  fi

  if [ "$FAAG_REASON" = "absent" ]; then
    add security fileperm_atallow_group "/var/adm/cron/at.allow group" NOT_APPLICABLE med \
        "/var/adm/cron/at.allow absent" \
        "The probe returned the determinate-absence signature ('/var/adm/cron/at.allow not found', exit $FAAG_RC); /var/adm/cron/at.allow need not exist, so the group boundary does not apply." \
        "n/a" "cis-l1"
    ...
    add security fileperm_atallow_group "/var/adm/cron/at.allow group" PASS med \
        ...
        "/var/adm/cron/at.allow has the required group boundary." \
        "n/a" "cis-l1"
  else
