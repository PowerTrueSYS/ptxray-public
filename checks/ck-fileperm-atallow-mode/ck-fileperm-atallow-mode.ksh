...
AIXRAY_TOOL=ck-fileperm-atallow-mode


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_atallow_mode — CIS L1-aligned /var/adm/cron/at.allow mode boundary.
  ...
  typeset FAAM_RAW FAAM_RC FAAM_PARSED FAAM_PARSE_RC FAAM_MODE FAAM_OK FAAM_REASON FAAM_ABSENT FAAM_DIAG

  FAAM_RAW=$(aixv fileperm_atallow_mode_ls ls -ln /var/adm/cron/at.allow)
  FAAM_RC=$?
  FAAM_PARSED=""
  FAAM_MODE=""
  FAAM_OK=""
  ...
  if [ "$FAAM_RC" -ne 0 ]; then
    FAAM_REASON="absent or unreadable (rc=$FAAM_RC)"
    case "$FAAM_RAW" in
      *"/var/adm/cron/at.allow not found"*) FAAM_ABSENT=1 ;;
    esac
    if [ "$FAAM_ABSENT" -eq 1 ]; then
      FAAM_REASON="absent"
    else
      FAAM_DIAG=$(printf '%s\n' "$FAAM_RAW" | awk 'NF { gsub(/[ \t]+/, " "); sub(/^ /, ""); sub(/ $/, ""); if (length("" $0) > 40) $0 = substr($0, 1, 37) "..."; print; exit }')
      if [ -n "$FAAM_DIAG" ]; then
        FAAM_REASON="absent or unreadable (rc=$FAAM_RC): $FAAM_DIAG"
      fi
    fi
  elif [ -z "$FAAM_RAW" ]; then
    FAAM_REASON="empty"
  else
    FAAM_PARSED=$(printf '%s\n' "$FAAM_RAW" | awk '
      NF {
        records++
        ...
        if (!date_ok) bad=1
        if ($NF != "/var/adm/cron/at.allow") bad=1
      }
      END {
    ...
    ')
    FAAM_PARSE_RC=$?
    if [ "$FAAM_PARSE_RC" -ne 0 ] || [ -z "$FAAM_PARSED" ]; then
      FAAM_REASON="unparseable"
    else
      FAAM_MODE=${FAAM_PARSED%%\|*}
      FAAM_OK=${FAAM_PARSED#*\|}
    fi
  fi

  if [ "$FAAM_REASON" = "absent" ]; then
    add security fileperm_atallow_mode "/var/adm/cron/at.allow mode" NOT_APPLICABLE med \
        "/var/adm/cron/at.allow absent" \
        "The probe returned the determinate-absence signature ('/var/adm/cron/at.allow not found', exit $FAAM_RC); /var/adm/cron/at.allow need not exist, so the mode boundary does not apply." \
        "n/a" "cis-l1"
    F_EVIDENCE_COMMANDS[$((NFIND-1))]="ls -ln /var/adm/cron/at.allow"
    ...
    add security fileperm_atallow_mode "/var/adm/cron/at.allow mode" PASS med \
        ...
        "/var/adm/cron/at.allow has the required mode boundary." \
        "n/a" "cis-l1"
  else
