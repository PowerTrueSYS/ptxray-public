...
AIXRAY_TOOL=ck-fileperm-atallow-owner


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_atallow_owner — CIS L1-aligned /var/adm/cron/at.allow owner boundary.
  ...
  typeset FAAO_RAW FAAO_RC FAAO_PARSED FAAO_PARSE_RC FAAO_UID FAAO_REASON FAAO_ABSENT FAAO_DIAG

  FAAO_RAW=$(aixv fileperm_atallow_owner_ls ls -ln /var/adm/cron/at.allow)
  FAAO_RC=$?
  FAAO_PARSED=""
  FAAO_UID=""
  FAAO_REASON=""
  ...
  if [ "$FAAO_RC" -ne 0 ]; then
    FAAO_REASON="absent or unreadable (rc=$FAAO_RC)"
    case "$FAAO_RAW" in
      *"/var/adm/cron/at.allow not found"*) FAAO_ABSENT=1 ;;
    esac
    if [ "$FAAO_ABSENT" -eq 1 ]; then
      FAAO_REASON="absent"
    else
      FAAO_DIAG=$(printf '%s\n' "$FAAO_RAW" | awk 'NF { gsub(/[ \t]+/, " "); sub(/^ /, ""); sub(/ $/, ""); if (length("" $0) > 40) $0 = substr($0, 1, 37) "..."; print; exit }')
      if [ -n "$FAAO_DIAG" ]; then
        FAAO_REASON="absent or unreadable (rc=$FAAO_RC): $FAAO_DIAG"
      fi
    fi
  elif [ -z "$FAAO_RAW" ]; then
    FAAO_REASON="empty"
  else
    FAAO_PARSED=$(printf '%s\n' "$FAAO_RAW" | awk '
      NF {
        records++
    ...
    ')
    FAAO_PARSE_RC=$?
    if [ "$FAAO_PARSE_RC" -ne 0 ] || [ -z "$FAAO_PARSED" ]; then
      FAAO_REASON="unparseable"
    else
      FAAO_UID=$FAAO_PARSED
    fi
  fi

  if [ "$FAAO_REASON" = "absent" ]; then
    add security fileperm_atallow_owner "/var/adm/cron/at.allow owner" NOT_APPLICABLE med \
        "/var/adm/cron/at.allow absent" \
        "The probe returned the determinate-absence signature ('/var/adm/cron/at.allow not found', exit $FAAO_RC); /var/adm/cron/at.allow need not exist, so the owner boundary does not apply." \
        "n/a" "cis-l1"
    ...
    add security fileperm_atallow_owner "/var/adm/cron/at.allow owner" PASS med \
        ...
        "/var/adm/cron/at.allow has the required owner boundary." \
        "n/a" "cis-l1"
  else
