...
AIXRAY_TOOL=ck-fileperm-atjobs-owner


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_atjobs_owner — DISA STIG V-245566.
  # This unit shares the fixed-directory capture key with the group/mode units.
  ...
  if [ -n "$FPA_REASON" ]; then
    ...
    case "$FPA_UID" in
      0)
        FPA_OWNER_OBSERVED="/var/spool/cron/atjobs owner=0 (root)"
        add security fileperm_atjobs_owner "Atjobs directory owner" PASS med \
            "$FPA_OWNER_OBSERVED" \
            "/var/spool/cron/atjobs is owned by an explicitly accepted account (root or bin) as required by DISA STIG V-245566." \
            "n/a" "stig:V-245566"
        ;;
      2)
        FPA_OWNER_OBSERVED="/var/spool/cron/atjobs owner=2 (bin)"
        add security fileperm_atjobs_owner "Atjobs directory owner" PASS med \
            "$FPA_OWNER_OBSERVED" \
            "/var/spool/cron/atjobs is owned by an explicitly accepted account (root or bin) as required by DISA STIG V-245566." \
            "n/a" "stig:V-245566"
        ;;
      *)
        FPA_OWNER_OBSERVED="/var/spool/cron/atjobs owner=$FPA_UID"
