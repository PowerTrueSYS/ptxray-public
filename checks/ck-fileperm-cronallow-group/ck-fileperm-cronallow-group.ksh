...
AIXRAY_TOOL=ck-fileperm-cronallow-group


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_cronallow_group — CIS L1-aligned /var/adm/cron/cron.allow group boundary.
  ...
  typeset FCAG_RAW FCAG_RC FCAG_PARSED FCAG_PARSE_RC FCAG_GID FCAG_REASON FCAG_ABSENT FCAG_DIAG

  FCAG_RAW=$(aixv fileperm_cronallow_group_ls ls -ln /var/adm/cron/cron.allow)
  FCAG_RC=$?
  FCAG_PARSED=""
  FCAG_GID=""
  FCAG_REASON=""
  ...
  if [ "$FCAG_RC" -ne 0 ]; then
    FCAG_REASON="absent or unreadable (rc=$FCAG_RC)"
    case "$FCAG_RAW" in
      *"/var/adm/cron/cron.allow not found"*) FCAG_ABSENT=1 ;;
    esac
    if [ "$FCAG_ABSENT" -eq 1 ]; then
      FCAG_REASON="absent"
    else
      FCAG_DIAG=$(printf '%s\n' "$FCAG_RAW" | awk 'NF { gsub(/[ \t]+/, " "); sub(/^ /, ""); sub(/ $/, ""); if (length("" $0) > 40) $0 = substr($0, 1, 37) "..."; print; exit }')
      if [ -n "$FCAG_DIAG" ]; then
        FCAG_REASON="absent or unreadable (rc=$FCAG_RC): $FCAG_DIAG"
      fi
    fi
  elif [ -z "$FCAG_RAW" ]; then
    FCAG_REASON="empty"
  else
    FCAG_PARSED=$(printf '%s\n' "$FCAG_RAW" | awk '
      NF {
        records++
    ...
    ')
    FCAG_PARSE_RC=$?
    if [ "$FCAG_PARSE_RC" -ne 0 ] || [ -z "$FCAG_PARSED" ]; then
      FCAG_REASON="unparseable"
    else
      FCAG_GID=$FCAG_PARSED
    fi
  fi

  if [ "$FCAG_REASON" = "absent" ]; then
    add security fileperm_cronallow_group "/var/adm/cron/cron.allow group" NOT_APPLICABLE med \
        "/var/adm/cron/cron.allow absent" \
        "The probe returned the determinate-absence signature ('/var/adm/cron/cron.allow not found', exit $FCAG_RC); /var/adm/cron/cron.allow need not exist, so the group boundary does not apply." \
        "n/a" "cis-l1"
    ...
    add security fileperm_cronallow_group "/var/adm/cron/cron.allow group" PASS med \
        ...
        "/var/adm/cron/cron.allow has the required group boundary." \
        "n/a" "cis-l1"
  else
