...
AIXRAY_TOOL=ck-fileperm-cronallow-mode


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_cronallow_mode — CIS L1-aligned /var/adm/cron/cron.allow mode boundary.
  ...
  typeset FCAM_RAW FCAM_RC FCAM_PARSED FCAM_PARSE_RC FCAM_MODE FCAM_OK FCAM_REASON FCAM_ABSENT FCAM_DIAG

  FCAM_RAW=$(aixv fileperm_cronallow_mode_ls ls -ln /var/adm/cron/cron.allow)
  FCAM_RC=$?
  FCAM_PARSED=""
  FCAM_MODE=""
  FCAM_OK=""
  ...
  if [ "$FCAM_RC" -ne 0 ]; then
    FCAM_REASON="absent or unreadable (rc=$FCAM_RC)"
    case "$FCAM_RAW" in
      *"/var/adm/cron/cron.allow not found"*) FCAM_ABSENT=1 ;;
    esac
    if [ "$FCAM_ABSENT" -eq 1 ]; then
      FCAM_REASON="absent"
    else
      FCAM_DIAG=$(printf '%s\n' "$FCAM_RAW" | awk 'NF { gsub(/[ \t]+/, " "); sub(/^ /, ""); sub(/ $/, ""); if (length("" $0) > 40) $0 = substr($0, 1, 37) "..."; print; exit }')
      if [ -n "$FCAM_DIAG" ]; then
        FCAM_REASON="absent or unreadable (rc=$FCAM_RC): $FCAM_DIAG"
      fi
    fi
  elif [ -z "$FCAM_RAW" ]; then
    FCAM_REASON="empty"
  else
    FCAM_PARSED=$(printf '%s\n' "$FCAM_RAW" | awk '
      NF {
        records++
        ...
        if (!date_ok) bad=1
        if ($NF != "/var/adm/cron/cron.allow") bad=1
      }
      END {
        if (records != 1 || bad) exit 1
        target="0400"; ceiling=0
    ...
    ')
    FCAM_PARSE_RC=$?
    if [ "$FCAM_PARSE_RC" -ne 0 ] || [ -z "$FCAM_PARSED" ]; then
      FCAM_REASON="unparseable"
    else
      FCAM_MODE=${FCAM_PARSED%%\|*}
      FCAM_OK=${FCAM_PARSED#*\|}
    fi
  fi

  if [ "$FCAM_REASON" = "absent" ]; then
    add security fileperm_cronallow_mode "/var/adm/cron/cron.allow mode" NOT_APPLICABLE med \
        "/var/adm/cron/cron.allow absent" \
        "The probe returned the determinate-absence signature ('/var/adm/cron/cron.allow not found', exit $FCAM_RC); /var/adm/cron/cron.allow need not exist, so the mode boundary does not apply." \
        "n/a" "cis-l1"
    F_EVIDENCE_COMMANDS[$((NFIND-1))]="ls -ln /var/adm/cron/cron.allow"
    ...
    add security fileperm_cronallow_mode "/var/adm/cron/cron.allow mode" PASS med \
        ...
        "/var/adm/cron/cron.allow has the required mode boundary." \
        "n/a" "cis-l1"
  else
