...
AIXRAY_TOOL=ck-fileperm-cronallow-owner


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_cronallow_owner — CIS L1-aligned /var/adm/cron/cron.allow owner boundary.
  ...
  typeset FCAO_RAW FCAO_RC FCAO_PARSED FCAO_PARSE_RC FCAO_UID FCAO_REASON FCAO_ABSENT FCAO_DIAG

  FCAO_RAW=$(aixv fileperm_cronallow_owner_ls ls -ln /var/adm/cron/cron.allow)
  FCAO_RC=$?
  FCAO_PARSED=""
  FCAO_UID=""
  FCAO_REASON=""
  ...
  if [ "$FCAO_RC" -ne 0 ]; then
    FCAO_REASON="absent or unreadable (rc=$FCAO_RC)"
    case "$FCAO_RAW" in
      *"/var/adm/cron/cron.allow not found"*) FCAO_ABSENT=1 ;;
    esac
    if [ "$FCAO_ABSENT" -eq 1 ]; then
      FCAO_REASON="absent"
    else
      FCAO_DIAG=$(printf '%s\n' "$FCAO_RAW" | awk 'NF { gsub(/[ \t]+/, " "); sub(/^ /, ""); sub(/ $/, ""); if (length("" $0) > 40) $0 = substr($0, 1, 37) "..."; print; exit }')
      if [ -n "$FCAO_DIAG" ]; then
        FCAO_REASON="absent or unreadable (rc=$FCAO_RC): $FCAO_DIAG"
      fi
    fi
  elif [ -z "$FCAO_RAW" ]; then
    FCAO_REASON="empty"
  else
    FCAO_PARSED=$(printf '%s\n' "$FCAO_RAW" | awk '
      NF {
        records++
    ...
    ')
    FCAO_PARSE_RC=$?
    if [ "$FCAO_PARSE_RC" -ne 0 ] || [ -z "$FCAO_PARSED" ]; then
      FCAO_REASON="unparseable"
    else
      FCAO_UID=$FCAO_PARSED
    fi
  fi

  if [ "$FCAO_REASON" = "absent" ]; then
    add security fileperm_cronallow_owner "/var/adm/cron/cron.allow owner" NOT_APPLICABLE med \
        "/var/adm/cron/cron.allow absent" \
        "The probe returned the determinate-absence signature ('/var/adm/cron/cron.allow not found', exit $FCAO_RC); /var/adm/cron/cron.allow need not exist, so the owner boundary does not apply." \
        "n/a" "cis-l1"
    ...
    add security fileperm_cronallow_owner "/var/adm/cron/cron.allow owner" PASS med \
        ...
        "/var/adm/cron/cron.allow has the required owner boundary." \
        "n/a" "cis-l1"
  else
