...
AIXRAY_TOOL=ck-fileperm-cronlog-group


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_cronlog_group — CIS L1-aligned /var/adm/cron/log group boundary.
  ...
  typeset FCLG_RAW FCLG_RC FCLG_PARSED FCLG_PARSE_RC FCLG_GID FCLG_REASON FCLG_ABSENT FCLG_DIAG

  FCLG_RAW=$(aixv fileperm_cronlog_group_ls ls -ln /var/adm/cron/log)
  FCLG_RC=$?
  FCLG_PARSED=""
  FCLG_GID=""
  FCLG_REASON=""
  ...
  if [ "$FCLG_RC" -ne 0 ]; then
    FCLG_REASON="absent or unreadable (rc=$FCLG_RC)"
    case "$FCLG_RAW" in
      *"/var/adm/cron/log not found"*) FCLG_ABSENT=1 ;;
    esac
    if [ "$FCLG_ABSENT" -eq 1 ]; then
      FCLG_REASON="absent"
    else
      FCLG_DIAG=$(printf '%s\n' "$FCLG_RAW" | awk 'NF { gsub(/[ \t]+/, " "); sub(/^ /, ""); sub(/ $/, ""); if (length("" $0) > 40) $0 = substr($0, 1, 37) "..."; print; exit }')
      if [ -n "$FCLG_DIAG" ]; then
        FCLG_REASON="absent or unreadable (rc=$FCLG_RC): $FCLG_DIAG"
      fi
    fi
  elif [ -z "$FCLG_RAW" ]; then
    FCLG_REASON="empty"
  else
    FCLG_PARSED=$(printf '%s\n' "$FCLG_RAW" | awk '
      NF {
        records++
    ...
    ')
    FCLG_PARSE_RC=$?
    if [ "$FCLG_PARSE_RC" -ne 0 ] || [ -z "$FCLG_PARSED" ]; then
      FCLG_REASON="unparseable"
    else
      FCLG_GID=$FCLG_PARSED
    fi
  fi

  if [ "$FCLG_REASON" = "absent" ]; then
    add security fileperm_cronlog_group "/var/adm/cron/log group" NOT_APPLICABLE med \
        "/var/adm/cron/log absent" \
        "The probe returned the determinate-absence signature ('/var/adm/cron/log not found', exit $FCLG_RC); /var/adm/cron/log need not exist, so the group boundary does not apply." \
        "n/a" "cis-l1"
    ...
    add security fileperm_cronlog_group "/var/adm/cron/log group" PASS med \
        ...
        "/var/adm/cron/log has the required group boundary." \
        "n/a" "cis-l1"
  else
