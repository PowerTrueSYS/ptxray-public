...
AIXRAY_TOOL=ck-fileperm-cronlog-mode


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_cronlog_mode — CIS L1-aligned /var/adm/cron/log mode boundary.
  ...
  typeset FCLM_RAW FCLM_RC FCLM_PARSED FCLM_PARSE_RC FCLM_MODE FCLM_OK FCLM_REASON FCLM_ABSENT FCLM_DIAG

  FCLM_RAW=$(aixv fileperm_cronlog_mode_ls ls -ln /var/adm/cron/log)
  FCLM_RC=$?
  FCLM_PARSED=""
  FCLM_MODE=""
  FCLM_OK=""
  ...
  if [ "$FCLM_RC" -ne 0 ]; then
    FCLM_REASON="absent or unreadable (rc=$FCLM_RC)"
    case "$FCLM_RAW" in
      *"/var/adm/cron/log not found"*) FCLM_ABSENT=1 ;;
    esac
    if [ "$FCLM_ABSENT" -eq 1 ]; then
      FCLM_REASON="absent"
    else
      FCLM_DIAG=$(printf '%s\n' "$FCLM_RAW" | awk 'NF { gsub(/[ \t]+/, " "); sub(/^ /, ""); sub(/ $/, ""); if (length("" $0) > 40) $0 = substr($0, 1, 37) "..."; print; exit }')
      if [ -n "$FCLM_DIAG" ]; then
        FCLM_REASON="absent or unreadable (rc=$FCLM_RC): $FCLM_DIAG"
      fi
    fi
  elif [ -z "$FCLM_RAW" ]; then
    FCLM_REASON="empty"
  else
    FCLM_PARSED=$(printf '%s\n' "$FCLM_RAW" | awk '
      NF {
        records++
        ...
        if (!date_ok) bad=1
        if ($NF != "/var/adm/cron/log") bad=1
      }
      END {
        if (records != 1 || bad) exit 1
        target="0660"; ceiling=0
    ...
    ')
    FCLM_PARSE_RC=$?
    if [ "$FCLM_PARSE_RC" -ne 0 ] || [ -z "$FCLM_PARSED" ]; then
      FCLM_REASON="unparseable"
    else
      FCLM_MODE=${FCLM_PARSED%%\|*}
      FCLM_OK=${FCLM_PARSED#*\|}
    fi
  fi

  if [ "$FCLM_REASON" = "absent" ]; then
    add security fileperm_cronlog_mode "/var/adm/cron/log mode" NOT_APPLICABLE med \
        "/var/adm/cron/log absent" \
        "The probe returned the determinate-absence signature ('/var/adm/cron/log not found', exit $FCLM_RC); /var/adm/cron/log need not exist, so the mode boundary does not apply." \
        "n/a" "cis-l1"
    ...
    add security fileperm_cronlog_mode "/var/adm/cron/log mode" PASS med \
        ...
        "/var/adm/cron/log has the required mode boundary." \
        "n/a" "cis-l1"
  else
