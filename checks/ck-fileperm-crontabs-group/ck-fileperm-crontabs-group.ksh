...
AIXRAY_TOOL=ck-fileperm-crontabs-group


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_crontabs_group — DISA STIG V-245569.
  # This unit shares the fixed-directory capture key with the mode unit.
  typeset FPC_RAW FPC_RC FPC_PARSED FPC_PARSE_RC FPC_REST
  typeset FPC_UID FPC_GID FPC_MODE FPC_MODE_OK FPC_REASON
  typeset FPC_MODE_OBSERVED FPC_GROUP_OBSERVED

  FPC_RAW=$(aix fileperm_crontabs_ls ls -ldn /var/spool/cron/crontabs)
  FPC_RC=$?
  FPC_PARSED=""
  FPC_UID=""
  FPC_GID=""
  ...
  if [ "$FPC_RC" -ne 0 ]; then
    if [ "$FPC_RC" -eq 2 ]; then
      ...
      FPC_REASON="capture failed (rc=$FPC_RC)"
    fi
  elif [ -z "$FPC_RAW" ]; then
    FPC_REASON="empty"
  else
    FPC_PARSED=$(printf '%s\n' "$FPC_RAW" | awk '
      ...
      END {
        ...
        print uid "|" gid "|" sprintf("%04o", bits) "|" mode_ok
      }
    ')
    FPC_PARSE_RC=$?
    if [ "$FPC_PARSE_RC" -ne 0 ] || [ -z "$FPC_PARSED" ]; then
      FPC_REASON="unparseable"
    else
      FPC_UID=${FPC_PARSED%%\|*}
      FPC_REST=${FPC_PARSED#*\|}
      FPC_GID=${FPC_REST%%\|*}
  ...
  if [ -n "$FPC_REASON" ]; then
    ...
    add security fileperm_crontabs_group "Crontabs directory group" PASS med \
        "/var/spool/cron/crontabs group=8 (cron)" \
        "/var/spool/cron/crontabs is group-owned by cron as required by DISA STIG V-245569." \
        "n/a" "stig:V-245569"
  else
    FPC_GROUP_OBSERVED="/var/spool/cron/crontabs group=$FPC_GID"
