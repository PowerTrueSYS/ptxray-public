...
AIXRAY_TOOL=ck-fileperm-dpid2log-group


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_dpid2log_group — CIS L1-aligned /var/tmp/dpid2.log group boundary.
  ...
  typeset FDPG_RAW FDPG_RC FDPG_PARSED FDPG_PARSE_RC FDPG_GID FDPG_REASON FDPG_ABSENT FDPG_DIAG

  FDPG_RAW=$(aixv fileperm_dpid2log_group_ls ls -ln /var/tmp/dpid2.log)
  FDPG_RC=$?
  FDPG_PARSED=""
  FDPG_GID=""
  FDPG_REASON=""
  ...
  if [ "$FDPG_RC" -ne 0 ]; then
    FDPG_REASON="absent or unreadable (rc=$FDPG_RC)"
    case "$FDPG_RAW" in
      *"/var/tmp/dpid2.log not found"*) FDPG_ABSENT=1 ;;
    esac
    if [ "$FDPG_ABSENT" -eq 1 ]; then
      FDPG_REASON="absent"
    else
      FDPG_DIAG=$(printf '%s\n' "$FDPG_RAW" | awk 'NF { gsub(/[ \t]+/, " "); sub(/^ /, ""); sub(/ $/, ""); if (length("" $0) > 40) $0 = substr($0, 1, 37) "..."; print; exit }')
      if [ -n "$FDPG_DIAG" ]; then
        FDPG_REASON="absent or unreadable (rc=$FDPG_RC): $FDPG_DIAG"
      fi
    fi
  elif [ -z "$FDPG_RAW" ]; then
    FDPG_REASON="empty"
  else
    FDPG_PARSED=$(printf '%s\n' "$FDPG_RAW" | awk '
      NF {
        records++
    ...
    ')
    FDPG_PARSE_RC=$?
    if [ "$FDPG_PARSE_RC" -ne 0 ] || [ -z "$FDPG_PARSED" ]; then
      FDPG_REASON="unparseable"
    else
      FDPG_GID=$FDPG_PARSED
    fi
  fi

  if [ "$FDPG_REASON" = "absent" ]; then
    add security fileperm_dpid2log_group "/var/tmp/dpid2.log group" NOT_APPLICABLE med \
        "/var/tmp/dpid2.log absent" \
        "The probe returned the determinate-absence signature ('/var/tmp/dpid2.log not found', exit $FDPG_RC); /var/tmp/dpid2.log need not exist, so the group boundary does not apply." \
        "n/a" "cis-l1"
    ...
    add security fileperm_dpid2log_group "/var/tmp/dpid2.log group" PASS med \
        ...
        "/var/tmp/dpid2.log has the required group boundary." \
        "n/a" "cis-l1"
  else
