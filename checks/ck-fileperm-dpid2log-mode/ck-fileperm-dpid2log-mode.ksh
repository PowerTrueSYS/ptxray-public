...
AIXRAY_TOOL=ck-fileperm-dpid2log-mode


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_dpid2log_mode — CIS L1-aligned /var/tmp/dpid2.log mode boundary.
  ...
  typeset FDPM_RAW FDPM_RC FDPM_PARSED FDPM_PARSE_RC FDPM_MODE FDPM_OK FDPM_REASON FDPM_ABSENT FDPM_DIAG

  FDPM_RAW=$(aixv fileperm_dpid2log_mode_ls ls -ln /var/tmp/dpid2.log)
  FDPM_RC=$?
  FDPM_PARSED=""
  FDPM_MODE=""
  FDPM_OK=""
  ...
  if [ "$FDPM_RC" -ne 0 ]; then
    FDPM_REASON="absent or unreadable (rc=$FDPM_RC)"
    case "$FDPM_RAW" in
      *"/var/tmp/dpid2.log not found"*) FDPM_ABSENT=1 ;;
    esac
    if [ "$FDPM_ABSENT" -eq 1 ]; then
      FDPM_REASON="absent"
    else
      FDPM_DIAG=$(printf '%s\n' "$FDPM_RAW" | awk 'NF { gsub(/[ \t]+/, " "); sub(/^ /, ""); sub(/ $/, ""); if (length("" $0) > 40) $0 = substr($0, 1, 37) "..."; print; exit }')
      if [ -n "$FDPM_DIAG" ]; then
        FDPM_REASON="absent or unreadable (rc=$FDPM_RC): $FDPM_DIAG"
      fi
    fi
  elif [ -z "$FDPM_RAW" ]; then
    FDPM_REASON="empty"
  else
    FDPM_PARSED=$(printf '%s\n' "$FDPM_RAW" | awk '
      NF {
        records++
    ...
    ')
    FDPM_PARSE_RC=$?
    if [ "$FDPM_PARSE_RC" -ne 0 ] || [ -z "$FDPM_PARSED" ]; then
      FDPM_REASON="unparseable"
    else
      FDPM_MODE=${FDPM_PARSED%%\|*}
      FDPM_OK=${FDPM_PARSED#*\|}
    fi
  fi

  if [ "$FDPM_REASON" = "absent" ]; then
    add security fileperm_dpid2log_mode "/var/tmp/dpid2.log mode" NOT_APPLICABLE med \
        "/var/tmp/dpid2.log absent" \
        "The probe returned the determinate-absence signature ('/var/tmp/dpid2.log not found', exit $FDPM_RC); /var/tmp/dpid2.log need not exist, so the mode boundary does not apply." \
        "n/a" "cis-l1"
    F_EVIDENCE_COMMANDS[$((NFIND-1))]="ls -ln /var/tmp/dpid2.log"
    ...
    add security fileperm_dpid2log_mode "/var/tmp/dpid2.log mode" PASS med \
        ...
        "/var/tmp/dpid2.log has the required mode boundary." \
        "n/a" "cis-l1"
  else
