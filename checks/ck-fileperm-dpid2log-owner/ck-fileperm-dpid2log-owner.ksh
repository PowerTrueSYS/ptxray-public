...
AIXRAY_TOOL=ck-fileperm-dpid2log-owner


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_dpid2log_owner — CIS L1-aligned /var/tmp/dpid2.log owner boundary.
  ...
  typeset FDPO_RAW FDPO_RC FDPO_PARSED FDPO_PARSE_RC FDPO_UID FDPO_REASON FDPO_ABSENT FDPO_DIAG

  FDPO_RAW=$(aixv fileperm_dpid2log_owner_ls ls -ln /var/tmp/dpid2.log)
  FDPO_RC=$?
  FDPO_PARSED=""
  FDPO_UID=""
  FDPO_REASON=""
  ...
  if [ "$FDPO_RC" -ne 0 ]; then
    FDPO_REASON="absent or unreadable (rc=$FDPO_RC)"
    case "$FDPO_RAW" in
      *"/var/tmp/dpid2.log not found"*) FDPO_ABSENT=1 ;;
    esac
    if [ "$FDPO_ABSENT" -eq 1 ]; then
      FDPO_REASON="absent"
    else
      FDPO_DIAG=$(printf '%s\n' "$FDPO_RAW" | awk 'NF { gsub(/[ \t]+/, " "); sub(/^ /, ""); sub(/ $/, ""); if (length("" $0) > 40) $0 = substr($0, 1, 37) "..."; print; exit }')
      if [ -n "$FDPO_DIAG" ]; then
        FDPO_REASON="absent or unreadable (rc=$FDPO_RC): $FDPO_DIAG"
      fi
    fi
  elif [ -z "$FDPO_RAW" ]; then
    FDPO_REASON="empty"
  else
    FDPO_PARSED=$(printf '%s\n' "$FDPO_RAW" | awk '
      NF {
        records++
    ...
    ')
    FDPO_PARSE_RC=$?
    if [ "$FDPO_PARSE_RC" -ne 0 ] || [ -z "$FDPO_PARSED" ]; then
      FDPO_REASON="unparseable"
    else
      FDPO_UID=$FDPO_PARSED
    fi
  fi

  if [ "$FDPO_REASON" = "absent" ]; then
    add security fileperm_dpid2log_owner "/var/tmp/dpid2.log owner" NOT_APPLICABLE med \
        "/var/tmp/dpid2.log absent" \
        "The probe returned the determinate-absence signature ('/var/tmp/dpid2.log not found', exit $FDPO_RC); /var/tmp/dpid2.log need not exist, so the owner boundary does not apply." \
        "n/a" "cis-l1"
    ...
    add security fileperm_dpid2log_owner "/var/tmp/dpid2.log owner" PASS med \
        ...
        "/var/tmp/dpid2.log has the required owner boundary." \
        "n/a" "cis-l1"
  else
