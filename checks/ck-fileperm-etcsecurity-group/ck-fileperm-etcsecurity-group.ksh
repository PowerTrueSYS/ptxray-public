...
AIXRAY_TOOL=ck-fileperm-etcsecurity-group


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_etcsecurity_group — CIS L1-aligned /etc/security group boundary.
  ...
  if [ "${MYUID:-0}" != "0" ]; then
    ...
    FIG_RAW=$(aix fileperm_etcsecurity_group_ls ls -ldn /etc/security)
    FIG_RC=$?
  fi
  FIG_PARSED=""
  FIG_GID=""
  ...
  if [ "$FIG_RC" -eq 126 ]; then
    FIG_REASON="requires root; capture not executed"
  elif [ "$FIG_RC" -ne 0 ]; then
  ...
  if [ -n "$FIG_REASON" ]; then
    ...
    add security fileperm_etcsecurity_group "/etc/security group" PASS med \
        ...
        "/etc/security has the required group boundary." \
        "n/a" "cis-l1"
  else
