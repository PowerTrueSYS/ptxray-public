...
AIXRAY_TOOL=ck-fileperm-etcsecurity-mode


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_etcsecurity_mode — CIS L1-aligned /etc/security mode boundary.
  ...
  if [ "${MYUID:-0}" != "0" ]; then
    ...
    FIM_RAW=$(aix fileperm_etcsecurity_mode_ls ls -ldn /etc/security)
    FIM_RC=$?
  fi
  FIM_PARSED=""
  FIM_MODE=""
  ...
  if [ "$FIM_RC" -eq 126 ]; then
    FIM_REASON="requires root; capture not executed"
  elif [ "$FIM_RC" -ne 0 ]; then
    ...
    FIM_PARSED=$(printf '%s\n' "$FIM_RAW" | awk '
      NF {
        ...
        if (!date_ok) bad=1
        if ($NF != "/etc/security") bad=1
      }
      END {
        if (records != 1 || bad) exit 1
        target="2750"; ceiling=0
        ...
        setgid=int(bits/1024)%2
        print sprintf("%04o", bits) "|" (extra == 0 && setgid == 1)
      }
    ')
    FIM_PARSE_RC=$?
  ...
  if [ -n "$FIM_REASON" ]; then
    ...
    add security fileperm_etcsecurity_mode "/etc/security mode" PASS med \
        ...
        "/etc/security has the required mode boundary." \
        "n/a" "cis-l1"
  else
