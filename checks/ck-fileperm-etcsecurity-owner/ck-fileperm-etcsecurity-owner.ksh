...
AIXRAY_TOOL=ck-fileperm-etcsecurity-owner


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_etcsecurity_owner — CIS L1-aligned /etc/security owner boundary.
  ...
  if [ "${MYUID:-0}" != "0" ]; then
    ...
    FIO_RAW=$(aix fileperm_etcsecurity_owner_ls ls -ldn /etc/security)
    FIO_RC=$?
  fi
  FIO_PARSED=""
  FIO_UID=""
  ...
  if [ "$FIO_RC" -eq 126 ]; then
    FIO_REASON="requires root; capture not executed"
  ...
  if [ -n "$FIO_REASON" ]; then
    ...
    add security fileperm_etcsecurity_owner "/etc/security owner" PASS med \
        ...
        "/etc/security has the required owner boundary." \
        "n/a" "cis-l1"
  else
