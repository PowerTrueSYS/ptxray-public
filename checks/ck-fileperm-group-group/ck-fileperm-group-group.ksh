...
AIXRAY_TOOL=ck-fileperm-group-group


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_group_group — CIS Level 1 alignment for /etc/group.
  # Read the fixed path exactly once; uncertain evidence is never accepted as PASS.
  typeset FPGG_RAW FPGG_RC FPGG_PARSED FPGG_PARSE_RC FPGG_REST
  typeset FPGG_UID FPGG_GID FPGG_MODE FPGG_MODE_DEC FPGG_REASON
  typeset FPGG_GROUP_OBSERVED

  FPGG_RAW=$(aix fileperm_group_group_ls ls -ln /etc/group)
  FPGG_RC=$?
  FPGG_PARSED=""
  FPGG_UID=""
  FPGG_GID=""
  ...
  if [ "$FPGG_RC" -ne 0 ]; then
    if [ "$FPGG_RC" -eq 2 ]; then
      FPGG_REASON="missing"
    else
      FPGG_REASON="capture failed (rc=$FPGG_RC)"
    fi
  elif [ -z "$FPGG_RAW" ]; then
    FPGG_REASON="empty"
  else
    FPGG_PARSED=$(printf '%s\n' "$FPGG_RAW" | awk '
      NF {
        records++
        if (records != 1 || NF < 9) {
          bad=1
    ...
    ')
    FPGG_PARSE_RC=$?
    if [ "$FPGG_PARSE_RC" -ne 0 ] || [ -z "$FPGG_PARSED" ]; then
      FPGG_REASON="unparseable"
    else
      FPGG_UID=${FPGG_PARSED%%\|*}
      FPGG_REST=${FPGG_PARSED#*\|}
      FPGG_GID=${FPGG_REST%%\|*}
      FPGG_REST=${FPGG_REST#*\|}
      FPGG_MODE=${FPGG_REST%%\|*}
      FPGG_MODE_DEC=${FPGG_REST#*\|}
    fi
  fi
  ...
  if [ -n "$FPGG_REASON" ]; then
    ...
    if [ "$FPGG_GID" -eq 7 ]; then
      add security fileperm_group_group "Group database file group" PASS med \
          "$FPGG_GROUP_OBSERVED" \
          "/etc/group has the required group boundary." \
          "n/a" "cis-l1"
    else
