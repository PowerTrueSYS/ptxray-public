...
AIXRAY_TOOL=ck-fileperm-group-mode


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_group_mode — CIS Level 1 alignment for /etc/group.
  # Read the fixed path exactly once; uncertain evidence is never accepted as PASS.
  typeset FPG_RAW FPG_RC FPG_PARSED FPG_PARSE_RC FPG_REST
  typeset FPG_UID FPG_GID FPG_MODE FPG_MODE_DEC FPG_REASON
  typeset FPG_MODE_OBSERVED

  FPG_RAW=$(aix fileperm_group_mode_ls ls -ln /etc/group)
  FPG_RC=$?
  FPG_PARSED=""
  FPG_UID=""
  FPG_GID=""
  ...
  if [ "$FPG_RC" -ne 0 ]; then
    if [ "$FPG_RC" -eq 2 ]; then
      ...
      FPG_REASON="capture failed (rc=$FPG_RC)"
    fi
  elif [ -z "$FPG_RAW" ]; then
    FPG_REASON="empty"
  else
    ...
    ')
    FPG_PARSE_RC=$?
    if [ "$FPG_PARSE_RC" -ne 0 ] || [ -z "$FPG_PARSED" ]; then
      FPG_REASON="unparseable"
    else
      FPG_UID=${FPG_PARSED%%\|*}
      FPG_REST=${FPG_PARSED#*\|}
      FPG_GID=${FPG_REST%%\|*}
  ...
  if [ -n "$FPG_REASON" ]; then
    ...
    # Use decimal 420: ksh93u+ treats a leading zero as octal, while newer
    # ksh93u+m treats it as decimal in arithmetic expressions.
    if [ $((FPG_MODE_DEC & ~420)) -eq 0 ]; then
      add security fileperm_group_mode "Group database file mode" PASS med \
          "$FPG_MODE_OBSERVED" \
          "/etc/group has the required mode boundary." \
          "n/a" "cis-l1"
    else
