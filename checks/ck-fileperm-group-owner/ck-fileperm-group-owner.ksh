...
AIXRAY_TOOL=ck-fileperm-group-owner


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_group_owner — CIS Level 1 alignment for /etc/group.
  # Read the fixed path exactly once; uncertain evidence is never accepted as PASS.
  typeset FPGO_RAW FPGO_RC FPGO_PARSED FPGO_PARSE_RC FPGO_REST
  typeset FPGO_UID FPGO_GID FPGO_MODE FPGO_MODE_DEC FPGO_REASON
  typeset FPGO_OWNER_OBSERVED

  FPGO_RAW=$(aix fileperm_group_owner_ls ls -ln /etc/group)
  FPGO_RC=$?
  FPGO_PARSED=""
  FPGO_UID=""
  FPGO_GID=""
  ...
  if [ "$FPGO_RC" -ne 0 ]; then
    if [ "$FPGO_RC" -eq 2 ]; then
      FPGO_REASON="missing"
    else
      FPGO_REASON="capture failed (rc=$FPGO_RC)"
    fi
  elif [ -z "$FPGO_RAW" ]; then
    FPGO_REASON="empty"
  else
    FPGO_PARSED=$(printf '%s\n' "$FPGO_RAW" | awk '
      NF {
        records++
        if (records != 1 || NF < 9) {
          bad=1
    ...
    ')
    FPGO_PARSE_RC=$?
    if [ "$FPGO_PARSE_RC" -ne 0 ] || [ -z "$FPGO_PARSED" ]; then
      FPGO_REASON="unparseable"
    else
      FPGO_UID=${FPGO_PARSED%%\|*}
      FPGO_REST=${FPGO_PARSED#*\|}
      FPGO_GID=${FPGO_REST%%\|*}
      FPGO_REST=${FPGO_REST#*\|}
      FPGO_MODE=${FPGO_REST%%\|*}
      FPGO_MODE_DEC=${FPGO_REST#*\|}
    fi
  fi
  ...
  if [ -n "$FPGO_REASON" ]; then
    ...
    if [ "$FPGO_UID" -eq 0 ]; then
      add security fileperm_group_owner "Group database file owner" PASS med \
          "$FPGO_OWNER_OBSERVED" \
          "/etc/group has the required owner boundary." \
          "n/a" "cis-l1"
    else
