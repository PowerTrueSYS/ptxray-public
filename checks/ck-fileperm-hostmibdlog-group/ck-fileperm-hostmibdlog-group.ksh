...
AIXRAY_TOOL=ck-fileperm-hostmibdlog-group


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_hostmibdlog_group — CIS L1-aligned /var/tmp/hostmibd.log group boundary.
  ...
  typeset FHMG_RAW FHMG_RC FHMG_PARSED FHMG_PARSE_RC FHMG_GID FHMG_REASON FHMG_ABSENT FHMG_DIAG

  FHMG_RAW=$(aixv fileperm_hostmibdlog_group_ls ls -ln /var/tmp/hostmibd.log)
  FHMG_RC=$?
  FHMG_PARSED=""
  FHMG_GID=""
  FHMG_REASON=""
  ...
  if [ "$FHMG_RC" -ne 0 ]; then
    FHMG_REASON="absent or unreadable (rc=$FHMG_RC)"
    case "$FHMG_RAW" in
      *"/var/tmp/hostmibd.log not found"*) FHMG_ABSENT=1 ;;
    esac
    if [ "$FHMG_ABSENT" -eq 1 ]; then
      FHMG_REASON="absent"
    else
      FHMG_DIAG=$(printf '%s\n' "$FHMG_RAW" | awk 'NF { gsub(/[ \t]+/, " "); sub(/^ /, ""); sub(/ $/, ""); if (length("" $0) > 40) $0 = substr($0, 1, 37) "..."; print; exit }')
      if [ -n "$FHMG_DIAG" ]; then
        FHMG_REASON="absent or unreadable (rc=$FHMG_RC): $FHMG_DIAG"
      fi
    fi
  elif [ -z "$FHMG_RAW" ]; then
    FHMG_REASON="empty"
  else
    FHMG_PARSED=$(printf '%s\n' "$FHMG_RAW" | awk '
      NF {
        records++
    ...
    ')
    FHMG_PARSE_RC=$?
    if [ "$FHMG_PARSE_RC" -ne 0 ] || [ -z "$FHMG_PARSED" ]; then
      FHMG_REASON="unparseable"
    else
      FHMG_GID=$FHMG_PARSED
    fi
  fi

  if [ "$FHMG_REASON" = "absent" ]; then
    add security fileperm_hostmibdlog_group "/var/tmp/hostmibd.log group" NOT_APPLICABLE med \
        "/var/tmp/hostmibd.log absent" \
        "The probe returned the determinate-absence signature ('/var/tmp/hostmibd.log not found', exit $FHMG_RC); /var/tmp/hostmibd.log need not exist, so the group boundary does not apply." \
        "n/a" "cis-l1"
    ...
    add security fileperm_hostmibdlog_group "/var/tmp/hostmibd.log group" PASS med \
        ...
        "/var/tmp/hostmibd.log has the required group boundary." \
        "n/a" "cis-l1"
  else
