...
AIXRAY_TOOL=ck-fileperm-hostmibdlog-mode


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_hostmibdlog_mode — CIS L1-aligned /var/tmp/hostmibd.log mode boundary.
  ...
  typeset FHMM_RAW FHMM_RC FHMM_PARSED FHMM_PARSE_RC FHMM_MODE FHMM_OK FHMM_REASON FHMM_ABSENT FHMM_DIAG

  FHMM_RAW=$(aixv fileperm_hostmibdlog_mode_ls ls -ln /var/tmp/hostmibd.log)
  FHMM_RC=$?
  FHMM_PARSED=""
  FHMM_MODE=""
  FHMM_OK=""
  ...
  if [ "$FHMM_RC" -ne 0 ]; then
    FHMM_REASON="absent or unreadable (rc=$FHMM_RC)"
    case "$FHMM_RAW" in
      *"/var/tmp/hostmibd.log not found"*) FHMM_ABSENT=1 ;;
    esac
    if [ "$FHMM_ABSENT" -eq 1 ]; then
      FHMM_REASON="absent"
    else
      FHMM_DIAG=$(printf '%s\n' "$FHMM_RAW" | awk 'NF { gsub(/[ \t]+/, " "); sub(/^ /, ""); sub(/ $/, ""); if (length("" $0) > 40) $0 = substr($0, 1, 37) "..."; print; exit }')
      if [ -n "$FHMM_DIAG" ]; then
        FHMM_REASON="absent or unreadable (rc=$FHMM_RC): $FHMM_DIAG"
      fi
    fi
  elif [ -z "$FHMM_RAW" ]; then
    FHMM_REASON="empty"
  else
    FHMM_PARSED=$(printf '%s\n' "$FHMM_RAW" | awk '
      NF {
        records++
        ...
        if (!date_ok) bad=1
        if ($NF != "/var/tmp/hostmibd.log") bad=1
      }
      END {
    ...
    ')
    FHMM_PARSE_RC=$?
    if [ "$FHMM_PARSE_RC" -ne 0 ] || [ -z "$FHMM_PARSED" ]; then
      FHMM_REASON="unparseable"
    else
      FHMM_MODE=${FHMM_PARSED%%\|*}
      FHMM_OK=${FHMM_PARSED#*\|}
    fi
  fi

  if [ "$FHMM_REASON" = "absent" ]; then
    add security fileperm_hostmibdlog_mode "/var/tmp/hostmibd.log mode" NOT_APPLICABLE med \
        "/var/tmp/hostmibd.log absent" \
        "The probe returned the determinate-absence signature ('/var/tmp/hostmibd.log not found', exit $FHMM_RC); /var/tmp/hostmibd.log need not exist, so the mode boundary does not apply." \
        "n/a" "cis-l1"
    ...
    add security fileperm_hostmibdlog_mode "/var/tmp/hostmibd.log mode" PASS med \
        "/var/tmp/hostmibd.log mode=$FHMM_MODE" \
        "/var/tmp/hostmibd.log has the required mode boundary." \
        "n/a" "cis-l1"
  else
