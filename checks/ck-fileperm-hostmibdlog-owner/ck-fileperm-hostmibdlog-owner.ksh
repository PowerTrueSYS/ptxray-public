...
AIXRAY_TOOL=ck-fileperm-hostmibdlog-owner


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_hostmibdlog_owner — CIS L1-aligned /var/tmp/hostmibd.log owner boundary.
  ...
  typeset FHMO_RAW FHMO_RC FHMO_PARSED FHMO_PARSE_RC FHMO_UID FHMO_REASON FHMO_ABSENT FHMO_DIAG

  FHMO_RAW=$(aixv fileperm_hostmibdlog_owner_ls ls -ln /var/tmp/hostmibd.log)
  FHMO_RC=$?
  FHMO_PARSED=""
  FHMO_UID=""
  FHMO_REASON=""
  ...
  if [ "$FHMO_RC" -ne 0 ]; then
    FHMO_REASON="absent or unreadable (rc=$FHMO_RC)"
    case "$FHMO_RAW" in
      *"/var/tmp/hostmibd.log not found"*) FHMO_ABSENT=1 ;;
    esac
    if [ "$FHMO_ABSENT" -eq 1 ]; then
      FHMO_REASON="absent"
    else
      FHMO_DIAG=$(printf '%s\n' "$FHMO_RAW" | awk 'NF { gsub(/[ \t]+/, " "); sub(/^ /, ""); sub(/ $/, ""); if (length("" $0) > 40) $0 = substr($0, 1, 37) "..."; print; exit }')
      if [ -n "$FHMO_DIAG" ]; then
        FHMO_REASON="absent or unreadable (rc=$FHMO_RC): $FHMO_DIAG"
      fi
    fi
  elif [ -z "$FHMO_RAW" ]; then
    FHMO_REASON="empty"
  else
    FHMO_PARSED=$(printf '%s\n' "$FHMO_RAW" | awk '
      NF {
        records++
    ...
    ')
    FHMO_PARSE_RC=$?
    if [ "$FHMO_PARSE_RC" -ne 0 ] || [ -z "$FHMO_PARSED" ]; then
      FHMO_REASON="unparseable"
    else
      FHMO_UID=$FHMO_PARSED
    fi
  fi

  if [ "$FHMO_REASON" = "absent" ]; then
    add security fileperm_hostmibdlog_owner "/var/tmp/hostmibd.log owner" NOT_APPLICABLE med \
        "/var/tmp/hostmibd.log absent" \
        "The probe returned the determinate-absence signature ('/var/tmp/hostmibd.log not found', exit $FHMO_RC); /var/tmp/hostmibd.log need not exist, so the owner boundary does not apply." \
        "n/a" "cis-l1"
    ...
    add security fileperm_hostmibdlog_owner "/var/tmp/hostmibd.log owner" PASS med \
        ...
        "/var/tmp/hostmibd.log has the required owner boundary." \
        "n/a" "cis-l1"
  else
