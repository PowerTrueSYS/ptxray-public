...
AIXRAY_TOOL=ck-fileperm-hosts-group


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_hosts_group — CIS Level 1 alignment for /etc/hosts.
  # Read the fixed path exactly once; uncertain evidence is never accepted as PASS.
  typeset FPHG_RAW FPHG_RC FPHG_PARSED FPHG_PARSE_RC FPHG_REST
  typeset FPHG_UID FPHG_GID FPHG_MODE FPHG_MODE_DEC FPHG_REASON
  typeset FPHG_GROUP_OBSERVED

  FPHG_RAW=$(aix fileperm_hosts_group_ls ls -ln /etc/hosts)
  FPHG_RC=$?
  FPHG_PARSED=""
  FPHG_UID=""
  FPHG_GID=""
  ...
  if [ "$FPHG_RC" -ne 0 ]; then
    if [ "$FPHG_RC" -eq 2 ]; then
      FPHG_REASON="missing"
    else
      FPHG_REASON="capture failed (rc=$FPHG_RC)"
    fi
  elif [ -z "$FPHG_RAW" ]; then
    FPHG_REASON="empty"
  else
    FPHG_PARSED=$(printf '%s\n' "$FPHG_RAW" | awk '
      NF {
        records++
        if (records != 1 || NF < 9) {
          bad=1
    ...
    ')
    FPHG_PARSE_RC=$?
    if [ "$FPHG_PARSE_RC" -ne 0 ] || [ -z "$FPHG_PARSED" ]; then
      FPHG_REASON="unparseable"
    else
      FPHG_UID=${FPHG_PARSED%%\|*}
      FPHG_REST=${FPHG_PARSED#*\|}
      FPHG_GID=${FPHG_REST%%\|*}
      FPHG_REST=${FPHG_REST#*\|}
      FPHG_MODE=${FPHG_REST%%\|*}
      FPHG_MODE_DEC=${FPHG_REST#*\|}
    fi
  fi
  ...
  if [ -n "$FPHG_REASON" ]; then
    ...
    if [ "$FPHG_GID" -eq 0 ]; then
      add security fileperm_hosts_group "Host name database file group" PASS med \
          "$FPHG_GROUP_OBSERVED" \
          "/etc/hosts has the required group boundary." \
          "n/a" "cis-l1"
    else
