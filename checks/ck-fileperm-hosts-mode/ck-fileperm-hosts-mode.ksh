...
AIXRAY_TOOL=ck-fileperm-hosts-mode


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_hosts_mode — CIS Level 1 alignment for /etc/hosts.
  # Read the fixed path exactly once; uncertain evidence is never accepted as PASS.
  typeset FPH_RAW FPH_RC FPH_PARSED FPH_PARSE_RC FPH_REST
  typeset FPH_UID FPH_GID FPH_MODE FPH_MODE_DEC FPH_REASON
  typeset FPH_MODE_OBSERVED

  FPH_RAW=$(aix fileperm_hosts_mode_ls ls -ln /etc/hosts)
  FPH_RC=$?
  FPH_PARSED=""
  FPH_UID=""
  FPH_GID=""
  ...
  if [ "$FPH_RC" -ne 0 ]; then
    if [ "$FPH_RC" -eq 2 ]; then
      ...
      FPH_REASON="capture failed (rc=$FPH_RC)"
    fi
  elif [ -z "$FPH_RAW" ]; then
    FPH_REASON="empty"
  else
    ...
    ')
    FPH_PARSE_RC=$?
    if [ "$FPH_PARSE_RC" -ne 0 ] || [ -z "$FPH_PARSED" ]; then
      FPH_REASON="unparseable"
    else
      FPH_UID=${FPH_PARSED%%\|*}
      FPH_REST=${FPH_PARSED#*\|}
      FPH_GID=${FPH_REST%%\|*}
  ...
  if [ -n "$FPH_REASON" ]; then
    ...
    if [ $((FPH_MODE_DEC & ~416)) -eq 0 ]; then
      add security fileperm_hosts_mode "Host name database file mode" PASS med \
          "$FPH_MODE_OBSERVED" \
          "/etc/hosts has the required mode boundary." \
          "n/a" "cis-l1"
    else
