...
AIXRAY_TOOL=ck-fileperm-hosts


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_hosts_owner — DISA STIG V-245557.
  # Read the fixed path exactly once; every uncertain evidence shape fails closed.
  typeset FPH_RAW FPH_RC FPH_PARSED FPH_PARSE_RC FPH_UID FPH_GID
  typeset FPH_REASON FPH_OBSERVED

  FPH_RAW=$(aix fileperm_hosts_ls ls -ldn /etc/hosts)
  FPH_RC=$?
  FPH_PARSED=""
  FPH_UID=""
  FPH_GID=""
  ...
  if [ -n "$FPH_REASON" ]; then
    ...
    add security fileperm_hosts_owner "Host name database file owner" PASS med \
        "/etc/hosts owner=0 (root)" \
        "/etc/hosts is owned by root as required by DISA STIG V-245557." \
        "n/a" "stig:V-245557"
  else
    FPH_OBSERVED="/etc/hosts owner=$FPH_UID"
