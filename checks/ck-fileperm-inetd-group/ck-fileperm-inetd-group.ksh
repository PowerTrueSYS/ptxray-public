...
AIXRAY_TOOL=ck-fileperm-inetd-group


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_inetd_group — CIS L1-aligned /etc/inetd.conf group boundary.
  typeset FIG_RAW FIG_RC FIG_PARSED FIG_PARSE_RC FIG_GID FIG_REASON

  FIG_RAW=$(aix fileperm_inetd_group_ls ls -ln /etc/inetd.conf)
  FIG_RC=$?
  FIG_PARSED=""
  FIG_GID=""
  FIG_REASON=""
  ...
  if [ -n "$FIG_REASON" ]; then
    ...
    add security fileperm_inetd_group "/etc/inetd.conf group" PASS med \
        ...
        "/etc/inetd.conf has the required group boundary." \
        "n/a" "cis-l1"
  else
