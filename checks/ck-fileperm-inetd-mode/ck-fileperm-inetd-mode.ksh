...
AIXRAY_TOOL=ck-fileperm-inetd-mode


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_inetd_mode — CIS L1-aligned /etc/inetd.conf mode boundary.
  typeset FIM_RAW FIM_RC FIM_PARSED FIM_PARSE_RC FIM_MODE FIM_OK FIM_REASON

  FIM_RAW=$(aix fileperm_inetd_mode_ls ls -ln /etc/inetd.conf)
  FIM_RC=$?
  FIM_PARSED=""
  FIM_MODE=""
  FIM_OK=""
  ...
  if [ -n "$FIM_REASON" ]; then
    ...
    add security fileperm_inetd_mode "/etc/inetd.conf mode" PASS med \
        "/etc/inetd.conf mode=$FIM_MODE" \
        "/etc/inetd.conf has the required mode boundary." \
        "n/a" "cis-l1"
  else
