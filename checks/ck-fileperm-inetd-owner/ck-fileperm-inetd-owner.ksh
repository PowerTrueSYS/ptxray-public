...
AIXRAY_TOOL=ck-fileperm-inetd-owner


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_inetd_owner — CIS L1-aligned /etc/inetd.conf owner boundary.
  typeset FIO_RAW FIO_RC FIO_PARSED FIO_PARSE_RC FIO_UID FIO_REASON

  FIO_RAW=$(aix fileperm_inetd_owner_ls ls -ln /etc/inetd.conf)
  FIO_RC=$?
  FIO_PARSED=""
  FIO_UID=""
  FIO_REASON=""
  ...
  if [ -n "$FIO_REASON" ]; then
    ...
    add security fileperm_inetd_owner "/etc/inetd.conf owner" PASS med \
        ...
        "/etc/inetd.conf has the required owner boundary." \
        "n/a" "cis-l1"
  else
