...
AIXRAY_TOOL=ck-fileperm-motd-group


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_motd_group — CIS L1-aligned /etc/motd group boundary.
  ...
  typeset FMOG_RAW FMOG_RC FMOG_PARSED FMOG_PARSE_RC FMOG_GID FMOG_REASON FMOG_ABSENT FMOG_DIAG

  FMOG_RAW=$(aixv fileperm_motd_group_ls ls -ln /etc/motd)
  FMOG_RC=$?
  FMOG_PARSED=""
  FMOG_GID=""
  FMOG_REASON=""
  ...
  if [ "$FMOG_RC" -ne 0 ]; then
    FMOG_REASON="absent or unreadable (rc=$FMOG_RC)"
    case "$FMOG_RAW" in
      *"/etc/motd not found"*) FMOG_ABSENT=1 ;;
    esac
    if [ "$FMOG_ABSENT" -eq 1 ]; then
      FMOG_REASON="absent"
    else
      FMOG_DIAG=$(printf '%s\n' "$FMOG_RAW" | awk 'NF { gsub(/[ \t]+/, " "); sub(/^ /, ""); sub(/ $/, ""); if (length("" $0) > 40) $0 = substr($0, 1, 37) "..."; print; exit }')
      if [ -n "$FMOG_DIAG" ]; then
        FMOG_REASON="absent or unreadable (rc=$FMOG_RC): $FMOG_DIAG"
      fi
    fi
  elif [ -z "$FMOG_RAW" ]; then
    FMOG_REASON="empty"
  else
    FMOG_PARSED=$(printf '%s\n' "$FMOG_RAW" | awk '
      NF {
        records++
    ...
    ')
    FMOG_PARSE_RC=$?
    if [ "$FMOG_PARSE_RC" -ne 0 ] || [ -z "$FMOG_PARSED" ]; then
      FMOG_REASON="unparseable"
    else
      FMOG_GID=$FMOG_PARSED
    fi
  fi

  if [ "$FMOG_REASON" = "absent" ]; then
    add security fileperm_motd_group "/etc/motd group" NOT_APPLICABLE med \
        "/etc/motd absent" \
        "The probe returned the determinate-absence signature ('/etc/motd not found', exit $FMOG_RC); /etc/motd need not exist, so the group boundary does not apply." \
        "n/a" "cis-l1"
    ...
    add security fileperm_motd_group "/etc/motd group" PASS med \
        ...
        "/etc/motd has the required group boundary." \
        "n/a" "cis-l1"
  else
