...
AIXRAY_TOOL=ck-fileperm-motd-mode


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_motd_mode — CIS L1-aligned /etc/motd mode boundary.
  ...
  typeset FMOM_RAW FMOM_RC FMOM_PARSED FMOM_PARSE_RC FMOM_MODE FMOM_OK FMOM_REASON FMOM_ABSENT FMOM_DIAG

  FMOM_RAW=$(aixv fileperm_motd_mode_ls ls -ln /etc/motd)
  FMOM_RC=$?
  FMOM_PARSED=""
  FMOM_MODE=""
  FMOM_OK=""
  ...
  if [ "$FMOM_RC" -ne 0 ]; then
    FMOM_REASON="absent or unreadable (rc=$FMOM_RC)"
    case "$FMOM_RAW" in
      *"/etc/motd not found"*) FMOM_ABSENT=1 ;;
    esac
    if [ "$FMOM_ABSENT" -eq 1 ]; then
      FMOM_REASON="absent"
    else
      FMOM_DIAG=$(printf '%s\n' "$FMOM_RAW" | awk 'NF { gsub(/[ \t]+/, " "); sub(/^ /, ""); sub(/ $/, ""); if (length("" $0) > 40) $0 = substr($0, 1, 37) "..."; print; exit }')
      if [ -n "$FMOM_DIAG" ]; then
        FMOM_REASON="absent or unreadable (rc=$FMOM_RC): $FMOM_DIAG"
      fi
    fi
  elif [ -z "$FMOM_RAW" ]; then
    FMOM_REASON="empty"
  else
    FMOM_PARSED=$(printf '%s\n' "$FMOM_RAW" | awk '
      NF {
        records++
    ...
    ')
    FMOM_PARSE_RC=$?
    if [ "$FMOM_PARSE_RC" -ne 0 ] || [ -z "$FMOM_PARSED" ]; then
      FMOM_REASON="unparseable"
    else
      FMOM_MODE=${FMOM_PARSED%%\|*}
      FMOM_OK=${FMOM_PARSED#*\|}
    fi
  fi

  if [ "$FMOM_REASON" = "absent" ]; then
    add security fileperm_motd_mode "/etc/motd mode" NOT_APPLICABLE med \
        "/etc/motd absent" \
        "The probe returned the determinate-absence signature ('/etc/motd not found', exit $FMOM_RC); /etc/motd need not exist, so the mode boundary does not apply." \
        "n/a" "cis-l1"
    ...
    add security fileperm_motd_mode "/etc/motd mode" PASS med \
        ...
        "/etc/motd has the required mode boundary." \
        "n/a" "cis-l1"
  else
