...
AIXRAY_TOOL=ck-fileperm-motd-owner


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_motd_owner — CIS L1-aligned /etc/motd owner boundary.
  ...
  typeset FMOO_RAW FMOO_RC FMOO_PARSED FMOO_PARSE_RC FMOO_UID FMOO_REASON FMOO_ABSENT FMOO_DIAG

  FMOO_RAW=$(aixv fileperm_motd_owner_ls ls -ln /etc/motd)
  FMOO_RC=$?
  FMOO_PARSED=""
  FMOO_UID=""
  FMOO_REASON=""
  ...
  if [ "$FMOO_RC" -ne 0 ]; then
    FMOO_REASON="absent or unreadable (rc=$FMOO_RC)"
    case "$FMOO_RAW" in
      *"/etc/motd not found"*) FMOO_ABSENT=1 ;;
    esac
    if [ "$FMOO_ABSENT" -eq 1 ]; then
      FMOO_REASON="absent"
    else
      FMOO_DIAG=$(printf '%s\n' "$FMOO_RAW" | awk 'NF { gsub(/[ \t]+/, " "); sub(/^ /, ""); sub(/ $/, ""); if (length("" $0) > 40) $0 = substr($0, 1, 37) "..."; print; exit }')
      if [ -n "$FMOO_DIAG" ]; then
        FMOO_REASON="absent or unreadable (rc=$FMOO_RC): $FMOO_DIAG"
      fi
    fi
  elif [ -z "$FMOO_RAW" ]; then
    FMOO_REASON="empty"
  else
    FMOO_PARSED=$(printf '%s\n' "$FMOO_RAW" | awk '
      NF {
        records++
    ...
    ')
    FMOO_PARSE_RC=$?
    if [ "$FMOO_PARSE_RC" -ne 0 ] || [ -z "$FMOO_PARSED" ]; then
      FMOO_REASON="unparseable"
    else
      FMOO_UID=$FMOO_PARSED
    fi
  fi

  if [ "$FMOO_REASON" = "absent" ]; then
    add security fileperm_motd_owner "/etc/motd owner" NOT_APPLICABLE med \
        "/etc/motd absent" \
        "The probe returned the determinate-absence signature ('/etc/motd not found', exit $FMOO_RC); /etc/motd need not exist, so the owner boundary does not apply." \
        "n/a" "cis-l1"
    ...
    add security fileperm_motd_owner "/etc/motd owner" PASS med \
        ...
        "/etc/motd has the required owner boundary." \
        "n/a" "cis-l1"
  else
