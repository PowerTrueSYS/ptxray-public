...
AIXRAY_TOOL=ck-fileperm-ntp-group


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_ntp_group — CIS L1-aligned /etc/ntp.conf group boundary.
  typeset FIG_RAW FIG_RC FIG_PARSED FIG_PARSE_RC FIG_GID FIG_REASON

  FIG_RAW=$(aix fileperm_ntp_group_ls ls -ln /etc/ntp.conf)
  FIG_RC=$?
  FIG_PARSED=""
  FIG_GID=""
  FIG_REASON=""
  ...
  if [ -n "$FIG_REASON" ]; then
    ...
    add security fileperm_ntp_group "/etc/ntp.conf group" PASS med \
        ...
        "/etc/ntp.conf has the required group boundary." \
        "n/a" ""
  else
