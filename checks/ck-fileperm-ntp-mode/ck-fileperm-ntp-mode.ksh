...
AIXRAY_TOOL=ck-fileperm-ntp-mode


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_ntp_mode — CIS L1-aligned /etc/ntp.conf mode boundary.
  typeset FNM_RAW FNM_RC FNM_PARSED FNM_PARSE_RC FNM_MODE FNM_OK FNM_REASON

  FNM_RAW=$(aix fileperm_ntp_mode_ls ls -ln /etc/ntp.conf)
  FNM_RC=$?
  FNM_PARSED=""
  FNM_MODE=""
  FNM_OK=""
  ...
  if [ "$FNM_RC" -ne 0 ]; then
    FNM_REASON="capture failed (rc=$FNM_RC)"
  elif [ -z "$FNM_RAW" ]; then
    FNM_REASON="empty"
  else
    FNM_PARSED=$(printf '%s\n' "$FNM_RAW" | awk '
    ...
    ')
    FNM_PARSE_RC=$?
    if [ "$FNM_PARSE_RC" -ne 0 ] || [ -z "$FNM_PARSED" ]; then
      FNM_REASON="unparseable"
    else
      FNM_MODE=${FNM_PARSED%%\|*}
      FNM_OK=${FNM_PARSED#*\|}
    fi
  ...
  if [ -n "$FNM_REASON" ]; then
    ...
    add security fileperm_ntp_mode "/etc/ntp.conf mode" PASS med \
        ...
        "/etc/ntp.conf has the required mode boundary." \
        "n/a" ""
  else
