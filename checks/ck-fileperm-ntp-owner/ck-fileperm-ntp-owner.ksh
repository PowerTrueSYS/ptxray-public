...
AIXRAY_TOOL=ck-fileperm-ntp-owner


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_ntp_owner — CIS L1-aligned /etc/ntp.conf owner boundary.
  typeset FNO_RAW FNO_RC FNO_PARSED FNO_PARSE_RC FNO_UID FNO_REASON

  FNO_RAW=$(aix fileperm_ntp_owner_ls ls -ln /etc/ntp.conf)
  FNO_RC=$?
  FNO_PARSED=""
  FNO_UID=""
  FNO_REASON=""
  ...
  if [ "$FNO_RC" -ne 0 ]; then
    FNO_REASON="capture failed (rc=$FNO_RC)"
  elif [ -z "$FNO_RAW" ]; then
    FNO_REASON="empty"
  else
    FNO_PARSED=$(printf '%s\n' "$FNO_RAW" | awk '
    ...
    ')
    FNO_PARSE_RC=$?
    if [ "$FNO_PARSE_RC" -ne 0 ] || [ -z "$FNO_PARSED" ]; then
      FNO_REASON="unparseable"
    else
      FNO_UID=$FNO_PARSED
    fi
  fi
  ...
  if [ -n "$FNO_REASON" ]; then
    ...
    add security fileperm_ntp_owner "/etc/ntp.conf owner" PASS med \
        ...
        "/etc/ntp.conf has the required owner boundary." \
        "n/a" ""
  else
