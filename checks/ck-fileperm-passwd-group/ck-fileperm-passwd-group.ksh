...
AIXRAY_TOOL=ck-fileperm-passwd-group


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_passwd_group — CIS L1-aligned /etc/passwd group boundary.
  typeset FIG_RAW FIG_RC FIG_PARSED FIG_PARSE_RC FIG_GID FIG_REASON

  FIG_RAW=$(aix fileperm_passwd_group_ls ls -ln /etc/passwd)
  FIG_RC=$?
  FIG_PARSED=""
  FIG_GID=""
  FIG_REASON=""
  ...
  if [ -n "$FIG_REASON" ]; then
    ...
    add security fileperm_passwd_group "/etc/passwd group" PASS med \
        ...
        "/etc/passwd has the required group boundary." \
        "n/a" "cis-l1"
  else
