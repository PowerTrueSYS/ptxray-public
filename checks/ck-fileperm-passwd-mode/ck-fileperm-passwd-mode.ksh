...
AIXRAY_TOOL=ck-fileperm-passwd-mode


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_passwd_mode — CIS L1-aligned /etc/passwd mode boundary.
  typeset FIM_RAW FIM_RC FIM_PARSED FIM_PARSE_RC FIM_MODE FIM_OK FIM_REASON

  FIM_RAW=$(aix fileperm_passwd_mode_ls ls -ln /etc/passwd)
  FIM_RC=$?
  FIM_PARSED=""
  FIM_MODE=""
  FIM_OK=""
  ...
  if [ -n "$FIM_REASON" ]; then
    ...
    add security fileperm_passwd_mode "/etc/passwd mode" PASS med \
        ...
        "/etc/passwd has the required mode boundary." \
        "n/a" "cis-l1"
  else
