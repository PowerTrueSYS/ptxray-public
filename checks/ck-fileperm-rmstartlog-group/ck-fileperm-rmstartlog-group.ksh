...
AIXRAY_TOOL=ck-fileperm-rmstartlog-group


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_rmstartlog_group — CIS L1-aligned /var/ct/RMstart.log group boundary.
  ...
  typeset FRMG_RAW FRMG_RC FRMG_PARSED FRMG_PARSE_RC FRMG_GID FRMG_REASON FRMG_ABSENT FRMG_DIAG

  FRMG_RAW=$(aixv fileperm_rmstartlog_group_ls ls -ln /var/ct/RMstart.log)
  FRMG_RC=$?
  FRMG_PARSED=""
  FRMG_GID=""
  FRMG_REASON=""
  ...
  if [ "$FRMG_RC" -ne 0 ]; then
    FRMG_REASON="absent or unreadable (rc=$FRMG_RC)"
    case "$FRMG_RAW" in
      *"/var/ct/RMstart.log not found"*) FRMG_ABSENT=1 ;;
    esac
    if [ "$FRMG_ABSENT" -eq 1 ]; then
      FRMG_REASON="absent"
    else
      FRMG_DIAG=$(printf '%s\n' "$FRMG_RAW" | awk 'NF { gsub(/[ \t]+/, " "); sub(/^ /, ""); sub(/ $/, ""); if (length("" $0) > 40) $0 = substr($0, 1, 37) "..."; print; exit }')
      if [ -n "$FRMG_DIAG" ]; then
        FRMG_REASON="absent or unreadable (rc=$FRMG_RC): $FRMG_DIAG"
      fi
    fi
  elif [ -z "$FRMG_RAW" ]; then
    FRMG_REASON="empty"
  else
    FRMG_PARSED=$(printf '%s\n' "$FRMG_RAW" | awk '
      NF {
        records++
    ...
    ')
    FRMG_PARSE_RC=$?
    if [ "$FRMG_PARSE_RC" -ne 0 ] || [ -z "$FRMG_PARSED" ]; then
      FRMG_REASON="unparseable"
    else
      FRMG_GID=$FRMG_PARSED
    fi
  fi

  if [ "$FRMG_REASON" = "absent" ]; then
    add security fileperm_rmstartlog_group "/var/ct/RMstart.log group" NOT_APPLICABLE med \
        "/var/ct/RMstart.log absent" \
        "The probe returned the determinate-absence signature ('/var/ct/RMstart.log not found', exit $FRMG_RC); /var/ct/RMstart.log need not exist, so the group boundary does not apply." \
        "n/a" "cis-l1"
    ...
    add security fileperm_rmstartlog_group "/var/ct/RMstart.log group" PASS med \
        ...
        "/var/ct/RMstart.log has the required group boundary." \
        "n/a" "cis-l1"
  else
