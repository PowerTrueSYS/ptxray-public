...
AIXRAY_TOOL=ck-fileperm-rmstartlog-mode


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_rmstartlog_mode — CIS L1-aligned /var/ct/RMstart.log mode boundary.
  ...
  typeset FRMM_RAW FRMM_RC FRMM_PARSED FRMM_PARSE_RC FRMM_MODE FRMM_OK FRMM_REASON FRMM_ABSENT FRMM_DIAG

  FRMM_RAW=$(aixv fileperm_rmstartlog_mode_ls ls -ln /var/ct/RMstart.log)
  FRMM_RC=$?
  FRMM_PARSED=""
  FRMM_MODE=""
  FRMM_OK=""
  ...
  if [ "$FRMM_RC" -ne 0 ]; then
    FRMM_REASON="absent or unreadable (rc=$FRMM_RC)"
    case "$FRMM_RAW" in
      *"/var/ct/RMstart.log not found"*) FRMM_ABSENT=1 ;;
    esac
    if [ "$FRMM_ABSENT" -eq 1 ]; then
      FRMM_REASON="absent"
    else
      FRMM_DIAG=$(printf '%s\n' "$FRMM_RAW" | awk 'NF { gsub(/[ \t]+/, " "); sub(/^ /, ""); sub(/ $/, ""); if (length("" $0) > 40) $0 = substr($0, 1, 37) "..."; print; exit }')
      if [ -n "$FRMM_DIAG" ]; then
        FRMM_REASON="absent or unreadable (rc=$FRMM_RC): $FRMM_DIAG"
      fi
    fi
  elif [ -z "$FRMM_RAW" ]; then
    FRMM_REASON="empty"
  else
    FRMM_PARSED=$(printf '%s\n' "$FRMM_RAW" | awk '
      NF {
        records++
        ...
        if (!date_ok) bad=1
        if ($NF != "/var/ct/RMstart.log") bad=1
      }
      END {
    ...
    ')
    FRMM_PARSE_RC=$?
    if [ "$FRMM_PARSE_RC" -ne 0 ] || [ -z "$FRMM_PARSED" ]; then
      FRMM_REASON="unparseable"
    else
      FRMM_MODE=${FRMM_PARSED%%\|*}
      FRMM_OK=${FRMM_PARSED#*\|}
    fi
  fi

  if [ "$FRMM_REASON" = "absent" ]; then
    add security fileperm_rmstartlog_mode "/var/ct/RMstart.log mode" NOT_APPLICABLE med \
        "/var/ct/RMstart.log absent" \
        "The probe returned the determinate-absence signature ('/var/ct/RMstart.log not found', exit $FRMM_RC); /var/ct/RMstart.log need not exist, so the mode boundary does not apply." \
        "n/a" "cis-l1"
    ...
    add security fileperm_rmstartlog_mode "/var/ct/RMstart.log mode" PASS med \
        ...
        "/var/ct/RMstart.log has the required mode boundary." \
        "n/a" "cis-l1"
  else
