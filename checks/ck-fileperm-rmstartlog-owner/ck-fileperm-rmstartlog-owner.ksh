...
AIXRAY_TOOL=ck-fileperm-rmstartlog-owner


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_rmstartlog_owner — CIS L1-aligned /var/ct/RMstart.log owner boundary.
  ...
  typeset FRMO_RAW FRMO_RC FRMO_PARSED FRMO_PARSE_RC FRMO_UID FRMO_REASON FRMO_ABSENT FRMO_DIAG

  FRMO_RAW=$(aixv fileperm_rmstartlog_owner_ls ls -ln /var/ct/RMstart.log)
  FRMO_RC=$?
  FRMO_PARSED=""
  FRMO_UID=""
  FRMO_REASON=""
  ...
  if [ "$FRMO_RC" -ne 0 ]; then
    FRMO_REASON="absent or unreadable (rc=$FRMO_RC)"
    case "$FRMO_RAW" in
      *"/var/ct/RMstart.log not found"*) FRMO_ABSENT=1 ;;
    esac
    if [ "$FRMO_ABSENT" -eq 1 ]; then
      FRMO_REASON="absent"
    else
      FRMO_DIAG=$(printf '%s\n' "$FRMO_RAW" | awk 'NF { gsub(/[ \t]+/, " "); sub(/^ /, ""); sub(/ $/, ""); if (length("" $0) > 40) $0 = substr($0, 1, 37) "..."; print; exit }')
      if [ -n "$FRMO_DIAG" ]; then
        FRMO_REASON="absent or unreadable (rc=$FRMO_RC): $FRMO_DIAG"
      fi
    fi
  elif [ -z "$FRMO_RAW" ]; then
    FRMO_REASON="empty"
  else
    FRMO_PARSED=$(printf '%s\n' "$FRMO_RAW" | awk '
      NF {
        records++
    ...
    ')
    FRMO_PARSE_RC=$?
    if [ "$FRMO_PARSE_RC" -ne 0 ] || [ -z "$FRMO_PARSED" ]; then
      FRMO_REASON="unparseable"
    else
      FRMO_UID=$FRMO_PARSED
    fi
  fi

  if [ "$FRMO_REASON" = "absent" ]; then
    add security fileperm_rmstartlog_owner "/var/ct/RMstart.log owner" NOT_APPLICABLE med \
        "/var/ct/RMstart.log absent" \
        "The probe returned the determinate-absence signature ('/var/ct/RMstart.log not found', exit $FRMO_RC); /var/ct/RMstart.log need not exist, so the owner boundary does not apply." \
        "n/a" "cis-l1"
    ...
    add security fileperm_rmstartlog_owner "/var/ct/RMstart.log owner" PASS med \
        ...
        "/var/ct/RMstart.log has the required owner boundary." \
        "n/a" "cis-l1"
  else
