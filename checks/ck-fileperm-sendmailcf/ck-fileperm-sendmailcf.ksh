...
AIXRAY_TOOL=ck-fileperm-sendmailcf
...
_AIXRAY_SESSION_KEYS=""
  ...
  # owner in $3, group in $4, size in $5, path in $NF.
  # It runs through aixv (stderr merged), not aix, because absence is only
  # visible on stderr: AIX ls writes "<path> not found" there and exits 2.
  # aix() discards stderr, so absent and unreadable both arrived as rc=2 with
  # empty stdout and were indistinguishable. Absence is keyed POSITIVELY on
  # that signature; any other nonzero exit still refuses (fail closed).
  typeset FSC_RAW FSC_RC FSC_PARSED FSC_PARSE_RC FSC_MODE FSC_OWNER FSC_GROUP FSC_REASON
  typeset FSC_REST FSC_OBSERVED FSC_ABSENT FSC_DIAG FSC_BAD

  FSC_RAW=$(aixv sendmailcf_ls ls -l /etc/mail/sendmail.cf) # network-lint: allow -- aixv() runs the LOCAL ls on /etc/mail/sendmail.cf metadata; ls makes no network call
  FSC_RC=$?
  FSC_PARSED=""
  FSC_MODE=""
  FSC_OWNER=""
  ...
  if [ "$FSC_RC" -ne 0 ]; then
    FSC_REASON="capture failed (rc=$FSC_RC)"
    case "$FSC_RAW" in
      *"/etc/mail/sendmail.cf not found"*) FSC_ABSENT=1 ;; # network-lint: allow -- absence signature is a local pathname, not a network call
    esac
    if [ "$FSC_ABSENT" -eq 1 ]; then
      FSC_REASON="absent"
    else
      FSC_DIAG=$(printf '%s\n' "$FSC_RAW" | awk 'NF { gsub(/[ \t]+/, " "); sub(/^ /, ""); sub(/ $/, ""); if (length("" $0) > 40) $0 = substr($0, 1, 37) "..."; print; exit }')
      if [ -n "$FSC_DIAG" ]; then
        FSC_REASON="capture failed (rc=$FSC_RC): $FSC_DIAG"
      fi
    fi
  elif [ -z "$FSC_RAW" ]; then
    FSC_REASON="empty"
  else
    # network-lint: allow-next=21 -- the awk program below compares the captured ls pathname to the mailer config file; it parses local capture text only, no network call
    FSC_PARSED=$(printf '%s\n' "$FSC_RAW" | awk '
      NF {
        records++
        ...
        if (substr(mode,1,1) !~ /^[-dlbcps]$/) bad=1
        ...
        if (length("" $4) < 1) bad=1
        if ($5 !~ /^[0-9][0-9]*$/) bad=1
        if ($NF != "/etc/mail/sendmail.cf") bad=1
      }
      END {
    ...
    ')
    FSC_PARSE_RC=$?
    if [ "$FSC_PARSE_RC" -ne 0 ] || [ -z "$FSC_PARSED" ]; then
      FSC_REASON="unparseable"
    else
      FSC_MODE=${FSC_PARSED%%\|*}
      FSC_REST=${FSC_PARSED#*\|}
      FSC_OWNER=${FSC_REST%%\|*}
  ...
  if [ "$FSC_REASON" = "absent" ]; then
    # network-lint: allow-next=3 -- NOT_APPLICABLE finding label and prose name the mailer config file; the add command emits a finding, no network call
    add security fileperm_sendmailcf "sendmail.cf owner/group/mode" NOT_APPLICABLE med \
        "/etc/mail/sendmail.cf absent" \
        "The probe returned the determinate-absence signature ('/etc/mail/sendmail.cf not found', exit $FSC_RC); sendmail need not be installed and the owner/group/mode requirement does not apply." \
        "n/a" "cis-l1"
  elif [ -n "$FSC_REASON" ]; then
    ...
    if [ "$FSC_MODE" = "-rw-r-----" ] && [ "$FSC_OWNER" = "root" ] && [ "$FSC_GROUP" = "system" ]; then
      # network-lint: allow-next=3 -- PASS finding prose names the mailer config file; no network call
      add security fileperm_sendmailcf "sendmail.cf owner/group/mode" PASS med \
          "$FSC_OBSERVED" \
          "/etc/mail/sendmail.cf is owned by root:system with mode -rw-r----- as required." \
          "n/a" "cis-l1"
    else
      FSC_BAD=""
      if [ "$FSC_MODE" != "-rw-r-----" ]; then FSC_BAD="mode"; fi
      if [ "$FSC_OWNER" != "root" ]; then FSC_BAD="${FSC_BAD:+$FSC_BAD }owner"; fi
      if [ "$FSC_GROUP" != "system" ]; then FSC_BAD="${FSC_BAD:+$FSC_BAD }group"; fi
      FSC_OBSERVED="$FSC_OBSERVED; requires -rw-r----- root:system; deviates: $FSC_BAD"
      # network-lint: allow-next=4 -- FAIL finding prose and remediation name the mailer config file; no network call
