...
AIXRAY_TOOL=ck-fileperm-smitlog-group


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_smitlog_group — CIS L1-aligned /smit.log group boundary.
  ...
  typeset FSMG_HOME_RAW FSMG_HOME_RC FSMG_HOME_OK FSMG_RAW FSMG_RC FSMG_PARSED FSMG_PARSE_RC FSMG_GID FSMG_REASON FSMG_ABSENT FSMG_DIAG

  FSMG_HOME_RAW=$(aix fileperm_smitlog_group_home lsuser -a home root)
  FSMG_HOME_RC=$?
  FSMG_HOME_OK=0
  if [ "$FSMG_HOME_RC" -eq 0 ] && [ "$FSMG_HOME_RAW" = "root home=/" ]; then
    FSMG_HOME_OK=1
  fi

  FSMG_RAW=$(aixv fileperm_smitlog_group_ls ls -ln /smit.log)
  FSMG_RC=$?
  FSMG_PARSED=""
  FSMG_GID=""
  FSMG_REASON=""
  ...
  if [ "$FSMG_HOME_OK" -ne 1 ]; then
    FSMG_REASON="root home not confirmed as / — smit.log path derivation unsupported"
  elif [ "$FSMG_RC" -ne 0 ]; then
    FSMG_REASON="absent or unreadable (rc=$FSMG_RC)"
    case "$FSMG_RAW" in
      *"/smit.log not found"*) FSMG_ABSENT=1 ;;
    esac
    if [ "$FSMG_ABSENT" -eq 1 ]; then
      FSMG_REASON="absent"
    else
      FSMG_DIAG=$(printf '%s\n' "$FSMG_RAW" | awk 'NF { gsub(/[ \t]+/, " "); sub(/^ /, ""); sub(/ $/, ""); if (length("" $0) > 40) $0 = substr($0, 1, 37) "..."; print; exit }')
      if [ -n "$FSMG_DIAG" ]; then
        FSMG_REASON="absent or unreadable (rc=$FSMG_RC): $FSMG_DIAG"
      fi
    fi
  elif [ -z "$FSMG_RAW" ]; then
    FSMG_REASON="empty"
  else
    FSMG_PARSED=$(printf '%s\n' "$FSMG_RAW" | awk '
      NF {
        records++
    ...
    ')
    FSMG_PARSE_RC=$?
    if [ "$FSMG_PARSE_RC" -ne 0 ] || [ -z "$FSMG_PARSED" ]; then
      FSMG_REASON="unparseable"
    else
      FSMG_GID=$FSMG_PARSED
    fi
  fi

  if [ "$FSMG_REASON" = "absent" ]; then
    add security fileperm_smitlog_group "/smit.log group" NOT_APPLICABLE med \
        "/smit.log absent" \
        "The probe returned the determinate-absence signature ('/smit.log not found', exit $FSMG_RC); /smit.log need not exist, so the group boundary does not apply." \
        "n/a" "cis-l1"
    ...
    add security fileperm_smitlog_group "/smit.log group" PASS med \
        ...
        "/smit.log has the required group boundary." \
        "n/a" "cis-l1"
  else
