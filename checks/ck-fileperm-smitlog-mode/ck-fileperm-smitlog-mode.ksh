...
AIXRAY_TOOL=ck-fileperm-smitlog-mode


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_smitlog_mode — CIS L1-aligned /smit.log mode boundary.
  ...
  typeset FSMM_HOME_RAW FSMM_HOME_RC FSMM_HOME_OK FSMM_RAW FSMM_RC FSMM_PARSED FSMM_PARSE_RC FSMM_MODE FSMM_OK FSMM_REASON FSMM_ABSENT FSMM_DIAG

  FSMM_HOME_RAW=$(aix fileperm_smitlog_mode_home lsuser -a home root)
  FSMM_HOME_RC=$?
  FSMM_HOME_OK=0
  if [ "$FSMM_HOME_RC" -eq 0 ] && [ "$FSMM_HOME_RAW" = "root home=/" ]; then
    FSMM_HOME_OK=1
  fi

  FSMM_RAW=$(aixv fileperm_smitlog_mode_ls ls -ln /smit.log)
  FSMM_RC=$?
  FSMM_PARSED=""
  FSMM_MODE=""
  FSMM_OK=""
  ...
  if [ "$FSMM_HOME_OK" -ne 1 ]; then
    FSMM_REASON="root home not confirmed as / — smit.log path derivation unsupported"
  elif [ "$FSMM_RC" -ne 0 ]; then
    FSMM_REASON="absent or unreadable (rc=$FSMM_RC)"
    case "$FSMM_RAW" in
      *"/smit.log not found"*) FSMM_ABSENT=1 ;;
    esac
    if [ "$FSMM_ABSENT" -eq 1 ]; then
      FSMM_REASON="absent"
    else
      FSMM_DIAG=$(printf '%s\n' "$FSMM_RAW" | awk 'NF { gsub(/[ \t]+/, " "); sub(/^ /, ""); sub(/ $/, ""); if (length("" $0) > 40) $0 = substr($0, 1, 37) "..."; print; exit }')
      if [ -n "$FSMM_DIAG" ]; then
        FSMM_REASON="absent or unreadable (rc=$FSMM_RC): $FSMM_DIAG"
      fi
    fi
  elif [ -z "$FSMM_RAW" ]; then
    FSMM_REASON="empty"
  else
    FSMM_PARSED=$(printf '%s\n' "$FSMM_RAW" | awk '
      NF {
        records++
        ...
        if (!date_ok) bad=1
        if ($NF != "/smit.log") bad=1
      }
      END {
        if (records != 1 || bad) exit 1
        target="0640"; ceiling=0
    ...
    ')
    FSMM_PARSE_RC=$?
    if [ "$FSMM_PARSE_RC" -ne 0 ] || [ -z "$FSMM_PARSED" ]; then
      FSMM_REASON="unparseable"
    else
      FSMM_MODE=${FSMM_PARSED%%\|*}
      FSMM_OK=${FSMM_PARSED#*\|}
    fi
  fi

  if [ "$FSMM_REASON" = "absent" ]; then
    add security fileperm_smitlog_mode "/smit.log mode" NOT_APPLICABLE med \
        "/smit.log absent" \
        "The probe returned the determinate-absence signature ('/smit.log not found', exit $FSMM_RC); /smit.log need not exist, so the mode boundary does not apply." \
        "n/a" "cis-l1"
    F_EVIDENCE_COMMANDS[$((NFIND-1))]="lsuser -a home root|ls -ln /smit.log"
    ...
    add security fileperm_smitlog_mode "/smit.log mode" PASS med \
        ...
        "/smit.log has the required mode boundary." \
        "n/a" "cis-l1"
  else
