...
AIXRAY_TOOL=ck-fileperm-smitlog-owner
...
_AIXRAY_SESSION_KEYS=""
  ...
  typeset FSMO_HOME_RAW FSMO_HOME_RC FSMO_HOME_OK FSMO_RAW FSMO_RC FSMO_PARSED FSMO_PARSE_RC FSMO_UID FSMO_REASON FSMO_ABSENT FSMO_DIAG

  FSMO_HOME_RAW=$(aix fileperm_smitlog_owner_home lsuser -a home root)
  FSMO_HOME_RC=$?
  FSMO_HOME_OK=0
  if [ "$FSMO_HOME_RC" -eq 0 ] && [ "$FSMO_HOME_RAW" = "root home=/" ]; then
    FSMO_HOME_OK=1
  fi

  FSMO_RAW=$(aixv fileperm_smitlog_owner_ls ls -ln /smit.log)
  FSMO_RC=$?
  FSMO_PARSED=""
  FSMO_UID=""
  FSMO_REASON=""
  ...
  if [ "$FSMO_HOME_OK" -ne 1 ]; then
    FSMO_REASON="root home not confirmed as / — smit.log path derivation unsupported"
  elif [ "$FSMO_RC" -ne 0 ]; then
    FSMO_REASON="absent or unreadable (rc=$FSMO_RC)"
    case "$FSMO_RAW" in
      *"/smit.log not found"*) FSMO_ABSENT=1 ;;
    esac
    if [ "$FSMO_ABSENT" -eq 1 ]; then
      FSMO_REASON="absent"
    else
      FSMO_DIAG=$(printf '%s\n' "$FSMO_RAW" | awk 'NF { gsub(/[ \t]+/, " "); sub(/^ /, ""); sub(/ $/, ""); if (length("" $0) > 40) $0 = substr($0, 1, 37) "..."; print; exit }')
      if [ -n "$FSMO_DIAG" ]; then
        FSMO_REASON="absent or unreadable (rc=$FSMO_RC): $FSMO_DIAG"
      fi
    fi
  elif [ -z "$FSMO_RAW" ]; then
    FSMO_REASON="empty"
  else
    FSMO_PARSED=$(printf '%s\n' "$FSMO_RAW" | awk '
      NF {
        records++
    ...
    ')
    FSMO_PARSE_RC=$?
    if [ "$FSMO_PARSE_RC" -ne 0 ] || [ -z "$FSMO_PARSED" ]; then
      FSMO_REASON="unparseable"
    else
      FSMO_UID=$FSMO_PARSED
    fi
  fi

  if [ "$FSMO_REASON" = "absent" ]; then
    add security fileperm_smitlog_owner "/smit.log owner" NOT_APPLICABLE med \
        "/smit.log absent" \
        "The probe returned the determinate-absence signature ('/smit.log not found', exit $FSMO_RC); /smit.log need not exist, so the owner boundary does not apply." \
        "n/a" "cis-l1"
    ...
    add security fileperm_smitlog_owner "/smit.log owner" PASS med \
        ...
        "/smit.log has the required owner boundary." \
        "n/a" "cis-l1"
  else
