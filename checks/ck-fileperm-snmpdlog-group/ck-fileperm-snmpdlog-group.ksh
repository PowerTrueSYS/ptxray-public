...
AIXRAY_TOOL=ck-fileperm-snmpdlog-group


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_snmpdlog_group — CIS L1-aligned /var/tmp/snmpd.log group boundary.
  ...
  typeset FSNG_RAW FSNG_RC FSNG_PARSED FSNG_PARSE_RC FSNG_GID FSNG_REASON FSNG_ABSENT FSNG_DIAG

  FSNG_RAW=$(aixv fileperm_snmpdlog_group_ls ls -ln /var/tmp/snmpd.log)
  FSNG_RC=$?
  FSNG_PARSED=""
  FSNG_GID=""
  FSNG_REASON=""
  ...
  if [ "$FSNG_RC" -ne 0 ]; then
    FSNG_REASON="absent or unreadable (rc=$FSNG_RC)"
    case "$FSNG_RAW" in
      *"/var/tmp/snmpd.log not found"*) FSNG_ABSENT=1 ;;
    esac
    if [ "$FSNG_ABSENT" -eq 1 ]; then
      FSNG_REASON="absent"
    else
      FSNG_DIAG=$(printf '%s\n' "$FSNG_RAW" | awk 'NF { gsub(/[ \t]+/, " "); sub(/^ /, ""); sub(/ $/, ""); if (length("" $0) > 40) $0 = substr($0, 1, 37) "..."; print; exit }')
      if [ -n "$FSNG_DIAG" ]; then
        FSNG_REASON="absent or unreadable (rc=$FSNG_RC): $FSNG_DIAG"
      fi
    fi
  elif [ -z "$FSNG_RAW" ]; then
    FSNG_REASON="empty"
  else
    FSNG_PARSED=$(printf '%s\n' "$FSNG_RAW" | awk '
      NF {
        records++
    ...
    ')
    FSNG_PARSE_RC=$?
    if [ "$FSNG_PARSE_RC" -ne 0 ] || [ -z "$FSNG_PARSED" ]; then
      FSNG_REASON="unparseable"
    else
      FSNG_GID=$FSNG_PARSED
    fi
  fi

  if [ "$FSNG_REASON" = "absent" ]; then
    add security fileperm_snmpdlog_group "/var/tmp/snmpd.log group" NOT_APPLICABLE med \
        "/var/tmp/snmpd.log absent" \
        "The probe returned the determinate-absence signature ('/var/tmp/snmpd.log not found', exit $FSNG_RC); /var/tmp/snmpd.log need not exist, so the group boundary does not apply." \
        "n/a" "cis-l1"
    ...
    add security fileperm_snmpdlog_group "/var/tmp/snmpd.log group" PASS med \
        ...
        "/var/tmp/snmpd.log has the required group boundary." \
        "n/a" "cis-l1"
  else
