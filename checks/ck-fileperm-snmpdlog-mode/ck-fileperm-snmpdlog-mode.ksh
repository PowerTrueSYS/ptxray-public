...
AIXRAY_TOOL=ck-fileperm-snmpdlog-mode


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_snmpdlog_mode — CIS L1-aligned /var/tmp/snmpd.log mode boundary.
  ...
  typeset FSNM_RAW FSNM_RC FSNM_PARSED FSNM_PARSE_RC FSNM_MODE FSNM_OK FSNM_REASON FSNM_ABSENT FSNM_DIAG

  FSNM_RAW=$(aixv fileperm_snmpdlog_mode_ls ls -ln /var/tmp/snmpd.log)
  FSNM_RC=$?
  FSNM_PARSED=""
  FSNM_MODE=""
  FSNM_OK=""
  ...
  if [ "$FSNM_RC" -ne 0 ]; then
    FSNM_REASON="absent or unreadable (rc=$FSNM_RC)"
    case "$FSNM_RAW" in
      *"/var/tmp/snmpd.log not found"*) FSNM_ABSENT=1 ;;
    esac
    if [ "$FSNM_ABSENT" -eq 1 ]; then
      FSNM_REASON="absent"
    else
      FSNM_DIAG=$(printf '%s\n' "$FSNM_RAW" | awk 'NF { gsub(/[ \t]+/, " "); sub(/^ /, ""); sub(/ $/, ""); if (length("" $0) > 40) $0 = substr($0, 1, 37) "..."; print; exit }')
      if [ -n "$FSNM_DIAG" ]; then
        FSNM_REASON="absent or unreadable (rc=$FSNM_RC): $FSNM_DIAG"
      fi
    fi
  elif [ -z "$FSNM_RAW" ]; then
    FSNM_REASON="empty"
  else
    FSNM_PARSED=$(printf '%s\n' "$FSNM_RAW" | awk '
      NF {
        records++
    ...
    ')
    FSNM_PARSE_RC=$?
    if [ "$FSNM_PARSE_RC" -ne 0 ] || [ -z "$FSNM_PARSED" ]; then
      FSNM_REASON="unparseable"
    else
      FSNM_MODE=${FSNM_PARSED%%\|*}
      FSNM_OK=${FSNM_PARSED#*\|}
    fi
  fi

  if [ "$FSNM_REASON" = "absent" ]; then
    add security fileperm_snmpdlog_mode "/var/tmp/snmpd.log mode" NOT_APPLICABLE med \
        "/var/tmp/snmpd.log absent" \
        "The probe returned the determinate-absence signature ('/var/tmp/snmpd.log not found', exit $FSNM_RC); /var/tmp/snmpd.log need not exist, so the mode boundary does not apply." \
        "n/a" "cis-l1"
    F_EVIDENCE_COMMANDS[$((NFIND-1))]="ls -ln /var/tmp/snmpd.log"
    ...
    add security fileperm_snmpdlog_mode "/var/tmp/snmpd.log mode" PASS med \
        ...
        "/var/tmp/snmpd.log has the required mode boundary." \
        "n/a" "cis-l1"
  else
