...
AIXRAY_TOOL=ck-fileperm-snmpdlog-owner


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_snmpdlog_owner — CIS L1-aligned /var/tmp/snmpd.log owner boundary.
  ...
  typeset FSNO_RAW FSNO_RC FSNO_PARSED FSNO_PARSE_RC FSNO_UID FSNO_REASON FSNO_ABSENT FSNO_DIAG

  FSNO_RAW=$(aixv fileperm_snmpdlog_owner_ls ls -ln /var/tmp/snmpd.log)
  FSNO_RC=$?
  FSNO_PARSED=""
  FSNO_UID=""
  FSNO_REASON=""
  ...
  if [ "$FSNO_RC" -ne 0 ]; then
    FSNO_REASON="absent or unreadable (rc=$FSNO_RC)"
    case "$FSNO_RAW" in
      *"/var/tmp/snmpd.log not found"*) FSNO_ABSENT=1 ;;
    esac
    if [ "$FSNO_ABSENT" -eq 1 ]; then
      FSNO_REASON="absent"
    else
      FSNO_DIAG=$(printf '%s\n' "$FSNO_RAW" | awk 'NF { gsub(/[ \t]+/, " "); sub(/^ /, ""); sub(/ $/, ""); if (length("" $0) > 40) $0 = substr($0, 1, 37) "..."; print; exit }')
      if [ -n "$FSNO_DIAG" ]; then
        FSNO_REASON="absent or unreadable (rc=$FSNO_RC): $FSNO_DIAG"
      fi
    fi
  elif [ -z "$FSNO_RAW" ]; then
    FSNO_REASON="empty"
  else
    FSNO_PARSED=$(printf '%s\n' "$FSNO_RAW" | awk '
      NF {
        records++
    ...
    ')
    FSNO_PARSE_RC=$?
    if [ "$FSNO_PARSE_RC" -ne 0 ] || [ -z "$FSNO_PARSED" ]; then
      FSNO_REASON="unparseable"
    else
      FSNO_UID=$FSNO_PARSED
    fi
  fi

  if [ "$FSNO_REASON" = "absent" ]; then
    add security fileperm_snmpdlog_owner "/var/tmp/snmpd.log owner" NOT_APPLICABLE med \
        "/var/tmp/snmpd.log absent" \
        "The probe returned the determinate-absence signature ('/var/tmp/snmpd.log not found', exit $FSNO_RC); /var/tmp/snmpd.log need not exist, so the owner boundary does not apply." \
        "n/a" "cis-l1"
    ...
    add security fileperm_snmpdlog_owner "/var/tmp/snmpd.log owner" PASS med \
        ...
        "/var/tmp/snmpd.log has the required owner boundary." \
        "n/a" "cis-l1"
  else
