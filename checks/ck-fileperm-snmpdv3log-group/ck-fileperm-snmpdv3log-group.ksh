...
AIXRAY_TOOL=ck-fileperm-snmpdv3log-group


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_snmpdv3log_group — CIS L1-aligned /var/tmp/snmpdv3.log group boundary.
  ...
  typeset FSVG_RAW FSVG_RC FSVG_PARSED FSVG_PARSE_RC FSVG_GID FSVG_REASON FSVG_ABSENT FSVG_DIAG

  FSVG_RAW=$(aixv fileperm_snmpdv3log_group_ls ls -ln /var/tmp/snmpdv3.log)
  FSVG_RC=$?
  FSVG_PARSED=""
  FSVG_GID=""
  FSVG_REASON=""
  ...
  if [ "$FSVG_RC" -ne 0 ]; then
    FSVG_REASON="absent or unreadable (rc=$FSVG_RC)"
    case "$FSVG_RAW" in
      *"/var/tmp/snmpdv3.log not found"*) FSVG_ABSENT=1 ;;
    esac
    if [ "$FSVG_ABSENT" -eq 1 ]; then
      FSVG_REASON="absent"
    else
      FSVG_DIAG=$(printf '%s\n' "$FSVG_RAW" | awk 'NF { gsub(/[ \t]+/, " "); sub(/^ /, ""); sub(/ $/, ""); if (length("" $0) > 40) $0 = substr($0, 1, 37) "..."; print; exit }')
      if [ -n "$FSVG_DIAG" ]; then
        FSVG_REASON="absent or unreadable (rc=$FSVG_RC): $FSVG_DIAG"
      fi
    fi
  elif [ -z "$FSVG_RAW" ]; then
    FSVG_REASON="empty"
  else
    FSVG_PARSED=$(printf '%s\n' "$FSVG_RAW" | awk '
      NF {
        records++
    ...
    ')
    FSVG_PARSE_RC=$?
    if [ "$FSVG_PARSE_RC" -ne 0 ] || [ -z "$FSVG_PARSED" ]; then
      FSVG_REASON="unparseable"
    else
      FSVG_GID=$FSVG_PARSED
    fi
  fi

  if [ "$FSVG_REASON" = "absent" ]; then
    add security fileperm_snmpdv3log_group "/var/tmp/snmpdv3.log group" NOT_APPLICABLE med \
        "/var/tmp/snmpdv3.log absent" \
        "The probe returned the determinate-absence signature ('/var/tmp/snmpdv3.log not found', exit $FSVG_RC); /var/tmp/snmpdv3.log need not exist, so the group boundary does not apply." \
        "n/a" "cis-l1"
    ...
    add security fileperm_snmpdv3log_group "/var/tmp/snmpdv3.log group" PASS med \
        ...
        "/var/tmp/snmpdv3.log has the required group boundary." \
        "n/a" "cis-l1"
  else
