...
AIXRAY_TOOL=ck-fileperm-snmpdv3log-mode


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_snmpdv3log_mode — CIS L1-aligned /var/tmp/snmpdv3.log mode boundary.
  ...
  typeset FSVM_RAW FSVM_RC FSVM_PARSED FSVM_PARSE_RC FSVM_MODE FSVM_OK FSVM_REASON FSVM_ABSENT FSVM_DIAG

  FSVM_RAW=$(aixv fileperm_snmpdv3log_mode_ls ls -ln /var/tmp/snmpdv3.log)
  FSVM_RC=$?
  FSVM_PARSED=""
  FSVM_MODE=""
  FSVM_OK=""
  ...
  if [ "$FSVM_RC" -ne 0 ]; then
    FSVM_REASON="absent or unreadable (rc=$FSVM_RC)"
    case "$FSVM_RAW" in
      *"/var/tmp/snmpdv3.log not found"*) FSVM_ABSENT=1 ;;
    esac
    if [ "$FSVM_ABSENT" -eq 1 ]; then
      FSVM_REASON="absent"
    else
      FSVM_DIAG=$(printf '%s\n' "$FSVM_RAW" | awk 'NF { gsub(/[ \t]+/, " "); sub(/^ /, ""); sub(/ $/, ""); if (length("" $0) > 40) $0 = substr($0, 1, 37) "..."; print; exit }')
      if [ -n "$FSVM_DIAG" ]; then
        FSVM_REASON="absent or unreadable (rc=$FSVM_RC): $FSVM_DIAG"
      fi
    fi
  elif [ -z "$FSVM_RAW" ]; then
    FSVM_REASON="empty"
  else
    FSVM_PARSED=$(printf '%s\n' "$FSVM_RAW" | awk '
      NF {
        records++
        ...
        if (!date_ok) bad=1
        if ($NF != "/var/tmp/snmpdv3.log") bad=1
      }
      END {
    ...
    ')
    FSVM_PARSE_RC=$?
    if [ "$FSVM_PARSE_RC" -ne 0 ] || [ -z "$FSVM_PARSED" ]; then
      FSVM_REASON="unparseable"
    else
      FSVM_MODE=${FSVM_PARSED%%\|*}
      FSVM_OK=${FSVM_PARSED#*\|}
    fi
  fi

  if [ "$FSVM_REASON" = "absent" ]; then
    add security fileperm_snmpdv3log_mode "/var/tmp/snmpdv3.log mode" NOT_APPLICABLE med \
        "/var/tmp/snmpdv3.log absent" \
        "The probe returned the determinate-absence signature ('/var/tmp/snmpdv3.log not found', exit $FSVM_RC); /var/tmp/snmpdv3.log need not exist, so the mode boundary does not apply." \
        "n/a" "cis-l1"
    ...
    add security fileperm_snmpdv3log_mode "/var/tmp/snmpdv3.log mode" PASS med \
        ...
        "/var/tmp/snmpdv3.log has the required mode boundary." \
        "n/a" "cis-l1"
  else
