...
AIXRAY_TOOL=ck-fileperm-snmpdv3log-owner


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_snmpdv3log_owner — CIS L1-aligned /var/tmp/snmpdv3.log owner boundary.
  ...
  typeset FSVO_RAW FSVO_RC FSVO_PARSED FSVO_PARSE_RC FSVO_UID FSVO_REASON FSVO_ABSENT FSVO_DIAG

  FSVO_RAW=$(aixv fileperm_snmpdv3log_owner_ls ls -ln /var/tmp/snmpdv3.log)
  FSVO_RC=$?
  FSVO_PARSED=""
  FSVO_UID=""
  FSVO_REASON=""
  ...
  if [ "$FSVO_RC" -ne 0 ]; then
    FSVO_REASON="absent or unreadable (rc=$FSVO_RC)"
    case "$FSVO_RAW" in
      *"/var/tmp/snmpdv3.log not found"*) FSVO_ABSENT=1 ;;
    esac
    if [ "$FSVO_ABSENT" -eq 1 ]; then
      FSVO_REASON="absent"
    else
      FSVO_DIAG=$(printf '%s\n' "$FSVO_RAW" | awk 'NF { gsub(/[ \t]+/, " "); sub(/^ /, ""); sub(/ $/, ""); if (length("" $0) > 40) $0 = substr($0, 1, 37) "..."; print; exit }')
      if [ -n "$FSVO_DIAG" ]; then
        FSVO_REASON="absent or unreadable (rc=$FSVO_RC): $FSVO_DIAG"
      fi
    fi
  elif [ -z "$FSVO_RAW" ]; then
    FSVO_REASON="empty"
  else
    FSVO_PARSED=$(printf '%s\n' "$FSVO_RAW" | awk '
      NF {
        records++
    ...
    ')
    FSVO_PARSE_RC=$?
    if [ "$FSVO_PARSE_RC" -ne 0 ] || [ -z "$FSVO_PARSED" ]; then
      FSVO_REASON="unparseable"
    else
      FSVO_UID=$FSVO_PARSED
    fi
  fi

  if [ "$FSVO_REASON" = "absent" ]; then
    add security fileperm_snmpdv3log_owner "/var/tmp/snmpdv3.log owner" NOT_APPLICABLE med \
        "/var/tmp/snmpdv3.log absent" \
        "The probe returned the determinate-absence signature ('/var/tmp/snmpdv3.log not found', exit $FSVO_RC); /var/tmp/snmpdv3.log need not exist, so the owner boundary does not apply." \
        "n/a" "cis-l1"
    ...
    add security fileperm_snmpdv3log_owner "/var/tmp/snmpdv3.log owner" PASS med \
        ...
        "/var/tmp/snmpdv3.log has the required owner boundary." \
        "n/a" "cis-l1"
  else
