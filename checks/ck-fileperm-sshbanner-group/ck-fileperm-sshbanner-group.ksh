...
AIXRAY_TOOL=ck-fileperm-sshbanner-group


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_sshbanner_group — CIS L1-aligned OpenSSH login banner file group boundary.
  ...
  typeset FSBG_RAW FSBG_RC FSBG_PARSED FSBG_PARSE_RC FSBG_GID FSBG_REASON FSBG_ABSENT FSBG_DIAG

  FSBG_RAW=$(aixv fileperm_sshbanner_group_ls ls -ln /etc/ssh/ssh_banner) # network-lint: allow -- reads local filesystem metadata only
  FSBG_RC=$?
  FSBG_PARSED=""
  FSBG_GID=""
  FSBG_REASON=""
  ...
  if [ "$FSBG_RC" -ne 0 ]; then
    FSBG_REASON="absent or unreadable (rc=$FSBG_RC)"
    case "$FSBG_RAW" in
      *"/etc/ssh/ssh_banner not found"*) FSBG_ABSENT=1 ;; # network-lint: allow -- absence signature is a local pathname, not a network call
    esac
    if [ "$FSBG_ABSENT" -eq 1 ]; then
      FSBG_REASON="absent"
    else
      FSBG_DIAG=$(printf '%s\n' "$FSBG_RAW" | awk 'NF { gsub(/[ \t]+/, " "); sub(/^ /, ""); sub(/ $/, ""); if (length("" $0) > 40) $0 = substr($0, 1, 37) "..."; print; exit }')
      if [ -n "$FSBG_DIAG" ]; then
        FSBG_REASON="absent or unreadable (rc=$FSBG_RC): $FSBG_DIAG"
      fi
    fi
  elif [ -z "$FSBG_RAW" ]; then
    FSBG_REASON="empty"
  else
    FSBG_PARSED=$(printf '%s\n' "$FSBG_RAW" | awk '
      NF {
        records++
        ...
        if ($2 !~ /^[0-9][0-9]*$/ || $3 !~ /^[0-9][0-9]*$/ || $4 !~ /^[0-9][0-9]*$/ || length("" $4) > 10 || $5 !~ /^[0-9][0-9]*$/) bad=1
    ...
    ')
    FSBG_PARSE_RC=$?
    if [ "$FSBG_PARSE_RC" -ne 0 ] || [ -z "$FSBG_PARSED" ]; then
      FSBG_REASON="unparseable"
    else
      FSBG_GID=$FSBG_PARSED
    fi
  fi

  if [ "$FSBG_REASON" = "absent" ]; then
    # network-lint: allow-next=3 -- NOT_APPLICABLE finding prose names the OpenSSH login banner file; the add command emits a finding, no network call
    add security fileperm_sshbanner_group "Secure Shell login banner group" NOT_APPLICABLE med \
        "OpenSSH login banner file absent" \
        "The probe returned the determinate-absence signature ('/etc/ssh/ssh_banner not found', exit $FSBG_RC); the OpenSSH login banner file need not exist, so the group boundary does not apply." \
        "n/a" "cis-l1"
    ...
    add security fileperm_sshbanner_group "Secure Shell login banner group" PASS med \
        ...
        "The OpenSSH login banner file has the required group boundary." \
        "n/a" "cis-l1"
  else
