...
AIXRAY_TOOL=ck-fileperm-sshbanner-mode


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_sshbanner_mode — CIS L1-aligned OpenSSH login banner file mode boundary.
  # The ls probe runs through aixv (stderr merged), not aix: on AIX a missing
  # file makes ls exit 2 and write "<path> not found" to STDERR, which aix()
  # discards -- absent and unreadable then look identical (rc=2, empty stdout)
  # and both refused. Absence is keyed POSITIVELY on that signature and renders
  # NOT_APPLICABLE; any other nonzero exit still refuses (fail closed).
  typeset FSBM_RAW FSBM_RC FSBM_PARSED FSBM_PARSE_RC FSBM_MODE FSBM_OK FSBM_REASON FSBM_ABSENT FSBM_DIAG

  FSBM_RAW=$(aixv fileperm_sshbanner_mode_ls ls -ln /etc/ssh/ssh_banner) # network-lint: allow -- reads local filesystem metadata only
  FSBM_RC=$?
  FSBM_PARSED=""
  FSBM_MODE=""
  FSBM_OK=""
  ...
  if [ "$FSBM_RC" -ne 0 ]; then
    FSBM_REASON="absent or unreadable (rc=$FSBM_RC)"
    case "$FSBM_RAW" in
      *"/etc/ssh/ssh_banner not found"*) FSBM_ABSENT=1 ;; # network-lint: allow -- absence signature is a local pathname, not a network call
    esac
    if [ "$FSBM_ABSENT" -eq 1 ]; then
      FSBM_REASON="absent"
    else
      FSBM_DIAG=$(printf '%s\n' "$FSBM_RAW" | awk 'NF { gsub(/[ \t]+/, " "); sub(/^ /, ""); sub(/ $/, ""); if (length("" $0) > 40) $0 = substr($0, 1, 37) "..."; print; exit }')
      if [ -n "$FSBM_DIAG" ]; then
        FSBM_REASON="absent or unreadable (rc=$FSBM_RC): $FSBM_DIAG"
      fi
    fi
  elif [ -z "$FSBM_RAW" ]; then
    FSBM_REASON="empty"
  else
    FSBM_PARSED=$(printf '%s\n' "$FSBM_RAW" | awk '
      NF {
        records++
        ...
        } else if (length("" mode) != 10) bad=1
        bits=0
        if (length("" mode) == 10) {
          if (substr(mode,1,1) != "-") bad=1
          ...
          if (substr(mode,2,1)=="r") bits+=256
          if (substr(mode,3,1)=="w") bits+=128
          c=substr(mode,4,1); if (c=="x" || c=="s") bits+=64; if (c=="s" || c=="S") bits+=2048
          if (substr(mode,5,1)=="r") bits+=32
          if (substr(mode,6,1)=="w") bits+=16
          c=substr(mode,7,1); if (c=="x" || c=="s") bits+=8; if (c=="s" || c=="S") bits+=1024
          if (substr(mode,8,1)=="r") bits+=4
          if (substr(mode,9,1)=="w") bits+=2
          c=substr(mode,10,1); if (c=="x" || c=="t") bits+=1; if (c=="t" || c=="T") bits+=512
        }
        if ($2 !~ /^[0-9][0-9]*$/ || $3 !~ /^[0-9][0-9]*$/ || $4 !~ /^[0-9][0-9]*$/ || $5 !~ /^[0-9][0-9]*$/) bad=1
        ...
        if (!date_ok) bad=1
        if ($NF != "/etc/ssh/ssh_banner") bad=1 # network-lint: allow -- validates the captured local pathname only
      }
      END {
        if (records != 1 || bad) exit 1
        target="0644"; ceiling=0
        ...
        print sprintf("%04o", bits) "|" (extra == 0)
      }
    ')
    FSBM_PARSE_RC=$?
    if [ "$FSBM_PARSE_RC" -ne 0 ] || [ -z "$FSBM_PARSED" ]; then
      FSBM_REASON="unparseable"
    else
      FSBM_MODE=${FSBM_PARSED%%\|*}
      FSBM_OK=${FSBM_PARSED#*\|}
    fi
  fi

  if [ "$FSBM_REASON" = "absent" ]; then
    # network-lint: allow-next=4 -- NOT_APPLICABLE finding prose names the OpenSSH login banner file; the add command emits a finding, no network call
    add security fileperm_sshbanner_mode "Secure Shell login banner mode" NOT_APPLICABLE med \
        "OpenSSH login banner file absent" \
        "The probe returned the determinate-absence signature ('/etc/ssh/ssh_banner not found', exit $FSBM_RC); the OpenSSH login banner file need not exist, so the mode boundary does not apply." \
        "n/a" "cis-l1"
    F_EVIDENCE_COMMANDS[$((NFIND-1))]="ls -ln /etc/ssh/ssh_banner" # network-lint: allow -- evidence display string for the LOCAL ls metadata read, no network call
    ...
    add security fileperm_sshbanner_mode "Secure Shell login banner mode" PASS med \
        ...
        "The OpenSSH login banner file has the required mode boundary." \
        "n/a" "cis-l1"
  else
