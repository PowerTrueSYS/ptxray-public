...
AIXRAY_TOOL=ck-fileperm-sshbanner-owner


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_sshbanner_owner — CIS L1-aligned OpenSSH login banner file owner boundary.
  ...
  typeset FSBO_RAW FSBO_RC FSBO_PARSED FSBO_PARSE_RC FSBO_UID FSBO_REASON FSBO_ABSENT FSBO_DIAG

  FSBO_RAW=$(aixv fileperm_sshbanner_owner_ls ls -ln /etc/ssh/ssh_banner) # network-lint: allow -- reads local filesystem metadata only
  FSBO_RC=$?
  FSBO_PARSED=""
  FSBO_UID=""
  FSBO_REASON=""
  ...
  if [ "$FSBO_RC" -ne 0 ]; then
    FSBO_REASON="absent or unreadable (rc=$FSBO_RC)"
    case "$FSBO_RAW" in
      *"/etc/ssh/ssh_banner not found"*) FSBO_ABSENT=1 ;; # network-lint: allow -- absence signature is a local pathname, not a network call
    esac
    if [ "$FSBO_ABSENT" -eq 1 ]; then
      FSBO_REASON="absent"
    else
      FSBO_DIAG=$(printf '%s\n' "$FSBO_RAW" | awk 'NF { gsub(/[ \t]+/, " "); sub(/^ /, ""); sub(/ $/, ""); if (length("" $0) > 40) $0 = substr($0, 1, 37) "..."; print; exit }')
      if [ -n "$FSBO_DIAG" ]; then
        FSBO_REASON="absent or unreadable (rc=$FSBO_RC): $FSBO_DIAG"
      fi
    fi
  elif [ -z "$FSBO_RAW" ]; then
    FSBO_REASON="empty"
  else
    FSBO_PARSED=$(printf '%s\n' "$FSBO_RAW" | awk '
      NF {
        records++
    ...
    ')
    FSBO_PARSE_RC=$?
    if [ "$FSBO_PARSE_RC" -ne 0 ] || [ -z "$FSBO_PARSED" ]; then
      FSBO_REASON="unparseable"
    else
      FSBO_UID=$FSBO_PARSED
    fi
  fi

  if [ "$FSBO_REASON" = "absent" ]; then
    ...
    add security fileperm_sshbanner_owner "Secure Shell login banner owner" NOT_APPLICABLE med \
        "OpenSSH login banner file absent" \
        "The probe returned the determinate-absence signature ('/etc/ssh/ssh_banner not found', exit $FSBO_RC); the OpenSSH login banner file need not exist, so the owner boundary does not apply." \
        "n/a" "cis-l1"
    ...
    add security fileperm_sshbanner_owner "Secure Shell login banner owner" PASS med \
        ...
        "The OpenSSH login banner file has the required owner boundary." \
        "n/a" "cis-l1"
  else
