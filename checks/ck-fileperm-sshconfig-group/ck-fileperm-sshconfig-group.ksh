...
AIXRAY_TOOL=ck-fileperm-sshconfig-group


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_sshconfig_group — CIS L1-aligned OpenSSH client configuration file group boundary.
  ...
  typeset FSSG_RAW FSSG_RC FSSG_PARSED FSSG_PARSE_RC FSSG_GID FSSG_REASON FSSG_ABSENT FSSG_DIAG

  FSSG_RAW=$(aixv fileperm_sshconfig_group_ls ls -ln /etc/ssh/ssh_config) # network-lint: allow -- reads local filesystem metadata only
  FSSG_RC=$?
  FSSG_PARSED=""
  FSSG_GID=""
  FSSG_REASON=""
  ...
  if [ "$FSSG_RC" -ne 0 ]; then
    FSSG_REASON="absent or unreadable (rc=$FSSG_RC)"
    case "$FSSG_RAW" in
      *"/etc/ssh/ssh_config not found"*) FSSG_ABSENT=1 ;; # network-lint: allow -- absence signature is a local pathname, not a network call
    esac
    if [ "$FSSG_ABSENT" -eq 1 ]; then
      FSSG_REASON="absent"
    else
      FSSG_DIAG=$(printf '%s\n' "$FSSG_RAW" | awk 'NF { gsub(/[ \t]+/, " "); sub(/^ /, ""); sub(/ $/, ""); if (length("" $0) > 40) $0 = substr($0, 1, 37) "..."; print; exit }')
      if [ -n "$FSSG_DIAG" ]; then
        FSSG_REASON="absent or unreadable (rc=$FSSG_RC): $FSSG_DIAG"
      fi
    fi
  elif [ -z "$FSSG_RAW" ]; then
    FSSG_REASON="empty"
  else
    FSSG_PARSED=$(printf '%s\n' "$FSSG_RAW" | awk '
      NF {
        records++
    ...
    ')
    FSSG_PARSE_RC=$?
    if [ "$FSSG_PARSE_RC" -ne 0 ] || [ -z "$FSSG_PARSED" ]; then
      FSSG_REASON="unparseable"
    else
      FSSG_GID=$FSSG_PARSED
    fi
  fi

  if [ "$FSSG_REASON" = "absent" ]; then
    # network-lint: allow-next=3 -- NOT_APPLICABLE finding prose names the OpenSSH client config file; the add command emits a finding, no network call
    add security fileperm_sshconfig_group "Secure Shell client configuration group" NOT_APPLICABLE med \
        "OpenSSH client configuration file absent" \
        "The probe returned the determinate-absence signature ('/etc/ssh/ssh_config not found', exit $FSSG_RC); the OpenSSH client configuration file need not exist, so the group boundary does not apply." \
        "n/a" "cis-l1"
    ...
    add security fileperm_sshconfig_group "Secure Shell client configuration group" PASS med \
        ...
        "The OpenSSH client configuration file has the required group boundary." \
        "n/a" "cis-l1"
  else
