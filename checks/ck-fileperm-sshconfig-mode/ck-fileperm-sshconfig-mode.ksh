...
AIXRAY_TOOL=ck-fileperm-sshconfig-mode


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_sshconfig_mode — CIS L1-aligned OpenSSH client configuration file mode boundary.
  ...
  typeset FSSM_RAW FSSM_RC FSSM_PARSED FSSM_PARSE_RC FSSM_MODE FSSM_OK FSSM_REASON FSSM_ABSENT FSSM_DIAG

  FSSM_RAW=$(aixv fileperm_sshconfig_mode_ls ls -ln /etc/ssh/ssh_config) # network-lint: allow -- reads local filesystem metadata only
  FSSM_RC=$?
  FSSM_PARSED=""
  FSSM_MODE=""
  FSSM_OK=""
  ...
  if [ "$FSSM_RC" -ne 0 ]; then
    FSSM_REASON="absent or unreadable (rc=$FSSM_RC)"
    case "$FSSM_RAW" in
      *"/etc/ssh/ssh_config not found"*) FSSM_ABSENT=1 ;; # network-lint: allow -- absence signature is a local pathname, not a network call
    esac
    if [ "$FSSM_ABSENT" -eq 1 ]; then
      FSSM_REASON="absent"
    else
      FSSM_DIAG=$(printf '%s\n' "$FSSM_RAW" | awk 'NF { gsub(/[ \t]+/, " "); sub(/^ /, ""); sub(/ $/, ""); if (length("" $0) > 40) $0 = substr($0, 1, 37) "..."; print; exit }')
      if [ -n "$FSSM_DIAG" ]; then
        FSSM_REASON="absent or unreadable (rc=$FSSM_RC): $FSSM_DIAG"
      fi
    fi
  elif [ -z "$FSSM_RAW" ]; then
    FSSM_REASON="empty"
  else
    FSSM_PARSED=$(printf '%s\n' "$FSSM_RAW" | awk '
      NF {
        records++
        ...
        if (!date_ok) bad=1
        if ($NF != "/etc/ssh/ssh_config") bad=1 # network-lint: allow -- validates the captured local pathname only
      }
      END {
    ...
    ')
    FSSM_PARSE_RC=$?
    if [ "$FSSM_PARSE_RC" -ne 0 ] || [ -z "$FSSM_PARSED" ]; then
      FSSM_REASON="unparseable"
    else
      FSSM_MODE=${FSSM_PARSED%%\|*}
      FSSM_OK=${FSSM_PARSED#*\|}
    fi
  fi

  if [ "$FSSM_REASON" = "absent" ]; then
    # network-lint: allow-next=4 -- NOT_APPLICABLE finding prose names the OpenSSH client config file; the add command emits a finding, no network call
    add security fileperm_sshconfig_mode "Secure Shell client configuration mode" NOT_APPLICABLE med \
        "OpenSSH client configuration file absent" \
        "The probe returned the determinate-absence signature ('/etc/ssh/ssh_config not found', exit $FSSM_RC); the OpenSSH client configuration file need not exist, so the mode boundary does not apply." \
        "n/a" "cis-l1"
    ...
    add security fileperm_sshconfig_mode "Secure Shell client configuration mode" PASS med \
        ...
        "The OpenSSH client configuration file has the required mode boundary." \
        "n/a" "cis-l1"
  else
