...
AIXRAY_TOOL=ck-fileperm-sshconfig-owner


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_sshconfig_owner — CIS L1-aligned OpenSSH client configuration file owner boundary.
  ...
  typeset FSSO_RAW FSSO_RC FSSO_PARSED FSSO_PARSE_RC FSSO_UID FSSO_REASON FSSO_ABSENT FSSO_DIAG

  FSSO_RAW=$(aixv fileperm_sshconfig_owner_ls ls -ln /etc/ssh/ssh_config) # network-lint: allow -- reads local filesystem metadata only
  FSSO_RC=$?
  FSSO_PARSED=""
  FSSO_UID=""
  FSSO_REASON=""
  ...
  if [ "$FSSO_RC" -ne 0 ]; then
    FSSO_REASON="absent or unreadable (rc=$FSSO_RC)"
    case "$FSSO_RAW" in
      *"/etc/ssh/ssh_config not found"*) FSSO_ABSENT=1 ;; # network-lint: allow -- absence signature is a local pathname, not a network call
    esac
    if [ "$FSSO_ABSENT" -eq 1 ]; then
      FSSO_REASON="absent"
    else
      FSSO_DIAG=$(printf '%s\n' "$FSSO_RAW" | awk 'NF { gsub(/[ \t]+/, " "); sub(/^ /, ""); sub(/ $/, ""); if (length("" $0) > 40) $0 = substr($0, 1, 37) "..."; print; exit }')
      if [ -n "$FSSO_DIAG" ]; then
        FSSO_REASON="absent or unreadable (rc=$FSSO_RC): $FSSO_DIAG"
      fi
    fi
  elif [ -z "$FSSO_RAW" ]; then
    FSSO_REASON="empty"
  else
    FSSO_PARSED=$(printf '%s\n' "$FSSO_RAW" | awk '
      NF {
        records++
    ...
    ')
    FSSO_PARSE_RC=$?
    if [ "$FSSO_PARSE_RC" -ne 0 ] || [ -z "$FSSO_PARSED" ]; then
      FSSO_REASON="unparseable"
    else
      FSSO_UID=$FSSO_PARSED
    fi
  fi

  if [ "$FSSO_REASON" = "absent" ]; then
    ...
    add security fileperm_sshconfig_owner "Secure Shell client configuration owner" NOT_APPLICABLE med \
        "OpenSSH client configuration file absent" \
        "The probe returned the determinate-absence signature ('/etc/ssh/ssh_config not found', exit $FSSO_RC); the OpenSSH client configuration file need not exist, so the owner boundary does not apply." \
        "n/a" "cis-l1"
    ...
    add security fileperm_sshconfig_owner "Secure Shell client configuration owner" PASS med \
        ...
        "The OpenSSH client configuration file has the required owner boundary." \
        "n/a" "cis-l1"
  else
