...
AIXRAY_TOOL=ck-fileperm-sshdconfig-group


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_sshdconfig_group — CIS L1-aligned OpenSSH configuration group boundary.
  typeset FIG_RAW FIG_RC FIG_PARSED FIG_PARSE_RC FIG_GID FIG_REASON

  FIG_RAW=$(aix fileperm_sshdconfig_group_ls ls -ln /etc/ssh/sshd_config) # network-lint: allow -- reads local filesystem metadata only
  FIG_RC=$?
  FIG_PARSED=""
  FIG_GID=""
  FIG_REASON=""
  ...
  if [ "$FIG_RC" -ne 0 ]; then
    FIG_REASON="capture failed (rc=$FIG_RC)"
  elif [ -z "$FIG_RAW" ]; then
    FIG_REASON="empty"
  else
    FIG_PARSED=$(printf '%s\n' "$FIG_RAW" | awk '
    ...
    ')
    FIG_PARSE_RC=$?
    if [ "$FIG_PARSE_RC" -ne 0 ] || [ -z "$FIG_PARSED" ]; then
      FIG_REASON="unparseable"
    else
      FIG_GID=$FIG_PARSED
    fi
  fi
  ...
  if [ -n "$FIG_REASON" ]; then
    ...
    add security fileperm_sshdconfig_group "Secure Shell configuration group" PASS med \
        ...
        "The OpenSSH daemon configuration file has the required group boundary." \
        "n/a" "cis-l1"
  else
