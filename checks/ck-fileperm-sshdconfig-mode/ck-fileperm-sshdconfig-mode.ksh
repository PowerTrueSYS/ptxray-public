...
AIXRAY_TOOL=ck-fileperm-sshdconfig-mode


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_sshdconfig_mode — CIS L1-aligned OpenSSH configuration mode boundary.
  typeset FIM_RAW FIM_RC FIM_PARSED FIM_PARSE_RC FIM_MODE FIM_OK FIM_REASON

  FIM_RAW=$(aix fileperm_sshdconfig_mode_ls ls -ln /etc/ssh/sshd_config) # network-lint: allow -- reads local filesystem metadata only
  FIM_RC=$?
  FIM_PARSED=""
  FIM_MODE=""
  FIM_OK=""
  ...
  if [ "$FIM_RC" -ne 0 ]; then
    FIM_REASON="capture failed (rc=$FIM_RC)"
  elif [ -z "$FIM_RAW" ]; then
    FIM_REASON="empty"
  else
    FIM_PARSED=$(printf '%s\n' "$FIM_RAW" | awk '
      NF {
        ...
        if (!date_ok) bad=1
        if ($NF != "/etc/ssh/sshd_config") bad=1 # network-lint: allow -- validates the captured local pathname only
      }
      END {
    ...
    ')
    FIM_PARSE_RC=$?
    if [ "$FIM_PARSE_RC" -ne 0 ] || [ -z "$FIM_PARSED" ]; then
      FIM_REASON="unparseable"
    else
      FIM_MODE=${FIM_PARSED%%\|*}
      FIM_OK=${FIM_PARSED#*\|}
    fi
  ...
  if [ -n "$FIM_REASON" ]; then
    ...
    add security fileperm_sshdconfig_mode "Secure Shell configuration mode" PASS med \
        ...
        "The OpenSSH daemon configuration file has the required mode boundary." \
        "n/a" "cis-l1"
  else
