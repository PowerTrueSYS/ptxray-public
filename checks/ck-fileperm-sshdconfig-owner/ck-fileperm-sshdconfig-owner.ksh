...
AIXRAY_TOOL=ck-fileperm-sshdconfig-owner


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_sshdconfig_owner — CIS L1-aligned OpenSSH configuration owner boundary.
  typeset FIO_RAW FIO_RC FIO_PARSED FIO_PARSE_RC FIO_UID FIO_REASON

  FIO_RAW=$(aix fileperm_sshdconfig_owner_ls ls -ln /etc/ssh/sshd_config) # network-lint: allow -- reads local filesystem metadata only
  FIO_RC=$?
  FIO_PARSED=""
  FIO_UID=""
  FIO_REASON=""
  ...
  if [ "$FIO_RC" -ne 0 ]; then
    FIO_REASON="capture failed (rc=$FIO_RC)"
  elif [ -z "$FIO_RAW" ]; then
    FIO_REASON="empty"
  else
    FIO_PARSED=$(printf '%s\n' "$FIO_RAW" | awk '
    ...
    ')
    FIO_PARSE_RC=$?
    if [ "$FIO_PARSE_RC" -ne 0 ] || [ -z "$FIO_PARSED" ]; then
      FIO_REASON="unparseable"
    else
      FIO_UID=$FIO_PARSED
    fi
  fi
  ...
  if [ -n "$FIO_REASON" ]; then
    ...
    add security fileperm_sshdconfig_owner "Secure Shell configuration owner" PASS med \
        ...
        "The OpenSSH daemon configuration file has the required owner boundary." \
        "n/a" "cis-l1"
  else
