...
AIXRAY_TOOL=ck-fileperm-submitcf-mode


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_submitcf_mode — CIS L1-aligned /etc/mail/submit.cf mode boundary.
  ...
  # TODO-VERIFY: CIS v1.2.0 item gives chmod only; owner/group facets deferred until the primary CIS PDF is read.
  typeset FSCM_RAW FSCM_RC FSCM_PARSED FSCM_PARSE_RC FSCM_MODE FSCM_OK FSCM_REASON FSCM_ABSENT FSCM_DIAG

  FSCM_RAW=$(aixv fileperm_submitcf_mode_ls ls -ln /etc/mail/submit.cf)
  FSCM_RC=$?
  FSCM_PARSED=""
  FSCM_MODE=""
  FSCM_OK=""
  ...
  if [ "$FSCM_RC" -ne 0 ]; then
    FSCM_REASON="absent or unreadable (rc=$FSCM_RC)"
    case "$FSCM_RAW" in
      *"/etc/mail/submit.cf not found"*) FSCM_ABSENT=1 ;;
    esac
    if [ "$FSCM_ABSENT" -eq 1 ]; then
      FSCM_REASON="absent"
    else
      FSCM_DIAG=$(printf '%s\n' "$FSCM_RAW" | awk 'NF { gsub(/[ \t]+/, " "); sub(/^ /, ""); sub(/ $/, ""); if (length("" $0) > 40) $0 = substr($0, 1, 37) "..."; print; exit }')
      if [ -n "$FSCM_DIAG" ]; then
        FSCM_REASON="absent or unreadable (rc=$FSCM_RC): $FSCM_DIAG"
      fi
    fi
  elif [ -z "$FSCM_RAW" ]; then
    FSCM_REASON="empty"
  else
    FSCM_PARSED=$(printf '%s\n' "$FSCM_RAW" | awk '
      NF {
        records++
        ...
        if (!date_ok) bad=1
        if ($NF != "/etc/mail/submit.cf") bad=1
      }
      END {
    ...
    ')
    FSCM_PARSE_RC=$?
    if [ "$FSCM_PARSE_RC" -ne 0 ] || [ -z "$FSCM_PARSED" ]; then
      FSCM_REASON="unparseable"
    else
      FSCM_MODE=${FSCM_PARSED%%\|*}
      FSCM_OK=${FSCM_PARSED#*\|}
    fi
  fi

  if [ "$FSCM_REASON" = "absent" ]; then
    add security fileperm_submitcf_mode "/etc/mail/submit.cf mode" NOT_APPLICABLE med \
        "/etc/mail/submit.cf absent" \
        "The probe returned the determinate-absence signature ('/etc/mail/submit.cf not found', exit $FSCM_RC); /etc/mail/submit.cf need not exist, so the mode boundary does not apply." \
        "n/a" "cis-l1"
    ...
    add security fileperm_submitcf_mode "/etc/mail/submit.cf mode" PASS med \
        "/etc/mail/submit.cf mode=$FSCM_MODE" \
        "/etc/mail/submit.cf has the required mode boundary." \
        "n/a" "cis-l1"
  else
