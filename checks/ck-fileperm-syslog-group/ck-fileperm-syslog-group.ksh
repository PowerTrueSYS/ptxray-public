...
AIXRAY_TOOL=ck-fileperm-syslog-group


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_syslog_group — CIS Level 1 alignment for /etc/syslog.conf.
  # Read the fixed path exactly once; uncertain evidence is never accepted as PASS.
  typeset FPSG_RAW FPSG_RC FPSG_PARSED FPSG_PARSE_RC FPSG_REST
  typeset FPSG_UID FPSG_GID FPSG_MODE FPSG_MODE_DEC FPSG_REASON
  typeset FPSG_GROUP_OBSERVED

  FPSG_RAW=$(aix fileperm_syslog_group_ls ls -ln /etc/syslog.conf)
  FPSG_RC=$?
  FPSG_PARSED=""
  FPSG_UID=""
  FPSG_GID=""
  ...
  if [ "$FPSG_RC" -ne 0 ]; then
    if [ "$FPSG_RC" -eq 2 ]; then
      FPSG_REASON="missing"
    else
      FPSG_REASON="capture failed (rc=$FPSG_RC)"
    fi
  elif [ -z "$FPSG_RAW" ]; then
    FPSG_REASON="empty"
  else
    FPSG_PARSED=$(printf '%s\n' "$FPSG_RAW" | awk '
      NF {
        records++
        if (records != 1 || NF < 9) {
          bad=1
    ...
    ')
    FPSG_PARSE_RC=$?
    if [ "$FPSG_PARSE_RC" -ne 0 ] || [ -z "$FPSG_PARSED" ]; then
      FPSG_REASON="unparseable"
    else
      FPSG_UID=${FPSG_PARSED%%\|*}
      FPSG_REST=${FPSG_PARSED#*\|}
      FPSG_GID=${FPSG_REST%%\|*}
      FPSG_REST=${FPSG_REST#*\|}
      FPSG_MODE=${FPSG_REST%%\|*}
      FPSG_MODE_DEC=${FPSG_REST#*\|}
    fi
  fi
  ...
  if [ -n "$FPSG_REASON" ]; then
    ...
    if [ "$FPSG_GID" -eq 0 ]; then
      add security fileperm_syslog_group "System log configuration file group" PASS med \
          "$FPSG_GROUP_OBSERVED" \
          "/etc/syslog.conf has the required group boundary." \
          "n/a" "cis-l1"
    else
