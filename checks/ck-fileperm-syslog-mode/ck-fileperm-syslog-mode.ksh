...
AIXRAY_TOOL=ck-fileperm-syslog-mode


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_syslog_mode — CIS Level 1 alignment for /etc/syslog.conf.
  # Read the fixed path exactly once; uncertain evidence is never accepted as PASS.
  typeset FPSM_RAW FPSM_RC FPSM_PARSED FPSM_PARSE_RC FPSM_REST
  typeset FPSM_UID FPSM_GID FPSM_MODE FPSM_MODE_DEC FPSM_REASON
  typeset FPSM_MODE_OBSERVED

  FPSM_RAW=$(aix fileperm_syslog_mode_ls ls -ln /etc/syslog.conf)
  FPSM_RC=$?
  FPSM_PARSED=""
  FPSM_UID=""
  FPSM_GID=""
  ...
  if [ "$FPSM_RC" -ne 0 ]; then
    if [ "$FPSM_RC" -eq 2 ]; then
      FPSM_REASON="missing"
    else
      FPSM_REASON="capture failed (rc=$FPSM_RC)"
    fi
  elif [ -z "$FPSM_RAW" ]; then
    FPSM_REASON="empty"
  else
    FPSM_PARSED=$(printf '%s\n' "$FPSM_RAW" | awk '
      NF {
        records++
        if (records != 1 || NF < 9) {
          bad=1
        ...
        if ($4 !~ /^[0-9][0-9]*$/) bad=1
        if ($5 !~ /^[0-9][0-9]*$/) bad=1
        if ($NF != "/etc/syslog.conf") bad=1
        uid=$3
        gid=$4
      }
      END {
        if (records != 1 || bad) exit 1
        print uid "|" gid "|" sprintf("%04o", bits) "|" bits
      }
    ')
    FPSM_PARSE_RC=$?
    if [ "$FPSM_PARSE_RC" -ne 0 ] || [ -z "$FPSM_PARSED" ]; then
      FPSM_REASON="unparseable"
    else
      FPSM_UID=${FPSM_PARSED%%\|*}
      FPSM_REST=${FPSM_PARSED#*\|}
      FPSM_GID=${FPSM_REST%%\|*}
      FPSM_REST=${FPSM_REST#*\|}
      FPSM_MODE=${FPSM_REST%%\|*}
      FPSM_MODE_DEC=${FPSM_REST#*\|}
    fi
  fi
  ...
  if [ -n "$FPSM_REASON" ]; then
    ...
    # Use decimal 416: ksh93u+ treats a leading zero as octal, while newer
    # ksh93u+m treats it as decimal in arithmetic expressions.
    if [ $((FPSM_MODE_DEC & ~416)) -eq 0 ]; then
      add security fileperm_syslog_mode "System log configuration file mode" PASS med \
          "$FPSM_MODE_OBSERVED" \
          "/etc/syslog.conf has the required mode boundary." \
          "n/a" "cis-l1"
    else
