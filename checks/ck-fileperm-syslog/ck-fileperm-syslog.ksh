...
AIXRAY_TOOL=ck-fileperm-syslog


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileperm_syslog_owner — DISA STIG V-245561.
  # Read the fixed path exactly once; every uncertain evidence shape fails closed.
  typeset FPS_RAW FPS_RC FPS_PARSED FPS_PARSE_RC FPS_UID FPS_GID
  typeset FPS_REASON FPS_OBSERVED

  FPS_RAW=$(aix fileperm_syslog_ls ls -ldn /etc/syslog.conf)
  FPS_RC=$?
  FPS_PARSED=""
  FPS_UID=""
  FPS_GID=""
  ...
  if [ "$FPS_RC" -ne 0 ]; then
    if [ "$FPS_RC" -eq 2 ]; then
      ...
      FPS_REASON="capture failed (rc=$FPS_RC)"
    fi
  elif [ -z "$FPS_RAW" ]; then
    FPS_REASON="empty"
  else
    ...
    ')
    FPS_PARSE_RC=$?
    if [ "$FPS_PARSE_RC" -ne 0 ] || [ -z "$FPS_PARSED" ]; then
      FPS_REASON="unparseable"
    else
      FPS_UID=${FPS_PARSED%%\|*}
      FPS_GID=${FPS_PARSED#*\|}
    fi
  ...
  if [ -n "$FPS_REASON" ]; then
    ...
    add security fileperm_syslog_owner "System log configuration file owner" PASS med \
        "/etc/syslog.conf owner=0 (root)" \
        "/etc/syslog.conf is owned by root as required by DISA STIG V-245561." \
        "n/a" "stig:V-245561"
  else
    FPS_OBSERVED="/etc/syslog.conf owner=$FPS_UID"
