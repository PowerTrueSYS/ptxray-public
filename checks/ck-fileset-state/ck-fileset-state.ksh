...
AIXRAY_TOOL=ck-fileset-state


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # fileset_state -- DISA STIG V-215336 / SV-215336r958936_rule / AIX7-00-003028,
  # CCI-002617: AIX must remove all software components after updated versions
  # have been installed. STIG check literal: 'lslpp -cl | grep :APPLIED:' -- if
  # that returns any entries this is a finding. A fileset left in the APPLIED
  # state still carries its superseded version on disk, so the old code stays
  # installed and runnable alongside the update.
  #
  # Colon fields of 'lslpp -cl':
  #   historic: 1=path 2=fileset 3=level 4=state 5=description
  #   AIX 7.3:  1=path 2=fileset 3=level 4=PTF Id 5=state 6=type 7=description
  #             8=EFIX Locked  (PTF Id and EFIX Locked are often empty)
  # The state word is spelled out here (COMMITTED/APPLIED/BROKEN/...), unlike the
  # one-letter form of 'lslpp -L'. One fileset yields one row per object
  # repository it is installed into, so the counts below are of records, not of
  # distinct filesets, and the evidence strings say so. Read-only: PTxray never runs the 'installp -c
  # all' remediation it recommends. Every uncertain evidence shape fails closed to
  # NOT_ASSESSED; a parseable listing with no fileset rows is the compliant state
  # of this prohibition rule and passes.
  typeset FSS_RAW FSS_RC FSS_PARSED FSS_PARSE_RC FSS_REST
  typeset FSS_TOTAL FSS_APPLIED FSS_LIST FSS_REASON

  FSS_RAW=$(aix fileset_state_lslpp lslpp -cl)
  FSS_RC=$?
  FSS_REASON=""
  FSS_TOTAL=""
  FSS_APPLIED=""
  ...
  if [ "$FSS_RC" -ne 0 ]; then
    FSS_REASON="'lslpp -cl' failed (rc=$FSS_RC)"
  elif [ -z "$FSS_RAW" ]; then
    FSS_REASON="'lslpp -cl' returned no output"
  else
    # A genuine row needs a path, a fileset name, a level bearing a digit and one
    # of AIX's real install-state words. Anything else on a non-header line is
    # unparseable evidence, never silent absence.
    FSS_PARSED=$(printf '%s\n' "$FSS_RAW" | awk -F: '
      BEGIN {
        bad=0; total=0; applied=0; list=""; extra=0
        states=":APPLIED:APPLYING:BROKEN:COMMITTED:COMMITTING:DEINSTALLING:EFIXLOCKED:INCONSISTENT:OBSOLETE:REJECTING:"
      }
      $0 == "" { next }
      substr($0, 1, 1) == "#" { next }
      {
        ...
        if ($3 !~ /[0-9]/) { bad=1; next }
        state=""
        if (index(states, ":" $4 ":") != 0) state=$4
        else if (NF >= 5 && index(states, ":" $5 ":") != 0) state=$5
        else { bad=1; next }
        total++
        if (state == "APPLIED") {
          applied++
      ...
      END {
        if (bad) exit 1
        if (extra > 0) list = list " +" extra " more"
        print total "|" applied "|" list
      }
    ')
    FSS_PARSE_RC=$?
    if [ "$FSS_PARSE_RC" -ne 0 ] || [ -z "$FSS_PARSED" ]; then
      FSS_REASON="'lslpp -cl' output was unparseable"
    else
      FSS_TOTAL=${FSS_PARSED%%\|*}
      FSS_REST=${FSS_PARSED#*\|}
      FSS_APPLIED=${FSS_REST%%\|*}
      FSS_LIST=${FSS_REST#*\|}
    fi
  fi

  if [ -n "$FSS_REASON" ]; then
    add security fileset_state "Uncommitted fileset updates" NOT_ASSESSED med \
        "not assessed - $FSS_REASON" \
        "PTxray did not obtain a parseable fileset listing, so filesets left in the APPLIED state could not be ruled out." \
        "run 'lslpp -cl', correct the capture or command problem, and rerun PTxray." \
        "stig:V-215336"
  elif [ "$FSS_APPLIED" -gt 0 ]; then
    add security fileset_state "Uncommitted fileset updates" FAIL med \
        "$FSS_APPLIED of $FSS_TOTAL fileset records in APPLIED state ($FSS_LIST)" \
        "Filesets in the APPLIED state keep their superseded version installed beside the update, leaving the old software component present and runnable (DISA STIG V-215336)." \
        "commit the applied updates with 'installp -c all' once they are validated; PTxray only recommends this command and never runs it." \
        "stig:V-215336"
  else
    add security fileset_state "Uncommitted fileset updates" PASS med \
        "$FSS_TOTAL fileset records, 0 in APPLIED state" \
        "No fileset is left in the APPLIED state, so no superseded software component remains installed (DISA STIG V-215336)." \
        "n/a" \
        "stig:V-215336"
  fi
}
