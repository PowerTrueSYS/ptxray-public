...
# BEGIN-ck-firmware-uak-expiry-currency-gate
# The HW_GEN table below is the ibm-aix-lifecycle payload. Score against it
# only while that source-registry row is CURRENT; any other status is a typed
# NOT_ASSESSED refusal — never a fallback to the inline table, never a WARN.
# currency_source_status (standalone prelude) prints "STATUS<TAB>reason".
FWUAK_CR_TAB=$(printf '\t')
FWUAK_CR_ROW=$(currency_source_status ibm-aix-lifecycle)
FWUAK_CR_STATUS=${FWUAK_CR_ROW%%"$FWUAK_CR_TAB"*}
FWUAK_CR_REASON=${FWUAK_CR_ROW#*"$FWUAK_CR_TAB"}
if [ -z "$FWUAK_CR_STATUS" ]; then
  FWUAK_CR_STATUS=INVALID
  FWUAK_CR_REASON="currency source status accessor returned no status"
fi
if [ "$FWUAK_CR_STATUS" != "CURRENT" ]; then
  add lifecycle firmware_uak_expiry "Firmware Update Access Key" NOT_ASSESSED med \
      "not assessed - ibm-aix-lifecycle source registry row is $FWUAK_CR_STATUS: $FWUAK_CR_REASON" \
      "The ibm-aix-lifecycle source-registry row this check scores against is not CURRENT ($FWUAK_CR_STATUS: $FWUAK_CR_REASON); the inline lifecycle table cannot be trusted as current, so this finding is refused rather than scored against a stale or unverified reference." \
      "load and verify the ibm-aix-lifecycle source-registry row (fresh, integrity-verified) and rerun this check."
  return 0
fi
# END-ck-firmware-uak-expiry-currency-gate
HW_GEN="
