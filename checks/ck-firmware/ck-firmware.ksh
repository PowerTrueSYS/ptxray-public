...
AIXRAY_TOOL=ck-firmware


function standalone_check {
_AIXRAY_SESSION_KEYS=""
# ck-firmware — the monolith's single "firmware" finding (FROZEN-SOURCE.txt
# 3960-4039) plus the captures and facts it grades over. Emits ONLY the
# "firmware" finding; the monolith's firmware_uak_expiry and hw_gen findings
# are separate tools, so their add() calls are omitted here, and the
# monolith's F_EVIDENCE_COMMANDS assignments are represented by manifest.json
# "commands" instead.

# === BEGIN currency-gate ===
# HW_GEN / FW_MIN below are the payload ibm-aix-lifecycle attests to. Score
# only when that source-registry row is CURRENT; any other status is a typed
# refusal. Never fall back to the inline tables.
CURRENCY_GATE=$(currency_source_status ibm-aix-lifecycle)
CURRENCY_TAB=$(printf '\t')
CURRENCY_STATUS=${CURRENCY_GATE%%"$CURRENCY_TAB"*}
if [ "$CURRENCY_STATUS" != "CURRENT" ]; then
  CURRENCY_REASON=${CURRENCY_GATE#*"$CURRENCY_TAB"}
  add lifecycle firmware "System firmware currency" NOT_ASSESSED med \
      "not assessed - ibm-aix-lifecycle source registry row is $CURRENCY_STATUS: $CURRENCY_REASON" \
      "The ibm-aix-lifecycle source-registry row this check scores against is not CURRENT ($CURRENCY_STATUS: $CURRENCY_REASON); the inline lifecycle table cannot be trusted as current, so this finding is refused rather than scored against a stale or unverified reference." \
      "load and verify the ibm-aix-lifecycle source-registry row (fresh, integrity-verified) and rerun this check." \
      "ffiec:II.C.11 cis-l1"
  return 0
fi
# === END currency-gate ===
  HW_GEN="
...
"
  ...
  # The standalone envelope runs under set -u; the monolith initialises its
  # fact store outside this slice, so promote the three facts this fragment
  # reads on paths where they are not otherwise assigned.
  FACT_FW_UAK_EXPIRY=""
  FACT_HW_GEN=""
  FACT_HW_EOS=""
  ...
  add lifecycle firmware "System firmware currency" "$FWST" "$FWSEV" "$FWOBS" "$FWMEAN" "$FWFIX" "ffiec:II.C.11 cis-l1"
}
