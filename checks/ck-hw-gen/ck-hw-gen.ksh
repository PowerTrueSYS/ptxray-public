...
AIXRAY_TOOL=ck-hw-gen


function standalone_check {
_AIXRAY_SESSION_KEYS=""
# Curator review 2026-09-19: exact IBM lifecycle records, not machine families.
# https://www.ibm.com/support/pages/node/ followed by each numeric ID:
# 8202-E4D:7093434 8231-E2B:7093685 8231-E2D:7093686
# 8284-21A:7093445 8286-42A:7093693 9008-22L:7093103
# 9009-22A:7093104 9009-41A:7093106 9009-42A:7093105
# 9223-22H:7093108 9223-42H:7093107 9223-22S:7093022
# 9080-M9S:7093110 9080-HEX:7094334 9105-22A:7235648 9105-42A:7094490
# SUPPORTED means no announced EOS on the reviewed record (curator inference).
HW_GEN="
...
# === BEGIN currency-gate ===
# Score the inline HW_GEN table only while ibm-aix-lifecycle is CURRENT.
# Other statuses refuse; never fall back to the table, never WARN.
HWGEN_CUR_ROW=$(currency_source_status ibm-aix-lifecycle)
HWGEN_CUR_STATUS=$(printf '%s\n' "$HWGEN_CUR_ROW" | awk -F '\t' '{print $1; exit}')
HWGEN_CUR_REASON=$(printf '%s\n' "$HWGEN_CUR_ROW" | awk -F '\t' '{print $2; exit}')
if [ -z "$HWGEN_CUR_STATUS" ]; then
  HWGEN_CUR_STATUS=INVALID
  HWGEN_CUR_REASON="currency source status accessor returned no status"
fi
if [ "$HWGEN_CUR_STATUS" != CURRENT ]; then
  add lifecycle hw_gen "Hardware generation" NOT_ASSESSED med \
      "not assessed - ibm-aix-lifecycle source registry row is $HWGEN_CUR_STATUS: $HWGEN_CUR_REASON" \
      "The ibm-aix-lifecycle source registry row is not CURRENT, so the bundled hardware-generation table has no attested vintage; this check does not score against a table of unknown age." \
      "Refresh the ibm-aix-lifecycle source registry row to a verified current state and re-run this check." "ffiec:II.C.11"
  return 0
fi
# === END currency-gate ===
...
if [ "$PRTCONF_RC" -ne 0 ]; then PRTCONF=""; fi
  ...
  if [ "$PRTCONF_RC" -ne 0 ] || [ -z "$PRTCONF" ] || [ -z "$TM" ]; then
    ...
    if [ -z "$ROW" ]; then
      add lifecycle hw_gen "Hardware generation" NOT_ASSESSED med "$TM" \
          "Machine type not in the bundled reference data." \
