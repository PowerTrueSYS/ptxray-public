...
AIXRAY_TOOL=ck-inetd-chargen


function standalone_check {
_AIXRAY_SESSION_KEYS=""
# inetd_chargen — detect whether the legacy chargen service is active in
  # /etc/inetd.conf.  An active chargen entry exposes a clear-text service
  # that can be abused for amplification or as a foothold; CIS requires it
  # disabled.  Capture: grep for the exact first-field token chargen followed
  # by whitespace (not a comment, not chargen-new, not a later-field match).
  #
  # Verdict logic:
  #   rc=0  AND  every nonblank row starts with exactly "chargen "  => FAIL
  #   rc=1  AND  empty output                                       => PASS
  #   anything else                                                 => NOT_ASSESSED
  #
  typeset IC_STATUS IC_RAW IC_RC IC_MATCH IC_OBSERVED IC_MEANING IC_FIX SEVERITY
  ...
  IC_RAW=$(aix inetd_chargen grep -E '^[	 ]*chargen[	 ]' /etc/inetd.conf) # network-lint: allow -- reads the LOCAL /etc/inetd.conf capture text (via the aix wrapper), never a network call
  IC_RC=$?

  case "$IC_RC" in
    0)
      # grep found at least one matching line — verify the first field is exactly 'chargen'
      IC_MATCH=$(printf '%s\n' "$IC_RAW" | awk '
        /^[ \t]*$/ { next }
        {
          line = $0
      ...
      ;;
    1)
      # grep found nothing — clean absence
      if [ -z "$IC_RAW" ]; then
        IC_STATUS=PASS
        IC_OBSERVED="chargen not active in /etc/inetd.conf"
        IC_MEANING="The service is not enabled."
        IC_FIX="n/a"
      else
        IC_STATUS=NOT_ASSESSED
  ...
  add security inetd_chargen "inetd chargen service" \
    "$IC_STATUS" "$SEVERITY" \
    "$IC_OBSERVED" \
    "$IC_MEANING" \
    "$IC_FIX" \
