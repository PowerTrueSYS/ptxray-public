...
AIXRAY_TOOL=ck-inetd-cleartext


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # inetd_cleartext
  INE=$(aix inetd_conf grep -E '^[[:space:]]*(telnet|telnetd|ftp|ftpd|shell|login|exec|rsh|rlogin)[[:space:]]' /etc/inetd.conf) # network-lint: allow -- reads the LOCAL /etc/inetd.conf capture text (via the aix wrapper), never a network call
  INE_RC=$?
  ...
  case "$INE_RC" in
    0)
      if [ -n "$INE" ]; then
        ...
        add security inetd_cleartext "Cleartext inetd services" NOT_ASSESSED low "not assessed — /etc/inetd.conf could not be read (rc=$INE_RC)" \
            "PTxray did not obtain trustworthy service-definition evidence." \
            "inspect /etc/inetd.conf, resolve the read or shape problem, and rerun PTxray." "stig:V-215258 stig:V-215259 cis-l1"
      fi
      ;;
    1)
      if [ -z "$INE" ]; then
