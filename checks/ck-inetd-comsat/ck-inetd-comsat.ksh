...
AIXRAY_TOOL=ck-inetd-comsat


function standalone_check {
_AIXRAY_SESSION_KEYS=""

  # inetd_comsat — detect whether the legacy comsat service is active in
  # /etc/inetd.conf.  An active comsat entry exposes a clear-text service
  # that can be abused for amplification or as a foothold; CIS requires it
  # disabled.  Capture: grep for the exact first-field token comsat followed
  # by whitespace (not a comment, not comsatd, not a later-field match).
  #
  # Verdict logic:
  #   rc=0  AND  every nonblank row starts with exactly "comsat "  => FAIL
  #   rc=1  AND  empty output                                       => PASS
  #   anything else                                                 => NOT_ASSESSED
  #
  typeset ID_COMSAT_RC ID_COMSAT_RAW ID_COMSAT_PARSED ID_COMSAT_SHAPE
  typeset ID_COMSAT_STATUS ID_COMSAT_OBSERVED ID_COMSAT_MEANING
  typeset ID_COMSAT_FIX SEVERITY
  ID_COMSAT_RC=0
  ID_COMSAT_RAW=""
  ID_COMSAT_PARSED="absent"
  ID_COMSAT_SHAPE=1

  ID_COMSAT_RAW=$(aix inetd_comsat grep -E '^[	 ]*comsat[	 ]' /etc/inetd.conf)
  ID_COMSAT_RC=$?

  if [ "$ID_COMSAT_RC" -eq 0 ] && [ -n "$ID_COMSAT_RAW" ]; then
    # Validate that every nonblank row has first field exactly "comsat"
    # and is well-shaped (has at least a service name, socket type, protocol,
    # wait flag, user, path — ≥6 nonblank fields).
    ID_COMSAT_PARSED=$(printf '%s\n' "$ID_COMSAT_RAW" | awk '
      /^[ \t]*$/ { next }
      {
        rows++
    ...
    case "$ID_COMSAT_PARSED" in
      single) ID_COMSAT_SHAPE=1 ;;
      multiple) ID_COMSAT_SHAPE=1 ;;
      *) ID_COMSAT_SHAPE=0 ;;
    esac
  fi

  # Compute verdict, then severity, then emit single finding.
  case "$ID_COMSAT_RC" in
    0)
      if [ "$ID_COMSAT_SHAPE" -eq 1 ] && [ "$ID_COMSAT_PARSED" != "absent" ]; then
        ...
        case "$ID_COMSAT_PARSED" in
          non-comsat-first)
            ID_COMSAT_OBSERVED="not assessed - comsat inetd probe captured output contains non-comsat-first rows" ;;
          malformed)
            ID_COMSAT_OBSERVED="not assessed - comsat inetd probe captured rows are malformed" ;;
          *)
      ...
      ;;
    1)
      if [ -z "$ID_COMSAT_RAW" ]; then
        ID_COMSAT_STATUS=PASS
        ID_COMSAT_OBSERVED="comsat not active in /etc/inetd.conf"
        ID_COMSAT_MEANING="The inetd service is not enabled."
        ID_COMSAT_FIX="n/a"
      else
        ID_COMSAT_STATUS=NOT_ASSESSED
  ...
  case "$ID_COMSAT_STATUS" in
    FAIL) SEVERITY=high ;;
    *) SEVERITY=low ;;
  esac

  add security inetd_comsat "inetd comsat service" \
    "$ID_COMSAT_STATUS" "$SEVERITY" \
    "$ID_COMSAT_OBSERVED" \
    "$ID_COMSAT_MEANING" \
    "$ID_COMSAT_FIX" \
    "cis-l1"
