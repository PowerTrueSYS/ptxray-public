...
AIXRAY_TOOL=ck-inetd-daytime


function standalone_check {
_AIXRAY_SESSION_KEYS=""

  # inetd_daytime — detect whether the legacy daytime service is active in
  # /etc/inetd.conf.  An active daytime entry exposes a clear-text service
  # that can be abused for amplification or as a foothold; CIS requires it
  # disabled.  Capture: grep for the exact first-field token daytime followed
  # by whitespace (not a comment, not daytimed, not a later-field match).
  #
  # Verdict logic:
  #   rc=0  AND  every nonblank row starts with exactly "daytime "  => FAIL
  #   rc=1  AND  empty output                                       => PASS
  #   anything else                                                 => NOT_ASSESSED
  #
  typeset ID_DAYTIME_RC ID_DAYTIME_RAW ID_DAYTIME_PARSED ID_DAYTIME_SHAPE
  typeset ID_DAYTIME_STATUS ID_DAYTIME_OBSERVED ID_DAYTIME_MEANING
  typeset ID_DAYTIME_FIX SEVERITY
  ID_DAYTIME_RC=0
  ID_DAYTIME_RAW=""
  ID_DAYTIME_PARSED="absent"
  ID_DAYTIME_SHAPE=1

  ID_DAYTIME_RAW=$(aix inetd_daytime grep -E '^[	 ]*daytime[	 ]' /etc/inetd.conf)
  ID_DAYTIME_RC=$?

  if [ "$ID_DAYTIME_RC" -eq 0 ] && [ -n "$ID_DAYTIME_RAW" ]; then
    # Validate that every nonblank row has first field exactly "daytime"
    # and is well-shaped (has at least a service name, socket type, protocol,
    # wait flag, user, path — ≥6 nonblank fields).
    ID_DAYTIME_PARSED=$(printf '%s\n' "$ID_DAYTIME_RAW" | awk '
      /^[ \t]*$/ { next }
      {
        rows++
    ...
    case "$ID_DAYTIME_PARSED" in
      single) ID_DAYTIME_SHAPE=1 ;;
      multiple) ID_DAYTIME_SHAPE=1 ;;
      *) ID_DAYTIME_SHAPE=0 ;;
    esac
  fi

  # Compute verdict, then severity, then emit single finding.
  case "$ID_DAYTIME_RC" in
    0)
      if [ "$ID_DAYTIME_SHAPE" -eq 1 ] && [ "$ID_DAYTIME_PARSED" != "absent" ]; then
        ...
        case "$ID_DAYTIME_PARSED" in
          non-daytime-first)
            ID_DAYTIME_OBSERVED="not assessed - daytime inetd probe captured output contains non-daytime-first rows" ;;
          malformed)
            ID_DAYTIME_OBSERVED="not assessed - daytime inetd probe captured rows are malformed" ;;
          *)
      ...
      ;;
    1)
      if [ -z "$ID_DAYTIME_RAW" ]; then
        ID_DAYTIME_STATUS=PASS
        ID_DAYTIME_OBSERVED="daytime not active in /etc/inetd.conf"
        ID_DAYTIME_MEANING="The inetd service is not enabled."
        ID_DAYTIME_FIX="n/a"
      else
        ID_DAYTIME_STATUS=NOT_ASSESSED
  ...
  case "$ID_DAYTIME_STATUS" in
    FAIL) SEVERITY=high ;;
    *) SEVERITY=low ;;
  esac

  add security inetd_daytime "inetd daytime service" \
    "$ID_DAYTIME_STATUS" "$SEVERITY" \
    "$ID_DAYTIME_OBSERVED" \
    "$ID_DAYTIME_MEANING" \
    "$ID_DAYTIME_FIX" \
    "cis-l1"
