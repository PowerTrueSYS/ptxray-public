...
AIXRAY_TOOL=ck-inetd-discard


function standalone_check {
_AIXRAY_SESSION_KEYS=""
# inetd_discard — legacy discard service active in /etc/inetd.conf.
# The 'discard' service echoes and discards any data sent to it; an active
# definition is a remote-attack surface vector with zero legitimate use in
# modern deployments. CIS AIX baseline requires it disabled. Capture:
# grep for the exact first-field token 'discard' followed by whitespace.

  IDISC=$(aix inetd_discard grep -E '^[[:space:]]*discard[[:space:]]' /etc/inetd.conf); IDISC_RC=$?
  IDISC_LINE=""
  if [ "$IDISC_RC" -eq 0 ] && [ -n "$IDISC" ]; then
    # Verify the first field is exactly 'discard' (no leading comment marker).
    IDISC_LINE=$(printf '%s\n' "$IDISC" | awk '/^[[:space:]]*discard[[:space:]]/ { print; exit }')
  fi
  if [ "$IDISC_RC" -gt 1 ]; then
    ...
    add security inetd_discard "Legacy inetd discard service" PASS high \
        ...
        "The service is not enabled." "n/a" "cis-l1"
  elif [ "$IDISC_RC" -eq 1 ] && [ -n "$IDISC" ]; then
    # rc=1 with output is contradictory; treat as inconclusive.
    add security inetd_discard "Legacy inetd discard service" NOT_ASSESSED high \
