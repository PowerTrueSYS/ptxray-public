...
AIXRAY_TOOL=ck-inetd-echo


function standalone_check {
_AIXRAY_SESSION_KEYS=""

  # inetd_echo — detect whether the legacy echo service is active in
  # /etc/inetd.conf.  An active echo entry exposes a clear-text service
  # that can be abused for amplification or as a foothold; CIS requires it
  # disabled.  Capture: grep for the exact first-field token echo followed
  # by whitespace (not a comment, not echod, not a later-field match).
  #
  # Verdict logic:
  #   rc=0  AND  every nonblank row starts with exactly "echo "  => FAIL
  #   rc=1  AND  empty output                                       => PASS
  #   anything else                                                 => NOT_ASSESSED
  #
  typeset ID_ECHO_RC ID_ECHO_RAW ID_ECHO_PARSED ID_ECHO_SHAPE
  typeset ID_ECHO_STATUS ID_ECHO_OBSERVED ID_ECHO_MEANING
  typeset ID_ECHO_FIX SEVERITY
  ID_ECHO_RC=0
  ID_ECHO_RAW=""
  ID_ECHO_PARSED="absent"
  ID_ECHO_SHAPE=1

  ID_ECHO_RAW=$(aix inetd_echo grep -E '^[	 ]*echo[	 ]' /etc/inetd.conf)
  ID_ECHO_RC=$?

  if [ "$ID_ECHO_RC" -eq 0 ] && [ -n "$ID_ECHO_RAW" ]; then
    # Validate that every nonblank row has first field exactly "echo"
    # and is well-shaped (has at least a service name, socket type, protocol,
    # wait flag, user, path — ≥6 nonblank fields).
    ID_ECHO_PARSED=$(printf '%s\n' "$ID_ECHO_RAW" | awk '
      /^[ \t]*$/ { next }
      {
        rows++
    ...
    case "$ID_ECHO_PARSED" in
      single) ID_ECHO_SHAPE=1 ;;
      multiple) ID_ECHO_SHAPE=1 ;;
      *) ID_ECHO_SHAPE=0 ;;
    esac
  fi
  ...
  case "$ID_ECHO_RC" in
    0)
      if [ "$ID_ECHO_SHAPE" -eq 1 ] && [ "$ID_ECHO_PARSED" != "absent" ]; then
        ID_ECHO_STATUS=FAIL
        ...
        case "$ID_ECHO_PARSED" in
          non-echo-first)
            ID_ECHO_OBSERVED="not assessed - echo inetd probe captured output contains non-echo-first rows" ;;
          malformed)
            ID_ECHO_OBSERVED="not assessed - echo inetd probe captured rows are malformed" ;;
          *)
      ...
      ;;
    1)
      if [ -z "$ID_ECHO_RAW" ]; then
        ID_ECHO_STATUS=PASS
        ID_ECHO_OBSERVED="echo not active in /etc/inetd.conf"
        ID_ECHO_MEANING="The inetd service is not enabled."
        ID_ECHO_FIX="n/a"
      else
        ID_ECHO_STATUS=NOT_ASSESSED
  ...
  case "$ID_ECHO_STATUS" in
    FAIL) SEVERITY=high ;;
    *) SEVERITY=low ;;
  esac

  add security inetd_echo "inetd echo service" \
    "$ID_ECHO_STATUS" "$SEVERITY" \
    "$ID_ECHO_OBSERVED" \
    "$ID_ECHO_MEANING" \
    "$ID_ECHO_FIX" \
    "cis-l1"
