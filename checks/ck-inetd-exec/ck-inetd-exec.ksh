...
AIXRAY_TOOL=ck-inetd-exec


function standalone_check {
_AIXRAY_SESSION_KEYS=""

  # inetd_exec — detect whether the legacy exec service is active in
  # /etc/inetd.conf.  An active exec entry exposes a clear-text service
  # that can be abused for amplification or as a foothold; CIS requires it
  # disabled.  Capture: grep for the exact first-field token exec followed
  # by whitespace (not a comment, not execd, not a later-field match).
  #
  # Verdict logic:
  #   rc=0  AND  every nonblank row starts with exactly "exec "  => FAIL
  #   rc=1  AND  empty output                                       => PASS
  #   anything else                                                 => NOT_ASSESSED
  #
  typeset ID_EXEC_RC ID_EXEC_RAW ID_EXEC_PARSED ID_EXEC_SHAPE
  typeset ID_EXEC_STATUS ID_EXEC_OBSERVED ID_EXEC_MEANING
  typeset ID_EXEC_FIX SEVERITY
  ID_EXEC_RC=0
  ID_EXEC_RAW=""
  ID_EXEC_PARSED="absent"
  ID_EXEC_SHAPE=1

  ID_EXEC_RAW=$(aix inetd_exec grep -E '^[	 ]*exec[	 ]' /etc/inetd.conf)
  ID_EXEC_RC=$?

  if [ "$ID_EXEC_RC" -eq 0 ] && [ -n "$ID_EXEC_RAW" ]; then
    # Validate that every nonblank row has first field exactly "exec"
    # and is well-shaped (has at least a service name, socket type, protocol,
    # wait flag, user, path — ≥6 nonblank fields).
    ID_EXEC_PARSED=$(printf '%s\n' "$ID_EXEC_RAW" | awk '
      /^[ \t]*$/ { next }
      {
        rows++
    ...
    case "$ID_EXEC_PARSED" in
      single) ID_EXEC_SHAPE=1 ;;
      multiple) ID_EXEC_SHAPE=1 ;;
      *) ID_EXEC_SHAPE=0 ;;
    esac
  fi
  ...
  case "$ID_EXEC_RC" in
    0)
      if [ "$ID_EXEC_SHAPE" -eq 1 ] && [ "$ID_EXEC_PARSED" != "absent" ]; then
        ID_EXEC_STATUS=FAIL
        ...
        case "$ID_EXEC_PARSED" in
          non-exec-first)
            ID_EXEC_OBSERVED="not assessed - exec inetd probe captured output contains non-exec-first rows" ;;
          malformed)
            ID_EXEC_OBSERVED="not assessed - exec inetd probe captured rows are malformed" ;;
          *)
      ...
      ;;
    1)
      if [ -z "$ID_EXEC_RAW" ]; then
        ID_EXEC_STATUS=PASS
        ID_EXEC_OBSERVED="exec not active in /etc/inetd.conf"
        ID_EXEC_MEANING="The inetd service is not enabled."
        ID_EXEC_FIX="n/a"
      else
        ID_EXEC_STATUS=NOT_ASSESSED
  ...
  case "$ID_EXEC_STATUS" in
    FAIL) SEVERITY=high ;;
    *) SEVERITY=low ;;
  esac

  add security inetd_exec "inetd exec service" \
    "$ID_EXEC_STATUS" "$SEVERITY" \
    "$ID_EXEC_OBSERVED" \
    "$ID_EXEC_MEANING" \
    "$ID_EXEC_FIX" \
    "cis-l1"
