...
AIXRAY_TOOL=ck-inetd-finger


function standalone_check {
_AIXRAY_SESSION_KEYS=""

  # inetd_finger — detect whether the legacy finger service is active in
  # /etc/inetd.conf.  An active finger entry exposes a clear-text service
  # that can be abused for amplification or as a foothold; CIS requires it
  # disabled.  Capture: grep for the exact first-field token finger followed
  # by whitespace (not a comment, not fingerd, not a later-field match).
  #
  # Verdict logic:
  #   rc=0  AND  every nonblank row starts with exactly "finger "  => FAIL
  #   rc=1  AND  empty output                                       => PASS
  #   anything else                                                 => NOT_ASSESSED
  #
  typeset ID_FINGER_RC ID_FINGER_RAW ID_FINGER_PARSED ID_FINGER_SHAPE
  typeset ID_FINGER_STATUS ID_FINGER_OBSERVED ID_FINGER_MEANING
  typeset ID_FINGER_FIX SEVERITY
  ID_FINGER_RC=0
  ID_FINGER_RAW=""
  ID_FINGER_PARSED="absent"
  ID_FINGER_SHAPE=1

  ID_FINGER_RAW=$(aix inetd_finger grep -E '^[	 ]*finger[	 ]' /etc/inetd.conf)
  ID_FINGER_RC=$?

  if [ "$ID_FINGER_RC" -eq 0 ] && [ -n "$ID_FINGER_RAW" ]; then
    # Validate that every nonblank row has first field exactly "finger"
    # and is well-shaped (has at least a service name, socket type, protocol,
    # wait flag, user, path — ≥6 nonblank fields).
    ID_FINGER_PARSED=$(printf '%s\n' "$ID_FINGER_RAW" | awk '
      /^[ \t]*$/ { next }
      {
        rows++
    ...
    case "$ID_FINGER_PARSED" in
      single) ID_FINGER_SHAPE=1 ;;
      multiple) ID_FINGER_SHAPE=1 ;;
      *) ID_FINGER_SHAPE=0 ;;
    esac
  fi

  # Compute verdict, then severity, then emit single finding.
  case "$ID_FINGER_RC" in
    0)
      if [ "$ID_FINGER_SHAPE" -eq 1 ] && [ "$ID_FINGER_PARSED" != "absent" ]; then
        ...
        case "$ID_FINGER_PARSED" in
          non-finger-first)
            ID_FINGER_OBSERVED="not assessed - finger inetd probe captured output contains non-finger-first rows" ;;
          malformed)
            ID_FINGER_OBSERVED="not assessed - finger inetd probe captured rows are malformed" ;;
          *)
      ...
      ;;
    1)
      if [ -z "$ID_FINGER_RAW" ]; then
        ID_FINGER_STATUS=PASS
        ID_FINGER_OBSERVED="finger not active in /etc/inetd.conf"
        ID_FINGER_MEANING="The inetd service is not enabled."
        ID_FINGER_FIX="n/a"
      else
        ID_FINGER_STATUS=NOT_ASSESSED
  ...
  case "$ID_FINGER_STATUS" in
    FAIL) SEVERITY=high ;;
    *) SEVERITY=low ;;
  esac

  add security inetd_finger "inetd finger service" \
    "$ID_FINGER_STATUS" "$SEVERITY" \
    "$ID_FINGER_OBSERVED" \
    "$ID_FINGER_MEANING" \
    "$ID_FINGER_FIX" \
    "cis-l1"
