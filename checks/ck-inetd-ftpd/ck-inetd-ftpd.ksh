...
AIXRAY_TOOL=ck-inetd-ftpd


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # inetd_ftpd — detect active FTP service definitions from the shared
  # /etc/inetd.conf capture. The shared capture retains tcp/tcp6 rows and the
  # parser accepts both ftp/ftpd service spellings; commented rows are ignored. # network-lint: allow -- documents the inetd service tokens the parser accepts, not a network call
  typeset ID_FTPD_RC ID_FTPD_RAW ID_FTPD_PARSED ID_FTPD_STATUS
  typeset ID_FTPD_OBSERVED ID_FTPD_MEANING ID_FTPD_FIX SEVERITY
  ID_FTPD_RAW=$(aix inetd_conf grep -E '^[[:space:]]*(telnet|telnetd|ftp|ftpd|shell|login|exec|rsh|rlogin)[[:space:]]' /etc/inetd.conf) # network-lint: allow -- reads the LOCAL /etc/inetd.conf capture text (via the aix wrapper), never a network call
  ID_FTPD_RC=$?
  ID_FTPD_PARSED=absent

  if [ "$ID_FTPD_RC" -eq 0 ] && [ -n "$ID_FTPD_RAW" ]; then
    ID_FTPD_PARSED=$(printf '%s\n' "$ID_FTPD_RAW" | awk '
      /^[ \t]*#/ || /^[ \t]*$/ { next }
      {
        line=$0
        sub(/^[ \t]+/, "", line)
        split(line, fields, /[ \t]+/)
        first=fields[1]
        if (first == "ftp" || first == "ftpd") active=1 # network-lint: allow -- compares an inetd.conf service token, not a network call
      }
      END { print active ? "active" : "absent" }
    ')
  fi
  ...
  case "$ID_FTPD_RC" in
    0)
      if [ -z "$ID_FTPD_RAW" ]; then
        ...
        ID_FTPD_MEANING="The inetd service is not enabled."
        ID_FTPD_FIX="n/a"
      fi
      ;;
    1)
  ...
  case "$ID_FTPD_STATUS" in FAIL) SEVERITY=high ;; *) SEVERITY=low ;; esac
  add security inetd_ftpd "inetd ftpd service" \
    "$ID_FTPD_STATUS" "$SEVERITY" "$ID_FTPD_OBSERVED" \
    "$ID_FTPD_MEANING" "$ID_FTPD_FIX" "cis-l1"
}
