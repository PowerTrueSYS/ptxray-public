...
AIXRAY_TOOL=ck-inetd-imap2


function standalone_check {
_AIXRAY_SESSION_KEYS=""

  # inetd_imap2 — detect whether the legacy imap2 service is active in
  # /etc/inetd.conf.  An active imap2 entry exposes a clear-text service
  # that can be abused for amplification or as a foothold; CIS requires it
  # disabled.  Capture: grep for the exact first-field token imap2 followed
  # by whitespace (not a comment, not imap2d, not a later-field match).
  #
  # Verdict logic:
  #   rc=0  AND  every nonblank row starts with exactly "imap2 "  => FAIL
  #   rc=1  AND  empty output                                       => PASS
  #   anything else                                                 => NOT_ASSESSED
  #
  typeset ID_IMAP2_RC ID_IMAP2_RAW ID_IMAP2_PARSED ID_IMAP2_SHAPE
  typeset ID_IMAP2_STATUS ID_IMAP2_OBSERVED ID_IMAP2_MEANING
  typeset ID_IMAP2_FIX SEVERITY
  ID_IMAP2_RC=0
  ID_IMAP2_RAW=""
  ID_IMAP2_PARSED="absent"
  ID_IMAP2_SHAPE=1

  ID_IMAP2_RAW=$(aix inetd_imap2 grep -E '^[	 ]*imap2[	 ]' /etc/inetd.conf)
  ID_IMAP2_RC=$?

  if [ "$ID_IMAP2_RC" -eq 0 ] && [ -n "$ID_IMAP2_RAW" ]; then
    # Validate that every nonblank row has first field exactly "imap2"
    # and is well-shaped (has at least a service name, socket type, protocol,
    # wait flag, user, path — ≥6 nonblank fields).
    ID_IMAP2_PARSED=$(printf '%s\n' "$ID_IMAP2_RAW" | awk '
      /^[ \t]*$/ { next }
      {
        rows++
    ...
    case "$ID_IMAP2_PARSED" in
      single) ID_IMAP2_SHAPE=1 ;;
      multiple) ID_IMAP2_SHAPE=1 ;;
      *) ID_IMAP2_SHAPE=0 ;;
    esac
  fi
  ...
  case "$ID_IMAP2_RC" in
    0)
      if [ "$ID_IMAP2_SHAPE" -eq 1 ] && [ "$ID_IMAP2_PARSED" != "absent" ]; then
        ...
        case "$ID_IMAP2_PARSED" in
          non-imap2-first)
            ID_IMAP2_OBSERVED="not assessed - imap2 inetd probe captured output contains non-imap2-first rows" ;;
          malformed)
            ID_IMAP2_OBSERVED="not assessed - imap2 inetd probe captured rows are malformed" ;;
          *)
      ...
      ;;
    1)
      if [ -z "$ID_IMAP2_RAW" ]; then
        ID_IMAP2_STATUS=PASS
        ID_IMAP2_OBSERVED="imap2 not active in /etc/inetd.conf"
        ID_IMAP2_MEANING="The inetd service is not enabled."
        ID_IMAP2_FIX="n/a"
      else
        ID_IMAP2_STATUS=NOT_ASSESSED
  ...
  case "$ID_IMAP2_STATUS" in
    FAIL) SEVERITY=high ;;
    *) SEVERITY=low ;;
  esac

  add security inetd_imap2 "inetd imap2 service" \
    "$ID_IMAP2_STATUS" "$SEVERITY" \
    "$ID_IMAP2_OBSERVED" \
    "$ID_IMAP2_MEANING" \
    "$ID_IMAP2_FIX" \
    "cis-l1"
