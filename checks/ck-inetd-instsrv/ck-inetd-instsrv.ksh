...
AIXRAY_TOOL=ck-inetd-instsrv


function standalone_check {
_AIXRAY_SESSION_KEYS=""

  # inetd_instsrv — detect whether the legacy instsrv service is active in
  # /etc/inetd.conf.  An active instsrv entry exposes a clear-text service
  # that can be abused for amplification or as a foothold; CIS requires it
  # disabled.  Capture: grep for the exact first-field token instsrv followed
  # by whitespace (not a comment, not instsrvd, not a later-field match).
  #
  # Verdict logic:
  #   rc=0  AND  every nonblank row starts with exactly "instsrv "  => FAIL
  #   rc=1  AND  empty output                                       => PASS
  #   anything else                                                 => NOT_ASSESSED
  #
  typeset ID_INSTSRV_RC ID_INSTSRV_RAW ID_INSTSRV_PARSED ID_INSTSRV_SHAPE
  typeset ID_INSTSRV_STATUS ID_INSTSRV_OBSERVED ID_INSTSRV_MEANING
  typeset ID_INSTSRV_FIX SEVERITY
  ID_INSTSRV_RC=0
  ID_INSTSRV_RAW=""
  ID_INSTSRV_PARSED="absent"
  ID_INSTSRV_SHAPE=1

  ID_INSTSRV_RAW=$(aix inetd_instsrv grep -E '^[	 ]*instsrv[	 ]' /etc/inetd.conf)
  ID_INSTSRV_RC=$?

  if [ "$ID_INSTSRV_RC" -eq 0 ] && [ -n "$ID_INSTSRV_RAW" ]; then
    # Validate that every nonblank row has first field exactly "instsrv"
    # and is well-shaped (has at least a service name, socket type, protocol,
    # wait flag, user, path — ≥6 nonblank fields).
    ID_INSTSRV_PARSED=$(printf '%s\n' "$ID_INSTSRV_RAW" | awk '
      /^[ \t]*$/ { next }
      {
        rows++
        # Strip leading whitespace and check first field
        line = $0
        sub(/^[ \t]+/, "", line)
        split(line, fields, /[ \t]+/)
        if (fields[1] != "instsrv") {
    ...
    case "$ID_INSTSRV_PARSED" in
      single) ID_INSTSRV_SHAPE=1 ;;
      multiple) ID_INSTSRV_SHAPE=1 ;;
      *) ID_INSTSRV_SHAPE=0 ;;
    esac
  fi

  # Compute verdict, then severity, then emit single finding.
  case "$ID_INSTSRV_RC" in
    0)
      if [ "$ID_INSTSRV_SHAPE" -eq 1 ] && [ "$ID_INSTSRV_PARSED" != "absent" ]; then
        ...
        case "$ID_INSTSRV_PARSED" in
          non-instsrv-first)
            ID_INSTSRV_OBSERVED="not assessed - instsrv inetd probe captured output contains non-instsrv-first rows" ;;
          malformed)
            ID_INSTSRV_OBSERVED="not assessed - instsrv inetd probe captured rows are malformed" ;;
          *)
      ...
      ;;
    1)
      if [ -z "$ID_INSTSRV_RAW" ]; then
        ID_INSTSRV_STATUS=PASS
        ID_INSTSRV_OBSERVED="instsrv not active in /etc/inetd.conf"
        ID_INSTSRV_MEANING="The inetd service is not enabled."
        ID_INSTSRV_FIX="n/a"
      else
        ID_INSTSRV_STATUS=NOT_ASSESSED
  ...
  case "$ID_INSTSRV_STATUS" in
    FAIL) SEVERITY=high ;;
    *) SEVERITY=low ;;
  esac

  add security inetd_instsrv "inetd instsrv service" \
    "$ID_INSTSRV_STATUS" "$SEVERITY" \
    "$ID_INSTSRV_OBSERVED" \
    "$ID_INSTSRV_MEANING" \
    "$ID_INSTSRV_FIX" \
    "cis-l1"
