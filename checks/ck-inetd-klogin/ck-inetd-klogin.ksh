...
AIXRAY_TOOL=ck-inetd-klogin


function standalone_check {
_AIXRAY_SESSION_KEYS=""

  # inetd_klogin — detect whether the legacy klogin service is active in
  # /etc/inetd.conf.  An active klogin entry exposes a clear-text service
  # that can be abused for amplification or as a foothold; CIS requires it
  # disabled.  Capture: grep for the exact first-field token klogin followed
  # by whitespace (not a comment, not klogind, not a later-field match).
  #
  # Verdict logic:
  #   rc=0  AND  every nonblank row starts with exactly "klogin "  => FAIL
  #   rc=1  AND  empty output                                       => PASS
  #   anything else                                                 => NOT_ASSESSED
  #
  typeset ID_KLOGIN_RC ID_KLOGIN_RAW ID_KLOGIN_PARSED ID_KLOGIN_SHAPE
  typeset ID_KLOGIN_STATUS ID_KLOGIN_OBSERVED ID_KLOGIN_MEANING
  typeset ID_KLOGIN_FIX SEVERITY
  ID_KLOGIN_RC=0
  ID_KLOGIN_RAW=""
  ID_KLOGIN_PARSED="absent"
  ID_KLOGIN_SHAPE=1

  ID_KLOGIN_RAW=$(aix inetd_klogin grep -E '^[	 ]*klogin[	 ]' /etc/inetd.conf)
  ID_KLOGIN_RC=$?

  if [ "$ID_KLOGIN_RC" -eq 0 ] && [ -n "$ID_KLOGIN_RAW" ]; then
    # Validate that every nonblank row has first field exactly "klogin"
    # and is well-shaped (has at least a service name, socket type, protocol,
    # wait flag, user, path — ≥6 nonblank fields).
    ID_KLOGIN_PARSED=$(printf '%s\n' "$ID_KLOGIN_RAW" | awk '
      /^[ \t]*$/ { next }
      {
        rows++
    ...
    case "$ID_KLOGIN_PARSED" in
      single) ID_KLOGIN_SHAPE=1 ;;
      multiple) ID_KLOGIN_SHAPE=1 ;;
      *) ID_KLOGIN_SHAPE=0 ;;
    esac
  fi

  # Compute verdict, then severity, then emit single finding.
  case "$ID_KLOGIN_RC" in
    0)
      if [ "$ID_KLOGIN_SHAPE" -eq 1 ] && [ "$ID_KLOGIN_PARSED" != "absent" ]; then
        ...
        case "$ID_KLOGIN_PARSED" in
          non-klogin-first)
            ID_KLOGIN_OBSERVED="not assessed - klogin inetd probe captured output contains non-klogin-first rows" ;;
          malformed)
            ID_KLOGIN_OBSERVED="not assessed - klogin inetd probe captured rows are malformed" ;;
          *)
      ...
      ;;
    1)
      if [ -z "$ID_KLOGIN_RAW" ]; then
        ID_KLOGIN_STATUS=PASS
        ID_KLOGIN_OBSERVED="klogin not active in /etc/inetd.conf"
        ID_KLOGIN_MEANING="The inetd service is not enabled."
        ID_KLOGIN_FIX="n/a"
      else
        ID_KLOGIN_STATUS=NOT_ASSESSED
  ...
  case "$ID_KLOGIN_STATUS" in
    FAIL) SEVERITY=high ;;
    *) SEVERITY=low ;;
  esac

  add security inetd_klogin "inetd klogin service" \
    "$ID_KLOGIN_STATUS" "$SEVERITY" \
    "$ID_KLOGIN_OBSERVED" \
    "$ID_KLOGIN_MEANING" \
    "$ID_KLOGIN_FIX" \
    "cis-l1"
