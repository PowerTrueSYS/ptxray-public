...
AIXRAY_TOOL=ck-inetd-kshell


function standalone_check {
_AIXRAY_SESSION_KEYS=""

  # inetd_kshell — detect whether the legacy kshell service is active in
  # /etc/inetd.conf.  An active kshell entry exposes a clear-text service
  # that can be abused for amplification or as a foothold; CIS requires it
  # disabled.  Capture: grep for the exact first-field token kshell followed
  # by whitespace (not a comment, not kshelld, not a later-field match).
  #
  # Verdict logic:
  #   rc=0  AND  every nonblank row starts with exactly "kshell "  => FAIL
  #   rc=1  AND  empty output                                       => PASS
  #   anything else                                                 => NOT_ASSESSED
  #
  typeset ID_KSHELL_RC ID_KSHELL_RAW ID_KSHELL_PARSED ID_KSHELL_SHAPE
  typeset ID_KSHELL_STATUS ID_KSHELL_OBSERVED ID_KSHELL_MEANING
  typeset ID_KSHELL_FIX SEVERITY
  ID_KSHELL_RC=0
  ID_KSHELL_RAW=""
  ID_KSHELL_PARSED="absent"
  ID_KSHELL_SHAPE=1

  ID_KSHELL_RAW=$(aix inetd_kshell grep -E '^[	 ]*kshell[	 ]' /etc/inetd.conf)
  ID_KSHELL_RC=$?

  if [ "$ID_KSHELL_RC" -eq 0 ] && [ -n "$ID_KSHELL_RAW" ]; then
    # Validate that every nonblank row has first field exactly "kshell"
    # and is well-shaped (has at least a service name, socket type, protocol,
    # wait flag, user, path — ≥6 nonblank fields).
    ID_KSHELL_PARSED=$(printf '%s\n' "$ID_KSHELL_RAW" | awk '
      /^[ \t]*$/ { next }
      {
        rows++
    ...
    case "$ID_KSHELL_PARSED" in
      single) ID_KSHELL_SHAPE=1 ;;
      multiple) ID_KSHELL_SHAPE=1 ;;
      *) ID_KSHELL_SHAPE=0 ;;
    esac
  fi

  # Compute verdict, then severity, then emit single finding.
  case "$ID_KSHELL_RC" in
    0)
      if [ "$ID_KSHELL_SHAPE" -eq 1 ] && [ "$ID_KSHELL_PARSED" != "absent" ]; then
        ...
        case "$ID_KSHELL_PARSED" in
          non-kshell-first)
            ID_KSHELL_OBSERVED="not assessed - kshell inetd probe captured output contains non-kshell-first rows" ;;
          malformed)
            ID_KSHELL_OBSERVED="not assessed - kshell inetd probe captured rows are malformed" ;;
          *)
      ...
      ;;
    1)
      if [ -z "$ID_KSHELL_RAW" ]; then
        ID_KSHELL_STATUS=PASS
        ID_KSHELL_OBSERVED="kshell not active in /etc/inetd.conf"
        ID_KSHELL_MEANING="The inetd service is not enabled."
        ID_KSHELL_FIX="n/a"
      else
        ID_KSHELL_STATUS=NOT_ASSESSED
  ...
  case "$ID_KSHELL_STATUS" in
    FAIL) SEVERITY=high ;;
    *) SEVERITY=low ;;
  esac

  add security inetd_kshell "inetd kshell service" \
    "$ID_KSHELL_STATUS" "$SEVERITY" \
    "$ID_KSHELL_OBSERVED" \
    "$ID_KSHELL_MEANING" \
    "$ID_KSHELL_FIX" \
    "cis-l1"
