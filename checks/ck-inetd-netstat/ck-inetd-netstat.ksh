...
AIXRAY_TOOL=ck-inetd-netstat


function standalone_check {
_AIXRAY_SESSION_KEYS=""

  # inetd_netstat — detect whether the legacy netstat service is active in
  # /etc/inetd.conf.  An active netstat entry exposes a clear-text service
  # that can be abused for amplification or as a foothold; CIS requires it
  # disabled.  Capture: grep for the exact first-field token netstat followed
  # by whitespace (not a comment, not netstatd, not a later-field match).
  #
  # Verdict logic:
  #   rc=0  AND  every nonblank row starts with exactly "netstat "  => FAIL
  #   rc=1  AND  empty output                                       => PASS
  #   anything else                                                 => NOT_ASSESSED
  #
  typeset ID_NETSTAT_RC ID_NETSTAT_RAW ID_NETSTAT_PARSED ID_NETSTAT_SHAPE
  typeset ID_NETSTAT_STATUS ID_NETSTAT_OBSERVED ID_NETSTAT_MEANING
  typeset ID_NETSTAT_FIX SEVERITY
  ID_NETSTAT_RC=0
  ID_NETSTAT_RAW=""
  ID_NETSTAT_PARSED="absent"
  ID_NETSTAT_SHAPE=1

  ID_NETSTAT_RAW=$(aix inetd_netstat grep -E '^[	 ]*netstat[	 ]' /etc/inetd.conf)
  ID_NETSTAT_RC=$?

  if [ "$ID_NETSTAT_RC" -eq 0 ] && [ -n "$ID_NETSTAT_RAW" ]; then
    # Validate that every nonblank row has first field exactly "netstat"
    # and is well-shaped (has at least a service name, socket type, protocol,
    # wait flag, user, path — ≥6 nonblank fields).
    ID_NETSTAT_PARSED=$(printf '%s\n' "$ID_NETSTAT_RAW" | awk '
      /^[ \t]*$/ { next }
      {
        rows++
    ...
    case "$ID_NETSTAT_PARSED" in
      single) ID_NETSTAT_SHAPE=1 ;;
      multiple) ID_NETSTAT_SHAPE=1 ;;
      *) ID_NETSTAT_SHAPE=0 ;;
    esac
  fi

  # Compute verdict, then severity, then emit single finding.
  case "$ID_NETSTAT_RC" in
    0)
      if [ "$ID_NETSTAT_SHAPE" -eq 1 ] && [ "$ID_NETSTAT_PARSED" != "absent" ]; then
        ...
        case "$ID_NETSTAT_PARSED" in
          non-netstat-first)
            ID_NETSTAT_OBSERVED="not assessed - netstat inetd probe captured output contains non-netstat-first rows" ;;
          malformed)
            ID_NETSTAT_OBSERVED="not assessed - netstat inetd probe captured rows are malformed" ;;
          *)
      ...
      ;;
    1)
      if [ -z "$ID_NETSTAT_RAW" ]; then
        ID_NETSTAT_STATUS=PASS
        ID_NETSTAT_OBSERVED="netstat not active in /etc/inetd.conf"
        ID_NETSTAT_MEANING="The inetd service is not enabled."
        ID_NETSTAT_FIX="n/a"
      else
        ID_NETSTAT_STATUS=NOT_ASSESSED
  ...
  case "$ID_NETSTAT_STATUS" in
    FAIL) SEVERITY=high ;;
    *) SEVERITY=low ;;
  esac

  add security inetd_netstat "inetd netstat service" \
    "$ID_NETSTAT_STATUS" "$SEVERITY" \
    "$ID_NETSTAT_OBSERVED" \
    "$ID_NETSTAT_MEANING" \
    "$ID_NETSTAT_FIX" \
    "cis-l1"
