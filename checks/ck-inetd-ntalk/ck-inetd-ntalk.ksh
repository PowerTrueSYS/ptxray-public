...
AIXRAY_TOOL=ck-inetd-ntalk


function standalone_check {
_AIXRAY_SESSION_KEYS=""

  # inetd_ntalk — detect whether the legacy ntalk service is active in
  # /etc/inetd.conf.  An active ntalk entry exposes a clear-text service
  # that can be abused for amplification or as a foothold; CIS requires it
  # disabled.  Capture: grep for the exact first-field token ntalk followed
  # by whitespace (not a comment, not ntalkd, not a later-field match).
  #
  # Verdict logic:
  #   rc=0  AND  every nonblank row starts with exactly "ntalk "  => FAIL
  #   rc=1  AND  empty output                                       => PASS
  #   anything else                                                 => NOT_ASSESSED
  #
  typeset ID_NTALK_RC ID_NTALK_RAW ID_NTALK_PARSED ID_NTALK_SHAPE
  typeset ID_NTALK_STATUS ID_NTALK_OBSERVED ID_NTALK_MEANING
  typeset ID_NTALK_FIX SEVERITY
  ID_NTALK_RC=0
  ID_NTALK_RAW=""
  ID_NTALK_PARSED="absent"
  ID_NTALK_SHAPE=1

  ID_NTALK_RAW=$(aix inetd_ntalk grep -E '^[	 ]*ntalk[	 ]' /etc/inetd.conf)
  ID_NTALK_RC=$?

  if [ "$ID_NTALK_RC" -eq 0 ] && [ -n "$ID_NTALK_RAW" ]; then
    # Validate that every nonblank row has first field exactly "ntalk"
    # and is well-shaped (has at least a service name, socket type, protocol,
    # wait flag, user, path — ≥6 nonblank fields).
    ID_NTALK_PARSED=$(printf '%s\n' "$ID_NTALK_RAW" | awk '
      /^[ \t]*$/ { next }
      {
        rows++
    ...
    case "$ID_NTALK_PARSED" in
      single) ID_NTALK_SHAPE=1 ;;
      multiple) ID_NTALK_SHAPE=1 ;;
      *) ID_NTALK_SHAPE=0 ;;
    esac
  fi
  ...
  case "$ID_NTALK_RC" in
    0)
      if [ "$ID_NTALK_SHAPE" -eq 1 ] && [ "$ID_NTALK_PARSED" != "absent" ]; then
        ...
        case "$ID_NTALK_PARSED" in
          non-ntalk-first)
            ID_NTALK_OBSERVED="not assessed - ntalk inetd probe captured output contains non-ntalk-first rows" ;;
          malformed)
            ID_NTALK_OBSERVED="not assessed - ntalk inetd probe captured rows are malformed" ;;
          *)
      ...
      ;;
    1)
      if [ -z "$ID_NTALK_RAW" ]; then
        ID_NTALK_STATUS=PASS
        ID_NTALK_OBSERVED="ntalk not active in /etc/inetd.conf"
        ID_NTALK_MEANING="The inetd service is not enabled."
        ID_NTALK_FIX="n/a"
      else
        ID_NTALK_STATUS=NOT_ASSESSED
  ...
  case "$ID_NTALK_STATUS" in
    FAIL) SEVERITY=high ;;
    *) SEVERITY=low ;;
  esac

  add security inetd_ntalk "inetd ntalk service" \
    "$ID_NTALK_STATUS" "$SEVERITY" \
    "$ID_NTALK_OBSERVED" \
    "$ID_NTALK_MEANING" \
    "$ID_NTALK_FIX" \
    "cis-l1"
