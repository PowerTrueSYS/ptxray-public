...
AIXRAY_TOOL=ck-inetd-pcnfsd


function standalone_check {
_AIXRAY_SESSION_KEYS=""

  # inetd_pcnfsd — detect whether the legacy pcnfsd service is active in
  # /etc/inetd.conf.  An active pcnfsd entry exposes a clear-text service
  # that can be abused for amplification or as a foothold; CIS requires it
  # disabled.  Capture: grep for the exact first-field token pcnfsd followed
  # by whitespace (not a comment, not pcnfsdd, not a later-field match).
  #
  # Verdict logic:
  #   rc=0  AND  every nonblank row starts with exactly "pcnfsd "  => FAIL
  #   rc=1  AND  empty output                                       => PASS
  #   anything else                                                 => NOT_ASSESSED
  #
  typeset ID_PCNFSD_RC ID_PCNFSD_RAW ID_PCNFSD_PARSED ID_PCNFSD_SHAPE
  typeset ID_PCNFSD_STATUS ID_PCNFSD_OBSERVED ID_PCNFSD_MEANING
  typeset ID_PCNFSD_FIX SEVERITY
  ID_PCNFSD_RC=0
  ID_PCNFSD_RAW=""
  ID_PCNFSD_PARSED="absent"
  ID_PCNFSD_SHAPE=1

  ID_PCNFSD_RAW=$(aix inetd_pcnfsd grep -E '^[	 ]*pcnfsd[	 ]' /etc/inetd.conf)
  ID_PCNFSD_RC=$?

  if [ "$ID_PCNFSD_RC" -eq 0 ] && [ -n "$ID_PCNFSD_RAW" ]; then
    # Validate that every nonblank row has first field exactly "pcnfsd"
    # and is well-shaped (has at least a service name, socket type, protocol,
    # wait flag, user, path — ≥6 nonblank fields).
    ID_PCNFSD_PARSED=$(printf '%s\n' "$ID_PCNFSD_RAW" | awk '
      /^[ \t]*$/ { next }
      {
        rows++
    ...
    case "$ID_PCNFSD_PARSED" in
      single) ID_PCNFSD_SHAPE=1 ;;
      multiple) ID_PCNFSD_SHAPE=1 ;;
      *) ID_PCNFSD_SHAPE=0 ;;
    esac
  fi

  # Compute verdict, then severity, then emit single finding.
  case "$ID_PCNFSD_RC" in
    0)
      if [ "$ID_PCNFSD_SHAPE" -eq 1 ] && [ "$ID_PCNFSD_PARSED" != "absent" ]; then
        ...
        case "$ID_PCNFSD_PARSED" in
          non-pcnfsd-first)
            ID_PCNFSD_OBSERVED="not assessed - pcnfsd inetd probe captured output contains non-pcnfsd-first rows" ;;
          malformed)
            ID_PCNFSD_OBSERVED="not assessed - pcnfsd inetd probe captured rows are malformed" ;;
          *)
      ...
      ;;
    1)
      if [ -z "$ID_PCNFSD_RAW" ]; then
        ID_PCNFSD_STATUS=PASS
        ID_PCNFSD_OBSERVED="pcnfsd not active in /etc/inetd.conf"
        ID_PCNFSD_MEANING="The inetd service is not enabled."
        ID_PCNFSD_FIX="n/a"
      else
        ID_PCNFSD_STATUS=NOT_ASSESSED
  ...
  case "$ID_PCNFSD_STATUS" in
    FAIL) SEVERITY=high ;;
    *) SEVERITY=low ;;
  esac

  add security inetd_pcnfsd "inetd pcnfsd service" \
    "$ID_PCNFSD_STATUS" "$SEVERITY" \
    "$ID_PCNFSD_OBSERVED" \
    "$ID_PCNFSD_MEANING" \
    "$ID_PCNFSD_FIX" \
    "cis-l1"
