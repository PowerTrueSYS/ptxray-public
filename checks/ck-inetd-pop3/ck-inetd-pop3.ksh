...
AIXRAY_TOOL=ck-inetd-pop3


function standalone_check {
_AIXRAY_SESSION_KEYS=""

  # inetd_pop3 — detect whether the legacy pop3 service is active in
  # /etc/inetd.conf.  An active pop3 entry exposes a clear-text service
  # that can be abused for amplification or as a foothold; CIS requires it
  # disabled.  Capture: grep for the exact first-field token pop3 followed
  # by whitespace (not a comment, not pop3d, not a later-field match).
  #
  # Verdict logic:
  #   rc=0  AND  every nonblank row starts with exactly "pop3 "  => FAIL
  #   rc=1  AND  empty output                                       => PASS
  #   anything else                                                 => NOT_ASSESSED
  #
  typeset ID_POP3_RC ID_POP3_RAW ID_POP3_PARSED ID_POP3_SHAPE
  typeset ID_POP3_STATUS ID_POP3_OBSERVED ID_POP3_MEANING
  typeset ID_POP3_FIX SEVERITY
  ID_POP3_RC=0
  ID_POP3_RAW=""
  ID_POP3_PARSED="absent"
  ID_POP3_SHAPE=1

  ID_POP3_RAW=$(aix inetd_pop3 grep -E '^[	 ]*pop3[	 ]' /etc/inetd.conf)
  ID_POP3_RC=$?

  if [ "$ID_POP3_RC" -eq 0 ] && [ -n "$ID_POP3_RAW" ]; then
    # Validate that every nonblank row has first field exactly "pop3"
    # and is well-shaped (has at least a service name, socket type, protocol,
    # wait flag, user, path — ≥6 nonblank fields).
    ID_POP3_PARSED=$(printf '%s\n' "$ID_POP3_RAW" | awk '
      /^[ \t]*$/ { next }
      {
        rows++
    ...
    case "$ID_POP3_PARSED" in
      single) ID_POP3_SHAPE=1 ;;
      multiple) ID_POP3_SHAPE=1 ;;
      *) ID_POP3_SHAPE=0 ;;
    esac
  fi
  ...
  case "$ID_POP3_RC" in
    0)
      if [ "$ID_POP3_SHAPE" -eq 1 ] && [ "$ID_POP3_PARSED" != "absent" ]; then
        ID_POP3_STATUS=FAIL
        ...
        case "$ID_POP3_PARSED" in
          non-pop3-first)
            ID_POP3_OBSERVED="not assessed - pop3 inetd probe captured output contains non-pop3-first rows" ;;
          malformed)
            ID_POP3_OBSERVED="not assessed - pop3 inetd probe captured rows are malformed" ;;
          *)
      ...
      ;;
    1)
      if [ -z "$ID_POP3_RAW" ]; then
        ID_POP3_STATUS=PASS
        ID_POP3_OBSERVED="pop3 not active in /etc/inetd.conf"
        ID_POP3_MEANING="The inetd service is not enabled."
        ID_POP3_FIX="n/a"
      else
        ID_POP3_STATUS=NOT_ASSESSED
  ...
  case "$ID_POP3_STATUS" in
    FAIL) SEVERITY=high ;;
    *) SEVERITY=low ;;
  esac

  add security inetd_pop3 "inetd pop3 service" \
    "$ID_POP3_STATUS" "$SEVERITY" \
    "$ID_POP3_OBSERVED" \
    "$ID_POP3_MEANING" \
    "$ID_POP3_FIX" \
    "cis-l1"
