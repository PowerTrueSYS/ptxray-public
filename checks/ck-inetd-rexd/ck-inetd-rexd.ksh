...
AIXRAY_TOOL=ck-inetd-rexd


function standalone_check {
_AIXRAY_SESSION_KEYS=""

  # inetd_rexd — detect whether the legacy rexd service is active in
  # /etc/inetd.conf.  An active rexd entry exposes a clear-text service
  # that can be abused for amplification or as a foothold; CIS requires it
  # disabled.  Capture: grep for the exact first-field token rexd followed
  # by whitespace (not a comment, not rexdd, not a later-field match).
  #
  # Verdict logic:
  #   rc=0  AND  every nonblank row starts with exactly "rexd "  => FAIL
  #   rc=1  AND  empty output                                       => PASS
  #   anything else                                                 => NOT_ASSESSED
  #
  typeset ID_REXD_RC ID_REXD_RAW ID_REXD_PARSED ID_REXD_SHAPE
  typeset ID_REXD_STATUS ID_REXD_OBSERVED ID_REXD_MEANING
  typeset ID_REXD_FIX SEVERITY
  ID_REXD_RC=0
  ID_REXD_RAW=""
  ID_REXD_PARSED="absent"
  ID_REXD_SHAPE=1

  ID_REXD_RAW=$(aix inetd_rexd grep -E '^[	 ]*rexd[	 ]' /etc/inetd.conf)
  ID_REXD_RC=$?

  if [ "$ID_REXD_RC" -eq 0 ] && [ -n "$ID_REXD_RAW" ]; then
    # Validate that every nonblank row has first field exactly "rexd"
    # and is well-shaped (has at least a service name, socket type, protocol,
    # wait flag, user, path — ≥6 nonblank fields).
    ID_REXD_PARSED=$(printf '%s\n' "$ID_REXD_RAW" | awk '
      /^[ \t]*$/ { next }
      {
        rows++
    ...
    case "$ID_REXD_PARSED" in
      single) ID_REXD_SHAPE=1 ;;
      multiple) ID_REXD_SHAPE=1 ;;
      *) ID_REXD_SHAPE=0 ;;
    esac
  fi
  ...
  case "$ID_REXD_RC" in
    0)
      if [ "$ID_REXD_SHAPE" -eq 1 ] && [ "$ID_REXD_PARSED" != "absent" ]; then
        ID_REXD_STATUS=FAIL
        ...
        case "$ID_REXD_PARSED" in
          non-rexd-first)
            ID_REXD_OBSERVED="not assessed - rexd inetd probe captured output contains non-rexd-first rows" ;;
          malformed)
            ID_REXD_OBSERVED="not assessed - rexd inetd probe captured rows are malformed" ;;
          *)
      ...
      ;;
    1)
      if [ -z "$ID_REXD_RAW" ]; then
        ID_REXD_STATUS=PASS
        ID_REXD_OBSERVED="rexd not active in /etc/inetd.conf"
        ID_REXD_MEANING="The inetd service is not enabled."
        ID_REXD_FIX="n/a"
      else
        ID_REXD_STATUS=NOT_ASSESSED
  ...
  case "$ID_REXD_STATUS" in
    FAIL) SEVERITY=high ;;
    *) SEVERITY=low ;;
  esac

  add security inetd_rexd "inetd rexd service" \
    "$ID_REXD_STATUS" "$SEVERITY" \
    "$ID_REXD_OBSERVED" \
    "$ID_REXD_MEANING" \
    "$ID_REXD_FIX" \
    "cis-l1"
