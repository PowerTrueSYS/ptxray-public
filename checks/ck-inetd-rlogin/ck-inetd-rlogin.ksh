...
AIXRAY_TOOL=ck-inetd-rlogin


function standalone_check {
_AIXRAY_SESSION_KEYS=""

  # inetd_rlogin — detect whether the legacy rlogin service is active in
  # /etc/inetd.conf.  An active rlogin entry exposes a clear-text service
  # that can be abused for amplification or as a foothold; CIS requires it
  # disabled.  Capture: grep for the exact first-field token login followed
  # by whitespace (not a comment, not logind, not a later-field match).
  #
  # Verdict logic:
  #   rc=0  AND  every nonblank row starts with exactly "login "  => FAIL
  #   rc=1  AND  empty output                                       => PASS
  #   anything else                                                 => NOT_ASSESSED
  #
  typeset ID_RLOGIN_RC ID_RLOGIN_RAW ID_RLOGIN_PARSED ID_RLOGIN_SHAPE
  typeset ID_RLOGIN_STATUS ID_RLOGIN_OBSERVED ID_RLOGIN_MEANING
  typeset ID_RLOGIN_FIX SEVERITY
  ID_RLOGIN_RC=0
  ID_RLOGIN_RAW=""
  ID_RLOGIN_PARSED="absent"
  ID_RLOGIN_SHAPE=1

  ID_RLOGIN_RAW=$(aix inetd_rlogin grep -E '^[	 ]*login[	 ]' /etc/inetd.conf)
  ID_RLOGIN_RC=$?

  if [ "$ID_RLOGIN_RC" -eq 0 ] && [ -n "$ID_RLOGIN_RAW" ]; then
    # Validate that every nonblank row has first field exactly "login"
    # and is well-shaped (has at least a service name, socket type, protocol,
    # wait flag, user, path — ≥6 nonblank fields).
    ID_RLOGIN_PARSED=$(printf '%s\n' "$ID_RLOGIN_RAW" | awk '
      /^[ \t]*$/ { next }
      {
        rows++
    ...
    case "$ID_RLOGIN_PARSED" in
      single) ID_RLOGIN_SHAPE=1 ;;
      multiple) ID_RLOGIN_SHAPE=1 ;;
      *) ID_RLOGIN_SHAPE=0 ;;
    esac
  fi

  # Compute verdict, then severity, then emit single finding.
  case "$ID_RLOGIN_RC" in
    0)
      if [ "$ID_RLOGIN_SHAPE" -eq 1 ] && [ "$ID_RLOGIN_PARSED" != "absent" ]; then
        ...
        case "$ID_RLOGIN_PARSED" in
          non-login-first)
            ID_RLOGIN_OBSERVED="not assessed - rlogin inetd probe captured output contains non-login-first rows" ;;
          malformed)
            ID_RLOGIN_OBSERVED="not assessed - rlogin inetd probe captured rows are malformed" ;;
          *)
      ...
      ;;
    1)
      if [ -z "$ID_RLOGIN_RAW" ]; then
        ID_RLOGIN_STATUS=PASS
        ID_RLOGIN_OBSERVED="rlogin not active in /etc/inetd.conf"
        ID_RLOGIN_MEANING="The inetd service is not enabled."
        ID_RLOGIN_FIX="n/a"
      else
        ID_RLOGIN_STATUS=NOT_ASSESSED
  ...
  case "$ID_RLOGIN_STATUS" in
    FAIL) SEVERITY=high ;;
    *) SEVERITY=low ;;
  esac

  add security inetd_rlogin "inetd rlogin service" \
    "$ID_RLOGIN_STATUS" "$SEVERITY" \
    "$ID_RLOGIN_OBSERVED" \
    "$ID_RLOGIN_MEANING" \
    "$ID_RLOGIN_FIX" \
    "cis-l1"
