...
AIXRAY_TOOL=ck-inetd-rquotad


function standalone_check {
_AIXRAY_SESSION_KEYS=""

  # inetd_rquotad — detect whether the legacy rquotad service is active in
  # /etc/inetd.conf.  An active rquotad entry exposes a clear-text service
  # that can be abused for amplification or as a foothold; CIS requires it
  # disabled.  Capture: grep for the exact first-field token rquotad followed
  # by whitespace (not a comment, not rquotadd, not a later-field match).
  #
  # Verdict logic:
  #   rc=0  AND  every nonblank row starts with exactly "rquotad "  => FAIL
  #   rc=1  AND  empty output                                       => PASS
  #   anything else                                                 => NOT_ASSESSED
  #
  typeset ID_RQUOTAD_RC ID_RQUOTAD_RAW ID_RQUOTAD_PARSED ID_RQUOTAD_SHAPE
  typeset ID_RQUOTAD_STATUS ID_RQUOTAD_OBSERVED ID_RQUOTAD_MEANING
  typeset ID_RQUOTAD_FIX SEVERITY
  ID_RQUOTAD_RC=0
  ID_RQUOTAD_RAW=""
  ID_RQUOTAD_PARSED="absent"
  ID_RQUOTAD_SHAPE=1

  ID_RQUOTAD_RAW=$(aix inetd_rquotad grep -E '^[	 ]*rquotad[	 ]' /etc/inetd.conf)
  ID_RQUOTAD_RC=$?

  if [ "$ID_RQUOTAD_RC" -eq 0 ] && [ -n "$ID_RQUOTAD_RAW" ]; then
    # Validate that every nonblank row has first field exactly "rquotad"
    # and is well-shaped (has at least a service name, socket type, protocol,
    # wait flag, user, path — ≥6 nonblank fields).
    ID_RQUOTAD_PARSED=$(printf '%s\n' "$ID_RQUOTAD_RAW" | awk '
      /^[ \t]*$/ { next }
      {
        rows++
    ...
    case "$ID_RQUOTAD_PARSED" in
      single) ID_RQUOTAD_SHAPE=1 ;;
      multiple) ID_RQUOTAD_SHAPE=1 ;;
      *) ID_RQUOTAD_SHAPE=0 ;;
    esac
  fi

  # Compute verdict, then severity, then emit single finding.
  case "$ID_RQUOTAD_RC" in
    0)
      if [ "$ID_RQUOTAD_SHAPE" -eq 1 ] && [ "$ID_RQUOTAD_PARSED" != "absent" ]; then
        ...
        case "$ID_RQUOTAD_PARSED" in
          non-rquotad-first)
            ID_RQUOTAD_OBSERVED="not assessed - rquotad inetd probe captured output contains non-rquotad-first rows" ;;
          malformed)
            ID_RQUOTAD_OBSERVED="not assessed - rquotad inetd probe captured rows are malformed" ;;
          *)
      ...
      ;;
    1)
      if [ -z "$ID_RQUOTAD_RAW" ]; then
        ID_RQUOTAD_STATUS=PASS
        ID_RQUOTAD_OBSERVED="rquotad not active in /etc/inetd.conf"
        ID_RQUOTAD_MEANING="The inetd service is not enabled."
        ID_RQUOTAD_FIX="n/a"
      else
        ID_RQUOTAD_STATUS=NOT_ASSESSED
  ...
  case "$ID_RQUOTAD_STATUS" in
    FAIL) SEVERITY=high ;;
    *) SEVERITY=low ;;
  esac

  add security inetd_rquotad "inetd rquotad service" \
    "$ID_RQUOTAD_STATUS" "$SEVERITY" \
    "$ID_RQUOTAD_OBSERVED" \
    "$ID_RQUOTAD_MEANING" \
    "$ID_RQUOTAD_FIX" \
    "cis-l1"
