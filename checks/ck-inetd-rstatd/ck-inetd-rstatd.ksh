...
AIXRAY_TOOL=ck-inetd-rstatd


function standalone_check {
_AIXRAY_SESSION_KEYS=""

  # inetd_rstatd — detect whether the legacy rstatd service is active in
  # /etc/inetd.conf.  An active rstatd entry exposes a clear-text service
  # that can be abused for amplification or as a foothold; CIS requires it
  # disabled.  Capture: grep for the exact first-field token rstatd followed
  # by whitespace (not a comment, not rstatdd, not a later-field match).
  #
  # Verdict logic:
  #   rc=0  AND  every nonblank row starts with exactly "rstatd "  => FAIL
  #   rc=1  AND  empty output                                       => PASS
  #   anything else                                                 => NOT_ASSESSED
  #
  typeset ID_RSTATD_RC ID_RSTATD_RAW ID_RSTATD_PARSED ID_RSTATD_SHAPE
  typeset ID_RSTATD_STATUS ID_RSTATD_OBSERVED ID_RSTATD_MEANING
  typeset ID_RSTATD_FIX SEVERITY
  ID_RSTATD_RC=0
  ID_RSTATD_RAW=""
  ID_RSTATD_PARSED="absent"
  ID_RSTATD_SHAPE=1

  ID_RSTATD_RAW=$(aix inetd_rstatd grep -E '^[	 ]*rstatd[	 ]' /etc/inetd.conf)
  ID_RSTATD_RC=$?

  if [ "$ID_RSTATD_RC" -eq 0 ] && [ -n "$ID_RSTATD_RAW" ]; then
    # Validate that every nonblank row has first field exactly "rstatd"
    # and is well-shaped (has at least a service name, socket type, protocol,
    # wait flag, user, path — ≥6 nonblank fields).
    ID_RSTATD_PARSED=$(printf '%s\n' "$ID_RSTATD_RAW" | awk '
      /^[ \t]*$/ { next }
      {
        rows++
    ...
    case "$ID_RSTATD_PARSED" in
      single) ID_RSTATD_SHAPE=1 ;;
      multiple) ID_RSTATD_SHAPE=1 ;;
      *) ID_RSTATD_SHAPE=0 ;;
    esac
  fi

  # Compute verdict, then severity, then emit single finding.
  case "$ID_RSTATD_RC" in
    0)
      if [ "$ID_RSTATD_SHAPE" -eq 1 ] && [ "$ID_RSTATD_PARSED" != "absent" ]; then
        ...
        case "$ID_RSTATD_PARSED" in
          non-rstatd-first)
            ID_RSTATD_OBSERVED="not assessed - rstatd inetd probe captured output contains non-rstatd-first rows" ;;
          malformed)
            ID_RSTATD_OBSERVED="not assessed - rstatd inetd probe captured rows are malformed" ;;
          *)
      ...
      ;;
    1)
      if [ -z "$ID_RSTATD_RAW" ]; then
        ID_RSTATD_STATUS=PASS
        ID_RSTATD_OBSERVED="rstatd not active in /etc/inetd.conf"
        ID_RSTATD_MEANING="The inetd service is not enabled."
        ID_RSTATD_FIX="n/a"
      else
        ID_RSTATD_STATUS=NOT_ASSESSED
  ...
  case "$ID_RSTATD_STATUS" in
    FAIL) SEVERITY=high ;;
    *) SEVERITY=low ;;
  esac

  add security inetd_rstatd "inetd rstatd service" \
    "$ID_RSTATD_STATUS" "$SEVERITY" \
    "$ID_RSTATD_OBSERVED" \
    "$ID_RSTATD_MEANING" \
    "$ID_RSTATD_FIX" \
    "cis-l1"
