...
AIXRAY_TOOL=ck-inetd-rsync


function standalone_check {
_AIXRAY_SESSION_KEYS=""

  # inetd_rsync — detect whether the legacy rsync service is active in
  # /etc/inetd.conf.  An active rsync entry exposes a clear-text service
  # that can be abused for amplification or as a foothold; CIS requires it
  # disabled.  Capture: grep for the exact first-field token rsync followed
  # by whitespace (not a comment, not rsyncd, not a later-field match).
  #
  # Verdict logic:
  #   rc=0  AND  every nonblank row starts with exactly "rsync "  => FAIL
  #   rc=1  AND  empty output                                       => PASS
  #   anything else                                                 => NOT_ASSESSED
  #
  typeset ID_RSYNC_RC ID_RSYNC_RAW ID_RSYNC_PARSED ID_RSYNC_SHAPE
  typeset ID_RSYNC_STATUS ID_RSYNC_OBSERVED ID_RSYNC_MEANING
  typeset ID_RSYNC_FIX SEVERITY
  ID_RSYNC_RC=0
  ID_RSYNC_RAW=""
  ID_RSYNC_PARSED="absent"
  ID_RSYNC_SHAPE=1

  ID_RSYNC_RAW=$(aix inetd_rsync grep -E '^[	 ]*rsync[	 ]' /etc/inetd.conf)
  ID_RSYNC_RC=$?

  if [ "$ID_RSYNC_RC" -eq 0 ] && [ -n "$ID_RSYNC_RAW" ]; then
    # Validate that every nonblank row has first field exactly "rsync"
    # and is well-shaped (has at least a service name, socket type, protocol,
    # wait flag, user, path — ≥6 nonblank fields).
    ID_RSYNC_PARSED=$(printf '%s\n' "$ID_RSYNC_RAW" | awk '
      /^[ \t]*$/ { next }
      {
        rows++
    ...
    case "$ID_RSYNC_PARSED" in
      single) ID_RSYNC_SHAPE=1 ;;
      multiple) ID_RSYNC_SHAPE=1 ;;
      *) ID_RSYNC_SHAPE=0 ;;
    esac
  fi
  ...
  case "$ID_RSYNC_RC" in
    0)
      if [ "$ID_RSYNC_SHAPE" -eq 1 ] && [ "$ID_RSYNC_PARSED" != "absent" ]; then
        ...
        case "$ID_RSYNC_PARSED" in
          non-rsync-first)
            ID_RSYNC_OBSERVED="not assessed - rsync inetd probe captured output contains non-rsync-first rows" ;;
          malformed)
            ID_RSYNC_OBSERVED="not assessed - rsync inetd probe captured rows are malformed" ;;
          *)
      ...
      ;;
    1)
      if [ -z "$ID_RSYNC_RAW" ]; then
        ID_RSYNC_STATUS=PASS
        ID_RSYNC_OBSERVED="rsync not active in /etc/inetd.conf"
        ID_RSYNC_MEANING="The inetd service is not enabled."
        ID_RSYNC_FIX="n/a"
      else
        ID_RSYNC_STATUS=NOT_ASSESSED
  ...
  case "$ID_RSYNC_STATUS" in
    FAIL) SEVERITY=high ;;
    *) SEVERITY=low ;;
  esac

  add security inetd_rsync "inetd rsync service" \
    "$ID_RSYNC_STATUS" "$SEVERITY" \
    "$ID_RSYNC_OBSERVED" \
    "$ID_RSYNC_MEANING" \
    "$ID_RSYNC_FIX" \
    "cis-l1"
