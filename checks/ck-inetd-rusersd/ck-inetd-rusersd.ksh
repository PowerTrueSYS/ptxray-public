...
AIXRAY_TOOL=ck-inetd-rusersd


function standalone_check {
_AIXRAY_SESSION_KEYS=""

  # inetd_rusersd — detect whether the legacy rusersd service is active in
  # /etc/inetd.conf.  An active rusersd entry exposes a clear-text service
  # that can be abused for amplification or as a foothold; CIS requires it
  # disabled.  Capture: grep for the exact first-field token rusersd followed
  # by whitespace (not a comment, not rusersdd, not a later-field match).
  #
  # Verdict logic:
  #   rc=0  AND  every nonblank row starts with exactly "rusersd "  => FAIL
  #   rc=1  AND  empty output                                       => PASS
  #   anything else                                                 => NOT_ASSESSED
  #
  typeset ID_RUSERSD_RC ID_RUSERSD_RAW ID_RUSERSD_PARSED ID_RUSERSD_SHAPE
  typeset ID_RUSERSD_STATUS ID_RUSERSD_OBSERVED ID_RUSERSD_MEANING
  typeset ID_RUSERSD_FIX SEVERITY
  ID_RUSERSD_RC=0
  ID_RUSERSD_RAW=""
  ID_RUSERSD_PARSED="absent"
  ID_RUSERSD_SHAPE=1

  ID_RUSERSD_RAW=$(aix inetd_rusersd grep -E '^[	 ]*rusersd[	 ]' /etc/inetd.conf)
  ID_RUSERSD_RC=$?

  if [ "$ID_RUSERSD_RC" -eq 0 ] && [ -n "$ID_RUSERSD_RAW" ]; then
    # Validate that every nonblank row has first field exactly "rusersd"
    # and is well-shaped (has at least a service name, socket type, protocol,
    # wait flag, user, path — ≥6 nonblank fields).
    ID_RUSERSD_PARSED=$(printf '%s\n' "$ID_RUSERSD_RAW" | awk '
      /^[ \t]*$/ { next }
      {
        rows++
    ...
    case "$ID_RUSERSD_PARSED" in
      single) ID_RUSERSD_SHAPE=1 ;;
      multiple) ID_RUSERSD_SHAPE=1 ;;
      *) ID_RUSERSD_SHAPE=0 ;;
    esac
  fi

  # Compute verdict, then severity, then emit single finding.
  case "$ID_RUSERSD_RC" in
    0)
      if [ "$ID_RUSERSD_SHAPE" -eq 1 ] && [ "$ID_RUSERSD_PARSED" != "absent" ]; then
        ...
        case "$ID_RUSERSD_PARSED" in
          non-rusersd-first)
            ID_RUSERSD_OBSERVED="not assessed - rusersd inetd probe captured output contains non-rusersd-first rows" ;;
          malformed)
            ID_RUSERSD_OBSERVED="not assessed - rusersd inetd probe captured rows are malformed" ;;
          *)
      ...
      ;;
    1)
      if [ -z "$ID_RUSERSD_RAW" ]; then
        ID_RUSERSD_STATUS=PASS
        ID_RUSERSD_OBSERVED="rusersd not active in /etc/inetd.conf"
        ID_RUSERSD_MEANING="The inetd service is not enabled."
        ID_RUSERSD_FIX="n/a"
      else
        ID_RUSERSD_STATUS=NOT_ASSESSED
  ...
  case "$ID_RUSERSD_STATUS" in
    FAIL) SEVERITY=high ;;
    *) SEVERITY=low ;;
  esac

  add security inetd_rusersd "inetd rusersd service" \
    "$ID_RUSERSD_STATUS" "$SEVERITY" \
    "$ID_RUSERSD_OBSERVED" \
    "$ID_RUSERSD_MEANING" \
    "$ID_RUSERSD_FIX" \
    "cis-l1"
