...
AIXRAY_TOOL=ck-inetd-rwalld


function standalone_check {
_AIXRAY_SESSION_KEYS=""

  # inetd_rwalld — detect whether the legacy rwalld service is active in
  # /etc/inetd.conf.  An active rwalld entry exposes a clear-text service
  # that can be abused for amplification or as a foothold; CIS requires it
  # disabled.  Capture: grep for the exact first-field token rwalld followed
  # by whitespace (not a comment, not rwalldd, not a later-field match).
  #
  # Verdict logic:
  #   rc=0  AND  every nonblank row starts with exactly "rwalld "  => FAIL
  #   rc=1  AND  empty output                                       => PASS
  #   anything else                                                 => NOT_ASSESSED
  #
  typeset ID_RWALLD_RC ID_RWALLD_RAW ID_RWALLD_PARSED ID_RWALLD_SHAPE
  typeset ID_RWALLD_STATUS ID_RWALLD_OBSERVED ID_RWALLD_MEANING
  typeset ID_RWALLD_FIX SEVERITY
  ID_RWALLD_RC=0
  ID_RWALLD_RAW=""
  ID_RWALLD_PARSED="absent"
  ID_RWALLD_SHAPE=1

  ID_RWALLD_RAW=$(aix inetd_rwalld grep -E '^[	 ]*rwalld[	 ]' /etc/inetd.conf)
  ID_RWALLD_RC=$?

  if [ "$ID_RWALLD_RC" -eq 0 ] && [ -n "$ID_RWALLD_RAW" ]; then
    # Validate that every nonblank row has first field exactly "rwalld"
    # and is well-shaped (has at least a service name, socket type, protocol,
    # wait flag, user, path — ≥6 nonblank fields).
    ID_RWALLD_PARSED=$(printf '%s\n' "$ID_RWALLD_RAW" | awk '
      /^[ \t]*$/ { next }
      {
        rows++
    ...
    case "$ID_RWALLD_PARSED" in
      single) ID_RWALLD_SHAPE=1 ;;
      multiple) ID_RWALLD_SHAPE=1 ;;
      *) ID_RWALLD_SHAPE=0 ;;
    esac
  fi

  # Compute verdict, then severity, then emit single finding.
  case "$ID_RWALLD_RC" in
    0)
      if [ "$ID_RWALLD_SHAPE" -eq 1 ] && [ "$ID_RWALLD_PARSED" != "absent" ]; then
        ...
        case "$ID_RWALLD_PARSED" in
          non-rwalld-first)
            ID_RWALLD_OBSERVED="not assessed - rwalld inetd probe captured output contains non-rwalld-first rows" ;;
          malformed)
            ID_RWALLD_OBSERVED="not assessed - rwalld inetd probe captured rows are malformed" ;;
          *)
      ...
      ;;
    1)
      if [ -z "$ID_RWALLD_RAW" ]; then
        ID_RWALLD_STATUS=PASS
        ID_RWALLD_OBSERVED="rwalld not active in /etc/inetd.conf"
        ID_RWALLD_MEANING="The inetd service is not enabled."
        ID_RWALLD_FIX="n/a"
      else
        ID_RWALLD_STATUS=NOT_ASSESSED
  ...
  case "$ID_RWALLD_STATUS" in
    FAIL) SEVERITY=high ;;
    *) SEVERITY=low ;;
  esac

  add security inetd_rwalld "inetd rwalld service" \
    "$ID_RWALLD_STATUS" "$SEVERITY" \
    "$ID_RWALLD_OBSERVED" \
    "$ID_RWALLD_MEANING" \
    "$ID_RWALLD_FIX" \
    "cis-l1"
