...
AIXRAY_TOOL=ck-inetd-shell


function standalone_check {
_AIXRAY_SESSION_KEYS=""

  # inetd_shell — detect whether the legacy shell service is active in
  # /etc/inetd.conf.  An active shell entry exposes a clear-text service
  # that can be abused for amplification or as a foothold; CIS requires it
  # disabled.  Capture: grep for the exact first-field token shell followed
  # by whitespace (not a comment, not shelld, not a later-field match).
  #
  # Verdict logic:
  #   rc=0  AND  every nonblank row starts with exactly "shell "  => FAIL
  #   rc=1  AND  empty output                                       => PASS
  #   anything else                                                 => NOT_ASSESSED
  #
  typeset ID_SHELL_RC ID_SHELL_RAW ID_SHELL_PARSED ID_SHELL_SHAPE
  typeset ID_SHELL_STATUS ID_SHELL_OBSERVED ID_SHELL_MEANING
  typeset ID_SHELL_FIX SEVERITY
  ID_SHELL_RC=0
  ID_SHELL_RAW=""
  ID_SHELL_PARSED="absent"
  ID_SHELL_SHAPE=1

  ID_SHELL_RAW=$(aix inetd_shell grep -E '^[	 ]*shell[	 ]' /etc/inetd.conf)
  ID_SHELL_RC=$?

  if [ "$ID_SHELL_RC" -eq 0 ] && [ -n "$ID_SHELL_RAW" ]; then
    # Validate that every nonblank row has first field exactly "shell"
    # and is well-shaped (has at least a service name, socket type, protocol,
    # wait flag, user, path — ≥6 nonblank fields).
    ID_SHELL_PARSED=$(printf '%s\n' "$ID_SHELL_RAW" | awk '
      /^[ \t]*$/ { next }
      {
        rows++
    ...
    case "$ID_SHELL_PARSED" in
      single) ID_SHELL_SHAPE=1 ;;
      multiple) ID_SHELL_SHAPE=1 ;;
      *) ID_SHELL_SHAPE=0 ;;
    esac
  fi
  ...
  case "$ID_SHELL_RC" in
    0)
      if [ "$ID_SHELL_SHAPE" -eq 1 ] && [ "$ID_SHELL_PARSED" != "absent" ]; then
        ...
        case "$ID_SHELL_PARSED" in
          non-shell-first)
            ID_SHELL_OBSERVED="not assessed - shell inetd probe captured output contains non-shell-first rows" ;;
          malformed)
            ID_SHELL_OBSERVED="not assessed - shell inetd probe captured rows are malformed" ;;
          *)
      ...
      ;;
    1)
      if [ -z "$ID_SHELL_RAW" ]; then
        ID_SHELL_STATUS=PASS
        ID_SHELL_OBSERVED="shell not active in /etc/inetd.conf"
        ID_SHELL_MEANING="The inetd service is not enabled."
        ID_SHELL_FIX="n/a"
      else
        ID_SHELL_STATUS=NOT_ASSESSED
  ...
  case "$ID_SHELL_STATUS" in
    FAIL) SEVERITY=high ;;
    *) SEVERITY=low ;;
  esac

  add security inetd_shell "inetd shell service" \
    "$ID_SHELL_STATUS" "$SEVERITY" \
    "$ID_SHELL_OBSERVED" \
    "$ID_SHELL_MEANING" \
    "$ID_SHELL_FIX" \
    "cis-l1"
