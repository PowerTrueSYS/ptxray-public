...
AIXRAY_TOOL=ck-inetd-sprayd


function standalone_check {
_AIXRAY_SESSION_KEYS=""

  # inetd_sprayd — detect whether the legacy sprayd service is active in
  # /etc/inetd.conf.  An active sprayd entry exposes a clear-text service
  # that can be abused for amplification or as a foothold; CIS requires it
  # disabled.  Capture: grep for the exact first-field token sprayd followed
  # by whitespace (not a comment, not spraydd, not a later-field match).
  #
  # Verdict logic:
  #   rc=0  AND  every nonblank row starts with exactly "sprayd "  => FAIL
  #   rc=1  AND  empty output                                       => PASS
  #   anything else                                                 => NOT_ASSESSED
  #
  typeset ID_SPRAYD_RC ID_SPRAYD_RAW ID_SPRAYD_PARSED ID_SPRAYD_SHAPE
  typeset ID_SPRAYD_STATUS ID_SPRAYD_OBSERVED ID_SPRAYD_MEANING
  typeset ID_SPRAYD_FIX SEVERITY
  ID_SPRAYD_RC=0
  ID_SPRAYD_RAW=""
  ID_SPRAYD_PARSED="absent"
  ID_SPRAYD_SHAPE=1

  ID_SPRAYD_RAW=$(aix inetd_sprayd grep -E '^[	 ]*sprayd[	 ]' /etc/inetd.conf)
  ID_SPRAYD_RC=$?

  if [ "$ID_SPRAYD_RC" -eq 0 ] && [ -n "$ID_SPRAYD_RAW" ]; then
    # Validate that every nonblank row has first field exactly "sprayd".
    ID_SPRAYD_PARSED=$(printf '%s\n' "$ID_SPRAYD_RAW" | awk '
      /^[ \t]*$/ { next }
      {
        rows++
    ...
    case "$ID_SPRAYD_PARSED" in
      single) ID_SPRAYD_SHAPE=1 ;;
      multiple) ID_SPRAYD_SHAPE=1 ;;
      *) ID_SPRAYD_SHAPE=0 ;;
    esac
  fi

  # Compute verdict, then severity, then emit single finding.
  case "$ID_SPRAYD_RC" in
    0)
      if [ "$ID_SPRAYD_SHAPE" -eq 1 ] && [ "$ID_SPRAYD_PARSED" != "absent" ]; then
        ...
        case "$ID_SPRAYD_PARSED" in
          non-sprayd-first)
            ID_SPRAYD_OBSERVED="not assessed - sprayd inetd probe captured output contains non-sprayd-first rows" ;;
          *)
      ...
      ;;
    1)
      if [ -z "$ID_SPRAYD_RAW" ]; then
        ID_SPRAYD_STATUS=PASS
        ID_SPRAYD_OBSERVED="sprayd not active in /etc/inetd.conf"
        ID_SPRAYD_MEANING="The inetd service is not enabled."
        ID_SPRAYD_FIX="n/a"
      else
        ID_SPRAYD_STATUS=NOT_ASSESSED
  ...
  case "$ID_SPRAYD_STATUS" in
    FAIL) SEVERITY=high ;;
    *) SEVERITY=low ;;
  esac

  add security inetd_sprayd "inetd sprayd service" \
    "$ID_SPRAYD_STATUS" "$SEVERITY" \
    "$ID_SPRAYD_OBSERVED" \
    "$ID_SPRAYD_MEANING" \
    "$ID_SPRAYD_FIX" \
    "cis-l1"
