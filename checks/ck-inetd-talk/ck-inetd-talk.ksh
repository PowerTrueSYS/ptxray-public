...
AIXRAY_TOOL=ck-inetd-talk


function standalone_check {
_AIXRAY_SESSION_KEYS=""

  # inetd_talk — detect whether the legacy talk service is active in
  # /etc/inetd.conf.  An active talk entry exposes a clear-text service
  # that can be abused for amplification or as a foothold; CIS requires it
  # disabled.  Capture: grep for the exact first-field token talk followed
  # by whitespace (not a comment, not talkd, not a later-field match).
  #
  # Verdict logic:
  #   rc=0  AND  every nonblank row starts with exactly "talk "  => FAIL
  #   rc=1  AND  empty output                                       => PASS
  #   anything else                                                 => NOT_ASSESSED
  #
  typeset ID_TALK_RC ID_TALK_RAW ID_TALK_PARSED ID_TALK_SHAPE
  typeset ID_TALK_STATUS ID_TALK_OBSERVED ID_TALK_MEANING
  typeset ID_TALK_FIX SEVERITY
  ID_TALK_RC=0
  ID_TALK_RAW=""
  ID_TALK_PARSED="absent"
  ID_TALK_SHAPE=1

  ID_TALK_RAW=$(aix inetd_talk grep -E '^[	 ]*talk[	 ]' /etc/inetd.conf)
  ID_TALK_RC=$?

  if [ "$ID_TALK_RC" -eq 0 ] && [ -n "$ID_TALK_RAW" ]; then
    # Validate that every nonblank row has first field exactly "talk".
    ID_TALK_PARSED=$(printf '%s\n' "$ID_TALK_RAW" | awk '
      /^[ \t]*$/ { next }
      {
        rows++
    ...
    case "$ID_TALK_PARSED" in
      single) ID_TALK_SHAPE=1 ;;
      multiple) ID_TALK_SHAPE=1 ;;
      *) ID_TALK_SHAPE=0 ;;
    esac
  fi
  ...
  case "$ID_TALK_RC" in
    0)
      if [ "$ID_TALK_SHAPE" -eq 1 ] && [ "$ID_TALK_PARSED" != "absent" ]; then
        ID_TALK_STATUS=FAIL
        ...
        case "$ID_TALK_PARSED" in
          non-talk-first)
            ID_TALK_OBSERVED="not assessed - talk inetd probe captured output contains non-talk-first rows" ;;
          *)
      ...
      ;;
    1)
      if [ -z "$ID_TALK_RAW" ]; then
        ID_TALK_STATUS=PASS
        ID_TALK_OBSERVED="talk not active in /etc/inetd.conf"
        ID_TALK_MEANING="The inetd service is not enabled."
        ID_TALK_FIX="n/a"
      else
        ID_TALK_STATUS=NOT_ASSESSED
  ...
  case "$ID_TALK_STATUS" in
    FAIL) SEVERITY=high ;;
    *) SEVERITY=low ;;
  esac

  add security inetd_talk "inetd talk service" \
    "$ID_TALK_STATUS" "$SEVERITY" \
    "$ID_TALK_OBSERVED" \
    "$ID_TALK_MEANING" \
    "$ID_TALK_FIX" \
    "cis-l1"
