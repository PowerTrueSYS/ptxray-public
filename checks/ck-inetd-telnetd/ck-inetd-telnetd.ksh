...
AIXRAY_TOOL=ck-inetd-telnetd


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # inetd_telnetd — detect active Telnet service definitions from the shared
  # /etc/inetd.conf capture. tcp/tcp6 and bare/absolute daemon paths are
  # retained; commented rows are ignored.
  typeset ID_TELNETD_RC ID_TELNETD_RAW ID_TELNETD_PARSED ID_TELNETD_STATUS
  typeset ID_TELNETD_OBSERVED ID_TELNETD_MEANING ID_TELNETD_FIX SEVERITY
  ID_TELNETD_RAW=$(aix inetd_conf grep -E '^[[:space:]]*(telnet|telnetd|ftp|ftpd|shell|login|exec|rsh|rlogin)[[:space:]]' /etc/inetd.conf) # network-lint: allow -- reads the LOCAL /etc/inetd.conf capture text (via the aix wrapper), never a network call
  ID_TELNETD_RC=$?
  ID_TELNETD_PARSED=absent

  if [ "$ID_TELNETD_RC" -eq 0 ] && [ -n "$ID_TELNETD_RAW" ]; then
    ID_TELNETD_PARSED=$(printf '%s\n' "$ID_TELNETD_RAW" | awk '
      /^[ \t]*#/ || /^[ \t]*$/ { next }
      {
        line=$0
        sub(/^[ \t]+/, "", line)
        split(line, fields, /[ \t]+/)
        first=fields[1]
        if (first == "telnet" || first == "telnetd") active=1 # network-lint: allow -- compares an inetd.conf service token, not a network call
      }
      END { print active ? "active" : "absent" }
    ')
  fi

  case "$ID_TELNETD_RC" in
    0)
      if [ -z "$ID_TELNETD_RAW" ]; then
        ...
        ID_TELNETD_MEANING="The inetd service is not enabled."
        ID_TELNETD_FIX="n/a"
      fi
      ;;
    1)
  ...
  case "$ID_TELNETD_STATUS" in FAIL) SEVERITY=high ;; *) SEVERITY=low ;; esac
  add security inetd_telnetd "inetd telnetd service" \
    "$ID_TELNETD_STATUS" "$SEVERITY" "$ID_TELNETD_OBSERVED" \
    "$ID_TELNETD_MEANING" "$ID_TELNETD_FIX" "cis-l1"
}
