...
AIXRAY_TOOL=ck-inetd-tftpd


function standalone_check {
_AIXRAY_SESSION_KEYS=""

  # inetd_tftpd — detect whether the legacy tftpd service is active in
  # /etc/inetd.conf.  An active tftpd entry exposes a clear-text service
  # that can be abused for amplification or as a foothold; CIS requires it
  # disabled.  Capture: grep for the exact first-field token tftpd followed
  # by whitespace (not a comment, not tftpdd, not a later-field match).
  #
  # Verdict logic:
  #   rc=0  AND  every nonblank row starts with exactly "tftpd "  => FAIL
  #   rc=1  AND  empty output                                       => PASS
  #   anything else                                                 => NOT_ASSESSED
  #
  typeset ID_TFTPD_RC ID_TFTPD_RAW ID_TFTPD_PARSED ID_TFTPD_SHAPE
  typeset ID_TFTPD_STATUS ID_TFTPD_OBSERVED ID_TFTPD_MEANING
  typeset ID_TFTPD_FIX SEVERITY
  ID_TFTPD_RC=0
  ID_TFTPD_RAW=""
  ID_TFTPD_PARSED="absent"
  ID_TFTPD_SHAPE=1

  ID_TFTPD_RAW=$(aix inetd_tftpd grep -E '^[	 ]*tftpd[	 ]' /etc/inetd.conf)
  ID_TFTPD_RC=$?

  if [ "$ID_TFTPD_RC" -eq 0 ] && [ -n "$ID_TFTPD_RAW" ]; then
    # Validate that every nonblank row has first field exactly "tftpd".
    ID_TFTPD_PARSED=$(printf '%s\n' "$ID_TFTPD_RAW" | awk '
      /^[ \t]*$/ { next }
      {
        rows++
    ...
    case "$ID_TFTPD_PARSED" in
      single) ID_TFTPD_SHAPE=1 ;;
      multiple) ID_TFTPD_SHAPE=1 ;;
      *) ID_TFTPD_SHAPE=0 ;;
    esac
  fi
  ...
  case "$ID_TFTPD_RC" in
    0)
      if [ "$ID_TFTPD_SHAPE" -eq 1 ] && [ "$ID_TFTPD_PARSED" != "absent" ]; then
        ...
        case "$ID_TFTPD_PARSED" in
          non-tftpd-first)
            ID_TFTPD_OBSERVED="not assessed - tftpd inetd probe captured output contains non-tftpd-first rows" ;;
          *)
      ...
      ;;
    1)
      if [ -z "$ID_TFTPD_RAW" ]; then
        ID_TFTPD_STATUS=PASS
        ID_TFTPD_OBSERVED="tftpd not active in /etc/inetd.conf"
        ID_TFTPD_MEANING="The inetd service is not enabled."
        ID_TFTPD_FIX="n/a"
      else
        ID_TFTPD_STATUS=NOT_ASSESSED
  ...
  case "$ID_TFTPD_STATUS" in
    FAIL) SEVERITY=high ;;
    *) SEVERITY=low ;;
  esac

  add security inetd_tftpd "inetd tftpd service" \
    "$ID_TFTPD_STATUS" "$SEVERITY" \
    "$ID_TFTPD_OBSERVED" \
    "$ID_TFTPD_MEANING" \
    "$ID_TFTPD_FIX" \
    "cis-l1"
