...
AIXRAY_TOOL=ck-inetd-time


function standalone_check {
_AIXRAY_SESSION_KEYS=""

  # inetd_time — detect whether the legacy time service is active in
  # /etc/inetd.conf.  An active time entry exposes a clear-text service
  # that can be abused for amplification or as a foothold; CIS requires it
  # disabled.  Capture: grep for the exact first-field token time followed
  # by whitespace (not a comment, not timed, not a later-field match).
  #
  # Verdict logic:
  #   rc=0  AND  every nonblank row starts with exactly "time "  => FAIL
  #   rc=1  AND  empty output                                       => PASS
  #   anything else                                                 => NOT_ASSESSED
  #
  typeset ID_TIME_RC ID_TIME_RAW ID_TIME_PARSED ID_TIME_SHAPE
  typeset ID_TIME_STATUS ID_TIME_OBSERVED ID_TIME_MEANING
  typeset ID_TIME_FIX SEVERITY
  ID_TIME_RC=0
  ID_TIME_RAW=""
  ID_TIME_PARSED="absent"
  ID_TIME_SHAPE=1

  ID_TIME_RAW=$(aix inetd_time grep -E '^[	 ]*time[	 ]' /etc/inetd.conf)
  ID_TIME_RC=$?

  if [ "$ID_TIME_RC" -eq 0 ] && [ -n "$ID_TIME_RAW" ]; then
    # Validate that every nonblank row has first field exactly "time".
    ID_TIME_PARSED=$(printf '%s\n' "$ID_TIME_RAW" | awk '
      /^[ \t]*$/ { next }
      {
        rows++
    ...
    case "$ID_TIME_PARSED" in
      single) ID_TIME_SHAPE=1 ;;
      multiple) ID_TIME_SHAPE=1 ;;
      *) ID_TIME_SHAPE=0 ;;
    esac
  fi
  ...
  case "$ID_TIME_RC" in
    0)
      if [ "$ID_TIME_SHAPE" -eq 1 ] && [ "$ID_TIME_PARSED" != "absent" ]; then
        ID_TIME_STATUS=FAIL
        ...
        case "$ID_TIME_PARSED" in
          non-time-first)
            ID_TIME_OBSERVED="not assessed - time inetd probe captured output contains non-time-first rows" ;;
          *)
      ...
      ;;
    1)
      if [ -z "$ID_TIME_RAW" ]; then
        ID_TIME_STATUS=PASS
        ID_TIME_OBSERVED="time not active in /etc/inetd.conf"
        ID_TIME_MEANING="The inetd service is not enabled."
        ID_TIME_FIX="n/a"
      else
        ID_TIME_STATUS=NOT_ASSESSED
  ...
  case "$ID_TIME_STATUS" in
    FAIL) SEVERITY=high ;;
    *) SEVERITY=low ;;
  esac

  add security inetd_time "inetd time service" \
    "$ID_TIME_STATUS" "$SEVERITY" \
    "$ID_TIME_OBSERVED" \
    "$ID_TIME_MEANING" \
    "$ID_TIME_FIX" \
    "cis-l1"
