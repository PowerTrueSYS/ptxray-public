...
AIXRAY_TOOL=ck-inetd-uucp


function standalone_check {
_AIXRAY_SESSION_KEYS=""

  # inetd_uucp — detect whether the legacy uucp service is active in
  # /etc/inetd.conf.  An active uucp entry exposes a clear-text service
  # that can be abused for amplification or as a foothold; CIS requires it
  # disabled.  Capture: grep for the exact first-field token uucp followed
  # by whitespace (not a comment, not uucpd, not a later-field match).
  #
  # Verdict logic:
  #   rc=0  AND  every nonblank row starts with exactly "uucp "  => FAIL
  #   rc=1  AND  empty output                                       => PASS
  #   anything else                                                 => NOT_ASSESSED
  #
  typeset ID_UUCP_RC ID_UUCP_RAW ID_UUCP_PARSED ID_UUCP_SHAPE
  typeset ID_UUCP_STATUS ID_UUCP_OBSERVED ID_UUCP_MEANING
  typeset ID_UUCP_FIX SEVERITY
  ID_UUCP_RC=0
  ID_UUCP_RAW=""
  ID_UUCP_PARSED="absent"
  ID_UUCP_SHAPE=1

  ID_UUCP_RAW=$(aix inetd_uucp grep -E '^[	 ]*uucp[	 ]' /etc/inetd.conf)
  ID_UUCP_RC=$?

  if [ "$ID_UUCP_RC" -eq 0 ] && [ -n "$ID_UUCP_RAW" ]; then
    # Validate that every nonblank row has first field exactly "uucp".
    ID_UUCP_PARSED=$(printf '%s\n' "$ID_UUCP_RAW" | awk '
      /^[ \t]*$/ { next }
      {
        rows++
    ...
    case "$ID_UUCP_PARSED" in
      single) ID_UUCP_SHAPE=1 ;;
      multiple) ID_UUCP_SHAPE=1 ;;
      *) ID_UUCP_SHAPE=0 ;;
    esac
  fi
  ...
  case "$ID_UUCP_RC" in
    0)
      if [ "$ID_UUCP_SHAPE" -eq 1 ] && [ "$ID_UUCP_PARSED" != "absent" ]; then
        ID_UUCP_STATUS=FAIL
        ...
        case "$ID_UUCP_PARSED" in
          non-uucp-first)
            ID_UUCP_OBSERVED="not assessed - uucp inetd probe captured output contains non-uucp-first rows" ;;
          *)
      ...
      ;;
    1)
      if [ -z "$ID_UUCP_RAW" ]; then
        ID_UUCP_STATUS=PASS
        ID_UUCP_OBSERVED="uucp not active in /etc/inetd.conf"
        ID_UUCP_MEANING="The inetd service is not enabled."
        ID_UUCP_FIX="n/a"
      else
        ID_UUCP_STATUS=NOT_ASSESSED
  ...
  case "$ID_UUCP_STATUS" in
    FAIL) SEVERITY=high ;;
    *) SEVERITY=low ;;
  esac

  add security inetd_uucp "inetd uucp service" \
    "$ID_UUCP_STATUS" "$SEVERITY" \
    "$ID_UUCP_OBSERVED" \
    "$ID_UUCP_MEANING" \
    "$ID_UUCP_FIX" \
    "cis-l1"
