...
AIXRAY_TOOL=ck-inetd-xmquery


function standalone_check {
_AIXRAY_SESSION_KEYS=""

  # inetd_xmquery — detect whether the legacy xmquery service is active in
  # /etc/inetd.conf.  An active xmquery entry exposes a clear-text service
  # that can be abused for amplification or as a foothold; CIS requires it
  # disabled.  Capture: grep for the exact first-field token xmquery followed
  # by whitespace (not a comment, not xmqueryd, not a later-field match).
  #
  # Verdict logic:
  #   rc=0  AND  every nonblank row starts with exactly "xmquery "  => FAIL
  #   rc=1  AND  empty output                                       => PASS
  #   anything else                                                 => NOT_ASSESSED
  #
  typeset ID_XMQUERY_RC ID_XMQUERY_RAW ID_XMQUERY_PARSED ID_XMQUERY_SHAPE
  typeset ID_XMQUERY_STATUS ID_XMQUERY_OBSERVED ID_XMQUERY_MEANING
  typeset ID_XMQUERY_FIX SEVERITY
  ID_XMQUERY_RC=0
  ID_XMQUERY_RAW=""
  ID_XMQUERY_PARSED="absent"
  ID_XMQUERY_SHAPE=1

  ID_XMQUERY_RAW=$(aix inetd_xmquery grep -E '^[	 ]*xmquery[	 ]' /etc/inetd.conf)
  ID_XMQUERY_RC=$?

  if [ "$ID_XMQUERY_RC" -eq 0 ] && [ -n "$ID_XMQUERY_RAW" ]; then
    # Validate that every nonblank row has first field exactly "xmquery".
    ID_XMQUERY_PARSED=$(printf '%s\n' "$ID_XMQUERY_RAW" | awk '
      /^[ \t]*$/ { next }
      {
        rows++
    ...
    case "$ID_XMQUERY_PARSED" in
      single) ID_XMQUERY_SHAPE=1 ;;
      multiple) ID_XMQUERY_SHAPE=1 ;;
      *) ID_XMQUERY_SHAPE=0 ;;
    esac
  fi

  # Compute verdict, then severity, then emit single finding.
  case "$ID_XMQUERY_RC" in
    0)
      if [ "$ID_XMQUERY_SHAPE" -eq 1 ] && [ "$ID_XMQUERY_PARSED" != "absent" ]; then
        ...
        case "$ID_XMQUERY_PARSED" in
          non-xmquery-first)
            ID_XMQUERY_OBSERVED="not assessed - xmquery inetd probe captured output contains non-xmquery-first rows" ;;
          *)
      ...
      ;;
    1)
      if [ -z "$ID_XMQUERY_RAW" ]; then
        ID_XMQUERY_STATUS=PASS
        ID_XMQUERY_OBSERVED="xmquery not active in /etc/inetd.conf"
        ID_XMQUERY_MEANING="The inetd service is not enabled."
        ID_XMQUERY_FIX="n/a"
      else
        ID_XMQUERY_STATUS=NOT_ASSESSED
  ...
  case "$ID_XMQUERY_STATUS" in
    FAIL) SEVERITY=high ;;
    *) SEVERITY=low ;;
  esac

  add security inetd_xmquery "inetd xmquery service" \
    "$ID_XMQUERY_STATUS" "$SEVERITY" \
    "$ID_XMQUERY_OBSERVED" \
    "$ID_XMQUERY_MEANING" \
    "$ID_XMQUERY_FIX" \
    "cis-l1"
