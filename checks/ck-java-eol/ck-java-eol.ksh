...
if [ "$VIOS_MARKER_RC" -eq 0 ] && [ "$VIOS_MARKER_MATCH" -eq 1 ] && [ "$VIOS_MARKER_NONEMPTY" -eq 1 ]; then
  IS_VIOS=1; ROLE=vios
elif [ "$VIOS_MARKER_RC" -ne 0 ] && [ "$VIOS_MARKER_NONEMPTY" -eq 0 ]; then
  IS_VIOS=0; ROLE=aix
else
  # A failed command or rc=0 garbage is not positive proof of a plain AIX role.
  # Keep the scanner operational, but record the role as unknown.
  IS_VIOS=0; ROLE=unknown
fi

  # java_eol — IBM SDK Java on AIX lifecycle (Java 7 and earlier out of service; Java 8 is the last).
  # Colon 'lslpp -qcL Java*' (field 2=fileset, 3=level). The established Java parse keys on the
  ...
  if [ "$RC" -eq 0 ]; then
    HAS_EOL=$(printf '%s\n' "$JAVALP" | awk -F: '$2 ~ /^Java([0-7]|13|131|14|142)([._]|$)/{f=1} END{print f+0}')
    HAS8=$(printf '%s\n' "$JAVALP" | awk -F: '$2 ~ /^Java8/{f=1} END{print f+0}')
    V_EOL=$(printf '%s\n' "$JAVALP" | awk -F: '$2 ~ /^Java([0-7]|13|131|14|142)([._]|$)/{if(!seen[$2]){seen[$2]=1;ord[++n]=$2}v[$2]=$3} END{for(i=1;i<=n;i++){printf "%s%s %s",(i>1?", ":""),ord[i],v[ord[i]]}}')
    V8=$(printf '%s\n' "$JAVALP" | awk -F: '$2 ~ /^Java8/{if(!seen[$2]){seen[$2]=1;ord[++n]=$2}v[$2]=$3} END{for(i=1;i<=n;i++){printf "%s%s %s",(i>1?", ":""),ord[i],v[ord[i]]}}')
    if [ "$HAS_EOL" -eq 1 ]; then
      add patch java_eol "IBM SDK Java lifecycle" WARN high "Java 7 or earlier installed (${V_EOL:-unknown})" \
          "IBM SDK Java 7 and earlier reached end of service on AIX (Java 7/7.1 ended 2022-09-30) — these runtimes no longer receive security fixes." \
          "move applications to Java 8 (the last IBM SDK on AIX) or IBM Semeru; then remove the end-of-service filesets."
    elif [ "$HAS8" -eq 1 ]; then
    ...
    if [ "$IS_VIOS" -eq 1 ] && [ "$NFIND" -gt 0 ] && [ "${F_ID[$((NFIND-1))]}" = "java_eol" ]; then
    ...
    # No Java installed and an unreadable inventory arrive as the same probe
    # result (non-zero rc, empty stdout); neither may read as a clean
    # inventory, so the finding is adverse and names both states.
    add patch java_eol "IBM SDK Java lifecycle" WARN high \
        "No Java fileset inventory was read (lslpp -qcL 'Java*' rc=$RC); either no Java is installed or the inventory is unreadable" \
        "Without a readable Java inventory, end-of-service status cannot be verified; an empty inventory is not proof of a compliant state." \
        "confirm whether IBM SDK Java is installed and that lslpp reads the software inventory; the control is unresolved until it does."
  fi
}
