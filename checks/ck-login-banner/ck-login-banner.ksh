...
AIXRAY_TOOL=ck-login-banner


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # login_banner — DISA STIG V-215200.
  ...
  # the fix text is advisory only.
  typeset LB_REQ_TEXT LB_RAW LB_RC LB_HERALD LB_REASON
  LB_REQ_TEXT='You are accessing a U.S. Government (USG) Information System (IS) that is provided for USG-authorized use only. By using this IS (which includes any device attached to this IS), you consent to the following conditions: -The USG routinely intercepts and monitors communications on this IS for purposes including, but not limited to, penetration testing, COMSEC monitoring, network operations and defense, personnel misconduct (PM), law enforcement (LE), and counterintelligence (CI) investigations. -At any time, the USG may inspect and seize data stored on this IS. -Communications using, or data stored on, this IS are not private, are subject to routine monitoring, interception, and search, and may be disclosed or used for any USG-authorized purpose. -This IS includes security measures (e.g., authentication and access controls) to protect USG interests--not for your personal benefit or privacy. -Notwithstanding the above, using this IS does not constitute consent to PM, LE or CI investigative searching or monitoring of the content of privileged communications, or work product, related to personal representation or services by attorneys, psychotherapists, or clergy, and their assistants. Such communications and work product are private and confidential. See User Agreement for details.'
  LB_RAW=$(aix login_herald lssec -f /etc/security/login.cfg -s default -a herald)
  LB_RC=$?
  LB_HERALD=""
  LB_REASON=""
  if [ "$LB_RC" -ne 0 ]; then
    LB_REASON="lssec failed (rc=$LB_RC)"
  elif [ -z "$LB_RAW" ]; then
    LB_REASON="lssec returned no output"
  else
    LB_HERALD=$(printf '%s\n' "$LB_RAW" | awk '
    ...
    ')
    if [ $? -ne 0 ]; then
      LB_REASON="unparseable lssec output"
    else
      case "$LB_HERALD" in
        '""') LB_HERALD="" ;;
        \"*\") LB_HERALD=${LB_HERALD#\"}; LB_HERALD=${LB_HERALD%\"} ;;
      esac
    fi
  fi
  if [ -n "$LB_REASON" ]; then
    add security login_banner "Login banner" NOT_ASSESSED med \
        "not assessed - $LB_REASON" \
        "PTxray could not obtain a trustworthy herald value to compare against the DoD banner." \
        "run 'lssec -f /etc/security/login.cfg -s default -a herald' as root and rerun PTxray." \
        "stig:V-215200"
  elif [ -z "$LB_HERALD" ]; then
    add security login_banner "Login banner" FAIL med \
        "no herald set" \
        "The default login herald is empty, so login proceeds without the required DoD notice." \
        "set the default herald to the Standard Mandatory DoD Notice and Consent Banner (chsec -a herald=\"...\")." \
        "stig:V-215200"
  elif printf '%s\n' "$LB_HERALD" | awk -v req="$LB_REQ_TEXT" '
      { s = s $0 ORS }
      END {
        r = req
        gsub(/\\n/, " ", s); gsub(/\\r/, " ", s)
    ...
    add security login_banner "Login banner" PASS med \
        "herald contains the Standard Mandatory DoD Notice and Consent Banner" \
        "The default login herald carries the full DoD notice required by V-215200." \
        "n/a" "stig:V-215200"
  else
    add security login_banner "Login banner" FAIL med \
