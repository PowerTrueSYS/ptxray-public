...
AIXRAY_TOOL=ck-login-logindelay
...
# ck-login-logindelay
#
# logindelay is the number of seconds the login process waits between
# failed login attempts; AIX multiplies the delay by the number of failed
# attempts (10, 20, 30... seconds). CIS AIX 7 v1.2.0 section 4.7.2
# requires logindelay >= 10 to slow automated brute-force login attempts.
#
# The threshold is a floor: the attribute must be present and at least 10.
# Zero or absent means the AIX default (no delay between attempts), which
# CIS flags as non-compliant. Threshold source: Tenable audit-item
# reproductions of the CIS AIX benchmark family (chsec ... logindelay=10).
# TODO-VERIFY: confirm the exact CIS_IBM_AIX_7_v1.2.0_L1.audit item
# before customer use.

  typeset LOGIN_LD_RAW LOGIN_LD_RC LOGIN_LD_NONBLANK LOGIN_LD_LINE
  typeset LOGIN_LD_VALUE LOGIN_LD_DIGITS LOGIN_LD_NORM LOGIN_LD_LEN
  typeset LOGIN_LD_REASON LOGIN_LD_FAILURE LOGIN_LD_STATUS
  LOGIN_LD_RAW=""
  LOGIN_LD_RC=126
  LOGIN_LD_REASON=""
  LOGIN_LD_FAILURE=""
  ...
  if [ "${MYUID:-0}" != "0" ]; then
    LOGIN_LD_REASON="requires root privileges"
  else
    LOGIN_LD_RAW=$(aix login_logindelay lssec -f /etc/security/login.cfg -s default -a logindelay)
    LOGIN_LD_RC=$?
    if [ "$LOGIN_LD_RC" -ne 0 ]; then
      LOGIN_LD_REASON="capture failed (rc=$LOGIN_LD_RC)"
    elif [ -z "$LOGIN_LD_RAW" ]; then
      LOGIN_LD_REASON="capture returned empty output"
    else
      LOGIN_LD_NONBLANK=$(printf '%s\n' "$LOGIN_LD_RAW" |
        awk 'NF { count++ } END { print count + 0 }')
      if [ "$LOGIN_LD_NONBLANK" -ne 1 ]; then
        LOGIN_LD_REASON="output was not one exact assignment"
      else
        LOGIN_LD_LINE=$(printf '%s\n' "$LOGIN_LD_RAW" |
          awk 'NF { print; exit }')
        case "$LOGIN_LD_LINE" in
          "default logindelay="*)
            LOGIN_LD_VALUE=${LOGIN_LD_LINE#default logindelay=}
            case "$LOGIN_LD_VALUE" in
              "")
                LOGIN_LD_FAILURE="logindelay absent; AIX default 0 (no delay between failed attempts)"
                LOGIN_LD_STATUS=FAIL
                ;;
              -[0-9]*)
                LOGIN_LD_DIGITS=${LOGIN_LD_VALUE#-}
                case "$LOGIN_LD_DIGITS" in
                  ""|*[!0-9]*)
                    LOGIN_LD_REASON="value was not a signed decimal integer"
                    ;;
                  *)
                    LOGIN_LD_FAILURE="logindelay=$LOGIN_LD_VALUE seconds (invalid non-positive value)"
                    LOGIN_LD_STATUS=FAIL
                    ;;
                esac
                ;;
              [0-9]*)
                case "$LOGIN_LD_VALUE" in
                  *[!0-9]*)
                    ...
                    ;;
                  *)
                    LOGIN_LD_NORM=$LOGIN_LD_VALUE
                    while [ "${LOGIN_LD_NORM#0}" != "$LOGIN_LD_NORM" ]; do
                      LOGIN_LD_NORM=${LOGIN_LD_NORM#0}
                    done
                    [ -n "$LOGIN_LD_NORM" ] || LOGIN_LD_NORM=0
                    if [ "$LOGIN_LD_NORM" -ge 10 ]; then
                      LOGIN_LD_STATUS=PASS
                    else
                      LOGIN_LD_FAILURE="logindelay=$LOGIN_LD_VALUE seconds (below 10-second minimum)"
                      LOGIN_LD_STATUS=FAIL
                    fi
                    ;;
                esac
            ...
            ;;
          *)
            LOGIN_LD_REASON="output did not contain the exact requested assignment"
            ;;
        esac
      fi
    fi
  ...
  if [ -n "$LOGIN_LD_REASON" ]; then
    ...
    add security login_logindelay "Terminal login prompt delay" PASS med \
        "logindelay=$LOGIN_LD_VALUE seconds" \
        "Failed login attempts are throttled by an escalating delay of at least 10 seconds, slowing brute-force attacks." \
        "n/a" "cis-l1"
  else
    add security login_logindelay "Terminal login prompt delay" FAIL med \
        "$LOGIN_LD_FAILURE" \
        ...
        "set logindelay to 10 or greater: chsec -f /etc/security/login.cfg -s default -a logindelay=10." \
        "cis-l1"
  fi
}
