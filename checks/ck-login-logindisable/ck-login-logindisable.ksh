...
AIXRAY_TOOL=ck-login-logindisable


function standalone_check {
_AIXRAY_SESSION_KEYS=""
# logindisable is the number of unsuccessful logins allowed before an AIX
# physical TTY port locks. Trust only one exact attribute-scoped assignment.
  ...
  if [ "${MYUID:-0}" != "0" ]; then
    ...
    LOGIN_LD_RAW=$(aix login_logindisable lssec -f /etc/security/login.cfg -s default -a logindisable)
    LOGIN_LD_RC=$?
    if [ "$LOGIN_LD_RC" -ne 0 ]; then
      ...
      if [ "$LOGIN_LD_NONBLANK" -ne 1 ]; then
        ...
        case "$LOGIN_LD_LINE" in
          "default logindisable="*)
            LOGIN_LD_VALUE=${LOGIN_LD_LINE#default logindisable=}
            case "$LOGIN_LD_VALUE" in
              "")
                LOGIN_LD_FAILURE="logindisable absent; AIX default 0 (port lock disabled)"
              ...
              -[0-9]*)
                ...
                case "$LOGIN_LD_DIGITS" in
                  ""|*[!0-9]*)
                    ...
                    ;;
                  *)
                    LOGIN_LD_FAILURE="logindisable=$LOGIN_LD_VALUE attempts (invalid non-positive value)"
                    LOGIN_LD_STATUS=FAIL
                    ;;
                esac
                ;;
              [0-9]*)
                case "$LOGIN_LD_VALUE" in
                  ...
                  *)
                    ...
                    LOGIN_LD_LEN=${#LOGIN_LD_NORM}
                    case "$LOGIN_LD_NORM:$LOGIN_LD_LEN" in
                      0:1)
                        LOGIN_LD_FAILURE="logindisable=$LOGIN_LD_VALUE attempts (port lock disabled)"
                        LOGIN_LD_STATUS=FAIL
                        ;;
                      [1-9]:1|10:2)
                        ...
                        ;;
                      *)
                        LOGIN_LD_FAILURE="logindisable=$LOGIN_LD_VALUE attempts (exceeds 10-attempt maximum)"
                        LOGIN_LD_STATUS=FAIL
                        ;;
                    esac
                    ;;
  ...
  if [ -n "$LOGIN_LD_REASON" ]; then
    ...
    add security login_logindisable "Terminal failed-login lock threshold" PASS med \
        "logindisable=$LOGIN_LD_VALUE attempts" \
        "Default TTY ports lock after no more than ten unsuccessful login attempts." \
        "n/a" ""
  else
