...
AIXRAY_TOOL=ck-login-logininterval


function standalone_check {
_AIXRAY_SESSION_KEYS=""
# logininterval is the seconds window in which unsuccessful logins count
# toward locking an AIX physical TTY port. Trust one exact assignment only.

  typeset LOGIN_LI_RAW LOGIN_LI_RC LOGIN_LI_NONBLANK LOGIN_LI_LINE
  typeset LOGIN_LI_VALUE LOGIN_LI_DIGITS LOGIN_LI_NORM LOGIN_LI_LEN
  typeset LOGIN_LI_REASON LOGIN_LI_FAILURE LOGIN_LI_STATUS
  LOGIN_LI_RAW=""
  LOGIN_LI_RC=126
  LOGIN_LI_REASON=""
  LOGIN_LI_FAILURE=""
  ...
  if [ "${MYUID:-0}" != "0" ]; then
    LOGIN_LI_REASON="requires root privileges"
  else
    LOGIN_LI_RAW=$(aix login_logininterval lssec -f /etc/security/login.cfg -s default -a logininterval)
    LOGIN_LI_RC=$?
    if [ "$LOGIN_LI_RC" -ne 0 ]; then
      LOGIN_LI_REASON="capture failed (rc=$LOGIN_LI_RC)"
    elif [ -z "$LOGIN_LI_RAW" ]; then
      LOGIN_LI_REASON="capture returned empty output"
    else
      LOGIN_LI_NONBLANK=$(printf '%s\n' "$LOGIN_LI_RAW" |
        awk 'NF { count++ } END { print count + 0 }')
      if [ "$LOGIN_LI_NONBLANK" -ne 1 ]; then
        LOGIN_LI_REASON="output was not one exact assignment"
      else
        LOGIN_LI_LINE=$(printf '%s\n' "$LOGIN_LI_RAW" |
          awk 'NF { print; exit }')
        case "$LOGIN_LI_LINE" in
          "default logininterval="*)
            LOGIN_LI_VALUE=${LOGIN_LI_LINE#default logininterval=}
            case "$LOGIN_LI_VALUE" in
              "")
                LOGIN_LI_FAILURE="logininterval absent; AIX default 0 (no interval)"
                LOGIN_LI_STATUS=FAIL
                ;;
              -[0-9]*)
                LOGIN_LI_DIGITS=${LOGIN_LI_VALUE#-}
                case "$LOGIN_LI_DIGITS" in
                  ""|*[!0-9]*)
                    LOGIN_LI_REASON="value was not a signed decimal integer"
                    ;;
                  *)
                    LOGIN_LI_FAILURE="logininterval=$LOGIN_LI_VALUE seconds (invalid non-positive value)"
                    LOGIN_LI_STATUS=FAIL
                    ;;
                esac
                ;;
              [0-9]*)
                case "$LOGIN_LI_VALUE" in
                  *[!0-9]*)
                    ...
                    ;;
                  *)
                    LOGIN_LI_NORM=$LOGIN_LI_VALUE
                    while [ "${LOGIN_LI_NORM#0}" != "$LOGIN_LI_NORM" ]; do
                      LOGIN_LI_NORM=${LOGIN_LI_NORM#0}
                    done
                    [ -n "$LOGIN_LI_NORM" ] || LOGIN_LI_NORM=0
                    LOGIN_LI_LEN=${#LOGIN_LI_NORM}
                    case "$LOGIN_LI_NORM:$LOGIN_LI_LEN" in
                      0:1)
                        LOGIN_LI_FAILURE="logininterval=$LOGIN_LI_VALUE seconds (counting window disabled)"
                        LOGIN_LI_STATUS=FAIL
                        ;;
                      [1-9]:1|[1-9][0-9]:2|[12][0-9][0-9]:3|300:3)
                        LOGIN_LI_STATUS=PASS
                        ;;
                      *)
                        LOGIN_LI_FAILURE="logininterval=$LOGIN_LI_VALUE seconds (exceeds 300-second maximum)"
                        LOGIN_LI_STATUS=FAIL
                        ;;
                    esac
                    ;;
            ...
            ;;
          *)
            LOGIN_LI_REASON="output did not contain the exact requested assignment"
            ;;
        esac
      fi
    fi
  ...
  if [ -n "$LOGIN_LI_REASON" ]; then
    ...
    add security login_logininterval "Terminal failed-login interval" PASS med \
        "logininterval=$LOGIN_LI_VALUE seconds" \
        "The default TTY failed-login interval is a positive value no greater than 300 seconds." \
        "n/a" ""
  else
