...
AIXRAY_TOOL=ck-login-loginretries


function standalone_check {
_AIXRAY_SESSION_KEYS=""
# ck-login-loginretries
#
# loginretries is the maximum number of failed login attempts allowed before
# the account is locked. CIS AIX 7 v1.2.0 section 4.7.3 requires loginretries
# <= 3 to prevent brute-force password attacks.
#
# The threshold is a ceiling: the attribute must be present, positive, and
# at most 3. Zero or absent means the AIX default (unlimited retries), which
# CIS flags as non-compliant. Values above 3 weaken account lockout.

  typeset LOGIN_RT_RAW LOGIN_RT_RC LOGIN_RT_NONBLANK LOGIN_RT_LINE
  typeset LOGIN_RT_VALUE LOGIN_RT_DIGITS LOGIN_RT_NORM LOGIN_RT_LEN
  typeset LOGIN_RT_REASON LOGIN_RT_FAILURE LOGIN_RT_STATUS
  LOGIN_RT_RAW=""
  LOGIN_RT_RC=126
  LOGIN_RT_REASON=""
  LOGIN_RT_FAILURE=""
  ...
  if [ "${MYUID:-0}" != "0" ]; then
    LOGIN_RT_REASON="requires root privileges"
  else
    LOGIN_RT_RAW=$(aix loginretries lssec -f /etc/security/user -s default -a loginretries)
    LOGIN_RT_RC=$?
    if [ "$LOGIN_RT_RC" -ne 0 ]; then
      LOGIN_RT_REASON="capture failed (rc=$LOGIN_RT_RC)"
    elif [ -z "$LOGIN_RT_RAW" ]; then
      LOGIN_RT_REASON="capture returned empty output"
    else
      LOGIN_RT_NONBLANK=$(printf '%s\n' "$LOGIN_RT_RAW" |
        awk 'NF { count++ } END { print count + 0 }')
      if [ "$LOGIN_RT_NONBLANK" -ne 1 ]; then
        LOGIN_RT_REASON="output was not one exact assignment"
      else
        LOGIN_RT_LINE=$(printf '%s\n' "$LOGIN_RT_RAW" |
          awk 'NF { print; exit }')
        case "$LOGIN_RT_LINE" in
          "default loginretries="*)
            LOGIN_RT_VALUE=${LOGIN_RT_LINE#default loginretries=}
            case "$LOGIN_RT_VALUE" in
              "")
                LOGIN_RT_FAILURE="loginretries absent; AIX default unlimited (no lockout)"
                LOGIN_RT_STATUS=FAIL
                ;;
              -[0-9]*)
                LOGIN_RT_DIGITS=${LOGIN_RT_VALUE#-}
                case "$LOGIN_RT_DIGITS" in
                  ""|*[!0-9]*)
                    LOGIN_RT_REASON="value was not a signed decimal integer"
                    ;;
                  *)
                    LOGIN_RT_FAILURE="loginretries=$LOGIN_RT_VALUE (invalid non-positive value)"
                    LOGIN_RT_STATUS=FAIL
                    ;;
                esac
                ;;
              [0-9]*)
                case "$LOGIN_RT_VALUE" in
                  *[!0-9]*)
                    ...
                    ;;
                  *)
                    LOGIN_RT_NORM=$LOGIN_RT_VALUE
                    while [ "${LOGIN_RT_NORM#0}" != "$LOGIN_RT_NORM" ]; do
                      LOGIN_RT_NORM=${LOGIN_RT_NORM#0}
                    done
                    [ -n "$LOGIN_RT_NORM" ] || LOGIN_RT_NORM=0
                    if [ "$LOGIN_RT_NORM" -eq 0 ]; then
                      LOGIN_RT_FAILURE="loginretries=$LOGIN_RT_VALUE (invalid non-positive value)"
                      LOGIN_RT_STATUS=FAIL
                    elif [ "$LOGIN_RT_NORM" -le 3 ]; then
                      LOGIN_RT_STATUS=PASS
                    else
                      LOGIN_RT_FAILURE="loginretries=$LOGIN_RT_VALUE (above 3-retry maximum)"
                      LOGIN_RT_STATUS=FAIL
                    fi
                    ;;
                esac
            ...
            ;;
          *)
            LOGIN_RT_REASON="output did not contain the exact requested assignment"
            ;;
        esac
      fi
    fi
  ...
  if [ -n "$LOGIN_RT_REASON" ]; then
    ...
    add security login_retries "Account login retry limit" PASS med \
        "loginretries=$LOGIN_RT_VALUE" \
        "Account lockout triggers after at most 3 failed login attempts, limiting brute-force risk." \
        "n/a" "cis-l1"
  else
    add security login_retries "Account login retry limit" FAIL med \
        "$LOGIN_RT_FAILURE" \
        "loginretries is zero or unset, which means unlimited login attempts — a CIS AIX 7 v1.2.0 section 4.7.3 finding." \
        "set loginretries to 3 or less (but > 0): chsec -f /etc/security/user -s default -a loginretries=<N>." \
        "cis-l1"
  fi
}
