...
AIXRAY_TOOL=ck-login-logintimeout


function standalone_check {
_AIXRAY_SESSION_KEYS=""
# ck-login-logintimeout
#
# logintimeout is the number of seconds a user has to finish typing the
# password once the login prompt is displayed. CIS AIX 7 v1.2.0 section
# 4.7.4 requires logintimeout <= 30 so that stalled or abandoned login
# prompts are torn down quickly. The AIX shipped default is 60 seconds.
#
# The threshold is a ceiling: the attribute must be present, positive, and
# at most 30. An absent attribute means the AIX default of 60 applies,
# which exceeds the ceiling and fails. Zero disables the timeout and fails.
# Threshold source: Tenable audit-item reproductions of the CIS AIX
# benchmark family ("30 or less", default 60). TODO-VERIFY: confirm the
# exact CIS_IBM_AIX_7_v1.2.0_L1.audit item before customer use.

  typeset LOGIN_TO_RAW LOGIN_TO_RC LOGIN_TO_NONBLANK LOGIN_TO_LINE
  typeset LOGIN_TO_VALUE LOGIN_TO_DIGITS LOGIN_TO_NORM LOGIN_TO_LEN
  typeset LOGIN_TO_REASON LOGIN_TO_FAILURE LOGIN_TO_STATUS
  LOGIN_TO_RAW=""
  LOGIN_TO_RC=126
  LOGIN_TO_REASON=""
  LOGIN_TO_FAILURE=""
  ...
  if [ "${MYUID:-0}" != "0" ]; then
    LOGIN_TO_REASON="requires root privileges"
  else
    LOGIN_TO_RAW=$(aix logintimeout lssec -f /etc/security/login.cfg -s usw -a logintimeout)
    LOGIN_TO_RC=$?
    if [ "$LOGIN_TO_RC" -ne 0 ]; then
      LOGIN_TO_REASON="capture failed (rc=$LOGIN_TO_RC)"
    elif [ -z "$LOGIN_TO_RAW" ]; then
      LOGIN_TO_REASON="capture returned empty output"
    else
      LOGIN_TO_NONBLANK=$(printf '%s\n' "$LOGIN_TO_RAW" |
        awk 'NF { count++ } END { print count + 0 }')
      if [ "$LOGIN_TO_NONBLANK" -ne 1 ]; then
        LOGIN_TO_REASON="output was not one exact assignment"
      else
        LOGIN_TO_LINE=$(printf '%s\n' "$LOGIN_TO_RAW" |
          awk 'NF { print; exit }')
        case "$LOGIN_TO_LINE" in
          "usw logintimeout="*)
            LOGIN_TO_VALUE=${LOGIN_TO_LINE#usw logintimeout=}
            case "$LOGIN_TO_VALUE" in
              "")
                LOGIN_TO_FAILURE="logintimeout absent; AIX default 60 exceeds the 30-second maximum"
                LOGIN_TO_STATUS=FAIL
                ;;
              -[0-9]*)
                LOGIN_TO_DIGITS=${LOGIN_TO_VALUE#-}
                case "$LOGIN_TO_DIGITS" in
                  ""|*[!0-9]*)
                    LOGIN_TO_REASON="value was not a signed decimal integer"
                    ;;
                  *)
                    LOGIN_TO_FAILURE="logintimeout=$LOGIN_TO_VALUE seconds (invalid non-positive value)"
                    LOGIN_TO_STATUS=FAIL
                    ;;
                esac
                ;;
              [0-9]*)
                case "$LOGIN_TO_VALUE" in
                  *[!0-9]*)
                    ...
                    ;;
                  *)
                    LOGIN_TO_NORM=$LOGIN_TO_VALUE
                    while [ "${LOGIN_TO_NORM#0}" != "$LOGIN_TO_NORM" ]; do
                      LOGIN_TO_NORM=${LOGIN_TO_NORM#0}
                    done
                    [ -n "$LOGIN_TO_NORM" ] || LOGIN_TO_NORM=0
                    if [ "$LOGIN_TO_NORM" -eq 0 ]; then
                      LOGIN_TO_FAILURE="logintimeout=$LOGIN_TO_VALUE seconds (invalid non-positive value)"
                      LOGIN_TO_STATUS=FAIL
                    elif [ "$LOGIN_TO_NORM" -le 30 ]; then
                      LOGIN_TO_STATUS=PASS
                    else
                      LOGIN_TO_FAILURE="logintimeout=$LOGIN_TO_VALUE seconds (above 30-second maximum)"
                      LOGIN_TO_STATUS=FAIL
                    fi
                    ;;
                esac
            ...
            ;;
          *)
            LOGIN_TO_REASON="output did not contain the exact requested assignment"
            ;;
        esac
      fi
    fi
  ...
  if [ -n "$LOGIN_TO_REASON" ]; then
    ...
    add security login_timeout "Login prompt timeout" PASS med \
        "logintimeout=$LOGIN_TO_VALUE seconds" \
        "Password entry at the login prompt must complete within 30 seconds, so abandoned prompts are torn down quickly." \
        "n/a" "cis-l1"
  else
    add security login_timeout "Login prompt timeout" FAIL med \
        "$LOGIN_TO_FAILURE" \
        "logintimeout is absent, zero, or above 30 seconds — a CIS AIX 7 v1.2.0 section 4.7.4 finding." \
        "set logintimeout to 30 or less (but > 0): chsec -f /etc/security/login.cfg -s usw -a logintimeout=30." \
        "cis-l1"
  fi
}
