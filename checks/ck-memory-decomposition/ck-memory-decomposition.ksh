...
AIXRAY_TOOL=ck-memory-decomposition
...
function checks_performance {
  ...
  typeset SIZE INUSE PIN AVAIL NUMPERM AVAILPCT SIZEI AVAILI PINI INUSEI
  typeset SVMON SVMONRC COMPRAW NUMRAW COMPFACT NUMFACT VMVRC
  ...
  # fact globals the envelope reads after this returns; the standalone prelude
  # does not set them, and set -u aborts on an unset reference, so initialize.
  FACT_MEM_TOTAL= FACT_MEM_INUSE= FACT_MEM_AVAILABLE= FACT_MEM_PINNED=
  FACT_MEM_COMP= FACT_MEM_NUMPERM= FACT_MEM_PRIMARY_READ=
  ...
  SIZE=${1:-}; INUSE=${2:-}; PIN=${4:-}; AVAIL=${6:-}
  NUMPERM=$(vmv 'numperm percentage')
