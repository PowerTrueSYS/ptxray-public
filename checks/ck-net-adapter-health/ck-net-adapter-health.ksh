...
AIXRAY_TOOL=ck-net-adapter-health
...
_AIXRAY_SESSION_KEYS=""
  ...
  IFL_RAW=$(aix ifconfig_l ifconfig -a); IFL_RC=$?
  IFL=""
  if [ "$IFL_RC" -eq 0 ]; then
    IFL=$(printf '%s\n' "$IFL_RAW" | awk '
      /^[A-Za-z0-9_]+([ \t]+[A-Za-z0-9_]+)*[ \t]*$/ { for(i=1;i<=NF;i++) print $i; next }
      /^[A-Za-z0-9_]+:/ { n=index($0,":"); print substr($0,1,n-1) }
    ')
  fi
  TOTERR=0; NETSUM=""; ADOWN=""; AUNREAD=""; NADAP=0
  for IF in $IFL; do
    case "$IF" in en[0-9]*) : ;; *) continue;; esac
    ESTAT=$(aix "entstat_$IF" entstat -d "$IF"); ESTAT_RC=$?
    if [ "$ESTAT_RC" -ne 0 ] || [ -z "$ESTAT" ]; then
      AUNREAD="$AUNREAD $IF"
      continue
    fi
    NADAP=$(( NADAP + 1 ))
  ...
  if [ -n "$AUNREAD" ]; then
    add performance net_adapter_health "Adapter health" WARN med "unreadable adapter statistics:${AUNREAD}" \
        "Could not read entstat output for one or more ethernet interfaces, so their health could not be confirmed." \
        "re-run 'entstat -d <if>' manually for each interface listed and investigate why the read failed."
  elif [ "$NADAP" -eq 0 ]; then
    add performance net_adapter_health "Adapter health" WARN low "no ethernet interfaces read" \
        "Could not read adapter statistics (ifconfig -a / entstat)." "check 'ifconfig -a' and 'entstat -d <if>' manually."
  elif [ -n "$ADOWN" ]; then
