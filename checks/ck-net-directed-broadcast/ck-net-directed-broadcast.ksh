...
AIXRAY_TOOL=ck-net-directed-broadcast


function standalone_check {
_AIXRAY_SESSION_KEYS=""

  # net_directed_broadcast — assess whether the kernel drops directed
  # broadcasts.  With directed_broadcast = 1 the host amplifies Smurf-style
  # reflected floods aimed at third parties; CIS AIX network-parameter
  # baseline requires directed_broadcast = 0.
  # Capture: no -o directed_broadcast
  # Status logic (T-INSECURE-DEFAULT):
  #   rc!=0  or empty-rc0 or line with no parseable "directed_broadcast = <n>"
  #     => NOT_ASSESSED
  #   value 0 => PASS
  #   nonzero numeric => WARN med
  typeset DBCAP DBCAPRC DBPARSED DBSTATE DBVAL
  DBCAP=$(aix net_directed_broadcast no -o directed_broadcast)
  DBCAPRC=$?
  if [ "$DBCAPRC" -ne 0 ]; then
    add security net_directed_broadcast "Directed broadcast" \
      NOT_ASSESSED low \
      "not assessed — directed_broadcast capture failed (rc=$DBCAPRC)" \
      "Could not confirm whether directed broadcasts are dropped. The host may be usable as a broadcast amplification reflector." \
      "verify 'no -o directed_broadcast' is supported on this AIX level; secure value is 0." \
      "cis-l1"
  elif [ -z "$DBCAP" ]; then
    add security net_directed_broadcast "Directed broadcast" \
      NOT_ASSESSED low \
    ...
    DBPARSED=$(printf '%s\n' "$DBCAP" | awk '
      ...
      {
        rows++
        if ($0 ~ /^directed_broadcast[ \t]*=[ \t]*[0-9]+$/) {
          value=$0
          sub(/^directed_broadcast[ \t]*=[ \t]*/, "", value)
          sub(/^0+/, "", value)
          if (value == "") value="0"
          matches++
          remember(value)
        } else if ($0 ~ /^directed_broadcast[ \t]*=/) {
          malformed=1
        } else {
          other=1
        }
      ...
      END {
        if (matches > 1 && distinct > 1) print "conflict|" values
        else if (matches > 1) print "multiple|" values
        else if (matches == 1 && rows == 1 && !malformed && !other)
          print "value|" values
        else if (malformed || matches > 0) print "bad|"
        else print "absent|"
      }
    ')
    DBSTATE=${DBPARSED%%\|*}
    ...
    case "$DBSTATE" in
      value)
        if [ "$DBVAL" = "0" ]; then
          add security net_directed_broadcast "Directed broadcast" PASS low \
            "directed_broadcast = 0 (dropped)" \
            "Directed broadcasts are dropped. The host cannot be used to amplify Smurf-style reflected floods aimed at third parties." \
            "n/a" \
            "cis-l1"
        else
          add security net_directed_broadcast "Directed broadcast" WARN med \
            "directed_broadcast = $DBVAL (not dropped)" \
            "Directed broadcasts are NOT dropped (value $DBVAL). The host can be used as a broadcast amplification reflector for Smurf-style reflected floods aimed at third-party targets." \
            "set 'no -p -o directed_broadcast=0' to drop directed broadcasts and persist across reboot." \
            "cis-l1"
        fi
        ;;
      conflict)
