...
AIXRAY_TOOL=ck-net-icmpaddressmask


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # icmpaddressmask — ICMP address mask reply suppression. AIX sends
  # ICMP Address Mask Replies that disclose the local subnet mask; CIS
  # AIX hardening requires icmpaddressmask = 0. Capture:
  # 'no -o icmpaddressmask' prints "icmpaddressmask = <n>".
  ICMPAM=$(aix no_icmpaddressmask no -o icmpaddressmask)
  ICMPAM_RC=$?
  ICMPAM_VAL=""
  ICMPAM_SHAPE=1
  if [ "$ICMPAM_RC" -eq 0 ] && [ -n "$ICMPAM" ]; then
    # Must contain exactly one line of the shape "icmpaddressmask = <value>"
    ICMPAM_VAL=$(printf '%s\n' "$ICMPAM" | awk '
      /^[ \t]*icmpaddressmask[ \t]+=[ \t]+/ {
        v=$0; sub(/^[ \t]*icmpaddressmask[ \t]+=[ \t]+/, "", v)
        if (v ~ /^[0-9]+$/) print v
        else print "__INVALID__"
      }')
    if [ "$ICMPAM_VAL" = "__INVALID__" ]; then
      ICMPAM_SHAPE=0
      ICMPAM_VAL=""
    fi
  fi
  if [ "$ICMPAM_RC" -ne 0 ]; then
    ...
    add security net_icmpaddressmask "ICMP address mask reply" WARN med "icmpaddressmask = $ICMPAM_VAL" \
        "ICMP address mask replies are enabled ($ICMPAM_VAL) — the host discloses the local subnet mask to ICMP requests, aiding network reconnaissance." \
        "disable with 'no -o icmpaddressmask=0' (requires root)." "cis-l1"
  fi
}
