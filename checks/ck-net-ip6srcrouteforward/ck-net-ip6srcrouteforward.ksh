...
AIXRAY_TOOL=ck-net-ip6srcrouteforward


function standalone_check {
_AIXRAY_SESSION_KEYS=""

  # net_ip6srcrouteforward — forward/source-routed IPv6 packets. A source-routed
  # packet lets an attacker dictate its own return path and bypass routing or
  # firewall filtering. CIS AIX hardening baseline requires this tunable off.
  typeset IP6SFR_RC IP6SFR_OUT IP6SFR_PARSED IP6SFR_STATE IP6SFR_VAL
  IP6SFR_OUT=$(aix net_ip6srcrouteforward no -o ip6srcrouteforward); IP6SFR_RC=$?
  if [ "$IP6SFR_RC" -ne 0 ]; then
    ...
    IP6SFR_PARSED=$(printf '%s\n' "$IP6SFR_OUT" | awk '
      ...
      {
        rows++
        if ($0 ~ /^ip6srcrouteforward[ \t]*=[ \t]*[0-9]+$/) {
          value=$0
          sub(/^ip6srcrouteforward[ \t]*=[ \t]*/, "", value)
          sub(/^0+/, "", value)
          if (value == "") value="0"
          matches++
          remember(value)
        } else if ($0 ~ /^ip6srcrouteforward[ \t]*=/) {
          malformed=1
        } else {
          other=1
        }
    ...
    ')
    IP6SFR_STATE=${IP6SFR_PARSED%%\|*}
    IP6SFR_VAL=${IP6SFR_PARSED#*\|}
    case "$IP6SFR_STATE" in
      value)
        if [ "$IP6SFR_VAL" = "0" ]; then
          add security net_ip6srcrouteforward "Source-routed IPv6 forwarding" \
              PASS high "ip6srcrouteforward=0" \
              "Source-routed IPv6 packets are NOT forwarded — the host ignores attacker-placed source-route headers." \
              "n/a" "cis-l1"
        else
          add security net_ip6srcrouteforward "Source-routed IPv6 forwarding" \
              FAIL high "ip6srcrouteforward=$IP6SFR_VAL" \
              "Source-routed IPv6 packets ARE forwarded (ip6srcrouteforward=$IP6SFR_VAL) — an attacker can dictate the packet's return path." \
              "set the tunable to 0 ('no -p -o ip6srcrouteforward=0') to stop forwarding source-routed IPv6 packets." "cis-l1"
        fi
        ;;
      conflict)
        add security net_ip6srcrouteforward "Source-routed IPv6 forwarding" \
