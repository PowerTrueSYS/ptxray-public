...
AIXRAY_TOOL=ck-net-ipignoreredirects


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # ipignoreredirects — ICMP redirect acceptance suppression. AIX accepts
  # ICMP Redirect messages by default, which can be exploited to alter
  # routing tables; CIS AIX hardening requires ipignoreredirects = 1.
  # Capture: 'no -o ipignoreredirects' prints "ipignoreredirects = <n>".
  IGR=$(aix no_ipignoreredirects no -o ipignoreredirects)
  IGR_RC=$?
  IGR_VAL=""
  IGR_SHAPE=1
  if [ "$IGR_RC" -eq 0 ] && [ -n "$IGR" ]; then
    # Must contain exactly one line of the shape "ipignoreredirects = <value>"
    IGR_VAL=$(printf '%s\n' "$IGR" | awk '
      /^[ \t]*ipignoreredirects[ \t]+=[ \t]+/ {
        v=$0; sub(/^[ \t]*ipignoreredirects[ \t]+=[ \t]+/, "", v)
        if (v ~ /^[0-9]+$/) print v
        else print "__INVALID__"
      }')
    if [ "$IGR_VAL" = "__INVALID__" ]; then
      IGR_SHAPE=0
      IGR_VAL=""
    fi
  fi
  if [ "$IGR_RC" -ne 0 ]; then
    ...
    add security net_ipignoreredirects "ICMP redirect acceptance" WARN med "ipignoreredirects = $IGR_VAL" \
        "ICMP redirect acceptance is enabled ($IGR_VAL) — the host modifies routing tables based on ICMP Redirect messages, risking route hijacking." \
        "enable with 'no -o ipignoreredirects=1' (requires root)." "cis-l1"
  fi
}
