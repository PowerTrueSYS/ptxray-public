...
AIXRAY_TOOL=ck-net-ipsendredirects


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # ipsendredirects — ICMP redirect emission off.
  # A host that sends ICMP redirects leaks topology and can be used to steer traffic;
  # only routers should redirect. The CIS AIX network-parameter baseline requires
  # ipsendredirects = 0. Capture: 'no -o ipsendredirects' prints "ipsendredirects = <n>".
  IPSR=$(aix no_ipsendredirects no -o ipsendredirects); IPSR_RC=$?
  IPSR_VAL=""
  IPSR_SHAPE=1
  if [ "$IPSR_RC" -eq 0 ] && [ -n "$IPSR" ]; then
    # Must contain exactly one line of the shape "ipsendredirects = <value>"
    IPSR_VAL=$(printf '%s\n' "$IPSR" | awk '
      /^[ \t]*ipsendredirects[ \t]+=[ \t]+/ {
        v=$0; sub(/^[ \t]*ipsendredirects[ \t]+=[ \t]+/, "", v)
        if (v ~ /^[0-9]+$/) print v
        else print "__INVALID__"
      }')
    if [ "$IPSR_VAL" = "__INVALID__" ]; then
      IPSR_SHAPE=0
      IPSR_VAL=""
    fi
  fi
  if [ "$IPSR_RC" -ne 0 ]; then
    ...
    add security net_ipsendredirects "ICMP redirect emission" WARN med "ipsendredirects = $IPSR_VAL" \
        "ICMP redirect emission is enabled ($IPSR_VAL) — the host advertises route changes via ICMP redirects, leaking topology and enabling traffic steering." \
        "disable with 'no -o ipsendredirects=0' (requires root)." "cis-l1"
  fi
}
