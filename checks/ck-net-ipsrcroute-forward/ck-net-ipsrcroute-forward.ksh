...
AIXRAY_TOOL=ck-net-ipsrcroute-forward


function standalone_check {
_AIXRAY_SESSION_KEYS=""

  # net_ipsrcrouteforward — forward/source-routed IP packets. A source-routed
  ...
  typeset NF_RC NF_OUT NF_PARSED NF_STATE NF_VAL
  NF_OUT=$(aix net_ipsrcrouteforward no -o ipsrcrouteforward); NF_RC=$?
  if [ "$NF_RC" -ne 0 ]; then
    ...
    NF_PARSED=$(printf '%s\n' "$NF_OUT" | awk '
      ...
      {
        rows++
        if ($0 ~ /^ipsrcrouteforward[ \t]*=[ \t]*[0-9]+$/) {
          value=$0
          sub(/^ipsrcrouteforward[ \t]*=[ \t]*/, "", value)
          sub(/^0+/, "", value)
          if (value == "") value="0"
          matches++
          remember(value)
        } else if ($0 ~ /^ipsrcrouteforward[ \t]*=/) {
          malformed=1
        } else {
          other=1
        }
    ...
    case "$NF_STATE" in
      value)
        if [ "$NF_VAL" = "0" ]; then
          add security net_ipsrcrouteforward "Source-routed IP forwarding" \
              PASS high "ipsrcrouteforward=0" \
              "Source-routed IP packets are NOT forwarded — the host ignores attacker-placed source-route headers." \
              "n/a" "cis-l1"
        else
          add security net_ipsrcrouteforward "Source-routed IP forwarding" \
              FAIL high "ipsrcrouteforward=$NF_VAL" \
              "Source-routed IP packets ARE forwarded (ipsrcrouteforward=$NF_VAL) — an attacker can dictate the packet's return path." \
              "set the tunable to 0 ('no -p -o ipsrcrouteforward=0') to stop forwarding source-routed packets." "cis-l1"
        fi
        ;;
      conflict)
        add security net_ipsrcrouteforward "Source-routed IP forwarding" \
