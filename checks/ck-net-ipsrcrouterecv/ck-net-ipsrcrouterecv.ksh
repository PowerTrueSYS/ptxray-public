...
AIXRAY_TOOL=ck-net-ipsrcrouterecv


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # ipsrcrouterecv — IP source routing reception off. A host that accepts
  # source-routed packets lets an attacker dictate the return path, bypassing
  # firewall rules and routing policy. CIS AIX hardening baseline requires
  # ipsrcrouterecv = 0. Capture: 'no -o ipsrcrouterecv' prints
  # "ipsrcrouterecv = <n>".
  IPSRRCV=$(aix no_ipsrcrouterecv no -o ipsrcrouterecv); IPSRRCV_RC=$?
  IPSRRCV_VAL=""
  IPSRRCV_SHAPE=1
  if [ "$IPSRRCV_RC" -eq 0 ] && [ -n "$IPSRRCV" ]; then
    # Must contain exactly one line of the shape "ipsrcrouterecv = <value>"
    IPSRRCV_VAL=$(printf '%s\n' "$IPSRRCV" | awk '
      /^[ \t]*ipsrcrouterecv[ \t]+=[ \t]+/ {
        v=$0; sub(/^[ \t]*ipsrcrouterecv[ \t]+=[ \t]+/, "", v)
        if (v ~ /^[0-9]+$/) print v
        else print "__INVALID__"
      }')
    if [ "$IPSRRCV_VAL" = "__INVALID__" ]; then
      IPSRRCV_SHAPE=0
      IPSRRCV_VAL=""
    fi
  fi
  if [ "$IPSRRCV_RC" -ne 0 ]; then
    ...
    add security net_ipsrcrouterecv "IP source routing reception" WARN med "ipsrcrouterecv = $IPSRRCV_VAL" \
        "IP source routing reception is enabled ($IPSRRCV_VAL) — the host accepts source-routed packets, enabling attacker-placed return-path manipulation." \
        "disable with 'no -o ipsrcrouterecv=0' (requires root)." "cis-l1"
  fi
}
