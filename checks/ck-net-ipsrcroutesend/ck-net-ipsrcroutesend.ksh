...
AIXRAY_TOOL=ck-net-ipsrcroutesend


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # ipsrcroutesend — source-route packet emission off.
  # A host that sends source-routed IP packets lets attackers dictate
  # the return path, bypassing routing and firewall filtering.
  # The CIS AIX network-parameter baseline requires ipsrcroutesend = 0.
  # Capture: 'no -o ipsrcroutesend' prints "ipsrcroutesend = <n>".
  IPSR=$(aix no_ipsrcroutesend no -o ipsrcroutesend); IPSR_RC=$?
  IPSR_VAL=""
  IPSR_SHAPE=1
  if [ "$IPSR_RC" -eq 0 ] && [ -n "$IPSR" ]; then
    # Must contain exactly one line of the shape "ipsrcroutesend = <value>"
    IPSR_VAL=$(printf '%s\n' "$IPSR" | awk '
      /^[ \t]*ipsrcroutesend[ \t]+=[ \t]+/ {
        v=$0; sub(/^[ \t]*ipsrcroutesend[ \t]+=[ \t]+/, "", v)
        if (v ~ /^[0-9]+$/) print v
  ...
  if [ "$IPSR_RC" -ne 0 ]; then
    ...
    add security net_ipsrcroutesend "Source-route packet emission" WARN med "ipsrcroutesend = $IPSR_VAL" \
        "Source-route packet emission is enabled ($IPSR_VAL) — the host sends source-routed packets, allowing attackers to dictate return paths." \
        "disable with 'no -o ipsrcroutesend=0' (requires root)." "cis-l1"
  fi
}
