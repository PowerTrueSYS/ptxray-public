...
AIXRAY_TOOL=ck-net-nonlocsrcroute


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # nonlocsrcroute — non-local source routing off. When enabled the host accepts
  # source-routed packets destined to non-local addresses, allowing an attacker
  # to dictate the return path through the network and bypass firewall rules.
  # CIS AIX hardening baseline requires nonlocsrcroute = 0. Capture: 'no -o
  # nonlocsrcroute' prints "nonlocsrcroute = <n>".
  NLSS=$(aix no_nonlocsrcroute no -o nonlocsrcroute); NLSS_RC=$?
  NLSS_VAL=""
  NLSS_SHAPE=1
  if [ "$NLSS_RC" -eq 0 ] && [ -n "$NLSS" ]; then
    # Must contain exactly one line of the shape "nonlocsrcroute = <value>"
    NLSS_VAL=$(printf '%s\n' "$NLSS" | awk '
      /^[ \t]*nonlocsrcroute[ \t]+=[ \t]+/ {
        v=$0; sub(/^[ \t]*nonlocsrcroute[ \t]+=[ \t]+/, "", v)
        if (v ~ /^[0-9]+$/) print v
        else print "__INVALID__"
      }')
    if [ "$NLSS_VAL" = "__INVALID__" ]; then
      NLSS_SHAPE=0
      NLSS_VAL=""
    fi
  fi
  if [ "$NLSS_RC" -ne 0 ]; then
    ...
    add security net_nonlocsrcroute "Non-local source routing" WARN med "nonlocsrcroute = $NLSS_VAL" \
        "Non-local source routing is enabled ($NLSS_VAL) — the host accepts source-routed packets for non-local addresses, enabling attacker-placed return-path manipulation." \
        "disable with 'no -o nonlocsrcroute=0' (requires root)." "cis-l1"
  fi
}
