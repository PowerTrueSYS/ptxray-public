...
AIXRAY_TOOL=ck-net-udp-pmtu-discover


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # udp_pmtu_discover — CIS AIX baseline requires udp_pmtu_discover=0 to
  # remove an ICMP-driven state/DoS avenue on untrusted networks. The no -o
  # wrapper prints "udp_pmtu_discover = <n>"; parse the value after "=".

  typeset NUP_RAW NUP_RC NUP_VAL
  NUP_RAW=$(aix net_udp_pmtu_discover no -o udp_pmtu_discover); NUP_RC=$?
  if [ "$NUP_RC" -ne 0 ]; then
    add security net_udp_pmtu_discover "UDP path-MTU discovery" NOT_ASSESSED low \
        "not assessed — no -o udp_pmtu_discover failed (rc=$NUP_RC)" \
        "The no -o command returned nonzero rc=$NUP_RC, so the tunable value cannot be read on this AIX level." \
        "run 'no -o udp_pmtu_discover' manually and check that the tunable is supported on this AIX release." "cis-l1"
  else
    NUP_VAL=$(printf '%s\n' "$NUP_RAW" | awk '/udp_pmtu_discover/ && $2=="=" && NF>=3 && $3 ~ /^[0-9]+$/ {print $3}')
    if [ -z "$NUP_VAL" ]; then
      # rc=0 but output has no parseable "udp_pmtu_discover = <digit+>" — could be empty,
      # malformed line, or a line with no numeric value after "=".
      add security net_udp_pmtu_discover "UDP path-MTU discovery" NOT_ASSESSED low \
          "not assessed — no -o udp_pmtu_discover returned empty or unparseable output" \
          "The command returned rc=0 but did not produce a parseable 'udp_pmtu_discover = <value>' line." \
          "run 'no -o udp_pmtu_discover' manually and inspect the output shape." "cis-l1"
    elif [ "$NUP_VAL" -eq 0 ]; then
      add security net_udp_pmtu_discover "UDP path-MTU discovery" PASS low "udp_pmtu_discover=0" \
          "UDP path-MTU discovery is disabled (0), matching the CIS AIX baseline." "n/a" "cis-l1"
    else
      add security net_udp_pmtu_discover "UDP path-MTU discovery" WARN low "udp_pmtu_discover=$NUP_VAL" \
          "UDP path-MTU discovery is enabled ($NUP_VAL) — per-path ICMP-driven state that can be abused for DoS." \
          "set 'no -p -o udp_pmtu_discover=0' to disable per CIS AIX baseline." "cis-l1"
    fi
  fi
}
