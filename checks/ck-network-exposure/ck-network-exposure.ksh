...
AIXRAY_TOOL=ck-network-exposure


function standalone_check {
_AIXRAY_SESSION_KEYS=""
# network_exposure — correlate non-loopback listeners against host IP filtering.
# ck-listeners normally captures the netstat evidence; this tool re-captures the
# same evidence under the same key so the correlation below stands alone.
NSL=$(aix netstat_Aan netstat -Aan)
NSL_RC=$?

# AIX IP Security packet filtering is treated as active when the ipsec_v4 device
# reports Available.
IPSEC=$(aix lsdev_ipsec lsdev -l ipsec_v4)
IPSEC_RC=$?
FILTON=0; printf '%s\n' "$IPSEC" | grep -q 'Available' && FILTON=1

# ruled deviation 2026-08-15: refuse on failed capture (class ruling)
if [ "$NSL_RC" -eq 0 ] && [ -n "$NSL" ] && printf '%s\n' "$NSL" | grep -q 'Active Internet connections'; then
  EXT=$(printf '%s\n' "$NSL" | awk '
    /LISTEN/{a=$5
      if(a=="" || a ~ /^127\./ || a ~ /^::1\./) next
      p=a; sub(/.*\./,"",p)
  ...
  if [ "$NEXT" -eq 0 ]; then
    ...
    if [ "$IPSEC_RC" -ne 0 ]; then
      add security network_exposure "Network exposure vs filtering" WARN med \
          "$NEXT external listener(s) on ports $PSHOW; host IP filtering state unknown" \
          "$NEXT services listen on non-loopback addresses (reachable from the network) and AIX host packet filtering (IP Security) could not be verified — the box relies entirely on the upstream network/firewall. One perimeter mistake, VLAN change, or misrouted subnet exposes every one of these ports directly." \
  ...
  add security network_exposure "Network exposure vs filtering" NOT_ASSESSED low \
      "not assessed — netstat capture failed or returned no listener table (rc=$NSL_RC)" \
      "PTxray did not obtain trustworthy network-listener evidence." \
      "inspect the netstat capture, resolve the capture problem, and rerun PTxray." ""
fi
}
