...
AIXRAY_TOOL=ck-nfs-client-mount-options

# Shared, read-only /etc/filesystems evidence for NFS client checks.
function capture_cap_etc_filesystems {
  ETC_FILESYSTEMS_CAPTURED=1
  ETC_FILESYSTEMS_EXISTS_RAW=$(aix etc_filesystems_exists ls -ld /etc/filesystems)
  ETC_FILESYSTEMS_EXISTS_RC=$?
  ETC_FILESYSTEMS_RAW=$(aix etc_filesystems cat /etc/filesystems)
  ETC_FILESYSTEMS_RC=$?
}
...
_AIXRAY_SESSION_KEYS=""
  # nfs_client_mount_options — require both nosuid and nodev on NFS stanzas.
  typeset NCO_EXISTS_RC NCO_RAW NCO_RC NCO_PARSE NCO_PARSE_RC
  typeset NCO_TOTAL NCO_FAIL_COUNT NCO_FAILURES NCO_OBS

  NCO_EXISTS_RC=$ETC_FILESYSTEMS_EXISTS_RC
  NCO_RAW=$ETC_FILESYSTEMS_RAW
  NCO_RC=$ETC_FILESYSTEMS_RC

  if [ "$NCO_RC" -ne 0 ]; then
    if [ "$NCO_EXISTS_RC" -eq 1 ]; then
      add security nfs_client_mount_options "NFS client mount options" PASS high \
          "/etc/filesystems is absent; no NFS client mount requires option assessment" \
          "No NFS client mount definition exists in the absent configuration file." \
          "n/a" "cis-l1"
    elif [ "$NCO_EXISTS_RC" -eq 0 ]; then
    ...
    NCO_PARSE=$(printf '%s\n' "$NCO_RAW" | awk '
      function finish_stanza(    count,item,i,has_nosuid,has_nodev,missing) {
        if (stanza == "" || vfs !~ /^nfs[234]?$/) return
        total++
        ...
        for (i=1; i<=count; i++) {
          ...
          if (tolower(item[i]) == "nodev") has_nodev=1
        }
        if (has_nosuid && has_nodev) {
          print "PASS\t" stanza
        } else {
          missing=""
          if (!has_nosuid) missing="nosuid"
          if (!has_nodev) missing=missing (missing == "" ? "" : ",") "nodev"
          print "FAIL\t" stanza "\tmissing " missing
        }
      }
      {
        line=$0
        # /etc/filesystems is an AIX stanza file and comments it with * , not # .
        # Every real box opens the file with the IBM prolog, whose lines all begin
        # with * ; treating them as unparseable took the whole inventory down. The
        # * is read positionally -- comment only as the first non-blank character
        # -- so a * inside an attribute value is still parsed literally.
        if (line ~ /^[ \t]*\*/) next
        sub(/[ \t]*#.*/, "", line)
        if (line ~ /^[ \t]*$/) next
        if (line ~ /^[^ \t:][^:]*:[ \t]*$/) {
        ...
        if (stanza == "" || line !~ /^[ \t]+[A-Za-z][A-Za-z0-9_]*[ \t]*=/) {
          bad=1
          next
        }
        text=line
        ...
        if (key == "vfs") {
          value=tolower(value)
          if (vfs != "" && vfs != value) bad=1
          vfs=value
        } else if (key == "options") {
          value=tolower(value)
          if (options != "" && options != value) bad=1
          options=value
        }
      }
      END {
    ...
    if [ "$NCO_PARSE_RC" -ne 0 ]; then
      ...
      NCO_FAILURES=$(printf '%s\n' "$NCO_PARSE" | awk -F '\t' '$1=="FAIL"{print $2 " (" $3 ")"}')
      if [ "$NCO_FAIL_COUNT" -gt 0 ]; then
        NCO_OBS=$(printf '%s\n%s' \
          "$NCO_FAIL_COUNT of $NCO_TOTAL NFS client mount(s) lack required options:" \
          "$NCO_FAILURES")
        ...
        add security nfs_client_mount_options "NFS client mount options" PASS high \
            ...
            "No configured NFS client mount requires option assessment." \
            "n/a" "cis-l1"
      else
        add security nfs_client_mount_options "NFS client mount options" PASS high \
            "$NCO_TOTAL NFS client mount(s) include nosuid and nodev" \
            "Every configured NFS client mount has both protective option tokens." \
            "n/a" "cis-l1"
      fi
    fi
  fi
...
function standalone_run {
  capture_cap_etc_filesystems || return 1
  standalone_check
}
