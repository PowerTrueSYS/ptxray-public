...
AIXRAY_TOOL=ck-nfs-client-mounts-disabled
...
_AIXRAY_SESSION_KEYS=""
  # nfs_client_mounts_disabled — configured NFS client stanzas.
  typeset NCM_EXISTS_RC NCM_RAW NCM_RC NCM_PARSE NCM_PARSE_RC
  typeset NCM_COUNT NCM_PATHS NCM_OBS

  if [ "${ETC_FILESYSTEMS_CAPTURED:-0}" -ne 1 ]; then
    ETC_FILESYSTEMS_CAPTURED=1
    ETC_FILESYSTEMS_EXISTS_RAW=$(aix etc_filesystems_exists ls -ld /etc/filesystems)
    ETC_FILESYSTEMS_EXISTS_RC=$?
    ETC_FILESYSTEMS_RAW=$(aix etc_filesystems cat /etc/filesystems)
  ...
  NCM_EXISTS_RC=$ETC_FILESYSTEMS_EXISTS_RC
  NCM_RAW=$ETC_FILESYSTEMS_RAW
  NCM_RC=$ETC_FILESYSTEMS_RC

  if [ "$ETC_FILESYSTEMS_RC" -ne 0 ]; then
    if [ "$NCM_EXISTS_RC" -eq 1 ]; then
      add security nfs_client_mounts_disabled "NFS client mount configuration" PASS high \
          "/etc/filesystems is absent; no NFS client mount is configured" \
          "No NFS client mount definition exists in the absent configuration file." \
          "n/a" "cis-l1"
    elif [ "$NCM_EXISTS_RC" -eq 0 ]; then
    ...
    NCM_PARSE=$(printf '%s\n' "$NCM_RAW" | awk '
      function finish_stanza() {
        if (stanza == "") return
        if (vfs ~ /^nfs[234]?$/) print "NFS\t" stanza
      }
      {
        line=$0
    ...
    if [ "$NCM_PARSE_RC" -ne 0 ]; then
      ...
      if [ "$NCM_COUNT" -gt 0 ]; then
        NCM_OBS=$(printf '%s\n%s' \
          "$NCM_COUNT configured NFS client mount(s):" "$NCM_PATHS")
        ...
        add security nfs_client_mounts_disabled "NFS client mount configuration" PASS high \
            ...
            "The readable configuration contains no NFS client mount stanza." \
            "n/a" "cis-l1"
      fi
    fi
  fi
