...
AIXRAY_TOOL=ck-nim-client-state


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # nim_client_state (LEVEL 2) — this fragment probes /etc/niminfo itself. If
  # the file exists the box is a NIM client; decode NIM_MASTER_HOSTNAME (and
  # name/configuration) from it. Veteran signal: on a NIM-managed box the
  # mksysb/lpp_source resources usually live on the NIM master, so a thin
  # local-backup story is not the whole picture — the real restore image is
  # centralized there. PTxray cannot reach the master, but niminfo names it.
  NIMLS=$(aix ls_niminfo ls /etc/niminfo)
  NIMPROBERC=$?
  if [ "$NIMPROBERC" -eq 0 ]; then
    NIMI=$(aix cat_niminfo cat /etc/niminfo)
    NIMI_RC=$?
    if [ "$NIMI_RC" -ne 0 ]; then
      add resilience nim_client_state "NIM client state" WARN low \
          "not assessed — /etc/niminfo present, cat failed (rc=$NIMI_RC)" \
          "This box has /etc/niminfo, so it is a NIM client, but the file could not be read (rc=$NIMI_RC); the NIM master hostname cannot be claimed from an unread file." \
    ...
    add resilience nim_client_state "NIM client state" NOT_APPLICABLE low \
        "not a NIM client — /etc/niminfo absent" \
        "No /etc/niminfo exists on this box, so it is not a NIM client; there is no NIM master to hold the centralized mksysb or lpp_source this control checks for, so it does not apply here." \
        "n/a" "ffiec:II.C.21"
  fi
}
