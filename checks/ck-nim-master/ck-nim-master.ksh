...
_AIXRAY_SESSION_KEYS=""
  typeset NIM_MASTER_MATCH_COUNT NIM_MASTER_MATCH_RC NIM_MASTER_STATUS NIM_MASTER_SEVERITY
  ...
  if [ "${LSLPPQ_RC:-1}" -ne 0 ] || [ "$CIS_PACKAGE_READY" -ne 1 ]; then
    ...
    NIM_MASTER_MATCH_RC=$?
    if [ "$NIM_MASTER_MATCH_RC" -ne 0 ]; then
      NIM_MASTER_STATUS=NOT_ASSESSED
      NIM_MASTER_SEVERITY=low
      ...
      NIM_MASTER_SEVERITY=low
      NIM_MASTER_OBSERVED="bos.sysmgt.nim.master not installed"
      NIM_MASTER_MEANING="The NIM master fileset is absent from the installed-software inventory."
      NIM_MASTER_FIX="n/a"
    fi
  fi

  add security nim_master_installed "NIM master package presence" \
    "$NIM_MASTER_STATUS" "$NIM_MASTER_SEVERITY" "$NIM_MASTER_OBSERVED" \
    "$NIM_MASTER_MEANING" "$NIM_MASTER_FIX" "cis-l1"
}
