...
_AIXRAY_SESSION_KEYS=""
  # ntp — STIG V-215208: xntpd active, at least one server line, maxpoll <= 16.
  # Three read-only probes; ambiguous evidence fails closed as NOT_ASSESSED.
  # Determinate absence of the NTP subject (0513-085 / not on file) is a FAIL.
  typeset XN XN_RC XN_STATUS XN_ROW XN_ABSENT NTPS NTPS_RC NTPS_COUNT MAXP MAXP_RC MAXP_STATE
  typeset NTP_REASON NTP_DETAIL

  XN=$(aix lssrc_xntpd lssrc -s xntpd)
  XN_RC=$?
  NTPS=$(aix ntp_conf grep ^server /etc/ntp.conf)
  NTPS_RC=$?
  MAXP=$(aix ntp_maxpoll grep maxpoll /etc/ntp.conf)
  MAXP_RC=$?

  XN_STATUS=$(printf '%s\n' "$XN" | awk '$1=="xntpd"{print $NF; exit}')
  XN_ROW=$(printf '%s\n' "$XN" | awk '$1=="xntpd"{n=1} END{print n+0}')
  # 0513-085 / "not on file" is lssrc's determinate signal that the xntpd
  # subsystem is not installed at all — absence, not a capture failure.
  XN_ABSENT=$(printf '%s\n' "$XN" | awk '/0513-085|not on file/{n=1} END{print n+0}')
  NTPS_COUNT=0
  if [ -n "$NTPS" ]; then
    NTPS_COUNT=$(printf '%s\n' "$NTPS" | awk 'NF{n++} END{print n+0}')
  fi

  # Only active (non-comment) maxpoll directives bound the poll interval; a
  # config with no active maxpoll is unbounded and fails the clause.
  MAXP_STATE=$(printf '%s\n' "$MAXP" | awk '
    {
      line = $0
      if (line ~ /^[ \t]*#/) next
      while (match(line, /maxpoll[ \t]+[0-9][0-9]*/)) {
    ...
    END {
      ...
      else if (bad) print "over"
      else print "ok"
    }
  ')

  NTP_REASON=""
  if [ "$XN_RC" -ne 0 ] && [ "$XN_ABSENT" = "0" ]; then
    NTP_REASON="lssrc -s xntpd probe failed (exit=$XN_RC)"
  elif [ -z "$XN" ]; then
    NTP_REASON="lssrc -s xntpd returned no output"
  elif [ "$NTPS_RC" -ne 0 ] && [ "$NTPS_RC" -ne 1 ]; then
    NTP_REASON="grep of /etc/ntp.conf server lines failed (exit=$NTPS_RC)"
  elif [ "$NTPS_RC" -eq 0 ] && [ -z "$NTPS" ]; then
    NTP_REASON="server grep returned no output with exit 0"
  elif [ "$NTPS_RC" -eq 1 ] && [ -n "$NTPS" ]; then
    NTP_REASON="server grep returned output with exit 1"
  elif [ "$MAXP_RC" -ne 0 ] && [ "$MAXP_RC" -ne 1 ]; then
    NTP_REASON="grep of /etc/ntp.conf maxpoll lines failed (exit=$MAXP_RC)"
  elif [ "$MAXP_RC" -eq 0 ] && [ -z "$MAXP" ]; then
    NTP_REASON="maxpoll grep returned no output with exit 0"
  elif [ "$MAXP_RC" -eq 1 ] && [ -n "$MAXP" ]; then
    NTP_REASON="maxpoll grep returned output with exit 1"
  fi

  if [ -n "$NTP_REASON" ]; then
    add security ntp "Time synchronization (NTP)" NOT_ASSESSED med \
        "not assessed - $NTP_REASON" \
        "PTxray could not obtain trustworthy evidence of the NTP time-sync configuration." \
        "run 'lssrc -s xntpd' and 'grep server /etc/ntp.conf' and 'grep maxpoll /etc/ntp.conf', fix the capture problem, and rerun PTxray." \
        "stig:V-215208 cis-l1"
  elif [ "$XN_ROW" != "1" ]; then
    add security ntp "Time synchronization (NTP)" FAIL med "NTP subject absent: xntpd subsystem is not configured" "The NTP time-sync application is not configured, so the clock cannot be disciplined to an external source." "install and configure the NTP subsystem with a time server (maxpoll 16 or less) and start xntpd." "stig:V-215208 cis-l1"
  else
    NTP_DETAIL=""
    if [ "$XN_STATUS" != "active" ]; then
      NTP_DETAIL="xntpd ${XN_STATUS:-inactive}"
    fi
    if [ "${NTPS_COUNT:-0}" -lt 1 ]; then
      NTP_DETAIL="${NTP_DETAIL:+$NTP_DETAIL; }no time server configured"
    fi
    if [ "$MAXP_STATE" = "over" ]; then
      NTP_DETAIL="${NTP_DETAIL:+$NTP_DETAIL; }maxpoll exceeds 16"
    fi
    if [ "$MAXP_STATE" = "none" ]; then
      NTP_DETAIL="${NTP_DETAIL:+$NTP_DETAIL; }no active maxpoll bound configured"
    fi
    if [ -n "$NTP_DETAIL" ]; then
      add security ntp "Time synchronization (NTP)" FAIL med \
          "$NTP_DETAIL" \
          "No working time sync — clock drift breaks log correlation, Kerberos and TLS." \
          "configure /etc/ntp.conf with a time server (maxpoll 16 or less) and restart xntpd ('stopsrc -s xntpd; startsrc -s xntpd'; PTxray only recommends this, it never runs it)." \
          "stig:V-215208 cis-l1"
    else
      add security ntp "Time synchronization (NTP)" PASS med \
          "xntpd active, ${NTPS_COUNT:-0} server(s), maxpoll <= 16" \
          "The clock is disciplined by NTP — log correlation, Kerberos and TLS all depend on it." \
          "n/a" "stig:V-215208 cis-l1"
    fi
  fi
}
