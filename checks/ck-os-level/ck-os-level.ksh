...
AIXRAY_TOOL=ck-os-level


function standalone_check {
_AIXRAY_SESSION_KEYS=""
# === BEGIN currency-gate ===
# Refuse with a typed NOT_ASSESSED when the ibm-aix-lifecycle source-registry
# row is not CURRENT. The EOS_OS / UPGRADE_PATH tables below are the payload
# that row attests to; scoring off them without knowing the row is current
# would be a hollow PASS. The status and reason come from
# currency_source_status (standalone prelude), never the inline tables.
...
if [ "$CURRENCY_STATUS" != "CURRENT" ]; then
  add lifecycle os_level "AIX release support" NOT_ASSESSED med \
    "not assessed - ibm-aix-lifecycle source registry row is $CURRENCY_STATUS: $CURRENCY_REASON" \
    "The AIX support-lifecycle reference data is not current, so the bundled EOS and upgrade-path tables cannot be trusted for a verdict." \
    "verify the ibm-aix-lifecycle source registry row is CURRENT, then re-run this check." \
    "ffiec:II.C.11"
  return 0
fi
# === END currency-gate ===
  ...
  if [ -z "$OSLEVEL" ]; then
    ...
    if [ -z "$EOS" ]; then
      add lifecycle os_level "AIX release support" WARN med "$OSLEVEL" \
          "Release $REL is not in the bundled reference data." \
