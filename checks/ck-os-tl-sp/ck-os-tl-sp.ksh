...
"
# === BEGIN currency-gate ===
# The EOS_OS / LATEST_TL / TL_SUPPORT / UPGRADE_PATH tables above are the
# payload the ibm-aix-lifecycle source-registry row attests to. Score only
# when that row is CURRENT; any other status is a typed NOT_ASSESSED refusal.
# Never fall back to the inline table, never WARN.
#
# Standalone doors expose currency_source_status. The monolith evaluates CU_*
# via currency_evaluate before this fragment and has currency_source_index,
# not the accessor. Read the already-evaluated row when the accessor is
# absent so a command-not-found cannot hollow-score or refuse CURRENT as empty.
# Names are CKOSTLSP_* so this fragment does not clobber CURRENCY_STATUS.
CKOSTLSP_CR_TAB=$(printf '\t')
CKOSTLSP_CR_ROW=""
if whence currency_source_status >/dev/null 2>/dev/null; then
  CKOSTLSP_CR_ROW=$(currency_source_status ibm-aix-lifecycle)
fi
if [ -z "$CKOSTLSP_CR_ROW" ] && whence currency_source_index >/dev/null 2>/dev/null; then
  CKOSTLSP_CR_CI=$(currency_source_index ibm-aix-lifecycle) || CKOSTLSP_CR_CI=""
  if [ -n "$CKOSTLSP_CR_CI" ]; then
    CKOSTLSP_CR_ROW=$(printf '%s\t%s' "${CU_STATUS[$CKOSTLSP_CR_CI]}" "${CU_REASON[$CKOSTLSP_CR_CI]}")
  fi
fi
CKOSTLSP_CR_STATUS=${CKOSTLSP_CR_ROW%%"$CKOSTLSP_CR_TAB"*}
CKOSTLSP_CR_REASON=${CKOSTLSP_CR_ROW#*"$CKOSTLSP_CR_TAB"}
if [ -z "$CKOSTLSP_CR_STATUS" ]; then
  CKOSTLSP_CR_STATUS=INVALID
  CKOSTLSP_CR_REASON="currency source status accessor returned no status"
fi
if [ "$CKOSTLSP_CR_STATUS" != "CURRENT" ]; then
  add lifecycle os_tl_sp "Technology Level currency" NOT_ASSESSED med \
    "not assessed - ibm-aix-lifecycle source registry row is $CKOSTLSP_CR_STATUS: $CKOSTLSP_CR_REASON" \
    "The ibm-aix-lifecycle source-registry row is not CURRENT, so the bundled lifecycle table has no attested vintage; this check does not score against a table of unknown age." \
    "check IBM's current AIX lifecycle and TL support pages, refresh the bundled reference data, and rerun PTxray." \
    "ffiec:II.C.11"
  return 0
fi
# === END currency-gate ===
