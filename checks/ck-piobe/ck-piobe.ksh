...
_AIXRAY_SESSION_KEYS=""
  typeset PIOBE_MATCH PIOBE_MATCH_RC PIOBE_COUNT PIOBE_STATE
    ...
    if [ "$PIOBE_MATCH_RC" -ne 0 ]; then
      ...
      if [ "$PIOBE_COUNT" -gt 1 ]; then
        ...
        add security piobe "piobe service" PASS low \
            ...
            "The piobe subsystem is absent and therefore is not in use." "n/a" "cis-l1"
      else
        PIOBE_STATE=$PIOBE_MATCH
        if [ "$PIOBE_STATE" = in_use ]; then
          ...
          add security piobe "piobe service" PASS low \
              ...
              "The piobe subsystem is present but not in use." "n/a" "cis-l1"
        fi
      fi
    fi
  fi
