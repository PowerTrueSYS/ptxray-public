...
_AIXRAY_SESSION_KEYS=""
  ...
  if [ "$HAFRC" -eq 0 ] && printf '%s\n' "$HAF" | grep -q 'cluster.es.server.rte'; then
    ...
    if [ "$HAFRC" -eq 1 ]; then
      add resilience powerha "PowerHA cluster state" NOT_APPLICABLE low \
          "PowerHA not installed — cluster.es.server.rte absent (lslpp rc=$HAFRC); no cluster state to report" \
          "The lslpp probe positively proved the PowerHA cluster fileset is absent from this host, so there is no cluster state to report." "n/a"
    else
      add resilience powerha "PowerHA cluster state" NOT_ASSESSED low \
          "PowerHA state not assessed — lslpp rc=$HAFRC did not prove absence" \
          "The lslpp probe capture failed or returned an unrecognized shape, so absence of the PowerHA cluster fileset could not be positively proven." \
          "run 'lslpp -L cluster.es.server.rte' manually and verify the fileset state."
    fi
  fi
}
