...
_AIXRAY_SESSION_KEYS=""
  ...
  typeset SYS0 MAXU PSU TOPUSER TOPN RATIO SYS0_RC PSU_RC
  ...
  TOPUSER=${1:-}; TOPN=${2:-0}
  if [ "$SYS0_RC" -ne 0 ] || [ -z "$SYS0" ] || [ "$PSU_RC" -ne 0 ] || [ -z "$PSU" ]; then
    add performance proc_limits "Process limits (maxuproc)" NOT_ASSESSED med \
        "not assessed — probe failed (lsattr rc=$SYS0_RC out=${#SYS0}; ps rc=$PSU_RC out=${#PSU})" \
        "Could not read maxuproc or the per-user process counts, so per-user process headroom cannot be assessed." \
        "check 'lsattr -El sys0 -a maxuproc' and 'ps -eo user' manually."
  else
    case "$MAXU" in ''|*[!0-9]*)
