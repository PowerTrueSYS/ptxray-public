...
AIXRAY_TOOL=ck-pw-dictionlist


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # pw_dictionlist — configured password dictionary paths inherited from the
  # default stanza. This diagnostic fragment never reads the referenced files.
  typeset PW_DICT_RAW PW_DICT_RC PW_DICT_PARSED PW_DICT_STATE PW_DICT_VALUE
  typeset PW_DICT_STATUS PW_DICT_OBSERVED PW_DICT_REASON PW_DICT_MEANING
  ...
  PW_DICT_STATE=""
  PW_DICT_VALUE=""
  PW_DICT_STATUS=NOT_ASSESSED
  PW_DICT_OBSERVED=""
  PW_DICT_REASON=""
  PW_DICT_MEANING=""

  if [ "${MYUID:-0}" != "0" ]; then
    PW_DICT_REASON="requires root"
  else
    PW_DICT_RAW=$(aix pw_dictionlist lssec -f /etc/security/user -s default -a dictionlist)
    PW_DICT_RC=$?
    if [ "$PW_DICT_RC" -ne 0 ]; then
      PW_DICT_REASON="capture failed (rc=$PW_DICT_RC)"
    elif [ -z "$PW_DICT_RAW" ]; then
      PW_DICT_REASON="capture empty (rc=0)"
    else
      PW_DICT_PARSED=$(printf '%s\n' "$PW_DICT_RAW" | awk '
        /^[[:space:]]*$/ { next }
        {
          rows++
          if ($0 !~ /^default dictionlist=/) {
            bad=1
            next
          }
          raw=$0
          sub(/^default dictionlist=/, "", raw)
          if (raw ~ /[[:space:]][A-Za-z][A-Za-z0-9_]*=/) {
            bad=1
            next
          }
          if (raw ~ /^"/) {
        ...
        END {
          if (rows != 1 || bad) {
            ...
            print "empty|"
          } else if (value ~ /[[:space:][:cntrl:]]/) {
            print "invalid|" value
          } else {
            count=split(value, item, ",")
            invalid=0
            for (i=1; i<=count; i++) {
              if (item[i] == "" || substr(item[i], 1, 1) != "/") invalid=1
            }
            if (invalid) print "invalid|" value
            else print "pass|" value
          }
        }')
      PW_DICT_STATE=${PW_DICT_PARSED%%\|*}
      PW_DICT_VALUE=${PW_DICT_PARSED#*\|}
      case "$PW_DICT_STATE" in
        pass)
          PW_DICT_STATUS=PASS
          PW_DICT_OBSERVED="dictionlist=$PW_DICT_VALUE"
          ;;
        empty)
          PW_DICT_STATUS=FAIL
          PW_DICT_OBSERVED="dictionlist is unset"
          ;;
        invalid)
          PW_DICT_STATUS=FAIL
          PW_DICT_OBSERVED="dictionlist=$PW_DICT_VALUE (invalid absolute-path list)"
          ;;
        *)
          PW_DICT_REASON="capture unparseable (rc=0)"
          ;;
      esac
    fi
  fi

  case "$PW_DICT_STATUS" in
    PASS)
      add security pw_dictionlist "Password dictionary enforcement" PASS high \
          "$PW_DICT_OBSERVED" \
          "The default password policy names at least one absolute dictionary path for rejecting dictionary-based passwords; this check does not validate the referenced files." \
          "n/a" "cis-l1 ffiec:II.C.15"
      ;;
    FAIL)
      if [ "$PW_DICT_STATE" = empty ]; then
        PW_DICT_MEANING="The default password policy has no dictionary configured, so dictionary-word checks are not enforced for accounts inheriting this stanza."
      else
        PW_DICT_MEANING="The default password dictionary setting is present but is not a comma-separated list of absolute paths, so it does not meet the configured-dictionary requirement."
      fi
