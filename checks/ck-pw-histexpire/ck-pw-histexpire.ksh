...
AIXRAY_TOOL=ck-pw-histexpire


function standalone_check {
_AIXRAY_SESSION_KEYS=""

  # pw_histexpire — minimum password-history reuse window. This fragment is
  # diagnostic only and accepts only one exact default-stanza assignment.
  typeset PW_HEX_RAW PW_HEX_RC PW_HEX_PARSED PW_HEX_STATE PW_HEX_VALUE
  typeset PW_HEX_STATUS PW_HEX_OBSERVED PW_HEX_REASON PW_HEX_REL
  typeset PW_HEX_OS PW_HEX_OS_RC PW_HEX_OS_REL

  PW_HEX_STATUS=NOT_ASSESSED
  PW_HEX_OBSERVED=""
  PW_HEX_REASON=""
  PW_HEX_REL=${REL:-}

  if [ "${MYUID:-0}" != "0" ]; then
    PW_HEX_REASON="requires root; no capture was executed"
  else
    PW_HEX_RAW=$(aix pw_histexpire lssec -f /etc/security/user -s default -a histexpire)
    PW_HEX_RC=$?
    if [ "$PW_HEX_RC" -ne 0 ]; then
      PW_HEX_REASON="read failed with rc $PW_HEX_RC"
    else
      PW_HEX_PARSED=$(printf '%s\n' "$PW_HEX_RAW" | awk '
        /^[[:space:]]*$/ { next }
        {
          rows++
          if ($0 !~ /^default histexpire=/) {
            bad=1
            next
          }
          value=$0
          sub(/^default histexpire=/, "", value)
        }
        END {
          if (rows == 0) {
            print "empty|"
            ...
            if (normalized ~ /^[0-9][0-9][0-9][0-9]/) {
              print "huge|"
            } else if ((normalized + 0) < 52) {
              print "below|" normalized
            } else if ((normalized + 0) <= 260) {
              print "pass|" normalized
            } else {
              print "above|" normalized
            }
          }
        }')
      PW_HEX_STATE=${PW_HEX_PARSED%%\|*}
      PW_HEX_VALUE=${PW_HEX_PARSED#*\|}

      case "$PW_HEX_STATE" in
        pass)
          PW_HEX_STATUS=PASS
          PW_HEX_OBSERVED="histexpire=$PW_HEX_VALUE weeks"
          ;;
        below)
          PW_HEX_STATUS=FAIL
          PW_HEX_OBSERVED="histexpire=$PW_HEX_VALUE weeks"
        ...
        above)
          PW_HEX_STATUS=FAIL
          PW_HEX_OBSERVED="histexpire=$PW_HEX_VALUE weeks; exceeds IBM documented maximum 260 weeks"
          ;;
        huge)
          PW_HEX_STATUS=FAIL
          PW_HEX_OBSERVED="histexpire exceeds IBM documented maximum 260 weeks"
          ;;
        absent)
          if [ -z "$PW_HEX_REL" ] &&
             [ "${AIXRAY_TOOL:-}" = "ck-pw-histexpire" ]; then
            PW_HEX_OS=$(aix oslevel_s oslevel -s)
            PW_HEX_OS_RC=$?
            if [ "$PW_HEX_OS_RC" -eq 0 ]; then
              PW_HEX_OS_REL=$(printf '%s\n' "$PW_HEX_OS" | awk '
                /^[[:space:]]*$/ { next }
                {
                  rows++
                  if ($0 ~ /^[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]-[0-9][0-9][0-9][0-9]$/) {
                    release=substr($0, 1, 4)
                  } else {
                    bad=1
                  }
                }
                END {
                  if (rows == 1 && !bad) print release
                }')
              if [ -n "$PW_HEX_OS_REL" ]; then
                PW_HEX_REL=$PW_HEX_OS_REL
              else
                PW_HEX_REASON="was absent and standalone oslevel -s output was unparseable"
              fi
            else
              PW_HEX_REASON="was absent and standalone oslevel -s read failed with rc $PW_HEX_OS_RC"
            fi
          fi
          ...
          case "$PW_HEX_REL" in
            7100)
              PW_HEX_STATUS=FAIL
              PW_HEX_OBSERVED="histexpire absent; AIX 7.1 documented default 0 weeks"
              ;;
            7200)
              PW_HEX_STATUS=FAIL
              PW_HEX_OBSERVED="histexpire absent; AIX 7.2 documented default 0 weeks"
              ;;
            7300)
              PW_HEX_STATUS=PASS
              PW_HEX_OBSERVED="histexpire absent; AIX 7.3 documented default 52 weeks"
              ;;
            "")
              if [ -z "$PW_HEX_REASON" ]; then
                PW_HEX_REASON="was absent but the trusted AIX release was unavailable"
              fi
              ;;
            *)
              PW_HEX_REASON="was absent but release $PW_HEX_REL has no documented default"
              ;;
          esac
          ;;
        empty)
          PW_HEX_REASON="read returned no assignment"
          ;;
        *)
          PW_HEX_REASON="output was not exactly one default histexpire assignment"
          ;;
      esac
    fi
  fi

  case "$PW_HEX_STATUS" in
    PASS)
      add security pw_histexpire "Password history expiration" PASS high \
          "$PW_HEX_OBSERVED" \
          "Previously used passwords remain unavailable for at least fifty-two weeks." \
          "n/a" "cis-l1 ffiec:II.C.15"
      ;;
    FAIL)
