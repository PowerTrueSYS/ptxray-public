...
AIXRAY_TOOL=ck-pw-loginretries


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # pw_loginretries — failed login retry limit inherited from the default stanza.
  # This diagnostic fragment accepts only one exact attribute-scoped assignment.
  typeset PW_LOGINRETRIES_RAW PW_LOGINRETRIES_RC PW_LOGINRETRIES_PARSED PW_LOGINRETRIES_STATE PW_LOGINRETRIES_VALUE
  typeset PW_LOGINRETRIES_STATUS PW_LOGINRETRIES_OBSERVED PW_LOGINRETRIES_REASON

  PW_LOGINRETRIES_RAW=""
  PW_LOGINRETRIES_RC=126
  PW_LOGINRETRIES_PARSED=""
  PW_LOGINRETRIES_STATE=""
  PW_LOGINRETRIES_VALUE=""
  PW_LOGINRETRIES_STATUS=NOT_ASSESSED
  PW_LOGINRETRIES_OBSERVED=""
  PW_LOGINRETRIES_REASON=""

  if [ "${MYUID:-0}" != "0" ]; then
    PW_LOGINRETRIES_REASON="requires root privileges"
  else
    PW_LOGINRETRIES_RAW=$(aix pw_loginretries lssec -f /etc/security/user -s default -a loginretries)
    PW_LOGINRETRIES_RC=$?
    if [ "$PW_LOGINRETRIES_RC" -ne 0 ]; then
      PW_LOGINRETRIES_REASON="capture failed (rc=$PW_LOGINRETRIES_RC)"
    elif [ -z "$PW_LOGINRETRIES_RAW" ]; then
      PW_LOGINRETRIES_REASON="capture returned empty output"
    else
      PW_LOGINRETRIES_PARSED=$(printf '%s\n' "$PW_LOGINRETRIES_RAW" | awk '
        /^[[:space:]]*$/ { next }
        {
          rows++
          if ($0 !~ /^default loginretries=/) {
            bad=1
            next
          }
          value=$0
          sub(/^default loginretries=/, "", value)
          if (value == "") {
            empty=1
          } else if (value !~ /^[0-9]+$/) {
            bad=1
      ...
      PW_LOGINRETRIES_STATE=${PW_LOGINRETRIES_PARSED%%\|*}
      PW_LOGINRETRIES_VALUE=${PW_LOGINRETRIES_PARSED#*\|}
      case "$PW_LOGINRETRIES_STATE" in
        pass)
          PW_LOGINRETRIES_STATUS=PASS
          PW_LOGINRETRIES_OBSERVED="loginretries=$PW_LOGINRETRIES_VALUE"
          ;;
        fail)
          PW_LOGINRETRIES_STATUS=FAIL
          PW_LOGINRETRIES_OBSERVED="loginretries=$PW_LOGINRETRIES_VALUE; required between 1 and 3 inclusive"
          ;;
        huge)
          PW_LOGINRETRIES_REASON="value was too large to compare safely"
          ;;
        empty)
          PW_LOGINRETRIES_REASON="assignment value was empty"
          ;;
        *)
          PW_LOGINRETRIES_REASON="output was not one exact unsigned-decimal assignment"
          ;;
      esac
    fi
  fi

  case "$PW_LOGINRETRIES_STATUS" in
    PASS)
      add security pw_loginretries "Failed login retry limit" PASS high \
          "$PW_LOGINRETRIES_OBSERVED" \
          "The default policy bounds consecutive failed login attempts at the approved boundary." \
          "n/a" "cis-l1"
      ;;
    FAIL)
