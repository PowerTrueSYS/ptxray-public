...
AIXRAY_TOOL=ck-pw-maxage


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # pw_maxage — maximum password age inherited from the default stanza.
  # This diagnostic fragment accepts only one exact attribute-scoped assignment.
  typeset PW_AGE_RAW PW_AGE_RC PW_AGE_PARSED PW_AGE_STATE PW_AGE_VALUE
  typeset PW_AGE_STATUS PW_AGE_OBSERVED PW_AGE_REASON
  ...
  PW_AGE_STATE=""
  PW_AGE_VALUE=""
  PW_AGE_STATUS=NOT_ASSESSED
  PW_AGE_OBSERVED=""
  PW_AGE_REASON=""

  if [ "${MYUID:-0}" != "0" ]; then
    PW_AGE_REASON="requires root privileges"
  else
    PW_AGE_RAW=$(aix pw_maxage lssec -f /etc/security/user -s default -a maxage)
    PW_AGE_RC=$?
    if [ "$PW_AGE_RC" -ne 0 ]; then
      PW_AGE_REASON="capture failed (rc=$PW_AGE_RC)"
    elif [ -z "$PW_AGE_RAW" ]; then
      PW_AGE_REASON="capture returned empty output"
    else
      PW_AGE_PARSED=$(printf '%s\n' "$PW_AGE_RAW" | awk '
        /^[[:space:]]*$/ { next }
        {
          rows++
          if ($0 !~ /^default maxage=/) {
      ...
      PW_AGE_STATE=${PW_AGE_PARSED%%\|*}
      PW_AGE_VALUE=${PW_AGE_PARSED#*\|}
      case "$PW_AGE_STATE" in
        pass)
          PW_AGE_STATUS=PASS
          PW_AGE_OBSERVED="maxage=$PW_AGE_VALUE"
          ;;
        fail)
          PW_AGE_STATUS=FAIL
          PW_AGE_OBSERVED="maxage=$PW_AGE_VALUE; required between 1 and 8 inclusive"
          ;;
        huge)
          PW_AGE_REASON="value was too large to compare safely"
          ;;
        empty)
          PW_AGE_REASON="assignment value was empty"
          ;;
        *)
          PW_AGE_REASON="output was not one exact unsigned-decimal assignment"
          ;;
      esac
    fi
  fi

  case "$PW_AGE_STATUS" in
    PASS)
      add security pw_maxage "Password maximum age" PASS high \
          "$PW_AGE_OBSERVED" \
          "The default policy prevents passwords inherited from the default stanza from never expiring at the approved boundary." \
          "n/a" "cis-l1"
      ;;
    FAIL)
