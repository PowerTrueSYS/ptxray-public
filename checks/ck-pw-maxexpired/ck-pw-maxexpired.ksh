...
AIXRAY_TOOL=ck-pw-maxexpired


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # pw_maxexpired — weeks after maxage when a user may still change an
  # expired password. This diagnostic fragment reads maxexpired and maxage
  # from one default-stanza row so an otherwise in-band but inert setting
  # cannot be reported as clean.
  typeset PW_MX_RAW PW_MX_RC PW_MX_PARSED PW_MX_STATE
  typeset PW_MX_VALUES PW_MX_VALUE PW_MX_MAXAGE
  typeset PW_MX_STATUS PW_MX_OBSERVED PW_MX_REASON
  ...
  PW_MX_VALUE=""
  PW_MX_MAXAGE=""
  PW_MX_STATUS=NOT_ASSESSED
  PW_MX_OBSERVED=""
  PW_MX_REASON=""

  if [ "${MYUID:-0}" != "0" ]; then
    PW_MX_REASON="requires root; no capture was executed"
  else
    PW_MX_RAW=$(aix pw_maxexpired lssec -f /etc/security/user -s default -a maxexpired -a maxage)
    PW_MX_RC=$?
    if [ "$PW_MX_RC" -ne 0 ]; then
      PW_MX_REASON="read failed with rc $PW_MX_RC"
    elif [ -z "$PW_MX_RAW" ]; then
      PW_MX_REASON="read returned no row"
    else
    # PW_MX_PARSER_BEGIN
    PW_MX_PARSED=$(printf '%s\n' "$PW_MX_RAW" | awk '
      {
        rows++
        if ($0 ~ /^[[:space:]]*#/) { comment=1; next }
        if ($1 != "default") { stanza=1; next }
        if (rows != 1) { duplicate=1; next }
        if (NF != 3) { row=1; next }
        if ($0 !~ /^default maxexpired=[^[:space:]]* maxage=[^[:space:]]*$/) {
          row=1
          next
        }
        record=$0
        sub(/^default maxexpired=/, "", record)
        separator=index(record, " maxage=")
        mx=substr(record, 1, separator-1)
        ma=substr(record, separator+length(" maxage="))
      }
      END {
        if (rows != 1 || comment || stanza || duplicate || row) {
          print "shape||"
        } else if (mx !~ /^(-1|-[1-9][0-9]*|0|[1-9][0-9]*)$/ ||
                   ma !~ /^(0|[1-9][0-9]*)$/) {
          print "values||"
        } else {
          print "value|" mx "|" ma
    ...
    # PW_MX_PARSER_END
      PW_MX_STATE=${PW_MX_PARSED%%\|*}
      PW_MX_VALUES=${PW_MX_PARSED#*\|}
      PW_MX_VALUE=${PW_MX_VALUES%%\|*}
      PW_MX_MAXAGE=${PW_MX_VALUES#*\|}
      case "$PW_MX_STATE" in
        shape)
          PW_MX_REASON="output was not one exact default-stanza row"
          ;;
        values)
          PW_MX_REASON="values were not exact decimal integers"
          ;;
      esac
    fi
  fi

  if [ "$PW_MX_STATE" = value ] && [ "$PW_MX_MAXAGE" = 0 ]; then
    PW_MX_STATUS=FAIL
    PW_MX_OBSERVED="maxexpired=$PW_MX_VALUE weeks; maxage=0 (maxexpired ignored)"
  elif [ "$PW_MX_STATE" = value ]; then
    case "$PW_MX_VALUE" in
      -1)
        PW_MX_STATUS=FAIL
        PW_MX_OBSERVED="maxexpired=-1 (unlimited); maxage=$PW_MX_MAXAGE weeks"
        ;;
      -[1-9]*)
        PW_MX_STATUS=FAIL
        PW_MX_OBSERVED="maxexpired=$PW_MX_VALUE (outside documented domain); maxage=$PW_MX_MAXAGE weeks"
        ;;
      0|1|2|3|4)
        PW_MX_STATUS=PASS
        PW_MX_OBSERVED="maxexpired=$PW_MX_VALUE weeks; maxage=$PW_MX_MAXAGE weeks"
        ;;
      *)
        PW_MX_STATUS=FAIL
        PW_MX_OBSERVED="maxexpired=$PW_MX_VALUE weeks (exceeds four-week bound); maxage=$PW_MX_MAXAGE weeks"
        ;;
    esac
  fi

  case "$PW_MX_STATUS" in
    PASS)
      add security pw_maxexpired "Password expiration grace period" PASS high \
          "$PW_MX_OBSERVED" \
          "The observed maxexpired setting provides no more than four weeks of grace and maxage keeps password expiration active." \
