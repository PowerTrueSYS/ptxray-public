...
AIXRAY_TOOL=ck-pw-maxrepeats


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # pw_maxrepeats — maximum occurrences of one character in a new password.
  # This check reads only the real maxrepeats attribute from the default stanza.
  typeset PW_MXR_RAW PW_MXR_RC PW_MXR_VALUE PW_MXR_NORM
  typeset PW_MXR_STATUS PW_MXR_OBSERVED PW_MXR_REASON

  if [ "${MYUID:-0}" != "0" ]; then
    PW_MXR_STATUS=NOT_ASSESSED
    PW_MXR_REASON="requires root"
  else
    PW_MXR_RAW=$(aix pw_maxrepeats lssec -f /etc/security/user -s default -a maxrepeats)
    PW_MXR_RC=$?
    if [ "$PW_MXR_RC" -ne 0 ]; then
      PW_MXR_STATUS=NOT_ASSESSED
      PW_MXR_REASON="read failed (rc=$PW_MXR_RC)"
    elif [ -z "$PW_MXR_RAW" ]; then
      PW_MXR_STATUS=NOT_ASSESSED
      PW_MXR_REASON="returned empty output"
    else
      case "$PW_MXR_RAW" in
        *'
'*)
          PW_MXR_STATUS=NOT_ASSESSED
          PW_MXR_REASON="returned multiple lines"
          ;;
        "default maxrepeats="*)
          PW_MXR_VALUE=${PW_MXR_RAW#default maxrepeats=}
          if [ -z "$PW_MXR_VALUE" ]; then
            PW_MXR_STATUS=FAIL
            PW_MXR_OBSERVED="maxrepeats absent; documented AIX default 8 (no effective maximum)"
          else
            case "$PW_MXR_VALUE" in
              *[!0-9]*)
                PW_MXR_STATUS=NOT_ASSESSED
                PW_MXR_REASON="value was not a digits-only decimal"
                ;;
              *)
                PW_MXR_NORM=$PW_MXR_VALUE
                while [ "${#PW_MXR_NORM}" -gt 1 ] &&
                    [ "${PW_MXR_NORM#0}" != "$PW_MXR_NORM" ]; do
                  PW_MXR_NORM=${PW_MXR_NORM#0}
                done
                if [ "${#PW_MXR_NORM}" -gt 1 ]; then
                  PW_MXR_STATUS=FAIL
                  PW_MXR_OBSERVED="maxrepeats=$PW_MXR_VALUE (outside documented range 0..8)"
                elif [ "$PW_MXR_NORM" -ge 1 ] &&
                    [ "$PW_MXR_NORM" -le 4 ]; then
                  PW_MXR_STATUS=PASS
                  PW_MXR_OBSERVED="maxrepeats=$PW_MXR_VALUE"
                elif [ "$PW_MXR_NORM" -le 8 ]; then
                  PW_MXR_STATUS=FAIL
                  PW_MXR_OBSERVED="maxrepeats=$PW_MXR_VALUE; required between 1 and 4 inclusive"
                else
                  PW_MXR_STATUS=FAIL
                  PW_MXR_OBSERVED="maxrepeats=$PW_MXR_VALUE (outside documented range 0..8)"
                fi
        ...
        *)
          PW_MXR_STATUS=NOT_ASSESSED
          PW_MXR_REASON="output did not match default maxrepeats=VALUE"
          ;;
      esac
    fi
  fi

  case "$PW_MXR_STATUS" in
    PASS)
      add security pw_maxrepeats "Repeated password characters" PASS high \
          "$PW_MXR_OBSERVED" \
          "The default password policy permits no more than the CIS-configured four occurrences of one character." \
