...
AIXRAY_TOOL=ck-pw-minage


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # pw_minage — minimum password age inherited from the default stanza.
  # This diagnostic fragment accepts only one exact attribute-scoped assignment.
  typeset PW_MINAGE_RAW PW_MINAGE_RC PW_MINAGE_PARSED PW_MINAGE_STATE PW_MINAGE_VALUE
  typeset PW_MINAGE_STATUS PW_MINAGE_OBSERVED PW_MINAGE_REASON
  ...
  PW_MINAGE_STATE=""
  PW_MINAGE_VALUE=""
  PW_MINAGE_STATUS=NOT_ASSESSED
  PW_MINAGE_OBSERVED=""
  PW_MINAGE_REASON=""

  if [ "${MYUID:-0}" != "0" ]; then
    PW_MINAGE_REASON="requires root privileges"
  else
    PW_MINAGE_RAW=$(aix pw_minage lssec -f /etc/security/user -s default -a minage)
    PW_MINAGE_RC=$?
    if [ "$PW_MINAGE_RC" -ne 0 ]; then
      PW_MINAGE_REASON="capture failed (rc=$PW_MINAGE_RC)"
    elif [ -z "$PW_MINAGE_RAW" ]; then
      PW_MINAGE_REASON="capture returned empty output"
    else
      PW_MINAGE_PARSED=$(printf '%s\n' "$PW_MINAGE_RAW" | awk '
        /^[[:space:]]*$/ { next }
        {
          rows++
          if ($0 !~ /^default minage=/) {
      ...
      PW_MINAGE_STATE=${PW_MINAGE_PARSED%%\|*}
      PW_MINAGE_VALUE=${PW_MINAGE_PARSED#*\|}
      case "$PW_MINAGE_STATE" in
        pass)
          PW_MINAGE_STATUS=PASS
          PW_MINAGE_OBSERVED="minage=$PW_MINAGE_VALUE"
          ;;
        fail)
          PW_MINAGE_STATUS=FAIL
          PW_MINAGE_OBSERVED="minage=$PW_MINAGE_VALUE; required at least 1"
          ;;
        huge)
          PW_MINAGE_REASON="value was too large to compare safely"
          ;;
        empty)
          PW_MINAGE_REASON="assignment value was empty"
          ;;
        *)
          PW_MINAGE_REASON="output was not one exact unsigned-decimal assignment"
          ;;
      esac
    fi
  fi

  case "$PW_MINAGE_STATUS" in
    PASS)
      add security pw_minage "Password minimum age" PASS high \
          "$PW_MINAGE_OBSERVED" \
          "The default policy prevents immediate password cycling at the approved boundary." \
          "n/a" "cis-l2"
      ;;
    FAIL)
