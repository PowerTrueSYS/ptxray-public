...
AIXRAY_TOOL=ck-pw-minalpha


function standalone_check {
_AIXRAY_SESSION_KEYS=""
# minalpha is the minimum number of alphabetic password characters. Trust
# only one exact attribute-scoped lssec assignment.

  typeset PW_MA_RAW PW_MA_RC PW_MA_NONBLANK PW_MA_LINE
  typeset PW_MA_VALUE PW_MA_DIGITS PW_MA_NORM
  typeset PW_MA_REASON PW_MA_FAILURE PW_MA_STATUS
  PW_MA_RAW=""
  PW_MA_RC=126
  PW_MA_REASON=""
  PW_MA_FAILURE=""
  ...
  if [ "${MYUID:-0}" != "0" ]; then
    PW_MA_REASON="requires root privileges"
  else
    PW_MA_RAW=$(aix pw_minalpha lssec -f /etc/security/user -s default -a minalpha)
    PW_MA_RC=$?
    if [ "$PW_MA_RC" -ne 0 ]; then
      PW_MA_REASON="capture failed (rc=$PW_MA_RC)"
    elif [ -z "$PW_MA_RAW" ]; then
      PW_MA_REASON="capture returned empty output"
    else
      PW_MA_NONBLANK=$(printf '%s\n' "$PW_MA_RAW" |
        awk 'NF { count++ } END { print count + 0 }')
      if [ "$PW_MA_NONBLANK" -ne 1 ]; then
        PW_MA_REASON="output was not one exact assignment"
      else
        PW_MA_LINE=$(printf '%s\n' "$PW_MA_RAW" |
          awk 'NF { print; exit }')
        case "$PW_MA_LINE" in
          "default minalpha="*)
            PW_MA_VALUE=${PW_MA_LINE#default minalpha=}
            case "$PW_MA_VALUE" in
              "")
                PW_MA_FAILURE="minalpha absent; AIX default 0"
                PW_MA_STATUS=FAIL
                ;;
              -[0-9]*)
                PW_MA_DIGITS=${PW_MA_VALUE#-}
                case "$PW_MA_DIGITS" in
                  ""|*[!0-9]*)
                    PW_MA_REASON="value was not a signed decimal integer"
                    ;;
                  *)
                    PW_MA_FAILURE="minalpha=$PW_MA_VALUE (below 3 or outside valid AIX range 0..8)"
                    PW_MA_STATUS=FAIL
                    ;;
                esac
                ;;
              [0-9]*)
                case "$PW_MA_VALUE" in
                  *[!0-9]*)
                    ...
                    ;;
                  *)
                    PW_MA_NORM=$PW_MA_VALUE
                    while [ "${PW_MA_NORM#0}" != "$PW_MA_NORM" ]; do
                      PW_MA_NORM=${PW_MA_NORM#0}
                    done
                    [ -n "$PW_MA_NORM" ] || PW_MA_NORM=0
                    case "$PW_MA_NORM" in
                      3|4|5|6|7|8)
                        PW_MA_STATUS=PASS
                        ;;
            ...
            ;;
          *)
            PW_MA_REASON="output did not contain the exact requested assignment"
            ;;
        esac
      fi
    fi
  ...
  if [ -n "$PW_MA_REASON" ]; then
    ...
    add security pw_minalpha "Password alphabetic minimum" PASS high \
        "minalpha=$PW_MA_VALUE" \
        "Passwords require at least three alphabetic characters." \
