...
AIXRAY_TOOL=ck-pw-mindiff


function standalone_check {
_AIXRAY_SESSION_KEYS=""
# pw_mindiff — minimum characters in a new password that were not in the old.
# IBM documents an absent mindiff as 0 (no minimum). CIS L1 requires at least
# four; supported password algorithms bound the documented domain at 255.

  typeset PW_MDF_RAW PW_MDF_RC PW_MDF_STATUS PW_MDF_OBSERVED
  typeset PW_MDF_REASON PW_MDF_VALUE PW_MDF_NUM PW_MDF_LENGTH
  PW_MDF_STATUS=NOT_ASSESSED
  PW_MDF_OBSERVED=""
  PW_MDF_REASON=""
  PW_MDF_VALUE=""
  PW_MDF_NUM=""
  ...
  if [ "${MYUID:-0}" != "0" ]; then
    PW_MDF_REASON="requires root privileges"
  else
    PW_MDF_RAW=$(aix pw_mindiff lssec -f /etc/security/user -s default -a mindiff)
    PW_MDF_RC=$?
    if [ "$PW_MDF_RC" -ne 0 ]; then
      PW_MDF_REASON="capture failed with rc $PW_MDF_RC"
    elif [ -z "$PW_MDF_RAW" ]; then
      PW_MDF_REASON="capture returned empty output"
    else
      case "$PW_MDF_RAW" in
        *'
'*)
          PW_MDF_REASON="capture was not exactly one default mindiff assignment"
          ;;
        "default mindiff=")
          PW_MDF_STATUS=FAIL
          PW_MDF_OBSERVED="mindiff absent; documented AIX default 0 (no minimum difference)"
          ;;
        default\ mindiff=*)
          PW_MDF_VALUE=${PW_MDF_RAW#default mindiff=}
          case "$PW_MDF_VALUE" in
            *[!0-9]*)
              PW_MDF_REASON="value was not a digits-only decimal integer"
              ;;
            *)
              PW_MDF_NUM=$(printf '%s\n' "$PW_MDF_VALUE" | sed 's/^0*//')
              if [ -z "$PW_MDF_NUM" ]; then
                PW_MDF_NUM=0
              fi
              PW_MDF_LENGTH=${#PW_MDF_NUM}
              if [ "$PW_MDF_LENGTH" -gt 3 ]; then
                PW_MDF_STATUS=FAIL
                PW_MDF_OBSERVED="mindiff numeric value exceeds documented 255 maximum"
              elif [ "$PW_MDF_NUM" -gt 255 ]; then
                PW_MDF_STATUS=FAIL
                PW_MDF_OBSERVED="mindiff=$PW_MDF_NUM (outside documented 0..255 domain)"
              elif [ "$PW_MDF_NUM" -ge 6 ]; then
                PW_MDF_STATUS=PASS
                PW_MDF_OBSERVED="mindiff=$PW_MDF_NUM"
              else
                PW_MDF_STATUS=FAIL
                PW_MDF_OBSERVED="mindiff=$PW_MDF_NUM"
              fi
  ...
  case "$PW_MDF_STATUS" in
    PASS)
      add security pw_mindiff "Password change difference" PASS high \
          "$PW_MDF_OBSERVED" \
          "New passwords must differ from the previous password by at least six characters." \
