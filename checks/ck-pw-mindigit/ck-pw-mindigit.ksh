...
AIXRAY_TOOL=ck-pw-mindigit


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # pw_mindigit — minimum digits required in a new password.
  # CIS IBM AIX 7 v1.2.0 5.2.14 requires mindigit >= 1.
  typeset PW_MDG_RAW PW_MDG_RC PW_MDG_VALUE PW_MDG_NORM
  typeset PW_MDG_STATUS PW_MDG_OBSERVED PW_MDG_REASON

  if [ "${MYUID:-0}" != "0" ]; then
    PW_MDG_STATUS=NOT_ASSESSED
    PW_MDG_REASON="requires root"
  else
    PW_MDG_RAW=$(aix pw_mindigit lssec -f /etc/security/user -s default -a mindigit)
    PW_MDG_RC=$?
    if [ "$PW_MDG_RC" -ne 0 ]; then
      PW_MDG_STATUS=NOT_ASSESSED
      PW_MDG_REASON="read failed (rc=$PW_MDG_RC)"
    elif [ -z "$PW_MDG_RAW" ]; then
      PW_MDG_STATUS=NOT_ASSESSED
      PW_MDG_REASON="returned empty output"
    else
      case "$PW_MDG_RAW" in
        *'
'*)
          PW_MDG_STATUS=NOT_ASSESSED
          PW_MDG_REASON="returned multiple lines"
          ;;
        "default mindigit="*)
          PW_MDG_VALUE=${PW_MDG_RAW#default mindigit=}
          if [ -z "$PW_MDG_VALUE" ]; then
            PW_MDG_STATUS=FAIL
            PW_MDG_OBSERVED="mindigit absent; documented AIX default 0 (no digit required)"
          else
            case "$PW_MDG_VALUE" in
              *[!0-9]*)
                PW_MDG_STATUS=NOT_ASSESSED
                PW_MDG_REASON="value was not a digits-only decimal"
                ;;
              *)
                PW_MDG_NORM=$PW_MDG_VALUE
                while [ "${#PW_MDG_NORM}" -gt 1 ] &&
                    [ "${PW_MDG_NORM#0}" != "$PW_MDG_NORM" ]; do
                  PW_MDG_NORM=${PW_MDG_NORM#0}
                done
                if [ "${#PW_MDG_NORM}" -gt 3 ] ||
                    [ "$PW_MDG_NORM" -gt 255 ]; then
                  PW_MDG_STATUS=FAIL
                  PW_MDG_OBSERVED="mindigit=$PW_MDG_VALUE (outside documented range 0..255)"
                elif [ "$PW_MDG_NORM" -ge 1 ]; then
                  PW_MDG_STATUS=PASS
                  PW_MDG_OBSERVED="mindigit=$PW_MDG_VALUE"
                else
                  PW_MDG_STATUS=FAIL
                  PW_MDG_OBSERVED="mindigit=0 (no digit required)"
                fi
                ;;
            esac
          fi
        ...
        *)
          PW_MDG_STATUS=NOT_ASSESSED
          PW_MDG_REASON="output did not match default mindigit=VALUE"
          ;;
      esac
    fi
  fi

  case "$PW_MDG_STATUS" in
    PASS)
      add security pw_mindigit "Password digit requirement" PASS high \
          "$PW_MDG_OBSERVED" \
          "The default password policy requires at least one digit in every new password." \
          "n/a" "cis-l1"
      ;;
    FAIL)
