...
AIXRAY_TOOL=ck-pw-minlen


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # pw_minlen — minimum length of a new password from the default stanza.
  # CIS IBM AIX 7 v1.2.0 5.2.9 requires minlen >= 14.
  typeset PW_MLN_RAW PW_MLN_RC PW_MLN_VALUE PW_MLN_NORM
  typeset PW_MLN_STATUS PW_MLN_OBSERVED PW_MLN_REASON

  if [ "${MYUID:-0}" != "0" ]; then
    PW_MLN_STATUS=NOT_ASSESSED
    PW_MLN_REASON="requires root"
  else
    PW_MLN_RAW=$(aix pw_minlen lssec -f /etc/security/user -s default -a minlen)
    PW_MLN_RC=$?
    if [ "$PW_MLN_RC" -ne 0 ]; then
      PW_MLN_STATUS=NOT_ASSESSED
      PW_MLN_REASON="read failed (rc=$PW_MLN_RC)"
    elif [ -z "$PW_MLN_RAW" ]; then
      PW_MLN_STATUS=NOT_ASSESSED
      PW_MLN_REASON="returned empty output"
    else
      case "$PW_MLN_RAW" in
        *'
'*)
          PW_MLN_STATUS=NOT_ASSESSED
          PW_MLN_REASON="returned multiple lines"
          ;;
        "default minlen="*)
          PW_MLN_VALUE=${PW_MLN_RAW#default minlen=}
          if [ -z "$PW_MLN_VALUE" ]; then
            PW_MLN_STATUS=FAIL
            PW_MLN_OBSERVED="minlen absent; documented AIX default 0 (no minimum length)"
          else
            case "$PW_MLN_VALUE" in
              *[!0-9]*)
                PW_MLN_STATUS=NOT_ASSESSED
                PW_MLN_REASON="value was not a digits-only decimal"
                ;;
              *)
                PW_MLN_NORM=$PW_MLN_VALUE
                while [ "${#PW_MLN_NORM}" -gt 1 ] &&
                    [ "${PW_MLN_NORM#0}" != "$PW_MLN_NORM" ]; do
                  PW_MLN_NORM=${PW_MLN_NORM#0}
                done
                if [ "${#PW_MLN_NORM}" -gt 3 ] ||
                    [ "$PW_MLN_NORM" -gt 255 ]; then
                  PW_MLN_STATUS=FAIL
                  PW_MLN_OBSERVED="minlen=$PW_MLN_VALUE (outside documented range 0..255)"
                elif [ "$PW_MLN_NORM" -ge 14 ]; then
                  PW_MLN_STATUS=PASS
                  PW_MLN_OBSERVED="minlen=$PW_MLN_VALUE"
                else
                  PW_MLN_STATUS=FAIL
                  PW_MLN_OBSERVED="minlen=$PW_MLN_VALUE; CIS baseline requires at least 14"
                fi
                ;;
            esac
          fi
        ...
        *)
          PW_MLN_STATUS=NOT_ASSESSED
          PW_MLN_REASON="output did not match default minlen=VALUE"
          ;;
      esac
    fi
  fi

  case "$PW_MLN_STATUS" in
    PASS)
      add security pw_minlen "Minimum password length" PASS high \
          "$PW_MLN_OBSERVED" \
          "The default password policy enforces the CIS-baseline minimum length for every new password." \
          "n/a" "cis-l1"
      ;;
    FAIL)
