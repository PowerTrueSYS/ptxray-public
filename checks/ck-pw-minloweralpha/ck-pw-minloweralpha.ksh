...
AIXRAY_TOOL=ck-pw-minloweralpha


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # pw_minloweralpha — minimum lower-case letters required in a new password.
  # CIS IBM AIX 7 v1.2.0 5.2.15 requires minloweralpha >= 1.
  typeset PW_MLA_RAW PW_MLA_RC PW_MLA_VALUE PW_MLA_NORM
  typeset PW_MLA_STATUS PW_MLA_OBSERVED PW_MLA_REASON

  if [ "${MYUID:-0}" != "0" ]; then
    PW_MLA_STATUS=NOT_ASSESSED
    PW_MLA_REASON="requires root"
  else
    PW_MLA_RAW=$(aix pw_minloweralpha lssec -f /etc/security/user -s default -a minloweralpha)
    PW_MLA_RC=$?
    if [ "$PW_MLA_RC" -ne 0 ]; then
      PW_MLA_STATUS=NOT_ASSESSED
      PW_MLA_REASON="read failed (rc=$PW_MLA_RC)"
    elif [ -z "$PW_MLA_RAW" ]; then
      PW_MLA_STATUS=NOT_ASSESSED
      PW_MLA_REASON="returned empty output"
    else
      case "$PW_MLA_RAW" in
        *'
'*)
          PW_MLA_STATUS=NOT_ASSESSED
          PW_MLA_REASON="returned multiple lines"
          ;;
        "default minloweralpha="*)
          PW_MLA_VALUE=${PW_MLA_RAW#default minloweralpha=}
          if [ -z "$PW_MLA_VALUE" ]; then
            PW_MLA_STATUS=FAIL
            PW_MLA_OBSERVED="minloweralpha absent; documented AIX default 0 (no lower-case letter required)"
          else
            case "$PW_MLA_VALUE" in
              *[!0-9]*)
                PW_MLA_STATUS=NOT_ASSESSED
                PW_MLA_REASON="value was not a digits-only decimal"
                ;;
              *)
                PW_MLA_NORM=$PW_MLA_VALUE
                while [ "${#PW_MLA_NORM}" -gt 1 ] &&
                    [ "${PW_MLA_NORM#0}" != "$PW_MLA_NORM" ]; do
                  PW_MLA_NORM=${PW_MLA_NORM#0}
                done
                if [ "${#PW_MLA_NORM}" -gt 3 ] ||
                    [ "$PW_MLA_NORM" -gt 255 ]; then
                  PW_MLA_STATUS=FAIL
                  PW_MLA_OBSERVED="minloweralpha=$PW_MLA_VALUE (outside documented range 0..255)"
                elif [ "$PW_MLA_NORM" -ge 1 ]; then
                  PW_MLA_STATUS=PASS
                  PW_MLA_OBSERVED="minloweralpha=$PW_MLA_VALUE"
                else
                  PW_MLA_STATUS=FAIL
                  PW_MLA_OBSERVED="minloweralpha=0 (no lower-case letter required)"
                fi
                ;;
            esac
          fi
        ...
        *)
          PW_MLA_STATUS=NOT_ASSESSED
          PW_MLA_REASON="output did not match default minloweralpha=VALUE"
          ;;
      esac
    fi
  fi

  case "$PW_MLA_STATUS" in
    PASS)
      add security pw_minloweralpha "Password lower-case requirement" PASS high \
          "$PW_MLA_OBSERVED" \
          "The default password policy requires at least one lower-case alphabetic character in every new password." \
          "n/a" "cis-l1"
      ;;
    FAIL)
