...
AIXRAY_TOOL=ck-pw-minother


function standalone_check {
_AIXRAY_SESSION_KEYS=""
# minother is the minimum number of non-alphabetic password characters. Trust
# only one exact attribute-scoped lssec assignment.

  typeset PW_MO_RAW PW_MO_RC PW_MO_NONBLANK PW_MO_LINE
  typeset PW_MO_VALUE PW_MO_DIGITS PW_MO_NORM
  typeset PW_MO_REASON PW_MO_FAILURE PW_MO_STATUS
  PW_MO_RAW=""
  PW_MO_RC=126
  PW_MO_REASON=""
  PW_MO_FAILURE=""
  ...
  if [ "${MYUID:-0}" != "0" ]; then
    PW_MO_REASON="requires root privileges"
  else
    PW_MO_RAW=$(aix pw_minother lssec -f /etc/security/user -s default -a minother)
    PW_MO_RC=$?
    if [ "$PW_MO_RC" -ne 0 ]; then
      PW_MO_REASON="capture failed (rc=$PW_MO_RC)"
    elif [ -z "$PW_MO_RAW" ]; then
      PW_MO_REASON="capture returned empty output"
    else
      PW_MO_NONBLANK=$(printf '%s\n' "$PW_MO_RAW" |
        awk 'NF { count++ } END { print count + 0 }')
      if [ "$PW_MO_NONBLANK" -ne 1 ]; then
        PW_MO_REASON="output was not one exact assignment"
      else
        PW_MO_LINE=$(printf '%s\n' "$PW_MO_RAW" |
          awk 'NF { print; exit }')
        case "$PW_MO_LINE" in
          "default minother="*)
            PW_MO_VALUE=${PW_MO_LINE#default minother=}
            case "$PW_MO_VALUE" in
              "")
                PW_MO_FAILURE="minother absent; AIX default 0"
                PW_MO_STATUS=FAIL
                ;;
              -[0-9]*)
                PW_MO_DIGITS=${PW_MO_VALUE#-}
                case "$PW_MO_DIGITS" in
                  ""|*[!0-9]*)
                    PW_MO_REASON="value was not a signed decimal integer"
                    ;;
                  *)
                    PW_MO_FAILURE="minother=$PW_MO_VALUE (below 3 or outside valid AIX range 0..8)"
                    PW_MO_STATUS=FAIL
                    ;;
                esac
                ;;
              [0-9]*)
                case "$PW_MO_VALUE" in
                  *[!0-9]*)
                    ...
                    ;;
                  *)
                    PW_MO_NORM=$PW_MO_VALUE
                    while [ "${PW_MO_NORM#0}" != "$PW_MO_NORM" ]; do
                      PW_MO_NORM=${PW_MO_NORM#0}
                    done
                    [ -n "$PW_MO_NORM" ] || PW_MO_NORM=0
                    case "$PW_MO_NORM" in
                      3|4|5|6|7|8)
                        PW_MO_STATUS=PASS
                        ;;
            ...
            ;;
          *)
            PW_MO_REASON="output did not contain the exact requested assignment"
            ;;
        esac
      fi
    fi
  ...
  if [ -n "$PW_MO_REASON" ]; then
    ...
    add security pw_minother "Password non-alphabetic minimum" PASS high \
        "minother=$PW_MO_VALUE" \
        "Passwords require at least three non-alphabetic characters." \
