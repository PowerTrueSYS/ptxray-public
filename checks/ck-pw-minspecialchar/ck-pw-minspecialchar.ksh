...
AIXRAY_TOOL=ck-pw-minspecialchar


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # pw_minspecialchar — minimum special characters required in a new password.
  # CIS IBM AIX 7 v1.2.0 5.2.17 requires minspecialchar >= 1.
  typeset PW_MSC_RAW PW_MSC_RC PW_MSC_VALUE PW_MSC_NORM
  typeset PW_MSC_STATUS PW_MSC_OBSERVED PW_MSC_REASON

  if [ "${MYUID:-0}" != "0" ]; then
    PW_MSC_STATUS=NOT_ASSESSED
    PW_MSC_REASON="requires root"
  else
    PW_MSC_RAW=$(aix pw_minspecialchar lssec -f /etc/security/user -s default -a minspecialchar)
    PW_MSC_RC=$?
    if [ "$PW_MSC_RC" -ne 0 ]; then
      PW_MSC_STATUS=NOT_ASSESSED
      PW_MSC_REASON="read failed (rc=$PW_MSC_RC)"
    elif [ -z "$PW_MSC_RAW" ]; then
      PW_MSC_STATUS=NOT_ASSESSED
      PW_MSC_REASON="returned empty output"
    else
      case "$PW_MSC_RAW" in
        *'
'*)
          PW_MSC_STATUS=NOT_ASSESSED
          PW_MSC_REASON="returned multiple lines"
          ;;
        "default minspecialchar="*)
          PW_MSC_VALUE=${PW_MSC_RAW#default minspecialchar=}
          if [ -z "$PW_MSC_VALUE" ]; then
            PW_MSC_STATUS=FAIL
            PW_MSC_OBSERVED="minspecialchar absent; documented AIX default 0 (no special character required)"
          else
            case "$PW_MSC_VALUE" in
              *[!0-9]*)
                PW_MSC_STATUS=NOT_ASSESSED
                PW_MSC_REASON="value was not a digits-only decimal"
                ;;
              *)
                PW_MSC_NORM=$PW_MSC_VALUE
                while [ "${#PW_MSC_NORM}" -gt 1 ] &&
                    [ "${PW_MSC_NORM#0}" != "$PW_MSC_NORM" ]; do
                  PW_MSC_NORM=${PW_MSC_NORM#0}
                done
                if [ "${#PW_MSC_NORM}" -gt 3 ] ||
                    [ "$PW_MSC_NORM" -gt 255 ]; then
                  PW_MSC_STATUS=FAIL
                  PW_MSC_OBSERVED="minspecialchar=$PW_MSC_VALUE (outside documented range 0..255)"
                elif [ "$PW_MSC_NORM" -ge 1 ]; then
                  PW_MSC_STATUS=PASS
                  PW_MSC_OBSERVED="minspecialchar=$PW_MSC_VALUE"
                else
                  PW_MSC_STATUS=FAIL
                  PW_MSC_OBSERVED="minspecialchar=0 (no special character required)"
                fi
                ;;
            esac
          fi
        ...
        *)
          PW_MSC_STATUS=NOT_ASSESSED
          PW_MSC_REASON="output did not match default minspecialchar=VALUE"
          ;;
      esac
    fi
  fi

  case "$PW_MSC_STATUS" in
    PASS)
      add security pw_minspecialchar "Password special-character requirement" PASS high \
          "$PW_MSC_OBSERVED" \
          "The default password policy requires at least one special character in every new password." \
          "n/a" "cis-l1"
      ;;
    FAIL)
