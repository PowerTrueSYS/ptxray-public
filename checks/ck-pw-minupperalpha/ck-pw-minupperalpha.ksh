...
AIXRAY_TOOL=ck-pw-minupperalpha


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # pw_minupperalpha — minimum upper-case letters required in a new password.
  # CIS IBM AIX 7 v1.2.0 5.2.16 requires minupperalpha >= 1.
  typeset PW_MUA_RAW PW_MUA_RC PW_MUA_VALUE PW_MUA_NORM
  typeset PW_MUA_STATUS PW_MUA_OBSERVED PW_MUA_REASON

  if [ "${MYUID:-0}" != "0" ]; then
    PW_MUA_STATUS=NOT_ASSESSED
    PW_MUA_REASON="requires root"
  else
    PW_MUA_RAW=$(aix pw_minupperalpha lssec -f /etc/security/user -s default -a minupperalpha)
    PW_MUA_RC=$?
    if [ "$PW_MUA_RC" -ne 0 ]; then
      PW_MUA_STATUS=NOT_ASSESSED
      PW_MUA_REASON="read failed (rc=$PW_MUA_RC)"
    elif [ -z "$PW_MUA_RAW" ]; then
      PW_MUA_STATUS=NOT_ASSESSED
      PW_MUA_REASON="returned empty output"
    else
      case "$PW_MUA_RAW" in
        *'
'*)
          PW_MUA_STATUS=NOT_ASSESSED
          PW_MUA_REASON="returned multiple lines"
          ;;
        "default minupperalpha="*)
          PW_MUA_VALUE=${PW_MUA_RAW#default minupperalpha=}
          if [ -z "$PW_MUA_VALUE" ]; then
            PW_MUA_STATUS=FAIL
            PW_MUA_OBSERVED="minupperalpha absent; documented AIX default 0 (no upper-case letter required)"
          else
            case "$PW_MUA_VALUE" in
              *[!0-9]*)
                PW_MUA_STATUS=NOT_ASSESSED
                PW_MUA_REASON="value was not a digits-only decimal"
                ;;
              *)
                PW_MUA_NORM=$PW_MUA_VALUE
                while [ "${#PW_MUA_NORM}" -gt 1 ] &&
                    [ "${PW_MUA_NORM#0}" != "$PW_MUA_NORM" ]; do
                  PW_MUA_NORM=${PW_MUA_NORM#0}
                done
                if [ "${#PW_MUA_NORM}" -gt 3 ] ||
                    [ "$PW_MUA_NORM" -gt 255 ]; then
                  PW_MUA_STATUS=FAIL
                  PW_MUA_OBSERVED="minupperalpha=$PW_MUA_VALUE (outside documented range 0..255)"
                elif [ "$PW_MUA_NORM" -ge 1 ]; then
                  PW_MUA_STATUS=PASS
                  PW_MUA_OBSERVED="minupperalpha=$PW_MUA_VALUE"
                else
                  PW_MUA_STATUS=FAIL
                  PW_MUA_OBSERVED="minupperalpha=0 (no upper-case letter required)"
                fi
                ;;
            esac
          fi
        ...
        *)
          PW_MUA_STATUS=NOT_ASSESSED
          PW_MUA_REASON="output did not match default minupperalpha=VALUE"
          ;;
      esac
    fi
  fi

  case "$PW_MUA_STATUS" in
    PASS)
      add security pw_minupperalpha "Password upper-case requirement" PASS high \
          "$PW_MUA_OBSERVED" \
          "The default password policy requires at least one upper-case alphabetic character in every new password." \
          "n/a" "cis-l1"
      ;;
    FAIL)
