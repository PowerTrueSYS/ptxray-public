...
AIXRAY_TOOL=ck-pw-nocheck


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # pw_nocheck — accounts whose /etc/security/passwd stanza carries the NOCHECK
  # flag, which exempts them from every password rule. CIS IBM AIX 7 v1.2.0
  # 5.2.4 requires that no account carries it. Evidence is usernames and counts
  # only; hash material is never captured.
  typeset PW_NCK_RAW PW_NCK_RC PW_NCK_SCANNED PW_NCK_COUNT PW_NCK_USERS PW_NCK_TAIL
  typeset PW_NCK_STATUS PW_NCK_OBSERVED PW_NCK_REASON

  if [ "${MYUID:-0}" != "0" ]; then
    PW_NCK_STATUS=NOT_ASSESSED
    PW_NCK_REASON="requires root"
  else
    PW_NCK_RAW=$(aix pw_nocheck awk '/^[^ \t:][^:]*:[ \t]*$/ { u=$1; sub(/:[ \t]*$/,"",u); n++ } $1=="flags" && $2=="=" && $0 ~ /NOCHECK/ && u!="" { k++; if (k<=6) users=(users=="")?u:users","u } END { printf "scanned=%d nocheck=%d users=%s\n", n, k, users }' /etc/security/passwd)
    PW_NCK_RC=$?
    if [ "$PW_NCK_RC" -ne 0 ]; then
      PW_NCK_STATUS=NOT_ASSESSED
      PW_NCK_REASON="scan failed (rc=$PW_NCK_RC)"
    else
      case "$PW_NCK_RAW" in
        *'
'*)
          PW_NCK_STATUS=NOT_ASSESSED
          PW_NCK_REASON="returned multiple lines"
          ;;
        "scanned="*" nocheck="*" users="*)
          PW_NCK_SCANNED=${PW_NCK_RAW#scanned=}
          PW_NCK_SCANNED=${PW_NCK_SCANNED%% *}
          PW_NCK_COUNT=${PW_NCK_RAW#*nocheck=}
          PW_NCK_COUNT=${PW_NCK_COUNT%% *}
          PW_NCK_USERS=${PW_NCK_RAW#*users=}
          case "$PW_NCK_SCANNED$PW_NCK_COUNT" in
            *[!0-9]*)
              PW_NCK_STATUS=NOT_ASSESSED
              PW_NCK_REASON="summary counters were not decimal"
              ;;
            *)
              if [ "$PW_NCK_SCANNED" -lt 1 ]; then
                PW_NCK_STATUS=NOT_ASSESSED
                PW_NCK_REASON="no stanzas parsed from /etc/security/passwd"
              elif [ "$PW_NCK_COUNT" -eq 0 ]; then
                PW_NCK_STATUS=PASS
                PW_NCK_OBSERVED="$PW_NCK_SCANNED stanza(s) scanned; no NOCHECK flag"
              else
                PW_NCK_TAIL=""
                [ "$PW_NCK_COUNT" -gt 6 ] && PW_NCK_TAIL=" (+$((PW_NCK_COUNT - 6)) more)"
                PW_NCK_STATUS=FAIL
                PW_NCK_OBSERVED="NOCHECK set for $PW_NCK_COUNT account(s): $PW_NCK_USERS$PW_NCK_TAIL"
              fi
              ;;
          esac
          ;;
        *)
          PW_NCK_STATUS=NOT_ASSESSED
          PW_NCK_REASON="scan output did not match the expected summary line"
          ;;
      esac
    fi
  fi

  case "$PW_NCK_STATUS" in
    PASS)
      add security pw_nocheck "Password rules bypass (NOCHECK)" PASS high \
          "$PW_NCK_OBSERVED" \
          "Every local account is subject to the configured password rules; none is exempted by the NOCHECK flag." \
          "n/a" "cis-l1"
      ;;
    FAIL)
