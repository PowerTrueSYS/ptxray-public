...
AIXRAY_TOOL=ck-pw-stored-hash


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # pw_stored_hash — accounts whose /etc/security/passwd stanza still stores a
  # legacy crypt-format password hash (exactly 13 characters). CIS IBM AIX 7
  # v1.2.0 5.2.8 requires that no non-system account still stores one. Evidence
  # is usernames and counts only; hash material is never captured.
  typeset PW_SHH_RAW PW_SHH_RC PW_SHH_SCANNED PW_SHH_COUNT PW_SHH_USERS PW_SHH_TAIL
  typeset PW_SHH_STATUS PW_SHH_OBSERVED PW_SHH_REASON

  if [ "${MYUID:-0}" != "0" ]; then
    PW_SHH_STATUS=NOT_ASSESSED
    PW_SHH_REASON="requires root"
  else
    # network-lint: allow-next=1 -- "ftp" below is CIS's own AIX system-account exclusion list entry (an account name), not a network command invocation.
    PW_SHH_RAW=$(aix pw_stored_hash awk '/^[^ \t:][^:]*:[ \t]*$/ { u=$1; sub(/:[ \t]*$/,"",u); n++ } $1=="password" && $2=="=" && NF==3 && length($3)==13 && u!="" && u !~ /^(adm|bin|daemon|invscout|ipsec|lp|lpd|nobody|nuucp|sshd|sys|uucp|esa|srvproxy|imnadm|anonymou|ftp)$/ { k++; if (k<=6) users=(users=="")?u:users","u } END { printf "scanned=%d crypt=%d users=%s\n", n, k, users }' /etc/security/passwd)
    PW_SHH_RC=$?
    if [ "$PW_SHH_RC" -ne 0 ]; then
      PW_SHH_STATUS=NOT_ASSESSED
      PW_SHH_REASON="scan failed (rc=$PW_SHH_RC)"
    else
      case "$PW_SHH_RAW" in
        *'
'*)
          PW_SHH_STATUS=NOT_ASSESSED
          PW_SHH_REASON="returned multiple lines"
          ;;
        "scanned="*" crypt="*" users="*)
          PW_SHH_SCANNED=${PW_SHH_RAW#scanned=}
          PW_SHH_SCANNED=${PW_SHH_SCANNED%% *}
          PW_SHH_COUNT=${PW_SHH_RAW#*crypt=}
          PW_SHH_COUNT=${PW_SHH_COUNT%% *}
          PW_SHH_USERS=${PW_SHH_RAW#*users=}
          case "$PW_SHH_SCANNED$PW_SHH_COUNT" in
            *[!0-9]*)
              PW_SHH_STATUS=NOT_ASSESSED
              PW_SHH_REASON="summary counters were not decimal"
              ;;
            *)
              if [ "$PW_SHH_SCANNED" -lt 1 ]; then
                PW_SHH_STATUS=NOT_ASSESSED
                PW_SHH_REASON="no stanzas parsed from /etc/security/passwd"
              elif [ "$PW_SHH_COUNT" -eq 0 ]; then
                PW_SHH_STATUS=PASS
                PW_SHH_OBSERVED="$PW_SHH_SCANNED stanza(s) scanned; no legacy crypt hash"
              else
                PW_SHH_TAIL=""
                [ "$PW_SHH_COUNT" -gt 6 ] && PW_SHH_TAIL=" (+$((PW_SHH_COUNT - 6)) more)"
                PW_SHH_STATUS=FAIL
                PW_SHH_OBSERVED="legacy crypt hash for $PW_SHH_COUNT account(s): $PW_SHH_USERS$PW_SHH_TAIL"
              fi
              ;;
          esac
          ;;
        *)
          PW_SHH_STATUS=NOT_ASSESSED
          PW_SHH_REASON="scan output did not match the expected summary line"
          ;;
      esac
    fi
  fi

  case "$PW_SHH_STATUS" in
    PASS)
      add security pw_stored_hash "Legacy crypt password hashes" PASS high \
          "$PW_SHH_OBSERVED" \
          "No non-system account still stores a password hashed with the legacy 56-bit crypt algorithm." \
          "n/a" "cis-l1"
      ;;
    FAIL)
