...
_AIXRAY_SESSION_KEYS=""
  typeset QDAEMON_MATCH QDAEMON_MATCH_RC QDAEMON_COUNT QDAEMON_STATE
    ...
    if [ "$QDAEMON_MATCH_RC" -ne 0 ]; then
      ...
      if [ "$QDAEMON_COUNT" -gt 1 ]; then
        ...
        add security qdaemon "qdaemon service" PASS low \
            ...
            "The qdaemon subsystem is absent and therefore is not in use." "n/a" "cis-l1"
      else
        QDAEMON_STATE=$QDAEMON_MATCH
        if [ "$QDAEMON_STATE" = in_use ]; then
          ...
          add security qdaemon "qdaemon service" PASS low \
              ...
              "The qdaemon subsystem is present but not in use." "n/a" "cis-l1"
        fi
      fi
    fi
  fi
