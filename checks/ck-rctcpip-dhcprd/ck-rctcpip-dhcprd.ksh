...
_AIXRAY_SESSION_KEYS=""
  # rctcpip_dhcprd — CIS IBM AIX 7 v1.2.0 4.3.2.3: dhcprd must not be in use.
  ...
  typeset RT_DHCPRD_BOOT RT_DHCPRD_SRC RT_DHCPRD_DETAIL RT_DHCPRD_BOOT_RC RT_DHCPRD_SRC_RC
  typeset RT_DHCPRD_STATUS RT_DHCPRD_SEV RT_DHCPRD_OBS RT_DHCPRD_MEAN RT_DHCPRD_FIX
  if [ "${RCTCPIP_READY:-0}" -eq 1 ]; then
    RT_DHCPRD_BOOT=$(printf '%s\n' "$RCTCPIP_ROWS" | awk -F'|' '
      $1 == "dhcprd" { found = 1 }
      END { if (found) print "boot"; else print "no-boot" }')
    RT_DHCPRD_BOOT_RC=$?
    [ "$RT_DHCPRD_BOOT_RC" -ne 0 ] && RT_DHCPRD_BOOT=unknown
  else
    RT_DHCPRD_BOOT=unknown
  fi
  if [ "${CIS_SERVICES_READY:-0}" -eq 1 ]; then
    RT_DHCPRD_SRC=$(printf '%s\n' "$CIS_SERVICES_ROWS" | awk -F'|' '
      $1 == "dhcprd" { n++; if ($2 == "in_use") active = 1 }
      END {
    ...
    RT_DHCPRD_SRC_RC=$?
    [ "$RT_DHCPRD_SRC_RC" -ne 0 ] && RT_DHCPRD_SRC=unknown
  else
    RT_DHCPRD_SRC=unknown
  fi

  RT_DHCPRD_DETAIL=""
  [ "$RT_DHCPRD_BOOT" = boot ] && RT_DHCPRD_DETAIL="starts at boot in /etc/rc.tcpip"
  if [ "$RT_DHCPRD_SRC" = in_use ]; then
    if [ -n "$RT_DHCPRD_DETAIL" ]; then
      RT_DHCPRD_DETAIL="$RT_DHCPRD_DETAIL and is active in the SRC inventory"
    else
      RT_DHCPRD_DETAIL="is active in the SRC inventory"
    fi
  fi

  if [ -n "$RT_DHCPRD_DETAIL" ]; then
    RT_DHCPRD_STATUS=FAIL
    RT_DHCPRD_SEV=high
    ...
    RT_DHCPRD_SEV=low
    if [ "$RT_DHCPRD_SRC" = duplicate ]; then
      RT_DHCPRD_OBS="not assessed - dhcprd has duplicate SRC inventory rows"
    elif [ "$RT_DHCPRD_BOOT" = unknown ]; then
      RT_DHCPRD_OBS="not assessed - dhcprd rc.tcpip evidence $RCTCPIP_REASON"
    else
    ...
    RT_DHCPRD_SEV=low
    RT_DHCPRD_OBS="dhcprd not started in /etc/rc.tcpip and not active in the SRC inventory"
    RT_DHCPRD_MEAN="The dhcprd service is not in use."
    RT_DHCPRD_FIX="n/a"
  fi

  add security rctcpip_dhcprd "dhcp relay service" \
    "$RT_DHCPRD_STATUS" "$RT_DHCPRD_SEV" \
    "$RT_DHCPRD_OBS" \
    "$RT_DHCPRD_MEAN" \
    "$RT_DHCPRD_FIX" \
    "cis-l1"
