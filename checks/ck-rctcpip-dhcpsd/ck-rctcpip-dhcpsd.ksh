...
_AIXRAY_SESSION_KEYS=""
  # rctcpip_dhcpsd — CIS IBM AIX 7 v1.2.0 4.3.2.4: dhcpsd must not be in use.
  ...
  typeset RT_DHCPSD_BOOT RT_DHCPSD_SRC RT_DHCPSD_DETAIL RT_DHCPSD_BOOT_RC RT_DHCPSD_SRC_RC
  typeset RT_DHCPSD_STATUS RT_DHCPSD_SEV RT_DHCPSD_OBS RT_DHCPSD_MEAN RT_DHCPSD_FIX
  if [ "${RCTCPIP_READY:-0}" -eq 1 ]; then
    RT_DHCPSD_BOOT=$(printf '%s\n' "$RCTCPIP_ROWS" | awk -F'|' '
      $1 == "dhcpsd" { found = 1 }
      END { if (found) print "boot"; else print "no-boot" }')
    RT_DHCPSD_BOOT_RC=$?
    [ "$RT_DHCPSD_BOOT_RC" -ne 0 ] && RT_DHCPSD_BOOT=unknown
  else
    RT_DHCPSD_BOOT=unknown
  fi
  if [ "${CIS_SERVICES_READY:-0}" -eq 1 ]; then
    RT_DHCPSD_SRC=$(printf '%s\n' "$CIS_SERVICES_ROWS" | awk -F'|' '
      $1 == "dhcpsd" { n++; if ($2 == "in_use") active = 1 }
      END {
    ...
    RT_DHCPSD_SRC_RC=$?
    [ "$RT_DHCPSD_SRC_RC" -ne 0 ] && RT_DHCPSD_SRC=unknown
  else
    RT_DHCPSD_SRC=unknown
  fi

  RT_DHCPSD_DETAIL=""
  [ "$RT_DHCPSD_BOOT" = boot ] && RT_DHCPSD_DETAIL="starts at boot in /etc/rc.tcpip"
  if [ "$RT_DHCPSD_SRC" = in_use ]; then
    if [ -n "$RT_DHCPSD_DETAIL" ]; then
      RT_DHCPSD_DETAIL="$RT_DHCPSD_DETAIL and is active in the SRC inventory"
    else
      RT_DHCPSD_DETAIL="is active in the SRC inventory"
    fi
  fi

  if [ -n "$RT_DHCPSD_DETAIL" ]; then
    RT_DHCPSD_STATUS=FAIL
    RT_DHCPSD_SEV=high
    ...
    RT_DHCPSD_SEV=low
    if [ "$RT_DHCPSD_SRC" = duplicate ]; then
      RT_DHCPSD_OBS="not assessed - dhcpsd has duplicate SRC inventory rows"
    elif [ "$RT_DHCPSD_BOOT" = unknown ]; then
      RT_DHCPSD_OBS="not assessed - dhcpsd rc.tcpip evidence $RCTCPIP_REASON"
    else
    ...
    RT_DHCPSD_SEV=low
    RT_DHCPSD_OBS="dhcpsd not started in /etc/rc.tcpip and not active in the SRC inventory"
    RT_DHCPSD_MEAN="The dhcpsd service is not in use."
    RT_DHCPSD_FIX="n/a"
  fi

  add security rctcpip_dhcpsd "dhcp server service" \
    "$RT_DHCPSD_STATUS" "$RT_DHCPSD_SEV" \
    "$RT_DHCPSD_OBS" \
    "$RT_DHCPSD_MEAN" \
    "$RT_DHCPSD_FIX" \
    "cis-l1"
