...
_AIXRAY_SESSION_KEYS=""
  # rctcpip_gated — CIS IBM AIX 7 v1.2.0 4.3.2.5: gated must not be in use.
  ...
  typeset RT_GATED_BOOT RT_GATED_SRC RT_GATED_DETAIL RT_GATED_BOOT_RC RT_GATED_SRC_RC
  typeset RT_GATED_STATUS RT_GATED_SEV RT_GATED_OBS RT_GATED_MEAN RT_GATED_FIX
  if [ "${RCTCPIP_READY:-0}" -eq 1 ]; then
    RT_GATED_BOOT=$(printf '%s\n' "$RCTCPIP_ROWS" | awk -F'|' '
      $1 == "gated" { found = 1 }
      END { if (found) print "boot"; else print "no-boot" }')
    RT_GATED_BOOT_RC=$?
    [ "$RT_GATED_BOOT_RC" -ne 0 ] && RT_GATED_BOOT=unknown
  else
    RT_GATED_BOOT=unknown
  fi
  if [ "${CIS_SERVICES_READY:-0}" -eq 1 ]; then
    RT_GATED_SRC=$(printf '%s\n' "$CIS_SERVICES_ROWS" | awk -F'|' '
      $1 == "gated" { n++; if ($2 == "in_use") active = 1 }
      END {
    ...
    RT_GATED_SRC_RC=$?
    [ "$RT_GATED_SRC_RC" -ne 0 ] && RT_GATED_SRC=unknown
  else
    RT_GATED_SRC=unknown
  fi

  RT_GATED_DETAIL=""
  [ "$RT_GATED_BOOT" = boot ] && RT_GATED_DETAIL="starts at boot in /etc/rc.tcpip"
  if [ "$RT_GATED_SRC" = in_use ]; then
    if [ -n "$RT_GATED_DETAIL" ]; then
      RT_GATED_DETAIL="$RT_GATED_DETAIL and is active in the SRC inventory"
    else
      RT_GATED_DETAIL="is active in the SRC inventory"
    fi
  fi

  if [ -n "$RT_GATED_DETAIL" ]; then
    RT_GATED_STATUS=FAIL
    RT_GATED_SEV=high
    ...
    RT_GATED_SEV=low
    if [ "$RT_GATED_SRC" = duplicate ]; then
      RT_GATED_OBS="not assessed - gated has duplicate SRC inventory rows"
    elif [ "$RT_GATED_BOOT" = unknown ]; then
      RT_GATED_OBS="not assessed - gated rc.tcpip evidence $RCTCPIP_REASON"
    else
    ...
    RT_GATED_SEV=low
    RT_GATED_OBS="gated not started in /etc/rc.tcpip and not active in the SRC inventory"
    RT_GATED_MEAN="The gated service is not in use."
    RT_GATED_FIX="n/a"
  fi

  add security rctcpip_gated "gated service" \
    "$RT_GATED_STATUS" "$RT_GATED_SEV" \
    "$RT_GATED_OBS" \
    "$RT_GATED_MEAN" \
    "$RT_GATED_FIX" \
    "cis-l1"
