...
_AIXRAY_SESSION_KEYS=""
  # rctcpip_inetd — CIS IBM AIX 7 v1.2.0 4.3.2.1: inetd must not be in use.
  ...
  typeset RT_INETD_BOOT RT_INETD_SRC RT_INETD_DETAIL RT_INETD_BOOT_RC RT_INETD_SRC_RC
  typeset RT_INETD_STATUS RT_INETD_SEV RT_INETD_OBS RT_INETD_MEAN RT_INETD_FIX
  if [ "${RCTCPIP_READY:-0}" -eq 1 ]; then
    RT_INETD_BOOT=$(printf '%s\n' "$RCTCPIP_ROWS" | awk -F'|' '
      $1 == "inetd" { found = 1 }
      END { if (found) print "boot"; else print "no-boot" }')
    RT_INETD_BOOT_RC=$?
    [ "$RT_INETD_BOOT_RC" -ne 0 ] && RT_INETD_BOOT=unknown
  else
    RT_INETD_BOOT=unknown
  fi
  if [ "${CIS_SERVICES_READY:-0}" -eq 1 ]; then
    RT_INETD_SRC=$(printf '%s\n' "$CIS_SERVICES_ROWS" | awk -F'|' '
      $1 == "inetd" { n++; if ($2 == "in_use") active = 1 }
      END {
    ...
    RT_INETD_SRC_RC=$?
    [ "$RT_INETD_SRC_RC" -ne 0 ] && RT_INETD_SRC=unknown
  else
    RT_INETD_SRC=unknown
  fi

  RT_INETD_DETAIL=""
  [ "$RT_INETD_BOOT" = boot ] && RT_INETD_DETAIL="starts at boot in /etc/rc.tcpip"
  if [ "$RT_INETD_SRC" = in_use ]; then
    if [ -n "$RT_INETD_DETAIL" ]; then
      RT_INETD_DETAIL="$RT_INETD_DETAIL and is active in the SRC inventory"
    else
      RT_INETD_DETAIL="is active in the SRC inventory"
    fi
  fi

  if [ -n "$RT_INETD_DETAIL" ]; then
    RT_INETD_STATUS=WARN
    RT_INETD_SEV=med
    ...
    RT_INETD_SEV=low
    if [ "$RT_INETD_SRC" = duplicate ]; then
      RT_INETD_OBS="not assessed - inetd has duplicate SRC inventory rows"
    elif [ "$RT_INETD_BOOT" = unknown ]; then
      RT_INETD_OBS="not assessed - inetd rc.tcpip evidence $RCTCPIP_REASON"
    else
    ...
    RT_INETD_SEV=low
    RT_INETD_OBS="inetd not started in /etc/rc.tcpip and not active in the SRC inventory"
    RT_INETD_MEAN="The inetd service is not in use."
    RT_INETD_FIX="n/a"
  fi

  add security rctcpip_inetd "inetd service" \
    "$RT_INETD_STATUS" "$RT_INETD_SEV" \
    "$RT_INETD_OBS" \
    "$RT_INETD_MEAN" \
    "$RT_INETD_FIX" \
    "cis-l1"
