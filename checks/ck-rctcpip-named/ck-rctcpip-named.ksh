...
_AIXRAY_SESSION_KEYS=""
  # rctcpip_named — CIS IBM AIX 7 v1.2.0 4.3.2.8: named must not be in use.
  ...
  typeset RT_NAMED_BOOT RT_NAMED_SRC RT_NAMED_DETAIL RT_NAMED_BOOT_RC RT_NAMED_SRC_RC
  typeset RT_NAMED_STATUS RT_NAMED_SEV RT_NAMED_OBS RT_NAMED_MEAN RT_NAMED_FIX
  if [ "${RCTCPIP_READY:-0}" -eq 1 ]; then
    RT_NAMED_BOOT=$(printf '%s\n' "$RCTCPIP_ROWS" | awk -F'|' '
      $1 == "named" { found = 1 }
      END { if (found) print "boot"; else print "no-boot" }')
    RT_NAMED_BOOT_RC=$?
    [ "$RT_NAMED_BOOT_RC" -ne 0 ] && RT_NAMED_BOOT=unknown
  else
    RT_NAMED_BOOT=unknown
  fi
  if [ "${CIS_SERVICES_READY:-0}" -eq 1 ]; then
    RT_NAMED_SRC=$(printf '%s\n' "$CIS_SERVICES_ROWS" | awk -F'|' '
      $1 == "named" { n++; if ($2 == "in_use") active = 1 }
      END {
    ...
    RT_NAMED_SRC_RC=$?
    [ "$RT_NAMED_SRC_RC" -ne 0 ] && RT_NAMED_SRC=unknown
  else
    RT_NAMED_SRC=unknown
  fi

  RT_NAMED_DETAIL=""
  [ "$RT_NAMED_BOOT" = boot ] && RT_NAMED_DETAIL="starts at boot in /etc/rc.tcpip"
  if [ "$RT_NAMED_SRC" = in_use ]; then
    if [ -n "$RT_NAMED_DETAIL" ]; then
      RT_NAMED_DETAIL="$RT_NAMED_DETAIL and is active in the SRC inventory"
    else
      RT_NAMED_DETAIL="is active in the SRC inventory"
    fi
  fi

  if [ -n "$RT_NAMED_DETAIL" ]; then
    RT_NAMED_STATUS=FAIL
    RT_NAMED_SEV=high
    ...
    RT_NAMED_SEV=low
    if [ "$RT_NAMED_SRC" = duplicate ]; then
      RT_NAMED_OBS="not assessed - named has duplicate SRC inventory rows"
    elif [ "$RT_NAMED_BOOT" = unknown ]; then
      RT_NAMED_OBS="not assessed - named rc.tcpip evidence $RCTCPIP_REASON"
    else
    ...
    RT_NAMED_SEV=low
    RT_NAMED_OBS="named not started in /etc/rc.tcpip and not active in the SRC inventory"
    RT_NAMED_MEAN="The named service is not in use."
    RT_NAMED_FIX="n/a"
  fi

  add security rctcpip_named "named service" \
    "$RT_NAMED_STATUS" "$RT_NAMED_SEV" \
    "$RT_NAMED_OBS" \
    "$RT_NAMED_MEAN" \
    "$RT_NAMED_FIX" \
    "cis-l1"
