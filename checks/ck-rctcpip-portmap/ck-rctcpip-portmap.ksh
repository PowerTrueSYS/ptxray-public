...
AIXRAY_TOOL=ck-rctcpip-portmap
...
_AIXRAY_SESSION_KEYS=""
  # rctcpip_portmap — CIS IBM AIX 7 v1.2.0 4.3.2.9: portmap must not be in use.
  ...
  typeset RT_PORTMAP_BOOT RT_PORTMAP_SRC RT_PORTMAP_DETAIL RT_PORTMAP_BOOT_RC RT_PORTMAP_SRC_RC
  typeset RT_PORTMAP_STATUS RT_PORTMAP_SEV RT_PORTMAP_OBS RT_PORTMAP_MEAN RT_PORTMAP_FIX
  if [ "${RCTCPIP_READY:-0}" -eq 1 ]; then
    RT_PORTMAP_BOOT=$(printf '%s\n' "$RCTCPIP_ROWS" | awk -F'|' '
      $1 == "portmap" { found = 1 }
      END { if (found) print "boot"; else print "no-boot" }')
    RT_PORTMAP_BOOT_RC=$?
    [ "$RT_PORTMAP_BOOT_RC" -ne 0 ] && RT_PORTMAP_BOOT=unknown
  else
    RT_PORTMAP_BOOT=unknown
  fi
  if [ "${CIS_SERVICES_READY:-0}" -eq 1 ]; then
    RT_PORTMAP_SRC=$(printf '%s\n' "$CIS_SERVICES_ROWS" | awk -F'|' '
      $1 == "portmap" { n++; if ($2 == "in_use") active = 1 }
      END {
    ...
    RT_PORTMAP_SRC_RC=$?
    [ "$RT_PORTMAP_SRC_RC" -ne 0 ] && RT_PORTMAP_SRC=unknown
  else
    RT_PORTMAP_SRC=unknown
  fi

  RT_PORTMAP_DETAIL=""
  [ "$RT_PORTMAP_BOOT" = boot ] && RT_PORTMAP_DETAIL="starts at boot in /etc/rc.tcpip"
  if [ "$RT_PORTMAP_SRC" = in_use ]; then
    if [ -n "$RT_PORTMAP_DETAIL" ]; then
      RT_PORTMAP_DETAIL="$RT_PORTMAP_DETAIL and is active in the SRC inventory"
    else
      RT_PORTMAP_DETAIL="is active in the SRC inventory"
    fi
  fi

  if [ -n "$RT_PORTMAP_DETAIL" ]; then
    RT_PORTMAP_STATUS=WARN
    RT_PORTMAP_SEV=med
    ...
    RT_PORTMAP_SEV=low
    if [ "$RT_PORTMAP_SRC" = duplicate ]; then
      RT_PORTMAP_OBS="not assessed - portmap has duplicate SRC inventory rows"
    elif [ "$RT_PORTMAP_BOOT" = unknown ]; then
      RT_PORTMAP_OBS="not assessed - portmap rc.tcpip evidence $RCTCPIP_REASON"
    else
    ...
    RT_PORTMAP_SEV=low
    RT_PORTMAP_OBS="portmap not started in /etc/rc.tcpip and not active in the SRC inventory"
    RT_PORTMAP_MEAN="The portmap service is not in use."
    RT_PORTMAP_FIX="n/a"
  fi

  add security rctcpip_portmap "portmap service" \
    "$RT_PORTMAP_STATUS" "$RT_PORTMAP_SEV" \
    "$RT_PORTMAP_OBS" \
    "$RT_PORTMAP_MEAN" \
    "$RT_PORTMAP_FIX" \
    "cis-l1"
