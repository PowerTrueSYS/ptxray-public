...
_AIXRAY_SESSION_KEYS=""
  # rctcpip_routed — CIS IBM AIX 7 v1.2.0 4.3.2.10: routed must not be in use.
  ...
  typeset RT_ROUTED_BOOT RT_ROUTED_SRC RT_ROUTED_DETAIL RT_ROUTED_BOOT_RC RT_ROUTED_SRC_RC
  typeset RT_ROUTED_STATUS RT_ROUTED_SEV RT_ROUTED_OBS RT_ROUTED_MEAN RT_ROUTED_FIX
  if [ "${RCTCPIP_READY:-0}" -eq 1 ]; then
    RT_ROUTED_BOOT=$(printf '%s\n' "$RCTCPIP_ROWS" | awk -F'|' '
      $1 == "routed" { found = 1 }
      END { if (found) print "boot"; else print "no-boot" }')
    RT_ROUTED_BOOT_RC=$?
    [ "$RT_ROUTED_BOOT_RC" -ne 0 ] && RT_ROUTED_BOOT=unknown
  else
    RT_ROUTED_BOOT=unknown
  fi
  if [ "${CIS_SERVICES_READY:-0}" -eq 1 ]; then
    RT_ROUTED_SRC=$(printf '%s\n' "$CIS_SERVICES_ROWS" | awk -F'|' '
      $1 == "routed" { n++; if ($2 == "in_use") active = 1 }
      END {
    ...
    RT_ROUTED_SRC_RC=$?
    [ "$RT_ROUTED_SRC_RC" -ne 0 ] && RT_ROUTED_SRC=unknown
  else
    RT_ROUTED_SRC=unknown
  fi

  RT_ROUTED_DETAIL=""
  [ "$RT_ROUTED_BOOT" = boot ] && RT_ROUTED_DETAIL="starts at boot in /etc/rc.tcpip"
  if [ "$RT_ROUTED_SRC" = in_use ]; then
    if [ -n "$RT_ROUTED_DETAIL" ]; then
      RT_ROUTED_DETAIL="$RT_ROUTED_DETAIL and is active in the SRC inventory"
    else
      RT_ROUTED_DETAIL="is active in the SRC inventory"
    fi
  fi

  if [ -n "$RT_ROUTED_DETAIL" ]; then
    RT_ROUTED_STATUS=FAIL
    RT_ROUTED_SEV=high
    ...
    RT_ROUTED_SEV=low
    if [ "$RT_ROUTED_SRC" = duplicate ]; then
      RT_ROUTED_OBS="not assessed - routed has duplicate SRC inventory rows"
    elif [ "$RT_ROUTED_BOOT" = unknown ]; then
      RT_ROUTED_OBS="not assessed - routed rc.tcpip evidence $RCTCPIP_REASON"
    else
    ...
    RT_ROUTED_SEV=low
    RT_ROUTED_OBS="routed not started in /etc/rc.tcpip and not active in the SRC inventory"
    RT_ROUTED_MEAN="The routed service is not in use."
    RT_ROUTED_FIX="n/a"
  fi

  add security rctcpip_routed "routed service" \
    "$RT_ROUTED_STATUS" "$RT_ROUTED_SEV" \
    "$RT_ROUTED_OBS" \
    "$RT_ROUTED_MEAN" \
    "$RT_ROUTED_FIX" \
    "cis-l1"
