...
AIXRAY_TOOL=ck-rctcpip-sendmail
...
_AIXRAY_SESSION_KEYS=""
  # rctcpip_sendmail — CIS IBM AIX 7 v1.2.0 4.3.2.12: sendmail must not be in use. # network-lint: allow -- prose header comment, no network call
  ...
  typeset RT_SENDMAIL_BOOT RT_SENDMAIL_SRC RT_SENDMAIL_DETAIL RT_SENDMAIL_BOOT_RC RT_SENDMAIL_SRC_RC
  typeset RT_SENDMAIL_STATUS RT_SENDMAIL_SEV RT_SENDMAIL_OBS RT_SENDMAIL_MEAN RT_SENDMAIL_FIX
  if [ "${RCTCPIP_READY:-0}" -eq 1 ]; then
    # network-lint: allow-next=2 -- the awk program below matches the SMTP mailer service name in captured rc.tcpip rows; it parses local text only, no network call
    RT_SENDMAIL_BOOT=$(printf '%s\n' "$RCTCPIP_ROWS" | awk -F'|' '
      $1 == "sendmail" { found = 1 }
      END { if (found) print "boot"; else print "no-boot" }')
    RT_SENDMAIL_BOOT_RC=$?
    [ "$RT_SENDMAIL_BOOT_RC" -ne 0 ] && RT_SENDMAIL_BOOT=unknown
  else
    RT_SENDMAIL_BOOT=unknown
  fi
  if [ "${CIS_SERVICES_READY:-0}" -eq 1 ]; then
    # network-lint: allow-next=2 -- the awk program below matches the SMTP mailer service name in captured SRC inventory rows; it parses local text only, no network call
    RT_SENDMAIL_SRC=$(printf '%s\n' "$CIS_SERVICES_ROWS" | awk -F'|' '
      $1 == "sendmail" { n++; if ($2 == "in_use") active = 1 }
      END {
    ...
    RT_SENDMAIL_SRC_RC=$?
    [ "$RT_SENDMAIL_SRC_RC" -ne 0 ] && RT_SENDMAIL_SRC=unknown
  else
    RT_SENDMAIL_SRC=unknown
  fi

  RT_SENDMAIL_DETAIL=""
  [ "$RT_SENDMAIL_BOOT" = boot ] && RT_SENDMAIL_DETAIL="starts at boot in /etc/rc.tcpip"
  if [ "$RT_SENDMAIL_SRC" = in_use ]; then
    if [ -n "$RT_SENDMAIL_DETAIL" ]; then
      RT_SENDMAIL_DETAIL="$RT_SENDMAIL_DETAIL and is active in the SRC inventory"
    else
      RT_SENDMAIL_DETAIL="is active in the SRC inventory"
    fi
  fi

  if [ -n "$RT_SENDMAIL_DETAIL" ]; then
    RT_SENDMAIL_STATUS=FAIL
    ...
    RT_SENDMAIL_SEV=low
    if [ "$RT_SENDMAIL_SRC" = duplicate ]; then
      RT_SENDMAIL_OBS="not assessed - sendmail has duplicate SRC inventory rows" # network-lint: allow -- prose finding text, no network call
    elif [ "$RT_SENDMAIL_BOOT" = unknown ]; then
      RT_SENDMAIL_OBS="not assessed - sendmail rc.tcpip evidence $RCTCPIP_REASON" # network-lint: allow -- prose finding text, no network call
    else
    ...
    RT_SENDMAIL_SEV=low
    RT_SENDMAIL_OBS="sendmail not started in /etc/rc.tcpip and not active in the SRC inventory" # network-lint: allow -- prose finding text, no network call
    RT_SENDMAIL_MEAN="The sendmail service is not in use." # network-lint: allow -- prose finding text, no network call
    RT_SENDMAIL_FIX="n/a"
  fi

  # network-lint: allow-next=6 -- the finding label names the SMTP mailer service; the add command emits a finding, no network call
  add security rctcpip_sendmail "sendmail service" \
    "$RT_SENDMAIL_STATUS" "$RT_SENDMAIL_SEV" \
    "$RT_SENDMAIL_OBS" \
    "$RT_SENDMAIL_MEAN" \
    "$RT_SENDMAIL_FIX" \
    "cis-l1"
