...
_AIXRAY_SESSION_KEYS=""
  # rctcpip_timed — CIS IBM AIX 7 v1.2.0 4.3.2.13: timed must not be in use.
  ...
  typeset RT_TIMED_BOOT RT_TIMED_SRC RT_TIMED_DETAIL RT_TIMED_BOOT_RC RT_TIMED_SRC_RC
  typeset RT_TIMED_STATUS RT_TIMED_SEV RT_TIMED_OBS RT_TIMED_MEAN RT_TIMED_FIX
  if [ "${RCTCPIP_READY:-0}" -eq 1 ]; then
    RT_TIMED_BOOT=$(printf '%s\n' "$RCTCPIP_ROWS" | awk -F'|' '
      $1 == "timed" { found = 1 }
      END { if (found) print "boot"; else print "no-boot" }')
    RT_TIMED_BOOT_RC=$?
    [ "$RT_TIMED_BOOT_RC" -ne 0 ] && RT_TIMED_BOOT=unknown
  else
    RT_TIMED_BOOT=unknown
  fi
  if [ "${CIS_SERVICES_READY:-0}" -eq 1 ]; then
    RT_TIMED_SRC=$(printf '%s\n' "$CIS_SERVICES_ROWS" | awk -F'|' '
      $1 == "timed" { n++; if ($2 == "in_use") active = 1 }
      END {
    ...
    RT_TIMED_SRC_RC=$?
    [ "$RT_TIMED_SRC_RC" -ne 0 ] && RT_TIMED_SRC=unknown
  else
    RT_TIMED_SRC=unknown
  fi

  RT_TIMED_DETAIL=""
  [ "$RT_TIMED_BOOT" = boot ] && RT_TIMED_DETAIL="starts at boot in /etc/rc.tcpip"
  if [ "$RT_TIMED_SRC" = in_use ]; then
    if [ -n "$RT_TIMED_DETAIL" ]; then
      RT_TIMED_DETAIL="$RT_TIMED_DETAIL and is active in the SRC inventory"
    else
      RT_TIMED_DETAIL="is active in the SRC inventory"
    fi
  fi

  if [ -n "$RT_TIMED_DETAIL" ]; then
    RT_TIMED_STATUS=FAIL
    RT_TIMED_SEV=high
    ...
    RT_TIMED_SEV=low
    if [ "$RT_TIMED_SRC" = duplicate ]; then
      RT_TIMED_OBS="not assessed - timed has duplicate SRC inventory rows"
    elif [ "$RT_TIMED_BOOT" = unknown ]; then
      RT_TIMED_OBS="not assessed - timed rc.tcpip evidence $RCTCPIP_REASON"
    else
    ...
    RT_TIMED_SEV=low
    RT_TIMED_OBS="timed not started in /etc/rc.tcpip and not active in the SRC inventory"
    RT_TIMED_MEAN="The timed service is not in use."
    RT_TIMED_FIX="n/a"
  fi

  add security rctcpip_timed "timed service" \
    "$RT_TIMED_STATUS" "$RT_TIMED_SEV" \
    "$RT_TIMED_OBS" \
    "$RT_TIMED_MEAN" \
    "$RT_TIMED_FIX" \
    "cis-l1"
