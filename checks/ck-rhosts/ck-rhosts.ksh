...
AIXRAY_TOOL=ck-rhosts


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # rhosts — DISA STIG V-215432 (AIX7-00-003138). There must be no .rhosts,
  # .shosts, hosts.equiv or shosts.equiv file anywhere on the system.
  #
  # Four filesystem-wide read-only scans, one probe key each. A clean scan
  # legitimately prints nothing, so every probe command ends in a scan-complete
  # sentinel: sentinel present and nothing else means determinate absence, while
  # rc 0 with no output at all is a swallowed error and refuses. A determinate
  # hit on any scan outranks a refusal on another.
  typeset RH_RHOSTS RH_RHOSTS_RC RH_SHOSTS RH_SHOSTS_RC
  typeset RH_HEQUIV RH_HEQUIV_RC RH_SEQUIV RH_SEQUIV_RC
  typeset RH_STREAM RH_CLASS RH_HITS RH_REFUSAL RH_LIST

  RH_RHOSTS=$(aix find_rhosts sh -c 'find / -name .rhosts -print 2>/dev/null; echo __ptxray_find_end__')
  RH_RHOSTS_RC=$?
  RH_SHOSTS=$(aix find_shosts sh -c 'find / -name .shosts -print 2>/dev/null; echo __ptxray_find_end__')
  RH_SHOSTS_RC=$?
  RH_HEQUIV=$(aix find_hosts_equiv sh -c 'find / -name hosts.equiv -print 2>/dev/null; echo __ptxray_find_end__')
  RH_HEQUIV_RC=$?
  RH_SEQUIV=$(aix find_shosts_equiv sh -c 'find / -name shosts.equiv -print 2>/dev/null; echo __ptxray_find_end__')
  RH_SEQUIV_RC=$?

  # One keyed stream so a single awk pass classifies all four scans identically.
  RH_STREAM=$(
    printf 'RC .rhosts %s\n' "$RH_RHOSTS_RC"
    printf '%s\n' "$RH_RHOSTS" | sed 's/^/L .rhosts /'
    printf 'RC .shosts %s\n' "$RH_SHOSTS_RC"
    printf '%s\n' "$RH_SHOSTS" | sed 's/^/L .shosts /'
    printf 'RC hosts.equiv %s\n' "$RH_HEQUIV_RC"
    printf '%s\n' "$RH_HEQUIV" | sed 's/^/L hosts.equiv /'
    printf 'RC shosts.equiv %s\n' "$RH_SEQUIV_RC"
    printf '%s\n' "$RH_SEQUIV" | sed 's/^/L shosts.equiv /'
  )
  ...
  RH_CLASS=$(printf '%s\n' "$RH_STREAM" | awk '
    ...
    $1 == "L" {
      ...
      if (p == "") next
      nb[key]++
      if (p == "__ptxray_find_end__") { sent[key]++; next }
      if (p !~ /^\//) { bad[key]++; next }
      hn[key]++
      hit[key, hn[key]] = p
    }
    END {
      n = split(".rhosts .shosts hosts.equiv shosts.equiv", name, " ")
      for (i = 1; i <= n; i++) {
        k = name[i]
        if (!(k in rc)) {
          reason = "scan did not run"
        } else if (rc[k] + 0 != 0) {
          reason = "probe failed (rc=" rc[k] ")"
        } else if (nb[k] + 0 == 0) {
          reason = "returned rc 0 with no output (swallowed error)"
        } else if (sent[k] + 0 != 1) {
          reason = "output missing the scan-complete sentinel"
        } else if (bad[k] + 0 > 0) {
          reason = "returned unparseable output"
        } else {
          reason = ""
        }
        if (reason != "" && refusal == "") refusal = "find / -name " k " " reason
        for (j = 1; j <= hn[k] + 0; j++) print "HIT " hit[k, j]
      }
      if (refusal != "") print "REFUSE " refusal
    }
  ')

  RH_HITS=$(printf '%s\n' "$RH_CLASS" |
    awk '$1 == "HIT" { p = $0; sub(/^HIT /, "", p); print p }' | LC_ALL=C sort -u)
  RH_REFUSAL=$(printf '%s\n' "$RH_CLASS" |
    awk '$1 == "REFUSE" { p = $0; sub(/^REFUSE /, "", p); print p; exit }')
  # Bounded evidence: first eight offending paths, then a residual count.
  RH_LIST=$(printf '%s\n' "$RH_HITS" | awk '
    NF { n++; if (n <= 8) printf "%s%s", (n > 1 ? ", " : ""), $0 }
    END { if (n > 8) printf " (+%d more)", n - 8 }
  ')

  if [ -n "$RH_LIST" ]; then
    add security rhosts "Trust files (.rhosts)" FAIL med "present: $RH_LIST" \
        ...
        "stig:V-215432" # network-lint: allow -- remediation-advice prose, not a network call
  elif [ -n "$RH_REFUSAL" ]; then
    add security rhosts "Trust files (.rhosts)" NOT_ASSESSED med "not assessed - $RH_REFUSAL" \
        "One of the four trust-file scans returned no trustworthy evidence, so PTxray cannot assert that no .rhosts, .shosts, hosts.equiv or shosts.equiv file exists." \
        "run the four 'find / -name ...' scans manually, correct the scan problem, and rerun PTxray." \
        "stig:V-215432"
  else
    add security rhosts "Trust files (.rhosts)" PASS med \
        "no .rhosts, .shosts, hosts.equiv or shosts.equiv files found by find /" \
        "All four filesystem-wide scans completed and returned no prohibited host-trust file." \
        "n/a" "stig:V-215432"
  fi
}
