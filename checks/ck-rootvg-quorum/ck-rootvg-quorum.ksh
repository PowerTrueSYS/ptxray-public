...
_AIXRAY_SESSION_KEYS=""
  ...
  # A failed probe is a refusal (no finding, rc 3), never a clean verdict.
  if [ "$ROOTPRC" -ne 0 ] || [ "$ROOTLRC" -ne 0 ]; then
    :
  elif [ "$NPV" -ge 2 ] && [ "$MIRR" = "full" ]; then
    RVGO=$(aix lsvg_rootvg lsvg rootvg); RVGORC=$?
    if [ "$RVGORC" -ne 0 ]; then
      :
    else
    ...
    add resilience rootvg_quorum "rootvg quorum" NOT_APPLICABLE low "no mirrored rootvg (NPV=$NPV, MIRR=$MIRR)" \
        "rootvg quorum applies only to a rootvg mirrored across at least 2 disks with every LV fully mirrored (NPV>=2 and MIRR=full); this box does not meet that precondition, so there is no quorum to assess." "n/a"
  fi
}
