...
_AIXRAY_SESSION_KEYS=""
  ...
  for VG in $VGL; do
    [ "$VG" = "rootvg" ] && continue
    VGD=$(aix "find_vgdata_${VG}_31d" find /tmp/vgdata -name '*.data' -mtime -31 -print)
    VGD_RC=$?
    VG_DATA=0
    case "$VGD" in
      *"/tmp/vgdata/$VG/"*) VG_DATA=1 ;;
    esac
    if [ "$VGD_RC" -ne 0 ]; then
      add resilience "savevg_$VG" "savevg evidence ($VG)" NOT_ASSESSED low "not assessed — 'find /tmp/vgdata' rc=$VGD_RC" \
          "The savevg-evidence probe failed, so a recent vgdata control file for $VG can be neither confirmed nor ruled out." \
          "verify the vgdata probe works, then re-run the check." "ffiec:II.C.21"
    elif [ "$VG_DATA" -eq 1 ]; then
      add resilience "savevg_$VG" "savevg evidence ($VG)" PASS low "recent vgdata control file under /tmp/vgdata/$VG" \
          "A recent vgdata control file for $VG — mkvgdata or savevg -r has written one, recording $VG's file list (not a backup image)." "n/a" "ffiec:II.C.21"
    else
  ...
  # ruled addition (2026-08-15): no online non-rootvg VG is determinate not-applicable
  found_data_vg=""
  for VG in $VGL; do
    [ "$VG" = "rootvg" ] || found_data_vg=1
  done
  if [ "$VGLS_RC" -ne 0 ]; then
    add resilience "savevg_datavg" "savevg evidence (datavg)" NOT_ASSESSED low "not assessed — 'lsvg -o' rc=$VGLS_RC" \
        "The online-volume-group capture failed, so the absence of a data volume group cannot be proven; savevg evidence is not assessed." \
        "verify 'lsvg -o' works, then re-run the check." "ffiec:II.C.21"
  elif [ -z "$found_data_vg" ]; then
    add resilience "savevg_datavg" "savevg evidence (datavg)" NOT_APPLICABLE low "no online non-rootvg volume group" \
        "savevg evidence applies only to online non-rootvg volume groups; this box has none, so there is no volume group whose application data needs a savevg restore point." "n/a" "ffiec:II.C.21"
  fi
}
