...
"
# === BEGIN currency-gate ===
# Refuse with a typed NOT_ASSESSED when the ibm-security-advisories
# source-registry row is not CURRENT. The SEC_APARS table above is the payload
# that row attests to; scoring off it without knowing the row is current would
# be a hollow PASS. The status and reason come from currency_source_status
# (standalone prelude), never the inline table.
CURRENCY_ROW=$(currency_source_status ibm-security-advisories)
CURRENCY_TAB=$(printf '\t')
CURRENCY_STATUS=${CURRENCY_ROW%%"$CURRENCY_TAB"*}
CURRENCY_REASON=${CURRENCY_ROW#*"$CURRENCY_TAB"}
if [ "$CURRENCY_STATUS" != "CURRENT" ]; then
  add patch sec_apars "Tracked security APARs" NOT_ASSESSED med \
    "not assessed - ibm-security-advisories source registry row is $CURRENCY_STATUS: $CURRENCY_REASON" \
    "The IBM security-advisories reference data is not current, so the bundled SEC_APARS table cannot be trusted for a verdict." \
    "verify the ibm-security-advisories source registry row is CURRENT, then re-run this check." \
    "ffiec:II.C.10"
  return 0
fi
# === END currency-gate ===
...
TLLEVEL=$(printf '%s' "$OSLEVEL" | awk -F- 'NF>=2{print $1"-"$2}')
  ...
  while IFS='|' read APAR PREF CVE TITLE; do
    ...
    # instfix rc 249 with stdout is ordinary "not installed". Empty stdout
    # with rc 0 or 249 is the same determinate absence; other empty-stdout
    # rcs (127, failed capture) stay a refusal.
    if [ -z "$IOUT" ] && [ "$IOUT_RC" -ne 0 ] && [ "$IOUT_RC" -ne 249 ]; then
