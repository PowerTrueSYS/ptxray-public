...
_AIXRAY_SESSION_KEYS=""
  # Normalize the monolith's already-captured inventory once. Standalone tools
  # receive the same prepared variables directly from cap-lslpp-qcl.
  typeset CIS_PACKAGE_PROFILE CIS_PACKAGE_PROFILE_RC
  if [ "${CIS_PACKAGE_PREPARED:-0}" -ne 1 ]; then
    CIS_PACKAGE_PREPARED=1
    CIS_PACKAGE_READY=0
    CIS_PACKAGE_CLEAN=0
    CIS_PACKAGE_ROWS=""
    ...
    case "${LSLPPQ_RC:-}" in
      0)
        if [ -z "${LSLPPQ:-}" ]; then
          ...
          ')
          CIS_PACKAGE_PROFILE_RC=$?
          if [ "$CIS_PACKAGE_PROFILE_RC" -ne 0 ]; then
            CIS_PACKAGE_REASON="capture normalization failed (rc=$CIS_PACKAGE_PROFILE_RC)"
          else
            CIS_PACKAGE_NONBLANK=$(printf '%s\n' "$CIS_PACKAGE_PROFILE" | awk -F: '{print $1+0}')
            CIS_PACKAGE_VALID=$(printf '%s\n' "$CIS_PACKAGE_PROFILE" | awk -F: '{print $2+0}')
            CIS_PACKAGE_BOS=$(printf '%s\n' "$CIS_PACKAGE_PROFILE" | awk -F: '{print $3+0}')
  ...
  typeset SNAPP_MATCH_COUNT SNAPP_MATCH_RC SNAPP_STATUS SNAPP_SEVERITY

  if [ "${LSLPPQ_RC:-1}" -ne 0 ] || [ "$CIS_PACKAGE_READY" -ne 1 ]; then
    ...
    if [ "$SNAPP_MATCH_RC" -ne 0 ]; then
      ...
      SNAPP_STATUS=PASS
      SNAPP_SEVERITY=low
      SNAPP_OBSERVED="bos.net.snapp not installed"
      SNAPP_MEANING="The snapp fileset is absent from the installed-software inventory."
      SNAPP_FIX="n/a"
    fi
  fi

  add security snapp_installed "snapp package presence" \
    "$SNAPP_STATUS" "$SNAPP_SEVERITY" "$SNAPP_OBSERVED" \
    "$SNAPP_MEANING" "$SNAPP_FIX" "cis-l1"
}
