...
AIXRAY_TOOL=ck-snmp-community


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # snmp_community — DISA STIG V-215231.
  # When the SNMP service is configured, no uncommented line of /etc/snmpd.conf
  # or /etc/snmpdv3.conf may carry a default password token. STIG check-content
  # drops comment lines then greps for "public|private|password"; the previous
  # community-only regex missed a smux peer password line (the V-215231 gap).
  # Absence of the SNMP configuration is the compliant state (svcoff
  # convention); a probe that cannot establish presence or read a present
  # config refuses as NOT_ASSESSED, never a silent PASS. Fix text is a
  # recommendation only — never executed.
  typeset SNMPD_EXISTS_RC SNMPDV3_EXISTS_RC SNMP_EXISTS_RC
  typeset SNMPD_PRESENT SNMPDV3_PRESENT
  typeset SNMPD_TOKENS SNMPD_RC SNMPD_ACTIVE
  typeset SNMPDV3_TOKENS SNMPDV3_RC SNMPDV3_ACTIVE
  typeset SNMP_FOUND SNMP_REFUSED

  # Probe each config independently. test(1) returns 1 when its -e expression
  # is false (determinate absence, the svcoff state) and >= 2 only on error,
  # so an error rc must refuse, never fall through to the absence PASS.
  aix snmpd_conf_exists test -e /etc/snmpd.conf
  SNMPD_EXISTS_RC=$?
  aix snmpdv3_conf_exists test -e /etc/snmpdv3.conf
  SNMPDV3_EXISTS_RC=$?
  SNMP_EXISTS_RC=$SNMPD_EXISTS_RC
  [ "$SNMPD_EXISTS_RC" -ge 2 ] || SNMP_EXISTS_RC=$SNMPDV3_EXISTS_RC

  if [ "$SNMPD_EXISTS_RC" -ge 2 ] || [ "$SNMPDV3_EXISTS_RC" -ge 2 ]; then
    add security snmp_community "SNMP community strings" NOT_ASSESSED med \
      "not assessed - SNMP existence probe failed (rc=$SNMP_EXISTS_RC)" \
      "PTxray could not tell whether the SNMP configuration exists, so default-password presence cannot be established." \
      "inspect 'test -e /etc/snmpd.conf' and 'test -e /etc/snmpdv3.conf' as root and rerun PTxray." \
      "stig:V-215231 cis-l1"
  elif [ "$SNMPD_EXISTS_RC" -eq 1 ] && [ "$SNMPDV3_EXISTS_RC" -eq 1 ]; then
    add security snmp_community "SNMP community strings" PASS med \
      "no SNMP configuration present" \
      "Neither /etc/snmpd.conf nor /etc/snmpdv3.conf exists, so no default SNMP password can be in use." \
      "n/a" "stig:V-215231 cis-l1"
  else
    # At least one config is present. Grep each present file on its own so a
    # missing sibling never turns a clean file into a probe error.
    SNMPD_PRESENT=0
    SNMPDV3_PRESENT=0
    [ "$SNMPD_EXISTS_RC" -eq 0 ] && SNMPD_PRESENT=1
    [ "$SNMPDV3_EXISTS_RC" -eq 0 ] && SNMPDV3_PRESENT=1
    SNMP_FOUND=""
    SNMP_REFUSED=""

    if [ "$SNMPD_PRESENT" -eq 1 ]; then
      SNMPD_TOKENS=$(aix snmpd_conf grep -h -E 'public|private|password' /etc/snmpd.conf)
      SNMPD_RC=$?
      # Re-implementation of `grep -v "^#" | grep -E "public|private|password"`:
      # keep only uncommented token-bearing lines.
      SNMPD_ACTIVE=$(printf '%s\n' "$SNMPD_TOKENS" | awk '
        substr($0, 1, 1) != "#" && $0 ~ /public|private|password/ { print }')
      if [ -n "$SNMPD_ACTIVE" ]; then
        SNMP_FOUND=1
      elif [ "$SNMPD_RC" -ge 2 ]; then
        SNMP_REFUSED=1
      elif [ "$SNMPD_RC" -eq 0 ] && [ -z "$SNMPD_TOKENS" ]; then
        SNMP_REFUSED=1
      fi
    fi

    if [ "$SNMPDV3_PRESENT" -eq 1 ]; then
      SNMPDV3_TOKENS=$(aix snmpdv3_conf grep -h -E 'public|private|password' /etc/snmpdv3.conf)
      SNMPDV3_RC=$?
      SNMPDV3_ACTIVE=$(printf '%s\n' "$SNMPDV3_TOKENS" | awk '
        substr($0, 1, 1) != "#" && $0 ~ /public|private|password/ { print }')
      if [ -n "$SNMPDV3_ACTIVE" ]; then
        SNMP_FOUND=1
      elif [ "$SNMPDV3_RC" -ge 2 ]; then
        SNMP_REFUSED=1
      elif [ "$SNMPDV3_RC" -eq 0 ] && [ -z "$SNMPDV3_TOKENS" ]; then
        SNMP_REFUSED=1
      fi
    fi

    if [ -n "$SNMP_FOUND" ]; then
      add security snmp_community "SNMP community strings" FAIL med \
        "default SNMP password in use" \
        ...
        "replace the default token with a strong community or password in /etc/snmpd.conf or /etc/snmpdv3.conf, then refresh snmpd." \
        "stig:V-215231 cis-l1"
    elif [ -n "$SNMP_REFUSED" ]; then
      if [ "${MYUID:-0}" != "0" ]; then
        nr_warn security snmp_community "SNMP community strings" "the SNMP configuration" \
          "grep -h -E 'public|private|password' /etc/snmpd.conf /etc/snmpdv3.conf" \
          "stig:V-215231 cis-l1"
      else
        add security snmp_community "SNMP community strings" NOT_ASSESSED med \
          "not assessed - SNMP configuration probe failed" \
          "A present SNMP configuration could not be read, so default-password presence cannot be established." \
          "inspect the SNMP configuration as root and rerun PTxray." \
          "stig:V-215231 cis-l1"
      fi
    else
      add security snmp_community "SNMP community strings" PASS med \
        "no uncommented default SNMP password" \
        "Only commented token lines matched; no active line of the SNMP configuration uses a default password." \
        "n/a" "stig:V-215231 cis-l1"
    fi
  fi
}
