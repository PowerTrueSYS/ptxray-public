...
AIXRAY_TOOL=ck-ssh-ciphers


function standalone_check {
_AIXRAY_SESSION_KEYS=""
# ssh_ciphers — DISA STIG V-215402 (medium): sshd must use only FIPS 140-2
# approved ciphers. Two read-only evidence sources, because `sshd -T` always
# prints an effective list even when sshd_config has no explicit Ciphers
# directive, so the directive's presence is undecidable from sshd -T alone:
#   1. sshd -T (root-gated on AIX) -> the effective cipher list.
#   2. grep -i ciphers /etc/ssh/sshd_config -> proof the operator set an # network-lint: allow -- comment describing a LOCAL file read
#      explicit Ciphers line. grep rc=1 (no match) is the determinate absence
#      the check-content calls a finding; any other rc!=0 is a probe failure.
# A cipher is approved only when it starts with 'aes' and is not a
# cbc/arcfour/3des/blowfish member, so chacha20-poly1305@openssh.com and
# 3des-cbc both fail even though the legacy weak-substring pass did not.
typeset SC_SSHT SC_SSHT_RC SC_CONF SC_CONF_RC SC_STATUS SC_OBSERVED
typeset SC_MEANING SC_FIX SC_REASON SC_EFFECTIVE SC_BAD SC_LINE
SC_STATUS=NOT_ASSESSED
SC_OBSERVED=""
SC_MEANING="PTxray did not obtain one trustworthy effective SSH cipher list plus an explicit sshd_config Ciphers directive, so it cannot establish the FIPS 140-2 cipher requirement."
SC_FIX="re-run PTxray as root, or run 'sshd -T' and 'grep -i ciphers /etc/ssh/sshd_config' manually, then rerun PTxray" # network-lint: allow -- the not-assessed fix quotes a LOCAL /etc/ssh/sshd_config read; it is never executed
SC_REASON=""

SC_SSHT=$(aix sshd_T sshd -T) # network-lint: allow -- reads sshd's LOCAL effective configuration; sshd -T does not make a network call
SC_SSHT_RC=$?
if [ "$SC_SSHT_RC" -ne 0 ]; then
  SC_REASON="sshd -T failed (rc=$SC_SSHT_RC)"
elif [ -z "$SC_SSHT" ]; then
  SC_REASON="sshd -T returned empty output"
else
  SC_CONF=$(aix sshd_config_ciphers grep -i ciphers /etc/ssh/sshd_config) # network-lint: allow -- reads the LOCAL /etc/ssh/sshd_config text, never a network call
  SC_CONF_RC=$?
  if [ "$SC_CONF_RC" -eq 2 ]; then
    SC_REASON="grep could not read sshd_config (rc=$SC_CONF_RC)"
  elif [ "$SC_CONF_RC" -eq 1 ]; then
    SC_STATUS=FAIL
    SC_OBSERVED="no explicit Ciphers line in sshd_config"
    SC_MEANING="sshd_config has no uncommented Ciphers directive, so the effective cipher list is a silent default that DISA STIG V-215402 does not accept."
    SC_FIX="add an explicit Ciphers line to sshd_config listing only aes*-ctr ciphers, then run 'stopsrc -s sshd; startsrc -s sshd'; PTxray only recommends these actions."
  elif [ -z "$SC_CONF" ]; then
    SC_REASON="grep of sshd_config returned empty output"
  else
    SC_LINE=$(printf '%s\n' "$SC_CONF" | awk '$0 !~ /^#/ {print; exit}')
    if [ -z "$SC_LINE" ]; then
      SC_STATUS=FAIL
      SC_OBSERVED="no explicit Ciphers line in sshd_config"
      SC_MEANING="sshd_config has no uncommented Ciphers directive, so the effective cipher list is a silent default that DISA STIG V-215402 does not accept."
      ...
      SC_EFFECTIVE=$(printf '%s\n' "$SC_SSHT" | awk '
        ...
        END {
          if (count != 1) print "__COUNT__"
          else if (invalid) print "__INVALID__"
          else print value
        }')
      case "$SC_EFFECTIVE" in
        __COUNT__) SC_REASON='effective ciphers row is missing or duplicate' ;;
        __INVALID__|''|+*|-*|^*|,*|*,|*,,*) SC_REASON='effective ciphers value is unparseable' ;;
        *)
          SC_BAD=$(printf '%s\n' "$SC_EFFECTIVE" | awk -F, '
            {
              for (i=1; i<=NF; i++) {
                if ($i !~ /^aes/ || $i ~ /cbc|arcfour|3des|blowfish/) {
                  print $i
                  exit
                }
              }
          ...
          if [ -n "$SC_BAD" ]; then
            SC_STATUS=FAIL
            SC_OBSERVED="ciphers $SC_EFFECTIVE; non-FIPS member $SC_BAD"
            SC_MEANING="sshd's effective cipher list includes a non-FIPS 140-2 cipher, so traffic that looks protected can be downgraded to a weak or non-approved algorithm."
            SC_FIX="remove cbc/arcfour/3des/blowfish and any non-aes cipher from the Ciphers line in sshd_config, then run 'stopsrc -s sshd; startsrc -s sshd'; PTxray only recommends these actions."
          else
            SC_STATUS=PASS
            SC_OBSERVED="ciphers $SC_EFFECTIVE"
            SC_MEANING="sshd's effective cipher list is limited to FIPS 140-2 approved aes*-ctr ciphers."
            SC_FIX="n/a"
          fi
          ;;
      esac
...
if [ "$SC_STATUS" = NOT_ASSESSED ]; then
  SC_OBSERVED="not assessed - $SC_REASON"
fi
add security ssh_ciphers "SSH ciphers" "$SC_STATUS" med \
    "$SC_OBSERVED" "$SC_MEANING" "$SC_FIX" "stig:V-215402 cis-l1"
}
