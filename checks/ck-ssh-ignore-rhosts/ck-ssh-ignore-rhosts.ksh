...
AIXRAY_TOOL=ck-ssh-ignore-rhosts


function standalone_check {
_AIXRAY_SESSION_KEYS=""

  # ssh_ignore_rhosts — assess the global host-based trust pair from `sshd -T`.
  typeset RH_RAW RH_CAPTURE_RC RH_PARSED IGN_RHOSTS HOST_BASED
  typeset RH_STATUS RH_SEVERITY RH_OBSERVED RH_MEANING RH_FIX RH_REASON
  ...
  if [ "${MYUID:-0}" != "0" ]; then
    RH_REASON='requires root'
  else
    if [ "${SSHT_RC+x}" = x ]; then
      RH_RAW=$SSHT
      RH_CAPTURE_RC=$SSHT_RC
    else
      RH_RAW=$(aix sshd_T sshd -T) # network-lint: allow -- reads sshd's LOCAL effective configuration; sshd -T does not make a network call
      RH_CAPTURE_RC=$?
    fi
    if [ "$RH_CAPTURE_RC" -ne 0 ]; then
      RH_REASON="sshd -T failed (rc=$RH_CAPTURE_RC)"
    elif [ -z "$RH_RAW" ]; then
      RH_REASON='sshd -T returned empty output'
    else
      RH_PARSED=$(printf '%s\n' "$RH_RAW" | awk '
        $1 == "ignorerhosts" {
          ignore_count++
          if (NF != 2 || ($2 != "yes" && $2 != "no")) invalid=1
          else ignore=$2
        }
        $1 == "hostbasedauthentication" {
          hostbased_count++
          if (NF != 2 || ($2 != "yes" && $2 != "no")) invalid=1
          else hostbased=$2
        }
        END {
          if (ignore_count != 1 || hostbased_count != 1) print "__COUNT__"
          else if (invalid) print "__INVALID__"
          else print ignore " " hostbased
        }')
      case "$RH_PARSED" in
        __COUNT__)
          RH_REASON='effective rows are missing or duplicate'
          ;;
        __INVALID__)
          RH_REASON='effective value is unparseable'
          ;;
        *)
          IGN_RHOSTS=${RH_PARSED%% *}
          HOST_BASED=${RH_PARSED##* }
          if [ "$IGN_RHOSTS" = yes ] && [ "$HOST_BASED" = no ]; then
            RH_STATUS=PASS
            RH_SEVERITY=med
            RH_OBSERVED='IgnoreRhosts yes, HostbasedAuthentication no'
            RH_MEANING='sshd host-based trust is disabled — IgnoreRhosts is yes and HostbasedAuthentication is no in the global effective configuration.'
            RH_FIX='n/a'
          else
            RH_STATUS=FAIL
            RH_SEVERITY=high
  ...
  if [ "$RH_STATUS" = NOT_ASSESSED ]; then
    RH_OBSERVED="not assessed — SSH host-based trust $RH_REASON"
  fi
  add security ssh_ignore_rhosts "SSH ignoreRhosts / hostbasedAuthentication" \
      "$RH_STATUS" "$RH_SEVERITY" \
      "$RH_OBSERVED" "$RH_MEANING" "$RH_FIX" "cis-l1"
}
