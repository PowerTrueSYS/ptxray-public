...
AIXRAY_TOOL=ck-ssh-loglevel


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # ssh_loglevel — assess the global value normalized by `sshd -T`.
  typeset SSH_LL_RAW SSH_LL_CAPTURE_RC SSH_LL_PARSED SSH_LL_VALUE
  ...
  if [ "${MYUID:-0}" != "0" ]; then
    SSH_LL_REASON='requires root'
  else
    if [ "${SSHT_RC+x}" = x ]; then
      SSH_LL_RAW=$SSHT
      SSH_LL_CAPTURE_RC=$SSHT_RC
    else
      SSH_LL_RAW=$(aix sshd_T sshd -T) # network-lint: allow -- reads sshd's LOCAL effective configuration; sshd -T does not make a network call
      SSH_LL_CAPTURE_RC=$?
    fi
    if [ "$SSH_LL_CAPTURE_RC" -ne 0 ]; then
      SSH_LL_REASON="sshd -T failed (rc=$SSH_LL_CAPTURE_RC)"
    elif [ -z "$SSH_LL_RAW" ]; then
      SSH_LL_REASON='sshd -T returned empty output'
    else
      SSH_LL_PARSED=$(printf '%s\n' "$SSH_LL_RAW" | awk '
        $1 == "loglevel" {
          count++
          if (NF != 2) invalid=1
          else value=$2
      ...
      case "$SSH_LL_PARSED" in
        INFO|VERBOSE)
          SSH_LL_VALUE=$SSH_LL_PARSED
          SSH_LL_STATUS=PASS
          SSH_LL_OBSERVED="LogLevel $SSH_LL_VALUE"
          SSH_LL_MEANING='sshd logging is set to INFO or VERBOSE, preserving useful security events without debug-level exposure.'
          SSH_LL_FIX='n/a'
          ;;
        QUIET|FATAL|ERROR|DEBUG|DEBUG1|DEBUG2|DEBUG3)
          SSH_LL_VALUE=$SSH_LL_PARSED
          SSH_LL_STATUS=FAIL
          SSH_LL_OBSERVED="LogLevel $SSH_LL_VALUE"
        ...
        __COUNT__)
          ...
          ;;
        *)
          SSH_LL_REASON='effective value is unparseable'
          ;;
      esac
    fi
  fi

  if [ "$SSH_LL_STATUS" = NOT_ASSESSED ]; then
    SSH_LL_OBSERVED="not assessed — LogLevel $SSH_LL_REASON"
  fi
  add security ssh_loglevel "SSH logging level" "$SSH_LL_STATUS" med \
      "$SSH_LL_OBSERVED" "$SSH_LL_MEANING" "$SSH_LL_FIX" "cis-l1"
}
