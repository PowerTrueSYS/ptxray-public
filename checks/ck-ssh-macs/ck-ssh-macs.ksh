...
AIXRAY_TOOL=ck-ssh-macs


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # ssh_macs — assess the global algorithm list normalized by `sshd -T`.
  typeset SSH_MACS_RAW SSH_MACS_CAPTURE_RC SSH_MACS_PARSED SSH_MACS_VALUE
  typeset SSH_MACS_BAD SSH_MACS_STATUS SSH_MACS_OBSERVED
  ...
  if [ "${MYUID:-0}" != "0" ]; then
    SSH_MACS_REASON='requires root'
  else
    if [ "${SSHT_RC+x}" = x ]; then
      SSH_MACS_RAW=$SSHT
      SSH_MACS_CAPTURE_RC=$SSHT_RC
    else
      SSH_MACS_RAW=$(aix sshd_T sshd -T) # network-lint: allow -- reads sshd's LOCAL effective configuration; sshd -T does not make a network call
      SSH_MACS_CAPTURE_RC=$?
    fi
    if [ "$SSH_MACS_CAPTURE_RC" -ne 0 ]; then
      SSH_MACS_REASON="sshd -T failed (rc=$SSH_MACS_CAPTURE_RC)"
    elif [ -z "$SSH_MACS_RAW" ]; then
      SSH_MACS_REASON='sshd -T returned empty output'
    else
      SSH_MACS_PARSED=$(printf '%s\n' "$SSH_MACS_RAW" | awk '
        $1 == "macs" {
          count++
          if (NF != 2) invalid=1
          else value=$2
      ...
      case "$SSH_MACS_PARSED" in
        __COUNT__)
          SSH_MACS_REASON='effective row is missing or duplicate'
          ;;
        __INVALID__|''|+*|-*|^*|,*|*,|*,,*)
          SSH_MACS_REASON='effective value is unparseable'
          ;;
        *)
          SSH_MACS_VALUE=$SSH_MACS_PARSED
          SSH_MACS_BAD=$(printf '%s\n' "$SSH_MACS_VALUE" | awk -F, '
            BEGIN {
              ok["umac-64-etm@openssh.com"]=1
              ok["umac-128-etm@openssh.com"]=1
              ok["hmac-sha2-256-etm@openssh.com"]=1
              ok["hmac-sha2-512-etm@openssh.com"]=1
              ok["hmac-sha1-etm@openssh.com"]=1
              ok["umac-64@openssh.com"]=1
              ok["umac-128@openssh.com"]=1
              ok["hmac-sha2-256"]=1
              ok["hmac-sha2-512"]=1
              ok["hmac-sha1"]=1
            }
          ...
          if [ -n "$SSH_MACS_BAD" ]; then
            ...
            SSH_MACS_MEANING='The global effective SSH MAC list is limited to the approved AIX CIS message-authentication algorithms.'
            SSH_MACS_FIX='n/a'
          fi
          ;;
      esac
  ...
  if [ "$SSH_MACS_STATUS" = NOT_ASSESSED ]; then
    SSH_MACS_OBSERVED="not assessed — MACs $SSH_MACS_REASON"
  fi
  add security ssh_macs "SSH MAC algorithms" "$SSH_MACS_STATUS" high \
      "$SSH_MACS_OBSERVED" "$SSH_MACS_MEANING" "$SSH_MACS_FIX" "cis-l1"
}
