...
AIXRAY_TOOL=ck-ssh-maxauthtries


function standalone_check {
_AIXRAY_SESSION_KEYS=""

  # ssh_maxauthtries — assess the global integer normalized by `sshd -T`.
  typeset MAT_RAW MAT_CAPTURE_RC MAT_PARSED MAT_VAL MAT_STATUS MAT_SEVERITY
  typeset MAT_OBSERVED MAT_MEANING MAT_FIX MAT_REASON
  ...
  if [ "${MYUID:-0}" != "0" ]; then
    MAT_REASON='requires root'
  else
    if [ "${SSHT_RC+x}" = x ]; then
      MAT_RAW=$SSHT
      MAT_CAPTURE_RC=$SSHT_RC
    else
      MAT_RAW=$(aix sshd_T sshd -T) # network-lint: allow -- reads sshd's LOCAL effective configuration; sshd -T does not make a network call
      MAT_CAPTURE_RC=$?
    fi
    if [ "$MAT_CAPTURE_RC" -ne 0 ]; then
      MAT_REASON="sshd -T failed (rc=$MAT_CAPTURE_RC)"
    elif [ -z "$MAT_RAW" ]; then
      MAT_REASON='sshd -T returned empty output'
    else
      MAT_PARSED=$(printf '%s\n' "$MAT_RAW" | awk '
        $1 == "maxauthtries" {
          count++
      ...
      case "$MAT_PARSED" in
        __COUNT__)
          MAT_REASON='effective row is missing or duplicate'
          ;;
        __INVALID__)
          MAT_REASON='effective value is unparseable'
          ;;
        *)
          MAT_VAL=$MAT_PARSED
          MAT_OBSERVED=$MAT_VAL
          if [ "${#MAT_VAL}" -eq 1 ] && [ "$MAT_VAL" -le 4 ]; then
            MAT_STATUS=PASS
            MAT_SEVERITY=low
            MAT_MEANING="MaxAuthTries is $MAT_VAL — within the required limit of 4 or fewer."
            MAT_FIX='n/a'
          else
            MAT_STATUS=FAIL
            MAT_SEVERITY=med
  ...
  if [ "$MAT_STATUS" = NOT_ASSESSED ]; then
    MAT_OBSERVED="not assessed — MaxAuthTries $MAT_REASON"
  fi
  add security ssh_maxauthtries "SSH MaxAuthTries" "$MAT_STATUS" \
      "$MAT_SEVERITY" "$MAT_OBSERVED" "$MAT_MEANING" "$MAT_FIX" "cis-l1"
}
