...
AIXRAY_TOOL=ck-ssh-rekeylimit


function standalone_check {
_AIXRAY_SESSION_KEYS=""

  # ssh_rekeylimit — assess the global effective rekeylimit normalized by `sshd -T`.
  typeset RKL_RAW RKL_CAPTURE_RC RKL_PARSED RKL_STATUS RKL_SEVERITY
  typeset RKL_OBSERVED RKL_MEANING RKL_FIX RKL_REASON
  ...
  if [ "${MYUID:-0}" != "0" ]; then
    RKL_REASON='requires root'
  else
    if [ "${SSHT_RC+x}" = x ]; then
      RKL_RAW=$SSHT
      RKL_CAPTURE_RC=$SSHT_RC
    else
      RKL_RAW=$(aix sshd_T sshd -T) # network-lint: allow -- reads sshd's LOCAL effective configuration; sshd -T does not make a network call
      RKL_CAPTURE_RC=$?
    fi
    if [ "$RKL_CAPTURE_RC" -ne 0 ]; then
      RKL_REASON="sshd -T failed (rc=$RKL_CAPTURE_RC)"
    elif [ -z "$RKL_RAW" ]; then
      RKL_REASON='sshd -T returned empty output'
    else
      RKL_PARSED=$(printf '%s\n' "$RKL_RAW" | awk '
        $1 == "rekeylimit" {
          count++
      ...
      case "$RKL_PARSED" in
        "1073741824 3600")
          RKL_STATUS=PASS
          RKL_OBSERVED='rekeylimit 1073741824 3600'
          RKL_MEANING='The effective rekeylimit forces a fresh session key after at most 1073741824 bytes of data or 3600 seconds.'
          RKL_FIX='n/a'
          ;;
        __ABSENT__)
        ...
        __DUPLICATE__*)
          ...
          ;;
        __INVALID__)
          RKL_REASON='effective value does not parse as two non-negative integers, so nothing was measured'
          ;;
        *)
          RKL_STATUS=FAIL
  ...
  if [ "$RKL_STATUS" = NOT_ASSESSED ]; then
    RKL_OBSERVED="not assessed — rekeylimit $RKL_REASON"
  fi
