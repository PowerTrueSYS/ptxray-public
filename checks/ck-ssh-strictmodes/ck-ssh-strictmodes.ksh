...
AIXRAY_TOOL=ck-ssh-strictmodes


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # ssh_strictmodes — assess the global value normalized by `sshd -T`.
  typeset SSH_SM_RAW SSH_SM_CAPTURE_RC SSH_SM_PARSED SSH_SM_VALUE
  ...
  if [ "${MYUID:-0}" != "0" ]; then
    SSH_SM_REASON='requires root'
  else
    if [ "${SSHT_RC+x}" = x ]; then
      SSH_SM_RAW=$SSHT
      SSH_SM_CAPTURE_RC=$SSHT_RC
    else
      SSH_SM_RAW=$(aix sshd_T sshd -T) # network-lint: allow -- reads sshd's LOCAL effective configuration; sshd -T does not make a network call
      SSH_SM_CAPTURE_RC=$?
    fi
    if [ "$SSH_SM_CAPTURE_RC" -ne 0 ]; then
      SSH_SM_REASON="sshd -T failed (rc=$SSH_SM_CAPTURE_RC)"
    elif [ -z "$SSH_SM_RAW" ]; then
      SSH_SM_REASON='sshd -T returned empty output'
    else
      SSH_SM_PARSED=$(printf '%s\n' "$SSH_SM_RAW" | awk '
        $1 == "strictmodes" {
          count++
          if (NF != 2) invalid=1
          else value=$2
      ...
      case "$SSH_SM_PARSED" in
        yes)
          SSH_SM_VALUE=yes
          SSH_SM_STATUS=PASS
          SSH_SM_OBSERVED='StrictModes yes'
          SSH_SM_MEANING='sshd checks ownership and modes of user files and home directories before accepting login.'
          SSH_SM_FIX='n/a'
          ;;
        no)
          SSH_SM_VALUE=no
        ...
        __COUNT__)
          ...
          ;;
        *)
          SSH_SM_REASON='effective value is unparseable'
          ;;
      esac
    fi
  fi

  if [ "$SSH_SM_STATUS" = NOT_ASSESSED ]; then
    SSH_SM_OBSERVED="not assessed — StrictModes $SSH_SM_REASON"
  fi
  add security ssh_strictmodes "SSH strict mode checks" "$SSH_SM_STATUS" high \
      "$SSH_SM_OBSERVED" "$SSH_SM_MEANING" "$SSH_SM_FIX" ""
}
