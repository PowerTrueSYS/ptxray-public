...
_AIXRAY_SESSION_KEYS=""
  ...
  if [ "$LSSLRC" -ne 0 ] || [ -z "$LSSL" ]; then
    ...
    # Grade what is present: the verdict is driven by the openssl.base level,
    # so a box with openssl but no openssh.base.server still grades (openssh
    # renders as n/a). Refuse only when the graded level itself is unparseable.
    add patch ssl_ssh "OpenSSH / OpenSSL levels" NOT_ASSESSED med \
