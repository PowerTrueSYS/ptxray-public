...
AIXRAY_TOOL=ck-stig-able-control-ability-remote


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_able_control_ability_remote — DISA STIG V-215233 (AIX7-00-001137).
  # MANUAL control.  AIX records rlogin= per user, but nothing on the box says
  # which of those users is *authorized* to reach it from the network.  That
  # list comes from the site (ISSO/SA) through the onboarding interview named
  # by manifest "question".  This fragment therefore only collects the rlogin
  # evidence and refuses: every reachable path renders NOT_ASSESSED with a
  # typed reason, and the chsec remediation appears only as recommendation
  # text.  PTxray never writes to the target.
  typeset RLA_RAW RLA_RC RLA_EV RLA_ASK RLA_OBSERVED

  RLA_ASK="Ask ISSO/SA to obtain a list of users who are not authorized to remotely log in"

  RLA_RAW=$(aix rlogin_all lsuser -a rlogin ALL)
  RLA_RC=$?

  if [ "$RLA_RC" -ne 0 ]; then
    RLA_OBSERVED="not assessed - probe 'lsuser -a rlogin ALL' failed (rc=$RLA_RC)"
  else
    # One "user=<name> rlogin=<value>" token per parseable line, space joined.
    # Blank and whitespace-only lines drop out; a line carrying no rlogin=
    # attribute is not evidence and is dropped too.
    RLA_EV=$(printf '%s\n' "$RLA_RAW" | awk '
      NF == 0 { next }
      {
        who = $1
        val = ""
        for (i = 2; i <= NF; i++) {
          if (substr($i, 1, 7) == "rlogin=") val = substr($i, 8)
        }
        if (val == "") next
        if (out != "") out = out " "
        out = out "user=" who " rlogin=" val
      }
    ...
    RLA_OBSERVED="not assessed - '$RLA_ASK'; rlogin evidence: $RLA_EV"
  fi

  add security stig_able_control_ability_remote "Remote login control for users" \
    NOT_ASSESSED med "$RLA_OBSERVED" \
    "An account with rlogin=true can start an interactive session from the network, so any account that is meant to be console-only or non-interactive is reachable by whoever holds its password. AIX cannot tell an authorized remote user from an unauthorized one, so this control stays undecided until the site supplies its own list." \
    "Recommend: for each user who must not log in remotely, run chsec -f /etc/security/user -s <user_name> -a rlogin=false. PTxray does not apply this." \
    "stig:V-215233"
}
