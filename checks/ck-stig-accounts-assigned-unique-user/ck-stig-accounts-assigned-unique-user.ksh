...
AIXRAY_TOOL=ck-stig-accounts-assigned-unique-user


function standalone_check {
_AIXRAY_SESSION_KEYS=""
# accounts_assigned_unique_user — DISA STIG V-215176.
typeset AAU_USRCK AAU_PASSWD AAU_USRCK_RC AAU_PASSWD_RC
typeset AAU_SCAN AAU_SCAN_RC AAU_RECORDS AAU_DUPS AAU_BAD AAU_REASON
typeset AAU_USRCK_DUP AAU_USRCK_DUP_RC

AAU_USRCK=$(aixv usrck_all usrck -n ALL)
AAU_USRCK_RC=$?
AAU_PASSWD=$(aix passwd_file cat /etc/passwd)
AAU_PASSWD_RC=$?
# usrck evidence is only consulted when the probe actually ran (rc 127 is the
# wrapper's "command absent / no capture") and returned output.
AAU_USRCK_DUP=""
AAU_USRCK_DUP_RC=0
if [ "$AAU_USRCK_RC" -ne 127 ] && [ -n "$AAU_USRCK" ]; then
  AAU_USRCK_DUP=$(printf '%s\n' "$AAU_USRCK" | awk '
    { line=tolower($0); if (line ~ /duplicate/ || line ~ /id[[:space:]].*in[[:space:]]+use/) { print; found=1 } }
    END { if (found) exit 1 }
  ')
  AAU_USRCK_DUP_RC=$?
fi

if [ "$AAU_PASSWD_RC" -ne 0 ]; then
  if [ "$AAU_USRCK_DUP_RC" -eq 1 ]; then
    add security stig_accounts_assigned_unique_user \
        "All accounts have unique User Identification Numbers" FAIL high \
        "usrck reports duplicate UID evidence: $AAU_USRCK_DUP" \
        "usrck reports that account UID assignment is not unique, so individual accountability is lost." \
        "Edit user accounts to provide unique names and UIDs for each account by editing the following files: /etc/passwd; /etc/group" \
        "stig:V-215176"
  else
    add security stig_accounts_assigned_unique_user \
        "All accounts have unique User Identification Numbers" NOT_ASSESSED med \
        "not assessed - cat /etc/passwd failed (rc=$AAU_PASSWD_RC)" \
        "PTxray could not obtain a trustworthy user registry for account UID uniqueness." \
        "run 'cat /etc/passwd', correct the capture or path problem, and rerun PTxray." \
        "stig:V-215176"
  fi
elif [ -z "$AAU_USRCK" ] && [ -z "$AAU_PASSWD" ]; then
  add security stig_accounts_assigned_unique_user \
      "All accounts have unique User Identification Numbers" NOT_ASSESSED med \
      "not assessed - user registry probes returned no output" \
      "The successful probes returned no evidence about account UID uniqueness." \
      "confirm both read-only probes return evidence, then rerun PTxray." \
      "stig:V-215176"
else
  AAU_SCAN=$(printf '%s\n' "$AAU_PASSWD" | awk -F: '
    /^[[:space:]]*$/ || /^[[:space:]]*#/ { next }
    /^[+@-]/ { next }
    {
      if (NF < 3 || $3 !~ /^[0-9][0-9]*$/) { bad=1; next }
      records++
      uid=$3
      names[uid]=names[uid] (names[uid] == "" ? "" : ", ") $1
      counts[uid]++
    }
    END {
      dup=""
      for (uid in counts) if (counts[uid] > 1)
        dup=dup (dup == "" ? "" : "; ") "duplicate UID " uid " shared by " counts[uid] " accounts (" names[uid] ")"
      print records "|" dup "|" bad
    }
  ')
  AAU_SCAN_RC=$?
  if [ "$AAU_SCAN_RC" -ne 0 ] || [ -z "$AAU_SCAN" ]; then
    AAU_REASON="passwd registry output was unparseable"
    AAU_RECORDS=0
    AAU_DUPS=""
  else
    AAU_RECORDS=${AAU_SCAN%%\|*}
    AAU_SCAN=${AAU_SCAN#*\|}
    AAU_DUPS=${AAU_SCAN%%\|*}
    AAU_BAD=${AAU_SCAN#*\|}
    AAU_REASON=""
  fi
  if [ "$AAU_SCAN_RC" -ne 0 ] || { [ "$AAU_BAD" = 1 ] && [ "$AAU_RECORDS" -eq 0 ]; }; then
    if [ "$AAU_USRCK_DUP_RC" -eq 1 ]; then
      add security stig_accounts_assigned_unique_user \
          "All accounts have unique User Identification Numbers" FAIL high \
          "usrck reports duplicate UID evidence: $AAU_USRCK_DUP" \
      ...
      add security stig_accounts_assigned_unique_user \
          "All accounts have unique User Identification Numbers" NOT_ASSESSED med \
          "not assessed - passwd registry output was unparseable" \
          "PTxray could not parse the user registry into trustworthy account and UID records." \
          "correct the /etc/passwd format or capture, then rerun PTxray." \
          "stig:V-215176"
    fi
  elif [ "$AAU_RECORDS" -eq 0 ] && [ "$AAU_USRCK_DUP_RC" -ne 1 ]; then
    if [ -z "$AAU_PASSWD" ]; then
      add security stig_accounts_assigned_unique_user \
          "All accounts have unique User Identification Numbers" NOT_ASSESSED med \
          "not assessed - passwd registry returned no output" \
          "PTxray could not obtain a user registry containing account records." \
          "confirm /etc/passwd is readable and non-empty, then rerun PTxray." \
          "stig:V-215176"
    else
      add security stig_accounts_assigned_unique_user \
          "All accounts have unique User Identification Numbers" FAIL high \
          "no account records present in the user registry" \
          "The required user registry contains no recoverable account records, so unique UID assignment is unmet." \
          "Edit user accounts to provide unique names and UIDs for each account by editing the following files: /etc/passwd; /etc/group" \
          "stig:V-215176"
    fi
  elif [ -n "$AAU_DUPS" ]; then
    add security stig_accounts_assigned_unique_user \
        "All accounts have unique User Identification Numbers" FAIL high \
        "$AAU_DUPS" \
        "The user registry assigns one UID to multiple accounts, so the system cannot preserve individual accountability." \
        "Edit user accounts to provide unique names and UIDs for each account by editing the following files: /etc/passwd; /etc/group" \
        "stig:V-215176"
  elif [ "$AAU_USRCK_DUP_RC" -eq 1 ]; then
    add security stig_accounts_assigned_unique_user \
        "All accounts have unique User Identification Numbers" FAIL high \
        "usrck reports duplicate UID evidence: $AAU_USRCK_DUP" \
        "usrck reports that account UID assignment is not unique, so individual accountability is lost." \
    ...
    add security stig_accounts_assigned_unique_user \
        "All accounts have unique User Identification Numbers" PASS med \
        "checked $AAU_RECORDS accounts; all UIDs are unique" \
        "All recoverable account records have distinct UIDs and usrck reports no duplicate UID diagnostic." \
        "n/a" "stig:V-215176"
  fi
fi
}
