...
AIXRAY_TOOL=ck-stig-aixmibd-daemon-disabled


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_aixmibd_daemon_disabled — DISA STIG V-215366 (AIX7-00-003061, CCI-000381).
  # The aixmibd SNMP MIB sub-agent must not be started from /etc/rc.tcpip. DISA's stated
  # command is grep "^start[[:blank:]]/usr/sbin/aixmibd" /etc/rc.tcpip, whose rc 1 on
  # no-match is indistinguishable from a failed probe, so the probe reads the file with
  # cat (equally read-only) and applies the identical anchored pattern in POSIX awk.
  # This check is authoritative for V-215366; ck-stig-svcoff keeps an untagged probe over
  # the same evidence and is deliberately not edited (one control = one tool).
  function eval_stig_aixmibd_daemon_disabled {
    typeset AMD_RAW AMD_RC AMD_HIT AMD_MENTION AMD_REASON AMD_OBS AMD_MEAN AMD_FIX

    AMD_RAW=$(aix rctcp cat /etc/rc.tcpip)
    AMD_RC=$?
    AMD_HIT=""
    AMD_MENTION=""
    AMD_REASON=""
    AMD_FIX="disable the daemon with 'chrctcp -d aixmibd' after confirming no local tooling depends on the SNMP MIB sub-agent; PTxray only recommends this command and never runs it."

    if [ "$AMD_RC" -ne 0 ]; then
      AMD_REASON="probe rctcp failed (rc=$AMD_RC) reading /etc/rc.tcpip"
    elif [ -z "$AMD_RAW" ]; then
      AMD_REASON="probe rctcp returned rc 0 with no output from /etc/rc.tcpip"
    else
      # Anchored exactly as DISA anchors it: column 0 "start", one or more blanks,
      # "/usr/sbin/aixmibd", then a blank or end of line. Leading whitespace or a
      # leading '#' disqualifies the line, so a commented-out stanza is not a finding.
      AMD_HIT=$(printf '%s\n' "$AMD_RAW" | awk '
        {
          if (substr($0, 1, 5) != "start") next
          i = 6
          n = 0
          while (i <= length($0)) {
            c = substr($0, i, 1)
          ...
          if (n == 0) next
          if (substr($0, i, 17) != "/usr/sbin/aixmibd") next
          j = i + 17
          if (j <= length($0)) {
            c = substr($0, j, 1)
            if (c != " " && c != "\t") next
      ...
      ')
      AMD_MENTION=$(printf '%s\n' "$AMD_RAW" | awk '
        index($0, "aixmibd") > 0 { print "yes"; exit 0 }
      ')
    fi

    if [ -n "$AMD_REASON" ]; then
      AMD_MEAN="PTxray did not obtain a trustworthy copy of /etc/rc.tcpip, so whether the aixmibd daemon starts at boot is unknown."
      add security stig_aixmibd_daemon_disabled "aixmibd SNMP MIB sub-agent disabled" NOT_ASSESSED med \
          "not assessed - $AMD_REASON" \
          "$AMD_MEAN" \
          "confirm /etc/rc.tcpip exists and is readable by the scanning account, then rerun PTxray." \
          "stig:V-215366"
    elif [ -n "$AMD_HIT" ]; then
      AMD_OBS="aixmibd started from /etc/rc.tcpip: $AMD_HIT"
      AMD_MEAN="The legacy aixmibd SNMP MIB sub-agent is started at boot from /etc/rc.tcpip, leaving an unneeded network listener and extra attack surface on the host (DISA STIG V-215366)."
      add security stig_aixmibd_daemon_disabled "aixmibd SNMP MIB sub-agent disabled" FAIL med \
          "$AMD_OBS" \
          "$AMD_MEAN" \
          "$AMD_FIX" \
          "stig:V-215366"
    else
      if [ -n "$AMD_MENTION" ]; then
        AMD_OBS="no active start /usr/sbin/aixmibd line in /etc/rc.tcpip (aixmibd stanza present but commented out)"
      else
        AMD_OBS="no active start /usr/sbin/aixmibd line in /etc/rc.tcpip (aixmibd not referenced)"
      fi
      AMD_MEAN="The aixmibd SNMP MIB sub-agent does not start at boot, as DISA STIG V-215366 requires."
      add security stig_aixmibd_daemon_disabled "aixmibd SNMP MIB sub-agent disabled" PASS med \
          "$AMD_OBS" \
          "$AMD_MEAN" \
          "n/a" \
          "stig:V-215366"
  ...
  # stig_aixmibd_daemon_disabled — one control, one tool: DISA STIG V-215366.
  eval_stig_aixmibd_daemon_disabled
}
