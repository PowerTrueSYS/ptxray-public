...
AIXRAY_TOOL=ck-stig-audio-dev-group


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_audio_dev_group — DISA STIG V-215271.
  typeset AUDIO_LSDEV AUDIO_LSDEV_RC AUDIO_DEVICES AUDIO_DEV
  typeset AUDIO_LS AUDIO_LS_RC AUDIO_ROWS AUDIO_ROW AUDIO_GROUP AUDIO_PATH
  typeset AUDIO_STATUS AUDIO_OBSERVED AUDIO_OFFENDERS AUDIO_CHECKED

  AUDIO_LSDEV=$(aix lsdev_audio /usr/sbin/lsdev -C)
  AUDIO_LSDEV_RC=$?
  AUDIO_STATUS=NOT_ASSESSED
  AUDIO_OBSERVED=""
  AUDIO_OFFENDERS=""
  AUDIO_CHECKED=""

  if [ "$AUDIO_LSDEV_RC" -ne 0 ]; then
    AUDIO_OBSERVED="not assessed - lsdev -C failed (rc=$AUDIO_LSDEV_RC)"
  elif ! printf '%s\n' "$AUDIO_LSDEV" | awk 'NF { found=1 } END { exit !found }'; then
    AUDIO_OBSERVED="not assessed - lsdev -C returned no output (rc 0)"
  else
    AUDIO_DEVICES=$(printf '%s\n' "$AUDIO_LSDEV" | awk 'tolower($0) ~ /audio/ { print $1 }')
    if [ -z "$AUDIO_DEVICES" ]; then
      AUDIO_STATUS=FAIL
      AUDIO_OBSERVED="no audio device configured (lsdev -C shows none)"
    else
      AUDIO_LS=$(aix ls_audio_dev ls -lL /dev/*aud*)
      AUDIO_LS_RC=$?
      if [ "$AUDIO_LS_RC" -ne 0 ]; then
        AUDIO_OBSERVED="not assessed - ls -lL /dev/*aud* failed (rc=$AUDIO_LS_RC)"
      elif [ -z "$AUDIO_LS" ]; then
        AUDIO_OBSERVED="not assessed - ls -lL /dev/*aud* returned no output (rc 0)"
      else
        AUDIO_ROWS=$(printf '%s\n' "$AUDIO_LS" | awk '
          $1 == "total" { next }
          {
            if (length($1) != 10 || substr($1, 1, 1) !~ /^[bcdlps-]$/) next
            if (NF < 4) { bad=1; next }
            rows++
            row[rows] = $4 "|" $NF
          }
          END {
            if (bad || rows == 0) print "__UNPARSEABLE__"
            else for (i = 1; i <= rows; i++) print row[i]
          }
        ')
        if [ "$AUDIO_ROWS" = "__UNPARSEABLE__" ] || [ -z "$AUDIO_ROWS" ]; then
          AUDIO_OBSERVED="not assessed - unparseable ls -lL output"
        else
          AUDIO_STATUS=PASS
          while IFS='|' read AUDIO_GROUP AUDIO_PATH; do
            [ -n "$AUDIO_GROUP" ] || {
              AUDIO_STATUS=NOT_ASSESSED
              AUDIO_OBSERVED="not assessed - unparseable ls -lL output"
              break
            }
            if [ -n "$AUDIO_CHECKED" ]; then
              AUDIO_CHECKED="$AUDIO_CHECKED, $AUDIO_PATH group $AUDIO_GROUP"
            else
              AUDIO_CHECKED="$AUDIO_PATH group $AUDIO_GROUP"
            fi
            case "$AUDIO_GROUP" in
              root|sys|bin|system) : ;;
              *)
                AUDIO_STATUS=FAIL
                if [ -n "$AUDIO_OFFENDERS" ]; then
                  AUDIO_OFFENDERS="$AUDIO_OFFENDERS, $AUDIO_PATH:$AUDIO_GROUP"
                else
                  AUDIO_OFFENDERS="$AUDIO_PATH:$AUDIO_GROUP"
                fi
                ;;
            esac
          done <<EOF
...
EOF
          if [ "$AUDIO_STATUS" = FAIL ]; then
            AUDIO_OBSERVED="audio device group ownership outside the allowlist: $AUDIO_OFFENDERS"
          elif [ "$AUDIO_STATUS" = PASS ]; then
            AUDIO_OBSERVED="checked audio devices: $AUDIO_CHECKED"
          fi
        fi
      fi
    fi
  ...
  case "$AUDIO_STATUS" in
    PASS)
      add security stig_audio_dev_group "Audio device group ownership" PASS med \
        "$AUDIO_OBSERVED" \
        "Audio devices are group-owned by root, sys, bin or system as required by DISA STIG V-215271." \
        "n/a" "stig:V-215271"
      ;;
    FAIL)
      add security stig_audio_dev_group "Audio device group ownership" FAIL med \
        "$AUDIO_OBSERVED" \
        "An audio device is group-owned by an ordinary group, allowing its members to open the device and record nearby audio." \
        "chgrp system <audio device>" "stig:V-215271"
      ;;
    NOT_APPLICABLE)
      add security stig_audio_dev_group "Audio device group ownership" NOT_APPLICABLE low \
        "not applicable" "The audio device group ownership rule does not apply." \
        "n/a" "stig:V-215271"
      ;;
    *)
      add security stig_audio_dev_group "Audio device group ownership" NOT_ASSESSED med \
        "$AUDIO_OBSERVED" \
        "PTxray did not obtain trustworthy evidence for audio device group ownership." \
        "verify the audio device inventory and permissions, then rerun PTxray." "stig:V-215271"
      ;;
  esac
}
