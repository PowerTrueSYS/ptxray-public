...
AIXRAY_TOOL=ck-stig-audit-bin-stanza


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_audit_bin_stanza — DISA STIG V-219956.
  typeset ABS_RAW ABS_RC ABS_PARSED ABS_STATUS ABS_OBSERVED ABS_MEANING ABS_FIX

  ABS_RAW=$(aix audit_config cat /etc/security/audit/config)
  ABS_RC=$?
  ABS_STATUS=NOT_ASSESSED
  ABS_OBSERVED=""
  ABS_MEANING=""
  ABS_FIX="The bin stanza in /etc/security/audit/config must set trail, bin1, bin2, cmds, backuppath, backupsize and bincompact, with binsize >= 25000 and freespace >= 65536; then restart the audit subsystem with /usr/sbin/audit shutdown followed by /usr/sbin/audit start."

  if [ "$ABS_RC" -ne 0 ]; then
    ABS_OBSERVED="not assessed - probe audit_config failed (rc=$ABS_RC)"
  elif [ -z "$ABS_RAW" ] || ! printf '%s\n' "$ABS_RAW" | awk 'NF { found=1 } END { exit !found }'; then
    ABS_OBSERVED="not assessed - /etc/security/audit/config returned no output (rc 0)"
  else
    ABS_PARSED=$(printf '%s\n' "$ABS_RAW" | awk '
      function add_deficiency(item) {
        if (deficiencies == "") deficiencies = item
        else deficiencies = deficiencies ", " item
      }
      /^[ \t]*#/ { next }
      {
        line = $0
      ...
      END {
        if (!shape) { print "UNPARSEABLE"; exit }
        if (!bin_seen) { print "NO_BIN"; exit }
        split("trail bin1 bin2 binsize cmds freespace backuppath backupsize bincompact", keys, " ")
        for (i = 1; i <= 9; i++) {
          key = keys[i]
          if (!(key in seen)) add_deficiency(key " missing")
          else if (value_for[key] == "") add_deficiency(key " empty")
          else if (key == "binsize") {
            n = value_for[key]
            sub(/^\+/, "", n)
            if (n !~ /^[0-9][0-9]*$/ || (n + 0) < 25000) add_deficiency("binsize=" value_for[key] " below 25000")
          }
          else if (key == "freespace") {
            n = value_for[key]
            sub(/^\+/, "", n)
            if (n !~ /^[0-9][0-9]*$/ || (n + 0) < 65536) add_deficiency("freespace=" value_for[key] " below 65536")
          }
        }
        if (deficiencies != "") print "BAD|" deficiencies
        else print "OK|" value_for["binsize"] "|" value_for["freespace"]
      }
    ')
    case "$ABS_PARSED" in
      OK\|*)
        ABS_STATUS=PASS
        ABS_BINSIZE=$(printf '%s\n' "$ABS_PARSED" | awk -F'|' '{ print $2 }')
        ABS_FREESPACE=$(printf '%s\n' "$ABS_PARSED" | awk -F'|' '{ print $3 }')
        ABS_OBSERVED="bin stanza in /etc/security/audit/config sets trail, bin1, bin2, cmds, backuppath, backupsize, bincompact with binsize=$ABS_BINSIZE (>=25000) and freespace=$ABS_FREESPACE (>=65536)"
        ;;
      NO_BIN)
        ABS_STATUS=FAIL
        ABS_OBSERVED="no bin: stanza in /etc/security/audit/config"
        ;;
      BAD\|*)
        ABS_STATUS=FAIL
        ABS_OBSERVED="bin stanza deficiencies: $(printf '%s\n' "$ABS_PARSED" | cut -d '|' -f 2-)"
        ;;
      *)
        ABS_OBSERVED="not assessed - /etc/security/audit/config output not parseable as stanza format"
        ;;
    esac
  fi

  case "$ABS_STATUS" in
    PASS) ABS_MEANING="The audit bin stanza retains the audit trail and preserves records when the audit filesystem fills." ;;
    FAIL) ABS_MEANING="The audit bin stanza lacks a required audit storage or retention setting, so audit records can be lost when storage fills." ;;
    *) ABS_MEANING="PTxray did not obtain trustworthy evidence for the audit bin stanza." ;;
  esac
  add security stig_audit_bin_stanza "Audit bin storage action" \
      "$ABS_STATUS" med "$ABS_OBSERVED" "$ABS_MEANING" "$ABS_FIX" "stig:V-219956"
