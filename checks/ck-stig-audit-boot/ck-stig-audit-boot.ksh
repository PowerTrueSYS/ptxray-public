...
AIXRAY_TOOL=ck-stig-audit-boot


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # ck_stig_audit_boot — DISA STIG V-215247.
  typeset SAB_STAT_RAW SAB_STAT_RC SAB_AUDIT_RAW SAB_AUDIT_RC
  typeset SAB_STATUS SAB_OBSERVED SAB_LINE

  SAB_STAT_RAW=$(aix rc_file_stat ls -ld /etc/rc)
  SAB_STAT_RC=$?
  if [ "$SAB_STAT_RC" -eq 0 ] && [ -z "$SAB_STAT_RAW" ]; then
    SAB_STATUS=NOT_ASSESSED
    SAB_OBSERVED="not assessed - probe rc_file_stat returned rc 0 with no output"
  elif [ "$SAB_STAT_RC" -ne 0 ] && [ -z "$SAB_STAT_RAW" ]; then
    SAB_STATUS=NOT_ASSESSED
    SAB_OBSERVED="not assessed - probe rc_file_stat failed (rc=$SAB_STAT_RC) with no diagnostic"
  elif [ "$SAB_STAT_RC" -ne 0 ]; then
    if printf '%s\n' "$SAB_STAT_RAW" | awk '{ line=tolower($0); if (line ~ /does not exist|no such file|0653-341/) found=1 } END { exit(found ? 0 : 1) }'; then
      SAB_STATUS=FAIL
      SAB_OBSERVED="/etc/rc not present - audit is not started at boot"
    else
      SAB_STATUS=NOT_ASSESSED
      SAB_OBSERVED="not assessed - unparseable rc_file_stat output"
    fi
  else
    SAB_AUDIT_RAW=$(aix rc_audit_start grep "audit start" /etc/rc)
    SAB_AUDIT_RC=$?
    if [ "$SAB_AUDIT_RC" -eq 0 ] && [ -z "$SAB_AUDIT_RAW" ]; then
      SAB_STATUS=NOT_ASSESSED
      SAB_OBSERVED="not assessed - probe rc_audit_start returned rc 0 with no output"
    elif [ "$SAB_AUDIT_RC" -eq 0 ]; then
      SAB_LINE=$(printf '%s\n' "$SAB_AUDIT_RAW" | awk '
        {
          line=$0
          sub(/^[ \t]*/, "", line)
          if (line ~ /(^|[^A-Za-z0-9_])audit[ \t]+start([^A-Za-z0-9_]|$)/) {
            matches++
            if (substr(line, 1, 1) != "#") {
              uncommented++
              if (first == "") { gsub(/[ \t]+/, " ", line); first=line }
            }
          }
        }
        END {
          if (uncommented) { if (length("" first) > 120) first=substr(first, 1, 120); print "pass:" first }
          else if (matches) print "commented"
          else print "unparseable"
        }')
      case "$SAB_LINE" in
        pass:*)
          SAB_STATUS=PASS
          SAB_OBSERVED="/etc/rc starts audit at boot: ${SAB_LINE#pass:}"
          ;;
        commented)
          SAB_STATUS=FAIL
          SAB_OBSERVED="only commented 'audit start' line(s) in /etc/rc - audit is not started at boot"
          ;;
        *)
          SAB_STATUS=NOT_ASSESSED
          SAB_OBSERVED="not assessed - unparseable rc_audit_start output"
          ;;
      esac
    elif [ "$SAB_AUDIT_RC" -eq 1 ] && [ -z "$SAB_AUDIT_RAW" ]; then
      SAB_STATUS=FAIL
      SAB_OBSERVED="no 'audit start' entry in /etc/rc"
    elif [ "$SAB_AUDIT_RC" -ne 0 ]; then
      SAB_STATUS=NOT_ASSESSED
      SAB_OBSERVED="not assessed - probe rc_audit_start failed (rc=$SAB_AUDIT_RC)"
    else
      SAB_STATUS=NOT_ASSESSED
      SAB_OBSERVED="not assessed - unparseable rc_audit_start output"
    fi
  ...
  add security stig_audit_boot "AIX must start audit at boot." \
    "$SAB_STATUS" med "$SAB_OBSERVED" \
    "AIX must start auditing at boot; actions taken before a later manual audit start have no audit trail." \
    "Add /usr/sbin/audit start to /etc/rc just prior to the line reading dspmsg rc.cat 5 'Multi-user initialization completed', and symmetrically add /usr/sbin/audit shutdown to /etc/rc.shutdown. PTxray never runs this fix." \
    "stig:V-215247"
}
