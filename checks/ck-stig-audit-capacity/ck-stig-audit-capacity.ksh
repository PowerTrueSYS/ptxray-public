...
AIXRAY_TOOL=ck-stig-audit-capacity


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # ck-stig-audit-capacity — DISA STIG V-215253, AIX7-00-002033.
  # The rule has no Not Applicable condition; this fragment never emits it.
  typeset SAC_CONFIG_RAW SAC_CONFIG_RC SAC_TRAIL SAC_DF_RAW SAC_DF_RC
  typeset SAC_MATCH SAC_MOUNT SAC_FREE SAC_STATUS SAC_OBSERVED
  SAC_REQUIRED_KB=1048576  # STIG figure: 1 GB of audit-record storage.

  SAC_CONFIG_RAW=$(aix audit_config_trail grep trail /etc/security/audit/config)
  SAC_CONFIG_RC=$?
  SAC_TRAIL=""
  SAC_STATUS=""
  SAC_OBSERVED=""

  if [ "$SAC_CONFIG_RC" -ge 2 ]; then
    SAC_STATUS=NOT_ASSESSED
    SAC_OBSERVED="not assessed - grep trail /etc/security/audit/config exited $SAC_CONFIG_RC"
  elif [ "$SAC_CONFIG_RC" -eq 1 ] && [ -z "$SAC_CONFIG_RAW" ]; then
    SAC_STATUS=FAIL
    SAC_OBSERVED="no trail directive in /etc/security/audit/config"
  elif [ "$SAC_CONFIG_RC" -eq 0 ] && [ -z "$SAC_CONFIG_RAW" ]; then
    SAC_STATUS=NOT_ASSESSED
    SAC_OBSERVED="not assessed - grep trail /etc/security/audit/config returned no output with status 0"
  elif [ "$SAC_CONFIG_RC" -ne 0 ]; then
    SAC_STATUS=NOT_ASSESSED
    SAC_OBSERVED="not assessed - grep trail /etc/security/audit/config returned status $SAC_CONFIG_RC with output"
  else
    SAC_TRAIL=$(printf '%s\n' "$SAC_CONFIG_RAW" | awk '
      {
        line=$0
        ...
        sub(/[ \t]*#.*/, "", line)
        sub(/[ \t]*$/, "", line)
        if (line ~ /^\// && line !~ /[ \t]/) trail=line
      }
      END { if (trail != "") print trail; else exit 1 }
    ')
    if [ $? -ne 0 ] || [ -z "$SAC_TRAIL" ]; then
      SAC_STATUS=NOT_ASSESSED
      SAC_OBSERVED="not assessed - trail directive was unparseable"
    fi
  fi

  if [ -z "$SAC_STATUS" ]; then
    SAC_DF_RAW=$(aix audit_trail_df df -k)
    SAC_DF_RC=$?
    if [ "$SAC_DF_RC" -ne 0 ]; then
      SAC_STATUS=NOT_ASSESSED
      SAC_OBSERVED="not assessed - df -k exited $SAC_DF_RC"
    else
      SAC_MATCH=$(printf '%s\n' "$SAC_DF_RAW" | awk -v trail="$SAC_TRAIL" '
        function boundary(m) {
          return (m == "/" || m == trail || substr(trail, 1, length("" m) + 1) == m "/")
        }
        $1 == "Filesystem" || NR == 1 { next }
        NF >= 7 && $3 ~ /^[0-9][0-9]*$/ && boundary($NF) {
          if (length("" $NF) > best) { best=length("" $NF); mount=$NF; free=$3 }
        }
        END { if (mount != "") print mount "|" free; else exit 1 }
      ')
      if [ $? -ne 0 ] || [ -z "$SAC_MATCH" ]; then
        SAC_STATUS=NOT_ASSESSED
        SAC_OBSERVED="not assessed - could not parse a filesystem row from df -k for $SAC_TRAIL"
      else
        SAC_MOUNT=${SAC_MATCH%%\|*}
        SAC_FREE=${SAC_MATCH#*\|}
        if [ "$SAC_FREE" -ge "$SAC_REQUIRED_KB" ]; then
          SAC_STATUS=PASS
          SAC_OBSERVED="audit trail $SAC_TRAIL on filesystem $SAC_MOUNT has $SAC_FREE KB free (>= $SAC_REQUIRED_KB KB required)"
        else
          SAC_STATUS=FAIL
          SAC_OBSERVED="audit trail $SAC_TRAIL on filesystem $SAC_MOUNT has $SAC_FREE KB free (< $SAC_REQUIRED_KB KB required)"
        fi
  ...
  case "$SAC_STATUS" in
    PASS)
      add security stig_audit_capacity "Audit trail storage capacity" PASS med \
        "$SAC_OBSERVED" \
        "The filesystem holding the audit trail has at least one GB free for audit records." \
        "chfs -a size=+1G <root of file system for audit logs>" "stig:V-215253"
      ;;
    FAIL)
      add security stig_audit_capacity "Audit trail storage capacity" FAIL med \
        "$SAC_OBSERVED" \
        "The filesystem holding the audit trail does not have the required free capacity." \
        "chfs -a size=+1G <root of file system for audit logs>" "stig:V-215253"
      ;;
    *)
      add security stig_audit_capacity "Audit trail storage capacity" NOT_ASSESSED med \
        "$SAC_OBSERVED" \
        "PTxray did not obtain trustworthy evidence for the audit trail storage capacity." \
        "chfs -a size=+1G <root of file system for audit logs>" "stig:V-215253"
      ;;
  esac
}
