...
AIXRAY_TOOL=ck-stig-audit-config-perms-audit-configuration-files-set


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_audit_config_perms_audit_configuration_files_set — V-215317.
  # The two probes are deliberately separate: directory absence is a finding,
  # while missing or unreadable evidence is a refusal.
  typeset AAC_DIR_RAW AAC_DIR_RC AAC_MODES_RAW AAC_MODES_RC AAC_DIR_PRESENT
  typeset AAC_PARSED AAC_SUMMARY AAC_COUNT AAC_OFFENDERS AAC_ITEM
  typeset AAC_STATUS AAC_OBSERVED AAC_MEANING AAC_FIX

  AAC_DIR_RAW=$(aix audit_dir_entries ls -a /etc/security)
  AAC_DIR_RC=$?
  AAC_DIR_PRESENT=""
  if [ "$AAC_DIR_RC" -ne 0 ]; then
    AAC_STATUS=NOT_ASSESSED
    AAC_OBSERVED="not assessed - could not list /etc/security (probe rc $AAC_DIR_RC)"
  elif [ -z "$AAC_DIR_RAW" ]; then
    AAC_STATUS=NOT_ASSESSED
    AAC_OBSERVED="not assessed - /etc/security listing was empty with rc 0"
  elif printf '%s\n' "$AAC_DIR_RAW" | awk '$0 == "audit" { found=1 } END { exit(found ? 0 : 1) }'; then
    AAC_DIR_PRESENT=yes
  else
    AAC_STATUS=FAIL
    AAC_OBSERVED="/etc/security/audit is absent; audit configuration files required to be mode 640 or less permissive are not present"
  fi

  if [ -n "$AAC_DIR_PRESENT" ]; then
    AAC_MODES_RAW=$(aix audit_cfg_modes ls -l /etc/security/audit)
    AAC_MODES_RC=$?
    if [ "$AAC_MODES_RC" -ne 0 ]; then
      AAC_STATUS=NOT_ASSESSED
      AAC_OBSERVED="not assessed - could not list /etc/security/audit (probe rc $AAC_MODES_RC)"
    elif [ -z "$AAC_MODES_RAW" ]; then
      AAC_STATUS=NOT_ASSESSED
      AAC_OBSERVED="not assessed - /etc/security/audit listing was empty with rc 0"
    else
      AAC_PARSED=$(printf '%s\n' "$AAC_MODES_RAW" | awk '
        /^total[ 	]/ { next }
        {
          if (NF < 2 || length("" $1) != 10 && length("" $1) != 11 || substr($1, 1, 1) !~ /^[^ 	]$/) {
            bad=1
            next
          }
          mode=$1
          ...
          if (substr(mode, 2, 1) !~ /^[-r]$/ || substr(mode, 3, 1) !~ /^[-w]$/ ||
              substr(mode, 4, 1) !~ /^[-xSs]$/ || substr(mode, 5, 1) !~ /^[-r]$/ ||
              substr(mode, 6, 1) !~ /^[-w]$/ || substr(mode, 7, 1) !~ /^[-xSs]$/ ||
              substr(mode, 8, 1) !~ /^[-r]$/ || substr(mode, 9, 1) !~ /^[-w]$/ ||
              substr(mode, 10, 1) !~ /^[-xTt]$/) { bad=1; next }
          count++
          owner=0; group=0; other=0
          if (substr(mode, 2, 1) != "-") owner+=4
          if (substr(mode, 3, 1) != "-") owner+=2
          if (substr(mode, 5, 1) != "-") group+=4
          if (substr(mode, 6, 1) != "-") group+=2
          if (substr(mode, 8, 1) != "-") other+=4
          if (substr(mode, 9, 1) != "-") other+=2
          if (substr(mode, 4, 1) == "x" || substr(mode, 4, 1) == "s") owner+=1
          if (substr(mode, 7, 1) == "x" || substr(mode, 7, 1) == "s") group+=1
          if (substr(mode, 10, 1) == "x" || substr(mode, 10, 1) == "t") other+=1
          special=0
          if (substr(mode, 4, 1) ~ /[sS]/) special+=4
          if (substr(mode, 7, 1) ~ /[sS]/) special+=2
          if (substr(mode, 10, 1) ~ /[tT]/) special+=1
          if (substr(mode, 4, 1) ~ /[xSs]/ || substr(mode, 6, 1) != "-" ||
              substr(mode, 7, 1) ~ /[xSs]/ || substr(mode, 8, 1) != "-" ||
              substr(mode, 9, 1) != "-" || substr(mode, 10, 1) != "-") {
            findings++
            if (special) print "offender|" $NF "|" sprintf("%d%d%d%d", special, owner, group, other)
            else print "offender|" $NF "|" sprintf("%d%d%d", owner, group, other)
          }
        }
        END { print "summary|" count "|" findings }
      ')
      AAC_SUMMARY=$(printf '%s\n' "$AAC_PARSED" | sed -n '$p' | sed 's/^summary|//')
      AAC_COUNT=$(printf '%s\n' "$AAC_SUMMARY" | cut -d'|' -f1)
      AAC_FINDINGS=$(printf '%s\n' "$AAC_SUMMARY" | cut -d'|' -f2)
      if [ "$AAC_COUNT" -eq 0 ]; then
        AAC_STATUS=NOT_ASSESSED
        AAC_OBSERVED="not assessed - could not parse ls -l output for /etc/security/audit"
      else
        AAC_OFFENDERS=""; AAC_ITEM=""; AAC_OFFENDER_N=0
        while IFS='|' read -r AAC_NAME AAC_OCTAL; do
          [ -n "$AAC_NAME" ] || continue
          [ "$AAC_OFFENDER_N" -lt 5 ] || continue
          AAC_OFFENDER_N=$((AAC_OFFENDER_N + 1))
          AAC_ITEM="$AAC_NAME($AAC_OCTAL)"
          if [ -z "$AAC_OFFENDERS" ]; then AAC_OFFENDERS=$AAC_ITEM
          else AAC_OFFENDERS="$AAC_OFFENDERS, $AAC_ITEM"; fi
        done <<EOF
$(printf '%s\n' "$AAC_PARSED" | sed -n 's/^offender|//p' | sort)
EOF
        if [ -n "$AAC_OFFENDERS" ]; then
          AAC_STATUS=FAIL
          [ "$AAC_FINDINGS" -gt 5 ] && AAC_OFFENDERS="$AAC_OFFENDERS, ..."
          AAC_OBSERVED="$AAC_FINDINGS of $AAC_COUNT files in /etc/security/audit are more permissive than 640: $AAC_OFFENDERS"
        else
          AAC_STATUS=PASS
          AAC_OBSERVED="all $AAC_COUNT audit configuration files in /etc/security/audit are 640 or less permissive"
        fi
      fi
    fi
  fi
  ...
  case "$AAC_STATUS" in
    FAIL)
      AAC_MEANING="One or more AIX audit configuration files can be read or changed beyond the root and audit group, allowing an unprivileged account to learn or alter what is recorded."
      AAC_FIX="chmod 640 /etc/security/audit/* (advisory only; PTxray does not execute this command)"
      ;;
    PASS)
      AAC_MEANING="AIX audit configuration files are restricted to mode 640 or less permissive."
      AAC_FIX="n/a"
      ;;
    *)
      AAC_MEANING="PTxray did not obtain trustworthy evidence for the AIX audit configuration file permissions."
      AAC_FIX="verify the audit configuration file permissions and rerun PTxray."
      ;;
  esac
  add security stig_audit_config_perms_audit_configuration_files_set \
    "The AIX audit configuration files must be set to 640 or less permissive" \
    "$AAC_STATUS" med "$AAC_OBSERVED" "$AAC_MEANING" "$AAC_FIX" "stig:V-215317"
}
