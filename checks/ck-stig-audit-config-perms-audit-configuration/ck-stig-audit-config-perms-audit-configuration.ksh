...
AIXRAY_TOOL=ck-stig-audit-config-perms-audit-configuration


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # STIG V-215316 — audit configuration files must be group-owned by audit.
  typeset ACG_RAW ACG_RC ACG_ROWS ACG_USABLE ACG_WRONG ACG_WRONG_COUNT
  typeset ACG_DETAILS ACG_OBSERVED ACG_STATUS ACG_ABSENT ACG_PARTIAL
  typeset ACG_UID ACG_UID_RC ACG_LIVE

  # Numeric uid from `id -u` is the liveness anchor: rc 2 with empty ls
  # stdout is determinate absence only when this probe proves the check ran.
  ACG_UID=$(aix id_u id -u)
  ACG_UID_RC=$?
  ACG_LIVE=0
  if [ "$ACG_UID_RC" -eq 0 ]; then
    if printf '%s\n' "$ACG_UID" | awk '
      {
        line = $0
        gsub(/[ \t\r]+/, "", line)
  ...
  # Quoted so the probe receives the literal pattern (the fixture contract) and the
  # local shell never expands the glob against this host's /etc/security/audit.
  ACG_RAW=$(aix audit_config_group ls -l '/etc/security/audit/*')
  ACG_RC=$?
  ACG_STATUS=NOT_ASSESSED
  ACG_OBSERVED=""
  ACG_ABSENT=0
  ACG_PARTIAL=0
  if [ "$ACG_RC" -eq 1 ] || [ "$ACG_RC" -eq 2 ]; then
    ACG_PARTIAL=1
  fi
  if printf '%s\n' "$ACG_RAW" | awk 'tolower($0) ~ /does not exist|no such file|not found/ { found=1 } END { exit !found }'; then
    ACG_ABSENT=1
  fi
  ACG_ROWS=$(printf '%s\n' "$ACG_RAW" | awk '
    {
      mode=$1
      if ((length("" mode) == 10 || length("" mode) == 11) &&
          substr(mode, 1, 1) ~ /^[-dlbcps]$/ &&
          (length("" mode) == 10 || substr(mode, 11, 1) == "+") && NF >= 4) {
        print $4 "\t" $NF
      }
    }
  ')
  ACG_USABLE=$(printf '%s\n' "$ACG_ROWS" | awk 'NF { n++ } END { print n + 0 }')

  if [ "$ACG_USABLE" -gt 0 ]; then
    if [ "$ACG_RC" -eq 0 ] || [ "$ACG_PARTIAL" -eq 1 ]; then
      ACG_WRONG=$(printf '%s\n' "$ACG_ROWS" | awk -F '\t' '$1 != "audit" { print $2 "\t" $1 }' | sort)
      ACG_WRONG_COUNT=$(printf '%s\n' "$ACG_WRONG" | awk 'NF { n++ } END { print n + 0 }')
      if [ "$ACG_WRONG_COUNT" -eq 0 ]; then
        ACG_STATUS=PASS
        ACG_OBSERVED="all $ACG_USABLE file(s) under /etc/security/audit are group-owned by audit"
        if [ "$ACG_PARTIAL" -eq 1 ]; then
          ACG_OBSERVED="$ACG_OBSERVED; some paths unreadable"
        fi
      else
        ACG_STATUS=FAIL
        ACG_DETAILS=$(printf '%s\n' "$ACG_WRONG" | awk -F '\t' '
          NR <= 5 { if (NR > 1) printf ", "; printf "%s (group %s)", $1, $2 }
          END { print "" }
        ')
        if [ "$ACG_WRONG_COUNT" -gt 5 ]; then
          ACG_DETAILS="${ACG_DETAILS}, and $((ACG_WRONG_COUNT - 5)) more"
        fi
        ACG_OBSERVED="$ACG_WRONG_COUNT file(s) under /etc/security/audit not group-owned by audit: $ACG_DETAILS"
      fi
    else
      ACG_OBSERVED="not assessed - probe 'ls -l /etc/security/audit/*' exited rc $ACG_RC"
    fi
  elif [ "$ACG_ABSENT" -eq 1 ]; then
    ACG_STATUS=FAIL
    ACG_OBSERVED="no files found under /etc/security/audit - audit configuration files are missing"
  elif [ "$ACG_RC" -eq 2 ] && [ -z "$ACG_RAW" ] && [ "$ACG_LIVE" -eq 1 ]; then
    ACG_STATUS=FAIL
    ACG_OBSERVED="no files found under /etc/security/audit; some paths unreadable"
  elif [ "$ACG_RC" -ne 0 ]; then
    ACG_OBSERVED="not assessed - probe 'ls -l /etc/security/audit/*' exited rc $ACG_RC"
  elif ! printf '%s\n' "$ACG_RAW" | awk 'NF { found=1 } END { exit !found }'; then
    ACG_OBSERVED="not assessed - probe returned rc 0 with empty output"
  else
    ACG_OBSERVED="not assessed - unparseable output from 'ls -l /etc/security/audit/*'"
  fi

  if [ "$ACG_STATUS" = PASS ]; then
    add security stig_audit_config_perms_audit_configuration \
      "The AIX audit configuration files must be group-owned by audit." PASS med \
      "$ACG_OBSERVED" \
      "Audit configuration files are all group-owned by audit as required by STIG V-215316." \
      "chgrp audit /etc/security/audit/*" "stig:V-215316"
  elif [ "$ACG_STATUS" = FAIL ]; then
    add security stig_audit_config_perms_audit_configuration \
      "The AIX audit configuration files must be group-owned by audit." FAIL med \
      "$ACG_OBSERVED" \
      "An incorrect group owner can let members of that group alter which events are recorded and disable the audit trail." \
      "chgrp audit /etc/security/audit/*" "stig:V-215316"
  else
    add security stig_audit_config_perms_audit_configuration \
      "The AIX audit configuration files must be group-owned by audit." NOT_ASSESSED med \
      "$ACG_OBSERVED" \
      "PTxray did not obtain trustworthy evidence for the audit configuration file group ownership." \
      "chgrp audit /etc/security/audit/*" "stig:V-215316"
  fi
}
