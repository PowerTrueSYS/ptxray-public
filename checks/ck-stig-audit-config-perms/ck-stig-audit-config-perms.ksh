...
AIXRAY_TOOL=ck-stig-audit-config-perms


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_audit_config_perms — DISA STIG V-215315.
  # Every audit configuration file under /etc/security/audit must be owned by
  # root.  ls -ldn is probed so the owner test is uid == 0, independent of name
  # resolution; -d keeps a directory argument from expanding into its contents.
  # Every uncertain evidence shape fails closed (NOT_ASSESSED), never PASS.
  function ck_stig_audit_config_perms {
    typeset ACAP_RAW ACAP_RC ACAP_PARSE ACAP_RECORDS ACAP_BAD
    typeset ACAP_ITEMS ACAP_STATUS ACAP_OBSERVED ACAP_MEANING ACAP_FIX

    ACAP_RAW=$(aix audit_config_ls ls -ldn /etc/security/audit/*)
    ACAP_RC=$?
    ACAP_RECORDS=""
    ACAP_BAD=""
    ACAP_ITEMS=""
    ...
    if [ "$ACAP_RC" -ne 0 ]; then
      if printf '%s\n' "$ACAP_RAW" | grep -iq -E 'does not exist|not found|No such file|0653-341'; then
        ACAP_STATUS=FAIL
        ACAP_OBSERVED="no files present under /etc/security/audit - audit configuration files are required and missing"
        ACAP_MEANING="The audit configuration files under /etc/security/audit are absent, so the audit configuration required by DISA STIG V-215315 is not in force."
        ACAP_FIX="restore the missing audit configuration files under /etc/security/audit owned by root; PTxray only recommends this and never runs 'chown root /etc/security/audit/*'."
      else
        ACAP_STATUS=NOT_ASSESSED
        ACAP_OBSERVED="not assessed - ls -ldn /etc/security/audit/* exited rc $ACAP_RC"
        ACAP_MEANING="PTxray did not obtain trustworthy evidence of the ownership of the audit configuration files."
        ACAP_FIX="run 'ls -ldn /etc/security/audit/*' as root, correct the probe failure, and rerun PTxray."
      fi
    elif [ -z "$ACAP_RAW" ]; then
      ACAP_STATUS=NOT_ASSESSED
      ACAP_OBSERVED="not assessed - ls -ldn /etc/security/audit/* returned rc 0 with no output"
      ACAP_MEANING="PTxray did not obtain trustworthy evidence of the ownership of the audit configuration files."
      ACAP_FIX="run 'ls -ldn /etc/security/audit/*' as root, correct the probe failure, and rerun PTxray."
    elif ! printf '%s\n' "$ACAP_RAW" | awk 'NF { found = 1 } END { exit found ? 0 : 1 }'; then
      ACAP_STATUS=NOT_ASSESSED
      ACAP_OBSERVED="not assessed - ls -ldn /etc/security/audit/* returned rc 0 with no output"
      ACAP_MEANING="PTxray did not obtain trustworthy evidence of the ownership of the audit configuration files."
      ACAP_FIX="run 'ls -ldn /etc/security/audit/*' as root, correct the probe failure, and rerun PTxray."
        ...
        NF < 9 { next }
        {
          ...
          if ($3 != "0") {
            ...
            if (bad <= 10) {
              ...
              items = items "not owned by root: " $NF " (uid " $3 ")"
            }
          }
        }
        END {
      ...
      ')
      ACAP_RECORDS=$(printf '%s\n' "$ACAP_PARSE" | sed -n '1p')
      ACAP_BAD=$(printf '%s\n' "$ACAP_PARSE" | sed -n '2p')
      ACAP_ITEMS=$(printf '%s\n' "$ACAP_PARSE" | sed -n '3p')
      if [ -z "$ACAP_RECORDS" ] || [ "$ACAP_RECORDS" -eq 0 ]; then
        ACAP_STATUS=NOT_ASSESSED
        ACAP_OBSERVED="not assessed - unparseable output from ls -ldn /etc/security/audit/*"
        ACAP_MEANING="PTxray did not obtain trustworthy evidence of the ownership of the audit configuration files."
        ACAP_FIX="run 'ls -ldn /etc/security/audit/*' as root, correct the probe failure, and rerun PTxray."
      elif [ "$ACAP_BAD" -eq 0 ]; then
        ACAP_STATUS=PASS
        ACAP_OBSERVED="all $ACAP_RECORDS file(s) under /etc/security/audit are owned by root (uid 0)"
        ACAP_MEANING="Every audit configuration file under /etc/security/audit is owned by root as DISA STIG V-215315 requires."
        ACAP_FIX="n/a"
      else
        ACAP_STATUS=FAIL
        ACAP_OBSERVED=$ACAP_ITEMS
        if [ "$ACAP_BAD" -gt 10 ]; then
          ACAP_OBSERVED="$ACAP_OBSERVED (+$(($ACAP_BAD - 10)) more)"
        fi
        ACAP_MEANING="A non-root account owns one or more audit configuration files under /etc/security/audit and can edit or truncate the audit policy, defeating the audit trail."
        ACAP_FIX="apply 'chown root /etc/security/audit/*' as an administrator to restore root ownership; PTxray only recommends this command and never executes it."
      fi
    fi

    add security stig_audit_config_perms "The AIX audit configuration files must be owned by root" \
      "$ACAP_STATUS" med "$ACAP_OBSERVED" "$ACAP_MEANING" "$ACAP_FIX" "stig:V-215315"
  }

  # The fragment body is wrapped in `function standalone_check { ... }` by
  # build/assemble.sh and that entry point is invoked; call the defined check
  # here so the assembled tool actually emits a finding.
  ck_stig_audit_config_perms
}
