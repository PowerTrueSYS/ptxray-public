...
AIXRAY_TOOL=ck-stig-audit-freespace


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # V-215241: retain at least 25% of the audit filesystem as freespace.
  # The AIX audit bin-stanza value is in 512-byte blocks; divide by two for KB.
  FREESPACE_BLOCK_BYTES=512
  typeset AF_CONFIG_RAW AF_CONFIG_RC AF_FS_RAW AF_FS_RC
  typeset AF_CONFIG_SHAPE AF_FS_KB AF_BLOCKS AF_FREE_KB AF_THRESHOLD
  typeset AF_STATUS AF_OBSERVED AF_MEANING AF_FIX

  AF_CONFIG_RAW=$(aix audit_config_freespace grep -E "freespace" /etc/security/audit/config)
  AF_CONFIG_RC=$?
  AF_FS_RAW=$(aix audit_fs_size df -k /audit)
  AF_FS_RC=$?

  AF_STATUS=NOT_ASSESSED
  AF_OBSERVED=""
  AF_CONFIG_SHAPE=""
  AF_FS_KB=""
  AF_BLOCKS=""

  if [ "$AF_CONFIG_RC" -ge 2 ]; then
    AF_OBSERVED="not assessed - grep -E \"freespace\" /etc/security/audit/config exited $AF_CONFIG_RC"
  elif [ "$AF_FS_RC" -ne 0 ]; then
    AF_OBSERVED="not assessed - df -k /audit exited $AF_FS_RC"
  elif [ "$AF_CONFIG_RC" -eq 0 ] && [ -z "$AF_CONFIG_RAW" ]; then
    AF_OBSERVED="not assessed - grep -E \"freespace\" /etc/security/audit/config returned no output with exit status 0"
  elif [ "$AF_FS_RC" -eq 0 ] && [ -z "$AF_FS_RAW" ]; then
    AF_OBSERVED="not assessed - df -k /audit returned no output with exit status 0"
  else
    AF_CONFIG_SHAPE=$(printf '%s\n' "$AF_CONFIG_RAW" | awk '
      /^[ 	]*$/ { next }
      /^[ 	]*\*/ { next }
      /^[ 	]*freespace[ 	]*=/ {
        found=1
    ...
    AF_FS_KB=$(printf '%s\n' "$AF_FS_RAW" | awk '
      ...
      {
        if (pending != "") total=$1
        else if (NF >= 2) total=$2
        pending=""
        if (total != "" && total ~ /^[0-9][0-9]*$/ && total + 0 > 0) {
          print total
          found=1
          exit
        }
      ...
      END { if (!found) exit 1 }
    ')
    case "$AF_CONFIG_SHAPE" in
      OK\|*)
        AF_BLOCKS=${AF_CONFIG_SHAPE#OK\|}
        AF_FREE_KB=$(awk -v blocks="$AF_BLOCKS" -v unit="$FREESPACE_BLOCK_BYTES" 'BEGIN { printf "%d", blocks * unit / 1024 }')
        ;;
      ABSENT) AF_BLOCKS="" ;;
      *) AF_BLOCKS="" ;;
    esac
    if [ -z "$AF_FS_KB" ]; then
      AF_OBSERVED="not assessed - unparseable df -k /audit output"
    elif [ "$AF_CONFIG_SHAPE" = BAD ]; then
      AF_OBSERVED="not assessed - unparseable freespace value"
    elif [ "$AF_CONFIG_SHAPE" = ABSENT ]; then
      AF_THRESHOLD=$(awk -v total="$AF_FS_KB" 'BEGIN { printf "%d", total * 25 / 100 }')
      AF_STATUS=FAIL
      AF_OBSERVED="no freespace attribute set in /etc/security/audit/config; audit filesystem /audit $AF_FS_KB KB; 25% threshold $AF_THRESHOLD KB"
    else
      AF_THRESHOLD=$(awk -v total="$AF_FS_KB" 'BEGIN { printf "%d", total * 25 / 100 }')
      AF_OBSERVED="freespace = $AF_BLOCKS (512-byte blocks) = $AF_FREE_KB KB; audit filesystem /audit $AF_FS_KB KB; 25% threshold $AF_THRESHOLD KB"
      if [ "$AF_FREE_KB" -lt "$AF_THRESHOLD" ]; then
        AF_STATUS=FAIL
      else
        AF_STATUS=PASS
      fi
  ...
  case "$AF_STATUS" in
    PASS)
      AF_MEANING="The audit filesystem retains at least 25% of its capacity for audit records when the trail reaches 75% full."
      AF_FIX="n/a"
      ;;
    FAIL)
      AF_MEANING="The audit filesystem may fill before the configured audit reserve warns an administrator, stopping the record of what happened."
      AF_FIX="raise freespace in the bin stanza of /etc/security/audit/config to at least 25% of the audit filesystem, then restart auditing with /usr/sbin/audit shutdown followed by /usr/sbin/audit start."
      ;;
    *)
      AF_MEANING="PTxray did not obtain trustworthy evidence for the audit filesystem freespace reserve."
      AF_FIX="verify the audit configuration and filesystem evidence as root, then rerun PTxray."
      ;;
  esac

  add security stig_audit_freespace "Audit filesystem freespace reserve" \
    "$AF_STATUS" med "$AF_OBSERVED" "$AF_MEANING" "$AF_FIX" "stig:V-215241"
}
