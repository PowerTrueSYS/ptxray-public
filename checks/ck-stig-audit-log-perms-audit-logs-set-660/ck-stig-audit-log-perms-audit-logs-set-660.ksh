...
AIXRAY_TOOL=ck-stig-audit-log-perms-audit-logs-set-660


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  typeset raw raw_rc rc config trail_dir result
  raw=$(aix audit_trail_config grep trail /etc/security/audit/config); raw_rc=$?
  if [ "$raw_rc" -ge 2 ]; then
    add security stig_audit_log_perms_audit_logs_set_660 "Audit logs on the AIX system must be set to 660 or less permissive." NOT_ASSESSED med "not assessed - probe audit_trail_config failed (rc $raw_rc)" "Audit log evidence was not obtained." "chmod 660 <auditlog file>" "stig:V-215245"; return
  fi
  if [ "$raw_rc" -eq 0 ] && [ -z "$raw" ]; then
    add security stig_audit_log_perms_audit_logs_set_660 "Audit logs on the AIX system must be set to 660 or less permissive." NOT_ASSESSED med "not assessed - probe audit_trail_config returned no output with rc 0" "Audit log evidence was not obtained." "chmod 660 <auditlog file>" "stig:V-215245"; return
  fi
  config=$(printf '%s\n' "$raw" | awk '
    /^[ \t]*\*/ { next }
    /^[ \t]*trail[ \t]*=/ {
      x=$0; sub(/^[ \t]*trail[ \t]*=[ \t]*/, "", x); sub(/[ \t]*$/, "", x)
      if (x ~ /^".*"$/) { sub(/^"/, "", x); sub(/"$/, "", x) }
      if (value == "") value=x
    }
    END {
      if (value !~ /\//) print "ABSENT"
      else { sub(/\/+$/, "", value); d=value; sub(/\/[^\/]*$/, "", d); if (d=="") d="/"; print d }
    }')
  if [ "$config" = ABSENT ]; then
    add security stig_audit_log_perms_audit_logs_set_660 "Audit logs on the AIX system must be set to 660 or less permissive." FAIL med "no audit trail configured in /etc/security/audit/config" "Audit trail configuration is missing." "chmod 660 <auditlog file>" "stig:V-215245"; return
  fi
  trail_dir=$config
  result=$(aix audit_log_ls ls -ln "$trail_dir"); rc=$?
  if [ "$rc" -ne 0 ]; then
    add security stig_audit_log_perms_audit_logs_set_660 "Audit logs on the AIX system must be set to 660 or less permissive." NOT_ASSESSED med "not assessed - probe audit_log_ls failed (rc $rc)" "Audit log evidence was not obtained." "chmod 660 <auditlog file>" "stig:V-215245"; return
  fi
  if [ -z "$result" ]; then
    add security stig_audit_log_perms_audit_logs_set_660 "Audit logs on the AIX system must be set to 660 or less permissive." NOT_ASSESSED med "not assessed - probe audit_log_ls returned no output with rc 0" "Audit log evidence was not obtained." "chmod 660 <auditlog file>" "stig:V-215245"; return
  fi
  result=$(printf '%s\n' "$result" | awk -v dir="$trail_dir" '
    function oct(n,s,p,d){s="";p=512;while(p>=1){d=int(n/p);s=s d;n-=d*p;p/=8}return s}
    function mode(m,i,v,c,p){v=0;for(i=2;i<=10;i++){if(i<=4)p=64;else if(i<=7)p=8;else p=1;c=substr(m,i,1);if(c=="r")v=v+4*p;if(c=="w")v=v+2*p;if(c=="x")v=v+p;if(c=="s"||c=="S"){if(i==4)v=v+2048;else if(i==7)v=v+1024;else v=v+512;if(c=="s")v=v+p}if(c=="t"||c=="T"){v=v+512;if(c=="t")v=v+p}}return v}
    function bad(v,i,p){p=1;for(i=0;i<12;i++){if(int(v/p)%2&&int(432/p)%2==0)return 1;p*=2}return 0}
    $1=="total"||$NF=="."||$NF==".." {next}
    {seen++;m=substr($1,1,10);if((length($1)!=10&&length($1)!=11)||(length($1)==11&&substr($1,11,1)!~/[+.]/)||m!~ /^[^[:space:]][rwx-][rwx-][-rwxSs][-rwx-][-rwx-][-rwxSs][-rwx-][-rwx-][-rwxTt]$/){skipped++;next};parsed++;if($1~/^-/){n++;v=mode(m);if(bad(v)){b++;if(b<=5)names=names (names?", ":"") $NF "(" oct(v) ")"}}}
    END{if(parsed==0&&seen>0)print "BAD";else if(n==0)print "EMPTY";else {if(b>5)names=names " (+" (b-5) " more)";print n "|" b "|" names "|" skipped}}'
  )
  case "$result" in
    BAD) add security stig_audit_log_perms_audit_logs_set_660 "Audit logs on the AIX system must be set to 660 or less permissive." NOT_ASSESSED med "not assessed - unparseable output from probe audit_log_ls" "Audit log evidence was not trustworthy." "chmod 660 <auditlog file>" "stig:V-215245" ;;
    EMPTY) add security stig_audit_log_perms_audit_logs_set_660 "Audit logs on the AIX system must be set to 660 or less permissive." FAIL med "audit trail directory $trail_dir contains no audit log files" "No audit log files were found." "chmod 660 <auditlog file>" "stig:V-215245" ;;
    *) count=$(printf '%s' "$result"|cut -d'|' -f1); badcount=$(printf '%s' "$result"|cut -d'|' -f2); names=$(printf '%s' "$result"|cut -d'|' -f3); skipped=$(printf '%s' "$result"|cut -d'|' -f4)
       suffix=""; [ "${skipped:-0}" -gt 0 ] && suffix=" (skipped $skipped unparseable line(s))"
       if [ "$badcount" -gt 0 ]; then obs="$badcount of $count audit log file(s) in $trail_dir more permissive than 660: $names$suffix"; add security stig_audit_log_perms_audit_logs_set_660 "Audit logs on the AIX system must be set to 660 or less permissive." FAIL med "$obs" "Audit log files are too permissive." "chmod 660 <auditlog file>" "stig:V-215245"
       else add security stig_audit_log_perms_audit_logs_set_660 "Audit logs on the AIX system must be set to 660 or less permissive." PASS med "all $count audit log file(s) in $trail_dir are mode 660 or less permissive$suffix" "Audit log files are protected." "chmod 660 <auditlog file>" "stig:V-215245"; fi ;;
  esac
}
