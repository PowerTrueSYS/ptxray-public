...
AIXRAY_TOOL=ck-stig-audit-log-perms-audit-logs


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_audit_log_perms_audit_logs — DISA STIG V-215244.
  # Audit logs must be group-owned by system.  Read-only probes: locate the
  # audit trail with grep(1) on the audit config, then read the group ownership
  # of the trail directory listing.  Every uncertain evidence shape fails to
  # NOT_ASSESSED; determinate absence of a configured trail (no `trail =` line,
  # absent or commented out) is a required-configuration FAIL (a host with no
  # audit trail has not escaped the group-ownership requirement, it has failed
  # to meet it).  A readable trail directory that lists no audit log files is
  # determinate absence in the other direction: nothing exists to violate the
  # requirement, so it passes.
  typeset ACT_RAW ACT_RC AUDIT_TRAIL TRAIL_PARSE_RC trail_dir
  typeset AUDIT_LS_RAW AUDIT_LS_RC AUDIT_LS_PARSED
  typeset AG_STATUS AG_OBSERVED AG_MEANING AG_FIX AG_COUNT AG_OFFENDERS

  ACT_RAW=$(aix audit_config_trail grep trail /etc/security/audit/config)
  ACT_RC=$?

  if [ "$ACT_RC" -eq 0 ] && [ -n "$ACT_RAW" ]; then
    AUDIT_TRAIL=$(printf '%s\n' "$ACT_RAW" | awk '
      /^[ \t]*trail[ \t]*=/ {
    ...
    ')
    TRAIL_PARSE_RC=$?
    if [ "$TRAIL_PARSE_RC" -ne 0 ] || [ -z "$AUDIT_TRAIL" ]; then
      # Probe 1 succeeded (rc 0, non-empty output) but no `trail =` assignment
      # is present, whether the line is missing entirely or disabled with a
      # comment.  That is determinate absence of a configured audit trail: a
      # required-configuration FAIL, not a refusal.
      AG_STATUS=FAIL
      AG_OBSERVED="no audit trail is configured in /etc/security/audit/config; the group-ownership requirement is unmet"
    elif [ "${AUDIT_TRAIL#/}" = "$AUDIT_TRAIL" ]; then
      AG_STATUS=NOT_ASSESSED
      AG_OBSERVED="not assessed - audit trail path '$AUDIT_TRAIL' is not absolute"
    else
      trail_dir=${AUDIT_TRAIL%/*}
      [ -z "$trail_dir" ] && trail_dir=/
      AUDIT_LS_RAW=$(aix audit_log_ls ls -ln $trail_dir)
      AUDIT_LS_RC=$?
      if [ "$AUDIT_LS_RC" -ne 0 ] || [ -z "$AUDIT_LS_RAW" ]; then
        if [ "$AUDIT_LS_RC" -eq 0 ]; then
          AG_STATUS=NOT_ASSESSED
          AG_OBSERVED="not assessed - probe audit_log_ls returned rc 0 with no output"
        else
          AG_STATUS=NOT_ASSESSED
          AG_OBSERVED="not assessed - probe audit_log_ls exited rc $AUDIT_LS_RC"
        fi
      else
        AUDIT_LS_PARSED=$(printf '%s\n' "$AUDIT_LS_RAW" | awk '
          /^[ \t]*$/ { next }
          {
            if ($1 == "total") next
            if (NF < 4) { bad = 1; next }
            ...
            if (group != "system" && group != "0") {
              nbad++
              if (nbad <= 5) {
                if (offenders != "") offenders = offenders " "
                offenders = offenders name "(" group ")"
              }
            }
          }
          END {
            ...
            if (nbad == 0) { print "ok " count; exit }
            if (nbad > 5) {
              more = nbad - 5
              print "bad " offenders " (+" more " more)"
            } else {
              print "bad " offenders
            }
          }
        ')
        case "$AUDIT_LS_PARSED" in
          malformed)
            AG_STATUS=NOT_ASSESSED
            AG_OBSERVED="not assessed - listing of the audit trail directory was malformed"
            ;;
          empty)
            # The configured trail directory is readable but lists no audit log
            # files.  Determinate absence of the subject: nothing exists to
            # violate the group-ownership requirement, so this is PASS.
            AG_STATUS=PASS
            AG_OBSERVED="audit trail directory $trail_dir contains no audit log files; nothing violates the group-ownership requirement"
            ;;
          ok*)
            AG_COUNT=${AUDIT_LS_PARSED#ok }
            AG_STATUS=PASS
            AG_OBSERVED="audit trail directory $trail_dir: all $AG_COUNT files group-owned by system"
            ;;
          bad*)
            AG_OFFENDERS=${AUDIT_LS_PARSED#bad }
            AG_STATUS=FAIL
            AG_OBSERVED="audit trail directory $trail_dir contains files not group-owned by system: $AG_OFFENDERS"
            ;;
          *)
            AG_STATUS=NOT_ASSESSED
            AG_OBSERVED="not assessed - listing of the audit trail directory was unparseable"
            ;;
        esac
      fi
    fi
  elif [ "$ACT_RC" -eq 1 ] && [ -z "$ACT_RAW" ]; then
    AG_STATUS=FAIL
    AG_OBSERVED="no audit trail is configured in /etc/security/audit/config; the group-ownership requirement is unmet"
  else
    if [ "$ACT_RC" -eq 0 ]; then
      AG_OBSERVED="not assessed - probe audit_config_trail returned rc 0 with no output"
    else
      AG_OBSERVED="not assessed - probe audit_config_trail exited rc $ACT_RC"
    fi
    AG_STATUS=NOT_ASSESSED
  fi
  ...
  case "$AG_STATUS" in
    PASS)
      AG_MEANING="Every audit log file is group-owned by system, so no other group can alter or truncate the audit trail."
      AG_FIX="n/a"
      ;;
    FAIL)
      AG_MEANING="An audit log file is group-owned by a non-system group, letting members of that group alter or truncate the record of their own activity."
      AG_FIX="change the group of each affected audit log file with 'chgrp system <auditlog file>'; PTxray only recommends this command."
      ;;
    *)
      AG_MEANING="PTxray did not obtain trustworthy evidence about the group ownership of the audit log files."
      AG_FIX="verify the audit trail configuration and the group ownership of the trail directory listing, then rerun PTxray."
      ;;
  esac

  add security stig_audit_log_perms_audit_logs "Audit log group ownership" \
    "$AG_STATUS" med "$AG_OBSERVED" \
    "$AG_MEANING" "$AG_FIX" "stig:V-215244"
}
