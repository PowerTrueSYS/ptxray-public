...
AIXRAY_TOOL=ck-stig-audit-log-perms


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_audit_log_perms — DISA STIG V-215243.  The trail directory is
  # derived from the last usable trail assignment in the audit config.
  typeset SALP_CONFIG_RAW SALP_CONFIG_RC SALP_TRAIL_INFO SALP_TRAIL SALP_DIR
  typeset SALP_LS_RAW SALP_LS_RC SALP_LS_PARSED SALP_META SALP_HEADER
  typeset SALP_ROWS SALP_BAD SALP_REST SALP_OFFENDERS SALP_STATUS SALP_OBSERVED
  typeset SALP_MEANING SALP_FIX SALP_REASON SALP_PARSE_RC

  SALP_CONFIG_RAW=$(aix audit_config_trail grep trail /etc/security/audit/config)
  SALP_CONFIG_RC=$?
  SALP_REASON=""

  if [ "$SALP_CONFIG_RC" -eq 1 ] && [ -z "$SALP_CONFIG_RAW" ]; then
    SALP_STATUS=FAIL
    SALP_OBSERVED="required audit configuration is absent: no trail directive in /etc/security/audit/config"
  elif [ "$SALP_CONFIG_RC" -ne 0 ]; then
    SALP_STATUS=NOT_ASSESSED
    SALP_OBSERVED="not assessed - probe audit_config_trail failed rc=$SALP_CONFIG_RC"
  elif [ -z "$SALP_CONFIG_RAW" ]; then
    SALP_STATUS=NOT_ASSESSED
    SALP_OBSERVED="not assessed - probe audit_config_trail returned empty output with rc 0"
  else
    SALP_TRAIL_INFO=$(printf '%s\n' "$SALP_CONFIG_RAW" | awk '
      /^[ \t]*#/ { next }
      /^[ \t]*$/ { next }
      /^[ \t]*trail[ \t]*=/ {
        value=$0
      ...
      END {
        ...
        else if (last == "") print "empty"
        else print "path:" last
      }')
    case "$SALP_TRAIL_INFO" in
      absent)
        SALP_STATUS=FAIL
        SALP_OBSERVED="required audit configuration is absent: no trail directive in /etc/security/audit/config"
        ;;
      empty)
        SALP_STATUS=NOT_ASSESSED
        SALP_OBSERVED="not assessed - trail directive present but value is empty"
        ;;
      path:*)
        SALP_TRAIL=${SALP_TRAIL_INFO#path:}
        SALP_DIR=${SALP_TRAIL%/*}
        [ -n "$SALP_DIR" ] || SALP_DIR=/
        SALP_LS_RAW=$(aix audit_dir_ls ls -ln "$SALP_DIR")
        SALP_LS_RC=$?
        if [ "$SALP_LS_RC" -ne 0 ]; then
          SALP_STATUS=NOT_ASSESSED
          SALP_OBSERVED="not assessed - probe audit_dir_ls failed rc=$SALP_LS_RC"
        elif [ -z "$SALP_LS_RAW" ]; then
          SALP_STATUS=NOT_ASSESSED
          SALP_OBSERVED="not assessed - probe audit_dir_ls returned empty output with rc 0"
        else
          SALP_LS_PARSED=$(printf '%s\n' "$SALP_LS_RAW" | awk '
            /^[ \t]*$/ { next }
            /^total[ \t]+[0-9]+[ \t]*$/ { header=1; next }
            {
              if (NF < 9 || $3 !~ /^[0-9][0-9]*$/) { badshape=1; next }
              rows++
              if ($3 != 0) {
                bad++
                print "offender:" $NF ":" $3
            ...
            END {
              if (badshape) exit 1
              print "summary:" header ":" rows ":" bad
            }')
          SALP_PARSE_RC=$?
          if [ "$SALP_PARSE_RC" -ne 0 ] || [ -z "$SALP_LS_PARSED" ]; then
            SALP_STATUS=NOT_ASSESSED
            SALP_OBSERVED="not assessed - unparseable ls -ln output"
          else
            SALP_META=$(printf '%s\n' "$SALP_LS_PARSED" | tail -1)
            SALP_REST=${SALP_META#summary:}
            SALP_HEADER=${SALP_REST%%:*}
            SALP_REST=${SALP_REST#*:}
            SALP_ROWS=${SALP_REST%%:*}
            SALP_BAD=${SALP_REST#*:}
            SALP_OFFENDERS=$(printf '%s\n' "$SALP_LS_PARSED" | sed '$d' | sed 's/^offender://' | sort)
            if [ "$SALP_ROWS" -eq 0 ] && [ "$SALP_HEADER" -ne 1 ]; then
              SALP_STATUS=NOT_ASSESSED
              SALP_OBSERVED="not assessed - unparseable ls -ln output"
            elif [ "$SALP_ROWS" -eq 0 ]; then
              SALP_STATUS=FAIL
              SALP_OBSERVED="required audit configuration is absent: trail directory $SALP_DIR contains no audit log files"
            elif [ "$SALP_BAD" -eq 0 ]; then
              SALP_STATUS=PASS
              SALP_OBSERVED="audit trail directory $SALP_DIR: all $SALP_ROWS file(s) owned by uid 0 (root)"
            else
              SALP_STATUS=FAIL
              SALP_OBSERVED="audit trail directory $SALP_DIR: $SALP_BAD file(s) not owned by root:"
              SALP_OFFENDER_COUNT=0
              SALP_MORE=0
              for SALP_OFFENDER in $SALP_OFFENDERS; do
                SALP_OFFENDER_COUNT=$((SALP_OFFENDER_COUNT + 1))
                if [ "$SALP_OFFENDER_COUNT" -le 5 ]; then
                  SALP_OBSERVED="$SALP_OBSERVED ${SALP_OFFENDER%%:*}(uid ${SALP_OFFENDER#*:});"
                else
                  SALP_MORE=$((SALP_MORE + 1))
                fi
              done
              [ "$SALP_MORE" -eq 0 ] || SALP_OBSERVED="$SALP_OBSERVED (+$SALP_MORE more)"
            fi
          fi
        fi
        ;;
      *)
        SALP_STATUS=NOT_ASSESSED
        SALP_OBSERVED="not assessed - unparseable audit configuration trail directive"
        ;;
    esac
  fi
  ...
  case "$SALP_STATUS" in
    PASS)
      SALP_MEANING="Every audit log found in the derived audit trail directory is owned by root."
      SALP_FIX="n/a"
      ;;
    FAIL)
      SALP_MEANING="An audit log owned by another account can be edited or truncated by that account, destroying the record of its activity."
      SALP_FIX="chown root <auditlog file>"
      ;;
    NOT_APPLICABLE)
      SALP_MEANING="This audit-log ownership rule does not apply."
      SALP_FIX="n/a"
      ;;
    *)
      SALP_MEANING="PTxray did not obtain trustworthy evidence for audit-log ownership."
      SALP_FIX="verify the audit trail configuration and listing as root, then rerun PTxray."
      ;;
  esac

  add security stig_audit_log_perms "Audit logs on the AIX system must be owned by root." \
    "$SALP_STATUS" med "$SALP_OBSERVED" "$SALP_MEANING" "$SALP_FIX" "stig:V-215243"
}
