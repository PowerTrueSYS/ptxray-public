...
AIXRAY_TOOL=ck-stig-audit-objects


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_audit_objects — DISA STIG V-215246 (SV-215246r1013689_rule,
  # AIX7-00-002016, severity medium).  Native ksh88 re-implementation of the
  # read-only checks DISA's stig_audit_check.sh performs.  Auditing must be on,
  # the stig_aud_class audit class must be defined with the STIG event list,
  # every user must carry that class, and the eight required audited-object
  # stanzas must be present in /etc/security/audit/objects.  All probes are
  # read-only; the fix text below is a recommendation only, never applied.
  typeset MIN_STIG_EVENTS=80
  typeset SAO_AS_RAW SAO_AS_RC SAO_CF_RAW SAO_CF_RC
  typeset SAO_UA_RAW SAO_UA_RC SAO_OS_RAW SAO_OS_RC
  typeset SAO_OF_RAW SAO_OF_RC
  typeset SAO_AUDIT SAO_CLASS SAO_EVENT_CNT
  typeset SAO_UA_STATE SAO_USERS SAO_OBJ_DEFS
  typeset SAO_DEF_STR SAO_DEF_COUNT SAO_TMP SAO_TMP2
  typeset SAO_STATUS SAO_OBSERVED SAO_MEANING SAO_FIX
  ...
  # Probe 1: audit subsystem state.
  SAO_AS_RAW=$(aix audit_state audit query)
  SAO_AS_RC=$?
  if [ "$SAO_AS_RC" -ne 0 ] || [ -z "$SAO_AS_RAW" ]; then
    add security stig_audit_objects "Audit record generation" NOT_ASSESSED med \
        "not assessed - audit query returned no usable output (rc $SAO_AS_RC)" \
        "PTxray could not confirm whether the audit subsystem is on." \
        "run 'audit query' as root and rerun PTxray." \
        "stig:V-215246"
    return
  fi

  # Probe 2: audit configuration file.
  SAO_CF_RAW=$(aix config_file cat /etc/security/audit/config)
  SAO_CF_RC=$?
  if [ "$SAO_CF_RC" -ne 0 ] || [ -z "$SAO_CF_RAW" ]; then
    add security stig_audit_objects "Audit record generation" NOT_ASSESSED med \
        "not assessed - audit configuration returned no usable output (rc $SAO_CF_RC)" \
        "PTxray could not read the audit subsystem configuration." \
        "verify /etc/security/audit/config is readable by root and rerun PTxray." \
        "stig:V-215246"
    return
  fi

  # Probe 3: per-user auditclasses.
  SAO_UA_RAW=$(aix user_auditclasses lsuser -c -a auditclasses ALL)
  SAO_UA_RC=$?
  if [ "$SAO_UA_RC" -ne 0 ] || [ -z "$SAO_UA_RAW" ]; then
    add security stig_audit_objects "Audit record generation" NOT_ASSESSED med \
        "not assessed - user auditclasses query returned no usable output (rc $SAO_UA_RC)" \
        "PTxray could not read every user's auditclasses attribute." \
        "verify 'lsuser -c -a auditclasses ALL' succeeds as root and rerun PTxray." \
        "stig:V-215246"
    return
  fi

  # Subject presence: /etc/security/audit/objects must exist.  Absence is a
  # determinate finding for this required-configuration rule, not a refusal;
  # the file body is only read after a successful stat.
  SAO_OS_RAW=$(aix objects_stat ls -ld /etc/security/audit/objects)
  SAO_OS_RC=$?
  if [ "$SAO_OS_RC" -ne 0 ] || [ -z "$SAO_OS_RAW" ]; then
    SAO_DEF_STR="/etc/security/audit/objects missing"
    SAO_DEF_COUNT=1
  else
    SAO_OF_RAW=$(aix objects_file cat /etc/security/audit/objects)
    SAO_OF_RC=$?
    if [ "$SAO_OF_RC" -ne 0 ] || [ -z "$SAO_OF_RAW" ]; then
      add security stig_audit_objects "Audit record generation" NOT_ASSESSED med \
          "not assessed - could not read /etc/security/audit/objects (rc $SAO_OF_RC)" \
          "PTxray found the audited-objects file but could not read its contents." \
          "verify /etc/security/audit/objects is readable by root and rerun PTxray." \
          "stig:V-215246"
      return
    fi
  fi

  # Condition 1: auditing is on.
  SAO_AUDIT=$(printf '%s\n' "$SAO_AS_RAW" | awk '
    { line = tolower($0)
      if (line ~ /auditing[ \t]+on/) on = 1
      else if (line ~ /auditing[ \t]+off/) off = 1
      else if (line ~ /^audit[ \t]/) banner = 1 }
    END { if (on) print "on"
          else if (off) print "off"
          else if (banner) print "banner"
          else print "unknown" }')
  case "$SAO_AUDIT" in
    on) : ;;
    unknown)
      add security stig_audit_objects "Audit record generation" NOT_ASSESSED med \
          "not assessed - unrecognized audit query output" \
          "PTxray could not interpret the audit query output." \
          "verify the 'audit query' output shape and rerun PTxray." \
          "stig:V-215246"
      return
      ;;
    *)
      SAO_DEF_STR="${SAO_DEF_STR:+$SAO_DEF_STR; }auditing is not on"
      SAO_DEF_COUNT=$((SAO_DEF_COUNT + 1))
      ;;
  esac

  # Condition 2: stig_aud_class is defined with the STIG event list.  Join
  # continuation lines (trailing backslash) and count comma-separated events.
  SAO_CLASS=$(printf '%s\n' "$SAO_CF_RAW" | awk '
    /^[ \t]*classes:[ \t]*$/ { in_classes = 1; next }
    /^[ \t]*[A-Za-z_][A-Za-z0-9_]*:[ \t]*$/ { in_classes = 0; next }
    in_classes && /^[ \t]*[A-Za-z_][A-Za-z0-9_]*[ \t]*=/ {
      line = $0
      while (line ~ /\\[ \t]*$/) {
      ...
      name = a[1]
      gsub(/[ \t]/, "", name)
      if (name == "stig_aud_class") {
        value = a[2]
        gsub(/[ \t\\]/, "", value)
        cnt = split(value, ev, ",")
        n = 0
        for (i = 1; i <= cnt; i++) if (ev[i] != "") n++
        print "defined@" n
        found = 1
      }
    }
    END { if (!found) print "absent" }')
  if [ "$SAO_CLASS" = "absent" ]; then
    SAO_DEF_STR="${SAO_DEF_STR:+$SAO_DEF_STR; }audit class stig_aud_class not defined in /etc/security/audit/config"
    SAO_DEF_COUNT=$((SAO_DEF_COUNT + 1))
  else
    SAO_EVENT_CNT=${SAO_CLASS#defined@}
    if [ "$SAO_EVENT_CNT" -lt "$MIN_STIG_EVENTS" ]; then
      SAO_DEF_STR="${SAO_DEF_STR:+$SAO_DEF_STR; }audit class stig_aud_class defines only $SAO_EVENT_CNT events"
      SAO_DEF_COUNT=$((SAO_DEF_COUNT + 1))
    fi
  fi

  # Condition 3: every user carries stig_aud_class.
  SAO_UA_STATE=$(printf '%s\n' "$SAO_UA_RAW" | awk '
    BEGIN { FS = ":" }
    /^[ \t]*#/ { next }
    /^[ \t]*$/ { next }
    {
      ...
      gsub(/^[ \t]+|[ \t]+$/, "", classes)
      total++
      if (classes == "") { missing[++m] = name; next }
      n = split(classes, c, ",")
      has = 0
      for (i = 1; i <= n; i++) {
        gsub(/^[ \t]+|[ \t]+$/, "", c[i])
        if (c[i] == "stig_aud_class") has = 1
      }
      if (!has) missing[++m] = name
    }
    END {
      if (total == 0) { print "unparseable"; exit }
      if (m == 0) { print "ok@" total; exit }
      printf "missing@%d@%d@", total, m
      for (i = 1; i <= m && i <= 5; i++) {
        if (i > 1) printf ","
        printf "%s", missing[i]
      }
      if (m > 5) printf " (+%d more)", m - 5
      print ""
    }')
  if [ "$SAO_UA_STATE" = "unparseable" ]; then
    add security stig_audit_objects "Audit record generation" NOT_ASSESSED med \
        "not assessed - unrecognized user auditclasses output" \
        "PTxray could not parse any user auditclasses record." \
        "verify 'lsuser -c -a auditclasses ALL' output and rerun PTxray." \
        "stig:V-215246"
    return
  elif [ "${SAO_UA_STATE#ok@}" != "$SAO_UA_STATE" ]; then
    SAO_USERS=${SAO_UA_STATE#ok@}
  else
    SAO_TMP=${SAO_UA_STATE#missing@}
    SAO_USERS=${SAO_TMP%%@*}
    SAO_TMP=${SAO_TMP#*@}
    SAO_TMP2=${SAO_TMP#*@}
    SAO_DEF_STR="${SAO_DEF_STR:+$SAO_DEF_STR; }users without stig_aud_class: $SAO_TMP2"
    SAO_DEF_COUNT=$((SAO_DEF_COUNT + 1))
  fi

  # Condition 4: the eight required audited objects (nine mode/label triples).
  if [ -n "$SAO_OF_RAW" ]; then
    SAO_OBJ_DEFS=$(printf '%s\n' "$SAO_OF_RAW" | awk '
      function req(pp, mm, ll) {
        n++
        rpath[n] = pp
        rmode[n] = mm
        rlabel[n] = ll
      ...
      BEGIN {
        req("/etc/security/environ", "w", "S_ENVIRON_WRITE")
        req("/etc/security/group", "w", "S_GROUP_WRITE")
        req("/etc/group", "w", "S_GROUP_WRITE")
        req("/etc/security/limits", "w", "S_LIMITS_WRITE")
        req("/etc/security/login.cfg", "w", "S_LOGIN_WRITE")
        req("/etc/security/passwd", "r", "S_PASSWD_READ")
        req("/etc/security/passwd", "w", "S_PASSWD_WRITE")
        req("/etc/security/user", "w", "S_USER_WRITE")
        req("/etc/security/audit/config", "w", "AUD_CONFIG_WR")
      }
      /^\/[^ \t:]*:[ \t]*$/ {
        path = $0
        sub(/^[ \t]*/, "", path)
      ...
      {
        if (path != "" && $0 ~ /^[ \t]*[rw][ \t]*=[ \t]*.[A-Za-z0-9_]+.[ \t]*$/) {
          s = $0
          gsub(/[ \t]/, "", s)
          split(s, a, "=")
          mode = a[1]
          label = a[2]
          gsub(/^[^A-Za-z0-9_]+|[^A-Za-z0-9_]+$/, "", label)
          seen[path SUBSEP mode SUBSEP label] = 1
        }
      }
      END {
        for (i = 1; i <= n; i++)
          if (!seen[rpath[i] SUBSEP rmode[i] SUBSEP rlabel[i]])
            printf "%s missing %s=\"%s\"\n", rpath[i], rmode[i], rlabel[i]
      }')
    if [ -n "$SAO_OBJ_DEFS" ]; then
      SAO_TMP=$(printf '%s\n' "$SAO_OBJ_DEFS" | awk '
        /^[ \t]*$/ { next }
        { if (n++) printf "; "; printf "%s", $0 }
        END { print "" }')
      SAO_DEF_STR="${SAO_DEF_STR:+$SAO_DEF_STR; }$SAO_TMP"
      SAO_TMP2=$(printf '%s\n' "$SAO_OBJ_DEFS" | awk 'END { print NR }')
      SAO_DEF_COUNT=$((SAO_DEF_COUNT + SAO_TMP2))
    fi
  fi

  # Verdict assembly: any recorded deficiency is FAIL at the STIG severity;
  # otherwise the audit configuration satisfies all four conditions.
  if [ "$SAO_DEF_COUNT" -gt 0 ]; then
    SAO_TMP=$(printf '%s\n' "$SAO_DEF_STR" | awk '
      { n = split($0, d, /; /)
        out = ""
        for (i = 1; i <= n && i <= 4; i++) {
        ...
        if (n > 4) out = out "; (+" (n - 4) " more)"
        print out }')
    SAO_OBSERVED="$SAO_DEF_COUNT deficiencies: $SAO_TMP"
    SAO_STATUS=FAIL
    SAO_MEANING="The audit subsystem does not record all required DoD-defined auditable events, so changes to accounts, passwords, groups, limits, or the audit configuration itself can leave no trail."
    SAO_FIX="Follow the STIG stig_audit_config.txt procedure: define the stig_aud_class audit class in /etc/security/audit/config, add it to every user's auditclasses, add the eight required stanzas to /etc/security/audit/objects, and start auditing. PTxray only recommends these actions; it never applies them."
  else
    SAO_OBSERVED="auditing on; stig_aud_class defined ($SAO_EVENT_CNT events); all $SAO_USERS users audited; all 8 required audit objects present"
    SAO_STATUS=PASS
    SAO_MEANING="Auditing is on, the STIG audit class is defined and applied to every user, and the required audited objects are present, so DoD-defined events are recorded."
    SAO_FIX="n/a"
  fi

  add security stig_audit_objects "Audit record generation" "$SAO_STATUS" med \
      "$SAO_OBSERVED" "$SAO_MEANING" "$SAO_FIX" "stig:V-215246"
}
