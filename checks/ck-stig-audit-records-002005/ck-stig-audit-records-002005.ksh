...
AIXRAY_TOOL=ck-stig-audit-records-002005


function standalone_check {
_AIXRAY_SESSION_KEYS=""
#!/usr/bin/ksh
# ck-stig-audit-records-002005 — DISA STIG V-215239.
# Read-only audit trail and auditpr status evidence; every uncertain shape fails closed.
typeset fid category controls expected fix meaning
typeset _cfg _cfg_rc _pr_rc _trail _pr _total _missing trail status severity observed
fid=stig_audit_records_002005
category=security
controls=stig:V-215239
expected="auditpr -helRtcp displays the status (result) column for every audit record in the configured trail"
fix="# /usr/sbin/audit shutdown; # /usr/sbin/audit start"
meaning="AIX audit records must show whether each audited event succeeded or failed, so an investigator can establish what an intruder achieved."
status=NOT_ASSESSED
severity=med
observed="not assessed - audit evidence was not obtained"

# grep rc 1 with empty output is a determinate no-match: the required trail is
# absent and therefore FAIL. rc 0 with empty output is a swallowed error.
_cfg=$(aix audit_config_trail grep trail /etc/security/audit/config)
_cfg_rc=$?
if [ "$_cfg_rc" -eq 1 ] && [ -z "$_cfg" ]; then
  status=FAIL
  observed="no active trail stanza in /etc/security/audit/config"
elif [ "$_cfg_rc" -ne 0 ]; then
  status=NOT_ASSESSED
  observed="not assessed - grep trail /etc/security/audit/config exited $_cfg_rc"
elif [ -z "$_cfg" ]; then
  status=NOT_ASSESSED
  observed="not assessed - grep trail /etc/security/audit/config returned rc 0 with empty output"
else
  _trail=$(printf '%s\n' "$_cfg" | awk '
    /^[ \t]*\*/ { next }
    /trail[ \t]*=/ {
  ...
  if [ -z "$_trail" ]; then
    ...
    trail=$_trail
    _pr=$(aix auditpr_status /usr/sbin/auditpr -i "$trail" -helRtcp)
    _pr_rc=$?
    if [ "$_pr_rc" -eq 2 ] && [ -z "$_pr" ]; then
      # Required-record rule: auditpr rc 2 with no stdout is an unreadable trail.
      status=FAIL
      observed="audit trail $trail; auditpr -i $trail exited 2 with no records"
    elif [ "$_pr_rc" -ne 0 ]; then
      status=NOT_ASSESSED
      observed="not assessed - auditpr -i $trail exited $_pr_rc"
    elif [ -z "$_pr" ]; then
      status=NOT_ASSESSED
      observed="not assessed - auditpr -i $trail returned rc 0 with empty output"
    else
      _total=$(printf '%s\n' "$_pr" | awk '
        NF {
          if ($1 == "event" && $2 == "login") next
          total++
          value=tolower($3)
          if (value !~ /^(ok|fail)$/) missing++
        }
        END { print total "|" missing }
      ')
      _missing=${_total#*\|}
      _total=${_total%%\|*}
      if [ "$_total" -gt 0 ] && [ "$_missing" -eq 0 ]; then
        status=PASS
        observed="audit trail $trail; auditpr -helRtcp shows a status for all $_total records"
      elif [ "$_missing" -gt 0 ]; then
        status=FAIL
        observed="audit trail $trail; $_missing of $_total auditpr records show no status"
      else
        status=NOT_ASSESSED
        observed="not assessed - auditpr output has no audit records to inspect"
      fi
    fi
  fi
fi

add "$category" "$fid" "$expected" "$status" "$severity" "$observed" \
    "$meaning" "$fix" "$controls"
}
