...
AIXRAY_TOOL=ck-stig-audit-records-002006


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_audit_records_002006 — DISA STIG V-215240: full-text recording of
  # privileged commands.  PARTIAL-AUTO: whether auditpr -v shows full-text
  # detail for the audited events is decidable on-host; whether the event
  # classes generating records cover every privileged command the site/ISSO
  # mandates is not decidable here and renders NOT_ASSESSED, never PASS.
  # The fix text below is advisory only; PTxray never runs it.
  typeset AAR_CFG_RAW AAR_CFG_RC AAR_PRP_RAW AAR_PRP_RC
  typeset AAR_TRAIL AAR_COUNTS
  typeset AAR_HEADER AAR_DETAIL AAR_RECORDS AAR_CMD AAR_PRIV
  typeset AAR_STATUS AAR_SEVERITY AAR_OBSERVED AAR_MEANING AAR_FIX

  AAR_CFG_RAW=$(aix audit_config_trail grep trail /etc/security/audit/config)
  AAR_CFG_RC=$?
  AAR_PRP_RAW=$(aix auditpr_trail_verbose /usr/sbin/auditpr -i /audit/trail -v)
  AAR_PRP_RC=$?
  ...
  # Ladder 1: a failing config probe (rc != 0) is never absence.
  if [ "$AAR_CFG_RC" -ne 0 ]; then
    AAR_STATUS=NOT_ASSESSED
    AAR_OBSERVED="not assessed - probe audit_config_trail failed (rc=$AAR_CFG_RC)"
  # Ladder 2: rc 0 with empty config output is a swallowed error, not absence.
  elif [ -z "$AAR_CFG_RAW" ]; then
    AAR_STATUS=NOT_ASSESSED
    AAR_OBSERVED="not assessed - probe audit_config_trail returned no output with rc 0 (swallowed error, not absence)"
  else
    # Ladder 3: a configured trail file is required for records to exist.
    # Comment lines in the audit config begin with '*' and are ignored.
    AAR_TRAIL=$(printf '%s\n' "$AAR_CFG_RAW" | awk '
      NF == 0 { next }
      {
        if ($1 == "*") { next }
    ...
    if [ -z "$AAR_TRAIL" ]; then
      AAR_STATUS=FAIL
      AAR_OBSERVED="no audit trail configured in /etc/security/audit/config - audit records are not being written"
    # auditpr rc 2 with empty stdout is an empty or absent trail: the
    # control requires records to be produced, so this is FAIL. rc 127
    # (command missing) and a permission message on stdout stay on the
    # generic failed-probe path below.
    elif [ "$AAR_PRP_RC" -eq 2 ] && [ -z "$AAR_PRP_RAW" ]; then
      AAR_STATUS=FAIL
      AAR_OBSERVED="no audit records in the trail"
    elif [ "$AAR_PRP_RC" -ne 0 ]; then
      AAR_STATUS=NOT_ASSESSED
      AAR_OBSERVED="not assessed - probe auditpr_trail_verbose failed (rc=$AAR_PRP_RC)"
    elif [ -z "$AAR_PRP_RAW" ]; then
      AAR_STATUS=NOT_ASSESSED
      AAR_OBSERVED="not assessed - probe auditpr_trail_verbose returned no output with rc 0 (swallowed error, not absence)"
    else
      # Ladder 4-7: classify the auditpr -v output in one POSIX awk pass.
      # A header line's first field is `event`; a record line is non-blank,
      # starts at column 1 and has >= 6 fields, its last field being the
      # command text; a `-v` detail line is non-blank and starts with
      # whitespace (detected via the first field not starting at column 1).
      AAR_COUNTS=$(printf '%s\n' "$AAR_PRP_RAW" | awk '
        NF == 0 { next }
        {
          if (index($0, $1) > 1) { detail++; next }
          if ($1 == "event") { header++; next }
          if (NF >= 6) {
            records++
            cmd = $NF
            if (cmd != "" && cmd != "-") cmd_records++
            if ($1 ~ /^PROC_Execute$/ || $1 ~ /^USER_SU$/ || $1 ~ /_(Change|Write)$/) {
              if (cmd != "" && cmd != "-") priv_records++
            }
          }
        }
        END {
          printf "%d %d %d %d %d\n", header, detail, records, cmd_records, priv_records
        }')
      set -A AAR_N $AAR_COUNTS
      AAR_HEADER=${AAR_N[0]}
      AAR_DETAIL=${AAR_N[1]}
      AAR_RECORDS=${AAR_N[2]}
      AAR_CMD=${AAR_N[3]}
      AAR_PRIV=${AAR_N[4]}
      if [ "$AAR_HEADER" -eq 0 ] && [ "$AAR_RECORDS" -eq 0 ]; then
        AAR_STATUS=NOT_ASSESSED
        AAR_OBSERVED="not assessed - auditpr -v output not parseable (no event header or records)"
      elif [ "$AAR_DETAIL" -eq 0 ] && [ "$AAR_CMD" -eq 0 ]; then
        AAR_STATUS=FAIL
        AAR_OBSERVED="auditpr -v shows no full-text command recording for audited events"
      elif [ "$AAR_PRIV" -eq 0 ]; then
        AAR_STATUS=NOT_ASSESSED
        AAR_OBSERVED="not assessed - privileged-command class coverage is site/ISSO-determined: auditpr -v full-text detail is present but the sampled trail holds no privileged-command records, so which classes generate them cannot be decided from the host"
      else
        AAR_STATUS=PASS
        AAR_OBSERVED="auditpr -v shows full-text command recording ($AAR_PRIV privileged-command records with detail) for trail $AAR_TRAIL"
      fi
    fi
  fi
  ...
  case "$AAR_STATUS" in
    PASS)
      AAR_MEANING="The audit trail records the full text of privileged commands, so an investigator can reconstruct exactly what a privileged action executed."
      AAR_FIX="n/a"
      ;;
    FAIL)
      AAR_MEANING="The audit trail does not record the full text of privileged commands, so it can show that a privileged action happened but not what was actually run."
      AAR_FIX="Enable and restart auditing so privileged commands are recorded in full text: /usr/sbin/audit shutdown; /usr/sbin/audit start"
      ;;
    NOT_APPLICABLE)
      AAR_SEVERITY=low
      AAR_MEANING="The rule text carries no Not Applicable clause; this branch is defined for convention parity only."
      AAR_FIX="n/a"
      ;;
    *)
      AAR_MEANING="PTxray could not decide whether privileged commands are recorded in full text; the remaining coverage clause is site/ISSO-determined."
      AAR_FIX="verify the audit configuration and trail as root, confirm which audit event classes generate privileged-command records, then rerun PTxray."
      ;;
  esac

  add security stig_audit_records_002006 \
      "AIX must produce audit records containing the full-text recording of privileged commands." \
      "$AAR_STATUS" "$AAR_SEVERITY" "$AAR_OBSERVED" \
      "$AAR_MEANING" "$AAR_FIX" "stig:V-215240"
}
