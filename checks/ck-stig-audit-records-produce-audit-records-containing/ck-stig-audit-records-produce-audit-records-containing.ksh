...
AIXRAY_TOOL=ck-stig-audit-records-produce-audit-records-containing


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  typeset AR_CFG_RAW AR_CFG_RC AR_TRAIL_PATH AR_PARSE_RC
  typeset AR_LOG_RAW AR_LOG_RC AR_COUNTS AR_RECORDS AR_DEFICIENT
  typeset AR_STATUS AR_OBSERVED AR_MEANING AR_FIX

  AR_CFG_RAW=$(aix audit_config_trail grep trail /etc/security/audit/config)
  AR_CFG_RC=$?
  AR_TRAIL_PATH=""
  if [ "$AR_CFG_RC" -eq 1 ] && [ -z "$AR_CFG_RAW" ]; then
    AR_STATUS=FAIL
    AR_OBSERVED="no audit trail is configured in /etc/security/audit/config"
  elif [ "$AR_CFG_RC" -ne 0 ]; then
    AR_STATUS=NOT_ASSESSED
    AR_OBSERVED="not assessed - audit config probe failed (rc $AR_CFG_RC)"
  elif [ -z "$AR_CFG_RAW" ]; then
    AR_STATUS=NOT_ASSESSED
    AR_OBSERVED="not assessed - audit config probe returned no output (rc 0)"
  else
    AR_TRAIL_PATH=$(printf '%s\n' "$AR_CFG_RAW" | awk '
      /trail/ {
        p = index($0, "=")
        if (p == 0) next
        v = substr($0, p + 1)
        ...
        if (substr(v, 1, 1) == "\"") {
          gsub(/^\"/, "", v)
          gsub(/\"$/, "", v)
        } else if (substr(v, 1, 1) == sprintf("%c", 39)) {
          gsub("^" sprintf("%c", 39), "", v)
          gsub(sprintf("%c", 39) "$", "", v)
        }
        if (v != "") { print v; exit }
    ...
    ')
    AR_PARSE_RC=$?
    if [ "$AR_PARSE_RC" -ne 0 ] || [ -z "$AR_TRAIL_PATH" ]; then
      AR_STATUS=NOT_ASSESSED
      AR_OBSERVED="not assessed - could not parse trail path from /etc/security/audit/config"
    else
      AR_LOG_RAW=$(aix auditpr_trail_fields /usr/sbin/auditpr -i "$AR_TRAIL_PATH" -helRtcp)
      AR_LOG_RC=$?
      if [ "$AR_LOG_RC" -eq 2 ] && [ -z "$AR_LOG_RAW" ]; then
        AR_STATUS=FAIL
        AR_OBSERVED="audit trail $AR_TRAIL_PATH; auditpr -i $AR_TRAIL_PATH exited 2 with no records"
      elif [ "$AR_LOG_RC" -ne 0 ]; then
        AR_STATUS=NOT_ASSESSED
        AR_OBSERVED="not assessed - auditpr probe failed (rc $AR_LOG_RC)"
      elif [ -z "$AR_LOG_RAW" ]; then
        AR_STATUS=NOT_ASSESSED
        AR_OBSERVED="not assessed - auditpr probe returned no output (rc 0)"
      else
        AR_COUNTS=$(printf '%s\n' "$AR_LOG_RAW" | awk '
          NF {
            if ($1 == "event") next
            records++
            if (NF < 6 || $2 == "" || $2 == "-" || $2 == "?" || $NF !~ /^[0-9][0-9]*$/)
              deficient++
          }
          END { print records + 0 "|" deficient + 0 }
        ')
        AR_RECORDS=${AR_COUNTS%%\|*}
        AR_DEFICIENT=${AR_COUNTS#*\|}
        if [ "$AR_RECORDS" -eq 0 ]; then
          AR_STATUS=NOT_ASSESSED
          AR_OBSERVED="not assessed - no audit records parsed from $AR_TRAIL_PATH"
        elif [ "$AR_DEFICIENT" -gt 0 ]; then
          AR_STATUS=FAIL
          AR_OBSERVED="$AR_DEFICIENT of $AR_RECORDS audit records lack a usable login or process id in $AR_TRAIL_PATH"
        else
          AR_STATUS=PASS
          AR_OBSERVED="$AR_TRAIL_PATH: $AR_RECORDS audit records carry both login and process id"
        fi
      fi
    fi
  fi
  ...
  case "$AR_STATUS" in
    PASS)
      AR_MEANING="Audit records identify the user and process associated with each parsed event, supporting reliable attribution during an investigation."
      AR_FIX="n/a"
      ;;
    FAIL)
      AR_MEANING="Audit records that do not identify the user and process behind an event cannot reliably attribute activity during an investigation."
      AR_FIX="restart auditing so records carry subject identity: /usr/sbin/audit shutdown; /usr/sbin/audit start"
      ;;
    *)
      AR_MEANING="PTxray did not obtain trustworthy evidence that audit records identify the user and process associated with events."
      AR_FIX="restart auditing so records carry subject identity: /usr/sbin/audit shutdown; /usr/sbin/audit start"
      ;;
  esac
  add security stig_audit_records_produce_audit_records_containing \
    ...
    "$AR_STATUS" med "$AR_OBSERVED" "$AR_MEANING" "$AR_FIX" "stig:V-215238"
}
