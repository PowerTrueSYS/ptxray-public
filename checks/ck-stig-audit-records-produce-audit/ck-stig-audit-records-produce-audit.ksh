...
AIXRAY_TOOL=ck-stig-audit-records-produce-audit


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_audit_records_produce_audit — DISA STIG V-215237.
  # AIX must produce audit records containing information to establish where
  # the events occurred.  Read-only bounded probe: read the trail file named in
  # /etc/security/audit/config, then run auditpr -v against that log and require
  # event detail to be displayed.  A missing trail configuration is a FAIL (a
  # required-configuration rule): with no configured trail no audit records can
  # be produced at all.
  typeset AUD_CFG_RAW AUD_CFG_RC AUD_TRAIL AUD_PR_FILE
  typeset AUD_PR_RAW AUD_PR_RC AUD_PR_SHAPE
  typeset AUD_STATUS AUD_OBSERVED AUD_MEANING AUD_FIX

  AUD_CFG_RAW=$(aix audit_config grep trail /etc/security/audit/config)
  AUD_CFG_RC=$?

  # Locate the trail file named in the audit configuration.  grep(1) exit
  # codes are meaningful here: 0 a match, 1 no trail line, 2 missing/unreadable.
  AUD_TRAIL=""
  if [ "$AUD_CFG_RC" -eq 0 ]; then
    if [ -z "$AUD_CFG_RAW" ]; then
      AUD_TRAIL="empty"
    else
      AUD_TRAIL=$(printf '%s\n' "$AUD_CFG_RAW" | awk '
        /^[ \t]*$/ { next }
  ...
  case "$AUD_TRAIL" in
    empty)
      AUD_STATUS=NOT_ASSESSED
      AUD_OBSERVED="not assessed - audit configuration probe returned no rows with rc 0"
      ;;
    malformed)
      AUD_STATUS=NOT_ASSESSED
      AUD_OBSERVED="not assessed - trail entry in /etc/security/audit/config is malformed (no file path)"
      ;;
    trail:*)
      AUD_PR_FILE=${AUD_TRAIL#trail:}
      AUD_PR_RAW=$(aix auditpr_verbose /usr/sbin/auditpr -i "$AUD_PR_FILE" -v)
      AUD_PR_RC=$?
      if [ "$AUD_PR_RC" -eq 2 ]; then
        # auditpr ran against the configured trail and reported it cannot
        # produce records (rc 2, typically a missing or unreadable trail
        # file). That is determinate noncompliance, not a missing probe.
        AUD_STATUS=FAIL
        AUD_OBSERVED="auditpr -v on $AUD_PR_FILE exited 2 with no records; audit records cannot establish where events occurred"
      elif [ "$AUD_PR_RC" -ne 0 ]; then
        AUD_STATUS=NOT_ASSESSED
        AUD_OBSERVED="not assessed - auditpr probe failed (rc=$AUD_PR_RC)"
      elif [ -z "$AUD_PR_RAW" ]; then
        AUD_STATUS=NOT_ASSESSED
        AUD_OBSERVED="not assessed - auditpr returned no rows with rc 0"
      else
        AUD_PR_SHAPE=$(printf '%s\n' "$AUD_PR_RAW" | awk '
          /^[ \t]*$/ { next }
          {
            if (skip < 2) { skip++; next }
            if ($0 ~ /^[ \t]/) { detail = 1; next }
          ...
          END {
            ...
            if (detail) { print "detail"; exit }
            print "nodetail"
          }')
        case "$AUD_PR_SHAPE" in
          detail)
            AUD_STATUS=PASS
            AUD_OBSERVED="auditpr -v shows event detail for the configured trail $AUD_PR_FILE, establishing where events occurred"
            ;;
          nodetail)
            AUD_STATUS=FAIL
            AUD_OBSERVED="auditpr -v on $AUD_PR_FILE displayed no event detailed information; audit records do not establish where events occurred"
            ;;
          *)
            AUD_STATUS=NOT_ASSESSED
            AUD_OBSERVED="not assessed - auditpr output was empty or unparseable"
            ;;
        esac
      fi
      ;;
    *)
      # No trail file named (grep rc 1) or the audit configuration is missing
      # or unreadable (grep rc 2): the audit trail subject is absent, so the
      # required configuration is unmet.
      if [ "$AUD_CFG_RC" -eq 1 ] || [ "$AUD_CFG_RC" -eq 2 ]; then
        AUD_STATUS=FAIL
        AUD_OBSERVED="audit trail not configured: /etc/security/audit/config is missing or unreadable (grep rc=$AUD_CFG_RC); no audit records can establish where events occurred"
      elif [ "$AUD_CFG_RC" -eq 0 ]; then
        AUD_STATUS=FAIL
        AUD_OBSERVED="audit trail not configured: no trail file named in /etc/security/audit/config; no audit records can establish where events occurred"
      else
        AUD_STATUS=NOT_ASSESSED
        AUD_OBSERVED="not assessed - audit configuration probe failed (rc=$AUD_CFG_RC)"
      fi
      ;;
  esac
  ...
  case "$AUD_STATUS" in
    PASS)
      AUD_MEANING="AIX auditpr -v displays event detail, so audit records establish where events occurred as required by DISA STIG V-215237."
      AUD_FIX="n/a"
      ;;
    FAIL)
      AUD_MEANING="The audit trail is not configured, or auditpr -v does not display event detailed information, so audit records cannot establish where events occurred."
      AUD_FIX="restart the audit subsystem with '/usr/sbin/audit shutdown; /usr/sbin/audit start'; PTxray only recommends this command"
      ;;
    *)
      AUD_MEANING="PTxray did not obtain trustworthy evidence that audit records establish where events occurred."
      AUD_FIX="verify the audit configuration and the auditpr run as root, then rerun PTxray"
      ;;
  esac

  add security stig_audit_records_produce_audit "AIX audit records establish where events occurred" \
    "$AUD_STATUS" med "$AUD_OBSERVED" "$AUD_MEANING" "$AUD_FIX" "stig:V-215237"
}
