...
AIXRAY_TOOL=ck-stig-audit-records


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_audit_records — DISA STIG V-215236.
  # AIX must produce audit records that carry date, time, and event type.
  # Read-only probes only; the fix text is a recommendation, never executed.
  typeset AR_QUERY AR_QUERY_RC AR_CFG AR_CFG_RC AR_PR
  typeset AR_PR_MARKER AR_STATE AR_STATUS AR_OBSERVED AR_MEANING AR_FIX AR_PR_SHAPE

  AR_QUERY=$(aix audit_query audit query)
  AR_QUERY_RC=$?
  AR_CFG=$(aix audit_config grep trail /etc/security/audit/config)
  AR_CFG_RC=$?
  # The trail can be gigabytes; bound the read to a header window.  Append the
  # producer's rc through a trailing marker so head cannot swallow it: an
  # absent marker means the window filled (bounded read, producer healthy),
  # a present non-zero marker is a probe failure.
  AR_PR=$( { aix auditpr /usr/sbin/auditpr -i /var/audit/audittrail -helRtcp; echo "__ar_rc=$?"; } | head -n 31 )

  AR_STATUS=""
  AR_STATE=""

  # --- auditing on/off from `audit query` ---
  if [ "$AR_QUERY_RC" -ne 0 ]; then
    AR_STATUS=NOT_ASSESSED
    AR_OBSERVED="not assessed - audit query probe failed (rc=$AR_QUERY_RC)"
  elif [ -z "$AR_QUERY" ]; then
    AR_STATUS=NOT_ASSESSED
    AR_OBSERVED="not assessed - audit query returned no status line"
  else
    AR_STATE=$(printf '%s\n' "$AR_QUERY" | awk '
      tolower($0) ~ /auditing/ {
        if (tolower($0) ~ /off|disabled/) { print "off"; exit }
        print "on"; exit
      }
    ')
    if [ -z "$AR_STATE" ]; then
      AR_STATUS=NOT_ASSESSED
      AR_OBSERVED="not assessed - audit query output did not state the auditing state"
    elif [ "$AR_STATE" = "off" ]; then
      AR_STATUS=FAIL
      AR_OBSERVED="audit query reports auditing off"
    fi
  fi

  # --- auditing is on: the trail must be configured and carry event types ---
  if [ -z "$AR_STATUS" ]; then
    if [ "$AR_CFG_RC" -eq 1 ] && [ -z "$AR_CFG" ]; then
      AR_STATUS=FAIL
      AR_OBSERVED="audit trail not configured (no trail line in /etc/security/audit/config)"
    elif [ "$AR_CFG_RC" -ne 0 ]; then
      AR_STATUS=NOT_ASSESSED
      AR_OBSERVED="not assessed - audit config probe failed (rc=$AR_CFG_RC)"
    elif [ -z "$AR_CFG" ]; then
      AR_STATUS=NOT_ASSESSED
      AR_OBSERVED="not assessed - audit config probe returned no rows"
    fi
  fi

  # --- auditpr must show the event type column ---
  if [ -z "$AR_STATUS" ]; then
    AR_PR_MARKER=$(printf '%s\n' "$AR_PR" | awk '
      /^__ar_rc=[0-9][0-9]*$/ { print substr($0, index($0, "=") + 1); exit }
    ')
    if [ -n "$AR_PR_MARKER" ]; then
      AR_PR=$(printf '%s\n' "$AR_PR" | awk '
        /^__ar_rc=[0-9][0-9]*$/ { next }
        { print }
      ')
      if [ "$AR_PR_MARKER" -ne 0 ]; then
        AR_STATUS=NOT_ASSESSED
        AR_OBSERVED="not assessed - auditpr probe failed (rc=$AR_PR_MARKER)"
      fi
    fi
    if [ -z "$AR_STATUS" ]; then
      if [ -z "$AR_PR" ]; then
        AR_STATUS=NOT_ASSESSED
        AR_OBSERVED="not assessed - auditpr returned no rows"
      else
        AR_PR_SHAPE=$(printf '%s\n' "$AR_PR" | awk '
          NF {
            lines++
            if (lines == 1) {
              if (tolower($0) ~ /event/) header_ok = 1
              next
            }
          }
          END {
            if (header_ok && lines >= 2) print "event-ok"
            else if (header_ok) print "header-only"
            else print "unparseable"
          }')
        case "$AR_PR_SHAPE" in
          event-ok)
            AR_STATUS=PASS
            AR_OBSERVED="auditing on; auditpr shows the audit trail with date, time, and event type columns"
            ;;
          header-only)
            AR_STATUS=FAIL
            AR_OBSERVED="audit trail exists but auditpr displayed no event type rows"
            ;;
          *)
            AR_STATUS=NOT_ASSESSED
            AR_OBSERVED="not assessed - auditpr output was not readable as event rows"
            ;;
        esac
      fi
    fi
  ...
  case "$AR_STATUS" in
    PASS)
      AR_MEANING="AIX is producing audit records that carry date, time, and event type."
      AR_FIX="n/a"
      ;;
    FAIL)
      AR_MEANING="AIX audit records do not carry the required date, time, and event type information, so the audit trail cannot establish what events occurred."
      AR_FIX="/usr/sbin/audit shutdown; /usr/sbin/audit start"
      ;;
    *)
      AR_MEANING="PTxray did not obtain trustworthy evidence that AIX audit records carry date, time, and event type."
      AR_FIX="correct the probe capture problem and rerun PTxray"
      ;;
  esac

  add security stig_audit_records "Audit records content" \
    "$AR_STATUS" med "$AR_OBSERVED" "$AR_MEANING" "$AR_FIX" "stig:V-215236"
}
