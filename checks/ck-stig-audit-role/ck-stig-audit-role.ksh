...
AIXRAY_TOOL=ck-stig-audit-role


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_audit_role — DISA STIG V-215252.
  # A role must carry all three aix.security.*audit authorizations so an
  # assigned ISSO or designated SA can change the auditing performed on
  # operating system components in near real time.  Reproduce the check
  # content's three-grep filter over `lsrole ALL` rows (one row per role,
  # authorizations comma-separated).  Uncertain evidence shapes fail closed.
  typeset SAR_RAW SAR_RC SAR_MATCH SAR_MATCH_RC
  typeset SAR_STATUS SAR_OBSERVED SAR_MEANING SAR_FIX

  SAR_RAW=$(aix lsrole_all lsrole ALL)
  SAR_RC=$?

  if [ "$SAR_RC" -ne 0 ]; then
    SAR_STATUS=NOT_ASSESSED
    SAR_OBSERVED="not assessed - lsrole ALL probe failed (rc=$SAR_RC)"
  elif [ -z "$SAR_RAW" ]; then
    SAR_STATUS=NOT_ASSESSED
    SAR_OBSERVED="not assessed - lsrole ALL returned empty output (rc=0)"
  else
    SAR_MATCH=$(printf '%s\n' "$SAR_RAW" | grep "aix.security.audit" | grep "aix.security.user.audit" | grep "aix.security.role.audit")
    SAR_MATCH_RC=$?
    if [ "$SAR_MATCH_RC" -eq 0 ] && [ -n "$SAR_MATCH" ]; then
      SAR_STATUS=PASS
      SAR_OBSERVED="a role lists aix.security.audit, aix.security.user.audit, and aix.security.role.audit"
    else
      SAR_STATUS=FAIL
      SAR_OBSERVED="no role carries all of aix.security.audit, aix.security.user.audit, and aix.security.role.audit"
    fi
  fi

  case "$SAR_STATUS" in
    PASS)
      SAR_MEANING="A role with the three audit authorizations exists, so an assigned ISSO or designated SA can change the auditing performed on operating system components in near real time."
      SAR_FIX="n/a"
      ;;
    FAIL)
      SAR_MEANING="No role carries all three audit authorizations, so no assigned ISSO or designated SA can change the audit configuration as DISA STIG V-215252 requires."
      SAR_FIX="create an audit administration role with 'mkrole authorizations=\"aix.security.audit,aix.security.user.audit,aix.security.role.audit\" auditadm'; PTxray only recommends this command."
      ;;
    *)
      SAR_MEANING="PTxray did not obtain trustworthy role-authorization evidence for DISA STIG V-215252."
      SAR_FIX="run 'lsrole ALL' as root to confirm the role authorizations, then rerun PTxray."
      ;;
  esac

  add security stig_audit_role "Audit configuration role (ISSO/SA)" \
    "$SAR_STATUS" med "$SAR_OBSERVED" "$SAR_MEANING" "$SAR_FIX" \
    "stig:V-215252"
}
