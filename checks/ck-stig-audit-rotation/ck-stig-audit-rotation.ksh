...
AIXRAY_TOOL=ck-stig-audit-rotation


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_audit_rotation — DISA STIG V-215256, daily AIX audit trail rotation.
  typeset ROT_CRON ROT_CRON_RC ROT_TRAIL ROT_TRAIL_RC ROT_TODAY ROT_TODAY_RC
  typeset ROTATION_FOUND ROT_STATUS ROT_OBSERVED ROT_MEANING ROT_FIX ROT_COUNT

  ROT_CRON=$(aix audit_crontab crontab -l)
  ROT_CRON_RC=$?
  ROT_TRAIL=$(aix audit_trail_dir ls -1 /audit)
  ROT_TRAIL_RC=$?
  ROT_TODAY=$(aix audit_today date '+%Y-%m-%d')
  ROT_TODAY_RC=$?
  ROTATION_FOUND=0
  if [ "$ROT_CRON_RC" -eq 0 ] && [ -n "$ROT_CRON" ] &&
      printf '%s\n' "$ROT_CRON" | awk '
        /audit/ && /(rotate|archive)/ { found=1 }
        END { exit !found }
      '; then
    ROTATION_FOUND=1
  fi

  ROT_STATUS=NOT_ASSESSED
  ROT_OBSERVED="not assessed - audit rotation evidence was unavailable"
  if [ "$ROT_CRON_RC" -ne 0 ] && [ "$ROT_CRON_RC" -ne 2 ]; then
    ROT_OBSERVED="not assessed - crontab probe failed (rc=$ROT_CRON_RC)"
  elif [ "$ROT_TRAIL_RC" -ne 0 ] && [ "$ROT_TRAIL_RC" -ne 2 ]; then
    ROT_OBSERVED="not assessed - audit trail directory probe failed (rc=$ROT_TRAIL_RC)"
  elif [ "$ROT_TODAY_RC" -ne 0 ] || [ -z "$ROT_TODAY" ]; then
    ROT_OBSERVED="not assessed - date probe failed or returned empty output"
  elif [ "$ROT_CRON_RC" -eq 0 ] && [ -z "$ROT_CRON" ] &&
      [ "$ROT_TRAIL_RC" -eq 0 ] && [ -z "$ROT_TRAIL" ]; then
    ROT_STATUS=NOT_ASSESSED
    ROT_OBSERVED="not assessed - crontab and audit trail probes returned empty output (swallowed error)"
  elif [ "$ROTATION_FOUND" -eq 1 ]; then
    ROT_STATUS=PASS
    ROT_OBSERVED="daily audit rotation entry found in crontab"
  elif [ "$ROTATION_FOUND" -eq 0 ] &&
      { [ "$ROT_CRON_RC" -eq 2 ] || [ "$ROT_CRON_RC" -eq 0 ]; } &&
      [ "$ROT_TRAIL_RC" -eq 2 ]; then
    ROT_STATUS=FAIL
    ROT_OBSERVED="crontab contains no daily audit rotation entry and the audit trail directory is absent/empty"
  elif [ "$ROT_TRAIL_RC" -eq 0 ] && [ -n "$ROT_TRAIL" ]; then
    ROT_COUNT=$(printf '%s\n' "$ROT_TRAIL" | awk 'NF { count++ } END { print count + 0 }')
    if [ "$ROT_COUNT" -ne 1 ]; then
      ROT_STATUS=FAIL
      ROT_OBSERVED="audit trail directory contains $ROT_COUNT files; exactly one dated-today archive is required"
    elif printf '%s\n' "$ROT_TRAIL" | awk -v today="$ROT_TODAY" '
        NF == 1 && ($1 ~ today || $1 ~ ("audit_" today)) { ok=1 }
        END { exit !ok }
      '; then
      ROT_STATUS=NOT_ASSESSED
      ROT_OBSERVED="not assessed - one dated-today audit archive is present, but the SA/ISSO manual rotation portion cannot be proven from these probes"
    else
      ROT_STATUS=FAIL
      ROT_OBSERVED="the sole audit trail archive is not dated today ($ROT_TODAY)"
    fi
  elif [ "$ROTATION_FOUND" -eq 0 ] &&
      { [ "$ROT_CRON_RC" -eq 2 ] || [ "$ROT_CRON_RC" -eq 0 ]; } &&
      [ "$ROT_TRAIL_RC" -eq 0 ] && [ -z "$ROT_TRAIL" ]; then
    ROT_STATUS=FAIL
    ROT_OBSERVED="no daily rotation entry was found and the audit trail directory is empty"
  fi

  case "$ROT_STATUS" in
    PASS)
      ROT_MEANING="A daily automated audit-log rotation entry was found."; ROT_FIX="n/a" ;;
    FAIL)
      ROT_MEANING="AIX audit logs are not shown to be rotated daily, so audit evidence may grow unbounded or fail to be archived."; ROT_FIX="Configure a cron job or other automated process to rotate the audit logs on a daily basis." ;;
    *)
      ROT_MEANING="PTxray could not prove the complete daily audit-log rotation requirement."; ROT_FIX="Configure a cron job or other automated process to rotate the audit logs on a daily basis." ;;
  esac
  add security stig_audit_rotation "AIX audit logs rotated daily" \
      "$ROT_STATUS" med "$ROT_OBSERVED" "$ROT_MEANING" "$ROT_FIX" \
      "stig:V-215256"
}
