...
AIXRAY_TOOL=ck-stig-audit-tool-perms-audit-tools-set-4550


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_audit_tool_perms_audit_tools_set_4550 - DISA STIG V-215250.
  # AIX audit tools must be mode 4550 or less; auditldap alone may be 500.
  # A missing required audit tool is an unmet required configuration (FAIL).
  typeset ATP_RAW ATP_RC ATP_LDAP ATP_LDAP_RC ATP_TOOLS_STATE ATP_LDAP_STATE
  typeset ATP_STATUS ATP_OBSERVED ATP_MEANING ATP_FIX ATP_DETAIL

  ATP_RAW=$(aix audit_tools_ls /usr/bin/ls -l /usr/sbin/audit /usr/sbin/auditbin /usr/sbin/auditcat /usr/sbin/auditconv /usr/sbin/auditmerge /usr/sbin/auditpr /usr/sbin/auditselect /usr/sbin/auditstream)
  ATP_RC=$?
  ATP_LDAP=$(aix audit_ldap_ls /usr/bin/ls -l /usr/sbin/auditldap)
  ATP_LDAP_RC=$?

  ATP_STATUS=""
  ATP_OBSERVED=""

  if [ "$ATP_RC" -ne 0 ] && [ -z "$ATP_RAW" ]; then
    ATP_STATUS=NOT_ASSESSED
    ATP_OBSERVED="not assessed - audit tool enumeration failed (rc=$ATP_RC)"
  elif [ "$ATP_RC" -eq 0 ] && [ -z "$ATP_RAW" ]; then
    ATP_STATUS=NOT_ASSESSED
    ATP_OBSERVED="not assessed - audit tool enumeration returned no rows"
  else
    ATP_TOOLS_STATE=$(printf '%s\n' "$ATP_RAW" | awk '
      BEGIN {
        req[1] = "/usr/sbin/audit"
        req[2] = "/usr/sbin/auditbin"
        req[3] = "/usr/sbin/auditcat"
        req[4] = "/usr/sbin/auditconv"
        req[5] = "/usr/sbin/auditmerge"
        req[6] = "/usr/sbin/auditpr"
        req[7] = "/usr/sbin/auditselect"
        req[8] = "/usr/sbin/auditstream"
      }
      /^[ \t]*$/ { next }
      index($0, "ldap") { next }
      {
        ...
        if (length("" mode) != 10) { bad = 1; next }
        path = $NF
        if (index(path, "/usr/sbin/audit") != 1) { bad = 1; next }
        n = 0
        if (substr(mode, 4, 1) ~ /[sS]/) n += 4000
        if (substr(mode, 7, 1) ~ /[sS]/) n += 2000
        if (substr(mode, 10, 1) ~ /[tT]/) n += 1000
        if (substr(mode, 2, 1) == "r") n += 400
        if (substr(mode, 3, 1) == "w") n += 200
        if (substr(mode, 4, 1) ~ /[xsS]/) n += 100
        if (substr(mode, 5, 1) == "r") n += 40
        if (substr(mode, 6, 1) == "w") n += 20
        if (substr(mode, 7, 1) ~ /[xsS]/) n += 10
        if (substr(mode, 8, 1) == "r") n += 4
        if (substr(mode, 9, 1) == "w") n += 2
        if (substr(mode, 10, 1) ~ /[xtT]/) n += 1
        # 4550 allows only setuid + owner r-x + group r-x. Any other bit
        # (setgid, sticky, owner/group write, any other-class bit) is more
        # permissive no matter what the bits sum to numerically.
        over = 0
        if (substr(mode, 7, 1) ~ /[sS]/) over = 1
        if (substr(mode, 10, 1) ~ /[tT]/) over = 1
        if (substr(mode, 3, 1) == "w") over = 1
        if (substr(mode, 6, 1) == "w") over = 1
        if (substr(mode, 8, 1) == "r") over = 1
        if (substr(mode, 9, 1) == "w") over = 1
        if (substr(mode, 10, 1) ~ /[xtT]/) over = 1
        seen[path] = 1
        if (over > 0 && worst == "") { worst = path; worstmode = n }
      }
      END {
        if (bad || rows == 0) { print "MALFORMED"; exit }
        for (i = 1; i <= 8; i++) {
          if ((req[i]) in seen) continue
          print "MISSING:" req[i]
          exit
        }
        if (worst != "") { print "BADMODE:" worst ":" worstmode; exit }
        print "OK"
      }
    ')
    case "$ATP_TOOLS_STATE" in
      MALFORMED)
        ATP_STATUS=NOT_ASSESSED
        ATP_OBSERVED="not assessed - audit tool listing was malformed or unparseable"
        ;;
      MISSING:*)
        ATP_DETAIL=${ATP_TOOLS_STATE#MISSING:}
        ATP_STATUS=FAIL
        ATP_OBSERVED="required audit tool $ATP_DETAIL is not installed; the audit tool set is incomplete"
        ;;
      BADMODE:*)
        ATP_DETAIL=${ATP_TOOLS_STATE#BADMODE:}
        ATP_STATUS=FAIL
        ATP_OBSERVED="audit tool ${ATP_DETAIL%%:*} mode ${ATP_DETAIL#*:} is more permissive than 4550"
        ;;
      OK) ;;
      *)
        ATP_STATUS=NOT_ASSESSED
        ATP_OBSERVED="not assessed - audit tool enumeration produced an unrecognized result"
        ;;
    esac
  fi

  if [ -z "$ATP_STATUS" ]; then
    if [ "$ATP_LDAP_RC" -ne 0 ] && [ -z "$ATP_LDAP" ]; then
      ATP_STATUS=NOT_ASSESSED
      ATP_OBSERVED="not assessed - auditldap listing failed (rc=$ATP_LDAP_RC)"
    elif [ "$ATP_LDAP_RC" -eq 0 ] && [ -z "$ATP_LDAP" ]; then
      ATP_STATUS=NOT_ASSESSED
      ATP_OBSERVED="not assessed - auditldap listing returned no rows"
    else
      ATP_LDAP_STATE=$(printf '%s\n' "$ATP_LDAP" | awk '
        /^[ \t]*$/ { next }
        {
          rows++
          if (rows > 1 || NF < 9) { bad = 1; next }
          mode = $1
          if (length("" mode) == 11 && substr(mode, 11, 1) == "+") mode = substr(mode, 1, 10)
          if (length("" mode) != 10) { bad = 1; next }
          path = $NF
          if (path != "/usr/sbin/auditldap") { bad = 1; next }
          n = 0
          if (substr(mode, 4, 1) ~ /[sS]/) n += 4000
          if (substr(mode, 7, 1) ~ /[sS]/) n += 2000
          if (substr(mode, 10, 1) ~ /[tT]/) n += 1000
          ...
          if (substr(mode, 9, 1) == "w") n += 2
          if (substr(mode, 10, 1) ~ /[xtT]/) n += 1
          # 500 allows only owner r-x; any other bit is more permissive.
          over = 0
          if (substr(mode, 4, 1) ~ /[sS]/) over = 1
          if (substr(mode, 7, 1) ~ /[sS]/) over = 1
          if (substr(mode, 10, 1) ~ /[tT]/) over = 1
          if (substr(mode, 3, 1) == "w") over = 1
          if (substr(mode, 5, 1) == "r") over = 1
          if (substr(mode, 6, 1) == "w") over = 1
          if (substr(mode, 7, 1) ~ /[xsS]/) over = 1
          if (substr(mode, 8, 1) == "r") over = 1
          if (substr(mode, 9, 1) == "w") over = 1
          if (substr(mode, 10, 1) ~ /[xtT]/) over = 1
        }
        END {
          if (bad || rows != 1) { print "MALFORMED"; exit }
          if (over > 0) { print "BADMODE:" path ":" n; exit }
          print "OK"
        }
      ')
      case "$ATP_LDAP_STATE" in
        MALFORMED)
          ATP_STATUS=NOT_ASSESSED
          ATP_OBSERVED="not assessed - auditldap listing was malformed or unparseable"
          ;;
        BADMODE:*)
          ATP_DETAIL=${ATP_LDAP_STATE#BADMODE:}
          ATP_STATUS=FAIL
          ATP_OBSERVED="audit tool ${ATP_DETAIL%%:*} mode ${ATP_DETAIL#*:} is more permissive than 500"
          ;;
        OK)
          ATP_STATUS=PASS
          ATP_OBSERVED="all required audit tools are 4550 or less and auditldap is 500 or less"
          ;;
        *)
          ATP_STATUS=NOT_ASSESSED
          ATP_OBSERVED="not assessed - auditldap listing produced an unrecognized result"
          ;;
      esac
    fi
  fi
  ...
  case "$ATP_STATUS" in
    PASS)
      ATP_MEANING="All required AIX audit tools are installed with mode 4550 or less and auditldap is 500 or less."
      ATP_FIX="n/a"
      ;;
    FAIL)
      ATP_MEANING="An audit tool is installed with a mode more permissive than the STIG ceiling, or a required audit tool is missing."
      ATP_FIX="chmod 4550 <audit tool>; chmod 500 /usr/sbin/auditldap. PTxray only recommends this fix; it does not change permissions."
      ;;
    *)
      ATP_MEANING="PTxray did not obtain trustworthy permission evidence for the AIX audit tools."
      ATP_FIX="verify the audit tool mode with 'ls -l /usr/sbin/audit*' and 'ls -l /usr/sbin/auditldap', then rerun PTxray."
      ;;
  esac

  add security stig_audit_tool_perms_audit_tools_set_4550 "AIX audit tool permissions" \
    "$ATP_STATUS" med "$ATP_OBSERVED" "$ATP_MEANING" "$ATP_FIX" "stig:V-215250"
}
