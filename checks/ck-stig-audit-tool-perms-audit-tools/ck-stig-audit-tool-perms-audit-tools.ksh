...
AIXRAY_TOOL=ck-stig-audit-tool-perms-audit-tools


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_audit_tool_perms_audit_tools — DISA STIG V-215249.
  # AIX audit tools must be group-owned by "audit"; /usr/sbin/auditldap must be
  # group-owned by "security".  Two read-only ls probes; every uncertain
  # evidence shape fails closed to NOT_ASSESSED.
  typeset ATP_AT_RAW ATP_AT_RC ATP_AT_STATE ATP_AT_SHAPE ATP_AT_PARSE_RC
  typeset ATP_AL_RAW ATP_AL_RC ATP_AL_STATE ATP_AL_SHAPE ATP_AL_PARSE_RC
  typeset ATP_STATUS ATP_OBSERVED ATP_MEANING ATP_FIX

  ATP_AT_RAW=$(aix audit_tools_ls /usr/bin/ls -l /usr/sbin/audit*)
  ATP_AT_RC=$?
  ATP_AL_RAW=$(aix auditldap_ls /usr/bin/ls -l /usr/sbin/auditldap)
  ATP_AL_RC=$?

  # --- classify the audit-tools probe (required subjects group audit) ---
  if [ "$ATP_AT_RC" -ne 0 ]; then
    ATP_AT_STATE="failed:rc=$ATP_AT_RC"
  elif [ -z "$ATP_AT_RAW" ]; then
    ATP_AT_STATE=empty
  else
    ATP_AT_SHAPE=$(printf '%s\n' "$ATP_AT_RAW" | awk '
      /^[ \t]*total[ \t]/ { next }
      /^[ \t]*$/ { next }
      {
        if (NF < 9) { bad = 1; next }
        mode = $1
        if (length("" mode) == 11) {
          if (substr(mode, 11, 1) != "+") bad = 1
          else mode = substr(mode, 1, 10)
        }
        if (length("" mode) != 10) bad = 1
        if ($2 !~ /^[0-9][0-9]*$/) bad = 1
        name = $NF
        if (index(name, "ldap") > 0) next
        if (name !~ /^\/usr\/sbin\/audit/) bad = 1
        grp[name] = $4
        seen[name] = 1
      }
      END {
        if (bad) { print "malformed"; exit 1 }
        miss = ""
        wrngrp = ""
        n = split("audit auditbin auditcat auditconv auditmerge auditpr auditselect auditstream", req, " ")
        for (i = 1; i <= n; i++) {
          p = "/usr/sbin/" req[i]
          if (!seen[p]) miss = miss " " p
          else if (grp[p] != "audit") wrngrp = wrngrp " " p
        }
        # Judge every row the probe listed, not just the eight required names:
        # a /usr/sbin/audit* binary added by a later AIX TL must still be
        # group-owned by audit (auditldap is excluded here; the auditldap probe
        # owns its verdict).
        for (p in seen) {
          if (p != "/usr/sbin/auditldap" && grp[p] != "audit") wrngrp = wrngrp " " p
        }
        if (miss != "") { print "missing:" miss; exit 0 }
        if (wrngrp != "") { print "badgroup:" wrngrp; exit 0 }
        print "ok"
      }')
    ATP_AT_PARSE_RC=$?
    if [ "$ATP_AT_PARSE_RC" -ne 0 ] || [ -z "$ATP_AT_SHAPE" ]; then
      ATP_AT_STATE=unparseable
    else
      ATP_AT_STATE=$ATP_AT_SHAPE
    fi
  fi

  # --- classify the auditldap probe (group security) ---
  if [ "$ATP_AL_RC" -ne 0 ]; then
    ATP_AL_STATE="failed:rc=$ATP_AL_RC"
  elif [ -z "$ATP_AL_RAW" ]; then
    ATP_AL_STATE=empty
  else
    ATP_AL_SHAPE=$(printf '%s\n' "$ATP_AL_RAW" | awk '
      /^[ \t]*total[ \t]/ { next }
      /^[ \t]*$/ { next }
      {
        if (NF < 9) { bad = 1; next }
        ...
        if ($2 !~ /^[0-9][0-9]*$/) bad = 1
        name = $NF
        if (name != "/usr/sbin/auditldap") bad = 1
        ldapgrp = $4
      }
      END {
        if (bad) { print "malformed"; exit 1 }
        if (ldapgrp != "security") { print "badgroup:" ldapgrp; exit 0 }
        print "ok"
      }')
    ATP_AL_PARSE_RC=$?
    if [ "$ATP_AL_PARSE_RC" -ne 0 ] || [ -z "$ATP_AL_SHAPE" ]; then
      ATP_AL_STATE=unparseable
    else
      ATP_AL_STATE=$ATP_AL_SHAPE
    fi
  fi

  # --- verdict: fail closed on any unreliable evidence ---
  ATP_STATUS=FAIL
  case "$ATP_AT_STATE" in
    failed:*|empty|unparseable) ATP_STATUS=NOT_ASSESSED ;;
  esac
  case "$ATP_AL_STATE" in
    failed:*|empty|unparseable) ATP_STATUS=NOT_ASSESSED ;;
  esac

  if [ "$ATP_AT_STATE" = ok ] && [ "$ATP_AL_STATE" = ok ]; then
    ATP_STATUS=PASS
  fi

  case "$ATP_STATUS" in
    PASS)
      ATP_OBSERVED="all required audit tools are group-owned by audit and /usr/sbin/auditldap is group-owned by security"
      ;;
    NOT_ASSESSED)
      case "$ATP_AT_STATE|$ATP_AL_STATE" in
        *failed:*)
          ATP_OBSERVED="not assessed - audit tool metadata probe failed (audit tools rc=$ATP_AT_RC, auditldap rc=$ATP_AL_RC)"
          ;;
        *empty*)
          ATP_OBSERVED="not assessed - audit tool metadata probe returned no rows with rc 0"
          ;;
        *)
          ATP_OBSERVED="not assessed - audit tool metadata was unparseable"
          ;;
      esac
      ;;
    *)
      ATP_OBSERVED="audit tool group ownership is not compliant"
      case "$ATP_AT_STATE" in
        missing:*) ATP_OBSERVED="required audit tool(s) not present:${ATP_AT_STATE#missing:}" ;;
        badgroup:*) ATP_OBSERVED="audit tool(s) not group-owned by audit:${ATP_AT_STATE#badgroup:}" ;;
      esac
      case "$ATP_AL_STATE" in
        ok) : ;;
        *) ATP_OBSERVED="$ATP_OBSERVED; /usr/sbin/auditldap is group-owned by ${ATP_AL_STATE#badgroup:}" ;;
      esac
      ;;
  esac
  ...
  case "$ATP_STATUS" in
    PASS)
      ATP_MEANING="All AIX audit tools are group-owned by audit and /usr/sbin/auditldap is group-owned by security, so only authorized groups can access the audit subsystem binaries."
      ATP_FIX="n/a"
      ;;
    FAIL)
      ATP_MEANING="An AIX audit tool is not group-owned as required, so an unauthorized group can access or alter the audit subsystem binaries and its evidence."
      ATP_FIX="chgrp audit <audit tool>; chgrp security /usr/sbin/auditldap — recommendation only; PTxray never executes it."
      ;;
    *)
      ATP_MEANING="PTxray did not obtain trustworthy group-ownership metadata for the AIX audit tools."
      ATP_FIX="verify /usr/sbin/audit* group ownership as root, correct the capture problem, and rerun PTxray."
      ;;
  esac

  add security stig_audit_tool_perms_audit_tools "AIX audit tools group ownership" \
    "$ATP_STATUS" med "$ATP_OBSERVED" \
    "$ATP_MEANING" "$ATP_FIX" "stig:V-215249"
}
