...
AIXRAY_TOOL=ck-stig-audit-tool-perms


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_audit_tool_perms — DISA STIG V-215248.
  # AIX audit tools must be owned by root.  The STIG check-content (manifest
  # 'commands' entry) is ls -l /usr/sbin/audit*|grep -v ldap.  The probe is an
  # argv listing of those binaries (no glob, no pipeline); awk applies the
  # check-content's ldap exclusion so /usr/sbin/auditldap is not an observable
  # subject.  The finding's fix text still covers /usr/sbin/auditldap per the
  # STIG.  Every uncertain evidence shape fails closed: probe failure (rc 127
  # / unreadable), rc0-empty output, or an unparseable listing is
  # NOT_ASSESSED, never PASS.  An empty ldap filter on a readable listing is
  # determinate absence of the required tools (FAIL), never NOT_ASSESSED.
  typeset ATP_RAW ATP_RC ATP_SHAPE ATP_REST ATP_PATH ATP_OWNER
  typeset ATP_STATE ATP_OBSERVED ATP_MEANING ATP_FIX

  ATP_RAW=$(aix audit_tool_ls /usr/bin/ls -l /usr/sbin/audit /usr/sbin/auditbin /usr/sbin/auditcat /usr/sbin/auditconv /usr/sbin/auditmerge /usr/sbin/auditpr /usr/sbin/auditselect /usr/sbin/auditstream)
  ATP_RC=$?

  if [ "$ATP_RC" -eq 127 ]; then
    ATP_STATE=NOT_ASSESSED
    ATP_OBSERVED="not assessed - audit tool listing probe failed (rc=$ATP_RC)"
  elif [ -z "$ATP_RAW" ]; then
    if [ "$ATP_RC" -ne 0 ]; then
      ATP_STATE=NOT_ASSESSED
      ATP_OBSERVED="not assessed - audit tool listing probe failed (rc=$ATP_RC)"
      ...
      ATP_STATE=NOT_ASSESSED
      ATP_OBSERVED="not assessed - audit tool listing returned no rows (rc=0); this is a swallowed error, not an absent install"
    fi
  else
    ATP_SHAPE=$(printf '%s\n' "$ATP_RAW" | awk '
      /^[ \t]*$/ { next }
      index($0, "ldap") { next }
      {
        if (NF < 9) { bad = 1; next }
        path = $NF
        if (path !~ /^\/usr\/sbin\/audit/) { bad = 1; next }
        if (seen[path]) { bad = 1; next }
        seen[path] = 1
        owner[path] = $3
      }
      END {
        ...
        for (i = 1; i <= 8; i++) {
          p = req[i]
          if (!(p in seen)) { print "absent:" p; exit }
          if (owner[p] != "root") { print "owner:" p ":" owner[p]; exit }
        }
        print "ok"
      }')
    case "$ATP_SHAPE" in
      ok)
        ATP_STATE=PASS
        ATP_OBSERVED="all required audit tools under /usr/sbin are owned by root"
        ;;
      absent:*)
        ATP_PATH=${ATP_SHAPE#absent:}
        ATP_STATE=FAIL
        ATP_OBSERVED="required audit tool $ATP_PATH is missing from /usr/sbin; the requirement is unmet"
        ;;
      owner:*)
        ATP_REST=${ATP_SHAPE#owner:}
        ATP_OWNER=${ATP_REST#*:}
        ATP_PATH=${ATP_REST%%:*}
        ATP_STATE=FAIL
        ATP_OBSERVED="audit tool $ATP_PATH is owned by $ATP_OWNER, not root"
        ;;
      *)
        ATP_STATE=NOT_ASSESSED
  ...
  case "$ATP_STATE" in
    PASS)
      ATP_MEANING="All AIX audit tools are owned by root, so an unprivileged account cannot tamper with the audit trail."
      ATP_FIX="n/a"
      ;;
    FAIL)
      ATP_MEANING="An AIX audit tool is owned by a non-root account, so that account could alter or disable the audit trail and evade detection."
      ATP_FIX="change only the owner with 'chown root <audit tool>' (and 'chown root /usr/sbin/auditldap' per the STIG) after validating the files; PTxray only recommends this command."
      ;;
    *)
      ATP_MEANING="PTxray did not obtain a trustworthy listing of AIX audit tool ownership."
      ATP_FIX="verify 'ls -l /usr/sbin/audit*' as root, correct the capture or path problem, and rerun PTxray."
      ;;
  esac

  add security stig_audit_tool_perms "AIX audit tools owner" \
    "$ATP_STATE" med "$ATP_OBSERVED" \
    "$ATP_MEANING" "$ATP_FIX" "stig:V-215248"
}
