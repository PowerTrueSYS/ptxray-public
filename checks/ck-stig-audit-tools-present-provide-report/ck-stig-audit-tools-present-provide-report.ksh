...
AIXRAY_TOOL=ck-stig-audit-tools-present-provide-report


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_audit_tools_present_provide_report — DISA STIG V-215254.
  # AIX's auditpr utility must be present for on-demand audit reporting.
  typeset AUDITPR_RAW AUDITPR_RC AUDITPR_STATUS AUDITPR_OBSERVED

  AUDITPR_RAW=$(aix auditpr_ls ls -l /usr/sbin/auditpr)
  AUDITPR_RC=$?
  AUDITPR_STATUS=NOT_ASSESSED
  AUDITPR_OBSERVED=""

  if [ "$AUDITPR_RC" -eq 0 ] && [ -z "$AUDITPR_RAW" ]; then
    AUDITPR_OBSERVED="not assessed - auditpr probe returned no output with rc=0"
  elif [ "$AUDITPR_RC" -eq 2 ] && [ -z "$AUDITPR_RAW" ]; then
    AUDITPR_STATUS=FAIL
    AUDITPR_OBSERVED="/usr/sbin/auditpr is absent (ls rc=2)"
  elif [ "$AUDITPR_RC" -ne 0 ]; then
    AUDITPR_OBSERVED="not assessed - auditpr probe failed with rc=$AUDITPR_RC"
  elif printf '%s\n' "$AUDITPR_RAW" | awk '{ for (i = 1; i <= NF; i++) if ($i == "/usr/sbin/auditpr") found=1 } END { exit !found }'; then
    AUDITPR_STATUS=PASS
    AUDITPR_OBSERVED="/usr/sbin/auditpr is present"
  else
    AUDITPR_STATUS=FAIL
    AUDITPR_OBSERVED="/usr/sbin/auditpr was not present in the ls listing"
  fi

  case "$AUDITPR_STATUS" in
    PASS)
      add security stig_audit_tools_present_provide_report \
        "Audit report generation utility" PASS med "$AUDITPR_OBSERVED" \
        "AIX provides auditpr for on-demand audit review, analysis, and after-the-fact investigation." \
        "n/a" "stig:V-215254"
      ;;
    FAIL)
      add security stig_audit_tools_present_provide_report \
        "Audit report generation utility" FAIL med "$AUDITPR_OBSERVED" \
        "AIX does not provide the auditpr report generation utility required for on-demand audit review and investigation." \
        "installp -aXYqg -d /dev/cd0 bos.rte.security" "stig:V-215254"
      ;;
    *)
      add security stig_audit_tools_present_provide_report \
        "Audit report generation utility" NOT_ASSESSED med "$AUDITPR_OBSERVED" \
        "PTxray did not obtain trustworthy evidence about the auditpr report generation utility." \
        "verify that /usr/sbin/auditpr is readable and rerun PTxray" "stig:V-215254"
      ;;
  esac
}
