...
AIXRAY_TOOL=ck-stig-audit-tools-present


function standalone_check {
_AIXRAY_SESSION_KEYS=""
# ck_stig_audit_tools_present — DISA STIG V-215242.
# AIX must provide auditselect so audit records can be filtered and reduced
# for on-demand review and after-the-fact investigation.
typeset AUDIT_TOOLS_RAW AUDIT_TOOLS_RC AUDIT_TOOLS_FOUND AUDIT_TOOLS_STATUS
typeset AUDIT_TOOLS_OBSERVED AUDIT_TOOLS_MEANING AUDIT_TOOLS_FIX

AUDIT_TOOLS_RAW=$(aix auditselect_bin ls -l /usr/sbin/auditselect)
AUDIT_TOOLS_RC=$?
AUDIT_TOOLS_FOUND=0

if [ "$AUDIT_TOOLS_RC" -ne 0 ]; then
  if [ "$AUDIT_TOOLS_RC" -eq 2 ]; then
    AUDIT_TOOLS_STATUS=FAIL
    AUDIT_TOOLS_OBSERVED="/usr/sbin/auditselect is absent (ls rc=2)"
  else
    AUDIT_TOOLS_STATUS=NOT_ASSESSED
    AUDIT_TOOLS_OBSERVED="not assessed - auditselect probe failed (rc=$AUDIT_TOOLS_RC)"
  fi
elif [ -z "$AUDIT_TOOLS_RAW" ]; then
  AUDIT_TOOLS_STATUS=NOT_ASSESSED
  AUDIT_TOOLS_OBSERVED="not assessed - auditselect probe returned empty output"
else
  AUDIT_TOOLS_FOUND=$(printf '%s\n' "$AUDIT_TOOLS_RAW" | awk '
    NF >= 9 && substr($1, 1, 1) == "-" && $NF == "/usr/sbin/auditselect" { found++ }
    END { print (found == 1 ? 1 : 0) }
  ')
  if [ "$AUDIT_TOOLS_FOUND" -eq 1 ]; then
    AUDIT_TOOLS_STATUS=PASS
    AUDIT_TOOLS_OBSERVED="/usr/sbin/auditselect is present"
  else
    AUDIT_TOOLS_STATUS=FAIL
    AUDIT_TOOLS_OBSERVED="/usr/sbin/auditselect is absent or its listing did not establish a regular file"
  fi
fi

case "$AUDIT_TOOLS_STATUS" in
  PASS)
    AUDIT_TOOLS_MEANING="The auditselect utility is available to filter and reduce audit records for review."
    AUDIT_TOOLS_FIX="n/a"
    ;;
  FAIL)
    AUDIT_TOOLS_MEANING="The required auditselect utility is absent, so audit records cannot be filtered and reduced as required."
    AUDIT_TOOLS_FIX="installp -aXYqg -d /dev/cd0 bos.rte.security"
    ;;
  *)
    AUDIT_TOOLS_MEANING="PTxray did not obtain trustworthy evidence about the required auditselect utility."
    AUDIT_TOOLS_FIX="verify the auditselect path and rerun PTxray"
    ;;
esac

add security stig_audit_tools_present "Audit record filtering and reduction utility" \
  "$AUDIT_TOOLS_STATUS" med "$AUDIT_TOOLS_OBSERVED" \
  "$AUDIT_TOOLS_MEANING" "$AUDIT_TOOLS_FIX" "stig:V-215242"
}
