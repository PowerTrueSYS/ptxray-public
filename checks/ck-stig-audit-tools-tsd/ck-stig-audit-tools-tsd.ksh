...
AIXRAY_TOOL=ck-stig-audit-tools-tsd


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_audit_tools_tsd — DISA STIG V-215251 (AIX7-00-002028).
  # trustchk -p must report TE=ON and CHKEXEC=ON, and every required audit
  # tool must carry a non-VOLATILE cert_tag/signature/hash_value in
  # /etc/security/tsd/tsd.dat.  Uncertain evidence shapes fail closed.
  typeset TAT_TRUST_RAW TAT_TRUST_RC TAT_TRUST_SHAPE TAT_TRUST_SHAPE_RC
  typeset TAT_TSD_ALL TAT_TSD_RC TAT_TSD_RAW TAT_TSD_HAS_ROWS TAT_TSD_SHAPE TAT_TSD_SHAPE_RC
  typeset TAT_STATUS TAT_OBSERVED TAT_MEANING TAT_FIX TAT_MISSING_SUBJECT

  TAT_TRUST_RAW=$(aix trustchk_p trustchk -p)
  TAT_TRUST_RC=$?
  TAT_TSD_ALL=$(aix tsd_audit_tools awk '/\/usr\/sbin\/audit/ {print; for(i=1; i<=10; i++) {getline; print}}' /etc/security/tsd/tsd.dat)
  TAT_TSD_RC=$?
  TAT_TSD_RAW=$(printf '%s\n' "$TAT_TSD_ALL" | grep -E '/usr/sbin/audit|cert_tag|signature|hash_value')
  TAT_TSD_HAS_ROWS=$(printf '%s\n' "$TAT_TSD_RAW" | awk '/^\/usr\/sbin\/audit[^:]*:/ { print 1; exit }')

  TAT_STATUS=""
  if [ "$TAT_TRUST_RC" -ne 0 ] || [ "$TAT_TSD_RC" -ne 0 ]; then TAT_STATUS=NOT_ASSESSED
    TAT_OBSERVED="not assessed - trustchk/tsd.dat probe evidence incomplete (trustchk rc=$TAT_TRUST_RC, tsd.dat rc=$TAT_TSD_RC)"
    TAT_MEANING="PTxray did not obtain trustworthy evidence of the audit-tool hash protection policy."
    TAT_FIX="verify that trustchk -p and /etc/security/tsd/tsd.dat are readable, then rerun PTxray."
  elif [ -z "$TAT_TRUST_RAW" ] && [ -z "$TAT_TSD_HAS_ROWS" ]; then TAT_STATUS=NOT_ASSESSED
    TAT_OBSERVED="not assessed - trustchk/tsd.dat probe evidence incomplete (trustchk rc=$TAT_TRUST_RC, tsd.dat rc=$TAT_TSD_RC)"
    TAT_MEANING="PTxray did not obtain trustworthy evidence of the audit-tool hash protection policy."
    TAT_FIX="verify that trustchk -p and /etc/security/tsd/tsd.dat are readable, then rerun PTxray."
  elif [ -z "$TAT_TRUST_RAW" ]; then TAT_STATUS=FAIL
    TAT_OBSERVED="trustchk reports TE=OFF or CHKEXEC=OFF; trusted execution is not protecting the audit tools"
    TAT_MEANING="With trusted execution or command-execution checking off, AIX does not verify the integrity of the audit tools before running them."
    TAT_FIX="/usr/sbin/trustchk -p TE=ON CHKEXEC=ON; installp -aXYqg -d /dev/cd0 bos.rte.security (PTxray only recommends this fix)."
  else
    TAT_TRUST_SHAPE=$(printf '%s\n' "$TAT_TRUST_RAW" | awk '
      /^[ \t]*TE=/ { v=$0; sub(/^[ \t]*TE=[ \t]*/, "", v); gsub(/[ \t]/, "", v); te=v }
      /^[ \t]*CHKEXEC=/ { v=$0; sub(/^[ \t]*CHKEXEC=[ \t]*/, "", v); gsub(/[ \t]/, "", v); ce=v }
      END {
        if (te == "" || ce == "") { print "?"; exit }
        if (te == "OFF" || ce == "OFF") print "off"
        else if (te == "ON" && ce == "ON") print "on"
        else print "?"
      }')
    TAT_TRUST_SHAPE_RC=$?
    if [ "$TAT_TRUST_SHAPE_RC" -ne 0 ] || [ "$TAT_TRUST_SHAPE" = "?" ]; then
      TAT_STATUS=NOT_ASSESSED
      TAT_OBSERVED="not assessed - trustchk -p output was unparseable"
      TAT_MEANING="PTxray did not obtain trustworthy evidence of the audit-tool hash protection policy."
      TAT_FIX="verify that trustchk -p is runnable, then rerun PTxray."
    elif [ "$TAT_TRUST_SHAPE" = "off" ]; then TAT_STATUS=FAIL
      TAT_OBSERVED="trustchk reports TE=OFF or CHKEXEC=OFF; trusted execution is not protecting the audit tools"
      TAT_MEANING="With trusted execution or command-execution checking off, AIX does not verify the integrity of the audit tools before running them."
      TAT_FIX="/usr/sbin/trustchk -p TE=ON CHKEXEC=ON; installp -aXYqg -d /dev/cd0 bos.rte.security (PTxray only recommends this fix)."
    else
      TAT_TSD_SHAPE=$(printf '%s\n' "$TAT_TSD_RAW" | awk '
        BEGIN {
          req[1]="/usr/sbin/auditselect"; req[2]="/usr/sbin/auditstream"
          req[3]="/usr/sbin/auditpr"; req[4]="/usr/sbin/auditcat"
          req[5]="/usr/sbin/auditbin"; req[6]="/usr/sbin/auditldap"
          req[7]="/usr/sbin/auditconv"; req[8]="/usr/sbin/audit"
          req[9]="/usr/sbin/auditmerge"
          nreq=9
        }
        /^\/usr\/sbin\/audit[^:]*:/ {
          hdr=$0
        ...
        /hash_value/ {
          if (cur == "") { mal=1; next }
          v=$0; sub(/^[ \t]*hash_value[ \t]*=[ \t]*/, "", v)
          gsub(/[ \t]/, "", v); gsub(/^"/, "", v); gsub(/"$/, "", v)
          hsh[cur]=v
          next
        }
        END {
          if (mal) { print "malformed"; exit }
          for (i = 1; i <= nreq; i++) {
            s = req[i]
            if (!(s in seen)) { print "missing-subject:" s; exit }
          }
          for (i = 1; i <= nreq; i++) {
            s = req[i]
            if (!(s in ctag) || !(s in sig) || !(s in hsh) || ctag[s] == "" || sig[s] == "" || hsh[s] == "") { print "missing-field:" s; exit }
            if (ctag[s] == "VOLATILE" || sig[s] == "VOLATILE" || hsh[s] == "VOLATILE") { print "volatile:" s; exit }
          }
          print "ok"
        }')
      TAT_TSD_SHAPE_RC=$?
      if [ "$TAT_TSD_SHAPE_RC" -ne 0 ] || [ -z "$TAT_TSD_SHAPE" ]; then
        TAT_STATUS=NOT_ASSESSED
        TAT_OBSERVED="not assessed - tsd.dat audit-tool rows could not be parsed"
        TAT_MEANING="PTxray did not obtain trustworthy evidence of the audit-tool hash protection policy."
        TAT_FIX="verify /etc/security/tsd/tsd.dat is intact, then rerun PTxray."
      else
        case "$TAT_TSD_SHAPE" in
          ok) TAT_STATUS=PASS
            TAT_OBSERVED="trustchk TE=ON CHKEXEC=ON; all required audit tools carry a non-VOLATILE cert_tag/signature/hash_value in tsd.dat"
            TAT_MEANING="The trusted signature database attests every required audit tool, so their hashes are verified before execution."
            TAT_FIX="n/a"
            ;;
          volatile:*|missing-field:*) TAT_STATUS=FAIL
            TAT_OBSERVED="trustchk TE=ON CHKEXEC=ON but an audit tool in tsd.dat has a missing or VOLATILE cert_tag/signature/hash_value"
            TAT_MEANING="An audit tool's hash is not pinned in the trusted signature database, so its integrity cannot be verified."
            TAT_FIX="restore a non-VOLATILE cert_tag/signature/hash_value for the affected audit tool in /etc/security/tsd/tsd.dat; PTxray only recommends this action."
            ;;
          missing-subject:*) TAT_STATUS=FAIL
            TAT_MISSING_SUBJECT=${TAT_TSD_SHAPE#missing-subject:}
            TAT_OBSERVED="tsd.dat does not list required audit tool $TAT_MISSING_SUBJECT"
            TAT_MEANING="An audit tool is absent from the trusted signature database, so its integrity is not verified."
            TAT_FIX="reinstall the audit tool fileset so it is registered in /etc/security/tsd/tsd.dat; PTxray only recommends this action."
            ;;
          *) TAT_STATUS=NOT_ASSESSED
            TAT_OBSERVED="not assessed - tsd.dat audit-tool rows were malformed"
            TAT_MEANING="PTxray did not obtain trustworthy evidence of the audit-tool hash protection policy."
            TAT_FIX="verify /etc/security/tsd/tsd.dat is intact, then rerun PTxray."
            ;;
        esac
  ...
  add security stig_audit_tools_tsd "Audit tool hash verification" \
    "$TAT_STATUS" med "$TAT_OBSERVED" \
    "$TAT_MEANING" "$TAT_FIX" "stig:V-215251"
}
