...
AIXRAY_TOOL=ck-stig-automated-file-mounting-tool


function standalone_check {
_AIXRAY_SESSION_KEYS=""
# STIG V-215395 — automated file system mounting tool must be disabled when not required.
typeset AFM_RAW AFM_RC AFM_STATE AFM_REASON
# command: lssrc -s automountd

AFM_RAW=$(aix automountd_lssrc lssrc -s automountd)
AFM_RC=$?
AFM_STATE=""
if [ -n "$AFM_RAW" ]; then
  AFM_STATE=$(printf '%s\n' "$AFM_RAW" | awk '
  BEGIN { rows=0; absent=0; state="" }
  /0513-085/ && /automountd/ { absent=1 }
  /automountd/ {
    rows++
    state=$NF
  }
  END {
    if (absent && rows == 1) print "absent"
    else if (rows == 1 && (state == "active" || state == "inoperative")) print state
    else print "unparseable"
  }
  ')
fi

# 0513-085 is lssrc's determinate "not on file" answer and carries a nonzero
# exit of its own, so absence is trusted regardless of rc. Every other reading
# of the subsystem state requires a probe that actually succeeded.
AFM_REASON=""
if [ "$AFM_STATE" = "absent" ]; then
  AFM_REASON=""
elif [ "$AFM_RC" -ne 0 ]; then
  AFM_REASON="probe failed (rc=$AFM_RC)"
elif [ -z "$AFM_RAW" ]; then
  AFM_REASON="empty probe output"
elif [ "$AFM_STATE" != "active" ] && [ "$AFM_STATE" != "inoperative" ]; then
  AFM_REASON="unparseable probe output"
fi

if [ -n "$AFM_REASON" ]; then
  add security stig_automated_file_mounting_tool "Automated file system mounting tool disabled" NOT_ASSESSED med \
      "not assessed - $AFM_REASON" \
      "PTxray did not obtain one trustworthy lssrc result for the automountd subsystem." \
      "stopsrc -s automountd" "stig:V-215395"
elif [ "$AFM_STATE" = "active" ]; then
  add security stig_automated_file_mounting_tool "Automated file system mounting tool disabled" FAIL med \
      "automountd active" \
      "The automountd subsystem is active. STIG V-215395 requires it be disabled unless automated file system mounting is documented as required." \
      "stopsrc -s automountd" "stig:V-215395"
else
  add security stig_automated_file_mounting_tool "Automated file system mounting tool disabled" PASS med \
      "automountd $AFM_STATE" \
      "The automountd subsystem is not active." \
      "n/a" "stig:V-215395"
fi
}
