...
AIXRAY_TOOL=ck-stig-automatically-lock-after-15


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_automatically_lock_after_15 — DISA STIG V-215318.
  # CDE dtsession saverTimeout and lockTimeout must be numeric and <= 15.
  # No vendor sys.resources (CDE unused) is Not Applicable. Uncertain
  # probe evidence is refused. Missing/loose config is a finding.
  typeset AL15_USR_RAW AL15_USR_RC AL15_ETC_RAW AL15_ETC_RC
  typeset AL15_VCLASS AL15_CLASS AL15_REASON AL15_PROBLEMS AL15_FAIL_RC
  typeset AL15_STATUS AL15_SEV AL15_OBSERVED AL15_MEANING AL15_FIX
  ...
  AL15_USR_RAW=$(aix usr_dt_sys_resources sh -c 'ls /usr/dt/config/*/sys.resources 2>&1')
  AL15_USR_RC=$?
  AL15_FAIL_RC=$AL15_USR_RC

  if [ -z "$AL15_USR_RAW" ]; then
    if [ "$AL15_USR_RC" -eq 0 ]; then
      AL15_REASON=probe_empty
    else
      AL15_REASON=probe_failed
    fi
  else
    AL15_VCLASS=$(printf '%s\n' "$AL15_USR_RAW" | awk '
      /^[ \t]*$/ { next }
      {
        line = $0
        if (index(line, "*") && index(line, "/usr/dt/config/")) absent_ev = 1
        if (line ~ /does not exist/) absent_ev = 1
        if (line ~ /No such file/) absent_ev = 1
        if (line ~ /^\/usr\/dt\/config\/[^\/]+\/sys\.resources$/) {
          n = split(line, p, "/")
          loc = p[5]
          if (loc != "" && index(loc, "*") == 0) nv++
        }
      }
      END {
        if (nv == 0) {
          if (absent_ev) print "absent"
          else print "unknown"
        } else print "present"
      }
    ')
    if [ "$AL15_VCLASS" = "absent" ]; then
      AL15_REASON=subject_absent
    elif [ "$AL15_USR_RC" -ne 0 ]; then
      AL15_REASON=probe_failed
    elif [ "$AL15_VCLASS" != "present" ]; then
      AL15_REASON=unparseable
    else
      AL15_ETC_RAW=$(aix etc_dt_sys_resources sh -c 'grep dtsession /etc/dt/config/*/sys.resources 2>&1')
      AL15_ETC_RC=$?
      AL15_FAIL_RC=$AL15_ETC_RC
      if [ "$AL15_ETC_RC" -ne 0 ]; then
        if [ "$AL15_ETC_RC" -eq 1 ] && [ -z "$AL15_ETC_RAW" ]; then
          AL15_REASON=noncompliant
          AL15_PROBLEMS="all locale sys.resources missing saverTimeout or lockTimeout"
        else
          AL15_REASON=probe_failed
        fi
      elif [ -z "$AL15_ETC_RAW" ]; then
        AL15_REASON=probe_empty
      else
        AL15_CLASS=$(printf '%s\n' "--USR--" "$AL15_USR_RAW" "--ETC--" "$AL15_ETC_RAW" | awk '
          $0 == "--USR--" { sect = "usr"; next }
          $0 == "--ETC--" { sect = "etc"; next }
          sect == "usr" {
            if ($0 ~ /^\/usr\/dt\/config\/[^\/]+\/sys\.resources$/) {
              n = split($0, p, "/")
              loc = p[5]
              if (loc != "" && index(loc, "*") == 0) {
                nv++
          ...
          sect == "etc" {
            ...
            file = ""
            rest = line
            m = index(line, "/sys.resources:")
            if (m) {
              file = substr(line, 1, m + 13)
              rest = substr(line, m + 15)
            }
            gsub(/^[ \t]+/, "", rest)
            key = ""
            if (index(rest, "dtsession*saverTimeout:") == 1) key = "s"
            else if (index(rest, "dtsession*lockTimeout:") == 1) key = "l"
            else next
            colon = index(rest, ":")
            valstr = substr(rest, colon + 1)
            gsub(/^[ \t]+/, "", valstr)
            gsub(/[ \t]+$/, "", valstr)
            loc = ""
            if (file != "") {
              n = split(file, p, "/")
              loc = p[5]
            } else if (nv == 1) loc = vloc[1]
            if (loc == "") next
            if (key == "s") saver[loc] = valstr
            else lock[loc] = valstr
            next
          }
          END {
            if (nv == 0) { print "unknown"; exit }
            problems = ""
            for (i = 1; i <= nv; i++) {
              loc = vloc[i]
              path = "/etc/dt/config/" loc "/sys.resources"
              s = saver[loc]
              l = lock[loc]
              if (s == "" || l == "") {
                if (problems != "") problems = problems "; "
                problems = problems path " missing saverTimeout or lockTimeout"
                continue
              }
              if (s !~ /^[0-9]+$/ || l !~ /^[0-9]+$/) {
                if (problems != "") problems = problems "; "
                problems = problems path " unparseable timeout value"
                continue
              }
              val = s + 0
              if (l + 0 > val) val = l + 0
              if (val + 0 > 15) {
                if (problems != "") problems = problems "; "
                problems = problems path " saverTimeout=" s " lockTimeout=" l
              }
            }
            if (problems != "") print "fail " problems
            else print "pass"
          }
        ')
        AL15_KIND=${AL15_CLASS%% *}
        if [ "$AL15_KIND" = "pass" ]; then
          AL15_REASON=compliant
        elif [ "$AL15_KIND" = "fail" ]; then
          AL15_REASON=noncompliant
          AL15_PROBLEMS=${AL15_CLASS#fail }
        else
          AL15_REASON=unparseable
        fi
      fi
  ...
  case "$AL15_REASON" in
    compliant)
      AL15_STATUS=PASS
      AL15_OBSERVED="dtsession saverTimeout and lockTimeout are <=15 minutes in every CDE locale sys.resources"
      AL15_MEANING="Unattended CDE sessions lock within 15 minutes of inactivity."
      AL15_FIX="n/a"
      ;;
    noncompliant)
      AL15_STATUS=FAIL
      AL15_OBSERVED="CDE dtsession timeouts exceed 15 minutes or are missing: $AL15_PROBLEMS"
      AL15_MEANING="An unattended CDE session stays unlocked beyond 15 minutes, or a locale sys.resources file is missing the required dtsession timeouts."
      AL15_FIX="set dtsession*saverTimeout: 15 and dtsession*lockTimeout: 15 in each /etc/dt/config locale sys.resources copied from the matching /usr/dt/config file; PTxray only recommends this action."
      ;;
    subject_absent)
      AL15_STATUS=NOT_APPLICABLE
      AL15_OBSERVED="CDE (X11) is not used; no /usr/dt/config/*/sys.resources"
      AL15_MEANING="CDE (X11) is not used on this host, so the graphical inactivity lock requirement is not applicable."
      AL15_FIX="n/a"
      ;;
    probe_failed) AL15_STATUS=NOT_ASSESSED
      AL15_OBSERVED="not assessed - CDE sys.resources probe failed (rc=$AL15_FAIL_RC)"
      AL15_MEANING="PTxray did not obtain trustworthy evidence for CDE dtsession saverTimeout and lockTimeout."
      AL15_FIX="verify /usr/dt/config/*/sys.resources and /etc/dt/config/*/sys.resources dtsession timeouts, then rerun PTxray."
      ;;
    probe_empty) AL15_STATUS=NOT_ASSESSED
      AL15_OBSERVED="not assessed - CDE sys.resources probe returned no output"
      AL15_MEANING="PTxray did not obtain trustworthy evidence for CDE dtsession saverTimeout and lockTimeout."
      AL15_FIX="verify /usr/dt/config/*/sys.resources and /etc/dt/config/*/sys.resources dtsession timeouts, then rerun PTxray."
      ;;
    *)
      AL15_STATUS=NOT_ASSESSED
      AL15_OBSERVED="not assessed - CDE sys.resources evidence was unparseable"
      AL15_MEANING="PTxray did not obtain trustworthy evidence for CDE dtsession saverTimeout and lockTimeout."
      AL15_FIX="verify /usr/dt/config/*/sys.resources and /etc/dt/config/*/sys.resources dtsession timeouts, then rerun PTxray."
      ;;
  esac
  ...
  case "$AL15_STATUS" in
    NOT_APPLICABLE) AL15_SEV=low ;;
    *) AL15_SEV=med ;;
  esac

  add security stig_automatically_lock_after_15 "CDE inactivity lock after 15 minutes" \
    "$AL15_STATUS" "$AL15_SEV" "$AL15_OBSERVED" \
    "$AL15_MEANING" "$AL15_FIX" "stig:V-215318"
}
