...
AIXRAY_TOOL=ck-stig-automatically-remove-disable-emergency


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_automatically_remove_disable_emergency — DISA STIG V-215180.
  # The emergency-account list is an onboarding interview input; this probe
  # records available lsuser evidence but cannot decide the control itself.
  typeset STIG_EMERGENCY_RAW STIG_EMERGENCY_RC STIG_EMERGENCY_EVIDENCE
  typeset STIG_EMERGENCY_STATUS STIG_EMERGENCY_OBSERVED

  STIG_EMERGENCY_RAW=$(aix lsuser_expires lsuser -a expires ALL)
  STIG_EMERGENCY_RC=$?
  STIG_EMERGENCY_STATUS=NOT_ASSESSED

  if [ "$STIG_EMERGENCY_RC" -ne 0 ]; then
    STIG_EMERGENCY_OBSERVED="not assessed - probe rc=$STIG_EMERGENCY_RC"
  elif [ -z "$STIG_EMERGENCY_RAW" ]; then
    STIG_EMERGENCY_OBSERVED="not assessed - empty lsuser expires evidence"
  else
    STIG_EMERGENCY_EVIDENCE=$(printf '%s\n' "$STIG_EMERGENCY_RAW" | awk '
      NF { if (seen) printf "; "; printf "%s", $0; seen=1 }
    ')
    if [ -z "$STIG_EMERGENCY_EVIDENCE" ]; then
      STIG_EMERGENCY_OBSERVED="not assessed - unparseable lsuser expires evidence"
    else
      STIG_EMERGENCY_OBSERVED="not assessed - Obtain a list of emergency accounts from the ISSO/ISSM; evidence=$STIG_EMERGENCY_EVIDENCE"
    fi
  fi

  add security stig_automatically_remove_disable_emergency \
    "Emergency accounts removed or disabled after crisis" \
    "$STIG_EMERGENCY_STATUS" med "$STIG_EMERGENCY_OBSERVED" \
    "PTxray cannot determine the emergency-account lifecycle without the ISSO/ISSM interview answer." \
    "chuser expires=1228093516 <emergency_user>" "stig:V-215180"
}
