...
AIXRAY_TOOL=ck-stig-automatically-remove-disable-temporary


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_automatically_remove_disable_temporary — DISA STIG V-215170.
  # The probe is useful evidence, but only the onboarding interview identifies
  # which accounts are temporary and confirms their required disposition.
  typeset STIG_TMP_RAW STIG_TMP_RC STIG_TMP_OBS

  STIG_TMP_RAW=$(aix lsuser_expires_tmp_user lsuser -a expires tmp_user)
  STIG_TMP_RC=$?
  if [ "$STIG_TMP_RC" -ne 0 ]; then
    STIG_TMP_OBS="not assessed - 'lsuser -a expires tmp_user' probe failed (rc=$STIG_TMP_RC); which accounts are temporary is not box state; onboarding interview answer required"
  elif [ -z "$STIG_TMP_RAW" ]; then
    STIG_TMP_OBS="not assessed - 'lsuser -a expires tmp_user' returned empty evidence; which accounts are temporary is not box state; onboarding interview answer required"
  else
    STIG_TMP_OBS="not assessed - 'lsuser -a expires tmp_user' returned: $STIG_TMP_RAW; which accounts are temporary is not box state; onboarding interview answer required"
  fi

  add security stig_automatically_remove_disable_temporary \
    "Temporary user account removal or disablement" NOT_ASSESSED med \
    "$STIG_TMP_OBS" \
    "The temporary-account population and its 72-hour removal or disablement policy require the onboarding interview." \
    "identify the temporary accounts from the onboarding interview, confirm each has an expiry with 'lsuser -a expires <account>', and set one with 'chuser expires=<MMDDhhmmyy> <account>'; PTxray only recommends these actions." \
    "stig:V-215170"
}
