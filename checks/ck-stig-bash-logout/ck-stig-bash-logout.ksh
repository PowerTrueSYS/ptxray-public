...
AIXRAY_TOOL=ck-stig-bash-logout


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_bash_logout — DISA STIG V-215309. If bash is used, AIX must display
  # logout messages. The rule is Not Applicable when no account is assigned the
  # bash shell; otherwise every bash user needs a per-user ~/.bash_logout that
  # displays a logout message (an echo).
  #
  # DISA audit commands, verbatim (decomposed into fixture-keyed aix() probes
  # below):
  ...
  #
  # passwd_file captures /etc/passwd once and reimplements the field-1/7 and
  # field-6 reads; bash_logout_ls is the for-loop body applied to bash users
  # only; bash_logout_cat is the per-user cat. An ls rc of 2 is the determinate
  # missing-file finding; every other failed or empty probe is a refusal.
  typeset BLO_PASSWD BLO_PASSWD_RC BLO_PARSE BLO_PARSE_RC BLO_N
  typeset BLO_FIRST_FAIL BLO_USER BLO_HOME
  typeset BLO_LS_RAW BLO_LS_RC BLO_CAT_RAW BLO_CAT_RC
  typeset BLO_STATUS BLO_OBSERVED BLO_SEV BLO_MEANING BLO_FIX

  BLO_PASSWD=$(aix passwd_file cat /etc/passwd)
  BLO_PASSWD_RC=$?
  BLO_STATUS=""
  BLO_OBSERVED=""
  BLO_FIRST_FAIL=""

  if [ "$BLO_PASSWD_RC" -ne 0 ]; then
    BLO_STATUS=NOT_ASSESSED
    BLO_OBSERVED="not assessed - probe passwd_file failed (rc=$BLO_PASSWD_RC)"
  elif [ -z "$BLO_PASSWD" ]; then
    BLO_STATUS=NOT_ASSESSED
    BLO_OBSERVED="not assessed - probe passwd_file returned rc 0 with no output"
  else
    BLO_PARSE=$(printf '%s\n' "$BLO_PASSWD" | awk -F: '
      /^[[:space:]]*$/ || /^[[:space:]]*#/ { next }
      /^[+@-]/ { next }
      {
        # An empty shell (field 7) is the AIX determinate "default shell" marker,
        # and an empty home (field 6) on a system account is normal; both are
        # skipped, never a parse error. Only a record that cannot be field-split
        # (fewer than 7 colon-separated fields) is unparseable.
        if (NF < 7) { bad = 1; next }
        if ($7 == "" || $6 == "") { next }
        if (tolower($7) ~ /bash/) print $1 "|" $6
      }
      END { if (bad) exit 1 }
    ')
    BLO_PARSE_RC=$?
    if [ "$BLO_PARSE_RC" -ne 0 ]; then
      BLO_STATUS=NOT_ASSESSED
      BLO_OBSERVED="not assessed - passwd_file output was unparseable"
    else
      BLO_N=$(printf '%s' "$BLO_PARSE" | awk 'NF { n++ } END { print n + 0 }')
      if [ "$BLO_N" -eq 0 ]; then
        BLO_STATUS=NOT_APPLICABLE
        BLO_OBSERVED="no users are assigned the bash shell; the rule is not applicable"
      else
        while IFS='|' read -r BLO_USER BLO_HOME; do
          [ -n "$BLO_USER" ] || continue
          BLO_LS_RAW=$(aix bash_logout_ls ls -alL "$BLO_HOME/.bash_logout")
          BLO_LS_RC=$?
          if [ "$BLO_LS_RC" -ne 0 ]; then
            if [ "$BLO_LS_RC" -eq 2 ]; then
              if [ -z "$BLO_FIRST_FAIL" ]; then
                BLO_FIRST_FAIL="no $BLO_HOME/.bash_logout for bash user '$BLO_USER'"
              fi
            else
              BLO_STATUS=NOT_ASSESSED
              BLO_OBSERVED="not assessed - probe bash_logout_ls failed (rc=$BLO_LS_RC)"
              break
            fi
          elif [ -z "$BLO_LS_RAW" ]; then
            BLO_STATUS=NOT_ASSESSED
            BLO_OBSERVED="not assessed - probe bash_logout_ls returned rc 0 with no output"
            break
          else
            BLO_CAT_RAW=$(aix bash_logout_cat cat "$BLO_HOME/.bash_logout")
            BLO_CAT_RC=$?
            if [ "$BLO_CAT_RC" -ne 0 ]; then
              BLO_STATUS=NOT_ASSESSED
              BLO_OBSERVED="not assessed - probe bash_logout_cat failed (rc=$BLO_CAT_RC)"
              break
            elif [ -z "$BLO_CAT_RAW" ]; then
              if [ -z "$BLO_FIRST_FAIL" ]; then
                BLO_FIRST_FAIL="no logout message in $BLO_HOME/.bash_logout for bash user '$BLO_USER'"
              fi
            elif ! printf '%s\n' "$BLO_CAT_RAW" | awk '
              {
                line = $0
...
$BLO_PARSE
EOF
        if [ -z "$BLO_STATUS" ]; then
          if [ -n "$BLO_FIRST_FAIL" ]; then
            BLO_STATUS=FAIL
            BLO_OBSERVED="$BLO_FIRST_FAIL"
          else
            BLO_STATUS=PASS
            BLO_OBSERVED="all bash users have a .bash_logout configured with a logout message"
          fi
        fi
      fi
    fi
  ...
  case "$BLO_STATUS" in
    PASS)
      BLO_SEV=med
      BLO_MEANING="Every bash user logs out with a visible logout message, so the end of the session is announced."
      BLO_FIX="n/a"
      ;;
    FAIL)
      BLO_SEV=low
      BLO_MEANING="A bash user logs out silently because ~/.bash_logout is missing or prints no logout message."
      BLO_FIX="Create the missing ~/.bash_logout and add a logout message line such as 'echo \"You are being disconnected.\"'; PTxray only recommends this action and never writes user files."
      ;;
    NOT_APPLICABLE)
      BLO_SEV=low
      BLO_MEANING="No account is assigned the bash shell, so there is no per-user ~/.bash_logout to assess."
      BLO_FIX="n/a"
      ;;
    *)
      BLO_SEV=med
      BLO_MEANING="PTxray did not obtain trustworthy evidence about the bash logout-message configuration."
      BLO_FIX="Confirm the read-only passwd and .bash_logout probes return evidence, then rerun PTxray."
      ;;
  esac

  add security stig_bash_logout \
      "AIX must display logout messages for bash users" \
      "$BLO_STATUS" "$BLO_SEV" "$BLO_OBSERVED" \
      "$BLO_MEANING" "$BLO_FIX" "stig:V-215309"
}
