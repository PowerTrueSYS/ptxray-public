...
AIXRAY_TOOL=ck-stig-bootlist-modes-use-removable


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_bootlist_modes_use_removable — DISA STIG V-215411.
  # Both normal and service bootlists must be checked; a removable boot device
  # in either list is a finding. Probe uncertainty fails closed.
  typeset BL_NORMAL BL_SERVICE BL_NORMAL_RC BL_SERVICE_RC BL_STATE BL_REASON
  typeset BL_STATUS BL_OBSERVED BL_MEANING BL_FIX BL_SHAPE

  BL_NORMAL=$(aix bootlist_normal bootlist -m normal -o)
  BL_NORMAL_RC=$?
  BL_SERVICE=$(aix bootlist_service bootlist -m service -o)
  BL_SERVICE_RC=$?
  BL_STATE=ok
  BL_REASON=""

  if [ "$BL_NORMAL_RC" -ne 0 ] || [ "$BL_SERVICE_RC" -ne 0 ]; then
    BL_STATE=unknown
    BL_REASON="bootlist probe failed (normal rc=$BL_NORMAL_RC, service rc=$BL_SERVICE_RC)"
  elif [ -z "$BL_NORMAL" ] || [ -z "$BL_SERVICE" ]; then
    BL_STATE=unknown # empty output is refusal
    BL_REASON="bootlist probe returned empty output (normal rc=0, service rc=0)"
  else
    BL_SHAPE=$(printf '%s\n%s\n' "$BL_NORMAL" "$BL_SERVICE" | awk '
      function check_line(line,    n, i, token) {
        if (line ~ /^[ \t]*$/) return
        if (line ~ /^[ \t]*no bootable devices([ \t].*)?$/) return
        n = split(line, fields, /[ \t]+/)
        for (i = 1; i <= n; i++) {
          token = fields[i]
          if (token == "") continue
          if (token ~ /^(cd|fd|rmt|ent)[0-9]+$/) removable = 1
          else if (token ~ /^(hdisk|vpath|ds[0-9]+|scsi[0-9]+)[0-9]*$/) next_token = 1
          else if (token ~ /^[A-Za-z_][A-Za-z_0-9]*=/) next_token = 1
          else bad = 1
        }
      }
      { check_line($0) }
      END {
        if (removable) print "removable"
        else if (bad) print "malformed"
        else print "safe"
      }')
    if [ "$BL_SHAPE" = malformed ]; then
      BL_STATE=unknown
      BL_REASON="bootlist output was not in a recognized device-list form"
    elif [ "$BL_SHAPE" = removable ]; then
      BL_STATE=removable
    elif [ "$BL_SHAPE" != safe ]; then
      BL_STATE=unknown
      BL_REASON="bootlist output could not be parsed"
    fi
  fi
  ...
  case "$BL_STATE" in
    removable)
      BL_STATUS=FAIL
      BL_OBSERVED="removable boot device found in normal or service bootlist"
      BL_MEANING="A removable device is configured as a boot loader, allowing boot from unauthorized media."
      BL_FIX="bootlist -m normal hdisk0; bootlist -m service hdisk0"
      ;;
    ok)
      BL_STATUS=PASS
      BL_OBSERVED="normal and service bootlists contain no removable boot devices"
      BL_MEANING="Normal and service bootlists do not use removable media as the boot loader."
      BL_FIX="n/a"
      ;;
    *)
      BL_STATUS=NOT_ASSESSED
      BL_OBSERVED="not assessed - $BL_REASON"
      BL_MEANING="PTxray did not obtain trustworthy evidence for the normal and service bootlists."
      BL_FIX="verify both bootlists with 'bootlist -m normal -o' and 'bootlist -m service -o', then rerun PTxray."
      ;;
  esac

  add security stig_bootlist_modes_use_removable "AIX bootlist does not use removable media" \
    "$BL_STATUS" med "$BL_OBSERVED" "$BL_MEANING" "$BL_FIX" "stig:V-215411"
}
