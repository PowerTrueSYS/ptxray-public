...
AIXRAY_TOOL=ck-stig-bootlist-modes


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_bootlist_modes — DISA STIG V-215410.
  # AIX must boot only from the system hard-disk boot device. The normal boot
  # list is read exactly once; a non-hard-disk device in it is a finding, and a
  # boot list with no boot device is a finding (required configuration unmet).
  # The SMS firmware multi-boot clause is not visible from the OS, so a
  # hard-disk-only result still renders a typed NOT_ASSESSED for the manual
  # portion — there is no OS-visible PASS path for V-215410. Probe uncertainty
  # fails closed.
  typeset BL_RAW BL_RC BL_SHAPE BL_DEVICES BL_STATUS BL_OBSERVED BL_MEANING BL_FIX

  BL_RAW=$(aix bootlist_normal bootlist -m normal -o)
  BL_RC=$?

  BL_MEANING="PTxray did not obtain trustworthy evidence for the normal boot list."
  BL_FIX="verify the normal boot list as root with 'bootlist -m normal -o', correct the capture problem, and rerun PTxray."

  if [ "$BL_RC" -ne 0 ]; then
    BL_STATUS=NOT_ASSESSED  # probe-failed
    BL_OBSERVED="not assessed - bootlist -m normal -o probe failed (rc=$BL_RC)"
  elif [ -z "$BL_RAW" ]; then
    BL_STATUS=NOT_ASSESSED  # probe-empty-rc0
    BL_OBSERVED="not assessed - bootlist -m normal -o returned empty output (rc=0)"
  else
    BL_SHAPE=$(printf '%s\n' "$BL_RAW" | awk '
      function classify(line,    n, i, tok) {
        if (line ~ /^[ \t]*$/) return
        if (line ~ /^[ \t]*[Nn][Oo] [Bb]ootable [Dd]evices/) { no_devices = 1; return }
        n = split(line, fields, /[ \t]+/)
        for (i = 1; i <= n; i++) {
          tok = fields[i]
          if (tok == "") continue
          ...
          if (tok ~ /^hdisk[0-9]+$/) {
            if (hdisk_list == "") hdisk_list = tok
            else hdisk_list = hdisk_list " " tok
            hdisk = 1
            continue
          }
          if (tok ~ /^(cd|fd|rmt|ent|et|en|tok|fddi|usb|scsi|nvme)[0-9]+$/) {
            if (nonhdisk_tok == "") nonhdisk_tok = tok
            nonhdisk = 1
            continue
          }
      ...
      { classify($0) }
      END {
        if (no_devices) print "subject-absent"
        else if (nonhdisk) print "nonhdisk=" nonhdisk_tok
        else if (hdisk) print "manual=" hdisk_list
        else if (bad) print "malformed"
        else print "subject-absent"
      }')
    case "$BL_SHAPE" in
      manual*)
        BL_DEVICES=${BL_SHAPE#manual=}
        BL_STATUS=NOT_ASSESSED  # manual-portion
        BL_OBSERVED="not assessed - SMS firmware multi-boot state is not visible from the OS; confirm with the system administrator that multi-boot is disabled in the SMS application"
        BL_MEANING="The OS-visible normal boot list is hard-disk-only ($BL_DEVICES), but the SMS firmware multi-boot setting cannot be verified from the OS and the manual clause remains open."
        BL_FIX="confirm with the system administrator that 'multi-boot' is disabled in the SMS application, then rerun PTxray."
        ;;
      nonhdisk=*)
        BL_DEVICES=${BL_SHAPE#nonhdisk=}
        BL_STATUS=FAIL  # non-hard-disk boot device
        BL_OBSERVED="normal boot list contains non-hard-disk boot device $BL_DEVICES"
        BL_MEANING="The normal boot list boots from a non-hard-disk device, allowing the system to boot from unauthorized media."
        BL_FIX="set the normal boot list to the system boot disk, e.g. 'bootlist -m normal hdisk0'; PTxray only recommends this command."
        ;;
      subject-absent)
        BL_STATUS=FAIL  # subject-absent: required configuration unmet
        BL_OBSERVED="no boot device is configured in the normal boot list (bootlist -m normal -o lists no devices)"
        BL_MEANING="No boot device is configured in the normal boot list, so the system is not configured to boot only from the system boot device."
        BL_FIX="set the normal boot list to the system boot disk, e.g. 'bootlist -m normal hdisk0'; PTxray only recommends this command."
        ;;
      malformed)
        BL_STATUS=NOT_ASSESSED  # unparseable output
        BL_OBSERVED="not assessed - bootlist -m normal -o output was not in a recognized device-list form"
        ;;
      *)
        BL_STATUS=NOT_ASSESSED
        BL_OBSERVED="not assessed - bootlist -m normal -o output could not be parsed"
        ;;
    esac
  fi

  add security stig_bootlist_modes "AIX system boot device" \
    "$BL_STATUS" med "$BL_OBSERVED" "$BL_MEANING" "$BL_FIX" "stig:V-215410"
}
