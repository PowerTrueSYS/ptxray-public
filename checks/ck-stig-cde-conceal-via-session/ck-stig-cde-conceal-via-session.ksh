...
AIXRAY_TOOL=ck-stig-cde-conceal-via-session


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_cde_conceal_via_session — DISA AIX STIG V-215212.
  # CDE must conceal, via the session lock, information previously visible on
  # the display with a publicly viewable image.  When CDE (X11) is not used the
  # rule is Not Applicable.  Otherwise every dtsession*saverTimeout and
  # dtsession*lockTimeout value in the /etc/dt/config/*/sys.resources site
  # files must be 15 minutes or less, and the site config file must exist.
  # A failed, contradictory, or unparseable probe refuses (NOT_ASSESSED) rather
  # than pass.  The site file path is the same read ck-cde-screensaver makes;
  # that check is untagged and not authoritative for this V-ID.
  typeset CDE_STIG_FS_RAW CDE_STIG_FS_RC CDE_STIG_FS_STATE
  typeset CDE_STIG_RES_RAW CDE_STIG_RES_RC CDE_STIG_SHAPE CDE_STIG_MAX
  typeset CDE_STIG_STATUS CDE_STIG_SEV CDE_STIG_OBSERVED
  typeset CDE_STIG_MEANING CDE_STIG_FIX

  CDE_STIG_FS_RAW=$(aix stig_cde_filesets lslpp -qcL 'X11.Dt*')
  CDE_STIG_FS_RC=$?
  if [ "$CDE_STIG_FS_RC" -eq 1 ] && [ -z "$CDE_STIG_FS_RAW" ]; then
    CDE_STIG_FS_STATE=absent
  elif [ "$CDE_STIG_FS_RC" -eq 0 ] && [ -n "$CDE_STIG_FS_RAW" ] &&
      printf '%s\n' "$CDE_STIG_FS_RAW" | awk -F: '/^[ \t]*$/ { next } { if (NF < 6 || $2 !~ /^X11\.Dt/) bad = 1 } END { exit bad }'; then
    CDE_STIG_FS_STATE=present
  elif [ "$CDE_STIG_FS_RC" -eq 0 ]; then
    CDE_STIG_FS_STATE=swallowed
  else
    CDE_STIG_FS_STATE=failed
  fi

  case "$CDE_STIG_FS_STATE" in
    absent)
      CDE_STIG_STATUS=NOT_APPLICABLE
      CDE_STIG_OBSERVED="CDE (X11) is not in use: no X11.Dt* fileset is installed, so the CDE session lock does not apply"
      ;;
    swallowed)
      CDE_STIG_STATUS=NOT_ASSESSED
      CDE_STIG_OBSERVED="not assessed - CDE fileset probe returned no rows with rc 0, a swallowed error"
      ;;
    failed)
      CDE_STIG_STATUS=NOT_ASSESSED
      CDE_STIG_OBSERVED="not assessed - CDE fileset probe failed (rc=$CDE_STIG_FS_RC), so install state is unproven"
      ;;
    present)
      CDE_STIG_RES_RAW=$(aix stig_cde_resources grep -E '^[[:blank:]]*dtsession[*](saverTimeout|lockTimeout)[[:blank:]]*:' /etc/dt/config/*/sys.resources)
      CDE_STIG_RES_RC=$?
      if [ "$CDE_STIG_RES_RC" -eq 0 ] && [ -n "$CDE_STIG_RES_RAW" ]; then
        CDE_STIG_SHAPE=$(printf '%s\n' "$CDE_STIG_RES_RAW" | awk '
          /^[ \t]*$/ { next }
          {
            line = $0
            # grep prefixes the file name only when it reads more than one
            # site file, so rows may arrive as "dtsession*saverTimeout: 15"
            # (single /etc/dt/config/<dir>/sys.resources) or as
            # "path:dtsession*saverTimeout: 15" (multiple site files).  Anchor
            # on the resource token rather than colon position so both shapes
            # parse.
            p1 = index(line, "dtsession*")
            if (p1 == 0) { bad = 1; next }
            rest = substr(line, p1)
            p2 = index(rest, ":")
            if (p2 == 0) { bad = 1; next }
            res = substr(rest, 1, p2 - 1)
            val = substr(rest, p2 + 1)
            gsub(/[ \t]/, "", res)
            gsub(/[ \t]/, "", val)
            if (val !~ /^[0-9][0-9]*$/) { bad = 1; next }
            if (res != "dtsession*saverTimeout" && res != "dtsession*lockTimeout") { bad = 1; next }
            count++
            # Initialise max from the first parsed row so a legitimate 0
            # timeout prints as "0" and flows to PASS, never an empty line.
            max = (count == 1 || val + 0 > max) ? val + 0 : max
          }
          END {
        ...
        if [ -z "$CDE_STIG_SHAPE" ]; then
          CDE_STIG_STATUS=NOT_ASSESSED
          CDE_STIG_OBSERVED="not assessed - dtsession resource rows did not parse"
        elif [ "$CDE_STIG_SHAPE" = "malformed" ]; then
          CDE_STIG_STATUS=NOT_ASSESSED
          CDE_STIG_OBSERVED="not assessed - dtsession saverTimeout/lockTimeout rows were malformed or non-numeric"
        elif [ "$CDE_STIG_SHAPE" = "empty" ]; then
          CDE_STIG_STATUS=NOT_ASSESSED
          CDE_STIG_OBSERVED="not assessed - dtsession resource probe returned no rows with rc 0, a swallowed error"
        elif [ "$CDE_STIG_SHAPE" -gt 15 ]; then
          CDE_STIG_STATUS=FAIL
          CDE_STIG_OBSERVED="dtsession saverTimeout/lockTimeout value of $CDE_STIG_SHAPE minutes exceeds the 15-minute concealment limit"
        else
          CDE_STIG_STATUS=PASS
          CDE_STIG_OBSERVED="dtsession saverTimeout/lockTimeout values are 15 minutes or less, so CDE conceals the display via the session lock"
        fi
      elif [ "$CDE_STIG_RES_RC" -eq 0 ]; then
        CDE_STIG_STATUS=NOT_ASSESSED
        CDE_STIG_OBSERVED="not assessed - dtsession resource probe returned no rows with rc 0, a swallowed error"
      elif [ "$CDE_STIG_RES_RC" -eq 1 ]; then
        CDE_STIG_STATUS=FAIL
        CDE_STIG_OBSERVED="no dtsession*saverTimeout or dtsession*lockTimeout resource is set in any /etc/dt/config/*/sys.resources site file"
      elif [ "$CDE_STIG_RES_RC" -eq 2 ]; then
        CDE_STIG_STATUS=FAIL
        CDE_STIG_OBSERVED="config file missing: no /etc/dt/config/*/sys.resources site configuration file exists"
      else
        CDE_STIG_STATUS=NOT_ASSESSED
        CDE_STIG_OBSERVED="not assessed - dtsession resource probe failed (rc=$CDE_STIG_RES_RC)"
      fi
      ;;
  esac

  if [ "$CDE_STIG_STATUS" = "NOT_APPLICABLE" ]; then
    CDE_STIG_SEV=low
  else
    CDE_STIG_SEV=med
  fi

  case "$CDE_STIG_STATUS" in
    PASS)
      CDE_STIG_MEANING="Unattended CDE sessions lock within the 15-minute concealment window, so the display is covered by the lock image."
      CDE_STIG_FIX="n/a"
      ;;
    FAIL)
      CDE_STIG_MEANING="An unattended CDE session stays visible beyond the 15-minute window or the lock policy is not configured, exposing the display to walk-up viewing."
      CDE_STIG_FIX="set 'dtsession*saverTimeout' and 'dtsession*lockTimeout' to 15 minutes or less in the /etc/dt/config/<config_dir>/sys.resources site files, creating the site file when it is missing; PTxray only recommends these actions."
      ;;
    NOT_APPLICABLE)
      CDE_STIG_MEANING="CDE (X11) is not used on this host, so there is no CDE session display to conceal."
      CDE_STIG_FIX="n/a"
      ;;
    *)
      CDE_STIG_MEANING="PTxray did not obtain trustworthy evidence of the CDE session-lock concealment policy."
      CDE_STIG_FIX="verify the X11.Dt* fileset state and the dtsession timeout resources in /etc/dt/config/*/sys.resources as root, then rerun PTxray."
      ;;
  esac

  add security stig_cde_conceal_via_session "CDE session lock conceals display" \
    "$CDE_STIG_STATUS" "$CDE_STIG_SEV" "$CDE_STIG_OBSERVED" \
    "$CDE_STIG_MEANING" "$CDE_STIG_FIX" "stig:V-215212"
}
