...
AIXRAY_TOOL=ck-stig-cde-dod-banner


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_cde_dod_banner -- DISA STIG V-215201.
  # The DoD login banner must be displayed immediately prior to, or as part
  # of, graphical desktop environment login prompts on AIX.  CDE's graphical
  # login greeting is the Dtlogin*greeting.labelString resource in the site
  # Xresources copy under /etc/dt/config; with CDE absent the rule is Not
  # Applicable.  Every probe that fails without a determinate answer refuses
  # as NOT_ASSESSED; only a definite absence or a definite mismatch fails.
  typeset CDE_DDB_RC CDE_DDB_RAW
  typeset CDE_DDB_LS_RC CDE_DDB_LS_RAW
  typeset CDE_DDB_GR_RC CDE_DDB_GR_RAW CDE_DDB_LABEL CDE_DDB_EXTRACT_RC
  typeset CDE_DDB_OK CDE_DDB_STATUS CDE_DDB_SEVERITY CDE_DDB_OBSERVED
  typeset CDE_DDB_MEANING CDE_DDB_FIX CDE_DDB_BANNER

  # Standard Mandatory DoD Notice and Consent Banner: the STIG herald with its
  # \n\r line-break escapes removed, compared after whitespace normalisation.
  CDE_DDB_BANNER='You are accessing a U.S. Government (USG) Information System (IS) that is provided for USG-authorized use only. By using this IS (which includes any device attached to this IS), you consent to the following conditions: -The USG routinely intercepts and monitors communications on this IS for purposes including, but not limited to, penetration testing, COMSEC monitoring, network operations and defense, personnel misconduct (PM), law enforcement (LE), and counterintelligence (CI) investigations. -At any time, the USG may inspect and seize data stored on this IS. -Communications using, or data stored on, this IS are not private, are subject to routine monitoring, interception, and search, and may be disclosed or used for any USG-authorized purpose. -This IS includes security measures (e.g., authentication and access controls) to protect USG interests--not for your personal benefit or privacy. -Notwithstanding the above, using this IS does not constitute consent to PM, LE or CI investigative searching or monitoring of the content of privileged communications, or work product, related to personal representation or services by attorneys, psychotherapists, or clergy, and their assistants. Such communications and work product are private and confidential. See User Agreement for details.'

  CDE_DDB_STATUS=""
  CDE_DDB_SEVERITY=med
  CDE_DDB_OBSERVED=""

  # 1. Is CDE (X11) used at all?  dtlogin is the CDE login manager; a clean
  #    "no such file" (rc 2, empty output) is the only determinate absence.
  CDE_DDB_RAW=$(aix cde_present ls /usr/dt/bin/dtlogin)
  CDE_DDB_RC=$?
  if [ "$CDE_DDB_RC" -eq 2 ] && [ -z "$CDE_DDB_RAW" ]; then
    CDE_DDB_STATUS=NOT_APPLICABLE
    CDE_DDB_SEVERITY=low
    CDE_DDB_OBSERVED="CDE (X11) is not used on this host (no /usr/dt/bin/dtlogin); the graphical DoD login banner rule is Not Applicable"
  elif [ "$CDE_DDB_RC" -ne 0 ] || [ -z "$CDE_DDB_RAW" ]; then
    CDE_DDB_STATUS=NOT_ASSESSED
    CDE_DDB_OBSERVED="not assessed - CDE presence probe failed (rc=$CDE_DDB_RC)"
  else
    # 2. The site Xresources copy must exist.
    CDE_DDB_LS_RAW=$(aix xresources_ls ls /etc/dt/config/en_US/Xresources)
    CDE_DDB_LS_RC=$?
    if [ "$CDE_DDB_LS_RC" -eq 2 ] && [ -z "$CDE_DDB_LS_RAW" ]; then
      CDE_DDB_STATUS=FAIL
      CDE_DDB_OBSERVED="/etc/dt/config/en_US/Xresources does not exist; the CDE DoD login banner is not configured"
    elif [ "$CDE_DDB_LS_RC" -ne 0 ] || [ -z "$CDE_DDB_LS_RAW" ]; then
      CDE_DDB_STATUS=NOT_ASSESSED
      CDE_DDB_OBSERVED="not assessed - Xresources existence probe failed (rc=$CDE_DDB_LS_RC)"
    else
      # 3. The greeting label resource must be set.
      CDE_DDB_GR_RAW=$(aix xresources_grep grep -F "Dtlogin*greeting.labelString" /etc/dt/config/en_US/Xresources)
      CDE_DDB_GR_RC=$?
      if [ "$CDE_DDB_GR_RC" -eq 1 ] && [ -z "$CDE_DDB_GR_RAW" ]; then
        CDE_DDB_STATUS=FAIL
        CDE_DDB_OBSERVED="Dtlogin*greeting.labelString is not set in /etc/dt/config/en_US/Xresources; the CDE DoD login banner is missing"
      elif [ "$CDE_DDB_GR_RC" -ne 0 ] || [ -z "$CDE_DDB_GR_RAW" ]; then
        CDE_DDB_STATUS=NOT_ASSESSED
        CDE_DDB_OBSERVED="not assessed - Dtlogin*greeting.labelString probe failed (rc=$CDE_DDB_GR_RC)"
      else
        # 4. Extract the label value after the first ':' and compare after
        #    normalisation: the STIG fix text writes the banner with literal
        #    \n and \r escape sequences (and may backslash-continue the
        #    value), so those become spaces first, then whitespace runs are
        #    collapsed and both ends trimmed, mirroring how the banner
        #    constant was authored.
        CDE_DDB_LABEL=$(printf '%s\n' "$CDE_DDB_GR_RAW" | awk '
          NR == 1 {
            line = $0
            p = index(line, ":")
            if (p == 0) exit 1
        ...
        CDE_DDB_EXTRACT_RC=$?
        if [ "$CDE_DDB_EXTRACT_RC" -ne 0 ]; then
          CDE_DDB_STATUS=NOT_ASSESSED
          CDE_DDB_OBSERVED="not assessed - Dtlogin*greeting.labelString line was malformed (no resource separator)"
        else
          CDE_DDB_OK=$(printf '%s\n' "$CDE_DDB_LABEL" | awk -v b="$CDE_DDB_BANNER" '
            {
              gsub(/[ \t]+/, " ", b)
              sub(/^ /, "", b)
              sub(/ $/, "", b)
              if (index($0, b) > 0) print "yes"; else print "no"
            }')
          if [ "$CDE_DDB_OK" = "yes" ]; then
            CDE_DDB_STATUS=PASS
            CDE_DDB_OBSERVED="Dtlogin*greeting.labelString contains the Standard Mandatory DoD Notice and Consent Banner"
          else
            CDE_DDB_STATUS=FAIL
            CDE_DDB_OBSERVED="Dtlogin*greeting.labelString does not contain the Standard Mandatory DoD Notice and Consent Banner"
          fi
        fi
      fi
    fi
  ...
  case "$CDE_DDB_STATUS" in
    PASS)
      CDE_DDB_MEANING="The CDE graphical login greeting carries the DoD banner, so every user sees the required notice immediately prior to graphical login."
      CDE_DDB_FIX="n/a"
      ;;
    FAIL)
      CDE_DDB_MEANING="The CDE graphical login greeting does not display the Standard Mandatory DoD Notice and Consent Banner, leaving users without the required warning before graphical login."
      CDE_DDB_FIX="copy the default CDE resources with 'cp /usr/dt/config/C/Xresources /etc/dt/config/en_US/Xresources' and set 'Dtlogin*greeting.labelString' to the Standard Mandatory DoD Notice and Consent Banner; PTxray only recommends these actions."
      ;;
    NOT_APPLICABLE)
      CDE_DDB_MEANING="No CDE graphical desktop is used on this host, so no graphical login prompt must display the DoD banner."
      CDE_DDB_FIX="n/a"
      ;;
    *)
      CDE_DDB_MEANING="PTxray did not obtain trustworthy evidence for the CDE DoD login banner configuration."
      CDE_DDB_FIX="verify CDE presence and the /etc/dt/config/en_US/Xresources Dtlogin*greeting.labelString resource as root, then rerun PTxray."
      ;;
  esac

  add security stig_cde_dod_banner "CDE DoD login banner" \
    "$CDE_DDB_STATUS" "$CDE_DDB_SEVERITY" "$CDE_DDB_OBSERVED" \
    "$CDE_DDB_MEANING" "$CDE_DDB_FIX" "stig:V-215201"
}
