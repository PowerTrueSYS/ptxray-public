...
AIXRAY_TOOL=ck-stig-cde-xlock


function standalone_check {
_AIXRAY_SESSION_KEYS=""
# cde_xlock — DISA STIG V-215188.
# AIX CDE must provide the xlock command so users can lock their sessions until
# reauthentication.  When AIX CDE (X11) is not used the rule text itself says the
# control is Not Applicable; with CDE in scope, the X11.apps.clients fileset and
# the /usr/bin/X11/xlock binary must both be present.  A missing xlock binary
# (any nonzero ls rc with empty stdout) is a determinate absence and renders
# FAIL; only a probe that could not execute (rc 126/127) or rc 0 with empty or
# unparseable output fails closed to NOT_ASSESSED.
typeset CDE_XL_LSLPP_RAW CDE_XL_LSLPP_RC CDE_XL_LSLPP_INSTALLED
typeset CDE_XL_LS_RAW CDE_XL_LS_RC CDE_XL_LS_STATE
typeset CDE_XL_STATUS CDE_XL_SEV CDE_XL_OBSERVED
typeset CDE_XL_MEANING CDE_XL_FIX

CDE_XL_LSLPP_RAW=$(aix lslpp_x11_apps_clients lslpp -L X11.apps.clients)
CDE_XL_LSLPP_RC=$?

# lslpp returns rc 0 with a fileset row when the named fileset is installed and
# rc 1 with no stdout when it is not (the AIX "not installed" contract,
# established by the ck-cde-screensaver exemplar).  Any other nonzero rc, or rc 0
# with no installed row, is a swallowed error and refuses as NOT_ASSESSED.
CDE_XL_LSLPP_INSTALLED=""
if [ "$CDE_XL_LSLPP_RC" -eq 0 ] && [ -n "$CDE_XL_LSLPP_RAW" ]; then
  if printf '%s\n' "$CDE_XL_LSLPP_RAW" | awk '
      NF && $1 == "X11.apps.clients" { found = 1 }
      END { exit (found ? 0 : 1) }'; then
    CDE_XL_LSLPP_INSTALLED=yes
  fi
fi

CDE_XL_STATUS=""
if [ "$CDE_XL_LSLPP_INSTALLED" = "yes" ]; then
  # CDE is in scope; the xlock binary must exist.  AIX ls documents only 0 on
  # success and >0 on error, so no single nonzero rc is guaranteed for a missing
  # operand.  A nonzero rc with empty stdout is therefore a determinate absence
  # and renders FAIL.  rc 126/127 means the probe itself could not execute
  # (command not found / not executable); rc 0 with empty or non-matching stdout
  # is a swallowed error — both refuse as NOT_ASSESSED.
  CDE_XL_LS_RAW=$(aix ls_xlock ls /usr/bin/X11/xlock)
  CDE_XL_LS_RC=$?
  CDE_XL_LS_STATE=""
  if [ "$CDE_XL_LS_RC" -eq 0 ]; then
    CDE_XL_LS_STATE=$(printf '%s\n' "$CDE_XL_LS_RAW" | awk '
      { seen = 1
        if ($0 ~ /\/usr\/bin\/X11\/xlock$/) { print "present"; exit }
        print "odd"; exit }
      END { if (!seen) print "empty" }')
  fi
  case "$CDE_XL_LS_STATE" in
    present)
      CDE_XL_STATUS=PASS
      CDE_XL_OBSERVED="X11.apps.clients installed and /usr/bin/X11/xlock present"
      ;;
    "")
      if [ "$CDE_XL_LS_RC" -eq 126 ] || [ "$CDE_XL_LS_RC" -eq 127 ]; then
        CDE_XL_STATUS=NOT_ASSESSED
        CDE_XL_OBSERVED="not assessed - xlock binary probe could not execute (rc=$CDE_XL_LS_RC)"
      elif [ -z "$CDE_XL_LS_RAW" ]; then
        CDE_XL_STATUS=FAIL
        CDE_XL_OBSERVED="/usr/bin/X11/xlock is missing while AIX CDE (X11) is in use"
      else
        CDE_XL_STATUS=NOT_ASSESSED
        CDE_XL_OBSERVED="not assessed - xlock binary probe returned unparseable output (rc=$CDE_XL_LS_RC)"
      fi
      ;;
    *)
      CDE_XL_STATUS=NOT_ASSESSED
      CDE_XL_OBSERVED="not assessed - xlock binary probe returned empty or unparseable output (rc=$CDE_XL_LS_RC)"
      ;;
  esac
elif [ "$CDE_XL_LSLPP_RC" -eq 1 ] && [ -z "$CDE_XL_LSLPP_RAW" ]; then
  CDE_XL_STATUS=NOT_APPLICABLE
  CDE_XL_OBSERVED="AIX CDE (X11) is not used; the X11.apps.clients fileset is not installed"
else
  CDE_XL_STATUS=NOT_ASSESSED
  CDE_XL_OBSERVED="not assessed - X11.apps.clients fileset probe failed or returned empty output (rc=$CDE_XL_LSLPP_RC)"
fi

case "$CDE_XL_STATUS" in
  NOT_APPLICABLE) CDE_XL_SEV=low ;;
  *) CDE_XL_SEV=med ;;
esac

case "$CDE_XL_STATUS" in
  PASS)
    CDE_XL_MEANING="AIX CDE provides the xlock command, so users can lock their sessions until reauthentication."
    CDE_XL_FIX="n/a"
    ;;
  FAIL)
    CDE_XL_MEANING="AIX CDE users cannot lock their sessions with xlock until reauthentication, leaving unattended sessions exposed."
    CDE_XL_FIX="install the missing CDE client support with 'installp -aXYgd /dev/cd0 -e /tmp/install.log X11.apps.clients'; PTxray only recommends this command."
    ;;
  NOT_APPLICABLE)
    CDE_XL_MEANING="AIX CDE (X11) is not used on this host, so the xlock requirement does not apply."
    CDE_XL_FIX="n/a"
    ;;
  *)
    CDE_XL_MEANING="PTxray did not obtain trustworthy evidence for the CDE xlock command."
    CDE_XL_FIX="verify the X11.apps.clients fileset state and the /usr/bin/X11/xlock binary as root, then rerun PTxray."
    ;;
esac

add security stig_cde_xlock "AIX CDE must provide the xlock command for session locking" \
  "$CDE_XL_STATUS" "$CDE_XL_SEV" "$CDE_XL_OBSERVED" \
  "$CDE_XL_MEANING" "$CDE_XL_FIX" "stig:V-215188"
}
