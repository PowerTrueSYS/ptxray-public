...
AIXRAY_TOOL=ck-stig-cron-allow-sysaccounts


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_cron_allow_sysaccounts — DISA STIG V-215192 (SV-215192r991589_rule,
  # AIX7-00-001033, severity medium, CCI-000366): AIX default system accounts
  # (with the exception of root) must not be listed in the cron.allow file or
  # must be included in the cron.deny file, if cron.allow does not exist.
  #
  # Read-only; the fix text is operator guidance and is never executed.
  #
  # Both probes run through aixv (stderr merged into stdout), not aix: AIX more
  # reports a missing file by exiting nonzero and writing the diagnostic to
  # STDERR, which aix() discards. Absence of /var/adm/cron/cron.allow is not a
  # refusal — it selects the cron.deny branch the STIG prescribes — so the
  # absence diagnostic must reach the parser. Through aix() a missing file and
  # a failed probe are the same evidence (rc!=0, empty stdout) and both would
  # refuse. Absence is keyed POSITIVELY on the diagnostic text, never on the
  # exit code alone: the diagnostic must name the probed file (the raw output
  # has to contain /var/adm/cron/cron.allow or /var/adm/cron/cron.deny) before
  # "No such file or directory", "not found", "cannot access" or "does not
  # exist" is believed. A command-not-found error (ksh: more: not found), a
  # permission-denied read, or any other rc!=0 text that names no file falls
  # through to failed and renders NOT_ASSESSED.
  #
  # Account parsing: cron.allow and cron.deny carry one account name per line;
  # blank lines and # comments are ignored, and the first whitespace token is
  # the account. Root is excepted. Only bin, sys, adm and lpd are the default
  # system accounts the STIG names.
  typeset SCA_AL_RAW SCA_AL_RC SCA_AL_STATE
  typeset SCA_DN_RAW SCA_DN_RC SCA_DN_STATE SCA_DN_PACK
  typeset SCA_AL_PARSE SCA_AL_PARSE_RC SCA_DN_PARSE SCA_MISSING
  typeset SCA_STATUS SCA_SEV SCA_OBSERVED SCA_MEANING SCA_FIX

  SCA_AL_RAW=$(aixv cron_allow more /var/adm/cron/cron.allow)
  SCA_AL_RC=$?
  # $(...) strips trailing newlines. A deny file of only blank lines is
  # still a successful read (no listed accounts), not a swallowed-empty
  # probe, so keep those bytes by packing the rc onto the body.
  SCA_DN_PACK=$(
    aixv cron_deny more /var/adm/cron/cron.deny
    printf '%s' "__SCA_RC__$?"
  )
  SCA_DN_RC=${SCA_DN_PACK##*__SCA_RC__}
  SCA_DN_RAW=${SCA_DN_PACK%__SCA_RC__*}
  SCA_AL_STATE=""
  SCA_DN_STATE=""
  SCA_STATUS=""
  SCA_SEV=med
  ...
  # --- classify each probe ------------------------------------------------
  # rc != 0 is a failure or an absence, told apart ONLY by the diagnostic text.
  if [ "$SCA_AL_RC" -ne 0 ]; then
    case "$SCA_AL_RAW" in
      *"/var/adm/cron/cron.allow"*"No such file or directory"*) SCA_AL_STATE=absent ;;
      *"/var/adm/cron/cron.allow"*"not found"*) SCA_AL_STATE=absent ;;
      *"/var/adm/cron/cron.allow"*"cannot access"*) SCA_AL_STATE=absent ;;
      *"/var/adm/cron/cron.allow"*"does not exist"*) SCA_AL_STATE=absent ;;
      *) SCA_AL_STATE=failed ;;
    esac
  elif [ -z "$SCA_AL_RAW" ]; then
    SCA_AL_STATE=empty
  else
    SCA_AL_STATE=content
  fi
  if [ "$SCA_DN_RC" -ne 0 ]; then
    case "$SCA_DN_RAW" in
      *"/var/adm/cron/cron.deny"*"No such file or directory"*) SCA_DN_STATE=absent ;;
      *"/var/adm/cron/cron.deny"*"not found"*) SCA_DN_STATE=absent ;;
      *"/var/adm/cron/cron.deny"*"cannot access"*) SCA_DN_STATE=absent ;;
      *"/var/adm/cron/cron.deny"*"does not exist"*) SCA_DN_STATE=absent ;;
      *) SCA_DN_STATE=failed ;;
    esac
  elif [ -z "$SCA_DN_RAW" ]; then
    SCA_DN_STATE=empty
  else
    SCA_DN_STATE=content
  fi

  # --- verdict -------------------------------------------------------------
  if [ "$SCA_AL_STATE" = failed ] || [ "$SCA_DN_STATE" = failed ]; then
    # Fail closed: a broken read proves nothing about the allow/deny pair.
    SCA_STATUS=NOT_ASSESSED
    if [ "$SCA_AL_STATE" = failed ]; then
      SCA_OBSERVED="not assessed - cron.allow probe failed (rc=$SCA_AL_RC)"
    else
      SCA_OBSERVED="not assessed - cron.deny probe failed (rc=$SCA_DN_RC)"
    fi
    SCA_MEANING="PTxray did not obtain a trustworthy read of the cron control files, so it cannot determine which accounts cron.allow or cron.deny grant access to."
    SCA_FIX="run 'more /var/adm/cron/cron.allow' and 'more /var/adm/cron/cron.deny', correct the capture or path problem, and rerun PTxray."
  elif [ "$SCA_AL_STATE" = absent ]; then
    # No cron.allow: every default system account must be listed in cron.deny.
    if [ "$SCA_DN_STATE" = absent ]; then
      SCA_STATUS=PASS
      SCA_OBSERVED="both /var/adm/cron/cron.allow and /var/adm/cron/cron.deny are absent"
      SCA_MEANING="With neither cron control file present, only root (uid 0) can submit jobs through crontab's default behavior, so no default system account is granted cron access; DISA STIG V-215192 is satisfied."
      SCA_FIX="n/a"
    elif [ "$SCA_DN_STATE" = empty ]; then
      # rc0+empty on the deny read is a swallowed error, not proof that
      # cron.deny omits the accounts: mirror the empty-allow refusal.
      SCA_STATUS=NOT_ASSESSED
      SCA_OBSERVED="not assessed - cron.deny probe returned no output with rc 0 (swallowed error)"
      SCA_MEANING="With /var/adm/cron/cron.allow absent, the STIG requires the default system accounts to be listed in /var/adm/cron/cron.deny, but the cron.deny read produced no output with rc 0; PTxray cannot tell an empty deny file from a dead probe, so it will not claim the finding."
      SCA_FIX="verify /var/adm/cron/cron.deny is readable and lists bin, sys, adm, and lpd, then rerun PTxray."
    else
      SCA_DN_PARSE=$(printf '%s\n' "$SCA_DN_RAW" | awk '
        BEGIN {
          order[1] = "bin"; order[2] = "sys"; order[3] = "adm"; order[4] = "lpd"
          sys["bin"] = 1; sys["sys"] = 1; sys["adm"] = 1; sys["lpd"] = 1
        }
        /^[ \t]*$/ { next }
        /^[ \t]*#/ { next }
        {
          ...
          sub(/[ \t]+.*$/, "", line)
          if (line == "") next
          if (line in sys && !(line in seen)) { seen[line] = 1 }
        }
        END {
          missing = ""
          for (i = 1; i <= 4; i++) {
            a = order[i]
            if (!(a in seen)) missing = missing (missing == "" ? "" : " ") a
          }
          if (missing == "") { print "all"; exit 0 }
          print "missing:" missing
          exit 1
        }
      ')
      if [ "$SCA_DN_PARSE" = "all" ]; then
        SCA_STATUS=PASS
        SCA_OBSERVED="cron.allow absent; cron.deny lists bin, sys, adm, and lpd"
        SCA_MEANING="With no /var/adm/cron/cron.allow, each default system account the STIG names is listed in /var/adm/cron/cron.deny, so none can submit cron jobs."
        SCA_FIX="n/a"
      else
        SCA_MISSING=${SCA_DN_PARSE#missing:}
        SCA_STATUS=FAIL
        SCA_OBSERVED="cron.allow absent and cron.deny does not list default system account(s): $SCA_MISSING"
        SCA_MEANING="With no /var/adm/cron/cron.allow, the default system accounts must be listed in /var/adm/cron/cron.deny; missing $SCA_MISSING leaves those accounts able to submit cron jobs."
        SCA_FIX="Remove default system accounts (such as bin, sys, adm, or lpd) from the /var/adm/cron/cron.allow file, or add those accounts to the /var/adm/cron/cron.deny file."
      fi
    fi
  elif [ "$SCA_AL_STATE" = empty ]; then
    # An empty cron.allow is a finding, but only when the deny probe gives a
    # trustworthy signal (real content, or a proven absence). When cron.deny is
    # ALSO rc0+empty the whole capture is empty and no empty-allow finding is
    # claimable: rc0+empty is a swallowed error, not evidence of an empty file.
    if [ "$SCA_DN_STATE" = empty ]; then
      SCA_STATUS=NOT_ASSESSED
      SCA_OBSERVED="not assessed - cron.allow probe returned no output with rc 0 (swallowed error)"
      SCA_MEANING="The cron.allow read succeeded with no output; PTxray cannot tell an empty file from a dead probe, so it will not claim the empty-allow finding."
      SCA_FIX="verify /var/adm/cron/cron.allow is readable and genuinely empty or populated, then rerun PTxray."
    else
      SCA_STATUS=FAIL
      SCA_OBSERVED="/var/adm/cron/cron.allow exists and is empty"
      SCA_MEANING="An empty /var/adm/cron/cron.allow is a finding under DISA STIG V-215192; the file must be removed or populated with the accounts permitted to use cron."
      SCA_FIX="Remove default system accounts (such as bin, sys, adm, or lpd) from the /var/adm/cron/cron.allow file, or add those accounts to the /var/adm/cron/cron.deny file."
    fi
  else
    # cron.allow exists with content: any default system account listed is a
    # finding. Root is excepted.
    SCA_AL_PARSE=$(printf '%s\n' "$SCA_AL_RAW" | awk '
      BEGIN {
        sys["bin"] = 1; sys["sys"] = 1; sys["adm"] = 1; sys["lpd"] = 1
        found = 0
        list = ""
      ...
      END {
        if (found) { print list; exit 0 }
        print ""
        exit 1
      }
    ')
    SCA_AL_PARSE_RC=$?
    if [ "$SCA_AL_PARSE_RC" -eq 0 ] && [ -n "$SCA_AL_PARSE" ]; then
      SCA_STATUS=FAIL
      SCA_OBSERVED="$SCA_AL_PARSE listed in /var/adm/cron/cron.allow"
      SCA_MEANING="A default system account is listed in /var/adm/cron/cron.allow and can submit cron jobs, which DISA STIG V-215192 prohibits (root excepted)."
      SCA_FIX="Remove default system accounts (such as bin, sys, adm, or lpd) from the /var/adm/cron/cron.allow file, or add those accounts to the /var/adm/cron/cron.deny file."
    else
      SCA_STATUS=PASS
      SCA_OBSERVED="no default system account (bin, sys, adm, lpd) in /var/adm/cron/cron.allow"
      SCA_MEANING="/var/adm/cron/cron.allow exists and lists no default system account other than root, so no default system account can submit cron jobs."
      SCA_FIX="n/a"
    fi
  fi

  add security stig_cron_allow_sysaccounts "Default system accounts in cron.allow/cron.deny" \
    "$SCA_STATUS" "$SCA_SEV" "$SCA_OBSERVED" "$SCA_MEANING" "$SCA_FIX" "stig:V-215192"
}
