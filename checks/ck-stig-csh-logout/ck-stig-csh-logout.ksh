...
AIXRAY_TOOL=ck-stig-csh-logout


function standalone_check {
_AIXRAY_SESSION_KEYS=""
# stig_csh_logout — DISA STIG V-215311.  csh/tcsh users must have a
# $HOME/.logout that displays a logout message when the session ends.
# The enumeration probe is the trusted subject: a nonzero rc or an rc-0
# empty listing is a failed or swallowed probe and refuses (NOT_ASSESSED).
# A determinate, non-empty user list decides: no csh/tcsh users ->
# NOT_APPLICABLE; any csh/tcsh user missing $HOME/.logout, or whose file
# does not display the STIG logout message, -> FAIL.
# lsuser rows whose shell is empty or absent are not csh/tcsh subjects and
# are skipped, not a parse failure (stock AIX ships many such accounts).
# Absence of a .logout is decided from the ls probe's output shape
# (nonzero rc with empty stdout), never from one magic rc value.  Each
# subject gets its own probe key so a fixture pair maps one-to-one with
# the observation.
typeset CSHLO_RAW CSHLO_RC CSHLO_PARSED CSHLO_PARSE_RC
typeset CSHLO_STATUS CSHLO_SEVERITY CSHLO_OBSERVED CSHLO_MEANING CSHLO_FIX
typeset CSHLO_USER CSHLO_HOME CSHLO_SHELL
typeset CSHLO_LS_RAW CSHLO_LS_RC CSHLO_CAT_RAW CSHLO_CAT_RC
typeset CSHLO_ANY_FAIL CSHLO_ANY_NA CSHLO_FAIL_MSG CSHLO_NA_MSG
...
CSHLO_RAW=$(aix csh_logout_users lsuser -a home shell ALL)
CSHLO_RC=$?
CSHLO_PARSED=""

if [ "$CSHLO_RC" -ne 0 ]; then
  CSHLO_STATUS=NOT_ASSESSED
  CSHLO_OBSERVED="not assessed - user enumeration failed (rc=$CSHLO_RC)"
elif [ -z "$CSHLO_RAW" ]; then
  CSHLO_STATUS=NOT_ASSESSED
  CSHLO_OBSERVED="not assessed - user enumeration returned no rows with rc 0"
else
  CSHLO_PARSED=$(printf '%s\n' "$CSHLO_RAW" | awk '
    NF {
      name = $1
      home = ""
      shell = ""
      has_attr = 0
      for (i = 2; i <= NF; i++) {
        if (index($i, "home=") == 1) { home = substr($i, 6); has_attr = 1 }
        else if (index($i, "shell=") == 1) { shell = substr($i, 7); has_attr = 1 }
        else if (index($i, "=") > 0) has_attr = 1
      }
      if (!has_attr) { bad = 1; next }
      if (home == "" || shell == "") next
      n = split(shell, parts, "/")
      base = parts[n]
      if (base == "csh" || base == "tcsh") print name "|" home "|" shell
    }
    END { if (bad) exit 2 }
  ')
  CSHLO_PARSE_RC=$?
  if [ "$CSHLO_PARSE_RC" -ne 0 ]; then
    CSHLO_STATUS=NOT_ASSESSED
    CSHLO_OBSERVED="not assessed - user enumeration was unparseable"
  elif [ -z "$CSHLO_PARSED" ]; then
    CSHLO_STATUS=NOT_APPLICABLE
    CSHLO_OBSERVED="no csh/tcsh users on this host"
  else
    while IFS='|' read -r CSHLO_USER CSHLO_HOME CSHLO_SHELL; do
      CSHLO_LS_RAW=$(aix "csh_logout_ls_$CSHLO_USER" ls -ld "$CSHLO_HOME/.logout")
      CSHLO_LS_RC=$?
      if [ "$CSHLO_LS_RC" -eq 0 ]; then
        if [ -z "$CSHLO_LS_RAW" ]; then
          # rc 0 with no listing: swallowed error, ambiguous -> NOT_ASSESSED
          CSHLO_ANY_NA=1
          CSHLO_NA_MSG="not assessed - ls of $CSHLO_HOME/.logout returned rc 0 with no output"
          continue
        fi
        # .logout listed: read its contents
        CSHLO_CAT_RAW=$(aix "csh_logout_cat_$CSHLO_USER" cat "$CSHLO_HOME/.logout")
        CSHLO_CAT_RC=$?
        if [ "$CSHLO_CAT_RC" -ne 0 ]; then
          CSHLO_ANY_NA=1
          CSHLO_NA_MSG="not assessed - read of $CSHLO_HOME/.logout failed (rc=$CSHLO_CAT_RC)"
          continue
        fi
        if printf '%s\n' "$CSHLO_CAT_RAW" | grep -F 'You are being disconnected' >/dev/null 2>/dev/null; then
          :
        else
          CSHLO_ANY_FAIL=1
          CSHLO_FAIL_MSG="$CSHLO_FAIL_MSG${CSHLO_FAIL_MSG:+, }$CSHLO_USER's .logout does not display the STIG logout message"
        fi
      elif [ -z "$CSHLO_LS_RAW" ]; then
        # nonzero rc with no listing: ls could not list the path, so the
        # required .logout is absent or unreadable -> the STIG finding
        CSHLO_ANY_FAIL=1
        CSHLO_FAIL_MSG="$CSHLO_FAIL_MSG${CSHLO_FAIL_MSG:+, }$CSHLO_USER is missing $CSHLO_HOME/.logout"
        continue
      else
        # nonzero rc with a partial listing: cannot attribute -> NOT_ASSESSED
        CSHLO_ANY_NA=1
        CSHLO_NA_MSG="not assessed - ls of $CSHLO_HOME/.logout failed (rc=$CSHLO_LS_RC)"
        continue
      fi
    done <<EOF
$CSHLO_PARSED
EOF
    if [ "$CSHLO_ANY_FAIL" -eq 1 ]; then
      CSHLO_STATUS=FAIL
      CSHLO_OBSERVED="$CSHLO_FAIL_MSG"
    elif [ "$CSHLO_ANY_NA" -eq 1 ]; then
      CSHLO_STATUS=NOT_ASSESSED
      CSHLO_OBSERVED="$CSHLO_NA_MSG"
    else
      CSHLO_STATUS=PASS
      CSHLO_OBSERVED="all csh/tcsh users have a .logout displaying the STIG logout message"
    fi
  fi
fi

case "$CSHLO_STATUS" in
  FAIL|NOT_APPLICABLE) CSHLO_SEVERITY=low ;;
  *) CSHLO_SEVERITY=med ;;
esac

case "$CSHLO_STATUS" in
  PASS)
    CSHLO_MEANING="Every csh/tcsh user's .logout file displays the required logout message, so an interactive csh/tcsh session ends with the STIG-mandated disconnection notice."
    CSHLO_FIX="n/a"
    ;;
  FAIL)
    CSHLO_MEANING="A csh/tcsh user lacks the required .logout file, or the file does not display the STIG logout message, so the user is not warned that they are being disconnected."
    CSHLO_FIX="create the missing .logout file and add 'echo \"You are being disconnected.\"' and 'sleep 5' to it; PTxray only recommends these actions."
    ;;
  NOT_APPLICABLE)
    CSHLO_MEANING="No csh/tcsh user exists on this host, so no .logout logout-message requirement applies."
    CSHLO_FIX="n/a"
    ;;
  *)
    CSHLO_MEANING="PTxray did not obtain trustworthy evidence for the csh/tcsh logout-message policy."
    CSHLO_FIX="verify the lsuser enumeration and the csh/tcsh users' .logout files as root, then rerun PTxray."
    ;;
esac

add security stig_csh_logout "csh/tcsh logout message" \
  "$CSHLO_STATUS" "$CSHLO_SEVERITY" "$CSHLO_OBSERVED" \
  "$CSHLO_MEANING" "$CSHLO_FIX" "stig:V-215311"
}
