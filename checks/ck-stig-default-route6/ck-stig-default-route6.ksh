...
AIXRAY_TOOL=ck-stig-default-route6


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_default_route6 — DISA STIG V-215264.
  # AIX must have a default IPv6 gateway when the host uses IPv6 unless the
  # host is itself a router. Three read-only probes decide: global inet6
  # presence (ifconfig -a), IPv6 forwarding (no -a, router-ness), and the
  # default inet6 route (netstat -rn -f inet6). Uncertain evidence fails
  # closed to NOT_ASSESSED; determinate absence of IPv6 or router-ness
  # renders NOT_APPLICABLE per the rule text.
  typeset SDR6_IFC_RAW SDR6_IFC_RC SDR6_IFC_SHAPE
  typeset SDR6_FWD_RAW SDR6_FWD_RC SDR6_FWD_VAL
  typeset SDR6_NET_RAW SDR6_NET_RC SDR6_NET_SHAPE
  typeset SDR6_STATUS SDR6_OBSERVED SDR6_MEANING SDR6_FIX SDR6_SEV

  SDR6_STATUS=""
  SDR6_OBSERVED=""

  SDR6_IFC_RAW=$(aix ifconfig_all ifconfig -a)
  SDR6_IFC_RC=$?
  SDR6_IFC_SHAPE=""
  if [ "$SDR6_IFC_RC" -ne 0 ]; then
    SDR6_STATUS=NOT_ASSESSED
    SDR6_OBSERVED="not assessed - ifconfig probe failed (rc=$SDR6_IFC_RC)"
  elif [ -z "$SDR6_IFC_RAW" ]; then
    SDR6_STATUS=NOT_ASSESSED
    SDR6_OBSERVED="not assessed - ifconfig probe returned empty output"
  else
    SDR6_IFC_SHAPE=$(printf '%s\n' "$SDR6_IFC_RAW" | awk '
      $1 == "inet6" {
        if (NF < 2) { bad = 1; next }
        addr = $2
        sub(/\/.*$/, "", addr)
        ...
        if (p == 0) { bad = 1; next }
        if (p == 1) next
        first = tolower(substr(addr, 1, p - 1))
        if (length("" first) == 4 && substr(first, 1, 2) == "fe") {
          c3 = substr(first, 3, 1)
          if (c3 >= "8" && c3 <= "b") next
        }
        global = 1
      ...
      END {
        ...
        if (global) print "global"; else print "none"
      }')
    case "$SDR6_IFC_SHAPE" in
      global) : ;;
      none)
        SDR6_STATUS=NOT_APPLICABLE
        SDR6_OBSERVED="no global inet6 address configured; the host does not use IPv6"
        ;;
      *)
        SDR6_STATUS=NOT_ASSESSED
        SDR6_OBSERVED="not assessed - ifconfig output was not parseable"
        ;;
    esac
  fi

  if [ -z "$SDR6_STATUS" ]; then
    SDR6_FWD_RAW=$(aix ip6_forward no -a)
    SDR6_FWD_RC=$?
    SDR6_FWD_VAL=""
    if [ "$SDR6_FWD_RC" -ne 0 ]; then
      SDR6_STATUS=NOT_ASSESSED
      SDR6_OBSERVED="not assessed - IPv6 forwarding probe failed (rc=$SDR6_FWD_RC)"
    elif [ -z "$SDR6_FWD_RAW" ]; then
      SDR6_STATUS=NOT_ASSESSED
      SDR6_OBSERVED="not assessed - IPv6 forwarding probe returned empty output"
    else
      SDR6_FWD_VAL=$(printf '%s\n' "$SDR6_FWD_RAW" | awk '
        $1 == "ip6forwarding" {
          val = $0
          sub(/^[^=]*=[ \t]*/, "", val)
          gsub(/[ \t]/, "", val)
          if (val != "0" && val != "1") exit 1
          print val
          exit
        }')
      if [ -z "$SDR6_FWD_VAL" ]; then
        SDR6_STATUS=NOT_ASSESSED
        SDR6_OBSERVED="not assessed - ip6forwarding row missing or not decidable"
      elif [ "$SDR6_FWD_VAL" = "1" ]; then
        SDR6_STATUS=NOT_APPLICABLE
        SDR6_OBSERVED="IPv6 forwarding is enabled; the host is a router"
      fi
    fi
  fi

  if [ -z "$SDR6_STATUS" ]; then
    SDR6_NET_RAW=$(aix netstat_inet6 netstat -rn -f inet6)
    SDR6_NET_RC=$?
    if [ "$SDR6_NET_RC" -ne 0 ]; then
      SDR6_STATUS=NOT_ASSESSED
      SDR6_OBSERVED="not assessed - inet6 route probe failed (rc=$SDR6_NET_RC)"
    elif [ -z "$SDR6_NET_RAW" ]; then
      SDR6_STATUS=NOT_ASSESSED
      SDR6_OBSERVED="not assessed - inet6 route probe returned empty output"
    else
      SDR6_NET_SHAPE=$(printf '%s\n' "$SDR6_NET_RAW" | awk '
        $1 == "Destination" { header = 1; next }
        header && $1 == "default" { print "default"; found = 1; exit }
        END {
          if (found) exit
          if (!header) { print "unparseable"; exit }
          print "nodefault"
        }')
      case "$SDR6_NET_SHAPE" in
        default)
          SDR6_STATUS=PASS
          SDR6_OBSERVED="default IPv6 route is defined"
          ;;
        nodefault)
          SDR6_STATUS=FAIL
          SDR6_OBSERVED="no default IPv6 route is defined"
          ;;
        *)
          SDR6_STATUS=NOT_ASSESSED
          SDR6_OBSERVED="not assessed - inet6 route table was not parseable"
          ;;
      esac
    fi
  fi
  ...
  if [ -z "$SDR6_STATUS" ]; then
    SDR6_STATUS=NOT_ASSESSED
    SDR6_OBSERVED="not assessed - IPv6 default route state could not be determined"
  fi

  case "$SDR6_STATUS" in
    PASS)
      SDR6_MEANING="A default IPv6 route is defined, so off-link IPv6 traffic has an exit path."
      SDR6_FIX="n/a"
      ;;
    FAIL)
      SDR6_MEANING="The host uses IPv6 but has no default inet6 route, so off-link IPv6 traffic cannot be routed."
      SDR6_FIX="define a default IPv6 gateway via 'smitty route' (route add -inet6 default <gateway>); PTxray only recommends this action."
      ;;
    NOT_APPLICABLE)
      SDR6_MEANING="The host either does not use IPv6 (no global inet6 address) or is itself a router, so the rule does not apply."
      SDR6_FIX="n/a"
      ;;
    *)
      SDR6_MEANING="PTxray did not obtain trustworthy evidence of the IPv6 default route configuration."
      SDR6_FIX="verify IPv6 usage, forwarding state, and the inet6 route table as root, then rerun PTxray."
      ;;
  esac

  SDR6_SEV=med
  if [ "$SDR6_STATUS" = "NOT_APPLICABLE" ]; then
    SDR6_SEV=low
  fi
  add security stig_default_route6 "Default IPv6 gateway" \
    "$SDR6_STATUS" "$SDR6_SEV" "$SDR6_OBSERVED" \
    "$SDR6_MEANING" "$SDR6_FIX" "stig:V-215264"
}
