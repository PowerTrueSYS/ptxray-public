...
AIXRAY_TOOL=ck-stig-devfile-worldwrite


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_devfile_worldwrite — DISA STIG V-215184 (medium).
  # Device files (block and char) on local jfs/jfs2 filesystems, and the
  # directories holding them, must not be world-writable except for vendor
  # intended device nodes such as /dev/null.  The probe mirrors the
  # check-content's two find passes but stays on local fstypes; one awk pass
  # applies the allowlist and the parent-directory test.  A host with no device
  # files at all is itself a failure of the required configuration, not a pass,
  # and a device-class probe that returns rc 0 with no rows cannot be told from
  # a swallowed error, so it refuses rather than assume absence.
  typeset DFW_B_RAW DFW_B_RC DFW_C_RAW DFW_C_RC
  typeset DFW_D_RAW DFW_D_RC DFW_RESULT DFW_PARSE_RC DFW_NDEV DFW_NHITS
  typeset DFW_NPAR DFW_REST DFW_DEV_PATHS DFW_PAR_PATHS
  typeset DFW_STATUS DFW_OBSERVED DFW_MEANING DFW_FIX

  DFW_ALLOWLIST=${DFW_ALLOWLIST:-"/dev/null /dev/zero /dev/tty /dev/random /dev/urandom"}

  DFW_B_RAW=$(aix devlist_b find / \( -fstype jfs -o -fstype jfs2 \) -type b -ls)
  DFW_B_RC=$?
  DFW_C_RAW=$(aix devlist_c find / \( -fstype jfs -o -fstype jfs2 \) -type c -ls)
  DFW_C_RC=$?
  DFW_D_RAW=$(aix devdirs_world find / \( -fstype jfs -o -fstype jfs2 \) -type d -perm -o+w -ls)
  DFW_D_RC=$?

  if [ "$DFW_B_RC" -ne 0 ] || [ "$DFW_C_RC" -ne 0 ] || [ "$DFW_D_RC" -ne 0 ]; then
    DFW_STATUS=NOT_ASSESSED
    DFW_OBSERVED="not assessed - device file probe failed (rc: b=$DFW_B_RC c=$DFW_C_RC d=$DFW_D_RC)"
  elif [ -z "$DFW_B_RAW" ] && [ -z "$DFW_C_RAW" ]; then
    DFW_STATUS=FAIL
    DFW_OBSERVED="no device files were found on any jfs/jfs2 filesystem; the required device configuration is missing"
  elif [ -z "$DFW_B_RAW" ] || [ -z "$DFW_C_RAW" ]; then
    DFW_STATUS=NOT_ASSESSED
    DFW_OBSERVED="not assessed - one device class probe returned no rows with rc 0; cannot distinguish a swallowed error from a genuine absence"
  else
    DFW_RESULT=$( {
        printf '%s\n' "$DFW_D_RAW"
        printf '%s\n' "--DEVS--"
        printf '%s\n' "$DFW_B_RAW"
        printf '%s\n' "$DFW_C_RAW"
      } | awk -v allow="$DFW_ALLOWLIST" '
      BEGIN {
        na = split(allow, atmp, " ")
        for (i = 1; i <= na; i++) if (atmp[i] != "") alw[atmp[i]] = 1
        section = "dirs"
        ndev = 0
        nhits = 0
        npar = 0
      ...
      {
        if ($0 == "--DEVS--") { section = "devs"; next }
        if ($0 == "") next
        if (section == "dirs") {
          if (NF < 7) { malformed = 1; next }
          mode = $3
          if (length("" mode) == 11 && substr(mode, 11, 1) == "+") mode = substr(mode, 1, 10)
          if (length("" mode) != 10 || substr(mode, 1, 1) != "d") { malformed = 1; next }
          pstart = 0
          for (i = 7; i <= NF; i++) if (substr($i, 1, 1) == "/") { pstart = i; break }
          if (pstart == 0) { malformed = 1; next }
          dpath = $pstart
          wdir[dpath] = 1
          next
        }
        ...
        ndev++
        ls = 0
        for (i = length("" dpath); i >= 1; i--) {
          if (substr(dpath, i, 1) == "/") { ls = i; break }
        }
        if (ls <= 1) parent = "/"
        else parent = substr(dpath, 1, ls - 1)
        if (parent in wdir && !(parent in parcounted)) {
          parcounted[parent] = 1
          npar++
          if (npstored < 10) {
            if (npstored == 0) parpaths = parent
            else parpaths = parpaths ", " parent
            npstored++
          }
        }
        if (substr(mode, 9, 1) == "w" && !(dpath in alw)) {
          nhits++
          if (nstored < 10) {
            if (nstored == 0) devpaths = dpath
            else devpaths = devpaths ", " dpath
            nstored++
          }
        }
      }
      END {
        if (malformed) { print "malformed"; exit 1 }
        printf "%d|%d|%d|%s|%s", ndev, nhits, npar, devpaths, parpaths
      }')
    DFW_PARSE_RC=$?
    if [ "$DFW_PARSE_RC" -ne 0 ]; then
      DFW_STATUS=NOT_ASSESSED
      DFW_OBSERVED="not assessed - device file listing was malformed"
    else
      DFW_NDEV=${DFW_RESULT%%\|*}
      DFW_REST=${DFW_RESULT#*\|}
      DFW_NHITS=${DFW_REST%%\|*}
      ...
      DFW_NPAR=${DFW_REST%%\|*}
      DFW_REST=${DFW_REST#*\|}
      DFW_DEV_PATHS=${DFW_REST%%\|*}
      DFW_PAR_PATHS=${DFW_REST#*\|}
      if [ "$DFW_NHITS" -gt 0 ] && [ "$DFW_NPAR" -gt 0 ]; then
        DFW_STATUS=FAIL
        DFW_OBSERVED="${DFW_NHITS} world-writable device file(s) (${DFW_DEV_PATHS}) and ${DFW_NPAR} world-writable device parent director(ies) (${DFW_PAR_PATHS}) on jfs/jfs2 filesystems"
      elif [ "$DFW_NHITS" -gt 0 ]; then
        DFW_STATUS=FAIL
        DFW_OBSERVED="${DFW_NHITS} world-writable device file(s): ${DFW_DEV_PATHS} on jfs/jfs2 filesystems"
      elif [ "$DFW_NPAR" -gt 0 ]; then
        DFW_STATUS=FAIL
        DFW_OBSERVED="${DFW_NPAR} world-writable device parent director(ies): ${DFW_PAR_PATHS} on jfs/jfs2 filesystems"
      else
        DFW_STATUS=PASS
        DFW_OBSERVED="no world-writable device file or device parent directory outside the vendor allowlist on jfs/jfs2 filesystems (${DFW_NDEV} device files reviewed)"
      fi
    fi
  fi
  ...
  case "$DFW_STATUS" in
    PASS)
      DFW_MEANING="Device files on local jfs/jfs2 filesystems and the directories holding them are not world-writable except for vendor-intended device nodes."
      DFW_FIX="n/a"
      ;;
    FAIL)
      DFW_MEANING="A device file or a directory holding device files is world-writable, letting any local user replace or remove a device node and so disrupt or divert the devices it represents."
      DFW_FIX="after confirming the write bit is not vendor-intended, run 'chmod o-w <path>' on each listed device file or parent directory; PTxray only recommends this action."
      ;;
    *)
      DFW_MEANING="PTxray did not obtain a trustworthy device file inventory."
      DFW_FIX="run the find probes as root, resolve the capture problem, and rerun PTxray."
      ;;
  esac

  add security stig_devfile_worldwrite "device file world-write" \
    "$DFW_STATUS" med "$DFW_OBSERVED" \
    "$DFW_MEANING" "$DFW_FIX" "stig:V-215184"
}
