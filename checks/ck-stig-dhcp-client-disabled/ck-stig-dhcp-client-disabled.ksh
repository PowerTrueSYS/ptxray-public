...
AIXRAY_TOOL=ck-stig-dhcp-client-disabled


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_dhcp_client_disabled — DISA STIG V-215355 (AIX7-00-003049): the AIX DHCP
  # client must be disabled. Read-only, non-root. Two aix probes capture raw
  # evidence: ps_dhcpcd -> "ps -ef" (process list), rc_tcpip_dhcpcd ->
  # "cat /etc/rc.tcpip" (boot configuration). The check-content commands from
  # manifest.json "commands" are reproduced verbatim below:
  #   ps -ef |grep dhcpcd
  #   grep "^start[[:blank:]]/usr/sbin/dhcpcd" /etc/rc.tcpip
  # Each grep filters the captured evidence, so the manifest command is the
  # effective command run once per probe. A probe that fails (rc != 0) or that
  # returns empty with rc 0 is NOT_ASSESSED — an rc-0 empty capture is a
  # swallowed error, never a determinate absence. Non-empty rc-0 evidence with
  # no dhcpcd process and no uncommented start line is PASS; a commented-out
  # "#start /usr/sbin/dhcpcd" line is the normal disabled state that
  # "chrctcp -d dhcpcd" produces, not documentation of a needed client. A
  # running dhcpcd process or an uncommented start line in /etc/rc.tcpip is
  # FAIL. The rule text's "needed and documented" exemption is carried only by
  # an explicit operator-authored "# PTXRAY-EXEMPT: V-215355 <justification>"
  # marker in /etc/rc.tcpip, and that marker takes precedence over FAIL.
  typeset PS_RAW PS_RC RC_RAW RC_RC PS_RUN RC_ON RC_EXEMPT ST SEV OBS MEAN FIX

  PS_RAW=$(aix ps_dhcpcd ps -ef)
  PS_RC=$?
  RC_RAW=$(aix rc_tcpip_dhcpcd cat /etc/rc.tcpip)
  RC_RC=$?

  if [ "$PS_RC" -ne 0 ]; then
    ST=NOT_ASSESSED; SEV=med
    OBS="not assessed - ps_dhcpcd probe failed (rc=$PS_RC)"
    MEAN="The process-list probe for V-215355 did not complete, so whether the DHCP client is running could not be determined."
    FIX="verify the ps capture succeeds, then rerun PTxray."
  elif [ "$RC_RC" -ne 0 ]; then
    ST=NOT_ASSESSED; SEV=med
    OBS="not assessed - rc_tcpip_dhcpcd probe failed (rc=$RC_RC)"
    MEAN="The /etc/rc.tcpip capture for V-215355 did not complete, so whether the DHCP client starts at boot could not be determined."
    FIX="verify /etc/rc.tcpip is readable, then rerun PTxray."
  elif [ -z "$PS_RAW" ]; then
    ST=NOT_ASSESSED; SEV=med
    OBS="not assessed - ps_dhcpcd capture empty (rc=0)"
    MEAN="The process-list capture returned empty with rc 0 — a swallowed capture, not evidence the DHCP client is absent."
    FIX="rerun PTxray so the process-list capture is populated."
  elif [ -z "$RC_RAW" ]; then
    ST=NOT_ASSESSED; SEV=med
    OBS="not assessed - rc_tcpip_dhcpcd capture empty (rc=0)"
    MEAN="The /etc/rc.tcpip capture returned empty with rc 0 — a swallowed capture, not evidence the DHCP client is absent."
    FIX="rerun PTxray so the /etc/rc.tcpip capture is populated."
  else
    PS_RUN=$(printf '%s\n' "$PS_RAW" | grep dhcpcd)
    RC_ON=$(printf '%s\n' "$RC_RAW" | grep "^start[[:blank:]]/usr/sbin/dhcpcd")
    RC_EXEMPT=$(printf '%s\n' "$RC_RAW" | grep "PTXRAY-EXEMPT:[[:blank:]]*V-215355")
    if [ -n "$RC_EXEMPT" ]; then
      ST=NOT_APPLICABLE; SEV=low
      OBS="operator-authored '# PTXRAY-EXEMPT: V-215355' marker in /etc/rc.tcpip records the documented-needed exemption"
      MEAN="V-215355 rule text makes the requirement Not Applicable when the DHCP client is needed by the system and is documented; the '# PTXRAY-EXEMPT: V-215355' marker in /etc/rc.tcpip records that documentation, so the control is Not Applicable."
      FIX="n/a"
    elif [ -n "$PS_RUN" ] || [ -n "$RC_ON" ]; then
      ST=FAIL; SEV=med
      if [ -n "$PS_RUN" ] && [ -n "$RC_ON" ]; then
        OBS="dhcpcd is running and is started from /etc/rc.tcpip"
      elif [ -n "$PS_RUN" ]; then
        OBS="dhcpcd is running"
      else
        OBS="dhcpcd is started from /etc/rc.tcpip"
      fi
      MEAN="The AIX DHCP client is not disabled: a dhcpcd process is running or /etc/rc.tcpip starts it at boot, so the system can obtain network configuration from an untrusted DHCP server. DISA STIG V-215355 requires the client be disabled."
      FIX="read-only recommendation (PTxray never runs it): 'chrctcp -d dhcpcd' removes the dhcpcd autostart entry from /etc/rc.tcpip, and a running dhcpcd process should be stopped."
    else
      ST=PASS; SEV=med
      OBS="dhcpcd is not running and is not started from /etc/rc.tcpip"
      MEAN="The AIX DHCP client is disabled: no dhcpcd process is running and /etc/rc.tcpip has no start entry for it. DISA STIG V-215355 is satisfied."
      FIX="n/a"
    fi
  fi

  add security stig_dhcp_client_disabled "The AIX DHCP client must be disabled" "$ST" "$SEV" "$OBS" "$MEAN" "$FIX" "stig:V-215355"
}
