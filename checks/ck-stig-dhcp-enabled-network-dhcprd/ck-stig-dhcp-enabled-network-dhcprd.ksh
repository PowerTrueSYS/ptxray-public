...
AIXRAY_TOOL=ck-stig-dhcp-enabled-network-dhcprd


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_dhcp_enabled_network_dhcprd — DISA STIG V-215356.
  # If DHCP is not enabled in the network on AIX, the dhcprd daemon must be disabled.
  # One read of /etc/rc.tcpip for an uncommented "start /usr/sbin/dhcprd" line. Any
  # output from the anchored grep is the finding (the daemon would start at boot).
  # Exit code gates first: grep exit 1 (no match) is the determinate absence that
  # passes; any other nonzero exit is a probe failure and renders NOT_ASSESSED even
  # if the probe emitted bytes; only an rc-0 capture decides FAIL (output present)
  # or the swallowed-error NOT_ASSESSED (empty).
  typeset DHR_RAW DHR_RC DHR_EVID
  DHR_RAW=$(aix dhcprd_rctcp grep "^start[[:blank:]]/usr/sbin/dhcprd" /etc/rc.tcpip)
  DHR_RC=$?
  if [ "$DHR_RC" -eq 1 ]; then
    add security stig_dhcp_enabled_network_dhcprd "DHCP daemon dhcprd starts at boot" PASS med \
        "no uncommented start /usr/sbin/dhcprd line in /etc/rc.tcpip" \
        "The dhcprd daemon does not start at boot, satisfying STIG V-215356 when DHCP is not enabled in the network." \
        "n/a" "stig:V-215356"
  elif [ "$DHR_RC" -ne 0 ]; then
    add security stig_dhcp_enabled_network_dhcprd "DHCP daemon dhcprd starts at boot" NOT_ASSESSED med \
        "not assessed - probe failed (rc=$DHR_RC)" \
        "PTxray could not read /etc/rc.tcpip to determine whether the dhcprd daemon starts at boot." \
        "restore read access to /etc/rc.tcpip and rerun PTxray." "stig:V-215356"
  elif [ -n "$DHR_RAW" ]; then
    DHR_EVID=$(printf '%s\n' "$DHR_RAW" | awk 'NR==1{print; exit}')
    add security stig_dhcp_enabled_network_dhcprd "DHCP daemon dhcprd starts at boot" FAIL med \
        "dhcprd starts at boot in /etc/rc.tcpip: $DHR_EVID" \
        "STIG V-215356 requires the dhcprd daemon to be disabled when DHCP is not enabled in the network; an uncommented start line in /etc/rc.tcpip would launch the daemon at every boot." \
        "disable the daemon with 'chrctcp -d dhcprd' (recommendation only; PTxray never runs it)." \
        "stig:V-215356"
  else
    add security stig_dhcp_enabled_network_dhcprd "DHCP daemon dhcprd starts at boot" NOT_ASSESSED med \
        "not assessed - probe returned no output with exit 0 (swallowed error, not absence)" \
        "PTxray could not tell an absent dhcprd start line from a probe that silently returned nothing." \
        "verify /etc/rc.tcpip is readable and rerun PTxray." "stig:V-215356"
  fi
}
