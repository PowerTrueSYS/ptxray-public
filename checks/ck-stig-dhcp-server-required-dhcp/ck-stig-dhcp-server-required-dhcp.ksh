...
AIXRAY_TOOL=ck-stig-dhcp-server-required-dhcp


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_dhcp_server_required_dhcp — DISA AIX STIG V3R3 V-215405
  # (SV-215405r958478_rule, AIX7-00-003104, CCI-000381, severity medium): if
  # the DHCP server is not required on AIX, the DHCP server must be disabled.
  #
  ...
  #
  # The rule is a prohibition on the dhcpsd boot start entry existing at all,
  # so the record text is NOT filtered or pattern-matched: any output is a
  # finding.  grep exits 0 when a line matches, 1 when nothing matches (the
  # documented no-match exit), and 2 on a read error.
  #
  # Probe: `grep "^start[[:blank:]]/usr/sbin/dhcpsd" /etc/rc.tcpip`
  # (probe key rctcp_dhcpsd).  Read-only; one command.
  #
  # Verdicts:
  #   rc 0 or 1, output non-empty -> FAIL med          (any output is a finding)
  #   rc 1, output empty           -> PASS med          (determinate absence; for
  #                                                      a prohibition rule absence
  #                                                      is the compliant state)
  ...
  # and "if the DHCP server is not required" is a site determination PTxray
  # cannot read, so the literal criterion is applied unconditionally.
  typeset DHCPSD_RC DHCPSD_RAW DHCPSD_PARSED DHCPSD_COUNT DHCPSD_BODY
  typeset DHCPSD_STATUS DHCPSD_OBSERVED DHCPSD_MEANING DHCPSD_FIX

  DHCPSD_RAW=$(aix rctcp_dhcpsd grep "^start[[:blank:]]/usr/sbin/dhcpsd" /etc/rc.tcpip)
  DHCPSD_RC=$?
  DHCPSD_COUNT=0
  DHCPSD_BODY=""
  ...
  if [ -n "$DHCPSD_RAW" ]; then
    DHCPSD_PARSED=$(printf '%s\n' "$DHCPSD_RAW" | awk '
      NF {
        records++
      ...
      END { printf "%d|%s\n", records + 0, body }
    ')
    DHCPSD_COUNT=${DHCPSD_PARSED%%\|*}
    DHCPSD_BODY=${DHCPSD_PARSED#*\|}
  fi

  if [ "$DHCPSD_RC" -ne 0 ] && [ "$DHCPSD_RC" -ne 1 ]; then
    DHCPSD_STATUS=NOT_ASSESSED
    DHCPSD_OBSERVED="not assessed - grep /etc/rc.tcpip for dhcpsd exited $DHCPSD_RC"
  elif [ "$DHCPSD_COUNT" -gt 0 ]; then
    DHCPSD_STATUS=FAIL
    DHCPSD_OBSERVED="grep /etc/rc.tcpip for dhcpsd returned $DHCPSD_COUNT record(s) - dhcpsd start entry present in /etc/rc.tcpip: $DHCPSD_BODY"
  elif [ "$DHCPSD_RC" -eq 0 ]; then
    DHCPSD_STATUS=NOT_ASSESSED
    DHCPSD_OBSERVED="not assessed - grep /etc/rc.tcpip for dhcpsd exited 0 with no output"
  else
    DHCPSD_STATUS=PASS
    DHCPSD_OBSERVED="grep /etc/rc.tcpip for dhcpsd returned no records (rc 1) - dhcpsd not present in /etc/rc.tcpip"
  fi

  case "$DHCPSD_STATUS" in
    PASS)
      DHCPSD_MEANING="No dhcpsd start entry exists in /etc/rc.tcpip, so the DHCP server is not started at boot, as required by DISA STIG V-215405."
      DHCPSD_FIX="n/a"
      ;;
    FAIL)
      DHCPSD_MEANING="A dhcpsd start entry exists in /etc/rc.tcpip, so the DHCP server is started at boot and exposes a DHCP attack surface this host may not need."
      DHCPSD_FIX="if this host is not required to serve DHCP, disable the boot entry: 'chrctcp -d dhcpsd'; PTxray only recommends this command and never runs it."
      ;;
    *)
      DHCPSD_MEANING="PTxray did not obtain trustworthy /etc/rc.tcpip evidence for the dhcpsd start entry, so V-215405 is unproven in either direction."
      DHCPSD_FIX="run 'grep \"^start[[:blank:]]/usr/sbin/dhcpsd\" /etc/rc.tcpip' as root, resolve the probe problem, and rerun PTxray."
      ;;
  esac

  add security stig_dhcp_server_required_dhcp "DHCP server disabled unless required" \
    "$DHCPSD_STATUS" med \
    "$DHCPSD_OBSERVED" \
    "$DHCPSD_MEANING" \
    "$DHCPSD_FIX" \
