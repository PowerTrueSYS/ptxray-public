...
AIXRAY_TOOL=ck-stig-dhcp-updatedns


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_dhcp_updatedns — DISA STIG V-215427 (medium).
  # The AIX DHCP client must not send dynamic DNS updates.  When AIX does not
  # run the DHCP client (lssrc -s dhcpcd reports the subsystem not on file) the
  # requirement is Not Applicable.  When the client is present, any "updateDNS"
  # directive in /etc/dhcpc.opt or /etc/dhcpcd.ini is a finding.  AIX grep exits
  # 2 when one of the named config files is absent while still scanning the
  # files it can open, so matched lines are graded FAIL regardless of rc; an
  # empty rc=2 result is disambiguated by a config-file readability probe.
  # Every uncertain evidence shape fails closed to NOT_ASSESSED, never PASS.
  typeset DHCP_NS_PRES_RAW DHCP_NS_PRES_RC DHCP_NS_PRESENT
  typeset DHCP_NS_CONF_RAW DHCP_NS_CONF_RC DHCP_NS_CONF_READ
  typeset DHCP_NS_RAW DHCP_NS_RC DHCP_NS_STATUS DHCP_NS_SEVERITY
  typeset DHCP_NS_OBSERVED DHCP_NS_MEANING DHCP_NS_FIX

  DHCP_NS_SEVERITY=med
  DHCP_NS_PRESENT=""
  DHCP_NS_PRES_RAW=$(aix dhcpcd_subsys lssrc -s dhcpcd)
  DHCP_NS_PRES_RC=$?

  if [ "$DHCP_NS_PRES_RC" -eq 0 ] && [ -n "$DHCP_NS_PRES_RAW" ]; then
    DHCP_NS_PRESENT=yes
  elif [ "$DHCP_NS_PRES_RC" -ne 0 ] &&
      printf '%s\n' "$DHCP_NS_PRES_RAW" | grep -q 'not on file'; then
    DHCP_NS_PRESENT=no
  fi

  if [ -z "$DHCP_NS_PRESENT" ]; then
    DHCP_NS_STATUS=NOT_ASSESSED
    if [ "$DHCP_NS_PRES_RC" -ne 0 ] && [ -z "$DHCP_NS_PRES_RAW" ]; then
      DHCP_NS_OBSERVED="not assessed - DHCP client presence probe failed (rc=$DHCP_NS_PRES_RC) with no output"
    elif [ "$DHCP_NS_PRES_RC" -ne 0 ]; then
      DHCP_NS_OBSERVED="not assessed - DHCP client presence probe output was unparseable (rc=$DHCP_NS_PRES_RC)"
    else
      DHCP_NS_OBSERVED="not assessed - DHCP client presence probe returned rc 0 with no output"
    fi
  elif [ "$DHCP_NS_PRESENT" = "no" ]; then
    DHCP_NS_STATUS=NOT_APPLICABLE
    DHCP_NS_SEVERITY=low
    DHCP_NS_OBSERVED="AIX does not run the DHCP client (lssrc -s dhcpcd: subsystem not on file); dynamic DNS update requirement is not applicable"
  else
    DHCP_NS_RAW=$(aix dhcp_updatedns grep "^updateDNS" /etc/dhcpc.opt /etc/dhcpcd.ini)
    DHCP_NS_RC=$?
    if [ -n "$DHCP_NS_RAW" ]; then
      DHCP_NS_STATUS=FAIL
      DHCP_NS_OBSERVED="DHCP client configured with updateDNS directive(s) (dynamic DNS updates enabled)"
    elif [ "$DHCP_NS_RC" -eq 1 ]; then
      DHCP_NS_STATUS=PASS
      DHCP_NS_OBSERVED="DHCP client present and no updateDNS directive in /etc/dhcpc.opt /etc/dhcpcd.ini"
    elif [ "$DHCP_NS_RC" -eq 0 ]; then
      DHCP_NS_STATUS=NOT_ASSESSED
      DHCP_NS_OBSERVED="not assessed - updateDNS grep returned rc 0 with no output (swallowed error, not a clean no-match)"
    elif [ "$DHCP_NS_RC" -eq 2 ]; then
      # grep could not open one or both config files but scanned the rest.
      # Count how many config files the readability probe actually lists.
      DHCP_NS_CONF_RAW=$(aix dhcpcd_conf_readable ls -l /etc/dhcpc.opt /etc/dhcpcd.ini)
      DHCP_NS_CONF_RC=$?
      DHCP_NS_CONF_READ=0
      # ls exits nonzero whenever one operand is absent, which is the very case
      # being measured; only 127 (command absent / no capture) means the probe
      # never produced a listing to count.
      if [ "$DHCP_NS_CONF_RC" -ne 127 ] && [ -n "$DHCP_NS_CONF_RAW" ]; then
        DHCP_NS_CONF_READ=$(printf '%s\n' "$DHCP_NS_CONF_RAW" | awk '
          { for (i = 1; i <= NF; i++)
              if ($i == "/etc/dhcpc.opt" || $i == "/etc/dhcpcd.ini") n++ }
          END { print n + 0 }')
        case "$DHCP_NS_CONF_READ" in
          ''|*[!0-9]*) DHCP_NS_CONF_READ=0 ;;
        esac
      fi
      if [ "$DHCP_NS_CONF_READ" -ge 1 ]; then
        DHCP_NS_STATUS=PASS
        DHCP_NS_OBSERVED="DHCP client present and no updateDNS directive in the readable DHCP config file(s) (one config file absent or unreadable)"
      else
        DHCP_NS_STATUS=NOT_ASSESSED
        DHCP_NS_OBSERVED="not assessed - no DHCP config file (/etc/dhcpc.opt /etc/dhcpcd.ini) was readable by the probe"
      fi
    else
      DHCP_NS_STATUS=NOT_ASSESSED
      DHCP_NS_OBSERVED="not assessed - updateDNS grep probe failed (rc=$DHCP_NS_RC)"
    fi
  fi

  case "$DHCP_NS_STATUS" in
    PASS)
      DHCP_NS_MEANING="No updateDNS directive is set in the AIX DHCP client configuration, so the client does not attempt dynamic DNS updates."
      DHCP_NS_FIX="n/a"
      ;;
    FAIL)
      DHCP_NS_MEANING="The AIX DHCP client is configured to send dynamic DNS updates, which can publish uncontrolled client records into the DNS namespace."
      DHCP_NS_FIX="Configure the system's DHCP client to not send dynamic DNS updates.; Remove or comment-out \"updateDNS\" lines from the \"/etc/dhcpcd.ini\" and \"/etc/dhcpc.opt\" files."
      ;;
    NOT_APPLICABLE)
      DHCP_NS_MEANING="AIX does not run the DHCP client, so this requirement does not apply."
      DHCP_NS_FIX="n/a"
      ;;
    *)
      DHCP_NS_MEANING="PTxray did not obtain trustworthy evidence of the DHCP client's dynamic DNS update setting."
      DHCP_NS_FIX="verify the DHCP client presence and the updateDNS configuration as root, then rerun PTxray."
      ;;
  esac

  add security stig_dhcp_updatedns "DHCP client dynamic DNS updates" \
    "$DHCP_NS_STATUS" "$DHCP_NS_SEVERITY" "$DHCP_NS_OBSERVED" \
    "$DHCP_NS_MEANING" "$DHCP_NS_FIX" "stig:V-215427"
}
