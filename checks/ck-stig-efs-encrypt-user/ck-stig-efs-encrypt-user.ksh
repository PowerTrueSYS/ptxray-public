...
AIXRAY_TOOL=ck-stig-efs-encrypt-user


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # efs_encrypt_user — DISA STIG V-215283.  AIX must encrypt user data at rest
  # using AIX Encrypted File System (EFS) when the organization requires it.
  # PARTIAL-AUTO: the decidable sub-condition is the EFS deployment state
  # (clic.rte.lib / clic.rte.kernext installed, and lsfs -q showing the
  # filesystem EFS-enabled).  The affirmative policy confirmation ("the
  # organization requires encryption of data at rest") is the site/ISSO manual
  # portion: when the probes cannot show EFS deployed and enabled the check
  # renders a typed NOT_ASSESSED naming that portion, never a silent PASS.
  # Absence of the EFS filesystem subject (/fs2_mnt not defined, or both
  # probes rc 127 command-not-found) renders NOT_APPLICABLE per the rule text
  # ("If the organization does not require to encrypt the data at rest, this
  # is Not Applicable").
  typeset EFS_FSET_RAW EFS_FSET_RC EFS_FSET_STATE EFS_FSET_PARSE EFS_FSET_PARSE_RC
  typeset EFS_LSLPP_RAW EFS_LSLPP_RC
  typeset EFS_FS_RAW EFS_FS_RC EFS_FS_STATE EFS_EFS_FLAG EFS_EFS_FLAG_RC
  typeset EFS_STATUS EFS_SEVERITY EFS_OBSERVED EFS_MEANING EFS_FIX

  # Both probes pass their argv as SEPARATE words. Quoting a whole pipeline as
  # one word made aix() -- which runs "$@" -- look for a binary literally named
  # "lslpp -l |grep clic": rc 127 on a live box, which this check then read as
  # determinate "EFS not installed". The clic filter now runs here, over the
  # captured lslpp output, which is where a pipeline belongs. rc semantics are
  # unchanged: lslpp's rc when lslpp fails, otherwise the filter's rc (0 hit,
  # 1 no match) -- exactly what the pipeline used to yield.
  EFS_LSLPP_RAW=$(aix efs_filesets lslpp -l)
  EFS_LSLPP_RC=$?
  if [ "$EFS_LSLPP_RC" -ne 0 ]; then
    EFS_FSET_RAW=""
    EFS_FSET_RC=$EFS_LSLPP_RC
  else
    EFS_FSET_RAW=$(printf '%s\n' "$EFS_LSLPP_RAW" | grep -i clic)
    EFS_FSET_RC=$?
  fi
  EFS_FS_RAW=$(aix efs_fs lsfs -q /fs2_mnt)
  EFS_FS_RC=$?
  ...
  # --- clic fileset probe: classify the lslpp -l |grep clic evidence ---
  # rc 127 + empty stdout is command-not-found: determinate EFS absence, not a
  # probe that could not run. grep no-match remains rc 1.
  if [ -z "$EFS_FSET_RAW" ]; then
    case "$EFS_FSET_RC" in
      0) EFS_FSET_STATE=swallowed ;;
      1) EFS_FSET_STATE=none ;;
      127) EFS_FSET_STATE=not-installed ;;
      *) EFS_FSET_STATE=probe-failed ;;
    esac
  elif [ "$EFS_FSET_RC" -ne 0 ]; then
    EFS_FSET_STATE=probe-failed
  else
    EFS_FSET_PARSE=$(printf '%s\n' "$EFS_FSET_RAW" | awk '
      /^[ \t]*$/ { next }
      {
        name = $1
        if (name == "clic.rte.lib") lib = 1
        else if (name == "clic.rte.kernext") kernext = 1
      }
      END {
        if (lib && kernext) print "complete"
        else print "incomplete"
      }
    ')
    EFS_FSET_PARSE_RC=$?
    if [ "$EFS_FSET_PARSE_RC" -ne 0 ]; then
      EFS_FSET_STATE=unparseable
    elif [ "$EFS_FSET_PARSE" = "complete" ]; then
      EFS_FSET_STATE=complete
    else
      EFS_FSET_STATE=incomplete
    fi
  fi

  # --- EFS filesystem probe: classify the lsfs -q /fs2_mnt evidence ---
  # rc 127 + empty stdout is command-not-found: determinate EFS absence
  # (the /fs2_mnt subject is not defined because EFS/lsfs is not present).
  # lsfs-undefined remains rc 1; probe-failed stays on other empty non-zero rcs.
  if [ -z "$EFS_FS_RAW" ]; then
    case "$EFS_FS_RC" in
      0) EFS_FS_STATE=swallowed ;;
      1) EFS_FS_STATE=subject-absent ;;
      127) EFS_FS_STATE=not-installed ;;
      *) EFS_FS_STATE=probe-failed ;;
    esac
  elif [ "$EFS_FS_RC" -ne 0 ]; then
    EFS_FS_STATE=probe-failed
  else
    EFS_EFS_FLAG=$(printf '%s\n' "$EFS_FS_RAW" | awk '
      /^[ \t]*$/ { next }
      {
        line = $0
        p = index(line, "EFS:")
        ...
        n = split(rest, f, /[ \t,)]+/)
        val = (n > 0 ? f[1] : "")
        if (val == "no") { print "no"; exit }
        if (val == "yes") { print "yes"; exit }
        print "unknown"; exit
      }
      END { if (!seen) print "absent" }
    ')
    EFS_EFS_FLAG_RC=$?
    if [ "$EFS_EFS_FLAG_RC" -ne 0 ]; then
      EFS_FS_STATE=unparseable
    else
      EFS_FS_STATE=present
    fi
  fi

  if [ "$EFS_FSET_STATE" = "probe-failed" ] || [ "$EFS_FS_STATE" = "probe-failed" ]; then
    EFS_STATUS=NOT_ASSESSED
    EFS_OBSERVED="not assessed - a probe failed to produce output (fileset probe state: $EFS_FSET_STATE, filesystem probe state: $EFS_FS_STATE); no trustworthy EFS evidence"
    EFS_MEANING="PTxray did not obtain trustworthy evidence for the EFS encryption state of /fs2_mnt."
    EFS_FIX="verify the clic fileset installation and 'lsfs -q /fs2_mnt' output, then rerun PTxray."
  elif [ "$EFS_FSET_STATE" = "swallowed" ] || [ "$EFS_FS_STATE" = "swallowed" ]; then
    EFS_STATUS=NOT_ASSESSED
    EFS_OBSERVED="not assessed - a probe returned rc 0 with no output (fileset probe state: $EFS_FSET_STATE, filesystem probe state: $EFS_FS_STATE); rc0+empty is a swallowed error, not absence"
    EFS_MEANING="PTxray did not obtain trustworthy evidence for the EFS encryption state of /fs2_mnt."
    EFS_FIX="verify the clic fileset installation and 'lsfs -q /fs2_mnt' output, then rerun PTxray."
  elif [ "$EFS_FSET_STATE" = "unparseable" ] || [ "$EFS_FS_STATE" = "unparseable" ]; then
    EFS_STATUS=NOT_ASSESSED
    EFS_OBSERVED="not assessed - probe output was not parseable; no trustworthy EFS evidence"
    EFS_MEANING="PTxray did not obtain trustworthy evidence for the EFS encryption state of /fs2_mnt."
    EFS_FIX="verify the clic fileset installation and 'lsfs -q /fs2_mnt' output, then rerun PTxray."
  elif [ "$EFS_FSET_STATE" = "not-installed" ] && [ "$EFS_FS_STATE" = "not-installed" ]; then
    EFS_STATUS=NOT_APPLICABLE
    EFS_OBSERVED="efs_filesets rc 127 and efs_fs rc 127 (command not found); Encrypted File System is not installed; per rule text ('If the organization does not require to encrypt the data at rest, this is Not Applicable') this is Not Applicable"
    EFS_MEANING="Encrypted File System is absent on this host: the clic fileset probe and the /fs2_mnt filesystem probe both returned command-not-found (rc 127) with no output. Per the rule text ('If the organization does not require to encrypt the data at rest, this is Not Applicable') this is Not Applicable."
    EFS_FIX="n/a"
  elif [ "$EFS_FSET_STATE" = "not-installed" ]; then
    EFS_STATUS=NOT_ASSESSED
    EFS_OBSERVED="not assessed - the efs_filesets probe could not run (rc 127, command not found); filesystem probe state: $EFS_FS_STATE"
    EFS_MEANING="PTxray did not obtain trustworthy evidence for the EFS encryption state of /fs2_mnt."
    EFS_FIX="verify the clic fileset installation and 'lsfs -q /fs2_mnt' output, then rerun PTxray."
  elif [ "$EFS_FS_STATE" = "not-installed" ]; then
    EFS_STATUS=NOT_ASSESSED
    EFS_OBSERVED="not assessed - the efs_fs probe could not run (rc 127, command not found); fileset probe state: $EFS_FSET_STATE"
    EFS_MEANING="PTxray did not obtain trustworthy evidence for the EFS encryption state of /fs2_mnt."
    EFS_FIX="verify the clic fileset installation and 'lsfs -q /fs2_mnt' output, then rerun PTxray."
  elif [ "$EFS_FS_STATE" = "subject-absent" ]; then
    EFS_STATUS=NOT_APPLICABLE
    EFS_OBSERVED="no EFS filesystem /fs2_mnt is defined; per rule text ('If the organization does not require to encrypt the data at rest, this is Not Applicable') this is Not Applicable - confirm the encrypt-at-rest requirement with the site ISSO"
    EFS_MEANING="No EFS filesystem subject (/fs2_mnt) is defined on this host; per the rule text ('If the organization does not require to encrypt the data at rest, this is Not Applicable') this is Not Applicable. Whether encryption of data at rest is actually required is confirmed with the site ISSO."
    EFS_FIX="n/a"
  elif [ "$EFS_FSET_STATE" = "none" ] || [ "$EFS_FSET_STATE" = "incomplete" ]; then
    EFS_STATUS=FAIL
    EFS_OBSERVED="clic.rte.lib or clic.rte.kernext is not installed; AIX EFS cannot encrypt user data at rest"
    EFS_MEANING="Without the clic.rte.lib and clic.rte.kernext filesets, AIX EFS cannot encrypt filesystems, so user data at rest is not encrypted (DISA STIG V-215283 finding)."
    EFS_FIX="install the EFS filesets and enable an EFS filesystem (PTxray only recommends these commands): installp -aXYgd /dev/cd0 -e /tmp/install.log clic.rte.lib; installp -aXYgd /dev/cd0 -e /tmp/install.log clic.rte.kernext; efsenable -a; crfs -v jfs2 -g rootvg -m /fs2 -a size=100M -a efs=yes; mount /fs2"
  elif [ "$EFS_EFS_FLAG" = "no" ]; then
    EFS_STATUS=FAIL
    EFS_OBSERVED="lsfs -q /fs2_mnt reports EFS: no; the data filesystem is not encrypted at rest"
    EFS_MEANING="The data filesystem /fs2_mnt is not encrypted with AIX EFS; user data at rest is exposed to read access (DISA STIG V-215283 finding)."
    EFS_FIX="install the EFS filesets and enable an EFS filesystem (PTxray only recommends these commands): installp -aXYgd /dev/cd0 -e /tmp/install.log clic.rte.lib; installp -aXYgd /dev/cd0 -e /tmp/install.log clic.rte.kernext; efsenable -a; crfs -v jfs2 -g rootvg -m /fs2 -a size=100M -a efs=yes; mount /fs2"
  elif [ "$EFS_EFS_FLAG" = "yes" ]; then
    EFS_STATUS=PASS
    EFS_OBSERVED="EFS enabled on /fs2_mnt (EFS: yes); clic.rte.lib and clic.rte.kernext installed"
    EFS_MEANING="User data at rest on /fs2_mnt is encrypted with AIX EFS: the clic.rte.lib and clic.rte.kernext filesets are installed and lsfs reports the filesystem EFS-enabled, satisfying the decidable sub-condition of DISA STIG V-215283."
    EFS_FIX="n/a"
  elif [ "$EFS_EFS_FLAG" = "unknown" ]; then
    EFS_STATUS=NOT_ASSESSED
    EFS_OBSERVED="not assessed - the EFS field in lsfs -q /fs2_mnt output is present but its value could not be parsed; whether the organization requires encryption of data at rest is a site/ISSO determination"
    EFS_MEANING="lsfs reports an EFS status for /fs2_mnt but PTxray could not read its value, so the decidable EFS deployment state cannot be confirmed; whether the organization requires encryption of data at rest is a site/ISSO determination; PTxray refuses a silent PASS."
    EFS_FIX="confirm with the site ISSO whether encryption of data at rest is required, and rerun PTxray."
  else
    EFS_STATUS=NOT_ASSESSED
    EFS_OBSERVED="not assessed - whether the organization requires encryption of data at rest is a site/ISSO determination; lsfs -q /fs2_mnt reports no EFS status for /fs2_mnt"
    EFS_MEANING="The decidable EFS deployment state cannot be confirmed and whether the organization requires encryption of data at rest is a site/ISSO determination; PTxray refuses a silent PASS."
    EFS_FIX="confirm with the site ISSO whether encryption of data at rest is required, and rerun PTxray."
  fi

  case "$EFS_STATUS" in
    FAIL) EFS_SEVERITY=high ;;
    NOT_APPLICABLE) EFS_SEVERITY=low ;;
    *) EFS_SEVERITY=med ;;
  esac

  add security stig_efs_encrypt_user "AIX EFS encryption of user data at rest" \
      "$EFS_STATUS" "$EFS_SEVERITY" "$EFS_OBSERVED" \
      "$EFS_MEANING" "$EFS_FIX" "stig:V-215283"
}
