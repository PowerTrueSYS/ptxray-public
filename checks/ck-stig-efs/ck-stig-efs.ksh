...
AIXRAY_TOOL=ck-stig-efs


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_efs — DISA STIG V-215207. AIX must protect the confidentiality and
  # integrity of all information at rest. PARTIAL-AUTO: the clic.rte fileset
  # presence and the lsfs -q EFS flag are decidable and may render PASS/FAIL.
  # /fs2_mnt is the STIG example path, not a required name. When it is not
  # configured, grade mounted jfs2 EFS flags. None enabled is NOT_APPLICABLE
  # (site designation required). NOT_ASSESSED when lsfs cannot run or the
  # all-fs EFS flags are unparseable.
  typeset EFS_CLIC_RAW EFS_CLIC_RC EFS_CLIC_STATE
  typeset EFS_FS_RAW EFS_FS_RC EFS_PARSED EFS_PARSE_RC
  typeset EFS_MP EFS_REST EFS_PARTIAL EFS_DETAIL
  typeset EFS_STATUS EFS_SEVERITY EFS_OBSERVED EFS_MEANING EFS_FIX

  # --- decidable: clic.rte fileset presence ---
  EFS_CLIC_RAW=$(aix clic_installed /usr/bin/lslpp -l)
  EFS_CLIC_RC=$?
  if [ -n "$EFS_CLIC_RAW" ]; then
    if printf '%s\n' "$EFS_CLIC_RAW" | awk '/clic\.rte/ { found = 1 } END { exit !found }'; then
      EFS_CLIC_STATE=installed
    else
      EFS_CLIC_STATE=absent
    fi
  elif [ "$EFS_CLIC_RC" -eq 1 ]; then
    EFS_CLIC_STATE=absent
  else
    EFS_CLIC_STATE=unknown
  fi

  if [ "$EFS_CLIC_STATE" = "unknown" ]; then
    EFS_STATUS=NOT_ASSESSED
    EFS_OBSERVED="not assessed - clic.rte fileset query returned no trustworthy row (rc=$EFS_CLIC_RC); install state is unproven"
  elif [ "$EFS_CLIC_STATE" = "absent" ]; then
    EFS_STATUS=FAIL
    EFS_OBSERVED="clic.rte fileset is not installed (lslpp -l |grep clic found no clic.rte row)"
  else
    # --- decidable: lsfs -q EFS flag on the required filesystem ---
    # aixv keeps stderr: AIX 7.3 prints the not-found line there.
    EFS_FS_RAW=$(aixv efs_mount /usr/sbin/lsfs -q /fs2_mnt)
    EFS_FS_RC=$?
    if [ "$EFS_FS_RC" -ne 0 ]; then
      if printf '%s\n' "$EFS_FS_RAW" | awk '/lsfs:.*0506-341/ && /Cannot find/ { found = 1 } /lsfs: No record matching/ && /was found in \/etc\/filesystems/ { found = 1 } END { exit !found }' || {
        [ "$EFS_FS_RC" -eq 1 ] && [ "x$EFS_FS_RAW" = x ]
      }; then
        # Both AIX not-found texts, and rc 1 with empty output (not-found
        # on stderr, discarded or replayed from efs_mount.err), mean
        # /fs2_mnt is not configured. Stream mounted jfs2 EFS flags; do
        # not buffer the listing in a ksh variable. Append producer rc so
        # a failing lsfs cannot be graded as absence.
        EFS_PARSED=$(
          {
            aix efs_all /usr/sbin/lsfs -q
            printf 'PTXRAY_EFS_ALL_RC=%s\n' "$?"
          } | awk '
          function finish_cand() {
            if (mp == "" || mp == "/fs2_mnt") return
            if (vfs != "jfs2") return
            if (sz !~ /^[0-9][0-9]*$/) return
            if (insp == "") insp = mp
            else insp = insp " " mp
            if (got_valid) {
              nvalid++
          ...
          $0 ~ /^PTXRAY_EFS_ALL_RC=/ {
            prod_seen = 1
            prod_rc = substr($0, index($0, "=") + 1) + 0
            next
          }
          $1 ~ /^\/dev\// && NF >= 3 {
            finish_cand()
            if ($2 == "--" && $3 ~ /^\//) { mp = $3; vfs = $4; sz = $5 }
            else if ($2 ~ /^\//) { mp = $2; vfs = $3; sz = $4 }
            else { mp = $3; vfs = $4; sz = $5 }
            got_valid = 0
            last_val = ""
            next
          }
          ...
          END {
            finish_cand()
            if (prod_seen != 1) exit 1
            if (prod_rc != 0) exit 1
            if (nvalid == 0) exit 1
            if (yes) { print "yes", yesl; exit }
            print "none", insp
          }')
        EFS_PARSE_RC=$?
        if [ "$EFS_PARSE_RC" -ne 0 ] || [ -z "$EFS_PARSED" ]; then
          EFS_STATUS=NOT_ASSESSED
          EFS_OBSERVED="not assessed - lsfs -q all-filesystems probe failed or EFS flags were unparseable"
        else
          EFS_REST=${EFS_PARSED#* }
          EFS_PARSED=${EFS_PARSED%% *}
          if [ "$EFS_PARSED" = "yes" ] && [ -n "$EFS_REST" ] && [ "$EFS_REST" != "yes" ]; then
            EFS_STATUS=PASS
            EFS_OBSERVED="clic.rte installed and lsfs -q reports EFS: yes on $EFS_REST (Encrypted File System enabled on the required filesystem)"
          else
            EFS_STATUS=NOT_APPLICABLE
            if [ "$EFS_PARSED" = "none" ] && [ -n "$EFS_REST" ] && [ "$EFS_REST" != "none" ]; then
              EFS_OBSERVED="no EFS-enabled filesystem; site designation required ($EFS_REST)"
            else
              EFS_OBSERVED="no EFS-enabled filesystem; site designation required"
            fi
          fi
        fi
      elif [ "$EFS_FS_RC" -eq 1 ] && [ -n "$EFS_FS_RAW" ]; then
        # rc=1 with output: defined in /etc/filesystems but not mounted or
        # not queryable. That filesystem cannot carry EFS data; grade any
        # remaining mounted candidates. Refuse only when none are readable.
        # Stream the all-fs listing; do not buffer it in a ksh variable.
        EFS_PARSED=$(
          {
            aix efs_all /usr/sbin/lsfs -q
            printf 'PTXRAY_EFS_ALL_RC=%s\n' "$?"
          ...
          function finish_cand() {
            ...
            if (got_valid) {
              ...
              if (last_val == "no") {
                no = 1
                if (first_no == "") first_no = mp
              } else if (first_yes == "") first_yes = mp
            } else unread = 1
          }
          $0 ~ /^PTXRAY_EFS_ALL_RC=/ {
            prod_seen = 1
          ...
          END {
            finish_cand()
            # lsfs -q exits 1 when one listed filesystem cannot be queried —
            # the very partial case this branch grades — so 0 and 1 are the
            # only producer statuses whose listing may be graded.
            if (prod_seen != 1) exit 1
            if (prod_rc != 0 && prod_rc != 1) exit 1
            if (!found) exit 1
            extra = ""
            if (unread) extra = " partial"
            if (no) { print "no", first_no extra; exit }
            print "yes", first_yes extra
          }')
        EFS_PARSE_RC=$?
        if [ "$EFS_PARSE_RC" -ne 0 ] || [ -z "$EFS_PARSED" ]; then
          EFS_STATUS=NOT_ASSESSED
          EFS_OBSERVED="not assessed - /fs2_mnt is defined, not mounted; every candidate filesystem is unreadable"
        else
          EFS_REST=${EFS_PARSED#* }
          EFS_PARSED=${EFS_PARSED%% *}
          EFS_MP=${EFS_REST%% *}
          if [ "$EFS_REST" = "$EFS_MP" ]; then
            EFS_PARTIAL=""
          else
            EFS_PARTIAL=${EFS_REST#* }
          fi
          if [ "$EFS_PARTIAL" = "partial" ]; then
            EFS_DETAIL="; WARN: candidate filesystem unreadable"
          else
            EFS_DETAIL=""
          fi
          if [ "$EFS_PARSED" = "no" ]; then
            EFS_STATUS=FAIL
            EFS_OBSERVED="/fs2_mnt is defined, not mounted; lsfs -q $EFS_MP shows EFS: no (information at rest is not encrypted on $EFS_MP)$EFS_DETAIL"
          else
            EFS_STATUS=PASS
            EFS_OBSERVED="/fs2_mnt is defined, not mounted; clic.rte installed and $EFS_MP reports EFS: yes (Encrypted File System enabled on the required filesystem)$EFS_DETAIL"
          fi
        fi
      else
        EFS_STATUS=NOT_ASSESSED
        EFS_OBSERVED="not assessed - lsfs -q /fs2_mnt probe failed (rc=$EFS_FS_RC)"
      fi
    elif [ -z "$EFS_FS_RAW" ]; then
      EFS_STATUS=NOT_ASSESSED
      EFS_OBSERVED="not assessed - lsfs -q /fs2_mnt returned empty with rc 0 (swallowed error, not absence)"
    else
      # Real lsfs -q prints the jfs2 attributes as one comma-separated
      # parenthesised block, e.g. "(..., VIX: yes, EFS: yes, ISNAPSHOT: no,
      # MAXEXT: 0, MountGuard: no)". Extract the EFS value as a delimited
      # token: after "EFS:" take the substring up to the next comma or ")",
      # then trim. Only a literal "yes" or "no" is decidable.
      EFS_PARSED=$(printf '%s\n' "$EFS_FS_RAW" | awk '
        /^[ \t]*$/ { next }
        {
          line = $0
          ...
          if (q > 0) val = substr(val, 1, q - 1)
          gsub(/^[ \t]+|[ \t]+$/, "", val)
          if (val == "yes" || val == "no") { flag = val; found = 1 }
          else bad = 1
        }
        END {
          if (bad || !found) exit 1
      ...
      if [ "$EFS_PARSE_RC" -ne 0 ] || [ -z "$EFS_PARSED" ]; then
        EFS_STATUS=NOT_ASSESSED
        EFS_OBSERVED="not assessed - lsfs -q /fs2_mnt output did not show a decidable EFS flag"
      elif [ "$EFS_PARSED" = "no" ]; then
        EFS_STATUS=FAIL
        EFS_OBSERVED="lsfs -q /fs2_mnt shows EFS: no (information at rest is not encrypted on /fs2_mnt)"
      else
        EFS_STATUS=PASS
        EFS_OBSERVED="clic.rte installed and /fs2_mnt reports EFS: yes (Encrypted File System enabled on the required filesystem)"
      fi
    fi
  fi

  if [ "$EFS_STATUS" = "NOT_APPLICABLE" ]; then
    EFS_SEVERITY=low
  else
    EFS_SEVERITY=med
  fi
  ...
  case "$EFS_STATUS" in
    PASS)
      EFS_MEANING="The clic.rte fileset is installed and the required filesystem reports EFS: yes, so information at rest on that filesystem is encrypted. Whether the organization requires data-at-rest encryption, and whether every in-scope data filesystem is EFS-enabled, remains a site/ISSO determination; the check verified the mechanism on the filesystem the STIG check text names."
      EFS_FIX="n/a"
      ;;
    FAIL)
      EFS_MEANING="Information at rest on AIX is not protected by the Encrypted File System: the clic.rte fileset is missing or the required filesystem reports EFS: no."
      EFS_FIX="Install the EFS runtime with 'installp -aXYqg -d /dev/cd0 clic.rte', enable EFS with 'efsenable -a', then create or convert the filesystem with EFS and mount it, e.g. 'crfs -v jfs2 -g rootvg -m /fs2 -a size=100M -a efs=yes' then 'mount /fs2'. PTxray only recommends these commands."
      ;;
    NOT_APPLICABLE)
      EFS_MEANING="The rule is Not Applicable when the organization does not require to encrypt the data at rest."
      EFS_FIX="n/a"
      ;;
    *)
      EFS_MEANING="PTxray did not obtain trustworthy evidence that the decidable EFS sub-conditions are met, or the filesystem the STIG check text names is not configured on this host."
      EFS_FIX="Confirm with the site ISSO which filesystems must be protected by EFS and whether the organization requires data-at-rest encryption, then rerun PTxray."
      ;;
  esac

  add security stig_efs "Encrypted File System for information at rest" \
    "$EFS_STATUS" "$EFS_SEVERITY" "$EFS_OBSERVED" \
    "$EFS_MEANING" "$EFS_FIX" "stig:V-215207"
}
