...
AIXRAY_TOOL=ck-stig-enhanced-rbac


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_enhanced_rbac -- DISA STIG V-215404 (AIX7-00-003102).
  # AIX must run with enhanced RBAC enabled: security functions stay isolated
  # from nonsecurity functions, system privilege is granted to OS admins
  # through RBAC roles, and unprivileged users cannot install system software.
  # One read-only probe of the sys0 enhanced_RBAC effective attribute; every
  # uncertain evidence shape fails closed to NOT_ASSESSED, never PASS.
  typeset ERBAC_RAW ERBAC_RC ERBAC_STATE ERBAC_VALUE
  typeset ERBAC_STATUS ERBAC_OBSERVED ERBAC_MEANING ERBAC_FIX

  ERBAC_RAW=$(aix lsattr_enhanced_rbac lsattr -E -l sys0 -a enhanced_RBAC)
  ERBAC_RC=$?
  ERBAC_VALUE=""

  if [ "$ERBAC_RC" -ne 0 ]; then
    ERBAC_STATE=probe-failed
  elif [ -z "$ERBAC_RAW" ]; then
    ERBAC_STATE=empty
  else
    ERBAC_STATE=$(printf '%s\n' "$ERBAC_RAW" | awk '
      BEGIN { saw = 0; done = 0 }
      {
        if (NF == 0) next
        saw = 1
        if ($1 == "enhanced_RBAC") {
          if (NF < 2) { print "novalue"; done = 1; exit }
          if ($2 == "true") { print "true"; done = 1; exit }
          print "v:" $2
          done = 1
          exit
        }
      }
      END { if (!done) { if (saw) print "absent"; else print "empty" } }
    ')
  fi
  ...
  case "$ERBAC_STATE" in
    true)
      ERBAC_STATUS=PASS
      ERBAC_OBSERVED="enhanced_RBAC=true (system-wide enhanced RBAC is enabled)"
      ERBAC_MEANING="Enhanced RBAC is enabled, isolating security functions from nonsecurity functions and gating system privilege through RBAC roles."
      ERBAC_FIX="n/a"
      ;;
    v:*)
      ERBAC_VALUE=${ERBAC_STATE#v:}
      ERBAC_STATUS=FAIL
      ERBAC_OBSERVED="enhanced_RBAC=$ERBAC_VALUE (system-wide enhanced RBAC is not enabled)"
      ERBAC_MEANING="Enhanced RBAC is disabled, so security functions are not isolated from nonsecurity functions and system privilege is not gated through RBAC roles."
      ERBAC_FIX="enable enhanced RBAC with 'chdev -l sys0 -a enhanced_RBAC=true' and reboot; PTxray only recommends this action."
      ;;
    absent)
      ERBAC_STATUS=FAIL
      ERBAC_OBSERVED="enhanced_RBAC attribute absent (required configuration missing)"
      ERBAC_MEANING="The enhanced_RBAC attribute is not configured on sys0, so enhanced RBAC is not enabled as required."
      ERBAC_FIX="enable enhanced RBAC with 'chdev -l sys0 -a enhanced_RBAC=true' and reboot; PTxray only recommends this action."
      ;;
    probe-failed)
      ERBAC_STATUS=NOT_ASSESSED
      ERBAC_OBSERVED="not assessed - lsattr probe failed (rc=$ERBAC_RC)"
      ERBAC_MEANING="PTxray did not obtain trustworthy evidence for the enhanced RBAC state."
      ERBAC_FIX="verify the 'lsattr -E -l sys0 -a enhanced_RBAC' probe as root, correct the capture problem, and rerun PTxray."
      ;;
    empty)
      ERBAC_STATUS=NOT_ASSESSED
      ERBAC_OBSERVED="not assessed - lsattr probe returned no output"
      ERBAC_MEANING="PTxray did not obtain trustworthy evidence for the enhanced RBAC state."
      ERBAC_FIX="verify the 'lsattr -E -l sys0 -a enhanced_RBAC' probe as root, correct the capture problem, and rerun PTxray."
      ;;
    novalue)
      ERBAC_STATUS=NOT_ASSESSED
      ERBAC_OBSERVED="not assessed - enhanced_RBAC row was missing its value"
      ERBAC_MEANING="PTxray did not obtain trustworthy evidence for the enhanced RBAC state."
      ERBAC_FIX="verify the 'lsattr -E -l sys0 -a enhanced_RBAC' probe as root, correct the capture problem, and rerun PTxray."
      ;;
    *)
      ERBAC_STATUS=NOT_ASSESSED
      ERBAC_OBSERVED="not assessed - unexpected lsattr output"
      ERBAC_MEANING="PTxray did not obtain trustworthy evidence for the enhanced RBAC state."
      ERBAC_FIX="verify the 'lsattr -E -l sys0 -a enhanced_RBAC' probe as root, correct the capture problem, and rerun PTxray."
      ;;
  esac

  add security stig_enhanced_rbac "Enhanced RBAC state" \
    "$ERBAC_STATUS" med "$ERBAC_OBSERVED" \
    "$ERBAC_MEANING" "$ERBAC_FIX" "stig:V-215404"
}
