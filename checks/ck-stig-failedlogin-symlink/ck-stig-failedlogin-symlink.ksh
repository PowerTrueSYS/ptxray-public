...
AIXRAY_TOOL=ck-stig-failedlogin-symlink


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_failedlogin_symlink — DISA STIG V-215286.
  # /etc/security/failedlogin must be a real file.  AIX appends one record
  # per unsuccessful remote login attempt to that file; when the path is a
  # symlink the record can be redirected to another file and the local audit
  # trail for failed remote logins is lost.  A determinately missing subject
  # fails the required configuration; every uncertain evidence shape is a
  # refusal (NOT_ASSESSED), never a PASS.
  typeset FLS_RAW FLS_RC FLS_LINE FLS_MODE FLS_STATUS FLS_OBSERVED
  typeset FLS_MEANING FLS_FIX

  FLS_RAW=$(aix failedlogin_ls ls -al /etc/security/failedlogin)
  FLS_RC=$?
  FLS_STATUS=""
  FLS_OBSERVED=""

  if [ "$FLS_RC" -ne 0 ]; then
    # ls exits 2 when the subject is absent; every other non-zero rc is a
    # capture failure and must stay a refusal (NOT_ASSESSED).
    if [ "$FLS_RC" -eq 2 ]; then
      FLS_STATUS=FAIL
      FLS_OBSERVED="/etc/security/failedlogin is absent (probe rc=2), so AIX cannot record unsuccessful remote logins locally"
    else
      FLS_STATUS=NOT_ASSESSED
      FLS_OBSERVED="not assessed - failedlogin probe failed (rc=$FLS_RC) with no usable output"
    fi
  elif [ -z "$FLS_RAW" ]; then
    FLS_STATUS=NOT_ASSESSED
    FLS_OBSERVED="not assessed - failedlogin probe returned empty output with rc=0; a swallowed error, not proof of a regular file"
  else
    FLS_LINE=$(printf '%s\n' "$FLS_RAW" | awk 'NF { print; exit }')
    FLS_MODE=$(printf '%s\n' "$FLS_LINE" | awk '{ print $1 }')
    case "$FLS_MODE" in
      l*)
        FLS_STATUS=FAIL
        FLS_OBSERVED="/etc/security/failedlogin is a symbolic link (mode=$FLS_MODE), so failed-login records can be redirected away from the local file"
        ;;
      -*)
        FLS_STATUS=PASS
        FLS_OBSERVED="/etc/security/failedlogin is a regular file (mode=$FLS_MODE)"
        ;;
      *)
        if printf '%s\n' "$FLS_LINE" | grep -Fq '->'; then
          FLS_STATUS=FAIL
          FLS_OBSERVED="/etc/security/failedlogin resolves through a symbolic link (mode=$FLS_MODE)"
        else
          FLS_STATUS=NOT_ASSESSED
          FLS_OBSERVED="not assessed - failedlogin probe output was not parseable (first field \"$FLS_MODE\")"
        fi
        ;;
    esac
  fi
  ...
  case "$FLS_STATUS" in
    PASS)
      FLS_MEANING="AIX records unsuccessful remote login attempts to the local file /etc/security/failedlogin, where they cannot be redirected."
      FLS_FIX="n/a"
      ;;
    FAIL)
      FLS_MEANING="A symbolic link at /etc/security/failedlogin lets the unsuccessful-login record be redirected to another file, so the local audit trail for failed remote logins is lost; a missing file means the required configuration is unmet."
      FLS_FIX="Remove the symlink so the path is a real file: rm /etc/security/failedlogin. PTxray only recommends this command; it never executes it."
      ;;
    *)
      FLS_MEANING="PTxray did not obtain trustworthy evidence that /etc/security/failedlogin is a real file, so the failed-login recording control cannot be confirmed."
      FLS_FIX="Verify the path as root with 'ls -al /etc/security/failedlogin' and rerun PTxray."
      ;;
  esac

  add security stig_failedlogin_symlink "Unsuccessful remote login recording" \
    "$FLS_STATUS" med "$FLS_OBSERVED" \
    "$FLS_MEANING" "$FLS_FIX" "stig:V-215286"
}
