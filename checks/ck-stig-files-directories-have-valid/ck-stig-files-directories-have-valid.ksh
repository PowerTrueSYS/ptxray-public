...
AIXRAY_TOOL=ck-stig-files-directories-have-valid


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_files_directories_have_valid — DISA STIG V-215420 (SV-215420r991589_rule,
  # AIX7-00-003125, severity medium, CCI-000366): all AIX files and directories
  # must have a valid group owner.
  #
  # One filesystem-wide read-only scan under the probe key find_nogroup, wrapped
  # so a completed scan always emits a completion sentinel on the wire:
  #   find / -nogroup -print; echo "fgh-scan-complete rc=$?"
  # The sentinel makes determinate absence distinguishable from a swallowed or
  # truncated probe: zero paths with the sentinel (embedded rc 0) is the
  # clean-scan PASS; a missing sentinel with no listed paths — including rc 0
  # with zero bytes — refuses (NOT_ASSESSED). find rc 1 or 2 with printed rows
  # is a partial scan: grade the rows and carry an unreadable-path WARN in the
  # observed text. A clean partial scan (sentinel rc 1 or 2, no listed paths)
  # is PASS with that WARN; listed paths on a partial scan are FAIL with the
  # same WARN. Other non-zero scan rc still refuses.
  # A find diagnostic naming the scan subject itself (/) is an unmet required
  # configuration (FAIL); per-path traversal diagnostics are noise and do not
  # change the verdict. The fix text is a recommendation only and is never
  # executed.
  typeset FGH_RAW FGH_RC FGH_GRADE FGH_PARSE FGH_PARSE_RC FGH_REST FGH_COUNT
  typeset FGH_FIRST FGH_DIAG FGH_EMBED_RC FGH_STATUS FGH_SEVERITY FGH_OBSERVED
  typeset FGH_MEANING FGH_FIX FGH_PARTIAL

  FGH_RAW=$(aix find_nogroup sh -c 'find / -nogroup -print; echo "fgh-scan-complete rc=$?"')
  FGH_RC=$?
  FGH_STATUS=""
  FGH_SEVERITY=med
  FGH_OBSERVED=""
  ...
  if [ "$FGH_RC" -eq 0 ]; then
    FGH_GRADE=1
  elif [ "$FGH_RC" -eq 1 ] || [ "$FGH_RC" -eq 2 ]; then
    [ -n "$FGH_RAW" ] && FGH_GRADE=1
  fi
  if [ "$FGH_GRADE" -eq 0 ]; then
    FGH_STATUS=NOT_ASSESSED
    FGH_OBSERVED="not assessed - find probe failed (rc=$FGH_RC)"
  else
    FGH_PARSE=$(printf '%s\n' "$FGH_RAW" | awk '
      NF {
        line = $0
        ...
        sub(/[ \t]+$/, "", line)
        if (line == "") next
        if (line ~ /^fgh-scan-complete rc=[0-9]+$/) {
          sentinel_line = line
          sub(/^fgh-scan-complete rc=/, "", sentinel_line)
          sentinel_rc = sentinel_line
          sentinel = 1
          next
        }
        if (line ~ /^\//) { count++; if (first == "") first = line; next }
        if (line ~ /No such file or directory|not found|cannot access|does not exist/) {
          if (line ~ /\/:/) { if (diag == "") diag = line }
          next
        }
        bad = 1
      }
      END {
        if (sentinel != 1) {
          if (count > 0) { print "P " count " " first; exit 0 }
          print "NS"
          exit 0
        }
        sentinel_rc = sentinel_rc + 0
        partial = 0
        if (sentinel_rc == 1 || sentinel_rc == 2) partial = 1
        else if (sentinel_rc != 0) { print "NR " sentinel_rc; exit 0 }
        if (count > 0) {
          if (partial) { print "PU " count " " first; exit 0 }
          print "P " count " " first
          exit 0
        }
        if (diag != "") { print "D " diag; exit 0 }
        if (bad) exit 2
        if (partial) { print "U"; exit 0 }
        print "C"
        exit 0
      }
    ')
    FGH_PARSE_RC=$?
    if [ "$FGH_PARSE_RC" -ne 0 ] || [ -z "$FGH_PARSE" ]; then
      FGH_STATUS=NOT_ASSESSED
      FGH_OBSERVED="not assessed - find output was not parseable"
    else
      case "$FGH_PARSE" in
        P*)
          case "$FGH_PARSE" in
            PU*)
              FGH_REST=${FGH_PARSE#PU }
              FGH_PARTIAL=1
              ;;
            *)
              FGH_REST=${FGH_PARSE#P }
              FGH_PARTIAL=0
              ;;
          esac
          FGH_COUNT=${FGH_REST%% *}
          FGH_FIRST=${FGH_REST#* }
          FGH_STATUS=FAIL
          FGH_OBSERVED="$FGH_COUNT path(s) without a valid group owner; first: $FGH_FIRST"
          if [ "$FGH_PARTIAL" -eq 1 ]; then
            FGH_OBSERVED="$FGH_OBSERVED; some paths unreadable"
          fi
          FGH_MEANING="A file or directory without a valid group owner is detached from any accountable group and can be claimed or altered by an unintended group."
          FGH_FIX="assign each listed path a valid group, e.g. 'chgrp <a-valid-group> $FGH_FIRST'; PTxray only recommends this command and never runs it."
          ;;
        D*)
          FGH_DIAG=${FGH_PARSE#D }
          FGH_STATUS=FAIL
          FGH_OBSERVED="the filesystem scan subject is missing: $FGH_DIAG"
          FGH_MEANING="The required configuration — every file and directory having a valid group owner — is unmet because the scan subject could not be read."
          FGH_FIX="restore the filesystem scan subject so the whole tree can be checked, then assign a valid group to every file or directory that lacks one; PTxray only recommends these actions."
          ;;
        NS)
          FGH_STATUS=NOT_ASSESSED
          FGH_OBSERVED="not assessed - scan did not complete"
          FGH_MEANING="The find probe produced no completion sentinel, so a clean-scan absence of listed paths cannot be told apart from a swallowed or truncated capture."
          FGH_FIX="rerun PTxray so the find scan completes to the sentinel, then reassess."
          ;;
        NR*)
          FGH_EMBED_RC=${FGH_PARSE#NR }
          FGH_STATUS=NOT_ASSESSED
          FGH_OBSERVED="not assessed - find scan failed (rc=$FGH_EMBED_RC)"
          FGH_MEANING="The find probe reached the completion sentinel but the scan itself exited non-zero, so the listing is not a complete scan of the whole tree."
          FGH_FIX="rerun PTxray so the find scan completes with rc 0, then reassess."
          ;;
        U)
          FGH_STATUS=PASS
          FGH_OBSERVED="no file or directory without a valid group owner; some paths unreadable"
          FGH_MEANING="Every listed file and directory has a group that resolves to a known identity, so no printed path can be claimed or altered by an unintended group. Some paths were unreadable, so the scan is partial."
          FGH_FIX="n/a"
          ;;
        C)
          FGH_STATUS=PASS
          FGH_OBSERVED="no file or directory without a valid group owner"
          FGH_MEANING="Every file and directory on the system has a group that resolves to a known identity, so no path can be claimed or altered by an unintended group."
          FGH_FIX="n/a"
          ;;
        *)
          FGH_STATUS=NOT_ASSESSED
  ...
  add security stig_files_directories_have_valid "Files and directories with a valid group owner" \
    "$FGH_STATUS" "$FGH_SEVERITY" "$FGH_OBSERVED" "$FGH_MEANING" "$FGH_FIX" "stig:V-215420"
}
