...
AIXRAY_TOOL=ck-stig-forward-absent


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_forward_absent — DISA STIG V-215413 (AIX7-00-003115, severity low).
  # STIG literal: `find / -name .forward -print`; if any .forward file exists,
  # this is a finding. find exits 0 whether it matched paths or matched nothing,
  # so rc 0 with empty output is determinate absence ONLY when a separate anchor
  # proves the scan actually ran. Absent that anchor an empty result is a
  # swallowed error and the state is unknown, never clean. find rc 1 or 2 over a
  # live tree means some path was unreadable or vanished while every readable
  # row still printed; those rows are graded. Empty stdout at rc 1/2 plus a
  # live anchor is determinate absence, with a WARN-level unreadable-path
  # detail, except a complete fixture overlay that already recorded home_tree
  # keeps the established probe-failed refusal. NOT_ASSESSED when the probe
  # produced no rows and no liveness anchor proved it ran, or when rc is
  # outside 0/1/2.
  typeset SFA_ANCHOR SFA_ANCHOR_RC SFA_ANCHOR_OK
  typeset SFA_FWD SFA_FWD_RC SFA_PARSED SFA_REST
  typeset SFA_COUNT SFA_BAD SFA_SAMPLE SFA_PARTIAL
  typeset SFA_HOME SFA_HOME_RC SFA_HOME_OK SFA_HOME_KNOWN SFA_DETAIL
  typeset SFA_STATUS SFA_SEVERITY SFA_OBSERVED SFA_MEANING SFA_FIX
  typeset SFA_CONTROLS SFA_REASON

  SFA_CONTROLS='stig:V-215413'
  SFA_STATUS=''
  SFA_REASON=''
  SFA_PARTIAL=0
  SFA_COUNT=0
  ...
  # proof that an empty .forward result means absence rather than a dead probe.
  SFA_ANCHOR=$(aix scan_anchor find / -prune -print)
  SFA_ANCHOR_RC=$?
  SFA_ANCHOR_OK=0
  if [ "$SFA_ANCHOR_RC" -ne 0 ]; then
    SFA_REASON="filesystem scan anchor unavailable (rc=$SFA_ANCHOR_RC)"
  elif [ -z "$SFA_ANCHOR" ]; then
    SFA_REASON='probe returned no output with rc 0 (swallowed error)'
  elif printf '%s\n' "$SFA_ANCHOR" | awk '
    ...
    SFA_REASON='filesystem scan anchor output unrecognized'
  fi

  if [ "$SFA_ANCHOR_OK" -eq 0 ]; then
    SFA_STATUS=NOT_ASSESSED
    SFA_SEVERITY=med
    SFA_OBSERVED="not assessed - $SFA_REASON"
    SFA_MEANING='PTxray could not prove a filesystem scan ran, so the absence of .forward files cannot be asserted; the state required by V-215413 is unknown.'
    SFA_FIX='confirm the account running PTxray can traverse / and rerun the assessment.'
  else
    # (2) The STIG literal, and the sole evidence of a finding.
    # rc 1 or 2 is partial evidence, not a dead probe: grade printed rows.
    SFA_FWD=$(aix find_forward find / -name .forward -print)
    SFA_FWD_RC=$?
    case "$SFA_FWD_RC" in
      0) ;;
      1|2) SFA_PARTIAL=1 ;;
      *)
        SFA_STATUS=NOT_ASSESSED
        SFA_SEVERITY=med
        SFA_OBSERVED="not assessed - find / -name .forward -print failed (rc=$SFA_FWD_RC)"
        SFA_MEANING='The system-wide .forward scan did not complete, so PTxray cannot assert that no .forward file exists; the state required by V-215413 is unknown.'
        SFA_FIX='resolve the scan failure (traversal permission or I/O error) and rerun the assessment.'
        ;;
    esac
    # Empty stdout at rc 1/2: a live box and an incomplete capture (home_tree
    # never recorded) are determinate absence. A complete fixture overlay that
    # already captured home_tree keeps the established probe-failed refusal.
    if [ -z "$SFA_STATUS" ] && [ "$SFA_PARTIAL" -eq 1 ] && [ -z "$SFA_FWD" ]; then
      if aix_capture_missing home_tree || [ -z "${AIXRAY_FIXTURES:-}" ]; then
        :
      else
        SFA_STATUS=NOT_ASSESSED
        SFA_SEVERITY=med
    ...
    if [ -z "$SFA_STATUS" ]; then
      SFA_PARSED=$(printf '%s\n' "$SFA_FWD" | awk '
        ...
        END { printf "%d|%d|%s\n", n + 0, bad + 0, sample }
      ')
      SFA_COUNT=${SFA_PARSED%%\|*}
      SFA_REST=${SFA_PARSED#*\|}
      SFA_BAD=${SFA_REST%%\|*}
      SFA_SAMPLE=${SFA_REST#*\|}
      case "$SFA_COUNT$SFA_BAD" in
        ''|*[!0-9]*)
          SFA_STATUS=NOT_ASSESSED
          SFA_SEVERITY=med
          SFA_OBSERVED='not assessed - .forward scan output could not be parsed'
          SFA_MEANING='The .forward scan returned output PTxray could not read as a path list, so the state required by V-215413 is unknown.'
          SFA_FIX='rerun the assessment and, if the output remains unreadable, capture it for review.'
          ;;
      esac
      # A non-blank line that is not an absolute path is unparseable evidence,
      # never absence. Refuse rather than manufacture a clean result — but only
      # when the scan found nothing at all: a confirmed .forward path is
      # dispositive evidence and wins over unreadable lines (reported as FAIL,
      # with the discarded lines counted in the observed string). Absolute paths
      # whose basename is not .forward are tolerated noise: find would never
      # print them, so they can only appear in a synthetic replay.
      if [ -z "$SFA_STATUS" ] && [ "$SFA_COUNT" -eq 0 ] && [ "$SFA_BAD" -gt 0 ]; then
        SFA_STATUS=NOT_ASSESSED
        SFA_SEVERITY=med
        SFA_OBSERVED='not assessed - .forward scan returned output that could not be read as absolute paths'
        SFA_MEANING='The .forward scan produced output PTxray could not read as absolute paths, so it cannot assert that no .forward file exists.'
        SFA_FIX='capture the raw output of the .forward scan for review and rerun the assessment.'
      fi
    fi
  fi

  if [ -z "$SFA_STATUS" ] && [ "$SFA_COUNT" -gt 0 ]; then
    # (3) Any .forward file at all is a finding. No threshold, no tolerance.
    if [ "$SFA_COUNT" -gt 3 ]; then
      SFA_SAMPLE="$SFA_SAMPLE, ..."
    fi
    SFA_STATUS=FAIL
    SFA_SEVERITY=low
    SFA_OBSERVED="$SFA_COUNT .forward file(s) present: $SFA_SAMPLE"
    if [ "$SFA_BAD" -gt 0 ]; then
      SFA_OBSERVED="$SFA_OBSERVED ($SFA_BAD unreadable line(s) ignored)"
    fi
    SFA_MEANING='At least one .forward file exists on the system. A .forward file silently diverts mail destined for one user to an arbitrary address, so an unintended account or system can collect it. DISA STIG V-215413 prohibits the file outright.'
    SFA_FIX='delete every .forward path this finding lists once the file owner confirms it is not required, then rerun the assessment; PTxray only reports the paths and never deletes a file.'
  fi

  if [ -z "$SFA_STATUS" ]; then
    # (4)/(5) Determinate absence. This is a prohibition rule, so a missing
    # subject tree is the compliant state, not a refusal. home_tree is skipped
    # when this capture never recorded it, rather than treating a missing
    # capture as "/home not present". rc 1/2 appends a WARN-level unreadable
    # path detail; stderr was not captured, so the literal is used.
    SFA_HOME_OK=0
    SFA_HOME_KNOWN=0
    if aix_capture_missing home_tree; then
      :
    else
      SFA_HOME_KNOWN=1
      SFA_HOME=$(aix home_tree find /home -prune -print)
      SFA_HOME_RC=$?
      if [ "$SFA_HOME_RC" -eq 0 ] && [ -n "$SFA_HOME" ]; then
        if printf '%s\n' "$SFA_HOME" | awk '
    ...
    SFA_FIX='n/a'
    SFA_DETAIL=''
    if [ "$SFA_HOME_KNOWN" -eq 1 ] && [ "$SFA_HOME_OK" -eq 1 ]; then
      SFA_DETAIL='user home tree present'
      SFA_MEANING='A system-wide scan completed and found no .forward file, as DISA STIG V-215413 requires.'
    elif [ "$SFA_HOME_KNOWN" -eq 1 ]; then
      SFA_DETAIL='user home tree /home not present'
      SFA_MEANING='A system-wide scan completed and found no .forward file. The /home tree that normally carries them is itself not present, which for a prohibition rule such as V-215413 is the compliant state.'
    else
      SFA_MEANING='A system-wide scan completed and found no .forward file, as DISA STIG V-215413 requires.'
    fi
    if [ "$SFA_PARTIAL" -eq 1 ]; then
      if [ -n "$SFA_DETAIL" ]; then
        SFA_DETAIL="$SFA_DETAIL; some paths unreadable"
      else
        SFA_DETAIL='some paths unreadable'
      fi
    fi
    if [ -n "$SFA_DETAIL" ]; then
      SFA_OBSERVED="no .forward files found under / ($SFA_DETAIL)"
    else
      SFA_OBSERVED='no .forward files found under /'
    fi
  fi

  add security stig_forward_absent "Forward file (.forward)" \
      "$SFA_STATUS" "$SFA_SEVERITY" \
      "$SFA_OBSERVED" "$SFA_MEANING" "$SFA_FIX" "$SFA_CONTROLS"
}
