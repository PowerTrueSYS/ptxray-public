...
AIXRAY_TOOL=ck-stig-globalinit-ldr-preload


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_globalinit_ldr_preload — DISA STIG V-215423 (AIX7-00-003128).
  # The global initialization files must list only absolute library paths (or
  # $PATH) in LDR_PRELOAD. A relative entry lets a process load a library from
  # the current directory, so an attacker can inject an attacker-controlled
  # library into any session that sources the file. The check probes with the
  # STIG's own command and inspects every LDR_PRELOAD value it emits. A missing
  # LDR_PRELOAD list is itself a FAIL (required configuration unmet) whenever
  # grep found no assignment, whether it exited 1 (no lines selected) or 2 (a
  # named file was unreadable or absent). Only a rc outside grep's own 0/1/2
  # (probe failure) or rc 0 with empty output (a swallowed error) is
  # NOT_ASSESSED.
  typeset GLP_RAW GLP_RC GLP_RESULT GLP_PARSE_RC
  typeset GLP_STATUS GLP_OBSERVED GLP_MEANING GLP_FIX
  typeset GLP_FILE GLP_REL GLP_TOKEN

  GLP_RAW=$(aix grep_ldr_preload grep LDR_PRELOAD /etc/profile /etc/bashrc /etc/security/.login /etc/environment /etc/security/environ)
  GLP_RC=$?

  # Decide on the parsed stdout first, not on rc alone. grep's own exit status
  # is 0 (a line selected), 1 (no lines selected), or 2 (an input file was
  # unreadable or absent); a rc outside that set means the probe binary itself
  # failed.
  if [ "$GLP_RC" -gt 2 ]; then
    GLP_STATUS=NOT_ASSESSED
    GLP_OBSERVED="not assessed - LDR_PRELOAD probe failed (rc=$GLP_RC)"
  else
    # Every grep line is "file:LDR_PRELOAD=value"; strip the filename, find the
    # assignment (export prefix and quotes ok), and split the value on ':'. An
    # empty slot (leading/trailing/double colon, or an empty value) is a FAIL;
    # a token that is neither an absolute path nor $PATH / ${PATH} is a FAIL.
    # Comment lines and lines that merely mention LDR_PRELOAD are ignored.
    GLP_RESULT=$(printf '%s\n' "$GLP_RAW" | awk '
      {
        if (bad) next
        line = $0
        ...
        sub(/^[ \t]*/, "", probe)
        if (probe ~ /^#/) next
        q = index(rest, "LDR_PRELOAD=")
        if (q == 0) next
        found = 1
        value = substr(rest, q + 12)
        gsub(/^[ \t]+/, "", value)
        gsub(/[ \t]+$/, "", value)
        sub(/[ \t]+#.*$/, "", value)
        gsub(/^[ \t]+/, "", value)
        ...
        if (length("" value) >= 2) {
          c1 = substr(value, 1, 1)
          c2 = substr(value, length("" value), 1)
          if ((c1 == "\"" && c2 == "\"") || (c1 == "\047" && c2 == "\047")) {
      ...
      END {
        if (bad) exit 1
        if (found) { print "PASS"; exit 0 }
        print "SUBJECT_ABSENT"
        exit 2
      }
    ')
    GLP_PARSE_RC=$?
    case "$GLP_RESULT" in
      PASS)
        GLP_STATUS=PASS
        GLP_OBSERVED="LDR_PRELOAD entries in global init files are absolute paths or \$PATH"
        ;;
      SUBJECT_ABSENT)
        # grep matched nothing (rc 1) or could not read a named file (rc 2), so
        # no LDR_PRELOAD assignment was recovered anywhere. The required list is
        # absent: FAIL, never NOT_ASSESSED. rc 0 with empty stdout is a swallowed
        # error and stays NOT_ASSESSED; rc 0 with only comment/mention lines is
        # the same determinate absence.
        if [ "$GLP_RC" -eq 1 ]; then
          GLP_STATUS=FAIL
          GLP_OBSERVED="no LDR_PRELOAD list is set in the global init files (grep rc 1: no matches; required configuration is unmet)"
        elif [ "$GLP_RC" -eq 2 ]; then
          GLP_STATUS=FAIL
          GLP_OBSERVED="no LDR_PRELOAD list is set in the global init files (grep rc 2: a named file was unreadable or absent; required configuration is unmet)"
        elif [ -z "$GLP_RAW" ]; then
          GLP_STATUS=NOT_ASSESSED
          GLP_OBSERVED="not assessed - LDR_PRELOAD probe returned no output with rc 0 (swallowed error)"
        else
          GLP_STATUS=FAIL
          GLP_OBSERVED="no LDR_PRELOAD list is set in the global init files (required configuration is unmet)"
        fi
        ;;
      EMPTY\|*)
        GLP_FILE=${GLP_RESULT#EMPTY\|}
        GLP_STATUS=FAIL
        GLP_OBSERVED="$GLP_FILE: LDR_PRELOAD has an empty entry (leading, trailing, or double colon)"
        ;;
      REL\|*)
        GLP_REL=${GLP_RESULT#REL\|}
        GLP_FILE=${GLP_REL%%\|*}
        GLP_TOKEN=${GLP_REL#*\|}
        GLP_STATUS=FAIL
        GLP_OBSERVED="$GLP_FILE: LDR_PRELOAD entry '$GLP_TOKEN' is a relative path"
        ;;
      *)
        GLP_STATUS=NOT_ASSESSED
        GLP_OBSERVED="not assessed - LDR_PRELOAD output was unparseable (rc=$GLP_PARSE_RC)"
        ;;
    esac
  fi
  ...
  case "$GLP_STATUS" in
    PASS)
      GLP_MEANING="Every LDR_PRELOAD entry in the global initialization files is an absolute library path or \$PATH, so a relative preload cannot inject an attacker-controlled library."
      GLP_FIX="n/a"
      ;;
    FAIL)
      GLP_MEANING="A global initialization file preloads a library by relative path or contains an empty preload slot, so a process started from those files can load an attacker-influenced library from the current directory."
      GLP_FIX="Edit the global initialization files and remove the relative path entry from the library preload list variable 'LDR_PRELOAD'."
      ;;
    *)
      GLP_MEANING="PTxray did not obtain trustworthy evidence for the global initialization file LDR_PRELOAD lists."
      GLP_FIX="verify that the global init files are readable and that the LDR_PRELOAD probe succeeds, then rerun PTxray."
      ;;
  esac

  add security stig_globalinit_ldr_preload "Global initialization file LDR_PRELOAD lists" \
    "$GLP_STATUS" med "$GLP_OBSERVED" \
    "$GLP_MEANING" "$GLP_FIX" "stig:V-215423"
}
