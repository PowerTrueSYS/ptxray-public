...
AIXRAY_TOOL=ck-stig-globalinit-path


function standalone_check {
_AIXRAY_SESSION_KEYS=""
# stig_globalinit_path — DISA STIG V-215416 (SV-215416r991589_rule, AIX7-00-003120).
# All global initialization file executable search paths must contain only
# absolute paths.  grep -i PATH over the seven global init files; every PATH
# entry must be absolute (starts with /) or the token $PATH.  An empty entry
# (leading/trailing/consecutive colon) or a relative entry is a finding.
# grep exits 2 when any operand cannot be opened while still emitting the
# matches it found, so evidence is parsed whenever output is non-empty. A
# finding in the emitted rows is FAIL even at rc 2. Emitted rows with no
# finding at rc 0 are PASS. Emitted rows with no finding at nonzero rc are
# WARN: the visible PATH is compliant, but an unread file may still hold a
# relative or empty entry. rc decides the empty-output and unparsed-output
# fallbacks.
typeset STIG_GPI_RAW STIG_GPI_RC STIG_GPI_SHAPE STIG_GPI_VERD STIG_GPI_DETAIL
typeset STIG_GPI_STATUS STIG_GPI_OBSERVED STIG_GPI_MEANING STIG_GPI_FIX

STIG_GPI_RAW=$(aix stig_globalinit_path grep -i PATH /etc/profile /etc/bashrc /etc/csh.login /etc/csh.cshrc /etc/environment /etc/.login /etc/security/environ)
STIG_GPI_RC=$?

if [ -n "$STIG_GPI_RAW" ]; then
  STIG_GPI_SHAPE=$(printf '%s\n' "$STIG_GPI_RAW" | awk '
  BEGIN {
    SQ = sprintf("%c", 39)
    DQ = sprintf("%c", 34)
    findings = 0
  ...
  }
  {
    ...
    if (line == "") next
    c = index(line, ":")
    if (c == 0) { file = "-"; content = line }
    else { file = substr(line, 1, c - 1); content = substr(line, c + 1) }
    low = tolower(content)
    n = length("" content)
    found = 0
    val = ""
    i = 1
    while (i <= n) {
      if (substr(low, i, 4) == "path") {
        if (i > 1 && substr(low, i - 1, 1) ~ /[a-z0-9_]/) { i++; continue }
        j = i + 4
        while (j <= n && (substr(content, j, 1) == " " || substr(content, j, 1) == "\t")) j++
        if (j <= n && substr(content, j, 1) == "=") {
          val = substr(content, j + 1)
          found = 1
          i = n + 1
        } else {
          b = i - 1
          while (b >= 1 && (substr(low, b, 1) == " " || substr(low, b, 1) == "\t")) b--
          if (b >= 6 && substr(low, b - 5, 6) == "setenv" &&
              (b - 5 == 1 || substr(low, b - 6, 1) ~ /[ \t]/)) {
            k = j
            while (k <= n && (substr(content, k, 1) == " " || substr(content, k, 1) == "\t")) k++
            if (k <= n) val = substr(content, k)
            found = 1
            i = n + 1
          } else {
            i++
    ...
    v = val
    gsub(/^[ \t]+/, "", v)
    if (v != "" && (substr(v, 1, 1) == SQ || substr(v, 1, 1) == DQ)) v = substr(v, 2)
    if (v != "" && (substr(v, length("" v), 1) == SQ || substr(v, length("" v), 1) == DQ)) v = substr(v, 1, length("" v) - 1)
    if (v == "") {
      findings++
      if (detail == "") detail = "PATH in " file " contains an empty entry (leading, trailing, or consecutive colon)"
      next
    }
    if (substr(v, 1, 1) == "(") {
      inner = v
      gsub(/^[ \t]+/, "", inner)
      if (substr(inner, 1, 1) == "(") inner = substr(inner, 2)
      gsub(/[ \t]+$/, "", inner)
      if (length("" inner) > 0 && substr(inner, length("" inner), 1) == ")") inner = substr(inner, 1, length("" inner) - 1)
      gsub(/^[ \t]+/, "", inner)
      gsub(/[ \t]+$/, "", inner)
      cnt = split(inner, comps, /[ \t]+/)
    } else {
      cnt = split(v, comps, ":")
    }
    for (k = 1; k <= cnt; k++) {
      if (comps[k] == "") {
        findings++
        if (detail == "") detail = "PATH in " file " contains an empty entry (leading, trailing, or consecutive colon)"
      } else if (substr(comps[k], 1, 1) != "/" && comps[k] != "$PATH") {
        findings++
        if (detail == "") detail = "PATH entry " comps[k] " in " file " is relative (must be an absolute path or $PATH)"
      }
    }
  }
  END {
    if (parsed == 0) { print "NONE"; print ""; exit 0 }
    if (findings == 0) { print "PASS"; print ""; exit 0 }
    print "FAIL"
    print detail
    exit 0
  }')
  STIG_GPI_VERD=$(printf '%s\n' "$STIG_GPI_SHAPE" | sed -n '1p')
  STIG_GPI_DETAIL=$(printf '%s\n' "$STIG_GPI_SHAPE" | sed -n '2p')
  case "$STIG_GPI_VERD" in
    FAIL)
      STIG_GPI_STATUS=FAIL
      STIG_GPI_OBSERVED="global init PATH: $STIG_GPI_DETAIL"
      ;;
    PASS)
      if [ "$STIG_GPI_RC" -eq 0 ]; then
        STIG_GPI_STATUS=PASS
        STIG_GPI_OBSERVED="all global init PATH entries are absolute or \$PATH"
      else
        STIG_GPI_STATUS=WARN
        STIG_GPI_OBSERVED="partial PATH probe (rc=$STIG_GPI_RC): emitted PATH entries are absolute or \$PATH; at least one named global init file was not read"
      fi
      ;;
    NONE)
      if [ "$STIG_GPI_RC" -eq 0 ]; then
        STIG_GPI_STATUS=FAIL
        STIG_GPI_OBSERVED="no PATH assignment found in any global initialization file; the required global-init PATH configuration is absent"
      else
        STIG_GPI_STATUS=NOT_ASSESSED
        STIG_GPI_OBSERVED="not assessed - PATH grep could not read every global init file (rc=$STIG_GPI_RC); an unreadable file may hold the PATH configuration"
      fi
      ;;
    *)
      STIG_GPI_STATUS=NOT_ASSESSED
      STIG_GPI_OBSERVED="not assessed - PATH grep output was not in a recognized shape"
      ;;
  esac
elif [ "$STIG_GPI_RC" -eq 1 ]; then
  STIG_GPI_STATUS=FAIL
  STIG_GPI_OBSERVED="no PATH setting found in any global initialization file; the required global-init PATH configuration is absent"
elif [ "$STIG_GPI_RC" -eq 0 ]; then
  STIG_GPI_STATUS=NOT_ASSESSED
  STIG_GPI_OBSERVED="not assessed - PATH grep returned no output with rc 0 (swallowed error)"
else
  STIG_GPI_STATUS=NOT_ASSESSED
  STIG_GPI_OBSERVED="not assessed - PATH grep probe failed (rc=$STIG_GPI_RC), no trustworthy PATH evidence"
fi

case "$STIG_GPI_STATUS" in
  PASS)
    STIG_GPI_MEANING="All executable search paths in the global initialization files are absolute or \$PATH references, as required by DISA STIG V-215416."
    STIG_GPI_FIX="n/a"
    ;;
  FAIL)
    STIG_GPI_MEANING="A global initialization file sets a PATH entry that is empty or relative, so an interactive session could search an unintended directory ahead of the system paths."
    STIG_GPI_FIX="edit the global initialization file(s) whose PATH variables contain relative or empty paths and remove those entries; PTxray only recommends this action."
    ;;
  WARN)
    STIG_GPI_MEANING="Emitted global-init PATH entries are absolute or \$PATH, but the PATH grep could not read every named initialization file, so an unread file may still hold a relative or empty entry."
    STIG_GPI_FIX="make every named global initialization file readable and rerun PTxray so the remaining files can be graded; PTxray only recommends this action."
    ;;
  *)
    STIG_GPI_MEANING="PTxray did not obtain trustworthy PATH evidence from the global initialization files."
    STIG_GPI_FIX="verify the global initialization files are readable and that the PATH grep probe returns clean output, then rerun PTxray."
    ;;
esac

add security stig_globalinit_path "Global init PATH entries" \
  "$STIG_GPI_STATUS" med "$STIG_GPI_OBSERVED" \
  "$STIG_GPI_MEANING" "$STIG_GPI_FIX" "stig:V-215416"
}
