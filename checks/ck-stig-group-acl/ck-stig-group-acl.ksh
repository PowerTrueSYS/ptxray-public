...
AIXRAY_TOOL=ck-stig-group-acl


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_group_acl — DISA STIG V-215328 (medium).  /etc/group must not have an
  # extended ACL.  aclget prints the ACL summary; the "extended permissions"
  # section is followed by "disabled" (compliant) or "enabled" (finding).
  # Absence of /etc/group is itself the compliant state — a prohibition rule:
  # with no group database file there is no extended ACL to forbid — so a
  # missing-file report for /etc/group renders PASS.  The carve-out is anchored
  # to the probed path: only a diagnostic line that names /etc/group and reports
  # it missing counts as absence.  Every other rc!=0 failure with rows fails
  # closed to NOT_ASSESSED, never PASS.
  typeset SGA_RAW SGA_RC SGA_SHAPE SGA_REASON SGA_STATUS
  typeset SGA_OBSERVED SGA_MEANING SGA_FIX SGA_MISSING
  SGA_RAW=""
  SGA_RC=0
  SGA_SHAPE=""
  SGA_REASON=""
  ...
  SGA_RAW=$(aix aclget_group aclget /etc/group)
  SGA_RC=$?

  if [ "$SGA_RC" -ne 0 ] && [ -z "$SGA_RAW" ]; then
    SGA_REASON="probe failed (rc=$SGA_RC)"
  elif [ "$SGA_RC" -eq 0 ] && [ -z "$SGA_RAW" ]; then
    SGA_REASON="empty"
  elif [ "$SGA_RC" -ne 0 ]; then
    SGA_MISSING=$(printf '%s\n' "$SGA_RAW" | awk '
      { if (index($0, "/etc/group") > 0 && (index($0, "No such file") > 0 || index($0, "does not exist") > 0)) miss = 1 }
      END { if (miss) print "yes"; else print "no" }')
    if [ "$SGA_MISSING" = "yes" ]; then
      SGA_STATUS=PASS
      SGA_OBSERVED="/etc/group absent; with no group database file there is no extended ACL to forbid (prohibition rule)"
    else
      SGA_REASON="probe failed with rows (rc=$SGA_RC)"
    fi
  else
    SGA_SHAPE=$(printf '%s\n' "$SGA_RAW" | awk '
      /^[ \t]*extended permissions[ \t]*$/ { in_ext = 1; next }
      in_ext {
        if ($0 ~ /^[ \t]*$/) next
      ...
      END { if (!in_ext) print "unparsed" }')
    case "$SGA_SHAPE" in
      disabled)
        SGA_STATUS=PASS
        SGA_OBSERVED="aclget reports extended permissions disabled for /etc/group; no extended ACL is present"
        ;;
      enabled)
        SGA_STATUS=FAIL
        SGA_OBSERVED="aclget reports extended permissions enabled for /etc/group; an extended ACL is present"
        ;;
      *)
        SGA_REASON="unparseable"
        ;;
    esac
  fi

  if [ -n "$SGA_REASON" ]; then
    SGA_STATUS=NOT_ASSESSED
    SGA_OBSERVED="not assessed - $SGA_REASON"
  fi

  case "$SGA_STATUS" in
    PASS)
      SGA_MEANING="/etc/group has no extended ACL, so only base owner/group/other permissions govern the group database."
      SGA_FIX="n/a"
      ;;
    FAIL)
      SGA_MEANING="An extended ACL on /etc/group adds access rules beyond owner/group/other, so accounts the base mode does not list can be granted access to the group database without the intended administrative control."
      SGA_FIX="remove the extended ACL with 'acledit /etc/group'; PTxray only recommends this command and never runs it."
      ;;
    *)
      SGA_MEANING="PTxray did not obtain trustworthy aclget evidence for the /etc/group ACL state."
      SGA_FIX="verify /etc/group exists and is readable, rerun the aclget probe as root, and rerun PTxray."
      ;;
  esac

  add security stig_group_acl "Group database extended ACL" "$SGA_STATUS" med \
    "$SGA_OBSERVED" "$SGA_MEANING" "$SGA_FIX" "stig:V-215328"
}
