...
AIXRAY_TOOL=ck-stig-have-accounts-configured-blank


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_have_accounts_configured_blank — DISA STIG V-215197.
  # AIX must not have accounts configured with blank or null passwords.
  # pwdck -n ALL is a read-only scan that reports every account whose stored
  # password is blank. rc 1 and rc 114 are determinate (no blank-password
  # report, or a completed scan that exited non-zero for other pwdck errors);
  # parse the stdout the same as rc 0. Other non-zero rcs and rc0+empty remain
  # refusals: pwdck can exit 0 without scanning, so empty rc 0 is never a
  # clean bill, and rc 127 / unreadable / no-anchor stay NOT_ASSESSED.
  typeset PW_SCAN_RAW PW_SCAN_RC PW_PARSE PW_ACCOUNTS
  typeset PW_VERDICT PW_REASON

  PW_SCAN_RAW=$(aix pwdck_n_all pwdck -n ALL)
  PW_SCAN_RC=$?
  PW_VERDICT=""
  PW_REASON=""
  PW_ACCOUNTS=""

  if [ "$PW_SCAN_RC" -ne 0 ] && [ "$PW_SCAN_RC" -ne 1 ] && [ "$PW_SCAN_RC" -ne 114 ]; then
    PW_VERDICT=NOT_ASSESSED
    PW_REASON="probe failed (rc=$PW_SCAN_RC)"
  elif [ -z "$PW_SCAN_RAW" ] && [ "$PW_SCAN_RC" -eq 0 ]; then
    PW_VERDICT=NOT_ASSESSED
    PW_REASON="probe returned no output (rc=0)"
  else
    PW_PARSE=$(printf '%s\n' "$PW_SCAN_RAW" | awk '
      {
        line = tolower($0)
      ...
      END {
        if (acc != "") print "ACCOUNTS:" acc
        else if (saw) print "UNPARSEABLE:"
        else print "NONE:"
      }')
    case "$PW_PARSE" in
      ACCOUNTS:*) PW_ACCOUNTS=${PW_PARSE#ACCOUNTS:}
        PW_VERDICT=FAIL ;;
      UNPARSEABLE:*) PW_VERDICT=NOT_ASSESSED
        PW_REASON="blank-password indicator without a recoverable account name" ;;
      NONE:*) PW_VERDICT=PASS ;;
      *) PW_VERDICT=NOT_ASSESSED
        PW_REASON="pwdck output was not parseable" ;;
    esac
  fi

  case "$PW_VERDICT" in
    PASS)
      add security stig_have_accounts_configured_blank \
          "Interactive accounts with blank passwords" PASS med \
          "no interactive account with a blank password was reported by pwdck -n ALL" \
          "pwdck -n ALL reported no interactive account with a blank password, so every account requires a credential to sign in." \
          "n/a" "stig:V-215197"
      ;;
    FAIL)
      add security stig_have_accounts_configured_blank \
          "Interactive accounts with blank passwords" FAIL high \
          "interactive account with blank password: $PW_ACCOUNTS" \
          "At least one interactive account has a blank (null) password, so that account can be signed into without supplying a credential." \
          "give each listed account a non-blank password with 'passwd <user>'; PTxray recommends this command and never runs it." \
          "stig:V-215197"
      ;;
    *)
      add security stig_have_accounts_configured_blank \
          "Interactive accounts with blank passwords" NOT_ASSESSED med \
          "not assessed - $PW_REASON" \
          "PTxray could not obtain a trustworthy pwdck -n ALL scan, so it cannot verify whether any interactive account has a blank password." \
          "verify 'pwdck -n ALL' completes and returns parseable output, then rerun PTxray." \
          "stig:V-215197"
      ;;
  esac
}
