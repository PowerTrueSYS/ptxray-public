...
AIXRAY_TOOL=ck-stig-home-file-groups


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_home_file_groups — DISA STIG V-215278.
  # Every file and directory inside each user's home directory must be
  # group-owned by a group of which the home directory owner is a member.
  # Probe output is streamed into awk; a home listing is never stored in a
  # ksh variable (ksh88 variable space is bounded). Each per-home walk is
  # capped (HFG_CAP rows). A genuine home that exceeds the cap is still
  # graded on the examined prefix; the cap is a WARN detail on that row,
  # never a refusal. Observed FAIL text names the mismatch count and at
  # most 3 offending paths.
  # System-directory homes (/, /etc, /usr, /var, /tmp, /opt, /bin, /sbin,
  # /dev), including equivalent spellings with trailing slashes, are not
  # STIG homes: skip the find walk and emit a typed detail. The cap is
  # never applied to them. The original passwd path is kept for the detail
  # and for any real-home probe. find rc 2 WITH listing rows on a real home
  # is partial evidence: grade the readable rows and note the
  # unreadable-path count as a WARN detail. A probe that could not run still
  # refuses.
  typeset HFG_PASSWD_RAW HFG_PASSWD_RC HFG_PASSWD_HOMES HFG_PASSWD_SHAPE
  typeset HFG_GROUPS_SUM HFG_GROUPS_RC
  typeset HFG_GROUPFILE_RAW HFG_GROUPFILE_RC
  typeset HFG_LISTING_SUM HFG_LISTING_CHECK_RC
  typeset HFG_UG_RC HFG_USER HFG_HOME HFG_GROUPS HFG_CAP
  typeset HFG_STATUS HFG_OBSERVED HFG_MEANING HFG_FIX
  typeset HFG_BAD_N HFG_BAD_PATHS HFG_SKIPPED HFG_SKIP_N HFG_PARTIAL HFG_UNREAD HFG_HOME_NORM HFG_OVER
  HFG_CAP=500
  HFG_STATUS=""
  HFG_OBSERVED=""
  HFG_SKIP_N=0
  ...
  HFG_PASSWD_RAW=$(aix passwd_users cut -d : -f 1,6 /etc/passwd)
  HFG_PASSWD_RC=$?

  if [ "$HFG_PASSWD_RC" -ne 0 ]; then
    HFG_STATUS=NOT_ASSESSED
    HFG_OBSERVED="not assessed - passwd probe failed (rc=$HFG_PASSWD_RC)"
  elif [ -z "$HFG_PASSWD_RAW" ]; then
    HFG_STATUS=NOT_ASSESSED
    HFG_OBSERVED="not assessed - passwd probe returned empty with rc 0"
  else
    HFG_PASSWD_HOMES=$(printf '%s\n' "$HFG_PASSWD_RAW" | awk -F: '
      /^[ \t]*$/ { next }
      {
        if (NF != 2 || $1 == "") { bad = 1; next }
        if ($2 != "") { print $1 "|" $2; n = n + 1 }
      }
      END { if (bad) exit 2; if (n == 0) exit 1; exit 0 }
    ')
    HFG_PASSWD_SHAPE=$?
    if [ "$HFG_PASSWD_SHAPE" -eq 2 ]; then
      HFG_STATUS=NOT_ASSESSED
      HFG_OBSERVED="not assessed - passwd rows were malformed"
    elif [ "$HFG_PASSWD_SHAPE" -eq 1 ]; then
      HFG_STATUS=FAIL
      HFG_OBSERVED="no user home directory exists in /etc/passwd; the required home group-ownership configuration is absent"
    fi
  fi

  if [ -z "$HFG_STATUS" ]; then
    HFG_GROUPS_SUM=$(
      {
        aix user_groups lsuser -a groups ALL
        printf '%s\n' "HFG_PROBE_STATUS $?"
      } | awk '
        $1 == "HFG_PROBE_STATUS" && NF == 2 { probe_rc = $2; next }
        /^[ \t]*$/ { next }
        {
          seen = seen + 1
          if (NF != 2 || $1 == "" || $2 !~ /^groups=/) { bad = 1; next }
        }
        END {
          if (probe_rc + 0 != 0) { print probe_rc; exit 5 }
          if (seen == 0) exit 4
          if (bad) exit 2
          exit 0
        }
    ...
    HFG_GROUPS_RC=$?
    if [ "$HFG_GROUPS_RC" -eq 5 ]; then
      HFG_STATUS=NOT_ASSESSED
      HFG_OBSERVED="not assessed - user groups probe failed (rc=$HFG_GROUPS_SUM)"
    elif [ "$HFG_GROUPS_RC" -eq 4 ]; then
      HFG_STATUS=NOT_ASSESSED
      HFG_OBSERVED="not assessed - user groups probe returned empty with rc 0"
    elif [ "$HFG_GROUPS_RC" -ne 0 ]; then
      HFG_STATUS=NOT_ASSESSED
      HFG_OBSERVED="not assessed - user groups rows were malformed"
    fi
  fi

  if [ -z "$HFG_STATUS" ]; then
    HFG_GROUPFILE_RAW=$(aix etc_group cat /etc/group)
    HFG_GROUPFILE_RC=$?
    if [ "$HFG_GROUPFILE_RC" -ne 0 ]; then
      HFG_STATUS=NOT_ASSESSED
      HFG_OBSERVED="not assessed - group file probe failed (rc=$HFG_GROUPFILE_RC)"
    elif [ -z "$HFG_GROUPFILE_RAW" ]; then
      HFG_STATUS=NOT_ASSESSED
      HFG_OBSERVED="not assessed - group file probe returned empty with rc 0"
    else
      printf '%s\n' "$HFG_GROUPFILE_RAW" | awk -F: '
        /^[ \t]*#/ { next }
        /^[ \t]*\+/ { next }
        /^[ \t]*$/ { next }
        { if (NF < 2 || $1 == "") { bad = 1; next } }
      ...
      if [ $? -ne 0 ]; then
        HFG_STATUS=NOT_ASSESSED
        HFG_OBSERVED="not assessed - group file rows were malformed"
      fi
    fi
  fi

  if [ -z "$HFG_STATUS" ]; then
    while IFS='|' read -r HFG_USER HFG_HOME; do
      [ -z "$HFG_HOME" ] && continue
      # Trailing slashes: /usr/ is /usr. Keep $HFG_HOME for detail and find.
      HFG_HOME_NORM=$HFG_HOME
      while [ "$HFG_HOME_NORM" != / ]; do
        case "$HFG_HOME_NORM" in
          */) HFG_HOME_NORM=${HFG_HOME_NORM%/} ;;
          *) break ;;
        esac
      done
      case "$HFG_HOME_NORM" in
        /|/etc|/usr|/var|/tmp|/opt|/bin|/sbin|/dev)
          HFG_SKIP_N=$((HFG_SKIP_N + 1))
          HFG_SKIPPED="${HFG_SKIPPED:+$HFG_SKIPPED }$HFG_USER:$HFG_HOME"
          continue
          ;;
      esac
      HFG_GROUPS=$(
        {
          ...
          $1 == "HFG_PROBE_STATUS" { next }
          $1 == u { sub(/^groups=/, "", $2); print $2; found = 1 }
          END { if (!found) exit 1 }
        '
      )
      HFG_UG_RC=$?
      if [ "$HFG_UG_RC" -ne 0 ]; then
        HFG_STATUS=NOT_ASSESSED
        HFG_OBSERVED="not assessed - user $HFG_USER is absent from the lsuser groups output"
        break
      fi
      # ls -lLd row shape: mode links owner group size date path -- the
      # group is the 4th whitespace field, immediately after the owner (3rd).
      # A non-numeric size at field 5 means the columns do not match that
      # shape; such a listing is malformed and refused, never PASSed.
      # The listing is piped into awk; only a compact summary is captured.
      HFG_LISTING_SUM=$(
        {
          aix home_listings find "$HFG_HOME" -exec ls -lLd {} \;
          printf '%s\n' "HFG_PROBE_STATUS $?"
        } | awk -v grps="$HFG_GROUPS" -v cap="$HFG_CAP" '
          BEGIN {
            n = split(grps, a, ",")
            for (i = 1; i <= n; i++) g[a[i]] = 1
            seen = 0
            nbad = 0
            nsamp = 0
            unread = 0
          ...
          $1 == "HFG_PROBE_STATUS" && NF == 2 { probe_rc = $2; next }
          /^[ \t]*$/ { next }
          /^find:/ || /^ls:/ { unread = unread + 1; next }
          {
            seen = seen + 1
            if (seen > cap) { over = 1; next }
            if (NF < 9 || length($1) < 10 || substr($1, 1, 1) !~ /[-dlbcps]/ || $2 !~ /^[0-9][0-9]*$/ || $5 !~ /^[0-9][0-9]*$/) { bad = 1; next }
            if (!($4 in g)) {
              nbad = nbad + 1
              if (nsamp < 3) {
                p = $9
                for (j = 10; j <= NF; j++) p = p " " $j
                if (nsamp == 0) samples = p
                else samples = samples ", " p
                nsamp = nsamp + 1
              }
              hit = 1
            }
          ...
          END {
            if (probe_rc + 0 != 0) {
              if (probe_rc + 0 == 2 && seen > 0) partial = 1
              else { print probe_rc; exit 5 }
            }
            if (seen == 0) exit 4
            if (bad) exit 2
            if (hit) { print nbad "|" samples "|" unread; if (partial) exit 6; exit 1 }
            if (partial) { print unread; exit 7 }
            if (over) exit 3
            exit 0
          }
        '
      )
      HFG_LISTING_CHECK_RC=$?
      if [ "$HFG_LISTING_CHECK_RC" -eq 5 ]; then
        HFG_STATUS=NOT_ASSESSED
        HFG_OBSERVED="not assessed - home listing probe failed for $HFG_HOME (rc=$HFG_LISTING_SUM)"
        break
      elif [ "$HFG_LISTING_CHECK_RC" -eq 4 ]; then
        HFG_STATUS=NOT_ASSESSED
        HFG_OBSERVED="not assessed - home listing probe for $HFG_HOME returned empty with rc 0"
        break
      elif [ "$HFG_LISTING_CHECK_RC" -eq 2 ]; then
        HFG_STATUS=NOT_ASSESSED
        HFG_OBSERVED="not assessed - home listing rows for $HFG_HOME were malformed"
        break
      elif [ "$HFG_LISTING_CHECK_RC" -eq 3 ]; then
        HFG_OVER="${HFG_OVER:+$HFG_OVER; }home listing for $HFG_HOME exceeds the $HFG_CAP entry evaluation cap; entries beyond the cap were not assessed"
      elif [ "$HFG_LISTING_CHECK_RC" -eq 1 ] || [ "$HFG_LISTING_CHECK_RC" -eq 6 ]; then
        HFG_STATUS=FAIL
        HFG_BAD_N=$(printf '%s\n' "$HFG_LISTING_SUM" | awk -F'|' '{ print $1 }')
        HFG_BAD_PATHS=$(printf '%s\n' "$HFG_LISTING_SUM" | awk -F'|' '{ print $2 }')
        HFG_OBSERVED="$HFG_BAD_N group-ownership mismatch(es) under $HFG_HOME (owner $HFG_USER is not a member of the file group; up to 3 paths shown): $HFG_BAD_PATHS"
        if [ "$HFG_LISTING_CHECK_RC" -eq 6 ]; then
          HFG_UNREAD=$(printf '%s\n' "$HFG_LISTING_SUM" | awk -F'|' '{ print $3 }')
          if [ -n "$HFG_UNREAD" ] && [ "$HFG_UNREAD" -gt 0 ]; then
            HFG_OBSERVED="$HFG_OBSERVED; $HFG_UNREAD unreadable path(s)"
          else
            HFG_OBSERVED="$HFG_OBSERVED; some paths unreadable"
          fi
        fi
        break
      elif [ "$HFG_LISTING_CHECK_RC" -eq 7 ]; then
        HFG_UNREAD=$HFG_LISTING_SUM
        if [ -n "$HFG_UNREAD" ] && [ "$HFG_UNREAD" -gt 0 ]; then
          HFG_PARTIAL="$HFG_UNREAD unreadable path(s) under $HFG_HOME"
        else
          HFG_PARTIAL="some paths unreadable under $HFG_HOME"
        fi
      fi
    done <<EOF
$HFG_PASSWD_HOMES
EOF
  ...
  if [ -z "$HFG_STATUS" ]; then
    HFG_STATUS=PASS
    HFG_OBSERVED="all files and directories under each user's home directory are group-owned by a group the home directory owner belongs to"
    if [ -n "$HFG_SKIPPED" ]; then
      HFG_OBSERVED="$HFG_OBSERVED; $HFG_SKIP_N system-directory home(s) skipped (not a STIG home): $HFG_SKIPPED"
    fi
    if [ -n "$HFG_PARTIAL" ]; then
      HFG_OBSERVED="$HFG_OBSERVED; $HFG_PARTIAL"
    fi
    if [ -n "$HFG_OVER" ]; then
      HFG_OBSERVED="$HFG_OBSERVED; $HFG_OVER"
    fi
  fi

  case "$HFG_STATUS" in
    PASS)
      HFG_MEANING="Every file and directory under each user's home directory is group-owned by a group the home's owner belongs to."
      if [ -n "$HFG_PARTIAL" ]; then
        HFG_MEANING="$HFG_MEANING Some paths were unreadable, so the scan is partial."
      fi
      HFG_FIX="n/a"
      ;;
    FAIL)
      HFG_MEANING="A file or directory under a user's home directory is group-owned by a group the home's owner does not belong to, violating the required group ownership."
      HFG_FIX="change the group of the offending entry to a group the home owner is a member of, e.g. 'chgrp [user's primary group] [file with bad group ownership]'; PTxray only recommends this command."
      ;;
    *)
      HFG_MEANING="PTxray did not obtain trustworthy evidence proving the group ownership of every entry in every user's home directory."
      HFG_FIX="verify the passwd, group, and home listing captures as root, then rerun PTxray."
      ;;
  esac

  add security stig_home_file_groups "Home directory file group ownership" \
    "$HFG_STATUS" med "$HFG_OBSERVED" "$HFG_MEANING" "$HFG_FIX" "stig:V-215278"
}
