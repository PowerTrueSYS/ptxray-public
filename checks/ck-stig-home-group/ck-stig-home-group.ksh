...
AIXRAY_TOOL=ck-stig-home-group


function standalone_check {
_AIXRAY_SESSION_KEYS=""
# stig_home_group — DISA STIG V-215277.
# Every AIX interactive user's home directory must be group-owned by that
# user's primary group.  Evidence is read-only: /etc/passwd gives each
# interactive user's home and primary gid; one ls -lLdn over all homes gives
# each home directory's numeric group id (the -n variant matches the
# ck-fileperm-hosts precedent so the comparison stays numeric).  Every
# uncertain evidence shape fails closed to NOT_ASSESSED; absence of the
# subject is a FAIL because this is a required configuration.  The comparison
# is keyed by user, not by home path, so two interactive accounts sharing one
# home directory are each checked against their own primary group.  A partial
# listing (one home absent, other rows present) is still compared; the absent
# home renders NOT_ASSESSED via the MISSING branch instead of discarding the
# usable rows because of a nonzero ls exit.
typeset SHG_RAW SHG_RC SHG_PARSED SHG_PARSE_RC
typeset SHG_LS_RAW SHG_LS_RC SHG_LS_PARSED SHG_LS_PARSE_RC
typeset SHG_USER SHG_GID SHG_HOME
typeset SHG_COUNT SHG_HOMES SHG_RESULT SHG_RESULT_RC
typeset SHG_STATUS SHG_OBSERVED SHG_MEANING SHG_FIX
...
SHG_RAW=$(aix home_group_passwd cat /etc/passwd)
SHG_RC=$?

if [ "$SHG_RC" -ne 0 ]; then
  SHG_STATUS=NOT_ASSESSED
  SHG_OBSERVED="not assessed - /etc/passwd probe failed (rc=$SHG_RC)"
elif [ -z "$SHG_RAW" ]; then
  SHG_STATUS=NOT_ASSESSED
  SHG_OBSERVED="not assessed - /etc/passwd probe returned empty output with rc 0"
else
  SHG_PARSED=$(printf '%s\n' "$SHG_RAW" | awk '
    BEGIN { FS = ":" }
    {
    ...
    NF < 7 { bad = 1; next }
    {
      ...
      shell = $7
      gsub(/^[ \t]+|[ \t]+$/, "", shell)
      if (user == "" || gid !~ /^[0-9][0-9]*$/) { bad = 1; next }
      if (home == "" || home == "*" || home == "NONE") next
      if (shell == "") next
      if (shell == "/usr/bin/false" || shell == "/bin/false" ||
          shell == "/usr/sbin/nologin" || shell == "/sbin/nologin" ||
          shell == "/usr/bin/true" || shell == "/bin/true") next
      print user "|" gid "|" home
    }
    END { if (bad) exit 1 }
  ')
  SHG_PARSE_RC=$?
  if [ "$SHG_PARSE_RC" -ne 0 ]; then
    SHG_STATUS=NOT_ASSESSED
    SHG_OBSERVED="not assessed - /etc/passwd rows were malformed or unparseable"
  else
    SHG_COUNT=0
    SHG_HOMES=""
    while IFS='|' read -r SHG_USER SHG_GID SHG_HOME; do
      [ -n "$SHG_USER" ] || continue
      SHG_COUNT=$((SHG_COUNT + 1))
      SHG_HOMES="$SHG_HOMES $SHG_HOME"
    done <<EOF
$SHG_PARSED
EOF
    if [ "$SHG_COUNT" -eq 0 ]; then
      SHG_STATUS=FAIL
      SHG_OBSERVED="no interactive user home directories found in /etc/passwd; the required configuration is absent"
    else
      SHG_LS_RAW=$(aix home_group_ls ls -lLdn $SHG_HOMES)
      SHG_LS_RC=$?
      if [ -z "$SHG_LS_RAW" ]; then
        # Empty output is a swallowed error whether or not ls exited nonzero.
        if [ "$SHG_LS_RC" -ne 0 ]; then
          SHG_STATUS=NOT_ASSESSED
          SHG_OBSERVED="not assessed - home directory listing failed (rc=$SHG_LS_RC)"
        else
          SHG_STATUS=NOT_ASSESSED
          SHG_OBSERVED="not assessed - home directory listing returned empty output with rc 0"
        fi
      else
        # Non-empty output is parsed and compared even when ls exited nonzero
        # (one absent home drives rc != 0 but the other rows stay usable); the
        # MISSING branch below renders the absent home as NOT_ASSESSED.
        SHG_LS_PARSED=$(printf '%s\n' "$SHG_LS_RAW" | awk '
          NF < 9 { bad = 1; next }
          {
            gid = $4
            if (gid !~ /^[0-9][0-9]*$/) { bad = 1; next }
            print $NF "|" gid
          }
          END { if (bad) exit 1 }
        ')
        SHG_LS_PARSE_RC=$?
        if [ "$SHG_LS_PARSE_RC" -ne 0 ]; then
          SHG_STATUS=NOT_ASSESSED
          SHG_OBSERVED="not assessed - home directory listing rows were malformed or unparseable"
        elif [ -z "$SHG_LS_PARSED" ]; then
          SHG_STATUS=NOT_ASSESSED
          SHG_OBSERVED="not assessed - home directory listing parsed to no rows"
        else
          SHG_RESULT=$( {
            printf '%s\n' "$SHG_PARSED" | sed 's/^/P/'
            printf '%s\n' "$SHG_LS_PARSED" | sed 's/^/L/'
          } | awk '
            /^P/ {
              rec = substr($0, 2)
              nf = split(rec, f, "|")
            ...
            END {
              if (bad) { print "UNPARSEABLE"; exit 2 }
              missing = 0
              mismatches = 0
              mm = ""
              for (i = 0; i < n; i++) {
                u = users[i]
                h = uhome[u]
                if (!(h in listed)) { missing = 1; continue }
                if (dirgid[h] != upgrp[u]) {
                  if (mismatches > 0) mm = mm "; "
                  mm = mm h " (user " u ", pgrp gid " upgrp[u] ", dir gid " dirgid[h] ")"
                  mismatches++
                }
              }
              if (missing) { print "MISSING"; exit 3 }
              if (mismatches > 0) { print mm; exit 1 }
              print "OK"
              exit 0
            }')
          SHG_RESULT_RC=$?
          case "$SHG_RESULT_RC" in
            0)
              SHG_STATUS=PASS
              SHG_OBSERVED="every interactive user home directory is group-owned by its owner's primary group"
              ;;
            1)
              SHG_STATUS=FAIL
              SHG_OBSERVED="home directory not group-owned by the owner's primary group: $SHG_RESULT"
              ;;
            2)
              SHG_STATUS=NOT_ASSESSED
              SHG_OBSERVED="not assessed - home directory group ownership could not be compared (unparseable evidence)"
              ;;
            3)
              SHG_STATUS=NOT_ASSESSED
              SHG_OBSERVED="not assessed - a user home directory was absent from the listing, so group ownership is unproven"
              ;;
            *)
              SHG_STATUS=NOT_ASSESSED
              SHG_OBSERVED="not assessed - home directory group comparison failed unexpectedly (rc=$SHG_RESULT_RC)"
              ;;
          esac
        fi
      fi
...
case "$SHG_STATUS" in
  PASS)
    SHG_MEANING="All AIX interactive users home directories are group-owned by the home directory owner primary group, satisfying DISA STIG V-215277."
    SHG_FIX="n/a"
    ;;
  FAIL)
    SHG_MEANING="At least one AIX interactive user home directory is not group-owned by the home directory owner primary group, violating DISA STIG V-215277."
    SHG_FIX="run 'chgrp <groupname> <directoryname>' to set the home directory group to the user's primary group; PTxray only recommends this command, it never executes it."
    ;;
  *)
    SHG_MEANING="PTxray did not obtain trustworthy evidence that every AIX interactive user home directory is group-owned by the home directory owner primary group."
    SHG_FIX="verify the /etc/passwd read and the home directory listing as root, then rerun PTxray."
    ;;
esac

add security stig_home_group "Home directory group ownership" \
    "$SHG_STATUS" med "$SHG_OBSERVED" "$SHG_MEANING" "$SHG_FIX" "stig:V-215277"
}
