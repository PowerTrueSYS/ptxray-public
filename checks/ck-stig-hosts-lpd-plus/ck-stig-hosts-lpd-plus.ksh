...
AIXRAY_TOOL=ck-stig-hosts-lpd-plus


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_hosts_lpd_plus — DISA STIG V-215343 (AIX7-00-003037).
  # The print-service access files — /etc/hosts.lpd, or the equivalent Systems
  # and printers.conf — must not contain a line consisting only of a "+"
  # character; such a line grants print access to every host.  When none of
  # the three files exists the rule is not applicable, and every uncertain
  # evidence shape fails closed to NOT_ASSESSED.
  typeset SLP_HLP_FIND SLP_HLP_FIND_RC SLP_HLP_RAW SLP_HLP_RAW_RC SLP_HLP_ST SLP_HLP_KIND
  typeset SLP_SYS_FIND SLP_SYS_FIND_RC SLP_SYS_RAW SLP_SYS_RAW_RC SLP_SYS_ST SLP_SYS_KIND
  typeset SLP_PSC_FIND SLP_PSC_FIND_RC SLP_PSC_RAW SLP_PSC_RAW_RC SLP_PSC_ST SLP_PSC_KIND
  typeset SLP_FOUND SLP_STATUS SLP_SEV SLP_OBSERVED SLP_MEANING SLP_FIX
  typeset SLP_REASON SLP_FILE
  SLP_FOUND=0
  SLP_STATUS=""
  SLP_REASON=""
  SLP_FILE=""
  ...
  # --- /etc/hosts.lpd ---
  SLP_HLP_FIND=$(aix hosts_lpd_find find /etc -name hosts.lpd -print)
  SLP_HLP_FIND_RC=$?
  SLP_HLP_ST=""
  if [ "$SLP_HLP_FIND_RC" -ne 0 ]; then
    SLP_HLP_ST="NA"
    SLP_REASON="find for hosts.lpd failed (rc=$SLP_HLP_FIND_RC)"
  elif [ -n "$SLP_HLP_FIND" ]; then
    # find /etc -name hosts.lpd may list other absolute paths; only /etc/hosts.lpd is the access file.
    SLP_HLP_KIND=$(printf '%s\n' "$SLP_HLP_FIND" | awk '
      {
        line = $0
        gsub(/^[ \t]+|[ \t]+$/, "", line)
        if (line == "") next
        if (line == "/etc/hosts.lpd") { hit = 1; next }
        if (line ~ /^\//) next
        bad = 1
      }
      END {
    ...
    if [ "$SLP_HLP_KIND" = "hit" ]; then
      SLP_FOUND=1
      SLP_HLP_RAW=$(aix hosts_lpd_content more /etc/hosts.lpd)
      SLP_HLP_RAW_RC=$?
      if [ "$SLP_HLP_RAW_RC" -ne 0 ]; then
        SLP_HLP_ST="NA"
        SLP_REASON="read of /etc/hosts.lpd failed (rc=$SLP_HLP_RAW_RC)"
      elif [ -z "$SLP_HLP_RAW" ]; then
        SLP_HLP_ST="NA"
        SLP_REASON="read of /etc/hosts.lpd returned empty with rc 0"
      elif printf '%s\n' "$SLP_HLP_RAW" | awk '
        { line = $0
          gsub(/^[ \t]+|[ \t]+$/, "", line)
          if (line == "+") { plus = 1 } }
        END { if (plus) exit 0; exit 1 }'; then
        SLP_HLP_ST="FAIL"
        [ -n "$SLP_FILE" ] || SLP_FILE="/etc/hosts.lpd"
      else
        SLP_HLP_ST="PASS"
      fi
    elif [ "$SLP_HLP_KIND" != "miss" ]; then
      SLP_HLP_ST="NA"
      SLP_REASON="unexpected find output for hosts.lpd"
    fi
  fi

  # --- /etc/Systems ---
  SLP_SYS_FIND=$(aix systems_find find /etc -name Systems -print)
  SLP_SYS_FIND_RC=$?
  SLP_SYS_ST=""
  if [ "$SLP_SYS_FIND_RC" -ne 0 ]; then
    SLP_SYS_ST="NA"
    SLP_REASON="find for Systems failed (rc=$SLP_SYS_FIND_RC)"
  elif [ -n "$SLP_SYS_FIND" ]; then
    # find /etc -name Systems also matches /etc/uucp/Systems (UUCP); only /etc/Systems is the access file.
    SLP_SYS_KIND=$(printf '%s\n' "$SLP_SYS_FIND" | awk '
      {
        line = $0
        gsub(/^[ \t]+|[ \t]+$/, "", line)
        if (line == "") next
        if (line == "/etc/Systems") { hit = 1; next }
        if (line ~ /^\//) next
        bad = 1
      }
      END {
    ...
    if [ "$SLP_SYS_KIND" = "hit" ]; then
      SLP_FOUND=1
      SLP_SYS_RAW=$(aix systems_content more /etc/Systems)
      SLP_SYS_RAW_RC=$?
      if [ "$SLP_SYS_RAW_RC" -ne 0 ]; then
        SLP_SYS_ST="NA"
        SLP_REASON="read of /etc/Systems failed (rc=$SLP_SYS_RAW_RC)"
      elif [ -z "$SLP_SYS_RAW" ]; then
        SLP_SYS_ST="NA"
        SLP_REASON="read of /etc/Systems returned empty with rc 0"
      elif printf '%s\n' "$SLP_SYS_RAW" | awk '
        { line = $0
          gsub(/^[ \t]+|[ \t]+$/, "", line)
          if (line == "+") { plus = 1 } }
        END { if (plus) exit 0; exit 1 }'; then
        SLP_SYS_ST="FAIL"
        [ -n "$SLP_FILE" ] || SLP_FILE="/etc/Systems"
      else
        SLP_SYS_ST="PASS"
      fi
    elif [ "$SLP_SYS_KIND" != "miss" ]; then
      SLP_SYS_ST="NA"
      SLP_REASON="unexpected find output for Systems"
    fi
  fi

  # --- /etc/printers.conf ---
  SLP_PSC_FIND=$(aix printers_conf_find find /etc -name printers.conf)
  SLP_PSC_FIND_RC=$?
  SLP_PSC_ST=""
  if [ "$SLP_PSC_FIND_RC" -ne 0 ]; then
    SLP_PSC_ST="NA"
    SLP_REASON="find for printers.conf failed (rc=$SLP_PSC_FIND_RC)"
  elif [ -n "$SLP_PSC_FIND" ]; then
    # find /etc -name printers.conf may list other absolute paths; only /etc/printers.conf is the access file.
    SLP_PSC_KIND=$(printf '%s\n' "$SLP_PSC_FIND" | awk '
      {
        line = $0
        gsub(/^[ \t]+|[ \t]+$/, "", line)
        if (line == "") next
        if (line == "/etc/printers.conf") { hit = 1; next }
        if (line ~ /^\//) next
        bad = 1
      }
      END {
    ...
    if [ "$SLP_PSC_KIND" = "hit" ]; then
      SLP_FOUND=1
      SLP_PSC_RAW=$(aix printers_conf_content more /etc/printers.conf)
      SLP_PSC_RAW_RC=$?
      if [ "$SLP_PSC_RAW_RC" -ne 0 ]; then
        SLP_PSC_ST="NA"
        SLP_REASON="read of /etc/printers.conf failed (rc=$SLP_PSC_RAW_RC)"
      elif [ -z "$SLP_PSC_RAW" ]; then
        SLP_PSC_ST="NA"
        SLP_REASON="read of /etc/printers.conf returned empty with rc 0"
      elif printf '%s\n' "$SLP_PSC_RAW" | awk '
        { line = $0
          gsub(/^[ \t]+|[ \t]+$/, "", line)
          if (line == "+") { plus = 1 } }
        END { if (plus) exit 0; exit 1 }'; then
        SLP_PSC_ST="FAIL"
        [ -n "$SLP_FILE" ] || SLP_FILE="/etc/printers.conf"
      else
        SLP_PSC_ST="PASS"
      fi
    elif [ "$SLP_PSC_KIND" != "miss" ]; then
      SLP_PSC_ST="NA"
      SLP_REASON="unexpected find output for printers.conf"
    fi
  fi

  if [ "$SLP_HLP_ST" = "FAIL" ] || [ "$SLP_SYS_ST" = "FAIL" ] || [ "$SLP_PSC_ST" = "FAIL" ]; then
    SLP_STATUS="FAIL"
    SLP_SEV="med"
    SLP_OBSERVED="$SLP_FILE contains a line consisting only of a \"+\" character (a bare + grants print access to all hosts)"
    SLP_MEANING="A bare \"+\" entry in a print-service access file grants every host print access, defeating host-based access control."
    SLP_FIX="Remove the \"+\" entries from the \"hosts.lpd\" (or equivalent) file; PTxray only recommends this command."
  elif [ -n "$SLP_REASON" ]; then
    SLP_STATUS="NOT_ASSESSED"
    SLP_SEV="med"
    SLP_OBSERVED="not assessed - $SLP_REASON"
    SLP_MEANING="PTxray did not obtain trustworthy evidence about whether a print-service access file contains a bare \"+\" grant."
    SLP_FIX="verify the presence and readability of /etc/hosts.lpd, /etc/Systems and /etc/printers.conf as root, then rerun PTxray."
  elif [ "$SLP_FOUND" -eq 1 ]; then
    SLP_STATUS="PASS"
    SLP_SEV="med"
    SLP_OBSERVED="no print-service access file contains a bare \"+\" grant"
    SLP_MEANING="None of the print-service access files present contains a bare \"+\" grant, so print access is not granted to every host."
    SLP_FIX="n/a"
  else
    SLP_STATUS="NOT_APPLICABLE"
    SLP_SEV="low"
    SLP_OBSERVED="no hosts.lpd, Systems or printers.conf file found under /etc"
    SLP_MEANING="No print-service access file exists, so the STIG rule does not apply to this host."
    SLP_FIX="n/a"
  fi

  add security stig_hosts_lpd_plus "AIX hosts.lpd file must not contain a + character" \
    "$SLP_STATUS" "$SLP_SEV" "$SLP_OBSERVED" \
    "$SLP_MEANING" "$SLP_FIX" "stig:V-215343"
}
