...
AIXRAY_TOOL=ck-stig-icmp-timestamp-filter


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_icmp_timestamp_filter — DISA STIG V-215429.
  # AIX must not process ICMP timestamp requests.  The IPsec filter device must
  # be Available (lsdev -Cc ipsec shows ipsec_v4 or ipsec_v6) and the filter
  # table (lsfilt) must deny ICMP type 13 inbound (timestamp request) and ICMP
  # type 14 outbound (timestamp reply).  A host with no IPsec/filter subject at
  # all is the compliant state (svcoff prohibition convention); a
  # present-but-broken filter is a finding.  This is distinct from
  # ck-net-icmptimestamp, which reads the 'no -o icmptimestamp' no option.
  typeset SITF_LSDEV_RAW SITF_LSDEV_RC SITF_LSFILT_RAW SITF_LSFILT_RC
  typeset SITF_LSDEV_SHAPE SITF_FILT_SHAPE SITF_MISSING
  typeset SITF_STATUS SITF_OBSERVED SITF_MEANING SITF_FIX

  SITF_LSDEV_RAW=$(aix lsdev_ipsec lsdev -Cc ipsec)
  SITF_LSDEV_RC=$?
  SITF_LSFILT_RAW=$(aix lsfilt lsfilt)
  SITF_LSFILT_RC=$?
  SITF_LSDEV_SHAPE=""
  SITF_FILT_SHAPE=""
  SITF_STATUS=""

  if [ "$SITF_LSDEV_RC" -eq 0 ] && [ -n "$SITF_LSDEV_RAW" ]; then
    SITF_LSDEV_SHAPE=$(printf '%s\n' "$SITF_LSDEV_RAW" | awk '
      { if ($1 == "ipsec_v4" || $1 == "ipsec_v6") {
          if ($2 == "Available") avail = 1
          else present = 1 } }
      END { if (avail) print "available"
            else if (present) print "not-available"
            else print "absent" }')
  fi
  if [ "$SITF_LSFILT_RC" -eq 0 ] && [ -n "$SITF_LSFILT_RAW" ]; then
    SITF_FILT_SHAPE=$(printf '%s\n' "$SITF_LSFILT_RAW" | awk '
      function evaluate() {
        if (action != "deny" || proto != "icmp" || bad) return
        if (type_num == "" || type_op != "eq") return
        if (type_num != 13 && type_num != 14) return
        if (direction == "" && scope == "") { bad = 1; return }
        if (type_num == 13 && (direction ~ /inbound/ || direction ~ /both/)) found13 = 1
        if (type_num == 14 && (direction ~ /outbound/ || direction ~ /both/)) found14 = 1
      }
      $0 ~ /^[ \t]*Rule[ \t]+[0-9]+/ {
        if (inrule) evaluate()
        inrule = 1
        action = ""; proto = ""
        direction = ""; scope = ""
        type_num = ""; type_op = ""; type_pending = "any"
        code_num = ""; code_op = ""; code_pending = "any"
        anyrule = 1
        next
      }
      inrule {
        ...
        if (low ~ /action/) {
          ...
          if (low ~ /icmptype|sourceport/) type_pending = lv
          else if (low ~ /icmpcode|destport|destinationport/) code_pending = lv
        } else if (low ~ /(icmptype|icmpcode|sourceport|destport|destinationport)$/) {
          is_type = (low ~ /icmptype$/ || low == "sourceport")
          num = lv
          o = ""
          if (num ~ /^[a-z]+[ \t]+/) {
            o = num
          ...
          gsub(/[ \t]+/, "", num)
          if (num !~ /^[0-9]+$/) { bad = 1; next }
          if (o == "") {
            if (is_type) o = type_pending
            else o = code_pending
          }
          if (is_type) { type_num = num + 0; type_op = o }
          else { code_num = num + 0; code_op = o }
        }
      }
      END {
        if (inrule) evaluate()
        if (bad) print "malformed"
        else if (!anyrule) print "norules"
        else if (found13 && found14) print "ok"
        else if (!found13 && !found14) print "missing:13,14"
        else if (!found13) print "missing:13"
        else print "missing:14"
      }')
  fi

  if [ "$SITF_LSDEV_RC" -ne 0 ]; then
    SITF_STATUS=NOT_ASSESSED
    SITF_OBSERVED="not assessed - lsdev -Cc ipsec failed (rc=$SITF_LSDEV_RC)"
  elif [ "$SITF_LSFILT_RC" -ne 0 ]; then
    SITF_STATUS=NOT_ASSESSED
    SITF_OBSERVED="not assessed - lsfilt failed (rc=$SITF_LSFILT_RC)"
  elif [ -z "$SITF_LSDEV_RAW" ] && [ -z "$SITF_LSFILT_RAW" ]; then
    SITF_STATUS=NOT_ASSESSED
    SITF_OBSERVED="not assessed - lsdev and lsfilt both returned empty output"
  elif [ -z "$SITF_LSDEV_RAW" ]; then
    # Empty lsdev (rc 0) is determinate: no IPsec device is Available.
    # A zero-rule filter table is subject absence (svcoff). A parseable
    # rule table means the filter subject is present, so grade it.
    case "$SITF_FILT_SHAPE" in
      norules)
        SITF_STATUS=PASS
        SITF_OBSERVED="IPsec filter subject absent: no IPsec device in Available and no ICMP filter rules"
        ;;
      malformed)
        SITF_STATUS=NOT_ASSESSED
        SITF_OBSERVED="not assessed - lsfilt output was unparseable"
        ;;
      missing:*|ok)
        SITF_STATUS=FAIL
        if [ "$SITF_FILT_SHAPE" = "ok" ]; then
          SITF_OBSERVED="no IPsec device is Available"
        else
          SITF_MISSING=${SITF_FILT_SHAPE#missing:}
          case "$SITF_MISSING" in
            13) SITF_OBSERVED="no IPsec device is Available; IPsec filter lacks a deny rule for ICMP type 13 (inbound)" ;;
            14) SITF_OBSERVED="no IPsec device is Available; IPsec filter lacks a deny rule for ICMP type 14 (outbound)" ;;
            *)  SITF_OBSERVED="no IPsec device is Available; IPsec filter lacks deny rules for ICMP types 13 (inbound) and 14 (outbound)" ;;
          esac
        fi
        ;;
      *)
  ...
  elif [ -z "$SITF_LSFILT_RAW" ]; then
    SITF_STATUS=NOT_ASSESSED
    SITF_OBSERVED="not assessed - lsfilt returned no rows with rc 0; cannot confirm the filter denies ICMP types 13 and 14"
  else
    # Both probes returned rows: the IPsec/filter subject is present.
    case "$SITF_FILT_SHAPE" in
      malformed)
        SITF_STATUS=NOT_ASSESSED
        SITF_OBSERVED="not assessed - lsfilt output was unparseable"
        ;;
      norules)
        if [ "$SITF_LSDEV_SHAPE" = "absent" ]; then
          SITF_STATUS=PASS
          SITF_OBSERVED="IPsec filter subject absent: no IPsec device in Available and no ICMP filter rules"
        else
          SITF_STATUS=FAIL
          SITF_OBSERVED="IPsec filter table has no rules; ICMP timestamp requests are not denied"
        fi
        ;;
      missing:*|ok)
        if [ "$SITF_LSDEV_SHAPE" != "available" ]; then
          SITF_STATUS=FAIL
          SITF_OBSERVED="IPsec device is present but not in Available state"
        elif [ "$SITF_FILT_SHAPE" = "ok" ]; then
          SITF_STATUS=PASS
          SITF_OBSERVED="IPsec filter denies ICMP type 13 (inbound) and ICMP type 14 (outbound)"
        else
          SITF_MISSING=${SITF_FILT_SHAPE#missing:}
          SITF_STATUS=FAIL
          case "$SITF_MISSING" in
            13) SITF_OBSERVED="IPsec filter lacks a deny rule for ICMP type 13 (inbound)" ;;
            14) SITF_OBSERVED="IPsec filter lacks a deny rule for ICMP type 14 (outbound)" ;;
            *)  SITF_OBSERVED="IPsec filter lacks deny rules for ICMP types 13 (inbound) and 14 (outbound)" ;;
          esac
        fi
        ;;
      *)
  ...
  case "$SITF_STATUS" in
    PASS)
      SITF_MEANING="AIX does not process ICMP timestamp requests: the IPsec filter is available and denies ICMP type 13 (timestamp request) inbound and ICMP type 14 (timestamp reply) outbound."
      SITF_FIX="n/a"
      ;;
    FAIL)
      SITF_MEANING="AIX answers ICMP timestamp requests because the IPsec filter does not deny them (the IPsec device is not Available or the ICMP type 13/14 deny rules are missing), widening the network attack surface."
      SITF_FIX="activate the IPsec devices and add the deny rules: mkdev -l ipsec -t 4; mkdev -l ipsec -t 6; genfilt -v 4 -a D -s 0 -m 0 -d 0 -M 0 -c icmp -o eq -p 13 -r B -w I -i all; genfilt -v 4 -a D -s 0 -m 0 -d 0 -M 0 -c icmp -o eq -p 14 -r B -w O -i all; mkfilt -u. PTxray only recommends these commands."
      ;;
    *)
      SITF_MEANING="PTxray did not obtain trustworthy IPsec filter evidence for the ICMP timestamp prohibition."
      SITF_FIX="verify the output of 'lsdev -Cc ipsec' and 'lsfilt' as root, then rerun PTxray."
      ;;
  esac

  add security stig_icmp_timestamp_filter "AIX must not process ICMP timestamp requests" \
    "$SITF_STATUS" med "$SITF_OBSERVED" \
    "$SITF_MEANING" "$SITF_FIX" "stig:V-215429"
}
