...
AIXRAY_TOOL=ck-stig-inn-disabled


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_inn_disabled — DISA STIG V-215392: the Internet Network News (INN)
  # server must be disabled on AIX.  Evidence is process-table rows, uncommented
  # inetd.conf service entries, and SRC subsystem registration; any one present
  # is a finding.  Indeterminate evidence fails closed to NOT_ASSESSED, never PASS.
  typeset INN_PS_RAW INN_PS_RC INN_PS_ROW
  typeset INN_PS_PROD INN_PS_PROD_RC
  typeset INN_INETD_RAW INN_INETD_RC
  typeset INN_INETD_PROD INN_INETD_PROD_RC
  typeset INN_LSSRC_INND_RAW INN_LSSRC_INND_RC
  typeset INN_LSSRC_NNTPD_RAW INN_LSSRC_NNTPD_RC
  typeset INN_PRESENT INN_UNKNOWN INN_DETAILS INN_REASON
  typeset INN_STATUS INN_OBSERVED INN_MEANING INN_FIX

  # STIG evidence (filtered in awk): ps -ef | egrep "innd|nntpd"
  INN_PS_PROD=$(aix ps_innd_nntpd /usr/bin/ps -ef)
  INN_PS_PROD_RC=$?
  INN_PS_RAW=$(printf '%s\n' "$INN_PS_PROD" | awk '/innd/ || /nntpd/ { print }')
  # Producer rc 0 is the only healthy basis: matches -> presence (rc 0),
  # nonempty producer with no matches -> determinate absence (rc 1).
  # Nonzero producer rc, including 1, is unreadable/error, never absence.
  if [ "$INN_PS_PROD_RC" -eq 0 ] && [ -n "$INN_PS_RAW" ]; then
    INN_PS_RC=0
  elif [ "$INN_PS_PROD_RC" -eq 0 ] && [ -n "$INN_PS_PROD" ]; then
    INN_PS_RC=1
  else
    INN_PS_RC=$INN_PS_PROD_RC
  fi

  # STIG evidence (filtered in awk): egrep "innd|nntpd" /etc/inetd.conf | grep -v ^#
  INN_INETD_PROD=$(aix inetd_innd_nntpd /usr/bin/cat /etc/inetd.conf)
  INN_INETD_PROD_RC=$?
  INN_INETD_RAW=$(printf '%s\n' "$INN_INETD_PROD" | awk '
    /^#/ { next }
    /innd/ || /nntpd/ { print }
  ')
  # Producer rc 0 with no uncommented match is determinate absence,
  # including empty producer output (empty readable file). Nonzero
  # producer rc, including 1, is unreadable/error, never absence.
  if [ "$INN_INETD_PROD_RC" -eq 0 ] && [ -n "$INN_INETD_RAW" ]; then
    INN_INETD_RC=0
  elif [ "$INN_INETD_PROD_RC" -eq 0 ]; then
    INN_INETD_RC=1
  else
    INN_INETD_RC=$INN_INETD_PROD_RC
  fi

  INN_LSSRC_INND_RAW=$(aix lssrc_innd /usr/bin/lssrc -s innd)
  INN_LSSRC_INND_RC=$?
  INN_LSSRC_NNTPD_RAW=$(aix lssrc_nntpd /usr/bin/lssrc -s nntpd)
  INN_LSSRC_NNTPD_RC=$?
  ...
  # ps probe: a row whose command names an innd/nntpd executable is a running
  # process. Producer rc 0 with no matching row is determinate absence (the
  # old grep rc 1 on a readable table). Nonzero producer rc, including 1, is
  # unreadable/error and refuses. The probe's own row matches its own pattern,
  # so drop rows whose command names grep/egrep or carries the literal
  # innd|nntpd pattern and require a surviving row that names an innd/nntpd
  # executable; rc 0 whose rows are all the probe itself is a determinate
  # absence, not a running server.
  if [ "$INN_PS_PROD_RC" -eq 0 ] && [ "$INN_PS_RC" -eq 1 ] && [ -z "$INN_PS_RAW" ]; then
    :
  elif [ "$INN_PS_PROD_RC" -eq 0 ] && [ "$INN_PS_RC" -eq 0 ] && [ -n "$INN_PS_RAW" ]; then
    INN_PS_ROW=$(printf '%s\n' "$INN_PS_RAW" | awk '
      /^[ \t]*$/ { next }
      NR == 1 && $1 == "UID" { next }
      {
        cmd = ""
        for (i = 8; i <= NF; i++) cmd = (i == 8) ? $i : cmd " " $i
        if (cmd ~ /egrep/ || cmd ~ /grep/ || cmd ~ /innd[|]nntpd/) next
        if (match(cmd, "(^|[ /])innd([ /]|$)") || match(cmd, "(^|[ /])nntpd([ /]|$)")) {
          print cmd
        }
      }
    ')
    if [ -n "$INN_PS_ROW" ]; then
      INN_PRESENT=1
      INN_DETAILS="process table shows: $(printf '%s\n' "$INN_PS_ROW" | sed -n '1p')"
    fi
  else
    INN_UNKNOWN=1
    INN_REASON="ps probe returned rc=$INN_PS_PROD_RC with unexpected output"
  fi

  # inetd probe: an uncommented innd/nntpd service line enables INN.
  # Producer rc 0 is required; cat rc 1 is unreadable, not absence.
  # Empty filtered output at producer rc 0 (including an empty readable
  # file) is determinate absence.
  if [ "$INN_INETD_PROD_RC" -eq 0 ] && [ "$INN_INETD_RC" -eq 0 ] && [ -n "$INN_INETD_RAW" ]; then
    INN_PRESENT=1
    INN_DETAILS="$INN_DETAILS${INN_DETAILS:+; }inetd.conf enables: $(printf '%s\n' "$INN_INETD_RAW" | sed -n '1p')"
  elif [ "$INN_INETD_PROD_RC" -eq 0 ] && [ "$INN_INETD_RC" -eq 1 ] && [ -z "$INN_INETD_RAW" ]; then
    :
  else
    INN_UNKNOWN=1
    INN_REASON="$INN_REASON${INN_REASON:+, }inetd probe returned rc=$INN_INETD_PROD_RC with unexpected output"
  fi

  # lssrc probes: rc 0 with a subsystem row means SRC defines the subsystem
  # (a finding even when inoperative); "not on file"/"not defined" with a
  # non-zero rc is a determinate absence.
  if [ "$INN_LSSRC_INND_RC" -eq 0 ] && [ -n "$INN_LSSRC_INND_RAW" ]; then
    INN_PRESENT=1
    INN_DETAILS="$INN_DETAILS${INN_DETAILS:+; }SRC defines innd"
  elif [ "$INN_LSSRC_INND_RC" -ne 0 ]; then
    case "$INN_LSSRC_INND_RAW" in
      *"not on file"*|*"not defined"*|*"not found"*) : ;;
      *)
        INN_UNKNOWN=1
        INN_REASON="$INN_REASON${INN_REASON:+, }lssrc -s innd returned rc=$INN_LSSRC_INND_RC with unexpected output"
        ;;
    esac
  else
    INN_UNKNOWN=1
    INN_REASON="$INN_REASON${INN_REASON:+, }lssrc -s innd returned rc=0 with no output"
  fi

  if [ "$INN_LSSRC_NNTPD_RC" -eq 0 ] && [ -n "$INN_LSSRC_NNTPD_RAW" ]; then
    INN_PRESENT=1
    INN_DETAILS="$INN_DETAILS${INN_DETAILS:+; }SRC defines nntpd"
  elif [ "$INN_LSSRC_NNTPD_RC" -ne 0 ]; then
    case "$INN_LSSRC_NNTPD_RAW" in
      *"not on file"*|*"not defined"*|*"not found"*) : ;;
      *)
        INN_UNKNOWN=1
        INN_REASON="$INN_REASON${INN_REASON:+, }lssrc -s nntpd returned rc=$INN_LSSRC_NNTPD_RC with unexpected output"
        ;;
    esac
  else
    INN_UNKNOWN=1
    INN_REASON="$INN_REASON${INN_REASON:+, }lssrc -s nntpd returned rc=0 with no output"
  fi

  if [ "$INN_PRESENT" -eq 1 ]; then
    INN_STATUS=FAIL
  elif [ "$INN_UNKNOWN" -eq 1 ]; then
    INN_STATUS=NOT_ASSESSED
  else
    INN_STATUS=PASS
  fi

  if [ "$INN_STATUS" = "FAIL" ]; then
    INN_OBSERVED="innd/nntpd is not disabled: $INN_DETAILS"
    INN_MEANING="The INN server is enabled: innd or nntpd is running, enabled in inetd.conf, or registered in SRC, so an unauthorized network news service is available."
    INN_FIX="stop and remove the INN service: stopsrc -s innd; stopsrc -s nntpd; kill -1 [innd_pid]; kill -1 [nntpd_pid]; rmssys -s innd; rmssys -s nntpd; refresh -s inetd (PTxray only recommends these commands)"
  elif [ "$INN_STATUS" = "NOT_ASSESSED" ]; then
    INN_OBSERVED="not assessed - $INN_REASON"
    INN_MEANING="PTxray did not obtain trustworthy evidence that the INN server is disabled."
    INN_FIX="re-run the ps, inetd.conf, and lssrc probes as root, then rerun PTxray."
  else
    INN_OBSERVED="no innd or nntpd process, no uncommented inetd.conf entry, and SRC defines neither subsystem"
    INN_MEANING="The INN server is disabled: innd/nntpd are not running, not enabled in inetd.conf, and not defined in SRC."
    INN_FIX="n/a"
  fi

  add security stig_inn_disabled "INN server disabled" \
    "$INN_STATUS" med "$INN_OBSERVED" \
    "$INN_MEANING" "$INN_FIX" "stig:V-215392"
}
