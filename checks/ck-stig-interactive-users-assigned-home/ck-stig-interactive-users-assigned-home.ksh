...
AIXRAY_TOOL=ck-stig-interactive-users-assigned-home


function standalone_check {
_AIXRAY_SESSION_KEYS=""
# stig_interactive_users_assigned_home — DISA STIG V-215435.
# Every interactive (login-capable) account in /etc/passwd must be assigned a
# non-empty home directory and that directory must exist on the filesystem.
# Interactive accounts are those whose login shell is non-empty and is not a
# nologin/false shell. Every uncertain evidence shape fails closed.
typeset SIU_RAW SIU_RC SIU_PARSE_RC SIU_CANDIDATES
typeset SIU_REC SIU_ACCT SIU_HOME SIU_HOME_RAW SIU_HOME_RC
typeset SIU_COUNT SIU_MISSING_COUNT SIU_MISSING_NAMES
typeset SIU_AMBIG_COUNT SIU_AMBIG_NAMES SIU_PROBE_N SIU_OBSERVED

SIU_RAW=$(aix passwd_cols cut -d: -f1,6,7 /etc/passwd)
SIU_RC=$?

if [ "$SIU_RC" -ne 0 ]; then
  add security stig_interactive_users_assigned_home "interactive users have assigned, existing home directories" NOT_ASSESSED med \
      "not assessed - passwd capture failed (rc=$SIU_RC)" \
      "PTxray could not read interactive-user home assignments from /etc/passwd, so it cannot assess whether every interactive user has an assigned, existing home directory." \
      "correct the passwd capture failure and rerun PTxray." \
      "stig:V-215435"
else
  SIU_CANDIDATES=$(printf '%s\n' "$SIU_RAW" | awk -F: '
      NF {
        records++
        user=$1
        home=$2
        shell=$3
        if (user !~ /^[A-Za-z0-9_.][A-Za-z0-9_.-]*$/) { bad=1; next }
        base=shell
        sub(/^.*\//, "", base)
        if (shell == "" || base == "nologin" || base == "false") next
        print user "|" home
      }
      END {
  ...
  if [ "$SIU_PARSE_RC" -ne 0 ]; then
    add security stig_interactive_users_assigned_home "interactive users have assigned, existing home directories" NOT_ASSESSED med \
        "not assessed - passwd output unparseable or empty" \
        "PTxray obtained /etc/passwd output that did not match the expected user:home:shell format, so it cannot determine which interactive users lack an assigned, existing home directory." \
        "inspect the passwd capture and rerun PTxray once the output is well-formed." \
        "stig:V-215435"
  elif [ -z "$SIU_CANDIDATES" ]; then
    add security stig_interactive_users_assigned_home "interactive users have assigned, existing home directories" FAIL med \
        "interactive_users=0" \
        "No interactive (login-capable) accounts were found in /etc/passwd; the required interactive-user configuration is absent." \
        "Restore or create an interactive account with an assigned, existing home directory, then rerun PTxray." \
        "stig:V-215435"
  else
    SIU_COUNT=0
    SIU_MISSING_COUNT=0
    ...
    for SIU_REC in $SIU_CANDIDATES; do
      SIU_ACCT=${SIU_REC%%\|*}
      SIU_HOME=${SIU_REC#*\|}
      SIU_COUNT=$(( SIU_COUNT + 1 ))

      if [ -z "$SIU_HOME" ]; then
        SIU_MISSING_COUNT=$(( SIU_MISSING_COUNT + 1 ))
        if [ -z "$SIU_MISSING_NAMES" ]; then
          SIU_MISSING_NAMES="$SIU_ACCT"
        else
          SIU_MISSING_NAMES="$SIU_MISSING_NAMES $SIU_ACCT"
        fi
      else
        SIU_PROBE_N=$(( SIU_PROBE_N + 1 ))
        # Positional probe key (home_ls_1..home_ls_n): never interpolated from
        # the account name, so AIX accounts such as svc-admin, Jdoe or db2inst.1
        # cannot yield a non-snake_case key.
        SIU_HOME_RAW=$(aix "home_ls_$SIU_PROBE_N" ls -ld "$SIU_HOME")
        SIU_HOME_RC=$?
        if [ "$SIU_HOME_RC" -ne 0 ]; then
          # Any non-zero rc means the directory could not be listed. That is a
          # determinate "home does not exist" unless the diagnostic positively
          # shows the probe was blocked (e.g. permission denied); AIX ls does not
          # guarantee a specific exit code for ENOENT, so do not key on rc alone.
          case "$SIU_HOME_RAW" in
            *permission*|*"not permitted"*|*denied*)
              SIU_AMBIG_COUNT=$(( SIU_AMBIG_COUNT + 1 ))
              if [ -z "$SIU_AMBIG_NAMES" ]; then
                SIU_AMBIG_NAMES="$SIU_ACCT:$SIU_HOME (rc=$SIU_HOME_RC)"
              else
                SIU_AMBIG_NAMES="$SIU_AMBIG_NAMES $SIU_ACCT:$SIU_HOME (rc=$SIU_HOME_RC)"
              fi
              ;;
            *)
              SIU_MISSING_COUNT=$(( SIU_MISSING_COUNT + 1 ))
              if [ -z "$SIU_MISSING_NAMES" ]; then
                SIU_MISSING_NAMES="$SIU_ACCT:$SIU_HOME"
              else
                SIU_MISSING_NAMES="$SIU_MISSING_NAMES $SIU_ACCT:$SIU_HOME"
              fi
              ;;
          esac
        fi
    ...
    if [ "$SIU_MISSING_COUNT" -gt 0 ]; then
      # A determinate violation is authoritative even when some other home could
      # not be probed; refusing here would hide a violation already established.
      if [ -n "$SIU_AMBIG_NAMES" ]; then
        SIU_OBSERVED="interactive_users=$SIU_COUNT missing=$SIU_MISSING_NAMES unprobed=$SIU_AMBIG_NAMES"
      else
        SIU_OBSERVED="interactive_users=$SIU_COUNT missing=$SIU_MISSING_NAMES"
      fi
      add security stig_interactive_users_assigned_home "interactive users have assigned, existing home directories" FAIL med \
          "$SIU_OBSERVED" \
          "At least one interactive (login-capable) account has no home directory assigned in /etc/passwd, or its assigned home directory does not exist." \
          ...
          "stig:V-215435"
    elif [ "$SIU_AMBIG_COUNT" -gt 0 ]; then
      add security stig_interactive_users_assigned_home "interactive users have assigned, existing home directories" NOT_ASSESSED med \
          "not assessed - home probe inconclusive for $SIU_AMBIG_NAMES" \
          "PTxray could not determine whether the home directory of at least one interactive account exists, so it cannot establish whether every interactive user has an existing home directory." \
          "resolve why the home directory could not be listed, then rerun PTxray." \
          "stig:V-215435"
    else
      add security stig_interactive_users_assigned_home "interactive users have assigned, existing home directories" PASS med \
          "interactive_users=$SIU_COUNT" \
          "Every interactive (login-capable) account has a home directory assigned in /etc/passwd and that directory exists on the filesystem." \
          "n/a" \
          "stig:V-215435"
    fi
  fi
