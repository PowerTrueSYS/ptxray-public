...
AIXRAY_TOOL=ck-stig-interactive-users-home-directories


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_interactive_users_home_directories — DISA STIG V-215276.
  # All AIX interactive users' home directories must be owned by the assigned
  # user. Check content: cut -d: -f6 /etc/passwd | xargs ls -lLd
  typeset IUHD_PASSWD IUHD_PASSWD_RC IUHD_LS IUHD_LS_RC IUHD_PARTIAL
  typeset IUHD_CANDIDATES IUHD_OWNERS IUHD_REASON IUHD_REC
  typeset IUHD_USER IUHD_UID IUHD_HOME IUHD_MAP IUHD_MAP_HOME IUHD_OWNER
  typeset IUHD_CHECK_COUNT IUHD_FAIL_COUNT IUHD_OBSERVED IUHD_STATUS
  typeset IUHD_PARSE_RC IUHD_OWNERS_RC

  IUHD_PASSWD=""; IUHD_PASSWD_RC=0; IUHD_LS=""; IUHD_LS_RC=0; IUHD_PARTIAL=0
  IUHD_CANDIDATES=""; IUHD_OWNERS=""; IUHD_REASON=""
  IUHD_REC=""; IUHD_USER=""; IUHD_UID=""; IUHD_HOME=""
  IUHD_MAP=""; IUHD_MAP_HOME=""; IUHD_OWNER=""
  IUHD_CHECK_COUNT=0; IUHD_FAIL_COUNT=0; IUHD_OBSERVED=""; IUHD_STATUS=""
  IUHD_PARSE_RC=0; IUHD_OWNERS_RC=0

  # 1. Passwd inventory of interactive (non-system) user homes. Well-known
  #    non-interactive AIX system accounts are skipped, per the interactive
  #    user scope of V-215276.
  IUHD_PASSWD=$(aix passwd_entries cat /etc/passwd)
  IUHD_PASSWD_RC=$?

  if [ "$IUHD_PASSWD_RC" -ne 0 ]; then
    IUHD_REASON="passwd capture failed (rc=$IUHD_PASSWD_RC)"
  elif [ -z "$IUHD_PASSWD" ]; then
    IUHD_REASON="passwd capture empty"
  else
    IUHD_CANDIDATES=$(printf '%s\n' "$IUHD_PASSWD" | awk '
      BEGIN { FS = ":" }
      function system_acct(n) {
        return (n == "daemon" || n == "bin" || n == "sys" || n == "adm" || n == "uucp" || n == "nuucp" || n == "lpd" || n == "nobody" || n == "lp" || n == "guest" || n == "invscout" || n == "snapp" || n == "ipsec" || n == "pconsole" || n == "sshd" || n == "esaadmin")
      }
      function nonlogin_shell(s) {
        return (s == "/usr/sbin/uucp/uucico" || s == "/usr/bin/false" || s == "/bin/false" || s == "/usr/sbin/nologin" || s == "/usr/bin/nologin" || s == "/sbin/nologin" || s == "/bin/nologin")
      }
      NF && $0 !~ /^[#]/ {
        records++
        if (NF < 7) { bad = 1; next }
        name = $1
        if (name !~ /^[A-Za-z0-9_.][A-Za-z0-9_.-]*$/) { bad = 1; next }
        if (system_acct(name)) next
        shell = $7
        if (nonlogin_shell(shell)) next
        uid = $3
        if (uid !~ /^[0-9][0-9]*$/) { bad = 1; next }
        home = $6
        if (home == "" || home == "/dev/null" || home == "NONE" || home == "-") next
        if (home ~ /[ \t|"]/) { bad = 1; next }
        print name "|" uid "|" home
      }
      END {
        if (records == 0 || bad) exit 1
      }
    ')
    IUHD_PARSE_RC=$?
    if [ "$IUHD_PARSE_RC" -ne 0 ]; then
      IUHD_REASON="passwd capture unparseable"
    fi
  fi

  if [ -n "$IUHD_REASON" ]; then
    IUHD_STATUS="NOT_ASSESSED"
  elif [ -z "$IUHD_CANDIDATES" ]; then
    IUHD_STATUS="FAIL"
    IUHD_OBSERVED="subject absent: no interactive user home directories found in /etc/passwd"
  else
    # 2. STIG check-content listing of every passwd home. The pipeline runs
    #    inside ksh -c so the live aix helper (argv exec, not a shell) runs the
    #    STIG command verbatim; fixtures still stub the single passwd_homes_ls key.
    IUHD_LS=$(aix passwd_homes_ls ksh -c 'cut -d: -f6 /etc/passwd | xargs ls -lLd')
    IUHD_LS_RC=$?
    # ls over a live tree returns rc 1/2 when some path is unreadable, while
    # still printing every row it could read. Grade those rows; refuse only
    # when the probe produced no rows.
    if [ -n "$IUHD_LS" ]; then
      if [ "$IUHD_LS_RC" -eq 1 ] || [ "$IUHD_LS_RC" -eq 2 ]; then
        IUHD_PARTIAL=1
      elif [ "$IUHD_LS_RC" -ne 0 ]; then
        IUHD_REASON="home listing capture failed (rc=$IUHD_LS_RC)"
      fi
    elif [ "$IUHD_LS_RC" -ne 0 ]; then
      IUHD_REASON="home listing capture failed (rc=$IUHD_LS_RC)"
    else
      IUHD_REASON="home listing empty"
    fi
    if [ -z "$IUHD_REASON" ]; then
      IUHD_OWNERS=$(printf '%s\n' "$IUHD_LS" | awk '
        NF {
          records++
          if (NF < 9) { bad = 1; next }
          owner = $3
      ...
      ')
      IUHD_OWNERS_RC=$?
      if [ "$IUHD_OWNERS_RC" -ne 0 ]; then
        IUHD_REASON="home listing unparseable"
      fi
    fi
    if [ -n "$IUHD_REASON" ]; then
      IUHD_STATUS="NOT_ASSESSED"
    else
      # 3. Compare each interactive home's owner to the assigned user.
      IUHD_CHECK_COUNT=0
      IUHD_FAIL_COUNT=0
      IUHD_OBSERVED=""
      for IUHD_REC in $IUHD_CANDIDATES; do
        IUHD_USER=${IUHD_REC%%\|*}
        IUHD_REST=${IUHD_REC#*\|}
        IUHD_UID=${IUHD_REST%%\|*}
        IUHD_HOME=${IUHD_REST#*\|}
        IUHD_CHECK_COUNT=$(( IUHD_CHECK_COUNT + 1 ))
        IUHD_OWNER=""
        for IUHD_MAP in $IUHD_OWNERS; do
          IUHD_MAP_HOME=${IUHD_MAP%%\|*}
          if [ "$IUHD_MAP_HOME" = "$IUHD_HOME" ]; then
            IUHD_OWNER=${IUHD_MAP#*\|}
            break
          fi
        done
        if [ -z "$IUHD_OWNER" ]; then
          IUHD_FAIL_COUNT=$(( IUHD_FAIL_COUNT + 1 ))
          if [ -n "$IUHD_OBSERVED" ]; then IUHD_OBSERVED="$IUHD_OBSERVED; "; fi
          IUHD_OBSERVED="${IUHD_OBSERVED}home=$IUHD_HOME missing from home listing"
        elif [ "$IUHD_OWNER" = "$IUHD_USER" ] || [ "$IUHD_OWNER" = "$IUHD_UID" ]; then
          :
        else
          IUHD_FAIL_COUNT=$(( IUHD_FAIL_COUNT + 1 ))
          if [ -n "$IUHD_OBSERVED" ]; then IUHD_OBSERVED="$IUHD_OBSERVED; "; fi
          IUHD_OBSERVED="${IUHD_OBSERVED}home=$IUHD_HOME owner=$IUHD_OWNER"
        fi
      done
      if [ "$IUHD_FAIL_COUNT" -gt 0 ]; then
        IUHD_STATUS="FAIL"
      else
        IUHD_STATUS="PASS"
        IUHD_OBSERVED="n=$IUHD_CHECK_COUNT interactive user home directories owned by their assigned users"
        if [ "$IUHD_PARTIAL" -eq 1 ]; then
          IUHD_OBSERVED="$IUHD_OBSERVED; some paths unreadable"
        fi
      fi
    fi
  fi

  if [ "$IUHD_STATUS" = "PASS" ]; then
    add security stig_interactive_users_home_directories "interactive users home directories" PASS med \
        "$IUHD_OBSERVED" \
        "Every interactive AIX user home directory is owned by the passwd user assigned to it, satisfying DISA STIG V-215276." \
        "n/a" \
        "stig:V-215276"
  elif [ "$IUHD_STATUS" = "NOT_ASSESSED" ]; then
    add security stig_interactive_users_home_directories "interactive users home directories" NOT_ASSESSED med \
        "not assessed - $IUHD_REASON" \
        "PTxray did not obtain a trustworthy passwd and home ownership listing, so it cannot confirm every interactive user home is owned by its assigned user." \
        "run 'cut -d: -f6 /etc/passwd | xargs ls -lLd', correct the capture problem, and rerun PTxray." \
        "stig:V-215276"
  else
    add security stig_interactive_users_home_directories "interactive users home directories" FAIL med \
        "$IUHD_OBSERVED" \
        "One or more interactive AIX user home directories are not owned by the passwd user assigned to them, violating DISA STIG V-215276." \
        "change the home directory owner after validation with 'chown <user> <home directory>'; PTxray only recommends this command and never executes it." \
        "stig:V-215276"
  fi
}
