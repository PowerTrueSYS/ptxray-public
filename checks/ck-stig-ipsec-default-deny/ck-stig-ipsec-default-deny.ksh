...
AIXRAY_TOOL=ck-stig-ipsec-default-deny


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_ipsec_default_deny — DISA STIG V-219057.
  # AIX must employ a deny-all, allow-by-exception IPsec default filter:
  # the Rule-ID 0 "all traffic" rule for IPv4 and IPv6 must be deny.  Any
  # default rule action of permit, or a listing that determinately lacks the
  # default deny rule, is a finding.  Uncertain evidence fails closed.
  typeset IPD_V4_RAW IPD_V4_RC IPD_V4_PARSE IPD_V4_STATE IPD_V4_WHY
  typeset IPD_V6_RAW IPD_V6_RC IPD_V6_PARSE IPD_V6_STATE IPD_V6_WHY
  typeset IPD_STATUS IPD_OBSERVED IPD_MEANING IPD_FIX IPD_REASON

  function ipd_parse_rules {
    awk '
/^[ \t]*Rule[ \t]+[0-9][0-9]*[ \t]*:/ {
  r = $0
  sub(/^[ \t]*Rule[ \t]+/, "", r)
...
END {
  if (p) print "permit"
  else if (d) print "deny"
  else if (b0 || bad) print "malformed"
  else if (act || hdr) print "absent"
  else print "nostructure"
}'
  }

  IPD_V4_RAW=$(aix lsfilt_v4_n0 lsfilt -v4 -n0)
  IPD_V4_RC=$?
  IPD_V6_RAW=$(aix lsfilt_v6_n0 lsfilt -v6 -n0)
  IPD_V6_RC=$?
  ...
  if [ "$IPD_V4_RC" -ne 0 ]; then
    IPD_V4_STATE=refuse
    IPD_V4_WHY="lsfilt -v4 -n0 failed (rc=$IPD_V4_RC)"
  elif [ -z "$IPD_V4_RAW" ]; then
    IPD_V4_STATE=refuse
    IPD_V4_WHY="lsfilt -v4 -n0 returned empty output"
  else
    IPD_V4_PARSE=$(printf '%s\n' "$IPD_V4_RAW" | ipd_parse_rules)
    case "$IPD_V4_PARSE" in
      permit)
        IPD_V4_STATE=permit
        IPD_V4_WHY="IPv4 default (Rule 0) action is permit"
        ;;
      deny)
        IPD_V4_STATE=deny
        IPD_V4_WHY="IPv4 default (Rule 0) action is deny"
        ;;
      malformed)
        IPD_V4_STATE=refuse
        IPD_V4_WHY="lsfilt -v4 -n0 output was not parseable"
        ;;
      absent)
        IPD_V4_STATE=absent
        IPD_V4_WHY="IPv4 listing lacks the Rule-ID 0 default deny rule"
        ;;
      *)
        if printf '%s\n' "$IPD_V4_RAW" | grep -Ei 'not active|no filter rules|no rules|not defined|not in effect|not configured|not present' >/dev/null 2>/dev/null; then
          IPD_V4_STATE=absent; IPD_V4_WHY="IPsec default filter is not present (no IPv4 Rule-ID 0 listing)"
        else
          IPD_V4_STATE=refuse; IPD_V4_WHY="lsfilt -v4 -n0 output was not a recognizable IPsec rule listing"
        fi
        ;;
    esac
  fi

  if [ "$IPD_V6_RC" -ne 0 ]; then
    IPD_V6_STATE=refuse
    IPD_V6_WHY="lsfilt -v6 -n0 failed (rc=$IPD_V6_RC)"
  elif [ -z "$IPD_V6_RAW" ]; then
    IPD_V6_STATE=refuse
    IPD_V6_WHY="lsfilt -v6 -n0 returned empty output"
  else
    IPD_V6_PARSE=$(printf '%s\n' "$IPD_V6_RAW" | ipd_parse_rules)
    case "$IPD_V6_PARSE" in
      permit)
        IPD_V6_STATE=permit
        IPD_V6_WHY="IPv6 default (Rule 0) action is permit"
        ;;
      deny)
        IPD_V6_STATE=deny
        IPD_V6_WHY="IPv6 default (Rule 0) action is deny"
        ;;
      malformed)
        IPD_V6_STATE=refuse
        IPD_V6_WHY="lsfilt -v6 -n0 output was not parseable"
        ;;
      absent)
        IPD_V6_STATE=absent
        IPD_V6_WHY="IPv6 listing lacks the Rule-ID 0 default deny rule"
        ;;
      *)
        if printf '%s\n' "$IPD_V6_RAW" | grep -Ei 'not active|no filter rules|no rules|not defined|not in effect|not configured|not present' >/dev/null 2>/dev/null; then
          IPD_V6_STATE=absent; IPD_V6_WHY="IPsec default filter is not present (no IPv6 Rule-ID 0 listing)"
        else
          IPD_V6_STATE=refuse; IPD_V6_WHY="lsfilt -v6 -n0 output was not a recognizable IPsec rule listing"
        fi
        ;;
    esac
  fi

  if [ "$IPD_V4_STATE" = refuse ] || [ "$IPD_V6_STATE" = refuse ]; then
    IPD_STATUS=NOT_ASSESSED
    if [ "$IPD_V4_STATE" = refuse ]; then
      IPD_REASON=$IPD_V4_WHY
    else
      IPD_REASON=$IPD_V6_WHY
    fi
    IPD_OBSERVED="not assessed - $IPD_REASON"
    IPD_MEANING="PTxray did not obtain trustworthy evidence of the IPsec default deny rule for both IPv4 and IPv6."
    IPD_FIX="verify that lsfilt -v4 -n0 and lsfilt -v6 -n0 return the Rule-ID 0 filter block, then rerun PTxray."
  elif [ "$IPD_V4_STATE" = deny ] && [ "$IPD_V6_STATE" = deny ]; then
    IPD_STATUS=PASS
    IPD_OBSERVED="IPv4 and IPv6 default IPsec filters both use Rule action deny (deny-all, allow-by-exception)"
    IPD_MEANING="The Rule-ID 0 all-traffic filter denies outbound packets for both IP versions, so connections to other systems are blocked unless an explicit permit exception exists."
    IPD_FIX="n/a"
  else
    IPD_STATUS=FAIL
    IPD_OBSERVED=""
    if [ "$IPD_V4_STATE" = permit ]; then
      IPD_OBSERVED="IPv4 default Rule action is permit"
    elif [ "$IPD_V4_STATE" = absent ]; then
      IPD_OBSERVED="IPv4 default deny rule is absent"
    fi
    if [ "$IPD_V6_STATE" = permit ]; then
      if [ -n "$IPD_OBSERVED" ]; then IPD_OBSERVED="$IPD_OBSERVED; "; fi
      IPD_OBSERVED="${IPD_OBSERVED}IPv6 default Rule action is permit"
    elif [ "$IPD_V6_STATE" = absent ]; then
      if [ -n "$IPD_OBSERVED" ]; then IPD_OBSERVED="$IPD_OBSERVED; "; fi
      IPD_OBSERVED="${IPD_OBSERVED}IPv6 default deny rule is absent"
    fi
    [ -n "$IPD_OBSERVED" ] || IPD_OBSERVED="IPsec default filter is not deny-all"
    IPD_MEANING="The default IPsec filter does not enforce a deny-all, allow-by-exception policy, so outbound connections to other systems may be permitted without an explicit exception."
    IPD_FIX="make the Rule-ID 0 all-traffic filter deny for both IP versions and add explicit permit exceptions, then activate: mkdev -l ipsec -t 4; mkdev -l ipsec -t 6; chfilt -a D -v 4 -n 0; chfilt -a D -v 6 -n 0; genfilt -w B -v 4 -s 10.10.10.10 -p 0 -P 0 -o any -O any -m 255.255.255.255 -M 255.255.255.255 -i all -g Y -d 11.11.11.11 -c all -a P; mkfilt -u"
  fi

  add security stig_ipsec_default_deny "IPsec default deny-all filter" \
    "$IPD_STATUS" med "$IPD_OBSERVED" \
    "$IPD_MEANING" "$IPD_FIX" "stig:V-219057"
}
