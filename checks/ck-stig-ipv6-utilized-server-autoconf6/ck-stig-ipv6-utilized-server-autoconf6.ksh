...
AIXRAY_TOOL=ck-stig-ipv6-utilized-server-autoconf6


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_ipv6_utilized_server_autoconf6 — DISA STIG V-215357.
  # If IPv6 is not utilized on an AIX server, the autoconf6 daemon must be disabled.
  # The probe greps /etc/rc.tcpip for an uncommented boot line for the daemon; only
  # 'start /usr/sbin/autoconf6' (start word + that path) boots autoconf6. A commented
  # '#start' rule is not a boot and never matches the anchored pattern; a bare
  # autoconf6 line (such as cloud-init injects) carries no 'start' word and so also
  # does not match here. grep's exit status is the oracle: rc 1 = no match (daemon
  # absent = the compliant state for a prohibition rule), rc 0 = match (daemon
  # enabled = finding), rc 2+ = the file could not be read (not assessed). rc 0 with
  # empty output is a swallowed error, never absence, so it is NOT_ASSESSED rather
  # than a hollow PASS.
  typeset AF6_RAW AF6_RC

  AF6_RAW=$(aix rctcp_autoconf6 grep "^start[[:blank:]]/usr/sbin/autoconf6" /etc/rc.tcpip)
  AF6_RC=$?

  if [ -n "$AF6_RAW" ]; then
    add security stig_ipv6_utilized_server_autoconf6 "IPv6 autoconf6 daemon state" FAIL med \
        "autoconf6 starts at boot in /etc/rc.tcpip" \
        "The autoconf6 daemon is enabled to start at boot in /etc/rc.tcpip. DISA STIG V-215357 requires autoconf6 to be disabled if IPv6 is not utilized on this server; PTxray checks only the /etc/rc.tcpip boot state and does not determine whether IPv6 is in use." \
        "disable the daemon with 'chrctcp -d autoconf6' after confirming IPv6 is not in use; PTxray only recommends this command, it does not run it." \
        "stig:V-215357"
  elif [ "$AF6_RC" -eq 1 ]; then
    add security stig_ipv6_utilized_server_autoconf6 "IPv6 autoconf6 daemon state" PASS med \
        "autoconf6 not enabled in /etc/rc.tcpip" \
        "The autoconf6 daemon does not start at boot in /etc/rc.tcpip. DISA STIG V-215357 requires autoconf6 to be disabled if IPv6 is not utilized on this server; PTxray checks only the /etc/rc.tcpip boot state and does not determine whether IPv6 is in use." \
        "n/a" "stig:V-215357"
  elif [ "$AF6_RC" -ne 0 ]; then
    add security stig_ipv6_utilized_server_autoconf6 "IPv6 autoconf6 daemon state" NOT_ASSESSED med \
        "not assessed - probe failed (rc=$AF6_RC)" \
        "PTxray could not read /etc/rc.tcpip to determine whether the autoconf6 daemon starts at boot, so the rule is not assessed rather than guessed compliant." \
        "verify /etc/rc.tcpip is readable, correct the capture problem, and rerun PTxray." \
        "stig:V-215357"
  else
    add security stig_ipv6_utilized_server_autoconf6 "IPv6 autoconf6 daemon state" NOT_ASSESSED med \
        "not assessed - probe returned rc 0 with no output (swallowed error)" \
        "The autoconf6 probe reported success with no captured output; for this grep, rc 0 always accompanies a selected line, so empty output with rc 0 is a swallowed error, not evidence the daemon is disabled." \
