...
AIXRAY_TOOL=ck-stig-ksh-logout


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # STIG V-215310 — per-user ksh/Bourne logout policy.
  typeset KLO_USERS KLO_USERS_RC KLO_LIST KLO_LIST_RC KLO_CONTENT KLO_CONTENT_RC
  typeset KLO_TRAPS KLO_TRAPS_RC KLO_SHAPE KLO_STATUS KLO_OBSERVED

  KLO_USERS=$(aix interactive_users awk -F: '$7 ~ /(^|\/)(ksh|sh)$/ { print $1 "|" $6; n++ } END { if (n == 0) print "none" }' /etc/passwd)
  KLO_USERS_RC=$?
  if [ "$KLO_USERS_RC" -ne 0 ]; then
    KLO_STATUS=NOT_ASSESSED
    KLO_OBSERVED="not assessed - interactive user enumeration failed (rc=$KLO_USERS_RC)"
  elif [ -z "$KLO_USERS" ]; then
    KLO_STATUS=NOT_ASSESSED
    KLO_OBSERVED="not assessed - interactive user enumeration was empty"
  elif [ "$KLO_USERS" = none ]; then
    KLO_STATUS=FAIL
    KLO_OBSERVED="no interactive ksh/Bourne users were enumerated; required logout configuration is absent"
  else
    KLO_LIST=$(aix logout_listing sh -c 'for home in `cut -d: -f6 /etc/passwd`; do if ls -alL $home/.logout >/dev/null 2>&1; then echo "$home|present"; else echo "$home|missing"; fi; done')
    KLO_LIST_RC=$?
    KLO_CONTENT=$(aix logout_contents sh -c 'for home in `cut -d: -f6 /etc/passwd`; do if cat $home/.logout 2>/dev/null | awk "NF { found=1 } END { exit !found }"; then echo "$home|message"; else echo "$home|empty"; fi; done')
    KLO_CONTENT_RC=$?
    KLO_TRAPS=$(aix profile_traps sh -c 'for home in `cut -d: -f6 /etc/passwd`; do if grep "trap '\''\$HOME/.logout'\'' EXIT " $home/.profile >/dev/null 2>&1; then echo "$home"; fi; done')
    KLO_TRAPS_RC=$?
    if [ "$KLO_LIST_RC" -ne 0 ] || [ "$KLO_CONTENT_RC" -ne 0 ] || [ "$KLO_TRAPS_RC" -ne 0 ]; then
      KLO_STATUS=NOT_ASSESSED
      KLO_OBSERVED="not assessed - one or more logout policy probes failed"
    elif [ -z "$KLO_LIST" ] || [ -z "$KLO_CONTENT" ]; then
      KLO_STATUS=NOT_ASSESSED
      KLO_OBSERVED="not assessed - logout policy probe output was empty"
    else
      # Shared passwd homes repeat the same path; identical repeats are determinate.
      KLO_SHAPE=$(printf '%s\n' "$KLO_USERS" | LIST="$KLO_LIST" CONTENT="$KLO_CONTENT" TRAPS="$KLO_TRAPS" awk -F'|' '
        BEGIN {
          list = ENVIRON["LIST"]
          content = ENVIRON["CONTENT"]
          traps = ENVIRON["TRAPS"]
          n = split(list, rec, "\n")
          for (i = 1; i <= n; i++) {
            if (rec[i] ~ /^[ \t]*$/) continue
            if (split(rec[i], f, "|") != 2) { bad = 1; continue }
            if (f[1] !~ /^\// || (f[2] != "present" && f[2] != "missing")) { bad = 1; continue }
            if (f[1] in have_list) {
              if (have_list[f[1]] != f[2]) bad = 1
              continue
            }
            have_list[f[1]] = f[2]
          }
          ...
          for (i = 1; i <= n; i++) {
            if (rec[i] ~ /^[ \t]*$/) continue
            if (split(rec[i], f, "|") != 2) { bad = 1; continue }
            if (f[1] !~ /^\// || (f[2] != "message" && f[2] != "empty")) { bad = 1; continue }
            if (f[1] in have_content) {
              if (have_content[f[1]] != f[2]) bad = 1
              continue
            }
            have_content[f[1]] = f[2]
          }
          n = split(traps, rec, "\n")
          for (i = 1; i <= n; i++) {
            if (rec[i] ~ /^[ \t]*$/) continue
            if (index(rec[i], "|") != 0 || rec[i] !~ /^\//) { bad = 1; continue }
            have_trap[rec[i]] = 1
          }
        }
        /^[ \t]*$/ { bad = 1; next }
        NF != 2 || $1 !~ /^[A-Za-z_][A-Za-z0-9_.-]*$/ || $2 !~ /^\// { bad = 1; next }
        {
          seen[$1]++
          records++
          if (have_list[$2] != "present") fail = 1
          if (have_content[$2] != "message") fail = 1
          if (!($2 in have_trap)) fail = 1
        }
        END {
          if (bad || records == 0) { print "malformed"; exit }
          for (name in seen) if (seen[name] != 1) { print "malformed"; exit }
          if (fail) print "fail"
          else print "ok"
        }')
      case "$KLO_SHAPE" in
        ok)
          KLO_STATUS=PASS
          KLO_OBSERVED="interactive ksh/Bourne users have logout messages and EXIT traps"
          ;;
        fail)
          KLO_STATUS=FAIL
          KLO_OBSERVED="interactive ksh/Bourne logout policy is incomplete (missing .logout, logout message, or .profile EXIT trap)"
          ;;
        *)
          KLO_STATUS=NOT_ASSESSED
          KLO_OBSERVED="not assessed - logout policy evidence was unparseable"
          ;;
      esac
    fi
  fi
  ...
  case "$KLO_STATUS" in
    PASS)
      add security stig_ksh_logout "Bourne/ksh logout messages" PASS med "$KLO_OBSERVED" \
        "Every enumerated interactive Bourne or ksh user has a logout message and an EXIT trap that calls it." \
        "n/a" "stig:V-215310"
      ;;
    FAIL)
      add security stig_ksh_logout "Bourne/ksh logout messages" FAIL low "$KLO_OBSERVED" \
        "An interactive Bourne or ksh user can log out without the required logout message." \
        "Create the \".logout\" file if it does not exist.; Add the following two lines to \".logout\" to display a logout message and sleep for \"5\" seconds:; echo \"You are being disconnected.\"" "stig:V-215310"
      ;;
    *)
      add security stig_ksh_logout "Bourne/ksh logout messages" NOT_ASSESSED med "$KLO_OBSERVED" \
        "PTxray did not obtain trustworthy evidence for the interactive users' logout policy." \
        "Verify the readable /etc/passwd, .logout, and .profile evidence, then rerun PTxray." "stig:V-215310"
      ;;
  esac
}
