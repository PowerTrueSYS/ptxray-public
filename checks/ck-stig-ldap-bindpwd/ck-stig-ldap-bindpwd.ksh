...
AIXRAY_TOOL=ck-stig-ldap-bindpwd


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_ldap_bindpwd -- DISA STIG V-215174 (AIX7-00-001007).
  # If AIX uses LDAP for authentication or account information, the LDAP client
  # configuration (/etc/security/ldap/ldap.cfg) must not store the bind password
  # or the SSL key database password in plaintext.  Absence of the file is the
  # compliant state (prohibition rule, svcoff convention).  The two FAIL branches
  # -- an unencrypted bindpwd line and any ldapsslkeypwd line -- are evaluated
  # independently and OR'd, so a plaintext bind password is a finding whether or
  # not an SSL key password is also present.  The file is secret-bearing: only
  # the encryption shape is reported, never a password value.
  typeset LBP_BIND_RAW LBP_BIND_RC LBP_SSL_RAW LBP_SSL_RC
  typeset LBP_BIND_STATE LBP_SSL_STATE
  typeset LBP_NOTES LBP_STATUS LBP_OBSERVED LBP_MEANING LBP_FIX LBP_SEVERITY
  typeset LBP_BAD LBP_DETAIL

  LBP_NOTES=""
  LBP_STATUS=""

  LBP_BIND_RAW=$(aix ldap_bindpwd grep ^bindpwd: /etc/security/ldap/ldap.cfg)
  LBP_BIND_RC=$?
  LBP_SSL_RAW=$(aix ldap_sslkeypwd grep '^ldapsslkeypwd' /etc/security/ldap/ldap.cfg)
  LBP_SSL_RC=$?

  # Classify each probe.  grep rc: 0 = matched a line, 1 = no match, 2 = file
  # unreadable/absent.  rc 0 with no rows is a swallowed error, never absence;
  # any other rc with no rows is a probe failure; rows with a failing rc are
  # contradictory and fail closed.
  if [ -n "$LBP_BIND_RAW" ]; then
    if [ "$LBP_BIND_RC" -eq 0 ]; then
      LBP_BIND_STATE=match
    else
      LBP_BIND_STATE=contradict
    fi
  else
    case "$LBP_BIND_RC" in
      0) LBP_BIND_STATE=empty_rc0 ;;
      1) LBP_BIND_STATE=no_match ;;
      2) LBP_BIND_STATE=file_absent ;;
      *) LBP_BIND_STATE=probe_failed ;;
    esac
  fi
  if [ -n "$LBP_SSL_RAW" ]; then
    if [ "$LBP_SSL_RC" -eq 0 ]; then
      LBP_SSL_STATE=match
    else
      LBP_SSL_STATE=contradict
    fi
  else
    case "$LBP_SSL_RC" in
      0) LBP_SSL_STATE=empty_rc0 ;;
      1) LBP_SSL_STATE=no_match ;;
      2) LBP_SSL_STATE=file_absent ;;
      *) LBP_SSL_STATE=probe_failed ;;
    esac
  fi

  # Subject absence and probe health: any refusal is NOT_ASSESSED, never PASS.
  if [ "$LBP_BIND_STATE" = file_absent ] && [ "$LBP_SSL_STATE" = file_absent ]; then
    LBP_STATUS=PASS
    LBP_OBSERVED="/etc/security/ldap/ldap.cfg absent; no LDAP bind or SSL key password is configured"
  elif [ "$LBP_BIND_STATE" = file_absent ] || [ "$LBP_SSL_STATE" = file_absent ]; then
    LBP_STATUS=NOT_ASSESSED
    LBP_NOTES="probe evidence contradictory (one read reports ldap.cfg absent while the other read a line)"
  elif [ "$LBP_BIND_STATE" = probe_failed ] || [ "$LBP_SSL_STATE" = probe_failed ]; then
    LBP_STATUS=NOT_ASSESSED
    if [ "$LBP_BIND_STATE" = probe_failed ]; then
      LBP_NOTES="ldap_bindpwd probe failed (rc=$LBP_BIND_RC)"
    else
      LBP_NOTES="ldap_sslkeypwd probe failed (rc=$LBP_SSL_RC)"
    fi
  elif [ "$LBP_BIND_STATE" = empty_rc0 ] || [ "$LBP_SSL_STATE" = empty_rc0 ]; then
    LBP_STATUS=NOT_ASSESSED
    LBP_NOTES="probe returned rc 0 with no rows (swallowed error, not absence)"
  elif [ "$LBP_BIND_STATE" = contradict ] || [ "$LBP_SSL_STATE" = contradict ]; then
    LBP_STATUS=NOT_ASSESSED
    LBP_NOTES="probe returned rows with a failing rc"
  fi

  # Both probes healthy (match or no_match): evaluate each FAIL branch
  # independently, then OR them.
  if [ -z "$LBP_STATUS" ]; then
    LBP_BAD=0
    LBP_DETAIL=""
    if [ "$LBP_BIND_STATE" = match ]; then
      if printf '%s\n' "$LBP_BIND_RAW" | awk 'index($0, "bindpwd:{DES") == 1 { next } { bad = 1 } END { exit bad }'; then
        LBP_DETAIL="bindpwd is encrypted (starts with bindpwd:{DES)"
      else
        LBP_BAD=1
        LBP_DETAIL="bindpwd is present but not encrypted (does not start with bindpwd:{DES)"
      fi
    fi
    if [ "$LBP_SSL_STATE" = match ]; then
      LBP_BAD=1
      if [ -n "$LBP_DETAIL" ]; then
        LBP_DETAIL="$LBP_DETAIL; ldapsslkeypwd is present"
      else
        LBP_DETAIL="ldapsslkeypwd is present"
      fi
    fi
    if [ "$LBP_BAD" -eq 0 ]; then
      LBP_STATUS=PASS
      if [ -n "$LBP_DETAIL" ]; then
        LBP_OBSERVED="$LBP_DETAIL"
      else
        LBP_OBSERVED="ldap.cfg holds no bind password and no ldapsslkeypwd entry"
      fi
    else
      LBP_STATUS=FAIL
      LBP_OBSERVED="$LBP_DETAIL"
  ...
  LBP_SEVERITY=med
  case "$LBP_STATUS" in
    FAIL) LBP_SEVERITY=high ;;
  esac

  case "$LBP_STATUS" in
    PASS)
      LBP_MEANING="The LDAP client stores no unencrypted bind or SSL key password, satisfying DISA STIG V-215174."
      LBP_FIX="n/a"
      ;;
    FAIL)
      LBP_MEANING="The LDAP client stores an unencrypted bind password or an ldapsslkeypwd entry in ldap.cfg, so any reader of the file recovers the directory credential."
      LBP_FIX="Recreate the LDAP client with an encrypted bind password and a stashed SSL key database password: 'mksecldap -c -h <LDAP_HOST:LDAP_PORT> -A <auth_type> -D <Default_Entry> -d <BASE_DN> -a <BIND_USER> -p <BIND_PASSWORD> -k <KDB_FILE> -w <KDB_PASSWORD>'; then 'gsk8capicmd -keydb -stashpw -db <KDB_FILE> -pw <KDB_PASSWORD>'. PTxray only recommends these commands."
      ;;
    NOT_ASSESSED)
      LBP_OBSERVED="not assessed - $LBP_NOTES"
      LBP_MEANING="PTxray did not obtain trustworthy evidence about the LDAP client password file."
      LBP_FIX="Verify /etc/security/ldap/ldap.cfg readability and the bindpwd/ldapsslkeypwd entries as root, then rerun PTxray."
      ;;
    NOT_APPLICABLE)
      LBP_SEVERITY=low
      LBP_OBSERVED="not applicable - LDAP is not used for authentication or account information"
      LBP_MEANING="This host does not use LDAP for authentication or account information."
      LBP_FIX="n/a"
      ;;
  esac

  add security stig_ldap_bindpwd "LDAP bind/SSL key passwords in ldap.cfg" \
    "$LBP_STATUS" "$LBP_SEVERITY" "$LBP_OBSERVED" \
    "$LBP_MEANING" "$LBP_FIX" "stig:V-215174"
}
