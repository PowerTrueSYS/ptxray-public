...
AIXRAY_TOOL=ck-stig-ldap-cachetimeout


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_ldap_cachetimeout — DISA STIG V-215205 (AIX7-00-001046, CCI-002007).
  # The LDAP client must refresh user and group caches in less than a day, so
  # usercachetimeout and groupcachetimeout in /etc/security/ldap/ldap.cfg must
  # both be present, uncommented, and no greater than 900 seconds.  When LDAP
  # authentication is not required the rule is Not Applicable.  Both STIG greps
  # are probed verbatim.  AIX grep exits 2 when its single operand file cannot
  # be opened, so two rc=2 empty greps are determinate absence of the LDAP
  # client configuration -> NOT_APPLICABLE.  A grep that exits 1 with no output
  # while the other grep shows the file is readable (rc != 2) is a determinate
  # missing keyword -> FAIL.  Every other empty result (a different nonzero rc
  # or rc 0) is a refusal, never a PASS.
  typeset LDAP_LCU_RAW LDAP_LCU_RC LDAP_LCG_RAW LDAP_LCG_RC
  typeset LDAP_LCU_SHAPE LDAP_LCG_SHAPE LDAP_LCU_VAL LDAP_LCG_VAL
  typeset LDAP_LC_OVER LDAP_LC_COM
  typeset LDAP_LC_MISSING LDAP_LC_MISS_LIST
  typeset LDAP_LC_REASON LDAP_LC_STATUS LDAP_LC_SEVERITY
  typeset LDAP_LC_OBSERVED LDAP_LC_MEANING LDAP_LC_FIX
  ...
  function ldp_shape {
    ...
    printf '%s\n' "$raw" | awk '
      ...
      END {
        ...
        if (present) { if (over) print "over:" max; else print "ok:" max; exit }
        if (commented) { print "commented"; exit }
        print "absent"
      }
    '
  }

  LDAP_LC_SEVERITY=med
  LDAP_LC_OVER=0
  LDAP_LC_COM=0
  LDAP_LCU_RAW=$(aix ldap_usercachetimeout grep -i usercachetimeout /etc/security/ldap/ldap.cfg)
  LDAP_LCU_RC=$?
  LDAP_LCG_RAW=$(aix ldap_groupcachetimeout grep -i groupcachetimeout /etc/security/ldap/ldap.cfg)
  LDAP_LCG_RC=$?

  if [ -z "$LDAP_LCU_RAW" ] && [ -z "$LDAP_LCG_RAW" ] \
      && [ "$LDAP_LCU_RC" -eq 2 ] && [ "$LDAP_LCG_RC" -eq 2 ]; then
    LDAP_LC_STATUS=NOT_APPLICABLE
    LDAP_LC_SEVERITY=low
    LDAP_LC_OBSERVED="LDAP client configuration /etc/security/ldap/ldap.cfg could not be opened (both STIG greps exited rc=2 with no output); LDAP authentication is not configured and is not required"
    LDAP_LC_MEANING="No LDAP client configuration is present, so the system does not authenticate against LDAP and the requirement to refresh LDAP user and group caches does not apply."
    LDAP_LC_FIX="n/a"
  else
    # A keyword whose grep exited 1 with no output while the other grep proves
    # ldap.cfg is readable (rc != 2) is a determinate missing entry: FAIL.
    LDAP_LC_MISSING=0
    LDAP_LC_MISS_LIST=""
    if [ -z "$LDAP_LCU_RAW" ] && [ "$LDAP_LCU_RC" -eq 1 ] && [ "$LDAP_LCG_RC" -ne 2 ]; then
      LDAP_LC_MISSING=1
      LDAP_LC_MISS_LIST="usercachetimeout"
    fi
    if [ -z "$LDAP_LCG_RAW" ] && [ "$LDAP_LCG_RC" -eq 1 ] && [ "$LDAP_LCU_RC" -ne 2 ]; then
      LDAP_LC_MISSING=1
      if [ -n "$LDAP_LC_MISS_LIST" ]; then
        LDAP_LC_MISS_LIST="$LDAP_LC_MISS_LIST and groupcachetimeout"
      else
        LDAP_LC_MISS_LIST="groupcachetimeout"
      fi
    fi
    if [ "$LDAP_LC_MISSING" -eq 1 ]; then
      LDAP_LC_STATUS=FAIL
      LDAP_LC_OBSERVED="LDAP cache timeout keyword is absent from /etc/security/ldap/ldap.cfg: $LDAP_LC_MISS_LIST"
      LDAP_LC_MEANING="The LDAP client configuration is present but a required cache timeout keyword is missing, so the LDAP client does not refresh user and group caches within a day."
      LDAP_LC_FIX="set usercachetimeout and groupcachetimeout to 900 or less in /etc/security/ldap/ldap.cfg, then restart the LDAP client; PTxray only recommends /usr/sbin/restart-secldapclntd and never runs it."
    elif [ -z "$LDAP_LCU_RAW" ] || [ -z "$LDAP_LCG_RAW" ]; then
      LDAP_LC_STATUS=NOT_ASSESSED
      if [ -z "$LDAP_LCU_RAW" ]; then
        if [ "$LDAP_LCU_RC" -ne 0 ]; then
          LDAP_LC_REASON="usercachetimeout grep exited rc=$LDAP_LCU_RC with no output"
        else
          LDAP_LC_REASON="usercachetimeout grep exited rc 0 with no output"
        fi
      elif [ "$LDAP_LCG_RC" -ne 0 ]; then
        LDAP_LC_REASON="groupcachetimeout grep exited rc=$LDAP_LCG_RC with no output"
      else
        LDAP_LC_REASON="groupcachetimeout grep exited rc 0 with no output"
      fi
      LDAP_LC_OBSERVED="not assessed - $LDAP_LC_REASON"
      LDAP_LC_MEANING="PTxray did not obtain trustworthy LDAP cache timeout evidence, so whether the LDAP client refreshes its user and group caches within a day is unknown."
      LDAP_LC_FIX="run the two STIG greps against /etc/security/ldap/ldap.cfg, correct the capture or path problem, and rerun PTxray."
    else
      LDAP_LCU_SHAPE=$(ldp_shape "$LDAP_LCU_RAW")
      LDAP_LCG_SHAPE=$(ldp_shape "$LDAP_LCG_RAW")
      case "$LDAP_LCU_SHAPE:$LDAP_LCG_SHAPE" in
        malformed:*|*:malformed|absent:*|*:absent)
          LDAP_LC_STATUS=NOT_ASSESSED
          LDAP_LC_OBSERVED="not assessed - LDAP cache timeout value could not be parsed from ldap.cfg"
          LDAP_LC_MEANING="The LDAP cache timeout lines in ldap.cfg are not in the expected keyword:seconds form, so PTxray cannot decide the requirement."
          LDAP_LC_FIX="inspect /etc/security/ldap/ldap.cfg for a malformed cache timeout entry, correct the file or the capture, and rerun PTxray."
          ;;
        *)
          LDAP_LCU_VAL=${LDAP_LCU_SHAPE#ok:}
          [ "$LDAP_LCU_VAL" = "$LDAP_LCU_SHAPE" ] && LDAP_LCU_VAL=${LDAP_LCU_SHAPE#over:}
          LDAP_LCG_VAL=${LDAP_LCG_SHAPE#ok:}
          [ "$LDAP_LCG_VAL" = "$LDAP_LCG_SHAPE" ] && LDAP_LCG_VAL=${LDAP_LCG_SHAPE#over:}
          case "$LDAP_LCU_SHAPE" in over:*) LDAP_LC_OVER=1 ;; esac
          case "$LDAP_LCG_SHAPE" in over:*) LDAP_LC_OVER=1 ;; esac
          [ "$LDAP_LCU_SHAPE" = commented ] && LDAP_LC_COM=1
          [ "$LDAP_LCG_SHAPE" = commented ] && LDAP_LC_COM=1
          if [ "$LDAP_LC_OVER" -eq 1 ]; then
            LDAP_LC_STATUS=FAIL
            LDAP_LC_OBSERVED="LDAP cache timeout(s) exceed 900 seconds: usercachetimeout=$LDAP_LCU_VAL groupcachetimeout=$LDAP_LCG_VAL"
            LDAP_LC_MEANING="The LDAP client holds user and group cache entries longer than 900 seconds, so account changes in the directory can stay hidden for a day or more."
            LDAP_LC_FIX="set usercachetimeout and groupcachetimeout to 900 or less in /etc/security/ldap/ldap.cfg, then restart the LDAP client; PTxray only recommends /usr/sbin/restart-secldapclntd and never runs it."
          elif [ "$LDAP_LC_COM" -eq 1 ]; then
            LDAP_LC_STATUS=FAIL
            LDAP_LC_OBSERVED="LDAP cache timeout keyword is commented out in /etc/security/ldap/ldap.cfg"
            LDAP_LC_MEANING="An LDAP cache timeout keyword line exists but is commented out, so the LDAP client does not honor the required sub-day cache refresh."
            LDAP_LC_FIX="uncomment and set usercachetimeout and groupcachetimeout to 900 or less in /etc/security/ldap/ldap.cfg, then restart the LDAP client; PTxray only recommends /usr/sbin/restart-secldapclntd and never runs it."
          else
            LDAP_LC_STATUS=PASS
            LDAP_LC_OBSERVED="LDAP client refreshes user and group caches within 900 seconds (usercachetimeout=$LDAP_LCU_VAL groupcachetimeout=$LDAP_LCG_VAL)"
            LDAP_LC_MEANING="Both LDAP cache timeout values are 900 seconds or less, so the LDAP client refreshes user and group caches in less than a day."
            LDAP_LC_FIX="n/a"
          fi
          ;;
      esac
  ...
  add security stig_ldap_cachetimeout "LDAP client user and group cache refresh" \
    "$LDAP_LC_STATUS" "$LDAP_LC_SEVERITY" "$LDAP_LC_OBSERVED" \
    "$LDAP_LC_MEANING" "$LDAP_LC_FIX" "stig:V-215205"
}
