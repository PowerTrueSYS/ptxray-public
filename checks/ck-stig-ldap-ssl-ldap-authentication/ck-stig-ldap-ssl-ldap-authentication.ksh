...
AIXRAY_TOOL=ck-stig-ldap-ssl-ldap-authentication


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_ldap_ssl_ldap_authentication — DISA STIG V-215214 (medium).
  # When LDAP authentication is required (authtype ldap_auth in
  # /etc/security/ldap/ldap.cfg), SSL must be used between LDAP clients and
  # their LDAP servers (useSSL yes) to protect remote access session integrity.
  # Determinate absence of the authtype line (grep no-match: rc 1, empty)
  # renders NOT_APPLICABLE per the rule text; every uncertain evidence shape
  # fails closed to NOT_ASSESSED. grep exits 0 only when it printed a match
  # and 1 with empty output for a readable file with no matching line, so
  # rc 1 + empty is subject absence while rc 0 + empty is a swallowed error,
  # never a PASS.
  typeset LDA_AUTH_RAW LDA_AUTH_RC LDA_USESSL_RAW LDA_USESSL_RC
  typeset LDA_STATUS LDA_SEV LDA_OBSERVED LDA_MEANING LDA_FIX

  LDA_AUTH_RAW=$(aix ldap_authtype grep -iE "^authtype:[[:blank:]]*ldap_auth" /etc/security/ldap/ldap.cfg)
  LDA_AUTH_RC=$?
  LDA_USESSL_RAW=$(aix ldap_usessl grep -iE "^useSSL:[[:blank:]]*yes" /etc/security/ldap/ldap.cfg)
  LDA_USESSL_RC=$?

  if [ "$LDA_AUTH_RC" -eq 1 ] && [ -z "$LDA_AUTH_RAW" ]; then
    LDA_STATUS=NOT_APPLICABLE
    LDA_OBSERVED="no 'authtype: ldap_auth' line in /etc/security/ldap/ldap.cfg; LDAP authentication not required"
  elif [ "$LDA_AUTH_RC" -ne 0 ]; then
    LDA_STATUS=NOT_ASSESSED
    LDA_OBSERVED="not assessed - LDAP authtype probe failed (rc=$LDA_AUTH_RC), subject presence unproven"
  elif [ -z "$LDA_AUTH_RAW" ]; then
    LDA_STATUS=NOT_ASSESSED
    LDA_OBSERVED="not assessed - LDAP authtype probe returned rc 0 with no output (swallowed error)"
  elif [ "$LDA_USESSL_RC" -eq 1 ] && [ -z "$LDA_USESSL_RAW" ]; then
    LDA_STATUS=FAIL
    LDA_OBSERVED="authtype: ldap_auth is set but 'useSSL: yes' is absent from /etc/security/ldap/ldap.cfg"
  elif [ "$LDA_USESSL_RC" -ne 0 ]; then
    LDA_STATUS=NOT_ASSESSED
    LDA_OBSERVED="not assessed - LDAP useSSL probe failed (rc=$LDA_USESSL_RC)"
  elif [ -z "$LDA_USESSL_RAW" ]; then
    LDA_STATUS=NOT_ASSESSED
    LDA_OBSERVED="not assessed - LDAP useSSL probe returned rc 0 with no output (swallowed error)"
  else
    LDA_STATUS=PASS
    LDA_OBSERVED="authtype: ldap_auth and 'useSSL: yes' both present in /etc/security/ldap/ldap.cfg"
  fi

  if [ "$LDA_STATUS" = "NOT_APPLICABLE" ]; then
    LDA_SEV=low
  else
    LDA_SEV=med
  fi
  ...
  case "$LDA_STATUS" in
    PASS)
      LDA_MEANING="LDAP authentication requires and uses SSL between LDAP clients and LDAP servers, protecting remote access session integrity."
      LDA_FIX="n/a"
      ;;
    FAIL)
      LDA_MEANING="LDAP authentication is enabled without SSL, so authentication exchanges can be observed or altered on the wire."
      LDA_FIX="enable SSL for the LDAP client daemon (secldapclntd) by setting 'useSSL: yes' in /etc/security/ldap/ldap.cfg and restarting secldapclntd; PTxray only recommends these actions."
      ;;
    NOT_APPLICABLE)
      LDA_MEANING="LDAP authentication is not required on this host, so no LDAP client SSL setting must be enforced."
      LDA_FIX="n/a"
      ;;
    *)
      LDA_MEANING="PTxray did not obtain trustworthy LDAP configuration evidence for the LDAP client SSL requirement."
      LDA_FIX="confirm /etc/security/ldap/ldap.cfg exists and is readable, then rerun PTxray."
      ;;
  esac

  add security stig_ldap_ssl_ldap_authentication "LDAP client SSL enforcement" \
    "$LDA_STATUS" "$LDA_SEV" "$LDA_OBSERVED" \
    "$LDA_MEANING" "$LDA_FIX" "stig:V-215214"
}
