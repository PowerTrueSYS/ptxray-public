...
AIXRAY_TOOL=ck-stig-ldap-ssl-ldap-used


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_ldap_ssl_ldap_used — DISA STIG V-215204.
  # When the AIX LDAP client is used for authentication (authtype:ldap_auth) it
  # must talk to the LDAP server over SSL.  authtype:ldap_auth without
  # useSSL:yes is a finding; any other authtype means the LDAP client is not
  # used for authentication, so the rule does not apply (NOT_APPLICABLE).  An
  # absent /etc/security/ldap/ldap.cfg likewise means the LDAP client is not
  # used, so the rule does not apply.  Probe failures and rc0-with-no-output
  # (a swallowed error) refuse with NOT_ASSESSED; refusal never passes.
  typeset LSLU_AUTO_RAW LSLU_AUTO_RC LSLU_AUTO_STATE
  typeset LSLU_SSL_RAW LSLU_SSL_RC LSLU_SSL_STATE
  typeset LSLU_STATUS LSLU_SEVERITY LSLU_OBSERVED LSLU_MEANING LSLU_FIX

  LSLU_AUTO_RAW=$(aix ldap_cfg_authtype grep -iE "^authtype:[[:blank:]]*ldap_auth" /etc/security/ldap/ldap.cfg)
  LSLU_AUTO_RC=$?
  LSLU_SSL_RAW=$(aix ldap_cfg_usessl grep -iE "^useSSL:[[:blank:]]*yes" /etc/security/ldap/ldap.cfg)
  LSLU_SSL_RC=$?

  LSLU_AUTO_STATE=error
  if [ "$LSLU_AUTO_RC" -eq 0 ] && [ -n "$LSLU_AUTO_RAW" ]; then
    LSLU_AUTO_STATE=found
  elif [ "$LSLU_AUTO_RC" -eq 1 ] && [ -z "$LSLU_AUTO_RAW" ]; then
    LSLU_AUTO_STATE=notfound
  elif [ "$LSLU_AUTO_RC" -eq 2 ] && [ -z "$LSLU_AUTO_RAW" ]; then
    LSLU_AUTO_STATE=missing
  elif [ "$LSLU_AUTO_RC" -eq 0 ] && [ -z "$LSLU_AUTO_RAW" ]; then
    LSLU_AUTO_STATE=empty
  fi

  LSLU_SSL_STATE=error
  if [ "$LSLU_SSL_RC" -eq 0 ] && [ -n "$LSLU_SSL_RAW" ]; then
    LSLU_SSL_STATE=found
  elif [ "$LSLU_SSL_RC" -eq 1 ] && [ -z "$LSLU_SSL_RAW" ]; then
    LSLU_SSL_STATE=notfound
  elif [ "$LSLU_SSL_RC" -eq 2 ] && [ -z "$LSLU_SSL_RAW" ]; then
    LSLU_SSL_STATE=missing
  elif [ "$LSLU_SSL_RC" -eq 0 ] && [ -z "$LSLU_SSL_RAW" ]; then
    LSLU_SSL_STATE=empty
  fi

  case "$LSLU_AUTO_STATE" in
    found)
      case "$LSLU_SSL_STATE" in
        found)
          LSLU_STATUS=PASS
          LSLU_OBSERVED="authtype:ldap_auth and useSSL:yes are both present in /etc/security/ldap/ldap.cfg"
          LSLU_MEANING="The AIX LDAP client is configured to use ldap_auth and requires the LDAP server connection to use SSL, satisfying DISA STIG V-215204."
          LSLU_FIX="n/a"
          ;;
        notfound)
          LSLU_STATUS=FAIL
          LSLU_OBSERVED="authtype:ldap_auth is set but useSSL:yes is not present in /etc/security/ldap/ldap.cfg"
          LSLU_MEANING="The AIX LDAP client authenticates to the LDAP server without requiring SSL, exposing credentials and directory data to the network in cleartext."
          LSLU_FIX="set useSSL:yes in /etc/security/ldap/ldap.cfg and restart the LDAP client daemon (restart-secldapclntd); PTxray only recommends this command."
          ;;
        empty)
          LSLU_STATUS=NOT_ASSESSED
          LSLU_OBSERVED="not assessed - the useSSL probe returned no output with rc 0, a swallowed error rather than a missing subject"
          LSLU_MEANING="PTxray did not obtain trustworthy evidence of the LDAP useSSL setting."
          LSLU_FIX="verify the LDAP client SSL setting as root and rerun PTxray."
          ;;
        missing)
          LSLU_STATUS=NOT_ASSESSED
          LSLU_OBSERVED="not assessed - the useSSL probe reported /etc/security/ldap/ldap.cfg unreadable (rc=2), so the SSL setting could not be verified"
          LSLU_MEANING="PTxray did not obtain trustworthy evidence of the LDAP useSSL setting."
          LSLU_FIX="verify the LDAP client SSL setting as root and rerun PTxray."
          ;;
        *)
          LSLU_STATUS=NOT_ASSESSED
          LSLU_OBSERVED="not assessed - the useSSL probe failed (rc=$LSLU_SSL_RC)"
          LSLU_MEANING="PTxray did not obtain trustworthy evidence of the LDAP useSSL setting."
          LSLU_FIX="verify the LDAP client SSL setting as root and rerun PTxray."
          ;;
      esac
      ;;
    notfound)
      LSLU_STATUS=NOT_APPLICABLE
      LSLU_OBSERVED="authtype is not ldap_auth in /etc/security/ldap/ldap.cfg; the LDAP client is not used for authentication"
      LSLU_MEANING="DISA STIG V-215204 applies only when the AIX LDAP client performs authentication, so it does not apply to this configuration."
      LSLU_FIX="n/a"
      ;;
    missing)
      LSLU_STATUS=NOT_APPLICABLE
      LSLU_OBSERVED="/etc/security/ldap/ldap.cfg is absent, so the AIX LDAP client is not used for authentication"
      LSLU_MEANING="DISA STIG V-215204 applies only when the AIX LDAP client performs authentication against an LDAP server; an absent ldap.cfg means LDAP is not used, so the rule does not apply."
      LSLU_FIX="n/a"
      ;;
    empty)
      LSLU_STATUS=NOT_ASSESSED
      LSLU_OBSERVED="not assessed - the authtype probe returned no output with rc 0, a swallowed error rather than a missing subject"
      LSLU_MEANING="PTxray did not obtain trustworthy evidence of the LDAP authtype setting."
      LSLU_FIX="verify the LDAP client configuration as root and rerun PTxray."
      ;;
    *)
      LSLU_STATUS=NOT_ASSESSED
      LSLU_OBSERVED="not assessed - the authtype probe failed (rc=$LSLU_AUTO_RC)"
      LSLU_MEANING="PTxray did not obtain trustworthy evidence of the LDAP authtype setting."
      LSLU_FIX="verify the LDAP client configuration as root and rerun PTxray."
      ;;
  esac
  ...
  case "$LSLU_STATUS" in
    PASS) LSLU_SEVERITY=med ;;
    FAIL) LSLU_SEVERITY=high ;;
    NOT_APPLICABLE) LSLU_SEVERITY=low ;;
    *) LSLU_SEVERITY=med ;;
  esac

  add security stig_ldap_ssl_ldap_used "LDAP client uses SSL for LDAP authentication" \
    "$LSLU_STATUS" "$LSLU_SEVERITY" "$LSLU_OBSERVED" \
    "$LSLU_MEANING" "$LSLU_FIX" "stig:V-215204"
}
