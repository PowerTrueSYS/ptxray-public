...
AIXRAY_TOOL=ck-stig-ldap-ssl-only-allow


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_ldap_ssl_only_allow — DISA STIG V-215215.
  # PARTIAL-AUTO: LDAP / useSSL / KDB-path are decidable; KDB password and
  # DoD PKI membership need ISSO/SA evidence. Issuer DNs are not proof.
  ...
  typeset LSO_USER_RAW LSO_USER_RC LSO_SSL_RAW LSO_SSL_RC LSO_SSL_VAL
  typeset LSO_KEYF_RAW LSO_KEYF_RC LSO_KDB LSO_PARSE_RC
  typeset LSO_KDB_LS LSO_KDB_LS_RC LSO_LIST_RAW LSO_LIST_RC
  typeset LSO_LABELS LSO_N LSO_LAB LSO_ONE LSO_ONE_RC LSO_FP LSO_FPS
  typeset LSO_ANCHOR_RAW LSO_ANCHOR_RC LSO_DOD
  typeset LSO_STATUS LSO_SEV LSO_OBSERVED LSO_MEANING LSO_FIX
  typeset LSO_MANUAL

  LSO_MANUAL="not assessed - Key Database password and DoD PKI review require ISSO/SA identification of the KDB password and approved DoD PKI trust-anchor fingerprints"
  LSO_STATUS=""
  LSO_OBSERVED=""

  LSO_USER_RAW=$(aix ldap_user grep LDAP /etc/security/user)
  LSO_USER_RC=$?

  if [ "$LSO_USER_RC" -eq 1 ] && [ -z "$LSO_USER_RAW" ]; then
    LSO_STATUS=NOT_APPLICABLE
    LSO_OBSERVED="LDAP is not in use (no LDAP lines in /etc/security/user)"
  elif [ "$LSO_USER_RC" -ne 0 ]; then
    LSO_STATUS=NOT_ASSESSED
    LSO_OBSERVED="not assessed - LDAP user probe failed (rc=$LSO_USER_RC)"
  elif [ -z "$LSO_USER_RAW" ]; then
    LSO_STATUS=NOT_ASSESSED
    LSO_OBSERVED="not assessed - LDAP user probe returned no output"
  else
    LSO_SSL_RAW=$(aix ldap_usessl grep '^useSSL' /etc/security/ldap/ldap.cfg)
    LSO_SSL_RC=$?
    if [ "$LSO_SSL_RC" -ne 0 ] && [ "$LSO_SSL_RC" -ne 1 ]; then
      LSO_STATUS=NOT_ASSESSED
      LSO_OBSERVED="not assessed - useSSL probe failed (rc=$LSO_SSL_RC)"
    elif [ "$LSO_SSL_RC" -eq 0 ] && [ -z "$LSO_SSL_RAW" ]; then
      LSO_STATUS=NOT_ASSESSED
      LSO_OBSERVED="not assessed - useSSL probe returned no output"
    else
      LSO_SSL_VAL=$(printf '%s\n' "$LSO_SSL_RAW" | awk '
        /^[ \t]*$/ { next }
        {
          line = $0
          c = index(line, ":")
          if (c == 0) { print "malformed"; exit }
          val = substr(line, c + 1)
          gsub(/[ \t]/, "", val)
          print tolower(val)
          exit
      ...
      ')
      if [ "$LSO_SSL_RC" -eq 1 ] || [ -z "$LSO_SSL_VAL" ] || [ "$LSO_SSL_VAL" = malformed ]; then
        LSO_STATUS=FAIL
        LSO_OBSERVED="useSSL is not yes"
      elif [ "$LSO_SSL_VAL" != yes ]; then
        LSO_STATUS=FAIL
        LSO_OBSERVED="useSSL is not yes (useSSL=$LSO_SSL_VAL)"
      else
        LSO_KEYF_RAW=$(aix ldap_sslkeyf grep -i '^ldapsslkeyf' /etc/security/ldap/ldap.cfg)
        LSO_KEYF_RC=$?
        if [ "$LSO_KEYF_RC" -ne 0 ] && [ "$LSO_KEYF_RC" -ne 1 ]; then
          LSO_STATUS=NOT_ASSESSED
          LSO_OBSERVED="not assessed - ldapsslkeyf probe failed (rc=$LSO_KEYF_RC)"
        elif [ "$LSO_KEYF_RC" -eq 0 ] && [ -z "$LSO_KEYF_RAW" ]; then
          LSO_STATUS=NOT_ASSESSED
          LSO_OBSERVED="not assessed - ldapsslkeyf probe returned no output"
        elif [ "$LSO_KEYF_RC" -eq 1 ] && [ -z "$LSO_KEYF_RAW" ]; then
          LSO_STATUS=FAIL
          LSO_OBSERVED="ldapsslkeyf/KDB path is missing"
        else
          LSO_KDB=$(printf '%s\n' "$LSO_KEYF_RAW" | awk '
            /^[ \t]*$/ { next }
            {
          ...
          ')
          LSO_PARSE_RC=$?
          if [ "$LSO_PARSE_RC" -ne 0 ] || [ -z "$LSO_KDB" ]; then
            LSO_STATUS=FAIL
            LSO_OBSERVED="ldapsslkeyf/KDB path is missing"
          else
            LSO_KDB_LS=$(aix kdb_ls ls -ld "$LSO_KDB")
            LSO_KDB_LS_RC=$?
            case "$LSO_KDB_LS" in
              *"does not exist"*|*"No such file"*)
                LSO_STATUS=FAIL
                LSO_OBSERVED="Key Database file is missing ($LSO_KDB)"
                ;;
            esac
            if [ -z "$LSO_STATUS" ]; then
              if [ "$LSO_KDB_LS_RC" -ne 0 ]; then
                LSO_STATUS=NOT_ASSESSED
                LSO_OBSERVED="not assessed - KDB ls probe failed (rc=$LSO_KDB_LS_RC)"
              elif [ -z "$LSO_KDB_LS" ]; then
                LSO_STATUS=NOT_ASSESSED
                LSO_OBSERVED="not assessed - KDB ls probe returned no output"
              else
                LSO_LIST_RAW=$(aix gsk_cert_list gsk8capicmd -cert -list CA -db "$LSO_KDB" -stashed)
                LSO_LIST_RC=$?
                if [ "$LSO_LIST_RC" -ne 0 ] || [ -z "$LSO_LIST_RAW" ]; then
                  LSO_STATUS=NOT_ASSESSED
                  LSO_OBSERVED="$LSO_MANUAL"
                else
                  LSO_LABELS=$(printf '%s\n' "$LSO_LIST_RAW" | awk '
                    {
                      q1 = index($0, "\"")
                  ...
                  ')
                  LSO_N=$(printf '%s\n' "$LSO_LABELS" | awk 'NF { n++ } END { print n+0 }')
                  if [ "$LSO_N" -eq 0 ]; then
                    LSO_STATUS=NOT_ASSESSED
                    LSO_OBSERVED="not assessed - gsk CA list was unparseable"
                  elif [ "$LSO_N" -gt 2 ]; then
                    LSO_STATUS=FAIL
                    LSO_OBSERVED="more certificates than the LDAP server and the client ($LSO_N in KDB)"
                  else
                    LSO_FPS=""
                    while IFS= read LSO_LAB
                    do
                      [ -n "$LSO_LAB" ] || continue
                      LSO_ONE=$(aix gsk_cert_details gsk8capicmd -cert -details -showOID -db "$LSO_KDB" -stashed -label "$LSO_LAB")
                      LSO_ONE_RC=$?
                      if [ "$LSO_ONE_RC" -ne 0 ] || [ -z "$LSO_ONE" ]; then
                        LSO_STATUS=NOT_ASSESSED
                        LSO_OBSERVED="not assessed - certificate details probe failed for label $LSO_LAB (rc=$LSO_ONE_RC)"
                        break
                      fi
                      LSO_FP=$(printf '%s\n' "$LSO_ONE" | awk -v want="$LSO_LAB" '
                        {
                          line = $0
                          low = tolower(line)
                          if (low ~ /^[ \t]*label[ \t]*:/) {
                        ...
                        END {
                          if (nlab != 1 || nmatch != 1 || nfp != 1 || lastfp == "") exit 1
                          print lastfp
                        }
                      ')
                      LSO_PARSE_RC=$?
                      if [ "$LSO_PARSE_RC" -ne 0 ] || [ -z "$LSO_FP" ]; then
                        LSO_STATUS=NOT_ASSESSED
                        LSO_OBSERVED="not assessed - certificate details were incomplete for label $LSO_LAB"
                        break
                      fi
                      LSO_FPS="$LSO_FPS$LSO_FP|"
                    done <<EOF
$LSO_LABELS
EOF
                    if [ -z "$LSO_STATUS" ]; then
                      LSO_ANCHOR_RAW=$(aix dod_pki_anchors cat /etc/security/ldap/dod-pki-anchors)
                      LSO_ANCHOR_RC=$?
                      if [ "$LSO_ANCHOR_RC" -ne 0 ] || [ -z "$LSO_ANCHOR_RAW" ]; then
                        LSO_STATUS=NOT_ASSESSED
                        LSO_OBSERVED="$LSO_MANUAL"
                      else
                        LSO_DOD=$(printf '%s\n' "$LSO_ANCHOR_RAW" | awk -v fps="$LSO_FPS" '
                          BEGIN {
                            n = split(fps, a, "|")
                            for (i = 1; i <= n; i++) {
                              gsub(/[^0-9A-Fa-f]/, "", a[i])
                          ...
                          END {
                            if (need == 0) { print "unparseable"; exit }
                            matched = 0
                            for (i = 1; i <= n; i++) {
                              if (a[i] == "") continue
                              if (!(a[i] in ok)) { print "not_dod"; exit }
                              matched++
                            }
                            if (matched == need) print "dod"
                            else print "unparseable"
                          }
                        ')
                        if [ -z "$LSO_DOD" ] || [ "$LSO_DOD" = unparseable ]; then
                          LSO_STATUS=NOT_ASSESSED
                          LSO_OBSERVED="$LSO_MANUAL"
                        elif [ "$LSO_DOD" != dod ]; then
                          LSO_STATUS=FAIL
                          LSO_OBSERVED="certificate is not issued by DoD PKI"
                        else
                          LSO_STATUS=PASS
                          LSO_OBSERVED="LDAP in use, useSSL=yes, KDB=$LSO_KDB, $LSO_N CA certificates match approved DoD PKI trust-anchor fingerprints"
                        fi
                      fi
                    fi
                  fi
  ...
  case "$LSO_STATUS" in
    NOT_APPLICABLE) LSO_SEV=low ;;
    *) LSO_SEV=med ;;
  esac

  case "$LSO_STATUS" in
    PASS)
      LSO_MEANING="LDAP SSL uses a Key Database whose CA certificates are DoD PKI-established."
      LSO_FIX="n/a"
      ;;
    FAIL)
      LSO_MEANING="LDAP SSL is not restricted to DoD PKI-established certificate authorities."
      LSO_FIX="gsk8capicmd -keydb -create -db <KDB_FILE> -pw <KDB_PASSWORD> -type cms -stash; gsk8capicmd -cert -add -db <KDB_FILE> -pw <KDB_PASSWORD> -file <CERT_FILE> -label <CERT_LABEL>; gsk8capicmd -cert -delete -db <KDB_FILE> -pw <KDB_PASSWORD> -label <CERT_LABEL>; /usr/sbin/restart-secldapclntd; PTxray only recommends these commands."
      ;;
    NOT_APPLICABLE)
      LSO_MEANING="LDAP is not in use, so the DoD PKI session-verification requirement does not apply."
      LSO_FIX="n/a"
      ;;
    *)
      LSO_MEANING="PTxray could not finish the DoD PKI CA review for LDAP SSL."
      LSO_FIX="Identify the Key Database and its password with the ISSO/SA, supply approved DoD PKI trust-anchor fingerprints, review CA certificates, then rerun PTxray."
      ;;
  esac

  add security stig_ldap_ssl_only_allow "LDAP SSL DoD PKI CAs only" \
    "$LSO_STATUS" "$LSO_SEV" "$LSO_OBSERVED" "$LSO_MEANING" "$LSO_FIX" "stig:V-215215"
}
