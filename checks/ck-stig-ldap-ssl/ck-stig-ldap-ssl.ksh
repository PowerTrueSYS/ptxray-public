...
AIXRAY_TOOL=ck-stig-ldap-ssl


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_ldap_ssl — DISA STIG V-215173.  If AIX uses LDAP for authentication or
  # account information, the LDAP SSL/TLS connection must require the server to
  # provide a certificate and that certificate must have a valid path to a
  # trusted CA.  PARTIAL-AUTO: the decidable sub-condition is LDAP-usage
  # detection (grep LDAP /etc/security/user), useSSL=yes, and a configured
  # ldapsslkeyf key database.  Inspecting the key database to confirm the server
  # certificate's path to a trusted CA requires the KDB password and gsk8capicmd,
  # which is the site/ISSO manual portion: when the decidable evidence cannot
  # identify a single key database the check renders a typed NOT_ASSESSED naming
  # that portion, never a silent PASS.
  typeset LDP_USER_RAW LDP_USER_RC LDP_USER_MATCH_RC LDP_USER_STATE
  typeset LDP_SSL_RAW LDP_SSL_RC LDP_SSL_MATCH_RC LDP_SSL_STATE
  typeset LDP_KEYF_RAW LDP_KEYF_RC LDP_KEYF_MATCH_RC LDP_KEYF_STATE
  typeset LDP_STATUS LDP_SEVERITY LDP_OBSERVED LDP_MEANING LDP_FIX
  ...
  # --- LDAP usage probe: cat the producer; keep lines containing LDAP ---
  LDP_USER_RAW=$(aix ldap_user /usr/bin/cat /etc/security/user)
  LDP_USER_RC=$?
  # The probe rc is kept as captured; the filter result is tracked separately so
  # a no-match never overwrites the producer's own exit status.
  LDP_USER_MATCH_RC=$LDP_USER_RC
  if [ "$LDP_USER_RC" -eq 0 ]; then
    LDP_USER_RAW=$(printf '%s\n' "$LDP_USER_RAW" | awk 'index($0, "LDAP") { print }')
    if [ -z "$LDP_USER_RAW" ]; then
      LDP_USER_MATCH_RC=1
    fi
  fi
  if [ -z "$LDP_USER_RAW" ]; then
    case "$LDP_USER_MATCH_RC" in
      0) LDP_USER_STATE=swallowed ;;
      1) LDP_USER_STATE=absent ;;
      *) LDP_USER_STATE=probe-failed ;;
    esac
  elif [ "$LDP_USER_MATCH_RC" -ne 0 ]; then
    LDP_USER_STATE=probe-failed
  else
    LDP_USER_STATE=used
  fi

  # --- useSSL probe (only reached when LDAP is used) ---
  if [ "$LDP_USER_STATE" = "used" ]; then
    LDP_SSL_RAW=$(aix use_ssl /usr/bin/cat /etc/security/ldap/ldap.cfg)
    LDP_SSL_RC=$?
    LDP_SSL_MATCH_RC=$LDP_SSL_RC
    if [ "$LDP_SSL_RC" -eq 0 ]; then
      LDP_SSL_RAW=$(printf '%s\n' "$LDP_SSL_RAW" | awk '/^useSSL/ { print }')
      if [ -z "$LDP_SSL_RAW" ]; then
        LDP_SSL_MATCH_RC=1
      fi
    fi
    if [ -z "$LDP_SSL_RAW" ]; then
      case "$LDP_SSL_MATCH_RC" in
        0) LDP_SSL_STATE=swallowed ;;
        1) LDP_SSL_STATE=absent ;;
        *) LDP_SSL_STATE=probe-failed ;;
      esac
    elif [ "$LDP_SSL_MATCH_RC" -ne 0 ]; then
      LDP_SSL_STATE=probe-failed
    else
      LDP_SSL_STATE=$(printf '%s\n' "$LDP_SSL_RAW" | awk '
        /^[ \t]*$/ { next }
        {
          n++
          line = $0
          ...
          gsub(/[ \t]/, "", val)
          if (val == "") { bad = 1; next }
          if (tolower(val) == "yes") { yes = 1 } else { notyes = 1 }
        }
        END {
          if (n == 0) { print "empty"; exit }
          if (n > 1) { print "ambiguous"; exit }
          if (bad) { print "not-yes"; exit }
  ...
  # --- ldapsslkeyf probe (only reached when useSSL is yes) ---
  if [ "$LDP_SSL_STATE" = "yes" ]; then
    LDP_KEYF_RAW=$(aix ldap_ssl_keyf /usr/bin/cat /etc/security/ldap/ldap.cfg)
    LDP_KEYF_RC=$?
    LDP_KEYF_MATCH_RC=$LDP_KEYF_RC
    if [ "$LDP_KEYF_RC" -eq 0 ]; then
      LDP_KEYF_RAW=$(printf '%s\n' "$LDP_KEYF_RAW" | awk 'tolower($0) ~ /^ldapsslkeyf/ { print }')
      if [ -z "$LDP_KEYF_RAW" ]; then
        LDP_KEYF_MATCH_RC=1
      fi
    fi
    if [ -z "$LDP_KEYF_RAW" ]; then
      case "$LDP_KEYF_MATCH_RC" in
        0) LDP_KEYF_STATE=swallowed ;;
        1) LDP_KEYF_STATE=absent ;;
        *) LDP_KEYF_STATE=probe-failed ;;
      esac
    elif [ "$LDP_KEYF_MATCH_RC" -ne 0 ]; then
      LDP_KEYF_STATE=probe-failed
    else
      LDP_KEYF_STATE=$(printf '%s\n' "$LDP_KEYF_RAW" | awk '
        /^[ \t]*$/ { next }
        {
          n++
          line = $0
  ...
  # --- verdict chain ---
  if [ "$LDP_USER_STATE" = "absent" ]; then
    LDP_STATUS=NOT_APPLICABLE
    LDP_OBSERVED="LDAP is not used for authentication or account information on this host; per rule text ('If LDAP authentication is not used on AIX, this is Not Applicable') this is Not Applicable"
    LDP_MEANING="No LDAP registry or LDAP account-information reference appears in /etc/security/user; per the rule text ('If LDAP authentication is not used on AIX, this is Not Applicable') this is Not Applicable. Confirm the LDAP-usage decision with the site ISSO."
    LDP_FIX="n/a"
  elif [ "$LDP_USER_STATE" = "swallowed" ]; then
    LDP_STATUS=NOT_ASSESSED
    LDP_OBSERVED="not assessed - grep LDAP /etc/security/user returned rc 0 with no output; rc0+empty is a swallowed error, not LDAP absence"
    LDP_MEANING="PTxray did not obtain trustworthy evidence for whether LDAP is used on this host, so applicability cannot be decided."
    LDP_FIX="verify that /etc/security/user is readable and 'grep LDAP /etc/security/user' behaves, then rerun PTxray."
  elif [ "$LDP_USER_STATE" = "probe-failed" ]; then
    LDP_STATUS=NOT_ASSESSED
    LDP_OBSERVED="not assessed - grep LDAP /etc/security/user failed (rc=$LDP_USER_RC); no trustworthy LDAP-usage evidence"
    LDP_MEANING="PTxray did not obtain trustworthy evidence for whether LDAP is used on this host, so applicability cannot be decided."
    LDP_FIX="verify that /etc/security/user is readable and 'grep LDAP /etc/security/user' behaves, then rerun PTxray."
  elif [ "$LDP_SSL_STATE" = "swallowed" ]; then
    LDP_STATUS=NOT_ASSESSED
    LDP_OBSERVED="not assessed - grep '^useSSL' /etc/security/ldap/ldap.cfg returned rc 0 with no output; rc0+empty is a swallowed error, not a missing SSL setting"
    LDP_MEANING="PTxray did not obtain trustworthy evidence for the LDAP SSL/TLS configuration."
    LDP_FIX="verify that /etc/security/ldap/ldap.cfg is readable and the useSSL grep behaves, then rerun PTxray."
  elif [ "$LDP_SSL_STATE" = "probe-failed" ]; then
    LDP_STATUS=NOT_ASSESSED
    LDP_OBSERVED="not assessed - grep '^useSSL' /etc/security/ldap/ldap.cfg failed (rc=$LDP_SSL_RC); no trustworthy SSL evidence"
    LDP_MEANING="PTxray did not obtain trustworthy evidence for the LDAP SSL/TLS configuration."
    LDP_FIX="verify that /etc/security/ldap/ldap.cfg is readable and the useSSL grep behaves, then rerun PTxray."
  elif [ "$LDP_SSL_STATE" = "ambiguous" ]; then
    LDP_STATUS=NOT_ASSESSED
    LDP_OBSERVED="not assessed - more than one useSSL setting in /etc/security/ldap/ldap.cfg; the effective LDAP SSL/TLS setting is ambiguous"
    LDP_MEANING="PTxray cannot determine which useSSL value takes effect, so it refuses to guess the LDAP SSL/TLS state."
    LDP_FIX="reduce /etc/security/ldap/ldap.cfg to one useSSL setting, then rerun PTxray."
  elif [ "$LDP_SSL_STATE" = "absent" ] || [ "$LDP_SSL_STATE" = "not-yes" ]; then
    LDP_STATUS=FAIL
    LDP_OBSERVED="useSSL in /etc/security/ldap/ldap.cfg is not 'yes'; LDAP SSL/TLS is not enabled (DISA STIG V-215173 finding)"
    LDP_MEANING="The LDAP client is not configured to require a server certificate over SSL/TLS, so the connection does not require a certificate with a valid path to a trusted CA."
    LDP_FIX="gsk8capicmd -keydb -create -db <KDB_FILE> -pw <KDB_PASSWORD> -type cms -stash; gsk8capicmd -cert -add -db <KDB_FILE> -pw <KDB_PASSWORD> -file <CERT_FILE> -label <CERT_LABEL>; gsk8capicmd -cert -delete -db <KDB_FILE> -pw <KDB_PASSWORD> -label <CERT_LABEL>; /usr/sbin/restart-secldapclntd (PTxray only recommends these commands)"
  elif [ "$LDP_KEYF_STATE" = "swallowed" ]; then
    LDP_STATUS=NOT_ASSESSED
    LDP_OBSERVED="not assessed - grep -i '^ldapsslkeyf' /etc/security/ldap/ldap.cfg returned rc 0 with no output; rc0+empty is a swallowed error, not a missing key database"
    LDP_MEANING="PTxray did not obtain trustworthy evidence for the LDAP client key database."
    LDP_FIX="verify that /etc/security/ldap/ldap.cfg is readable and the ldapsslkeyf grep behaves, then rerun PTxray."
  elif [ "$LDP_KEYF_STATE" = "probe-failed" ]; then
    LDP_STATUS=NOT_ASSESSED
    LDP_OBSERVED="not assessed - grep -i '^ldapsslkeyf' /etc/security/ldap/ldap.cfg failed (rc=$LDP_KEYF_RC); no trustworthy key database evidence"
    LDP_MEANING="PTxray did not obtain trustworthy evidence for the LDAP client key database."
    LDP_FIX="verify that /etc/security/ldap/ldap.cfg is readable and the ldapsslkeyf grep behaves, then rerun PTxray."
  elif [ "$LDP_KEYF_STATE" = "ambiguous" ]; then
    LDP_STATUS=NOT_ASSESSED
    LDP_OBSERVED="not assessed - multiple ldapsslkeyf entries; the LDAP client key database cannot be uniquely identified. Confirming that the server certificate has a valid path to a trusted CA requires the KDB password and gsk8capicmd; identify the Key Database (KDB) and its password by asking the ISSO/SA"
    LDP_MEANING="The decidable LDAP SSL/TLS configuration is present but the key database cannot be uniquely identified, and the certificate-path confirmation is the site/ISSO manual portion; PTxray refuses a silent PASS."
    LDP_FIX="identify the Key Database (KDB) and its password by asking the ISSO/SA, inspect the certificate with gsk8capicmd, and rerun PTxray."
  elif [ "$LDP_KEYF_STATE" = "absent" ] || [ "$LDP_KEYF_STATE" = "empty-value" ]; then
    LDP_STATUS=FAIL
    LDP_OBSERVED="no usable ldapsslkeyf entry in /etc/security/ldap/ldap.cfg; the LDAP client key database is not configured (DISA STIG V-215173 finding)"
    LDP_MEANING="With LDAP SSL/TLS enabled but no key database configured, the LDAP client has no certificate store with which to require and validate a server certificate."
    LDP_FIX="gsk8capicmd -keydb -create -db <KDB_FILE> -pw <KDB_PASSWORD> -type cms -stash; gsk8capicmd -cert -add -db <KDB_FILE> -pw <KDB_PASSWORD> -file <CERT_FILE> -label <CERT_LABEL>; gsk8capicmd -cert -delete -db <KDB_FILE> -pw <KDB_PASSWORD> -label <CERT_LABEL>; /usr/sbin/restart-secldapclntd (PTxray only recommends these commands)"
  else
    LDP_STATUS=PASS
    LDP_OBSERVED="LDAP SSL/TLS is configured: useSSL=yes and ldapsslkeyf points to a key database; the decidable sub-condition of DISA STIG V-215173 is satisfied"
    LDP_MEANING="The LDAP client is configured to require SSL/TLS with a key database, satisfying the decidable sub-condition of DISA STIG V-215173. Confirming that the key database's server certificate was issued by DoD PKI or a DoD-approved external PKI requires the KDB password and gsk8capicmd; that manual verification remains a separate step."
    LDP_FIX="n/a"
  fi

  case "$LDP_STATUS" in
    NOT_APPLICABLE) LDP_SEVERITY=low ;;
    *) LDP_SEVERITY=med ;;
  esac

  add security stig_ldap_ssl "LDAP SSL/TLS server certificate validation" \
      "$LDP_STATUS" "$LDP_SEVERITY" "$LDP_OBSERVED" \
      "$LDP_MEANING" "$LDP_FIX" "stig:V-215173"
}
