...
AIXRAY_TOOL=ck-stig-ldd-mode


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_ldd_mode — DISA STIG V-215329. PARTIAL-AUTO.
  # Decidable: find / -name ldd plus ls -lL mode (0000 or more
  # permissive), or determinate absence. Vendor-documentation remainder
  # (ACL/EA "+" on an otherwise 0000 ldd) is typed NOT_ASSESSED.
  typeset SLD_FIND_RAW SLD_FIND_RC SLD_PATHS SLD_PARSE_RC
  typeset SLD_PATH SLD_LS_RAW SLD_LS_RC SLD_KIND SLD_MODE
  typeset SLD_HAS_BAD SLD_HAS_MANUAL SLD_REFUSE SLD_BAD_OBS SLD_OK_OBS
  typeset SLD_STATUS SLD_SEV SLD_OBSERVED SLD_MEANING SLD_FIX SLD_FIX_PATH
  typeset SLD_FAILCLOSED_PROBE SLD_FAILCLOSED_EMPTY
  typeset SLD_FAILCLOSED_MANUAL SLD_FAILCLOSED_UNPARSE
  typeset SLD_PARTIAL SLD_UNREADABLE

  SLD_FAILCLOSED_PROBE=NOT_ASSESSED
  SLD_FAILCLOSED_EMPTY=NOT_ASSESSED
  SLD_FAILCLOSED_MANUAL=NOT_ASSESSED
  SLD_FAILCLOSED_UNPARSE=NOT_ASSESSED
  ...
  SLD_STATUS=""
  SLD_OBSERVED=""
  SLD_FIX_PATH=/usr/bin/ldd
  SLD_PATHS=""
  SLD_KIND=""
  SLD_MODE=""
  SLD_PARTIAL=0
  SLD_UNREADABLE="some paths unreadable"

  # find / -name ldd
  SLD_FIND_RAW=$(aix find_ldd find / -name ldd)
  SLD_FIND_RC=$?

  if [ "$SLD_FIND_RC" -eq 1 ] || [ "$SLD_FIND_RC" -eq 2 ]; then
    if [ -z "$SLD_FIND_RAW" ]; then
      SLD_STATUS=$SLD_FAILCLOSED_PROBE
      SLD_OBSERVED="not assessed - ldd find probe failed (rc=$SLD_FIND_RC)"
    else
      SLD_PARTIAL=1
    fi
  elif [ "$SLD_FIND_RC" -ne 0 ]; then
    SLD_STATUS=$SLD_FAILCLOSED_PROBE
    SLD_OBSERVED="not assessed - ldd find probe failed (rc=$SLD_FIND_RC)"
  fi

  if [ -z "$SLD_STATUS" ] && [ -z "$SLD_FIND_RAW" ]; then
    SLD_STATUS=PASS
    SLD_OBSERVED="no ldd command present; prohibition holds"
  elif [ -z "$SLD_STATUS" ]; then
    SLD_PATHS=$(printf '%s\n' "$SLD_FIND_RAW" | awk '
      /^[ \t]*$/ { next }
      /^[ \t]*#/ { next }
      /^find:/ { diag=1; next }
    ...
    ')
    SLD_PARSE_RC=$?
    if [ "$SLD_PARSE_RC" -ne 0 ] || [ -z "$SLD_PATHS" ]; then
      SLD_STATUS=$SLD_FAILCLOSED_UNPARSE
      SLD_OBSERVED="not assessed - ldd find output was unparseable"
    else
      # One complete find line at a time; no word-split, no glob.
      while IFS= read -r SLD_PATH
      do
        [ -n "$SLD_PATH" ] || continue
        SLD_LS_RAW=$(aix ls_ldd ls -lL "$SLD_PATH")
        SLD_LS_RC=$?
        if aix_capture_missing ls_ldd; then
          [ -n "$SLD_REFUSE" ] || \
            SLD_REFUSE="not assessed - ldd listing was not captured"
          continue
        fi
        if [ "$SLD_LS_RC" -eq 1 ] || [ "$SLD_LS_RC" -eq 2 ]; then
          if [ -z "$SLD_LS_RAW" ]; then
            [ -n "$SLD_REFUSE" ] || \
              SLD_REFUSE="not assessed - ldd listing failed (rc=$SLD_LS_RC)"
            continue
          fi
          SLD_PARTIAL=1
        elif [ "$SLD_LS_RC" -ne 0 ]; then
          [ -n "$SLD_REFUSE" ] || \
            SLD_REFUSE="not assessed - ldd listing failed (rc=$SLD_LS_RC)"
          continue
        fi
        if [ -z "$SLD_LS_RAW" ]; then
          [ -n "$SLD_REFUSE" ] || \
            SLD_REFUSE="not assessed - ldd listing returned empty output with rc=0"
          continue
        fi
        SLD_KIND=$(printf '%s\n' "$SLD_LS_RAW" | awk -v want="$SLD_PATH" '
          /^[ \t]*$/ { next }
          /^[ \t]*#/ { next }
          {
            records++
            ...
            wlen = length("" want)
            llen = length($0)
            if (want !~ /^\// || wlen < 1 || llen < wlen) { bad=1; next }
            if (substr($0, llen - wlen + 1) != want) { bad=1; next }
            if (llen > wlen && substr($0, llen - wlen, 1) !~ /[ \t]/) { bad=1; next }
            bits=substr(mode, 2, 9)
            if (bits != "---------") perm=1
          }
          END {
            if (records != 1 || bad) exit 1
            if (perm) { print "bad:" mode; exit 0 }
            if (plus) { print "manual:" mode; exit 0 }
            print "ok:" mode
          }
        ')
        if [ $? -ne 0 ] || [ -z "$SLD_KIND" ]; then
          [ -n "$SLD_REFUSE" ] || \
            SLD_REFUSE="not assessed - ldd listing was unparseable"
          continue
        fi
        SLD_MODE=${SLD_KIND#*:}
        case "$SLD_KIND" in
          bad:*)
            SLD_HAS_BAD=1
            SLD_FIX_PATH=$SLD_PATH
            if [ -z "$SLD_BAD_OBS" ]; then
              SLD_BAD_OBS="more permissive than 0000: ldd $SLD_PATH mode $SLD_MODE"
            fi
            ;;
          manual:*)
            SLD_HAS_MANUAL=1
            ...
            ;;
          ok:*)
            if [ -z "$SLD_OK_OBS" ]; then
              SLD_OK_OBS="ldd $SLD_PATH mode 0000"
            fi
            ;;
          *)
            [ -n "$SLD_REFUSE" ] || \
...
SLD_PATHS_END
      ...
      if [ "$SLD_HAS_BAD" -ne 0 ]; then
        SLD_STATUS=FAIL
        SLD_OBSERVED="$SLD_BAD_OBS"
      elif [ -n "$SLD_REFUSE" ]; then
        case "$SLD_REFUSE" in
          *'empty output'*)
            SLD_STATUS=$SLD_FAILCLOSED_EMPTY
            ;;
          *)
            SLD_STATUS=$SLD_FAILCLOSED_UNPARSE
            ;;
        esac
        SLD_OBSERVED="$SLD_REFUSE"
      elif [ "$SLD_HAS_MANUAL" -ne 0 ]; then
        SLD_STATUS=$SLD_FAILCLOSED_MANUAL
        SLD_OBSERVED="not assessed - manual portion: consult vendor documentation concerning the ldd command and whether it provides protection from the execution of untrusted executables"
      else
        SLD_STATUS=PASS
        SLD_OBSERVED="$SLD_OK_OBS"
        if [ "$SLD_PARTIAL" -ne 0 ]; then
          SLD_OBSERVED="$SLD_OBSERVED; $SLD_UNREADABLE"
        fi
      fi
    fi
  fi
  ...
  case "$SLD_STATUS" in
    PASS)
      SLD_SEV=med
      SLD_MEANING="The ldd command is mode 0000 or absent, so it cannot execute untrusted programs."
      SLD_FIX="n/a"
      ;;
    FAIL)
      SLD_SEV=med
      SLD_MEANING="The ldd command is more permissive than mode 0000, so it can be pointed at an untrusted executable."
      SLD_FIX="# chmod 0000 $SLD_FIX_PATH"
      ;;
    *)
      if [ -z "$SLD_STATUS" ]; then
        SLD_STATUS=$SLD_FAILCLOSED_UNPARSE
        SLD_OBSERVED="not assessed - ldd evidence was unparseable"
      fi
      SLD_SEV=med
      SLD_MEANING="PTxray did not obtain a complete automatic determination for STIG V-215329."
      SLD_FIX="consult vendor documentation concerning the ldd command, verify find / -name ldd and ls -lL /usr/bin/ldd, then rerun PTxray."
      ;;
  esac

  add security stig_ldd_mode "ldd command disabled" \
    "$SLD_STATUS" "$SLD_SEV" "$SLD_OBSERVED" \
    "$SLD_MEANING" "$SLD_FIX" "stig:V-215329"
}
