...
AIXRAY_TOOL=ck-stig-lib-acl


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_lib_acl — DISA STIG V-215326.  No regular file under /usr/lib/security
  # or /usr/lib/methods may carry an extended ACL (extended permissions enabled).
  # Read-only port of the STIG's inline aclget-to-tempfile loop: aclget writes
  # to stdout here, never to a temp file.  The probe prints the path of every
  # file whose aclget output shows extended permissions enabled, then a
  # "scan complete" summary line; a completed scan is never empty, so an empty
  # rc-0 probe is a swallowed capture error, not absence.
  typeset LIB_ACL_RAW LIB_ACL_RC LIB_ACL_STATUS LIB_ACL_REASON
  typeset LIB_ACL_OFFENDING LIB_ACL_LIST
  typeset LIB_ACL_OBSERVED LIB_ACL_MEANING LIB_ACL_FIX

  LIB_ACL_RAW=$(aix lib_acl sh -c 'find /usr/lib/security /usr/lib/methods -type f | while read file; do aclget "$file" 2>/dev/null | grep -e "[[:space:]]enabled$" >/dev/null 2>&1 && printf "%s\n" "$file"; done; printf "scan complete\n"')
  LIB_ACL_RC=$?
  ...
  if [ "$LIB_ACL_RC" -ne 0 ]; then
    LIB_ACL_STATUS=NOT_ASSESSED
    LIB_ACL_REASON="probe failed (rc=$LIB_ACL_RC)"
  elif [ -z "$LIB_ACL_RAW" ]; then
    LIB_ACL_STATUS=NOT_ASSESSED
    LIB_ACL_REASON="probe returned no output"
  else
    LIB_ACL_OFFENDING=$(printf '%s\n' "$LIB_ACL_RAW" | grep -Fvx 'scan complete')
    if [ -n "$LIB_ACL_OFFENDING" ]; then
      LIB_ACL_STATUS=FAIL
    else
      LIB_ACL_STATUS=PASS
    fi
  ...
  case "$LIB_ACL_STATUS" in
    FAIL)
      LIB_ACL_LIST=$(printf '%s' "$LIB_ACL_OFFENDING" | tr '\n' ' ')
      LIB_ACL_OBSERVED="system library files with extended ACLs enabled: $LIB_ACL_LIST"
      LIB_ACL_MEANING="An extended ACL on a system library file can grant principals beyond the intended base permissions access to security-critical library code."
      LIB_ACL_FIX="remove the extended ACL(s) from the offending file(s) under /usr/lib/security and /usr/lib/methods and disable extended permissions; PTxray only recommends these commands."
      ;;
    PASS)
      LIB_ACL_OBSERVED="no system library file under /usr/lib/security or /usr/lib/methods has extended ACLs enabled"
      LIB_ACL_MEANING="Every system library file in the checked directories carries only base permissions."
      LIB_ACL_FIX="n/a"
      ;;
    *)
      LIB_ACL_OBSERVED="not assessed - $LIB_ACL_REASON"
      LIB_ACL_MEANING="PTxray did not obtain trustworthy evidence about extended ACLs on system library files."
      LIB_ACL_FIX="verify the aclget capture for /usr/lib/security and /usr/lib/methods as root, then rerun PTxray."
      ;;
  esac

  add security stig_lib_acl "Extended ACLs on system library files" \
    "$LIB_ACL_STATUS" med "$LIB_ACL_OBSERVED" \
    "$LIB_ACL_MEANING" "$LIB_ACL_FIX" "stig:V-215326"
}
