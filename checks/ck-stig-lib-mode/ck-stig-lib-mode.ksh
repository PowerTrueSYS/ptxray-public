...
AIXRAY_TOOL=ck-stig-lib-mode


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # lib_mode — DISA STIG V-215279.
  # Library files under /usr/lib and /lib must be mode 0755 or less
  # permissive; a more permissive mode lets an unprivileged account replace
  # a library used by privileged processes.  Every uncertain evidence shape
  # fails closed. The listing is streamed into one POSIX awk; it is never
  # stored in a ksh variable (ksh88 variable space is bounded). Observed
  # FAIL text names the first path+mode, the mismatch count, and at most
  # 3 sample paths.
  typeset LM_SUM LM_RC LM_TOKEN LM_STATUS LM_OBSERVED
  typeset LM_MEANING LM_FIX LM_PATH LM_MODE LM_PARTIAL
  typeset LM_NFAIL LM_SAMPLES LM_PARSE_RC
  ...
  # rows. Refuse only when the probe produced no rows. A sentinel after
  # the listing carries the probe rc so awk can tell failed from empty.
  LM_SUM=$(
    {
      aix lib_listing ls -lLR /usr/lib /lib
      printf '\n%s\n' "LM_PROBE_STATUS $?"
    } | awk '
      BEGIN { bad = 0; seen = 0; nfail = 0; nsamp = 0; samples = ""; fail_path = ""; fail_mode = ""; nonempty = 0; got_rc = 0 }
      $1 == "LM_PROBE_STATUS" && NF == 2 { probe_rc = $2; got_rc = 1; next }
      {
        line = $0
        if (line ~ /^[ \t]*$/) next
        nonempty = 1
        ...
        for (i = 2; i <= 10; i++) {
          c = substr(mode, i, 1)
          if (c != "r" && c != "w" && c != "x" && c != "s" && c != "S" && c != "t" && c != "T" && c != "-") {
            bad = 1
            next
          }
        }
        seen = 1
        gw = (substr(mode, 6, 1) == "w")
        ow = (substr(mode, 9, 1) == "w")
        su = (substr(mode, 4, 1) == "s" || substr(mode, 4, 1) == "S")
        sg = (substr(mode, 7, 1) == "s" || substr(mode, 7, 1) == "S")
        st = (substr(mode, 10, 1) == "t" || substr(mode, 10, 1) == "T")
        if (gw || ow || su || sg || st) {
          nfail = nfail + 1
          if (curdir == "") { p = $NF } else { p = curdir "/" $NF }
          if (fail_path == "") {
            fail_path = p
            fail_mode = mode
          }
      ...
      END {
        if (!got_rc) { print "0"; print "UNPARSEABLE"; exit 0 }
        print probe_rc
        if (!nonempty) { print "EMPTY"; exit 0 }
        if (bad) { print "UNPARSEABLE"; exit 0 }
        if (!seen) { print "ABSENT"; exit 0 }
        if (fail_path != "") {
          print "FAIL"
          print nfail
          print fail_path
  ...
  if [ "$LM_PARSE_RC" -ne 0 ] || [ -z "$LM_SUM" ]; then
    LM_STATUS=NOT_ASSESSED
    LM_OBSERVED="not assessed - ls -lLR /usr/lib /lib output was unparseable"
  else
    LM_RC=$(printf '%s\n' "$LM_SUM" | sed -n '1p')
    LM_TOKEN=$(printf '%s\n' "$LM_SUM" | sed -n '2p')
    if [ -z "$LM_RC" ] || [ -z "$LM_TOKEN" ]; then
      LM_STATUS=NOT_ASSESSED
      LM_OBSERVED="not assessed - ls -lLR /usr/lib /lib output was unparseable"
    elif [ "$LM_TOKEN" = EMPTY ]; then
      if [ "$LM_RC" -ne 0 ]; then
        LM_STATUS=NOT_ASSESSED
        LM_OBSERVED="not assessed - ls -lLR /usr/lib /lib failed (rc=$LM_RC)"
      else
        LM_STATUS=NOT_ASSESSED
        LM_OBSERVED="not assessed - ls -lLR /usr/lib /lib returned no output; this is a swallowed error, not a verified empty subject"
      fi
    elif [ "$LM_RC" -ne 0 ]; then
      case "$LM_RC" in
        1|2)
          LM_PARTIAL=1
          ;;
  ...
  if [ -z "$LM_STATUS" ]; then
    case "$LM_TOKEN" in
      PASS)
        if [ "$LM_PARTIAL" -eq 1 ]; then
          LM_STATUS=WARN
          LM_OBSERVED="all library files under /usr/lib and /lib are mode 0755 or less permissive; some paths unreadable"
        else
          LM_STATUS=PASS
          LM_OBSERVED="all library files under /usr/lib and /lib are mode 0755 or less permissive"
        fi
        ;;
      ABSENT)
        LM_STATUS=FAIL
        LM_OBSERVED="no library files present under /usr/lib and /lib; a required configuration with a missing subject is unmet"
        ;;
      FAIL)
        LM_NFAIL=$(printf '%s\n' "$LM_SUM" | sed -n '3p')
        LM_PATH=$(printf '%s\n' "$LM_SUM" | sed -n '4p')
        ...
        LM_SAMPLES=$(printf '%s\n' "$LM_SUM" | sed -n '6p')
        LM_STATUS=FAIL
        LM_OBSERVED="$LM_PATH mode $LM_MODE (more permissive than 0755); $LM_NFAIL more-permissive library file(s) (up to 3 paths shown): $LM_SAMPLES"
        ;;
      UNPARSEABLE)
        LM_STATUS=NOT_ASSESSED
        LM_OBSERVED="not assessed - ls -lLR /usr/lib /lib output was unparseable"
  ...
  case "$LM_STATUS" in
    PASS)
      LM_MEANING="AIX library files under /usr/lib and /lib are mode 0755 or less permissive as required by DISA STIG V-215279."
      LM_FIX="n/a"
      ;;
    WARN)
      LM_MEANING="AIX library files under /usr/lib and /lib that could be listed are mode 0755 or less permissive as required by DISA STIG V-215279; some paths were unreadable so the scan is incomplete."
      LM_FIX="rerun 'ls -lLR /usr/lib /lib' as root so every path is readable, then rerun PTxray."
      ;;
    FAIL)
      LM_MEANING="A library file more permissive than 0755 lets an unprivileged account replace or modify a library used by privileged processes."
      LM_FIX="change the library mode with 'chmod 0755 <path>/<library-file>'; PTxray only recommends this command, it never runs it."
      ;;
    *)
      LM_MEANING="PTxray did not obtain trustworthy evidence of the library file modes under /usr/lib and /lib."
      LM_FIX="run 'ls -lLR /usr/lib /lib' as root, correct the capture or path problem, and rerun PTxray."
      ;;
  esac

  add security stig_lib_mode "AIX library files mode" "$LM_STATUS" med "$LM_OBSERVED" \
      "$LM_MEANING" "$LM_FIX" "stig:V-215279"
}
