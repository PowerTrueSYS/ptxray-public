...
AIXRAY_TOOL=ck-stig-limit-number-concurrent-sessions


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_limit_number_concurrent_sessions — DISA STIG V-215172.
  # Required configuration: every account has maxulogs set in 1..10.
  # Uncertain probe evidence is refused; a missing subject is a finding.
  typeset LNCS_RAW LNCS_RC LNCS_PARSED LNCS_PARSE_RC LNCS_CLASS LNCS_DETAIL
  typeset LNCS_STATUS LNCS_SEV LNCS_OBSERVED LNCS_MEANING LNCS_FIX

  LNCS_RAW=$(aix lsuser_maxulogs_all lsuser -a maxulogs ALL)
  LNCS_RC=$?
  LNCS_CLASS=""
  LNCS_DETAIL=""

  if [ "$LNCS_RC" -ne 0 ]; then
    LNCS_CLASS=probe_failed
  elif [ -z "$LNCS_RAW" ]; then
    LNCS_CLASS=probe_empty
  else
    LNCS_PARSED=$(printf '%s\n' "$LNCS_RAW" | awk '
      function is_user(u) {
      ...
      }
      {
        ...
        if (NF == 1) {
          have = 0
        } else if (NF == 2 && index($2, "maxulogs=") == 1) {
          val = substr($2, 10)
          have = 1
        } else {
          bad = 1
    ...
    ')
    LNCS_PARSE_RC=$?
    if [ "$LNCS_PARSE_RC" -ne 0 ] || [ -z "$LNCS_PARSED" ]; then
      LNCS_CLASS=unparseable
    else
      case "$LNCS_PARSED" in
        ok) LNCS_CLASS=ok ;;
        fail\ *)
          LNCS_CLASS=fail
          LNCS_DETAIL=${LNCS_PARSED#fail }
          ;;
        absent) LNCS_CLASS=subject_absent ;;
        absent_with_users) LNCS_CLASS=unparseable ;;
        *) LNCS_CLASS=unparseable ;;
      esac
    fi
  fi
  ...
  case "$LNCS_CLASS" in
    ok)
      LNCS_STATUS=PASS
      LNCS_OBSERVED="all listed accounts have maxulogs in 1..10"
      LNCS_MEANING="Every listed account has maxulogs set between 1 and 10, so concurrent sessions cannot exceed 10."
      LNCS_FIX="n/a"
      ;;
    fail) LNCS_STATUS=FAIL
      LNCS_OBSERVED="maxulogs not in 1..10: $LNCS_DETAIL"
      LNCS_MEANING="One or more accounts have maxulogs unset, 0, or greater than 10, so concurrent sessions are not limited to 10."
      LNCS_FIX="chsec -f /etc/security/user -s default -a maxulogs=10; chuser maxulogs=10 [user_name]; PTxray only recommends these commands."
      ;;
    subject_absent) LNCS_STATUS=FAIL
      LNCS_OBSERVED="required maxulogs subject is absent"
      LNCS_MEANING="No account list was returned, so the required concurrent-session limit is not configured."
      LNCS_FIX="chsec -f /etc/security/user -s default -a maxulogs=10; chuser maxulogs=10 [user_name]; PTxray only recommends these commands."
      ;;
    probe_failed) LNCS_STATUS=NOT_ASSESSED
      LNCS_OBSERVED="not assessed - lsuser maxulogs probe failed (rc=$LNCS_RC)"
      LNCS_MEANING="PTxray did not obtain trustworthy per-account maxulogs evidence."
      LNCS_FIX="run 'lsuser -a maxulogs ALL', correct the capture problem, and rerun PTxray."
      ;;
    probe_empty) LNCS_STATUS=NOT_ASSESSED
      LNCS_OBSERVED="not assessed - lsuser maxulogs probe returned no output"
      LNCS_MEANING="PTxray did not obtain trustworthy per-account maxulogs evidence."
      LNCS_FIX="run 'lsuser -a maxulogs ALL', correct the capture problem, and rerun PTxray."
      ;;
    unparseable) LNCS_STATUS=NOT_ASSESSED
      LNCS_OBSERVED="not assessed - lsuser maxulogs output was unparseable"
      LNCS_MEANING="PTxray did not obtain trustworthy per-account maxulogs evidence."
      LNCS_FIX="run 'lsuser -a maxulogs ALL', correct the capture problem, and rerun PTxray."
      ;;
    *)
      LNCS_STATUS=NOT_ASSESSED
      LNCS_OBSERVED="not assessed - lsuser maxulogs output was unparseable"
      LNCS_MEANING="PTxray did not obtain trustworthy per-account maxulogs evidence."
      LNCS_FIX="run 'lsuser -a maxulogs ALL', correct the capture problem, and rerun PTxray."
      ;;
  ...
  case "$LNCS_STATUS" in
    NOT_APPLICABLE) LNCS_SEV=low ;;
    *) LNCS_SEV=med ;;
  esac

  add security stig_limit_number_concurrent_sessions "Concurrent session limit (maxulogs)" \
    "$LNCS_STATUS" "$LNCS_SEV" "$LNCS_OBSERVED" \
    "$LNCS_MEANING" "$LNCS_FIX" "stig:V-215172"
}
