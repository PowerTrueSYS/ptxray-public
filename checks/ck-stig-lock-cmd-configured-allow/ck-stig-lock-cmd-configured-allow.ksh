...
AIXRAY_TOOL=ck-stig-lock-cmd-configured-allow


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_lock_cmd_configured_allow — DISA STIG V-215211.
  # Users must be able to directly initiate a session lock for all connection
  # types, which requires both /usr/bin/lock and /usr/bin/X11/xlock to exist.
  # Required-configuration rule: a determinately missing subject is a FAIL,
  # never a refusal.  Uncertain evidence (a probe that errored with no rows,
  # or rc-0 empty rows) is NOT_ASSESSED — empty rc-0 output is a swallowed
  # error, not proof of absence.
  typeset SLC_LOCK_RAW SLC_LOCK_RC SLC_XLOCK_RAW SLC_XLOCK_RC
  typeset SLC_LOCK_STATE SLC_XLOCK_STATE SLC_LOCK_REASON SLC_XLOCK_REASON
  typeset SLC_STATUS SLC_OBSERVED SLC_MEANING SLC_FIX
  # SLC_PROBE_STATE/SLC_PROBE_REASON carry slc_probe_state's result back to the
  # caller, so they must stay plain (non-typeset) globals: under a function
  # wrapper such as assemble's standalone_check, typeset in the caller scope
  # breaks the nested function's assignment under set -u.
  SLC_PROBE_STATE=""
  SLC_PROBE_REASON=""

  SLC_LOCK_STATE=unknown
  SLC_XLOCK_STATE=unknown
  SLC_LOCK_REASON=""
  SLC_XLOCK_REASON=""

  SLC_LOCK_RAW=$(aix lock_ls ls /usr/bin/lock)
  SLC_LOCK_RC=$?
  SLC_XLOCK_RAW=$(aix xlock_ls ls /usr/bin/X11/xlock)
  SLC_XLOCK_RC=$?

  # Classify one ls probe.  present: rc 0 and the subject path appears in the
  # rows.  absent: ls exited 2 (the AIX "no such file" signal — live stderr is
  # discarded, so rc 2 with empty rows still means the path is gone), or ls
  # returned a nonzero status with error rows, or rc 0 but the path is not
  # shown.  unassessed: no rows at all (capture failure or swallowed empty rc-0
  # output) — unusable evidence, never absence.
  function slc_probe_state {
    typeset subject raw rc
    subject=$1
    raw=$2
    rc=$3
    SLC_PROBE_STATE=unassessed
    SLC_PROBE_REASON=""
    if [ "$rc" -eq 2 ]; then
      SLC_PROBE_STATE=absent
      SLC_PROBE_REASON="$subject is not present (ls reports no such file)"
    elif [ "$rc" -ne 0 ]; then
      if [ -n "$raw" ]; then
        SLC_PROBE_STATE=absent
        SLC_PROBE_REASON="$subject is not present (ls reports no such file)"
      else
        SLC_PROBE_REASON="$subject probe failed with no output (rc=$rc)"
      fi
    elif [ -z "$raw" ]; then
      SLC_PROBE_REASON="$subject probe returned empty output with rc 0"
    elif printf '%s\n' "$raw" | awk -v subject="$subject" \
        'index($0, subject) { found = 1 } END { exit found ? 0 : 1 }'; then
      SLC_PROBE_STATE=present
    else
      SLC_PROBE_STATE=absent
      SLC_PROBE_REASON="$subject was not shown by ls"
    fi
  }

  slc_probe_state "/usr/bin/lock" "$SLC_LOCK_RAW" "$SLC_LOCK_RC"
  SLC_LOCK_STATE=$SLC_PROBE_STATE
  SLC_LOCK_REASON=$SLC_PROBE_REASON

  slc_probe_state "/usr/bin/X11/xlock" "$SLC_XLOCK_RAW" "$SLC_XLOCK_RC"
  SLC_XLOCK_STATE=$SLC_PROBE_STATE
  SLC_XLOCK_REASON=$SLC_PROBE_REASON

  if [ "$SLC_LOCK_STATE" = "absent" ] || [ "$SLC_XLOCK_STATE" = "absent" ]; then
    SLC_STATUS=FAIL
    if [ "$SLC_LOCK_STATE" = "absent" ] && [ "$SLC_XLOCK_STATE" = "absent" ]; then
      SLC_OBSERVED="neither /usr/bin/lock nor /usr/bin/X11/xlock is present"
    elif [ "$SLC_LOCK_STATE" = "absent" ]; then
      SLC_OBSERVED="$SLC_LOCK_REASON"
    else
      SLC_OBSERVED="$SLC_XLOCK_REASON"
    fi
  elif [ "$SLC_LOCK_STATE" = "unassessed" ] || [ "$SLC_XLOCK_STATE" = "unassessed" ]; then
    SLC_STATUS=NOT_ASSESSED
    SLC_OBSERVED="not assessed - "
    if [ "$SLC_LOCK_STATE" = "unassessed" ]; then
      SLC_OBSERVED="$SLC_OBSERVED$SLC_LOCK_REASON"
    fi
    if [ "$SLC_XLOCK_STATE" = "unassessed" ]; then
      if [ "$SLC_LOCK_STATE" = "unassessed" ]; then
        SLC_OBSERVED="$SLC_OBSERVED; "
      fi
      SLC_OBSERVED="$SLC_OBSERVED$SLC_XLOCK_REASON"
    fi
  else
    SLC_STATUS=PASS
    SLC_OBSERVED="/usr/bin/lock and /usr/bin/X11/xlock are present"
  fi

  case "$SLC_STATUS" in
    PASS)
      SLC_MEANING="Both /usr/bin/lock and /usr/bin/X11/xlock are available, so a user can directly initiate a session lock for every connection type."
      SLC_FIX="n/a"
      ;;
    FAIL)
      SLC_MEANING="A required session lock command is unavailable, so a user cannot directly initiate a session lock for every connection type."
      SLC_FIX="installp -aXYgd /dev/cd0 -e /tmp/install.log bos.rte.security to provide the lock and xlock commands; PTxray only recommends this command."
      ;;
    *)
      SLC_MEANING="PTxray did not obtain trustworthy evidence that both session lock commands are present."
      SLC_FIX="verify /usr/bin/lock and /usr/bin/X11/xlock with 'ls', correct the capture or path problem, and rerun PTxray."
      ;;
  esac

  add security stig_lock_cmd_configured_allow "Session lock command configured and allowed" \
    "$SLC_STATUS" med "$SLC_OBSERVED" \
    "$SLC_MEANING" "$SLC_FIX" "stig:V-215211"
}
