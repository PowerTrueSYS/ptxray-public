...
AIXRAY_TOOL=ck-stig-lock-cmd


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_lock_cmd — DISA STIG V-215187.  AIX must provide the lock command so
  # a user can retain a session lock until reauthenticated.  Two required
  # subjects: the bos.rte.security fileset and /usr/bin/lock.  Every uncertain
  # evidence shape fails closed to NOT_ASSESSED, never PASS.
  typeset LKC_FS_RAW LKC_FS_RC LKC_FS_STATE
  typeset LKC_LS_RAW LKC_LS_RC LKC_LS_STATE
  typeset LKC_STATUS LKC_OBSERVED LKC_MEANING LKC_FIX LKC_FAIL_CAUSE

  LKC_FS_RAW=$(aix lslpp_bos_rte_security lslpp -L bos.rte.security)
  LKC_FS_RC=$?
  LKC_LS_RAW=$(aix ls_usr_bin_lock ls /usr/bin/lock)
  LKC_LS_RC=$?

  # lslpp -L on a not-installed fileset is rc 1 with empty stdout; any other
  # non-zero rc (or rc 0 with empty output) is an unproven probe, not absence.
  if [ "$LKC_FS_RC" -eq 0 ] && [ -n "$LKC_FS_RAW" ]; then
    if printf '%s\n' "$LKC_FS_RAW" | awk '$1 == "bos.rte.security" && ($3 == "C" || $3 == "A") { found = 1 } END { exit found ? 0 : 1 }'; then
      LKC_FS_STATE=installed
    else
      LKC_FS_STATE=failed
    fi
  elif [ "$LKC_FS_RC" -eq 1 ] && [ -z "$LKC_FS_RAW" ]; then
    LKC_FS_STATE=missing
  else
    LKC_FS_STATE=failed
  fi

  # ls on a missing path is rc 2 with empty stdout; anything else non-zero
  # (or rc 0 with empty output) is an unproven probe, not absence.
  if [ "$LKC_LS_RC" -eq 0 ] && [ -n "$LKC_LS_RAW" ]; then
    if printf '%s\n' "$LKC_LS_RAW" | awk 'NR == 1 && $1 == "/usr/bin/lock" { found = 1 } END { exit found ? 0 : 1 }'; then
      LKC_LS_STATE=exists
    else
      LKC_LS_STATE=failed
    fi
  elif [ "$LKC_LS_RC" -eq 2 ] && [ -z "$LKC_LS_RAW" ]; then
    LKC_LS_STATE=missing
  else
    LKC_LS_STATE=failed
  fi

  if [ "$LKC_FS_STATE" = missing ] || [ "$LKC_LS_STATE" = missing ]; then
    LKC_STATUS=FAIL
    if [ "$LKC_FS_STATE" = missing ]; then
      LKC_OBSERVED="the bos.rte.security fileset is not installed"
    else
      LKC_OBSERVED="/usr/bin/lock is not present"
    fi
  elif [ "$LKC_FS_STATE" = installed ] && [ "$LKC_LS_STATE" = exists ]; then
    LKC_STATUS=PASS
    LKC_OBSERVED="bos.rte.security installed and /usr/bin/lock present"
  else
    LKC_STATUS=NOT_ASSESSED
    LKC_FAIL_CAUSE=""
    if [ "$LKC_FS_STATE" = failed ]; then
      if [ "$LKC_FS_RC" -eq 0 ]; then
        LKC_FAIL_CAUSE="bos.rte.security query returned no usable rows"
      else
        LKC_FAIL_CAUSE="bos.rte.security probe failed (rc=$LKC_FS_RC)"
      fi
    fi
    if [ "$LKC_LS_STATE" = failed ]; then
      if [ -n "$LKC_FAIL_CAUSE" ]; then
        LKC_FAIL_CAUSE="${LKC_FAIL_CAUSE}; "
      fi
      if [ "$LKC_LS_RC" -eq 0 ]; then
        LKC_FAIL_CAUSE="${LKC_FAIL_CAUSE}/usr/bin/lock query returned no usable rows"
      else
        LKC_FAIL_CAUSE="${LKC_FAIL_CAUSE}/usr/bin/lock probe failed (rc=$LKC_LS_RC)"
      fi
    fi
    LKC_OBSERVED="not assessed - $LKC_FAIL_CAUSE"
  fi

  case "$LKC_STATUS" in
    PASS)
      LKC_MEANING="Users can lock their session with /usr/bin/lock until they are reauthenticated."
      LKC_FIX="n/a"
      ;;
    FAIL)
      LKC_MEANING="An unattended session cannot be locked for reauthentication because the lock command or its supporting fileset is missing."
      LKC_FIX="install the bos.rte.security fileset with 'installp -aXYgd /dev/cd0 -e /tmp/install.log bos.rte.security'; PTxray only recommends this command."
      ;;
    *)
      LKC_MEANING="PTxray did not obtain trustworthy evidence that the lock command and its supporting fileset are present."
      LKC_FIX="verify 'lslpp -L bos.rte.security' and 'ls /usr/bin/lock' as root, then rerun PTxray."
      ;;
  esac

  add security stig_lock_cmd "Lock command present" \
    "$LKC_STATUS" med "$LKC_OBSERVED" \
    "$LKC_MEANING" "$LKC_FIX" "stig:V-215187"
}
