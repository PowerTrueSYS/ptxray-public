...
AIXRAY_TOOL=ck-stig-logfile-acl


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_logfile_acl — DISA STIG V-215324.  Log files under /var/log and
  # /var/adm must not have extended ACLs, except as needed to support
  # authorized software.  PARTIAL-AUTO: aclget of those trees is the
  # decidable sub-condition.  Identifying every system log file, and whether
  # an enabled ACL is documented with the ISSO, is the site/ISSO manual
  # portion and never a silent PASS.
  typeset SLA_LIST_RAW SLA_LIST_RC SLA_PARSED SLA_PARSE_RC SLA_TOKEN
  typeset SLA_PATH SLA_ACL_RAW SLA_ACL_RC SLA_SHAPE
  typeset SLA_FAIL SLA_MANUAL SLA_REASON SLA_FAIL_PATH
  typeset SLA_STATUS SLA_OBSERVED SLA_MEANING SLA_FIX
  ...
  SLA_LIST_RAW=$(aix log_files find /var/log /var/adm -type f)
  SLA_LIST_RC=$?

  if [ -z "$SLA_LIST_RAW" ]; then
    if [ "$SLA_LIST_RC" -eq 0 ]; then
      SLA_REASON="find /var/log /var/adm -type f returned rc 0 with no output; rc0+empty is a swallowed error, not absence"
    else
      SLA_REASON="log file listing probe failed (rc=$SLA_LIST_RC)"
    fi
  else
    SLA_PARSED=$(printf '%s\n' "$SLA_LIST_RAW" | awk '
      {
        ...
        if (line == "") next
        low = tolower(line)
        if (index(low, "no file or directory") != 0 || index(low, "no such file") != 0) {
          absent++
          next
        }
        if (index(line, "/var/log/") != 1 && index(line, "/var/adm/") != 1) {
          bad = 1
          next
        }
        n++
    ...
    ')
    SLA_PARSE_RC=$?
    if [ "$SLA_PARSE_RC" -ne 0 ] || [ -z "$SLA_PARSED" ]; then
      SLA_REASON="log file listing was unparseable"
    else
      SLA_TOKEN=$(printf '%s\n' "$SLA_PARSED" | sed -n '1p')
      if [ "$SLA_TOKEN" = "ABSENT" ]; then
        if [ "$SLA_LIST_RC" -eq 1 ]; then
          SLA_STATUS=PASS
          SLA_OBSERVED="no in-scope log files under /var/log or /var/adm"
        else
          SLA_REASON="log file listing probe failed (rc=$SLA_LIST_RC)"
        fi
      elif [ "$SLA_LIST_RC" -ne 0 ]; then
        SLA_REASON="log file listing probe failed (rc=$SLA_LIST_RC) while also returning paths"
      else
        while read SLA_PATH
        do
          [ -n "$SLA_PATH" ] || continue
          [ "$SLA_PATH" = "ABSENT" ] && continue
          [ -z "$SLA_REASON" ] || continue
          SLA_ACL_RAW=$(aix log_aclget aclget "$SLA_PATH")
          SLA_ACL_RC=$?
          if [ "$SLA_ACL_RC" -ne 0 ]; then
            SLA_REASON="aclget $SLA_PATH failed (rc=$SLA_ACL_RC)"
          elif [ -z "$SLA_ACL_RAW" ]; then
            SLA_REASON="aclget $SLA_PATH returned rc 0 with no output; rc0+empty is a swallowed error, not absence"
          else
            SLA_SHAPE=$(printf '%s\n' "$SLA_ACL_RAW" | awk '
              {
                line = $0
                ...
                low = tolower(line)
                if (low == "extended permissions") { want = 1; next }
                if (want) {
                  if (low == "enabled") en = 1
                  else if (low == "disabled") dis = 1
                  else bad = 1
                  want = 0
                  next
                }
                if (index(low, "permit ") == 1 || index(low, "permit\t") == 1 \
                    || index(low, "deny ") == 1 || index(low, "deny\t") == 1 \
                    || index(low, "specify ") == 1 || index(low, "specify\t") == 1) {
                  entries = 1
                }
              }
              END {
                if (bad || (en && dis) || (!en && !dis)) { print "unparseable"; exit }
                if (en && entries) { print "manual"; exit }
                if (en) { print "enabled"; exit }
                print "disabled"
              }
            ')
            case "$SLA_SHAPE" in
              ...
              disabled)
                ;;
              *)
                SLA_REASON="aclget $SLA_PATH output was unparseable"
                ;;
            esac
          fi
        done <<EOF
...
EOF
  ...
  if [ -z "$SLA_STATUS" ]; then
    if [ -n "$SLA_REASON" ]; then
      SLA_STATUS=NOT_ASSESSED
      SLA_OBSERVED="not assessed - $SLA_REASON"
    elif [ "$SLA_FAIL" -eq 1 ]; then
      SLA_STATUS=FAIL
      SLA_OBSERVED="$SLA_FAIL_PATH extended permissions enabled (undocumented ACL)"
    elif [ "$SLA_MANUAL" -eq 1 ]; then
      SLA_STATUS=NOT_ASSESSED
      SLA_OBSERVED="not assessed - log file extended ACL may be needed to support authorized software; ISSO must confirm the ACL is documented"
    else
      SLA_STATUS=PASS
      SLA_OBSERVED="all in-scope log files have extended permissions disabled"
    fi
  fi

  case "$SLA_STATUS" in
    PASS)
      SLA_MEANING="In-scope AIX log files under /var/log and /var/adm have no extended ACLs, or no such log file exists, satisfying the decidable sub-condition of DISA STIG V-215324."
      SLA_FIX="n/a"
      ;;
    FAIL)
      SLA_MEANING="A system log file has extended permissions enabled and that ACL is not documented, so an unauthorized principal can alter or hide audit evidence."
      SLA_FIX="# acledit <system_log_file>  (PTxray only recommends this command; it never runs it)"
      ;;
    *)
      SLA_MEANING="PTxray did not obtain trustworthy evidence for log-file extended ACLs, or the ISSO documentation clause remains."
      SLA_FIX="run 'aclget <system_log_file>' on each system log file, confirm with the ISSO whether any enabled ACL is documented as needed for authorized software, then rerun PTxray."
      ;;
  esac

  add security stig_logfile_acl "AIX log file extended ACLs" \
      "$SLA_STATUS" med "$SLA_OBSERVED" \
      "$SLA_MEANING" "$SLA_FIX" "stig:V-215324"
}
