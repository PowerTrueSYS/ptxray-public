...
AIXRAY_TOOL=ck-stig-logfile-mode


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_logfile_mode — DISA STIG V-215323.
  # Log files under /var/log and /var/adm must be mode 0640 or less
  # permissive.  PARTIAL-AUTO: the decidable sub-condition is the listing
  # mode check.  Whether a subsystem's less-stringent permissions are
  # documented with the ISSO is not observable from ls -lL; a synthetic
  # overlay may supply PTXRAY_ISSO_EXCEPTION= so that branch is proven
  # without claiming live evidence, and that branch is a typed refusal
  # rather than a silent PASS.
  typeset SLM_RAW SLM_RC SLM_PARSED SLM_PARSE_RC SLM_STATUS SLM_OBSERVED
  typeset SLM_MEANING SLM_FIX SLM_TOKEN SLM_PATH SLM_MODE SLM_PARTIAL SLM_UNREAD

  SLM_RAW=$(aixv logfile_mode ls -lL /var/log /var/log/syslog /var/adm)
  SLM_RC=$?
  SLM_PARSED=""
  SLM_STATUS=""
  SLM_OBSERVED=""
  ...
  if [ "$SLM_RC" -eq 1 ] || [ "$SLM_RC" -eq 2 ]; then
    if [ -n "$SLM_RAW" ]; then
      SLM_PARTIAL=1
      SLM_UNREAD=$(printf '%s\n' "$SLM_RAW" | awk '
        BEGIN { n = 0 }
        {
          if (index($0, "Permission denied") > 0) n++
          else if (index($0, "file access permissions do not allow") > 0) n++
          else if (index($0, "cannot open") > 0) n++
          else if (index($0, "Cannot open") > 0) n++
          else if (index($0, "cannot access") > 0) n++
        }
        END { print n+0 }
      ')
    fi
  ...
  if [ "$SLM_RC" -ne 0 ] && [ "$SLM_PARTIAL" -eq 0 ]; then
    SLM_STATUS=NOT_ASSESSED
    SLM_OBSERVED="not assessed - ls -lL /var/log /var/log/syslog /var/adm failed (rc=$SLM_RC)"
  elif [ -z "$SLM_RAW" ]; then
    SLM_STATUS=NOT_ASSESSED
    SLM_OBSERVED="not assessed - ls -lL /var/log /var/log/syslog /var/adm returned no output; this is a swallowed error, not a verified empty subject"
  else
    SLM_PARSED=$(printf '%s\n' "$SLM_RAW" | awk -v partial="$SLM_PARTIAL" '
      BEGIN { bad = 0; seen_log = 0; seen_adm = 0; missing_log = 0; missing_adm = 0; fail_path = ""; manual = 0 }
      {
        line = $0
        if (line ~ /^[ \t]*$/) next
        if (index(line, "PTXRAY_ISSO_EXCEPTION=") > 0) { manual = 1; next }
        if (index(line, "/var/adm does not exist") > 0 || index(line, "/var/adm: No such file") > 0) {
          missing_adm = 1
          next
        }
        if (index(line, "/var/log/syslog does not exist") > 0 || index(line, "/var/log/syslog: No such file") > 0) next
        if ((index(line, "/var/log does not exist") > 0 && index(line, "/var/log/") == 0) || index(line, "/var/log: No such file") > 0) {
          missing_log = 1
          next
        }
        if (index(line, "Permission denied") > 0 || index(line, "file access permissions do not allow") > 0 || index(line, "cannot open") > 0 || index(line, "Cannot open") > 0 || index(line, "cannot access") > 0) next
        if (line ~ /:$/ && substr(line, 1, 1) == "/") {
          curdir = line
          sub(/:$/, "", curdir)
          if (curdir == "/var/log" || index(curdir, "/var/log/") == 1) seen_log = 1
          if (curdir == "/var/adm" || index(curdir, "/var/adm/") == 1) seen_adm = 1
          next
        }
        if ($1 == "total") next
        mode = $1
        ...
        if (type != "-" && type != "d" && type != "l" && type != "c" && type != "b" && type != "p" && type != "s") {
          bad = 1
          next
        }
        ...
        if (type != "-") next
        if (substr($NF, 1, 1) == "/") { path = $NF }
        else if (curdir == "") { path = $NF }
        else { path = curdir "/" $NF }
        if (index(path, "/var/log/") == 1 || path == "/var/log") seen_log = 1
        if (index(path, "/var/adm/") == 1 || path == "/var/adm") seen_adm = 1
        owner_x = (substr(mode, 4, 1) != "-")
        group_w = (substr(mode, 6, 1) == "w")
        group_x = (substr(mode, 7, 1) != "-")
        other_r = (substr(mode, 8, 1) != "-")
        other_w = (substr(mode, 9, 1) != "-")
        other_x = (substr(mode, 10, 1) != "-")
        if (owner_x || group_w || group_x || other_r || other_w || other_x) {
          if (fail_path == "") {
            fail_path = path
            fail_mode = mode
          }
      ...
      END {
        if (manual) { print "MANUAL"; exit 0 }
        if (bad) exit 1
        if (partial) {
          if (fail_path != "") { print "FAIL"; print fail_path; print fail_mode; exit 0 }
          if (missing_log || missing_adm) { print "ABSENT"; exit 0 }
          print "PASS"
          exit 0
        }
        if (missing_log || missing_adm || !seen_log || !seen_adm) { print "ABSENT"; exit 0 }
        if (fail_path != "") { print "FAIL"; print fail_path; print fail_mode; exit 0 }
        print "PASS"
      }
    ')
    SLM_PARSE_RC=$?
    if [ "$SLM_PARSE_RC" -ne 0 ]; then
      SLM_STATUS=NOT_ASSESSED
      SLM_OBSERVED="not assessed - ls -lL /var/log /var/log/syslog /var/adm output was unparseable"
    else
      SLM_TOKEN=$(printf '%s\n' "$SLM_PARSED" | sed -n '1p')
      case "$SLM_TOKEN" in
        PASS)
          SLM_STATUS=PASS
          SLM_OBSERVED="all listed log files under /var/log and /var/adm are mode 0640 or less permissive"
          if [ "$SLM_PARTIAL" -eq 1 ]; then
            if [ "$SLM_UNREAD" -gt 0 ]; then
              SLM_OBSERVED="$SLM_OBSERVED; $SLM_UNREAD path(s) unreadable"
            else
              SLM_OBSERVED="$SLM_OBSERVED; some paths unreadable"
            fi
          fi
          ;;
        ABSENT)
          SLM_STATUS=FAIL
          SLM_OBSERVED="required log directory /var/adm or /var/log is absent; a required configuration with a missing subject is unmet"
          ;;
        MANUAL)
          SLM_STATUS=NOT_ASSESSED
          SLM_OBSERVED="not assessed - ISSO documentation of less-stringent subsystem log-file permission exceptions cannot be verified from ls -lL"
          ;;
        FAIL)
          SLM_PATH=$(printf '%s\n' "$SLM_PARSED" | sed -n '2p')
          SLM_MODE=$(printf '%s\n' "$SLM_PARSED" | sed -n '3p')
          SLM_STATUS=FAIL
          SLM_OBSERVED="$SLM_PATH mode $SLM_MODE (more permissive than 0640)"
          ;;
        *)
          SLM_STATUS=NOT_ASSESSED
          SLM_OBSERVED="not assessed - ls -lL /var/log /var/log/syslog /var/adm output was unparseable"
  ...
  case "$SLM_STATUS" in
    PASS)
      SLM_MEANING="AIX log files under /var/log and /var/adm are mode 0640 or less permissive as required by DISA STIG V-215323."
      SLM_FIX="n/a"
      ;;
    FAIL)
      SLM_MEANING="A log file more permissive than 0640 can leak or be altered."
      SLM_FIX="change the log file mode with 'chmod 0640 /path/to/system-log-file'; PTxray only recommends this command, it never runs it."
      ;;
    *)
      SLM_MEANING="PTxray did not obtain trustworthy evidence of AIX log file modes, or the ISSO exception-documentation clause remains manual."
      SLM_FIX="run 'ls -lL /var/log /var/log/syslog /var/adm' as root, document any less-stringent subsystem exceptions with the ISSO, and rerun PTxray."
      ;;
  esac

  add security stig_logfile_mode "AIX log file mode" "$SLM_STATUS" med "$SLM_OBSERVED" \
      "$SLM_MEANING" "$SLM_FIX" "stig:V-215323"
}
