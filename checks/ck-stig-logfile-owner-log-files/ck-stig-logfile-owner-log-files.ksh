...
AIXRAY_TOOL=ck-stig-logfile-owner-log-files


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_logfile_owner_log_files — DISA STIG V-215267.
  # AIX log files must be owned by a system group.  The decidable
  # sub-condition is membership in the authored system-group set (names and
  # GIDs).  A listed log file whose group is a known non-system group is a
  # finding.  A listed log file whose group is outside the authored set and
  # not a known non-system group leaves the site/ISSO system-group policy
  # unproven and renders a typed NOT_ASSESSED; it is never silently passed.
  # Required subjects /var/log and /var/adm: a missing subject is a
  # required-configuration FAIL.  Probe rc 2 with nonempty output is
  # partial evidence: grade the printed rows and, when those rows are
  # clean, carry a WARN-level unreadable-path detail.  rc 1 keeps the
  # legacy probe-failed refusal, including when rows were printed.
  # Empty output with nonzero rc, any other nonzero rc, rc 0 with empty
  # output, or unparseable listings refuse with NOT_ASSESSED.
  typeset LFO_RAW LFO_RC LFO_PARSED LFO_TOKEN LFO_DETAIL LFO_PARTIAL
  typeset LFO_STATUS LFO_OBSERVED LFO_MEANING LFO_FIX

  LFO_RAW=$(aix ls_log_dirs ls -lL /var/log /var/log/syslog /var/adm)
  LFO_RC=$?
  LFO_PARSED=""
  LFO_TOKEN=""
  LFO_DETAIL=""
  ...
  if [ "$LFO_RC" -ne 0 ]; then
    case "$LFO_RC" in
      2)
        [ -n "$LFO_RAW" ] && LFO_PARTIAL=1
        ;;
    esac
    if [ "$LFO_PARTIAL" -eq 0 ]; then
      LFO_TOKEN=PROBE_FAILED
    fi
  elif [ -z "$LFO_RAW" ]; then
    LFO_TOKEN=EMPTY_RC0
  fi
  if [ -z "$LFO_TOKEN" ] && [ -n "$LFO_RAW" ]; then
    LFO_PARSED=$(printf '%s\n' "$LFO_RAW" | awk '
      function is_sys(g) {
        return g == "system" || g == "bin" || g == "sys" || g == "adm" ||
          g == "uucp" || g == "mail" || g == "security" || g == "cron" ||
          g == "printq" || g == "audit" || g == "ecs" ||
          g == "0" || g == "2" || g == "3" || g == "4" || g == "5" ||
          g == "6" || g == "7" || g == "8" || g == "9" || g == "10" ||
          g == "28"
      }
      function is_nonsys(g) {
        if (g == "staff" || g == "usr" || g == "nobody" || g == "users" ||
            g == "guest" || g == "1")
          return 1
        if (g ~ /^[0-9][0-9]*$/ && (g + 0) >= 100)
          return 1
        return 0
      }
      function mark_path(p) {
        if (p == "/var/log" || p ~ /^\/var\/log\//) saw_log = 1
        if (p == "/var/adm" || p ~ /^\/var\/adm\//) saw_adm = 1
      }
      /^[ \t]*$/ { next }
      /^ls:/ { next }
      /does not exist/ { next }
      {
        ...
        if (name == "." || name == "..") next
        if (name ~ /^\//) path = name
        else if (dir != "") path = dir "/" name
        else path = name
        mark_path(path)
        if (t != "-") next
        group = $4
        if (group == "") { bad = 1; next }
        if (is_sys(group)) next
        if (is_nonsys(group)) {
          if (fail_detail == "") fail_detail = path " group=" group
        } else {
          if (manual_detail == "") manual_detail = path " group=" group
        }
      }
      END {
        if (bad) {
          print "UNPARSEABLE#listing"
          exit 0
        }
        miss = ""
        if (!saw_log) miss = "/var/log"
        else if (!saw_adm) miss = "/var/adm"
        if (miss != "") {
          print "MISSING#" miss
          exit 0
        }
        if (fail_detail != "") {
          print "FAIL#" fail_detail
          exit 0
        }
        if (manual_detail != "") {
          print "MANUAL#" manual_detail
          exit 0
        }
        print "PASS#ok"
      }
    ')
    if [ -z "$LFO_PARSED" ]; then
      LFO_TOKEN=UNPARSEABLE
      LFO_DETAIL="listing"
    else
      LFO_TOKEN=${LFO_PARSED%%#*}
      LFO_DETAIL=${LFO_PARSED#*#}
    fi
  fi

  case "$LFO_TOKEN" in
    PROBE_FAILED)
      LFO_STATUS=NOT_ASSESSED
      LFO_OBSERVED="not assessed - log directory probe failed (rc=$LFO_RC)"
      ;;
    EMPTY_RC0)
      LFO_STATUS=NOT_ASSESSED
      LFO_OBSERVED="not assessed - log directory probe returned empty output with rc 0 (a swallowed error, not absence)"
      ;;
    UNPARSEABLE)
      LFO_STATUS=NOT_ASSESSED
      LFO_OBSERVED="not assessed - log directory listing was unparseable"
      ;;
    MISSING)
      LFO_STATUS=FAIL
      LFO_OBSERVED="required log directory $LFO_DETAIL is absent"
      ;;
    FAIL)
      LFO_STATUS=FAIL
      LFO_OBSERVED="$LFO_DETAIL is not owned by a system group"
      ;;
    MANUAL)
      LFO_STATUS=NOT_ASSESSED
      LFO_OBSERVED="not assessed - $LFO_DETAIL is outside the authored system-group set; confirming whether it is a site system group is an ISSO judgement"
      ;;
    PASS)
      LFO_STATUS=PASS
      LFO_OBSERVED="log files use authored system groups"
      if [ "$LFO_PARTIAL" -eq 1 ]; then
        LFO_OBSERVED="$LFO_OBSERVED (some paths unreadable)"
      fi
      ;;
    *)
      LFO_STATUS=NOT_ASSESSED
      LFO_OBSERVED="not assessed - log directory evidence was not decidable"
      ;;
  esac

  case "$LFO_STATUS" in
    PASS)
      LFO_MEANING="Listed log files under /var/log, /var/log/syslog, and /var/adm are owned by an authored system group; DISA STIG V-215267 is satisfied on the decidable system-group sub-condition."
      LFO_FIX="n/a"
      ;;
    FAIL)
      LFO_MEANING="A required log directory is missing, or a listed log file is owned by a group that is not an authored system group, so a non-system group can alter or suppress logs."
      LFO_FIX="chgrp [system_group] /path/to/system-log-file"
      ;;
    *)
      LFO_MEANING="PTxray did not obtain trustworthy log-file group evidence, or the site's system-group policy beyond the authored set remains an ISSO judgement."
      LFO_FIX="verify 'ls -lL /var/log /var/log/syslog /var/adm', confirm the site system-group policy with the ISSO, then rerun PTxray."
      ;;
  esac

  add security stig_logfile_owner_log_files "AIX log files system group" \
    "$LFO_STATUS" med "$LFO_OBSERVED" \
    "$LFO_MEANING" "$LFO_FIX" "stig:V-215267"
}
