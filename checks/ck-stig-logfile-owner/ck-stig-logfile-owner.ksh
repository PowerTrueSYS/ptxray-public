...
AIXRAY_TOOL=ck-stig-logfile-owner


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_logfile_owner — DISA STIG V-215266.
  # PARTIAL-AUTO: ls -lL owners vs an authored system-account set is
  # decidable. A name outside that set is the ISSO remainder.
  typeset LFO_RAW LFO_RC LFO_CLASS LFO_PARSE_RC LFO_KIND LFO_REST
  typeset LFO_PATH LFO_OWNER LFO_STATUS LFO_SEV
  typeset LFO_OBSERVED LFO_MEANING LFO_FIX
  typeset LFO_INBAND LFO_PARTIAL
  ...
  # rc 1/2 with listing-only stdout is partial evidence (unreadable or
  # vanished paths); grade the rows. In-band `ls:` diagnostics on stdout
  # are not a listing-only record (real ls writes those to stderr) and
  # keep the original refusal. Refuse also when there are no rows, or
  # the probe rc is outside 0/1/2.
  if [ -n "$LFO_RAW" ]; then
    LFO_INBAND=$(printf '%s\n' "$LFO_RAW" | awk '/^ls:/ { print 1; exit }')
  fi
  if [ -z "$LFO_RAW" ]; then
    LFO_STATUS=NOT_ASSESSED
    if [ "$LFO_RC" -ne 0 ]; then
      LFO_OBSERVED="not assessed - log directory listing failed (rc=$LFO_RC)"
    else
      LFO_OBSERVED="not assessed - log directory listing returned no output"
    fi
  elif [ "$LFO_RC" -ne 0 ]; then
    case "$LFO_RC" in
      1|2)
        if [ -z "$LFO_INBAND" ]; then
          LFO_PARTIAL=1
        fi
        ;;
    esac
  ...
  if [ -z "$LFO_STATUS" ]; then
    LFO_CLASS=$(printf '%s\n' "$LFO_RAW" | awk '
      BEGIN {
        sysnames = " root daemon bin sys adm uucp mail nobody lpd nuucp invscout snapp ipsec sshd pconsole esaadmin lp printq imnadm "
      }
      function classify(owner,    uid) {
        if (index(sysnames, " " owner " ")) return "sys"
        if (owner ~ /^[0-9][0-9]*$/) {
          if (owner == "4294967294") return "sys"
          uid = owner + 0
          if (uid > 128) return "nonsystem"
          return "sys"
        }
        if (owner ~ /^[A-Za-z_][A-Za-z0-9_-]*$/) return "manual"
        return "bad"
      }
      /^[ \t]*$/ { next }
      /^ls:/ { next }
      ...
      NF == 1 && $0 ~ /^\/.*:$/ {
        dir = $0
        sub(/:$/, "", dir)
        if (dir == "/var/log") seen_log = 1
        if (dir == "/var/adm") seen_adm = 1
        cur = dir
        next
      }
      {
        ...
        if (path == "/var/log") seen_log = 1
        if (path == "/var/adm") seen_adm = 1
        kind = classify($3)
        if (kind == "bad") { bad = 1; next }
        if (kind == "nonsystem" && ns_path == "") {
          ns_path = path
          ns_owner = $3
        }
        if (kind == "manual" && man_path == "") {
          man_path = path
          man_owner = $3
        }
      }
      END {
        ...
        if (!seen_log || !seen_adm) {
          miss = ""
          if (!seen_log) miss = "/var/log"
          if (!seen_adm) {
            if (miss != "") miss = miss " /var/adm"
            else miss = "/var/adm"
          }
          print "missing " miss
          exit
        ...
        if (ns_path != "") {
          print "nonsystem " ns_path " " ns_owner
          exit
        }
        if (man_path != "") {
          print "manual " man_path " " man_owner
          exit
        }
        print "ok"
      }
    ')
    LFO_PARSE_RC=$?
    if [ "$LFO_PARSE_RC" -ne 0 ] || [ -z "$LFO_CLASS" ]; then
      LFO_STATUS=NOT_ASSESSED
      ...
      case "$LFO_KIND" in
        ok)
          LFO_STATUS=PASS
          LFO_OBSERVED="log files owned by authored system accounts"
        ...
        missing)
          LFO_STATUS=FAIL
          LFO_OBSERVED="required log directory missing: $LFO_REST"
          ;;
        nonsystem)
          LFO_OWNER=${LFO_REST##* }
          LFO_PATH=${LFO_REST% *}
          LFO_STATUS=FAIL
          LFO_OBSERVED="log file not owned by a system account: $LFO_PATH owner=$LFO_OWNER"
          ;;
        manual)
          LFO_OWNER=${LFO_REST##* }
          LFO_PATH=${LFO_REST% *}
          LFO_STATUS=NOT_ASSESSED
          LFO_OBSERVED="not assessed - owner $LFO_OWNER on $LFO_PATH is outside the authored system-account set; ISSO must decide whether that account is a system account"
          ;;
        *)
          LFO_STATUS=NOT_ASSESSED
          LFO_OBSERVED="not assessed - log directory listing was unparseable"
  ...
  case "$LFO_STATUS" in
    PASS)
      LFO_MEANING="Log files under /var/log and /var/adm are owned by authored system accounts."
      LFO_FIX="n/a"
      ;;
    FAIL)
      LFO_MEANING="A required log directory is missing or a log file is not owned by a system account, so logged activity can be altered or hidden."
      LFO_FIX="chown [system_account] /path/to/system-log-file"
      ;;
    NOT_APPLICABLE)
      LFO_MEANING="This log-file owner requirement is not applicable."
      LFO_FIX="n/a"
      ;;
    *)
      LFO_MEANING="PTxray did not obtain a complete, decidable owner listing for AIX log files."
      LFO_FIX="verify 'ls -lL /var/log /var/log/syslog /var/adm', have the ISSO classify any owner outside the authored system-account set, then rerun PTxray."
      ;;
  esac

  case "$LFO_STATUS" in
    NOT_APPLICABLE) LFO_SEV=low ;;
    *) LFO_SEV=med ;;
  esac

  add security stig_logfile_owner "AIX log file owner is a system account" \
    "$LFO_STATUS" "$LFO_SEV" "$LFO_OBSERVED" \
    "$LFO_MEANING" "$LFO_FIX" "stig:V-215266"
}
