...
AIXRAY_TOOL=ck-stig-lppchk-cron


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # lppchk_cron — DISA STIG V-215426.  The AIX package management tool must
  # be run daily (via the root crontab) to verify installed software.  Only the
  # crontab listing is read; the fix text is a recommendation and is never
  # executed by PTxray.
  typeset LPP_RAW LPP_RC LPP_STATUS LPP_OBSERVED LPP_MEANING LPP_FIX

  LPP_RAW=$(aix crontab_l crontab -l)
  LPP_RC=$?

  if [ "$LPP_RC" -ne 0 ]; then
    LPP_STATUS=NOT_ASSESSED
    LPP_OBSERVED="not assessed - crontab listing failed (rc=$LPP_RC)"
  elif [ -z "$LPP_RAW" ]; then
    LPP_STATUS=NOT_ASSESSED
    LPP_OBSERVED="not assessed - crontab listing came back empty with rc=0, so no trustworthy job evidence exists"
  elif printf '%s\n' "$LPP_RAW" | awk '!/^[[:space:]]*#/ && NF' | grep lppchk >/dev/null; then
    LPP_STATUS=PASS
    LPP_OBSERVED="root crontab schedules a daily lppchk job"
  else
    LPP_STATUS=FAIL
    LPP_OBSERVED="root crontab has no lppchk job"
  fi

  case "$LPP_STATUS" in
    PASS)
      LPP_MEANING="A crontab entry runs lppchk, so installed software is verified daily as DISA STIG V-215426 requires."
      LPP_FIX="n/a"
      ;;
    FAIL)
      LPP_MEANING="No crontab entry runs lppchk, so installed software is not verified daily and corrupt or mismatched filesets can go unnoticed."
      LPP_FIX="run 'crontab -e' as root to add a daily lppchk job (PTxray only recommends this command and never edits the crontab)."
      ;;
    *)
      LPP_MEANING="PTxray did not obtain trustworthy evidence that a daily lppchk crontab job exists."
      LPP_FIX="verify the root crontab as root and rerun PTxray."
      ;;
  esac

  add security stig_lppchk_cron "AIX lppchk daily cron" \
    "$LPP_STATUS" med "$LPP_OBSERVED" \
    "$LPP_MEANING" "$LPP_FIX" "stig:V-215426"
}
