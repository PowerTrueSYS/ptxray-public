...
AIXRAY_TOOL=ck-stig-lsitab-lpd


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_lsitab_lpd — DISA STIG V-215349.
  # Prohibition rule: when this AIX system does not act as a remote print
  # server for other servers, the lpd daemon must be disabled.  The compliant
  # state is that no lpd entry exists in /etc/inittab.  lsitab exits 1 with no
  # stdout when the named entry is absent, which is the determinate absence
  # signature; any non-empty stdout is an lpd inittab entry and is a finding.
  typeset LSD_RAW LSD_RC LSD_STATUS LSD_OBSERVED LSD_MEANING LSD_FIX

  LSD_RAW=$(aix lsitab_lpd lsitab lpd)
  LSD_RC=$?

  if [ -n "$LSD_RAW" ]; then
    LSD_STATUS=FAIL
    LSD_OBSERVED="inittab entry present: $(printf '%s' "$LSD_RAW" | sed -n '1p')"
  elif [ "$LSD_RC" -eq 1 ]; then
    LSD_STATUS=PASS
    LSD_OBSERVED="no lpd entry in /etc/inittab (lsitab lpd found none, rc 1)"
  elif [ "$LSD_RC" -eq 0 ]; then
    LSD_STATUS=NOT_ASSESSED
    LSD_OBSERVED="not assessed - lsitab lpd returned empty output with rc 0 (swallowed error, not absence)"
  else
    LSD_STATUS=NOT_ASSESSED
    LSD_OBSERVED="not assessed - lsitab lpd probe failed (rc=$LSD_RC)"
  fi

  case "$LSD_STATUS" in
    PASS)
      LSD_MEANING="The lpd daemon is not configured in /etc/inittab, so this host does not act as a remote print server."
      LSD_FIX="n/a"
      ;;
    FAIL)
      LSD_MEANING="An lpd entry exists in /etc/inittab, so the line printer daemon is enabled and can accept remote print jobs on a host that is not a remote print server."
      LSD_FIX="remove the entry with 'rmitab lpd' and reload inittab with 'telinit q'; PTxray only recommends this command."
      ;;
    *)
      LSD_MEANING="PTxray did not obtain trustworthy evidence about the lpd inittab entry."
      LSD_FIX="verify the /etc/inittab lpd state as root, then rerun PTxray."
      ;;
  esac

  add security stig_lsitab_lpd "lpd daemon disabled" \
    "$LSD_STATUS" med "$LSD_OBSERVED" \
    "$LSD_MEANING" "$LSD_FIX" "stig:V-215349"
}
