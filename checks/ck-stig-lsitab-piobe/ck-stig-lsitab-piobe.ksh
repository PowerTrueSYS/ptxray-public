...
AIXRAY_TOOL=ck-stig-lsitab-piobe


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_lsitab_piobe — DISA STIG V-215350.  If the AIX system does not support
  # either local or remote printing, the piobe (print job backend) service must
  # be disabled.  The probe is the single non-root, read-only read 'lsitab piobe'
  # of /etc/inittab: lsitab prints the matching inittab record and exits 0 when
  # the piobe entry is present; it exits 1 with no stdout when the entry is
  # absent.  Any stdout at all is the finding the STIG names.  Determinate
  # absence (the documented 'not found' exit 1 with empty stdout) is PASS by the
  # svcoff absence-is-compliant convention.  An empty result with exit 0 is a
  # swallowed error, and any other non-zero exit is a probe failure: both are
  # NOT_ASSESSED, never PASS.
  typeset LSP_RAW LSP_RC LSP_STATUS LSP_OBSERVED LSP_MEANING LSP_FIX

  LSP_RAW=$(aix lsitab_piobe lsitab piobe)
  LSP_RC=$?

  if [ -n "$LSP_RAW" ]; then
    LSP_STATUS=FAIL
    LSP_OBSERVED="piobe inittab entry present: $LSP_RAW"
  elif [ "$LSP_RC" -eq 1 ]; then
    LSP_STATUS=PASS
    LSP_OBSERVED="piobe not present in /etc/inittab (lsitab returned no entry)"
  elif [ "$LSP_RC" -eq 0 ]; then
    LSP_STATUS=NOT_ASSESSED
    LSP_OBSERVED="not assessed - lsitab piobe returned no output with exit 0; a swallowed error, not an absent entry"
  else
    LSP_STATUS=NOT_ASSESSED
    LSP_OBSERVED="not assessed - lsitab piobe probe failed (rc=$LSP_RC)"
  fi

  case "$LSP_STATUS" in
    PASS)
      LSP_MEANING="The piobe print job backend is not registered in /etc/inittab, so init cannot start it; a non-printing host exposes no print service."
      LSP_FIX="n/a"
      ;;
    FAIL)
      LSP_MEANING="The piobe inittab entry is present, so init starts the print job backend; on a host with no printing role this is unnecessary service exposure."
      LSP_FIX="remove the entry with 'rmitab piobe' and reread inittab with 'telinit q'; PTxray only recommends these commands."
      ;;
    *)
      LSP_MEANING="PTxray did not obtain trustworthy evidence about the piobe inittab entry."
      LSP_FIX="run 'lsitab piobe' as a user who can read /etc/inittab, then rerun PTxray."
      ;;
  esac

  add security stig_lsitab_piobe "piobe inittab entry" \
    "$LSP_STATUS" med "$LSP_OBSERVED" \
    "$LSP_MEANING" "$LSP_FIX" "stig:V-215350"
}
