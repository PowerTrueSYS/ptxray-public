...
AIXRAY_TOOL=ck-stig-lsitab-qdaemon


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_lsitab_qdaemon — DISA STIG V-215348.  The qdaemon inittab entry starts
  # the print spooler at boot; when printing is not required the entry must be
  # absent.  lsitab prints the named inittab row and returns rc 0 when the entry
  # exists, and returns rc 1 with empty stdout when it does not — that rc 1 /
  # empty shape is the determinate-absence PASS.  rc 0 with any row is the
  # finding.  rc 0 with empty output (a swallowed error, which lsitab never
  # produces) or any other probe failure fails closed as NOT_ASSESSED.
  typeset LQ_RAW LQ_RC LQ_STATUS LQ_OBSERVED LQ_MEANING LQ_FIX

  LQ_RAW=$(aix lsitab_qdaemon lsitab qdaemon)
  LQ_RC=$?
  ...
  case "$LQ_RC" in
    0)
      if [ -n "$LQ_RAW" ]; then
        LQ_STATUS=FAIL
        LQ_OBSERVED="qdaemon inittab entry present: $(printf '%s\n' "$LQ_RAW" | sed -n '1p')"
      else
        LQ_STATUS=NOT_ASSESSED
        LQ_OBSERVED="not assessed - lsitab qdaemon returned rc 0 with empty output (swallowed error, not absence)"
      fi
      ;;
    1)
      if [ -z "$LQ_RAW" ]; then
        LQ_STATUS=PASS
        LQ_OBSERVED="no qdaemon entry in /etc/inittab (lsitab rc 1, empty output)"
      else
        LQ_STATUS=NOT_ASSESSED
        LQ_OBSERVED="not assessed - lsitab qdaemon returned rc 1 with unexpected output"
      fi
      ;;
    *)
      LQ_STATUS=NOT_ASSESSED
      LQ_OBSERVED="not assessed - lsitab qdaemon probe failed (rc=$LQ_RC)"
      ;;
  esac

  case "$LQ_STATUS" in
    PASS)
      LQ_MEANING="No qdaemon entry exists in /etc/inittab, so the print spooler is not started at boot, as required by DISA STIG V-215348 when printing is not required."
      LQ_FIX="n/a"
      ;;
    FAIL)
      LQ_MEANING="A qdaemon entry in /etc/inittab starts the print spooler at boot even though printing is not required."
      LQ_FIX="remove the qdaemon inittab entry with 'rmitab qdaemon' and reload inittab with 'telinit q'; PTxray only recommends this command, it does not run it."
      ;;
    *)
      LQ_MEANING="PTxray did not obtain trustworthy evidence of the qdaemon inittab entry state."
      LQ_FIX="verify with 'lsitab qdaemon' whether a qdaemon entry exists in /etc/inittab, then rerun PTxray."
      ;;
  esac

  add security stig_lsitab_qdaemon "qdaemon inittab entry" \
    "$LQ_STATUS" med "$LQ_OBSERVED" \
    "$LQ_MEANING" "$LQ_FIX" "stig:V-215348"
}
