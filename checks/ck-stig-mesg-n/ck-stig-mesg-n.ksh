...
AIXRAY_TOOL=ck-stig-mesg-n


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_mesg_n — DISA STIG V-215342 (AIX7-00-003036).
  # The six AIX global initialization files must contain "mesg n" (or
  # "mesg -n") so users are denied write access to the terminal, and none may
  # carry "mesg y".  Every listed file is a required subject: a file missing
  # from the grep output (absent, or present without any mesg line) is a
  # finding, exactly as an absent csh file is.  grep rc 1 with empty stdout
  # is POSIX "no lines selected": determinate absence, a FAIL.  Other empty
  # output (rc 0 with no rows, rc 2, rc 127) remains a refusal.
  typeset MN_RAW MN_RC MN_PARSE MN_PARSE_RC MN_REASON MN_PROBLEMS
  typeset MN_STATUS MN_OBSERVED MN_MEANING MN_FIX MN_EMPTY_NOMATCH

  MN_RAW=$(aix mesg_init_files grep "mesg" /etc/profile /etc/environment /etc/security/environ /etc/security/.profile /etc/csh.login /etc/csh.cshrc)
  MN_RC=$?
  MN_REASON=""
  MN_EMPTY_NOMATCH=0
  ...
  if [ -z "$MN_RAW" ]; then
    if [ "$MN_RC" -eq 1 ]; then
      ...
      MN_REASON="mesg probe returned no rows (rc=$MN_RC)"
    fi
  fi

  if [ -z "$MN_REASON" ]; then
    MN_PARSE=$(printf '%s\n' "$MN_RAW" | awk -v empty_nomatch="$MN_EMPTY_NOMATCH" '
      BEGIN {
        nreq = split("/etc/profile /etc/environment /etc/security/environ /etc/security/.profile /etc/csh.login /etc/csh.cshrc", req, " ")
      ...
      /^[ \t]*$/ { next }
      {
        ...
        if (!(file in known)) { next }
        rest = substr(line, p1 + 1)
        rec++
        seen[file] = 1
        if (rest ~ /mesg[ \t]+-?n([ \t#]|$)/) has_n[file] = 1
        if (rest ~ /mesg[ \t]+-?y([ \t#]|$)/) has_y[file] = 1
      }
      END {
        if (rec == 0 && empty_nomatch != 1) { print "unparseable"; exit 1 }
        pc = 0
        problems = ""
        sep = ""
        for (i = 1; i <= nreq; i++) {
          f = req[i]
          if (f in has_y) { pc++; problems = problems sep f " contains mesg y"; sep = "; " }
          else if (f in seen) {
            if (!(f in has_n)) { pc++; problems = problems sep f " lacks mesg n"; sep = "; " }
          }
          else { pc++; problems = problems sep "missing required global init file " f; sep = "; " }
        }
        if (pc == 0) print "ok"
        else print "bad:" problems
      }')
    MN_PARSE_RC=$?
    if [ "$MN_PARSE_RC" -ne 0 ] || [ -z "$MN_PARSE" ]; then
      MN_REASON="mesg probe output was unparseable"
    fi
  fi

  if [ -n "$MN_REASON" ]; then
    MN_STATUS=NOT_ASSESSED
    MN_OBSERVED="not assessed - $MN_REASON"
    MN_MEANING="PTxray did not obtain trustworthy evidence that every global initialization file denies terminal write access."
    MN_FIX="rerun the mesg probe as root, confirm each of the six global initialization files exists and is readable, then rerun PTxray."
  elif [ "$MN_PARSE" = "ok" ]; then
    MN_STATUS=PASS
    MN_OBSERVED="all global initialization files contain mesg n (or mesg -n) and none contain mesg y"
    MN_MEANING="Every global initialization file denies other users write access to the terminal, so no walk-up user can capture or disturb a session."
    MN_FIX="n/a"
  else
    MN_PROBLEMS=${MN_PARSE#bad:}
    MN_STATUS=FAIL
    MN_OBSERVED="global init files not compliant: $MN_PROBLEMS"
    MN_MEANING="At least one global initialization file does not deny terminal write access, letting another user write to the terminal and capture the session."
    MN_FIX="edit the global initialization files that do not contain mesg n and add the line: mesg n"
  fi

  add security stig_mesg_n "Global init files deny terminal write access (mesg n)" \
    "$MN_STATUS" med "$MN_OBSERVED" \
    "$MN_MEANING" "$MN_FIX" "stig:V-215342"
}
