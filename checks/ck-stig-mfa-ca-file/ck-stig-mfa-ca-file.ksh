...
AIXRAY_TOOL=ck-stig-mfa-ca-file


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_mfa_ca_file — DISA STIG V-215440.
  # A missing pam_pmfa.conf or server_ca.pem is a finding; uncertain probes refuse.
  # grep -i "trustedcas" /etc/security/pmfa/pam_pmfa.conf | grep -v '#'
  # ls -ld /etc/security/pmfa/pam_pmfa.conf
  # ls -la /<path_to_file>/server_ca.pem
  typeset MFA_CA_CONF MFA_CA_CONF_RC MFA_CA_PATH MFA_CA_PARSE_RC
  typeset MFA_CA_LS MFA_CA_LS_RC MFA_CA_KIND MFA_CA_BASE
  typeset MFA_CA_CONF_LS MFA_CA_CONF_LS_RC MFA_CA_CONF_ABSENT
  typeset MFA_CA_STATUS MFA_CA_OBSERVED MFA_CA_MEANING MFA_CA_FIX

  MFA_CA_CONF=$(aix pmfa_trustedcas grep -i "trustedcas" /etc/security/pmfa/pam_pmfa.conf)
  MFA_CA_CONF_RC=$?
  MFA_CA_PATH=""
  MFA_CA_STATUS=""
  MFA_CA_OBSERVED=""
  MFA_CA_CONF_ABSENT=0

  # grep/ls rc 1/2 with no stdout is a completed probe: AIX writes absence
  # diagnostics to stderr, which aix() discards. Grade that as absence.
  if [ -z "$MFA_CA_CONF" ]; then
    case "$MFA_CA_CONF_RC" in
      1|2)
        MFA_CA_CONF_LS=$(aix pmfa_conf_ls ls -ld /etc/security/pmfa/pam_pmfa.conf)
        MFA_CA_CONF_LS_RC=$?
        case "$MFA_CA_CONF_LS" in
          *"does not exist"*|*"No such file"*)
            MFA_CA_CONF_ABSENT=1
            ;;
          "")
            case "$MFA_CA_CONF_LS_RC" in
              1|2) MFA_CA_CONF_ABSENT=1 ;;
            esac
            ;;
        esac
        ;;
  ...
  if [ "$MFA_CA_CONF_ABSENT" -eq 1 ]; then
    MFA_CA_STATUS=FAIL
    MFA_CA_OBSERVED="pam_pmfa.conf is absent"
  elif [ -n "$MFA_CA_CONF" ]; then
    # rc 1/2 with rows is partial evidence; grade the captured TRUSTEDCAS line.
    MFA_CA_PATH=$(printf '%s\n' "$MFA_CA_CONF" | awk '
      /#/ { next }
      /^[ \t]*$/ { next }
      {
        line = $0
        ...
        sub(/^[ \t]*/, "", val)
        sub(/[ \t]*$/, "", val)
        if (key != "trustedcas") { bad = 1; next }
        matches++
        path = val
      }
      END {
        if (bad) exit 1
        if (matches == 0 || path == "") exit 2
        if (matches != 1) exit 1
        if (path !~ /^\//) exit 1
        print path
      }
    ')
    MFA_CA_PARSE_RC=$?
    if [ "$MFA_CA_PARSE_RC" -eq 2 ] || [ -z "$MFA_CA_PATH" ]; then
      MFA_CA_STATUS=FAIL
      MFA_CA_OBSERVED="pam_pmfa.conf is absent of a TRUSTEDCAS path"
    elif [ "$MFA_CA_PARSE_RC" -ne 0 ]; then
      MFA_CA_STATUS=NOT_ASSESSED
      MFA_CA_OBSERVED="not assessed - TRUSTEDCAS line was unparseable"
    else
      MFA_CA_BASE=${MFA_CA_PATH##*/}
      if [ "$MFA_CA_BASE" != "server_ca.pem" ]; then
        MFA_CA_STATUS=FAIL
        MFA_CA_OBSERVED="TRUSTEDCAS=$MFA_CA_PATH is not server_ca.pem"
      else
        MFA_CA_LS=$(aix server_ca_ls ls -la "$MFA_CA_PATH")
        MFA_CA_LS_RC=$?
        case "$MFA_CA_LS" in
          *"does not exist"*|*"No such file"*)
            MFA_CA_STATUS=FAIL
            MFA_CA_OBSERVED="server_ca.pem is absent"
            ;;
        esac
        if [ -z "$MFA_CA_STATUS" ]; then
          if [ -n "$MFA_CA_LS" ]; then
            MFA_CA_KIND=$(printf '%s\n' "$MFA_CA_LS" | awk '
              NF {
                records++
                mode = $1
                if (length("" mode) == 11 && substr(mode, 11, 1) == "+") {
                  mode = substr(mode, 1, 10)
                }
                if (records != 1 || length("" mode) != 10) { bad = 1; next }
                kind = substr(mode, 1, 1)
              }
              END {
            ...
            ')
            MFA_CA_PARSE_RC=$?
            if [ "$MFA_CA_PARSE_RC" -ne 0 ] || [ -z "$MFA_CA_KIND" ]; then
              MFA_CA_STATUS=NOT_ASSESSED
              MFA_CA_OBSERVED="not assessed - server_ca.pem ls output was unparseable"
            elif [ "$MFA_CA_KIND" = file ]; then
              MFA_CA_STATUS=PASS
              MFA_CA_OBSERVED="TRUSTEDCAS=$MFA_CA_PATH is a regular file"
              case "$MFA_CA_LS_RC" in
                1|2) MFA_CA_OBSERVED="$MFA_CA_OBSERVED; some paths unreadable" ;;
              esac
            else
              MFA_CA_STATUS=FAIL
              MFA_CA_OBSERVED="server_ca.pem is absent as a regular file"
            fi
          else
            case "$MFA_CA_LS_RC" in
              1|2)
              ...
              0)
                MFA_CA_STATUS=NOT_ASSESSED
                MFA_CA_OBSERVED="not assessed - server_ca.pem ls probe returned no output"
                ;;
              *)
                MFA_CA_STATUS=NOT_ASSESSED
                MFA_CA_OBSERVED="not assessed - server_ca.pem ls probe failed (rc=$MFA_CA_LS_RC)"
                ;;
            esac
          fi
        fi
  ...
  elif [ "$MFA_CA_CONF_RC" -ne 0 ]; then
    MFA_CA_STATUS=NOT_ASSESSED
    MFA_CA_OBSERVED="not assessed - TRUSTEDCAS probe failed (rc=$MFA_CA_CONF_RC)"
  else
    MFA_CA_STATUS=NOT_ASSESSED
    MFA_CA_OBSERVED="not assessed - TRUSTEDCAS probe returned no output"
  fi

  case "$MFA_CA_STATUS" in
    PASS)
      MFA_CA_MEANING="TRUSTEDCAS names a regular server_ca.pem file the MFA authenticator can use."
      MFA_CA_FIX="n/a"
      ;;
    FAIL)
      MFA_CA_MEANING="The MFA authenticator has no usable server_ca.pem file."
      MFA_CA_FIX="Configure the system to use a valid \"server_ca.pem\" file."
      ;;
    *)
      MFA_CA_MEANING="PTxray did not obtain trustworthy evidence for the MFA CA file."
      MFA_CA_FIX="verify pam_pmfa.conf TRUSTEDCAS and that server_ca.pem is readable, then rerun PTxray."
      ;;
  esac

  add security stig_mfa_ca_file "MFA server_ca.pem file" \
    "$MFA_CA_STATUS" med "$MFA_CA_OBSERVED" "$MFA_CA_MEANING" "$MFA_CA_FIX" "stig:V-215440"
}
