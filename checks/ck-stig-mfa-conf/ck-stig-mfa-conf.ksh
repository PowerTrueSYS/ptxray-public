...
AIXRAY_TOOL=ck-stig-mfa-conf


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_mfa_conf — DISA STIG V-215439.  This fragment only reads pmfa policy.
  ...
  # Executed as one grep so a file-read failure (rc 2) is not masked as no-match (rc 1).
  typeset PMFA_TRUSTED PMFA_URL PMFA_VERSION PMFA_PROMPT
  typeset PMFA_TRUSTED_RC PMFA_URL_RC PMFA_VERSION_RC PMFA_PROMPT_RC
  typeset PMFA_REASON PMFA_STATE PMFA_OBSERVED PMFA_MEANING PMFA_FIX

  PMFA_REASON=""
  # grep rc 1 is no-match: determinate absence of the setting, not a probe failure.
  PMFA_TRUSTED=$(aix pmfa_trustedcas sh -c "grep -i \"^[^#]*trustedcas\" /etc/security/pmfa/pam_pmfa.conf")
  PMFA_TRUSTED_RC=$?
  # grep rc 2 with no stdout is a missing pam_pmfa.conf (no MFA), not a probe
  # failure. Refuse only rc 127 (grep missing) or rc 2 with output (unreadable).
  if [ "$PMFA_TRUSTED_RC" -eq 2 ] && [ -z "$PMFA_TRUSTED" ]; then
    PMFA_REASON="pmfa.conf absent: no MFA configured"
  elif [ "$PMFA_TRUSTED_RC" -ne 1 ] && { [ "$PMFA_TRUSTED_RC" -ne 0 ] || [ -z "$PMFA_TRUSTED" ]; }; then
    PMFA_REASON="TRUSTEDCAS probe failed or returned no evidence (rc=$PMFA_TRUSTED_RC)"
  fi
  PMFA_URL=$(aix pmfa_url sh -c "grep -i \"^[^#]*mfa-url\" /etc/security/pmfa/pam_pmfa.conf")
  PMFA_URL_RC=$?
  if [ -z "$PMFA_REASON" ] && [ "$PMFA_URL_RC" -ne 1 ] && { [ "$PMFA_URL_RC" -ne 0 ] || [ -z "$PMFA_URL" ]; }; then
    PMFA_REASON="MFA-URL probe failed or returned no evidence (rc=$PMFA_URL_RC)"
  fi
  PMFA_VERSION=$(aix pmfa_server_version sh -c "grep -i \"^[^#]*server-version\" /etc/security/pmfa/pam_pmfa.conf")
  PMFA_VERSION_RC=$?
  if [ -z "$PMFA_REASON" ] && [ "$PMFA_VERSION_RC" -ne 1 ] && { [ "$PMFA_VERSION_RC" -ne 0 ] || [ -z "$PMFA_VERSION" ]; }; then
    PMFA_REASON="SERVER-VERSION probe failed or returned no evidence (rc=$PMFA_VERSION_RC)"
  fi
  PMFA_PROMPT=$(aix pmfa_ctc_prompt sh -c "grep -i \"^[^#]*ctc-prompt\" /etc/security/pmfa/pam_pmfa.conf")
  PMFA_PROMPT_RC=$?
  if [ -z "$PMFA_REASON" ] && [ "$PMFA_PROMPT_RC" -ne 1 ] && { [ "$PMFA_PROMPT_RC" -ne 0 ] || [ -z "$PMFA_PROMPT" ]; }; then
    PMFA_REASON="CTC-PROMPT-ONLY probe failed or returned no evidence (rc=$PMFA_PROMPT_RC)"
  fi

  if [ "$PMFA_REASON" = "pmfa.conf absent: no MFA configured" ]; then
    PMFA_STATE=FAIL
    PMFA_OBSERVED="$PMFA_REASON"
  elif [ -n "$PMFA_REASON" ]; then
    PMFA_STATE=NOT_ASSESSED
    PMFA_OBSERVED="not assessed - $PMFA_REASON"
  else
    PMFA_STATE=$(printf '%s\n%s\n%s\n%s\n' "$PMFA_TRUSTED" "$PMFA_URL" "$PMFA_VERSION" "$PMFA_PROMPT" | awk '
      /^[ \t]*#/ || /^[ \t]*$/ { next }
      {
        line=$0; sub(/^[ \t]*/, "", line)
        if (index(line, "=") == 0) { bad=1; next }
        split(line, p, "=")
        if (p[1] == "") { bad=1; next }
        key=tolower(p[1]); gsub(/[ \t]/, "", key)
        val=p[2]; sub(/^[ \t]*/, "", val); sub(/[ \t]*$/, "", val)
        if (key == "trustedcas") trusted=val
        else if (key == "mfa-url") url=val
        else if (key == "server-version") version=val
        else if (key == "ctc-prompt-only") prompt=val
        else bad=1
      }
      END {
        if (bad) print "unknown"
        else if (trusted == "" || url == "" || version == "" || prompt == "") print "absent"
        else if (version == "2" && prompt == "Y") print "manual"
        else print "wrong"
      }')
    case "$PMFA_STATE" in
      pass) PMFA_STATE=PASS; PMFA_OBSERVED="TRUSTEDCAS, MFA-URL, SERVER-VERSION=2, and CTC-PROMPT-ONLY=Y are configured" ;;
      absent|wrong) PMFA_STATE=FAIL; PMFA_OBSERVED="PowerSC MFA required configuration is missing or SERVER-VERSION is not 2 or CTC-PROMPT-ONLY is not Y" ;;
      manual) PMFA_STATE=NOT_ASSESSED; PMFA_OBSERVED="not assessed - SA/ISSO must verify the TRUSTEDCAS server_ca.pem location and MFA-URL valid address" ;;
      *) PMFA_STATE=NOT_ASSESSED; PMFA_OBSERVED="not assessed - pam_pmfa.conf rows were unparseable" ;;
    esac
  fi

  case "$PMFA_STATE" in
    PASS) PMFA_MEANING="The decidable PowerSC MFA settings are configured."; PMFA_FIX="n/a" ;;
    FAIL) PMFA_MEANING="PowerSC MFA configuration does not meet the required settings."; PMFA_FIX="add or update TRUSTEDCAS = /<path_to_file>/server_ca.pem, MFA-URL, SERVER-VERSION = 2, and CTC-PROMPT-ONLY = Y in /etc/security/pmfa/pam_pmfa.conf; verify the server_ca.pem location with the SA/ISSO." ;;
    *) PMFA_MEANING="PTxray could not establish the required MFA configuration."; PMFA_FIX="verify the pam_pmfa.conf evidence and have the SA/ISSO validate the server_ca.pem location and MFA-URL address." ;;
  esac
  add security stig_mfa_conf "PowerSC Multi Factor Authentication configuration" "$PMFA_STATE" med "$PMFA_OBSERVED" "$PMFA_MEANING" "$PMFA_FIX" "stig:V-215439"
}
