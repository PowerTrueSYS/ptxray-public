...
AIXRAY_TOOL=ck-stig-mfa-filesets


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_mfa_filesets — DISA STIG V-215436.
  # Required configuration: IBM PowerSC MFA filesets installed.
  # Probe unfiltered lslpp -l; filter in this fragment so a successful
  # listing with no PowerSC MFA filesets is FAIL, not a probe failure.
  typeset MFAS_RAW MFAS_RC MFAS_CLASS MFAS_PARSE_RC MFAS_REASON
  typeset MFAS_STATUS MFAS_SEV MFAS_OBSERVED MFAS_MEANING MFAS_FIX
  typeset MFAS_REQ

  MFAS_RAW=$(aix lslpp_listing lslpp -l)
  MFAS_RC=$?
  MFAS_CLASS=unknown
  MFAS_REQ="powerscmfa.core powerscmfa.pam"

  if [ "$MFAS_RC" -ne 0 ]; then
    MFAS_REASON=probe_failed
  elif [ -z "$MFAS_RAW" ]; then
    MFAS_REASON=probe_empty
  else
    MFAS_CLASS=$(printf '%s\n' "$MFAS_RAW" | awk -v reqlist="$MFAS_REQ" '
      BEGIN {
        nreq = split(reqlist, req, " ")
      }
      /^[ \t]*$/ { next }
      {
        name = tolower($1)
        if (NF >= 3 && $2 ~ /^[0-9][0-9.]*$/ &&
          ...
          if (name ~ /^powerscmfa(\.|$)/) {
            mfa = 1
            if ($3 == "COMMITTED" || $3 == "APPLIED") inst[name] = 1
            else broken[name] = 1
          }
          next
        }
        if (name ~ /^powerscmfa(\.|$)/) bad = 1
      }
      END {
        if (bad || nreq < 1) exit 1
        if (!mfa) {
          if (listing) print "absent"
          else exit 1
          exit 0
        }
        for (n in broken) any_broken = 1
        for (i = 1; i <= nreq; i++) {
          if (!(req[i] in inst)) miss = 1
        }
        if (miss || any_broken) print "missing"
        else print "ok"
      }
    ')
    MFAS_PARSE_RC=$?
    if [ "$MFAS_PARSE_RC" -ne 0 ] || [ -z "$MFAS_CLASS" ]; then
      MFAS_REASON=unparseable
    else
      case "$MFAS_CLASS" in
        ok) MFAS_REASON=compliant ;;
        missing) MFAS_REASON=noncompliant ;;
        absent) MFAS_REASON=subject_absent ;;
        *) MFAS_REASON=unparseable ;;
      esac
    fi
  fi
  ...
  case "$MFAS_REASON" in
    compliant)
      MFAS_STATUS=PASS
      MFAS_OBSERVED="PowerSC MFA filesets are installed"
      MFAS_MEANING="IBM PowerSC MFA filesets are installed, so Multi Factor Authentication is available."
      MFAS_FIX="n/a"
      ;;
    noncompliant) MFAS_STATUS=FAIL
      MFAS_OBSERVED="PowerSC MFA filesets are not fully installed"
      MFAS_MEANING="One or more IBM PowerSC MFA filesets are not installed, so Multi Factor Authentication is not available."
      MFAS_FIX="Install the IBM PowerSC MFA product."
      ;;
    subject_absent) MFAS_STATUS=FAIL
      MFAS_OBSERVED="PowerSC MFA filesets are absent"
      MFAS_MEANING="IBM PowerSC MFA is not installed, so Multi Factor Authentication is not available."
      MFAS_FIX="Install the IBM PowerSC MFA product."
      ;;
    probe_failed) MFAS_STATUS=NOT_ASSESSED
      MFAS_OBSERVED="not assessed - PowerSC MFA fileset probe failed (rc=$MFAS_RC)"
      MFAS_MEANING="PTxray did not obtain trustworthy PowerSC MFA fileset evidence."
      MFAS_FIX="verify that 'lslpp -l' runs successfully, then rerun PTxray."
      ;;
    probe_empty) MFAS_STATUS=NOT_ASSESSED
      MFAS_OBSERVED="not assessed - PowerSC MFA fileset probe returned no output"
      MFAS_MEANING="PTxray did not obtain trustworthy PowerSC MFA fileset evidence."
      MFAS_FIX="verify that 'lslpp -l' returns a fileset listing, then rerun PTxray."
      ;;
    *)
      MFAS_STATUS=NOT_ASSESSED
      MFAS_OBSERVED="not assessed - PowerSC MFA fileset evidence was unparseable"
      MFAS_MEANING="PTxray did not obtain trustworthy PowerSC MFA fileset evidence."
      MFAS_FIX="verify PowerSC MFA fileset output, then rerun PTxray."
      ;;
  esac

  case "$MFAS_STATUS" in
    NOT_APPLICABLE) MFAS_SEV=low ;;
    *) MFAS_SEV=med ;;
  esac

  add security stig_mfa_filesets "IBM PowerSC MFA filesets" \
    "$MFAS_STATUS" "$MFAS_SEV" "$MFAS_OBSERVED" \
    "$MFAS_MEANING" "$MFAS_FIX" "stig:V-215436"
}
