...
AIXRAY_TOOL=ck-stig-mfa-hotplug


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_mfa_hotplug — DISA STIG V-215441.
  # Required configuration: bos.ahafs installed and pmfahotplugd active.
  # Uncertain probe evidence is refused; a missing subject is a finding.
  typeset MFAHP_LSLPP_RAW MFAHP_LSLPP_RC MFAHP_LSSRC_RAW MFAHP_LSSRC_RC
  typeset MFAHP_PKG MFAHP_SVC MFAHP_REASON MFAHP_STATUS MFAHP_SEV
  typeset MFAHP_OBSERVED MFAHP_MEANING MFAHP_FIX MFAHP_PARSE_RC

  # Pipeline runs inside aix via sh -c; a quoted pipeline is not an executable.
  MFAHP_LSLPP_RAW=$(aix lslpp_bos_ahafs sh -c 'lslpp -l |grep bos.ahafs')
  MFAHP_LSLPP_RC=$?
  MFAHP_LSSRC_RAW=$(aix lssrc_pmfahotplugd lssrc -s pmfahotplugd)
  MFAHP_LSSRC_RC=$?
  MFAHP_PKG=unknown
  MFAHP_SVC=unknown

  if [ "$MFAHP_LSLPP_RC" -ne 0 ]; then
    MFAHP_PKG=pkg_failed
  elif [ -z "$MFAHP_LSLPP_RAW" ]; then
    MFAHP_PKG=pkg_empty
  else
    MFAHP_PKG=$(printf '%s\n' "$MFAHP_LSLPP_RAW" | awk '
      /^[ \t]*$/ { next }
      /^lslpp: 0504-132[ \t]+Fileset bos\.ahafs not installed\.?[ \t]*$/ {
        absent = 1
        next
      }
      $1 == "bos.ahafs" {
    ...
    ')
    MFAHP_PARSE_RC=$?
    if [ "$MFAHP_PARSE_RC" -ne 0 ] || [ -z "$MFAHP_PKG" ]; then
      MFAHP_PKG=pkg_unparseable
    fi
  fi

  # lssrc rc 1 is the documented not-found for a missing SRC subsystem.
  if [ "$MFAHP_LSSRC_RC" -ne 0 ] && [ "$MFAHP_LSSRC_RC" -ne 1 ]; then
    MFAHP_SVC=svc_failed
  elif [ -z "$MFAHP_LSSRC_RAW" ]; then
    MFAHP_SVC=svc_empty
  else
    MFAHP_SVC=$(printf '%s\n' "$MFAHP_LSSRC_RAW" | awk '
      /^[ \t]*$/ { next }
    ...
    ')
    MFAHP_PARSE_RC=$?
    if [ "$MFAHP_PARSE_RC" -ne 0 ] || [ -z "$MFAHP_SVC" ]; then
      MFAHP_SVC=svc_unparseable
    fi
  fi

  case "$MFAHP_PKG:$MFAHP_SVC" in
    pkg_failed:*|*:svc_failed) MFAHP_REASON=probe_failed ;;
    pkg_empty:*|*:svc_empty) MFAHP_REASON=probe_empty ;;
    pkg_unparseable:*|*:svc_unparseable) MFAHP_REASON=unparseable ;;
    pkg_ok:svc_active) MFAHP_REASON=compliant ;;
    pkg_ok:svc_inactive) MFAHP_REASON=noncompliant ;;
    pkg_absent:*|*:svc_absent) MFAHP_REASON=subject_absent ;;
    *) MFAHP_REASON=unparseable ;;
  esac

  case "$MFAHP_REASON" in
    compliant)
      MFAHP_STATUS=PASS
      MFAHP_OBSERVED="bos.ahafs installed; pmfahotplugd active"
      MFAHP_MEANING="bos.ahafs is installed and pmfahotplugd is active, so PIV hotplug credentials can be accepted and verified."
      MFAHP_FIX="n/a"
      ;;
    noncompliant) MFAHP_STATUS=FAIL
      MFAHP_OBSERVED="bos.ahafs installed; pmfahotplugd is not active"
      MFAHP_MEANING="pmfahotplugd is not active, so PIV hotplug credentials are not being accepted and verified."
      MFAHP_FIX="installp -aXYgd /dev/cd0 -e /tmp/install.log bos.ahafs; startsrc -s pmfahotplugd; PTxray only recommends these commands."
      ;;
    subject_absent) MFAHP_STATUS=FAIL
      MFAHP_OBSERVED="required PIV hotplug subject is absent (bos.ahafs and/or pmfahotplugd)"
      MFAHP_MEANING="bos.ahafs is not installed and/or pmfahotplugd is not present, so PIV hotplug credentials cannot be accepted and verified."
      MFAHP_FIX="installp -aXYgd /dev/cd0 -e /tmp/install.log bos.ahafs; startsrc -s pmfahotplugd; PTxray only recommends these commands."
      ;;
    probe_failed) MFAHP_STATUS=NOT_ASSESSED
      MFAHP_OBSERVED="not assessed - PIV hotplug probe failed (lslpp_rc=$MFAHP_LSLPP_RC lssrc_rc=$MFAHP_LSSRC_RC)"
      MFAHP_MEANING="PTxray did not obtain trustworthy bos.ahafs and pmfahotplugd evidence."
      MFAHP_FIX="verify that 'lslpp -l |grep bos.ahafs' and 'lssrc -s pmfahotplugd' run successfully, then rerun PTxray."
      ;;
    probe_empty) MFAHP_STATUS=NOT_ASSESSED
      MFAHP_OBSERVED="not assessed - PIV hotplug probe returned no output"
      MFAHP_MEANING="PTxray did not obtain trustworthy bos.ahafs and pmfahotplugd evidence."
      MFAHP_FIX="verify that 'lslpp -l |grep bos.ahafs' and 'lssrc -s pmfahotplugd' return complete evidence, then rerun PTxray."
      ;;
    *)
      MFAHP_STATUS=NOT_ASSESSED
      MFAHP_OBSERVED="not assessed - PIV hotplug evidence was unparseable"
      MFAHP_MEANING="PTxray did not obtain trustworthy bos.ahafs and pmfahotplugd evidence."
      MFAHP_FIX="verify bos.ahafs fileset output and pmfahotplugd SRC status, then rerun PTxray."
      ;;
  esac

  case "$MFAHP_STATUS" in
    NOT_APPLICABLE) MFAHP_SEV=low ;;
    *) MFAHP_SEV=med ;;
  esac

  add security stig_mfa_hotplug "PIV credential hotplug (bos.ahafs / pmfahotplugd)" \
    "$MFAHP_STATUS" "$MFAHP_SEV" "$MFAHP_OBSERVED" \
    "$MFAHP_MEANING" "$MFAHP_FIX" "stig:V-215441"
}
