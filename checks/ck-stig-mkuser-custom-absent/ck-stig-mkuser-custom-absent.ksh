...
AIXRAY_TOOL=ck-stig-mkuser-custom-absent


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_mkuser_custom_absent — DISA STIG V-215169.
  # Prohibition: /etc/security/mkuser.sys.custom must not exist.
  # aix() discards ls stderr. Empty stdout at rc=2 is the AIX missing-file
  # shape (absence diagnostic was on stderr) and is determinate absence.
  # Empty stdout at rc=0 is a swallowed probe. Other empty nonzero rc is
  # an unproven probe. Non-empty rc 0/1/2 is graded: absence diagnostics
  # PASS, otherwise present.
  typeset MKUC_RAW MKUC_RC MKUC_REASON MKUC_STATUS MKUC_SEV
  typeset MKUC_OBSERVED MKUC_MEANING MKUC_FIX

  MKUC_RAW=$(aix ls_mkuser_sys_custom ls /etc/security/mkuser.sys.custom)
  MKUC_RC=$?
  MKUC_REASON=""

  if [ -z "$MKUC_RAW" ]; then
    if [ "$MKUC_RC" -eq 0 ]; then
      MKUC_REASON=probe_empty
    elif [ "$MKUC_RC" -eq 2 ]; then
      MKUC_REASON=absent
    else
      MKUC_REASON=probe_failed
    fi
  else
    case "$MKUC_RAW" in
      *"does not exist"*|*"No such file"*|*"0653-341"*)
        MKUC_REASON=absent
        ;;
      *)
        if [ "$MKUC_RC" -eq 0 ] || [ "$MKUC_RC" -eq 1 ] || [ "$MKUC_RC" -eq 2 ]; then
          MKUC_REASON=present
        else
          MKUC_REASON=unparseable
        fi
        ;;
    esac
  fi
  ...
  case "$MKUC_REASON" in
    absent)
      MKUC_STATUS=PASS
      MKUC_OBSERVED="/etc/security/mkuser.sys.custom is absent"
      MKUC_MEANING="The mkuser.sys.custom overlay is not present, so new accounts are not customized by that file."
      MKUC_FIX="n/a"
      ;;
    present) MKUC_STATUS=FAIL
      MKUC_OBSERVED="/etc/security/mkuser.sys.custom exists"
      MKUC_MEANING="The mkuser.sys.custom overlay exists and will customize newly created user accounts; DISA STIG V-215169 treats that as a finding unless the overlay is required."
      MKUC_FIX="rm /etc/security/mkuser.sys.custom; PTxray only recommends this command."
      ;;
    probe_failed) MKUC_STATUS=NOT_ASSESSED
      MKUC_OBSERVED="not assessed - mkuser.sys.custom probe failed (rc=$MKUC_RC)"
      MKUC_MEANING="PTxray did not obtain trustworthy evidence for /etc/security/mkuser.sys.custom."
      MKUC_FIX="run 'ls /etc/security/mkuser.sys.custom', resolve the probe failure, and rerun PTxray."
      ;;
    probe_empty) MKUC_STATUS=NOT_ASSESSED
      MKUC_OBSERVED="not assessed - mkuser.sys.custom probe returned no output"
      MKUC_MEANING="PTxray did not obtain trustworthy evidence for /etc/security/mkuser.sys.custom."
      MKUC_FIX="run 'ls /etc/security/mkuser.sys.custom' and confirm it returns a listing or a determinate absence, then rerun PTxray."
      ;;
    *)
      MKUC_STATUS=NOT_ASSESSED
      MKUC_OBSERVED="not assessed - mkuser.sys.custom probe output was unparseable"
      MKUC_MEANING="PTxray did not obtain trustworthy evidence for /etc/security/mkuser.sys.custom."
      MKUC_FIX="run 'ls /etc/security/mkuser.sys.custom', correct the capture, and rerun PTxray."
      ;;
  esac

  case "$MKUC_STATUS" in
    NOT_APPLICABLE) MKUC_SEV=low ;;
    *) MKUC_SEV=med ;;
  esac

  add security stig_mkuser_custom_absent "mkuser.sys.custom must not exist" \
    "$MKUC_STATUS" "$MKUC_SEV" "$MKUC_OBSERVED" \
    "$MKUC_MEANING" "$MKUC_FIX" "stig:V-215169"
}
