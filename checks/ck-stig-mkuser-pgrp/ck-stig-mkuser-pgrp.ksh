...
AIXRAY_TOOL=ck-stig-mkuser-pgrp


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_mkuser_pgrp — DISA STIG V-215182.
  # The default primary group for new regular users must be staff (or equivalent).
  # lssec on the mkuser.default user stanza reports the default pgrp; any other
  # value (or a user stanza without pgrp) means accounts can be created in an
  # overly privileged or untracked group. Read-only probe; the fix is only a
  # recommendation string and is never executed.
  typeset MKU_PGRP_RAW MKU_PGRP_RC MKU_PGRP_VAL MKU_PGRP_PARSE_RC
  typeset MKU_PGRP_STATUS MKU_PGRP_OBSERVED MKU_PGRP_MEANING MKU_PGRP_FIX

  MKU_PGRP_RAW=$(aix lssec_mkuser_default_pgrp lssec -f /etc/security/mkuser.default -s user -a pgrp)
  MKU_PGRP_RC=$?
  MKU_PGRP_VAL=""

  if [ "$MKU_PGRP_RC" -ne 0 ]; then
    MKU_PGRP_STATUS=NOT_ASSESSED
    MKU_PGRP_OBSERVED="not assessed - mkuser.default pgrp probe failed (rc=$MKU_PGRP_RC)"
  elif [ -z "$MKU_PGRP_RAW" ]; then
    MKU_PGRP_STATUS=NOT_ASSESSED
    MKU_PGRP_OBSERVED="not assessed - mkuser.default pgrp probe returned no rows (swallowed error, not absence)"
  else
    MKU_PGRP_VAL=$(printf '%s\n' "$MKU_PGRP_RAW" | awk '
      NF == 0 { next }
      {
        n = split($0, f, /[ \t]+/)
        for (i = 1; i <= n; i++) {
    ...
    ')
    MKU_PGRP_PARSE_RC=$?
    if [ "$MKU_PGRP_PARSE_RC" -ne 0 ] || [ -z "$MKU_PGRP_VAL" ]; then
      MKU_PGRP_STATUS=FAIL
      MKU_PGRP_OBSERVED="mkuser.default user stanza does not set a pgrp value (required default primary group missing)"
    elif [ "$MKU_PGRP_VAL" = "staff" ]; then
      MKU_PGRP_STATUS=PASS
      MKU_PGRP_OBSERVED="mkuser.default user pgrp=staff"
    else
      MKU_PGRP_STATUS=FAIL
      MKU_PGRP_OBSERVED="mkuser.default user pgrp=$MKU_PGRP_VAL (not staff)"
    fi
  fi

  case "$MKU_PGRP_STATUS" in
    PASS)
      MKU_PGRP_MEANING="New regular user accounts default to the staff primary group, so they are not created in an overly privileged or untracked group."
      MKU_PGRP_FIX="n/a"
      ;;
    FAIL)
      MKU_PGRP_MEANING="New regular user accounts default to a primary group other than staff, or the required default is unset, so accounts can be created in an overly privileged or untracked group."
      MKU_PGRP_FIX="chsec -f /etc/security/mkuser.default -s user -a pgrp=staff"
      ;;
    *)
      MKU_PGRP_MEANING="PTxray did not obtain trustworthy evidence for the default primary group setting in /etc/security/mkuser.default."
      MKU_PGRP_FIX="verify the setting as root with 'lssec -f /etc/security/mkuser.default -s user -a pgrp', then rerun PTxray."
      ;;
  esac

  add security stig_mkuser_pgrp "Default primary group for new users" \
    "$MKU_PGRP_STATUS" med "$MKU_PGRP_OBSERVED" \
    "$MKU_PGRP_MEANING" "$MKU_PGRP_FIX" "stig:V-215182"
}
