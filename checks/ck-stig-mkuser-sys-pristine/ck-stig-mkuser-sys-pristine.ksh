...
AIXRAY_TOOL=ck-stig-mkuser-sys-pristine


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_mkuser_sys_pristine — DISA STIG V-215181 (AIX7-00-001015).
  # The shipped /etc/security/mkuser.sys must not be customized directly; site
  # customization belongs in /etc/security/mkuser.sys.custom.  Read the file once
  # and judge it against the shipped contract: every distinctive shipped marker
  # (the "no longer user customizable" notice, the mkuser.sys.custom -x/exec/exit
  # handoff, the argument-count check, the named-directory creation, and the
  # default-.profile copy) must be present and intact, with no site logic added.
  # The IBM prolog/copyright header varies by AIX release, so the comparison
  # starts at the "no longer user customizable" notice (the anchor line) and runs
  # to the end of the file; everything before it is version-specific prolog and
  # is ignored.  Whitespace-tolerant (trailing whitespace and blank lines
  # dropped) but content-strict.
  typeset MS_SHIPPED MS_RAW MS_RC MS_ACT_CT MS_REF_CT MS_MATCH
  typeset MS_STATUS MS_OBSERVED MS_MEANING MS_FIX

  # The realistic shipped script: shebang, IBM prolog/copyright header, and the
  # code the STIG-listed comments annotate.  Only the portion from the anchor
  # line ("no longer user customizable") to the end is compared, so prolog
  # version strings never decide the verdict.  Built with printf '%s\n' and
  # single-quoted lines so the assembled door (fragment wrapped in
  # function standalone_check) stays ksh88-parseable.  A quoted here-document
  # of this text does not parse in that wrapper.
  MS_SHIPPED=$(printf '%s\n' \
    '#!/bin/ksh' \
    '# IBM_PROLOG_BEGIN_TAG' \
    '# This is an automatically generated prolog.' \
    '' \
    '# bos.rte.security 7.2.0.0' \
    '' \
    '# Licensed Materials - Property of IBM' \
    '# 5692-A1A' \
    '# (C) Copyright International Business Machines Corp. 1990, 2015' \
    '# All Rights Reserved' \
    ...
    'if [ $# -ne 1 ]; then' \
    '  echo "usage: mkuser.sys username" >&2' \
    '  exit 1' \
    'fi' \
    '' \
    ...
    'if [ -x /etc/security/mkuser.sys.custom ]; then' \
  ...
  if [ "$MS_RC" -ne 0 ]; then
    if [ -z "$MS_RAW" ]; then
      if [ "$MS_RC" -eq 1 ] || [ "$MS_RC" -eq 2 ]; then
        MS_STATUS=PASS
        MS_OBSERVED="mkuser.sys is absent (probe rc=$MS_RC, empty output); absence of the subject is the compliant state"
      else
        MS_STATUS=NOT_ASSESSED
        MS_OBSERVED="not assessed - mkuser.sys probe failed with no rows (rc=$MS_RC)"
      fi
    else
      MS_STATUS=NOT_ASSESSED
      MS_OBSERVED="not assessed - mkuser.sys probe failed (rc=$MS_RC) with output"
    fi
  elif [ -z "$MS_RAW" ]; then
    MS_STATUS=NOT_ASSESSED
    MS_OBSERVED="not assessed - mkuser.sys probe returned empty output with rc 0 (swallowed error, not absence)"
  else
    MS_ACT_CT=$(printf '%s\n' "$MS_RAW" | awk '
      {
        sub(/[ \t]+$/, "", $0)
        if ($0 == "") next
        if (!started && index($0, "no longer user customizable") > 0) started = 1
        if (started) print
      }')
    MS_REF_CT=$(printf '%s\n' "$MS_SHIPPED" | awk '
      {
    ...
    if [ "$MS_ACT_CT" = "$MS_REF_CT" ]; then
      MS_MATCH=yes
    fi
    if [ "$MS_MATCH" = "yes" ]; then
      MS_STATUS=PASS
      MS_OBSERVED="mkuser.sys matches the shipped fingerprint (no longer user customizable; mkuser.sys.custom handoff; argument check; named-directory creation; default .profile copy)"
    elif [ -z "$MS_ACT_CT" ]; then
      MS_STATUS=FAIL
      MS_OBSERVED="mkuser.sys lacks the shipped notice ('no longer user customizable'): the file has been rewritten or customized directly"
    else
      MS_STATUS=FAIL
      MS_OBSERVED="mkuser.sys differs from the shipped fingerprint: the file has been customized directly (missing or altered shipped marker, extra logic, or a rewritten body)"
    fi
  fi

  case "$MS_STATUS" in
    PASS)
      MS_MEANING="The shipped /etc/security/mkuser.sys is unmodified (or absent); site customization, if any, must live in /etc/security/mkuser.sys.custom."
      MS_FIX="n/a"
      ;;
    FAIL)
      MS_MEANING="A directly customized /etc/security/mkuser.sys can be overwritten by system updates and bypasses the supported customization contract, breaking or silently altering user account creation."
      MS_FIX="restore the shipped /etc/security/mkuser.sys (for example by repairing or reinstalling bos.rte.security) and place any site customization in /etc/security/mkuser.sys.custom; PTxray only recommends this action."
      ;;
    *)
      MS_MEANING="PTxray did not obtain trustworthy evidence for the state of /etc/security/mkuser.sys."
      MS_FIX="verify /etc/security/mkuser.sys is readable and rerun PTxray."
      ;;
  esac

  add security stig_mkuser_sys_pristine "Shipped mkuser.sys must not be customized" \
    "$MS_STATUS" med "$MS_OBSERVED" \
    "$MS_MEANING" "$MS_FIX" "stig:V-215181"
}
