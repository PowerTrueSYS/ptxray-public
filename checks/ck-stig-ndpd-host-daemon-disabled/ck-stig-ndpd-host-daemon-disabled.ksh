...
AIXRAY_TOOL=ck-stig-ndpd-host-daemon-disabled


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_ndpd_host_daemon_disabled — DISA STIG V-215367.
  # The ndpd-host daemon must be disabled unless the system is using IPv6, in
  # which case the rule is Not Applicable. Two read-only probes decide:
  #   ifconfig -a  — the IPv6 Not-Applicable clause is decided per interface
  #                  stanza: the lo0 loopback stanza is ignored (stock AIX
  #                  shows inet6 ::1%1/0 there), and inet6 fe80: link-local
  #                  addresses on other interfaces are autoconfiguration
  #                  artifacts, not evidence the system uses IPv6. Only a
  #                  non-loopback interface carrying a non-link-local IPv6
  #                  address makes the rule Not Applicable (checked first).
  #   grep "^start[[:blank:]]/usr/sbin/ndpd-host" /etc/rc.tcpip — an uncommented
  #   'start /usr/sbin/ndpd-host' line boots the daemon; a commented '#start'
  #   rule never matches the anchored pattern. grep's exit status is the oracle:
  #   rc 1 = no match (daemon absent = compliant), rc 0 = match (daemon enabled
  #   = finding), rc 2+ = file unreadable (not assessed). rc 0 with empty output
  #   is a swallowed error, never absence, so it is NOT_ASSESSED rather than a
  #   hollow PASS.
  typeset IF_RAW IF_RC IF_IPV6 RCT_RAW RCT_RC

  IF_RAW=$(aix ifconfig_ipv6 ifconfig -a)
  IF_RC=$?
  IF_IPV6=$(printf '%s\n' "$IF_RAW" | awk '
  /^[^[:space:]]+: flags=/ { iface = $0; sub(/:.*/, "", iface); next }
  iface != "lo0" && /^[[:space:]]*inet6([[:space:]]|$)/ {
    if (substr($2, 1, 5) == "fe80:") next
    print; exit
  }
')

  if [ "$IF_RC" -ne 0 ]; then
    add security stig_ndpd_host_daemon_disabled "IPv6 ndpd-host daemon state" NOT_ASSESSED med \
        "not assessed - ifconfig -a failed (rc=$IF_RC)" \
        "PTxray could not read the interface configuration, so it cannot decide whether the IPv6 Not-Applicable clause applies or assess the ndpd-host boot state. DISA STIG V-215367 is therefore not assessed rather than guessed clean." \
        "verify ifconfig -a is readable, correct the capture problem, and rerun PTxray." \
        "stig:V-215367"
  elif [ -z "$IF_RAW" ]; then
    add security stig_ndpd_host_daemon_disabled "IPv6 ndpd-host daemon state" NOT_ASSESSED med \
        "not assessed - ifconfig -a returned no output (swallowed error)" \
        "The ifconfig probe reported success with no captured output; an interface configuration read always yields lines on a live system, so empty output with rc 0 is a swallowed error, not evidence about IPv6 use." \
        "rerun PTxray; if the condition persists, investigate the ifconfig -a capture path." \
        "stig:V-215367"
  elif [ -n "$IF_IPV6" ]; then
    add security stig_ndpd_host_daemon_disabled "IPv6 ndpd-host daemon state" NOT_APPLICABLE low \
        "system is using IPv6 (non-loopback interface carries a non-link-local IPv6 address)" \
        "DISA STIG V-215367 states the ndpd-host daemon must be disabled unless the system is using IPv6, in which case the rule is Not Applicable. This system configures IPv6 on an interface, so the rule does not apply." \
        "n/a" \
        "stig:V-215367"
  else
    RCT_RAW=$(aix rctcp_ndpd_host grep "^start[[:blank:]]/usr/sbin/ndpd-host" /etc/rc.tcpip)
    RCT_RC=$?
    if [ -n "$RCT_RAW" ]; then
      add security stig_ndpd_host_daemon_disabled "IPv6 ndpd-host daemon state" FAIL med \
          "ndpd-host starts at boot in /etc/rc.tcpip" \
          "The ndpd-host daemon is enabled to start at boot in /etc/rc.tcpip on a system that does not use IPv6. DISA STIG V-215367 requires the ndpd-host daemon to be disabled; an enabled daemon runs the IPv6 Neighbor Discovery host protocol, an unneeded network-facing service when IPv6 is not in use." \
          "disable the daemon with 'chrctcp -d ndpd-host'; PTxray only recommends this command, it does not run it." \
          "stig:V-215367"
    elif [ "$RCT_RC" -eq 1 ]; then
      add security stig_ndpd_host_daemon_disabled "IPv6 ndpd-host daemon state" PASS med \
          "ndpd-host not enabled in /etc/rc.tcpip" \
          "The ndpd-host daemon does not start at boot in /etc/rc.tcpip and this system does not use IPv6, so the daemon is disabled as DISA STIG V-215367 requires." \
          "n/a" "stig:V-215367"
    elif [ "$RCT_RC" -ne 0 ]; then
      add security stig_ndpd_host_daemon_disabled "IPv6 ndpd-host daemon state" NOT_ASSESSED med \
          "not assessed - rc.tcpip capture failed (rc=$RCT_RC)" \
          "PTxray could not read /etc/rc.tcpip to determine whether the ndpd-host daemon starts at boot, so the rule is not assessed rather than guessed compliant." \
          "verify /etc/rc.tcpip is readable, correct the capture problem, and rerun PTxray." \
          "stig:V-215367"
    else
      add security stig_ndpd_host_daemon_disabled "IPv6 ndpd-host daemon state" NOT_ASSESSED med \
          "not assessed - rc.tcpip probe returned rc 0 with no output (swallowed error)" \
          "The rc.tcpip probe reported success with no captured output; for this grep, rc 0 always accompanies a selected line, so empty output with rc 0 is a swallowed error, not evidence the daemon is disabled." \
          "rerun PTxray; if the condition persists, investigate the /etc/rc.tcpip capture path." \
          "stig:V-215367"
    fi
  fi
}
