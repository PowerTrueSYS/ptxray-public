...
AIXRAY_TOOL=ck-stig-ndpd-router-disabled


function standalone_check {
_AIXRAY_SESSION_KEYS=""
# stig_ndpd_router_disabled -- DISA STIG V-215368.
# The ndpd-router daemon must not be started from /etc/rc.tcpip. Any matching
# start command is a finding; a probe that cannot be trusted fails closed as
# NOT_ASSESSED (rc 0 with no output is a swallowed error, never an absence).
typeset NRD_RAW NRD_RC NRD_OBSERVED

NRD_RAW=$(aix rctcp_ndpd_router grep "^start[[:blank:]]/usr/sbin/ndpd-router" /etc/rc.tcpip)
NRD_RC=$?

if [ "$NRD_RC" -eq 0 ] && [ -n "$NRD_RAW" ]; then
  # grep rc 0 with output: a start command for ndpd-router is present in
  # rc.tcpip. Output carrying any other status is not a trustworthy match.
  NRD_OBSERVED=$(printf '%s\n' "$NRD_RAW" | awk 'NR==1{print; exit}')
  add security stig_ndpd_router_disabled "ndpd-router daemon must be disabled" FAIL med \
      "$NRD_OBSERVED" \
      "/etc/rc.tcpip starts the ndpd-router IPv6 router discovery daemon, which DISA STIG V-215368 requires disabled." \
      "run 'chrctcp -d ndpd-router' to remove the start entry; PTxray only recommends this command." \
      "stig:V-215368"
elif [ "$NRD_RC" -eq 1 ] && [ -z "$NRD_RAW" ]; then
  # grep found no matching start line: determinate absence is the compliant state.
  add security stig_ndpd_router_disabled "ndpd-router daemon must be disabled" PASS med \
      "no start entry for /usr/sbin/ndpd-router in /etc/rc.tcpip" \
      "/etc/rc.tcpip does not start the ndpd-router daemon, as required by DISA STIG V-215368." \
      "n/a" "stig:V-215368"
elif [ "$NRD_RC" -eq 0 ]; then
  # rc 0 with no output is a swallowed error, not an absence.
  add security stig_ndpd_router_disabled "ndpd-router daemon must be disabled" NOT_ASSESSED med \
      "not assessed - empty rc.tcpip probe output with rc 0 (swallowed error)" \
      "PTxray did not obtain trustworthy /etc/rc.tcpip evidence: the probe returned success with no output." \
      "verify /etc/rc.tcpip is readable and rerun PTxray." \
      "stig:V-215368"
else
  add security stig_ndpd_router_disabled "ndpd-router daemon must be disabled" NOT_ASSESSED med \
      "not assessed - probe failed (rc=$NRD_RC)" \
      "PTxray could not read /etc/rc.tcpip to determine whether ndpd-router is started." \
      "verify /etc/rc.tcpip is readable and rerun PTxray." \
      "stig:V-215368"
fi
}
