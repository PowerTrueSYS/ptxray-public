...
function eval_nettune {
  typeset CMDS CAP RAW DETAIL SUM CTLS NPASS NFAIL NNA NAPPLIC FLIST ST SEV OBS MEAN FIX NT_CMD NT_RAW NT_RC GRADED PARSED PARSED_RC PROBE_FAIL BADPARSE

  # distinct commands (no, nfso) in first-seen order
  CMDS=$(printf '%s\n' "$R_NETTUNE" | awk -F'|' 'NF>=5 && $1 !~ /^#/ && $2!="" && !seen[$2]++{print $2}')
  [ -z "$CMDS" ] && return 0

  # one 'no -a' / 'nfso -a' per command. Each probe is captured BEFORE any loop so
  # its rc survives (a while-read pipeline would run in a subshell and lose it).
  # Every record is validated here, not just the last stored value: a line is
  # well-formed only as "<name> = <value>" — at least one space or tab on each
  # side of "=" — with a lowercase name and a numeric value (all four graded
  # tunables are integer-valued in the STIG text). A malformed graded record
  # refuses the whole check; a garbled tunable this check does not grade is
  # ignored. An unusable probe (rc!=0 or empty output) is a cannot-assess
  # path, distinct from determinate absence below.
  CAP=""
  PROBE_FAIL=0
  BADPARSE=0
  for NT_CMD in $CMDS; do
    [ -z "$NT_CMD" ] && continue
    # The command WORD is a literal per token, not an expansion: the shipped
    # command allowlist can only vouch for an argv it can read, and R_NETTUNE
    # above is the closed vocabulary that feeds $CMDS. An unknown token is
    # unusable evidence, not absence, so it takes the probe-failed path.
    case "$NT_CMD" in
      (no) NT_RAW=$(aix "stig_${NT_CMD}_a" no -a); NT_RC=$? ;;
    ...
    GRADED=$(printf '%s\n' "$R_NETTUNE" | awk -F'|' -v cmd="$NT_CMD" 'NF>=5 && $2==cmd{g=(g==""?$3:g " " $3)} END{print g}')
    PARSED=$(printf '%s\n' "$NT_RAW" | awk -v cmd="$NT_CMD" -v graded="$GRADED" '
      BEGIN {
        gcount=split(graded, garr, " ")
        for (i=1; i<=gcount; i++) if (garr[i] != "") G[garr[i]] = 1
      }
      {
        sub(/^[ \t]+/, "")
        sub(/[ \t]+$/, "")
        ...
        for (i = 1; i <= n; i++) {
          c = substr($0, i, 1)
          if (c ~ /[a-z0-9_]/) name = name c
          else break
        }
        if (name == "") { bad = 1; next }
        if (i > n) { if (name in G) bad = 1; next }
        if (substr($0, i, 1) !~ /[ \t]/) { if (name in G) bad = 1; next }
        while (i <= n && substr($0, i, 1) ~ /[ \t]/) i++
        if (i > n || substr($0, i, 1) != "=") { if (name in G) bad = 1; next }
        i++
        if (i > n || substr($0, i, 1) !~ /[ \t]/) { if (name in G) bad = 1; next }
        while (i <= n && substr($0, i, 1) ~ /[ \t]/) i++
        if (i > n) { if (name in G) bad = 1; next }
        ...
        # well-formed records are all emitted (graded or not) so a readable
        # probe that carries only ungraded tunables stays a usable probe —
        # determinate absence, never a refusal.
    ...
    if [ -z "$PARSED" ]; then
      # readable probe (rc 0, non-empty) that yielded no tunable records
      PROBE_FAIL=1
      continue
    fi
    CAP="${CAP}${PARSED}
"
  ...
  if [ "$PROBE_FAIL" -eq 1 ]; then
    ...
    OBS="not assessed - 'no -a' unreadable; no network-option rule evaluated"
  ...
  elif [ "$BADPARSE" -eq 1 ]; then
    ST=NOT_ASSESSED; SEV=med
    OBS="not assessed - unparseable no -a output"
    MEAN="The 'no -a' / 'nfso -a' capture contained a malformed record for a graded STIG network-option tunable; a partially-parseable probe is not a graded probe."
    FIX="inspect the 'no -a' output for a corrupted line among the graded tunables and re-run; or verify the 'no' subsystem is healthy."
  elif [ "$NAPPLIC" -eq 0 ]; then
    ST=NOT_APPLICABLE; SEV=low
    OBS="no applicable network-option rule on this box"
    MEAN="Every STIG network-option rule refers to a tunable absent from this system's 'no -a' / 'nfso -a' output; no rule is applicable."
    FIX="n/a"
