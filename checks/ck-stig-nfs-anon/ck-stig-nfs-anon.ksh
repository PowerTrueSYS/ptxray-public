...
AIXRAY_TOOL=ck-stig-nfs-anon


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_nfs_anon — DISA STIG V-215209.
  # Every exported file system must map anonymous NFS access to a no-permission ID.
  typeset NFS_ANON_RAW NFS_ANON_RC NFS_ANON_SHAPE NFS_ANON_STATUS
  typeset NFS_ANON_OBSERVED NFS_ANON_MEANING NFS_ANON_FIX

  NFS_ANON_RAW=$(aix exportfs exportfs -v)
  NFS_ANON_RC=$?
  NFS_ANON_SHAPE=""

  if [ "$NFS_ANON_RC" -ne 0 ]; then
    NFS_ANON_STATUS=NOT_ASSESSED
    NFS_ANON_OBSERVED="not assessed - exportfs -v failed (rc=$NFS_ANON_RC)"
  elif [ -z "$NFS_ANON_RAW" ]; then
    NFS_ANON_STATUS=NOT_ASSESSED
    NFS_ANON_OBSERVED="not assessed - exportfs -v returned no output"
  else
    NFS_ANON_SHAPE=$(printf '%s\n' "$NFS_ANON_RAW" | awk '
      /^[ \t]*$/ { next }
      $0 == "exportfs: no NFS file systems are exported" || $0 == "exportfs: nothing exported" {
        if (records != 0 || subject_absent) bad = 1
        subject_absent = 1
        next
      }
      {
        ...
        if ($1 !~ /^\//) { bad = 1; next }
        path = $1
        nfs_anon_scan = substr($0, index($0, $1) + length($1))
        if (match(nfs_anon_scan, /(^|[, \t-])anon=[^, \t]*/)) {
          anon = substr(nfs_anon_scan, RSTART, RLENGTH)
          sub(/^.*anon=/, "", anon)
          allowed = (anon == "-1" || anon == "60001" || anon == "60002" || anon == "65534" || anon == "65535")
          missing = 0
        } else {
          anon = ""
          allowed = 0
        ...
        if (missing || !allowed) {
          if (!bad_export) bad_export = path "|" (missing ? "missing" : anon)
        }
      }
      END {
        if (bad || (subject_absent && records != 0)) print "unparseable"
        else if (subject_absent) print "subject_absent"
        else if (records == 0) print "unparseable"
        else if (bad_export) print "bad:" bad_export
        else print "ok"
      }
    ')
    case "$NFS_ANON_SHAPE" in
      ok)
        NFS_ANON_STATUS=PASS
        NFS_ANON_OBSERVED="every exported file system has a permitted anon value"
        ;;
      bad:*)
        NFS_ANON_STATUS=FAIL
        NFS_ANON_OBSERVED="export ${NFS_ANON_SHAPE#bad:} is not configured with a permitted anonymous ID"
        NFS_ANON_OBSERVED=$(printf '%s\n' "$NFS_ANON_OBSERVED" | sed 's/|/ has anon=/')
        ;;
      subject_absent) NFS_ANON_STATUS=FAIL
        NFS_ANON_OBSERVED="no NFS file systems are exported; this required configuration is unmet"
        ;;
      *)
        NFS_ANON_STATUS=NOT_ASSESSED
        NFS_ANON_OBSERVED="not assessed - exportfs -v output was unparseable"
        ;;
    esac
  fi

  case "$NFS_ANON_STATUS" in
    PASS)
      NFS_ANON_MEANING="Every NFS export explicitly maps anonymous access to a permitted no-permission ID."
      NFS_ANON_FIX="n/a"
      ;;
    FAIL)
      NFS_ANON_MEANING="An NFS export can map anonymous access to an ID that may have permissions, or the required NFS export configuration is absent."
      NFS_ANON_FIX="run 'exportfs -a' after configuring each export with a permitted anon value; PTxray only recommends this command."
      ;;
    *)
      NFS_ANON_MEANING="PTxray did not obtain trustworthy exportfs evidence for the anonymous NFS identity setting."
      NFS_ANON_FIX="verify exportfs -v as an administrator and rerun PTxray."
      ;;
  esac

  add security stig_nfs_anon "NFS anonymous UID and GID" "$NFS_ANON_STATUS" med \
    "$NFS_ANON_OBSERVED" "$NFS_ANON_MEANING" "$NFS_ANON_FIX" "stig:V-215209"
}
