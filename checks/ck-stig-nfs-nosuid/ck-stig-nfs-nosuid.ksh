...
AIXRAY_TOOL=ck-stig-nfs-nosuid


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_nfs_nosuid — DISA STIG V-215234.
  # mount | grep -E "options|nfs|---"
  typeset NFS_NS_RAW NFS_NS_RC NFS_NS_VIEW NFS_NS_SHAPE
  typeset NFS_NS_STATUS NFS_NS_OBSERVED NFS_NS_MEANING NFS_NS_FIX

  NFS_NS_RAW=$(aix nfs_mounts mount)
  NFS_NS_RC=$?
  NFS_NS_VIEW=""
  NFS_NS_SHAPE=""

  if [ "$NFS_NS_RC" -ne 0 ]; then
    NFS_NS_STATUS=NOT_ASSESSED
    NFS_NS_OBSERVED="not assessed - mount probe failed (rc=$NFS_NS_RC)"
  elif [ -z "$NFS_NS_RAW" ]; then
    NFS_NS_STATUS=NOT_ASSESSED
    NFS_NS_OBSERVED="not assessed - mount returned no output with rc=0"
  else
    NFS_NS_VIEW=$(printf '%s\n' "$NFS_NS_RAW" | grep -E "options|nfs|---")
    if [ -z "$NFS_NS_VIEW" ]; then
      NFS_NS_STATUS=NOT_ASSESSED
      NFS_NS_OBSERVED="not assessed - mount output did not match the NFS mount table filter"
    else
      NFS_NS_SHAPE=$(printf '%s\n' "$NFS_NS_VIEW" | awk '
        /^[ \t]*$/ { next }
        /---/ { saw_table = 1; next }
        /[Oo]ptions/ && /mounted/ { saw_table = 1; next }
        {
          vfs_i = 0
          for (i = 1; i <= NF; i++) {
            if ($i ~ /^nfs[0-9]*$/) vfs_i = i
          }
          if (vfs_i == 0) next
          if (vfs_i < 2 || NF < vfs_i + 2) { bad = 1; next }
          over = $(vfs_i - 1)
          opts = $NF
          if (over !~ /^\//) { bad = 1; next }
          if (opts ~ /^[0-9][0-9]:[0-9][0-9]$/) { bad = 1; next }
          if (opts !~ /^[A-Za-z0-9,=._\/:+-]+$/) { bad = 1; next }
          wrapped = "," opts ","
          gsub(/[ \t]/, "", wrapped)
          has_nosuid = (index(wrapped, ",nosuid,") != 0)
          has_suid = (index(wrapped, ",suid,") != 0)
          nfs_count++
          if (!has_nosuid && !has_suid) {
            if (fail_list == "") fail_list = over
          } else if (has_suid && !has_nosuid) {
            if (suid_list == "") suid_list = over
          } else {
            if (ok_list == "") ok_list = over
            else ok_list = ok_list " " over
          }
        }
        END {
          if (bad) print "unparseable"
          else if (nfs_count == 0) {
            if (saw_table) print "subject_absent"
            else print "unparseable"
          } else if (fail_list != "") print "fail:" fail_list
          else if (suid_list != "") print "manual:" suid_list
          else print "ok:" ok_list
        }
      ')
      case "$NFS_NS_SHAPE" in
        ok:*)
          NFS_NS_STATUS=PASS
          NFS_NS_OBSERVED="NFS mounts have nosuid (${NFS_NS_SHAPE#ok:})"
          ;;
        fail:*)
          NFS_NS_STATUS=FAIL
          NFS_NS_OBSERVED="NFS mount ${NFS_NS_SHAPE#fail:} lacks nosuid"
          ;;
        subject_absent) NFS_NS_STATUS=FAIL
          NFS_NS_OBSERVED="no NFS file systems are mounted; this required configuration is unmet"
          ;;
        manual:*) NFS_NS_STATUS=NOT_ASSESSED
          NFS_NS_OBSERVED="not assessed - ISSO/ISSM list of NFS file systems that contain approved setuid or setgid programs is required"
          ;;
        *)
          NFS_NS_STATUS=NOT_ASSESSED
          NFS_NS_OBSERVED="not assessed - mount output was unparseable"
          ;;
      esac
    fi
  fi

  case "$NFS_NS_STATUS" in
    PASS)
      NFS_NS_MEANING="Every NFS file system is mounted with nosuid, so setuid programs from NFS servers cannot run with local privilege."
      NFS_NS_FIX="n/a"
      ;;
    FAIL)
      NFS_NS_MEANING="An NFS file system is mounted without nosuid, or no NFS file system is mounted, so the required nosuid configuration is unmet."
      NFS_NS_FIX="# chfs -a options=ro,bg,hard,intr,nosuid,sec=sys <NFS_mount_point>"
      ;;
    *)
      NFS_NS_MEANING="PTxray could not complete the required nosuid determination from trustworthy automated evidence."
      NFS_NS_FIX="Verify NFS mount options and obtain the ISSO/ISSM list of NFS file systems that contain approved setuid or setgid programs, then rerun PTxray."
      ;;
  esac

  add security stig_nfs_nosuid "NFS nosuid mount option" \
    "$NFS_NS_STATUS" med "$NFS_NS_OBSERVED" "$NFS_NS_MEANING" \
    "$NFS_NS_FIX" "stig:V-215234"
}
