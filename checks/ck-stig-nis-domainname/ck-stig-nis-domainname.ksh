...
AIXRAY_TOOL=ck-stig-nis-domainname


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_nis_domainname — DISA STIG V-215418.
  # domainname
  # Empty/unset domain is the only automated FAIL. A configured name is never
  # PASS: guessability (org/building/room) is site/ISSO judgment.
  typeset NIS_DN_RAW NIS_DN_RC NIS_DN_VALUE NIS_DN_STATUS NIS_DN_OBSERVED
  typeset NIS_DN_MEANING NIS_DN_FIX

  NIS_DN_RAW=$(aix nis_domainname domainname)
  NIS_DN_RC=$?
  NIS_DN_VALUE=$(printf '%s\n' "$NIS_DN_RAW" | awk '
    NF {
      rows++
      if (rows != 1 || $0 ~ /[[:cntrl:]]/) bad=1
      value=$0
    }
    END { if (bad || rows > 1) exit 1; print value }
  ')

  if [ "$NIS_DN_RC" -ne 0 ]; then
    NIS_DN_STATUS=NOT_ASSESSED
    NIS_DN_OBSERVED="not assessed - domainname probe failed (rc=$NIS_DN_RC)"
    NIS_DN_MEANING="PTxray could not make the required determination from trustworthy automated evidence."
    NIS_DN_FIX="Verify the NIS domain name and have the site/ISSO judge whether it is difficult to guess, then rerun PTxray."
  elif [ -z "$NIS_DN_RAW" ] || [ "$NIS_DN_VALUE" = none ]; then
    NIS_DN_STATUS=FAIL
    NIS_DN_OBSERVED="NIS domain name is not configured"
    NIS_DN_MEANING="NIS maps do not have a configured domain name, so the required NIS domain-name protection is unmet."
    NIS_DN_FIX="Change the NIS domain name to a value difficult to guess.; Consult vendor documentation for the required procedure."
  elif [ -z "$NIS_DN_VALUE" ]; then
    NIS_DN_STATUS=NOT_ASSESSED
    NIS_DN_OBSERVED="not assessed - domainname output was unparseable"
    NIS_DN_MEANING="PTxray could not make the required determination from trustworthy automated evidence."
    NIS_DN_FIX="Verify the NIS domain name and have the site/ISSO judge whether it is difficult to guess, then rerun PTxray."
  else
    NIS_DN_STATUS=NOT_ASSESSED
    NIS_DN_OBSERVED="not assessed - whether this NIS domain name is simple to guess requires site/ISSO judgment"
    NIS_DN_MEANING="A configured NIS domain name is present; whether that name is simple to guess (organization, building, or room name) requires site/ISSO judgment."
    NIS_DN_FIX="Have the site/ISSO judge whether the NIS domain name is difficult to guess, and change it if it is not."
  fi

  add security stig_nis_domainname "NIS domain name protection" \
    "$NIS_DN_STATUS" med "$NIS_DN_OBSERVED" "$NIS_DN_MEANING" \
    "$NIS_DN_FIX" "stig:V-215418"
}
