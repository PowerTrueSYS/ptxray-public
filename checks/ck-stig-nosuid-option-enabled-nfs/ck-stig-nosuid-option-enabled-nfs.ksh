...
AIXRAY_TOOL=ck-stig-nosuid-option-enabled-nfs


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_nosuid_option_enabled_nfs — DISA STIG V-215210.
  typeset SNO_RAW SNO_RC SNO_PARSE SNO_PARSE_RC
  typeset SNO_TOTAL SNO_FAIL_COUNT SNO_FAILURES SNO_OBS

  SNO_RAW=$(aix lsfs_nfs lsfs -v nfs)
  SNO_RC=$?

  if [ "$SNO_RC" -ne 0 ]; then
    add security stig_nosuid_option_enabled_nfs "NFS client nosuid option" NOT_ASSESSED med \
        "not assessed - lsfs -v nfs failed (rc=$SNO_RC)" \
        "PTxray could not obtain trustworthy NFS client mount evidence." \
        "correct the lsfs -v nfs capture and rerun PTxray." \
        "stig:V-215210"
  elif [ -z "$SNO_RAW" ]; then
    add security stig_nosuid_option_enabled_nfs "NFS client nosuid option" NOT_APPLICABLE med \
        "no NFS filesystems defined" \
        "lsfs -v nfs exited 0 with empty output, so no NFS filesystems are defined and DISA STIG V-215210 does not apply." \
        "n/a" \
        "stig:V-215210"
  else
    SNO_PARSE=$(printf '%s\n' "$SNO_RAW" | awk '
      function finish_row(    n,item,i,has_nosuid) {
        if (NF != 8) { bad=1; return }
        if ($4 !~ /^nfs[234]?$/) { bad=1; return }
        if ($(NF-1) != "yes" && $(NF-1) != "no") { bad=1; return }
        if ($NF != "yes" && $NF != "no") { bad=1; return }
        n=split($6, item, /[ \t,]+/)
        has_nosuid=0
        for (i=1; i<=n; i++) {
          if (tolower(item[i]) == "nosuid") has_nosuid=1
        }
        if (has_nosuid) {
          print "PASS\t" $3
        } else {
      ...
      function is_header() {
        return (NF == 9 && $1 == "Name" && $2 == "Nodename" && $3 == "Mount" && $4 == "Pt" && $5 == "VFS" && $6 == "Size" && $7 == "Options" && $8 == "Auto" && $9 == "Accounting")
      }
      /^[ \t]*$/ { next }
      {
        if (is_header()) {
          if (header_count > 0 || saw_row) bad=1
          header_count++
          next
        }
        saw_row=1
      ...
      END {
        if (bad || header_count != 1) exit 1
      }
    ')
    SNO_PARSE_RC=$?
    if [ "$SNO_PARSE_RC" -ne 0 ]; then
      add security stig_nosuid_option_enabled_nfs "NFS client nosuid option" NOT_ASSESSED med \
          "not assessed - lsfs -v nfs output was unparseable" \
          "The complete NFS client nosuid set could not be classified safely." \
          "inspect lsfs -v nfs output and rerun PTxray." \
          "stig:V-215210"
    else
      SNO_TOTAL=$(printf '%s\n' "$SNO_PARSE" | awk -F '\t' '$1=="PASS" || $1=="FAIL"{n++} END{print n+0}')
      SNO_FAIL_COUNT=$(printf '%s\n' "$SNO_PARSE" | awk -F '\t' '$1=="FAIL"{n++} END{print n+0}')
      SNO_FAILURES=$(printf '%s\n' "$SNO_PARSE" | awk -F '\t' '$1=="FAIL"{if (n++) printf ", "; printf "%s", $2}')
      if [ "$SNO_FAIL_COUNT" -gt 0 ]; then
        SNO_OBS="$SNO_FAIL_COUNT of $SNO_TOTAL NFS client mount(s) lack nosuid: $SNO_FAILURES"
        add security stig_nosuid_option_enabled_nfs "NFS client nosuid option" FAIL med \
            "$SNO_OBS" \
            "One or more NFS client mounts allow set-ID execution from a remote file system." \
            ...
            "stig:V-215210"
      elif [ "$SNO_TOTAL" -eq 0 ]; then
        add security stig_nosuid_option_enabled_nfs "NFS client nosuid option" FAIL med \
            "no NFS client mounts; nosuid is required on every NFS client mount" \
            "No NFS client mount is present; DISA STIG V-215210 requires nosuid on every NFS client mount." \
            "Edit \"/etc/filesystems\" and add the \"nosuid\" option for all NFS file systems.; Remount the NFS file systems to make the change take effect." \
            "stig:V-215210"
      else
        add security stig_nosuid_option_enabled_nfs "NFS client nosuid option" PASS med \
            "$SNO_TOTAL NFS client mount(s) include nosuid" \
            "Every NFS client mount includes the nosuid option as required by DISA STIG V-215210." \
            "n/a" "stig:V-215210"
      fi
    fi
  fi
