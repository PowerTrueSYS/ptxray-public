...
AIXRAY_TOOL=ck-stig-openssl-version


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_openssl_version — DISA STIG V-215216.
  # openssl.base VRMF must be >= 20.13.102.1000 (FIPS-validated floor).
  # Absence of the fileset is a finding (required-configuration rule).
  # Nonzero probe rc is NOT_ASSESSED unless the output is a positively
  # recognized "openssl.base not installed" diagnostic (then FAIL). rc 0
  # with empty output, or unparseable listing -> NOT_ASSESSED, never PASS.
  typeset SOV_RAW SOV_RC SOV_KIND SOV_VER
  typeset SOV_STATUS SOV_SEV SOV_OBSERVED SOV_MEANING SOV_FIX
  typeset SOV_FAILCLOSED_PROBE SOV_FAILCLOSED_EMPTY SOV_FAILCLOSED_UNPARSE
  typeset SOV_ABSENT_VERDICT SOV_FLOOR

  SOV_FAILCLOSED_PROBE=NOT_ASSESSED
  SOV_FAILCLOSED_EMPTY=NOT_ASSESSED
  SOV_FAILCLOSED_UNPARSE=NOT_ASSESSED
  SOV_ABSENT_VERDICT=FAIL
  SOV_FLOOR=20.13.102.1000
  SOV_KIND=""
  SOV_VER=""
  SOV_STATUS=""
  SOV_OBSERVED=""

  SOV_RAW=$(aix openssl_base lslpp -l openssl.base)
  SOV_RC=$?
    ...
    SOV_KIND=$(printf '%s\n' "$SOV_RAW" | awk '
      {
        ...
        if ($0 ~ /^[ \t]*$/) next
        if ($0 ~ /^[ \t]*#/) next
        if ($0 ~ /openssl\.base not installed/) { absent_sig = 1; next }
        other = 1
      }
      END {
        if (absent_sig && !other) print "absent"
        else print "untrusted"
      }
    ')
    case "$SOV_KIND" in
      absent)
        SOV_STATUS=$SOV_ABSENT_VERDICT
        SOV_OBSERVED="openssl.base is not installed"
        ;;
      *)
        SOV_STATUS=$SOV_FAILCLOSED_PROBE
        SOV_OBSERVED="not assessed - openssl.base probe failed (rc=$SOV_RC)"
        ;;
    esac
  elif [ -z "$SOV_RAW" ]; then
    SOV_STATUS=$SOV_FAILCLOSED_EMPTY
    SOV_OBSERVED="not assessed - openssl.base probe returned empty output with rc=0"
  else
    SOV_KIND=$(printf '%s\n' "$SOV_RAW" | awk -v FLOOR="$SOV_FLOOR" '
      function vrmf_ok(v,    n, a, i) {
        n = split(v, a, ".")
        if (n != 4) return 0
        for (i = 1; i <= 4; i++) {
          if (a[i] !~ /^[0-9]+$/) return 0
      ...
      }
      {
        ...
        if ($0 ~ /^[ \t]*$/) next
        if ($0 ~ /^[ \t]*#/) next
        if ($0 ~ /not installed/) { absent_sig = 1; next }
        if ($0 ~ /^[ \t]*Path:/) { listing = 1; next }
        if ($0 ~ /^[ \t]*Fileset/) { listing = 1; next }
        if ($0 ~ /^[- \t]+$/) next
        if (NF >= 2 && $1 == "openssl.base") {
          if (!vrmf_ok($2)) { bad = 1; next }
          listing = 1
          nbase++
          if (minver == "" || vcmp($2, minver) < 0) minver = $2
          if (vcmp($2, FLOOR) < 0) old = 1
          next
        }
        if (NF >= 2 && $1 ~ /^[A-Za-z][A-Za-z0-9_.+-]*$/ && vrmf_ok($2)) {
          listing = 1
          next
        }
        if ($0 ~ /^lslpp:/) { diag = 1; next }
      }
      END {
        if (bad) exit 1
        if (nbase > 0) {
          if (old) { print "old:" minver; exit 0 }
          print "ok:" minver
          exit 0
        }
        if (absent_sig || listing) { print "absent"; exit 0 }
        exit 1
      }
    ')
    if [ $? -ne 0 ] || [ -z "$SOV_KIND" ]; then
      SOV_STATUS=$SOV_FAILCLOSED_UNPARSE
      SOV_OBSERVED="not assessed - openssl.base listing was unparseable"
    else
      SOV_VER=${SOV_KIND#*:}
      case "$SOV_KIND" in
        ok:*)
          SOV_STATUS=PASS
          SOV_OBSERVED="openssl.base $SOV_VER meets floor $SOV_FLOOR"
          ;;
        old:*)
          SOV_STATUS=FAIL
          SOV_OBSERVED="openssl.base $SOV_VER is older than floor $SOV_FLOOR"
          ;;
        absent)
          SOV_STATUS=$SOV_ABSENT_VERDICT
          SOV_OBSERVED="openssl.base is not installed"
  ...
  case "$SOV_STATUS" in
    PASS)
      SOV_SEV=med
      SOV_MEANING="openssl.base meets the STIG V-215216 FIPS-validated VRMF floor $SOV_FLOOR."
      SOV_FIX="n/a"
      ;;
    FAIL)
      SOV_SEV=med
      SOV_MEANING="openssl.base is missing or older than $SOV_FLOOR, so NIST FIPS-validated cryptography is not in place."
      SOV_FIX="# smitty install"
      ;;
    *)
      if [ -z "$SOV_STATUS" ]; then
        SOV_STATUS=$SOV_FAILCLOSED_UNPARSE
        SOV_OBSERVED="not assessed - openssl.base listing was unparseable"
      fi
      SOV_SEV=med
      SOV_MEANING="PTxray did not obtain trustworthy openssl.base version evidence for STIG V-215216."
      SOV_FIX="run 'lslpp -l openssl.base', correct the capture or path problem, and rerun PTxray."
      ;;
  esac

  add security stig_openssl_version "OpenSSL FIPS version floor" \
    "$SOV_STATUS" "$SOV_SEV" "$SOV_OBSERVED" \
    "$SOV_MEANING" "$SOV_FIX" "stig:V-215216"
}
