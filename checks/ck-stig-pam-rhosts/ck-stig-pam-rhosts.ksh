...
AIXRAY_TOOL=ck-stig-pam-rhosts


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_pam_rhosts — DISA STIG V-215433.
  # The .rhosts file must not be supported in AIX PAM.  An uncommented
  # "rhosts_auth" module line in /etc/pam.conf would enable host-based
  # (rhosts) authentication; absence of an enabled rhosts_auth entry is the
  # compliant state (prohibition rule).  STIG check-content:
  #   grep rhosts_auth /etc/pam.conf |grep -v \#
  # The pipeline cannot be an aix probe (the helper execs "$@" so a quoted
  # string is a command name, rc 127). Probe grep as argv and filter comments
  # in awk. grep rc 1 (no match on a readable file) is determinate absence
  # and renders PASS; only rc 127, an unreadable file, or other failed probes
  # are NOT_ASSESSED.
  typeset PAM_RAW PAM_RC PAM_HIT PAM_STATUS PAM_OBSERVED
  typeset PAM_MEANING PAM_FIX

  PAM_RAW=$(aix pam_conf /usr/bin/grep rhosts_auth /etc/pam.conf)
  PAM_RC=$?
  # grep: 0 = match, 1 = no match (determinate absence), 2+ = probe failure.
  if [ "$PAM_RC" -eq 1 ]; then
    PAM_RC=0
    PAM_RAW=""
  fi
  if [ "$PAM_RC" -eq 0 ] && [ -n "$PAM_RAW" ]; then
    PAM_RAW=$(printf '%s\n' "$PAM_RAW" | awk '
      /#/ { next }
      { print }
    ')
  ...
  if [ "$PAM_RC" -ne 0 ]; then
    PAM_STATUS=NOT_ASSESSED
    PAM_OBSERVED="not assessed - rhosts_auth probe failed (rc=$PAM_RC)"
  elif [ -z "$PAM_RAW" ]; then
    PAM_STATUS=PASS
    PAM_OBSERVED="no uncommented rhosts_auth module entry in /etc/pam.conf"
  else
    PAM_HIT=$(printf '%s\n' "$PAM_RAW" | awk '
      /rhosts_auth/ && $0 !~ /#/ { print; exit }
    ')
    if [ -n "$PAM_HIT" ]; then
      PAM_STATUS=FAIL
      PAM_OBSERVED="uncommented rhosts_auth module entry enabled in /etc/pam.conf: $PAM_HIT"
    else
      PAM_STATUS=PASS
      PAM_OBSERVED="no uncommented rhosts_auth module entry in /etc/pam.conf"
    fi
  ...
  case "$PAM_STATUS" in
    PASS)
      PAM_MEANING="AIX PAM has no enabled rhosts_auth module, so .rhosts trust is not honored through PAM."
      PAM_FIX="n/a"
      ;;
    FAIL)
      PAM_MEANING="AIX PAM is configured with an enabled rhosts_auth module, making the .rhosts file a supported authentication source."
      PAM_FIX='Edit "/etc/pam.conf" and remove the reference(s) to the "rhosts_auth" module.'
      ;;
    *)
      PAM_MEANING="PTxray did not obtain trustworthy evidence of the PAM rhosts_auth configuration."
      PAM_FIX="Verify /etc/pam.conf is present and readable and contains no uncommented rhosts_auth entry, then rerun PTxray."
      ;;
  esac

  add security stig_pam_rhosts "PAM rhosts_auth support" "$PAM_STATUS" med \
      "$PAM_OBSERVED" "$PAM_MEANING" "$PAM_FIX" "stig:V-215433"
}
