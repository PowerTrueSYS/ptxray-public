...
AIXRAY_TOOL=ck-stig-pam-sshd-mfa


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_pam_sshd_mfa — DISA STIG V-215438.
  # First three ^sshd lines of /etc/pam.conf must be the PowerSC MFA
  # auth stack (pam_ckfile, pam_permission, pam_pmfa). Missing, commented,
  # or mismatched lines are a finding. The probe prints ABSENT when the
  # file is readable and has no ^sshd line; a missing file keeps nonzero
  # awk status. Empty rc0 is a swallowed error. Unparseable nonempty
  # output is a refusal, never determinate absence.
  typeset PAM_MFA_RAW PAM_MFA_RC PAM_MFA_SHAPE
  typeset PAM_MFA_STATUS PAM_MFA_OBSERVED PAM_MFA_MEANING PAM_MFA_FIX

  PAM_MFA_RAW=$(aix pam_sshd_mfa awk '/^sshd/ { n++; if (n <= 3) print } END { if (n == 0) print "ABSENT" }' /etc/pam.conf)
  PAM_MFA_RC=$?

  if [ "$PAM_MFA_RC" -ne 0 ]; then
    PAM_MFA_STATUS=NOT_ASSESSED
    PAM_MFA_OBSERVED="not assessed - sshd PAM MFA probe failed (rc=$PAM_MFA_RC)"
  elif [ -z "$PAM_MFA_RAW" ]; then
    PAM_MFA_STATUS=NOT_ASSESSED
    PAM_MFA_OBSERVED="not assessed - sshd PAM MFA probe returned no output"
  else
    PAM_MFA_SHAPE=$(printf '%s\n' "$PAM_MFA_RAW" | awk '
      BEGIN {
        e1 = "sshd auth required pam_ckfile"
        e2 = "sshd auth required pam_permission file=/etc/security/access.conf found=allow"
        e3 = "sshd auth required pam_pmfa"
      }
      {
        line = $0
        gsub(/[ \t]+/, " ", line)
      ...
      END {
        if (n == 1 && got[1] == "ABSENT") { print "absent"; exit }
        if (n == 0) { print "unparseable"; exit }
        for (i = 1; i <= n && i <= 3; i++) {
          if (index(got[i], "sshd") != 1) { print "unparseable"; exit }
        }
        if (n >= 3 && got[1] == e1 && got[2] == e2 && got[3] == e3) { print "ok"; exit }
        print "mismatch"
      }
    ')
    case "$PAM_MFA_SHAPE" in
      ok)
        PAM_MFA_STATUS=PASS
        PAM_MFA_OBSERVED="sshd PAM MFA stack matches pam_ckfile, pam_permission, and pam_pmfa"
        ;;
      absent)
        PAM_MFA_STATUS=FAIL
        PAM_MFA_OBSERVED="sshd PAM MFA stack is absent from /etc/pam.conf"
        ;;
      mismatch)
        PAM_MFA_STATUS=FAIL
        PAM_MFA_OBSERVED="sshd PAM MFA stack does not match the required pam_ckfile/pam_permission/pam_pmfa lines"
        ;;
      *)
        PAM_MFA_STATUS=NOT_ASSESSED
        PAM_MFA_OBSERVED="not assessed - sshd PAM MFA probe output was unparseable"
        ;;
    esac
  fi

  case "$PAM_MFA_STATUS" in
    PASS)
      PAM_MFA_MEANING="Remote sshd authentication uses the required PowerSC MFA PAM stack."
      PAM_MFA_FIX="n/a"
      ;;
    FAIL)
      PAM_MFA_MEANING="Remote sshd connections are not configured to use Multi Factor Authentication through PAM."
      PAM_MFA_FIX="Add or update the following lines in /etc/pam.conf: sshd auth required pam_ckfile; sshd auth required pam_permission file=/etc/security/access.conf found=allow; sshd auth required pam_pmfa. PTxray only recommends this change."
      ;;
    *)
      PAM_MFA_MEANING="PTxray did not obtain trustworthy evidence of the sshd PAM MFA stack."
      PAM_MFA_FIX="Verify /etc/pam.conf is present and readable, then rerun PTxray."
      ;;
  esac

  add security stig_pam_sshd_mfa "sshd PAM multi-factor authentication" \
    "$PAM_MFA_STATUS" med "$PAM_MFA_OBSERVED" "$PAM_MFA_MEANING" "$PAM_MFA_FIX" "stig:V-215438"
}
