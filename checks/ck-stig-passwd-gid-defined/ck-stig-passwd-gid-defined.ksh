...
AIXRAY_TOOL=ck-stig-passwd-gid-defined


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_passwd_gid_defined — DISA STIG V-215339.
  # Every GID in /etc/passwd field 4 must be defined by a group in /etc/group
  # field 3.  A missing subject (rc 2) and a present-but-empty /etc/group are
  # failed required configurations (FAIL); a present-but-empty /etc/passwd probe
  # with rc 0 cannot be told apart from a swallowed probe error (NOT_ASSESSED),
  # and any other probe failure is NOT_ASSESSED.  Undefined GIDs are reported in
  # first-seen /etc/passwd order so the observed string is a stable function of
  # the input across ksh88/nawk/gawk.
  typeset PG_PASSWD_RAW PG_PASSWD_RC PG_GROUP_RAW PG_GROUP_RC
  typeset PG_MISSING PG_STATUS PG_OBSERVED PG_MEANING PG_FIX

  PG_PASSWD_RAW=$(aix passwd_gids cut -d: -f4 /etc/passwd)
  PG_PASSWD_RC=$?
  PG_GROUP_RAW=$(aix group_gids cut -d: -f3 /etc/group)
  PG_GROUP_RC=$?
  ...
  if [ "$PG_PASSWD_RC" -ne 0 ] || [ "$PG_GROUP_RC" -ne 0 ]; then
    if [ "$PG_PASSWD_RC" -eq 2 ] || [ "$PG_GROUP_RC" -eq 2 ]; then
      PG_STATUS=FAIL
      if [ "$PG_PASSWD_RC" -eq 2 ]; then
        PG_OBSERVED="required subject /etc/passwd is absent"
      else
        PG_OBSERVED="required subject /etc/group is absent"
      fi
      PG_MEANING="A required subject of this rule is missing, so the requirement that every /etc/passwd GID be defined in /etc/group is unmet."
      PG_FIX="restore the missing /etc/passwd or /etc/group, then rerun PTxray; PTxray only recommends this action."
    else
      PG_STATUS=NOT_ASSESSED
      PG_OBSERVED="not assessed - GID probe failed (passwd rc=$PG_PASSWD_RC, group rc=$PG_GROUP_RC)"
      PG_MEANING="PTxray did not obtain trustworthy GID lists from both /etc/passwd and /etc/group."
      PG_FIX="verify the /etc/passwd and /etc/group probes as root, then rerun PTxray."
    fi
  elif [ -z "$PG_GROUP_RAW" ] && [ -n "$PG_PASSWD_RAW" ]; then
    # A present /etc/group that yields no GID rows with rc 0 is determinate
    # absence of group definitions, not a swallowed probe error: every GID
    # referenced in /etc/passwd is therefore undefined -> FAIL.
    PG_STATUS=FAIL
    PG_OBSERVED="/etc/group defines no GIDs, so every GID referenced in /etc/passwd is undefined"
    PG_MEANING="The /etc/group file is present but empty (or has no group definitions), so no GID referenced in /etc/passwd resolves to a defined group."
    PG_FIX="add a group for each undefined GID with the 'mkgroup' command; PTxray only recommends this command."
  elif [ -z "$PG_PASSWD_RAW" ]; then
    # Empty /etc/passwd probe output with rc 0 is a swallowed error: with no
    # GID list at all, absence cannot be told apart from a failed probe.
    PG_STATUS=NOT_ASSESSED
    PG_OBSERVED="not assessed - the /etc/passwd GID probe returned no rows with rc 0 (a swallowed error, not absence)"
    PG_MEANING="PTxray did not obtain trustworthy GID lists from both /etc/passwd and /etc/group."
    PG_FIX="verify the /etc/passwd and /etc/group probes as root, then rerun PTxray."
  else
    PG_MISSING=$( ( printf '%s\n' "$PG_GROUP_RAW"; printf '%s\n' "%GID_BOUNDARY%"; printf '%s\n' "$PG_PASSWD_RAW" ) | awk '
      $0 == "%GID_BOUNDARY%" { collect = 1; next }
      collect == 0 { if ($0 != "") gset[$0] = 1; next }
      {
        if ($0 != "" && !($0 in gset) && !($0 in miss)) {
          miss[$0] = 1
          out = (out == "") ? $0 : (out " " $0)
        }
      }
    ...
    if [ -n "$PG_MISSING" ]; then
      PG_STATUS=FAIL
      PG_OBSERVED="GID(s) referenced in /etc/passwd without a defined group in /etc/group: $PG_MISSING"
      PG_MEANING="At least one GID used in /etc/passwd has no matching entry in /etc/group, so the owning account maps to an undefined group."
      PG_FIX="add a group for each undefined GID with the 'mkgroup' command; PTxray only recommends this command."
    else
      PG_STATUS=PASS
      PG_OBSERVED="every GID referenced in /etc/passwd has a defined group in /etc/group"
      PG_MEANING="Every group identifier referenced in /etc/passwd resolves to a group defined in /etc/group."
      PG_FIX="n/a"
    fi
  fi

  add security stig_passwd_gid_defined "AIX group identifiers referenced in /etc/passwd must be defined in /etc/group" \
    "$PG_STATUS" med "$PG_OBSERVED" \
    "$PG_MEANING" "$PG_FIX" "stig:V-215339"
}
