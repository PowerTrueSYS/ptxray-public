...
AIXRAY_TOOL=ck-stig-passwd-nntp


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_passwd_nntp — DISA STIG V-215327.
  # passwd.nntp on jfs/jfs2: permission bits must be a subset of owner r/w.
  # Determinate absence (NNTP not in use) is Not Applicable.
  typeset PWN_RAW PWN_RC PWN_CLASS PWN_PARSE_RC PWN_REASON
  typeset PWN_STATUS PWN_SEV PWN_OBSERVED PWN_MEANING PWN_FIX

  PWN_RAW=$(aix find_passwd_nntp find / -name passwd.nntp \( -fstype jfs -o -fstype jfs2 \) -exec ls -lL {} \;)
  PWN_RC=$?
  PWN_REASON=""

  if [ "$PWN_RC" -ne 0 ]; then
    PWN_REASON=probe_failed
  elif [ -z "$PWN_RAW" ]; then
    PWN_REASON=probe_empty
  else
    PWN_CLASS=$(printf '%s\n' "$PWN_RAW" | awk '
      /^[ \t]*$/ { next }
      /^[ \t]*#/ {
        if ($0 ~ /no match:[ \t]*passwd\.nntp/) nomatch = 1
        else bad = 1
        next
      }
      {
        ...
        if ($NF !~ /passwd\.nntp$/) bad = 1
        if (bad) next
        oct = 0
        if (substr(mode, 2, 1) == "r") oct += 400
        if (substr(mode, 3, 1) == "w") oct += 200
        c = substr(mode, 4, 1)
        if (c == "x" || c == "s") oct += 100
        if (c == "s" || c == "S") oct += 4000
        if (substr(mode, 5, 1) == "r") oct += 40
        if (substr(mode, 6, 1) == "w") oct += 20
        c = substr(mode, 7, 1)
        if (c == "x" || c == "s") oct += 10
        if (c == "s" || c == "S") oct += 2000
        if (substr(mode, 8, 1) == "r") oct += 4
        if (substr(mode, 9, 1) == "w") oct += 2
        c = substr(mode, 10, 1)
        if (c == "x" || c == "t") oct += 1
        if (c == "t" || c == "T") oct += 1000
        rec = $NF " (" sprintf("%04d", oct) ")"
        bits_outside_owner_rw = 0
        i = 4
        while (i <= 10) {
          if (substr(mode, i, 1) != "-") bits_outside_owner_rw = 1
          i++
        }
        if (bits_outside_owner_rw) {
          if (failing != "") failing = failing ", "
          failing = failing rec
        } else {
          if (okfiles != "") okfiles = okfiles ", "
          okfiles = okfiles rec
        }
      }
      END {
        if (bad) exit 1
        if (records == 0) {
          if (nomatch) print "absent"
          else exit 1
        } else if (failing != "") print "fail " failing
        else print "pass " okfiles
      }
    ')
    PWN_PARSE_RC=$?
    if [ "$PWN_PARSE_RC" -ne 0 ] || [ -z "$PWN_CLASS" ]; then
      PWN_REASON=unparseable
    else
      case "$PWN_CLASS" in
        pass\ *) PWN_REASON=compliant ;;
        fail\ *) PWN_REASON=noncompliant ;;
        absent) PWN_REASON=absent ;;
        *) PWN_REASON=unparseable ;;
      esac
    fi
  fi
  ...
  case "$PWN_REASON" in
    compliant)
      PWN_STATUS=PASS
      PWN_OBSERVED="passwd.nntp mode 0600 or less permissive: ${PWN_CLASS#pass }"
      PWN_MEANING="Every passwd.nntp file on jfs/jfs2 has mode 0600 or less permissive."
      PWN_FIX="n/a"
      ;;
    noncompliant)
      PWN_STATUS=FAIL
      PWN_OBSERVED="passwd.nntp more permissive than 0600: ${PWN_CLASS#fail }"
      PWN_MEANING="A passwd.nntp file is more permissive than 0600, exposing NNTP credentials."
      PWN_FIX="chmod 0600 <passwd.nntp_file>"
      ;;
    absent) PWN_STATUS=NOT_APPLICABLE
      PWN_OBSERVED="NNTP/passwd.nntp is not in use"
      PWN_MEANING="No passwd.nntp file exists on jfs/jfs2, so NNTP is not in use."
      PWN_FIX="n/a"
      ;;
    probe_failed) PWN_STATUS=NOT_ASSESSED
      PWN_OBSERVED="not assessed - passwd.nntp probe failed (rc=$PWN_RC)"
      PWN_MEANING="PTxray did not obtain trustworthy passwd.nntp mode evidence."
      PWN_FIX="verify that find for passwd.nntp on jfs/jfs2 runs successfully, then rerun PTxray."
      ;;
    probe_empty) PWN_STATUS=NOT_APPLICABLE
      PWN_OBSERVED="NNTP/passwd.nntp is not in use"
      PWN_MEANING="No passwd.nntp file exists on jfs/jfs2, so NNTP is not in use."
      PWN_FIX="n/a"
      ;;
    *)
      PWN_STATUS=NOT_ASSESSED
      PWN_OBSERVED="not assessed - passwd.nntp listing was unparseable"
      PWN_MEANING="PTxray did not obtain trustworthy passwd.nntp mode evidence."
      PWN_FIX="verify the passwd.nntp listing, then rerun PTxray."
      ;;
  esac

  case "$PWN_STATUS" in
    NOT_APPLICABLE) PWN_SEV=low ;;
    *) PWN_SEV=med ;;
  esac

  add security stig_passwd_nntp "passwd.nntp file mode" \
    "$PWN_STATUS" "$PWN_SEV" "$PWN_OBSERVED" \
    "$PWN_MEANING" "$PWN_FIX" "stig:V-215327"
}
