...
AIXRAY_TOOL=ck-stig-passwd-shells


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_passwd_shells — DISA STIG V-215288.
  # Every /etc/passwd login shell must be in /etc/shells or login.cfg usw
  # shells, excluding nologin/false/dev/null equivalents. Required AIX
  # shells must appear in that allowed set.
  typeset SPS_PASSWD_RAW SPS_PASSWD_RC SPS_SHELLS_RAW SPS_SHELLS_RC
  typeset SPS_LOGIN_RAW SPS_LOGIN_RC SPS_RESULT SPS_KIND SPS_DETAIL
  typeset SPS_STATUS SPS_SEV SPS_OBSERVED SPS_MEANING SPS_FIX
  typeset SPS_PASSWD_FAIL SPS_REQUIRED_FAIL
  typeset SPS_FAILCLOSED_PROBE SPS_FAILCLOSED_EMPTY SPS_FAILCLOSED_UNPARSE

  SPS_PASSWD_FAIL=FAIL
  SPS_REQUIRED_FAIL=FAIL
  SPS_FAILCLOSED_PROBE=NOT_ASSESSED
  SPS_FAILCLOSED_EMPTY=NOT_ASSESSED
  SPS_FAILCLOSED_UNPARSE=NOT_ASSESSED
  SPS_STATUS=""
  SPS_OBSERVED=""
  SPS_KIND=""
  SPS_DETAIL=""
  ...
  SPS_PASSWD_RAW=$(aix passwd cat /etc/passwd)
  SPS_PASSWD_RC=$?
  SPS_SHELLS_RAW=$(aix etc_shells cat /etc/shells)
  SPS_SHELLS_RC=$?
  # grep shells /etc/security/login.cfg | grep -v '*'
  SPS_LOGIN_RAW=$(aix login_cfg_shells sh -c "grep shells /etc/security/login.cfg | grep -v '*'")
  SPS_LOGIN_RC=$?

  if [ "$SPS_PASSWD_RC" -ne 0 ]; then
    SPS_STATUS=$SPS_FAILCLOSED_PROBE
    SPS_OBSERVED="not assessed - passwd probe failed (rc=$SPS_PASSWD_RC)"
  elif [ -z "$SPS_PASSWD_RAW" ]; then
    SPS_STATUS=$SPS_FAILCLOSED_EMPTY
    SPS_OBSERVED="not assessed - passwd probe returned empty output with rc 0"
  elif [ "$SPS_SHELLS_RC" -ne 0 ]; then
    SPS_STATUS=$SPS_FAILCLOSED_PROBE
    SPS_OBSERVED="not assessed - /etc/shells probe failed (rc=$SPS_SHELLS_RC)"
  elif [ -z "$SPS_SHELLS_RAW" ]; then
    SPS_STATUS=$SPS_FAILCLOSED_EMPTY
    SPS_OBSERVED="not assessed - /etc/shells probe returned empty output with rc 0"
  elif [ "$SPS_LOGIN_RC" -ne 0 ]; then
    SPS_STATUS=$SPS_FAILCLOSED_PROBE
    SPS_OBSERVED="not assessed - login.cfg shells probe failed (rc=$SPS_LOGIN_RC)"
  elif [ -z "$SPS_LOGIN_RAW" ]; then
    SPS_STATUS=$SPS_FAILCLOSED_EMPTY
    SPS_OBSERVED="not assessed - login.cfg shells probe returned empty output with rc 0"
  else
    SPS_RESULT=$(
      {
        printf '%s\n' "$SPS_SHELLS_RAW" | awk '{ print "S" $0 }'
        printf '%s\n' "$SPS_LOGIN_RAW" | awk '{ print "L" $0 }'
        printf '%s\n' "$SPS_PASSWD_RAW" | awk '{ print "P" $0 }'
      } | awk '
        BEGIN {
          nreq = split("/bin/csh /bin/ksh /bin/psh /bin/tsh /bin/bsh /usr/bin/csh /usr/bin/ksh /usr/bin/psh /usr/bin/tsh /usr/bin/bsh", req, " ")
        }
        function trim(s) {
          gsub(/^[ \t]+/, "", s)
          gsub(/[ \t]+$/, "", s)
        ...
        function is_shell_path(sh) {
          return (sh ~ /^\/[-A-Za-z0-9\/._+]+$/)
        }
        function is_excluded(sh,   n, p, base) {
          if (sh == "/dev/null" || sh == "false" || sh == "nologin") return 1
          n = split(sh, p, "/")
          base = p[n]
          if (base == "false" || base == "nologin" || base == "null") return 1
          return 0
        }
        /^S/ {
          line = trim(substr($0, 2))
          if (line == "" || line ~ /^[#*]/) next
          if (!is_shell_path(line)) { bad_parse = 1; next }
          allowed[line] = 1
          next
        }
        /^L/ {
          line = substr($0, 2)
          if (!match(line, /shells[ \t]*=[ \t]*/)) next
          rest = substr(line, RSTART + RLENGTH)
          n = split(rest, items, ",")
          for (j = 1; j <= n; j++) {
            item = trim(items[j])
            if (item == "") continue
            if (!is_shell_path(item)) { bad_parse = 1; next }
            allowed[item] = 1
            saw_login_shells = 1
          }
          next
        ...
        /^P/ {
          ...
          if (line ~ /^[+#]/) next
          nf = split(line, f, ":")
          if (nf < 7) { bad_parse = 1; next }
          sh = trim(f[7])
          if (sh == "" || is_excluded(sh)) next
          if (!(sh in allowed)) {
            n_bad++
            if (bad_list == "") bad_list = f[1] "=" sh
            else bad_list = bad_list " " f[1] "=" sh
          }
        }
        END {
          if (bad_parse || !saw_login_shells) { print "UNPARSEABLE"; exit }
          for (i = 1; i <= nreq; i++) {
            if (!(req[i] in allowed)) {
              n_miss++
              if (miss_list == "") miss_list = req[i]
              else miss_list = miss_list " " req[i]
            }
          }
          if (n_bad > 0 && n_miss > 0) print "FAIL_BOTH:" bad_list " ; " miss_list
          else if (n_bad > 0) print "FAIL_PASSWD:" bad_list
          else if (n_miss > 0) print "FAIL_REQUIRED:" miss_list
          else print "PASS"
        }
      '
    )
    SPS_KIND=${SPS_RESULT%%:*}
    SPS_DETAIL=${SPS_RESULT#*:}
    case "$SPS_KIND" in
      PASS)
        SPS_STATUS=PASS
        SPS_OBSERVED="all passwd login shells are listed in /etc/shells or login.cfg usw shells; required subjects present"
        ;;
      FAIL_PASSWD)
        SPS_STATUS=$SPS_PASSWD_FAIL
        SPS_OBSERVED="passwd shells not listed in /etc/shells or login.cfg: $SPS_DETAIL"
        ;;
      FAIL_REQUIRED)
        SPS_STATUS=$SPS_REQUIRED_FAIL
        SPS_OBSERVED="required shells not listed in /etc/shells or login.cfg: $SPS_DETAIL"
        ;;
      FAIL_BOTH)
        SPS_STATUS=$SPS_PASSWD_FAIL
        SPS_OBSERVED="passwd shells not listed in /etc/shells or login.cfg: $SPS_DETAIL"
        ;;
      *)
        SPS_STATUS=$SPS_FAILCLOSED_UNPARSE
        SPS_OBSERVED="not assessed - passwd or shells evidence was unparseable"
        ;;
    esac
  fi
  ...
  case "$SPS_STATUS" in
    PASS)
      SPS_SEV=med
      SPS_MEANING="Every login shell in /etc/passwd is listed in /etc/shells or the login.cfg usw shells list, and the required AIX shells are present."
      SPS_FIX="n/a"
      ;;
    FAIL)
      SPS_SEV=med
      SPS_MEANING="A passwd login shell is not listed in /etc/shells or login.cfg usw shells, or a required AIX shell is missing from that allowed set."
      SPS_FIX="Use the chsh utility or edit /etc/passwd and change the default shell of the account in error to an acceptable shell name contained in /etc/shells. Alternatively, use SMIT to change the /etc/passwd shell entry."
      ;;
    *)
      if [ -z "$SPS_STATUS" ]; then
        SPS_STATUS=$SPS_FAILCLOSED_UNPARSE
        SPS_OBSERVED="not assessed - passwd or shells evidence was unparseable"
      fi
      SPS_SEV=med
      SPS_MEANING="PTxray did not obtain trustworthy evidence for passwd shells versus /etc/shells."
      SPS_FIX="verify cat /etc/passwd, cat /etc/shells, and grep shells /etc/security/login.cfg, then rerun PTxray."
      ;;
  esac

  add security stig_passwd_shells "Passwd shells listed in /etc/shells" \
    "$SPS_STATUS" "$SPS_SEV" "$SPS_OBSERVED" \
    "$SPS_MEANING" "$SPS_FIX" "stig:V-215288"
}
