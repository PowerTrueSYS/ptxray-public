...
AIXRAY_TOOL=ck-stig-plus-entries


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_plus_entries — DISA STIG V-215206.
  # /etc/passwd, /etc/security/passwd, and /etc/group must not contain '+'
  # unless NIS+ or LDAP netgroup entries are defined. A prohibition rule:
  # determinate absence of those files is PASS (svcoff). File-probe rc 2 is
  # ENOENT. Grep pipeline rc 1 is no-match (no plus). Other rc != 0 or rc 0
  # with empty output is refused. Nonzero netgroup probes are unusable, not
  # absence: plus entries then render NOT_ASSESSED unless a usable source
  # already shows netgroup definitions.
  # Pipelines run inside aix via sh -c; a quoted pipeline is not an executable.
  # cat /etc/passwd | grep -v "^#" | grep "\+"
  # cat /etc/security/passwd | grep -v "^#" | grep "\+"
  # cat /etc/group | grep -v "^#" | grep "\+"
  typeset SPE_RAW SPE_RC SPE_HITS SPE_HITS_RC SPE_KEY SPE_PATH SPE_OIFS
  typeset SPE_PW_RAW SPE_PW_RC SPE_SEC_RAW SPE_SEC_RC SPE_GRP_RAW SPE_GRP_RC
  typeset SPE_NG_TEXT
  typeset SPE_FAILED SPE_FAIL_RC SPE_EMPTY SPE_PRESENT_N
  typeset SPE_PLUS SPE_PLUS_LIST SPE_KIND
  typeset SPE_NG_RAW SPE_NG_RC SPE_LDAP_RAW SPE_LDAP_RC SPE_NIS_RAW SPE_NIS_RC
  typeset SPE_NG_DEF SPE_NG_UNK SPE_NG_N
  typeset SPE_STATUS SPE_SEV SPE_OBSERVED SPE_MEANING SPE_FIX
  ...
  SPE_OIFS=$IFS
  while IFS='|' read -r SPE_KEY SPE_PATH
  do
    IFS=$SPE_OIFS
    case "$SPE_KEY" in ''|'#'*) continue ;; esac
    case "$SPE_KEY" in
      etc_passwd_plus)
        SPE_PW_RAW=$(aix etc_passwd_plus sh -c "cat /etc/passwd | grep -v \"^#\" | grep \"\\+\""); SPE_PW_RC=$?
        # rc 2 is ENOENT for this file: determinate absence, not evidence.
        if [ "$SPE_PW_RC" -eq 2 ]; then continue; fi
        SPE_RAW=$SPE_PW_RAW
        SPE_RC=$SPE_PW_RC
        ;;
      etc_security_passwd_plus)
        SPE_SEC_RAW=$(aix etc_security_passwd_plus sh -c "cat /etc/security/passwd | grep -v \"^#\" | grep \"\\+\""); SPE_SEC_RC=$?
        # rc 2 is ENOENT for this file: determinate absence, not evidence.
        if [ "$SPE_SEC_RC" -eq 2 ]; then continue; fi
        SPE_RAW=$SPE_SEC_RAW
        SPE_RC=$SPE_SEC_RC
        ;;
      etc_group_plus)
        SPE_GRP_RAW=$(aix etc_group_plus sh -c "cat /etc/group | grep -v \"^#\" | grep \"\\+\""); SPE_GRP_RC=$?
        # rc 2 is ENOENT for this file: determinate absence, not evidence.
        if [ "$SPE_GRP_RC" -eq 2 ]; then continue; fi
        SPE_RAW=$SPE_GRP_RAW
        SPE_RC=$SPE_GRP_RC
        ;;
      *)
    ...
    ')
    SPE_HITS_RC=$?
    if [ "$SPE_HITS_RC" -gt 1 ]; then
      SPE_FAILED=1
      SPE_FAIL_RC=$SPE_HITS_RC
      continue
    fi
    if [ "$SPE_HITS_RC" -eq 0 ]; then
      SPE_PLUS=1
      if [ -z "$SPE_PLUS_LIST" ]; then
        SPE_PLUS_LIST=$SPE_PATH
      else
        SPE_PLUS_LIST="$SPE_PLUS_LIST $SPE_PATH"
      fi
    fi
  done <<'SPE_FILES'
etc_passwd_plus|/etc/passwd
etc_security_passwd_plus|/etc/security/passwd
etc_group_plus|/etc/group
SPE_FILES
  IFS=$SPE_OIFS

  SPE_NG_RAW=$(aix etc_netgroup cat /etc/netgroup)
  SPE_NG_RC=$?
  SPE_LDAP_RAW=$(aix lsldap_netgroup lsldap -a netgroup)
  SPE_LDAP_RC=$?
  SPE_NIS_RAW=$(aix niscat_netgroup niscat netgroup.org_dir)
  SPE_NIS_RC=$?

  # Each netgroup source is gated on its own exit status and its own output:
  # a probe that failed, or returned nothing with rc 0, is unusable evidence
  # (SPE_NG_UNK) and never contributes lines to the definition count.
  SPE_NG_TEXT=""
  if [ "$SPE_NG_RC" -ne 0 ] || [ -z "$SPE_NG_RAW" ]; then
    SPE_NG_UNK=1
  else
    SPE_NG_TEXT=$SPE_NG_RAW
  fi
  if [ "$SPE_LDAP_RC" -ne 0 ] || [ -z "$SPE_LDAP_RAW" ]; then
    SPE_NG_UNK=1
  else
    SPE_NG_TEXT=$(printf '%s\n%s\n' "$SPE_NG_TEXT" "$SPE_LDAP_RAW")
  fi
  if [ "$SPE_NIS_RC" -ne 0 ] || [ -z "$SPE_NIS_RAW" ]; then
    SPE_NG_UNK=1
  else
    SPE_NG_TEXT=$(printf '%s\n%s\n' "$SPE_NG_TEXT" "$SPE_NIS_RAW")
  fi
  if [ -n "$SPE_NG_TEXT" ]; then
    SPE_NG_N=$(printf '%s\n' "$SPE_NG_TEXT" | awk '
      /^[ \t]*$/ { next }
      /^[ \t]*#/ { next }
      { c++ }
      END { print c + 0 }
    ')
    [ "$SPE_NG_N" -gt 0 ] && SPE_NG_DEF=1
  fi

  if [ "$SPE_FAILED" -eq 1 ]; then
    SPE_KIND=failed
  elif [ "$SPE_EMPTY" -eq 1 ]; then
    SPE_KIND=empty
  elif [ "$SPE_PRESENT_N" -eq 0 ]; then
    SPE_KIND=absent
  elif [ "$SPE_PLUS" -eq 1 ]; then
    if [ "$SPE_NG_DEF" -eq 1 ]; then
      SPE_KIND=plus_defined
    elif [ "$SPE_NG_UNK" -eq 1 ]; then
      SPE_KIND=ng_unknown
    else
      SPE_KIND=plus_undefined
    fi
  else
    SPE_KIND=clean
  fi
  ...
  case "$SPE_KIND" in
    clean)
      SPE_STATUS=PASS
      SPE_OBSERVED="no plus entries in /etc/passwd, /etc/security/passwd, or /etc/group"
      SPE_MEANING="None of the three files contains a plus, so NIS+/LDAP netgroup inclusion is not enabled through these databases."
      SPE_FIX="n/a"
      ;;
    plus_defined)
      SPE_STATUS=PASS
      SPE_OBSERVED="plus entries present with NIS+ or LDAP netgroup definitions"
      SPE_MEANING="Plus entries exist in passwd/group files and NIS+ or LDAP netgroup entries are defined."
      SPE_FIX="n/a"
      ;;
    absent)
      SPE_STATUS=PASS
      SPE_OBSERVED="passwd/group plus-entry subject is absent"
      SPE_MEANING="The passwd/group files that would hold plus entries are absent, so the prohibition holds."
      SPE_FIX="n/a"
      ;;
    plus_undefined)
      SPE_STATUS=FAIL
      SPE_OBSERVED="plus entries without NIS+ or LDAP netgroup definitions: $SPE_PLUS_LIST"
      SPE_MEANING="A plus in /etc/passwd, /etc/security/passwd, or /etc/group enables NIS+/LDAP inclusion without defined netgroup entries, so network-defined users or groups can participate in authentication."
      SPE_FIX="Edit \"/etc/passwd\", \"/etc/security/passwd\", and/or \"/etc/group\" files and remove entries containing a plus (+)."
      ;;
    failed)
      SPE_STATUS=NOT_ASSESSED
      SPE_OBSERVED="not assessed - plus-entry probe failed (rc=$SPE_FAIL_RC)"
      SPE_MEANING="PTxray did not obtain trustworthy evidence for plus entries in passwd/group files."
      SPE_FIX="verify /etc/passwd, /etc/security/passwd, and /etc/group are readable, then rerun PTxray."
      ;;
    empty)
      SPE_STATUS=NOT_ASSESSED
      SPE_OBSERVED="not assessed - plus-entry probe returned empty output with rc 0"
      SPE_MEANING="PTxray did not obtain trustworthy evidence for plus entries in passwd/group files."
      SPE_FIX="verify /etc/passwd, /etc/security/passwd, and /etc/group return content, then rerun PTxray."
      ;;
    *)
      SPE_STATUS=NOT_ASSESSED
      SPE_OBSERVED="not assessed - plus-entry netgroup definition probe was unusable"
      SPE_MEANING="PTxray found plus entries but could not determine whether NIS+ or LDAP netgroups are defined."
      SPE_FIX="verify /etc/netgroup, lsldap -a netgroup, and niscat netgroup.org_dir, then rerun PTxray."
      ;;
  esac

  add security stig_plus_entries "Passwd and group plus entries" \
    "$SPE_STATUS" "$SPE_SEV" "$SPE_OBSERVED" \
    "$SPE_MEANING" "$SPE_FIX" "stig:V-215206"
}
