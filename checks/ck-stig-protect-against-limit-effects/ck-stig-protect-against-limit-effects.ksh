...
AIXRAY_TOOL=ck-stig-protect-against-limit-effects


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_protect_against_limit_effects — DISA STIG V-215399.
  # Required: bos.net.tcp.client_core COMMITTED and clean_partial_conns=1.
  # Uncertain probes refuse; a missing package or tunable is a finding.
  typeset PALE_LSLPP_RAW PALE_LSLPP_RC PALE_NO_RAW PALE_NO_RC
  typeset PALE_PKG PALE_TUN PALE_REASON PALE_STATUS PALE_SEV
  typeset PALE_OBSERVED PALE_MEANING PALE_FIX PALE_PARSE_RC

  # Pipeline runs inside aix via sh -c; a quoted pipeline is not an executable.
  PALE_LSLPP_RAW=$(aix lslpp_tcp_client_core sh -c 'lslpp -l | grep bos.net.tcp.client_core')
  PALE_LSLPP_RC=$?
  PALE_NO_RAW=$(aix no_clean_partial_conns /usr/sbin/no -o clean_partial_conns)
  PALE_NO_RC=$?
  PALE_PKG=unknown
  PALE_TUN=unknown

  if [ "$PALE_LSLPP_RC" -ne 0 ]; then
    PALE_PKG=pkg_failed
  elif [ -z "$PALE_LSLPP_RAW" ]; then
    PALE_PKG=pkg_empty
  else
    PALE_PKG=$(printf '%s\n' "$PALE_LSLPP_RAW" | awk '
      /^[ \t]*$/ { next }
      /^lslpp: 0504-132[ \t]+Fileset bos\.net\.tcp\.client_core not installed\.?[ \t]*$/ {
        absent = 1
        next
      }
      $1 == "bos.net.tcp.client_core" {
        if (NF < 3 || $2 !~ /^[0-9][0-9.]*$/ ||
            $3 !~ /^(COMMITTED|APPLIED|BROKEN|OBSOLETE|EFIX_LOCKED)$/) {
          bad = 1
          next
        }
        if ($3 == "COMMITTED") committed = 1
        else uncommitted = 1
        next
      }
      { bad = 1 }
      END {
        if (bad || (absent && (committed || uncommitted))) exit 1
        if (uncommitted) print "pkg_uncommitted"
        else if (committed) print "pkg_committed"
        else print "pkg_absent"
      }
    ')
    PALE_PARSE_RC=$?
    if [ "$PALE_PARSE_RC" -ne 0 ] || [ -z "$PALE_PKG" ]; then
      PALE_PKG=pkg_unparseable
    fi
  fi

  if [ "$PALE_NO_RC" -ne 0 ]; then
    PALE_TUN=tun_failed
  elif [ -z "$PALE_NO_RAW" ]; then
    PALE_TUN=tun_empty
  else
    PALE_TUN=$(printf '%s\n' "$PALE_NO_RAW" | awk '
      /^[ \t]*$/ { next }
      $1 == "clean_partial_conns" && $2 == "=" && NF == 3 {
        if ($3 !~ /^[0-9]+$/) { bad = 1; next }
        found++
        val = $3 + 0
        next
      }
      /0552-011/ && /clean_partial_conns/ {
        absent = 1
        next
      }
      { bad = 1 }
      END {
        if (bad || found > 1 || (found && absent)) exit 1
        if (found == 1 && val == 1) print "tun_ok"
        else if (found == 1) print "tun_bad"
        else if (absent) print "tun_absent"
        else exit 1
      }
    ')
    PALE_PARSE_RC=$?
    if [ "$PALE_PARSE_RC" -ne 0 ] || [ -z "$PALE_TUN" ]; then
      PALE_TUN=tun_unparseable
    fi
  fi

  case "$PALE_PKG:$PALE_TUN" in
    pkg_failed:*|*:tun_failed) PALE_REASON=probe_failed ;;
    pkg_empty:*|*:tun_empty) PALE_REASON=probe_empty ;;
    pkg_unparseable:*|*:tun_unparseable) PALE_REASON=unparseable ;;
    pkg_committed:tun_ok) PALE_REASON=compliant ;;
    pkg_committed:tun_bad|pkg_uncommitted:*) PALE_REASON=noncompliant ;;
    pkg_absent:*|*:tun_absent) PALE_REASON=subject_absent ;;
    *) PALE_REASON=unparseable ;;
  esac

  case "$PALE_REASON" in
    compliant)
      PALE_STATUS=PASS
      PALE_OBSERVED="bos.net.tcp.client_core COMMITTED; clean_partial_conns=1"
      PALE_MEANING="bos.net.tcp.client_core is COMMITTED and clean_partial_conns is 1, so AIX rate-limits half-open TCP connections."
      PALE_FIX="n/a"
      ;;
    noncompliant) PALE_STATUS=FAIL
      PALE_OBSERVED="clean_partial_conns=0 (or bos.net.tcp.client_core is not COMMITTED)"
      PALE_MEANING="Half-open TCP cleanup is disabled or bos.net.tcp.client_core is not COMMITTED, so Denial of Service effects are not rate-limited."
      PALE_FIX="# /usr/sbin/no -o clean_partial_conns=1"
      ;;
    subject_absent) PALE_STATUS=FAIL
      PALE_OBSERVED="required DoS rate-limit subject is absent (bos.net.tcp.client_core and/or clean_partial_conns)"
      PALE_MEANING="bos.net.tcp.client_core is not installed and/or clean_partial_conns is not present, so Denial of Service rate-limiting cannot be applied."
      PALE_FIX="# /usr/sbin/no -o clean_partial_conns=1"
      ;;
    probe_failed) PALE_STATUS=NOT_ASSESSED
      PALE_OBSERVED="not assessed - DoS rate-limit probe failed (lslpp_rc=$PALE_LSLPP_RC no_rc=$PALE_NO_RC)"
      PALE_MEANING="PTxray did not obtain trustworthy bos.net.tcp.client_core and clean_partial_conns evidence."
      PALE_FIX="verify that 'lslpp -l | grep bos.net.tcp.client_core' and '/usr/sbin/no -o clean_partial_conns' run successfully, then rerun PTxray."
      ;;
    probe_empty) PALE_STATUS=NOT_ASSESSED
      PALE_OBSERVED="not assessed - DoS rate-limit probe returned no output"
      PALE_MEANING="PTxray did not obtain trustworthy bos.net.tcp.client_core and clean_partial_conns evidence."
      PALE_FIX="verify that 'lslpp -l | grep bos.net.tcp.client_core' and '/usr/sbin/no -o clean_partial_conns' return complete evidence, then rerun PTxray."
      ;;
    *)
      PALE_STATUS=NOT_ASSESSED
      PALE_OBSERVED="not assessed - DoS rate-limit evidence was unparseable"
      PALE_MEANING="PTxray did not obtain trustworthy bos.net.tcp.client_core and clean_partial_conns evidence."
      PALE_FIX="verify bos.net.tcp.client_core fileset state and clean_partial_conns, then rerun PTxray."
      ;;
  esac

  case "$PALE_STATUS" in
    NOT_APPLICABLE) PALE_SEV=low ;;
    *) PALE_SEV=med ;;
  esac

  add security stig_protect_against_limit_effects "DoS rate-limiting (clean_partial_conns)" \
    "$PALE_STATUS" "$PALE_SEV" "$PALE_OBSERVED" \
    "$PALE_MEANING" "$PALE_FIX" "stig:V-215399"
}
