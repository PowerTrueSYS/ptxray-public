...
AIXRAY_TOOL=ck-stig-pubdir-owner


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_pubdir_owner — DISA STIG V-215190 (SV-215190r991589_rule,
  # AIX7-00-001031, severity medium, CCI-000366): all AIX public directories
  # (world-writable with the sticky bit set) must be owned by root or an
  # application account.
  #
  # PARTIAL-AUTO. The decidable sub-condition is the public-directory scan plus
  # root ownership: a bare numeric uid or a well-known system account is a
  # decidable FAIL, and a named non-root account that could be an application
  # account needs ISSO judgement, so it renders the typed NOT_ASSESSED naming
  # that clause — never a silent PASS. The probe wraps find with a completion
  # sentinel so a clean scan that finds zero public directories (a required
  # configuration, hence FAIL, never PASS) is distinguishable from a swallowed
  # or truncated capture (NOT_ASSESSED). find/ls rc 1 or 2 with printed rows is
  # partial evidence: grade the rows; a clean partial PASS carries a WARN-level
  # unreadable-path detail (count from aixv stderr, else 'some paths unreadable').
  # Read-only; the fix text is operator guidance and is never executed.
  typeset PDO_RAW PDO_RC PDO_PARSED PDO_PARSE_RC PDO_GRADE
  typeset PDO_ARG1 PDO_ARG2 PDO_REST PDO_UNREAD
  typeset PDO_STATUS PDO_SEVERITY PDO_OBSERVED PDO_MEANING PDO_FIX
  typeset PDO_DETAIL_TYPE PDO_DETAIL_TEXT

  PDO_RAW=$(aixv pubdir_ls sh -c 'find / -type d -perm -1002 -exec ls -ld {} \;; echo "pubdir-scan-complete rc=$?"')
  PDO_RC=$?
  PDO_STATUS=""
  PDO_SEVERITY=med
  PDO_OBSERVED=""
  ...
  if [ "$PDO_RC" -eq 0 ]; then
    PDO_GRADE=1
  elif [ "$PDO_RC" -eq 1 ] || [ "$PDO_RC" -eq 2 ]; then
    [ -n "$PDO_RAW" ] && PDO_GRADE=1
  fi
  if [ "$PDO_GRADE" -eq 0 ]; then
    PDO_STATUS=NOT_ASSESSED
    PDO_OBSERVED="not assessed - find probe failed (rc=$PDO_RC)"
  else
    PDO_PARSED=$(printf '%s\n' "$PDO_RAW" | awk -v wrap_rc="$PDO_RC" '
      BEGIN {
        n = split("bin daemon adm sys nobody uucp nuucp guest lp lpd operator shutdown halt", sarr, " ")
        for (i = 1; i <= n; i++) sysacct[sarr[i]] = 1
      }
      {
        line = $0
        sub(/^[ \t]+/, "", line)
        sub(/[ \t]+$/, "", line)
        if (line == "") next
        if (line ~ /^pubdir-scan-complete rc=[0-9]+$/) {
          sl = line
          sub(/^pubdir-scan-complete rc=/, "", sl)
          sentinel_rc = sl + 0
          sentinel = 1
          next
        }
        if (line ~ /^find:/ || line ~ /^ls:/) { unread++; next }
        if (NF < 9) { malformed = 1; next }
        mode = $1
        if (length("" mode) == 11 && substr(mode, 11, 1) == "+") mode = substr(mode, 1, 10)
        if (length("" mode) != 10) { malformed = 1; next }
        if (substr(mode, 1, 1) != "d") { malformed = 1; next }
        if (substr(mode, 9, 1) != "w") { malformed = 1; next }
        if (substr(mode, 10, 1) != "t" && substr(mode, 10, 1) != "T") { malformed = 1; next }
        if ($2 !~ /^[0-9][0-9]*$/) { malformed = 1; next }
        owner = $3
        group = $4
        if (owner !~ /^[A-Za-z0-9_][A-Za-z0-9_.-]*$/) { malformed = 1; next }
        if (group !~ /^[A-Za-z0-9_][A-Za-z0-9_.-]*$/) { malformed = 1; next }
        if (length("" owner) > 32 || length("" group) > 32) { malformed = 1; next }
        if ($5 !~ /^[0-9][0-9]*$/) { malformed = 1; next }
        if (length("" $6) < 1 || length("" $6) > 16) { malformed = 1; next }
        if ($7 !~ /^[0-9][0-9]*$/ || ($7 + 0) < 1 || ($7 + 0) > 31) { malformed = 1; next }
        date_ok = 0
        date_field = $8
        if (length("" date_field) == 4 && date_field ~ /^[0-9][0-9][0-9][0-9]$/) {
          date_ok = 1
        } else if (length("" date_field) == 5 && substr(date_field, 3, 1) == ":") {
          hour = substr(date_field, 1, 2)
          minute = substr(date_field, 4, 2)
          if (hour ~ /^[0-9][0-9]$/ && minute ~ /^[0-9][0-9]$/ &&
              (hour + 0) <= 23 && (minute + 0) <= 59) date_ok = 1
        }
        if (!date_ok) { malformed = 1; next }
        path = $9
        for (i = 10; i <= NF; i++) path = path " " $i
        if (substr(path, 1, 1) != "/") { malformed = 1; next }
        ndir++
        if (owner ~ /^[0-9][0-9]*$/) {
        ...
        if (owner == "root" || owner == "0") {
          rootdir++
        } else if (owner ~ /^[0-9][0-9]*$/) {
          if (badfail == 0) { badfail = 1; badowner = owner; badpath = path }
        } else if (owner in sysacct) {
          if (badfail == 0) { badfail = 1; badowner = owner; badpath = path }
        } else {
          if (manual == 0) { manual = 1; manowner = owner; manpath = path }
        }
      }
      END {
        partial = (sentinel_rc == 1 || sentinel_rc == 2 || wrap_rc == 1 || wrap_rc == 2)
        if (sentinel != 1) { print "NS"; exit 0 }
        if (sentinel_rc != 0 && !partial) { print "NR " sentinel_rc; exit 0 }
        if (malformed) { print "E"; exit 0 }
        if (badfail) { print "F|" badowner "|" badpath; exit 0 }
        if (manual) { print "M|" manowner "|" manpath; exit 0 }
        if (ndir == 0) { print "Z"; exit 0 }
        if (partial) {
          if (unread > 0) { print "C|" rootdir "|P|" unread; exit 0 }
          print "C|" rootdir "|P"
          exit 0
        }
        print "C|" rootdir
    ...
    PDO_PARSE_RC=$?
    if [ "$PDO_PARSE_RC" -ne 0 ] || [ -z "$PDO_PARSED" ]; then
      PDO_STATUS=NOT_ASSESSED
      PDO_OBSERVED="not assessed - public directory listing was unparseable"
    else
      case "$PDO_PARSED" in
        C\|*)
          PDO_REST=${PDO_PARSED#C\|}
          PDO_ARG1=${PDO_REST%%\|*}
          PDO_STATUS=PASS
          PDO_OBSERVED="$PDO_ARG1 world-writable sticky public director(s), all owned by root"
          PDO_MEANING="Every public directory — a world-writable directory with the sticky bit set — is owned by root, so no non-root identity can manage the files public users may write."
          if [ "$PDO_REST" != "$PDO_ARG1" ]; then
            PDO_ARG2=${PDO_REST#*\|}
            PDO_UNREAD=${PDO_ARG2#P}
            PDO_UNREAD=${PDO_UNREAD#\|}
            if [ -n "$PDO_UNREAD" ]; then
              PDO_DETAIL_TEXT="$PDO_UNREAD unreadable paths"
            else
              PDO_DETAIL_TEXT="some paths unreadable"
            fi
            PDO_DETAIL_TYPE=WARN
            PDO_OBSERVED="$PDO_OBSERVED; $PDO_DETAIL_TEXT"
            PDO_MEANING="Every listed public directory — a world-writable directory with the sticky bit set — is owned by root, so no non-root identity can manage the files public users may write. Some paths were unreadable, so the scan is partial."
          fi
          PDO_FIX="n/a"
          ;;
        Z)
          PDO_STATUS=FAIL
          PDO_OBSERVED="no world-writable sticky public directory was found; the required public-directory configuration is unmet"
          PDO_MEANING="DISA STIG V-215190 polices the ownership of public directories; a host with no public directory has not demonstrated the required configuration, so the requirement is unmet rather than satisfied."
          PDO_FIX="confirm with the site whether public directories are in scope; where they are, create each required public directory owned by root and set its mode, then rerun PTxray (PTxray only recommends these actions)."
          ;;
        F\|*)
          PDO_REST=${PDO_PARSED#*\|}
          PDO_ARG1=${PDO_REST%%\|*}
          PDO_ARG2=${PDO_REST#*\|}
          PDO_STATUS=FAIL
          PDO_OBSERVED="public dir $PDO_ARG2 owner '$PDO_ARG1' is neither root nor an application account"
          PDO_MEANING="A public directory is owned by an identity that is neither root nor a site application account, so that identity controls a directory in which any local user may write files."
          PDO_FIX="correct the ownership after validation: 'chown root $PDO_ARG2'; PTxray only recommends this command and never executes it."
          ;;
        M\|*)
          PDO_REST=${PDO_PARSED#*\|}
          PDO_ARG1=${PDO_REST%%\|*}
          PDO_ARG2=${PDO_REST#*\|}
          PDO_STATUS=NOT_ASSESSED
          PDO_OBSERVED="not assessed - public dir $PDO_ARG2 owner '$PDO_ARG1' needs ISSO classification as an application account"
          PDO_MEANING="PTxray can only decide the root-ownership sub-condition; whether a named non-root owner is an approved application account is a site judgement this control leaves to the ISSO."
          PDO_FIX="have the ISSO classify '$PDO_ARG1'; if it is not an approved application account, run 'chown root $PDO_ARG2' after validation; PTxray only recommends these actions."
          ;;
        NS)
          PDO_STATUS=NOT_ASSESSED
          PDO_OBSERVED="not assessed - public directory scan did not complete"
          PDO_MEANING="The find probe produced no completion sentinel, so a clean-scan result cannot be told apart from a swallowed or truncated capture."
          PDO_FIX="rerun PTxray so the public-directory scan completes, then reassess."
          ;;
        NR*)
          PDO_ARG1=${PDO_PARSED#NR }
          PDO_STATUS=NOT_ASSESSED
          PDO_OBSERVED="not assessed - public directory scan failed (rc=$PDO_ARG1)"
          PDO_MEANING="The find probe reached the completion sentinel but the scan exited non-zero, so the listing is not a complete scan."
          PDO_FIX="rerun PTxray so the public-directory scan completes with rc 0, then reassess."
          ;;
        E)
          PDO_STATUS=NOT_ASSESSED
          PDO_OBSERVED="not assessed - public directory listing was unparseable"
          PDO_MEANING="The public-directory listing contained rows that are not trustworthy ls -ld directory records, so the owner cannot be determined."
          PDO_FIX="run 'find / -type d -perm -1002 -exec ls -ld {} \\;' as root, correct the capture problem, and rerun PTxray."
          ;;
        *)
          PDO_STATUS=NOT_ASSESSED
          PDO_OBSERVED="not assessed - public directory listing was unparseable"
          PDO_MEANING="PTxray did not obtain a trustworthy public-directory owner inventory."
          PDO_FIX="rerun PTxray so the public-directory scan completes, then reassess."
          ;;
      esac
    fi
  ...
  if [ -n "$PDO_DETAIL_TYPE" ]; then
    add security stig_pubdir_owner "Public directory ownership" \
      "$PDO_STATUS" "$PDO_SEVERITY" "$PDO_OBSERVED" "$PDO_MEANING" "$PDO_FIX" "stig:V-215190" \
      "$PDO_DETAIL_TYPE" "$PDO_DETAIL_TEXT"
  else
    add security stig_pubdir_owner "Public directory ownership" \
      "$PDO_STATUS" "$PDO_SEVERITY" "$PDO_OBSERVED" "$PDO_MEANING" "$PDO_FIX" "stig:V-215190"
  fi
