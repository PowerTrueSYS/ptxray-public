...
AIXRAY_TOOL=ck-stig-rc-ldr-preload


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_rc_ldr_preload — DISA STIG V-215422.
  # RC-script LDR_PRELOAD lists must be absolute paths or $PATH, with no
  # empty colon slots. Missing subject is a finding (required configuration).
  # grep no-match is rc 1 with empty stdout and is that absence. Comments
  # and incidental LDR_PRELOAD text are not active assignments.
  typeset RCLP_RAW RCLP_RC RCLP_CLASS RCLP_PARSE_RC
  typeset RCLP_STATUS RCLP_OBSERVED RCLP_MEANING RCLP_FIX RCLP_SEV

  RCLP_RAW=$(aix grep_rc_ldr_preload grep -r LDR_PRELOAD /etc/rc*)
  RCLP_RC=$?
  RCLP_CLASS=""

  if [ "$RCLP_RC" -ne 0 ] && [ "$RCLP_RC" -ne 1 ]; then
    RCLP_CLASS=probe_failed
  elif [ -z "$RCLP_RAW" ]; then
    if [ "$RCLP_RC" -eq 1 ]; then
      RCLP_CLASS=absent
    else
      RCLP_CLASS=probe_empty
    fi
  else
    RCLP_CLASS=$(printf '%s\n' "$RCLP_RAW" | awk '
      {
        ...
        gsub(/\r/, "", line)
        if (line ~ /^[ \t]*$/) next
        assign = index(line, "LDR_PRELOAD=")
        if (assign == 0) next
        colon = index(line, ":")
        if (colon > 0 && colon < assign) rest = substr(line, colon + 1)
        else rest = line
        gsub(/^[ \t]+/, "", rest)
        if (substr(rest, 1, 1) == "#") next
        if (rest ~ /^export[ \t]+/) sub(/^export[ \t]+/, "", rest)
        else if (rest ~ /^typeset[ \t]+/) sub(/^typeset[ \t]+/, "", rest)
        if (rest !~ /^LDR_PRELOAD=/) next
        val = rest
        sub(/^LDR_PRELOAD=/, "", val)
        sc = index(val, ";")
        if (sc > 0) val = substr(val, 1, sc - 1)
        gsub(/^[ \t]+/, "", val)
        gsub(/[ \t]+$/, "", val)
        sub(/^"/, "", val)
        sub(/"$/, "", val)
    ...
    ')
    RCLP_PARSE_RC=$?
    if [ "$RCLP_PARSE_RC" -ne 0 ] || [ -z "$RCLP_CLASS" ]; then
      RCLP_CLASS=probe_failed
    fi
  fi
  ...
  case "$RCLP_CLASS" in
    ok)
      RCLP_STATUS=PASS
      RCLP_OBSERVED='rc LDR_PRELOAD lists contain only absolute paths or $PATH'
      ;;
    bad) RCLP_STATUS=FAIL
      RCLP_OBSERVED="relative or empty LDR_PRELOAD entries in rc scripts"
      ;;
    absent) RCLP_STATUS=FAIL
      RCLP_OBSERVED="no LDR_PRELOAD list in /etc/rc* (required configuration unmet)"
      ;;
    probe_failed) RCLP_STATUS=NOT_ASSESSED
      RCLP_OBSERVED="not assessed - rc LDR_PRELOAD probe failed (rc=$RCLP_RC)"
      ;;
    probe_empty) RCLP_STATUS=NOT_ASSESSED
      RCLP_OBSERVED="not assessed - rc LDR_PRELOAD probe returned no output"
      ;;
    *)
      RCLP_STATUS=NOT_ASSESSED
      RCLP_OBSERVED="not assessed - rc LDR_PRELOAD evidence was unparseable"
      ;;
  esac

  case "$RCLP_STATUS" in
    PASS)
      RCLP_MEANING="Run-control LDR_PRELOAD lists name only absolute paths or \$PATH, with no empty colon slots."
      RCLP_FIX="n/a"
      ;;
    FAIL)
      RCLP_MEANING="A relative or empty LDR_PRELOAD entry in a run-control script can load an unintended library, or the required preload list is missing."
      RCLP_FIX="Edit the run control scripts' library preload list and remove relative paths."
      ;;
    *)
      RCLP_MEANING="PTxray did not obtain trustworthy evidence for run-control LDR_PRELOAD lists."
      RCLP_FIX="verify that 'grep -r LDR_PRELOAD /etc/rc*' runs successfully, then rerun PTxray."
      ;;
  esac

  case "$RCLP_STATUS" in
    NOT_APPLICABLE) RCLP_SEV=low ;;
    *) RCLP_SEV=med ;;
  esac

  add security stig_rc_ldr_preload "Control script LDR_PRELOAD paths" \
    "$RCLP_STATUS" "$RCLP_SEV" "$RCLP_OBSERVED" \
    "$RCLP_MEANING" "$RCLP_FIX" "stig:V-215422"
}
