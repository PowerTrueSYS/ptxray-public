...
AIXRAY_TOOL=ck-stig-rc-libpath


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_rc_libpath — DISA STIG V-215421 (AIX7-00-003126).
  # AIX run control scripts' LIBPATH search paths must contain only absolute
  # paths; the $PATH variable is accepted because it expands to absolute
  # directories.  The /etc/rc* glob is quoted so the local shell does not
  # expand it: the probe reaches aix() as the literal manifest command and
  # the aix wrapper / remote runner expands the pattern on the target.
  # grep's exit status is authoritative: 0 = matches found,
  # 1 = no LIBPATH subject (required configuration absent -> FAIL), and any
  # other nonzero rc with empty output = probe error (refuse, never PASS).
  typeset RL_RAW RL_RC RL_SHAPE RL_STATUS RL_OBSERVED RL_MEANING RL_FIX

  RL_RAW=$(aix rc_libpath grep -r LIBPATH '/etc/rc*')
  RL_RC=$?
  ...
  if [ -n "$RL_RAW" ]; then
    RL_SHAPE=$(printf '%s\n' "$RL_RAW" | awk '
      {
        line = $0
        p = index(line, ":")
        if (p == 0) { refuse = 1; next }   # no file:content separator -> unparseable
        rest = substr(line, p + 1)
        q = index(rest, "LIBPATH=")
        if (q == 0) { hit = 1; next }      # mentions LIBPATH but assigns nothing
        hit = 1
        assign = 1
        value = substr(rest, q + 8)
        gsub(/[ \t]/, "", value)
        n = split(value, entry, ":")
        for (i = 1; i <= n; i++) {
          if (entry[i] == "") { bad = 1 }
          else if (substr(entry[i], 1, 1) == "/") { }
          else if (index(entry[i], "$PATH") == 1) { }
          else { bad = 1 }
        }
      }
      END {
        if (refuse) { print "refuse"; exit }
        if (bad) { print "bad"; exit }
        if (assign) { print "ok"; exit }
    ...
    case "$RL_SHAPE" in
      ok)
        RL_STATUS=PASS
        RL_OBSERVED="every LIBPATH entry in /etc/rc* scripts is absolute (or \$PATH)"
        ;;
      bad)
        RL_STATUS=FAIL
        RL_OBSERVED="an LIBPATH entry in /etc/rc* scripts is empty or relative (not absolute, not \$PATH)"
        ;;
      absent)
        RL_STATUS=FAIL
        RL_OBSERVED="LIBPATH appears in /etc/rc* scripts but no LIBPATH= assignment is made; the required configuration is absent"
        ;;
      refuse)
        RL_STATUS=NOT_ASSESSED
        RL_OBSERVED="not assessed - LIBPATH grep output is unparseable (a matched line has no file:content separator)"
        ;;
      *)
        RL_STATUS=NOT_ASSESSED
        RL_OBSERVED="not assessed - LIBPATH grep output could not be parsed"
        ;;
    esac
  elif [ "$RL_RC" -eq 1 ]; then
    RL_STATUS=FAIL
    RL_OBSERVED="no LIBPATH assignment found in /etc/rc* scripts; the required configuration is absent"
  elif [ "$RL_RC" -eq 0 ]; then
    RL_STATUS=NOT_ASSESSED
    RL_OBSERVED="not assessed - LIBPATH probe returned rc 0 with empty output (a swallowed error)"
  else
    RL_STATUS=NOT_ASSESSED
    RL_OBSERVED="not assessed - LIBPATH probe failed (rc=$RL_RC)"
  fi

  case "$RL_STATUS" in
    PASS)
      RL_MEANING="Library search paths used by AIX run control scripts contain only absolute directories (or the absolute \$PATH variable), so the dynamic loader cannot be steered to an unintended directory."
      RL_FIX="n/a"
      ;;
    FAIL)
      RL_MEANING="A run control script sets LIBPATH with an empty entry (leading, trailing, or doubled colon) or an entry that is not an absolute path, which can load a substituted library from a directory the site does not intend."
      RL_FIX="Edit run control scripts' library search PATH variables.; Remove empty entries or entries that are not absolute paths."
      ;;
    *)
      RL_MEANING="PTxray did not obtain trustworthy evidence of the LIBPATH entries in the run control scripts."
      RL_FIX="verify the LIBPATH assignments in /etc/rc* scripts as root, then rerun PTxray."
      ;;
  esac

  add security stig_rc_libpath "AIX control scripts library search paths" \
    "$RL_STATUS" med "$RL_OBSERVED" "$RL_MEANING" "$RL_FIX" "stig:V-215421"
}
