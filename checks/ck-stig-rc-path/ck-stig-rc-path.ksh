...
AIXRAY_TOOL=ck-stig-rc-path


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_rc_path — DISA STIG V-215345.
  # Run-control PATH must have no empty entries and no relative components.
  # Only active PATH=/PATH+= assignments after optional export/typeset/readonly
  # are evaluated. Missing /etc/rc* (glob unmatched) is unmet required
  # configuration (FAIL). Mixed matches plus per-file errors are a refusal.
  typeset RCP_RAW RCP_RC RCP_CLASS RCP_PARSE_RC
  typeset RCP_STATUS RCP_OBSERVED RCP_MEANING RCP_FIX

  RCP_RAW=$(aix grep_rc_path grep -r PATH /etc/rc*)
  RCP_RC=$?
  RCP_STATUS=""
  RCP_OBSERVED=""
  ...
  if [ -z "$RCP_RAW" ]; then
    if [ "$RCP_RC" -eq 0 ]; then
      RCP_STATUS=NOT_ASSESSED
      RCP_OBSERVED="not assessed - rc PATH probe returned no output"
    else
      RCP_STATUS=NOT_ASSESSED
      RCP_OBSERVED="not assessed - rc PATH probe failed (rc=$RCP_RC)"
    fi
  elif [ "$RCP_RC" -ne 0 ]; then
    RCP_CLASS=$(printf '%s\n' "$RCP_RAW" | awk '
      {
        gsub(/\r/, "")
        if ($0 ~ /^[ \t]*$/) next
        if ($0 ~ /^grep:/) {
          if (index($0, "/etc/rc*") > 0 && \
              ($0 ~ /No such file/ || $0 ~ /does not exist/ || \
               $0 ~ /can.t open/ || $0 ~ /cannot open/))
            glob_miss++
          else
            other_err++
          next
      ...
      END {
        if (hits == 0 && other_err == 0 && glob_miss > 0) print "absent"
        else print "mixed"
      }
    ')
    RCP_PARSE_RC=$?
    if [ "$RCP_PARSE_RC" -ne 0 ] || [ -z "$RCP_CLASS" ]; then
      RCP_STATUS=NOT_ASSESSED
      RCP_OBSERVED="not assessed - rc PATH output was unparseable"
    elif [ "$RCP_CLASS" = "absent" ]; then
      RCP_STATUS=FAIL
      RCP_OBSERVED="run control scripts are absent under /etc/rc*"
    else
      RCP_STATUS=NOT_ASSESSED
      RCP_OBSERVED="not assessed - rc PATH probe mixed matches with errors"
    fi
  else
    RCP_CLASS=$(printf '%s\n' "$RCP_RAW" | awk '
      function unquote_word(s,    n, i, ch, q, out) {
        n = length("" s)
        q = ""
        out = ""
        for (i = 1; i <= n; i++) {
      ...
      function check_val(val,    n, i, p) {
        if (val == "") {
          empty_any = 1
          return
        }
        if (substr(val, 1, 1) == ":") empty_any = 1
        if (substr(val, length("" val), 1) == ":") empty_any = 1
        if (index(val, "::") > 0) empty_any = 1
        n = split(val, a, ":")
        for (i = 1; i <= n; i++) {
          p = a[i]
          gsub(/^[ \t]+/, "", p)
          gsub(/[ \t]+$/, "", p)
          if (p == "") continue
          if (p != "$PATH" && substr(p, 1, 1) != "/") rel_any = 1
        }
      }
      {
        gsub(/\r/, "")
        ...
        col = index(line, ":")
        if (col == 0) next
        body = substr(line, col + 1)
        gsub(/^[ \t]+/, "", body)
        if (body == "" || substr(body, 1, 1) == "#") next
        while (body ~ /^(export|typeset|readonly)[ \t]+/) {
          sub(/^(export|typeset|readonly)[ \t]+/, "", body)
        }
        if (body ~ /^PATH\+=/) val = substr(body, 7)
        else if (body ~ /^PATH=/) val = substr(body, 6)
        else next
        found++
        check_val(unquote_word(val))
      }
      END {
        if (found == 0) { print "none"; exit }
        if (empty_any && rel_any) print "empty,relative"
        else if (empty_any) print "empty"
        else if (rel_any) print "relative"
        else print "ok"
      }
    ')
    RCP_PARSE_RC=$?
    if [ "$RCP_PARSE_RC" -ne 0 ] || [ -z "$RCP_CLASS" ]; then
      RCP_STATUS=NOT_ASSESSED
      RCP_OBSERVED="not assessed - rc PATH output was unparseable"
    elif [ "$RCP_CLASS" = "none" ]; then
      RCP_STATUS=NOT_ASSESSED
      RCP_OBSERVED="not assessed - rc PATH output had no PATH assignment"
    elif [ "$RCP_CLASS" = "ok" ]; then
      RCP_STATUS=PASS
      RCP_OBSERVED="run control PATH entries are absolute"
    elif [ "$RCP_CLASS" = "empty" ]; then
      RCP_STATUS=FAIL
      RCP_OBSERVED="run control PATH has empty entries"
    elif [ "$RCP_CLASS" = "relative" ]; then
      RCP_STATUS=FAIL
      RCP_OBSERVED="run control PATH has relative entries"
    else
      RCP_STATUS=FAIL
      RCP_OBSERVED="run control PATH has empty and relative entries"
    fi
  fi

  case "$RCP_STATUS" in
    PASS)
      RCP_MEANING="Run control scripts set executable search paths to absolute entries only."
      RCP_FIX="n/a"
      ;;
    FAIL)
      RCP_MEANING="A run control PATH contains an empty or relative entry, or the required run control scripts are absent."
      RCP_FIX="Edit the run control script and remove the relative path entry from the executable search path variable."
      ;;
    *)
      RCP_MEANING="PTxray did not obtain trustworthy evidence for run control PATH entries."
      RCP_FIX="verify grep -r PATH /etc/rc* is readable, then rerun PTxray."
      ;;
  esac

  add security stig_rc_path "Run control executable search paths" \
    "$RCP_STATUS" med "$RCP_OBSERVED" "$RCP_MEANING" "$RCP_FIX" \
    "stig:V-215345"
}
