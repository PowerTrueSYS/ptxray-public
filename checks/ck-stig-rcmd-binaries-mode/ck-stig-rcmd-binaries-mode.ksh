...
AIXRAY_TOOL=ck-stig-rcmd-binaries-mode


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_rcmd_binaries_mode — DISA STIG V-215322.
  # rcp, rlogin, rsh, rexec and telnet must be mode 0000 (----------) or absent.  # network-lint: allow -- prose comment naming the r-command and telnet client binaries as subjects/path arguments; PTxray never executes them; no network call
  typeset RCMD_STATUS RCMD_SEV RCMD_OBSERVED RCMD_MEANING RCMD_FIX
  typeset RCMD_HAS_BAD RCMD_REFUSE RCMD_BITS
  typeset RCMD_RCP_RAW RCMD_RCP_RC
  typeset RCMD_RLOGIN_RAW RCMD_RLOGIN_RC
  typeset RCMD_RSH_RAW RCMD_RSH_RC
  typeset RCMD_TELNET_RAW RCMD_TELNET_RC
  typeset RCMD_REXEC_RAW RCMD_REXEC_RC

  RCMD_HAS_BAD=0
  RCMD_REFUSE=""
  RCMD_BITS=""

  function rcmd_ingest {
    typeset name binpath raw rc parsed mode prc
    name=$1
    binpath=$2
    raw=$3
    rc=$4
    ...
    if [ "$rc" -eq 2 ]; then
      ...
      mode=unreadable
      [ -n "$RCMD_REFUSE" ] || RCMD_REFUSE="not assessed - ls -l of ${binpath} failed (rc=${rc})"
    elif [ -z "$raw" ]; then
      mode=empty
      [ -n "$RCMD_REFUSE" ] || RCMD_REFUSE="not assessed - empty ls -l output for ${binpath} (rc=0)"
    else
      parsed=$(printf '%s\n' "$raw" | awk '
        NF {
          n++
        ...
        END {
          if (n != 1) exit 1
          if (m == "----------") { print "ok"; exit 0 }
          if (m ~ /^[-bcdlps][-r][-w][-xsS][-r][-w][-xsS][-r][-w][-xtT][+@]?$/) {
            print "bad:" m
            exit 0
          }
          exit 1
      ...
      if [ "$prc" -ne 0 ] || [ -z "$parsed" ]; then
        mode=unparseable
        [ -n "$RCMD_REFUSE" ] || RCMD_REFUSE="not assessed - unparseable ls -l output for ${binpath}"
      elif [ "$parsed" = "ok" ]; then
        mode="----------"
      else
        mode=${parsed#bad:}
    ...
    if [ -z "$RCMD_BITS" ]; then
      ...
      RCMD_BITS="${RCMD_BITS} ${name}=${mode}"
    fi
  }

  # ls -l /usr/bin/rcp | awk '{print $1}'  # network-lint: allow -- prose comment; 'rcp' is a /usr/bin path argument, never executed; no network call
  RCMD_RCP_RAW=$(aix usr_bin_rcp ls -l /usr/bin/rcp)  # network-lint: allow -- aix() capture of a local read-only command; 'rcp' is a /usr/bin path argument, never executed; no network call
  RCMD_RCP_RC=$?
  # Evidence is forwarded only when the probe succeeded and returned output.
  if [ "$RCMD_RCP_RC" -eq 0 ] && [ -n "$RCMD_RCP_RAW" ]; then
    rcmd_ingest rcp /usr/bin/rcp "$RCMD_RCP_RAW" "$RCMD_RCP_RC"  # network-lint: allow -- local helper-function argument; 'rcp' is a /usr/bin path argument, never executed; no network call
  else
    rcmd_ingest rcp /usr/bin/rcp "" "$RCMD_RCP_RC"  # network-lint: allow -- local helper-function argument; 'rcp' is a /usr/bin path argument, never executed; no network call
  fi

  # ls -l /usr/bin/rlogin | awk '{print $1}'
  RCMD_RLOGIN_RAW=$(aix usr_bin_rlogin ls -l /usr/bin/rlogin)
  RCMD_RLOGIN_RC=$?
  # Evidence is forwarded only when the probe succeeded and returned output.
  if [ "$RCMD_RLOGIN_RC" -eq 0 ] && [ -n "$RCMD_RLOGIN_RAW" ]; then
    rcmd_ingest rlogin /usr/bin/rlogin "$RCMD_RLOGIN_RAW" "$RCMD_RLOGIN_RC"
  else
    rcmd_ingest rlogin /usr/bin/rlogin "" "$RCMD_RLOGIN_RC"
  fi

  # ls -l /usr/bin/rsh | awk '{print $1}'  # network-lint: allow -- prose comment; 'rsh' is a /usr/bin path argument, never executed; no network call
  RCMD_RSH_RAW=$(aix usr_bin_rsh ls -l /usr/bin/rsh)  # network-lint: allow -- aix() capture of a local read-only command; 'rsh' is a /usr/bin path argument, never executed; no network call
  RCMD_RSH_RC=$?
  # Evidence is forwarded only when the probe succeeded and returned output.
  if [ "$RCMD_RSH_RC" -eq 0 ] && [ -n "$RCMD_RSH_RAW" ]; then
    rcmd_ingest rsh /usr/bin/rsh "$RCMD_RSH_RAW" "$RCMD_RSH_RC"  # network-lint: allow -- local helper-function argument; 'rsh' is a /usr/bin path argument, never executed; no network call
  else
    rcmd_ingest rsh /usr/bin/rsh "" "$RCMD_RSH_RC"  # network-lint: allow -- local helper-function argument; 'rsh' is a /usr/bin path argument, never executed; no network call
  fi

  # ls -l /usr/bin/telnet | awk '{print $1}'  # network-lint: allow -- prose comment; 'telnet' is the ls -l path argument /usr/bin/telnet; no network call
  RCMD_TELNET_RAW=$(aix usr_bin_telnet ls -l /usr/bin/telnet)  # network-lint: allow -- aix() capture of a local read-only command; 'telnet' is the ls -l path argument /usr/bin/telnet; no network call
  RCMD_TELNET_RC=$?
  # Evidence is forwarded only when the probe succeeded and returned output.
  if [ "$RCMD_TELNET_RC" -eq 0 ] && [ -n "$RCMD_TELNET_RAW" ]; then
    rcmd_ingest telnet /usr/bin/telnet "$RCMD_TELNET_RAW" "$RCMD_TELNET_RC"  # network-lint: allow -- local helper-function argument; 'telnet' is the ls -l path argument /usr/bin/telnet; no network call
  else
    rcmd_ingest telnet /usr/bin/telnet "" "$RCMD_TELNET_RC"  # network-lint: allow -- local helper-function argument; 'telnet' is the ls -l path argument /usr/bin/telnet; no network call
  fi

  # ls -l /usr/bin/rexec | awk '{print $1}'  # network-lint: allow -- prose comment; 'rexec' is a /usr/bin path argument, never executed; no network call
  RCMD_REXEC_RAW=$(aix usr_bin_rexec ls -l /usr/bin/rexec)  # network-lint: allow -- aix() capture of a local read-only command; 'rexec' is a /usr/bin path argument, never executed; no network call
  RCMD_REXEC_RC=$?
  # Evidence is forwarded only when the probe succeeded and returned output.
  if [ "$RCMD_REXEC_RC" -eq 0 ] && [ -n "$RCMD_REXEC_RAW" ]; then
    rcmd_ingest rexec /usr/bin/rexec "$RCMD_REXEC_RAW" "$RCMD_REXEC_RC"  # network-lint: allow -- local helper-function argument; 'rexec' is a /usr/bin path argument, never executed; no network call
  else
    rcmd_ingest rexec /usr/bin/rexec "" "$RCMD_REXEC_RC"  # network-lint: allow -- local helper-function argument; 'rexec' is a /usr/bin path argument, never executed; no network call
  fi

  if [ "$RCMD_HAS_BAD" -ne 0 ]; then
    RCMD_STATUS=FAIL
    RCMD_SEV=high
    RCMD_OBSERVED="more permissive than 0000: ${RCMD_BITS}"
    RCMD_MEANING="One or more of /usr/bin/rcp, /usr/bin/rlogin, /usr/bin/rsh, /usr/bin/rexec or /usr/bin/telnet is more permissive than mode 0000, so a user can still start those cleartext remote-command clients."  # network-lint: allow -- finding meaning text naming the r-command and telnet client binaries as subjects/path arguments; PTxray never executes them; no network call
    RCMD_FIX="chmod ugo= /usr/bin/rcp; chmod ugo= /usr/bin/rlogin; chmod ugo= /usr/bin/rsh; chmod ugo= /usr/bin/rexec; chmod ugo= /usr/bin/telnet; PTxray only recommends these commands."  # network-lint: allow -- remediation advice text naming the r-command and telnet client binaries as chmod targets; PTxray only recommends these commands and never runs them; no network call
  elif [ -n "$RCMD_REFUSE" ]; then
    RCMD_STATUS=NOT_ASSESSED
    RCMD_SEV=med
    RCMD_OBSERVED="$RCMD_REFUSE"
    RCMD_MEANING="PTxray did not obtain trustworthy ls -l evidence for one or more of the rcmd and telnet binaries."  # network-lint: allow -- finding meaning text; 'telnet' names the STIG subject service in prose; no network call
    RCMD_FIX="correct the ls -l capture or path problem and rerun PTxray."
  else
    RCMD_STATUS=PASS
    RCMD_SEV=med
    RCMD_OBSERVED="$RCMD_BITS"
    RCMD_MEANING="The rcmd and telnet client binaries are mode 0000 or absent, so those programs cannot be executed."  # network-lint: allow -- finding meaning text; 'telnet' names the STIG subject service in prose; no network call
    RCMD_FIX="n/a"
  fi

  # network-lint: allow-next=1 -- the backslash-continued 'add' argument list; 'Rcmd and telnet' is the finding title text and a trailing comment would break the continuation
  add security stig_rcmd_binaries_mode "Rcmd and telnet command binaries disabled" \
    "$RCMD_STATUS" "$RCMD_SEV" "$RCMD_OBSERVED" \
    "$RCMD_MEANING" "$RCMD_FIX" "stig:V-215322"
}
