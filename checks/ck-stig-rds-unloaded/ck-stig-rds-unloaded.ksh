...
AIXRAY_TOOL=ck-stig-rds-unloaded


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_rds_unloaded — DISA STIG V-215394 (AIX7-00-003089, CCI-000382).
  # The Reliable Datagram Sockets (RDS) protocol must not be loaded.  The
  # probe is the manifest command  genkex | grep rds : any matching line in
  # the loaded-kernel-extension list means RDS is loaded and is a finding.
  # genkex's own return code is what separates "probe failed" from "RDS
  # absent", so the probe captures genkex's raw output and rc first, then
  # applies the same grep filter; empty output with rc 0 is a swallowed
  # error and stays NOT_ASSESSED, never PASS.
  typeset RDS_RAW RDS_RC RDS_HITS RDS_STATUS RDS_OBSERVED RDS_MEANING RDS_FIX

  RDS_RAW=$(aix rds_genkex genkex)
  RDS_RC=$?
  RDS_HITS=$(printf '%s\n' "$RDS_RAW" | grep rds)

  if [ -n "$RDS_HITS" ]; then
    RDS_STATUS=FAIL
    RDS_OBSERVED="rds matched in the kernel extension list"
  elif [ "$RDS_RC" -ne 0 ]; then
    RDS_STATUS=NOT_ASSESSED
    RDS_OBSERVED="not assessed - genkex probe failed (rc=$RDS_RC)"
  elif [ -z "$RDS_RAW" ]; then
    RDS_STATUS=NOT_ASSESSED
    RDS_OBSERVED="not assessed - genkex returned empty output with rc 0 (swallowed error, not absence)"
  else
    RDS_STATUS=PASS
    RDS_OBSERVED="no rds extension in the genkex output"
  fi

  case "$RDS_STATUS" in
    PASS)
      RDS_MEANING="RDS is not loaded, so the Reliable Datagram Sockets protocol cannot be used against this host."
      RDS_FIX="n/a"
      ;;
    FAIL)
      RDS_MEANING="The RDS kernel extension is loaded, so the Reliable Datagram Sockets protocol that DISA requires disabled is active."
      RDS_FIX="unload RDS with 'bypassctrl unload rds' and remove its boot-time load source; PTxray only recommends this command."
      ;;
    *)
      RDS_MEANING="PTxray did not obtain trustworthy evidence that RDS is unloaded."
      RDS_FIX="rerun 'genkex | grep rds' as root and confirm the RDS kernel extension is absent, then rerun PTxray."
      ;;
  esac

  add security stig_rds_unloaded "RDS protocol disabled" \
    "$RDS_STATUS" med "$RDS_OBSERVED" \
    "$RDS_MEANING" "$RDS_FIX" "stig:V-215394"
}
