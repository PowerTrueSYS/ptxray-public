...
AIXRAY_TOOL=ck-stig-reserved-gids


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_reserved_gids — DISA STIG V-215194.
  # Decidable: reserved primary GID 0-99 on a clearly non-system account
  # (UID>=100, name not in the authored set) is FAIL. Remaining
  # system-vs-non-system classification is ISSO-dependent and refuses.
  typeset SRG_RAW SRG_RC SRG_PARSED SRG_KIND SRG_DETAIL
  typeset SRG_STATUS SRG_OBSERVED SRG_MEANING SRG_FIX
  typeset SRG_FAILCLOSED_PROBE SRG_FAILCLOSED_EMPTY
  typeset SRG_FAILCLOSED_MANUAL SRG_FAILCLOSED_UNPARSE
  typeset SRG_NAME SRG_GID

  SRG_FAILCLOSED_PROBE=NOT_ASSESSED
  SRG_FAILCLOSED_EMPTY=NOT_ASSESSED
  SRG_FAILCLOSED_MANUAL=NOT_ASSESSED
  SRG_FAILCLOSED_UNPARSE=NOT_ASSESSED

  SRG_RAW=$(aix passwd more /etc/passwd)
  SRG_RC=$?

  if [ "$SRG_RC" -ne 0 ]; then
    SRG_STATUS=$SRG_FAILCLOSED_PROBE
    SRG_OBSERVED="not assessed - /etc/passwd probe failed (rc=$SRG_RC)"
  elif [ -z "$SRG_RAW" ]; then
    SRG_STATUS=$SRG_FAILCLOSED_EMPTY
    SRG_OBSERVED="not assessed - /etc/passwd probe returned empty output with rc=0"
  else
    SRG_PARSED=$(printf '%s\n' "$SRG_RAW" | awk -F: '
      BEGIN {
        sys["root"]=1
      ...
      /^[ \t]*$/ { next }
      {
        ...
        uid=$3
        gid=$4
        if (name == "" || uid !~ /^[0-9][0-9]*$/ || gid !~ /^[0-9][0-9]*$/) {
          unparseable++
          next
        }
        uidn = uid + 0
        ...
        if (name in sys) next
        if (uidn >= 100) {
          if (fails == "") fails = name ":" gidn
          else fails = fails "," name ":" gidn
          next
        }
        if (manuals == "")
          manuals = name " (UID " uidn ", primary GID " gidn ")"
        else
          manuals = manuals "; " name " (UID " uidn ", primary GID " gidn ")"
      }
      END {
        if (fails != "") { print "FAIL@" fails; exit }
        if (unparseable > 0) { print "UNPARSEABLE"; exit }
        if (manuals != "") { print "MANUAL@" manuals; exit }
        if (accounts == 0) { print "ABSENT"; exit }
        print "PASS"
      }
    ')
    SRG_KIND=${SRG_PARSED%%@*}
    SRG_DETAIL=${SRG_PARSED#*@}
    case "$SRG_KIND" in
      FAIL)
        SRG_NAME=${SRG_DETAIL%%:*}
        SRG_GID=${SRG_DETAIL#*:}
        case "$SRG_GID" in
          *,*)
            SRG_STATUS=FAIL
            SRG_OBSERVED="non-system accounts $SRG_DETAIL use reserved primary GIDs"
            ;;
          *)
            SRG_STATUS=FAIL
            SRG_OBSERVED="non-system account $SRG_NAME uses reserved primary GID $SRG_GID"
            ;;
        esac
        ;;
      UNPARSEABLE)
        SRG_STATUS=$SRG_FAILCLOSED_UNPARSE
        SRG_OBSERVED="not assessed - /etc/passwd has an account with no parseable UID or GID"
        ;;
      MANUAL)
        SRG_STATUS=$SRG_FAILCLOSED_MANUAL
        SRG_OBSERVED="not assessed - manual portion: ISSO must classify whether account $SRG_DETAIL is a system account"
        ;;
      ABSENT)
        SRG_STATUS=PASS
        SRG_OBSERVED="no passwd accounts present; reserved GID prohibition holds"
        ;;
      PASS)
        SRG_STATUS=PASS
        SRG_OBSERVED="no non-system account uses a reserved primary GID (0-99)"
        ;;
      *)
        SRG_STATUS=$SRG_FAILCLOSED_UNPARSE
        SRG_OBSERVED="not assessed - reserved GID scan returned an unusable result"
        ;;
    esac
  fi
  ...
  case "$SRG_STATUS" in
    PASS)
      SRG_MEANING="Reserved GIDs 0-99 are used only by the authored AIX system-account set, or no passwd accounts exist, so the V-215194 prohibition holds."
      SRG_FIX="n/a"
      ;;
    FAIL)
      SRG_MEANING="A non-system account has a reserved primary GID (0-99), which DISA STIG V-215194 prohibits."
      SRG_FIX="chuser pgrp=<non_reserved_group_name> <non_system_user_name>; PTxray only recommends this command."
      ;;
    *)
      SRG_MEANING="PTxray could not finish the reserved-GID assessment from /etc/passwd: the probe failed, returned empty output, was unparseable, or an account needs ISSO classification as system vs non-system."
      SRG_FIX="verify /etc/passwd is readable and have the ISSO classify any unclassified reserved-GID account, then rerun PTxray."
      ;;
  esac

  add security stig_reserved_gids "Reserved GIDs for system accounts" \
    "$SRG_STATUS" med "$SRG_OBSERVED" \
    "$SRG_MEANING" "$SRG_FIX" "stig:V-215194"
}
