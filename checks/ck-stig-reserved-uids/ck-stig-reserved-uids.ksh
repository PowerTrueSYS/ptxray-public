...
AIXRAY_TOOL=ck-stig-reserved-uids


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_reserved_uids — DISA STIG V-215195.
  # UIDs reserved for system accounts (0-128) must not be assigned to
  # non-system accounts on AIX.  PARTIAL-AUTO: the reserved-UID occupancy is
  # the decidable sub-condition, judged against an authored set of standard
  # AIX system accounts (name -> canonical reserved UID).  An account that
  # holds a reserved UID but is not in that set is a non-system account and is
  # a finding.  A standard system account holding a non-canonical reserved UID
  # cannot be adjudicated from /etc/passwd alone, so that shape renders a
  # typed NOT_ASSESSED naming the ISSO portion — never a silent PASS.  A host
  # with no reserved-UID occupant at all is the compliant state (svcoff
  # prohibition convention).  Read the file exactly once via the probe.
  typeset SRU_RAW SRU_RC SRU_SHAPE
  typeset SRU_STATUS SRU_OBSERVED SRU_MEANING SRU_FIX

  SRU_RAW=$(aix passwd_etc more /etc/passwd)
  SRU_RC=$?
  SRU_SHAPE=""
  SRU_STATUS=""
  SRU_OBSERVED=""
  ...
  if [ "$SRU_RC" -ne 0 ]; then
    SRU_STATUS=NOT_ASSESSED
    SRU_OBSERVED="not assessed - /etc/passwd probe failed (rc=$SRU_RC)"
  else
    SRU_SHAPE=$(printf '%s\n' "$SRU_RAW" | awk -F: '
      function canon(n) {
        if (n == "root") return "0"
        if (n == "daemon") return "1"
        if (n == "bin") return "2"
        if (n == "sys") return "3"
        if (n == "adm") return "4"
        if (n == "uucp") return "5"
        if (n == "invscout") return "6"
        if (n == "pconsole") return "7"
        if (n == "lpd") return "9"
        if (n == "nuucp") return "10"
        if (n == "guest") return "100"
        return ""
      }
      {
        if ($0 ~ /^[ \t]*$/) next
        ...
        if (c == "") {
          fail = 1
          if (failn == "") failn = name "(" uidstr ")"
          else failn = failn ", " name "(" uidstr ")"
          next
        }
        if (c != uidstr) {
          man = 1
          if (mann == "") mann = name "(" uidstr ", canonical " c ")"
          else mann = mann ", " name "(" uidstr ", canonical " c ")"
          next
        }
        if (okn == "") okn = name "(" uidstr ")"
        else okn = okn ", " name "(" uidstr ")"
      }
      END {
        if (lines == 0) print "empty"
        else if (records == 0) print "unparseable"
        else if (fail) print "fail:" failn
        else if (man) print "manual:" mann
        else if (okn == "") print "absent"
        else print "ok:" okn
      }')
    case "$SRU_SHAPE" in
      empty)
        SRU_STATUS=NOT_ASSESSED
        SRU_OBSERVED="not assessed - /etc/passwd probe returned empty output with rc 0 (swallowed error, not absence)"
        ;;
      unparseable)
        SRU_STATUS=NOT_ASSESSED
        SRU_OBSERVED="not assessed - /etc/passwd output was unparseable (no account records)"
        ;;
      absent)
        SRU_STATUS=PASS
        SRU_OBSERVED="no account occupies a reserved UID (0-128); the subject is absent and that is the compliant state"
        ;;
      ok:*)
        SRU_STATUS=PASS
        SRU_OBSERVED="all reserved UID (0-128) occupants are standard AIX system accounts: ${SRU_SHAPE#ok:}"
        ;;
      fail:*)
        SRU_STATUS=FAIL
        SRU_OBSERVED="reserved UID (0-128) assigned to a non-system account: ${SRU_SHAPE#fail:}"
        ;;
      manual:*)
        SRU_STATUS=NOT_ASSESSED
        SRU_OBSERVED="not assessed - reserved UID (0-128) occupants include ${SRU_SHAPE#manual:}; whether these are system accounts requires ISSO adjudication of the site's system-account set"
        ;;
      *)
        SRU_STATUS=NOT_ASSESSED
        SRU_OBSERVED="not assessed - /etc/passwd output was unparseable"
        ;;
    esac
  fi
  ...
  case "$SRU_STATUS" in
    PASS)
      SRU_MEANING="Every account holding a reserved UID (0-128) is a standard AIX system account; no non-system account uses a UID reserved for system accounts."
      SRU_FIX="n/a"
      ;;
    FAIL)
      SRU_MEANING="A reserved UID (0-128) is assigned to an account that is not a standard AIX system account, violating the DISA STIG requirement that reserved UIDs belong only to system accounts."
      SRU_FIX="change the account to a UID outside the reserved range, for example 'usermod -u <uid> [user_name]'; PTxray only recommends this command."
      ;;
    *)
      SRU_MEANING="PTxray did not obtain trustworthy /etc/passwd evidence for the reserved-UID reservation, or the reserved-UID occupants need ISSO adjudication of the site's system-account set."
      SRU_FIX="confirm the site's system-account set with the ISSO, verify the reserved-UID occupants as root, and rerun PTxray."
      ;;
  esac

  add security stig_reserved_uids "UIDs reserved for system accounts" \
    "$SRU_STATUS" med "$SRU_OBSERVED" \
    "$SRU_MEANING" "$SRU_FIX" "stig:V-215195"
}
