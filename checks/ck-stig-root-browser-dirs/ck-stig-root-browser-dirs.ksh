...
AIXRAY_TOOL=ck-stig-root-browser-dirs


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_root_browser_dirs — DISA STIG V-215191.  Browser profile directories
  # prove only that a browser may have been used; authorization for local
  # service administration is an ISSO decision and must not be inferred.
  typeset RBD_NETSCAPE RBD_MOZILLA RBD_NETSCAPE_RC RBD_MOZILLA_RC
  typeset RBD_STATUS RBD_OBSERVED RBD_MEANING RBD_FIX

  RBD_NETSCAPE=$(aix find_root_netscape find /root -name .netscape)
  RBD_NETSCAPE_RC=$?
  RBD_MOZILLA=$(aix find_root_mozilla find /root -name .mozilla)
  RBD_MOZILLA_RC=$?

  if [ "$RBD_NETSCAPE_RC" -ne 0 ] || [ "$RBD_MOZILLA_RC" -ne 0 ]; then
    RBD_STATUS=NOT_ASSESSED
    RBD_OBSERVED="not assessed - browser directory probe failed (netscape rc=$RBD_NETSCAPE_RC, mozilla rc=$RBD_MOZILLA_RC)"
  elif [ -n "$RBD_NETSCAPE" ] && ! printf '%s\n' "$RBD_NETSCAPE" | awk 'NF { found = 1 } END { exit !found }'; then
    RBD_STATUS=NOT_ASSESSED
    RBD_OBSERVED="not assessed - .netscape probe returned whitespace-only output"
  elif [ -n "$RBD_MOZILLA" ] && ! printf '%s\n' "$RBD_MOZILLA" | awk 'NF { found = 1 } END { exit !found }'; then
    RBD_STATUS=NOT_ASSESSED
    RBD_OBSERVED="not assessed - .mozilla probe returned whitespace-only output"
  elif [ -n "$RBD_NETSCAPE" ] || [ -n "$RBD_MOZILLA" ]; then
    RBD_STATUS=NOT_ASSESSED
    RBD_OBSERVED="not assessed - browser profile directory found; verify with the root users and the ISSO the intent"
  else
    RBD_STATUS=PASS
    RBD_OBSERVED="no /root/.netscape or /root/.mozilla directory found"
  fi

  case "$RBD_STATUS" in
    PASS)
      RBD_MEANING="No root browser profile directory was found by either required read-only probe."
      RBD_FIX="n/a"
      ;;
    *)
      RBD_MEANING="PTxray cannot determine browser-use authorization from directory evidence alone."
      RBD_FIX="Enforce policy requiring administrative accounts use web browsers only for local service administration."
      ;;
  esac

  add security stig_root_browser_dirs "Root browser profile directories" \
    "$RBD_STATUS" med "$RBD_OBSERVED" "$RBD_MEANING" "$RBD_FIX" "stig:V-215191"
}
