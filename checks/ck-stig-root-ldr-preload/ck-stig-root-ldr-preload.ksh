...
AIXRAY_TOOL=ck-stig-root-ldr-preload


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_root_ldr_preload — DISA STIG V-215196.
  # Deviation from STIG check-content: the auditor runs `env | grep LDR_PRELOAD`
  # in the current root shell. This scanner reads /etc/environment, /.profile,
  # and root's home .profile instead.
  typeset RLP_ENV_LS_RAW RLP_ENV_LS_RC RLP_ENV_RAW RLP_ENV_RC
  typeset RLP_DOT_LS_RAW RLP_DOT_LS_RC RLP_DOT_RAW RLP_DOT_RC
  typeset RLP_HOME_RAW RLP_HOME_RC RLP_HOME
  typeset RLP_HP_LS_RAW RLP_HP_LS_RC RLP_HP_RAW RLP_HP_RC RLP_HP_PATH
  typeset RLP_FAILED RLP_EMPTY RLP_ABSENT RLP_SET RLP_PARSE RLP_PARSE_RC
  typeset RLP_REASON RLP_STATUS RLP_SEV RLP_OBSERVED RLP_MEANING RLP_FIX

  RLP_ENV_LS_RAW=$(aix etc_environment_ls ls -ld /etc/environment)
  RLP_ENV_LS_RC=$?
  RLP_ENV_RAW=$(aix etc_environment cat /etc/environment)
  RLP_ENV_RC=$?
  RLP_DOT_LS_RAW=$(aix root_dot_profile_ls ls -ld /.profile)
  RLP_DOT_LS_RC=$?
  RLP_DOT_RAW=$(aix root_dot_profile cat /.profile)
  RLP_DOT_RC=$?
  RLP_HOME_RAW=$(aix root_home lsuser -a home root)
  RLP_HOME_RC=$?
  ...
  function rlp_parse_env {
    ...
    /^[ \t]*$/ { next }
    {
      ...
      if (line ~ /^LDR_PRELOAD=/) {
        ...
        sub(/^LDR_PRELOAD=/, "", val)
        semi = index(val, ";")
        if (semi > 0) val = substr(val, 1, semi - 1)
        n = length("" val)
        q1 = substr(val, 1, 1)
    ...
    END {
      if (found != "") print "set:" found
      else print "clear"
    }
  '
  }

  if [ "$RLP_ENV_LS_RC" -eq 2 ]; then
    RLP_ABSENT=1
  elif [ "$RLP_ENV_LS_RC" -ne 0 ]; then
    RLP_FAILED=1
  elif [ -z "$RLP_ENV_LS_RAW" ]; then
    RLP_EMPTY=1
  elif [ "$RLP_ENV_RC" -ne 0 ]; then
    RLP_FAILED=1
  elif [ -z "$RLP_ENV_RAW" ]; then
    RLP_EMPTY=1
  else
    RLP_PARSE=$(printf '%s\n' "$RLP_ENV_RAW" | rlp_parse_env)
    RLP_PARSE_RC=$?
    if [ "$RLP_PARSE_RC" -ne 0 ] || [ -z "$RLP_PARSE" ]; then
      RLP_FAILED=1
    elif [ "$RLP_PARSE" != "clear" ]; then
      RLP_SET="/etc/environment ${RLP_PARSE#set:}"
    fi
  fi

  if [ "$RLP_DOT_LS_RC" -eq 2 ]; then
    RLP_ABSENT=1
  elif [ "$RLP_DOT_LS_RC" -ne 0 ]; then
    RLP_FAILED=1
  elif [ -z "$RLP_DOT_LS_RAW" ]; then
    RLP_EMPTY=1
  elif [ "$RLP_DOT_RC" -ne 0 ]; then
    RLP_FAILED=1
  elif [ -z "$RLP_DOT_RAW" ]; then
    RLP_EMPTY=1
  else
    RLP_PARSE=$(printf '%s\n' "$RLP_DOT_RAW" | rlp_parse_env)
    RLP_PARSE_RC=$?
    if [ "$RLP_PARSE_RC" -ne 0 ] || [ -z "$RLP_PARSE" ]; then
      RLP_FAILED=1
    elif [ "$RLP_PARSE" != "clear" ]; then
      ...
      RLP_SET="$RLP_SET/.profile ${RLP_PARSE#set:}"
    fi
  fi

  if [ "$RLP_HOME_RC" -ne 0 ]; then
    RLP_FAILED=1
  elif [ -z "$RLP_HOME_RAW" ]; then
    RLP_EMPTY=1
  else
    RLP_HOME=$(printf '%s\n' "$RLP_HOME_RAW" | awk '
      {
    ...
    ')
    RLP_PARSE_RC=$?
    if [ "$RLP_PARSE_RC" -ne 0 ] || [ -z "$RLP_HOME" ]; then
      RLP_FAILED=1
    elif [ "$RLP_HOME" != "/" ]; then
      RLP_HP_PATH="$RLP_HOME/.profile"
      RLP_HP_LS_RAW=$(aix root_home_profile_ls ls -ld "$RLP_HP_PATH")
      RLP_HP_LS_RC=$?
      RLP_HP_RAW=$(aix root_home_profile cat "$RLP_HP_PATH")
      RLP_HP_RC=$?
      if [ "$RLP_HP_LS_RC" -eq 2 ]; then
        RLP_ABSENT=1
      elif [ "$RLP_HP_LS_RC" -ne 0 ]; then
        RLP_FAILED=1
      elif [ -z "$RLP_HP_LS_RAW" ]; then
        RLP_EMPTY=1
      elif [ "$RLP_HP_RC" -ne 0 ]; then
        RLP_FAILED=1
      elif [ -z "$RLP_HP_RAW" ]; then
        RLP_EMPTY=1
      else
        RLP_PARSE=$(printf '%s\n' "$RLP_HP_RAW" | rlp_parse_env)
        RLP_PARSE_RC=$?
        if [ "$RLP_PARSE_RC" -ne 0 ] || [ -z "$RLP_PARSE" ]; then
          RLP_FAILED=1
        elif [ "$RLP_PARSE" != "clear" ]; then
          ...
          RLP_SET="$RLP_SET$RLP_HP_PATH ${RLP_PARSE#set:}"
        fi
      fi
    fi
  fi

  if [ "$RLP_FAILED" -eq 1 ]; then
    RLP_REASON=probe_failed
  elif [ "$RLP_EMPTY" -eq 1 ]; then
    RLP_REASON=probe_empty
  elif [ "$RLP_ABSENT" -eq 1 ]; then
    RLP_REASON=subject_absent
  elif [ -n "$RLP_SET" ]; then
    RLP_REASON=noncompliant
  else
    RLP_REASON=compliant
  fi

  case "$RLP_REASON" in
    compliant)
      RLP_STATUS=PASS
      RLP_OBSERVED="root LDR_PRELOAD is empty"
      RLP_MEANING="The root account list of preloaded libraries is empty."
      RLP_FIX="n/a"
      ;;
    noncompliant) RLP_STATUS=FAIL
      RLP_OBSERVED="LDR_PRELOAD is set ($RLP_SET)"
      RLP_MEANING="A non-empty LDR_PRELOAD for root lets a chosen library load into root processes."
      RLP_FIX="Edit the root user's initialization files and remove any definition of LDR_PRELOAD."
      ;;
    subject_absent) RLP_STATUS=FAIL
      RLP_OBSERVED="required root LDR_PRELOAD subject is absent"
      RLP_MEANING="Required root initialization files that would define LDR_PRELOAD are missing, so the required configuration is unmet."
      RLP_FIX="Edit the root user's initialization files and remove any definition of LDR_PRELOAD."
      ;;
    probe_failed) RLP_STATUS=NOT_ASSESSED
      RLP_OBSERVED="not assessed - root LDR_PRELOAD probe failed"
      RLP_MEANING="PTxray did not obtain trustworthy evidence for root LDR_PRELOAD."
      RLP_FIX="verify /etc/environment, /.profile, and root's profile are readable, then rerun PTxray."
      ;;
    probe_empty) RLP_STATUS=NOT_ASSESSED
      RLP_OBSERVED="not assessed - root LDR_PRELOAD probe returned no output"
      RLP_MEANING="PTxray did not obtain trustworthy evidence for root LDR_PRELOAD."
      RLP_FIX="verify /etc/environment, /.profile, and root's profile return content, then rerun PTxray."
      ;;
    *)
      RLP_STATUS=NOT_ASSESSED
      RLP_OBSERVED="not assessed - root LDR_PRELOAD evidence was unparseable"
      RLP_MEANING="PTxray did not obtain trustworthy evidence for root LDR_PRELOAD."
      RLP_FIX="verify /etc/environment, /.profile, and root's profile, then rerun PTxray."
      ;;
  esac

  case "$RLP_STATUS" in
    NOT_APPLICABLE) RLP_SEV=low ;;
    *) RLP_SEV=med ;;
  esac

  add security stig_root_ldr_preload "Root LDR_PRELOAD list" \
    "$RLP_STATUS" "$RLP_SEV" "$RLP_OBSERVED" \
    "$RLP_MEANING" "$RLP_FIX" "stig:V-215196"
}
