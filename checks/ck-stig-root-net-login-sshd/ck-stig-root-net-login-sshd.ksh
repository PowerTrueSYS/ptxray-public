...
AIXRAY_TOOL=ck-stig-root-net-login-sshd


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_root_net_login_sshd — DISA STIG V-215221.
  # FAIL iff last shows a non-console root network login AND sshd is down.
  # last is wtmp history vs current sshd; still FAIL when both hold.
  # last is capped to 200 lines inside the probe so a large wtmp cannot stall.
  typeset RNLS_LAST_RAW RNLS_LAST_RC RNLS_SSHD_RAW RNLS_SSHD_RC
  typeset RNLS_LAST RNLS_SSHD RNLS_PARSE_RC
  typeset RNLS_STATUS RNLS_SEV RNLS_OBSERVED RNLS_MEANING RNLS_FIX

  # Pipeline runs inside aix via sh -c; a quoted pipeline is not an executable.
  # more is the STIG pager; aix captures stdout so it is not a tty.
  # last() caps the wtmp scan at 200 lines so a large wtmp cannot stall the probe.
  RNLS_LAST_RAW=$(aix last_root_net sh -c 'last() { command last | head -n 200; }; last | grep "root " | egrep -v "reboot|console" | more')
  RNLS_LAST_RC=$?
  RNLS_SSHD_RAW=$(aix ps_sshd sh -c 'ps -ef |grep sshd')
  RNLS_SSHD_RC=$?
  RNLS_LAST=""
  RNLS_SSHD=""
  RNLS_STATUS=""
  ...
  if [ "$RNLS_LAST_RC" -ne 0 ] || [ "$RNLS_SSHD_RC" -ne 0 ]; then
    RNLS_STATUS=NOT_ASSESSED
    RNLS_OBSERVED="not assessed - last/sshd probe failed (last_rc=$RNLS_LAST_RC sshd_rc=$RNLS_SSHD_RC)"
  elif [ -z "$RNLS_LAST_RAW" ] || [ -z "$RNLS_SSHD_RAW" ]; then
    RNLS_STATUS=NOT_ASSESSED
    RNLS_OBSERVED="not assessed - last/sshd probe returned no output"
  else
    RNLS_LAST=$(printf '%s\n' "$RNLS_LAST_RAW" | awk '
      BEGIN { limit = 200 }
      NR > limit { exit }
      /^[ \t]*$/ { next }
      /^wtmp begins / { saw_absent = 1; next }
      $1 == "root" {
        if ($2 == "console" || $2 == "reboot" || $2 == "~") next
        net = 1
        next
      }
      { bad = 1 }
      END {
        if (bad) exit 1
        if (net) print "net"
        else if (saw_absent) print "none"
        else exit 1
      }
    ')
    RNLS_PARSE_RC=$?
    if [ "$RNLS_PARSE_RC" -ne 0 ] || [ -z "$RNLS_LAST" ]; then
      RNLS_LAST=unparseable
    fi
    RNLS_SSHD=$(printf '%s\n' "$RNLS_SSHD_RAW" | awk '
      BEGIN { limit = 200 }
      NR > limit { exit }
      /^[ \t]*$/ { next }
      /^[ \t]*UID[ \t]+PID/ { header = 1; next }
      /grep[ \t].*sshd/ { grephit = 1; next }
      /sshd/ { up = 1; next }
      { extra = 1 }
      END {
        if (up) { print "up"; exit 0 }
        if (extra) exit 1
        if (header || grephit) { print "down"; exit 0 }
        exit 1
      }
    ')
    RNLS_PARSE_RC=$?
    if [ "$RNLS_PARSE_RC" -ne 0 ] || [ -z "$RNLS_SSHD" ]; then
      RNLS_SSHD=unparseable
    fi
    if [ "$RNLS_LAST" = "unparseable" ] || [ "$RNLS_SSHD" = "unparseable" ]; then
      RNLS_STATUS=NOT_ASSESSED
      RNLS_OBSERVED="not assessed - last/sshd evidence was unparseable"
    elif [ "$RNLS_LAST" = "net" ] && [ "$RNLS_SSHD" = "down" ]; then
      RNLS_STATUS=FAIL
      RNLS_OBSERVED="root network login with sshd not running"
    elif [ "$RNLS_SSHD" = "up" ]; then
      RNLS_STATUS=PASS
      RNLS_OBSERVED="sshd is running"
    else
      RNLS_STATUS=PASS
      RNLS_OBSERVED="no non-console root network login"
    fi
  fi
  ...
  case "$RNLS_STATUS" in
    FAIL)
      RNLS_SEV=high
      RNLS_MEANING="Root logged in over the network while sshd was not running, so the root password may have traveled in clear text."
      RNLS_FIX="installp -aXYgd /dev/cd0 -e /tmp/install.log openssh.base.server; startsrc -s sshd; chsec -f /etc/security/user -s root -a rlogin=false; PTxray only recommends these commands."
      ;;
    PASS)
      RNLS_SEV=med
      RNLS_MEANING="Either sshd is running so the network path is encrypted, or last shows no non-console root network login."
      RNLS_FIX="n/a"
      ;;
    NOT_APPLICABLE)
      RNLS_SEV=low
      RNLS_MEANING="The STIG rule text marks this control Not Applicable on this host."
      RNLS_FIX="n/a"
      ;;
    *)
      RNLS_SEV=med
      RNLS_MEANING="PTxray did not obtain trustworthy last and sshd evidence for a clear-text root network login."
      RNLS_FIX="verify that last and ps -ef |grep sshd return complete evidence, then rerun PTxray."
      ;;
  esac

  add security stig_root_net_login_sshd "Root passwords never in network clear text" \
    "$RNLS_STATUS" "$RNLS_SEV" "$RNLS_OBSERVED" \
    "$RNLS_MEANING" "$RNLS_FIX" "stig:V-215221"
}
