...
AIXRAY_TOOL=ck-stig-root-passwd-set


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_root_passwd_set - DISA STIG V-215308.
  # AIX must require authentication upon booting into single-user and maintenance
  # modes.  An unlocked root password field in /etc/passwd lets such a boot happen
  # without a password, so the root field must be locked ("!") to pass.
  typeset RPS_RAW RPS_RC RPS_AWK_RC RPS_ROOT_FIELD RPS_REASON
  typeset RPS_STATUS RPS_OBSERVED RPS_MEANING RPS_FIX

  RPS_RAW=$(aix passwd_root_fields cut -d: -f1,2 /etc/passwd)
  RPS_RC=$?
  RPS_ROOT_FIELD=""
  RPS_REASON=""
  RPS_STATUS=""
  ...
  if [ "$RPS_RC" -ne 0 ]; then
    RPS_REASON="failed (rc=$RPS_RC)"
  elif [ -z "$RPS_RAW" ]; then
    RPS_REASON="empty output"
  else
    RPS_ROOT_FIELD=$(printf '%s\n' "$RPS_RAW" | awk '
      /^[ \t]*$/ { next }
      /^#/ { next }
      {
        p = index($0, ":")
        ...
        user = substr($0, 1, p - 1)
        field2 = substr($0, p + 1)
        if (!found && user == "root") { rootfield = field2; found = 1 }
        records++
      }
      END {
        if (bad) exit 2
        if (!found && records == 0) exit 3
        if (!found) exit 1
        print rootfield
      }')
    RPS_AWK_RC=$?
    if [ "$RPS_AWK_RC" -eq 3 ]; then
      RPS_REASON="empty output"
    elif [ "$RPS_AWK_RC" -eq 2 ]; then
      RPS_REASON="unparseable"
    elif [ "$RPS_AWK_RC" -eq 1 ]; then
      RPS_STATUS=FAIL
      RPS_OBSERVED="root account not listed in /etc/passwd"
    elif [ "$RPS_ROOT_FIELD" = "!" ]; then
      RPS_STATUS=PASS
      RPS_OBSERVED="root password field is locked (!)"
    elif [ -z "$RPS_ROOT_FIELD" ]; then
      RPS_STATUS=FAIL
      RPS_OBSERVED="root password field is '' (not locked)"
    else
      RPS_STATUS=FAIL
      RPS_OBSERVED="root password field is '$RPS_ROOT_FIELD' (not locked)"
    fi
  fi

  if [ -n "$RPS_REASON" ]; then
    RPS_STATUS=NOT_ASSESSED
    RPS_OBSERVED="not assessed - passwd root field probe $RPS_REASON"
  fi

  case "$RPS_STATUS" in
    PASS)
      RPS_MEANING="The root password field is locked, so entering single-user or maintenance mode still requires authentication."
      RPS_FIX="n/a"
      ;;
    FAIL)
      RPS_MEANING="An unlocked root password field lets single-user or maintenance boot proceed without a password, violating V-215308."
      RPS_FIX="lock the root account password with 'passwd' (PTxray only recommends this command; it never runs it)."
      ;;
    *)
      RPS_MEANING="PTxray did not obtain trustworthy evidence of the root password field state."
      RPS_FIX="verify the root entry in /etc/passwd as root, then rerun PTxray."
      ;;
  esac

  add security stig_root_passwd_set "Root password set" \
    "$RPS_STATUS" med "$RPS_OBSERVED" \
    "$RPS_MEANING" "$RPS_FIX" "stig:V-215308"
}
