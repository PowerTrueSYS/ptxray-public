...
AIXRAY_TOOL=ck-stig-root-path-worldwrite


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_root_path_worldwrite — DISA STIG V-215193.
  # Prohibition: root PATH must not contain world-writable directories.
  # Determinate empty PATH / no directory entries is compliant absence.
  typeset RPWW_PATH_RAW RPWW_PATH_RC RPWW_HAS_DIRS
  typeset RPWW_LS_RAW RPWW_LS_RC RPWW_CLASS RPWW_PARSE_RC
  typeset RPWW_HAS_LINK RPWW_STATUS RPWW_OBSERVED RPWW_MEANING RPWW_FIX
  typeset RPWW_PARTIAL RPWW_UNREADABLE RPWW_SKIP_LINKS
  typeset RPWW_FOLLOW_RAW RPWW_FOLLOW_RC RPWW_PATH_DIRS RPWW_EFF_RAW

  RPWW_PATH_RAW=$(aix root_path echo "$PATH")
  RPWW_PATH_RC=$?
  RPWW_STATUS=""
  RPWW_OBSERVED=""

  if [ "$RPWW_PATH_RC" -ne 0 ]; then
    RPWW_STATUS=NOT_ASSESSED
    RPWW_OBSERVED="not assessed - root PATH probe failed (rc=$RPWW_PATH_RC)"
  elif [ -z "$RPWW_PATH_RAW" ]; then
    RPWW_STATUS=PASS
    RPWW_OBSERVED="root PATH contains no directories"
  else
    RPWW_HAS_DIRS=$(printf '%s\n' "$RPWW_PATH_RAW" | awk '
      {
        n = split($0, a, ":")
        for (i = 1; i <= n; i++) {
          s = a[i]
    ...
    ')
    if [ "$RPWW_HAS_DIRS" != "yes" ]; then
      RPWW_STATUS=PASS
      RPWW_OBSERVED="root PATH contains no directories"
    else
      # The PATH word list is built first so the capture's exit status is the
      # probe's own and not a nested substitution's.
      RPWW_PATH_DIRS=$(echo $PATH | sed "s/:/ /g")
      RPWW_LS_RAW=$(aix root_path_ls ls -ld $RPWW_PATH_DIRS)
      RPWW_LS_RC=$?
      RPWW_PARTIAL=0
      RPWW_UNREADABLE=""
      RPWW_SKIP_LINKS=no
      if [ -n "$RPWW_LS_RAW" ] && { [ "$RPWW_LS_RC" -eq 1 ] || [ "$RPWW_LS_RC" -eq 2 ]; }; then
        RPWW_PARTIAL=1
        RPWW_UNREADABLE="some paths unreadable"
      elif [ "$RPWW_LS_RC" -ne 0 ]; then
        RPWW_STATUS=NOT_ASSESSED
        RPWW_OBSERVED="not assessed - root PATH directory listing failed (rc=$RPWW_LS_RC)"
      elif [ -z "$RPWW_LS_RAW" ]; then
        RPWW_STATUS=NOT_ASSESSED
        RPWW_OBSERVED="not assessed - root PATH directory listing returned no output"
      fi
      # The graded listing starts as the ls -ld capture and is replaced only by
      # a symlink-following capture that passed its own gate; neither probe's
      # exit status is ever overwritten by the other.
      RPWW_EFF_RAW=$RPWW_LS_RAW
      if [ -z "$RPWW_STATUS" ]; then
        RPWW_HAS_LINK=$(printf '%s\n' "$RPWW_LS_RAW" | awk '
          NF {
            if (substr($1, 1, 1) == "l") {
              print "yes"
              exit
        ...
        ')
        if [ "$RPWW_HAS_LINK" = "yes" ]; then
          RPWW_FOLLOW_RAW=$(aix root_path_ls_follow ls -ldL $RPWW_PATH_DIRS)
          RPWW_FOLLOW_RC=$?
          if [ -n "$RPWW_FOLLOW_RAW" ] && { [ "$RPWW_FOLLOW_RC" -eq 0 ] || [ "$RPWW_FOLLOW_RC" -eq 1 ] || [ "$RPWW_FOLLOW_RC" -eq 2 ]; }; then
            RPWW_EFF_RAW=$RPWW_FOLLOW_RAW
            RPWW_SKIP_LINKS=no
            if [ "$RPWW_FOLLOW_RC" -eq 1 ] || [ "$RPWW_FOLLOW_RC" -eq 2 ]; then
              RPWW_PARTIAL=1
              RPWW_UNREADABLE="some paths unreadable"
            fi
          elif [ "$RPWW_PARTIAL" -eq 1 ]; then
            RPWW_SKIP_LINKS=yes
          elif [ "$RPWW_FOLLOW_RC" -ne 0 ]; then
            RPWW_STATUS=NOT_ASSESSED
            RPWW_OBSERVED="not assessed - root PATH symlink target listing failed (rc=$RPWW_FOLLOW_RC)"
          else
            RPWW_STATUS=NOT_ASSESSED
            RPWW_OBSERVED="not assessed - root PATH symlink target listing returned no output"
          fi
        fi
        if [ -z "$RPWW_STATUS" ]; then
          RPWW_CLASS=$(printf '%s\n' "$RPWW_EFF_RAW" | awk -v skip_links="$RPWW_SKIP_LINKS" '
            NF {
              mode=$1
              if (length("" mode) == 11) {
              ...
              if (type == "l" && skip_links == "yes") next
              if (type != "d") { bad=1; next }
          ...
          ')
          RPWW_PARSE_RC=$?
          if [ "$RPWW_PARSE_RC" -ne 0 ] || [ -z "$RPWW_CLASS" ]; then
            RPWW_STATUS=NOT_ASSESSED
            RPWW_OBSERVED="not assessed - root PATH directory listing was unparseable"
          elif [ "$RPWW_CLASS" = "pass" ]; then
            if [ "$RPWW_PARTIAL" -eq 1 ]; then
              RPWW_STATUS=WARN
              RPWW_OBSERVED="no world-writable directories in root PATH; $RPWW_UNREADABLE"
            else
              RPWW_STATUS=PASS
              RPWW_OBSERVED="no world-writable directories in root PATH"
            fi
          else
            RPWW_STATUS=FAIL
            RPWW_OBSERVED="world-writable directories in root PATH: ${RPWW_CLASS#fail }"
          fi
        fi
      fi
    fi
  ...
  case "$RPWW_STATUS" in
    PASS)
      RPWW_MEANING="The root executable search path contains no world-writable directories."
      RPWW_FIX="n/a"
      ;;
    FAIL)
      RPWW_MEANING="A world-writable directory in the root executable search path can let another user plant a command that root will execute."
      RPWW_FIX="chmod o-w <path>"
      ;;
    WARN)
      RPWW_MEANING="Readable directories in the root executable search path are not world-writable; some PATH entries could not be listed."
      RPWW_FIX="verify every directory in the root PATH is readable, then rerun PTxray."
      ;;
    *)
      RPWW_MEANING="PTxray did not obtain trustworthy evidence for world-writable directories in the root PATH."
      RPWW_FIX="verify the root PATH directory listing, then rerun PTxray."
      ;;
  esac

  add security stig_root_path_worldwrite "Root PATH world-writable directories" \
    "$RPWW_STATUS" med "$RPWW_OBSERVED" "$RPWW_MEANING" "$RPWW_FIX" \
    "stig:V-215193"
}
