...
AIXRAY_TOOL=ck-stig-root-rlogin


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_root_rlogin — DISA STIG V-215189 (AIX7-00-001030).
  # The root account must not be able to rlogin in directly; only the system
  # console may admit root.  lsuser -a rlogin root prints the rlogin flag for
  # the root subject.  A parsed value of false is compliant; any other parsed
  # value, or a successful probe that prints no root record for the subject,
  # is a finding (the required configuration is missing).  A probe that fails,
  # returns empty with rc 0, or prints a root line whose rlogin attribute
  # cannot be read, is NOT_ASSESSED.
  typeset RLOGIN_RAW RLOGIN_RC RLOGIN_STATE RLOGIN_VALUE
  typeset RLOGIN_STATUS RLOGIN_OBSERVED RLOGIN_MEANING RLOGIN_FIX

  RLOGIN_RAW=$(aix lsuser_rlogin_root lsuser -a rlogin root)
  RLOGIN_RC=$?
  RLOGIN_STATUS=""

  if [ "$RLOGIN_RC" -ne 0 ]; then
    RLOGIN_STATUS=NOT_ASSESSED
    RLOGIN_OBSERVED="not assessed - lsuser probe failed (rc=$RLOGIN_RC)"
  elif [ -z "$RLOGIN_RAW" ]; then
    RLOGIN_STATUS=NOT_ASSESSED
    RLOGIN_OBSERVED="not assessed - lsuser returned no output with rc 0"
  else
    RLOGIN_STATE=$(printf '%s\n' "$RLOGIN_RAW" | awk '
      /^[ \t]*$/ { next }
      {
        if ($1 != "root") { next }
        seen = 1
    ...
    ')
    if [ -z "$RLOGIN_STATE" ]; then
      RLOGIN_STATUS=NOT_ASSESSED
      RLOGIN_OBSERVED="not assessed - lsuser rlogin output could not be interpreted"
    fi
  fi

  if [ -z "$RLOGIN_STATUS" ]; then
    case "$RLOGIN_STATE" in
      absent)
        RLOGIN_STATUS=FAIL
        RLOGIN_OBSERVED="no root record in lsuser output; root rlogin attribute is not set to false"
        ;;
      unparseable)
        RLOGIN_STATUS=NOT_ASSESSED
        RLOGIN_OBSERVED="not assessed - root rlogin attribute is missing or malformed in lsuser output"
        ;;
      value=false)
        RLOGIN_STATUS=PASS
        RLOGIN_OBSERVED="root rlogin=false (direct root login is restricted to the system console)"
        ;;
      value=*)
        RLOGIN_VALUE=${RLOGIN_STATE#value=}
        RLOGIN_STATUS=FAIL
        RLOGIN_OBSERVED="root rlogin=$RLOGIN_VALUE (must be false)"
        ;;
      *)
        RLOGIN_STATUS=NOT_ASSESSED
        RLOGIN_OBSERVED="not assessed - lsuser rlogin output could not be interpreted"
  ...
  case "$RLOGIN_STATUS" in
    PASS)
      RLOGIN_MEANING="Direct rlogin by root is disabled, so the root account cannot log in over the network through rlogin; only the system console may admit root."
      RLOGIN_FIX="n/a"
      ;;
    FAIL)
      RLOGIN_MEANING="The root account has rlogin enabled, allowing direct network login outside the system console in violation of the STIG requirement."
      RLOGIN_FIX="run 'chsec -f /etc/security/user -s root -a rlogin=false' to disable direct root rlogin; PTxray only recommends this command."
      ;;
    *)
      RLOGIN_MEANING="PTxray did not obtain trustworthy evidence for the root rlogin attribute."
      RLOGIN_FIX="verify the rlogin attribute for root with 'lsuser -a rlogin root', then rerun PTxray."
      ;;
  esac

  add security stig_root_rlogin "Root account rlogin restriction" \
    "$RLOGIN_STATUS" med "$RLOGIN_OBSERVED" \
    "$RLOGIN_MEANING" "$RLOGIN_FIX" "stig:V-215189"
}
