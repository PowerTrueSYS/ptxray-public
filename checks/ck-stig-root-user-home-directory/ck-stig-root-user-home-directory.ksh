...
AIXRAY_TOOL=ck-stig-root-user-home-directory


function standalone_check {
_AIXRAY_SESSION_KEYS=""
# stig_root_user_home_directory -- DISA STIG V-215434.
# The AIX root user home directory must not be the root directory (/).
# Probe /etc/passwd exactly once; every uncertain evidence shape fails closed.
typeset RH_RAW RH_RC RH_PARSED RH_PARSE_RC RH_REASON RH_ABSENT

# probe: grep "^root" /etc/passwd | awk -F":" '{print $6}'
# Command substitution strips trailing newlines, so a blank home field (awk
# emits a lone newline) would collapse to a zero-byte capture and be read as
# a swallowed error. Frame the probe with a non-newline sentinel so the blank
# field keeps its byte shape; remove the sentinel immediately after capture.
RH_RAW=$(
  aix root_home sh -c 'grep "^root" /etc/passwd | awk -F":" '\''{print $6}'\'''
  RH_RC=$?
  printf '%s' ';'
  exit "$RH_RC"
)
...
if [ "$RH_RC" -ne 0 ]; then
  RH_REASON="root home capture failed (rc=$RH_RC)"
elif [ -z "$RH_RAW" ]; then
  RH_REASON="root home probe returned empty output"
else
  RH_PARSED=$(printf '%s\n' "$RH_RAW" | awk '
    NF == 0 { next }
    /^[[:space:]]*$/ { next }
    { homes++; if (homes == 1) h=$0 }
    END {
      if (homes > 1) exit 1
      if (homes == 1) print h
    }
  ')
  RH_PARSE_RC=$?
  if [ "$RH_PARSE_RC" -ne 0 ]; then
    RH_REASON="root home evidence ambiguous"
  elif [ -z "$RH_PARSED" ]; then
    # The root passwd entry exists but carries no home directory: determinate
    # absence of the subject, which is the compliant state (svcoff convention).
    RH_ABSENT=1
  fi
fi

if [ -n "$RH_REASON" ]; then
  add security stig_root_user_home_directory "Root user home directory" NOT_ASSESSED med \
      "not assessed - $RH_REASON" \
      "PTxray could not obtain one trustworthy record proving the root user home directory." \
      "run 'grep \"^root\" /etc/passwd', correct the capture or path problem, and rerun PTxray." \
      "stig:V-215434"
elif [ "$RH_ABSENT" = "1" ]; then
  add security stig_root_user_home_directory "Root user home directory" PASS med \
      "root home directory is not set" \
      "The root user has no home directory configured in /etc/passwd, so it is not the root directory /." \
      "n/a" "stig:V-215434"
elif [ "$RH_PARSED" = "/" ]; then
  add security stig_root_user_home_directory "Root user home directory" FAIL med \
      "root home directory is /" \
      "The root user home directory is the root directory /, violating DISA STIG V-215434." \
      "mkdir /root; chown root /root; chgrp system /root; chmod 700 /root" \
      "stig:V-215434"
elif [ -n "$RH_PARSED" ]; then
  add security stig_root_user_home_directory "Root user home directory" PASS med \
      "root home directory is $RH_PARSED" \
      "The root user home directory is not the root directory /, as required by DISA STIG V-215434." \
      "n/a" "stig:V-215434"
else
  add security stig_root_user_home_directory "Root user home directory" NOT_ASSESSED med \
      "not assessed - root home unparseable" \
      "PTxray could not determine the root user home directory from /etc/passwd." \
      "run 'grep \"^root\" /etc/passwd' and rerun PTxray." \
      "stig:V-215434"
fi
}
