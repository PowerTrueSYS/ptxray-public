...
AIXRAY_TOOL=ck-stig-roothome-acl


function standalone_check {
_AIXRAY_SESSION_KEYS=""
# stig_roothome_acl — DISA STIG V-215199.
# The root account's home directory must not have an extended ACL.  aclget
# renders the directory ACL as text: a plain ACL shows only the "base
# permissions" block, while extended ACL entries add an "extended permissions"
# section.  The presence of that section is the violation.  Every uncertain
# evidence shape fails closed to NOT_ASSESSED.
typeset RTH_RAW RTH_RC RTH_SHAPE RTH_PARSE_RC
typeset RTH_STATUS RTH_OBSERVED RTH_MEANING RTH_FIX

RTH_RAW=$(aix roothome_acl aclget ~root)
RTH_RC=$?
RTH_SHAPE=""
RTH_STATUS=""

if [ "$RTH_RC" -eq 2 ] && [ -z "$RTH_RAW" ]; then
  # Root has no resolvable home directory: nothing can carry an extended ACL.
  RTH_STATUS=PASS
  RTH_OBSERVED="root home directory absent; no directory exists to carry an extended ACL"
elif [ "$RTH_RC" -ne 0 ]; then
  RTH_STATUS=NOT_ASSESSED
  RTH_OBSERVED="not assessed - aclget probe failed (rc=$RTH_RC)"
elif [ -z "$RTH_RAW" ]; then
  # rc 0 with no rows is a swallowed error, not evidence of absence.
  RTH_STATUS=NOT_ASSESSED
  RTH_OBSERVED="not assessed - aclget returned empty output with rc 0 (swallowed error)"
else
  RTH_SHAPE=$(printf '%s\n' "$RTH_RAW" | awk '
    /^[[:space:]]*$/ { next }
    {
      line = $0
      if (line ~ /^[[:space:]]*base permissions[[:space:]]*$/) base = 1
      else if (line ~ /^[[:space:]]*extended permissions[[:space:]]*$/) ext = 1
      else if (base == 1 && line ~ /^[[:space:]]*owner\(/) owner = 1
      else if (base == 1 && line ~ /^[[:space:]]*group\(/) group = 1
      else if (base == 1 && line ~ /^[[:space:]]*others/) others = 1
    }
    END {
      if (base == 1 && owner == 1 && group == 1 && others == 1) {
        if (ext == 1) print "ext"
        else print "base"
      } else {
        exit 1
  ...
  RTH_PARSE_RC=$?
  if [ "$RTH_PARSE_RC" -ne 0 ] || [ -z "$RTH_SHAPE" ]; then
    RTH_STATUS=NOT_ASSESSED
    RTH_OBSERVED="not assessed - aclget output was unparseable"
  elif [ "$RTH_SHAPE" = "ext" ]; then
    RTH_STATUS=FAIL
    RTH_OBSERVED="aclget ~root shows an extended permissions section"
  else
    RTH_STATUS=PASS
    RTH_OBSERVED="aclget ~root shows base permissions only, no extended ACL"
  fi
fi

case "$RTH_STATUS" in
  PASS)
    RTH_MEANING="The root account's home directory carries no extended ACL, so no principal beyond the base owner/group/others gains rights to it."
    RTH_FIX="n/a"
    ;;
  FAIL)
    RTH_MEANING="An extended ACL on the root home directory grants rights to principals the base permissions do not list, widening the root environment boundary."
    RTH_FIX="remove the extended ACL entries with 'acledit ~root'; PTxray only recommends this command."
    ;;
  *)
    RTH_MEANING="PTxray did not obtain trustworthy evidence for the root home directory ACL."
    RTH_FIX="verify the root home ACL as root with 'aclget ~root', correct the capture or path problem, and rerun PTxray."
    ;;
esac

add security stig_roothome_acl "Root home directory extended ACL" \
    "$RTH_STATUS" med "$RTH_OBSERVED" \
    "$RTH_MEANING" "$RTH_FIX" "stig:V-215199"
}
