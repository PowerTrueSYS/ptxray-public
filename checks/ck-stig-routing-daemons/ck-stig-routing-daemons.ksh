...
AIXRAY_TOOL=ck-stig-routing-daemons


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_routing_daemons — DISA STIG V-215428.
  # Prohibition: routing protocol daemons must not run unless the host is a
  # site-designated router. Router role is a site exception, not auto-detected.
  typeset SRD_RAW SRD_RC SRD_SHAPE SRD_NAMES
  typeset SRD_STATUS SRD_OBSERVED SRD_MEANING SRD_FIX

  # Producer is /usr/bin/ps -ef (argv). Filtering of routing-protocol
  # executables is the awk below, not a shell pipeline.
  SRD_RAW=$(aix ps_routing_daemons /usr/bin/ps -ef)
  SRD_RC=$?
  SRD_SHAPE=""
  SRD_NAMES=""

  if [ "$SRD_RC" -ne 0 ]; then
    SRD_STATUS=NOT_ASSESSED
    SRD_OBSERVED="not assessed - routing-daemon probe failed (rc=$SRD_RC)"
  elif [ -z "$SRD_RAW" ]; then
    SRD_STATUS=NOT_ASSESSED
    SRD_OBSERVED="not assessed - routing-daemon probe returned no output"
  else
    SRD_SHAPE=$(printf '%s\n' "$SRD_RAW" | awk '
      /^[ \t]*$/ { next }
      $1 == "UID" && $2 == "PID" && $3 == "PPID" && $4 == "C" && $5 == "STIME" {
        headers++
        next
      }
      {
        if ($2 !~ /^[0-9][0-9]*$/ || $3 !~ /^[0-9][0-9]*$/ || $4 !~ /^[0-9][0-9]*$/) {
          bad++
          next
        }
        if ($5 ~ /^[0-9][0-9]:[0-9][0-9]:[0-9][0-9]$/) {
          timef = $7
          cmdi = 8
        } else if ($5 ~ /^(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)$/ && $6 ~ /^[0-9][0-9]*$/) {
          timef = $8
          cmdi = 9
        } else {
          bad++
        ...
        if (NF < cmdi || timef !~ /^[0-9][0-9]*:[0-9][0-9]/) {
          bad++
          next
        }
        exe = $cmdi
        ...
        rows++
        if (exe ~ /^(routed|ospfd|ospf6d|ospf|bgpd|bgp|zebra|quagga|gated)$/) {
      ...
      END {
        ...
        else if (hits) print "daemons|" names
        else if (headers || collectors || rows) print "none"
        else print "unparseable"
      }
    ')
    case "$SRD_SHAPE" in
      daemons\|*)
        SRD_NAMES=${SRD_SHAPE#daemons\|}
        SRD_STATUS=FAIL
        SRD_OBSERVED="routing protocol daemons listed: $SRD_NAMES"
        ;;
      none)
        SRD_STATUS=PASS
        SRD_OBSERVED="no routing protocol daemons listed"
        ;;
      *)
        SRD_STATUS=NOT_ASSESSED
        SRD_OBSERVED="not assessed - routing-daemon process list was unparseable"
        ;;
    esac
  fi
  ...
  case "$SRD_STATUS" in
    PASS)
      SRD_MEANING="No routing protocol daemons are running, which is the required state unless this host is a site-designated router."
      SRD_FIX="n/a"
      ;;
    FAIL)
      SRD_MEANING="A routing protocol daemon is running. Unless this host is a site-designated router, this does not meet DISA STIG V-215428."
      SRD_FIX="ps -ef |egrep '(ospf|route|bgp|zebra|quagga|gate)' | grep -v egrep | awk -F \" \" '{print \$2}' | while read pid;do kill \$pid;done"
      ;;
    *)
      SRD_MEANING="PTxray did not obtain trustworthy routing-daemon process evidence."
      SRD_FIX="verify that the routing-daemon process probe can run, then rerun PTxray."
      ;;
  esac

  add security stig_routing_daemons "Routing protocol daemons not running" \
    "$SRD_STATUS" med "$SRD_OBSERVED" "$SRD_MEANING" "$SRD_FIX" "stig:V-215428"
}
