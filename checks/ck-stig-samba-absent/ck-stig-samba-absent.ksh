...
AIXRAY_TOOL=ck-stig-samba-absent


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_samba_absent — DISA STIG V-215280.  This is a prohibition rule:
  # explicit absence of Samba is compliant; uncertain lslpp evidence is refused.
  typeset SAMBA_RAW SAMBA_RC SAMBA_STATE SAMBA_INSTALLED
  typeset SAMBA_OBSERVED SAMBA_MEANING SAMBA_FIX

  SAMBA_RAW=$(aix samba_packages lslpp -l 'samba*')
  SAMBA_RC=$?
  SAMBA_STATE=unknown
  SAMBA_INSTALLED=""

  if [ "$SAMBA_RC" -eq 1 ] && [ -z "$SAMBA_RAW" ]; then
    SAMBA_STATE=absent
  elif [ "$SAMBA_RC" -ne 0 ]; then
    SAMBA_STATE=probe_failed
  elif [ -z "$SAMBA_RAW" ]; then
    SAMBA_STATE=empty
  else
    SAMBA_INSTALLED=$(printf '%s\n' "$SAMBA_RAW" | awk '
      /^[[:space:]]*$/ { next }
      /^[[:space:]]*lslpp: 0504-132 Fileset samba\* not installed\.[[:space:]]*$/ {
        absent = 1
        next
      }
      /^[[:space:]]*lslpp: 0504-132 No filesets matched samba\*\.[[:space:]]*$/ {
        subject_absent = 1
        next
      }
      /^[[:space:]]*Fileset[[:space:]]+Level[[:space:]]+State[[:space:]]+Description[[:space:]]*$/ {
        header = 1
        next
      }
      /^[[:space:]]*-[-][-]*[[:space:]]*$/ {
      ...
      {
        if (!header || NF < 3 || $1 !~ /^samba([._-]|$)/ ||
            $3 !~ /^(COMMITTED|APPLIED|BROKEN|EFIX_LOCKED|OBSOLETE)$/) {
          bad = 1
          next
        }
        installed = installed (installed ? ", " : "") $1 "=" $2 " (" $3 ")"
      }
      END {
        if (bad || ((absent || subject_absent) && installed != "") ||
            (absent && subject_absent) || (!absent && !subject_absent && installed == "")) exit 1
        if (installed != "") print "installed|" installed
        else if (subject_absent) print "subject_absent"
        else print "absent"
      }
    ')
    if [ "$?" -eq 0 ]; then
      case "$SAMBA_INSTALLED" in
        installed\|*) SAMBA_STATE=installed; SAMBA_INSTALLED=${SAMBA_INSTALLED#installed\|} ;;
        absent) SAMBA_STATE=absent ;;
        subject_absent) SAMBA_STATE=subject_absent ;;
        *) SAMBA_STATE=unparseable ;;
      esac
    else
      SAMBA_STATE=unparseable
    fi
  ...
  case "$SAMBA_STATE" in
    installed)
      SAMBA_OBSERVED="installed Samba package(s): $SAMBA_INSTALLED"
      SAMBA_MEANING="Samba packages are installed, so this host does not meet DISA STIG V-215280."
      SAMBA_FIX="remove Samba packages with 'installp -ug samba*' after validating package dependencies; PTxray only recommends this command."
      add security stig_samba_absent "Samba packages absent" FAIL med \
          "$SAMBA_OBSERVED" "$SAMBA_MEANING" "$SAMBA_FIX" "stig:V-215280"
      ;;
    absent)
      add security stig_samba_absent "Samba packages absent" PASS med \
          "no installed Samba packages reported" \
          "Samba packages are not installed, as required by DISA STIG V-215280." \
          "n/a" "stig:V-215280"
      ;;
    subject_absent)
      add security stig_samba_absent "Samba packages absent" PASS med \
          "Samba subject absent: no installed Samba packages reported" \
          "Samba is absent, which is the compliant state for this prohibition rule." \
          "n/a" "stig:V-215280"
      ;;
    probe_failed)
      add security stig_samba_absent "Samba packages absent" NOT_ASSESSED med \
          "not assessed - lslpp Samba package probe failed (rc=$SAMBA_RC)" \
          "PTxray did not obtain trustworthy Samba package evidence." \
          "verify that 'lslpp -l samba*' can run successfully, then rerun PTxray." "stig:V-215280"
      ;;
    empty)
      add security stig_samba_absent "Samba packages absent" NOT_ASSESSED med \
          "not assessed - lslpp Samba package probe returned no output" \
          "PTxray did not obtain trustworthy Samba package evidence." \
          "verify that 'lslpp -l samba*' returns complete package evidence, then rerun PTxray." "stig:V-215280"
      ;;
    *)
      add security stig_samba_absent "Samba packages absent" NOT_ASSESSED med \
          "not assessed - lslpp Samba package output was unparseable" \
          "PTxray did not obtain trustworthy Samba package evidence." \
          "verify that 'lslpp -l samba*' returns recognizable package evidence, then rerun PTxray." "stig:V-215280"
      ;;
  esac
}
