...
AIXRAY_TOOL=ck-stig-sctp-absent


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_sctp_absent — DISA STIG V-215393.  SCTP (bos.net.sctp) must not be
  # installed.  Absence of the fileset is the compliant state (svcoff
  # convention); a listed fileset is a finding.  lslpp rc 1 is not-found
  # semantics: a readable table that does not list the fileset is determinate
  # absence (PASS).  Refuse only on rc 127, empty/unreadable output, or no
  # anchor — never on rc 1 with a table.
  typeset SCTP_RAW SCTP_RC SCTP_STATUS SCTP_OBSERVED SCTP_MEANING SCTP_FIX

  SCTP_RAW=$(aix lslpp_bos_net_sctp lslpp -L bos.net.sctp)
  SCTP_RC=$?

  if [ "$SCTP_RC" -eq 127 ]; then
    SCTP_STATUS=NOT_ASSESSED
    SCTP_OBSERVED="not assessed - lslpp probe of bos.net.sctp failed (rc=$SCTP_RC)"
  elif [ -z "$SCTP_RAW" ]; then
    SCTP_STATUS=NOT_ASSESSED
    if [ "$SCTP_RC" -eq 0 ]; then
      SCTP_OBSERVED="not assessed - lslpp probe of bos.net.sctp returned empty output with rc 0"
    else
      SCTP_OBSERVED="not assessed - lslpp probe of bos.net.sctp failed (rc=$SCTP_RC)"
    fi
  elif printf '%s\n' "$SCTP_RAW" | awk '
        /^[ \t]*$/ { next }
        $1 == "bos.net.sctp" { listed = 1 }
        END { if (listed) exit 0; else exit 1 }
      '; then
    SCTP_STATUS=FAIL
    SCTP_OBSERVED="bos.net.sctp is installed (listed by lslpp -L bos.net.sctp)"
  elif printf '%s\n' "$SCTP_RAW" | awk '
        /^[ \t]*$/ { next }
        $1 == "Fileset" && $2 == "Level" && $3 == "State" { found = 1 }
        END { if (found) exit 0; else exit 1 }
      '; then
    SCTP_STATUS=PASS
    SCTP_OBSERVED="bos.net.sctp is not installed (not listed by lslpp -L bos.net.sctp)"
  else
    SCTP_STATUS=NOT_ASSESSED
    SCTP_OBSERVED="not assessed - lslpp probe of bos.net.sctp returned unreadable output (no table)"
  fi

  case "$SCTP_STATUS" in
    PASS)
      SCTP_MEANING="SCTP is not installed on this AIX host, so no SCTP endpoint or transport can be exposed."
      SCTP_FIX="n/a"
      ;;
    FAIL)
      SCTP_MEANING="SCTP is installed, allowing SCTP traffic that the site security posture does not require."
      SCTP_FIX="unload SCTP and uninstall its fileset with 'sctpctrl unload; installp -ug bos.net.sctp'; PTxray only recommends these actions."
      ;;
    *)
      SCTP_MEANING="PTxray did not obtain trustworthy evidence that SCTP is absent."
      SCTP_FIX="verify whether bos.net.sctp is installed with 'lslpp -L bos.net.sctp', then rerun PTxray."
      ;;
  esac

  add security stig_sctp_absent "SCTP disabled" \
    "$SCTP_STATUS" med "$SCTP_OBSERVED" \
    "$SCTP_MEANING" "$SCTP_FIX" "stig:V-215393"
}
