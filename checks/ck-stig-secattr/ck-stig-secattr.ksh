...
# disabled/unlimited on AIX). If no probe was usable at all (NAPPLIC=0 and NPROBE_OK=0) the
# finding is a root-gated NOT_ASSESSED refusal rather than a hollow PASS; a readable stanza
# whose rules still land NA is determinate absence and renders NOT_APPLICABLE low.
function eval_secattr {
  typeset PAIRS CAP RAW DETAIL SUM CTLS NPASS NFAIL NNA NAPPLIC NPROBE_OK FLIST ST SEV OBS MEAN FIX
  ...
  # lines. Each probe emits a PROBE|<key>|ok/fail marker (a while-read in a pipeline runs in a
  # subshell, so a usability flag set inside is lost; the markers are counted outside). An
  # unusable stanza (rc!=0 or empty) emits a fail marker and no attr lines -> its rules fall NA.
  ...
  CAP=$(printf '%s\n' "$PAIRS" | while IFS='|' read SA_F SA_S; do
    ...
    SA_RC=$?
    # ok only when the output actually parses: at least one attr=value token.
    # rc 0 with nonempty but unparseable bytes (no attr=val) is a refusal, not
    # determinate absence — it must emit fail so NPROBE_OK stays 0.
    if [ "$SA_RC" -eq 0 ] && [ -n "$SA_RAW" ] && \
       [ "$(printf '%s\n' "$SA_RAW" | awk '{for(i=1;i<=NF;i++) if(index($i,"=")>1){print 1; exit}}')" = 1 ]; then
      printf 'PROBE|%s|ok\n' "$SA_SLUG"
  ...
  NAPPLIC=$(( NPASS + NFAIL ))
  NPROBE_OK=$(printf '%s\n' "$CAP" | awk -F'|' '$1=="PROBE" && $3=="ok"{n++} END{print n+0}')
  CTLS=$(printf '%s\n' "$DETAIL" | awk -F'|' 'NF>=3 && $1!=""{printf "%s%s",(n++?" ":""),$1}')

  if [ "$NAPPLIC" -eq 0 ]; then
    if [ "$NPROBE_OK" -eq 0 ]; then
      ST=NOT_ASSESSED; SEV=med
      OBS="not assessed - /etc/security stanzas unreadable (needs root); no account-policy rule evaluated"
      ...
      OBS="no applicable account-policy rule on this box"
      MEAN="Every /etc/security account-policy stanza was readable, but none of the STIG-mandated attributes carried a value, so no account-policy rule is applicable on this box."
      FIX="n/a"
    fi
