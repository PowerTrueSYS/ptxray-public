...
AIXRAY_TOOL=ck-stig-sed-mode


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_sed_mode — DISA STIG V-215398.
  # The Stack Execution Disable (SED) system-wide mode must be "all".  A
  # determinate absence of the SED subject (rc 2 with no output) is the
  # compliant state (svcoff prohibition convention), so it passes.  A probe
  # failure, rc 0 with no output (a swallowed error), or output with no
  # parseable systemwide mode line refuses with NOT_ASSESSED; refusal never
  # passes.
  typeset SSM_RAW SSM_RC SSM_MODE SSM_PARSE_RC SSM_REASON
  typeset SSM_STATUS SSM_SEVERITY SSM_OBSERVED SSM_MEANING SSM_FIX

  SSM_RAW=$(aix sedmgr_mode sedmgr)
  SSM_RC=$?
  SSM_MODE=""
  SSM_REASON=""

  if [ "$SSM_RC" -ne 0 ]; then
    if [ "$SSM_RC" -eq 2 ] && [ -z "$SSM_RAW" ]; then
      SSM_MODE="absent"
    else
      SSM_REASON="probe failed (rc=$SSM_RC)"
    fi
  elif [ -z "$SSM_RAW" ]; then
    SSM_REASON="probe returned no output with rc 0 (a swallowed error, not absence)"
  else
    SSM_MODE=$(printf '%s\n' "$SSM_RAW" | awk '
      {
        lc = tolower($0)
        # Scan every line.  Prefer the system-wide SED mode label
        # ("system" then optional spaces/hyphens, "wide", optional
        # spaces/hyphens, optional "sed", then "mode") over the AIX
        # sedmgr native label "Stack Execution Disable (SED) mode".
        # Requiring the leading "system" stops an earlier, unrelated
        # "mode" word from hijacking the parse; a native-label line is
        # kept only as a fallback so a native header that precedes a
        # system-wide line still grades the system-wide token.
        kind = ""
        if (match(lc, /system[ -]*wide[ -]*(sed[ -]*)?mode/) != 0) {
          kind = "sys"
        } else if (match(lc, /stack[ -]*execution[ -]*disable[ -]*[(]sed[)][ -]*mode/) != 0) {
          kind = "native"
        } else {
          next
        }
        # Take the exact token that follows the label; never truncate it
        # to an alphabetic prefix.  The token is the first whitespace
        # field; any further field is residue and that line is skipped.
        rest = substr($0, RSTART + RLENGTH)
        sub(/^[ \t:=]+/, "", rest)
        sub(/[ \t]+$/, "", rest)
        if (rest == "") next
        if (split(rest, parts) != 1) next
        token = tolower(parts[1])
        if (kind == "sys") {
          if (!sys_found) { sys_mode = token; sys_found = 1 }
        } else if (!native_found) {
          native_mode = token
          native_found = 1
        }
      }
      END {
        if (sys_found) { print sys_mode; exit 0 }
        if (native_found) { print native_mode; exit 0 }
        exit 1
      }
    ')
    SSM_PARSE_RC=$?
    if [ "$SSM_PARSE_RC" -ne 0 ] || [ -z "$SSM_MODE" ]; then
      SSM_REASON="output had no parseable systemwide SED mode line"
    fi
  fi

  if [ -n "$SSM_REASON" ]; then
    SSM_STATUS=NOT_ASSESSED
    SSM_OBSERVED="not assessed - sedmgr $SSM_REASON"
    SSM_MEANING="PTxray did not obtain trustworthy evidence of the Stack Execution Disable system-wide mode."
    SSM_FIX="run 'sedmgr' as root, confirm the system-wide SED mode, and rerun PTxray."
  elif [ "$SSM_MODE" = "absent" ]; then
    SSM_STATUS=PASS
    SSM_OBSERVED="the SED subject is absent (sedmgr reports no system-wide SED mode); absence is the compliant state"
    SSM_MEANING="No Stack Execution Disable subject exists on this host, so there is no SED mode to misconfigure; the prohibition convention (svcoff) treats absence as the compliant state."
    SSM_FIX="n/a"
  elif [ "$SSM_MODE" = "all" ]; then
    SSM_STATUS=PASS
    SSM_OBSERVED="systemwide SED mode is all"
    SSM_MEANING="The Stack Execution Disable system-wide mode is set to all, satisfying DISA STIG V-215398."
    SSM_FIX="n/a"
  else
    SSM_STATUS=FAIL
    SSM_OBSERVED="systemwide SED mode is $SSM_MODE"
    SSM_MEANING="The Stack Execution Disable system-wide mode is not all, so the system does not enforce stack execution disable system-wide as DISA STIG V-215398 requires."
    SSM_FIX="set the system-wide SED mode to all with 'sedmgr -m all'; PTxray only recommends this command."
  fi

  case "$SSM_STATUS" in
    NOT_APPLICABLE) SSM_SEVERITY=low ;;
    *) SSM_SEVERITY=med ;;
  esac

  add security stig_sed_mode "Stack Execution Disable system-wide mode" \
    "$SSM_STATUS" "$SSM_SEVERITY" "$SSM_OBSERVED" \
    "$SSM_MEANING" "$SSM_FIX" "stig:V-215398"
}
