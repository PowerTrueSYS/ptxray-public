...
AIXRAY_TOOL=ck-stig-sendmail-helpfile


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # smf_helpfile — DISA STIG V-215417.  The SMTP service HELP command must not
  # be enabled.  sendmail serves HELP when the O HelpFile option in  # network-lint: allow -- prose comment; 'sendmail' names the mail daemon in prose; no network call
  # /etc/mail/sendmail.cf names a help text file that exists; absence of the  # network-lint: allow -- prose comment; 'sendmail' is part of a local file pathname; no network call
  # option, of the subject sendmail.cf, or of the named file is the compliant  # network-lint: allow -- prose comment; 'sendmail' names the mail daemon in prose; no network call
  # state (prohibition / svcoff convention).  Uncertain evidence fails closed
  # to NOT_ASSESSED.
  typeset SMF_GREP_RAW SMF_GREP_RC SMF_HELPFILE SMF_LS_RAW SMF_LS_RC
  typeset SMF_REASON SMF_STATUS SMF_OBSERVED SMF_MEANING SMF_FIX
  ...
  SMF_GREP_RAW=$(aix sendmail_cf grep "^O HelpFile" /etc/mail/sendmail.cf)  # network-lint: allow -- aix() capture of a local read-only command; 'sendmail' is part of a local file pathname; no network call
  SMF_GREP_RC=$?

  if [ "$SMF_GREP_RC" -ne 0 ]; then
    if [ "$SMF_GREP_RC" -eq 1 ] && [ -z "$SMF_GREP_RAW" ]; then
      SMF_STATUS=PASS
      SMF_OBSERVED="no 'O HelpFile' option in /etc/mail/sendmail.cf; the SMTP HELP command is not enabled"  # network-lint: allow -- observed-state report string; 'sendmail' is part of a local file pathname; no network call
    elif [ "$SMF_GREP_RC" -eq 2 ] && [ -z "$SMF_GREP_RAW" ]; then
      SMF_STATUS=PASS
      SMF_OBSERVED="subject absent - /etc/mail/sendmail.cf is missing or unreadable; no sendmail config exists to enable the SMTP HELP command"  # network-lint: allow -- observed-state report string; 'sendmail' is part of a local file pathname; no network call
    else
      SMF_STATUS=NOT_ASSESSED
      SMF_REASON="sendmail HELP probe failed (rc=$SMF_GREP_RC)"  # network-lint: allow -- not-assessed reason string; 'sendmail' names the mail daemon in prose; no network call
    fi
  elif [ -z "$SMF_GREP_RAW" ]; then
    SMF_STATUS=NOT_ASSESSED
    SMF_REASON="sendmail HELP probe returned empty output with rc=0"  # network-lint: allow -- not-assessed reason string; 'sendmail' names the mail daemon in prose; no network call
  else
    SMF_HELPFILE=$(printf '%s\n' "$SMF_GREP_RAW" | awk '
      /^O HelpFile=/ {
        if (seen) { bad = 1; next }
        seen = 1
    ...
    ')
    if [ -z "$SMF_HELPFILE" ]; then
      SMF_STATUS=NOT_ASSESSED
      SMF_REASON="sendmail HELP option present but its help file path is unparseable"  # network-lint: allow -- not-assessed reason string; 'sendmail' names the mail daemon in prose; no network call
    else
      SMF_LS_RAW=$(aix helpfile_ls ls -ld "$SMF_HELPFILE")
      SMF_LS_RC=$?
      if [ "$SMF_LS_RC" -eq 0 ] && [ -n "$SMF_LS_RAW" ]; then
        SMF_STATUS=FAIL
        SMF_OBSERVED="O HelpFile=$SMF_HELPFILE is set and the help text file exists; the SMTP HELP command is enabled"
      elif [ "$SMF_LS_RC" -eq 2 ] && [ -z "$SMF_LS_RAW" ]; then
        SMF_STATUS=PASS
        SMF_OBSERVED="O HelpFile=$SMF_HELPFILE is set but the help text file is not present; the SMTP HELP command has no help file to serve"
      elif [ "$SMF_LS_RC" -eq 0 ] && [ -z "$SMF_LS_RAW" ]; then
        SMF_STATUS=NOT_ASSESSED
        SMF_REASON="help file existence probe returned empty output with rc=0"
      else
        SMF_STATUS=NOT_ASSESSED
        SMF_REASON="help file existence probe failed (rc=$SMF_LS_RC)"
      fi
    fi
  fi
  ...
  case "$SMF_STATUS" in
    PASS)
      SMF_MEANING="The SMTP service is not configured to serve the HELP command: the O HelpFile option is absent, the subject sendmail.cf is absent, or the named help text file does not exist."  # network-lint: allow -- finding meaning text; 'sendmail' names the mail daemon in prose; no network call
      SMF_FIX="n/a"
      ;;
    FAIL)
      SMF_MEANING="The SMTP service serves the HELP command, disclosing the sendmail command grammar to any connecting peer."  # network-lint: allow -- finding meaning text; 'sendmail' names the mail daemon in prose; no network call
      SMF_FIX="remove the named help text file and the 'O HelpFile' option from /etc/mail/sendmail.cf, then restart sendmail; PTxray only recommends 'rm $SMF_HELPFILE' and never runs it."  # network-lint: allow -- remediation advice text PTxray never executes; 'sendmail' is part of a local file pathname; no network call
      ;;
    *)
      SMF_MEANING="PTxray did not obtain trustworthy evidence for the SMTP HELP command state."
      SMF_FIX="verify the 'O HelpFile' option in /etc/mail/sendmail.cf and the named help text file as root, then rerun PTxray."  # network-lint: allow -- remediation advice text PTxray never executes; 'sendmail' is part of a local file pathname; no network call
      ;;
  esac

  if [ "$SMF_STATUS" = NOT_ASSESSED ]; then
    SMF_OBSERVED="not assessed - $SMF_REASON"
  fi

  add security stig_sendmail_helpfile "SMTP service HELP command" \
    "$SMF_STATUS" med "$SMF_OBSERVED" \
    "$SMF_MEANING" "$SMF_FIX" "stig:V-215417"
}
