...
AIXRAY_TOOL=ck-stig-sendmail-loglevel


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_sendmail_loglevel — DISA STIG V-215344.
  # sendmail LogLevel must not be below 9. Absent sendmail.cf is a  # network-lint: allow -- prose comment; 'sendmail' names the mail daemon in prose; no network call
  # prohibition PASS (svcoff): no sendmail config, no weak logging.  # network-lint: allow -- prose comment; 'sendmail' names the mail daemon in prose; no network call
  typeset SMLL_RAW SMLL_RC SMLL_VAL SMLL_PARSE_RC SMLL_CLASS
  typeset SMLL_STATUS SMLL_OBSERVED SMLL_MEANING SMLL_FIX

  SMLL_RAW=$(aix sendmail_cf_loglevel grep '^O LogLevel' /etc/mail/sendmail.cf)  # network-lint: allow -- aix() capture of a local read-only command; 'sendmail' is part of a local file pathname; no network call
  SMLL_RC=$?
  SMLL_CLASS=""
  SMLL_VAL=""

  if [ "$SMLL_RC" -ne 0 ]; then
    case "$SMLL_RAW" in
      *"can't open"*|*"Cannot open"*|*"cannot open"*|*"No such file"*|*"does not exist"*)
        SMLL_CLASS=subject_absent
        ;;
      *)
        SMLL_CLASS=probe_failed
        ;;
    esac
  elif [ -z "$SMLL_RAW" ]; then
    SMLL_CLASS=probe_empty
  else
    SMLL_VAL=$(printf '%s\n' "$SMLL_RAW" | awk '
      /^O LogLevel/ {
        if (got) next
        ...
        line = $0
        sub(/#.*$/, "", line)
        if (line !~ /^O LogLevel[ \t]*=[ \t]*[0-9]+[ \t]*$/) { bad = 1; next }
        sub(/^O LogLevel[ \t]*=[ \t]*/, "", line)
        gsub(/[ \t]/, "", line)
        print line + 0
      }
    ...
    ')
    SMLL_PARSE_RC=$?
    if [ "$SMLL_PARSE_RC" -ne 0 ] || [ -z "$SMLL_VAL" ]; then
      SMLL_CLASS=unparseable
    elif [ "$SMLL_VAL" -ge 9 ]; then
      SMLL_CLASS=compliant
    else
      SMLL_CLASS=noncompliant
    fi
  fi
  ...
  case "$SMLL_CLASS" in
    compliant)
      SMLL_STATUS=PASS
      SMLL_OBSERVED="sendmail LogLevel=$SMLL_VAL (>= 9)"  # network-lint: allow -- observed-state report string; 'sendmail' names the mail daemon in prose; no network call
      SMLL_MEANING="sendmail logs mail activity at STIG depth (LogLevel 9 or higher)."  # network-lint: allow -- finding meaning text; 'sendmail' names the mail daemon in prose; no network call
      SMLL_FIX="n/a"
      ;;
    noncompliant)
      SMLL_STATUS=FAIL
      SMLL_OBSERVED="sendmail LogLevel=$SMLL_VAL (less than 9)"  # network-lint: allow -- observed-state report string; 'sendmail' names the mail daemon in prose; no network call
      SMLL_MEANING="sendmail logging is below 9, so mail activity is not logged at STIG depth."  # network-lint: allow -- finding meaning text; 'sendmail' names the mail daemon in prose; no network call
      SMLL_FIX="edit /etc/mail/sendmail.cf so the line is O LogLevel=9; PTxray only recommends this change."  # network-lint: allow -- remediation advice text PTxray never executes; 'sendmail' is part of a local file pathname; no network call
      ;;
    subject_absent)
      SMLL_STATUS=PASS
      SMLL_OBSERVED="sendmail.cf is absent; sendmail logging is not configured"  # network-lint: allow -- observed-state report string; 'sendmail' names the mail daemon in prose; no network call
      SMLL_MEANING="No sendmail.cf is present, so sendmail is not configured with a weak LogLevel."  # network-lint: allow -- finding meaning text; 'sendmail' names the mail daemon in prose; no network call
      SMLL_FIX="n/a"
      ;;
    probe_failed) SMLL_STATUS=NOT_ASSESSED
      SMLL_OBSERVED="not assessed - sendmail LogLevel probe failed (rc=$SMLL_RC)"  # network-lint: allow -- observed-state report string; 'sendmail' names the mail daemon in prose; no network call
      SMLL_MEANING="PTxray did not obtain trustworthy sendmail LogLevel evidence."  # network-lint: allow -- finding meaning text; 'sendmail' names the mail daemon in prose; no network call
      SMLL_FIX="verify that grep '^O LogLevel' /etc/mail/sendmail.cf runs, then rerun PTxray."  # network-lint: allow -- remediation advice text PTxray never executes; 'sendmail' is part of a local file pathname; no network call
      ;;
    probe_empty) SMLL_STATUS=NOT_ASSESSED
      SMLL_OBSERVED="not assessed - sendmail LogLevel probe returned no output"  # network-lint: allow -- observed-state report string; 'sendmail' names the mail daemon in prose; no network call
      SMLL_MEANING="PTxray did not obtain trustworthy sendmail LogLevel evidence."  # network-lint: allow -- finding meaning text; 'sendmail' names the mail daemon in prose; no network call
      SMLL_FIX="verify that grep '^O LogLevel' /etc/mail/sendmail.cf returns a LogLevel line, then rerun PTxray."  # network-lint: allow -- remediation advice text PTxray never executes; 'sendmail' is part of a local file pathname; no network call
      ;;
    *)
      SMLL_STATUS=NOT_ASSESSED
      SMLL_OBSERVED="not assessed - sendmail LogLevel value was unparseable"  # network-lint: allow -- observed-state report string; 'sendmail' names the mail daemon in prose; no network call
      SMLL_MEANING="PTxray did not obtain trustworthy sendmail LogLevel evidence."  # network-lint: allow -- finding meaning text; 'sendmail' names the mail daemon in prose; no network call
      SMLL_FIX="verify the O LogLevel line in /etc/mail/sendmail.cf, then rerun PTxray."  # network-lint: allow -- remediation advice text PTxray never executes; 'sendmail' is part of a local file pathname; no network call
      ;;
  esac

  add security stig_sendmail_loglevel "Sendmail LogLevel" \
    "$SMLL_STATUS" med "$SMLL_OBSERVED" \
    "$SMLL_MEANING" "$SMLL_FIX" "stig:V-215344"
}
