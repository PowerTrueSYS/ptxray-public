...
AIXRAY_TOOL=ck-stig-sendmail-version


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_sendmail_version — DISA STIG V-215414.
  # Deviation: check-content invokes the MTA in address-test debug mode.
  # Evidence is lslpp -qcL bos.net.tcp.sendmail. DEBUG was removed in  # network-lint: allow -- prose comment; 'sendmail' is part of the fileset name bos.net.tcp.sendmail; no network call
  # sendmail 8.6. AIX 6/7 fileset VRMF (6.x/7.x) is packaging, not the  # network-lint: allow -- prose comment; 'sendmail' names the mail daemon in prose; no network call
  # sendmail version; those releases ship 8.14+ and meet the floor.  # network-lint: allow -- prose comment; 'sendmail' names the mail daemon in prose; no network call
  typeset SMV_RAW SMV_RC SMV_SHAPE SMV_PARSE_RC SMV_LEVEL SMV_REASON
  typeset SMV_STATUS SMV_SEV SMV_OBSERVED SMV_MEANING SMV_FIX

  SMV_RAW=$(aix sendmail_lslpp lslpp -qcL bos.net.tcp.sendmail)  # network-lint: allow -- aix() capture of a local read-only command; 'sendmail' is part of the fileset name bos.net.tcp.sendmail; no network call
  SMV_RC=$?
  SMV_SHAPE=""
  SMV_LEVEL=""
  SMV_REASON=""
  ...
  if [ -z "$SMV_RAW" ]; then
    if [ "$SMV_RC" -ne 0 ]; then
      SMV_REASON=probe_failed
    else
      SMV_REASON=probe_empty
    fi
  else
    # network-lint: allow-next=13 -- awk program parsing lslpp output; 'sendmail' occurs only inside the fileset name bos.net.tcp.sendmail; a trailing comment would corrupt the awk source
    SMV_SHAPE=$(printf '%s\n' "$SMV_RAW" | awk '
      BEGIN { FS = ":" }
      /^[ \t]*$/ { next }
      /^lslpp: 0504-132[ \t]+Fileset bos\.net\.tcp\.sendmail not installed\.?[ \t]*$/ {
        absent++
        next
      }
      /^lslpp:[ \t]+Fileset bos\.net\.tcp\.sendmail not installed\.?[ \t]*$/ {
        absent++
        next
      }
      {
        if (NF < 6 || $2 != "bos.net.tcp.sendmail") { bad = 1; next }
        if ($3 !~ /^[0-9][0-9]*(\.[0-9][0-9]*)*$/) { bad = 1; next }
        if (seen && $3 != level) { bad = 1; next }
        seen = 1
        level = $3
      }
      END {
        if (bad || (seen && absent) || (!seen && !absent)) exit 1
        if (absent) { print "absent"; exit }
        n = split(level, a, ".")
        meet = 0
        if (a[1] + 0 == 6 || a[1] + 0 == 7) meet = 1
        else if (a[1] + 0 > 8) meet = 1
        else if (a[1] + 0 == 8 && n >= 2 && a[2] + 0 >= 6) meet = 1
        if (meet) print "ok:" level
        else print "old:" level
      }
    ')
    SMV_PARSE_RC=$?
    if [ "$SMV_PARSE_RC" -eq 0 ] && [ "$SMV_SHAPE" = absent ]; then
      SMV_REASON=absent
    elif [ "$SMV_RC" -ne 0 ]; then
      SMV_REASON=probe_failed
    elif [ "$SMV_PARSE_RC" -ne 0 ] || [ -z "$SMV_SHAPE" ]; then
      SMV_REASON=unparseable
    else
      case "$SMV_SHAPE" in
        ok:*)
          SMV_REASON=ok
          SMV_LEVEL=${SMV_SHAPE#ok:}
          ;;
        old:*)
          SMV_REASON=old
          SMV_LEVEL=${SMV_SHAPE#old:}
          ;;
        *) SMV_REASON=unparseable ;;
      esac
    fi
  ...
  case "$SMV_REASON" in
    ok)
      SMV_STATUS=PASS
      SMV_OBSERVED="bos.net.tcp.sendmail level=$SMV_LEVEL (meets 8.6 floor)"  # network-lint: allow -- observed-state report string; 'sendmail' is part of the fileset name bos.net.tcp.sendmail; no network call
      SMV_MEANING="Installed sendmail meets the 8.6 floor, so the DEBUG SMTP command is not implemented."  # network-lint: allow -- finding meaning text; 'sendmail' names the mail daemon in prose; no network call
      SMV_FIX="n/a"
      ;;
    old) SMV_STATUS=FAIL
      SMV_OBSERVED="bos.net.tcp.sendmail level=$SMV_LEVEL (below 8.6)"  # network-lint: allow -- observed-state report string; 'sendmail' is part of the fileset name bos.net.tcp.sendmail; no network call
      SMV_MEANING="Installed sendmail is older than 8.6 and still implements the DEBUG SMTP command."  # network-lint: allow -- finding meaning text; 'sendmail' names the mail daemon in prose; no network call
      SMV_FIX="Obtain and install a more recent version of Sendmail, which does not implement the DEBUG feature. PTxray only recommends this action."
      ;;
    absent)
      SMV_STATUS=PASS
      SMV_OBSERVED="bos.net.tcp.sendmail is not installed"  # network-lint: allow -- observed-state report string; 'sendmail' is part of the fileset name bos.net.tcp.sendmail; no network call
      SMV_MEANING="Sendmail is not installed, so the DEBUG SMTP command cannot be offered."
      SMV_FIX="n/a"
      ;;
    probe_failed) SMV_STATUS=NOT_ASSESSED
      SMV_OBSERVED="not assessed - sendmail fileset probe failed (rc=$SMV_RC)"  # network-lint: allow -- observed-state report string; 'sendmail' names the mail daemon in prose; no network call
      SMV_MEANING="PTxray did not obtain trustworthy bos.net.tcp.sendmail fileset evidence."  # network-lint: allow -- finding meaning text; 'sendmail' is part of the fileset name bos.net.tcp.sendmail; no network call
      SMV_FIX="verify that 'lslpp -qcL bos.net.tcp.sendmail' runs successfully, then rerun PTxray."  # network-lint: allow -- remediation advice text PTxray never executes; 'sendmail' is part of the fileset name bos.net.tcp.sendmail; no network call
      ;;
    probe_empty) SMV_STATUS=NOT_ASSESSED
      SMV_OBSERVED="not assessed - sendmail fileset probe returned no output"  # network-lint: allow -- observed-state report string; 'sendmail' names the mail daemon in prose; no network call
      SMV_MEANING="PTxray did not obtain trustworthy bos.net.tcp.sendmail fileset evidence."  # network-lint: allow -- finding meaning text; 'sendmail' is part of the fileset name bos.net.tcp.sendmail; no network call
      SMV_FIX="verify that 'lslpp -qcL bos.net.tcp.sendmail' returns fileset evidence, then rerun PTxray."  # network-lint: allow -- remediation advice text PTxray never executes; 'sendmail' is part of the fileset name bos.net.tcp.sendmail; no network call
      ;;
    *)
      SMV_STATUS=NOT_ASSESSED
      SMV_OBSERVED="not assessed - sendmail fileset evidence was unparseable"  # network-lint: allow -- observed-state report string; 'sendmail' names the mail daemon in prose; no network call
      SMV_MEANING="PTxray did not obtain trustworthy bos.net.tcp.sendmail fileset evidence."  # network-lint: allow -- finding meaning text; 'sendmail' is part of the fileset name bos.net.tcp.sendmail; no network call
      SMV_FIX="verify lslpp output for bos.net.tcp.sendmail, then rerun PTxray."  # network-lint: allow -- remediation advice text PTxray never executes; 'sendmail' is part of the fileset name bos.net.tcp.sendmail; no network call
      ;;
  esac

  case "$SMV_STATUS" in
    NOT_APPLICABLE) SMV_SEV=low ;;
    *) SMV_SEV=med ;;
  esac

  add security stig_sendmail_version "Sendmail DEBUG feature disabled" \
    "$SMV_STATUS" "$SMV_SEV" "$SMV_OBSERVED" \
    "$SMV_MEANING" "$SMV_FIX" "stig:V-215414"
}
