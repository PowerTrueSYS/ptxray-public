...
AIXRAY_TOOL=ck-stig-server-does-host-snmp


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_server_does_host_snmp — DISA STIG V-215364.
  # PARTIAL-AUTO. Prohibition: dpid2 must not start from /etc/rc.tcpip when
  # this host does not host an SNMP agent. Determinate absence of a start
  # line is compliant. A matching start line is the manual portion: the
  # no-SNMP-agent premise is a site/ISSO determination this grep cannot
  # prove, so that branch is typed NOT_ASSESSED until answered, never FAIL.
  # Nonempty rc=0 output is a match only when every nonempty line matches
  # ^start[[:blank:]]/usr/sbin/dpid2; any other shape is unparseable.
  typeset DPID2_RAW DPID2_RC DPID2_CLASS DPID2_SHAPE
  typeset DPID2_STATUS DPID2_OBSERVED DPID2_MEANING DPID2_FIX

  DPID2_RAW=$(aix rc_tcpip_dpid2 grep '^start[[:blank:]]/usr/sbin/dpid2' /etc/rc.tcpip)
  DPID2_RC=$?
  DPID2_CLASS=""
  DPID2_SHAPE=""

  if [ "$DPID2_RC" -eq 0 ]; then
    if [ -z "$DPID2_RAW" ]; then
      DPID2_CLASS=probe_empty
    else
      DPID2_SHAPE=$(printf '%s\n' "$DPID2_RAW" | awk '
        /^start[ \t]\/usr\/sbin\/dpid2/ { hit=1; next }
        NF { other=1 }
        END {
          if (hit && !other) print "match"
          else print "unparseable"
        }
      ')
      if [ "$DPID2_SHAPE" = match ]; then
        DPID2_CLASS=match
      else
        DPID2_CLASS=unparseable
      fi
    fi
  elif [ "$DPID2_RC" -eq 1 ]; then
    if [ -z "$DPID2_RAW" ]; then
      DPID2_CLASS=nomatch
    else
      DPID2_CLASS=unparseable
    ...
    DPID2_CLASS=probe_failed
  fi

  case "$DPID2_CLASS" in
    match) DPID2_STATUS=NOT_ASSESSED
      DPID2_OBSERVED="not assessed - dpid2 starts at boot in /etc/rc.tcpip; whether this host hosts an SNMP agent is a site/ISSO determination (manual portion of DISA STIG V-215364)"
      DPID2_MEANING="The dpid2 start line is present. DISA STIG V-215364 applies only when this host does not host an SNMP agent; PTxray cannot prove that premise from /etc/rc.tcpip, so this control stays undecided until the site answers."
      DPID2_FIX="# chrctcp -d dpid2"
      ;;
    nomatch) DPID2_STATUS=PASS
      DPID2_OBSERVED="dpid2 does not start from /etc/rc.tcpip"
      DPID2_MEANING="dpid2 is not started from /etc/rc.tcpip."
      DPID2_FIX="n/a"
      ;;
    probe_failed) DPID2_STATUS=NOT_ASSESSED
      DPID2_OBSERVED="not assessed - dpid2 rc.tcpip probe failed (rc=$DPID2_RC)"
      DPID2_MEANING="PTxray did not obtain trustworthy /etc/rc.tcpip evidence for the dpid2 start line."
      DPID2_FIX="verify that grep '^start[[:blank:]]/usr/sbin/dpid2' /etc/rc.tcpip runs successfully, then rerun PTxray."
      ;;
    probe_empty) DPID2_STATUS=NOT_ASSESSED
      DPID2_OBSERVED="not assessed - dpid2 rc.tcpip probe returned no output"
      DPID2_MEANING="PTxray did not obtain trustworthy /etc/rc.tcpip evidence for the dpid2 start line."
      DPID2_FIX="verify that grep '^start[[:blank:]]/usr/sbin/dpid2' /etc/rc.tcpip returns complete evidence, then rerun PTxray."
      ;;
    *)
      DPID2_STATUS=NOT_ASSESSED
      DPID2_OBSERVED="not assessed - dpid2 rc.tcpip evidence was unparseable"
      DPID2_MEANING="PTxray did not obtain trustworthy /etc/rc.tcpip evidence for the dpid2 start line."
      DPID2_FIX="verify /etc/rc.tcpip is readable, then rerun PTxray."
      ;;
  esac

  add security stig_server_does_host_snmp \
      "dpid2 disabled when host does not host an SNMP agent" \
      "$DPID2_STATUS" med "$DPID2_OBSERVED" "$DPID2_MEANING" "$DPID2_FIX" \
      "stig:V-215364"
}
