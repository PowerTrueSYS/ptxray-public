...
AIXRAY_TOOL=ck-stig-server-functioning-dns-server


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_server_functioning_dns_server — DISA STIG V-215360.
  # Prohibition rule: named must not start from /etc/rc.tcpip unless this host
  # is a DNS server. grep rc 1 with empty stdout is no-match (compliant);
  # rc 0 with empty stdout is a swallowed error, not absence.
  typeset NAMED_RAW NAMED_RC NAMED_HIT NAMED_STATUS NAMED_OBSERVED
  typeset NAMED_MEANING NAMED_FIX

  NAMED_RAW=$(aix rctcp_named grep "^start[[:blank:]]/usr/sbin/named" /etc/rc.tcpip)
  NAMED_RC=$?
  NAMED_HIT=""
  NAMED_STATUS=""

  if [ "$NAMED_RC" -eq 1 ] && [ -z "$NAMED_RAW" ]; then
    NAMED_STATUS=PASS
    NAMED_OBSERVED="named is not started from /etc/rc.tcpip"
  elif [ "$NAMED_RC" -ne 0 ]; then
    NAMED_STATUS=NOT_ASSESSED
    NAMED_OBSERVED="not assessed - rc.tcpip named probe failed (rc=$NAMED_RC)"
  elif [ -z "$NAMED_RAW" ]; then
    NAMED_STATUS=NOT_ASSESSED
    NAMED_OBSERVED="not assessed - rc.tcpip named probe returned no output with rc 0"
  else
    NAMED_HIT=$(printf '%s\n' "$NAMED_RAW" | awk '
      /^start[	 ]\/usr\/sbin\/named/ { hit=1 }
      END { if (hit) print "hit" }
    ')
    if [ -n "$NAMED_HIT" ]; then
      NAMED_STATUS=FAIL
      NAMED_OBSERVED="named starts at boot in /etc/rc.tcpip"
    else
      NAMED_STATUS=PASS
      NAMED_OBSERVED="named is not started from /etc/rc.tcpip"
    fi
  ...
  case "$NAMED_STATUS" in
    FAIL)
      NAMED_MEANING="The named daemon is started from /etc/rc.tcpip, so this host is functioning as a DNS server; DISA STIG V-215360 requires named disabled when the host is not a DNS server."
      NAMED_FIX="disable named at boot with 'chrctcp -d named' after validating the host is not a DNS server; PTxray only recommends this command."
      ;;
    PASS)
      NAMED_MEANING="named is not started from /etc/rc.tcpip, which is the required state when this host is not a DNS server."
      NAMED_FIX="n/a"
      ;;
    *)
      NAMED_MEANING="PTxray did not obtain trustworthy /etc/rc.tcpip evidence for the named start line."
      NAMED_FIX="verify that 'grep \"^start[[:blank:]]/usr/sbin/named\" /etc/rc.tcpip' can run and returns complete output, then rerun PTxray."
      ;;
  esac

  add security stig_server_functioning_dns_server \
      "named daemon disabled when not a DNS server" \
      "$NAMED_STATUS" med "$NAMED_OBSERVED" \
      "$NAMED_MEANING" "$NAMED_FIX" "stig:V-215360"
}
