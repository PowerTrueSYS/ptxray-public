...
AIXRAY_TOOL=ck-stig-server-functioning-multicast-router


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_server_functioning_multicast_router — DISA STIG V-215359.
  # Prohibition: mrouted must not start from /etc/rc.tcpip. A matching
  # start line is FAIL. grep no-match (rc 1, empty) is determinate
  # absence = PASS (svcoff). rc 0 + empty is a swallowed error.
  typeset SFMR_RAW SFMR_RC SFMR_KIND SFMR_STATUS SFMR_SEV
  typeset SFMR_OBSERVED SFMR_MEANING SFMR_FIX
  typeset SFMR_FAILCLOSED_PROBE SFMR_FAILCLOSED_EMPTY

  SFMR_FAILCLOSED_PROBE=NOT_ASSESSED
  SFMR_FAILCLOSED_EMPTY=NOT_ASSESSED

  SFMR_RAW=$(aix rc_tcpip_mrouted grep "^start[[:blank:]]/usr/sbin/mrouted" /etc/rc.tcpip)
  SFMR_RC=$?

  if [ "$SFMR_RC" -ne 0 ]; then
    if [ "$SFMR_RC" -eq 1 ] && [ -z "$SFMR_RAW" ]; then
      SFMR_STATUS=PASS
      SFMR_OBSERVED="no mrouted start line in /etc/rc.tcpip; multicast-router prohibition holds"
    else
      SFMR_STATUS=$SFMR_FAILCLOSED_PROBE
      SFMR_OBSERVED="not assessed - mrouted rc.tcpip probe failed (rc=$SFMR_RC)"
    fi
  elif [ -z "$SFMR_RAW" ]; then
    SFMR_STATUS=$SFMR_FAILCLOSED_EMPTY
    SFMR_OBSERVED="not assessed - mrouted rc.tcpip probe returned empty output with rc=0"
  else
    SFMR_KIND=$(printf '%s\n' "$SFMR_RAW" | awk '
      /^start[ \t]\/usr\/sbin\/mrouted/ { hit=1 }
      END { if (hit) print "hit"; else print "miss" }
    ')
    case "$SFMR_KIND" in
      hit) SFMR_STATUS=FAIL
        SFMR_OBSERVED="mrouted starts from /etc/rc.tcpip"
        ;;
      miss) SFMR_STATUS=PASS
        SFMR_OBSERVED="mrouted is not started from /etc/rc.tcpip"
        ;;
      *)
        SFMR_STATUS=NOT_ASSESSED
        SFMR_OBSERVED="not assessed - mrouted rc.tcpip probe output was unparseable"
        ;;
    esac
  fi
  ...
  case "$SFMR_STATUS" in
    PASS)
      SFMR_MEANING="The mrouted daemon is not started from /etc/rc.tcpip, so this host is not functioning as a multicast router."
      SFMR_FIX="n/a"
      ;;
    FAIL)
      SFMR_MEANING="The mrouted daemon is started from /etc/rc.tcpip; if this server is not a multicast router, that is a finding."
      SFMR_FIX="chrctcp -d mrouted; PTxray only recommends this command."
      ;;
    *)
      SFMR_MEANING="PTxray could not finish the mrouted assessment: the /etc/rc.tcpip probe failed or returned empty output."
      SFMR_FIX="verify that grep \"^start[[:blank:]]/usr/sbin/mrouted\" /etc/rc.tcpip runs, then rerun PTxray."
      ;;
  esac

  case "$SFMR_STATUS" in
    NOT_APPLICABLE) SFMR_SEV=low ;;
    *) SFMR_SEV=med ;;
  esac

  add security stig_server_functioning_multicast_router \
    "mrouted must be disabled when not a multicast router" \
    "$SFMR_STATUS" "$SFMR_SEV" "$SFMR_OBSERVED" \
    "$SFMR_MEANING" "$SFMR_FIX" "stig:V-215359"
}
