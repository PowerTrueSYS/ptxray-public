...
AIXRAY_TOOL=ck-stig-ssh-clientalive


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_ssh_clientalive — DISA STIG V-215290.
  # STIG-literal ClientAliveInterval <= 600 and ClientAliveCountMax == 0.
  typeset SSCA_RAW SSCA_RC SSCA_SHAPE SSCA_INT SSCA_MAX
  typeset SSCA_STATUS SSCA_OBSERVED SSCA_MEANING SSCA_FIX

  SSCA_RAW=$(aix sshd_clientalive grep -E "^ClientAliveInterval|^ClientAliveCountMax" /etc/ssh/sshd_config)  # network-lint: allow -- aix() capture of a local read-only command; 'ssh' is a component of a local /etc/ssh pathname; no network call
  SSCA_RC=$?
  SSCA_SHAPE=""
  SSCA_INT=""
  SSCA_MAX=""
  ...
  if [ -z "$SSCA_RAW" ]; then
    if [ "$SSCA_RC" -eq 1 ]; then
      SSCA_STATUS=FAIL
      SSCA_OBSERVED="ClientAliveInterval and ClientAliveCountMax not set in /etc/ssh/sshd_config"  # network-lint: allow -- observed-state report string; 'ssh' is a component of a local /etc/ssh pathname; no network call
    elif [ "$SSCA_RC" -eq 0 ]; then
      SSCA_STATUS=NOT_ASSESSED
      SSCA_OBSERVED="not assessed - ClientAlive probe returned empty output"
    else
      SSCA_STATUS=NOT_ASSESSED
      SSCA_OBSERVED="not assessed - ClientAlive probe failed (rc=$SSCA_RC)"
    fi
  elif [ "$SSCA_RC" -ne 0 ]; then
    SSCA_STATUS=NOT_ASSESSED
    SSCA_OBSERVED="not assessed - ClientAlive probe failed (rc=$SSCA_RC)"
  else
    SSCA_SHAPE=$(printf '%s\n' "$SSCA_RAW" | awk '
      ...
      /^[ \t]*$/ { next }
      {
        ...
        if ($1 == "ClientAliveInterval") {
          if (interval == "") interval = $2
        } else if ($1 == "ClientAliveCountMax") {
          if (countmax == "") countmax = $2
        }
      }
      END {
        if (bad) { print "unparseable"; exit 0 }
        if (interval == "" && countmax == "") { print "unset::"; exit 0 }
        if (interval == "" || countmax == "") {
          print "unset:" interval ":" countmax
          exit 0
        }
        if (countmax == "0" && interval + 0 <= 600) print "ok:" interval ":" countmax
        else print "bad:" interval ":" countmax
      }
    ')
    case "$SSCA_SHAPE" in
      unparseable)
        SSCA_STATUS=NOT_ASSESSED
        SSCA_OBSERVED="not assessed - ClientAlive settings were unparseable"
        ;;
      unset:*)
        SSCA_STATUS=FAIL
        SSCA_INT=${SSCA_SHAPE#unset:}
        SSCA_MAX=${SSCA_INT#*:}
        SSCA_INT=${SSCA_INT%%:*}
        SSCA_OBSERVED="ClientAliveInterval=${SSCA_INT:-unset} ClientAliveCountMax=${SSCA_MAX:-unset}"
        ;;
      ok:*)
        SSCA_INT=${SSCA_SHAPE#ok:}
        SSCA_MAX=${SSCA_INT#*:}
        SSCA_INT=${SSCA_INT%%:*}
        SSCA_STATUS=PASS
        SSCA_OBSERVED="ClientAliveInterval=$SSCA_INT ClientAliveCountMax=$SSCA_MAX"
        ;;
      bad:*)
        SSCA_INT=${SSCA_SHAPE#bad:}
        SSCA_MAX=${SSCA_INT#*:}
        SSCA_INT=${SSCA_INT%%:*}
        SSCA_STATUS=FAIL
        SSCA_OBSERVED="ClientAliveInterval=$SSCA_INT ClientAliveCountMax=$SSCA_MAX"
      ...
      *)
        SSCA_STATUS=NOT_ASSESSED
        SSCA_OBSERVED="not assessed - ClientAlive parser returned an unexpected shape"
        ;;
    esac
  fi
  ...
  case "$SSCA_STATUS" in
    PASS)
      SSCA_MEANING="SSH idle sessions are terminated within the STIG interval."
      SSCA_FIX="n/a"
      ;;
    FAIL)
      SSCA_MEANING="SSH idle sessions may persist beyond the required interval, leaving an unattended session available to reuse."
      SSCA_FIX="set ClientAliveInterval to 600 or less and ClientAliveCountMax to 0 in /etc/ssh/sshd_config, then stopsrc -s sshd and startsrc -s sshd; PTxray only recommends these commands."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      ;;
    *)
      SSCA_MEANING="PTxray did not obtain trustworthy ClientAliveInterval/ClientAliveCountMax evidence from /etc/ssh/sshd_config."  # network-lint: allow -- finding meaning text; 'ssh' is a component of a local /etc/ssh pathname; no network call
      SSCA_FIX="verify /etc/ssh/sshd_config is readable and rerun PTxray."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      ;;
  esac

  add security stig_ssh_clientalive "SSH idle timeout interval" \
    "$SSCA_STATUS" med "$SSCA_OBSERVED" \
    "$SSCA_MEANING" "$SSCA_FIX" "stig:V-215290"
}
