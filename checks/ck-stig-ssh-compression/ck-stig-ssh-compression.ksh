...
AIXRAY_TOOL=ck-stig-ssh-compression


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_ssh_compression — DISA STIG V-215296.
  # OpenSSH 7.4 or above is not applicable; otherwise Compression must be no.
  typeset SSC_VER_RAW SSC_VER_RC SSC_VER_SHAPE SSC_VER_PARSE_RC
  typeset SSC_CFG_RAW SSC_CFG_RC SSC_VAL SSC_VAL_RC
  typeset SSC_STATUS SSC_SEV SSC_OBSERVED SSC_MEANING SSC_FIX
  ...
  SSC_VER_RAW=$(aix openssh_version sh -c "ssh -V 2>&1")  # network-lint: allow -- 'ssh -V' is a local version query: it prints the OpenSSH client version to stderr and exits without resolving a name or opening a socket. This lint is the PR-time tripwire; the syscall trace in egress-trace.sh is what actually enforces the network boundary, and allowing a socket-free version probe does not weaken it.
  SSC_VER_RC=$?

  if [ "$SSC_VER_RC" -ne 0 ]; then
    SSC_STATUS=NOT_ASSESSED
    SSC_OBSERVED="not assessed - OpenSSH version probe failed (rc=$SSC_VER_RC)"
  elif [ -z "$SSC_VER_RAW" ]; then
    SSC_STATUS=NOT_ASSESSED
    SSC_OBSERVED="not assessed - OpenSSH version probe returned empty output"
  else
    SSC_VER_SHAPE=$(printf '%s\n' "$SSC_VER_RAW" | awk '
      {
        idx = index($0, "OpenSSH_")
        if (idx == 0) next
        rest = substr($0, idx + 8)
        n = split(rest, a, /[^0-9]/)
        if (n < 1 || a[1] == "") next
        major = a[1] + 0
        minor = 0
        if (n >= 2 && a[2] != "") minor = a[2] + 0
        if (major > 7 || (major == 7 && minor >= 4)) print "na"
        else print "apply"
        found = 1
        exit
      }
    ...
    ')
    SSC_VER_PARSE_RC=$?
    if [ "$SSC_VER_PARSE_RC" -ne 0 ] || [ -z "$SSC_VER_SHAPE" ]; then
      SSC_STATUS=NOT_ASSESSED
      SSC_OBSERVED="not assessed - OpenSSH version output was unparseable"
    elif [ "$SSC_VER_SHAPE" = "na" ]; then
      SSC_STATUS=NOT_APPLICABLE
      SSC_SEV=low
      SSC_OBSERVED="OpenSSH 7.4 or above; SSH compression requirement is not applicable"
    fi
  fi

  if [ -z "$SSC_STATUS" ]; then
    SSC_CFG_RAW=$(aix sshd_config_compression sh -c "grep -i Compression /etc/ssh/sshd_config | grep -v '^#'")  # network-lint: allow -- aix() capture of a local read-only command; 'ssh' is a component of a local /etc/ssh pathname; no network call
    SSC_CFG_RC=$?
    if [ -z "$SSC_CFG_RAW" ]; then
      if [ "$SSC_CFG_RC" -eq 0 ]; then
        SSC_STATUS=NOT_ASSESSED
        SSC_OBSERVED="not assessed - sshd_config Compression probe returned empty output"
      elif [ "$SSC_CFG_RC" -eq 1 ]; then
        SSC_STATUS=FAIL
        SSC_OBSERVED="Compression not present"
      else
        SSC_STATUS=NOT_ASSESSED
        SSC_OBSERVED="not assessed - sshd_config Compression probe failed (rc=$SSC_CFG_RC)"
      fi
    elif [ "$SSC_CFG_RC" -ne 0 ]; then
      SSC_STATUS=NOT_ASSESSED
      SSC_OBSERVED="not assessed - sshd_config Compression probe failed (rc=$SSC_CFG_RC)"
    else
      SSC_VAL=$(printf '%s\n' "$SSC_CFG_RAW" | awk '
        {
          ...
          for (i = 1; i <= n; i++) {
            if (f[i] == "") continue
            if (substr(f[i], 1, 1) == "#") break
            if (key == "") { key = f[i]; continue }
            if (val == "") { val = f[i]; continue }
            extra = 1
          }
          if (tolower(key) != "compression") next
          if (val == "" || extra) { ssc_bad = 1; next }
          if (ssc_got == 0) {
            ssc_first = val
            ssc_got = 1
          }
        ...
        END {
          if (ssc_bad || ssc_first == "") exit 1
          print tolower(ssc_first)
        }
      ')
      SSC_VAL_RC=$?
      if [ "$SSC_VAL_RC" -ne 0 ] || [ -z "$SSC_VAL" ]; then
        SSC_STATUS=NOT_ASSESSED
        SSC_OBSERVED="not assessed - sshd_config Compression value was unparseable"
      elif [ "$SSC_VAL" = "no" ]; then
        SSC_STATUS=PASS
        SSC_OBSERVED="Compression no"
      else
        SSC_STATUS=FAIL
        SSC_OBSERVED="Compression $SSC_VAL"
      fi
    fi
  fi
  ...
  case "$SSC_STATUS" in
    PASS)
      SSC_MEANING="The SSH daemon does not allow compression, as required by DISA STIG V-215296."
      SSC_FIX="n/a"
      SSC_SEV=med
      ;;
    FAIL)
      SSC_MEANING="The SSH daemon allows compression, which can leak data from encrypted sessions on OpenSSH versions before 7.4."
      SSC_FIX="set 'Compression no' in /etc/ssh/sshd_config, then recycle the SSH daemon with 'stopsrc -s sshd' and 'startsrc -s sshd'; PTxray only recommends these commands."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      SSC_SEV=med
      ;;
    NOT_APPLICABLE)
      SSC_MEANING="OpenSSH 7.4 or above makes the SSH compression requirement not applicable."
      SSC_FIX="n/a"
      SSC_SEV=low
      ;;
    *)
      SSC_MEANING="PTxray did not obtain trustworthy evidence for the SSH daemon compression setting."
      SSC_FIX="verify OpenSSH with 'ssh -V' and the Compression setting in /etc/ssh/sshd_config, then rerun PTxray."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      SSC_SEV=med
      ;;
  esac

  add security stig_ssh_compression "SSH daemon compression" \
    "$SSC_STATUS" "$SSC_SEV" "$SSC_OBSERVED" \
    "$SSC_MEANING" "$SSC_FIX" "stig:V-215296"
}
