...
AIXRAY_TOOL=ck-stig-ssh-daemon-configured-disable-ssh-daemon


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_ssh_daemon_configured_disable_ssh_daemon — DISA STIG V-215303.
  # Prohibition: sshd must ignore user .rhosts files (IgnoreRhosts yes).
  # Absence of /etc/ssh/sshd_config is compliant (svcoff convention) only when  # network-lint: allow -- prose comment; 'ssh' is a component of a local /etc/ssh pathname; no network call
  # ls evidence determinately says the path does not exist.
  # Uncertain probe evidence is refused, never a clean result.
  typeset SIRH_LS_RAW SIRH_LS_RC SIRH_LS_MISSING
  typeset SIRH_IRH_RAW SIRH_IRH_RC SIRH_FILTERED SIRH_PARSED
  typeset SIRH_VALUE SIRH_REASON SIRH_STATUS SIRH_SEV
  typeset SIRH_OBSERVED SIRH_MEANING SIRH_FIX

  SIRH_LS_RAW=$(aix sshd_config_ls ls -ld /etc/ssh/sshd_config)  # network-lint: allow -- aix() capture of a local read-only command; 'ssh' is a component of a local /etc/ssh pathname; no network call
  SIRH_LS_RC=$?
  # First-stage capture of STIG check-content
  # grep -i IgnoreRhosts /etc/ssh/sshd_config | grep -v '^#'  # network-lint: allow -- prose comment; 'ssh' is a component of a local /etc/ssh pathname; no network call
  # A pipeline would mask a read failure as grep -v no-match (rc 1).
  SIRH_IRH_RAW=$(aix sshd_ignorerhosts grep -i IgnoreRhosts /etc/ssh/sshd_config)  # network-lint: allow -- aix() capture of a local read-only command; 'ssh' is a component of a local /etc/ssh pathname; no network call
  SIRH_IRH_RC=$?
  SIRH_VALUE=""
  SIRH_REASON=""
  SIRH_LS_MISSING=0
  case "$SIRH_LS_RAW" in
    *0653-341*|*"does not exist"*|*"No such file or directory"*)
      SIRH_LS_MISSING=1
      ;;
  esac

  if [ "$SIRH_LS_RC" -ne 0 ] && [ "$SIRH_LS_MISSING" -eq 1 ]; then
    SIRH_REASON=subject_absent
  elif [ "$SIRH_LS_RC" -ne 0 ]; then
    SIRH_REASON=ls_uncertain
  elif [ -z "$SIRH_LS_RAW" ]; then
    SIRH_REASON=probe_empty
  elif [ "$SIRH_IRH_RC" -ne 0 ] && [ "$SIRH_IRH_RC" -ne 1 ]; then
    SIRH_REASON=probe_failed
  elif [ -z "$SIRH_IRH_RAW" ] && [ "$SIRH_IRH_RC" -eq 0 ]; then
    SIRH_REASON=probe_empty
  elif [ -z "$SIRH_IRH_RAW" ]; then
    SIRH_REASON=directive_absent
  else
    # Local equivalent of grep -v '^#' on the first-stage capture.
    SIRH_FILTERED=$(printf '%s\n' "$SIRH_IRH_RAW" | awk '
      substr($0, 1, 1) == "#" { next }
      { print }
    ')
    if [ -z "$SIRH_FILTERED" ]; then
      SIRH_REASON=directive_absent
    else
      SIRH_PARSED=$(printf '%s\n' "$SIRH_FILTERED" | awk '
        {
          line = $0
          gsub(/^[ \t]+/, "", line)
          gsub(/[ \t]+$/, "", line)
          ...
          n = split(line, f, /[ \t=]+/)
          if (n != 2) { bad = 1; next }
          if (tolower(f[1]) != "ignorerhosts") { bad = 1; next }
          val = tolower(f[2])
          if (val != "yes" && val != "no") { bad = 1; next }
          count++
          last = val
          if (val != "yes") {
            nonyes++
            if (first_nonyes == "") first_nonyes = val
          }
        }
        END {
          if (bad) { print "bad"; exit 0 }
          if (count < 1) { print "empty"; exit 0 }
          if (nonyes > 0) { print first_nonyes; exit 0 }
          print "yes"
        }
      ')
      if [ "$SIRH_PARSED" = bad ] || [ "$SIRH_PARSED" = empty ] || [ -z "$SIRH_PARSED" ]; then
        SIRH_REASON=unparseable
      elif [ "$SIRH_PARSED" = yes ]; then
        SIRH_REASON=compliant
        SIRH_VALUE=yes
      else
        SIRH_REASON=noncompliant
        SIRH_VALUE=$SIRH_PARSED
      fi
    fi
  fi
  ...
  case "$SIRH_REASON" in
    compliant)
      SIRH_STATUS=PASS
      SIRH_OBSERVED="IgnoreRhosts yes"
      SIRH_MEANING="sshd is configured to ignore user .rhosts files, so host-based .rhosts trust cannot authenticate over SSH."
      SIRH_FIX="n/a"
      ;;
    noncompliant) SIRH_STATUS=FAIL
      SIRH_OBSERVED="IgnoreRhosts $SIRH_VALUE"
      SIRH_MEANING="sshd is not configured to ignore user .rhosts files, so a per-user .rhosts file can grant host-based login."
      SIRH_FIX="set IgnoreRhosts yes in /etc/ssh/sshd_config; stopsrc -s sshd; startsrc -s sshd; PTxray only recommends these commands."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      ;;
    directive_absent) SIRH_STATUS=FAIL
      SIRH_OBSERVED="IgnoreRhosts is not set"
      SIRH_MEANING="No uncommented IgnoreRhosts yes directive is present, so sshd may honor user .rhosts files."
      SIRH_FIX="set IgnoreRhosts yes in /etc/ssh/sshd_config; stopsrc -s sshd; startsrc -s sshd; PTxray only recommends these commands."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      ;;
    subject_absent) SIRH_STATUS=PASS
      SIRH_OBSERVED="/etc/ssh/sshd_config is absent"  # network-lint: allow -- observed-state report string; 'ssh' is a component of a local /etc/ssh pathname; no network call
      SIRH_MEANING="The SSH daemon configuration is absent, so sshd cannot honor user .rhosts files."
      SIRH_FIX="n/a"
      ;;
    ls_uncertain) SIRH_STATUS=NOT_ASSESSED
      SIRH_OBSERVED="not assessed - ls of /etc/ssh/sshd_config did not confirm absence (ls_rc=$SIRH_LS_RC)"  # network-lint: allow -- observed-state report string; 'ssh' is a component of a local /etc/ssh pathname; no network call
      SIRH_MEANING="PTxray did not obtain trustworthy IgnoreRhosts evidence from /etc/ssh/sshd_config."  # network-lint: allow -- finding meaning text; 'ssh' is a component of a local /etc/ssh pathname; no network call
      SIRH_FIX="verify that 'ls -ld /etc/ssh/sshd_config' and 'grep -i IgnoreRhosts /etc/ssh/sshd_config | grep -v '^#'' run successfully, then rerun PTxray."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      ;;
    probe_failed) SIRH_STATUS=NOT_ASSESSED
      SIRH_OBSERVED="not assessed - IgnoreRhosts probe failed (ls_rc=$SIRH_LS_RC ignorerhosts_rc=$SIRH_IRH_RC)"
      SIRH_MEANING="PTxray did not obtain trustworthy IgnoreRhosts evidence from /etc/ssh/sshd_config."  # network-lint: allow -- finding meaning text; 'ssh' is a component of a local /etc/ssh pathname; no network call
      SIRH_FIX="verify that 'ls -ld /etc/ssh/sshd_config' and 'grep -i IgnoreRhosts /etc/ssh/sshd_config | grep -v '^#'' run successfully, then rerun PTxray."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      ;;
    probe_empty) SIRH_STATUS=NOT_ASSESSED
      SIRH_OBSERVED="not assessed - IgnoreRhosts probe returned no output"
      SIRH_MEANING="PTxray did not obtain trustworthy IgnoreRhosts evidence from /etc/ssh/sshd_config."  # network-lint: allow -- finding meaning text; 'ssh' is a component of a local /etc/ssh pathname; no network call
      SIRH_FIX="verify that 'grep -i IgnoreRhosts /etc/ssh/sshd_config | grep -v '^#'' returns complete evidence, then rerun PTxray."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      ;;
    unparseable) SIRH_STATUS=NOT_ASSESSED
      SIRH_OBSERVED="not assessed - IgnoreRhosts evidence was unparseable"
      SIRH_MEANING="PTxray did not obtain trustworthy IgnoreRhosts evidence from /etc/ssh/sshd_config."  # network-lint: allow -- finding meaning text; 'ssh' is a component of a local /etc/ssh pathname; no network call
      SIRH_FIX="verify uncommented IgnoreRhosts lines in /etc/ssh/sshd_config, then rerun PTxray."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      ;;
    *)
      SIRH_STATUS=NOT_ASSESSED
      SIRH_OBSERVED="not assessed - IgnoreRhosts evidence was unparseable"
      SIRH_MEANING="PTxray did not obtain trustworthy IgnoreRhosts evidence from /etc/ssh/sshd_config."  # network-lint: allow -- finding meaning text; 'ssh' is a component of a local /etc/ssh pathname; no network call
      SIRH_FIX="verify uncommented IgnoreRhosts lines in /etc/ssh/sshd_config, then rerun PTxray."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      ;;
  ...
  case "$SIRH_STATUS" in
    NOT_APPLICABLE) SIRH_SEV=low ;;
    *) SIRH_SEV=med ;;
  esac

  add security stig_ssh_daemon_configured_disable_ssh_daemon \
    "SSH daemon IgnoreRhosts" \
    "$SIRH_STATUS" "$SIRH_SEV" "$SIRH_OBSERVED" \
    "$SIRH_MEANING" "$SIRH_FIX" "stig:V-215303"
}
