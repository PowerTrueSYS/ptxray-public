...
AIXRAY_TOOL=ck-stig-ssh-daemon-configured-disable


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_ssh_daemon_configured_disable — DISA STIG V-215302.
  # Uncommented PermitEmptyPasswords no is PASS. yes, or no such lines, is FAIL.
  # sshd_config determinate absence is PASS (prohibition / svcoff).
  # Generic probe rc != 0 with empty output is NOT_ASSESSED. A missing
  # directive FAIL requires a successful cat of sshd_config (readable
  # search) plus grep rc=1 empty. rc 0 empty is a swallowed error, not
  # absence. File presence alone plus grep rc=1 is not determinate.
  typeset SSDD_CAT SSDD_CAT_RC SSDD_CAT_KIND
  typeset SSDD_RAW SSDD_RC SSDD_KIND SSDD_PARSE_RC
  typeset SSDD_STATUS SSDD_SEV SSDD_OBSERVED SSDD_MEANING SSDD_FIX

  SSDD_CAT=$(aix sshd_config_cat cat /etc/ssh/sshd_config)  # network-lint: allow -- aix() capture of a local read-only command; 'ssh' is a component of a local /etc/ssh pathname; no network call
  SSDD_CAT_RC=$?

  # Pipeline runs inside aix via sh -c; a quoted pipeline is not an executable.
  SSDD_RAW=$(aix sshd_permit_empty_passwords sh -c "grep -i PermitEmptyPasswords /etc/ssh/sshd_config | grep -v '^#'")  # network-lint: allow -- aix() capture of a local read-only command; 'ssh' is a component of a local /etc/ssh pathname; no network call
  SSDD_RC=$?
  SSDD_KIND=""
  SSDD_CAT_KIND=none

  if [ -n "$SSDD_CAT" ]; then
    SSDD_CAT_KIND=$(printf '%s\n' "$SSDD_CAT" | awk '
      /No such file or directory/ || /does not exist/ || /cannot open/ { a++ }
      END { if (a) print "absent"; else print "other" }
    ')
    if [ "$SSDD_CAT_KIND" = other ] && [ "$SSDD_CAT_RC" -eq 0 ]; then
      SSDD_CAT_KIND=body
    elif [ "$SSDD_CAT_KIND" != absent ]; then
      SSDD_CAT_KIND=none
    fi
  fi

  if [ "$SSDD_CAT_KIND" = absent ]; then
    SSDD_KIND=missing_file
  elif [ -n "$SSDD_RAW" ]; then
    SSDD_KIND=$(printf '%s\n' "$SSDD_RAW" | awk '
      /^[ \t]*#/ { next }
      /^[ \t]*$/ { next }
      ...
      END {
        ...
        if (absent && seen) { print "unparseable"; exit }
        if (absent) { print "missing_file"; exit }
        if (seen == 0) { print "unparseable"; exit }
    ...
    ')
    SSDD_PARSE_RC=$?
    if [ "$SSDD_PARSE_RC" -ne 0 ] || [ -z "$SSDD_KIND" ]; then
      SSDD_KIND=unparseable
    fi
  elif [ "$SSDD_RC" -eq 0 ]; then
    SSDD_KIND=probe_empty
  elif [ "$SSDD_CAT_KIND" = body ] && [ "$SSDD_RC" -eq 1 ]; then
    SSDD_KIND=$(printf '%s\n' "$SSDD_CAT" | awk '
      /^[ \t]*#/ { next }
      /^[ \t]*$/ { next }
      {
        ...
        if (n < 1) next
        key = tolower(f[1])
        if (key != "permitemptypasswords") next
        seen++
        if (n != 2) {
          bad++
          next
        ...
        val = tolower(f[2])
        if (val != "yes" && val != "no") bad++
      }
      END {
        if (bad) { print "unparseable"; exit }
        if (seen == 0) { print "missing_directive"; exit }
        print "unparseable"
      }
    ')
    SSDD_PARSE_RC=$?
  ...
  elif [ "$SSDD_RC" -ne 0 ]; then
    SSDD_KIND=probe_failed
  else
    SSDD_KIND=unparseable
  fi
  ...
  case "$SSDD_KIND" in
    no) SSDD_STATUS=PASS
      SSDD_OBSERVED="PermitEmptyPasswords no"
      SSDD_MEANING="sshd rejects password-less account logins; an account with no password cannot authenticate over SSH."
      SSDD_FIX="n/a"
      ;;
    yes) SSDD_STATUS=FAIL
      SSDD_OBSERVED="PermitEmptyPasswords yes"
      SSDD_MEANING="sshd permits accounts with empty passwords to log in, so any account left without a password is reachable without credentials."
      SSDD_FIX="set 'PermitEmptyPasswords no' in /etc/ssh/sshd_config, then stopsrc -s sshd; startsrc -s sshd; PTxray only recommends these actions."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      ;;
    missing_directive) SSDD_STATUS=FAIL
      SSDD_OBSERVED="PermitEmptyPasswords is missing"
      SSDD_MEANING="sshd_config has no uncommented PermitEmptyPasswords no directive, so empty-password SSH logins are not proven disabled."
      SSDD_FIX="set 'PermitEmptyPasswords no' in /etc/ssh/sshd_config, then stopsrc -s sshd; startsrc -s sshd; PTxray only recommends these actions."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      ;;
    missing_file) SSDD_STATUS=PASS
      SSDD_OBSERVED="sshd_config is absent; empty-password prohibition holds"
      SSDD_MEANING="The SSH daemon configuration is absent, so empty-password SSH logins cannot be offered."
      SSDD_FIX="n/a"
      ;;
    probe_failed) SSDD_STATUS=NOT_ASSESSED
      SSDD_OBSERVED="not assessed - PermitEmptyPasswords probe failed (rc=$SSDD_RC)"
      SSDD_MEANING="PTxray did not obtain trustworthy PermitEmptyPasswords evidence from sshd_config."
      SSDD_FIX="run \"grep -i PermitEmptyPasswords /etc/ssh/sshd_config | grep -v '^#'\" and rerun PTxray."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      ;;
    probe_empty) SSDD_STATUS=NOT_ASSESSED
      SSDD_OBSERVED="not assessed - PermitEmptyPasswords probe returned empty output with rc=0"
      SSDD_MEANING="PTxray did not obtain trustworthy PermitEmptyPasswords evidence from sshd_config."
      SSDD_FIX="run \"grep -i PermitEmptyPasswords /etc/ssh/sshd_config | grep -v '^#'\" and rerun PTxray."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      ;;
    *) SSDD_STATUS=NOT_ASSESSED
      SSDD_OBSERVED="not assessed - PermitEmptyPasswords evidence was unparseable"
      SSDD_MEANING="PTxray did not obtain trustworthy PermitEmptyPasswords evidence from sshd_config."
      SSDD_FIX="run \"grep -i PermitEmptyPasswords /etc/ssh/sshd_config | grep -v '^#'\" and rerun PTxray."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      ;;
  esac
  ...
  case "$SSDD_STATUS" in
    NOT_APPLICABLE) SSDD_SEV=low ;;
    *) SSDD_SEV=med ;;
  esac

  add security stig_ssh_daemon_configured_disable "SSH daemon empty passwords disabled" \
    "$SSDD_STATUS" "$SSDD_SEV" "$SSDD_OBSERVED" \
    "$SSDD_MEANING" "$SSDD_FIX" "stig:V-215302"
}
