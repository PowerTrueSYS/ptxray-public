...
AIXRAY_TOOL=ck-stig-ssh-daemon-configured-only


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_ssh_daemon_configured_only — DISA STIG V-215294.
  # Required configuration: sshd MACs must be FIPS 140-2 approved (includes hmac-sha1).
  # Uncertain probe evidence is refused; a missing MACs setting is a finding.
  typeset SSHD_MACS_STAGE1 SSHD_MACS_RAW SSHD_MACS_RC SSHD_MACS_PARSED SSHD_MACS_PARSE_RC
  typeset SSHD_MACS_REASON SSHD_MACS_STATUS SSHD_MACS_SEV
  typeset SSHD_MACS_OBSERVED SSHD_MACS_MEANING SSHD_MACS_FIX
  typeset SSHD_MACS_VALUE SSHD_MACS_BAD SSHD_MACS_REST

  # STIG check-content: grep -i macs /etc/ssh/sshd_config | grep -v '^#'  # network-lint: allow -- prose comment; 'ssh' is a component of a local /etc/ssh pathname; no network call
  # Capture the file-read stage alone so POSIX grep rc 1 (no match) is not
  # confused with rc >= 2 (unreadable / execution error) or a later grep -v miss.
  SSHD_MACS_STAGE1=$(aix sshd_config_macs grep -i macs /etc/ssh/sshd_config)  # network-lint: allow -- aix() capture of a local read-only command; 'ssh' is a component of a local /etc/ssh pathname; no network call
  SSHD_MACS_RC=$?
  SSHD_MACS_RAW=""
  SSHD_MACS_VALUE=""
  SSHD_MACS_BAD=""
  ...
  if [ "$SSHD_MACS_RC" -ge 2 ]; then
    SSHD_MACS_REASON=probe_failed
  elif [ "$SSHD_MACS_RC" -eq 1 ]; then
    if [ -z "$SSHD_MACS_STAGE1" ]; then
      SSHD_MACS_REASON=absent
    else
      SSHD_MACS_REASON=unparseable
    fi
  elif [ -z "$SSHD_MACS_STAGE1" ]; then
    SSHD_MACS_REASON=probe_empty
  else
    SSHD_MACS_RAW=$(printf '%s\n' "$SSHD_MACS_STAGE1" | grep -v '^#')
    if [ -z "$SSHD_MACS_RAW" ]; then
      SSHD_MACS_REASON=absent
    else
      SSHD_MACS_PARSED=$(printf '%s\n' "$SSHD_MACS_RAW" | awk '
        BEGIN {
          ok["hmac-sha1"]=1
          ok["hmac-sha2-256"]=1
          ok["hmac-sha2-512"]=1
          ok["hmac-sha1-etm@openssh.com"]=1
          ok["hmac-sha2-256-etm@openssh.com"]=1
          ok["hmac-sha2-512-etm@openssh.com"]=1
        }
        {
          raw=$0
          gsub(/^[ \t]+/, "", raw)
          ...
          n=split(raw, f, /[ \t]+/)
          if (n < 1 || tolower(f[1]) != "macs") {
            extra=1
            next
          }
          found=1
          value=raw
          sub(/^[ \t]*[Mm][Aa][Cc][Ss][ \t]*/, "", value)
          gsub(/[ \t]+/, "", value)
        }
        END {
          if (!found) {
            if (extra) print "unparseable"
            else print "absent"
            exit 0
          }
          if (value == "") {
          ...
          for (i=1; i<=n; i++) {
            if (t[i] == "" || !(t[i] in ok)) {
              badtok=t[i]
              if (badtok == "") badtok="(empty)"
              break
            }
          }
          if (badtok == "") print "ok " value; else print "bad " badtok " " value
        }
      ')
      SSHD_MACS_PARSE_RC=$?
      if [ "$SSHD_MACS_PARSE_RC" -ne 0 ] || [ -z "$SSHD_MACS_PARSED" ]; then
        SSHD_MACS_REASON=unparseable
      else
        case "$SSHD_MACS_PARSED" in
          absent)
            SSHD_MACS_REASON=absent
            ;;
          empty_list)
            SSHD_MACS_REASON=empty_list
            ;;
          unparseable)
            SSHD_MACS_REASON=unparseable
            ;;
          ok\ *)
            SSHD_MACS_REASON=compliant
            SSHD_MACS_VALUE=${SSHD_MACS_PARSED#ok }
            ;;
          bad\ *)
            SSHD_MACS_REASON=noncompliant
            SSHD_MACS_REST=${SSHD_MACS_PARSED#bad }
            SSHD_MACS_BAD=${SSHD_MACS_REST%% *}
            SSHD_MACS_VALUE=${SSHD_MACS_REST#* }
            ;;
          *)
            SSHD_MACS_REASON=unparseable
            ;;
  ...
  case "$SSHD_MACS_REASON" in
    compliant)
      SSHD_MACS_STATUS=PASS
      SSHD_MACS_OBSERVED="MACs $SSHD_MACS_VALUE"
      SSHD_MACS_MEANING="The SSH daemon MAC list is limited to FIPS 140-2 approved algorithms."
      SSHD_MACS_FIX="n/a"
      ;;
    noncompliant)
      SSHD_MACS_STATUS=FAIL
      SSHD_MACS_OBSERVED="MACs $SSHD_MACS_VALUE; unapproved $SSHD_MACS_BAD"
      SSHD_MACS_MEANING="The SSH daemon MAC list contains an algorithm that is not FIPS 140-2 approved."
      SSHD_MACS_FIX="stopsrc -s sshd; startsrc -s sshd; PTxray only recommends these commands."
      ;;
    empty_list)
      SSHD_MACS_STATUS=FAIL
      SSHD_MACS_OBSERVED="MACs list is empty"
      SSHD_MACS_MEANING="The SSH daemon MACs setting is present but lists no algorithms."
      SSHD_MACS_FIX="stopsrc -s sshd; startsrc -s sshd; PTxray only recommends these commands."
      ;;
    absent) SSHD_MACS_STATUS=FAIL
      SSHD_MACS_OBSERVED="required sshd_config MACs subject is absent"
      SSHD_MACS_MEANING="sshd_config has no uncommented MACs setting, so FIPS-approved MACs are not configured."
      SSHD_MACS_FIX="stopsrc -s sshd; startsrc -s sshd; PTxray only recommends these commands."
      ;;
    probe_failed) SSHD_MACS_STATUS=NOT_ASSESSED
      SSHD_MACS_OBSERVED="not assessed - sshd_config MACs probe failed (rc=$SSHD_MACS_RC)"
      SSHD_MACS_MEANING="PTxray did not obtain trustworthy sshd_config MACs evidence."
      SSHD_MACS_FIX="rerun PTxray after the sshd_config MACs probe succeeds."
      ;;
    probe_empty) SSHD_MACS_STATUS=NOT_ASSESSED
      SSHD_MACS_OBSERVED="not assessed - sshd_config MACs probe returned no output"
      SSHD_MACS_MEANING="PTxray did not obtain trustworthy sshd_config MACs evidence."
      SSHD_MACS_FIX="rerun PTxray after the sshd_config MACs probe returns complete evidence."
      ;;
    *)
      SSHD_MACS_STATUS=NOT_ASSESSED
      SSHD_MACS_OBSERVED="not assessed - sshd_config MACs evidence was unparseable"
      SSHD_MACS_MEANING="PTxray did not obtain trustworthy sshd_config MACs evidence."
      SSHD_MACS_FIX="verify the sshd_config MACs line, then rerun PTxray."
      ;;
  esac

  case "$SSHD_MACS_STATUS" in
    NOT_APPLICABLE) SSHD_MACS_SEV=low ;;
    *) SSHD_MACS_SEV=med ;;
  esac

  add security stig_ssh_daemon_configured_only "SSH daemon FIPS MACs" \
      "$SSHD_MACS_STATUS" "$SSHD_MACS_SEV" "$SSHD_MACS_OBSERVED" \
      "$SSHD_MACS_MEANING" "$SSHD_MACS_FIX" "stig:V-215294"
}
