...
AIXRAY_TOOL=ck-stig-ssh-daemon-perform-strict


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_ssh_daemon_perform_strict — DISA STIG V-215299.
  # Required configuration: sshd StrictModes must be yes.
  typeset SSM_RAW SSM_RC SSM_PARSED SSM_PARSE_RC SSM_CLASS
  typeset SSM_STATUS SSM_SEV SSM_OBSERVED SSM_MEANING SSM_FIX

  # Pipeline runs inside aix via sh -c; a quoted pipeline is not an executable.
  # test -r establishes sshd_config is readable; grep no-match then emits
  # STRICTMODES_ABSENT so a missing directive is not a failed probe.
  SSM_RAW=$(aix sshd_strictmodes sh -c "test -r /etc/ssh/sshd_config || exit 1; grep -i StrictModes /etc/ssh/sshd_config | grep -v '^#' || echo STRICTMODES_ABSENT") # network-lint: allow -- reads LOCAL sshd_config; grep makes no network call
  SSM_RC=$?
  SSM_CLASS=""

  if [ "$SSM_RC" -ne 0 ]; then
    SSM_CLASS=probe_failed
  elif [ -z "$SSM_RAW" ]; then
    SSM_CLASS=probe_empty
  elif [ "$SSM_RAW" = "STRICTMODES_ABSENT" ]; then
    SSM_CLASS=missing
  else
    SSM_PARSED=$(printf '%s\n' "$SSM_RAW" | awk '
      {
      ...
      END {
        ...
        else if (invalid) print "unparseable"
        else print value
      }
    ')
    SSM_PARSE_RC=$?
    if [ "$SSM_PARSE_RC" -ne 0 ] || [ -z "$SSM_PARSED" ]; then
      SSM_CLASS=unparseable
    else
      SSM_CLASS=$SSM_PARSED
    fi
  fi
  ...
  case "$SSM_CLASS" in
    yes)
      SSM_STATUS=PASS
      SSM_OBSERVED="StrictModes yes"
      SSM_MEANING="sshd checks ownership and modes of home-directory configuration files before accepting login."
      SSM_FIX="n/a"
      ;;
    no) SSM_STATUS=FAIL
      SSM_OBSERVED="StrictModes no"
      SSM_MEANING="sshd StrictModes is disabled, so unauthorized users can plant or alter home-directory configuration files to hijack sessions."
      SSM_FIX="set 'StrictModes yes' in /etc/ssh/sshd_config, then 'stopsrc -s sshd' and 'startsrc -s sshd'; PTxray only recommends these actions."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      ;;
    missing) SSM_STATUS=FAIL
      SSM_OBSERVED="StrictModes is not set"
      SSM_MEANING="sshd StrictModes is not set, so home-directory configuration files are not required to be checked before accepting login."
      SSM_FIX="set 'StrictModes yes' in /etc/ssh/sshd_config, then 'stopsrc -s sshd' and 'startsrc -s sshd'; PTxray only recommends these actions."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      ;;
    probe_failed) SSM_STATUS=NOT_ASSESSED
      SSM_OBSERVED="not assessed - sshd StrictModes probe failed (rc=$SSM_RC)"
      SSM_MEANING="PTxray did not obtain trustworthy StrictModes evidence from sshd_config."
      SSM_FIX="restore read access to /etc/ssh/sshd_config, rerun the StrictModes grep, then rerun PTxray."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      ;;
    probe_empty) SSM_STATUS=NOT_ASSESSED
      SSM_OBSERVED="not assessed - sshd StrictModes probe returned no output"
      SSM_MEANING="PTxray did not obtain trustworthy StrictModes evidence from sshd_config."
      SSM_FIX="restore a complete sshd_config read, rerun the StrictModes grep, then rerun PTxray."
      ;;
    *)
      SSM_STATUS=NOT_ASSESSED
      SSM_OBSERVED="not assessed - sshd StrictModes value is unparseable"
      SSM_MEANING="PTxray did not obtain trustworthy StrictModes evidence from sshd_config."
      SSM_FIX="verify the StrictModes directive in /etc/ssh/sshd_config, then rerun PTxray."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      ;;
  esac

  case "$SSM_STATUS" in
    NOT_APPLICABLE) SSM_SEV=low ;;
    *) SSM_SEV=med ;;
  esac

  add security stig_ssh_daemon_perform_strict "SSH daemon StrictModes" \
    "$SSM_STATUS" "$SSM_SEV" "$SSM_OBSERVED" \
    "$SSM_MEANING" "$SSM_FIX" "stig:V-215299"
}
