...
AIXRAY_TOOL=ck-stig-ssh-dod-banner


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_ssh_dod_banner — DISA STIG V-215202.
  # The configuration must provide the complete DoD notice through sshd's
  # effective Banner setting.  Missing evidence is refused, except that a
  # definitely absent required banner file is itself a finding.
  typeset SSH_DOD_EXISTS SSH_DOD_EXISTS_RC SSH_DOD_MOTD SSH_DOD_MOTD_RC
  typeset SSH_DOD_BANNER SSH_DOD_BANNER_RC SSH_DOD_TEXT SSH_DOD_SHAPE
  typeset SSH_DOD_STATUS SSH_DOD_OBSERVED SSH_DOD_MEANING SSH_DOD_FIX
  typeset SSH_DOD_REQUIRED

  SSH_DOD_REQUIRED='You are accessing a U.S. Government (USG) Information System (IS) that is provided for USG-authorized use only. By using this IS (which includes any device attached to this IS), you consent to the following conditions: -The USG routinely intercepts and monitors communications on this IS for purposes including, but not limited to, penetration testing, COMSEC monitoring, network operations and defense, personnel misconduct (PM), law enforcement (LE), and counterintelligence (CI) investigations. -At any time, the USG may inspect and seize data stored on this IS. -Communications using, or data stored on, this IS are not private, are subject to routine monitoring, interception, and search, and may be disclosed or used for any USG-authorized purpose. -This IS includes security measures (e.g., authentication and access controls) to protect USG interests--not for your personal benefit or privacy. -Notwithstanding the above, using this IS does not constitute consent to PM, LE or CI investigative searching or monitoring of the content of privileged communications, or work product, related to personal representation or services by attorneys, psychotherapists, or clergy, and their assistants. Such communications and work product are private and confidential. See User Agreement for details.'

  SSH_DOD_EXISTS=$(aix motd_ssh_exists ls /etc/motd.ssh)  # network-lint: allow -- aix() capture of a local read-only command; 'ssh' is a component of a local /etc/ssh pathname; no network call
  SSH_DOD_EXISTS_RC=$?
  if [ "$SSH_DOD_EXISTS_RC" -eq 2 ] && [ -z "$SSH_DOD_EXISTS" ]; then
    SSH_DOD_STATUS=FAIL
    SSH_DOD_OBSERVED="/etc/motd.ssh is absent"  # network-lint: allow -- observed-state report string; 'ssh' is a component of a local /etc/ssh pathname; no network call
  elif [ "$SSH_DOD_EXISTS_RC" -ne 0 ]; then
    SSH_DOD_STATUS=NOT_ASSESSED
    SSH_DOD_OBSERVED="not assessed - /etc/motd.ssh existence probe failed (rc=$SSH_DOD_EXISTS_RC)"  # network-lint: allow -- observed-state report string; 'ssh' is a component of a local /etc/ssh pathname; no network call
  elif [ -z "$SSH_DOD_EXISTS" ]; then
    SSH_DOD_STATUS=NOT_ASSESSED
    SSH_DOD_OBSERVED="not assessed - /etc/motd.ssh existence probe returned no output"  # network-lint: allow -- observed-state report string; 'ssh' is a component of a local /etc/ssh pathname; no network call
  else
    SSH_DOD_MOTD=$(aix motd_ssh cat /etc/motd.ssh)  # network-lint: allow -- aix() capture of a local read-only command; 'ssh' is a component of a local /etc/ssh pathname; no network call
    SSH_DOD_MOTD_RC=$?
    if [ "$SSH_DOD_MOTD_RC" -ne 0 ]; then
      SSH_DOD_STATUS=NOT_ASSESSED
      SSH_DOD_OBSERVED="not assessed - /etc/motd.ssh content probe failed (rc=$SSH_DOD_MOTD_RC)"  # network-lint: allow -- observed-state report string; 'ssh' is a component of a local /etc/ssh pathname; no network call
    elif [ -z "$SSH_DOD_MOTD" ]; then
      SSH_DOD_STATUS=NOT_ASSESSED
      SSH_DOD_OBSERVED="not assessed - /etc/motd.ssh content probe returned no output"  # network-lint: allow -- observed-state report string; 'ssh' is a component of a local /etc/ssh pathname; no network call
    else
      SSH_DOD_TEXT=$(printf '%s\n' "$SSH_DOD_MOTD" | awk -v req="$SSH_DOD_REQUIRED" '
        { text = text " " $0 }
        END {
          gsub(/[[:space:]]+/, " ", text)
          sub(/^ /, "", text)
          ...
          sub(/^ /, "", req)
          sub(/ $/, "", req)
          if (req == "" || index(text, req) == 0) print "missing"
          else print "ok"
        }
      ')
      if [ "$SSH_DOD_TEXT" != "ok" ]; then
        SSH_DOD_STATUS=FAIL
        SSH_DOD_OBSERVED="/etc/motd.ssh does not contain the required DoD notice"  # network-lint: allow -- observed-state report string; 'ssh' is a component of a local /etc/ssh pathname; no network call
      else
        SSH_DOD_BANNER=$(aix sshd_banner sshd -T)
        SSH_DOD_BANNER_RC=$?
        if [ "$SSH_DOD_BANNER_RC" -ne 0 ]; then
          SSH_DOD_STATUS=NOT_ASSESSED
          SSH_DOD_OBSERVED="not assessed - sshd effective Banner probe failed (rc=$SSH_DOD_BANNER_RC)"
        elif [ -z "$SSH_DOD_BANNER" ]; then
          SSH_DOD_STATUS=NOT_ASSESSED
          SSH_DOD_OBSERVED="not assessed - sshd effective Banner probe returned no output"
        else
          # network-lint: allow-next=8 -- awk program classifying the captured banner text; 'ssh' occurs only in the /etc/motd.ssh pathname; a trailing comment would corrupt the awk source
          SSH_DOD_SHAPE=$(printf '%s\n' "$SSH_DOD_BANNER" | awk '
            /^[[:space:]]*$/ { next }
            tolower($1) != "banner" { next }
            NF != 2 { bad = 1; next }
            { count++; path = $2 }
            END {
              if (bad || count != 1) print "malformed"
              else if (path == "/etc/motd.ssh") print "ok"
              else print "wrong"
            }
          ')
          case "$SSH_DOD_SHAPE" in
            ok)
              SSH_DOD_STATUS=PASS
              SSH_DOD_OBSERVED="/etc/motd.ssh contains the required DoD notice and sshd Banner=/etc/motd.ssh"  # network-lint: allow -- observed-state report string; 'ssh' is a component of a local /etc/ssh pathname; no network call
              ;;
            wrong)
              SSH_DOD_STATUS=FAIL
              SSH_DOD_OBSERVED="sshd effective Banner does not point to /etc/motd.ssh"  # network-lint: allow -- observed-state report string; 'ssh' is a component of a local /etc/ssh pathname; no network call
              ;;
            *)
              SSH_DOD_STATUS=NOT_ASSESSED
              SSH_DOD_OBSERVED="not assessed - sshd effective Banner output was unparseable"
              ;;
          esac
        fi
      fi
  ...
  case "$SSH_DOD_STATUS" in
    PASS)
      SSH_DOD_MEANING="SSH, sftp, and scp sessions receive the required DoD notice before access."  # network-lint: allow -- finding meaning text; 'scp' names an OpenSSH subsystem in prose and 'sftp' names an OpenSSH subsystem in prose; no network call
      SSH_DOD_FIX="n/a"
      ;;
    FAIL)
      SSH_DOD_MEANING="SSH users may not receive the required DoD notice and consent banner."
      SSH_DOD_FIX="configure /etc/motd.ssh with the Standard Mandatory DoD Notice and Consent Banner, set sshd Banner to /etc/motd.ssh, then run 'stopsrc -s sshd; startsrc -s sshd'; PTxray only recommends these actions."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      ;;
    *)
      SSH_DOD_MEANING="PTxray did not obtain trustworthy evidence for the SSH DoD banner configuration."
      SSH_DOD_FIX="verify /etc/motd.ssh and the effective sshd Banner setting, then rerun PTxray."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      ;;
  esac

  add security stig_ssh_dod_banner "DoD SSH login banner" "$SSH_DOD_STATUS" med \
    "$SSH_DOD_OBSERVED" "$SSH_DOD_MEANING" "$SSH_DOD_FIX" "stig:V-215202"
}
