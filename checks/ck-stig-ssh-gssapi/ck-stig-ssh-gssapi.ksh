...
AIXRAY_TOOL=ck-stig-ssh-gssapi


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_ssh_gssapi — DISA STIG V-215292.
  # The site decision about whether GSSAPI is used is not observable from
  # sshd_config. Synthetic overlays may supply PTXRAY_GSSAPI_AUTHENTICATION_USAGE
  # (not_in_use, in_use, unknown). Grep rc 1 is directive-absent FAIL. When a
  # setting is present, without an explicit not_in_use decision it is never
  # PASS or FAIL. The grep pipeline reports rc 1 for both no-match and an
  # unreadable file; sshd_config_readable (test -r) distinguishes those.
  typeset SSG_RAW SSG_RC SSG_VALUE SSG_DECISION SSG_STATUS SSG_SEVERITY
  typeset SSG_OBSERVED SSG_MEANING SSG_FIX SSG_READ_RC

  aix sshd_config_readable test -r /etc/ssh/sshd_config  # network-lint: allow -- aix() capture of a local read-only command; 'ssh' is a component of a local /etc/ssh pathname; no network call
  SSG_READ_RC=$?
  if aix_capture_missing sshd_config_readable; then
    SSG_READ_RC=0
  fi
  SSG_RAW=$(aix sshd_gssapi sh -c "grep -i GSSAPIAuthentication /etc/ssh/sshd_config | grep -v '^#'")  # network-lint: allow -- aix() capture of a local read-only command; 'ssh' is a component of a local /etc/ssh pathname; no network call
  SSG_RC=$?
  SSG_VALUE=""
  SSG_DECISION=""

  if [ "$SSG_READ_RC" -ne 0 ]; then
    SSG_STATUS=NOT_ASSESSED
    SSG_SEVERITY=med
    SSG_OBSERVED="not assessed - /etc/ssh/sshd_config is unreadable (rc=$SSG_READ_RC)"  # network-lint: allow -- observed-state report string; 'ssh' is a component of a local /etc/ssh pathname; no network call
  elif [ "$SSG_RC" -eq 1 ]; then
    # grep not-found: directive absent, STIG requires no
    SSG_STATUS=FAIL
    SSG_SEVERITY=med
    SSG_OBSERVED="GSSAPIAuthentication directive is absent from active sshd configuration"
  elif [ "$SSG_RC" -ne 0 ]; then
    SSG_STATUS=NOT_ASSESSED
    SSG_SEVERITY=med
    SSG_OBSERVED="not assessed - SSH daemon GSSAPIAuthentication probe failed (rc=$SSG_RC)"
  elif [ -z "$SSG_RAW" ]; then
    SSG_STATUS=NOT_ASSESSED
    SSG_SEVERITY=med
    SSG_OBSERVED="not assessed - SSH daemon GSSAPIAuthentication probe returned no evidence"
  else
    SSG_DECISION=$(printf '%s\n' "$SSG_RAW" | awk '
      /^[ \t]*PTXRAY_GSSAPI_AUTHENTICATION_USAGE=/ {
        if (seen_u++) bad = 1
        sub(/^[ \t]*PTXRAY_GSSAPI_AUTHENTICATION_USAGE=/, "")
        if ($0 != "in_use" && $0 != "unknown" && $0 != "not_in_use") bad = 1
        usage = $0
        next
      }
      /^[ \t]*#/ || /^[ \t]*$/ { next }
      {
        if (NF != 2 || tolower($1) != "gssapiauthentication") bad = 1
        else setting = tolower($2)
      }
      END {
        if (bad) exit 1
        if (usage == "") usage = "unknown"
        if (usage == "in_use") print "in_use"
        else if (usage == "unknown") print "unknown"
        else {
          if (setting == "") setting = "unset"
          print "not_in_use:" setting
        }
      }')
    if [ $? -ne 0 ] || [ -z "$SSG_DECISION" ]; then
      SSG_STATUS=NOT_ASSESSED
      SSG_SEVERITY=med
      SSG_OBSERVED="not assessed - SSH daemon GSSAPIAuthentication output was unparseable"
    elif [ "$SSG_DECISION" = in_use ]; then
      SSG_STATUS=NOT_APPLICABLE
      SSG_SEVERITY=low
      SSG_OBSERVED="GSSAPI authentication is used for SSH authentication; this rule is not applicable"
    elif [ "$SSG_DECISION" = unknown ]; then
      SSG_STATUS=NOT_ASSESSED
      SSG_SEVERITY=med
      SSG_OBSERVED="not assessed - SA/ISSO confirmation is required to determine whether GSSAPI authentication is used for SSH authentication"
    else
      SSG_VALUE=${SSG_DECISION#not_in_use:}
      if [ "$SSG_VALUE" = no ]; then
        SSG_STATUS=PASS
        SSG_SEVERITY=med
        SSG_OBSERVED="GSSAPIAuthentication=no"
      else
        SSG_STATUS=FAIL
        SSG_SEVERITY=med
        SSG_OBSERVED="GSSAPIAuthentication=$SSG_VALUE (must be no when GSSAPI is not required)"
      fi
    fi
  fi
  ...
  case "$SSG_STATUS" in
    PASS)
      SSG_MEANING="The SSH daemon disables GSSAPI authentication when it is not required."
      SSG_FIX="n/a"
      ;;
    FAIL)
      SSG_MEANING="The SSH daemon does not disable GSSAPI authentication when it is not required."
      SSG_FIX="set GSSAPIAuthentication to no, then run 'stopsrc -s sshd; startsrc -s sshd'; PTxray only recommends these actions."
      ;;
    NOT_APPLICABLE)
      SSG_MEANING="GSSAPI authentication is used for SSH authentication, so this rule does not apply."
      SSG_FIX="n/a"
      ;;
    *)
      SSG_MEANING="PTxray could not establish both the SSH daemon setting and the site GSSAPI-use decision."
      SSG_FIX="have the SA or ISSO confirm whether GSSAPI authentication is used for SSH authentication, then verify GSSAPIAuthentication in /etc/ssh/sshd_config."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      ;;
  esac

  add security stig_ssh_gssapi "SSH GSSAPI authentication" "$SSG_STATUS" \
    "$SSG_SEVERITY" "$SSG_OBSERVED" "$SSG_MEANING" "$SSG_FIX" "stig:V-215292"
}
