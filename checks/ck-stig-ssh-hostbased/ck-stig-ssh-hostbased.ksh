...
AIXRAY_TOOL=ck-stig-ssh-hostbased


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_ssh_hostbased — DISA STIG V-215304.
  typeset SSHB_RAW SSHB_RC SSHB_ACTIVE SSHB_PARSE_RC
  typeset SSHB_STATUS SSHB_OBSERVED SSHB_MEANING SSHB_FIX

  SSHB_RAW=$(aix sshd_hostbased_auth sh -c "grep -i HostbasedAuthentication /etc/ssh/sshd_config | grep -v '^#'")  # network-lint: allow -- aix() capture of a local read-only command; 'ssh' is a component of a local /etc/ssh pathname; no network call
  SSHB_RC=$?
  SSHB_ACTIVE=""

  if [ "$SSHB_RC" -eq 1 ]; then
    SSHB_STATUS=FAIL # grep not-found
    SSHB_OBSERVED="HostbasedAuthentication directive is absent from active sshd configuration"
  elif [ "$SSHB_RC" -ne 0 ]; then
    SSHB_STATUS=NOT_ASSESSED # probe failure
    SSHB_OBSERVED="not assessed - sshd HostbasedAuthentication probe failed (rc=$SSHB_RC)"
  elif [ -z "$SSHB_RAW" ]; then
    SSHB_STATUS=NOT_ASSESSED # empty successful probe
    SSHB_OBSERVED="not assessed - sshd HostbasedAuthentication probe returned no output with rc=0"
  else
    SSHB_ACTIVE=$(printf '%s\n' "$SSHB_RAW" | awk '
      /^[ \t]*#/ { next }
      /^[ \t]*$/ { next }
      {
        key = tolower($1)
        if (key != "hostbasedauthentication") {
          next
        }
        if (NF != 2) { bad = 1; next }
        value = tolower($2)
    ...
    ')
    SSHB_PARSE_RC=$?
    if [ "$SSHB_PARSE_RC" -eq 1 ]; then
      SSHB_STATUS=FAIL # missing subject
      SSHB_OBSERVED="HostbasedAuthentication directive is absent from active sshd configuration"
    elif [ "$SSHB_PARSE_RC" -ne 0 ]; then
      SSHB_STATUS=NOT_ASSESSED
      SSHB_OBSERVED="not assessed - sshd HostbasedAuthentication output was ambiguous or malformed"
    else
      case "$SSHB_ACTIVE" in
        yes)
          SSHB_STATUS=FAIL
          SSHB_OBSERVED="HostbasedAuthentication yes"
          ;;
        no)
          SSHB_STATUS=PASS
          SSHB_OBSERVED="HostbasedAuthentication no"
          ;;
        *)
          SSHB_STATUS=NOT_ASSESSED
          SSHB_OBSERVED="not assessed - sshd HostbasedAuthentication value was not yes or no"
          ;;
      esac
    fi
  fi
  ...
  case "$SSHB_STATUS" in
    PASS)
      SSHB_MEANING="The SSH daemon is explicitly configured not to use host-based authentication."
      SSHB_FIX="n/a"
      ;;
    FAIL)
      SSHB_MEANING="The SSH daemon can use host-based authentication or lacks the required explicit disabling directive."
      SSHB_FIX="set 'HostbasedAuthentication no' in /etc/ssh/sshd_config, then run 'stopsrc -s sshd; startsrc -s sshd'; PTxray only recommends these commands."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      ;;
    *)
      SSHB_MEANING="PTxray did not obtain trustworthy evidence for the SSH host-based authentication setting."
      SSHB_FIX="verify /etc/ssh/sshd_config is readable and contains one explicit HostbasedAuthentication directive, then rerun PTxray."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      ;;
  esac

  add security stig_ssh_hostbased "SSH host-based authentication disabled" \
    "$SSHB_STATUS" med "$SSHB_OBSERVED" "$SSHB_MEANING" "$SSHB_FIX" "stig:V-215304"
}
