...
AIXRAY_TOOL=ck-stig-ssh-hostkey-mode


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_ssh_hostkey_mode — DISA STIG V-215321.
  # The fixed glob intentionally covers private host keys only; a malformed
  # listing is not evidence that the requirement is satisfied.
  typeset SSH_HK_RAW SSH_HK_RC SSH_HK_STATE SSH_HK_OBSERVED
  typeset SSH_HK_MEANING SSH_HK_FIX

  SSH_HK_RAW=$(aix ls_ssh_hostkey ls -lL /etc/ssh/*key)  # network-lint: allow -- aix() capture of a local read-only command; 'ssh' is a component of a local /etc/ssh pathname; no network call
  SSH_HK_RC=$?
  SSH_HK_STATE=""

  if [ -z "$SSH_HK_RAW" ]; then
    if [ "$SSH_HK_RC" -eq 0 ]; then
      SSH_HK_STATE="unknown-empty"
    else
      SSH_HK_STATE="unknown-probe"
    fi
  else
    # network-lint: allow-next=4 -- awk program parsing ls -lL output; 'ssh' occurs only in the /etc/ssh/*key pathname; a trailing comment would corrupt the awk source
    SSH_HK_STATE=$(printf '%s\n' "$SSH_HK_RAW" | awk '
      /^[ \t]*$/ { next }
      {
        if ($0 ~ /^ls: .*\/etc\/ssh\/\*key .*does not exist\.$/) {
          absent++
          next
        }
        records++
        mode=$1
        if (NF < 9 || length("" mode) != 10 || substr(mode, 1, 1) != "-") {
          malformed=1
          next
        }
        ...
        if (substr(mode, 4, 1) != "-" || substr(mode, 5, 1) != "-" ||
            substr(mode, 6, 1) != "-" || substr(mode, 7, 1) != "-" ||
            substr(mode, 8, 1) != "-" || substr(mode, 9, 1) != "-" ||
            substr(mode, 10, 1) != "-") bad=1
      }
      END {
        if (absent == 1 && records == 0 && !bad && !malformed) print "absent"
        else if (malformed || records == 0 || absent != 0) print "unparseable"
        else if (bad) print "FAIL"
        else print "PASS"
      }
    ')
    if [ "$SSH_HK_RC" -ne 0 ] && [ "$SSH_HK_STATE" != "absent" ]; then
      SSH_HK_STATE="unknown-probe"
    fi
  fi

  case "$SSH_HK_STATE" in
    PASS)
      SSH_HK_OBSERVED="all listed SSH private host keys are mode 0600 or less permissive"
      SSH_HK_MEANING="SSH private host keys are restricted to their owner as required by DISA STIG V-215321."
      SSH_HK_FIX="n/a"
      SSH_HK_STATUS=PASS
      ;;
    FAIL)
      SSH_HK_OBSERVED="one or more SSH private host key files are more permissive than 0600"
      SSH_HK_MEANING="A private SSH host key can be read or used by users other than its owner."
      SSH_HK_FIX="run 'chmod 0600 /etc/ssh/*key' after validating the host; PTxray only recommends this command."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      SSH_HK_STATUS=FAIL
      ;;
    absent)
      SSH_HK_OBSERVED="no SSH private host key files matched /etc/ssh/*key"  # network-lint: allow -- observed-state report string; 'ssh' is a component of a local /etc/ssh pathname; no network call
      SSH_HK_MEANING="SSH host keys are required configuration; no private host key was found."
      SSH_HK_FIX="configure the required SSH host keys, then run 'chmod 0600 /etc/ssh/*key'; PTxray only recommends this command."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      SSH_HK_STATUS=FAIL
      ;;
    unknown-empty)
      SSH_HK_OBSERVED="not assessed - SSH host key listing returned no rows with rc=0"
      SSH_HK_MEANING="PTxray cannot distinguish an empty successful listing from swallowed probe output."
      SSH_HK_FIX="verify 'ls -lL /etc/ssh/*key' output as root, then rerun PTxray."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      SSH_HK_STATUS=NOT_ASSESSED
      ;;
    unknown-probe)
      SSH_HK_OBSERVED="not assessed - SSH host key listing failed (rc=$SSH_HK_RC)"
      SSH_HK_MEANING="PTxray did not obtain trustworthy SSH host key metadata."
      SSH_HK_FIX="verify 'ls -lL /etc/ssh/*key' can be read as root, then rerun PTxray."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      SSH_HK_STATUS=NOT_ASSESSED
      ;;
    *)
      SSH_HK_OBSERVED="not assessed - SSH host key listing was unparseable"
      SSH_HK_MEANING="PTxray did not obtain trustworthy SSH host key metadata."
      SSH_HK_FIX="verify 'ls -lL /etc/ssh/*key' output as root, then rerun PTxray."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      SSH_HK_STATUS=NOT_ASSESSED
      ;;
  ...
  add security stig_ssh_hostkey_mode "SSH private host key file mode" \
    "$SSH_HK_STATUS" med "$SSH_HK_OBSERVED" "$SSH_HK_MEANING" "$SSH_HK_FIX" \
    "stig:V-215321"
}
