...
AIXRAY_TOOL=ck-stig-ssh-kerberos


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_ssh_kerberos — DISA STIG V-215291.
  # grep -i KerberosAuthentication /etc/ssh/sshd_config | grep -v '^#'  # network-lint: allow -- prose comment; 'ssh' is a component of a local /etc/ssh pathname; no network call
  typeset SSK_RAW SSK_RC SSK_STATE SSK_VALUE SSK_OBSERVED

  SSK_RAW=$(aix sshd_kerberos_auth grep -i KerberosAuthentication /etc/ssh/sshd_config)  # network-lint: allow -- aix() capture of a local read-only command; 'ssh' is a component of a local /etc/ssh pathname; no network call
  SSK_RC=$?
  SSK_STATE=""
  SSK_VALUE=""

  if [ "$SSK_RC" -ne 0 ]; then
    SSK_STATE=unavailable
    SSK_OBSERVED="not assessed - sshd KerberosAuthentication probe failed (rc=$SSK_RC)"
  elif [ -z "$SSK_RAW" ]; then
    SSK_STATE=unavailable
    SSK_OBSERVED="not assessed - sshd KerberosAuthentication probe returned no evidence"
  else
    SSK_STATE=$(printf '%s\n' "$SSK_RAW" | awk '
      /^[ \t]*$/ { blank = 1; next }
      /^[ \t]*#/ { comment = 1; next }
      {
        ...
        key = words[1]
        value = words[2]
        if (key ~ /^[Kk][Ee][Rr][Bb][Ee][Rr][Oo][Ss][Aa][Uu][Tt][Hh][Ee][Nn][Tt][Ii][Cc][Aa][Tt][Ii][Oo][Nn]$/ && NF == 2) {
          if (found++) { bad = 1; next }
          print value
          next
        }
      ...
      END {
        ...
        if (!found) {
          if (comment && !blank) print "absent"
          else print "empty"
        }
      }
    ')
    if [ $? -ne 0 ] || [ -z "$SSK_STATE" ]; then
      SSK_STATE=unavailable
      SSK_OBSERVED="not assessed - sshd KerberosAuthentication evidence was malformed"
    elif [ "$SSK_STATE" = absent ]; then
      SSK_OBSERVED="KerberosAuthentication setting absent"
    elif [ "$SSK_STATE" = empty ]; then
      SSK_STATE=unavailable
      SSK_OBSERVED="not assessed - sshd KerberosAuthentication probe contained no usable setting"
    else
      SSK_VALUE=$(printf '%s\n' "$SSK_STATE" | tr '[:upper:]' '[:lower:]')
      case "$SSK_VALUE" in
        yes|no) SSK_STATE=$SSK_VALUE ;;
        *) SSK_STATE=unavailable; SSK_OBSERVED="not assessed - KerberosAuthentication value was not yes or no" ;;
      esac
    fi
  fi
  ...
  case "$SSK_STATE" in
    unavailable)
      add security stig_ssh_kerberos "SSH Kerberos authentication disabled" NOT_ASSESSED med \
        "$SSK_OBSERVED" \
        "PTxray did not obtain trustworthy sshd configuration evidence for KerberosAuthentication." \
        "verify /etc/ssh/sshd_config is readable and rerun PTxray." "stig:V-215291"  # network-lint: allow -- finding text argument to add; 'ssh' is a component of a local /etc/ssh pathname; no network call
      ;;
    absent)
      add security stig_ssh_kerberos "SSH Kerberos authentication disabled" PASS med \
        "$SSK_OBSERVED" \
        "No active KerberosAuthentication setting enables Kerberos authentication in sshd." \
        "n/a" "stig:V-215291"
      ;;
    yes)
      add security stig_ssh_kerberos "SSH Kerberos authentication disabled" FAIL med \
        "KerberosAuthentication yes" \
        "sshd permits Kerberos authentication, which can bypass the required access restrictions." \
        "set KerberosAuthentication no in /etc/ssh/sshd_config, then run 'stopsrc -s sshd; startsrc -s sshd'; PTxray only recommends these actions." "stig:V-215291"  # network-lint: allow -- finding text argument to add; 'ssh' is a component of a local /etc/ssh pathname; no network call
      ;;
    no)
      add security stig_ssh_kerberos "SSH Kerberos authentication disabled" PASS med \
        "KerberosAuthentication disabled (no)" \
        "sshd does not permit Kerberos authentication." \
        "n/a" "stig:V-215291"
      ;;
  esac
}
