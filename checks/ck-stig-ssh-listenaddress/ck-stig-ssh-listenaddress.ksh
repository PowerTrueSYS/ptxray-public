...
AIXRAY_TOOL=ck-stig-ssh-listenaddress


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_ssh_listenaddress — DISA STIG V-215306.
  # PARTIAL-AUTO: ListenAddress presence / all-interfaces bind is decidable.
  # Whether non-loopback addresses are ISSO-permitted needs site policy.
  typeset SSLA_RAW SSLA_RC SSLA_CLASS SSLA_PARSE_RC SSLA_REASON
  typeset SSLA_STATUS SSLA_SEV SSLA_OBSERVED SSLA_MEANING SSLA_FIX

  # Pipeline runs inside aix via sh -c; a quoted pipeline is not an executable.
  # grep no-match on a readable file is rc 1 with empty stdout (absent).
  # Genuine probe errors are rc 2+. rc 0 with empty stdout is a swallowed error.
  SSLA_RAW=$(aix sshd_listenaddress sh -c "grep -i ListenAddress /etc/ssh/sshd_config | grep -v '^#'")  # network-lint: allow -- aix() capture of a local read-only command; 'ssh' is a component of a local /etc/ssh pathname; no network call
  SSLA_RC=$?
  SSLA_CLASS=""

  if [ "$SSLA_RC" -eq 0 ]; then
    if [ -z "$SSLA_RAW" ]; then
      SSLA_CLASS=probe_empty
    else
      SSLA_CLASS=$(printf '%s\n' "$SSLA_RAW" | awk '
        {
          sub(/^[ \t]+/, "")
          if ($0 == "" || substr($0, 1, 1) == "#") next
          if (tolower($1) != "listenaddress") next
          if (NF < 2) { bad = 1; next }
          found = 1
          a = tolower($2)
          if (a == "0.0.0.0" || a == "::" || a == "*" || a == "[::]" ||
        ...
        END {
          ...
          else if (unbound) print "unbound"
          else if (other) print "policy"
          else print "loopback"
        }
      ')
      SSLA_PARSE_RC=$?
      if [ "$SSLA_PARSE_RC" -ne 0 ] || [ -z "$SSLA_CLASS" ]; then
        SSLA_CLASS=unparseable
      fi
    fi
  elif [ "$SSLA_RC" -eq 1 ] && [ -z "$SSLA_RAW" ]; then
    SSLA_CLASS=absent
  else
    SSLA_CLASS=probe_failed
  fi

  case "$SSLA_CLASS" in
    loopback) SSLA_REASON=compliant ;;
    unbound) SSLA_REASON=noncompliant ;;
    absent) SSLA_REASON=subject_absent ;;
    policy) SSLA_REASON=policy ;;
    probe_failed) SSLA_REASON=probe_failed ;;
    probe_empty) SSLA_REASON=probe_empty ;;
    *) SSLA_REASON=unparseable ;;
  esac

  case "$SSLA_REASON" in
    compliant)
      SSLA_STATUS=PASS
      SSLA_OBSERVED="ListenAddress present; bound to loopback only"
      SSLA_MEANING="SSH daemon ListenAddress is set to loopback only, so the daemon is not listening on all interfaces."
      SSLA_FIX="n/a"
      ;;
    noncompliant) SSLA_STATUS=FAIL
      SSLA_OBSERVED="ListenAddress binds all interfaces"
      SSLA_MEANING="SSH daemon ListenAddress binds all interfaces, so the daemon does not restrict listening to approved addresses."
      SSLA_FIX="vi /etc/ssh/sshd_config; stopsrc -s sshd; startsrc -s sshd; PTxray only recommends these commands."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      ;;
    subject_absent) SSLA_STATUS=FAIL
      SSLA_OBSERVED="ListenAddress is not present"
      SSLA_MEANING="ListenAddress is not set, so the SSH daemon uses the unrestricted default listen and the STIG requirement is unmet."
      SSLA_FIX="vi /etc/ssh/sshd_config; stopsrc -s sshd; startsrc -s sshd; PTxray only recommends these commands."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      ;;
    policy) SSLA_STATUS=NOT_ASSESSED
      SSLA_OBSERVED="not assessed - addresses are not known permitted without site policy"
      SSLA_MEANING="ListenAddress is set to non-loopback addresses; whether those addresses are ISSO-permitted cannot be decided without site policy."
      SSLA_FIX="compare ListenAddress values in /etc/ssh/sshd_config to the ISSO-approved list, then rerun PTxray."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      ;;
    probe_failed) SSLA_STATUS=NOT_ASSESSED
      SSLA_OBSERVED="not assessed - ListenAddress probe failed (rc=$SSLA_RC)"
      SSLA_MEANING="PTxray did not obtain trustworthy ListenAddress evidence."
      SSLA_FIX="verify that grep -i ListenAddress /etc/ssh/sshd_config | grep -v '^#' runs, then rerun PTxray."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      ;;
    probe_empty) SSLA_STATUS=NOT_ASSESSED
      SSLA_OBSERVED="not assessed - ListenAddress probe returned no output"
      SSLA_MEANING="PTxray did not obtain trustworthy ListenAddress evidence."
      SSLA_FIX="verify that grep -i ListenAddress /etc/ssh/sshd_config | grep -v '^#' returns complete evidence, then rerun PTxray."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      ;;
    *)
      SSLA_STATUS=NOT_ASSESSED
      SSLA_OBSERVED="not assessed - ListenAddress value was unparseable"
      SSLA_MEANING="PTxray did not obtain trustworthy ListenAddress evidence."
      SSLA_FIX="verify the uncommented ListenAddress directive in /etc/ssh/sshd_config, then rerun PTxray."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      ;;
  esac

  case "$SSLA_STATUS" in
    NOT_APPLICABLE) SSLA_SEV=low ;;
    *) SSLA_SEV=med ;;
  esac

  add security stig_ssh_listenaddress "SSH daemon listen addresses" \
    "$SSLA_STATUS" "$SSLA_SEV" "$SSLA_OBSERVED" \
    "$SSLA_MEANING" "$SSLA_FIX" "stig:V-215306"
}
