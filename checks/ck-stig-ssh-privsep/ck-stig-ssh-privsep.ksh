...
AIXRAY_TOOL=ck-stig-ssh-privsep


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_ssh_privsep — DISA STIG V-215297.
  # Required configuration: uncommented UsePrivilegeSeparation must be yes.
  # A missing directive is a finding. OpenSSH 7.5+ always-on behaviour is
  # declared in README.md; STIG-literal still FAILs when the token is absent.
  #
  # Capture the first grep only. The STIG pipeline
  # grep -i UsePrivilegeSeparation /etc/ssh/sshd_config | grep -v '^#'  # network-lint: allow -- prose comment; 'ssh' is a component of a local /etc/ssh pathname; no network call
  # exits 1 for both no-match and a missing file; grep itself uses rc 1
  # (readable file, no token) vs rc 2 (read/execution error). Comment
  # filtering is done in the parser so those rc values stay distinct.
  typeset SSP_RAW SSP_RC SSP_CLASS SSP_PARSE_RC SSP_REASON
  typeset SSP_STATUS SSP_SEV SSP_OBSERVED SSP_MEANING SSP_FIX

  SSP_RAW=$(aix sshd_use_privsep grep -i UsePrivilegeSeparation /etc/ssh/sshd_config)  # network-lint: allow -- aix() capture of a local read-only command; 'ssh' is a component of a local /etc/ssh pathname; no network call
  SSP_RC=$?
  SSP_CLASS=""
    ...
    SSP_CLASS=$(printf '%s\n' "$SSP_RAW" | awk '
      {
        ...
        if (tolower($1) != "useprivilegeseparation") next
        if (NF < 2) { bad = 1; next }
        found = 1
        last = tolower($2)
      }
      END {
        ...
        else if (last == "yes") print "yes"
        else if (last == "no") print "no"
        else print "unparseable"
      }
    ')
    SSP_PARSE_RC=$?
    if [ "$SSP_PARSE_RC" -ne 0 ] || [ -z "$SSP_CLASS" ]; then
      SSP_CLASS=unparseable
    fi
  elif [ "$SSP_RC" -eq 1 ]; then
    SSP_CLASS=absent
  elif [ "$SSP_RC" -ne 0 ]; then
    SSP_CLASS=probe_failed
  else
    SSP_CLASS=probe_empty
  fi

  case "$SSP_CLASS" in
    yes) SSP_REASON=compliant ;;
    no) SSP_REASON=noncompliant ;;
    absent) SSP_REASON=subject_absent ;;
    probe_failed) SSP_REASON=probe_failed ;;
    probe_empty) SSP_REASON=probe_empty ;;
    *) SSP_REASON=unparseable ;;
  esac

  case "$SSP_REASON" in
    compliant)
      SSP_STATUS=PASS
      SSP_OBSERVED="UsePrivilegeSeparation yes"
      SSP_MEANING="SSH daemon privilege separation is enabled, so the daemon does not remain fully privileged after authentication."
      SSP_FIX="n/a"
      ;;
    noncompliant) SSP_STATUS=FAIL
      SSP_OBSERVED="UsePrivilegeSeparation no"
      SSP_MEANING="SSH daemon privilege separation is disabled, so the daemon can remain fully privileged after authentication."
      SSP_FIX="set UsePrivilegeSeparation yes in /etc/ssh/sshd_config, then stopsrc -s sshd; startsrc -s sshd; PTxray only recommends these commands."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      ;;
    subject_absent) SSP_STATUS=FAIL
      SSP_OBSERVED="UsePrivilegeSeparation is not present"
      SSP_MEANING="UsePrivilegeSeparation is not set, so the STIG requirement that SSH daemon privilege separation be turned on is unmet."
      SSP_FIX="set UsePrivilegeSeparation yes in /etc/ssh/sshd_config, then stopsrc -s sshd; startsrc -s sshd; PTxray only recommends these commands."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      ;;
    probe_failed) SSP_STATUS=NOT_ASSESSED
      SSP_OBSERVED="not assessed - UsePrivilegeSeparation probe failed (rc=$SSP_RC)"
      SSP_MEANING="PTxray did not obtain trustworthy UsePrivilegeSeparation evidence."
      SSP_FIX="verify that grep -i UsePrivilegeSeparation /etc/ssh/sshd_config | grep -v '^#' runs, then rerun PTxray."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      ;;
    probe_empty)
      SSP_STATUS=NOT_ASSESSED
      SSP_OBSERVED="not assessed - UsePrivilegeSeparation probe returned no output"
      SSP_MEANING="PTxray did not obtain trustworthy UsePrivilegeSeparation evidence."
      SSP_FIX="verify that grep -i UsePrivilegeSeparation /etc/ssh/sshd_config | grep -v '^#' returns complete evidence, then rerun PTxray."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      ;;
    *)
      SSP_STATUS=NOT_ASSESSED
      SSP_OBSERVED="not assessed - UsePrivilegeSeparation value was unparseable"
      SSP_MEANING="PTxray did not obtain trustworthy UsePrivilegeSeparation evidence."
      SSP_FIX="verify the uncommented UsePrivilegeSeparation directive in /etc/ssh/sshd_config, then rerun PTxray."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      ;;
  esac

  case "$SSP_STATUS" in
    NOT_APPLICABLE) SSP_SEV=low ;;
    *) SSP_SEV=med ;;
  esac

  add security stig_ssh_privsep "SSH daemon privilege separation" \
    "$SSP_STATUS" "$SSP_SEV" "$SSP_OBSERVED" \
    "$SSP_MEANING" "$SSP_FIX" "stig:V-215297"
}
