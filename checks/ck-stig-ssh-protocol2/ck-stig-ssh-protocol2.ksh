...
AIXRAY_TOOL=ck-stig-ssh-protocol2


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_ssh_protocol2 — DISA STIG V-215289.  OpenSSH 7.4 removed the
  # Protocol directive because it supports only protocol 2; an absent directive
  # is therefore compliant when the installed sshd reports 7.4 or later, or
  # when sshd -V completes at rc 0 with no stdout (determinate absence of
  # Protocol 1; a completed empty probe is not a refusal).
  typeset SSP_EXISTS_RC SSP_PROTOCOL_RAW SSP_PROTOCOL_RC SSP_VERSION_RAW
  typeset SSP_VERSION_RC SSP_PROTOCOL_SHAPE SSP_VERSION_SHAPE SSP_STATUS
  typeset SSP_OBSERVED SSP_MEANING SSP_FIX

  SSP_EXISTS_RC=0
  aix sshd_config_exists test -f /etc/ssh/sshd_config  # network-lint: allow -- aix() capture of a local read-only command; 'ssh' is a component of a local /etc/ssh pathname; no network call
  SSP_EXISTS_RC=$?
  SSP_PROTOCOL_RAW=""
  SSP_PROTOCOL_RC=0
  SSP_VERSION_RAW=""
  ...
  if [ "$SSP_EXISTS_RC" -eq 1 ]; then
    SSP_STATUS=FAIL
    SSP_OBSERVED="/etc/ssh/sshd_config is absent; the required SSH Protocol 2 configuration is unmet"  # network-lint: allow -- observed-state report string; 'ssh' is a component of a local /etc/ssh pathname; no network call
  elif [ "$SSP_EXISTS_RC" -ne 0 ]; then
    SSP_STATUS=NOT_ASSESSED
    SSP_OBSERVED="not assessed - unable to determine whether /etc/ssh/sshd_config exists (rc=$SSP_EXISTS_RC)"  # network-lint: allow -- observed-state report string; 'ssh' is a component of a local /etc/ssh pathname; no network call
  else
    SSP_PROTOCOL_RAW=$(aix sshd_protocol grep -i '^[[:space:]]*Protocol' /etc/ssh/sshd_config)  # network-lint: allow -- aix() capture of a local read-only command; 'ssh' is a component of a local /etc/ssh pathname; no network call
    SSP_PROTOCOL_RC=$?
    if [ "$SSP_PROTOCOL_RC" -eq 0 ] && [ -z "$SSP_PROTOCOL_RAW" ]; then
      SSP_STATUS=NOT_ASSESSED
      SSP_OBSERVED="not assessed - Protocol probe returned rc=0 with no output"
    elif [ "$SSP_PROTOCOL_RC" -eq 0 ]; then
      SSP_PROTOCOL_SHAPE=$(printf '%s\n' "$SSP_PROTOCOL_RAW" | awk '
        /^[ \t]*$/ { next }
        {
          line = $0
          sub(/^[ \t]+/, "", line)
          sub(/[ \t]+$/, "", line)
          n = split(line, f, /[ \t]+/)
          if (n == 2 && toupper(f[1]) == "PROTOCOL" && f[2] == "2") {
            seen++
            next
          }
          bad = 1
        }
        END { if (seen > 0 && !bad) print "protocol2"; else print "other" }
      ')
      if [ "$SSP_PROTOCOL_SHAPE" = protocol2 ]; then
        SSP_STATUS=PASS
        SSP_OBSERVED="Protocol 2 only is configured in /etc/ssh/sshd_config"  # network-lint: allow -- observed-state report string; 'ssh' is a component of a local /etc/ssh pathname; no network call
      else
        SSP_STATUS=FAIL
        SSP_OBSERVED="Protocol directive does not support SSH Protocol 2 only"
      fi
    elif [ "$SSP_PROTOCOL_RC" -eq 1 ] && [ -z "$SSP_PROTOCOL_RAW" ]; then
      SSP_VERSION_RAW=$(aixv sshd_version sshd -V)
      SSP_VERSION_RC=$?
      if [ "$SSP_VERSION_RC" -ne 0 ]; then
        SSP_STATUS=NOT_ASSESSED
        SSP_OBSERVED="not assessed - Protocol directive is absent and sshd -V failed (rc=$SSP_VERSION_RC)"
      elif [ -z "$SSP_VERSION_RAW" ]; then
        SSP_STATUS=PASS
        SSP_OBSERVED="Protocol directive absent; sshd -V completed with no stdout"
      else
        SSP_VERSION_SHAPE=$(printf '%s\n' "$SSP_VERSION_RAW" | awk '
          match($0, /OpenSSH_[0-9][0-9]*\.[0-9][0-9]*/) {
            v = substr($0, RSTART + 8, RLENGTH - 8)
            split(v, p, ".")
            if (p[1] > 7 || (p[1] == 7 && p[2] >= 4)) good = 1
            else old = 1
            found = 1
          }
          END {
            if (!found) print "unparseable"
            else if (good && !old) print "modern"
            else print "old-or-mixed"
          }
        ')
        case "$SSP_VERSION_SHAPE" in
          modern)
            SSP_STATUS=PASS
            SSP_OBSERVED="Protocol directive absent; sshd reports OpenSSH 7.4 or later, which supports Protocol 2 only"
            ;;
          old-or-mixed)
            SSP_STATUS=FAIL
            SSP_OBSERVED="Protocol directive absent and sshd does not prove OpenSSH 7.4 or later"
            ;;
          *)
            SSP_STATUS=NOT_ASSESSED
            SSP_OBSERVED="not assessed - Protocol directive is absent and sshd -V output was unparseable"
            ;;
        esac
      fi
    else
      SSP_STATUS=NOT_ASSESSED
      SSP_OBSERVED="not assessed - Protocol probe failed (rc=$SSP_PROTOCOL_RC)"
    fi
  fi

  case "$SSP_STATUS" in
    PASS)
      SSP_MEANING="The SSH server is configured for SSH Protocol 2 only."
      SSP_FIX="n/a"
      ;;
    FAIL)
      SSP_MEANING="The SSH server does not have trustworthy Protocol 2-only configuration."
      SSP_FIX="configure SSH Protocol 2 only, then restart the service with 'stopsrc -s sshd; startsrc -s sshd'; PTxray only recommends these actions."
      ;;
    *)
      SSP_MEANING="PTxray did not obtain trustworthy evidence of the SSH protocol configuration."
      SSP_FIX="verify /etc/ssh/sshd_config and the sshd version, then rerun PTxray."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      ;;
  esac

  add security stig_ssh_protocol2 "SSH server Protocol 2 only" "$SSP_STATUS" med \
    "$SSP_OBSERVED" "$SSP_MEANING" "$SSP_FIX" "stig:V-215289"
}
