...
AIXRAY_TOOL=ck-stig-ssh-revokedkeys


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_ssh_revokedkeys — DISA STIG V-215293 (PARTIAL-AUTO).
  # Decidable: RevokedKeys is set to a non-none path, plus pubkey-use
  # approximation (PubkeyAuthentication yes and authorized_keys present).
  # PubkeyAuthentication no and no authorized_keys is Not Applicable.
  # RevokedKeys missing or none is FAIL even when pubkey-use is only
  # approximated (determinate absence, never a refusal). Inconclusive
  # pubkey-use with RevokedKeys already a path is the site/ISSO remainder
  # (typed NOT_ASSESSED; never a silent PASS).
  # grep: rc 0 + nonempty = parse; rc 1 + empty = directive absent.
  # ls: rc 0 + nonempty = parse; rc 2 + empty = authorized_keys absent.
  # Any other rc/output pair is typed NOT_ASSESSED, never PASS/FAIL.
  typeset SSRK_RK_RAW SSRK_RK_RC SSRK_PA_RAW SSRK_PA_RC SSRK_AK_RAW SSRK_AK_RC
  typeset SSRK_FAILED SSRK_FAIL_RC SSRK_FAIL_WHICH SSRK_FAIL_DETAIL
  typeset SSRK_EMPTY SSRK_EMPTY_WHICH
  typeset SSRK_RK_STATE SSRK_RK_VALUE SSRK_PA_STATE SSRK_PA_VALUE SSRK_AK_STATE
  typeset SSRK_PARSED SSRK_KIND SSRK_STATUS SSRK_SEV SSRK_OBSERVED
  typeset SSRK_MEANING SSRK_FIX SSRK_PA_SHOW
  ...
  SSRK_RK_RAW=$(aix revoked_keys grep "^RevokedKeys" /etc/ssh/sshd_config)  # network-lint: allow -- aix() capture of a local read-only command; 'ssh' is a component of a local /etc/ssh pathname; no network call
  SSRK_RK_RC=$?
  SSRK_PA_RAW=$(aix pubkey_authentication grep "^PubkeyAuthentication" /etc/ssh/sshd_config)  # network-lint: allow -- aix() capture of a local read-only command; 'ssh' is a component of a local /etc/ssh pathname; no network call
  SSRK_PA_RC=$?
  SSRK_AK_RAW=$(aix authorized_keys ls -ld /root/.ssh/authorized_keys)  # network-lint: allow -- aix() capture of a local read-only command; 'ssh' is a component of a local /etc/ssh pathname; no network call
  SSRK_AK_RC=$?

  if [ "$SSRK_RK_RC" -eq 0 ]; then
    if [ -z "$SSRK_RK_RAW" ] && [ "$SSRK_EMPTY" -eq 0 ]; then
      SSRK_EMPTY=1
      SSRK_EMPTY_WHICH=RevokedKeys
    fi
  elif [ "$SSRK_RK_RC" -eq 1 ]; then
    if [ -n "$SSRK_RK_RAW" ]; then
      if [ "$SSRK_FAILED" -eq 0 ]; then
        SSRK_FAILED=1
        SSRK_FAIL_RC=$SSRK_RK_RC
        SSRK_FAIL_WHICH=RevokedKeys
        SSRK_FAIL_DETAIL="rc $SSRK_RK_RC with nonempty output"
      fi
    else
      SSRK_RK_STATE=missing
    fi
  elif [ "$SSRK_FAILED" -eq 0 ]; then
    SSRK_FAILED=1
    SSRK_FAIL_RC=$SSRK_RK_RC
    SSRK_FAIL_WHICH=RevokedKeys
    SSRK_FAIL_DETAIL="failed (rc=$SSRK_RK_RC)"
  fi
  if [ "$SSRK_PA_RC" -eq 0 ]; then
    if [ -z "$SSRK_PA_RAW" ] && [ "$SSRK_EMPTY" -eq 0 ]; then
      SSRK_EMPTY=1
      SSRK_EMPTY_WHICH=PubkeyAuthentication
    fi
  elif [ "$SSRK_PA_RC" -eq 1 ]; then
    if [ -n "$SSRK_PA_RAW" ]; then
      if [ "$SSRK_FAILED" -eq 0 ]; then
        SSRK_FAILED=1
        SSRK_FAIL_RC=$SSRK_PA_RC
        SSRK_FAIL_WHICH=PubkeyAuthentication
        SSRK_FAIL_DETAIL="rc $SSRK_PA_RC with nonempty output"
      fi
    else
      SSRK_PA_STATE=unset
    fi
    ...
    SSRK_FAIL_RC=$SSRK_PA_RC
    SSRK_FAIL_WHICH=PubkeyAuthentication
    SSRK_FAIL_DETAIL="failed (rc=$SSRK_PA_RC)"
  fi
  if [ "$SSRK_AK_RC" -eq 0 ]; then
    if [ -z "$SSRK_AK_RAW" ] && [ "$SSRK_EMPTY" -eq 0 ]; then
      SSRK_EMPTY=1
      SSRK_EMPTY_WHICH=authorized_keys
    fi
  elif [ "$SSRK_AK_RC" -eq 2 ]; then
    if [ -n "$SSRK_AK_RAW" ]; then
      if [ "$SSRK_FAILED" -eq 0 ]; then
        SSRK_FAILED=1
        SSRK_FAIL_RC=$SSRK_AK_RC
        SSRK_FAIL_WHICH=authorized_keys
        SSRK_FAIL_DETAIL="rc $SSRK_AK_RC with nonempty output"
      fi
    else
      SSRK_AK_STATE=absent
    fi
  elif [ -n "$SSRK_AK_RAW" ]; then
    if [ "$SSRK_FAILED" -eq 0 ]; then
      SSRK_FAILED=1
      SSRK_FAIL_RC=$SSRK_AK_RC
      SSRK_FAIL_WHICH=authorized_keys
    ...
    SSRK_FAIL_RC=$SSRK_AK_RC
    SSRK_FAIL_WHICH=authorized_keys
    SSRK_FAIL_DETAIL="failed (rc=$SSRK_AK_RC)"
  fi

  if [ "$SSRK_FAILED" -eq 0 ] && [ "$SSRK_EMPTY" -eq 0 ]; then
    if [ -z "$SSRK_RK_STATE" ]; then
      SSRK_PARSED=$(printf '%s\n' "$SSRK_RK_RAW" | awk '
        {
          gsub(/\r/, "")
          if ($0 ~ /^[ \t]*$/) next
          n++
          if ($1 != "RevokedKeys") { bad=1; next }
          if (NF != 2) { bad=1; next }
          val=$2
        }
        END {
          if (n==0) print "missing|"
          else if (n!=1 || bad || val=="") print "unparseable|"
          else if (val=="none") print "none|"
          else if (val ~ /^[A-Za-z0-9\/._-]+$/) print "path|" val
          else print "unparseable|"
        }')
      SSRK_RK_STATE=${SSRK_PARSED%%\|*}
      SSRK_RK_VALUE=${SSRK_PARSED#*\|}
    fi

    if [ -z "$SSRK_PA_STATE" ]; then
      SSRK_PARSED=$(printf '%s\n' "$SSRK_PA_RAW" | awk '
        {
          gsub(/\r/, "")
          if ($0 ~ /^[ \t]*$/) next
          n++
          if ($1 != "PubkeyAuthentication") { bad=1; next }
          if (NF != 2) { bad=1; next }
          val=$2
        }
        END {
          ...
          else if (val=="yes" || val=="no") print "value|" val
          else print "unparseable|"
        }')
      SSRK_PA_STATE=${SSRK_PARSED%%\|*}
      SSRK_PA_VALUE=${SSRK_PARSED#*\|}
    fi

    if [ -z "$SSRK_AK_STATE" ]; then
      # network-lint: allow-next=8 -- awk program parsing ls -ld output; 'ssh' occurs only in the /root/.ssh/authorized_keys pathname; a trailing comment would corrupt the awk source
      SSRK_PARSED=$(printf '%s\n' "$SSRK_AK_RAW" | awk '
        {
          gsub(/\r/, "")
          if ($0 ~ /^[ \t]*$/) next
          ...
          if (n != 1) { bad=1; next }
          if (NF < 9) { bad=1; next }
          if ($NF != "/root/.ssh/authorized_keys") { bad=1; next }
          mode=$1
          ml=length("" mode)
          if (ml != 10 && ml != 11) { bad=1; next }
          if ($2 !~ /^[0-9][0-9]*$/) { bad=1; next }
        }
        END {
          if (n != 1 || bad) print "unparseable"
          else print "present"
        }')
      SSRK_AK_STATE=$SSRK_PARSED
    fi
  fi

  if [ "$SSRK_FAILED" -eq 1 ]; then
    SSRK_KIND=failed
  elif [ "$SSRK_EMPTY" -eq 1 ]; then
    SSRK_KIND=empty
  elif [ "$SSRK_RK_STATE" = "unparseable" ] || \
       [ "$SSRK_PA_STATE" = "unparseable" ] || \
       [ "$SSRK_AK_STATE" = "unparseable" ]; then
    SSRK_KIND=unparseable
  elif [ "$SSRK_PA_STATE" = "value" ] && [ "$SSRK_PA_VALUE" = "no" ] && \
       [ "$SSRK_AK_STATE" = "absent" ]; then
    SSRK_KIND=absent
  elif [ "$SSRK_PA_STATE" = "value" ] && [ "$SSRK_PA_VALUE" = "yes" ] && \
       [ "$SSRK_AK_STATE" = "present" ]; then
    if [ "$SSRK_RK_STATE" = "none" ] || [ "$SSRK_RK_STATE" = "missing" ]; then
      SSRK_KIND=violation
    elif [ "$SSRK_RK_STATE" = "path" ]; then
      SSRK_KIND=ok
    else
      SSRK_KIND=unparseable
    fi
  elif [ "$SSRK_RK_STATE" = "none" ] || [ "$SSRK_RK_STATE" = "missing" ]; then
    SSRK_KIND=violation_approx
  else
    SSRK_KIND=manual
  fi
  ...
  case "$SSRK_KIND" in
    ok)
      SSRK_STATUS=PASS
      SSRK_OBSERVED="RevokedKeys $SSRK_RK_VALUE; PubkeyAuthentication yes; authorized_keys present"
      SSRK_MEANING="The SSH daemon is configured to refuse revoked public keys."
      SSRK_FIX="n/a"
      ;;
    violation)
      SSRK_STATUS=FAIL
      if [ "$SSRK_RK_STATE" = "missing" ]; then
        SSRK_OBSERVED="RevokedKeys missing; PubkeyAuthentication yes; authorized_keys present"
      else
        SSRK_OBSERVED="RevokedKeys none; PubkeyAuthentication yes; authorized_keys present"
      fi
      SSRK_MEANING="The SSH daemon does not disable revoked public keys, so a revoked key can still authenticate."
      SSRK_FIX="set 'RevokedKeys /etc/ssh/revoked_keys' in /etc/ssh/sshd_config then stopsrc -s sshd; startsrc -s sshd; PTxray only recommends these commands."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      ;;
    violation_approx)
      SSRK_STATUS=FAIL
      if [ "$SSRK_PA_STATE" = "value" ]; then
        SSRK_PA_SHOW=$SSRK_PA_VALUE
      else
        SSRK_PA_SHOW=unset
      fi
      if [ "$SSRK_RK_STATE" = "missing" ]; then
        SSRK_OBSERVED="RevokedKeys missing; PubkeyAuthentication $SSRK_PA_SHOW; authorized_keys $SSRK_AK_STATE"
      else
        SSRK_OBSERVED="RevokedKeys none; PubkeyAuthentication $SSRK_PA_SHOW; authorized_keys $SSRK_AK_STATE"
      fi
      SSRK_MEANING="The SSH daemon does not disable revoked public keys, so a revoked key can still authenticate."
      SSRK_FIX="set 'RevokedKeys /etc/ssh/revoked_keys' in /etc/ssh/sshd_config then stopsrc -s sshd; startsrc -s sshd; PTxray only recommends these commands."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      ;;
    absent)
      SSRK_STATUS=NOT_APPLICABLE
      SSRK_SEV=low
      SSRK_OBSERVED="public keys are not used for SSH authentication (PubkeyAuthentication no and no authorized_keys)"
      SSRK_MEANING="Public-key SSH authentication is not in use, so RevokedKeys does not apply."
      SSRK_FIX="n/a"
      ;;
    failed)
      SSRK_STATUS=NOT_ASSESSED
      SSRK_OBSERVED="not assessed - $SSRK_FAIL_WHICH probe $SSRK_FAIL_DETAIL"
      SSRK_MEANING="PTxray did not obtain trustworthy evidence for the SSH RevokedKeys setting."
      SSRK_FIX="verify /etc/ssh/sshd_config and /root/.ssh/authorized_keys are readable, then rerun PTxray."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      ;;
    empty)
      SSRK_STATUS=NOT_ASSESSED
      SSRK_OBSERVED="not assessed - $SSRK_EMPTY_WHICH probe returned empty output with rc 0"
      SSRK_MEANING="PTxray did not obtain trustworthy evidence for the SSH RevokedKeys setting."
      SSRK_FIX="verify grep of /etc/ssh/sshd_config returns content, then rerun PTxray."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      ;;
    manual)
      SSRK_STATUS=NOT_ASSESSED
      SSRK_OBSERVED="not assessed - whether public keys are used for SSH authentication is site/ISSO determined (PubkeyAuthentication and authorized_keys presence are only an approximation)"
      SSRK_MEANING="PTxray can only approximate public-key SSH use; the ISSO must decide whether this control applies."
      SSRK_FIX="confirm whether public keys are used for SSH authentication, then rerun PTxray."
      ;;
    *)
      SSRK_STATUS=NOT_ASSESSED
      SSRK_OBSERVED="not assessed - SSH RevokedKeys or pubkey-use evidence could not be parsed"
      SSRK_MEANING="PTxray did not obtain trustworthy evidence for the SSH RevokedKeys setting."
      SSRK_FIX="run grep \"^RevokedKeys\" /etc/ssh/sshd_config and rerun PTxray."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      ;;
  esac

  add security stig_ssh_revokedkeys "SSH revoked public keys" \
    "$SSRK_STATUS" "$SSRK_SEV" "$SSRK_OBSERVED" \
    "$SSRK_MEANING" "$SSRK_FIX" "stig:V-215293"
}
