...
AIXRAY_TOOL=ck-stig-ssh-rhostsrsa


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_ssh_rhostsrsa — DISA STIG V-215305.
  # Prohibition: any uncommented RhostsRSAAuthentication yes is FAIL.
  # A syntactically valid non-yes value, or absence (readable rc-0
  # output with no token, or grep rc 1 no-match), is PASS. A matching
  # line with no parseable value is unparseable and is NOT_ASSESSED,
  # never PASS.
  # RhostsRSAAuthentication is obsolete (removed in OpenSSH 7.4); this
  # check still evaluates the STIG directive literally.
  # Probe rc is taken from the first grep (a pipeline would collapse
  # file errors and no-match). Comments are then dropped.
  # grep rc 1 is determinate absence and is PASS. Other nonzero rc
  # (file error, rc 127), or rc 0 with empty output, is NOT_ASSESSED,
  # never PASS.
  typeset RHR_ALL RHR_RC RHR_RAW RHR_SHAPE
  typeset RHR_STATUS RHR_SEV RHR_OBSERVED RHR_MEANING RHR_FIX

  # grep -i RhostsRSAAuthentication /etc/ssh/sshd_config | grep -v '^#'  # network-lint: allow -- prose comment; 'ssh' is a component of a local /etc/ssh pathname; no network call
  RHR_ALL=$(aix sshd_config grep -i RhostsRSAAuthentication /etc/ssh/sshd_config)  # network-lint: allow -- aix() capture of a local read-only command; 'ssh' is a component of a local /etc/ssh pathname; no network call
  RHR_RC=$?
  RHR_RAW=$(printf '%s\n' "$RHR_ALL" | grep -v '^#')
  RHR_STATUS=""
  RHR_OBSERVED=""
  ...
  if [ "$RHR_RC" -eq 1 ]; then
    RHR_STATUS=PASS
    RHR_OBSERVED="RhostsRSAAuthentication is absent"
  elif [ "$RHR_RC" -ne 0 ]; then
    RHR_STATUS=NOT_ASSESSED
    RHR_OBSERVED="not assessed - sshd_config probe failed (rc=$RHR_RC)"
  elif [ -z "$RHR_ALL" ]; then
    RHR_STATUS=NOT_ASSESSED
    RHR_OBSERVED="not assessed - sshd_config probe returned no output"
  else
    RHR_SHAPE=$(printf '%s\n' "$RHR_RAW" | tr 'ABCDEFGHIJKLMNOPQRSTUVWXYZ' 'abcdefghijklmnopqrstuvwxyz' | awk '
      /^#/ { next }
      /^[ \t]*$/ { next }
      $1 == "rhostsrsaauthentication" {
        seen = 1
        if (NF < 2) {
          malformed = 1
          next
      ...
      END {
        ...
        else if (!seen) print "absent"
        else if (other) print "other"
        else if (no) print "no"
      }')
    if [ -z "$RHR_SHAPE" ]; then
      RHR_STATUS=NOT_ASSESSED
      RHR_OBSERVED="not assessed - sshd_config probe was unparseable"
    else
      case "$RHR_SHAPE" in
        yes) RHR_STATUS=FAIL
          RHR_OBSERVED="RhostsRSAAuthentication yes"
          ;;
        no)
          RHR_STATUS=PASS
          RHR_OBSERVED="RhostsRSAAuthentication no"
          ;;
        absent)
          RHR_STATUS=PASS
          RHR_OBSERVED="RhostsRSAAuthentication is absent"
        ...
        malformed)
          RHR_STATUS=NOT_ASSESSED
          RHR_OBSERVED="not assessed - RhostsRSAAuthentication directive is unparseable"
          ;;
        *)
          RHR_STATUS=PASS
          RHR_OBSERVED="RhostsRSAAuthentication is not yes"
          ;;
      esac
    fi
  fi
  ...
  case "$RHR_STATUS" in
    PASS)
      RHR_SEV=med
      RHR_MEANING="The SSH daemon does not allow RhostsRSAAuthentication, so rhosts plus RSA host-key trust is not enabled."
      RHR_FIX="n/a"
      ;;
    FAIL)
      RHR_SEV=med
      RHR_MEANING="The SSH daemon allows RhostsRSAAuthentication, which trusts rhosts plus RSA host keys and weakens SSH authentication."
      RHR_FIX="set 'RhostsRSAAuthentication no' in /etc/ssh/sshd_config, then # stopsrc -s sshd; # startsrc -s sshd; PTxray only recommends these actions."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      ;;
    *)
      RHR_STATUS=NOT_ASSESSED
      RHR_SEV=med
      RHR_MEANING="PTxray did not obtain trustworthy evidence for SSH RhostsRSAAuthentication."
      RHR_FIX="verify /etc/ssh/sshd_config is readable and rerun PTxray."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      ;;
  esac

  add security stig_ssh_rhostsrsa "SSH RhostsRSAAuthentication" \
    "$RHR_STATUS" "$RHR_SEV" "$RHR_OBSERVED" "$RHR_MEANING" "$RHR_FIX" \
    "stig:V-215305"
}
