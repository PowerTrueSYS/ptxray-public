...
AIXRAY_TOOL=ck-stig-ssh-tcpforwarding


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_ssh_tcpforwarding — DISA STIG V-215301.
  # Decidable: uncommented AllowTcpForwarding must be no. A missing
  # directive or yes is a finding. ISSO approval of TCP forwarding is
  # the rule-text NA overlay; the remaining ISSO clause is a typed
  # refusal, never a silent PASS.
  typeset STF_RAW STF_RC STF_CLASS STF_PARSE_RC STF_REASON
  typeset STF_STATUS STF_SEV STF_OBSERVED STF_MEANING STF_FIX

  # Pipeline runs inside aix via sh -c; a quoted pipeline is not an executable.
  # grep no-match is rc 1 and must not be treated as a probe failure: emit an
  # rc 0 absence sentinel. Unreadable sshd_config stays nonzero.
  STF_RAW=$(aix sshd_allowtcpforwarding sh -c "[ -r /etc/ssh/sshd_config ] || exit 2; grep -i AllowTcpForwarding /etc/ssh/sshd_config | grep -v '^#'; STF_PRC=\$?; if [ \$STF_PRC -eq 1 ]; then printf '%s\n' ptxray_absent_allowtcpforwarding; exit 0; fi; exit \$STF_PRC")  # network-lint: allow -- aix() capture of a local read-only command; 'ssh' is a component of a local /etc/ssh pathname; no network call
  STF_RC=$?
  STF_CLASS=""

  if [ "$STF_RC" -ne 0 ]; then
    STF_CLASS=probe_failed
  elif [ -z "$STF_RAW" ]; then
    STF_CLASS=probe_empty
  else
    STF_CLASS=$(printf '%s\n' "$STF_RAW" | awk '
      {
        sub(/^[ \t]+/, "")
        ...
        low = tolower($0)
        if (low == "isso_approved_tcp_forwarding") { isso = 1; next }
        if (low == "manual_isso_tcp_forwarding_approval") { manual = 1; next }
        if (low == "ptxray_absent_allowtcpforwarding") { missing = 1; next }
        if (tolower($1) != "allowtcpforwarding") next
        if (NF < 2) { bad = 1; next }
        found = 1
        val = tolower($2)
        if (val == "yes") saw_yes = 1
        else if (val == "no") saw_no = 1
        else bad = 1
      }
      END {
        if (isso) { print "isso_approved"; exit }
        if (manual) { print "manual_isso"; exit }
        if (saw_yes) { print "yes"; exit }
        if (bad) { print "unparseable"; exit }
        if (saw_no) { print "no"; exit }
        if (missing) { print "absent"; exit }
        print "absent"
      }
    ')
    STF_PARSE_RC=$?
    if [ "$STF_PARSE_RC" -ne 0 ] || [ -z "$STF_CLASS" ]; then
      STF_CLASS=unparseable
    fi
  fi

  case "$STF_CLASS" in
    no) STF_REASON=compliant ;;
    yes) STF_REASON=noncompliant ;;
    absent) STF_REASON=subject_absent_setting ;;
    isso_approved) STF_REASON=isso_approved ;;
    manual_isso) STF_REASON=manual_isso ;;
    probe_failed) STF_REASON=probe_failed ;;
    probe_empty) STF_REASON=probe_empty ;;
    *) STF_REASON=unparseable ;;
  esac

  case "$STF_REASON" in
    compliant)
      STF_STATUS=PASS
      STF_OBSERVED="AllowTcpForwarding no"
      STF_MEANING="SSH daemon TCP forwarding is turned off, so an authenticated session cannot relay TCP connections through this host."
      STF_FIX="n/a"
      ;;
    noncompliant) STF_STATUS=FAIL
      STF_OBSERVED="AllowTcpForwarding yes"
      STF_MEANING="SSH daemon TCP forwarding is enabled, so an authenticated session can relay TCP connections through this host."
      STF_FIX="set AllowTcpForwarding no in /etc/ssh/sshd_config, then stopsrc -s sshd; startsrc -s sshd; PTxray only recommends these commands."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      ;;
    subject_absent_setting) STF_STATUS=FAIL
      STF_OBSERVED="AllowTcpForwarding is not present"
      STF_MEANING="AllowTcpForwarding is not set, so the STIG requirement that SSH daemon TCP forwarding be turned off is unmet."
      STF_FIX="set AllowTcpForwarding no in /etc/ssh/sshd_config, then stopsrc -s sshd; startsrc -s sshd; PTxray only recommends these commands."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      ;;
    isso_approved)
      STF_STATUS=NOT_APPLICABLE
      STF_OBSERVED="TCP forwarding approved for use by the ISSO; rule is not applicable"
      STF_MEANING="TCP forwarding is approved for use by the ISSO, so this rule is not applicable."
      STF_FIX="n/a"
      ;;
    manual_isso) STF_STATUS=NOT_ASSESSED
      STF_OBSERVED="not assessed - ISSO approval of SSH TCP forwarding is a manual determination"
      STF_MEANING="Whether the ISSO has approved SSH TCP forwarding cannot be decided from sshd_config; that remainder is manual."
      STF_FIX="record the ISSO determination for SSH TCP forwarding, then rerun PTxray."
      ;;
    probe_failed) STF_STATUS=NOT_ASSESSED
      STF_OBSERVED="not assessed - AllowTcpForwarding probe failed (rc=$STF_RC)"
      STF_MEANING="PTxray did not obtain trustworthy AllowTcpForwarding evidence."
      STF_FIX="verify that grep -i AllowTcpForwarding /etc/ssh/sshd_config | grep -v '^#' runs, then rerun PTxray."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      ;;
    probe_empty) STF_STATUS=NOT_ASSESSED
      STF_OBSERVED="not assessed - AllowTcpForwarding probe returned no output"
      STF_MEANING="PTxray did not obtain trustworthy AllowTcpForwarding evidence."
      STF_FIX="verify that grep -i AllowTcpForwarding /etc/ssh/sshd_config | grep -v '^#' returns complete evidence, then rerun PTxray."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      ;;
    *)
      STF_STATUS=NOT_ASSESSED
      STF_OBSERVED="not assessed - AllowTcpForwarding value was unparseable"
      STF_MEANING="PTxray did not obtain trustworthy AllowTcpForwarding evidence."
      STF_FIX="verify the uncommented AllowTcpForwarding directive in /etc/ssh/sshd_config, then rerun PTxray."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      ;;
  esac

  case "$STF_STATUS" in
    NOT_APPLICABLE) STF_SEV=low ;;
    *) STF_SEV=med ;;
  esac

  add security stig_ssh_tcpforwarding "SSH daemon TCP forwarding" \
    "$STF_STATUS" "$STF_SEV" "$STF_OBSERVED" \
    "$STF_MEANING" "$STF_FIX" "stig:V-215301"
}
