...
AIXRAY_TOOL=ck-stig-ssh-tcpwrappers


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_ssh_tcpwrappers — DISA STIG V-215295.
  # OpenSSH 6.7 or above: requirement is not applicable. Older OpenSSH needs
  # sshd entries in both /etc/hosts.allow and /etc/hosts.deny. Uncertain
  # probe evidence is refused; grep rc 1 with empty output is determinate
  # absence of sshd wrapper entries.
  typeset SSTW_VER_RAW SSTW_VER_RC SSTW_PARSED SSTW_PARSE_RC
  typeset SSTW_MAJ SSTW_MIN SSTW_VER_STATE
  typeset SSTW_DENY_RAW SSTW_DENY_RC SSTW_DENY_STATE
  typeset SSTW_ALLOW_RAW SSTW_ALLOW_RC SSTW_ALLOW_STATE
  typeset SSTW_REASON SSTW_STATUS SSTW_SEV
  typeset SSTW_OBSERVED SSTW_MEANING SSTW_FIX

  # sshd -V prints its banner on stderr; aix discards stderr, so merge
  # inside the probed command.
  SSTW_VER_RAW=$(aix sshd_version sh -c 'sshd -V 2>&1') # network-lint: allow -- `sshd -V` prints a version string and exits; it binds no socket and makes no network call
  SSTW_VER_RC=$?
  SSTW_DENY_RAW=$(aix hosts_deny_sshd grep sshd /etc/hosts.deny)
  SSTW_DENY_RC=$?
  SSTW_ALLOW_RAW=$(aix hosts_allow_sshd grep sshd /etc/hosts.allow)
  SSTW_ALLOW_RC=$?

  SSTW_MAJ=""
  SSTW_MIN=""
  if [ "$SSTW_VER_RC" -ne 0 ]; then
    SSTW_VER_STATE=failed
  elif [ -z "$SSTW_VER_RAW" ]; then
    SSTW_VER_STATE=empty
  else
    SSTW_PARSED=$(printf '%s\n' "$SSTW_VER_RAW" | awk '
      {
        if (match($0, /OpenSSH_[0-9][0-9]*/)) {
          rest = substr($0, RSTART + 8)
          if (match(rest, /^[0-9][0-9]*/)) {
            major = substr(rest, 1, RLENGTH) + 0
            rest = substr(rest, RLENGTH + 1)
            minor = 0
            if (substr(rest, 1, 1) == ".") {
              rest = substr(rest, 2)
              if (match(rest, /^[0-9][0-9]*/)) {
                minor = substr(rest, 1, RLENGTH) + 0
              }
            }
            print major " " minor
            found = 1
    ...
    ')
    SSTW_PARSE_RC=$?
    if [ "$SSTW_PARSE_RC" -ne 0 ] || [ -z "$SSTW_PARSED" ]; then
      SSTW_VER_STATE=unparseable
    else
      SSTW_MAJ=${SSTW_PARSED%% *}
      SSTW_MIN=${SSTW_PARSED#* }
      if [ "$SSTW_MAJ" -gt 6 ]; then
        SSTW_VER_STATE=not_applicable
      elif [ "$SSTW_MAJ" -eq 6 ] && [ "$SSTW_MIN" -ge 7 ]; then
        SSTW_VER_STATE=not_applicable
      else
        SSTW_VER_STATE=old
      fi
  ...
  if [ "$SSTW_DENY_RC" -eq 0 ]; then
    if [ -z "$SSTW_DENY_RAW" ]; then
      SSTW_DENY_STATE=empty
    else
      SSTW_DENY_STATE=present
    fi
  elif [ "$SSTW_DENY_RC" -eq 1 ] && [ -z "$SSTW_DENY_RAW" ]; then
    SSTW_DENY_STATE=none
  else
    SSTW_DENY_STATE=failed
  fi

  if [ "$SSTW_ALLOW_RC" -eq 0 ]; then
    if [ -z "$SSTW_ALLOW_RAW" ]; then
      SSTW_ALLOW_STATE=empty
    else
      SSTW_ALLOW_STATE=present
    fi
  elif [ "$SSTW_ALLOW_RC" -eq 1 ] && [ -z "$SSTW_ALLOW_RAW" ]; then
    SSTW_ALLOW_STATE=none
  else
    SSTW_ALLOW_STATE=failed
  fi

  case "$SSTW_VER_STATE" in
    failed) SSTW_REASON=probe_failed ;;
    empty) SSTW_REASON=probe_empty ;;
    unparseable) SSTW_REASON=unparseable ;;
    not_applicable) SSTW_REASON=not_applicable ;;
    old)
      case "$SSTW_DENY_STATE:$SSTW_ALLOW_STATE" in
        failed:*|*:failed) SSTW_REASON=probe_failed ;;
        empty:*|*:empty) SSTW_REASON=probe_empty ;;
        present:present) SSTW_REASON=compliant ;;
        none:*|*:none) SSTW_REASON=noncompliant ;;
        *) SSTW_REASON=unparseable ;;
      esac
      ;;
    *) SSTW_REASON=unparseable ;;
  esac
  ...
  case "$SSTW_REASON" in
    compliant)
      SSTW_STATUS=PASS
      SSTW_OBSERVED="OpenSSH $SSTW_MAJ.$SSTW_MIN; sshd entries present in /etc/hosts.allow and /etc/hosts.deny"
      SSTW_MEANING="TCP wrappers are configured for sshd, so the SSH daemon is IP-filtered."
      SSTW_FIX="n/a"
      ;;
    noncompliant) SSTW_STATUS=FAIL
      SSTW_OBSERVED="OpenSSH $SSTW_MAJ.$SSTW_MIN; TCP wrappers are not configured for sshd"
      SSTW_MEANING="No sshd entries were found in /etc/hosts.allow and /etc/hosts.deny, so the SSH daemon is not IP-filtered."
      SSTW_FIX="installp -aXYgd /dev/cd0 -e /tmp/install.log netsec.options.tcpwrappers; PTxray only recommends this command."
      ;;
    not_applicable)
      SSTW_STATUS=NOT_APPLICABLE
      SSTW_OBSERVED="OpenSSH $SSTW_MAJ.$SSTW_MIN; 7 or above, this requirement is not applicable"
      SSTW_MEANING="OpenSSH 6.7 or above does not use TCP wrappers for sshd, so this requirement is not applicable."
      SSTW_FIX="n/a"
      ;;
    probe_failed) SSTW_STATUS=NOT_ASSESSED
      SSTW_OBSERVED="not assessed - sshd TCP wrapper probe failed (sshd_V_rc=$SSTW_VER_RC hosts_deny_rc=$SSTW_DENY_RC hosts_allow_rc=$SSTW_ALLOW_RC)"
      SSTW_MEANING="PTxray did not obtain trustworthy sshd version or TCP wrapper evidence."
      SSTW_FIX="verify that 'sshd -V', 'grep sshd /etc/hosts.deny', and 'grep sshd /etc/hosts.allow' run successfully, then rerun PTxray."
      ;;
    probe_empty) SSTW_STATUS=NOT_ASSESSED
      SSTW_OBSERVED="not assessed - sshd TCP wrapper probe returned no output"
      SSTW_MEANING="PTxray did not obtain trustworthy sshd version or TCP wrapper evidence."
      SSTW_FIX="verify that 'sshd -V', 'grep sshd /etc/hosts.deny', and 'grep sshd /etc/hosts.allow' return complete evidence, then rerun PTxray."
      ;;
    *)
      SSTW_STATUS=NOT_ASSESSED
      SSTW_OBSERVED="not assessed - sshd TCP wrapper evidence was unparseable"
      SSTW_MEANING="PTxray did not obtain trustworthy sshd version or TCP wrapper evidence."
      SSTW_FIX="verify sshd -V output and sshd lines in /etc/hosts.allow and /etc/hosts.deny, then rerun PTxray."
      ;;
  esac

  case "$SSTW_STATUS" in
    NOT_APPLICABLE) SSTW_SEV=low ;;
    *) SSTW_SEV=med ;;
  esac

  add security stig_ssh_tcpwrappers "SSH daemon IP filtering (TCP wrappers)" \
    "$SSTW_STATUS" "$SSTW_SEV" "$SSTW_OBSERVED" \
    "$SSTW_MEANING" "$SSTW_FIX" "stig:V-215295"
}
