...
AIXRAY_TOOL=ck-stig-ssh-verifyreversemapping


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_ssh_verifyreversemapping — DISA STIG V-215298.
  # Required-configuration: uncommented VerifyReverseMapping yes is PASS.
  # no, or absence in a readable sshd_config, is FAIL.
  # VerifyReverseMapping is obsolete (UseDNS in modern OpenSSH); this
  # check still evaluates the STIG directive literally.
  # Probe is cat, not the STIG grep. grep -i VerifyReverseMapping
  # /etc/ssh/sshd_config returns rc 1 and empty stdout when the  # network-lint: allow -- prose comment; 'ssh' is a component of a local /etc/ssh pathname; no network call
  # directive is absent, which collapses into NOT_ASSESSED if every
  # nonzero rc is treated as probe failure. cat distinguishes an
  # unreadable file (rc != 0) from a readable file with no matching
  # directive (rc 0, content). Comments are then dropped.
  # Nonzero rc, or rc 0 with empty output, is NOT_ASSESSED, never PASS.
  typeset VRM_ALL VRM_RC VRM_RAW VRM_SHAPE
  typeset VRM_STATUS VRM_SEV VRM_OBSERVED VRM_MEANING VRM_FIX

  # grep -i VerifyReverseMapping /etc/ssh/sshd_config | grep -v '^#'  # network-lint: allow -- prose comment; 'ssh' is a component of a local /etc/ssh pathname; no network call
  VRM_ALL=$(aix sshd_verifyreversemapping cat /etc/ssh/sshd_config)  # network-lint: allow -- aix() capture of a local read-only command; 'ssh' is a component of a local /etc/ssh pathname; no network call
  VRM_RC=$?
  VRM_RAW=$(printf '%s\n' "$VRM_ALL" | grep -v '^#')
  VRM_STATUS=""
  VRM_OBSERVED=""
  ...
  if [ "$VRM_RC" -ne 0 ]; then
    VRM_STATUS=NOT_ASSESSED
    VRM_OBSERVED="not assessed - sshd_config probe failed (rc=$VRM_RC)"
  elif [ -z "$VRM_ALL" ]; then
    VRM_STATUS=NOT_ASSESSED
    VRM_OBSERVED="not assessed - sshd_config probe returned no output"
  else
    VRM_SHAPE=$(printf '%s\n' "$VRM_RAW" | tr 'ABCDEFGHIJKLMNOPQRSTUVWXYZ' 'abcdefghijklmnopqrstuvwxyz' | awk '
      /^#/ { next }
      /^[ \t]*$/ { next }
      $1 == "verifyreversemapping" {
        seen = 1
        val = $2
      }
      END {
    ...
    if [ -z "$VRM_SHAPE" ]; then
      VRM_STATUS=NOT_ASSESSED
      VRM_OBSERVED="not assessed - sshd_config probe was unparseable"
    else
      case "$VRM_SHAPE" in
        yes)
          VRM_STATUS=PASS
          VRM_OBSERVED="VerifyReverseMapping yes"
          ;;
        no) VRM_STATUS=FAIL
          VRM_OBSERVED="VerifyReverseMapping no"
          ;;
        absent) VRM_STATUS=FAIL
          VRM_OBSERVED="VerifyReverseMapping is missing"
          ;;
        *)
          VRM_STATUS=NOT_ASSESSED
          VRM_OBSERVED="not assessed - VerifyReverseMapping value was unparseable"
          ;;
      esac
    fi
  fi
  ...
  case "$VRM_STATUS" in
    PASS)
      VRM_SEV=med
      VRM_MEANING="sshd reverse name checking is on, so SSH does not trust a client whose claimed hostname does not match a reverse lookup of its address."
      VRM_FIX="n/a"
      ;;
    FAIL)
      VRM_SEV=med
      VRM_MEANING="sshd reverse name checking is not on, so SSH may trust a client whose claimed hostname does not match a reverse lookup of its address."
      VRM_FIX="set 'VerifyReverseMapping yes' in /etc/ssh/sshd_config, then # stopsrc -s sshd; # startsrc -s sshd; PTxray only recommends these actions."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      ;;
    *)
      VRM_STATUS=NOT_ASSESSED
      VRM_SEV=med
      VRM_MEANING="PTxray did not obtain trustworthy evidence for SSH VerifyReverseMapping."
      VRM_FIX="verify /etc/ssh/sshd_config is readable and rerun PTxray."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      ;;
  esac

  add security stig_ssh_verifyreversemapping "SSH VerifyReverseMapping" \
    "$VRM_STATUS" "$VRM_SEV" "$VRM_OBSERVED" "$VRM_MEANING" "$VRM_FIX" \
    "stig:V-215298"
}
