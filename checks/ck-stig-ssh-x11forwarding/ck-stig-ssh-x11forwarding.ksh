...
AIXRAY_TOOL=ck-stig-ssh-x11forwarding


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_ssh_x11forwarding — DISA STIG V-215300.
  # Decidable: one unambiguous X11Forwarding no is PASS; missing or yes is FAIL.
  # Authorized-use overlay is Not Applicable per rule text.
  # Remaining ISSO authorization is a typed refusal, never a silent PASS.
  typeset X11F_RAW X11F_RC X11F_CLASS X11F_PARSE_RC
  typeset X11F_STATUS X11F_SEV X11F_OBSERVED X11F_MEANING X11F_FIX

  # STIG check-content: grep -i X11Forwarding /etc/ssh/sshd_config | grep -v '^#'  # network-lint: allow -- prose comment; 'ssh' is a component of a local /etc/ssh pathname; no network call
  # Capture the first grep's status before filtering comments; a pipeline's
  # status is the last grep and would turn a read failure into a false miss.
  X11F_RAW=$(aix sshd_x11forwarding sh -c 'X11F_GREP=$(grep -i X11Forwarding /etc/ssh/sshd_config); X11F_GREP_RC=$?; if [ "$X11F_GREP_RC" -ne 0 ]; then exit "$X11F_GREP_RC"; fi; printf "%s\n" "$X11F_GREP" | grep -v "^#"')  # network-lint: allow -- aix() capture of a local read-only command; 'ssh' is a component of a local /etc/ssh pathname; no network call
  X11F_RC=$?
  X11F_CLASS=""

  if [ "$X11F_RC" -ne 0 ]; then
    # grep rc 1 with no output is determinate: no uncommented X11Forwarding
    # line. That is the STIG "setting is not present" finding, not a probe
    # error. I/O and other failures are rc >= 2.
    if [ "$X11F_RC" -eq 1 ] && [ -z "$X11F_RAW" ]; then
      X11F_CLASS=missing
    else
      X11F_CLASS=probe_failed
    fi
  elif [ -z "$X11F_RAW" ]; then
    X11F_CLASS=probe_empty
  else
    X11F_CLASS=$(printf '%s\n' "$X11F_RAW" | awk '
      /^[ \t]*$/ { next }
      {
        ...
        n = split(line, f, /[ \t]+/)
        if (f[1] == "ptxray_overlay") {
          if (n == 2 && f[2] == "authorized-use") auth = 1
          else if (n == 2 && f[2] == "manual-isso") manual = 1
          else bad = 1
          next
        }
        hash = index(line, "#")
        ...
        n = split(line, f, /[ \t=]+/)
        if (n == 0 || f[1] == "") next
        if (tolower(f[1]) != "x11forwarding") { bad = 1; next }
        if (n != 2) { bad = 1; next }
        val = tolower(f[2])
      ...
      END {
        if (bad || found > 1) { print "unparseable"; exit }
        if (auth) { print "authorized_use"; exit }
        if (manual) { print "manual_isso"; exit }
        if (!found) { print "missing"; exit }
        if (last == "no") print "compliant"
        else if (last == "yes") print "noncompliant"
        else print "unparseable"
      }
    ')
    X11F_PARSE_RC=$?
    if [ "$X11F_PARSE_RC" -ne 0 ] || [ -z "$X11F_CLASS" ]; then
      X11F_CLASS=unparseable
    fi
  fi
  ...
  case "$X11F_CLASS" in
    compliant)
      X11F_STATUS=PASS
      X11F_OBSERVED="X11Forwarding no"
      X11F_MEANING="The SSH daemon has X11 forwarding turned off as required by DISA STIG V-215300."
      X11F_FIX="n/a"
      ;;
    noncompliant) X11F_STATUS=FAIL
      X11F_OBSERVED="X11Forwarding yes"
      X11F_MEANING="The SSH daemon permits X11 forwarding, allowing graphical sessions to be tunneled through SSH."
      X11F_FIX="stopsrc -s sshd; startsrc -s sshd"
      ;;
    missing) X11F_STATUS=FAIL
      X11F_OBSERVED="X11Forwarding setting is not present"
      X11F_MEANING="The SSH daemon X11Forwarding setting is not present, so X11 forwarding is not turned off as required by DISA STIG V-215300."
      X11F_FIX="stopsrc -s sshd; startsrc -s sshd"
      ;;
    authorized_use) X11F_STATUS=NOT_APPLICABLE
      X11F_OBSERVED="X11 forwarding has been authorized for use"
      X11F_MEANING="X11 forwarding has been authorized for use, so DISA STIG V-215300 is Not Applicable."
      X11F_FIX="n/a"
      ;;
    manual_isso) X11F_STATUS=NOT_ASSESSED
      X11F_OBSERVED="not assessed - ISSO authorization of SSH X11 forwarding is a manual determination"
      X11F_MEANING="Whether X11 forwarding has been authorized for use is an ISSO determination; PTxray does not treat that remaining manual portion as a clean result."
      X11F_FIX="document ISSO authorization of SSH X11 forwarding or set X11Forwarding no, then rerun PTxray."
      ;;
    probe_failed) X11F_STATUS=NOT_ASSESSED
      X11F_OBSERVED="not assessed - sshd X11Forwarding probe failed (rc=$X11F_RC)"
      X11F_MEANING="PTxray did not obtain trustworthy sshd_config X11Forwarding evidence."
      X11F_FIX="verify that grep -i X11Forwarding /etc/ssh/sshd_config runs successfully, then rerun PTxray."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      ;;
    probe_empty) X11F_STATUS=NOT_ASSESSED
      X11F_OBSERVED="not assessed - sshd X11Forwarding probe returned no output"
      X11F_MEANING="PTxray did not obtain trustworthy sshd_config X11Forwarding evidence."
      X11F_FIX="verify that grep -i X11Forwarding /etc/ssh/sshd_config returns complete evidence, then rerun PTxray."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      ;;
    unparseable) X11F_STATUS=NOT_ASSESSED
      X11F_OBSERVED="not assessed - sshd X11Forwarding evidence was unparseable"
      X11F_MEANING="PTxray did not obtain trustworthy sshd_config X11Forwarding evidence."
      X11F_FIX="verify the uncommented X11Forwarding setting in /etc/ssh/sshd_config, then rerun PTxray."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      ;;
    *) X11F_STATUS=NOT_ASSESSED
      X11F_OBSERVED="not assessed - sshd X11Forwarding evidence was unparseable"
      X11F_MEANING="PTxray did not obtain trustworthy sshd_config X11Forwarding evidence."
      X11F_FIX="verify the uncommented X11Forwarding setting in /etc/ssh/sshd_config, then rerun PTxray."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a component of a local /etc/ssh pathname; no network call
      ;;
  ...
  case "$X11F_STATUS" in
    NOT_APPLICABLE) X11F_SEV=low ;;
    *) X11F_SEV=med ;;
  esac

  add security stig_ssh_x11forwarding "SSH daemon X11 forwarding" \
    "$X11F_STATUS" "$X11F_SEV" "$X11F_OBSERVED" \
    "$X11F_MEANING" "$X11F_FIX" "stig:V-215300"
}
