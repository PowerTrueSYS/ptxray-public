...
AIXRAY_TOOL=ck-stig-sshd-active-protect-confidentiality


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_sshd_active_protect_confidentiality — DISA STIG V-215284.
  # Required configuration: openssh.base.server installed and sshd active.
  # Direct fileset and unfiltered SRC probes keep determinate negative
  # evidence. Uncertain probe evidence is refused. Determinate absence of
  # both the fileset and the sshd subsystem is a prohibition (svcoff):
  # that fully absent state is PASS. A missing fileset with a present
  # sshd subsystem is a finding.
  typeset SSAPC_LSLPP_RAW SSAPC_LSLPP_RC SSAPC_LSSRC_RAW SSAPC_LSSRC_RC
  typeset SSAPC_PKG SSAPC_SVC SSAPC_REASON SSAPC_STATUS SSAPC_SEV
  typeset SSAPC_OBSERVED SSAPC_MEANING SSAPC_FIX SSAPC_PARSE_RC

  SSAPC_LSLPP_RAW=$(aix lslpp_ssh lslpp -L openssh.base.server)
  SSAPC_LSLPP_RC=$?
  SSAPC_LSSRC_RAW=$(aix lssrc_sshd lssrc -s sshd)
  SSAPC_LSSRC_RC=$?
  SSAPC_PKG=unknown
  SSAPC_SVC=unknown

  if [ "$SSAPC_LSLPP_RC" -ge 2 ]; then
    SSAPC_PKG=pkg_failed
  elif [ -z "$SSAPC_LSLPP_RAW" ]; then
    if [ "$SSAPC_LSLPP_RC" -eq 0 ]; then
      SSAPC_PKG=pkg_empty
    else
      SSAPC_PKG=pkg_failed
    fi
    ...
    SSAPC_PKG=$(printf '%s\n' "$SSAPC_LSLPP_RAW" | awk '
      /^[ \t]*$/ { next }
      /^[ \t]*Fileset[ \t]/ { listing = 1; next }
      /^[ \t]*-+/ { listing = 1; next }
      /^[ \t]*Path:/ { listing = 1; next }
      /^[ \t]*State codes?/ { listing = 1; next }
      /^[ \t]*Type codes?/ { listing = 1; next }
      /^[ \t]*[A-Z?] -- / { next }
      $1 == "lslpp:" && /openssh\.base\.server/ && /not installed/ {
        absent_diag = 1
        listing = 1
        next
      }
      $1 == "openssh.base.server" {
        if (NF < 3 || $2 !~ /^[0-9][0-9.]*$/) {
          bad = 1
          next
        }
      ...
      { bad = 1 }
      END {
        if (bad || (installed && absent_diag)) exit 1
        if (installed) print "pkg_ok"
        else if (listing || absent_diag) print "pkg_absent"
        else exit 1
      }
    ')
    SSAPC_PARSE_RC=$?
    if [ "$SSAPC_PARSE_RC" -ne 0 ] || [ -z "$SSAPC_PKG" ]; then
      SSAPC_PKG=pkg_unparseable
    fi
  fi

  if [ -n "$SSAPC_LSSRC_RAW" ]; then
    SSAPC_SVC=$(printf '%s\n' "$SSAPC_LSSRC_RAW" | awk '
      /^[ \t]*$/ { next }
      $1 == "Subsystem" && $NF == "Status" { next }
      $1 == "sshd" {
        if ($NF == "active") active = 1
        else if ($NF == "inoperative") inactive = 1
        else bad = 1
        next
      }
      /0513-[0-9][0-9][0-9].*sshd/ {
        missing = 1
        next
      }
      { bad = 1 }
      END {
        if (bad || (active && inactive) ||
            (missing && (active || inactive))) exit 1
        if (active) print "svc_active"
        else if (inactive) print "svc_inactive"
        else if (missing) print "svc_absent"
        else exit 1
      }
    ')
    SSAPC_PARSE_RC=$?
    if [ "$SSAPC_PARSE_RC" -ne 0 ] || [ -z "$SSAPC_SVC" ]; then
      SSAPC_SVC=svc_unparseable
    fi
  elif [ "$SSAPC_LSSRC_RC" -ne 0 ]; then
    SSAPC_SVC=svc_failed
  else
    SSAPC_SVC=svc_empty
  fi

  case "$SSAPC_PKG:$SSAPC_SVC" in
    pkg_failed:*|*:svc_failed) SSAPC_REASON=probe_failed ;;
    pkg_empty:*|*:svc_empty) SSAPC_REASON=probe_empty ;;
    pkg_unparseable:*|*:svc_unparseable) SSAPC_REASON=unparseable ;;
    pkg_ok:svc_active) SSAPC_REASON=compliant ;;
    pkg_ok:svc_inactive|pkg_ok:svc_absent) SSAPC_REASON=sshd_inactive ;;
    pkg_absent:svc_active|pkg_absent:svc_inactive) SSAPC_REASON=pkg_missing ;;
    pkg_absent:svc_absent) SSAPC_REASON=subject_absent ;;
    *) SSAPC_REASON=unparseable ;;
  esac

  case "$SSAPC_REASON" in
    compliant)
      SSAPC_STATUS=PASS
      SSAPC_OBSERVED="openssh.base.server installed; sshd active"
      SSAPC_MEANING="openssh.base.server is installed and sshd is active, so transmitted information can be protected in confidence and integrity."
      SSAPC_FIX="n/a"
      ;;
    sshd_inactive) SSAPC_STATUS=FAIL
      SSAPC_OBSERVED="openssh.base.server installed; sshd is not active"
      SSAPC_MEANING="sshd is not active, so confidentiality and integrity of transmitted information are not protected and non-encryption access is not forced off."
      SSAPC_FIX="# startsrc -s sshd; PTxray only recommends this command."
      ;;
    pkg_missing) SSAPC_STATUS=FAIL
      SSAPC_OBSERVED="openssh.base.server is not installed"
      SSAPC_MEANING="openssh.base.server is not installed while an sshd subsystem is present, so confidentiality and integrity of transmitted information are not protected."
      SSAPC_FIX="# startsrc -s sshd; PTxray only recommends this command."
      ;;
    subject_absent) SSAPC_STATUS=PASS
      SSAPC_OBSERVED="sshd subject is absent (openssh.base.server and sshd subsystem)"
      SSAPC_MEANING="The sshd subject is not present; under the svcoff prohibition convention, absence of that subject is the compliant state."
      SSAPC_FIX="n/a"
      ;;
    probe_failed) SSAPC_STATUS=NOT_ASSESSED
      SSAPC_OBSERVED="not assessed - sshd confidentiality probe failed (lslpp_rc=$SSAPC_LSLPP_RC lssrc_rc=$SSAPC_LSSRC_RC)"
      SSAPC_MEANING="PTxray did not obtain trustworthy openssh.base.server and sshd evidence."
      SSAPC_FIX="verify that 'lslpp -L openssh.base.server' and 'lssrc -s sshd' run successfully, then rerun PTxray."
      ;;
    probe_empty) SSAPC_STATUS=NOT_ASSESSED
      SSAPC_OBSERVED="not assessed - sshd confidentiality probe returned no output"
      SSAPC_MEANING="PTxray did not obtain trustworthy openssh.base.server and sshd evidence."
      SSAPC_FIX="verify that 'lslpp -L openssh.base.server' and 'lssrc -s sshd' return complete evidence, then rerun PTxray."
      ;;
    *)
      SSAPC_STATUS=NOT_ASSESSED
      SSAPC_OBSERVED="not assessed - sshd confidentiality evidence was unparseable"
      SSAPC_MEANING="PTxray did not obtain trustworthy openssh.base.server and sshd evidence."
      SSAPC_FIX="verify openssh.base.server fileset output and sshd SRC status, then rerun PTxray."
      ;;
  esac

  case "$SSAPC_STATUS" in
    NOT_APPLICABLE) SSAPC_SEV=low ;;
    *) SSAPC_SEV=med ;;
  esac

  add security stig_sshd_active_protect_confidentiality "sshd active to protect transmitted information" \
    "$SSAPC_STATUS" "$SSAPC_SEV" "$SSAPC_OBSERVED" \
    "$SSAPC_MEANING" "$SSAPC_FIX" "stig:V-215284"
}
