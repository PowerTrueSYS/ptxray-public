...
AIXRAY_TOOL=ck-stig-sshd-active


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_sshd_active — DISA STIG V-215179.
  ...
  typeset SSHDA_LSLPP_RAW SSHDA_LSLPP_RC SSHDA_LSSRC_RAW SSHDA_LSSRC_RC
  typeset SSHDA_PKG SSHDA_SVC SSHDA_REASON SSHDA_STATUS SSHDA_SEV
  typeset SSHDA_OBSERVED SSHDA_MEANING SSHDA_FIX SSHDA_PARSE_RC

  # STIG listing is lslpp -i |grep -i ssh. Capture lslpp's rc separately so a  # network-lint: allow -- prose comment; 'ssh' is a grep pattern applied to local lslpp output; no network call
  # successful listing with zero SSH matches is absence, not a probe failure.
  SSHDA_LSLPP_RAW=$(aix lslpp_ssh sh -c '_out=$(lslpp -i) || exit $?; printf "%s\n" "$_out" |grep -i ssh || echo "openssh.base.server not installed"')  # network-lint: allow -- aix() capture of a local read-only command; 'ssh' is a grep pattern applied to local lslpp output; no network call
  SSHDA_LSLPP_RC=$?
  SSHDA_LSSRC_RAW=$(aix lssrc_sshd lssrc -s sshd)
  SSHDA_LSSRC_RC=$?
  SSHDA_PKG=unknown
  SSHDA_SVC=unknown

  if [ "$SSHDA_LSLPP_RC" -ne 0 ]; then
    SSHDA_PKG=pkg_failed
  elif [ -z "$SSHDA_LSLPP_RAW" ]; then
    SSHDA_PKG=pkg_empty
  else
    SSHDA_PKG=$(printf '%s\n' "$SSHDA_LSLPP_RAW" | awk '
      /^[ \t]*$/ { expect_wrap = 0; next }
      /openssh\.base\.server/ && /not installed/ {
        absent = 1
        expect_wrap = 0
        next
      }
      ...
      # lslpp -i wraps IBM product id and area onto the next line.
      $1 ~ /^[0-9][0-9][0-9][0-9]-/ {
        if (expect_wrap && $1 ~ /^[0-9][0-9][0-9][0-9]-[A-Z0-9][A-Z0-9]*$/ && NF == 2 && $2 ~ /^openssh\./) {
          expect_wrap = 0
          next
        }
        bad = 1
      ...
      { bad = 1 }
      END {
        if (bad || (installed && absent)) exit 1
        if (installed) print "pkg_ok"
        else if (absent || seen) print "pkg_absent"
        else exit 1
      }
    ')
    SSHDA_PARSE_RC=$?
    if [ "$SSHDA_PARSE_RC" -ne 0 ] || [ -z "$SSHDA_PKG" ]; then
      SSHDA_PKG=pkg_unparseable
    fi
  fi

  if [ "$SSHDA_LSSRC_RC" -ne 0 ]; then
    SSHDA_SVC=svc_failed
  elif [ -z "$SSHDA_LSSRC_RAW" ]; then
    SSHDA_SVC=svc_empty
  else
    SSHDA_SVC=$(printf '%s\n' "$SSHDA_LSSRC_RAW" | awk '
      /^[ \t]*$/ { next }
      $1 == "Subsystem" && $NF == "Status" {
        header = 1
        next
      }
      header && $1 == "sshd" {
      ...
      END {
        if (bad || (active && inactive) ||
            (missing && (active || inactive || header))) exit 1
    ...
    ')
    SSHDA_PARSE_RC=$?
    if [ "$SSHDA_PARSE_RC" -ne 0 ] || [ -z "$SSHDA_SVC" ]; then
      SSHDA_SVC=svc_unparseable
    fi
  fi

  case "$SSHDA_PKG:$SSHDA_SVC" in
    pkg_failed:*|*:svc_failed) SSHDA_REASON=probe_failed ;;
    pkg_empty:*|*:svc_empty) SSHDA_REASON=probe_empty ;;
    pkg_unparseable:*|*:svc_unparseable) SSHDA_REASON=unparseable ;;
    pkg_ok:svc_active) SSHDA_REASON=compliant ;;
    pkg_ok:svc_inactive) SSHDA_REASON=noncompliant ;;
    pkg_absent:*|*:svc_absent) SSHDA_REASON=subject_absent ;;
    *) SSHDA_REASON=unparseable ;;
  esac

  case "$SSHDA_REASON" in
    compliant)
      SSHDA_STATUS=PASS
      SSHDA_OBSERVED="openssh.base.server installed; sshd active"
      SSHDA_MEANING="openssh.base.server is installed and sshd is active, so AIX uses the SSH server for replay-resistant authentication."
      SSHDA_FIX="n/a"
      ;;
    noncompliant) SSHDA_STATUS=FAIL
      SSHDA_OBSERVED="openssh.base.server installed; sshd is not active"
      SSHDA_MEANING="sshd is not active, so replay-resistant SSH authentication is not in effect for network logins."
      SSHDA_FIX="installp -aXYgd /dev/cd0 -e /tmp/install.log openssh.base.server; startsrc -s sshd; PTxray only recommends these commands."
      ;;
    subject_absent) SSHDA_STATUS=FAIL
      SSHDA_OBSERVED="required SSH server subject is absent (openssh.base.server and/or sshd)"
      SSHDA_MEANING="openssh.base.server is not installed and/or sshd is not present, so replay-resistant SSH authentication cannot be used."
      SSHDA_FIX="installp -aXYgd /dev/cd0 -e /tmp/install.log openssh.base.server; startsrc -s sshd; PTxray only recommends these commands."
      ;;
    probe_failed) SSHDA_STATUS=NOT_ASSESSED
      SSHDA_OBSERVED="not assessed - SSH server probe failed (lslpp_rc=$SSHDA_LSLPP_RC lssrc_rc=$SSHDA_LSSRC_RC)"
      SSHDA_MEANING="PTxray did not obtain trustworthy openssh.base.server and sshd evidence."
      SSHDA_FIX="verify that 'lslpp -i |grep -i ssh' and 'lssrc -s sshd' run successfully, then rerun PTxray."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a grep pattern applied to local lslpp output; no network call
      ;;
    probe_empty) SSHDA_STATUS=NOT_ASSESSED
      SSHDA_OBSERVED="not assessed - SSH server probe returned no output"
      SSHDA_MEANING="PTxray did not obtain trustworthy openssh.base.server and sshd evidence."
      SSHDA_FIX="verify that 'lslpp -i |grep -i ssh' and 'lssrc -s sshd' return complete evidence, then rerun PTxray."  # network-lint: allow -- remediation advice text PTxray never executes; 'ssh' is a grep pattern applied to local lslpp output; no network call
      ;;
    *)
      SSHDA_STATUS=NOT_ASSESSED
      SSHDA_OBSERVED="not assessed - SSH server evidence was unparseable"
      SSHDA_MEANING="PTxray did not obtain trustworthy openssh.base.server and sshd evidence."
      SSHDA_FIX="verify openssh.base.server fileset output and sshd SRC status, then rerun PTxray."
      ;;
  esac

  case "$SSHDA_STATUS" in
    FAIL) SSHDA_SEV=high ;;
    NOT_APPLICABLE) SSHDA_SEV=low ;;
    *) SSHDA_SEV=med ;;
  esac

  add security stig_sshd_active "SSH server active for replay-resistant authentication" \
    "$SSHDA_STATUS" "$SSHDA_SEV" "$SSHDA_OBSERVED" \
    "$SSHDA_MEANING" "$SSHDA_FIX" "stig:V-215179"
}
