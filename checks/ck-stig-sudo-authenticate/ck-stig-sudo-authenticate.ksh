...
AIXRAY_TOOL=ck-stig-sudo-authenticate


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_sudo_authenticate — DISA STIG V-215261.
  # Prohibition: sudoers must not grant !authenticate. If sudo is not used
  # on AIX, the rule is Not Applicable. lslpp rc 1 is not-found semantics
  # (determinate absence), including empty stdout — 0504-132 often lands
  # on stderr. Refuse rc 127, other probe errors, empty rc 0, or no anchor.
  typeset SSA_LSLPP_RAW SSA_LSLPP_RC SSA_S_RAW SSA_S_RC SSA_D_RAW SSA_D_RC
  typeset SSA_PKG SSA_SUDOERS SSA_DROPIN SSA_REASON SSA_PARSE_RC
  typeset SSA_STATUS SSA_SEV SSA_OBSERVED SSA_MEANING SSA_FIX

  SSA_LSLPP_RAW=$(aix lslpp_sudo_rte lslpp -l sudo.rte)
  SSA_LSLPP_RC=$?
  SSA_S_RAW=$(aix sudoers_authenticate grep "!authenticate" /etc/sudoers)
  SSA_S_RC=$?
  SSA_D_RAW=$(aix sudoers_d_authenticate find /etc/sudoers.d -type f -exec grep -l "!authenticate" {} \;)
  SSA_D_RC=$?
  SSA_PKG=unknown
  SSA_SUDOERS=unknown
  SSA_DROPIN=unknown

  if [ -z "$SSA_LSLPP_RAW" ]; then
    if [ "$SSA_LSLPP_RC" -eq 1 ]; then
      SSA_PKG=pkg_absent
    elif [ "$SSA_LSLPP_RC" -ne 0 ]; then
      SSA_PKG=pkg_failed
    else
      SSA_PKG=pkg_empty
    fi
    ...
    SSA_PKG=$(printf '%s\n' "$SSA_LSLPP_RAW" | awk '
      /^[ \t]*$/ { next }
      /^[ \t]*Fileset[ \t]+Level[ \t]+State/ { next }
      /^[ \t]*-+[ \t]*$/ { next }
      /^Path:/ { next }
      /^lslpp: 0504-132[ \t]+Fileset sudo\.rte not installed\.?[ \t]*$/ {
        absent = 1
        next
      }
      $1 == "sudo.rte" {
        if (NF < 3 || $2 !~ /^[0-9][0-9.]*$/ ||
            $3 !~ /^(COMMITTED|APPLIED|BROKEN|OBSOLETE|EFIX_LOCKED)$/) {
          bad = 1
          next
        }
        installed = 1
      ...
      END {
        ...
        else if (absent) print "pkg_absent"
        else exit 1
      }
    ')
    SSA_PARSE_RC=$?
    if [ "$SSA_PARSE_RC" -ne 0 ] || [ -z "$SSA_PKG" ]; then
      SSA_PKG=pkg_unparseable
    elif [ "$SSA_PKG" = pkg_ok ] && [ "$SSA_LSLPP_RC" -ne 0 ]; then
      SSA_PKG=pkg_unparseable
    fi
  fi

  if [ "$SSA_S_RC" -eq 0 ]; then
    if [ -z "$SSA_S_RAW" ]; then
      SSA_SUDOERS=sudoers_empty
    else
      SSA_SUDOERS=$(printf '%s\n' "$SSA_S_RAW" | awk '
        /^[ \t]*$/ { next }
        {
          if (index($0, "!authenticate") == 0) { bad = 1; next }
          hit = 1
        }
        END {
          if (bad) exit 1
          if (hit) print "sudoers_hit"
          else exit 1
        }
      ')
      SSA_PARSE_RC=$?
      if [ "$SSA_PARSE_RC" -ne 0 ] || [ -z "$SSA_SUDOERS" ]; then
        SSA_SUDOERS=sudoers_unparseable
      fi
    fi
  elif [ "$SSA_S_RC" -eq 1 ]; then
    if [ -z "$SSA_S_RAW" ]; then
      SSA_SUDOERS=sudoers_none
    else
      SSA_SUDOERS=sudoers_unparseable
    fi
  else
    SSA_SUDOERS=sudoers_failed
  fi

  if [ "$SSA_D_RC" -ne 0 ]; then
    SSA_DROPIN=dropin_failed
  elif [ -z "$SSA_D_RAW" ]; then
    SSA_DROPIN=dropin_none
  else
    SSA_DROPIN=$(printf '%s\n' "$SSA_D_RAW" | awk '
      /^[ \t]*$/ { next }
      {
      ...
      END {
        if (bad) exit 1
        if (hit) print "dropin_hit"
        else print "dropin_none"
      }
    ')
    SSA_PARSE_RC=$?
    if [ "$SSA_PARSE_RC" -ne 0 ] || [ -z "$SSA_DROPIN" ]; then
      SSA_DROPIN=dropin_unparseable
    fi
  fi

  case "$SSA_PKG" in
    pkg_failed) SSA_REASON=probe_failed ;;
    pkg_empty) SSA_REASON=probe_empty ;;
    pkg_unparseable) SSA_REASON=unparseable ;;
    pkg_absent) SSA_REASON=subject_absent ;;
    pkg_ok)
      case "$SSA_SUDOERS:$SSA_DROPIN" in
        sudoers_hit:*|*:dropin_hit) SSA_REASON=noncompliant ;;
        sudoers_failed:*|*:dropin_failed) SSA_REASON=probe_failed ;;
        sudoers_empty:*) SSA_REASON=probe_empty ;;
        sudoers_unparseable:*|*:dropin_unparseable) SSA_REASON=unparseable ;;
        sudoers_none:dropin_none) SSA_REASON=compliant ;;
        *) SSA_REASON=unparseable ;;
      esac
      ;;
    *) SSA_REASON=unparseable ;;
  esac
  ...
  case "$SSA_REASON" in
    compliant)
      SSA_STATUS=PASS
      SSA_OBSERVED="no !authenticate option in /etc/sudoers or /etc/sudoers.d"
      SSA_MEANING="sudo is in use and does not grant a !authenticate exception, so sudo commands still require re-authentication."
      SSA_FIX="n/a"
      ;;
    noncompliant) SSA_STATUS=FAIL
      SSA_OBSERVED="!authenticate found in sudoers configuration"
      SSA_MEANING="A sudoers !authenticate option lets a command run with elevated rights without re-authenticating the invoking user."
      SSA_FIX="# visudo -f /etc/sudoers; # visudo -f /etc/sudoers.d/<config_file_name>"
      ;;
    subject_absent) SSA_STATUS=NOT_APPLICABLE
      SSA_OBSERVED="sudo is not used (sudo.rte not installed)"
      SSA_MEANING="sudo is not used on this AIX host, so the !authenticate prohibition does not apply."
      SSA_FIX="n/a"
      ;;
    probe_failed) SSA_STATUS=NOT_ASSESSED
      SSA_OBSERVED="not assessed - sudo !authenticate probe failed (lslpp_rc=$SSA_LSLPP_RC sudoers_rc=$SSA_S_RC sudoers_d_rc=$SSA_D_RC)"
      SSA_MEANING="PTxray did not obtain trustworthy sudoers !authenticate evidence."
      SSA_FIX="verify that 'lslpp -l sudo.rte', 'grep \"!authenticate\" /etc/sudoers', and the sudoers.d find run successfully, then rerun PTxray."
      ;;
    probe_empty) SSA_STATUS=NOT_ASSESSED
      SSA_OBSERVED="not assessed - sudo !authenticate probe returned no output"
      SSA_MEANING="PTxray did not obtain trustworthy sudoers !authenticate evidence."
      SSA_FIX="verify that the sudo fileset listing and sudoers greps return complete evidence, then rerun PTxray."
      ;;
    *)
      SSA_STATUS=NOT_ASSESSED
      SSA_OBSERVED="not assessed - sudo !authenticate evidence was unparseable"
      SSA_MEANING="PTxray did not obtain trustworthy sudoers !authenticate evidence."
      SSA_FIX="verify sudo.rte fileset output and sudoers !authenticate greps, then rerun PTxray."
      ;;
  esac

  case "$SSA_STATUS" in
    NOT_APPLICABLE) SSA_SEV=low ;;
    *) SSA_SEV=med ;;
  esac

  add security stig_sudo_authenticate "sudo !authenticate option" \
    "$SSA_STATUS" "$SSA_SEV" "$SSA_OBSERVED" \
    "$SSA_MEANING" "$SSA_FIX" "stig:V-215261"
}
