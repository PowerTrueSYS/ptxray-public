...
AIXRAY_TOOL=ck-stig-sudo-nopasswd


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_sudo_nopasswd — DISA STIG V-215260.
  # Prohibition: sudoers must not contain a NOPASSWD tag.
  # Determinate sudo absence is NOT_APPLICABLE per the rule text.
  typeset SNP_BIN_RAW SNP_BIN_RC SNP_S_RAW SNP_S_RC SNP_D_RAW SNP_D_RC
  typeset SNP_S_STATE SNP_D_STATE SNP_HIT SNP_REASON
  typeset SNP_STATUS SNP_SEV SNP_OBSERVED SNP_MEANING SNP_FIX
  ...
  # aix records stdout; wrap so ls/find diagnostics stay inside the capture.
  SNP_BIN_RAW=$(aix sudo_binary sh -c 'ls -ld /usr/bin/sudo 2>&1')
  SNP_BIN_RC=$?

  if [ "$SNP_BIN_RC" -eq 0 ]; then
    if [ -z "$SNP_BIN_RAW" ]; then
      SNP_REASON=probe_empty
    else
      SNP_HIT=$(printf '%s\n' "$SNP_BIN_RAW" | awk '
        NF && $NF == "/usr/bin/sudo" { present=1 }
        END { if (present) print "present"; else print "bad" }
      ')
      if [ "$SNP_HIT" != "present" ]; then
        SNP_REASON=unparseable
      fi
    fi
  elif [ -z "$SNP_BIN_RAW" ]; then
    SNP_REASON=probe_failed
  else
    SNP_HIT=$(printf '%s\n' "$SNP_BIN_RAW" | awk '
      /\/usr\/bin\/sudo/ && (/does not exist/ || /No such file or directory/ || /not found/) { absent=1 }
      END { if (absent) print "absent"; else print "failed" }
    ')
    if [ "$SNP_HIT" = "absent" ]; then
      SNP_REASON=subject_absent
    else
      SNP_REASON=probe_failed
    fi
  fi

  if [ -z "$SNP_REASON" ]; then
    SNP_S_RAW=$(aix sudoers_nopasswd grep NOPASSWD /etc/sudoers)
    SNP_S_RC=$?
    # find -exec runs inside aix via sh -c; a quoted -exec line is not an executable.
    SNP_D_RAW=$(aix sudoers_d_nopasswd sh -c 'find /etc/sudoers.d -type f -exec grep -l NOPASSWD {} \; 2>&1')
    SNP_D_RC=$?

    if [ "$SNP_S_RC" -eq 0 ]; then
      if [ -z "$SNP_S_RAW" ]; then
        SNP_S_STATE=empty
      else
        SNP_HIT=$(printf '%s\n' "$SNP_S_RAW" | awk '/NOPASSWD/ { found=1 } END { if (found) print "yes"; else print "no" }')
        if [ "$SNP_HIT" = "yes" ]; then
          SNP_S_STATE=hit
        else
          SNP_S_STATE=unparseable
        fi
      fi
    elif [ "$SNP_S_RC" -eq 1 ]; then
      if [ -z "$SNP_S_RAW" ]; then
        SNP_S_STATE=clean
      else
        SNP_S_STATE=unparseable
    ...
    if [ "$SNP_D_RC" -ne 0 ]; then
      SNP_D_STATE=failed
    else
      SNP_HIT=$(printf '%s\n' "$SNP_D_RAW" | awk '
        BEGIN { d_hits=0; d_bad=0 }
        /^[ \t]*$/ { next }
        /^\/etc\/sudoers\.d\/[^ \t]+$/ { d_hits++; next }
        { d_bad=1 }
        END {
          if (d_bad) { print "failed"; exit }
          if (d_hits) { print "hit"; exit }
          print "clean"
        }
      ')
      case "$SNP_HIT" in
        hit) SNP_D_STATE=hit ;;
        clean) SNP_D_STATE=clean ;;
        *) SNP_D_STATE=failed ;;
      esac
    fi

    case "$SNP_S_STATE:$SNP_D_STATE" in
      failed:*|*:failed) SNP_REASON=probe_failed ;;
      hit:*|*:hit) SNP_REASON=noncompliant ;;
      empty:*) SNP_REASON=probe_empty ;;
      unparseable:*|*:unparseable) SNP_REASON=unparseable ;;
      clean:clean) SNP_REASON=compliant ;;
      *) SNP_REASON=unparseable ;;
    esac
  fi
  ...
  case "$SNP_REASON" in
    compliant)
      SNP_STATUS=PASS
      SNP_OBSERVED="no NOPASSWD tag in /etc/sudoers or /etc/sudoers.d"
      SNP_MEANING="sudoers does not grant passwordless privilege via a NOPASSWD tag."
      SNP_FIX="n/a"
      ;;
    noncompliant) SNP_STATUS=FAIL
      case "$SNP_S_STATE:$SNP_D_STATE" in
        hit:hit) SNP_OBSERVED="NOPASSWD tag in /etc/sudoers and /etc/sudoers.d" ;;
        hit:*) SNP_OBSERVED="NOPASSWD tag in /etc/sudoers" ;;
        *:hit) SNP_OBSERVED="NOPASSWD tag in /etc/sudoers.d" ;;
        *) SNP_OBSERVED="NOPASSWD tag in sudoers configuration" ;;
      esac
      SNP_MEANING="A NOPASSWD tag in sudoers lets a user run privileged commands without re-authentication."
      SNP_FIX="edit the sudoers tree with 'visudo -f' and 'visudo -f /etc/sudoers.d/<config_file_name>' to remove the NOPASSWD tag; PTxray only recommends these commands."
      ;;
    subject_absent) SNP_STATUS=NOT_APPLICABLE
      SNP_OBSERVED="sudo is not used on this host (/usr/bin/sudo absent)"
      SNP_MEANING="sudo is not used on AIX, so the NOPASSWD prohibition does not apply."
      SNP_FIX="n/a"
      ;;
    probe_failed) SNP_STATUS=NOT_ASSESSED
      SNP_OBSERVED="not assessed - sudo NOPASSWD probe failed (bin_rc=$SNP_BIN_RC sudoers_rc=${SNP_S_RC:-n/a} sudoers_d_rc=${SNP_D_RC:-n/a})"
      SNP_MEANING="PTxray did not obtain trustworthy sudoers evidence for the NOPASSWD tag."
      SNP_FIX="verify that 'ls -ld /usr/bin/sudo', 'grep NOPASSWD /etc/sudoers', and the sudoers.d find run successfully, then rerun PTxray."
      ;;
    probe_empty) SNP_STATUS=NOT_ASSESSED
      SNP_OBSERVED="not assessed - sudo NOPASSWD probe returned no output"
      SNP_MEANING="PTxray did not obtain trustworthy sudoers evidence for the NOPASSWD tag."
      SNP_FIX="verify the sudo presence and sudoers NOPASSWD probes return complete evidence, then rerun PTxray."
      ;;
    *)
      SNP_STATUS=NOT_ASSESSED
      SNP_OBSERVED="not assessed - sudoers NOPASSWD evidence was unparseable"
      SNP_MEANING="PTxray did not obtain trustworthy sudoers evidence for the NOPASSWD tag."
      SNP_FIX="verify /etc/sudoers and /etc/sudoers.d are readable, then rerun PTxray."
      ;;
  esac

  case "$SNP_STATUS" in
    FAIL) SNP_SEV=high ;;
    NOT_APPLICABLE) SNP_SEV=low ;;
    *) SNP_SEV=med ;;
  esac

  add security stig_sudo_nopasswd "sudo NOPASSWD tag" \
    "$SNP_STATUS" "$SNP_SEV" "$SNP_OBSERVED" \
    "$SNP_MEANING" "$SNP_FIX" "stig:V-215260"
}
