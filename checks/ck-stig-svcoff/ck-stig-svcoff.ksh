...
# undefined (lssrc), is compliant = PASS. Probe usability is evidence: a required probe that
# failed (rc != 0) or read empty (rc 0) grades nothing — its rules are NA. When no rule's probe
# was usable the finding refuses: NOT_ASSESSED if any required source was unreadable (rc != 0),
# NOT_APPLICABLE for a readable-but-empty inventory (every required probe rc 0, no output).
# This is the broad per-V-ID STIG service list; it does NOT duplicate the inetd_cleartext check
# (which flags cleartext-LOGIN daemons enabled).
function eval_svcoff {
  typeset TYPES CAP INE LSS RCT INERC LSSRC RCTRC RAW DETAIL SUM CTLS NPASS NFAIL NNA NAPPLIC FLIST REFUSE REFUSE_REASON ST SEV OBS MEAN FIX
  ...
  # Capture each required probe with its rc — the rc is evidence. A failed or
  # empty probe must never grade; only a usable probe's rules resolve PASS/FAIL.
  INERC=1; LSSRC=1; RCTRC=1
  case " $TYPES " in
    (*" inetd "*)
      INE=$(aix stig_inetd_active grep -v '^#' /etc/inetd.conf); INERC=$?
      ;;
  esac
  case " $TYPES " in
    (*" lssrc "*)
      LSS=$(aix stig_lssrc_a lssrc -a); LSSRC=$?
      ;;
  esac
  case " $TYPES " in
    (*" rctcp "*)
      RCT=$(aix stig_rctcp grep -E '^[	 ]*start ' /etc/rc.tcpip); RCTRC=$?
      ;;
  esac

  # Build the evidence: enabled inetd tokens and SRC subsystem states, plus a
  # per-probe usability marker. The marker carries the probe state into the
  # verdict awk; a flag set inside this subshell would be lost. A probe whose
  # output cannot be parsed is unusable evidence, not absence: it emits
  # PROBE|<key>|unparseable so the verdict refuses instead of grading empty.
  CAP=$(
    case " $TYPES " in
      (*" inetd "*)
        if [ "$INERC" -eq 0 ]; then
          printf '%s\n' "$INE" | awk '
            {
              ...
              if ($1 ~ /^#/) next
              n++
              if (NF >= 6) print "inetd|" $1 "|on"
              else bad = 1
            }
            END {
              if (bad) print "PROBE|inetd|unparseable"
              else if (n == 0) print "PROBE|inetd|empty"
              else print "PROBE|inetd|ok"
            }'
        else
          echo "PROBE|inetd|fail"
        fi
        ;;
    esac
    case " $TYPES " in
          ...
          printf '%s\n' "$LSS" | awk '
            {
              ...
              if ($1 == "Subsystem") next
              n++
              if (NF >= 2) print "lssrc|" $1 "|" $NF
              else bad = 1
            }
            END {
              if (bad) print "PROBE|lssrc|unparseable"
              else if (n == 0) print "PROBE|lssrc|empty"
              else print "PROBE|lssrc|ok"
            }'
        else
          echo "PROBE|lssrc|fail"
        fi
        ;;
    esac
    case " $TYPES " in
      (*" rctcp "*)
        ...
        if [ "$RCTRC" -eq 0 ]; then
          printf '%s\n' "$RCT" | awk '
            {
              if (NF == 0) next
              if ($1 != "start") { bad = 1; next }
              n++
              if ($2 != "") { k = split($2, a, "/"); print "rctcp|" a[k] "|on" }
              else bad = 1
            }
            END {
              if (bad) print "PROBE|rctcp|unparseable"
              else if (n == 0) print "PROBE|rctcp|empty"
              else print "PROBE|rctcp|ok"
            }'
        else
          echo "PROBE|rctcp|fail"
        fi
        ;;
    esac
  )
  ...
  RAW=$({ printf '%s\n' "$R_SVCOFF"; echo "AIXRAY_CAP"; printf '%s\n' "$CAP"; } | awk -F'|' '
    ...
    END{
      ...
      for(i=1;i<=nr;i++){
        ...
        graded=0
        if(t=="inetd" || t=="lssrc" || t=="rctcp"){
          ps=(t in pstate)?pstate[t]:"fail"
          if(ps=="fail"){
            st="NA"; obs=n" (" t " inventory unreadable)"; nna++; graded=1
          }
          else if(ps=="empty"){
            if(t=="inetd") obs=n" (no inetd service enabled)"
            else if(t=="rctcp") obs=n" (no rctcp daemon boot-started)"
            else obs=n" (no lssrc subsystem)"
            st="NA"; nna++; graded=1
          }
          else if(ps=="unparseable"){
            st="NA"; obs=n" (" t " inventory unparseable)"; nna++; graded=1
          }
        }
        if(graded==0){
          if(t=="inetd"){
      ...
      anyfail=0; anyempty=0; anyunparseable=0
      for(t in required){
        ps=(t in pstate)?pstate[t]:"fail"
        if(ps=="fail") anyfail=1
        else if(ps=="empty") anyempty=1
        else if(ps=="unparseable") anyunparseable=1
      }
      refuse=0; reason=""
      if(npass+nfail==0){
        if(anyfail || anyunparseable){
          refuse=1
          r=""
          if(anyfail) r="unreadable"
          if(anyunparseable) r=(r!=""?r" or ":"")"unparseable"
          reason="service inventory " r
        }
        else if(anyempty) refuse=2
      }
      printf "SUMMARY\t%d\t%d\t%d\t%s\t%d\t%s\n", npass, nfail, nna, flist, refuse, reason
    }
  ')
  ...
  REFUSE=$(printf '%s' "$SUM" | awk -F'\t' '{print $6+0}')
  REFUSE_REASON=$(printf '%s' "$SUM" | awk -F'\t' '{print $7}')
  NAPPLIC=$(( NPASS + NFAIL ))
  CTLS=$(printf '%s\n' "$DETAIL" | awk -F'|' 'NF>=3 && $1!=""{printf "%s%s",(n++?" ":""),$1}')

  if [ "$NAPPLIC" -eq 0 ] && [ "$REFUSE" -eq 2 ]; then
    ST=NOT_APPLICABLE; SEV=low
    OBS="no applicable disabled-service rule on this box"
    MEAN="The service inventory was read, but no service the DISA STIG for IBM AIX 7.x requires disabled is enabled, boot-started, or defined here; no disabled-service rule applies on this box."
    FIX="n/a"
  elif [ "$NAPPLIC" -eq 0 ]; then
    ST=NOT_ASSESSED; SEV=med
    OBS="not assessed - ${REFUSE_REASON:-service inventory unreadable}; no disabled-service rule evaluated"
    MEAN="Could not read or reliably parse the service inventory (/etc/inetd.conf, /etc/rc.tcpip, and lssrc -a); no STIG disabled-service rule could be assessed on this run."
    FIX="verify /etc/inetd.conf and /etc/rc.tcpip are readable and well-formed and the SRC is running; or inspect 'lssrc -a' manually."
