...
AIXRAY_TOOL=ck-stig-sysdir-acl


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_sysdir_acl — DISA STIG V-215325.
  # Prohibition: system command directories must not have extended ACLs.
  # Determinate absence of a directory is compliant (no ACL subject).
  typeset SDA_PATH SDA_ST SDA_PARSE_RC SDA_FAIL_LIST
  typeset SDA_ETC_RAW SDA_ETC_RC SDA_BIN_RAW SDA_BIN_RC SDA_USRBIN_RAW SDA_USRBIN_RC SDA_USRLBIN_RAW SDA_USRLBIN_RC
  typeset SDA_USRUCB_RAW SDA_USRUCB_RC SDA_SBIN_RAW SDA_SBIN_RC SDA_USRSBIN_RAW SDA_USRSBIN_RC
  typeset SDA_DISABLED SDA_ABSENT SDA_UNCERTAIN SDA_WHY
  typeset SDA_STATUS SDA_SEV SDA_OBSERVED SDA_MEANING SDA_FIX
  ...
  SDA_UNCERTAIN=0
  SDA_WHY=""
  function sda_parse_acl {
    awk '
    /does not exist/ { absent = 1; next }
    /extended[ \t]+permissions/ { want = 1; next }
    want {
      if ($1 == "disabled") { disabled = 1; want = 0; next }
      if ($1 == "enabled") { enabled = 1; want = 0; next }
      bad = 1
      want = 0
      next
    }
  ...
  SDA_ETC_RAW=$(aix aclget_etc aclget /etc); SDA_ETC_RC=$?; SDA_PATH=/etc
  SDA_ST=failed
  if [ -n "$SDA_ETC_RAW" ]; then
    SDA_ST=$(printf '%s\n' "$SDA_ETC_RAW" | sda_parse_acl)
    SDA_PARSE_RC=$?
    [ "$SDA_PARSE_RC" -eq 0 ] && [ -n "$SDA_ST" ] || SDA_ST=unparseable
  elif [ "$SDA_ETC_RC" -eq 0 ]; then
    SDA_ST=empty
  fi
  case "$SDA_ST" in
    enabled) [ "$SDA_ETC_RC" -eq 0 ] && SDA_FAIL_LIST=$SDA_FAIL_LIST${SDA_FAIL_LIST:+ }$SDA_PATH || { SDA_UNCERTAIN=1; SDA_WHY=${SDA_WHY:-failed}; } ;;
    absent) SDA_ABSENT=1 ;;
    disabled) [ "$SDA_ETC_RC" -eq 0 ] && SDA_DISABLED=1 || { SDA_UNCERTAIN=1; SDA_WHY=${SDA_WHY:-failed}; } ;;
    failed) SDA_UNCERTAIN=1; SDA_WHY=${SDA_WHY:-failed} ;;
    empty) SDA_UNCERTAIN=1; SDA_WHY=${SDA_WHY:-empty} ;;
    *) SDA_UNCERTAIN=1; SDA_WHY=${SDA_WHY:-unparseable} ;;
  esac

  SDA_BIN_RAW=$(aix aclget_bin aclget /bin); SDA_BIN_RC=$?; SDA_PATH=/bin
  SDA_ST=failed
  if [ -n "$SDA_BIN_RAW" ]; then
    SDA_ST=$(printf '%s\n' "$SDA_BIN_RAW" | sda_parse_acl)
    SDA_PARSE_RC=$?
    [ "$SDA_PARSE_RC" -eq 0 ] && [ -n "$SDA_ST" ] || SDA_ST=unparseable
  elif [ "$SDA_BIN_RC" -eq 0 ]; then
    SDA_ST=empty
  fi
  case "$SDA_ST" in
    enabled) [ "$SDA_BIN_RC" -eq 0 ] && SDA_FAIL_LIST=$SDA_FAIL_LIST${SDA_FAIL_LIST:+ }$SDA_PATH || { SDA_UNCERTAIN=1; SDA_WHY=${SDA_WHY:-failed}; } ;;
    absent) SDA_ABSENT=1 ;;
    disabled) [ "$SDA_BIN_RC" -eq 0 ] && SDA_DISABLED=1 || { SDA_UNCERTAIN=1; SDA_WHY=${SDA_WHY:-failed}; } ;;
    failed) SDA_UNCERTAIN=1; SDA_WHY=${SDA_WHY:-failed} ;;
    empty) SDA_UNCERTAIN=1; SDA_WHY=${SDA_WHY:-empty} ;;
    *) SDA_UNCERTAIN=1; SDA_WHY=${SDA_WHY:-unparseable} ;;
  esac

  SDA_USRBIN_RAW=$(aix aclget_usr_bin aclget /usr/bin); SDA_USRBIN_RC=$?; SDA_PATH=/usr/bin
  SDA_ST=failed
  if [ -n "$SDA_USRBIN_RAW" ]; then
    SDA_ST=$(printf '%s\n' "$SDA_USRBIN_RAW" | sda_parse_acl)
    SDA_PARSE_RC=$?
    [ "$SDA_PARSE_RC" -eq 0 ] && [ -n "$SDA_ST" ] || SDA_ST=unparseable
  elif [ "$SDA_USRBIN_RC" -eq 0 ]; then
    SDA_ST=empty
  fi
  case "$SDA_ST" in
    enabled) [ "$SDA_USRBIN_RC" -eq 0 ] && SDA_FAIL_LIST=$SDA_FAIL_LIST${SDA_FAIL_LIST:+ }$SDA_PATH || { SDA_UNCERTAIN=1; SDA_WHY=${SDA_WHY:-failed}; } ;;
    absent) SDA_ABSENT=1 ;;
    disabled) [ "$SDA_USRBIN_RC" -eq 0 ] && SDA_DISABLED=1 || { SDA_UNCERTAIN=1; SDA_WHY=${SDA_WHY:-failed}; } ;;
    failed) SDA_UNCERTAIN=1; SDA_WHY=${SDA_WHY:-failed} ;;
    empty) SDA_UNCERTAIN=1; SDA_WHY=${SDA_WHY:-empty} ;;
    *) SDA_UNCERTAIN=1; SDA_WHY=${SDA_WHY:-unparseable} ;;
  esac

  SDA_USRLBIN_RAW=$(aix aclget_usr_lbin aclget /usr/lbin); SDA_USRLBIN_RC=$?; SDA_PATH=/usr/lbin
  SDA_ST=failed
  if [ -n "$SDA_USRLBIN_RAW" ]; then
    SDA_ST=$(printf '%s\n' "$SDA_USRLBIN_RAW" | sda_parse_acl)
    SDA_PARSE_RC=$?
    [ "$SDA_PARSE_RC" -eq 0 ] && [ -n "$SDA_ST" ] || SDA_ST=unparseable
  elif [ "$SDA_USRLBIN_RC" -eq 0 ]; then
    SDA_ST=empty
  fi
  case "$SDA_ST" in
    enabled) [ "$SDA_USRLBIN_RC" -eq 0 ] && SDA_FAIL_LIST=$SDA_FAIL_LIST${SDA_FAIL_LIST:+ }$SDA_PATH || { SDA_UNCERTAIN=1; SDA_WHY=${SDA_WHY:-failed}; } ;;
    absent) SDA_ABSENT=1 ;;
    disabled) [ "$SDA_USRLBIN_RC" -eq 0 ] && SDA_DISABLED=1 || { SDA_UNCERTAIN=1; SDA_WHY=${SDA_WHY:-failed}; } ;;
    failed) SDA_UNCERTAIN=1; SDA_WHY=${SDA_WHY:-failed} ;;
    empty) SDA_UNCERTAIN=1; SDA_WHY=${SDA_WHY:-empty} ;;
    *) SDA_UNCERTAIN=1; SDA_WHY=${SDA_WHY:-unparseable} ;;
  esac

  SDA_USRUCB_RAW=$(aix aclget_usr_ucb aclget /usr/ucb); SDA_USRUCB_RC=$?; SDA_PATH=/usr/ucb
  SDA_ST=failed
  if [ -n "$SDA_USRUCB_RAW" ]; then
    SDA_ST=$(printf '%s\n' "$SDA_USRUCB_RAW" | sda_parse_acl)
    SDA_PARSE_RC=$?
    [ "$SDA_PARSE_RC" -eq 0 ] && [ -n "$SDA_ST" ] || SDA_ST=unparseable
  elif [ "$SDA_USRUCB_RC" -eq 0 ]; then
    SDA_ST=empty
  fi
  case "$SDA_ST" in
    enabled) [ "$SDA_USRUCB_RC" -eq 0 ] && SDA_FAIL_LIST=$SDA_FAIL_LIST${SDA_FAIL_LIST:+ }$SDA_PATH || { SDA_UNCERTAIN=1; SDA_WHY=${SDA_WHY:-failed}; } ;;
    absent) SDA_ABSENT=1 ;;
    disabled) [ "$SDA_USRUCB_RC" -eq 0 ] && SDA_DISABLED=1 || { SDA_UNCERTAIN=1; SDA_WHY=${SDA_WHY:-failed}; } ;;
    failed) SDA_UNCERTAIN=1; SDA_WHY=${SDA_WHY:-failed} ;;
    empty) SDA_UNCERTAIN=1; SDA_WHY=${SDA_WHY:-empty} ;;
    *) SDA_UNCERTAIN=1; SDA_WHY=${SDA_WHY:-unparseable} ;;
  esac

  SDA_SBIN_RAW=$(aix aclget_sbin aclget /sbin); SDA_SBIN_RC=$?; SDA_PATH=/sbin
  SDA_ST=failed
  if [ -n "$SDA_SBIN_RAW" ]; then
    SDA_ST=$(printf '%s\n' "$SDA_SBIN_RAW" | sda_parse_acl)
    SDA_PARSE_RC=$?
    [ "$SDA_PARSE_RC" -eq 0 ] && [ -n "$SDA_ST" ] || SDA_ST=unparseable
  elif [ "$SDA_SBIN_RC" -eq 0 ]; then
    SDA_ST=empty
  fi
  case "$SDA_ST" in
    enabled) [ "$SDA_SBIN_RC" -eq 0 ] && SDA_FAIL_LIST=$SDA_FAIL_LIST${SDA_FAIL_LIST:+ }$SDA_PATH || { SDA_UNCERTAIN=1; SDA_WHY=${SDA_WHY:-failed}; } ;;
    absent) SDA_ABSENT=1 ;;
    disabled) [ "$SDA_SBIN_RC" -eq 0 ] && SDA_DISABLED=1 || { SDA_UNCERTAIN=1; SDA_WHY=${SDA_WHY:-failed}; } ;;
    failed) SDA_UNCERTAIN=1; SDA_WHY=${SDA_WHY:-failed} ;;
    empty) SDA_UNCERTAIN=1; SDA_WHY=${SDA_WHY:-empty} ;;
    *) SDA_UNCERTAIN=1; SDA_WHY=${SDA_WHY:-unparseable} ;;
  esac

  SDA_USRSBIN_RAW=$(aix aclget_usr_sbin aclget /usr/sbin); SDA_USRSBIN_RC=$?; SDA_PATH=/usr/sbin
  SDA_ST=failed
  if [ -n "$SDA_USRSBIN_RAW" ]; then
    SDA_ST=$(printf '%s\n' "$SDA_USRSBIN_RAW" | sda_parse_acl)
    SDA_PARSE_RC=$?
    [ "$SDA_PARSE_RC" -eq 0 ] && [ -n "$SDA_ST" ] || SDA_ST=unparseable
  elif [ "$SDA_USRSBIN_RC" -eq 0 ]; then
    SDA_ST=empty
  fi
  case "$SDA_ST" in
    enabled) [ "$SDA_USRSBIN_RC" -eq 0 ] && SDA_FAIL_LIST=$SDA_FAIL_LIST${SDA_FAIL_LIST:+ }$SDA_PATH || { SDA_UNCERTAIN=1; SDA_WHY=${SDA_WHY:-failed}; } ;;
    absent) SDA_ABSENT=1 ;;
    disabled) [ "$SDA_USRSBIN_RC" -eq 0 ] && SDA_DISABLED=1 || { SDA_UNCERTAIN=1; SDA_WHY=${SDA_WHY:-failed}; } ;;
    failed) SDA_UNCERTAIN=1; SDA_WHY=${SDA_WHY:-failed} ;;
    empty) SDA_UNCERTAIN=1; SDA_WHY=${SDA_WHY:-empty} ;;
    *) SDA_UNCERTAIN=1; SDA_WHY=${SDA_WHY:-unparseable} ;;
  esac

  if [ -n "$SDA_FAIL_LIST" ]; then SDA_STATUS=FAIL
    SDA_OBSERVED="extended permissions enabled on $SDA_FAIL_LIST"
    SDA_MEANING="An extended ACL on a system command directory lets a non-owner change or replace those files."
    SDA_FIX="acledit [command-path]/[command-file]"
  elif [ "$SDA_UNCERTAIN" -eq 1 ]; then SDA_STATUS=NOT_ASSESSED
    case "$SDA_WHY" in
      failed) SDA_OBSERVED="not assessed - aclget probe failed" ;;
      empty) SDA_OBSERVED="not assessed - aclget probe returned no output" ;;
      *) SDA_OBSERVED="not assessed - aclget output was unparseable" ;;
    esac
    SDA_MEANING="PTxray did not obtain trustworthy aclget evidence for the system command directories."
    SDA_FIX="run aclget on /etc /bin /usr/bin /usr/lbin /usr/ucb /sbin /usr/sbin, correct the capture or path problem, and rerun PTxray."
  elif [ "$SDA_DISABLED" -eq 1 ]; then SDA_STATUS=PASS
    SDA_OBSERVED="extended permissions disabled on system command directories"
    SDA_MEANING="System command directories do not have extended ACLs."
    SDA_FIX="n/a"
  elif [ "$SDA_ABSENT" -eq 1 ]; then SDA_STATUS=PASS
    SDA_OBSERVED="system command directories are absent; no extended ACL subject"
    SDA_MEANING="The system command directories are absent, so no extended ACL subject exists to violate the prohibition."
    SDA_FIX="n/a"
  else SDA_STATUS=NOT_ASSESSED
    SDA_OBSERVED="not assessed - no aclget evidence"
    SDA_MEANING="PTxray did not obtain trustworthy aclget evidence for the system command directories."
    SDA_FIX="run aclget on /etc /bin /usr/bin /usr/lbin /usr/ucb /sbin /usr/sbin, then rerun PTxray."
  fi

  case "$SDA_STATUS" in
    NOT_APPLICABLE) SDA_SEV=low ;;
    *) SDA_SEV=med ;;
  esac

  add security stig_sysdir_acl "System command directory extended ACLs" \
    "$SDA_STATUS" "$SDA_SEV" "$SDA_OBSERVED" \
    "$SDA_MEANING" "$SDA_FIX" "stig:V-215325"
}
