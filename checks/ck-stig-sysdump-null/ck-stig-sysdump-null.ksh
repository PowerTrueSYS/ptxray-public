...
AIXRAY_TOOL=ck-stig-sysdump-null


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_sysdump_null — DISA STIG V-215397. Kernel core dumps disabled unless
  # needed. A configured dump device is needed on a managed AIX system
  # (V-215407 wins); this check never FAILs a real dump device.
  typeset SSD_RAW SSD_RC SSD_PARSE_RC SSD_SHAPE
  typeset SSD_STATUS SSD_SEV SSD_OBSERVED SSD_MEANING SSD_FIX

  SSD_RAW=$(aix sysdumpdev_l sysdumpdev -l)
  SSD_RC=$?
  SSD_STATUS=""
  SSD_SEV=med
  SSD_OBSERVED=""
  SSD_FIX="# sysdumpdev -P -p /dev/sysdumpnull; # sysdumpdev -P -s /dev/sysdumpnull"

  if [ "$SSD_RC" -eq 127 ]; then
    SSD_STATUS=NOT_ASSESSED
    SSD_OBSERVED="not assessed - sysdumpdev missing (rc=127)"
  elif [ "$SSD_RC" -ne 0 ]; then
    SSD_STATUS=NOT_ASSESSED
    SSD_OBSERVED="not assessed - sysdumpdev -l failed (rc=$SSD_RC)"
  elif [ -z "$SSD_RAW" ]; then
    SSD_STATUS=NOT_ASSESSED
    SSD_OBSERVED="not assessed - sysdumpdev -l produced no output"
  else
    SSD_SHAPE=$(printf '%s\n' "$SSD_RAW" | awk '
      $1 == "primary" {
        pcount++
      ...
      END {
        if (pcount != 1 || scount != 1 || primary == "" || secondary == "") {
          exit 1
        }
        if (primary == "/dev/sysdumpnull" && secondary == "/dev/sysdumpnull") {
          print "both_null"
        } else {
          print "configured"
        }
    ...
    ')
    SSD_PARSE_RC=$?
    if [ "$SSD_PARSE_RC" -ne 0 ] || [ -z "$SSD_SHAPE" ]; then
      SSD_STATUS=NOT_ASSESSED
      SSD_OBSERVED="not assessed - sysdumpdev -l output unparseable"
    elif [ "$SSD_SHAPE" = "both_null" ]; then
      SSD_STATUS=PASS
      SSD_SEV=med
      SSD_OBSERVED="primary=/dev/sysdumpnull secondary=/dev/sysdumpnull"
      SSD_FIX="n/a"
    elif [ "$SSD_SHAPE" = "configured" ]; then
      SSD_STATUS=NOT_APPLICABLE
      SSD_SEV=low
      SSD_OBSERVED="dump device intentionally configured per V-215407 (STIG: \"unless needed\")"
      SSD_FIX="n/a"
    else
      SSD_STATUS=NOT_ASSESSED
      SSD_OBSERVED="not assessed - sysdumpdev -l output unparseable"
  ...
  case "$SSD_STATUS" in
    PASS)
      SSD_MEANING="Kernel core dumps are disabled; both dump devices are /dev/sysdumpnull."
      ;;
    NOT_APPLICABLE)
      SSD_MEANING="A dump device is configured. V-215397 does not apply when dumps are needed; V-215407 requires a primary dump device and copy directory."
      ;;
    *)
      SSD_MEANING="PTxray did not obtain trustworthy sysdumpdev evidence."
      ;;
  esac

  add security stig_sysdump_null "AIX kernel core dumps disabled unless needed" \
    "$SSD_STATUS" "$SSD_SEV" "$SSD_OBSERVED" \
    "$SSD_MEANING" "$SSD_FIX" "stig:V-215397"
}
