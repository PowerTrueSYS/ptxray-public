...
AIXRAY_TOOL=ck-stig-sysfile-group


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_sysfile_group — DISA STIG V-215268.
  # System group := GID < 100 from `lsgroup -a id ALL`, or name in {ipsec, sshd}.
  # A name that does not resolve in that map is the remaining ISSO clause.
  typeset SFG_RAW SFG_RC SFG_MAP SFG_MAP_RC SFG_CLASS SFG_PARSE_RC SFG_REASON
  typeset SFG_STATUS SFG_SEV SFG_OBSERVED SFG_MEANING SFG_FIX SFG_DETAIL

  SFG_RAW=$(aix sysfile_group_ls ls -lLa /etc /bin /usr/bin /usr/lbin /usr/ucb /sbin /usr/sbin)
  SFG_RC=$?
  SFG_STATUS=""
  SFG_OBSERVED=""
  SFG_REASON=""
  ...
  if [ "$SFG_RC" -ne 0 ]; then
    SFG_REASON=probe_failed
  elif [ -z "$SFG_RAW" ]; then
    SFG_REASON=probe_empty
  else
    SFG_MAP=$(aix lsgroup_all_ids lsgroup -a id ALL)
    SFG_MAP_RC=$?
    if [ "$SFG_MAP_RC" -ne 0 ]; then
      SFG_REASON=gid_map_failed
    elif [ -z "$SFG_MAP" ]; then
      SFG_REASON=gid_map_empty
    else
      SFG_CLASS=$(
        { printf '%s\n' "$SFG_MAP"; echo "__SFG_LS__"; printf '%s\n' "$SFG_RAW"; } | awk '
      BEGIN {
        nreq = split("/etc /bin /usr/bin /usr/lbin /usr/ucb /sbin /usr/sbin", req, " ")
      }
      $0 == "__SFG_LS__" { in_ls = 1; next }
      !in_ls {
        if (NF >= 2 && $2 ~ /^id=[0-9][0-9]*$/) {
          split($2, a, "=")
          gid[$1] = a[2] + 0
          ngid++
        }
      ...
      NF {
        ...
        if (type !~ /^[-dlcbps]$/) bad = 1
        if (substr(mode, 2, 1) !~ /^[-r]$/) bad = 1
        if (substr(mode, 3, 1) !~ /^[-w]$/) bad = 1
        if (substr(mode, 4, 1) !~ /^[-xSs]$/) bad = 1
        if (substr(mode, 5, 1) !~ /^[-r]$/) bad = 1
        if (substr(mode, 6, 1) !~ /^[-w]$/) bad = 1
        if (substr(mode, 7, 1) !~ /^[-xSs]$/) bad = 1
        if (substr(mode, 8, 1) !~ /^[-r]$/) bad = 1
        ...
        if (grp == "") { bad = 1; next }
        name = $NF
        if (name == "." && cur != "") path = cur
        else if (name ~ /^\//) path = name
        else if (name == "..") {
          if (cur != "") path = cur "/.."
          else path = ".."
        }
        else if (cur != "") path = cur "/" name
        else { bad = 1; next }
        if (name != "..") {
          for (i = 1; i <= nreq; i++) {
            if (path == req[i]) seen[path] = 1
          }
        }
        sysgrp = 0
        g = -1
        if (grp == "ipsec" || grp == "sshd") sysgrp = 1
        else if (grp ~ /^[0-9][0-9]*$/) g = grp + 0
        else if (grp in gid) g = gid[grp]
        else {
          if (manual == "") manual = grp " " path
          next
        }
        if (sysgrp) next
        if (g < 100) next
        if (fail == "") fail = path ":" grp
        else fail = fail " " path ":" grp
      }
      END {
        if (ngid == 0 || records == 0 || bad) exit 1
        miss = ""
        for (i = 1; i <= nreq; i++) {
          if (!(req[i] in seen)) {
            if (miss != "") miss = miss " "
        ...
        else if (fail != "") print "fail " fail
        else if (manual != "") print "manual " manual
        else print "pass"
      }
    '
      )
      SFG_PARSE_RC=$?
      if [ "$SFG_PARSE_RC" -ne 0 ] || [ -z "$SFG_CLASS" ]; then
        SFG_REASON=unparseable
      else
        case "$SFG_CLASS" in
          pass) SFG_REASON=compliant ;;
          fail\ *) SFG_REASON=noncompliant; SFG_DETAIL=${SFG_CLASS#fail } ;;
          missing\ *) SFG_REASON=subject_absent; SFG_DETAIL=${SFG_CLASS#missing } ;;
          manual\ *) SFG_REASON=manual_portion; SFG_DETAIL=${SFG_CLASS#manual } ;;
          *) SFG_REASON=unparseable ;;
        esac
      fi
    fi
  ...
  case "$SFG_REASON" in
    compliant) SFG_STATUS=PASS
      SFG_OBSERVED="listed system paths are group-owned by a system group"
      SFG_MEANING="Listed system files, programs, and directories are group-owned by a system group."
      SFG_FIX="n/a"
      ;;
    noncompliant) SFG_STATUS=FAIL
      SFG_OBSERVED="non-system group owner: $SFG_DETAIL"
      SFG_MEANING="A system file, program, or directory is group-owned by a non-system group, so that group can modify it."
      SFG_FIX="chgrp sys /path/to/system/file"
      ;;
    subject_absent) SFG_STATUS=FAIL
      SFG_OBSERVED="required system path missing: $SFG_DETAIL"
      SFG_MEANING="A required system path is absent, so the group-ownership requirement is unmet."
      SFG_FIX="chgrp sys /path/to/system/file"
      ;;
    probe_failed) SFG_STATUS=NOT_ASSESSED
      SFG_OBSERVED="not assessed - system-file group listing failed (rc=$SFG_RC)"
      SFG_MEANING="PTxray did not obtain trustworthy evidence for system-file group ownership."
      SFG_FIX="verify the listing from 'ls -lLa /etc /bin /usr/bin /usr/lbin /usr/ucb /sbin /usr/sbin', then rerun PTxray."
      ;;
    probe_empty) SFG_STATUS=NOT_ASSESSED
      SFG_OBSERVED="not assessed - system-file group listing returned no output"
      SFG_MEANING="PTxray did not obtain trustworthy evidence for system-file group ownership."
      SFG_FIX="verify the listing from 'ls -lLa /etc /bin /usr/bin /usr/lbin /usr/ucb /sbin /usr/sbin', then rerun PTxray."
      ;;
    gid_map_failed) SFG_STATUS=NOT_ASSESSED
      SFG_OBSERVED="not assessed - group id map from lsgroup failed (rc=$SFG_MAP_RC)"
      SFG_MEANING="PTxray did not obtain trustworthy evidence for which groups are system groups."
      SFG_FIX="verify 'lsgroup -a id ALL' is readable, then rerun PTxray."
      ;;
    gid_map_empty) SFG_STATUS=NOT_ASSESSED
      SFG_OBSERVED="not assessed - group id map from lsgroup returned no output"
      SFG_MEANING="PTxray did not obtain trustworthy evidence for which groups are system groups."
      SFG_FIX="verify 'lsgroup -a id ALL' is readable, then rerun PTxray."
      ;;
    manual_portion) SFG_STATUS=NOT_ASSESSED
      SFG_OBSERVED="not assessed - ISSO must determine whether group ${SFG_DETAIL%% *} is a system group"
      SFG_MEANING="A listed group owner does not resolve in lsgroup, so whether it is a system group is undecided."
      SFG_FIX="ask the ISSO whether the named group is a system group, then rerun PTxray."
      ;;
    *)
      SFG_STATUS=NOT_ASSESSED
      SFG_OBSERVED="not assessed - system-file group listing was unparseable"
      SFG_MEANING="PTxray did not obtain trustworthy evidence for system-file group ownership."
      SFG_FIX="verify the listing from 'ls -lLa /etc /bin /usr/bin /usr/lbin /usr/ucb /sbin /usr/sbin', then rerun PTxray."
      ;;
  esac
  ...
  case "$SFG_STATUS" in
    NOT_APPLICABLE) SFG_SEV=low ;;
    *) SFG_SEV=med ;;
  esac

  add security stig_sysfile_group "System files group-owned by a system group" \
    "$SFG_STATUS" "$SFG_SEV" "$SFG_OBSERVED" "$SFG_MEANING" "$SFG_FIX" \
    "stig:V-215268"
}
