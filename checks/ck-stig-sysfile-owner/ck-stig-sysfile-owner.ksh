...
AIXRAY_TOOL=ck-stig-sysfile-owner


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_sysfile_owner — DISA STIG V-215183.
  # UID <= 128 or authored system name, or smmsp + sendmail bos.* fileset.  # network-lint: allow -- prose comment; 'sendmail' names the mail daemon in prose; no network call
  # Other shipped-service owners, unresolved owners, and aux probe
  # failures (rc != 0/127) stay typed NOT_ASSESSED. rc 127 is overlay-absent.
  typeset SSFO_RAW SSFO_RC SSFO_CLASS SSFO_PARSE_RC SSFO_KIND
  typeset SSFO_LS_RAW SSFO_LS_RC SSFO_SUM_RAW SSFO_SUM_RC  # per-probe status
  typeset SSFO_OWNER SSFO_PATH SSFO_STATUS SSFO_SEV SSFO_OBSERVED SSFO_MEANING SSFO_FIX
  typeset SSFO_IDS SSFO_BOS SSFO_IDS_RC SSFO_BOS_RC SSFO_AUXCMD SSFO_AUXRC SSFO_AUXOUT

  SSFO_IDS=$(aix lsuser_all_ids lsuser -a id ALL)
  SSFO_IDS_RC=$?
  SSFO_BOS=$(aix lsuser_smmsp lslpp -w /usr/sbin/sendmail)  # network-lint: allow -- aix() capture of a local read-only command; 'sendmail' is part of a local file pathname; no network call
  SSFO_BOS_RC=$?
  # rc 127, no output: absent overlay; fall back to the summary probe, graded on its own rc.
  SSFO_LS_RAW=$(aix sysfile_owner_ls ls -lLa /etc /bin /usr/bin /usr/lbin /usr/ucb /sbin /usr/sbin); SSFO_LS_RC=$?
  SSFO_RAW=$SSFO_LS_RAW; SSFO_RC=$SSFO_LS_RC
  if [ -z "$SSFO_LS_RAW" ] && [ "$SSFO_LS_RC" -eq 127 ]; then
    SSFO_SUM_RAW=$(aix sysfile_owners_summary ls -lLa /etc /bin /usr/bin /usr/lbin /usr/ucb /sbin /usr/sbin); SSFO_SUM_RC=$?
    [ "$SSFO_SUM_RC" -eq 127 ] || { SSFO_RAW=$SSFO_SUM_RAW; SSFO_RC=$SSFO_SUM_RC; }
  fi
  SSFO_KIND=""; SSFO_CLASS=""; SSFO_OWNER=""; SSFO_PATH=""
  SSFO_AUXCMD=""; SSFO_AUXRC=0; SSFO_AUXOUT=""

  if [ "$SSFO_IDS_RC" -ne 0 ] && [ "$SSFO_IDS_RC" -ne 127 ]; then
    SSFO_AUXCMD="lsuser -a id ALL"; SSFO_AUXRC=$SSFO_IDS_RC; SSFO_AUXOUT=$SSFO_IDS
  elif [ "$SSFO_BOS_RC" -ne 0 ] && [ "$SSFO_BOS_RC" -ne 127 ]; then
    SSFO_AUXCMD="lslpp -w /usr/sbin/sendmail"; SSFO_AUXRC=$SSFO_BOS_RC; SSFO_AUXOUT=$SSFO_BOS  # network-lint: allow -- evidence display string; 'sendmail' is part of a local file pathname; no network call
  fi
  if [ -n "$SSFO_AUXCMD" ]; then
    if printf '%s\n' "$SSFO_AUXOUT" | awk 'tolower($0) ~ /authori|permission|not allowed/ { f=1 } END { if (f) exit 0; exit 1 }'; then
      SSFO_KIND=aux_auth
    else
      SSFO_KIND=aux_fail
    fi
  elif [ "$SSFO_RC" -ne 0 ] && [ -z "$SSFO_RAW" ]; then
    SSFO_KIND=probe_failed
  elif [ -z "$SSFO_RAW" ]; then
    SSFO_KIND=probe_empty
  else
    SSFO_CLASS=$(printf '%s\n' "===IDS===" "$SSFO_IDS" "===BOS===" "$SSFO_BOS" "===LS===" "$SSFO_RAW" | awk '
      BEGIN {
        sys["root"]=1
        sys["daemon"]=1
        sys["bin"]=1
      ...
      function is_sys(o) {
        if (o ~ /^[0-9][0-9]*$/ && o + 0 <= 128) return 1
        if (o in sys) return 1
        if (o in uid && uid[o] + 0 <= 128) return 1
        if (o in smmsp_owner && bos) return 1
        return 0
      }
      function is_nonsys(o) {
        if (o == "nobody") return 1
        if (o ~ /^[0-9][0-9]*$/ && o + 0 > 128) return 1
        if (o in uid && uid[o] + 0 > 128) {
          if (o in svc && o != "smmsp") return 0
          return 1
        }
        return 0
      }
      ...
      function classify(o, p) {
        if (is_sys(o)) return
        if (is_nonsys(o)) {
          if (nonsys == "") nonsys = o " " p
          return
        }
        if (o in svc) {
          if (nofileset == "") nofileset = o " " p
          return
        }
        if (manual == "") manual = o
      }
      $0 == "===IDS===" { section = "ids"; next }
      $0 == "===BOS===" { section = "bos"; next }
      $0 == "===LS===" { section = "ls"; next }
      section == "ids" {
        gsub(/\r/, "")
      ...
      /^ls:/ {
        ...
        sub(/^ls:[ \t]*/, "", rest)
        colon = index(rest, ":")
        if (colon > 1) path = substr(rest, 1, colon - 1)
        else path = rest
        if ((path in req) && ($0 ~ /does not exist/ || $0 ~ /No such file/)) {
          miss[path] = 1
        } else {
          other_err = 1
        }
      ...
      }
      {
        ...
        mlen = length("" mode)
        if (mlen == 11) {
          if (substr(mode, 11, 1) != "+") { bad=1; next }
        ...
        if (NF < 9) { bad=1; next }
        if (substr(mode, 1, 1) !~ /^[-dlcbps]$/) { bad=1; next }
        if ($2 !~ /^[0-9][0-9]*$/) { bad=1; next }
        owner=$3
        if (owner !~ /^[A-Za-z_][A-Za-z0-9_.-]*$/ && owner !~ /^[0-9][0-9]*$/) {
          bad=1
          next
        }
        if (cur in req) drec[cur]++
        classify(owner, recpath())
      }
      END {
        nmiss=0; ntrust=0; mp=""
        for (d in req) {
          if (d in miss) { nmiss++; if (mp == "") mp = d }
          if ((d in seen) && drec[d] > 0) ntrust++
        }
        if (nmiss > 0) {
          if (nmiss == 7) print "absent"
          else print "absent " mp
          exit 0
        }
        if (other_err) { print "probe_error"; exit 0 }
        if (summary > 0 && records == 0) {
          if (nonsys != "") { print "fail " nonsys; exit 0 }
          if (nofileset != "") { print "nofileset " nofileset; exit 0 }
          if (manual != "") { print "manual " manual; exit 0 }
          print "pass"
          exit 0
        }
        if (bad) { print "unparseable"; exit 1 }
        if (records == 0) { print "unparseable"; exit 1 }
        if (nonsys != "") { print "fail " nonsys; exit 0 }
        if (nofileset != "") { print "nofileset " nofileset; exit 0 }
        if (manual != "") { print "manual " manual; exit 0 }
        if (ntrust != 7) { print "incomplete"; exit 0 }
        print "pass"
      }
    ')
    SSFO_PARSE_RC=$?
    if [ "$SSFO_PARSE_RC" -ne 0 ] || [ -z "$SSFO_CLASS" ]; then
      SSFO_KIND=unparseable
    else
      SSFO_KIND=${SSFO_CLASS%% *}
    fi
  fi

  if [ "$SSFO_KIND" = incomplete ] && [ "$SSFO_RC" -ne 0 ]; then
    SSFO_KIND=probe_failed
  fi
  if [ "$SSFO_KIND" = probe_error ] || { [ "$SSFO_KIND" = pass ] && [ "$SSFO_RC" -ne 0 ]; }; then
    SSFO_KIND=probe_failed
  fi

  case "$SSFO_KIND" in
    pass) SSFO_STATUS=PASS
      SSFO_OBSERVED="all listed system files owned by system accounts (UID 0-128 or shipped-service bos.* fileset)"
      SSFO_MEANING="System files, programs, and directories in the STIG listing are owned by system accounts."
      SSFO_FIX="n/a"
      ;;
    fail) SSFO_STATUS=FAIL
      SSFO_OWNER=${SSFO_CLASS#fail }
      SSFO_PATH=${SSFO_OWNER#* }
      SSFO_OWNER=${SSFO_OWNER%% *}
      SSFO_OBSERVED="non-system owner $SSFO_OWNER on $SSFO_PATH"
      SSFO_MEANING="A listed system file, program, or directory is not owned by a system account, so that account can replace it."
      SSFO_FIX="chown root <public/directory>; PTxray only recommends this command."
      ;;
    absent) SSFO_STATUS=FAIL
      SSFO_PATH=${SSFO_CLASS#absent}
      SSFO_PATH=${SSFO_PATH# }
      if [ -n "$SSFO_PATH" ]; then
        SSFO_OBSERVED="required system directory $SSFO_PATH is absent"
      else
        SSFO_OBSERVED="required system directories are absent"
      fi
      SSFO_MEANING="The required system directories are missing, so the ownership requirement is unmet."
      SSFO_FIX="restore /etc /bin /usr/bin /usr/lbin /usr/ucb /sbin /usr/sbin then chown root <public/directory>; PTxray only recommends these commands."
      ;;
    probe_failed) SSFO_STATUS=NOT_ASSESSED
      SSFO_OBSERVED="not assessed - system file listing failed (rc=$SSFO_RC)"
      SSFO_MEANING="PTxray did not obtain trustworthy ownership evidence for the STIG system directories."
      SSFO_FIX="run 'ls -lLa /etc /bin /usr/bin /usr/lbin /usr/ucb /sbin /usr/sbin' and rerun PTxray."
      ;;
    probe_empty) SSFO_STATUS=NOT_ASSESSED
      SSFO_OBSERVED="not assessed - system file listing returned no output"
      SSFO_MEANING="PTxray did not obtain trustworthy ownership evidence for the STIG system directories."
      SSFO_FIX="run 'ls -lLa /etc /bin /usr/bin /usr/lbin /usr/ucb /sbin /usr/sbin' and rerun PTxray."
      ;;
    incomplete) SSFO_STATUS=NOT_ASSESSED
      SSFO_OBSERVED="not assessed - system file listing is incomplete"
      SSFO_MEANING="PTxray did not obtain trustworthy ownership records for every required system directory."
      SSFO_FIX="run 'ls -lLa /etc /bin /usr/bin /usr/lbin /usr/ucb /sbin /usr/sbin' and rerun PTxray."
      ;;
    manual) SSFO_STATUS=NOT_ASSESSED
      SSFO_OWNER=${SSFO_CLASS#manual }
      SSFO_OBSERVED="not assessed - manual portion: ISSO must classify whether owner $SSFO_OWNER is a system account"
      SSFO_MEANING="lsuser did not resolve owner $SSFO_OWNER, so that name stays unassessed."
      SSFO_FIX="resolve the owner with 'lsuser -a id' or change it with 'chown root <public/directory>'; PTxray only recommends this command."
      ;;
    nofileset) SSFO_STATUS=NOT_ASSESSED
      SSFO_OWNER=${SSFO_CLASS#nofileset }
      SSFO_PATH=${SSFO_OWNER#* }
      SSFO_OWNER=${SSFO_OWNER%% *}
      SSFO_OBSERVED="not assessed - owner $SSFO_OWNER on $SSFO_PATH: no lslpp -w fileset evidence for this path"
      SSFO_MEANING="Shipped-service owner $SSFO_OWNER has no captured lslpp -w bos.* fileset for this path."
      SSFO_FIX="run 'lslpp -w $SSFO_PATH' or 'chown root $SSFO_PATH'; PTxray only recommends these commands."
      ;;
    aux_auth) SSFO_STATUS=NOT_ASSESSED
      SSFO_OBSERVED="not assessed - $SSFO_AUXCMD failed: authority gap"
      SSFO_MEANING="The scan user cannot run $SSFO_AUXCMD, so ownership cannot be graded."
      SSFO_FIX="run '$SSFO_AUXCMD' with a profile that can read that data, then rerun PTxray."
      ;;
    aux_fail) SSFO_STATUS=NOT_ASSESSED
      SSFO_OBSERVED="not assessed - $SSFO_AUXCMD failed (rc=$SSFO_AUXRC)"
      SSFO_MEANING="PTxray did not obtain trustworthy $SSFO_AUXCMD evidence."
      SSFO_FIX="run '$SSFO_AUXCMD' and rerun PTxray."
      ;;
    *)
      SSFO_STATUS=NOT_ASSESSED
      SSFO_OBSERVED="not assessed - system file listing was unparseable"
      SSFO_MEANING="PTxray did not obtain trustworthy ownership evidence for the STIG system directories."
      SSFO_FIX="run 'ls -lLa /etc /bin /usr/bin /usr/lbin /usr/ucb /sbin /usr/sbin' and rerun PTxray."
      ;;
  esac
  ...
  case "$SSFO_STATUS" in
    NOT_APPLICABLE) SSFO_SEV=low ;;
    *) SSFO_SEV=med ;;
  esac

  add security stig_sysfile_owner "System file owner is a system account" \
    "$SSFO_STATUS" "$SSFO_SEV" "$SSFO_OBSERVED" "$SSFO_MEANING" "$SSFO_FIX" \
    "stig:V-215183"
}
