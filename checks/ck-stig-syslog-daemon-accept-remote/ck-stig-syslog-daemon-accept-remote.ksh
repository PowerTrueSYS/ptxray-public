...
AIXRAY_TOOL=ck-stig-syslog-daemon-accept-remote


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_syslog_daemon_accept_remote — DISA STIG V-215313.
  # Decidable: syslogd with -R is PASS; syslogd with no -R (and no -r) is FAIL.
  # Determinate absence is pipeline rc=1 with empty output (PASS / svcoff).
  # rc=0 nonempty output needs a structurally valid ps -ef record whose
  # command/executable field has basename syslogd; otherwise NOT_ASSESSED,
  # never ABSENT-to-PASS. The site/ISSO "documented syslog server" exception
  # is never auto-PASS: -r without -R renders typed NOT_ASSESSED.
  typeset SSDA_RAW SSDA_RC SSDA_PARSED SSDA_KIND
  typeset SSDA_STATUS SSDA_SEV SSDA_OBSERVED SSDA_MEANING SSDA_FIX
  typeset SSDA_FAILCLOSED_PROBE SSDA_FAILCLOSED_EMPTY SSDA_FAILCLOSED_MANUAL
  typeset SSDA_FAILCLOSED_UNPARSE

  SSDA_FAILCLOSED_PROBE=NOT_ASSESSED
  SSDA_FAILCLOSED_EMPTY=NOT_ASSESSED
  SSDA_FAILCLOSED_MANUAL=NOT_ASSESSED
  SSDA_FAILCLOSED_UNPARSE=NOT_ASSESSED

  # Pipeline runs inside aix via sh -c; a quoted pipeline is not an executable.
  SSDA_RAW=$(aix syslogd_ps sh -c 'ps -ef | grep syslogd | grep -v grep')
  SSDA_RC=$?

  if [ "$SSDA_RC" -ne 0 ]; then
    if [ "$SSDA_RC" -eq 1 ] && [ -z "$SSDA_RAW" ]; then
      SSDA_STATUS=PASS
      SSDA_OBSERVED="no syslogd process present; remote-accept prohibition holds"
    else
      SSDA_STATUS=$SSDA_FAILCLOSED_PROBE
      SSDA_OBSERVED="not assessed - syslogd process-list probe failed (rc=$SSDA_RC)"
    fi
  elif [ -z "$SSDA_RAW" ]; then
    SSDA_STATUS=$SSDA_FAILCLOSED_EMPTY
    SSDA_OBSERVED="not assessed - syslogd process-list probe returned empty output with rc=0"
  else
    SSDA_PARSED=$(printf '%s\n' "$SSDA_RAW" | awk '
      /^[ \t]*#/ { next }
      /^[ \t]*$/ { next }
      /grep/ { next }
      {
        ...
        if ($4 !~ /^[0-9]+$/) next
        cmd_idx = 0
        if ($5 ~ /^(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)$/ && $6 ~ /^[0-9]+$/) {
          cmd_idx = 9
        } else if ($5 ~ /^[0-9]+:[0-9][0-9](:[0-9][0-9])?$/) {
          cmd_idx = 8
        } else {
          next
        }
        if (NF < cmd_idx) next
        time_idx = cmd_idx - 1
        if ($time_idx !~ /^[0-9]+:[0-9][0-9](:[0-9][0-9])?$/) next
        cmd_base = $cmd_idx
        sub(/.*\//, "", cmd_base)
        if (cmd_base != "syslogd") next
        seen++
        has_R = 0
        has_r = 0
        for (i = cmd_idx + 1; i <= NF; i++) {
          if ($i == "-R") has_R = 1
          if ($i == "-r") has_r = 1
        }
        if (has_R) next
        if (has_r) { manual_count++; next }
        fail_count++
      }
      END {
        if (fail_count) { print "FAIL"; exit }
        if (manual_count) { print "MANUAL"; exit }
        if (seen == 0) { print "UNPARSEABLE"; exit }
        print "PASS"
      }
    ')
    SSDA_KIND=$SSDA_PARSED
    case "$SSDA_KIND" in
      FAIL) SSDA_STATUS=FAIL
        SSDA_OBSERVED="syslogd running without -R"
        ;;
      MANUAL)
        SSDA_STATUS=$SSDA_FAILCLOSED_MANUAL
        SSDA_OBSERVED="not assessed - manual portion: ISSO must document whether this host is a syslog server using site-defined procedures"
        ;;
      UNPARSEABLE)
        SSDA_STATUS=$SSDA_FAILCLOSED_UNPARSE
        SSDA_OBSERVED="not assessed - syslogd process-list evidence was unparseable"
        ;;
      PASS)
        SSDA_STATUS=PASS
        SSDA_OBSERVED="syslogd running with -R (remote messages not accepted)"
        ;;
      *)
        SSDA_STATUS=$SSDA_FAILCLOSED_UNPARSE
        SSDA_OBSERVED="not assessed - syslogd process-list evidence was unparseable"
  ...
  case "$SSDA_STATUS" in
    PASS)
      SSDA_MEANING="The syslogd daemon is not accepting remote messages, or is not running, so the V-215313 prohibition holds."
      SSDA_FIX="n/a"
      ;;
    FAIL)
      SSDA_MEANING="The syslogd daemon is running without -R, so it accepts remote messages."
      SSDA_FIX="chssys -s syslogd -a '-R'; stopsrc -s syslogd; startsrc -s syslogd; PTxray only recommends these commands."
      ;;
    *)
      SSDA_MEANING="PTxray could not finish the syslogd remote-accept assessment: the probe failed, returned empty output, was unparseable, or the documented-syslog-server exception needs ISSO review."
      SSDA_FIX="verify 'ps -ef | grep syslogd | grep -v grep' returns a complete process list and have the ISSO document whether this host is a syslog server using site-defined procedures, then rerun PTxray."
      ;;
  esac

  case "$SSDA_STATUS" in
    NOT_APPLICABLE) SSDA_SEV=low ;;
    *) SSDA_SEV=med ;;
  esac

  add security stig_syslog_daemon_accept_remote "syslogd must not accept remote messages" \
    "$SSDA_STATUS" "$SSDA_SEV" "$SSDA_OBSERVED" \
    "$SSDA_MEANING" "$SSDA_FIX" "stig:V-215313"
}
