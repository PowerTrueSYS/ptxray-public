...
AIXRAY_TOOL=ck-stig-syslog-mail-facility


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_syslog_mail_facility — DISA STIG V-215314.
  # TCPD events are logged through syslogd when /etc/syslog.conf has an
  # uncommented mail. selector. Missing syslog.conf is unmet required
  # configuration (FAIL), not a refusal. Permission/I/O probe errors and
  # unparseable content are refusals.
  typeset SMF_RAW SMF_RC SMF_CLASS SMF_PARSE_RC
  typeset SMF_STATUS SMF_OBSERVED SMF_MEANING SMF_FIX

  SMF_RAW=$(aix syslog_conf more /etc/syslog.conf)
  SMF_RC=$?
  SMF_STATUS=""
  SMF_OBSERVED=""
  ...
  if [ -z "$SMF_RAW" ]; then
    if [ "$SMF_RC" -eq 0 ]; then
      ...
      SMF_OBSERVED="not assessed - syslog.conf probe returned no output"
    else
      SMF_STATUS=NOT_ASSESSED
      SMF_OBSERVED="not assessed - syslog.conf probe failed (rc=$SMF_RC)"
    fi
  elif [ "$SMF_RC" -ne 0 ]; then
    case "$SMF_RAW" in
      *"No such file"*|*"does not exist"*)
        SMF_STATUS=FAIL
        SMF_OBSERVED="/etc/syslog.conf is absent"
        ;;
      *)
        SMF_STATUS=NOT_ASSESSED
        SMF_OBSERVED="not assessed - syslog.conf probe failed (rc=$SMF_RC)"
    ...
    # network-lint: allow-next=4 -- awk program validating syslog facility names; 'ftp' is one of the syslog facility tokens in that list; a trailing comment would corrupt the awk source
    SMF_CLASS=$(printf '%s\n' "$SMF_RAW" | awk '
      function is_facility(f) {
        # Finite set: BSD syslog facilities, local0-7, AIX aso/caa.
        return f ~ /^(\*|kern|user|mail|daemon|auth|syslog|lpr|news|uucp|cron|ftp|authpriv|mark|local[0-7]|aso|caa)$/
      }
      function is_level(lv) {
        if (lv ~ /^[!=]/) lv = substr(lv, 2)
        return lv ~ /^(\*|emerg|alert|crit|err|error|warning|warn|notice|info|debug|none|panic)$/
      }
      function valid_selector(sel,    n, i, pair, dot, facs, pri, nf, j, f) {
        if (sel == "") return 0
        n = split(sel, parts, ";")
        for (i = 1; i <= n; i++) {
          pair = parts[i]
          ...
          dot = index(pair, ".")
          if (dot < 2) return 0
          facs = substr(pair, 1, dot - 1)
          pri = substr(pair, dot + 1)
          if (facs == "" || pri == "" || index(pri, ".") != 0) return 0
          if (!is_level(pri)) return 0
          nf = split(facs, flist, ",")
          if (nf < 1) return 0
          for (j = 1; j <= nf; j++) {
            f = flist[j]
            if (!is_facility(f)) return 0
            if (f == "mail") has_mail = 1
          }
        }
        return 1
      }
      ...
      {
        if (NF < 2) { bad = 1; next }
        if (!valid_selector($1)) { bad = 1; next }
      }
      END {
        if (bad) print "unparseable"
        else if (has_mail) print "present"
        else print "absent"
      }
    ')
    SMF_PARSE_RC=$?
    if [ "$SMF_PARSE_RC" -ne 0 ] || [ -z "$SMF_CLASS" ]; then
      SMF_STATUS=NOT_ASSESSED
      SMF_OBSERVED="not assessed - syslog.conf output was unparseable"
    elif [ "$SMF_CLASS" = "present" ]; then
      SMF_STATUS=PASS
      SMF_OBSERVED="uncommented mail. selector present in /etc/syslog.conf"
    elif [ "$SMF_CLASS" = "absent" ]; then
      SMF_STATUS=FAIL
      SMF_OBSERVED="no uncommented mail. selector in /etc/syslog.conf"
    else
      SMF_STATUS=NOT_ASSESSED
      SMF_OBSERVED="not assessed - syslog.conf output was unparseable"
    fi
  ...
  case "$SMF_STATUS" in
    PASS)
      SMF_MEANING="syslogd has an uncommented mail. selector, so TCPD events can be logged."
      SMF_FIX="n/a"
      ;;
    FAIL)
      SMF_MEANING="TCPD is not logging through syslogd: /etc/syslog.conf has no uncommented mail. selector, or the file is absent."
      SMF_FIX="touch /var/log/messages; refresh -s syslogd"
      ;;
    *)
      SMF_MEANING="PTxray did not obtain trustworthy evidence for the syslogd mail. selector."
      SMF_FIX="verify more /etc/syslog.conf is readable, then rerun PTxray."
      ;;
  esac

  add security stig_syslog_mail_facility "TCPD syslog mail facility" \
    "$SMF_STATUS" med "$SMF_OBSERVED" "$SMF_MEANING" "$SMF_FIX" \
    "stig:V-215314"
}
