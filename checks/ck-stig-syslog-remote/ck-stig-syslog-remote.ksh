...
AIXRAY_TOOL=ck-stig-syslog-remote


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_syslog_remote — DISA STIG V-215312. PARTIAL-AUTO.
  # Decidable: uncommented action token in /etc/syslog.conf whose
  # destination syntax begins with @. Missing, local-only, or
  # column-one-commented @loghost is unmet required configuration (FAIL).
  # ISSO/SA documentation is never inferred from syslog comments; it is
  # taken only from PTXRAY_STIG_SYSLOG_REMOTE_DOCUMENTED=confirmed.
  # grep -v "^#" /etc/syslog.conf | grep '@'
  typeset SSR_RAW SSR_RC SSR_CLASS SSR_PARSE_RC SSR_REASON SSR_DOC
  typeset SSR_STATUS SSR_SEV SSR_OBSERVED SSR_MEANING SSR_FIX

  SSR_RAW=$(aix syslog_conf cat /etc/syslog.conf)
  SSR_RC=$?
  SSR_CLASS=""
  SSR_REASON=""
  SSR_DOC=${PTXRAY_STIG_SYSLOG_REMOTE_DOCUMENTED-}

  if [ "$SSR_RC" -ne 0 ]; then
    SSR_REASON=probe_failed
  elif [ -z "$SSR_RAW" ]; then
    SSR_REASON=probe_empty
  else
    SSR_CLASS=$(printf '%s\n' "$SSR_RAW" | awk '
      function remote_action(s,    n, tok, i, nf, sel, act) {
        n = split(s, tok)
        sel = ""
        act = ""
        nf = 0
        for (i = 1; i <= n; i++) {
          ...
          nf++
          if (nf == 1) sel = tok[i]
          else if (nf == 2) act = tok[i]
        }
        if (sel == "" || act == "" || index(sel, ".") == 0) return 0
        if (act ~ /^@./) return 1
        return 0
      }
      {
        ...
        if (substr(line, 1, 1) == "#") {
          body = substr(line, 2)
          sub(/^[ \t]+/, "", body)
          if (remote_action(body)) commented = 1
          next
        }
        hashpos = index(line, "#")
        if (hashpos > 0)
          code = substr(line, 1, hashpos - 1)
        else
          code = line
        n = split(code, tok)
        sel = ""
      ...
      END {
        ...
        else if (commented) print "commented"
        else print "absent"
      }
    ')
    SSR_PARSE_RC=$?
    if [ "$SSR_PARSE_RC" -ne 0 ] || [ -z "$SSR_CLASS" ]; then
      SSR_REASON=unparseable
    else
      case "$SSR_CLASS" in
        present)
          if [ "$SSR_DOC" = confirmed ]; then
            SSR_REASON=compliant
          else
            SSR_REASON=manual_portion
          fi
          ;;
        commented) SSR_REASON=noncompliant ;;
        absent) SSR_REASON=subject_absent ;;
        *) SSR_REASON=unparseable ;;
      esac
    fi
  fi
  ...
  case "$SSR_REASON" in
    compliant)
      SSR_STATUS=PASS
      SSR_OBSERVED="uncommented @loghost present; ISSO documentation confirmed"
      SSR_MEANING="syslog.conf forwards to a remote log host and ISSO/SA documentation was confirmed on the trusted interface."
      SSR_FIX="n/a"
      ;;
    noncompliant) SSR_STATUS=FAIL
      SSR_OBSERVED="no uncommented @loghost (commented out)"
      SSR_MEANING="The remote syslog destination is commented out, so logs are not forwarded off the box."
      SSR_FIX="Edit the /etc/syslog.conf file to include a documented and approved remote log host."
      ;;
    subject_absent) SSR_STATUS=FAIL
      SSR_OBSERVED="no uncommented @loghost (subject absent)"
      SSR_MEANING="No remote syslog destination is defined, so logs are not forwarded off the box."
      SSR_FIX="Edit the /etc/syslog.conf file to include a documented and approved remote log host."
      ;;
    probe_failed) SSR_STATUS=NOT_ASSESSED
      SSR_OBSERVED="not assessed - syslog.conf probe failed (rc=$SSR_RC)"
      SSR_MEANING="PTxray did not obtain trustworthy /etc/syslog.conf evidence."
      SSR_FIX="verify that cat /etc/syslog.conf runs, then rerun PTxray."
      ;;
    probe_empty) SSR_STATUS=NOT_ASSESSED
      SSR_OBSERVED="not assessed - syslog.conf probe returned no output"
      SSR_MEANING="PTxray did not obtain trustworthy /etc/syslog.conf evidence."
      SSR_FIX="verify that cat /etc/syslog.conf returns complete evidence, then rerun PTxray."
      ;;
    manual_portion) SSR_STATUS=NOT_ASSESSED
      SSR_OBSERVED="not assessed - Ask ISSO/SA for a list of valid remote syslog servers justified and documented"
      SSR_MEANING="An uncommented @loghost is present, but whether that host is a documented, justified, approved remote syslog server is a site/ISSO question."
      SSR_FIX="Ask ISSO/SA for a list of valid remote syslog servers justified and documented, then confirm /etc/syslog.conf names one of them."
      ;;
    *)
      SSR_STATUS=NOT_ASSESSED
      SSR_OBSERVED="not assessed - syslog.conf destination output was unparseable"
      SSR_MEANING="PTxray did not obtain trustworthy /etc/syslog.conf destination evidence."
      SSR_FIX="verify uncommented @ destination lines in /etc/syslog.conf, then rerun PTxray."
      ;;
  esac

  case "$SSR_STATUS" in
    NOT_APPLICABLE) SSR_SEV=low ;;
    *) SSR_SEV=med ;;
  esac

  add security stig_syslog_remote "Remote syslog server" \
    "$SSR_STATUS" "$SSR_SEV" "$SSR_OBSERVED" \
    "$SSR_MEANING" "$SSR_FIX" "stig:V-215312"
}
