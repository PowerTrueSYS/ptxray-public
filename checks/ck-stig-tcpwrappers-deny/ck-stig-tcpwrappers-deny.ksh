...
AIXRAY_TOOL=ck-stig-tcpwrappers-deny


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_tcpwrappers_deny — DISA STIG V-215419.
  # Required configuration: /etc/hosts.allow and /etc/hosts.deny must exist,
  # and hosts.deny must contain ALL:ALL (optional blanks after the colon).
  # Missing either file is FAIL, not a refusal. aix() discards ls stderr;
  # empty stdout at ls rc 1 or 2 is the AIX missing-file shape (absence
  # diagnostic was on stderr) and is determinate absence. ls rc 1/2 with
  # listing rows is graded; a clean PASS also notes some paths unreadable.
  # Grep rc=0 with a complete non-comment ALL:ALL token pair may PASS;
  # rc=1 empty is FAIL; any other nonzero grep rc is NOT_ASSESSED even
  # when output contains ALL: ALL. Probe rc 0 with empty output is
  # NOT_ASSESSED.
  typeset TWD_ALLOW_RAW TWD_ALLOW_RC TWD_DENY_RAW TWD_DENY_RC
  typeset TWD_GREP_RAW TWD_GREP_RC TWD_ALLOW_STATE TWD_DENY_STATE
  typeset TWD_GREP_HAS TWD_ABSENT TWD_GREP_EMPTY TWD_PARTIAL
  typeset TWD_STATUS TWD_SEV TWD_OBSERVED TWD_MEANING TWD_FIX
  typeset TWD_FAILCLOSED_PROBE TWD_FAILCLOSED_EMPTY
  typeset TWD_FAILCLOSED_MISSING TWD_FAILCLOSED_NOALL

  TWD_FAILCLOSED_PROBE=NOT_ASSESSED
  TWD_FAILCLOSED_EMPTY=NOT_ASSESSED
  TWD_FAILCLOSED_MISSING=FAIL
  TWD_FAILCLOSED_NOALL=FAIL

  TWD_ALLOW_RAW=$(aix hosts_allow_ls ls -la /etc/hosts.allow)
  TWD_ALLOW_RC=$?
  TWD_DENY_RAW=$(aix hosts_deny_ls ls -la /etc/hosts.deny)
  TWD_DENY_RC=$?
  TWD_GREP_RAW=$(aix hosts_deny_grep grep -E "ALL:[[:blank:]]*ALL" /etc/hosts.deny)
  TWD_GREP_RC=$?

  TWD_ALLOW_STATE=$(printf '%s\n' "$TWD_ALLOW_RAW" | awk -v rc="$TWD_ALLOW_RC" '
    /^[ \t]*$/ { next }
    /^[ \t]*#/ { next }
    /No such file or directory/ { miss=1; next }
    /does not exist/ { miss=1; next }
    /[Aa]bsent/ { miss=1; next }
    NF { rows++ }
    END {
      if (miss) { print "missing"; exit }
      if (rows) {
        if (rc == 0 || rc == 1 || rc == 2) { print "present"; exit }
        print "failed"; exit
      }
      if (rc == 0) { print "empty"; exit }
      if (rc == 1 || rc == 2) { print "missing"; exit }
      print "failed"
    }
  ')
  TWD_DENY_STATE=$(printf '%s\n' "$TWD_DENY_RAW" | awk -v rc="$TWD_DENY_RC" '
    /^[ \t]*$/ { next }
    /^[ \t]*#/ { next }
    /No such file or directory/ { miss=1; next }
    /does not exist/ { miss=1; next }
  ...
  TWD_GREP_HAS=no
  if [ -n "$TWD_GREP_RAW" ]; then
    TWD_GREP_HAS=$(printf '%s\n' "$TWD_GREP_RAW" | awk '
      /^[ \t]*$/ { next }
      /^[ \t]*#/ { next }
      {
        line = $0
  ...
  TWD_ABSENT=""
  TWD_PARTIAL=0
  case "$TWD_ALLOW_RC" in
    1|2) [ "$TWD_ALLOW_STATE" = present ] && TWD_PARTIAL=1 ;;
  esac
  case "$TWD_DENY_RC" in
    1|2) [ "$TWD_DENY_STATE" = present ] && TWD_PARTIAL=1 ;;
  esac

  TWD_GREP_EMPTY=0
  if [ "$TWD_GREP_RC" -eq 0 ] && [ -z "$TWD_GREP_RAW" ]; then
    TWD_GREP_EMPTY=1
  fi
  if [ "$TWD_ALLOW_STATE" = empty ] || [ "$TWD_DENY_STATE" = empty ] || \
      [ "$TWD_GREP_EMPTY" -eq 1 ]; then
    TWD_STATUS=$TWD_FAILCLOSED_EMPTY
    if [ "$TWD_ALLOW_STATE" = empty ]; then
      TWD_OBSERVED="not assessed - hosts.allow listing returned empty output with rc=0"
    elif [ "$TWD_DENY_STATE" = empty ]; then
      TWD_OBSERVED="not assessed - hosts.deny listing returned empty output with rc=0"
    else
      TWD_OBSERVED="not assessed - hosts.deny grep returned empty output with rc=0"
    fi
  elif [ "$TWD_ALLOW_STATE" = missing ] || [ "$TWD_DENY_STATE" = missing ]; then
    TWD_STATUS=$TWD_FAILCLOSED_MISSING
    if [ "$TWD_ALLOW_STATE" = missing ] && [ "$TWD_DENY_STATE" = missing ]; then
      TWD_ABSENT="hosts.allow and hosts.deny are absent"
    elif [ "$TWD_ALLOW_STATE" = missing ]; then
      TWD_ABSENT="hosts.allow is absent"
    else
      TWD_ABSENT="hosts.deny is absent"
    fi
    TWD_OBSERVED="$TWD_ABSENT"
  elif [ "$TWD_ALLOW_STATE" = failed ] || [ "$TWD_DENY_STATE" = failed ]; then
    TWD_STATUS=$TWD_FAILCLOSED_PROBE
    if [ "$TWD_ALLOW_STATE" = failed ]; then
      TWD_OBSERVED="not assessed - hosts.allow listing failed (rc=$TWD_ALLOW_RC)"
    else
      TWD_OBSERVED="not assessed - hosts.deny listing failed (rc=$TWD_DENY_RC)"
    fi
  elif [ "$TWD_GREP_RC" -eq 0 ] && [ "$TWD_GREP_HAS" = yes ]; then
    TWD_STATUS=PASS
    TWD_OBSERVED="hosts.allow and hosts.deny exist; hosts.deny contains ALL:ALL"
    if [ "$TWD_PARTIAL" -eq 1 ]; then
      TWD_OBSERVED="$TWD_OBSERVED; some paths unreadable"
    fi
  elif [ "$TWD_GREP_RC" -eq 1 ] && [ -z "$TWD_GREP_RAW" ]; then
    TWD_STATUS=$TWD_FAILCLOSED_NOALL
    TWD_OBSERVED="hosts.deny does not contain ALL:ALL"
  elif [ "$TWD_GREP_RC" -eq 0 ]; then
    TWD_STATUS=$TWD_FAILCLOSED_NOALL
    TWD_OBSERVED="hosts.deny does not contain ALL:ALL"
  else
    TWD_STATUS=$TWD_FAILCLOSED_PROBE
    TWD_OBSERVED="not assessed - hosts.deny grep probe failed (rc=$TWD_GREP_RC)"
  fi

  case "$TWD_STATUS" in
    PASS)
      TWD_SEV=med
      TWD_MEANING="TCP wrappers deny hosts not listed in hosts.allow because hosts.deny contains ALL:ALL."
      TWD_FIX="n/a"
      ;;
    FAIL)
      TWD_SEV=med
      TWD_MEANING="TCP wrappers are not configured to deny unlisted hosts, so any host may be granted access."
      TWD_FIX="Edit /etc/hosts.allow and /etc/hosts.deny to configure access restrictions; add ALL: ALL to hosts.deny."
      ;;
    *)
      TWD_SEV=med
      TWD_MEANING="PTxray did not obtain trustworthy TCP wrappers evidence."
      TWD_FIX="verify ls -la /etc/hosts.allow, ls -la /etc/hosts.deny, and grep -E \"ALL:[[:blank:]]*ALL\" /etc/hosts.deny, then rerun PTxray."
      ;;
  esac

  add security stig_tcpwrappers_deny "TCP wrappers hosts.deny ALL:ALL" \
    "$TWD_STATUS" "$TWD_SEV" "$TWD_OBSERVED" \
    "$TWD_MEANING" "$TWD_FIX" "stig:V-215419"
}
