...
AIXRAY_TOOL=ck-stig-telnet-subserver


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_telnet_subserver — DISA STIG V-215213.
  # Telnet subserver must not be active. lssrc rc 1 (not on file) is
  # determinate absence and PASS. Uncertain probe evidence is refused.
  # A missing subject at rc 0 is a required-configuration finding.
  typeset TNS_RAW TNS_RC TNS_PARSED TNS_PARSE_RC TNS_CLASS
  typeset TNS_STATUS TNS_SEV TNS_OBSERVED TNS_MEANING TNS_FIX

  TNS_RAW=$(aix lssrc_telnet lssrc -t telnet)  # network-lint: allow -- aix() capture of a local read-only command; 'telnet' is the subsystem-name argument to lssrc; no network call
  TNS_RC=$?
  TNS_CLASS=""

  if [ "$TNS_RC" -eq 127 ]; then
    TNS_CLASS=probe_failed
  elif [ -z "$TNS_RAW" ]; then
    if [ "$TNS_RC" -eq 0 ]; then
      TNS_CLASS=probe_empty
    else
      TNS_CLASS=probe_failed
    fi
  else
    # network-lint: allow-next=15 -- awk program parsing the lssrc -t telnet output; 'telnet' occurs only as the subsystem name being matched; a trailing comment would corrupt the awk source
    TNS_PARSED=$(printf '%s\n' "$TNS_RAW" | awk '
      {
        gsub(/\r/, "")
        if ($0 ~ /^[ \t]*$/) next
        if ($1 ~ /^0513-[0-9][0-9][0-9]$/ && $0 ~ /not found|not on file/) {
          absent = 1
          next
        }
        if ($1 == "Subsystem" && $NF == "Status") {
          next
        }
        if ($1 == "Subserver") {
          next
        }
        if ($1 == "telnet" || $1 == "telnetd") {
          seen = 1
          if ($NF == "active") active = 1
          else if ($NF == "inoperative") inop = 1
          else bad = 1
          next
        }
        bad = 1
      ...
      END {
        if (bad || (absent && seen) || (active && inop)) {
          print "unparseable"
          exit 0
        }
        if (absent) print "absent"
        else if (active) print "active"
        else if (seen && inop) print "ok"
        else print "unparseable"
      }
    ')
    TNS_PARSE_RC=$?
    if [ "$TNS_PARSE_RC" -ne 0 ] || [ -z "$TNS_PARSED" ]; then
      TNS_CLASS=unparseable
    else
      case "$TNS_PARSED" in
        ok) TNS_CLASS=ok ;;
        active) TNS_CLASS=noncompliant ;;
        absent)
          if [ "$TNS_RC" -eq 1 ]; then
            TNS_CLASS=not_on_file
          else
            TNS_CLASS=subject_absent
          fi
          ;;
        *) TNS_CLASS=unparseable ;;
      esac
  ...
  case "$TNS_CLASS" in
    ok)
      TNS_STATUS=PASS
      TNS_OBSERVED="telnet subserver is not active"  # network-lint: allow -- observed-state report string; 'telnet' names the STIG subject service in prose; no network call
      TNS_MEANING="The telnet subserver is present and inoperative, so nonlocal maintenance is not using cleartext telnet."  # network-lint: allow -- finding meaning text; 'telnet' names the STIG subject service in prose; no network call
      TNS_FIX="n/a"
      ;;
    not_on_file) TNS_STATUS=PASS
      TNS_OBSERVED="telnet subserver is not on file"  # network-lint: allow -- observed-state report string; 'telnet' names the STIG subject service in prose; no network call
      TNS_MEANING="The telnet subserver type is not on file, so nonlocal maintenance is not using cleartext telnet."  # network-lint: allow -- finding meaning text; 'telnet' names the STIG subject service in prose; no network call
      TNS_FIX="n/a"
      ;;
    noncompliant) TNS_STATUS=FAIL
      TNS_OBSERVED="telnet subserver is active"  # network-lint: allow -- observed-state report string; 'telnet' names the STIG subject service in prose; no network call
      TNS_MEANING="The telnet subserver is active, so nonlocal maintenance can use a cleartext authenticator."  # network-lint: allow -- finding meaning text; 'telnet' names the STIG subject service in prose; no network call
      TNS_FIX="stopsrc -t telnet; PTxray only recommends this command."  # network-lint: allow -- remediation advice text PTxray never executes; 'telnet' names the STIG subject service in prose; no network call
      ;;
    subject_absent) TNS_STATUS=FAIL
      TNS_OBSERVED="required telnet subserver subject is absent"  # network-lint: allow -- observed-state report string; 'telnet' names the STIG subject service in prose; no network call
      TNS_MEANING="The telnet subserver is not defined, so the required configuration for this control is unmet."  # network-lint: allow -- finding meaning text; 'telnet' names the STIG subject service in prose; no network call
      TNS_FIX="stopsrc -t telnet; PTxray only recommends this command."  # network-lint: allow -- remediation advice text PTxray never executes; 'telnet' names the STIG subject service in prose; no network call
      ;;
    probe_failed) TNS_STATUS=NOT_ASSESSED
      TNS_OBSERVED="not assessed - lssrc telnet probe failed (rc=$TNS_RC)"  # network-lint: allow -- observed-state report string; 'telnet' is the subsystem-name argument to lssrc; no network call
      TNS_MEANING="PTxray did not obtain trustworthy telnet subserver evidence."  # network-lint: allow -- finding meaning text; 'telnet' names the STIG subject service in prose; no network call
      TNS_FIX="run 'lssrc -t telnet', correct the capture problem, and rerun PTxray."  # network-lint: allow -- remediation advice text PTxray never executes; 'telnet' is the subsystem-name argument to lssrc; no network call
      ;;
    probe_empty) TNS_STATUS=NOT_ASSESSED
      TNS_OBSERVED="not assessed - lssrc telnet probe returned no output"  # network-lint: allow -- observed-state report string; 'telnet' is the subsystem-name argument to lssrc; no network call
      TNS_MEANING="PTxray did not obtain trustworthy telnet subserver evidence."  # network-lint: allow -- finding meaning text; 'telnet' names the STIG subject service in prose; no network call
      TNS_FIX="run 'lssrc -t telnet', correct the capture problem, and rerun PTxray."  # network-lint: allow -- remediation advice text PTxray never executes; 'telnet' is the subsystem-name argument to lssrc; no network call
      ;;
    *)
      TNS_STATUS=NOT_ASSESSED
      TNS_OBSERVED="not assessed - lssrc telnet output was unparseable"  # network-lint: allow -- observed-state report string; 'telnet' is the subsystem-name argument to lssrc; no network call
      TNS_MEANING="PTxray did not obtain trustworthy telnet subserver evidence."  # network-lint: allow -- finding meaning text; 'telnet' names the STIG subject service in prose; no network call
      TNS_FIX="run 'lssrc -t telnet', correct the capture problem, and rerun PTxray."  # network-lint: allow -- remediation advice text PTxray never executes; 'telnet' is the subsystem-name argument to lssrc; no network call
      ;;
  esac
  ...
  case "$TNS_STATUS" in
    FAIL) TNS_SEV=high ;;
    NOT_APPLICABLE) TNS_SEV=low ;;
    *) TNS_SEV=med ;;
  esac

  add security stig_telnet_subserver "Telnet subserver must not be active" \
    "$TNS_STATUS" "$TNS_SEV" "$TNS_OBSERVED" \
    "$TNS_MEANING" "$TNS_FIX" "stig:V-215213"
}
