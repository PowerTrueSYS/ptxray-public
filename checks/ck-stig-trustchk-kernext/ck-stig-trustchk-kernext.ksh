...
AIXRAY_TOOL=ck-stig-trustchk-kernext


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_trustchk_kernext — DISA STIG V-215333.
  # Required configuration: TE, CHKEXEC, and CHKKERNEXT are all ON.
  # Uncertain probe evidence is refused; a missing or empty subject is a finding.
  # Empty output at rc 0 or the documented not-found rc (127) is determinate
  # absence. Other nonzero rc values remain a probe that could not run.
  typeset TKX_RAW TKX_RC TKX_PARSED TKX_PARSE_RC TKX_CLASS TKX_DETAIL
  typeset TKX_TE TKX_REST TKX_CHKEXEC TKX_CHKKERNEXT
  typeset TKX_STATUS TKX_SEV TKX_OBSERVED TKX_MEANING TKX_FIX
  typeset TKX_NOTFOUND=127

  TKX_RAW=$(aix trustchk_p trustchk -p)
  TKX_RC=$?
  TKX_CLASS=""
  TKX_DETAIL=""

  if [ -z "$TKX_RAW" ]; then
    if [ "$TKX_RC" -eq 0 ] || [ "$TKX_RC" -eq "$TKX_NOTFOUND" ]; then
      TKX_CLASS=probe_empty
    else
      TKX_CLASS=probe_failed
    fi
  elif [ "$TKX_RC" -ne 0 ]; then
    TKX_CLASS=probe_failed
  else
    TKX_PARSED=$(printf '%s\n' "$TKX_RAW" | awk '
      function classify(v) {
        if (v == "ON" || v == "on") return "ON"
        if (v == "OFF" || v == "off") return "OFF"
        return "BAD"
      }
      {
        gsub(/\r/, "")
        ...
        gsub(/[ \t]+$/, "", val)
        if (key == "TE") te = val
        else if (key == "CHKEXEC") chkexec = val
        else if (key == "CHKKERNEXT") chkkernext = val
      }
      END {
        if (te == "" || chkexec == "" || chkkernext == "") {
          print "absent"
          exit 0
        }
        t = classify(te)
        e = classify(chkexec)
        k = classify(chkkernext)
        if (t == "BAD" || e == "BAD" || k == "BAD") {
          print "unparseable"
          exit 0
        }
        if (t == "ON" && e == "ON" && k == "ON") print "ok " t " " e " " k
        else print "fail " t " " e " " k
      }
    ')
    TKX_PARSE_RC=$?
    if [ "$TKX_PARSE_RC" -ne 0 ] || [ -z "$TKX_PARSED" ]; then
      TKX_CLASS=unparseable
    else
      case "$TKX_PARSED" in
        ok\ *)
          TKX_CLASS=ok
          TKX_DETAIL=${TKX_PARSED#ok }
          ;;
        fail\ *)
          TKX_CLASS=fail
          TKX_DETAIL=${TKX_PARSED#fail }
          ;;
        absent) TKX_CLASS=subject_absent ;;
        *) TKX_CLASS=unparseable ;;
      esac
    fi
  fi
  ...
  case "$TKX_CLASS" in
    ok)
      ...
      TKX_TE=${TKX_DETAIL%% *}
      TKX_REST=${TKX_DETAIL#* }
      TKX_CHKEXEC=${TKX_REST%% *}
      TKX_CHKKERNEXT=${TKX_REST#* }
      TKX_OBSERVED="TE=$TKX_TE CHKEXEC=$TKX_CHKEXEC CHKKERNEXT=$TKX_CHKKERNEXT"
      TKX_MEANING="Trusted Execution Check policy has TE, CHKEXEC, and CHKKERNEXT all ON."
      TKX_FIX="n/a"
      ;;
    fail) TKX_STATUS=FAIL
      TKX_TE=${TKX_DETAIL%% *}
      TKX_REST=${TKX_DETAIL#* }
      TKX_CHKEXEC=${TKX_REST%% *}
      TKX_CHKKERNEXT=${TKX_REST#* }
      TKX_OBSERVED="TE=$TKX_TE CHKEXEC=$TKX_CHKEXEC CHKKERNEXT=$TKX_CHKKERNEXT"
      TKX_MEANING="Trusted Execution Check policy is not fully enabled; TE, CHKEXEC, and CHKKERNEXT must all be ON."
      TKX_FIX="trustchk -p TE=on CHKEXEC=on CHKKERNEXT=on; PTxray only recommends this command."
      ;;
    subject_absent) TKX_STATUS=FAIL
      TKX_OBSERVED="required TE/CHKEXEC/CHKKERNEXT subject is absent"
      TKX_MEANING="The trustchk policy dump did not include TE, CHKEXEC, and CHKKERNEXT, so the required Check policy is not configured."
      TKX_FIX="trustchk -p TE=on CHKEXEC=on CHKKERNEXT=on; PTxray only recommends this command."
      ;;
    probe_failed) TKX_STATUS=NOT_ASSESSED
      TKX_OBSERVED="not assessed - trustchk -p probe failed (rc=$TKX_RC)"
      TKX_MEANING="PTxray did not obtain trustworthy Trusted Execution Check policy evidence."
      TKX_FIX="run 'trustchk -p', resolve the probe failure, and rerun PTxray."
      ;;
    probe_empty) TKX_STATUS=FAIL
      TKX_OBSERVED="required TE/CHKEXEC/CHKKERNEXT subject is absent"
      TKX_MEANING="The trustchk policy dump did not include TE, CHKEXEC, and CHKKERNEXT, so the required Check policy is not configured."
      TKX_FIX="trustchk -p TE=on CHKEXEC=on CHKKERNEXT=on; PTxray only recommends this command."
      ;;
    unparseable) TKX_STATUS=NOT_ASSESSED
      TKX_OBSERVED="not assessed - trustchk -p output was unparseable"
      TKX_MEANING="PTxray did not obtain trustworthy Trusted Execution Check policy evidence."
      TKX_FIX="run 'trustchk -p', correct the capture, and rerun PTxray."
      ;;
    *) TKX_STATUS=NOT_ASSESSED
      TKX_OBSERVED="not assessed - trustchk -p output was unparseable"
      TKX_MEANING="PTxray did not obtain trustworthy Trusted Execution Check policy evidence."
      TKX_FIX="run 'trustchk -p', correct the capture, and rerun PTxray."
      ;;
  ...
  case "$TKX_STATUS" in
    NOT_APPLICABLE) TKX_SEV=low ;;
    *) TKX_SEV=med ;;
  esac

  add security stig_trustchk_kernext "Trusted Execution Check policy" \
    "$TKX_STATUS" "$TKX_SEV" "$TKX_OBSERVED" \
    "$TKX_MEANING" "$TKX_FIX" "stig:V-215333"
}
