...
AIXRAY_TOOL=ck-stig-user-auth-type


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_user_auth_type — DISA STIG V-215437 (AIX7-00-003201).
  # usw auth_type must be PAM_AUTH (required configuration). Prefer lssec
  # for the value; grep is corroborating. Missing auth_type / missing usw
  # subject is FAIL. Probe rc != 0 or rc 0 + empty output is NOT_ASSESSED.
  # STIG check-content:
  # grep auth_type /etc/security/login.cfg |grep AUTH
  # lssec -f /etc/security/login.cfg -susw -a auth_type
  typeset SUAT_GREP_RAW SUAT_GREP_RC SUAT_GREP_AUTH SUAT_LS_RAW SUAT_LS_RC
  typeset SUAT_PARSED SUAT_STATE SUAT_VALUE SUAT_KIND
  typeset SUAT_STATUS SUAT_SEV SUAT_OBSERVED SUAT_MEANING SUAT_FIX
  ...
  SUAT_GREP_RAW=$(aix grep_auth_type grep auth_type /etc/security/login.cfg)
  SUAT_GREP_RC=$?
  # Corroborating AUTH filter; `|| :` keeps a no-match from tripping set -e.
  SUAT_GREP_AUTH=$(printf '%s\n' "$SUAT_GREP_RAW" | grep AUTH) || :
  SUAT_LS_RAW=$(aix lssec_auth_type lssec -f /etc/security/login.cfg -susw -a auth_type)
  SUAT_LS_RC=$?

  # Grep is corroborating, but a failed or empty probe is a refusal, not a
  # determinate lssec verdict. Evaluate grep status and AUTH-filtered output
  # before PASS/FAIL.
  if [ "$SUAT_GREP_RC" -ne 0 ] || [ "$SUAT_LS_RC" -ne 0 ]; then
    SUAT_KIND=failed
  elif [ -z "$SUAT_GREP_RAW" ] || [ -z "$SUAT_LS_RAW" ]; then
    SUAT_KIND=empty
  else
    SUAT_PARSED=$(printf '%s\n' "$SUAT_LS_RAW" | awk '
      NF {
        rows++
        if ($1 != "usw") bad=1
        for (i=2; i<=NF; i++) {
          if (index($i, "auth_type=")==1) {
            attrs++
            value=substr($i, length("auth_type=")+1)
          } else {
            bad=1
          }
        }
    ...
    SUAT_STATE=${SUAT_PARSED%%\|*}
    SUAT_VALUE=${SUAT_PARSED#*\|}
    case "$SUAT_STATE" in
      empty|unparseable)
        SUAT_KIND=unparseable
        ;;
      absent)
        SUAT_KIND=absent
        ;;
      value)
        if [ "$SUAT_VALUE" = "PAM_AUTH" ]; then
          if [ -n "$SUAT_GREP_RAW" ] && [ -z "$SUAT_GREP_AUTH" ]; then
            SUAT_KIND=grep_no_auth
          else
            SUAT_KIND=ok
          fi
        else
  ...
  case "$SUAT_KIND" in
    ok)
      SUAT_STATUS=PASS
      SUAT_OBSERVED="auth_type=PAM_AUTH"
      SUAT_MEANING="The usw auth_type is PAM_AUTH, so AIX authenticates through PAM as required for multi-factor authentication."
      SUAT_FIX="n/a"
      ;;
    absent)
      SUAT_STATUS=FAIL
      SUAT_OBSERVED="auth_type unset (required configuration unmet)"
      SUAT_MEANING="auth_type is not set in the usw stanza, so the operating system is not configured to authenticate using multi-factor authentication."
      SUAT_FIX="chsec -f /etc/security/login.cfg -susw -a auth_type=PAM_AUTH"
      ;;
    violation)
      SUAT_STATUS=FAIL
      SUAT_OBSERVED="auth_type=$SUAT_VALUE"
      SUAT_MEANING="The usw auth_type is not PAM_AUTH, so the operating system is not configured to authenticate using multi-factor authentication."
      SUAT_FIX="chsec -f /etc/security/login.cfg -susw -a auth_type=PAM_AUTH"
      ;;
    failed)
      SUAT_STATUS=NOT_ASSESSED
      if [ "$SUAT_GREP_RC" -ne 0 ]; then
        SUAT_OBSERVED="not assessed - grep auth_type probe failed (rc=$SUAT_GREP_RC)"
      else
        SUAT_OBSERVED="not assessed - auth_type probe failed (rc=$SUAT_LS_RC)"
      fi
      SUAT_MEANING="PTxray did not obtain trustworthy evidence for the usw auth_type MFA requirement."
      SUAT_FIX="verify lssec and grep against /etc/security/login.cfg are readable, then rerun PTxray."
      ;;
    empty)
      SUAT_STATUS=NOT_ASSESSED
      if [ -z "$SUAT_GREP_RAW" ]; then
        SUAT_OBSERVED="not assessed - grep auth_type probe returned empty output with rc 0"
      else
        SUAT_OBSERVED="not assessed - auth_type probe returned empty output with rc 0"
      fi
      SUAT_MEANING="PTxray did not obtain trustworthy evidence for the usw auth_type MFA requirement."
      SUAT_FIX="verify lssec against /etc/security/login.cfg returns content, then rerun PTxray."
      ;;
    grep_no_auth)
      SUAT_STATUS=NOT_ASSESSED
      SUAT_OBSERVED="not assessed - grep AUTH filter produced no matching output"
      SUAT_MEANING="PTxray did not obtain trustworthy evidence for the usw auth_type MFA requirement."
      SUAT_FIX="run 'grep auth_type /etc/security/login.cfg |grep AUTH' and rerun PTxray."
      ;;
    *)
      SUAT_STATUS=NOT_ASSESSED
      SUAT_OBSERVED="not assessed - auth_type output could not be parsed"
      SUAT_MEANING="PTxray did not obtain trustworthy evidence for the usw auth_type MFA requirement."
      SUAT_FIX="run 'lssec -f /etc/security/login.cfg -susw -a auth_type' and rerun PTxray."
      ;;
  esac

  add security stig_user_auth_type "Multi-factor authentication type" \
    "$SUAT_STATUS" "$SUAT_SEV" "$SUAT_OBSERVED" \
    "$SUAT_MEANING" "$SUAT_FIX" "stig:V-215437"
}
