...
AIXRAY_TOOL=ck-stig-user-core


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_user_core — DISA STIG V-215396.
  # Prohibition: every account has core=0 (process core dumps disabled).
  # Uncertain probe evidence is refused; absence of the subject is PASS.
  typeset SUC_RAW SUC_RC SUC_PARSED SUC_PARSE_RC SUC_CLASS SUC_DETAIL
  typeset SUC_STATUS SUC_SEV SUC_OBSERVED SUC_MEANING SUC_FIX

  SUC_RAW=$(aix lsuser_core lsuser -a core ALL)
  SUC_RC=$?
  SUC_CLASS=""
  SUC_DETAIL=""

  if [ "$SUC_RC" -ne 0 ]; then
    SUC_CLASS=probe_failed
  elif [ -z "$SUC_RAW" ]; then
    SUC_CLASS=probe_empty
  else
    SUC_PARSED=$(printf '%s\n' "$SUC_RAW" | awk '
      function is_user(u) {
    ...
    ')
    SUC_PARSE_RC=$?
    if [ "$SUC_PARSE_RC" -ne 0 ] || [ -z "$SUC_PARSED" ]; then
      SUC_CLASS=unparseable
    else
      case "$SUC_PARSED" in
        ok) SUC_CLASS=ok ;;
        fail\ *)
          SUC_CLASS=fail
          SUC_DETAIL=${SUC_PARSED#fail }
          ;;
        absent) SUC_CLASS=subject_absent ;;
        absent_with_users) SUC_CLASS=unparseable ;;
        *) SUC_CLASS=unparseable ;;
      esac
    fi
  fi
  ...
  case "$SUC_CLASS" in
    ok)
      SUC_STATUS=PASS
      SUC_OBSERVED="all listed accounts have core=0"
      SUC_MEANING="Every listed account has core=0, so process core dumps cannot leak memory contents."
      SUC_FIX="n/a"
      ;;
    fail) SUC_STATUS=FAIL
      SUC_OBSERVED="core is not 0: $SUC_DETAIL"
      SUC_MEANING="One or more accounts allow process core dumps, so process memory contents can leak to disk."
      SUC_FIX="chsec -f /etc/security/limits -s default -a core=0; PTxray only recommends this command."
      ;;
    subject_absent) SUC_STATUS=PASS
      SUC_OBSERVED="core dump subject is absent"
      SUC_MEANING="No accounts were listed, so no process can produce a core dump."
      SUC_FIX="n/a"
      ;;
    probe_failed) SUC_STATUS=NOT_ASSESSED
      SUC_OBSERVED="not assessed - lsuser core probe failed (rc=$SUC_RC)"
      SUC_MEANING="PTxray did not obtain trustworthy per-account core-dump evidence."
      SUC_FIX="run 'lsuser -a core ALL', correct the capture problem, and rerun PTxray."
      ;;
    probe_empty) SUC_STATUS=NOT_ASSESSED
      SUC_OBSERVED="not assessed - lsuser core probe returned no output"
      SUC_MEANING="PTxray did not obtain trustworthy per-account core-dump evidence."
      SUC_FIX="run 'lsuser -a core ALL', correct the capture problem, and rerun PTxray."
      ;;
    unparseable) SUC_STATUS=NOT_ASSESSED
      SUC_OBSERVED="not assessed - lsuser core output was unparseable"
      SUC_MEANING="PTxray did not obtain trustworthy per-account core-dump evidence."
      SUC_FIX="run 'lsuser -a core ALL', correct the capture problem, and rerun PTxray."
      ;;
    *)
      SUC_STATUS=NOT_ASSESSED
      SUC_OBSERVED="not assessed - lsuser core output was unparseable"
      SUC_MEANING="PTxray did not obtain trustworthy per-account core-dump evidence."
      SUC_FIX="run 'lsuser -a core ALL', correct the capture problem, and rerun PTxray."
      ;;
  ...
  case "$SUC_STATUS" in
    NOT_APPLICABLE) SUC_SEV=low ;;
    *) SUC_SEV=med ;;
  esac

  add security stig_user_core "AIX process core dumps must be disabled" \
    "$SUC_STATUS" "$SUC_SEV" "$SUC_OBSERVED" \
    "$SUC_MEANING" "$SUC_FIX" "stig:V-215396"
}
