...
AIXRAY_TOOL=ck-stig-user-dictionlist


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_user_dictionlist — DISA STIG V-215229.
  # Required configuration: default and every user have a non-empty dictionlist.
  # Uncertain probe evidence is refused; a missing subject is a finding.
  typeset UDL_DEF_RAW UDL_DEF_RC UDL_USR_RAW UDL_USR_RC
  typeset UDL_DEF_KIND UDL_USR_KIND UDL_USR_DETAIL
  typeset UDL_CLASS UDL_DETAIL
  typeset UDL_STATUS UDL_SEV UDL_OBSERVED UDL_MEANING UDL_FIX

  UDL_DEF_RAW=$(aix lssec_user_default_dictionlist lssec -f /etc/security/user -s default -a dictionlist)
  UDL_DEF_RC=$?
  UDL_USR_RAW=$(aix lsuser_dictionlist_all lsuser -a dictionlist ALL)
  UDL_USR_RC=$?
  UDL_CLASS=""
  UDL_DETAIL=""
  UDL_DEF_KIND=""
  ...
  if [ "$UDL_DEF_RC" -ne 0 ] || [ "$UDL_USR_RC" -ne 0 ]; then
    UDL_CLASS=probe_failed
  elif [ -z "$UDL_DEF_RAW" ] || [ -z "$UDL_USR_RAW" ]; then
    UDL_CLASS=probe_empty
  else
    UDL_DEF_KIND=$(printf '%s\n' "$UDL_DEF_RAW" | awk '
      {
        gsub(/\r/, "")
        if ($0 ~ /^[ \t]*$/) next
        records++
        ...
        if (index(rest, "dictionlist=") != 1) { bad = 1; next }
        val = substr(rest, 13)
        if (val == "") kind = "empty"
        else kind = "ok"
      }
    ...
    UDL_USR_KIND=$(printf '%s\n' "$UDL_USR_RAW" | awk '
      ...
      END {
        ...
        if (nfail > 0) print "empty " fails
        else print "ok"
      }
    ')
    case "$UDL_USR_KIND" in
      empty\ *)
        UDL_USR_DETAIL=${UDL_USR_KIND#empty }
        UDL_USR_KIND=empty
        ;;
    esac
    if [ -z "$UDL_DEF_KIND" ] || [ -z "$UDL_USR_KIND" ]; then
      UDL_CLASS=unparseable
    elif [ "$UDL_DEF_KIND" = unparseable ] || [ "$UDL_USR_KIND" = unparseable ]; then
      UDL_CLASS=unparseable
    elif [ "$UDL_DEF_KIND" = absent ] || [ "$UDL_USR_KIND" = absent ]; then
      UDL_CLASS=subject_absent
    elif [ "$UDL_DEF_KIND" = empty ] || [ "$UDL_USR_KIND" = empty ]; then
      UDL_CLASS=noncompliant
      if [ "$UDL_DEF_KIND" = empty ]; then
        UDL_DETAIL=default
      fi
      if [ "$UDL_USR_KIND" = empty ]; then
        if [ -n "$UDL_DETAIL" ]; then
          UDL_DETAIL="$UDL_DETAIL $UDL_USR_DETAIL"
        else
          UDL_DETAIL=$UDL_USR_DETAIL
        fi
      fi
    elif [ "$UDL_DEF_KIND" = ok ] && [ "$UDL_USR_KIND" = ok ]; then
      UDL_CLASS=compliant
    else
      UDL_CLASS=unparseable
    fi
  ...
  case "$UDL_CLASS" in
    compliant)
      UDL_STATUS=PASS
      UDL_OBSERVED="default and all listed users have a non-empty dictionlist"
      UDL_MEANING="Default and every listed user have a non-empty dictionlist, so dictionary-word passwords are rejected."
      UDL_FIX="n/a"
      ;;
    noncompliant) UDL_STATUS=FAIL
      UDL_OBSERVED="empty dictionlist: $UDL_DETAIL"
      UDL_MEANING="Default or a listed user has an empty dictionlist, so dictionary-word passwords are not prevented."
      UDL_FIX='chsec -f /etc/security/user -s default -a dictionlist="/etc/security/ice/dictionary/English"; chsec -f /etc/security/user -s [user_name] -a dictionlist="/etc/security/ice/dictionary/English"; PTxray only recommends these commands.'
      ;;
    subject_absent) UDL_STATUS=FAIL
      UDL_OBSERVED="required dictionlist subject is absent"
      UDL_MEANING="The required dictionlist configuration is missing, so dictionary-word passwords are not prevented."
      UDL_FIX='chsec -f /etc/security/user -s default -a dictionlist="/etc/security/ice/dictionary/English"; chsec -f /etc/security/user -s [user_name] -a dictionlist="/etc/security/ice/dictionary/English"; PTxray only recommends these commands.'
      ;;
    probe_failed) UDL_STATUS=NOT_ASSESSED
      UDL_OBSERVED="not assessed - dictionlist probe failed (lssec_rc=$UDL_DEF_RC lsuser_rc=$UDL_USR_RC)"
      UDL_MEANING="PTxray did not obtain trustworthy dictionlist evidence."
      UDL_FIX="run 'lssec -f /etc/security/user -s default -a dictionlist' and 'lsuser -a dictionlist ALL', correct the capture problem, and rerun PTxray."
      ;;
    probe_empty) UDL_STATUS=NOT_ASSESSED
      UDL_OBSERVED="not assessed - dictionlist probe returned no output"
      UDL_MEANING="PTxray did not obtain trustworthy dictionlist evidence."
      UDL_FIX="run 'lssec -f /etc/security/user -s default -a dictionlist' and 'lsuser -a dictionlist ALL', correct the capture problem, and rerun PTxray."
      ;;
    *)
      UDL_STATUS=NOT_ASSESSED
      UDL_OBSERVED="not assessed - dictionlist probe output was unparseable"
      UDL_MEANING="PTxray did not obtain trustworthy dictionlist evidence."
      UDL_FIX="run 'lssec -f /etc/security/user -s default -a dictionlist' and 'lsuser -a dictionlist ALL', correct the capture problem, and rerun PTxray."
      ;;
  esac
  ...
  case "$UDL_STATUS" in
    NOT_APPLICABLE) UDL_SEV=low ;;
    *) UDL_SEV=med ;;
  esac

  add security stig_user_dictionlist "Password dictionlist on default and every user" \
    "$UDL_STATUS" "$UDL_SEV" "$UDL_OBSERVED" \
    "$UDL_MEANING" "$UDL_FIX" "stig:V-215229"
}
