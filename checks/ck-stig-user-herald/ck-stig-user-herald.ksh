...
AIXRAY_TOOL=ck-stig-user-herald


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_user_herald — DISA STIG V-215203.
  # PASS only when the default-stanza herald in /etc/security/login.cfg
  # contains the Standard Mandatory DoD Notice and Consent Banner after
  # whitespace normalisation. Empty herald is FAIL (a required configuration
  # is missing); a probe that cannot be trusted is NOT_ASSESSED. Read-only;
  # the fix text is advisory only. ck-login-banner is left untouched.
  typeset SUH_REQ_TEXT SUH_RAW SUH_RC SUH_HERALD SUH_REASON SUH_PARSE_RC
  SUH_REQ_TEXT='You are accessing a U.S. Government (USG) Information System (IS) that is provided for USG-authorized use only. By using this IS (which includes any device attached to this IS), you consent to the following conditions: -The USG routinely intercepts and monitors communications on this IS for purposes including, but not limited to, penetration testing, COMSEC monitoring, network operations and defense, personnel misconduct (PM), law enforcement (LE), and counterintelligence (CI) investigations. -At any time, the USG may inspect and seize data stored on this IS. -Communications using, or data stored on, this IS are not private, are subject to routine monitoring, interception, and search, and may be disclosed or used for any USG-authorized purpose. -This IS includes security measures (e.g., authentication and access controls) to protect USG interests--not for your personal benefit or privacy. -Notwithstanding the above, using this IS does not constitute consent to PM, LE or CI investigative searching or monitoring of the content of privileged communications, or work product, related to personal representation or services by attorneys, psychotherapists, or clergy, and their assistants. Such communications and work product are private and confidential. See User Agreement for details.'
  SUH_RAW=$(aix lssec_herald lssec -f /etc/security/login.cfg -s default -a herald)
  SUH_RC=$?
  SUH_HERALD=""
  SUH_REASON=""
  if [ "$SUH_RC" -ne 0 ]; then
    SUH_REASON="lssec failed (rc=$SUH_RC)"
  elif [ -z "$SUH_RAW" ]; then
    SUH_REASON="lssec returned no output"
  else
    SUH_HERALD=$(printf '%s\n' "$SUH_RAW" | awk '
      /herald=/ { sub(/^.*herald=/, ""); print; found=1; exit }
      END { if (!found) exit 1 }
    ')
    SUH_PARSE_RC=$?
    if [ "$SUH_PARSE_RC" -ne 0 ]; then
      SUH_REASON="unparseable lssec output"
    else
      case "$SUH_HERALD" in
        '""') SUH_HERALD="" ;;
        \"*\") SUH_HERALD=${SUH_HERALD#\"}; SUH_HERALD=${SUH_HERALD%\"} ;;
      esac
    fi
  fi
  if [ -n "$SUH_REASON" ]; then
    add security stig_user_herald "Standard Mandatory DoD Notice and Consent Banner" NOT_ASSESSED med \
        "not assessed - $SUH_REASON" \
        "PTxray could not obtain a trustworthy herald value to compare against the DoD banner." \
        "run 'lssec -f /etc/security/login.cfg -s default -a herald' and rerun PTxray." \
        "stig:V-215203"
  elif [ -z "$SUH_HERALD" ]; then
    add security stig_user_herald "Standard Mandatory DoD Notice and Consent Banner" FAIL med "no herald set" \
        "The default login herald is empty, so login proceeds without the required DoD notice." \
        "chsec -f /etc/security/login.cfg -s default -a herald=\"<Standard Mandatory DoD Notice and Consent Banner>\\n\\r\\n\\rlogin:\"; PTxray only recommends this command." \
        "stig:V-215203"
  elif printf '%s\n' "$SUH_HERALD" | awk -v req="$SUH_REQ_TEXT" '
      { s = s $0 ORS }
      END {
        r = req
        gsub(/\\n/, " ", s); gsub(/\\r/, " ", s)
        ...
        gsub(/[[:space:]]+/, " ", r)
        sub(/^ /, "", r); sub(/ $/, "", r)
        exit (index(s, r) > 0 ? 0 : 1)
      }'; then
    add security stig_user_herald "Standard Mandatory DoD Notice and Consent Banner" PASS med \
        "herald contains the Standard Mandatory DoD Notice and Consent Banner" \
        "The default login herald carries the full DoD notice required by V-215203." \
        "n/a" "stig:V-215203"
  else
    add security stig_user_herald "Standard Mandatory DoD Notice and Consent Banner" FAIL med \
        "herald does not contain the Standard Mandatory DoD Notice and Consent Banner" \
        "A herald is set, but it lacks the full DoD notice, so login still proceeds without the required banner." \
        "chsec -f /etc/security/login.cfg -s default -a herald=\"<Standard Mandatory DoD Notice and Consent Banner>\\n\\r\\n\\rlogin:\"; PTxray only recommends this command." \
        "stig:V-215203"
  fi
}
