...
AIXRAY_TOOL=ck-stig-user-home-directories-have


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_user_home_directories_have — DISA STIG V-215332.
  # Prohibition: user home directories must not have extended ACLs.
  # Determinate absence of home/ACL subjects is PASS (svcoff convention).
  # rc!=0 with empty output, or rc 0 with empty output, is a refusal.
  # Each concatenated aclget block must carry one parseable extended-
  # permissions state; a disabled block does not excuse a truncated sibling.
  # aclget rc=2: skip /. Other silent rc=2 is a per-user FAIL detail
  # 'home missing' kept alongside any enabled FAIL from a readable home.
  # Unexplained nonzero on a sibling of a readable home is typed WARN
  # detail, not a refusal. NOT_ASSESSED only when no home could be read.
  typeset UHDA_RAW UHDA_RC UHDA_ACL_RC UHDA_ST UHDA_PARSE_RC UHDA_REASON
  typeset UHDA_STATUS UHDA_SEV UHDA_OBSERVED UHDA_MEANING UHDA_FIX
  typeset UHDA_PW UHDA_HOMES UHDA_HOME UHDA_ONE UHDA_ONE_RC
  typeset UHDA_UNEXPLAINED UHDA_USER UHDA_MISSING UHDA_MISSING_USERS
  typeset UHDA_PARTIAL_RC UHDA_PROBE_USERS

  UHDA_PW=$(aix user_home_aclget /usr/bin/cat /etc/passwd)
  UHDA_RC=$?
  UHDA_REASON=""
  UHDA_RAW=""
  UHDA_UNEXPLAINED=0
  ...
  UHDA_PARTIAL_RC=0
  UHDA_PROBE_USERS=""
  UHDA_ACL_RC=$UHDA_RC  # aggregate home-aclget status; seeded with the passwd probe rc

  if [ "$UHDA_RC" -eq 0 ] && [ -n "$UHDA_PW" ]; then
    UHDA_HOMES=$(printf '%s\n' "$UHDA_PW" | awk -F: '
      {
        gsub(/\r/, "")
        if ($1 != "" && $6 != "") print $1 "|" $6
    ...
    ')
    UHDA_ACL_RC=0
    while IFS='|' read UHDA_USER UHDA_HOME; do
      [ -n "$UHDA_HOME" ] || continue
      UHDA_ONE=$(aix home_aclget /usr/bin/aclget "$UHDA_HOME")
      UHDA_ONE_RC=$?
      if [ "$UHDA_HOME" = / ] && [ "$UHDA_ONE_RC" -eq 2 ]; then
        continue
      fi
      if [ "$UHDA_ONE_RC" -eq 2 ]; then
        case "$UHDA_ONE" in
          *"does not exist"*) ;;
          *)
            UHDA_MISSING=1
            if [ -n "$UHDA_MISSING_USERS" ]; then
              UHDA_MISSING_USERS="$UHDA_MISSING_USERS $UHDA_USER"
            else
              UHDA_MISSING_USERS=$UHDA_USER
            fi
            continue
            ;;
        esac
      fi
      if [ "$UHDA_ONE_RC" -ne 0 ]; then
        # helper swallows stderr. Nonzero without captured does-not-exist
        # text is unexplained; skip it so a readable sibling can be graded.
        case "$UHDA_ONE" in
          *"does not exist"*)
            UHDA_ACL_RC=$UHDA_ONE_RC
            ;;
          *)
            UHDA_UNEXPLAINED=1
            UHDA_PARTIAL_RC=$UHDA_ONE_RC
            if [ -n "$UHDA_PROBE_USERS" ]; then
              UHDA_PROBE_USERS="$UHDA_PROBE_USERS $UHDA_USER"
            else
              UHDA_PROBE_USERS=$UHDA_USER
            fi
            continue
            ;;
        esac
...
UHDA_HOME_EOF
  ...
  if [ -z "$UHDA_RAW" ]; then
    if [ "$UHDA_UNEXPLAINED" -ne 0 ]; then
      UHDA_REASON=probe_failed
      if [ "$UHDA_ACL_RC" -eq 0 ]; then
        UHDA_ACL_RC=$UHDA_PARTIAL_RC
      fi
    elif [ "$UHDA_ACL_RC" -eq 0 ]; then
      UHDA_REASON=probe_empty
    else
      UHDA_REASON=probe_failed
    fi
  else
    UHDA_ST=$(printf '%s\n' "$UHDA_RAW" | awk '
      function close_blk() {
        if (in_blk == 0) return
        if (want != 0 || nstate != 1) incomplete=1
        else if (last == "enabled") enabled=1
        else if (last == "disabled") disabled=1
        else incomplete=1
        in_blk=0
        nstate=0
        last=""
      ...
      in_blk && /extended[ \t]+permissions/ {
        want=1
        next
      }
      in_blk && want {
        if ($1 == "disabled" || $1 == "enabled") {
          nstate++
          last=$1
        } else {
          incomplete=1
      ...
      END {
        ...
        else if (absent) print "absent"
        else exit 1
      }
    ')
    UHDA_PARSE_RC=$?
    if [ "$UHDA_PARSE_RC" -ne 0 ] || [ -z "$UHDA_ST" ]; then
      UHDA_REASON=unparseable
    else
      case "$UHDA_ST" in
        enabled)
          if [ "$UHDA_ACL_RC" -eq 0 ]; then
        ...
        disabled)
          if [ "$UHDA_ACL_RC" -eq 0 ]; then
            UHDA_REASON=disabled
          else
            UHDA_REASON=probe_failed
          fi
          ;;
        absent) UHDA_REASON=absent ;;
        *) UHDA_REASON=unparseable ;;
      esac
    fi
    if [ "$UHDA_UNEXPLAINED" -ne 0 ]; then
      case "$UHDA_REASON" in
        disabled|absent)
          UHDA_REASON=partial_probe
          ;;
      esac
    fi
  fi

  if [ "$UHDA_MISSING" -ne 0 ]; then
    case "$UHDA_REASON" in
      disabled|absent|probe_empty|probe_failed|partial_probe)
        UHDA_REASON=home_missing
        ;;
    esac
  fi

  case "$UHDA_REASON" in
    enabled) UHDA_STATUS=FAIL
      UHDA_OBSERVED="extended permissions enabled on at least one user home directory"
      UHDA_MEANING="An extended ACL on a user home directory lets a non-owner change or replace that account's files."
      UHDA_FIX="acledit <directory>; PTxray only recommends this command."
      if [ -n "$UHDA_PROBE_USERS" ]; then
        UHDA_OBSERVED="aclget probe failed (rc=$UHDA_PARTIAL_RC) for $UHDA_PROBE_USERS; $UHDA_OBSERVED"
      fi
      if [ "$UHDA_MISSING" -ne 0 ]; then
        UHDA_OBSERVED="home missing: $UHDA_MISSING_USERS; $UHDA_OBSERVED"
      fi
      ;;
    disabled) UHDA_STATUS=PASS
      UHDA_OBSERVED="extended permissions disabled on user home directories"
      UHDA_MEANING="User home directories do not have extended ACLs."
      UHDA_FIX="n/a"
      ;;
    absent) UHDA_STATUS=PASS
      UHDA_OBSERVED="user home directories are absent; no extended ACL subject"
      UHDA_MEANING="No user home directory is present, so no extended ACL subject exists to violate the prohibition."
      UHDA_FIX="n/a"
      ;;
    home_missing) UHDA_STATUS=FAIL
      UHDA_OBSERVED="home missing: $UHDA_MISSING_USERS"
      UHDA_MEANING="An interactive user's home directory does not exist or is not a directory."
      UHDA_FIX="create the missing home directory; PTxray only recommends this action."
      ;;
    partial_probe) UHDA_STATUS=WARN
      UHDA_OBSERVED="aclget probe failed (rc=$UHDA_PARTIAL_RC) for $UHDA_PROBE_USERS"
      UHDA_MEANING="Readable user home directories were graded, but aclget failed for at least one other home so the scan is incomplete."
      UHDA_FIX="correct the user-home aclget probe for the named accounts, then rerun PTxray."
      ;;
    probe_failed) UHDA_STATUS=NOT_ASSESSED
      UHDA_OBSERVED="not assessed - user home aclget probe failed (rc=$UHDA_ACL_RC)"
      UHDA_MEANING="PTxray did not obtain trustworthy aclget evidence for user home directories."
      UHDA_FIX="correct the user-home aclget probe, then rerun PTxray."
      ;;
    probe_empty) UHDA_STATUS=NOT_ASSESSED
      UHDA_OBSERVED="not assessed - user home aclget probe returned no output"
      UHDA_MEANING="PTxray did not obtain trustworthy aclget evidence for user home directories."
      UHDA_FIX="correct the user-home aclget probe, then rerun PTxray."
      ;;
    *)
      UHDA_STATUS=NOT_ASSESSED
      UHDA_OBSERVED="not assessed - user home aclget output was unparseable"
      UHDA_MEANING="PTxray did not obtain trustworthy aclget evidence for user home directories."
      UHDA_FIX="correct the user-home aclget probe, then rerun PTxray."
      ;;
  esac
  ...
  case "$UHDA_STATUS" in
    NOT_APPLICABLE) UHDA_SEV=low ;;
    *) UHDA_SEV=med ;;
  esac

  add security stig_user_home_directories_have "User home directory extended ACLs" \
    "$UHDA_STATUS" "$UHDA_SEV" "$UHDA_OBSERVED" \
    "$UHDA_MEANING" "$UHDA_FIX" "stig:V-215332"
}
