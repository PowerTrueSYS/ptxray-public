...
AIXRAY_TOOL=ck-stig-user-loginretries


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_user_loginretries — DISA STIG V-215171.
  # Required configuration: default and every user have loginretries in 1..3.
  # Uncertain probe evidence is refused; a missing subject is a finding.
  typeset ULR_DEF_RAW ULR_DEF_RC ULR_USR_RAW ULR_USR_RC
  typeset ULR_DEF_KIND ULR_USR_KIND ULR_DEF_DETAIL ULR_USR_DETAIL
  typeset ULR_CLASS ULR_DETAIL
  typeset ULR_STATUS ULR_SEV ULR_OBSERVED ULR_MEANING ULR_FIX

  ULR_DEF_RAW=$(aix lssec_user_default_loginretries lssec -f /etc/security/user -s default -a loginretries)
  ULR_DEF_RC=$?
  ULR_USR_RAW=$(aix lsuser_loginretries_all lsuser -a loginretries ALL)
  ULR_USR_RC=$?
  ULR_CLASS=""
  ULR_DETAIL=""
  ULR_DEF_KIND=""
  ...
  if [ "$ULR_DEF_RC" -ne 0 ] || [ "$ULR_USR_RC" -ne 0 ]; then
    ULR_CLASS=probe_failed
  elif [ -z "$ULR_DEF_RAW" ] || [ -z "$ULR_USR_RAW" ]; then
    ULR_CLASS=probe_empty
  else
    ULR_DEF_KIND=$(printf '%s\n' "$ULR_DEF_RAW" | awk '
      function class_int(val,    sign, digits, n) {
        if (val !~ /^[+-]?[0-9]+$/) return "unparseable"
        sign = 1
        digits = val
        if (substr(val, 1, 1) == "-") sign = -1
        if (substr(val, 1, 1) == "+" || substr(val, 1, 1) == "-") digits = substr(val, 2)
        sub(/^0+/, "", digits)
        if (digits == "") return "fail 0"
        if (length("" digits) > 8) {
          if (sign < 0) return "fail -" digits
          return "fail " digits
        }
        n = sign * (digits + 0)
        if (n < 1 || n > 3) return "fail " n
        return "ok"
      }
      {
        gsub(/\r/, "")
        ...
        if (index(rest, "loginretries=") != 1) { bad = 1; next }
        val = substr(rest, 14)
        if (val == "") { kind = "absent"; next }
        cres = class_int(val)
        if (cres == "unparseable") { bad = 1; next }
        if (index(cres, "fail ") == 1) { kind = "fail"; detail = substr(cres, 6); next }
        kind = "ok"
        detail = cres
      }
      END {
    ...
    ULR_USR_KIND=$(printf '%s\n' "$ULR_USR_RAW" | awk '
      ...
      }
      {
        ...
        if (NF == 1) {
          have = 0
        } else if (NF == 2 && index($2, "loginretries=") == 1) {
          val = substr($2, 14)
          have = 1
        } else {
          bad = 1
        ...
        if (!have || val == "") {
          ...
          if (cres == "unparseable") {
            bad = 1
          } else if (index(cres, "fail ") == 1) {
            if (fails != "") fails = fails " "
            fails = fails $1 "=" substr(cres, 6)
            nfail++
          }
        }
      }
    ...
    ')
    case "$ULR_DEF_KIND" in
      fail\ *)
        ULR_DEF_DETAIL=${ULR_DEF_KIND#fail }
        ULR_DEF_KIND=fail
        ;;
    esac
    case "$ULR_USR_KIND" in
      fail\ *)
        ULR_USR_DETAIL=${ULR_USR_KIND#fail }
        ULR_USR_KIND=fail
        ;;
    esac
    if [ -z "$ULR_DEF_KIND" ] || [ -z "$ULR_USR_KIND" ]; then
      ULR_CLASS=unparseable
    elif [ "$ULR_DEF_KIND" = unparseable ] || [ "$ULR_USR_KIND" = unparseable ]; then
      ULR_CLASS=unparseable
    elif [ "$ULR_DEF_KIND" = absent ] || [ "$ULR_USR_KIND" = absent ]; then
      ULR_CLASS=subject_absent
    elif [ "$ULR_DEF_KIND" = fail ] || [ "$ULR_USR_KIND" = fail ]; then
      ULR_CLASS=noncompliant
      if [ "$ULR_DEF_KIND" = fail ]; then
        ULR_DETAIL="default=$ULR_DEF_DETAIL"
      fi
      if [ "$ULR_USR_KIND" = fail ]; then
        if [ -n "$ULR_DETAIL" ]; then
          ULR_DETAIL="$ULR_DETAIL $ULR_USR_DETAIL"
        else
          ULR_DETAIL=$ULR_USR_DETAIL
        fi
      fi
    elif [ "$ULR_DEF_KIND" = ok ] && [ "$ULR_USR_KIND" = ok ]; then
      ULR_CLASS=compliant
    else
      ULR_CLASS=unparseable
    fi
  ...
  case "$ULR_CLASS" in
    compliant)
      ULR_STATUS=PASS
      ULR_OBSERVED="default and all listed users have loginretries in 1..3"
      ULR_MEANING="Default and every listed user have loginretries between 1 and 3, so three consecutive invalid logins lock the account."
      ULR_FIX="n/a"
      ;;
    noncompliant) ULR_STATUS=FAIL
      ULR_OBSERVED="loginretries 0 or greater than 3: $ULR_DETAIL"
      ULR_MEANING="Default or a listed user has loginretries unset, 0, or greater than 3, so consecutive invalid logins do not lock the account after three attempts."
      ULR_FIX="chsec -f /etc/security/user -s default -a loginretries=3; chsec -f /etc/security/user -s [user_name] -a loginretries=3; PTxray only recommends these commands."
      ;;
    subject_absent) ULR_STATUS=FAIL
      ULR_OBSERVED="required loginretries subject is absent"
      ULR_MEANING="The required loginretries configuration is missing, so consecutive invalid logins do not lock the account after three attempts."
      ULR_FIX="chsec -f /etc/security/user -s default -a loginretries=3; chsec -f /etc/security/user -s [user_name] -a loginretries=3; PTxray only recommends these commands."
      ;;
    probe_failed) ULR_STATUS=NOT_ASSESSED
      ULR_OBSERVED="not assessed - loginretries probe failed (lssec_rc=$ULR_DEF_RC lsuser_rc=$ULR_USR_RC)"
      ULR_MEANING="PTxray did not obtain trustworthy loginretries evidence."
      ULR_FIX="run 'lssec -f /etc/security/user -s default -a loginretries' and 'lsuser -a loginretries ALL', correct the capture problem, and rerun PTxray."
      ;;
    probe_empty) ULR_STATUS=NOT_ASSESSED
      ULR_OBSERVED="not assessed - loginretries probe returned no output"
      ULR_MEANING="PTxray did not obtain trustworthy loginretries evidence."
      ULR_FIX="run 'lssec -f /etc/security/user -s default -a loginretries' and 'lsuser -a loginretries ALL', correct the capture problem, and rerun PTxray."
      ;;
    *)
      ULR_STATUS=NOT_ASSESSED
      ULR_OBSERVED="not assessed - loginretries probe output was unparseable"
      ULR_MEANING="PTxray did not obtain trustworthy loginretries evidence."
      ULR_FIX="run 'lssec -f /etc/security/user -s default -a loginretries' and 'lsuser -a loginretries ALL', correct the capture problem, and rerun PTxray."
      ;;
  esac
  ...
  case "$ULR_STATUS" in
    NOT_APPLICABLE) ULR_SEV=low ;;
    *) ULR_SEV=med ;;
  esac

  add security stig_user_loginretries "Consecutive invalid login lock (loginretries)" \
    "$ULR_STATUS" "$ULR_SEV" "$ULR_OBSERVED" \
    "$ULR_MEANING" "$ULR_FIX" "stig:V-215171"
}
