...
AIXRAY_TOOL=ck-stig-user-maxage


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_user_maxage — DISA STIG V-215223.
  # Required configuration: default and every user have maxage set in 1..8 weeks.
  # Uncertain probe evidence is refused; a missing subject is a finding.
  typeset UMA_DEF_RAW UMA_DEF_RC UMA_USR_RAW UMA_USR_RC
  typeset UMA_DEF_KIND UMA_USR_KIND UMA_USR_DETAIL
  typeset UMA_CLASS UMA_DETAIL
  typeset UMA_STATUS UMA_SEV UMA_OBSERVED UMA_MEANING UMA_FIX

  UMA_DEF_RAW=$(aix lssec_user_default_maxage lssec -f /etc/security/user -s default -a maxage)
  UMA_DEF_RC=$?
  UMA_USR_RAW=$(aix lsuser_maxage_all lsuser -a maxage ALL)
  UMA_USR_RC=$?
  UMA_CLASS=""
  UMA_DETAIL=""
  UMA_DEF_KIND=""
  ...
  if [ "$UMA_DEF_RC" -ne 0 ] || [ "$UMA_USR_RC" -ne 0 ]; then
    UMA_CLASS=probe_failed
  elif [ -z "$UMA_DEF_RAW" ] || [ -z "$UMA_USR_RAW" ]; then
    UMA_CLASS=probe_empty
  else
    UMA_DEF_KIND=$(printf '%s\n' "$UMA_DEF_RAW" | awk '
      {
        gsub(/\r/, "")
        if ($0 ~ /^[ \t]*$/) next
        records++
        ...
        if (index(rest, "maxage=") != 1) { bad = 1; next }
        val = substr(rest, 8)
        if (val == "") { kind = "fail"; next }
        if (val !~ /^[0-9]+$/) { bad = 1; next }
        if (length("" val) > 8) { kind = "fail"; next }
        n = val + 0
        if (n == 0 || n > 8) kind = "fail"
        else kind = "ok"
      }
      END {
    ...
    ')
    case "$UMA_USR_KIND" in
      fail\ *)
        UMA_USR_DETAIL=${UMA_USR_KIND#fail }
        UMA_USR_KIND=fail
        ;;
    esac
    if [ -z "$UMA_DEF_KIND" ] || [ -z "$UMA_USR_KIND" ]; then
      UMA_CLASS=unparseable
    elif [ "$UMA_DEF_KIND" = unparseable ] || [ "$UMA_USR_KIND" = unparseable ]; then
      UMA_CLASS=unparseable
    elif [ "$UMA_DEF_KIND" = absent ] || [ "$UMA_USR_KIND" = absent ]; then
      UMA_CLASS=subject_absent
    elif [ "$UMA_DEF_KIND" = fail ] || [ "$UMA_USR_KIND" = fail ]; then
      UMA_CLASS=noncompliant
      if [ "$UMA_DEF_KIND" = fail ]; then
        UMA_DETAIL=default
      fi
      if [ "$UMA_USR_KIND" = fail ]; then
        if [ -n "$UMA_DETAIL" ]; then
          UMA_DETAIL="$UMA_DETAIL $UMA_USR_DETAIL"
        else
          UMA_DETAIL=$UMA_USR_DETAIL
        fi
      fi
    elif [ "$UMA_DEF_KIND" = ok ] && [ "$UMA_USR_KIND" = ok ]; then
      UMA_CLASS=compliant
    else
      UMA_CLASS=unparseable
    fi
  ...
  case "$UMA_CLASS" in
    compliant)
      UMA_STATUS=PASS
      UMA_OBSERVED="default and all listed users have maxage in 1..8"
      UMA_MEANING="Default and every listed user have maxage set between 1 and 8 weeks, so passwords cannot live longer than 60 days."
      UMA_FIX="n/a"
      ;;
    noncompliant) UMA_STATUS=FAIL
      UMA_OBSERVED="maxage not in 1..8: $UMA_DETAIL"
      UMA_MEANING="Default or a listed user has maxage unset, 0, or greater than 8, so a password can live longer than 60 days."
      UMA_FIX="chsec -f /etc/security/user -s default -a maxage=8; chsec -f /etc/security/user -s [user_name] -a maxage=8; PTxray only recommends these commands."
      ;;
    subject_absent) UMA_STATUS=FAIL
      UMA_OBSERVED="required maxage subject is absent"
      UMA_MEANING="The required maxage configuration is missing, so a 60-day maximum password lifetime is not enforced."
      UMA_FIX="chsec -f /etc/security/user -s default -a maxage=8; chsec -f /etc/security/user -s [user_name] -a maxage=8; PTxray only recommends these commands."
      ;;
    probe_failed) UMA_STATUS=NOT_ASSESSED
      UMA_OBSERVED="not assessed - maxage probe failed (lssec_rc=$UMA_DEF_RC lsuser_rc=$UMA_USR_RC)"
      UMA_MEANING="PTxray did not obtain trustworthy maxage evidence."
      UMA_FIX="run 'lssec -f /etc/security/user -s default -a maxage' and 'lsuser -a maxage ALL', correct the capture problem, and rerun PTxray."
      ;;
    probe_empty) UMA_STATUS=NOT_ASSESSED
      UMA_OBSERVED="not assessed - maxage probe returned no output"
      UMA_MEANING="PTxray did not obtain trustworthy maxage evidence."
      UMA_FIX="run 'lssec -f /etc/security/user -s default -a maxage' and 'lsuser -a maxage ALL', correct the capture problem, and rerun PTxray."
      ;;
    *)
      UMA_STATUS=NOT_ASSESSED
      UMA_OBSERVED="not assessed - maxage probe output was unparseable"
      UMA_MEANING="PTxray did not obtain trustworthy maxage evidence."
      UMA_FIX="run 'lssec -f /etc/security/user -s default -a maxage' and 'lsuser -a maxage ALL', correct the capture problem, and rerun PTxray."
      ;;
  esac
  ...
  add security stig_user_maxage "60-day maximum password lifetime (maxage)" \
    "$UMA_STATUS" "$UMA_SEV" "$UMA_OBSERVED" \
    "$UMA_MEANING" "$UMA_FIX" "stig:V-215223"
}
