...
AIXRAY_TOOL=ck-stig-user-maxrepeats


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_user_maxrepeats — DISA STIG V-215232.
  # Required configuration: default and every account have maxrepeats in 1..3.
  # Uncertain probe evidence is refused; a missing subject is a finding.
  typeset UMR_SEC_RAW UMR_SEC_RC UMR_USR_RAW UMR_USR_RC
  typeset UMR_DPARSED UMR_UPARSED UMR_PARSE_RC
  typeset UMR_DCLASS UMR_UCLASS UMR_CLASS UMR_DETAIL UMR_UDETAIL
  typeset UMR_STATUS UMR_SEV UMR_OBSERVED UMR_MEANING UMR_FIX

  UMR_SEC_RAW=$(aix lssec_user_default_maxrepeats lssec -f /etc/security/user -s default -a maxrepeats)
  UMR_SEC_RC=$?
  UMR_USR_RAW=$(aix lsuser_maxrepeats_all lsuser -a maxrepeats ALL)
  UMR_USR_RC=$?
  UMR_CLASS=""
  UMR_DETAIL=""

  if [ "$UMR_SEC_RC" -ne 0 ] || [ "$UMR_USR_RC" -ne 0 ]; then
    UMR_CLASS=probe_failed
  elif [ -z "$UMR_SEC_RAW" ] || [ -z "$UMR_USR_RAW" ]; then
    UMR_CLASS=probe_empty
  else
    UMR_DPARSED=$(printf '%s\n' "$UMR_SEC_RAW" | awk '
      {
        gsub(/\r/, "")
        if ($0 ~ /^[ \t]*$/) next
        records++
      ...
      END {
        ...
        if (!have) { print "absent"; exit 0 }
        if (val == "") { print "fail default=unset"; exit 0 }
        if (val !~ /^[0-9]+$/) { print "unparseable"; exit 0 }
        if (length("" val) > 8) { print "fail default=" val; exit 0 }
        n = val + 0
        if (n < 1 || n > 3) { print "fail default=" n; exit 0 }
        print "ok"
      }
    ')
    UMR_PARSE_RC=$?
    UMR_UPARSED=$(printf '%s\n' "$UMR_USR_RAW" | awk '
      function is_user(u) {
      ...
      }
      {
        ...
        if (NF == 1) {
          have = 0
        } else if (NF == 2 && index($2, "maxrepeats=") == 1) {
          val = substr($2, 12)
          have = 1
        } else {
          bad = 1
    ...
    ')
    if [ "$UMR_PARSE_RC" -ne 0 ] || [ -z "$UMR_DPARSED" ] || [ -z "$UMR_UPARSED" ]; then
      UMR_CLASS=unparseable
    else
      UMR_DCLASS=""
      UMR_UCLASS=""
      case "$UMR_DPARSED" in
        ok) UMR_DCLASS=ok ;;
        absent) UMR_DCLASS=absent ;;
        fail\ *)
          UMR_DCLASS=fail
          UMR_DETAIL=${UMR_DPARSED#fail }
          ;;
        *) UMR_DCLASS=unparseable ;;
      esac
      case "$UMR_UPARSED" in
        ok) UMR_UCLASS=ok ;;
        absent) UMR_UCLASS=absent ;;
        fail\ *)
          UMR_UCLASS=fail
          UMR_UDETAIL=${UMR_UPARSED#fail }
          if [ -n "$UMR_DETAIL" ]; then
            UMR_DETAIL="$UMR_DETAIL $UMR_UDETAIL"
          else
            UMR_DETAIL=$UMR_UDETAIL
          fi
          ;;
        *) UMR_UCLASS=unparseable ;;
      esac
      case "$UMR_DCLASS:$UMR_UCLASS" in
        unparseable:*|*:unparseable) UMR_CLASS=unparseable ;;
        fail:*|*:fail) UMR_CLASS=noncompliant ;;
        ok:ok) UMR_CLASS=compliant ;;
        absent:absent|absent:ok|ok:absent) UMR_CLASS=subject_absent ;;
        *) UMR_CLASS=unparseable ;;
      esac
    fi
  fi
  ...
  case "$UMR_CLASS" in
    compliant)
      UMR_STATUS=PASS
      UMR_OBSERVED="default and all listed users have maxrepeats in 1..3"
      UMR_MEANING="Default and every listed account have maxrepeats between 1 and 3, so passwords cannot contain more than three consecutive repeating characters."
      UMR_FIX="n/a"
      ;;
    noncompliant) UMR_STATUS=FAIL
      UMR_OBSERVED="maxrepeats not in 1..3: $UMR_DETAIL"
      UMR_MEANING="Default maxrepeats is unset, 0, or greater than 3, or a listed account has maxrepeats 0 or greater than 3, so passwords may contain unlimited consecutive repeating characters."
      UMR_FIX="chsec -f /etc/security/user -s default -a maxrepeats=3; chuser maxrepeats=3 [user_name]; PTxray only recommends these commands."
      ;;
    subject_absent) UMR_STATUS=FAIL
      UMR_OBSERVED="required maxrepeats subject is absent"
      UMR_MEANING="The required maxrepeats subject is absent, so the consecutive-repeating-character limit is not configured."
      UMR_FIX="chsec -f /etc/security/user -s default -a maxrepeats=3; chuser maxrepeats=3 [user_name]; PTxray only recommends these commands."
      ;;
    probe_failed) UMR_STATUS=NOT_ASSESSED
      UMR_OBSERVED="not assessed - maxrepeats probe failed (lssec rc=$UMR_SEC_RC lsuser rc=$UMR_USR_RC)"
      UMR_MEANING="PTxray did not obtain trustworthy default and per-account maxrepeats evidence."
      UMR_FIX="run 'lssec -f /etc/security/user -s default -a maxrepeats' and 'lsuser -a maxrepeats ALL', correct the capture problem, and rerun PTxray."
      ;;
    probe_empty) UMR_STATUS=NOT_ASSESSED
      UMR_OBSERVED="not assessed - maxrepeats probe returned no output"
      UMR_MEANING="PTxray did not obtain trustworthy default and per-account maxrepeats evidence."
      UMR_FIX="run 'lssec -f /etc/security/user -s default -a maxrepeats' and 'lsuser -a maxrepeats ALL', correct the capture problem, and rerun PTxray."
      ;;
    *)
      UMR_STATUS=NOT_ASSESSED
      UMR_OBSERVED="not assessed - maxrepeats output was unparseable"
      UMR_MEANING="PTxray did not obtain trustworthy default and per-account maxrepeats evidence."
      UMR_FIX="run 'lssec -f /etc/security/user -s default -a maxrepeats' and 'lsuser -a maxrepeats ALL', correct the capture problem, and rerun PTxray."
      ;;
  esac
  ...
  case "$UMR_STATUS" in
    NOT_APPLICABLE) UMR_SEV=low ;;
    *) UMR_SEV=med ;;
  esac

  add security stig_user_maxrepeats "Password consecutive repeating character limit (maxrepeats)" \
    "$UMR_STATUS" "$UMR_SEV" "$UMR_OBSERVED" \
    "$UMR_MEANING" "$UMR_FIX" "stig:V-215232"
}
