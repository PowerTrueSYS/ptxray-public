...
AIXRAY_TOOL=ck-stig-user-minage


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_user_minage — DISA STIG V-215222.
  # Required configuration: default minage is set and >= 1, and every
  # account has minage >= 1. Uncertain probe evidence is refused; a
  # missing subject is a finding.
  typeset UMA_LSSEC_RAW UMA_LSSEC_RC UMA_LSUSER_RAW UMA_LSUSER_RC
  typeset UMA_LSSEC_PARSED UMA_LSSEC_PARSE_RC UMA_LSUSER_PARSED UMA_LSUSER_PARSE_RC
  typeset UMA_LSSEC_KIND UMA_LSUSER_KIND UMA_LSSEC_VAL UMA_LSUSER_VAL
  typeset UMA_CLASS UMA_DETAIL
  typeset UMA_STATUS UMA_SEV UMA_OBSERVED UMA_MEANING UMA_FIX

  UMA_LSSEC_RAW=$(aix lssec_default_minage lssec -f /etc/security/user -s default -a minage)
  UMA_LSSEC_RC=$?
  UMA_LSUSER_RAW=$(aix lsuser_minage_all lsuser -a minage ALL)
  UMA_LSUSER_RC=$?
  UMA_CLASS=""
  UMA_DETAIL=""
  UMA_LSSEC_KIND=""
  ...
  if [ "$UMA_LSSEC_RC" -ne 0 ] || [ "$UMA_LSUSER_RC" -ne 0 ]; then
    UMA_CLASS=probe_failed
  elif [ -z "$UMA_LSSEC_RAW" ] || [ -z "$UMA_LSUSER_RAW" ]; then
    UMA_CLASS=probe_empty
  else
    UMA_LSSEC_PARSED=$(printf '%s\n' "$UMA_LSSEC_RAW" | awk '
      {
        gsub(/\r/, "")
        if ($0 ~ /^[ \t]*$/) next
        records++
      ...
      END {
        ...
        if (records == 0) { print "unparseable"; exit 0 }
        if (!have || val == "") { print "absent"; exit 0 }
        if (val !~ /^[0-9]+$/ || length("" val) > 8) { print "unparseable"; exit 0 }
        n = val + 0
        if (n >= 1) print "ok " n
        else print "fail " n
      }
    ...
    if [ "$UMA_LSSEC_PARSE_RC" -ne 0 ] || [ -z "$UMA_LSSEC_PARSED" ]; then
      UMA_LSSEC_KIND=unparseable
    else
      case "$UMA_LSSEC_PARSED" in
        ok\ *)
          UMA_LSSEC_KIND=ok
          UMA_LSSEC_VAL=${UMA_LSSEC_PARSED#ok }
          ;;
        fail\ *)
          UMA_LSSEC_KIND=fail
          UMA_LSSEC_VAL=${UMA_LSSEC_PARSED#fail }
          ;;
        absent) UMA_LSSEC_KIND=absent ;;
        *) UMA_LSSEC_KIND=unparseable ;;
      esac
    fi

    if [ "$UMA_LSUSER_PARSE_RC" -ne 0 ] || [ -z "$UMA_LSUSER_PARSED" ]; then
      UMA_LSUSER_KIND=unparseable
    else
      case "$UMA_LSUSER_PARSED" in
        ok) UMA_LSUSER_KIND=ok ;;
        fail\ *)
          UMA_LSUSER_KIND=fail
          UMA_LSUSER_VAL=${UMA_LSUSER_PARSED#fail }
          ;;
        absent) UMA_LSUSER_KIND=absent ;;
        *) UMA_LSUSER_KIND=unparseable ;;
      esac
    fi

    case "$UMA_LSSEC_KIND:$UMA_LSUSER_KIND" in
      unparseable:*|*:unparseable) UMA_CLASS=unparseable ;;
      absent:*|*:absent) UMA_CLASS=subject_absent ;;
      fail:*|*:fail)
        UMA_CLASS=fail
        UMA_DETAIL=""
        if [ "$UMA_LSSEC_KIND" = fail ]; then
          UMA_DETAIL="default=$UMA_LSSEC_VAL"
        fi
        if [ "$UMA_LSUSER_KIND" = fail ]; then
          if [ -n "$UMA_DETAIL" ]; then
            UMA_DETAIL="$UMA_DETAIL $UMA_LSUSER_VAL"
          else
            UMA_DETAIL=$UMA_LSUSER_VAL
          fi
        fi
        ;;
      ok:ok) UMA_CLASS=ok ;;
      *) UMA_CLASS=unparseable ;;
    esac
  fi
  ...
  case "$UMA_CLASS" in
    ok)
      UMA_STATUS=PASS
      UMA_OBSERVED="default minage=$UMA_LSSEC_VAL; all listed accounts have minage>=1"
      UMA_MEANING="Default and per-account minage are set to at least 1, so a password cannot be changed within 24 hours."
      UMA_FIX="n/a"
      ;;
    fail) UMA_STATUS=FAIL
      UMA_OBSERVED="minage not >= 1: $UMA_DETAIL"
      UMA_MEANING="The default minage is less than 1, or an account has minage less than 1, so a password can be changed immediately after it is set."
      UMA_FIX="chsec -f /etc/security/user -s default -a minage=1; chsec -f /etc/security/user -s [user_name] -a minage=1"
      ;;
    subject_absent) UMA_STATUS=FAIL
      UMA_OBSERVED="required minage subject is absent"
      UMA_MEANING="The default minage attribute or the account list is missing, so the required 24-hour minimum password lifetime is not configured."
      UMA_FIX="chsec -f /etc/security/user -s default -a minage=1; chsec -f /etc/security/user -s [user_name] -a minage=1"
      ;;
    probe_failed) UMA_STATUS=NOT_ASSESSED
      UMA_OBSERVED="not assessed - minage probe failed (lssec_rc=$UMA_LSSEC_RC lsuser_rc=$UMA_LSUSER_RC)"
      UMA_MEANING="PTxray did not obtain trustworthy default and per-account minage evidence."
      UMA_FIX="run 'lssec -f /etc/security/user -s default -a minage' and 'lsuser -a minage ALL', correct the capture problem, and rerun PTxray."
      ;;
    probe_empty) UMA_STATUS=NOT_ASSESSED
      UMA_OBSERVED="not assessed - minage probe returned no output"
      UMA_MEANING="PTxray did not obtain trustworthy default and per-account minage evidence."
      UMA_FIX="run 'lssec -f /etc/security/user -s default -a minage' and 'lsuser -a minage ALL', correct the capture problem, and rerun PTxray."
      ;;
    *)
      UMA_STATUS=NOT_ASSESSED
      UMA_OBSERVED="not assessed - minage output was unparseable"
      UMA_MEANING="PTxray did not obtain trustworthy default and per-account minage evidence."
      UMA_FIX="run 'lssec -f /etc/security/user -s default -a minage' and 'lsuser -a minage ALL', correct the capture problem, and rerun PTxray."
      ;;
  esac
  ...
  case "$UMA_STATUS" in
    NOT_APPLICABLE) UMA_SEV=low ;;
    *) UMA_SEV=med ;;
  esac

  add security stig_user_minage "Minimum password lifetime (minage)" \
    "$UMA_STATUS" "$UMA_SEV" "$UMA_OBSERVED" \
    "$UMA_MEANING" "$UMA_FIX" "stig:V-215222"
}
