...
AIXRAY_TOOL=ck-stig-user-mindiff


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_user_mindiff — DISA STIG V-215220.
  # Required configuration: default and every user have mindiff >= 8.
  # Uncertain probe evidence is refused; a missing subject is a finding.
  typeset SMD_DEF_RAW SMD_DEF_RC SMD_USR_RAW SMD_USR_RC
  typeset SMD_DEF_KIND SMD_USR_KIND SMD_USR_DETAIL SMD_DEF_DETAIL
  typeset SMD_CLASS SMD_DETAIL
  typeset SMD_STATUS SMD_SEV SMD_OBSERVED SMD_MEANING SMD_FIX

  SMD_DEF_RAW=$(aix lssec_user_default_mindiff lssec -f /etc/security/user -s default -a mindiff)
  SMD_DEF_RC=$?
  SMD_USR_RAW=$(aix lsuser_mindiff_all lsuser -a mindiff ALL)
  SMD_USR_RC=$?
  SMD_CLASS=""
  SMD_DETAIL=""
  SMD_DEF_KIND=""
  ...
  if [ "$SMD_DEF_RC" -ne 0 ] || [ "$SMD_USR_RC" -ne 0 ]; then
    SMD_CLASS=probe_failed
  elif [ -z "$SMD_DEF_RAW" ] || [ -z "$SMD_USR_RAW" ]; then
    SMD_CLASS=probe_empty
  else
    SMD_DEF_KIND=$(printf '%s\n' "$SMD_DEF_RAW" | awk '
      {
        gsub(/\r/, "")
        if ($0 ~ /^[ \t]*$/) next
        records++
        ...
        if (index(rest, "mindiff=") != 1) { bad = 1; next }
        val = substr(rest, 9)
    ...
    SMD_USR_KIND=$(printf '%s\n' "$SMD_USR_RAW" | awk '
      ...
      }
      {
        ...
        if (NF == 1) {
          have = 0
        } else if (NF == 2 && index($2, "mindiff=") == 1) {
          val = substr($2, 9)
          have = 1
        } else {
          bad = 1
    ...
    ')
    case "$SMD_DEF_KIND" in
      low\ *)
        SMD_DEF_DETAIL=${SMD_DEF_KIND#low }
        SMD_DEF_KIND=low
        ;;
    esac
    case "$SMD_USR_KIND" in
      fail\ *)
        SMD_USR_DETAIL=${SMD_USR_KIND#fail }
        SMD_USR_KIND=fail
        ;;
    esac
    if [ -z "$SMD_DEF_KIND" ] || [ -z "$SMD_USR_KIND" ]; then
      SMD_CLASS=unparseable
    elif [ "$SMD_DEF_KIND" = unparseable ] || [ "$SMD_USR_KIND" = unparseable ]; then
      SMD_CLASS=unparseable
    elif [ "$SMD_DEF_KIND" = absent ] || [ "$SMD_USR_KIND" = absent ]; then
      SMD_CLASS=subject_absent
    elif [ "$SMD_DEF_KIND" = unset ] || [ "$SMD_DEF_KIND" = low ] || [ "$SMD_USR_KIND" = fail ]; then
      SMD_CLASS=noncompliant
      SMD_DETAIL=""
      if [ "$SMD_DEF_KIND" = unset ]; then
        SMD_DETAIL="default=unset"
      elif [ "$SMD_DEF_KIND" = low ]; then
        SMD_DETAIL="default=$SMD_DEF_DETAIL"
      fi
      if [ "$SMD_USR_KIND" = fail ]; then
        if [ -n "$SMD_DETAIL" ]; then
          SMD_DETAIL="$SMD_DETAIL $SMD_USR_DETAIL"
        else
          SMD_DETAIL=$SMD_USR_DETAIL
        fi
      fi
    elif [ "$SMD_DEF_KIND" = ok ] && [ "$SMD_USR_KIND" = ok ]; then
      SMD_CLASS=compliant
    else
      SMD_CLASS=unparseable
    fi
  ...
  case "$SMD_CLASS" in
    compliant)
      SMD_STATUS=PASS
      SMD_OBSERVED="default and all listed users have mindiff>=8"
      SMD_MEANING="Default and every listed user have mindiff at least 8, so password changes must replace at least 50 percent of characters."
      SMD_FIX="n/a"
      ;;
    noncompliant) SMD_STATUS=FAIL
      SMD_OBSERVED="mindiff below 8: $SMD_DETAIL"
      SMD_MEANING="Default or a listed user has mindiff unset or below 8, so password changes need not replace 50 percent of characters."
      SMD_FIX="chsec -f /etc/security/user -s default -a mindiff=8; chsec -f /etc/security/user -s [user_name] -a mindiff=8; PTxray only recommends these commands."
      ;;
    subject_absent) SMD_STATUS=FAIL
      SMD_OBSERVED="required mindiff subject is absent"
      SMD_MEANING="The required mindiff configuration is missing, so password changes need not replace 50 percent of characters."
      SMD_FIX="chsec -f /etc/security/user -s default -a mindiff=8; chsec -f /etc/security/user -s [user_name] -a mindiff=8; PTxray only recommends these commands."
      ;;
    probe_failed) SMD_STATUS=NOT_ASSESSED
      SMD_OBSERVED="not assessed - mindiff probe failed (lssec_rc=$SMD_DEF_RC lsuser_rc=$SMD_USR_RC)"
      SMD_MEANING="PTxray did not obtain trustworthy mindiff evidence."
      SMD_FIX="run 'lssec -f /etc/security/user -s default -a mindiff' and 'lsuser -a mindiff ALL', correct the capture problem, and rerun PTxray."
      ;;
    probe_empty) SMD_STATUS=NOT_ASSESSED
      SMD_OBSERVED="not assessed - mindiff probe returned no output"
      SMD_MEANING="PTxray did not obtain trustworthy mindiff evidence."
      SMD_FIX="run 'lssec -f /etc/security/user -s default -a mindiff' and 'lsuser -a mindiff ALL', correct the capture problem, and rerun PTxray."
      ;;
    *)
      SMD_STATUS=NOT_ASSESSED
      SMD_OBSERVED="not assessed - mindiff probe output was unparseable"
      SMD_MEANING="PTxray did not obtain trustworthy mindiff evidence."
      SMD_FIX="run 'lssec -f /etc/security/user -s default -a mindiff' and 'lsuser -a mindiff ALL', correct the capture problem, and rerun PTxray."
      ;;
  esac
  ...
  case "$SMD_STATUS" in
    FAIL) SMD_SEV=high ;;
    NOT_APPLICABLE) SMD_SEV=low ;;
    *) SMD_SEV=med ;;
  esac

  add security stig_user_mindiff "Password character-change minimum (mindiff)" \
    "$SMD_STATUS" "$SMD_SEV" "$SMD_OBSERVED" \
    "$SMD_MEANING" "$SMD_FIX" "stig:V-215220"
}
