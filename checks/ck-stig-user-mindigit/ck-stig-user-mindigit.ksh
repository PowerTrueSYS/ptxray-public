...
AIXRAY_TOOL=ck-stig-user-mindigit


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_user_mindigit — DISA STIG V-215219.
  # Required configuration: default and every user have mindigit >= 1.
  # Uncertain probe evidence is refused; a missing subject is a finding.
  typeset UMD_DEF_RAW UMD_DEF_RC UMD_USR_RAW UMD_USR_RC
  typeset UMD_PARSED UMD_CLASS UMD_DETAIL
  typeset UMD_STATUS UMD_SEV UMD_OBSERVED UMD_MEANING UMD_FIX

  UMD_DEF_RAW=$(aix lssec_user_default_mindigit lssec -f /etc/security/user -s default -a mindigit)
  UMD_DEF_RC=$?
  UMD_USR_RAW=$(aix lsuser_mindigit_all lsuser -a mindigit ALL)
  UMD_USR_RC=$?
  UMD_CLASS=""
  UMD_DETAIL=""

  if [ "$UMD_DEF_RC" -ne 0 ] || [ "$UMD_USR_RC" -ne 0 ]; then
    UMD_CLASS=probe_failed
  elif [ -z "$UMD_DEF_RAW" ] || [ -z "$UMD_USR_RAW" ]; then
    UMD_CLASS=probe_empty
  else
    UMD_PARSED=$(
      printf '%s\n' "AIXRAY_DEFAULT" "$UMD_DEF_RAW" "AIXRAY_USERS" "$UMD_USR_RAW" | awk '
        function is_user(u) {
          return (u ~ /^[A-Za-z_][A-Za-z0-9_.-]*$/)
        }
        function too_small(n) {
          return (n < 1)
        }
        function addfail(item) {
          if (fails != "") fails = fails " "
          fails = fails item
          nfail++
        }
        BEGIN { phase = 0 }
        $0 == "AIXRAY_DEFAULT" { phase = 1; next }
        $0 == "AIXRAY_USERS" { phase = 2; next }
        {
          gsub(/\r/, "")
          if ($0 ~ /^[ \t]*$/) next
          if (phase == 1) {
            def_records++
            if (def_records != 1) { def_bad = 1; next }
            if ($1 != "default") { def_bad = 1; next }
            rest = $0
            sub(/^[^ \t]+[ \t]*/, "", rest)
            if (rest == "") { def_absent = 1; next }
            if (index(rest, "mindigit=") != 1) { def_bad = 1; next }
            val = substr(rest, 10)
            if (val == "") addfail("default=unset")
            else if (val !~ /^[0-9]+$/) def_bad = 1
            else {
              n = val + 0
              if (too_small(n)) addfail("default=" n)
            }
            next
          }
          if (phase == 2) {
            ...
            if (!is_user($1)) { usr_bad = 1; next }
            users++
            rest = $0
            sub(/^[^ \t]+[ \t]*/, "", rest)
            if (rest == "") { addfail($1 "=unset"); next }
            if (index(rest, "mindigit=") != 1) { usr_bad = 1; next }
            val = substr(rest, 10)
            if (val == "") addfail($1 "=unset")
            else if (val !~ /^[0-9]+$/) usr_bad = 1
            else {
              n = val + 0
              if (too_small(n)) addfail($1 "=" n)
            }
          }
        }
        END {
          if (phase < 2 || def_bad || usr_bad) { print "unparseable"; exit 0 }
          if (usr_absent && users > 0) { print "unparseable"; exit 0 }
          if (def_records == 0) { print "unparseable"; exit 0 }
          if (def_absent || (usr_absent && users == 0)) { print "absent"; exit 0 }
    ...
    if [ -z "$UMD_PARSED" ]; then
      UMD_CLASS=unparseable
    else
      case "$UMD_PARSED" in
        ok) UMD_CLASS=compliant ;;
        fail\ *)
          UMD_CLASS=noncompliant
          UMD_DETAIL=${UMD_PARSED#fail }
          ;;
        absent) UMD_CLASS=subject_absent ;;
        *) UMD_CLASS=unparseable ;;
      esac
    fi
  fi
  ...
  case "$UMD_CLASS" in
    compliant)
      UMD_STATUS=PASS
      UMD_OBSERVED="default and all listed users have mindigit>=1"
      UMD_MEANING="Default and every listed user require at least one numeric character in passwords."
      UMD_FIX="n/a"
      ;;
    noncompliant) UMD_STATUS=FAIL
      UMD_OBSERVED="mindigit unset or less than 1: $UMD_DETAIL"
      UMD_MEANING="Default or a listed user has mindigit unset or less than 1, so passwords with no digits are allowed."
      UMD_FIX="chsec -f /etc/security/user -s default -a mindigit=1; chsec -f /etc/security/user -s [user_name] -a mindigit=1; PTxray only recommends these commands."
      ;;
    subject_absent) UMD_STATUS=FAIL
      UMD_OBSERVED="required mindigit subject is absent"
      UMD_MEANING="The required mindigit configuration is missing, so passwords with no digits are allowed."
      UMD_FIX="chsec -f /etc/security/user -s default -a mindigit=1; chsec -f /etc/security/user -s [user_name] -a mindigit=1; PTxray only recommends these commands."
      ;;
    probe_failed) UMD_STATUS=NOT_ASSESSED
      UMD_OBSERVED="not assessed - mindigit probe failed (lssec_rc=$UMD_DEF_RC lsuser_rc=$UMD_USR_RC)"
      UMD_MEANING="PTxray did not obtain trustworthy mindigit evidence."
      UMD_FIX="run 'lssec -f /etc/security/user -s default -a mindigit' and 'lsuser -a mindigit ALL', correct the capture problem, and rerun PTxray."
      ;;
    probe_empty) UMD_STATUS=NOT_ASSESSED
      UMD_OBSERVED="not assessed - mindigit probe returned no output"
      UMD_MEANING="PTxray did not obtain trustworthy mindigit evidence."
      UMD_FIX="run 'lssec -f /etc/security/user -s default -a mindigit' and 'lsuser -a mindigit ALL', correct the capture problem, and rerun PTxray."
      ;;
    *)
      UMD_STATUS=NOT_ASSESSED
      UMD_OBSERVED="not assessed - mindigit probe output was unparseable"
      UMD_MEANING="PTxray did not obtain trustworthy mindigit evidence."
      UMD_FIX="run 'lssec -f /etc/security/user -s default -a mindigit' and 'lsuser -a mindigit ALL', correct the capture problem, and rerun PTxray."
      ;;
  esac
  ...
  case "$UMD_STATUS" in
    FAIL) UMD_SEV=high ;;
    NOT_APPLICABLE) UMD_SEV=low ;;
    *) UMD_SEV=med ;;
  esac

  add security stig_user_mindigit "Password numeric complexity (mindigit)" \
    "$UMD_STATUS" "$UMD_SEV" "$UMD_OBSERVED" \
    "$UMD_MEANING" "$UMD_FIX" "stig:V-215219"
}
