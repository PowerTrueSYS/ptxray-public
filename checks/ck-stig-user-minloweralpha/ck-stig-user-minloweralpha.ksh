...
AIXRAY_TOOL=ck-stig-user-minloweralpha


function standalone_check {
_AIXRAY_SESSION_KEYS=""
# ck-stig-user-minloweralpha -- DISA STIG V-215218.
# Require default minloweralpha >= 1 and every account minloweralpha >= 1.
# Unset is a finding. Probe failure, empty rc0, or unparseable output refuses.

function ck_stig_user_minloweralpha {
  typeset MLA_FLOOR MLA_UNSET_FAIL
  typeset MLA_LSSEC_RAW MLA_LSSEC_RC MLA_LSUSER_RAW MLA_LSUSER_RC
  typeset MLA_DEF MLA_DEF_RC MLA_USERS MLA_USERS_RC
  typeset MLA_KIND MLA_VAL MLA_UKIND MLA_UVAL MLA_FAILS
  typeset MLA_TITLE MLA_NA_MEAN MLA_NA_FIX MLA_FAIL_MEAN MLA_FAIL_FIX MLA_PASS_MEAN

  MLA_FLOOR=1
  MLA_UNSET_FAIL=1
  MLA_TITLE="Password complexity requires a lower-case character"
  MLA_NA_MEAN="PTxray could not obtain a trustworthy minloweralpha reading from lssec and lsuser."
  MLA_NA_FIX="correct the lssec/lsuser capture and rerun PTxray."
  MLA_FAIL_MEAN="AIX does not require at least one lower-case character in passwords for the default stanza or for one or more accounts (minloweralpha not set or less than 1)."
  MLA_FAIL_FIX="chsec -f /etc/security/user -s default -a minloweralpha=1; chsec -f /etc/security/user -s [user_name] -a minloweralpha=1"
  MLA_PASS_MEAN="AIX requires at least one lower-case character in passwords: default minloweralpha and every listed account meet the floor of 1."

  MLA_LSSEC_RAW=$(aix lssec_user_default_minloweralpha lssec -f /etc/security/user -s default -a minloweralpha)
  MLA_LSSEC_RC=$?
  MLA_LSUSER_RAW=$(aix lsuser_minloweralpha_all lsuser -a minloweralpha ALL)
  MLA_LSUSER_RC=$?

  if [ "$MLA_LSSEC_RC" -ne 0 ]; then
    add security stig_user_minloweralpha "$MLA_TITLE" NOT_ASSESSED med \
      "not assessed - lssec failed (rc=$MLA_LSSEC_RC)" \
      "$MLA_NA_MEAN" "$MLA_NA_FIX" "stig:V-215218"
    return
  fi
  if [ "$MLA_LSUSER_RC" -ne 0 ]; then
    add security stig_user_minloweralpha "$MLA_TITLE" NOT_ASSESSED med \
      "not assessed - lsuser failed (rc=$MLA_LSUSER_RC)" \
      "$MLA_NA_MEAN" "$MLA_NA_FIX" "stig:V-215218"
    return
  fi
  if [ -z "$MLA_LSSEC_RAW" ]; then
    add security stig_user_minloweralpha "$MLA_TITLE" NOT_ASSESSED med \
      "not assessed - lssec output empty" \
      "$MLA_NA_MEAN" "$MLA_NA_FIX" "stig:V-215218"
    return
  fi
  if [ -z "$MLA_LSUSER_RAW" ]; then
    add security stig_user_minloweralpha "$MLA_TITLE" NOT_ASSESSED med \
      "not assessed - lsuser output empty" \
      "$MLA_NA_MEAN" "$MLA_NA_FIX" "stig:V-215218"
    return
  fi

  MLA_DEF=$(printf '%s\n' "$MLA_LSSEC_RAW" | awk -v floor="$MLA_FLOOR" '
    BEGIN { found=0; rec=0; bad=0; val="" }
    NF {
      rec++
      if ($1 != "default") bad=1
      for (i=1; i<=NF; i++) {
        if (index($i, "minloweralpha=") == 1) {
          found++
          val=substr($i, length("minloweralpha=")+1)
        }
      }
    }
    END {
      if (rec < 1) { print "EMPTY:"; exit 2 }
      if (rec != 1 || bad || found > 1) { print "BAD:"; exit 1 }
      if (found == 0 || val == "") { print "UNSET:"; exit 0 }
      if (val !~ /^[0-9][0-9]*$/) { print "BAD:"; exit 1 }
      if (val+0 < floor+0) { print "LOW:" val; exit 0 }
      print "OK:" val
      exit 0
    }
  ')
  ...
  MLA_USERS=$(printf '%s\n' "$MLA_LSUSER_RAW" | awk -v floor="$MLA_FLOOR" -v unset_fail="$MLA_UNSET_FAIL" '
    BEGIN { rec=0; bad=0; nfail=0; list="" }
    NF {
      rec++
      user=$1
      found=0
      ...
      if (user == "" || found != 1) { bad=1; next }
      if (val == "") {
        if (unset_fail+0 == 1) {
          nfail++
          item=user " minloweralpha is not set (need >= 1)"
          if (list == "") list=item; else list=list "; " item
        }
        next
      }
      ...
      if (val+0 < floor+0) {
        nfail++
        item=user " minloweralpha=" val " (need >= 1)"
        if (list == "") list=item; else list=list "; " item
      }
    }
    END {
  ...
  if [ "$MLA_DEF_RC" -ne 0 ] || [ "$MLA_DEF" = "EMPTY:" ] || [ "$MLA_DEF" = "BAD:" ]; then
    add security stig_user_minloweralpha "$MLA_TITLE" NOT_ASSESSED med \
      "not assessed - unparseable lssec output" \
      "$MLA_NA_MEAN" "$MLA_NA_FIX" "stig:V-215218"
    return
  fi
  if [ "$MLA_USERS_RC" -ne 0 ] || [ "$MLA_USERS" = "EMPTY:" ] || [ "$MLA_USERS" = "BAD:" ]; then
    add security stig_user_minloweralpha "$MLA_TITLE" NOT_ASSESSED med \
      "not assessed - unparseable lsuser output" \
      "$MLA_NA_MEAN" "$MLA_NA_FIX" "stig:V-215218"
    return
  fi
  ...
  MLA_FAILS=""
  if [ "$MLA_KIND" = UNSET ]; then
    if [ "$MLA_UNSET_FAIL" -eq 1 ]; then
      MLA_FAILS="default minloweralpha is not set (need >= 1)"
    fi
  elif [ "$MLA_KIND" = LOW ]; then
    MLA_FAILS="default minloweralpha=$MLA_VAL (need >= 1)"
  elif [ "$MLA_KIND" != OK ]; then
    add security stig_user_minloweralpha "$MLA_TITLE" NOT_ASSESSED med \
      "not assessed - unparseable lssec output" \
      "$MLA_NA_MEAN" "$MLA_NA_FIX" "stig:V-215218"
    return
  ...
  if [ "$MLA_UKIND" = FAIL ]; then
    if [ -z "$MLA_FAILS" ]; then
      MLA_FAILS=$MLA_UVAL
    else
      MLA_FAILS="$MLA_FAILS; $MLA_UVAL"
    fi
  elif [ "$MLA_UKIND" != OK ]; then
    add security stig_user_minloweralpha "$MLA_TITLE" NOT_ASSESSED med \
      "not assessed - unparseable lsuser output" \
      "$MLA_NA_MEAN" "$MLA_NA_FIX" "stig:V-215218"
    return
  ...
  if [ -n "$MLA_FAILS" ]; then
    add security stig_user_minloweralpha "$MLA_TITLE" FAIL high \
      "$MLA_FAILS" "$MLA_FAIL_MEAN" "$MLA_FAIL_FIX" "stig:V-215218"
    return
  fi

  add security stig_user_minloweralpha "$MLA_TITLE" PASS med \
    "default minloweralpha=$MLA_VAL; all accounts minloweralpha>=1" \
    "$MLA_PASS_MEAN" "n/a" "stig:V-215218"
}
ck_stig_user_minloweralpha
}
