...
AIXRAY_TOOL=ck-stig-user-minspecialchar


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_user_minspecialchar — DISA STIG V-215227.
  # Required configuration: default and every user have minspecialchar >= 1.
  # Uncertain probe evidence is refused; a missing subject is a finding.
  typeset MSC_DEF_RAW MSC_DEF_RC MSC_USR_RAW MSC_USR_RC
  typeset MSC_DEF_KIND MSC_USR_KIND MSC_DEF_DETAIL MSC_USR_DETAIL
  typeset MSC_CLASS MSC_DETAIL
  typeset MSC_STATUS MSC_SEV MSC_OBSERVED MSC_MEANING MSC_FIX

  MSC_DEF_RAW=$(aix lssec_user_default_minspecialchar lssec -f /etc/security/user -s default -a minspecialchar)
  MSC_DEF_RC=$?
  MSC_USR_RAW=$(aix lsuser_minspecialchar_all lsuser -a minspecialchar ALL)
  MSC_USR_RC=$?
  MSC_CLASS=""
  MSC_DETAIL=""
  MSC_DEF_KIND=""
  ...
  if [ "$MSC_DEF_RC" -ne 0 ] || [ "$MSC_USR_RC" -ne 0 ]; then
    MSC_CLASS=probe_failed
  elif [ -z "$MSC_DEF_RAW" ] || [ -z "$MSC_USR_RAW" ]; then
    MSC_CLASS=probe_empty
  else
    MSC_DEF_KIND=$(printf '%s\n' "$MSC_DEF_RAW" | awk '
      {
        gsub(/\r/, "")
        if ($0 ~ /^[ \t]*$/) next
        records++
        ...
        if (index(rest, "minspecialchar=") != 1) { bad = 1; next }
        val = substr(rest, 16)
        if (val == "") kind = "fail default=empty"
        else if (val !~ /^[0-9]+$/) { bad = 1; next }
        else if ((val + 0) < 1) kind = "fail default=" val
        else kind = "ok"
      }
      END {
    ...
    MSC_USR_KIND=$(printf '%s\n' "$MSC_USR_RAW" | awk '
      ...
      }
      {
        ...
        if (index(rest, "minspecialchar=") != 1) {
          bad = 1
          next
        }
        val = substr(rest, 16)
    ...
    ')
    case "$MSC_DEF_KIND" in
      fail\ *)
        MSC_DEF_DETAIL=${MSC_DEF_KIND#fail }
        MSC_DEF_KIND=fail
        ;;
    esac
    case "$MSC_USR_KIND" in
      fail\ *)
        MSC_USR_DETAIL=${MSC_USR_KIND#fail }
        MSC_USR_KIND=fail
        ;;
    esac
    if [ -z "$MSC_DEF_KIND" ] || [ -z "$MSC_USR_KIND" ]; then
      MSC_CLASS=unparseable
    elif [ "$MSC_DEF_KIND" = unparseable ] || [ "$MSC_USR_KIND" = unparseable ]; then
      MSC_CLASS=unparseable
    elif [ "$MSC_DEF_KIND" = absent ] || [ "$MSC_USR_KIND" = absent ]; then
      MSC_CLASS=subject_absent
    elif [ "$MSC_DEF_KIND" = fail ] || [ "$MSC_USR_KIND" = fail ]; then
      MSC_CLASS=noncompliant
      if [ "$MSC_DEF_KIND" = fail ]; then
        MSC_DETAIL=$MSC_DEF_DETAIL
      fi
      if [ "$MSC_USR_KIND" = fail ]; then
        if [ -n "$MSC_DETAIL" ]; then
          MSC_DETAIL="$MSC_DETAIL $MSC_USR_DETAIL"
        else
          MSC_DETAIL=$MSC_USR_DETAIL
        fi
      fi
    elif [ "$MSC_DEF_KIND" = ok ] && [ "$MSC_USR_KIND" = ok ]; then
      MSC_CLASS=compliant
    else
      MSC_CLASS=unparseable
    fi
  ...
  case "$MSC_CLASS" in
    compliant)
      MSC_STATUS=PASS
      MSC_OBSERVED="default minspecialchar>=1 and all listed accounts minspecialchar>=1"
      MSC_MEANING="Default and every listed account have minspecialchar set to at least 1, so passwords must contain a special character."
      MSC_FIX="n/a"
      ;;
    noncompliant) MSC_STATUS=FAIL
      MSC_OBSERVED="minspecialchar 0 or unset: $MSC_DETAIL"
      MSC_MEANING="Default or a listed account has minspecialchar 0 or unset, so passwords need not contain a special character."
      MSC_FIX='chsec -f /etc/security/user -s default -a minspecialchar=1; chsec -f /etc/security/user -s [user_name] -a minspecialchar=1; PTxray only recommends these commands.'
      ;;
    subject_absent) MSC_STATUS=FAIL
      MSC_OBSERVED="required minspecialchar subject is absent"
      MSC_MEANING="The required minspecialchar configuration is missing, so passwords need not contain a special character."
      MSC_FIX='chsec -f /etc/security/user -s default -a minspecialchar=1; chsec -f /etc/security/user -s [user_name] -a minspecialchar=1; PTxray only recommends these commands.'
      ;;
    probe_failed) MSC_STATUS=NOT_ASSESSED
      MSC_OBSERVED="not assessed - minspecialchar probe failed (lssec_rc=$MSC_DEF_RC lsuser_rc=$MSC_USR_RC)"
      MSC_MEANING="PTxray did not obtain trustworthy minspecialchar evidence."
      MSC_FIX="run 'lssec -f /etc/security/user -s default -a minspecialchar' and 'lsuser -a minspecialchar ALL', correct the capture problem, and rerun PTxray."
      ;;
    probe_empty) MSC_STATUS=NOT_ASSESSED
      MSC_OBSERVED="not assessed - minspecialchar probe returned no output"
      MSC_MEANING="PTxray did not obtain trustworthy minspecialchar evidence."
      MSC_FIX="run 'lssec -f /etc/security/user -s default -a minspecialchar' and 'lsuser -a minspecialchar ALL', correct the capture problem, and rerun PTxray."
      ;;
    *)
      MSC_STATUS=NOT_ASSESSED
      MSC_OBSERVED="not assessed - minspecialchar probe output was unparseable"
      MSC_MEANING="PTxray did not obtain trustworthy minspecialchar evidence."
      MSC_FIX="run 'lssec -f /etc/security/user -s default -a minspecialchar' and 'lsuser -a minspecialchar ALL', correct the capture problem, and rerun PTxray."
      ;;
  esac
  ...
  case "$MSC_STATUS" in
    NOT_APPLICABLE) MSC_SEV=low ;;
    *) MSC_SEV=med ;;
  esac

  add security stig_user_minspecialchar "Password minspecialchar on default and every user" \
    "$MSC_STATUS" "$MSC_SEV" "$MSC_OBSERVED" \
    "$MSC_MEANING" "$MSC_FIX" "stig:V-215227"
}
