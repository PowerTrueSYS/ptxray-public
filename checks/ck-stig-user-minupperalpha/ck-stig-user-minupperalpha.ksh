...
AIXRAY_TOOL=ck-stig-user-minupperalpha


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_user_minupperalpha - DISA STIG V-215217.
  # Default stanza and every account must have minupperalpha >= 1.
  # Unset default is unmet required configuration (FAIL), not NA.
  # Probe rc != 0, empty rc0, or unparseable -> NOT_ASSESSED.
  typeset MUA_DEF_RAW MUA_DEF_RC MUA_USR_RAW MUA_USR_RC
  typeset MUA_REASON MUA_EVAL MUA_EVAL_RC MUA_KIND MUA_DETAIL

  MUA_DEF_RAW=$(aix lssec_user_default_minupperalpha lssec -f /etc/security/user -s default -a minupperalpha)
  MUA_DEF_RC=$?
  MUA_USR_RAW=$(aix lsuser_minupperalpha_all lsuser -a minupperalpha ALL)
  MUA_USR_RC=$?
  MUA_REASON=""
  MUA_EVAL=""
  MUA_KIND=""
  ...
  if [ "$MUA_DEF_RC" -ne 0 ]; then
    MUA_REASON="lssec failed (rc=$MUA_DEF_RC)"
  elif [ -z "$MUA_DEF_RAW" ]; then
    MUA_REASON="empty lssec output"
  elif [ "$MUA_USR_RC" -ne 0 ]; then
    MUA_REASON="lsuser failed (rc=$MUA_USR_RC)"
  elif [ -z "$MUA_USR_RAW" ]; then
    MUA_REASON="empty lsuser output"
  else
    MUA_EVAL=$(printf '%s\n' "AIXRAY_MUA_DEFAULT" "$MUA_DEF_RAW" "AIXRAY_MUA_USERS" "$MUA_USR_RAW" | awk '
      BEGIN {
        phase=0
        def_bad=0
        usr_bad=0
      ...
      $0=="AIXRAY_MUA_DEFAULT" { phase=1; next }
      $0=="AIXRAY_MUA_USERS" { phase=2; next }
      NF==0 { next }
      phase==1 {
        ...
        for (i=2; i<=NF; i++) {
          eq=index($i, "=")
          if (eq<2) { def_bad=1; continue }
          attr=substr($i, 1, eq-1)
          val=substr($i, eq+1)
          if (attr=="minupperalpha") {
            found_attr=1
            dval=val
          }
        }
      ...
      phase==2 {
        if (NF<2 || index($1, "=")>0) { usr_bad=1; next }
        user=$1
        found_ua=0
        uval=""
        for (i=2; i<=NF; i++) {
          if (index($i, "minupperalpha=")==1) {
            found_ua=1
            uval=substr($i, index($i, "=")+1)
          } else if (index($i, "=")<2) {
            usr_bad=1
          }
        }
        if (!found_ua) { usr_bad=1; next }
        found_usr++
        if (uval=="") {
          if (nfail++) fails=fails "; "
          fails=fails user " minupperalpha not set (need >= 1)"
        } else if (uval !~ /^-?[0-9]+$/) {
          usr_bad=1
        } else if ((uval + 0) < 1) {
          if (nfail++) fails=fails "; "
          fails=fails user " minupperalpha=" uval " (need >= 1)"
        }
        next
      }
      { def_bad=1 }
      END {
        if (!found_default) def_bad=1
        if (!found_attr || dval=="") { def_unset=1 }
        if (def_bad) { print "ASSESS|unparseable lssec output"; exit 0 }
        if (usr_bad) { print "ASSESS|unparseable lsuser output"; exit 0 }
        if (found_usr==0) { print "ASSESS|unparseable lsuser output"; exit 0 }
        if (def_unset) {
          if (nfail++) fails=fails "; "
          fails=fails "default minupperalpha not set (need >= 1)"
        } else if (dval !~ /^-?[0-9]+$/) {
          print "ASSESS|unparseable lssec output"
          exit 0
        } else if ((dval + 0) < 1) {
          if (nfail++) fails=fails "; "
          fails=fails "default minupperalpha=" dval " (need >= 1)"
        }
        if (nfail>0) print "FAIL|" fails
        else print "PASS|default minupperalpha=" dval "; every account minupperalpha>=1"
      }
    ')
    MUA_EVAL_RC=$?
    if [ "$MUA_EVAL_RC" -ne 0 ] || [ -z "$MUA_EVAL" ]; then
      MUA_REASON="unparseable minupperalpha evidence"
    else
      MUA_KIND=$(printf '%s\n' "$MUA_EVAL" | awk -F'|' '{print $1}')
      MUA_DETAIL=$(printf '%s\n' "$MUA_EVAL" | awk -F'|' '{print substr($0, index($0, "|")+1)}')
      if [ "$MUA_KIND" = "ASSESS" ]; then
        MUA_REASON="$MUA_DETAIL"
      elif [ "$MUA_KIND" != "FAIL" ] && [ "$MUA_KIND" != "PASS" ]; then
        MUA_REASON="unparseable minupperalpha evidence"
      fi
    fi
  fi

  if [ -n "$MUA_REASON" ]; then
    add security stig_user_minupperalpha "Password uppercase character requirement" NOT_ASSESSED med \
        "not assessed - $MUA_REASON" \
        "PTxray could not obtain trustworthy minupperalpha values from lssec and lsuser." \
        "correct the lssec/lsuser capture problem and rerun PTxray." \
        "stig:V-215217"
  elif [ "$MUA_KIND" = "FAIL" ]; then
    add security stig_user_minupperalpha "Password uppercase character requirement" FAIL high \
        "$MUA_DETAIL" \
        "AIX is not requiring at least one upper-case character in passwords: default minupperalpha or an account minupperalpha is unset or less than 1." \
        "chsec -f /etc/security/user -s default -a minupperalpha=1; chsec -f /etc/security/user -s [user_name] -a minupperalpha=1" \
        "stig:V-215217"
  else
    add security stig_user_minupperalpha "Password uppercase character requirement" PASS med \
        "$MUA_DETAIL" \
        "The default stanza and every account require at least one upper-case character in passwords (minupperalpha >= 1)." \
        "n/a" \
        "stig:V-215217"
  fi
}
