...
AIXRAY_TOOL=ck-stig-user-pwd-algorithm


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_user_pwd_algorithm — DISA STIG V-215230 (AIX7-00-001134).
  # Combined clauses: pwd_algorithm is ssha256 or ssha512; no hashes in
  # /etc/passwd field 2; every /etc/security/passwd hash begins with
  # {ssha256} or {ssha512}. Missing pwd_algorithm is FAIL (required
  # configuration unmet). Probe rc != 0 or rc 0 + empty output is
  # NOT_ASSESSED. Unparseable stored-hash records are NOT_ASSESSED.
  # Empty, *, and ! password values are not hashes.
  # STIG check-content (each complete command is one aix probe):
  # lssec -f /etc/security/login.cfg -s usw -a pwd_algorithm
  # cat /etc/passwd | cut -f2,2 -d":"
  # cat /etc/security/passwd | grep password
  typeset SUPA_ALG_RAW SUPA_ALG_RC SUPA_PW_RAW SUPA_PW_RC SUPA_SP_RAW SUPA_SP_RC
  typeset SUPA_FAILED SUPA_FAIL_RC SUPA_EMPTY SUPA_PARSED SUPA_STATE SUPA_VALUE
  typeset SUPA_F2_N SUPA_SP_N SUPA_SP_PARSED SUPA_SP_STATE
  typeset SUPA_KIND SUPA_STATUS SUPA_SEV SUPA_OBSERVED SUPA_MEANING SUPA_FIX
  typeset SUPA_REASONS
  ...
  SUPA_ALG_RAW=$(aix pwd_algorithm lssec -f /etc/security/login.cfg -s usw -a pwd_algorithm)
  SUPA_ALG_RC=$?
  SUPA_PW_RAW=$(aix passwd_field2 sh -c 'cat /etc/passwd | cut -f2,2 -d":"')
  SUPA_PW_RC=$?
  SUPA_SP_RAW=$(aix security_passwd sh -c 'cat /etc/security/passwd | grep password')
  SUPA_SP_RC=$?

  if [ "$SUPA_ALG_RC" -ne 0 ]; then
    SUPA_FAILED=1
    SUPA_FAIL_RC=$SUPA_ALG_RC
  elif [ -z "$SUPA_ALG_RAW" ]; then
    SUPA_EMPTY=1
  fi
  if [ "$SUPA_PW_RC" -ne 0 ]; then
    SUPA_FAILED=1
    SUPA_FAIL_RC=$SUPA_PW_RC
  elif [ -z "$SUPA_PW_RAW" ]; then
    SUPA_EMPTY=1
  fi
  if [ "$SUPA_SP_RC" -ne 0 ]; then
    SUPA_FAILED=1
    SUPA_FAIL_RC=$SUPA_SP_RC
  elif [ -z "$SUPA_SP_RAW" ]; then
    SUPA_EMPTY=1
  fi

  if [ "$SUPA_FAILED" -eq 0 ] && [ "$SUPA_EMPTY" -eq 0 ]; then
    SUPA_PARSED=$(printf '%s\n' "$SUPA_ALG_RAW" | awk '
      NF {
        rows++
        if ($1 != "usw") bad=1
        for (i=2; i<=NF; i++) {
      ...
      END {
        ...
        else if (rows!=1 || bad || attrs>1) print "unparseable|"
        else if (attrs==0 || value=="") print "absent|"
        else if (value ~ /^[A-Za-z0-9_.-]+$/) print "value|" value
        else print "unparseable|"
      }')
    SUPA_STATE=${SUPA_PARSED%%\|*}
    SUPA_VALUE=${SUPA_PARSED#*\|}
    ...
    SUPA_SP_PARSED=$(printf '%s\n' "$SUPA_SP_RAW" | awk '
      /^#/ { next }
      /^[ \t]*$/ { next }
      {
        if ($1 != "password" || $2 != "=" || NF > 3) {
          bad++
          next
        }
        val=""
        if (NF == 3) val=$3
        if (val == "" || val == "*" || val == "!") next
        if (index(val, "{ssha256}")==1) next
        if (index(val, "{ssha512}")==1) next
        n++
      }
      END {
        if (bad) print "unparseable|0"
        else print "ok|" n+0
      }
    ')
    SUPA_SP_STATE=${SUPA_SP_PARSED%%\|*}
    SUPA_SP_N=${SUPA_SP_PARSED#*\|}

    if [ "$SUPA_STATE" = "value" ]; then
      case "$SUPA_VALUE" in
        ssha256|ssha512) ;;
        *) SUPA_STATE=alg_bad ;;
      esac
  ...
  if [ "$SUPA_FAILED" -eq 1 ]; then
    SUPA_KIND=failed
  elif [ "$SUPA_EMPTY" -eq 1 ]; then
    SUPA_KIND=empty
  elif [ "$SUPA_STATE" = "unparseable" ] || [ "$SUPA_STATE" = "empty" ] || \
       [ "$SUPA_SP_STATE" = "unparseable" ]; then
    SUPA_KIND=unparseable
  elif [ "$SUPA_STATE" = "absent" ] && [ "$SUPA_F2_N" -eq 0 ] && [ "$SUPA_SP_N" -eq 0 ]; then
    SUPA_KIND=absent
  elif [ "$SUPA_STATE" = "absent" ] || [ "$SUPA_STATE" = "alg_bad" ] || \
       [ "$SUPA_F2_N" -gt 0 ] || [ "$SUPA_SP_N" -gt 0 ]; then
    SUPA_KIND=violation
  else
    SUPA_KIND=ok
  fi
  ...
  case "$SUPA_KIND" in
    ok)
      SUPA_STATUS=PASS
      SUPA_OBSERVED="pwd_algorithm=$SUPA_VALUE; no hashes in /etc/passwd field 2; stored hashes begin with {ssha256} or {ssha512}"
      SUPA_MEANING="Password hashes are generated and stored using a FIPS 140-2 approved algorithm (ssha256 or ssha512), with no hashes in /etc/passwd."
      SUPA_FIX="n/a"
      ;;
    absent)
      SUPA_STATUS=FAIL
      SUPA_OBSERVED="pwd_algorithm unset (required configuration unmet)"
      SUPA_MEANING="An unset pwd_algorithm defaults to legacy DES crypt, so stored password hashes are not generated with a FIPS 140-2 approved algorithm."
      SUPA_FIX="chsec -f /etc/security/login.cfg -s usw -a pwd_algorithm=ssha512; passwd [user_name]"
      ;;
    violation)
      SUPA_STATUS=FAIL
      if [ "$SUPA_STATE" = "absent" ]; then
        SUPA_REASONS="pwd_algorithm unset (required configuration unmet)"
      elif [ "$SUPA_STATE" = "alg_bad" ]; then
        SUPA_REASONS="pwd_algorithm=$SUPA_VALUE (requires ssha256 or ssha512)"
      else
        SUPA_REASONS="pwd_algorithm=$SUPA_VALUE"
      fi
      if [ "$SUPA_F2_N" -gt 0 ]; then
        SUPA_REASONS="$SUPA_REASONS; password hashes in /etc/passwd field 2"
      fi
      if [ "$SUPA_SP_N" -gt 0 ]; then
        SUPA_REASONS="$SUPA_REASONS; stored hashes not beginning with {ssha256} or {ssha512}"
      fi
      SUPA_OBSERVED=$SUPA_REASONS
      SUPA_MEANING="The password hashing algorithm is not a FIPS 140-2 approved Loadable Password Algorithm, hashes appear in /etc/passwd, or stored hashes are not {ssha256} or {ssha512}."
      SUPA_FIX="chsec -f /etc/security/login.cfg -s usw -a pwd_algorithm=ssha512; passwd [user_name]"
      ;;
    failed)
      SUPA_STATUS=NOT_ASSESSED
      SUPA_OBSERVED="not assessed - password-hashing probe failed (rc=$SUPA_FAIL_RC)"
      SUPA_MEANING="PTxray did not obtain trustworthy evidence for the FIPS 140-2 password hashing requirement."
      SUPA_FIX="verify lssec pwd_algorithm, /etc/passwd, and /etc/security/passwd are readable, then rerun PTxray."
      ;;
    empty)
      SUPA_STATUS=NOT_ASSESSED
      SUPA_OBSERVED="not assessed - password-hashing probe returned empty output with rc 0"
      SUPA_MEANING="PTxray did not obtain trustworthy evidence for the FIPS 140-2 password hashing requirement."
      SUPA_FIX="verify lssec pwd_algorithm, /etc/passwd, and /etc/security/passwd return content, then rerun PTxray."
      ;;
    *)
      SUPA_STATUS=NOT_ASSESSED
      if [ "$SUPA_SP_STATE" = "unparseable" ]; then
        SUPA_OBSERVED="not assessed - /etc/security/passwd password records could not be parsed"
      else
        SUPA_OBSERVED="not assessed - pwd_algorithm output could not be parsed"
      fi
      SUPA_MEANING="PTxray did not obtain trustworthy evidence for the FIPS 140-2 password hashing requirement."
      SUPA_FIX="run 'lssec -f /etc/security/login.cfg -s usw -a pwd_algorithm' and inspect /etc/security/passwd, then rerun PTxray."
      ;;
  esac

  add security stig_user_pwd_algorithm "FIPS 140-2 approved password hashing algorithm" \
    "$SUPA_STATUS" "$SUPA_SEV" "$SUPA_OBSERVED" \
    "$SUPA_MEANING" "$SUPA_FIX" "stig:V-215230"
}
