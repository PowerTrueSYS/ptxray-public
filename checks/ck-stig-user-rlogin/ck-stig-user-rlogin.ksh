...
AIXRAY_TOOL=ck-stig-user-rlogin


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_user_rlogin — DISA STIG V-215178 (AIX7-00-001011).
  # Direct logins must not be permitted to shared, default, application, and
  # utility accounts.  PARTIAL-AUTO: the decidable name set is the default
  # accounts (adm, bin, daemon, guest, lpd, nobody, nuucp, sys, uucp) plus
  # IBM-shipped utility accounts (lp, sshd, slotd, invscout, esaadmin,
  # ragent, snapp, ipsec, smmsp).  rlogin=true on any of those is FAIL;
  # all present decidable accounts rlogin=false, or none present, is PASS
  # for that subset.  Human/application accounts have no account-class
  # attribute on AIX and remain a typed NOT_ASSESSED residual naming them.
  typeset SUR_RAW SUR_RC SUR_PARSE SUR_KIND SUR_DETAIL SUR_ONE SUR_RESIDUAL
  typeset SUR_STATUS SUR_SEV SUR_OBSERVED SUR_MEANING SUR_FIX
  typeset SUR_ISSO_STATUS SUR_ISSO_OBSERVED SUR_ISSO_MEANING SUR_ISSO_FIX
  typeset SUR_CLASS SUR_CLASS_RC SUR_SHIP SUR_SHIP_RC

  SUR_RAW=$(aix lsuser_rlogin_all lsuser -a rlogin ALL)
  SUR_RC=$?
  SUR_CLASS=$(aix lsuser_class_attrs lsuser -a id login rlogin shell account_locked ALL)
  SUR_CLASS_RC=$?
  SUR_SHIP=$(aix shipped_account_evidence lsuser -a id gecos loginretries maxage ALL)
  SUR_SHIP_RC=$?
  SUR_KIND=""
  SUR_DETAIL=""
  SUR_PARSE=""
  SUR_RESIDUAL=""
  SUR_ISSO_STATUS=NOT_ASSESSED
  SUR_ISSO_OBSERVED="not assessed - shared/application/utility accounts must be identified by the ISSO/ISSM"
  SUR_ISSO_MEANING="PTxray can decide rlogin for the default-account subset (adm, bin, daemon, guest, lpd, nobody, nuucp, sys, uucp) and the IBM-shipped utility accounts (lp, sshd, slotd, invscout, esaadmin, ragent, snapp, ipsec, smmsp). Human and application accounts have no account-class attribute and stay unassessed."
  SUR_ISSO_FIX="obtain the ISSO/ISSM list of shared, application, and remaining utility accounts, run 'lsuser -a rlogin' for each, and set rlogin=false with 'chuser rlogin=false [shared_account]'; PTxray only recommends these actions."
  if [ "$SUR_CLASS_RC" -ne 0 ] || [ "$SUR_SHIP_RC" -ne 0 ]; then
    : # extra lsuser probes are capture-keyed evidence; grade from rlogin ALL
  fi
  ...
  if [ "$SUR_RC" -ne 0 ]; then
    ...
    SUR_PARSE=$(printf '%s\n' "$SUR_RAW" | awk '
      function is_def(u) {
        if (u == "adm" || u == "bin" || u == "daemon" || u == "guest" || u == "lpd") return 1
        if (u == "nobody" || u == "nuucp" || u == "sys" || u == "uucp") return 1
        if (u == "lp" || u == "sshd" || u == "slotd" || u == "invscout" || u == "esaadmin") return 1
        if (u == "ragent" || u == "snapp" || u == "ipsec" || u == "smmsp") return 1
        return 0
      }
      function is_user(u) {
      ...
      }
      {
        gsub(/\r/, "")
        if ($0 ~ /^[ \t]*$/) next
        if ($0 ~ /3004-687 User "ALL" does not exist/) next
        if (!is_user($1)) { bad = 1; next }
        users++
        val = ""
        for (i = 2; i <= NF; i++) {
          if ($i ~ /^rlogin=/) {
        ...
        if (val != "true" && val != "false") { bad = 1; next }
        if (is_def($1)) {
          defs++
          if (oknames != "") oknames = oknames " "
          oknames = oknames $1
          if (val == "true") {
      ...
      END {
        ...
        if (nfail > 0) { print "fail " fails "/" residual; exit 0 }
        if (defs > 0) { print "ok " oknames "/" residual; exit 0 }
        print "absent/" residual
      }
    ')
    case "$SUR_PARSE" in
      unparseable) SUR_KIND=unparseable ;;
      absent*)
        SUR_KIND=absent
        SUR_RESIDUAL=${SUR_PARSE#absent}
        SUR_RESIDUAL=${SUR_RESIDUAL#/}
        ;;
      fail\ *)
        SUR_KIND=fail
        SUR_DETAIL=${SUR_PARSE#fail }
        SUR_RESIDUAL=${SUR_DETAIL#*/}
        SUR_DETAIL=${SUR_DETAIL%%/*}
        ;;
      ok\ *)
        SUR_KIND=ok
        SUR_DETAIL=${SUR_PARSE#ok }
        SUR_RESIDUAL=${SUR_DETAIL#*/}
        SUR_DETAIL=${SUR_DETAIL%%/*}
        ;;
      *) SUR_KIND=unparseable ;;
    esac
  fi

  case "$SUR_KIND" in
    failed)
      SUR_STATUS=NOT_ASSESSED
      SUR_OBSERVED="not assessed - lsuser probe failed (rc=$SUR_RC)"
      SUR_MEANING="PTxray did not obtain trustworthy rlogin evidence for default accounts."
      SUR_FIX="run 'lsuser -a rlogin ALL', correct the capture problem, and rerun PTxray."
      ;;
    empty)
      SUR_STATUS=NOT_ASSESSED
      SUR_OBSERVED="not assessed - lsuser returned no output with rc 0"
      SUR_MEANING="PTxray did not obtain trustworthy rlogin evidence for default accounts."
      SUR_FIX="run 'lsuser -a rlogin ALL', correct the capture problem, and rerun PTxray."
      ;;
    unparseable)
      SUR_STATUS=NOT_ASSESSED
      SUR_OBSERVED="not assessed - lsuser rlogin output could not be interpreted"
      SUR_MEANING="PTxray did not obtain trustworthy rlogin evidence for default accounts."
      SUR_FIX="run 'lsuser -a rlogin ALL', correct the capture problem, and rerun PTxray."
      ;;
    fail)
      SUR_STATUS=FAIL
      SUR_ONE=${SUR_DETAIL#* }
      if [ "$SUR_ONE" = "$SUR_DETAIL" ]; then
        SUR_OBSERVED="default account $SUR_DETAIL rlogin=true (must be false)"
      else
        SUR_OBSERVED="default accounts $SUR_DETAIL rlogin=true (must be false)"
      fi
      SUR_MEANING="A default AIX account allows direct rlogin, so a shared or built-in account can be reached over the network."
      SUR_FIX="chuser rlogin=false [shared_account]; PTxray only recommends this command."
      ;;
    ok)
      SUR_STATUS=PASS
      SUR_OBSERVED="default accounts rlogin=false ($SUR_DETAIL)"
      SUR_MEANING="Default AIX accounts have rlogin disabled."
      SUR_FIX="n/a"
      ;;
    absent)
      SUR_STATUS=PASS
      SUR_OBSERVED="no default accounts present to evaluate"
      SUR_MEANING="No default AIX accounts were listed, so none of them can accept a direct rlogin."
      SUR_FIX="n/a"
      ;;
    *)
      SUR_STATUS=NOT_ASSESSED
      SUR_OBSERVED="not assessed - unexpected rlogin parser result"
      SUR_MEANING="PTxray did not obtain trustworthy rlogin evidence for default accounts."
      SUR_FIX="run 'lsuser -a rlogin ALL', correct the capture problem, and rerun PTxray."
      ;;
  esac

  if [ -n "$SUR_RESIDUAL" ]; then
    SUR_ISSO_OBSERVED="not assessed - residual human/application accounts still need ISSO/ISSM review: $SUR_RESIDUAL"
  fi

  case "$SUR_STATUS" in
    NOT_APPLICABLE) SUR_SEV=low ;;
    *) SUR_SEV=med ;;
  esac

  add security stig_user_rlogin "Direct logins to shared, default, application, and utility accounts" \
    "$SUR_STATUS" "$SUR_SEV" "$SUR_OBSERVED" \
    "$SUR_MEANING" "$SUR_FIX" "stig:V-215178"

  add security stig_user_rlogin_isso "Direct logins to ISSO/ISSM-managed shared, application, and utility accounts" \
    "$SUR_ISSO_STATUS" med "$SUR_ISSO_OBSERVED" \
    "$SUR_ISSO_MEANING" "$SUR_ISSO_FIX" "stig:V-215178"
}
