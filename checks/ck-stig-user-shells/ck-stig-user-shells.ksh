...
AIXRAY_TOOL=ck-stig-user-shells


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_user_shells — DISA STIG V-215408. PARTIAL-AUTO.
  # Decidable: /etc/shells exists and login.cfg usw shells is a non-empty
  # subset of it. Missing file or empty shells attribute is unmet required
  # configuration (FAIL). Ask ISSO/SA for other approved shells is site
  # judgement and never a silent PASS.
  # ls -la /etc/shells
  # cat /etc/shells
  # lssec -f /etc/security/login.cfg -s usw -a shells
  typeset SUS_LS_RAW SUS_LS_RC SUS_CAT_RAW SUS_CAT_RC
  typeset SUS_LSSEC_RAW SUS_LSSEC_RC SUS_CLASS SUS_PARSE_RC
  typeset SUS_REASON SUS_STATUS SUS_SEV SUS_OBSERVED SUS_MEANING SUS_FIX

  SUS_LS_RAW=$(aix ls_etc_shells ls -la /etc/shells)
  SUS_LS_RC=$?
  SUS_CAT_RAW=$(aix cat_etc_shells cat /etc/shells)
  SUS_CAT_RC=$?
  SUS_LSSEC_RAW=$(aix lssec_login_cfg_shells lssec -f /etc/security/login.cfg -s usw -a shells)
  SUS_LSSEC_RC=$?
  SUS_REASON=""
  SUS_CLASS=""

  if [ "$SUS_LS_RC" -ne 0 ]; then
    case "$SUS_LS_RAW" in
      *'does not exist'*|*'No such file'*|*'not found'*)
        SUS_REASON=subject_absent
        ;;
      *)
        SUS_REASON=probe_failed
        ;;
    esac
  elif [ "$SUS_CAT_RC" -ne 0 ] || [ "$SUS_LSSEC_RC" -ne 0 ]; then
    SUS_REASON=probe_failed
  elif [ -z "$SUS_LS_RAW" ] || [ -z "$SUS_CAT_RAW" ] || [ -z "$SUS_LSSEC_RAW" ]; then
    SUS_REASON=probe_empty
  else
    case "$SUS_LS_RAW" in
      *'/etc/shells'*)
        SUS_CLASS=$(
          {
            ...
            BEGIN {
              want_stanza = "usw"
              nreq = split("/bin/sh /bin/bsh /bin/csh /bin/ksh /bin/tsh /bin/ksh93 /usr/bin/sh /usr/bin/bsh /usr/bin/csh /usr/bin/ksh /usr/bin/tsh /usr/bin/ksh93 /usr/bin/rksh /usr/bin/rksh93 /usr/sbin/uucp/uucico /usr/sbin/sliplogin /usr/sbin/snappd /bin/psh", req, " ")
              for (i = 1; i <= nreq; i++) def[req[i]] = 1
            }
            $0 == "__SUS_SEC_SHELLS__" { sec = "file"; next }
            $0 == "__SUS_SEC_LSSEC__" { sec = "lssec"; next }
            sec == "file" {
              line = $0
              gsub(/^[ \t]+/, "", line)
              gsub(/[ \t]+$/, "", line)
            ...
            sec == "lssec" {
              ...
              stanza = $1
              if (stanza != want_stanza) { bad = 1; next }
              rest = $0
              sub(/^[^ \t]+[ \t]*/, "", rest)
              if (rest == "") next
              if (index(rest, "shells=") != 1) { bad = 1; next }
              saw_key = 1
              val = substr(rest, 8)
              n = split(val, a, ",")
              for (i = 1; i <= n; i++) {
                ...
                gsub(/[ \t]+$/, "", s)
                if (s == "") continue
                if (substr(s, 1, 1) != "/") { bad = 1; continue }
                nlogin++
                login_list[nlogin] = s
                if (!(s in def)) extra = 1
              }
            ...
            END {
              ...
              if (nbad) { print "fail " out; exit }
              if (extra) { print "manual"; exit }
              print "pass"
            }
          '
        )
        SUS_PARSE_RC=$?
        if [ "$SUS_PARSE_RC" -ne 0 ] || [ -z "$SUS_CLASS" ]; then
          SUS_REASON=unparseable
        elif [ "$SUS_CLASS" = "pass" ]; then
          SUS_REASON=compliant
        elif [ "$SUS_CLASS" = "absent" ]; then
          SUS_REASON=subject_absent
        elif [ "$SUS_CLASS" = "manual" ]; then
          SUS_REASON=manual_portion
        elif [ "$SUS_CLASS" = "unparseable" ]; then
          SUS_REASON=unparseable
        else
          SUS_REASON=noncompliant
        fi
        ;;
      *)
        SUS_REASON=unparseable
  ...
  case "$SUS_REASON" in
    compliant)
      SUS_STATUS=PASS
      SUS_OBSERVED="/etc/shells exists; login.cfg usw shells is a non-empty subset of /etc/shells"
      SUS_MEANING="The shells file exists and every login.cfg usw shell is listed in it."
      SUS_FIX="n/a"
      ;;
    noncompliant) SUS_STATUS=FAIL
      SUS_OBSERVED="login.cfg usw shells lists a shell not in /etc/shells: ${SUS_CLASS#fail }"
      SUS_MEANING="A shell listed in login.cfg usw shells is not defined in /etc/shells, so a login may use a shell the file does not advertise."
      SUS_FIX="chsec -f /etc/security/login.cfg -s usw -a shells=<list of approved shells separated by comma>; vi /etc/shells; chown bin.bin /etc/shells; chmod 644 /etc/shells"
      ;;
    subject_absent) SUS_STATUS=FAIL
      if [ "$SUS_CLASS" = "absent" ]; then
        SUS_OBSERVED="login.cfg usw shells attribute is empty or absent"
      else
        SUS_OBSERVED="/etc/shells is absent"
      fi
      SUS_MEANING="The required /etc/shells file or login.cfg usw shells attribute is missing, so the allowed-shells configuration is unmet."
      SUS_FIX="chsec -f /etc/security/login.cfg -s usw -a shells=<list of approved shells separated by comma>; vi /etc/shells; chown bin.bin /etc/shells; chmod 644 /etc/shells"
      ;;
    probe_failed) SUS_STATUS=NOT_ASSESSED
      SUS_OBSERVED="not assessed - /etc/shells or login.cfg shells probe failed"
      SUS_MEANING="PTxray did not obtain trustworthy /etc/shells or login.cfg usw shells evidence."
      SUS_FIX="verify ls -la /etc/shells, cat /etc/shells, and lssec -f /etc/security/login.cfg -s usw -a shells, then rerun PTxray."
      ;;
    probe_empty) SUS_STATUS=NOT_ASSESSED
      SUS_OBSERVED="not assessed - /etc/shells or login.cfg shells probe returned no output"
      SUS_MEANING="PTxray did not obtain trustworthy /etc/shells or login.cfg usw shells evidence."
      SUS_FIX="verify ls -la /etc/shells, cat /etc/shells, and lssec -f /etc/security/login.cfg -s usw -a shells return complete evidence, then rerun PTxray."
      ;;
    manual_portion) SUS_STATUS=NOT_ASSESSED
      SUS_OBSERVED="not assessed - Ask ISSO/SA for other approved shells"
      SUS_MEANING="The decidable subset holds, but whether a listed shell is ISSO-approved is a site question this check does not silently pass."
      SUS_FIX="Ask ISSO/SA for other approved shells, then confirm /etc/shells and login.cfg usw shells contain only that list."
      ;;
    unparseable) SUS_STATUS=NOT_ASSESSED
      SUS_OBSERVED="not assessed - /etc/shells or login.cfg shells evidence was unparseable"
      SUS_MEANING="PTxray did not obtain trustworthy /etc/shells or login.cfg usw shells evidence."
      SUS_FIX="verify ls -la /etc/shells, cat /etc/shells, and lssec -f /etc/security/login.cfg -s usw -a shells, then rerun PTxray."
      ;;
    *)
      SUS_STATUS=NOT_ASSESSED
      SUS_OBSERVED="not assessed - /etc/shells or login.cfg shells evidence was unparseable"
      SUS_MEANING="PTxray did not obtain trustworthy /etc/shells or login.cfg usw shells evidence."
      SUS_FIX="verify ls -la /etc/shells, cat /etc/shells, and lssec -f /etc/security/login.cfg -s usw -a shells, then rerun PTxray."
      ;;
  ...
  case "$SUS_STATUS" in
    NOT_APPLICABLE) SUS_SEV=low ;;
    *) SUS_SEV=med ;;
  esac

  add security stig_user_shells "The /etc/shells file must exist" \
    "$SUS_STATUS" "$SUS_SEV" "$SUS_OBSERVED" \
    "$SUS_MEANING" "$SUS_FIX" "stig:V-215408"
}
