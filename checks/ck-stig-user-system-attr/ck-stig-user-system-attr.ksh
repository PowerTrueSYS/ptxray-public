...
AIXRAY_TOOL=ck-stig-user-system-attr


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_user_system_attr — DISA STIG V-215177.
  # The AIX SYSTEM attribute must not be NONE for any account. Prohibition:
  # determinate absence of accounts is PASS; empty rc0 output is a swallowed
  # error and refuses. Any malformed record refuses before FAIL is consulted.
  typeset SUSA_RAW SUSA_RC SUSA_KIND SUSA_PARSE SUSA_PARSE_RC SUSA_DETAIL
  typeset SUSA_STATUS SUSA_SEV SUSA_OBSERVED SUSA_MEANING SUSA_FIX
  typeset SUSA_NONE_STATUS SUSA_FAILCLOSED_PROBE SUSA_FAILCLOSED_EMPTY

  SUSA_NONE_STATUS=FAIL
  SUSA_FAILCLOSED_PROBE=NOT_ASSESSED
  SUSA_FAILCLOSED_EMPTY=NOT_ASSESSED

  SUSA_RAW=$(aix lsuser_system_all lsuser -a SYSTEM ALL)
  SUSA_RC=$?
  SUSA_KIND=""
  SUSA_DETAIL=""
  SUSA_PARSE=""

  if [ "$SUSA_RC" -ne 0 ]; then
    SUSA_KIND=failed
  elif [ -z "$SUSA_RAW" ]; then
    SUSA_KIND=empty
  else
    SUSA_PARSE=$(printf '%s\n' "$SUSA_RAW" | awk '
      BEGIN {
        abs = sprintf("3004-687 User \"%s\" does not exist.", "ALL")
      }
      {
        gsub(/\r/, "")
        gsub(/^[ \t]+/, "")
        ...
        if ($0 == abs) { diag++; next }
        if (NF != 2) { bad = 1; next }
        if (index($2, "SYSTEM=") != 1) { bad = 1; next }
        val = substr($2, 8)
        if (val == "") { bad = 1; next }
        users++
        if (val == "NONE") {
      ...
      END {
        ...
        if (diag && users) { print "unparseable"; exit 0 }
        if (nfail > 0) { print "fail:" fails; exit 0 }
        if (users > 0) { print "ok"; exit 0 }
        if (diag) { print "absent"; exit 0 }
        print "unparseable"
      }
    ')
    SUSA_PARSE_RC=$?
    if [ "$SUSA_PARSE_RC" -ne 0 ] || [ -z "$SUSA_PARSE" ]; then
      SUSA_KIND=unparseable
    else
      case "$SUSA_PARSE" in
        unparseable) SUSA_KIND=unparseable ;;
        absent) SUSA_KIND=absent ;;
        fail:*)
          SUSA_KIND=fail
          SUSA_DETAIL=${SUSA_PARSE#fail:}
          ;;
        ok) SUSA_KIND=ok ;;
        *) SUSA_KIND=unparseable ;;
      esac
  ...
  case "$SUSA_KIND" in
    failed)
      SUSA_STATUS=$SUSA_FAILCLOSED_PROBE
      SUSA_OBSERVED="not assessed - lsuser probe failed (rc=$SUSA_RC)"
      SUSA_MEANING="PTxray did not obtain trustworthy SYSTEM attribute evidence."
      SUSA_FIX="run 'lsuser -a SYSTEM ALL' and rerun PTxray."
      ;;
    empty)
      SUSA_STATUS=$SUSA_FAILCLOSED_EMPTY
      SUSA_OBSERVED="not assessed - lsuser returned no output with rc 0"
      SUSA_MEANING="PTxray did not obtain trustworthy SYSTEM attribute evidence."
      SUSA_FIX="run 'lsuser -a SYSTEM ALL' and rerun PTxray."
      ;;
    unparseable)
      SUSA_STATUS=NOT_ASSESSED
      SUSA_OBSERVED="not assessed - lsuser SYSTEM output could not be interpreted"
      SUSA_MEANING="PTxray did not obtain trustworthy SYSTEM attribute evidence."
      SUSA_FIX="run 'lsuser -a SYSTEM ALL' and rerun PTxray."
      ;;
    fail)
      SUSA_STATUS=$SUSA_NONE_STATUS
      SUSA_OBSERVED="SYSTEM=NONE on $SUSA_DETAIL"
      SUSA_MEANING="An AIX account has SYSTEM=NONE, so a login to that account is accepted without authentication."
      SUSA_FIX="# chuser SYSTEM=compat [user_name]"
      ;;
    ok)
      SUSA_STATUS=PASS
      SUSA_OBSERVED="no account has SYSTEM=NONE"
      SUSA_MEANING="Every listed AIX account has a SYSTEM value other than NONE."
      SUSA_FIX="n/a"
      ;;
    absent)
      SUSA_STATUS=PASS
      SUSA_OBSERVED="no accounts present to evaluate SYSTEM"
      SUSA_MEANING="No AIX accounts were listed, so none of them can have SYSTEM=NONE."
      SUSA_FIX="n/a"
      ;;
    *)
      SUSA_STATUS=NOT_ASSESSED
      SUSA_OBSERVED="not assessed - unexpected SYSTEM parser result"
      SUSA_MEANING="PTxray did not obtain trustworthy SYSTEM attribute evidence."
      SUSA_FIX="run 'lsuser -a SYSTEM ALL' and rerun PTxray."
      ;;
  esac
  ...
  case "$SUSA_STATUS" in
    FAIL) SUSA_SEV=high ;;
    NOT_APPLICABLE) SUSA_SEV=low ;;
    *) SUSA_SEV=med ;;
  esac

  add security stig_user_system_attr "AIX SYSTEM attribute must not be NONE" \
    "$SUSA_STATUS" "$SUSA_SEV" "$SUSA_OBSERVED" \
    "$SUSA_MEANING" "$SUSA_FIX" "stig:V-215177"
}
