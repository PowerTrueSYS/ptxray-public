...
AIXRAY_TOOL=ck-stig-user-ttys


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_user_ttys — DISA STIG V-215186.
  # Required configuration: default ttys is ALL, and every listed account
  # has ttys=ALL. Uncertain probe evidence is refused; a missing subject
  # is a finding.
  typeset UTT_LSSEC_RAW UTT_LSSEC_RC UTT_LSUSER_RAW UTT_LSUSER_RC
  typeset UTT_LSSEC_PARSED UTT_LSSEC_PARSE_RC UTT_LSUSER_PARSED UTT_LSUSER_PARSE_RC
  typeset UTT_LSSEC_KIND UTT_LSUSER_KIND UTT_LSSEC_VAL UTT_LSUSER_VAL
  typeset UTT_CLASS UTT_DETAIL
  typeset UTT_STATUS UTT_SEV UTT_OBSERVED UTT_MEANING UTT_FIX

  UTT_LSSEC_RAW=$(aix lssec_default_ttys lssec -f /etc/security/user -s default -a ttys)
  UTT_LSSEC_RC=$?
  UTT_LSUSER_RAW=$(aix lsuser_ttys_all lsuser -a ttys ALL)
  UTT_LSUSER_RC=$?
  UTT_CLASS=""
  UTT_DETAIL=""
  UTT_LSSEC_KIND=""
  ...
  if [ "$UTT_LSSEC_RC" -ne 0 ] || [ "$UTT_LSUSER_RC" -ne 0 ]; then
    UTT_CLASS=probe_failed
  elif [ -z "$UTT_LSSEC_RAW" ] || [ -z "$UTT_LSUSER_RAW" ]; then
    UTT_CLASS=probe_empty
  else
    UTT_LSSEC_PARSED=$(printf '%s\n' "$UTT_LSSEC_RAW" | awk '
      {
        gsub(/\r/, "")
        if ($0 ~ /^[ \t]*$/) next
        records++
      ...
      END {
        ...
        if (val == "ALL") print "ok " val
        else print "fail " val
      }
    ')
    UTT_LSSEC_PARSE_RC=$?
    UTT_LSUSER_PARSED=$(printf '%s\n' "$UTT_LSUSER_RAW" | awk '
      function is_user(u) {
    ...
    if [ "$UTT_LSSEC_PARSE_RC" -ne 0 ] || [ -z "$UTT_LSSEC_PARSED" ]; then
      UTT_LSSEC_KIND=unparseable
    else
      case "$UTT_LSSEC_PARSED" in
        ok\ *)
          UTT_LSSEC_KIND=ok
          UTT_LSSEC_VAL=${UTT_LSSEC_PARSED#ok }
          ;;
        fail\ *)
          UTT_LSSEC_KIND=fail
          UTT_LSSEC_VAL=${UTT_LSSEC_PARSED#fail }
          ;;
        absent) UTT_LSSEC_KIND=absent ;;
        *) UTT_LSSEC_KIND=unparseable ;;
      esac
    fi

    if [ "$UTT_LSUSER_PARSE_RC" -ne 0 ] || [ -z "$UTT_LSUSER_PARSED" ]; then
      UTT_LSUSER_KIND=unparseable
    else
      case "$UTT_LSUSER_PARSED" in
        ok) UTT_LSUSER_KIND=ok ;;
        fail\ *)
          UTT_LSUSER_KIND=fail
          UTT_LSUSER_VAL=${UTT_LSUSER_PARSED#fail }
          ;;
        absent) UTT_LSUSER_KIND=absent ;;
        *) UTT_LSUSER_KIND=unparseable ;;
      esac
    fi

    case "$UTT_LSSEC_KIND:$UTT_LSUSER_KIND" in
      unparseable:*|*:unparseable) UTT_CLASS=unparseable ;;
      absent:*|*:absent) UTT_CLASS=subject_absent ;;
      fail:*|*:fail)
        UTT_CLASS=fail
        UTT_DETAIL=""
        if [ "$UTT_LSSEC_KIND" = fail ]; then
          UTT_DETAIL="default=$UTT_LSSEC_VAL"
        fi
        if [ "$UTT_LSUSER_KIND" = fail ]; then
          if [ -n "$UTT_DETAIL" ]; then
            UTT_DETAIL="$UTT_DETAIL $UTT_LSUSER_VAL"
          else
            UTT_DETAIL=$UTT_LSUSER_VAL
          fi
        fi
        ;;
      ok:ok) UTT_CLASS=ok ;;
      *) UTT_CLASS=unparseable ;;
    esac
  fi
  ...
  case "$UTT_CLASS" in
    ok)
      UTT_STATUS=PASS
      UTT_OBSERVED="default ttys=$UTT_LSSEC_VAL; all listed accounts have ttys=ALL"
      UTT_MEANING="Default and per-account ttys are ALL, so interactive users may log in on any terminal as required."
      UTT_FIX="n/a"
      ;;
    fail) UTT_STATUS=FAIL
      UTT_OBSERVED="ttys is not ALL: $UTT_DETAIL"
      UTT_MEANING="The default ttys value is not ALL, or an interactive account does not have ttys=ALL, so a required login path can be blocked or narrowed."
      UTT_FIX="chsec -f /etc/security/user -s default -a ttys=ALL; chsec -f /etc/security/user -s [user_name] -a ttys=ALL"
      ;;
    subject_absent) UTT_STATUS=FAIL
      UTT_OBSERVED="required ttys subject is absent"
      UTT_MEANING="The default ttys attribute or the account list is missing, so the required ttys=ALL configuration is not in place."
      UTT_FIX="chsec -f /etc/security/user -s default -a ttys=ALL; chsec -f /etc/security/user -s [user_name] -a ttys=ALL"
      ;;
    probe_failed) UTT_STATUS=NOT_ASSESSED
      UTT_OBSERVED="not assessed - ttys probe failed (lssec_rc=$UTT_LSSEC_RC lsuser_rc=$UTT_LSUSER_RC)"
      UTT_MEANING="PTxray did not obtain trustworthy default and per-account ttys evidence."
      UTT_FIX="run 'lssec -f /etc/security/user -s default -a ttys' and 'lsuser -a ttys ALL', correct the capture problem, and rerun PTxray."
      ;;
    probe_empty) UTT_STATUS=NOT_ASSESSED
      UTT_OBSERVED="not assessed - ttys probe returned no output"
      UTT_MEANING="PTxray did not obtain trustworthy default and per-account ttys evidence."
      UTT_FIX="run 'lssec -f /etc/security/user -s default -a ttys' and 'lsuser -a ttys ALL', correct the capture problem, and rerun PTxray."
      ;;
    *)
      UTT_STATUS=NOT_ASSESSED
      UTT_OBSERVED="not assessed - ttys output was unparseable"
      UTT_MEANING="PTxray did not obtain trustworthy default and per-account ttys evidence."
      UTT_FIX="run 'lssec -f /etc/security/user -s default -a ttys' and 'lsuser -a ttys ALL', correct the capture problem, and rerun PTxray."
      ;;
  esac
  ...
  case "$UTT_STATUS" in
    NOT_APPLICABLE) UTT_SEV=low ;;
    *) UTT_SEV=med ;;
  esac

  add security stig_user_ttys "Interactive user ttys value" \
    "$UTT_STATUS" "$UTT_SEV" "$UTT_OBSERVED" \
    "$UTT_MEANING" "$UTT_FIX" "stig:V-215186"
}
