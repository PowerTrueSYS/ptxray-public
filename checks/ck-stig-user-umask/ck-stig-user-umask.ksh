...
AIXRAY_TOOL=ck-stig-user-umask


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_user_umask — DISA STIG V-215431.
  # Required configuration: umask 077 in /etc/security/.profile and
  # default stanza umask=077 (STIG literal, not CIS 027).
  # Uncertain probe evidence is refused; a missing subject is a finding.
  typeset SUU_PROF_RAW SUU_PROF_RC SUU_LSSEC_RAW SUU_LSSEC_RC
  typeset SUU_PROF_KIND SUU_LSSEC_KIND SUU_LSSEC_DETAIL SUU_CLASS
  typeset SUU_STATUS SUU_SEV SUU_OBSERVED SUU_MEANING SUU_FIX

  SUU_PROF_RAW=$(aix grep_umask_profile grep "umask 077" /etc/security/.profile)
  SUU_PROF_RC=$?
  SUU_LSSEC_RAW=$(aix lssec_default_umask lssec -f /etc/security/user -s default -a umask)
  SUU_LSSEC_RC=$?
  SUU_PROF_KIND=""
  SUU_LSSEC_KIND=""
  SUU_LSSEC_DETAIL=""
  ...
  if [ "$SUU_PROF_RC" -ne 0 ] && [ "$SUU_PROF_RC" -ne 1 ]; then
    SUU_PROF_KIND=probe_failed
  elif [ -z "$SUU_PROF_RAW" ]; then
    if [ "$SUU_PROF_RC" -eq 1 ]; then
      SUU_PROF_KIND=absent
    else
      SUU_PROF_KIND=probe_empty
    fi
  else
    SUU_PROF_KIND=$(printf '%s\n' "$SUU_PROF_RAW" | awk '
      {
        gsub(/\r/, "")
        line = $0
        sub(/^[ \t]+/, "", line)
        ...
        sub(/;[ \t]*$/, "", line)
        $0 = line
        if ($1 == "umask" && $2 == "077" && NF == 2) found = 1
      }
      END {
        if (found) print "ok"
        else if (saw_text) print "absent"
        else print "unparseable"
      }
    ')
    if [ -z "$SUU_PROF_KIND" ]; then
      SUU_PROF_KIND=unparseable
    fi
  fi

  if [ "$SUU_LSSEC_RC" -ne 0 ]; then
    SUU_LSSEC_KIND=probe_failed
  elif [ -z "$SUU_LSSEC_RAW" ]; then
    SUU_LSSEC_KIND=probe_empty
  else
    SUU_LSSEC_KIND=$(printf '%s\n' "$SUU_LSSEC_RAW" | awk '
      {
        gsub(/\r/, "")
        if ($0 ~ /^[ \t]*$/) next
        records++
        ...
        if (index(rest, "umask=") != 1) { bad = 1; next }
        val = substr(rest, 7)
        if (val == "") kind = "absent"
        else if (val !~ /^[0-7][0-7]*$/) { bad = 1; next }
        else if (val == "077") kind = "ok"
        else kind = "fail " val
      }
      END {
    ...
    ')
    if [ -z "$SUU_LSSEC_KIND" ]; then
      SUU_LSSEC_KIND=unparseable
    fi
  fi

  case "$SUU_LSSEC_KIND" in
    fail\ *)
      SUU_LSSEC_DETAIL=${SUU_LSSEC_KIND#fail }
      SUU_LSSEC_KIND=fail
      ;;
  esac

  if [ "$SUU_PROF_KIND" = probe_failed ] || [ "$SUU_LSSEC_KIND" = probe_failed ]; then
    SUU_CLASS=probe_failed
  elif [ "$SUU_PROF_KIND" = probe_empty ] || [ "$SUU_LSSEC_KIND" = probe_empty ]; then
    SUU_CLASS=probe_empty
  elif [ "$SUU_PROF_KIND" = unparseable ] || [ "$SUU_LSSEC_KIND" = unparseable ]; then
    SUU_CLASS=unparseable
  elif [ "$SUU_PROF_KIND" = ok ] && [ "$SUU_LSSEC_KIND" = ok ]; then
    SUU_CLASS=compliant
  elif [ "$SUU_PROF_KIND" = fail ] || [ "$SUU_LSSEC_KIND" = fail ]; then
    SUU_CLASS=noncompliant
  elif [ "$SUU_PROF_KIND" = absent ] || [ "$SUU_LSSEC_KIND" = absent ]; then
    SUU_CLASS=subject_absent
  else
    SUU_CLASS=unparseable
  fi
  ...
  case "$SUU_CLASS" in
    compliant)
      SUU_STATUS=PASS
      SUU_OBSERVED="umask 077 in /etc/security/.profile; default umask=077"
      SUU_MEANING="Default umask 077 in /etc/security/.profile and the default stanza restricts new files to the owner."
      SUU_FIX="n/a"
      ;;
    noncompliant) SUU_STATUS=FAIL
      if [ -n "$SUU_LSSEC_DETAIL" ]; then
        SUU_OBSERVED="default umask is not 077 ($SUU_LSSEC_DETAIL)"
      else
        SUU_OBSERVED="default umask is not 077"
      fi
      SUU_MEANING="A default umask other than 077 lets authenticated users create group- or world-readable files."
      SUU_FIX="chsec -f /etc/security/user -s default -a umask=077; PTxray only recommends this command."
      ;;
    subject_absent) SUU_STATUS=FAIL
      SUU_OBSERVED="required umask 077 subject is absent"
      SUU_MEANING="The required umask 077 configuration is missing, so new files are not restricted to the owner."
      SUU_FIX="chsec -f /etc/security/user -s default -a umask=077; PTxray only recommends this command."
      ;;
    probe_failed) SUU_STATUS=NOT_ASSESSED
      SUU_OBSERVED="not assessed - umask probe failed (grep_rc=$SUU_PROF_RC lssec_rc=$SUU_LSSEC_RC)"
      SUU_MEANING="PTxray did not obtain trustworthy umask 077 evidence."
      SUU_FIX="run 'grep \"umask 077\" /etc/security/.profile' and 'lssec -f /etc/security/user -s default -a umask', correct the capture problem, and rerun PTxray."
      ;;
    probe_empty) SUU_STATUS=NOT_ASSESSED
      SUU_OBSERVED="not assessed - umask probe returned no output"
      SUU_MEANING="PTxray did not obtain trustworthy umask 077 evidence."
      SUU_FIX="run 'grep \"umask 077\" /etc/security/.profile' and 'lssec -f /etc/security/user -s default -a umask', correct the capture problem, and rerun PTxray."
      ;;
    *)
      SUU_STATUS=NOT_ASSESSED
      SUU_OBSERVED="not assessed - umask probe output was unparseable"
      SUU_MEANING="PTxray did not obtain trustworthy umask 077 evidence."
      SUU_FIX="run 'grep \"umask 077\" /etc/security/.profile' and 'lssec -f /etc/security/user -s default -a umask', correct the capture problem, and rerun PTxray."
      ;;
  esac
  ...
  case "$SUU_STATUS" in
    NOT_APPLICABLE) SUU_SEV=low ;;
    *) SUU_SEV=med ;;
  esac

  add security stig_user_umask "Default user umask 077" \
    "$SUU_STATUS" "$SUU_SEV" "$SUU_OBSERVED" \
    "$SUU_MEANING" "$SUU_FIX" "stig:V-215431"
}
