...
AIXRAY_TOOL=ck-stig-userinit-ldr-preload


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_userinit_ldr_preload — DISA STIG V-215425. PARTIAL-AUTO.
  # Empty/relative LDR_PRELOAD or missing required subjects FAIL; absolute paths are ISSO.
  typeset SUL_PW_RAW SUL_PW_RC SUL_USERS SUL_PARSE_RC SUL_LIST_RAW SUL_LIST_RC
  typeset SUL_LIST SUL_PATH SUL_N SUL_LS_RAW SUL_LS_RC SUL_RAW SUL_RC
  typeset SUL_HITS SUL_HITS_RC SUL_PARSE SUL_KEY SUL_REQ SUL_CMD SUL_OIFS SUL_FILES
  typeset SUL_FAILED SUL_EMPTY SUL_ABSENT SUL_BAD SUL_UNPARSED SUL_MANUAL
  typeset SUL_USE_LOCATOR SUL_REASON SUL_STATUS SUL_SEV SUL_OBSERVED SUL_MEANING
  typeset SUL_FIX SUL_FAILCLOSED_PROBE SUL_FAILCLOSED_EMPTY
  typeset SUL_FAILCLOSED_MANUAL SUL_FAILCLOSED_UNPARSE

  SUL_FAILCLOSED_PROBE=NOT_ASSESSED
  SUL_FAILCLOSED_EMPTY=NOT_ASSESSED
  SUL_FAILCLOSED_MANUAL=NOT_ASSESSED
  SUL_FAILCLOSED_UNPARSE=NOT_ASSESSED
  SUL_USE_LOCATOR=1
  SUL_FAILED=0 SUL_EMPTY=0 SUL_ABSENT=0 SUL_BAD=0 SUL_UNPARSED=0 SUL_MANUAL=0


  SUL_PW_RAW=$(aix etc_passwd cat /etc/passwd)
  SUL_PW_RC=$?
  if [ "$SUL_PW_RC" -ne 0 ]; then
    SUL_FAILED=1
  elif [ -z "$SUL_PW_RAW" ]; then
    SUL_EMPTY=1
  else
    printf '%s\n' "$SUL_PW_RAW" | awk -F: '
      /^[ \t]*$/ { next }
    ...
    '
    SUL_PARSE_RC=$?
    if [ "$SUL_PARSE_RC" -ne 0 ]; then
      SUL_UNPARSED=1
    else
      SUL_USERS=$(printf '%s\n' "$SUL_PW_RAW" | cut -f 1,1 -d ":")
      [ -n "$SUL_USERS" ] || SUL_EMPTY=1
    fi
  fi

  SUL_LIST_RAW=$(aix userinit_ldr_list sh -c 'cat /etc/passwd | cut -f 1,1 -d ":" | xargs -n1 -IUSER sh -c "grep -l LDR_PRELOAD ~USER/.*"')
  SUL_LIST_RC=$?
  SUL_LIST=""
  if [ -z "$SUL_LIST_RAW" ]; then
    if [ "$SUL_LIST_RC" -eq 0 ]; then
      SUL_EMPTY=1
    elif [ "$SUL_LIST_RC" -ne 1 ] && [ "$SUL_LIST_RC" -ne 123 ]; then
      SUL_FAILED=1
    fi
  else
    SUL_LIST=$(printf '%s\n' "$SUL_LIST_RAW" | awk '
  ...
  SUL_FILES='root_sh_history_ls|1|ls -ld /root/.sh_history
root_sh_history|1|cat /root/.sh_history
...
'
  SUL_N=0
  if [ "$SUL_USE_LOCATOR" -eq 1 ] && [ -n "$SUL_LIST" ]; then
    while IFS= read -r SUL_PATH; do
      [ -n "$SUL_PATH" ] || continue
      case "$SUL_PATH" in
        /root/.sh_history|/home/doejohn/.profile|/home/doejane/.profile) continue ;;
        *[!/A-Za-z0-9._-]*|*/../*|*/..) SUL_UNPARSED=1; continue ;;
        /*) ;;
        *) SUL_UNPARSED=1; continue ;;
      esac
      case "$SUL_FILES" in
        *"|ls -ld ${SUL_PATH}
"*) continue ;;
      ...
      SUL_N=$((SUL_N + 1))
      SUL_KEY=locator_$SUL_N
      SUL_FILES="${SUL_FILES}${SUL_KEY}_ls|0|ls -ld ${SUL_PATH}
${SUL_KEY}|0|cat ${SUL_PATH}
"
    done <<SUL_LIST_END
$SUL_LIST
SUL_LIST_END
  ...
  SUL_OIFS=$IFS
  SUL_LS_RC=0
  while IFS='|' read -r SUL_KEY SUL_REQ SUL_CMD; do
    IFS=$SUL_OIFS
    case "$SUL_KEY" in ''|'#'*) continue ;; esac
    SUL_PATH=${SUL_CMD##* }
    case "$SUL_CMD" in
      "ls -ld "*)
        SUL_LS_RAW=$(aix "$SUL_KEY" ls -ld "$SUL_PATH")
        SUL_LS_RC=$?
        continue
        ;;
    esac
    if [ "$SUL_LS_RC" -eq 2 ]; then
      [ "$SUL_REQ" -eq 1 ] && SUL_ABSENT=1
      continue
    elif [ "$SUL_LS_RC" -ne 0 ]; then
      SUL_FAILED=1
      continue
    elif [ -z "$SUL_LS_RAW" ]; then
      SUL_EMPTY=1
      continue
    fi
    SUL_RAW=$(aix "$SUL_KEY" cat "$SUL_PATH")
    SUL_RC=$?
    if [ "$SUL_RC" -ne 0 ]; then
      SUL_FAILED=1
      continue
    ...
    SUL_HITS=$(printf '%s\n' "$SUL_RAW" | grep -Ei 'ldr|preload')
    SUL_HITS_RC=$?
    if [ "$SUL_HITS_RC" -gt 1 ]; then
      SUL_FAILED=1
      continue
    fi
    [ "$SUL_HITS_RC" -eq 1 ] && continue
    SUL_PARSE=$(printf '%s\n' "$SUL_HITS" | awk '
    function ok_entry(s) {
    ...
    function looks_preload(s) {
      return (s ~ /LDR_PRELOAD=/ || s ~ /LD_PRELOAD=/ || \
        s ~ /setenv[ \t]+(LDR_PRELOAD|LD_PRELOAD)[ \t]/)
    }
    function eval_val(val, n, q1, nslots, i, s) {
      ...
      for (i = 1; i <= nslots; i++) {
        ...
        else if (substr(s, 1, 1) == "/") hasabs = 1
      }
    }
    function eval_cmd(cmd, name, val, rest) {
      cmd = trim(cmd); if (cmd == "") return
      if (cmd ~ /^setenv[ \t]+/) {
        rest = cmd; sub(/^setenv[ \t]+/, "", rest)
        name = rest; sub(/[ \t].*/, "", name)
        val = rest
        if (val ~ /[ \t]/) sub(/^[^ \t]+[ \t]+/, "", val); else val = ""
        if (name == "LDR_PRELOAD" || name == "LD_PRELOAD") {
          gotpre = 1; eval_val(val)
        }
        return
      }
      rest = cmd
      while (rest ~ /^(export|typeset)[ \t]+/) {
        sub(/^(export|typeset)[ \t]+/, "", rest)
        while (rest ~ /^-[A-Za-z][ \t]+/) sub(/^-[A-Za-z][ \t]+/, "", rest)
      }
      if (rest ~ /^(LDR_PRELOAD|LD_PRELOAD)=/) {
        val = rest; sub(/^[^=]+=/, "", val); gotpre = 1; eval_val(val)
      }
    }
    {
      line = $0; gsub(/\r/, "", line); line = trim(line); if (line == "") next
      line = trim(strip_comment(line)); if (line == "") next
      ncmd = split_cmds(line, cmds); gotpre = 0
      for (ci = 1; ci <= ncmd; ci++) eval_cmd(cmds[ci])
      if (!gotpre && looks_preload(line)) unparsed = 1
    }
    END {
      if (unparsed) print "unparseable"
      else if (empty || rel) print "bad"
      else if (hasabs) print "manual"
      else print "ok"
    }
  ')
    SUL_PARSE_RC=$?
    if [ "$SUL_PARSE_RC" -ne 0 ] || [ -z "$SUL_PARSE" ]; then
      SUL_FAILED=1
      continue
    fi
    case "$SUL_PARSE" in
...
SUL_FILES_END
  ...
  SUL_REASON=compliant
  [ "$SUL_MANUAL" -eq 1 ] && SUL_REASON=manual
  [ "$SUL_UNPARSED" -eq 1 ] && SUL_REASON=unparseable
  [ "$SUL_EMPTY" -eq 1 ] && SUL_REASON=probe_empty
  [ "$SUL_FAILED" -eq 1 ] && SUL_REASON=probe_failed
  [ "$SUL_ABSENT" -eq 1 ] && SUL_REASON=subject_absent
  [ "$SUL_BAD" -eq 1 ] && SUL_REASON=noncompliant

  SUL_MEANING="PTxray did not obtain trustworthy evidence for local init-file LDR_PRELOAD lists."
  SUL_FIX="verify /etc/passwd and the required local initialization files, then rerun PTxray."
  case "$SUL_REASON" in
    compliant)
      SUL_STATUS=PASS
      SUL_OBSERVED="no LDR_PRELOAD lists in local init files"
      SUL_MEANING="Local initialization files do not list preloaded libraries, so there is no relative or empty LDR_PRELOAD slot."
      SUL_FIX="n/a"
      ;;
    noncompliant)
      SUL_STATUS=FAIL
      SUL_OBSERVED="relative or empty LDR_PRELOAD entry in local init files"
      SUL_MEANING="A relative or empty LDR_PRELOAD entry in a local initialization file can load an unintended library."
      SUL_FIX='Edit the local initialization file and remove the relative path entry from the library preload variable "LDR_PRELOAD".'
      ;;
    subject_absent)
      SUL_STATUS=FAIL
      SUL_OBSERVED="required local init file is absent"
      SUL_MEANING="A required local initialization file is missing, so the required configuration is unmet."
      SUL_FIX='Edit the local initialization file and remove the relative path entry from the library preload variable "LDR_PRELOAD".'
      ;;
    probe_failed)
      SUL_STATUS=$SUL_FAILCLOSED_PROBE
      SUL_OBSERVED="not assessed - local init-file LDR_PRELOAD probe failed"
      ;;
    probe_empty)
      SUL_STATUS=$SUL_FAILCLOSED_EMPTY
      SUL_OBSERVED="not assessed - local init-file LDR_PRELOAD probe returned no output"
      ;;
    manual)
      SUL_STATUS=$SUL_FAILCLOSED_MANUAL
      SUL_OBSERVED="not assessed - manual portion: ISSO/ISSM must document and authorize listed LDR_PRELOAD paths"
      SUL_MEANING="Absolute LDR_PRELOAD paths are present; whether they are documented and authorized by the ISSO/ISSM is not decided here."
      SUL_FIX="obtain ISSO/ISSM authorization for each listed LDR_PRELOAD path, or remove unauthorized entries; PTxray only recommends these actions."
      ;;
    *)
      SUL_STATUS=$SUL_FAILCLOSED_UNPARSE
      SUL_OBSERVED="not assessed - local init-file LDR_PRELOAD evidence was unparseable"
      ;;
  esac

  SUL_SEV=med
  [ "$SUL_STATUS" = NOT_APPLICABLE ] && SUL_SEV=low

  add security stig_userinit_ldr_preload "Local init-file LDR_PRELOAD paths" \
    "$SUL_STATUS" "$SUL_SEV" "$SUL_OBSERVED" \
    "$SUL_MEANING" "$SUL_FIX" "stig:V-215425"
}
