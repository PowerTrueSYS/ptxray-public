...
AIXRAY_TOOL=ck-stig-users-home-directories-have


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_users_home_directories_have — DISA STIG V-215331.
  # Interactive-user homes must be mode 0750 or less permissive.
  # Application directories listed as homes are allowed 0755 or less permissive.
  typeset UHDH_RAW UHDH_RC UHDH_KIND UHDH_PATH UHDH_PARSE_RC
  typeset UHDH_STATUS UHDH_SEV UHDH_OBSERVED UHDH_MEANING UHDH_FIX
  typeset UHDH_FAILCLOSED_PROBE UHDH_FAILCLOSED_EMPTY
  typeset UHDH_BADMODE_STATUS UHDH_ABSENT_STATUS UHDH_PARTIAL

  UHDH_FAILCLOSED_PROBE=NOT_ASSESSED
  UHDH_FAILCLOSED_EMPTY=NOT_ASSESSED
  UHDH_BADMODE_STATUS=FAIL
  UHDH_ABSENT_STATUS=FAIL

  # Pipeline runs inside aix via sh -c; a quoted pipeline is not an executable.
  # cut -d : -f 6 /etc/passwd | xargs -n1 ls -ld
  UHDH_RAW=$(aix passwd_homes_ls_ld sh -c "cut -d : -f 6 /etc/passwd | xargs -n1 ls -ld")
  UHDH_RC=$?
  UHDH_KIND=""
  UHDH_PATH=""
  UHDH_PARTIAL=0

  # ls over a live tree returns rc 1 or 2 when some path is unreadable or
  # vanishes, while still printing every row it could read. Grade those
  # rows. Refuse only when the probe produced no rows.
  if [ "$UHDH_RC" -eq 1 ] || [ "$UHDH_RC" -eq 2 ]; then
    if [ -n "$UHDH_RAW" ]; then
      UHDH_PARTIAL=1
    fi
  fi

  if [ "$UHDH_RC" -ne 0 ] && [ "$UHDH_PARTIAL" -eq 0 ]; then
    UHDH_KIND=failed
  elif [ -z "$UHDH_RAW" ]; then
    UHDH_KIND=empty
  else
    UHDH_KIND=$(printf '%s\n' "$UHDH_RAW" | awk '
      function is_application(p) {
        if (p == "/" || p == "/bin" || p == "/usr/bin") return 1
        if (p == "/sbin" || p == "/usr/sbin" || p == "/etc") return 1
        if (p == "/tmp" || p == "/usr" || p == "/var" || p == "/opt") return 1
        if (p == "/dev" || p == "/usr/lib" || p == "/usr/lpp" || p == "/usr/sys") return 1
        if (p == "/var/adm" || p == "/var/empty" || p == "/var/spool") return 1
        if (p == "/usr/lib/uucp" || p == "/var/spool/uucppublic") return 1
        if (index(p, "/opt/") == 1) return 1
        if (index(p, "/usr/lpp/") == 1) return 1
        if (index(p, "/usr/lib/") == 1) return 1
        if (index(p, "/var/adm/") == 1) return 1
        if (index(p, "/var/empty/") == 1) return 1
        if (index(p, "/var/spool/") == 1) return 1
        return 0
      }
      function has_special(m,    ox, gx, tx) {
        ox = substr(m, 4, 1)
        gx = substr(m, 7, 1)
        tx = substr(m, 10, 1)
        if (ox == "s" || ox == "S") return 1
        if (gx == "s" || gx == "S") return 1
        if (tx == "t" || tx == "T") return 1
        return 0
      }
      function le_0755(m) {
        if (has_special(m)) return 0
        if (substr(m, 6, 1) == "w") return 0
        if (substr(m, 9, 1) == "w") return 0
        return 1
      }
      function le_0750(m) {
        if (has_special(m)) return 0
        if (substr(m, 6, 1) != "-") return 0
        if (substr(m, 8, 1) != "-") return 0
        if (substr(m, 9, 1) != "-") return 0
        if (substr(m, 10, 1) != "-") return 0
        return 1
      }
      function allowed(m, p) {
        if (le_0750(m)) return 1
        if (le_0755(m) && is_application(p)) return 1
        return 0
      }
      function valid_mode(m) {
        if (substr(m, 1, 1) != "d") return 0
        if (substr(m, 2, 1) !~ /^[-r]$/) return 0
        if (substr(m, 3, 1) !~ /^[-w]$/) return 0
        if (substr(m, 4, 1) !~ /^[-xSs]$/) return 0
        if (substr(m, 5, 1) !~ /^[-r]$/) return 0
        if (substr(m, 6, 1) !~ /^[-w]$/) return 0
        if (substr(m, 7, 1) !~ /^[-xSs]$/) return 0
        if (substr(m, 8, 1) !~ /^[-r]$/) return 0
        if (substr(m, 9, 1) !~ /^[-w]$/) return 0
        if (substr(m, 10, 1) !~ /^[-xTt]$/) return 0
        return 1
      }
      /^[ \t]*$/ { next }
      /^[ \t]*#/ { next }
      {
        ...
        if (NF < 9) { bad = 1; next }
        if (valid_mode(mode) == 0) { bad = 1; next }
        path = $NF
        if (path !~ /^\//) { bad = 1; next }
        if (path == "/dev/null") next
        if (is_application(path) == 0) nsubject++
        if (allowed(mode, path) == 0) {
          nfail++
          if (firstfail == "") firstfail = path
        }
      }
      END {
        if (records + 0 == 0) { print "unparseable"; exit }
        if (nfail + 0 > 0) { print "fail:" firstfail; exit }
        if (bad) { print "unparseable"; exit }
        if (nsubject + 0 == 0) { print "absent"; exit }
        print "ok"
      }
    ')
    UHDH_PARSE_RC=$?
    if [ "$UHDH_PARSE_RC" -ne 0 ] || [ -z "$UHDH_KIND" ]; then
      UHDH_KIND=unparseable
    fi
  fi

  case "$UHDH_KIND" in
    failed)
      UHDH_STATUS=$UHDH_FAILCLOSED_PROBE
      UHDH_OBSERVED="not assessed - home directory listing failed (rc=$UHDH_RC)"
      UHDH_MEANING="PTxray did not obtain trustworthy home-directory mode evidence."
      UHDH_FIX="verify that cut -d : -f 6 /etc/passwd | xargs -n1 ls -ld runs, then rerun PTxray."
      ;;
    empty)
      UHDH_STATUS=$UHDH_FAILCLOSED_EMPTY
      UHDH_OBSERVED="not assessed - home directory listing returned empty output with rc 0"
      UHDH_MEANING="PTxray did not obtain trustworthy home-directory mode evidence."
      UHDH_FIX="verify that cut -d : -f 6 /etc/passwd | xargs -n1 ls -ld runs, then rerun PTxray."
      ;;
    unparseable)
      UHDH_STATUS=NOT_ASSESSED
      UHDH_OBSERVED="not assessed - home directory listing was unparseable"
      UHDH_MEANING="PTxray did not obtain trustworthy home-directory mode evidence."
      UHDH_FIX="verify that cut -d : -f 6 /etc/passwd | xargs -n1 ls -ld runs, then rerun PTxray."
      ;;
    absent)
      UHDH_STATUS=$UHDH_ABSENT_STATUS
      UHDH_OBSERVED="required interactive user home directory subject is absent"
      UHDH_MEANING="No interactive-user home directory was listed, so the required 0750 home-directory configuration is unmet."
      UHDH_FIX="# chmod 0750 <home directory>"
      ;;
    fail:*)
      UHDH_STATUS=$UHDH_BADMODE_STATUS
      UHDH_PATH=${UHDH_KIND#fail:}
      UHDH_OBSERVED="home $UHDH_PATH is more permissive than 0750"
      UHDH_MEANING="A user home directory is more permissive than 0750, so another identity can access the account's files."
      UHDH_FIX="# chmod 0750 <home directory>"
      ;;
    ok)
      UHDH_STATUS=PASS
      UHDH_OBSERVED="interactive user homes are mode 0750 or less permissive"
      UHDH_MEANING="Every interactive-user home directory is mode 0750 or less permissive."
      UHDH_FIX="n/a"
      ;;
    *)
      UHDH_STATUS=NOT_ASSESSED
  ...
  # Partial ls (rc 1/2 with rows) is determinate. Attach the unreadable-path
  # detail on every graded result, not only PASS. Capture has no aixv stderr.
  if [ "$UHDH_PARTIAL" -eq 1 ]; then
    case "$UHDH_STATUS" in
      NOT_ASSESSED) ;;
      *) UHDH_OBSERVED="$UHDH_OBSERVED; some paths unreadable" ;;
    esac
  fi

  case "$UHDH_STATUS" in
    NOT_APPLICABLE) UHDH_SEV=low ;;
    *) UHDH_SEV=med ;;
  esac

  add security stig_users_home_directories_have "User home directory mode" \
    "$UHDH_STATUS" "$UHDH_SEV" "$UHDH_OBSERVED" \
    "$UHDH_MEANING" "$UHDH_FIX" "stig:V-215331"
}
