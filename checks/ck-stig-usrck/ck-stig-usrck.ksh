...
AIXRAY_TOOL=ck-stig-usrck


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_usrck — DISA STIG V-215175. Unique account names via usrck -n ALL
  # (report mode). PASS on rc 0 plus a uniqueness-completed success form,
  # or on rc 114 with empty stdout (determinate absence of duplicate names;
  # usrck not-found semantics, not a dead probe). Duplicate names and a
  # missing user database fail as required evidence. Refuse rc 127,
  # unreadable output, empty rc 0, and any other nonzero rc.
  typeset USRCK_RAW USRCK_RC USRCK_SHAPE USRCK_NAMES USRCK_PARSE_RC
  typeset USRCK_ST USRCK_SEV USRCK_OBS USRCK_MEANING USRCK_FIX

  USRCK_RAW=$(aix usrck_n_all usrck -n ALL)
  USRCK_RC=$?
  USRCK_SHAPE=""
  USRCK_NAMES=""
  USRCK_ST=""
  ...
  USRCK_SHAPE=$(printf '%s\n' "$USRCK_RAW" | awk '
    function addname(n,   sp) {
      if (n == "" || n == "usrck") return
      sp = "," names ","
      if (index(sp, "," n ",") != 0) return
      if (names == "") names = n
      else names = names "," n
    }
    {
      ...
      if (match(line, /^[A-Za-z_][A-Za-z0-9_.-]*:/)) {
        curuser = substr(line, 1, index(line, ":") - 1)
        if (curuser == "usrck") curuser = ""
      } else if (NF == 1 && $1 ~ /^[A-Za-z_][A-Za-z0-9_.-]*$/ && $1 != "usrck") {
        curuser = $1
      }
      isabs = 0
      if (low ~ /cannot[ \t]+(access|open)/ && low ~ /passwd/) isabs = 1
      if (low ~ /no[ \t]+such[ \t]+file/ && low ~ /passwd/) isabs = 1
      if (low ~ /user[ \t]+database/ && (low ~ /absent/ || low ~ /missing/ || low ~ /empty/ || low ~ /cannot/)) isabs = 1
      if (low ~ /no[ \t]+user[ \t]+names/ || low ~ /no[ \t]+accounts/) isabs = 1
      if (low ~ /required subject absent/) isabs = 1
      if (isabs) absent = 1
      isdup = 0
      if (line ~ /3004-694/) isdup = 1
      if (low ~ /duplicate[ \t]+user[ \t]+name/) isdup = 1
      if (low ~ /duplicate[ \t]+account[ \t]+name/) isdup = 1
      if (low ~ /duplicate[ \t]+identification[ \t]+name/) isdup = 1
      if (isdup) {
        dup = 1
        if (match(line, /"[^"]+"/)) addname(substr(line, RSTART + 1, RLENGTH - 2))
        if (curuser != "") addname(curuser)
      }
      issuccess = 0
      if (low ~ /user name uniqueness/) issuccess = 1
      if (low ~ /unique account names/) issuccess = 1
      if (issuccess) okshape = 1
      else if (!isdup && !isabs) other = 1
    }
    END {
      if (nlines == 0) { print "empty"; exit }
      if (dup) { print "dup:" names; exit }
      if (absent) { print "absent"; exit }
      if (okshape && !other) { print "unique"; exit }
      print "unparseable"
    }
  ')
  USRCK_PARSE_RC=$?

  if [ "$USRCK_PARSE_RC" -ne 0 ] || [ -z "$USRCK_SHAPE" ]; then
    USRCK_ST=NOT_ASSESSED
    USRCK_OBS="not assessed - usrck -n ALL output was unparseable"
  else
    case "$USRCK_SHAPE" in
      empty)
        if [ "$USRCK_RC" -eq 114 ]; then
          USRCK_ST=PASS
          USRCK_OBS="usrck -n ALL listed no duplicate account names"
        elif [ "$USRCK_RC" -ne 0 ]; then
          USRCK_ST=NOT_ASSESSED
          USRCK_OBS="not assessed - usrck -n ALL probe failed (rc=$USRCK_RC)"
        else
          USRCK_ST=NOT_ASSESSED
          USRCK_OBS="not assessed - usrck -n ALL produced no output (rc=0)"
        fi
        ;;
      unique)
        if [ "$USRCK_RC" -eq 0 ]; then
          USRCK_ST=PASS
          USRCK_OBS="usrck -n ALL reports unique account names"
        else
          USRCK_ST=NOT_ASSESSED
          USRCK_OBS="not assessed - usrck -n ALL probe failed (rc=$USRCK_RC)"
        fi
      ...
      dup:*)
        USRCK_ST=FAIL
        USRCK_NAMES=${USRCK_SHAPE#dup:}
        if [ -n "$USRCK_NAMES" ]; then
          USRCK_OBS="duplicate account name(s): $USRCK_NAMES"
        else
          USRCK_OBS="duplicate account name(s) found"
        fi
        ;;
      absent)
        USRCK_ST=FAIL
        USRCK_OBS="required subject absent: user account database is not present"
        ;;
      *)
        if [ "$USRCK_RC" -ne 0 ]; then
          USRCK_ST=NOT_ASSESSED
          USRCK_OBS="not assessed - usrck -n ALL probe failed (rc=$USRCK_RC)"
        else
          USRCK_ST=NOT_ASSESSED
  ...
  case "$USRCK_ST" in
    PASS)
      USRCK_SEV=med
      USRCK_MEANING="usrck -n ALL reported no duplicate account names, so each account name is unique."
      USRCK_FIX="n/a"
      ;;
    FAIL)
      USRCK_SEV=high
      if [ "$USRCK_SHAPE" = "absent" ]; then
        USRCK_MEANING="The user account database is not present, so the unique-name requirement is unmet."
        USRCK_FIX="restore /etc/passwd and /etc/security/passwd, then give each account a unique name; PTxray only recommends this and does not edit those files."
      else
        USRCK_MEANING="Duplicate account names break accountability because one name maps to more than one account record."
        USRCK_FIX="Edit user accounts to provide unique name for each account by editing the following files:; /etc/passwd; /etc/security/passwd. PTxray only recommends this and does not edit those files."
      fi
      ;;
    NOT_APPLICABLE)
      USRCK_SEV=low
      USRCK_MEANING="This unique-name rule is not applicable on this host."
      USRCK_FIX="n/a"
      ;;
    *)
      USRCK_SEV=med
      USRCK_MEANING="PTxray did not obtain trustworthy usrck -n ALL evidence for unique account names."
      USRCK_FIX="run 'usrck -n ALL', correct the capture or permission problem, and rerun PTxray."
      ;;
  esac

  add security stig_usrck "Unique account names" \
    "$USRCK_ST" "$USRCK_SEV" "$USRCK_OBS" \
    "$USRCK_MEANING" "$USRCK_FIX" "stig:V-215175"
}
