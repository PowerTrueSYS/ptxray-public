...
AIXRAY_TOOL=ck-stig-wall-present


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_wall_present — DISA STIG V-215400.
  # Required configuration: /usr/sbin/wall must exist so administrators can
  # send a message to currently logged-in users. aix() discards ls stderr,
  # so live absence is rc=2 with empty stdout.
  typeset WALL_RAW WALL_RC WALL_STATE WALL_PARSE_RC
  typeset WALL_STATUS WALL_SEV WALL_OBSERVED WALL_MEANING WALL_FIX

  WALL_RAW=$(aix wall_ls ls -al /usr/sbin/wall)
  WALL_RC=$?
  WALL_STATE=""
  WALL_PARSE_RC=0
  WALL_STATUS=""
  ...
  if [ "$WALL_RC" -ne 0 ] && [ -z "$WALL_RAW" ]; then
    if [ "$WALL_RC" -eq 2 ]; then
      WALL_STATE=missing
    else
      WALL_STATE=probe_failed
    fi
  elif [ "$WALL_RC" -eq 0 ] && [ -z "$WALL_RAW" ]; then
    WALL_STATE=probe_empty
  else
    case "$WALL_RAW" in
      ...
      *)
        printf '%s\n' "$WALL_RAW" | awk '
          NF {
            ...
            if ($NF != "/usr/sbin/wall" && $NF != "wall") bad=1
          }
          END { if (records != 1 || bad) exit 1 }
        '
        WALL_PARSE_RC=$?
        if [ "$WALL_PARSE_RC" -eq 0 ] && [ "$WALL_RC" -eq 0 ]; then
          WALL_STATE=present
        else
          WALL_STATE=unparseable
        fi
        ;;
    esac
  fi
  ...
  case "$WALL_STATE" in
    present)
      WALL_STATUS=PASS
      WALL_OBSERVED="/usr/sbin/wall is present"
      WALL_MEANING="The wall command is installed, so administrators can send a message to currently logged-in users."
      WALL_FIX="n/a"
      ;;
    missing) WALL_STATUS=FAIL
      WALL_OBSERVED="/usr/sbin/wall is absent"
      WALL_MEANING="/usr/sbin/wall is not present, so administrators cannot send a message to currently logged-in users."
      WALL_FIX="installp -aXYgd /dev/cd0 -e /tmp/install.log bos.rte.misc_cmds; PTxray only recommends this command."
      ;;
    probe_failed) WALL_STATUS=NOT_ASSESSED
      WALL_OBSERVED="not assessed - /usr/sbin/wall listing failed (rc=$WALL_RC)"
      WALL_MEANING="PTxray did not obtain trustworthy evidence that /usr/sbin/wall exists."
      WALL_FIX="run 'ls -al /usr/sbin/wall', resolve the probe failure, and rerun PTxray."
      ;;
    probe_empty) WALL_STATUS=NOT_ASSESSED
      WALL_OBSERVED="not assessed - /usr/sbin/wall listing returned no output"
      WALL_MEANING="PTxray did not obtain trustworthy evidence that /usr/sbin/wall exists."
      WALL_FIX="run 'ls -al /usr/sbin/wall', confirm determinate output, and rerun PTxray."
      ;;
    unparseable) WALL_STATUS=NOT_ASSESSED
      WALL_OBSERVED="not assessed - /usr/sbin/wall listing was unparseable"
      WALL_MEANING="PTxray did not obtain trustworthy evidence that /usr/sbin/wall exists."
      WALL_FIX="run 'ls -al /usr/sbin/wall', correct the capture, and rerun PTxray."
      ;;
  esac

  case "$WALL_STATUS" in
    NOT_APPLICABLE) WALL_SEV=low ;;
    *) WALL_SEV=med ;;
  esac

  add security stig_wall_present "wall command present" \
    "$WALL_STATUS" "$WALL_SEV" "$WALL_OBSERVED" \
    "$WALL_MEANING" "$WALL_FIX" "stig:V-215400"
}
