...
AIXRAY_TOOL=ck-stig-worldwrite-objects


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_worldwrite_objects — DISA STIG V-215409.
  # PARTIAL-AUTO: find / -perm -2 is decidable against /tmp and /dev/null.
  # Additional "required for proper system or application operation" objects
  # are ISSO judgement and must not silently PASS.
  # find / -perm -2 -a \( -type d -o -type f \) -exec ls -ld {} \;
  typeset SWO_RAW SWO_RC SWO_PARSE SWO_KIND SWO_DETAIL
  typeset SWO_STATUS SWO_SEV SWO_OBSERVED SWO_MEANING SWO_FIX
  typeset SWO_OUTSIDE_STATUS SWO_ABSENT_STATUS
  typeset SWO_FAILCLOSED_PROBE SWO_FAILCLOSED_EMPTY SWO_FAILCLOSED_PARSE
  typeset SWO_FAILCLOSED_INCOMPLETE SWO_MANUAL_STATUS
  typeset SWO_PARTIAL SWO_GRADE

  SWO_OUTSIDE_STATUS=FAIL
  SWO_ABSENT_STATUS=FAIL
  SWO_FAILCLOSED_PROBE=NOT_ASSESSED
  SWO_FAILCLOSED_EMPTY=NOT_ASSESSED
  SWO_FAILCLOSED_PARSE=NOT_ASSESSED
  SWO_FAILCLOSED_INCOMPLETE=NOT_ASSESSED
  SWO_MANUAL_STATUS=NOT_ASSESSED

  SWO_RAW=$(aix worldwrite_find find / -perm -2 -a \( -type d -o -type f \) -exec ls -ld {} \;)
  SWO_RC=$?
  SWO_KIND=""
  SWO_DETAIL=""
  SWO_PARSE=""
  ...
  # find rc 1/2 over a live tree still prints every readable row. Grade those
  # rows. Refuse only when there are no rows (and no proof the probe ran).
  if [ "$SWO_RC" -eq 0 ]; then
    SWO_GRADE=1
  elif [ "$SWO_RC" -eq 1 ] || [ "$SWO_RC" -eq 2 ]; then
    if [ -n "$SWO_RAW" ]; then
      SWO_GRADE=1
      SWO_PARTIAL=1
    fi
    ...
    SWO_PARSE=$(printf '%s\n' "$SWO_RAW" | awk '
      {
        ...
        if (substr(mode, 1, 1) !~ /^[-bcdlps]$/) { bad = 1; next }
        if (substr(mode, 2, 1) !~ /^[-r]$/) { bad = 1; next }
        if (substr(mode, 3, 1) !~ /^[-w]$/) { bad = 1; next }
        if (substr(mode, 4, 1) !~ /^[-xSs]$/) { bad = 1; next }
        if (substr(mode, 5, 1) !~ /^[-r]$/) { bad = 1; next }
        if (substr(mode, 6, 1) !~ /^[-w]$/) { bad = 1; next }
        if (substr(mode, 7, 1) !~ /^[-xSs]$/) { bad = 1; next }
        if (substr(mode, 8, 1) !~ /^[-r]$/) { bad = 1; next }
        if (substr(mode, 9, 1) !~ /^[-w]$/) { bad = 1; next }
        if (substr(mode, 10, 1) !~ /^[-xTt]$/) { bad = 1; next }
        t = substr(mode, 1, 1)
        ow = substr(mode, 9, 1)
        sticky = substr(mode, 10, 1)
        if (ow != "w") { bad = 1; next }
        if ($2 !~ /^[0-9][0-9]*$/) { bad = 1; next }
        if ($3 == "" || substr($3, 1, 1) == "/") { bad = 1; next }
        if ($4 == "" || substr($4, 1, 1) == "/") { bad = 1; next }
        if ($5 ~ /^[0-9][0-9]*,$/) {
          if ($6 !~ /^[0-9][0-9]*$/) { bad = 1; next }
          di = 7
        } else if ($5 ~ /^[0-9][0-9]*$/ || $5 ~ /^[0-9][0-9]*,[0-9][0-9]*$/) {
          di = 6
        } else {
          bad = 1
          next
        }
        if (NF < di + 3) { bad = 1; next }
        months = " Jan Feb Mar Apr May Jun Jul Aug Sep Oct Nov Dec "
        if (index(months, " " $di " ") == 0) { bad = 1; next }
        if ($(di+1) !~ /^[0-9][0-9]*$/) { bad = 1; next }
        day = $(di+1) + 0
        if (day < 1 || day > 31) { bad = 1; next }
        tf = $(di+2)
        if (tf !~ /^[0-9][0-9]:[0-9][0-9]$/ && tf !~ /^[0-9][0-9][0-9][0-9]$/) { bad = 1; next }
        pstart = di + 3
        if (substr($pstart, 1, 1) != "/") { bad = 1; next }
        path = $pstart
        for (i = pstart + 1; i <= NF; i++) {
          if ($i == "->") break
          path = path " " $i
        }
        if (path == "/tmp") have_tmp = 1
        if (path == "/tmp" || path == "/dev/null" || index(path, "/tmp/") == 1) {
          allow++
          next
        }
        if (t == "d" && (sticky == "t" || sticky == "T")) {
          if (manuals != "") manuals = manuals " "
          manuals = manuals path
          nmanual++
          next
        }
      ...
      END {
        ...
        if (nfail > 0) { print "fail " fails; exit 0 }
        if (!have_tmp) { print "absent"; exit 0 }
        if (nmanual > 0) { print "manual " manuals; exit 0 }
        if (allow > 0) { print "ok"; exit 0 }
        print "unparseable"
      }
    ')
    case "$SWO_PARSE" in
      unparseable) SWO_KIND=unparseable ;;
      absent) SWO_KIND=absent ;;
      fail\ *)
        SWO_KIND=fail
        SWO_DETAIL=${SWO_PARSE#fail }
        ;;
      manual\ *)
        SWO_KIND=manual
        SWO_DETAIL=${SWO_PARSE#manual }
        ;;
      ok) SWO_KIND=ok ;;
      *) SWO_KIND=unparseable ;;
    esac
  fi

  case "$SWO_KIND" in
    failed)
      SWO_STATUS=$SWO_FAILCLOSED_PROBE
      SWO_OBSERVED="not assessed - world-writable object probe failed (rc=$SWO_RC)"
      SWO_MEANING="PTxray did not obtain a trustworthy world-writable file and directory listing."
      SWO_FIX="run 'find / -perm -2 -a \\( -type d -o -type f \\) -exec ls -ld {} \\;', correct the capture problem, and rerun PTxray."
      ;;
    empty)
      SWO_STATUS=$SWO_FAILCLOSED_EMPTY
      SWO_OBSERVED="not assessed - world-writable object probe returned empty output with rc 0"
      SWO_MEANING="PTxray did not obtain a trustworthy world-writable file and directory listing."
      SWO_FIX="run 'find / -perm -2 -a \\( -type d -o -type f \\) -exec ls -ld {} \\;', correct the capture problem, and rerun PTxray."
      ;;
    unparseable)
      SWO_STATUS=$SWO_FAILCLOSED_PARSE
      SWO_OBSERVED="not assessed - world-writable object listing could not be parsed"
      SWO_MEANING="PTxray did not obtain a trustworthy world-writable file and directory listing."
      SWO_FIX="run 'find / -perm -2 -a \\( -type d -o -type f \\) -exec ls -ld {} \\;', correct the capture problem, and rerun PTxray."
      ;;
    incomplete)
      SWO_STATUS=$SWO_FAILCLOSED_INCOMPLETE
      SWO_OBSERVED="not assessed - world-writable object scan did not complete (rc=$SWO_RC)"
      SWO_MEANING="The world-writable scan did not finish, so absence of objects outside public directories is not established."
      SWO_FIX="resolve traversal errors from 'find / -perm -2' and rerun PTxray."
      ;;
    fail)
      SWO_STATUS=$SWO_OUTSIDE_STATUS
      SWO_OBSERVED="world-writable objects outside public directories: $SWO_DETAIL"
      SWO_MEANING="A world-writable file or directory sits outside the public-directory allowlist, so any local account can modify that object."
      SWO_FIX="chmod o-w <file/directory>"
      ;;
    absent)
      SWO_STATUS=$SWO_ABSENT_STATUS
      SWO_OBSERVED="required public directory /tmp is not world-writable"
      SWO_MEANING="The required public directory /tmp is missing from the world-writable listing, so the public-directory requirement is unmet."
      SWO_FIX="restore the public directory /tmp with the mode required for system operation; PTxray only recommends this action."
      ;;
    manual)
      SWO_STATUS=$SWO_MANUAL_STATUS
      SWO_OBSERVED="not assessed - ISSO must confirm whether additional world-writable objects are required for proper system or application operation ($SWO_DETAIL)"
      SWO_MEANING="World-writable objects beyond /tmp and /dev/null may be required for operation; that exception is site/ISSO judgement and is not auto-passed."
      SWO_FIX="have the ISSO decide whether each extra world-writable object is required; if not, chmod o-w <file/directory>."
      ;;
    ok)
      SWO_STATUS=PASS
      SWO_OBSERVED="world-writable objects confined to public directories /tmp and /dev/null"
      if [ "$SWO_PARTIAL" -eq 1 ]; then
        SWO_OBSERVED="$SWO_OBSERVED; some paths unreadable"
      fi
      SWO_MEANING="The decidable world-writable scan found only the public-directory allowlist named by the STIG."
      SWO_FIX="n/a"
      ;;
    *)
      SWO_STATUS=NOT_ASSESSED
      SWO_OBSERVED="not assessed - unexpected world-writable parser result"
      SWO_MEANING="PTxray did not obtain a trustworthy world-writable file and directory listing."
      SWO_FIX="run 'find / -perm -2 -a \\( -type d -o -type f \\) -exec ls -ld {} \\;' and rerun PTxray."
      ;;
  esac

  case "$SWO_STATUS" in
    FAIL) SWO_SEV=med ;;
    NOT_APPLICABLE) SWO_SEV=low ;;
    *) SWO_SEV=med ;;
  esac

  add security stig_worldwrite_objects "World-writable objects confined to public directories" \
    "$SWO_STATUS" "$SWO_SEV" "$SWO_OBSERVED" \
    "$SWO_MEANING" "$SWO_FIX" "stig:V-215409"
}
