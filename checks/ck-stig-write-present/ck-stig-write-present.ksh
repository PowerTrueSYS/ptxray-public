...
AIXRAY_TOOL=ck-stig-write-present


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_write_present — DISA STIG V-215401.
  # Required configuration: /usr/bin/write must exist so admins can message
  # a currently logged-in user. Empty stdout is unproven for every nonzero
  # rc, including rc 2; FAIL requires a nonempty missing-file diagnostic.
  typeset SWP_RAW SWP_RC SWP_KIND SWP_STATUS SWP_SEV
  typeset SWP_OBSERVED SWP_MEANING SWP_FIX

  SWP_RAW=$(aix write_present ls -al /usr/bin/write)
  SWP_RC=$?
  SWP_KIND=""
  SWP_STATUS=""
  SWP_SEV=med
  ...
  if [ -z "$SWP_RAW" ]; then
    if [ "$SWP_RC" -eq 0 ]; then
      SWP_KIND=probe_empty
    else
      SWP_KIND=probe_failed
    fi
  else
    SWP_KIND=$(printf '%s\n' "$SWP_RAW" | awk '
      /No such file or directory/ || /does not exist/ || /0653-341/ {
        absent++
        next
      }
      /^[ \t]*$/ { next }
      ...
      }
      {
        ...
        found = 0
        for (i = 1; i <= NF; i++) {
          if ($i == "/usr/bin/write") found = 1
        }
        if (found) present++
        else other++
      }
      END {
        if (absent && present + 0 == 0) { print "absent"; exit }
        if (bad || other_err) { print "unparseable"; exit }
        if (records == 1 && present && other + 0 == 0) { print "present"; exit }
        if (records >= 1 && other && present + 0 == 0) { print "other"; exit }
        print "unparseable"
      }
    ')
    if [ -z "$SWP_KIND" ]; then
      SWP_KIND=unparseable
    fi
    if [ "$SWP_KIND" != "absent" ] && [ "$SWP_RC" -ne 0 ]; then
      SWP_KIND=probe_failed
    fi
  fi

  case "$SWP_KIND" in
    present) SWP_STATUS=PASS
      SWP_OBSERVED="/usr/bin/write is present"
      SWP_MEANING="The write command is installed, so an admin can send a message to a currently logged-in user."
      SWP_FIX="n/a"
      ;;
    absent) SWP_STATUS=FAIL
      SWP_OBSERVED="/usr/bin/write is absent"
      SWP_MEANING="/usr/bin/write is missing, so an admin cannot send a message to a currently logged-in user."
      SWP_FIX="installp -aXYgd /dev/cd0 -e /tmp/install.log bos.rte.misc_cmds; PTxray only recommends this command."
      ;;
    other) SWP_STATUS=FAIL
      SWP_OBSERVED="/usr/bin/write is not present"
      SWP_MEANING="The listing does not show /usr/bin/write, so an admin cannot send a message to a currently logged-in user."
      SWP_FIX="installp -aXYgd /dev/cd0 -e /tmp/install.log bos.rte.misc_cmds; PTxray only recommends this command."
      ;;
    probe_failed) SWP_STATUS=NOT_ASSESSED
      SWP_OBSERVED="not assessed - /usr/bin/write probe failed (rc=$SWP_RC)"
      SWP_MEANING="PTxray did not obtain trustworthy evidence that /usr/bin/write exists."
      SWP_FIX="run 'ls -al /usr/bin/write', correct the capture or path problem, and rerun PTxray."
      ;;
    probe_empty) SWP_STATUS=NOT_ASSESSED
      SWP_OBSERVED="not assessed - /usr/bin/write probe returned empty output"
      SWP_MEANING="PTxray did not obtain trustworthy evidence that /usr/bin/write exists."
      SWP_FIX="run 'ls -al /usr/bin/write', correct the capture or path problem, and rerun PTxray."
      ;;
    *) SWP_STATUS=NOT_ASSESSED
      SWP_OBSERVED="not assessed - /usr/bin/write listing was unparseable"
      SWP_MEANING="PTxray did not obtain trustworthy evidence that /usr/bin/write exists."
      SWP_FIX="run 'ls -al /usr/bin/write', correct the capture or path problem, and rerun PTxray."
      ;;
  esac
  ...
  case "$SWP_STATUS" in
    NOT_APPLICABLE) SWP_SEV=low ;;
    *) SWP_SEV=med ;;
  esac

  add security stig_write_present "write command present for currently logged-in users" \
    "$SWP_STATUS" "$SWP_SEV" "$SWP_OBSERVED" \
    "$SWP_MEANING" "$SWP_FIX" "stig:V-215401"
}
