...
AIXRAY_TOOL=ck-stig-wtmp-symlink


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # stig_wtmp_symlink — DISA STIG V-215285.
  # /var/adm/wtmp must be a real login accounting file, not a symlink.
  typeset SWT_RAW SWT_RC SWT_KIND SWT_PARSE_RC
  typeset SWT_STATUS SWT_OBSERVED SWT_MEANING SWT_FIX

  SWT_RAW=$(aix wtmp_ls ls -al /var/adm/wtmp)
  SWT_RC=$?
  SWT_KIND=""
  SWT_STATUS=""
  SWT_OBSERVED=""
  ...
  if [ -z "$SWT_RAW" ]; then
    if [ "$SWT_RC" -ne 0 ]; then
      SWT_KIND=probe_failed
    else
      SWT_KIND=probe_empty
    fi
  else
    SWT_KIND=$(printf '%s\n' "$SWT_RAW" | awk '
      /^[ \t]*$/ { next }
      /^ls:/ {
        if ($0 ~ /does not exist/ || $0 ~ /No such file/) absent++
        else bad = 1
        next
      }
      {
        ...
        t = substr(mode, 1, 1)
        if (t !~ /^[-dlcbps]$/) { bad = 1; next }
        found = 0
        for (i = 9; i <= NF; i++) {
          if ($i == "/var/adm/wtmp") { found = 1; break }
        }
        if (!found) { bad = 1; next }
        if (t == "l") kind = "symlink"
        else if (t == "-") kind = "file"
      ...
      END {
        if (absent && records == 0 && !bad) { print "absent"; exit 0 }
        if (bad || records != 1) { print "unparseable"; exit 1 }
        if (kind == "symlink") print "symlink"
        else if (kind == "file") print "file"
        else print "unparseable"
      }
    ')
    SWT_PARSE_RC=$?
    if [ "$SWT_PARSE_RC" -ne 0 ] || [ -z "$SWT_KIND" ]; then
      SWT_KIND=unparseable
    fi
    # Nonzero probe rc is untrustworthy except a recognized missing-file
    # diagnostic (ls typically prints that and returns 2).
    if [ "$SWT_KIND" != "absent" ] && [ "$SWT_RC" -ne 0 ]; then
      SWT_KIND=probe_failed
    fi
  fi
  ...
  case "$SWT_KIND" in
    file)
      SWT_STATUS=PASS
      SWT_OBSERVED="/var/adm/wtmp is a regular file (not a symlink)"
      ;;
    symlink)
      SWT_STATUS=FAIL
      SWT_OBSERVED="/var/adm/wtmp is a symlink"
      ;;
    absent)
      SWT_STATUS=FAIL
      SWT_OBSERVED="/var/adm/wtmp is absent"
      ;;
    probe_failed)
      SWT_STATUS=NOT_ASSESSED
      SWT_OBSERVED="not assessed - wtmp listing failed (rc=$SWT_RC)"
      ;;
    probe_empty)
      SWT_STATUS=NOT_ASSESSED
      SWT_OBSERVED="not assessed - wtmp listing returned no output"
      ;;
    *)
      SWT_STATUS=NOT_ASSESSED
      SWT_OBSERVED="not assessed - wtmp listing was unparseable"
      ;;
  esac

  case "$SWT_STATUS" in
    PASS)
      SWT_MEANING="Successful remote logins are recorded in a real /var/adm/wtmp file."
      SWT_FIX="n/a"
      ;;
    FAIL)
      SWT_MEANING="Successful remote logins are not recorded in /var/adm/wtmp because the path is a symlink or the file is absent."
      SWT_FIX="rm /var/adm/wtmp; PTxray only recommends this command."
      ;;
    *)
      SWT_MEANING="PTxray did not obtain a trustworthy listing of /var/adm/wtmp."
      SWT_FIX="run 'ls -al /var/adm/wtmp', correct the capture or path problem, and rerun PTxray."
      ;;
  esac

  add security stig_wtmp_symlink "wtmp is not a symlink" \
    "$SWT_STATUS" med "$SWT_OBSERVED" \
    "$SWT_MEANING" "$SWT_FIX" "stig:V-215285"
}
