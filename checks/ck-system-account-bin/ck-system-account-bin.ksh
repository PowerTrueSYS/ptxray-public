...
AIXRAY_TOOL=ck-system-account-bin
...
_AIXRAY_SESSION_KEYS=""
  # system_account_bin — evaluate the four AIX account gates for bin.
  typeset SA_RESULT SA_RESULT_RC SA_STATE SA_DETAIL SA_BAD
  if [ "${SYSACCT_RC:-127}" -ne 0 ] || [ "$SYSACCT_OK" -ne 1 ]; then
    nr_warn security system_account_bin "bin system account" \
      "system-account login attributes ($SYSACCT_WHY)" \
      "lsuser -a login rlogin shell account_locked ALL" \
      "cis-l1" NOT_ASSESSED high
  else
    SA_RESULT=$(printf '%s\n' "$SYSACCT_STATES" | awk -F'|' '$1=="bin" {print $2 "|" $3}')
    SA_RESULT_RC=$?
    if [ "$SA_RESULT_RC" -ne 0 ] || [ -z "$SA_RESULT" ]; then
      nr_warn security system_account_bin "bin system account" \
        "system-account state missing from validated capture" \
        "lsuser -a login rlogin shell account_locked ALL" \
        "cis-l1" NOT_ASSESSED high
    else
      ...
      case "$SA_STATE" in
      secured)
        add security system_account_bin "bin system account" PASS high "$SA_DETAIL" \
          "The bin account is locked and its local, remote, and shell login paths are disabled." \
          "n/a" "cis-l1" ;;
      absent)
        add security system_account_bin "bin system account" PASS high "$SA_DETAIL" \
          "The bin account is absent from the complete system account listing and cannot be used." \
          "n/a" "cis-l1" ;;
      *)
        SA_BAD=""
        ...
        add security system_account_bin "bin system account" FAIL high "$SA_DETAIL; requires login=false rlogin=false shell=/usr/bin/false account_locked=true; deviates: $SA_BAD" \
          "The bin account retains at least one usable login path or is not locked." \
          "set login=false, rlogin=false, shell=/usr/bin/false, and account_locked=true for bin." \
          "cis-l1" ;;
      esac
    fi
  fi
