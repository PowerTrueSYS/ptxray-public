...
AIXRAY_TOOL=ck-system-account-guest
...
_AIXRAY_SESSION_KEYS=""
  # system_account_guest — evaluate the four AIX account gates for guest.
  typeset SA_RESULT SA_RESULT_RC SA_STATE SA_DETAIL SA_BAD
  if [ "${SYSACCT_RC:-1}" -ne 0 ] || [ "${SYSACCT_OK:-0}" -ne 1 ]; then
    nr_warn security system_account_guest "guest system account" \
      "system-account login attributes ($SYSACCT_WHY)" \
      "lsuser -a login rlogin shell account_locked ALL" \
      "cis-l1" NOT_ASSESSED high
  else
    SA_RESULT=$(printf '%s\n' "$SYSACCT_STATES" | awk -F'|' '$1=="guest" {print $2 "|" $3}')
    SA_RESULT_RC=$?
    SA_STATE=${SA_RESULT%%\|*}; SA_DETAIL=${SA_RESULT#*\|}
    case "$SA_RESULT_RC:$SA_STATE" in
      0:secured)
        add security system_account_guest "guest system account" PASS high "$SA_DETAIL" \
          "The guest account is locked and its local, remote, and shell login paths are disabled." \
          "n/a" "cis-l1" ;;
      0:absent)
        add security system_account_guest "guest system account" PASS high "$SA_DETAIL" \
          "The guest account is absent from the complete system account listing and cannot be used." \
          "n/a" "cis-l1" ;;
      0:*)
        SA_BAD=""
        ...
        add security system_account_guest "guest system account" FAIL high "$SA_DETAIL; requires login=false rlogin=false shell=/usr/bin/false account_locked=true; deviates: $SA_BAD" \
          "The guest account retains at least one usable login path or is not locked." \
          "set login=false, rlogin=false, shell=/usr/bin/false, and account_locked=true for guest." \
          "cis-l1" ;;
      *)
        nr_warn security system_account_guest "guest system account" \
