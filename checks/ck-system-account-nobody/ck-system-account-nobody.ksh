...
AIXRAY_TOOL=ck-system-account-nobody

# Shared account-gate capture for the nine named system-account checks.
# lsuser omits any attribute it cannot report: an account whose /etc/passwd
# shell field is empty produces a row with NO shell= token (proven on real
# AIX 7.2). A missing or empty shell is evidence of an unsecured login path
# and must reach the verdict logic — it is never an evidence failure.
function capture_cap_system_accounts {
  SYSACCT_RAW=$(aix system_accounts lsuser -a login rlogin shell account_locked ALL)
  SYSACCT_RC=$?
  SYSACCT_OK=0
  SYSACCT_WHY=""
  SYSACCT_STATES=""

  if [ "$SYSACCT_RC" -ne 0 ]; then
    SYSACCT_WHY="capture failed (rc=$SYSACCT_RC)"
  elif [ -z "$SYSACCT_RAW" ]; then
    SYSACCT_WHY="capture empty (rc=0)"
  elif printf '%s\n' "$SYSACCT_RAW" | awk '
      NF {
        rows++
        if ($1 !~ /^[A-Za-z0-9_.-]+$/) bad=1
        for (seen_key in seen) delete seen[seen_key]
        login=""; rlogin=""; shell=""; locked=""
        for (i=2; i<=NF; i++) {
          split($i, pair, "=")
          key=pair[1]
          if (seen[key]++) bad=1
          if (key == "login") login=substr($i,7)
          else if (key == "rlogin") rlogin=substr($i,8)
          else if (key == "shell") shell=substr($i,7)
          else if (key == "account_locked") locked=substr($i,16)
          else bad=1
        }
        if (!("login" in seen) || !("rlogin" in seen) ||
            !("account_locked" in seen)) bad=1
        if (login !~ /^(true|false)$/ || rlogin !~ /^(true|false)$/ ||
            locked !~ /^(true|false)$/) bad=1
        if (("shell" in seen) && shell != "" &&
            shell !~ /^\/[A-Za-z0-9_.\/-]+$/) bad=1
        if (users[$1]++) bad=1
      }
  ...
  then
    SYSACCT_OK=1
    SYSACCT_STATES=$(printf '%s\n' "$SYSACCT_RAW" | awk '
      BEGIN {
        split("adm bin daemon guest lpd nobody nuucp sys uucp", target, " ")
        for (i=1; i<=9; i++) wanted[target[i]]=1
      }
      $1 in wanted {
        login=""; rlogin=""; shell=""; locked=""
        for (i=2; i<=NF; i++) {
          if (index($i,"login=")==1) login=substr($i,7)
          else if (index($i,"rlogin=")==1) rlogin=substr($i,8)
          else if (index($i,"shell=")==1) shell=substr($i,7)
          else if (index($i,"account_locked=")==1) locked=substr($i,16)
        }
        state=(login=="false" && rlogin=="false" &&
               shell=="/usr/bin/false" && locked=="true") ? "secured" : "insecure"
        detail="login=" login ", rlogin=" rlogin ", shell=" (shell=="" ? "(unset)" : shell) ", account_locked=" locked
        result[$1]=$1 "|" state "|" detail
      }
      END {
        for (i=1; i<=9; i++) {
          name=target[i]
          if (name in result) print result[name]
          else print name "|absent|account absent"
        }
      }
    ')
  else
    SYSACCT_WHY="capture unparseable (rc=0)"
  fi
  return 0
}
...
_AIXRAY_SESSION_KEYS=""
  # system_account_nobody — evaluate the four AIX account gates for nobody.
  typeset SA_RESULT SA_RESULT_RC SA_STATE SA_DETAIL SA_BAD
  if [ "${SYSACCT_RC:-1}" -ne 0 ] || [ "${SYSACCT_OK:-0}" -ne 1 ]; then
    nr_warn security system_account_nobody "nobody system account" \
      "system-account login attributes ($SYSACCT_WHY)" \
      "lsuser -a login rlogin shell account_locked ALL" \
      "cis-l1" NOT_ASSESSED high
  else
    SA_RESULT=$(printf '%s\n' "$SYSACCT_STATES" | awk -F'|' '$1=="nobody" {print $2 "|" $3}')
    SA_RESULT_RC=$?
    SA_STATE=${SA_RESULT%%\|*}; SA_DETAIL=${SA_RESULT#*\|}
    case "$SA_RESULT_RC:$SA_STATE" in
      0:secured)
        add security system_account_nobody "nobody system account" PASS high "$SA_DETAIL" \
          "The nobody account is locked and its local, remote, and shell login paths are disabled." \
          "n/a" "cis-l1" ;;
      0:absent)
        add security system_account_nobody "nobody system account" PASS high "$SA_DETAIL" \
          "The nobody account is absent from the complete system account listing and cannot be used." \
          "n/a" "cis-l1" ;;
      0:*)
        SA_BAD=""
        case "$SA_DETAIL" in "login=false, "*) ;; *) SA_BAD="login" ;; esac
        case "$SA_DETAIL" in *", rlogin=false, "*) ;; *) SA_BAD="${SA_BAD:+$SA_BAD }rlogin" ;; esac
        case "$SA_DETAIL" in *", shell=/usr/bin/false, "*) ;; *) SA_BAD="${SA_BAD:+$SA_BAD }shell" ;; esac
        case "$SA_DETAIL" in *", account_locked=true") ;; *) SA_BAD="${SA_BAD:+$SA_BAD }account_locked" ;; esac
        add security system_account_nobody "nobody system account" FAIL high "$SA_DETAIL; requires login=false rlogin=false shell=/usr/bin/false account_locked=true; deviates: $SA_BAD" \
          "The nobody account retains at least one usable login path or is not locked." \
          "set login=false, rlogin=false, shell=/usr/bin/false, and account_locked=true for nobody." \
          "cis-l1" ;;
      *)
        nr_warn security system_account_nobody "nobody system account" \
          "system-account state extraction failed (rc=$SA_RESULT_RC)" \
          "parse validated cap-system-accounts evidence" \
          "cis-l1" NOT_ASSESSED high ;;
    esac
  fi
}
...
function standalone_run {
  capture_cap_system_accounts || return 1
  standalone_check
}
