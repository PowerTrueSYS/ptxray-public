...
AIXRAY_TOOL=ck-system-accounts-ftp


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # system_accounts_ftpusers — root and the nine named system accounts must be
  # listed in /etc/ftpusers so none of them can authenticate over FTP.
  # An absent or empty /etc/ftpusers is a real FAIL (nothing is denied), never
  # an evidence refusal. Present-but-unreadable stays NOT_ASSESSED. No root
  # gate: presence is visible to any user and the file is world-readable when
  # created with default permissions.
  typeset SAF_LS_RAW SAF_LS_RC SAF_RAW SAF_RC SAF_PARSED SAF_STATE SAF_DETAIL
  SAF_LS_RAW=$(aix system_ftpusers_exists ls -ld /etc/ftpusers)
  SAF_LS_RC=$?
  if [ "$SAF_LS_RC" -ne 0 ] && [ -z "$SAF_LS_RAW" ]; then
    ...
    nr_warn security system_accounts_ftpusers "system accounts blocked from FTP" \
      "/etc/ftpusers presence (ls rc=$SAF_LS_RC with unexpected output)" \
      "ls -ld /etc/ftpusers" \
      "cis-l1" NOT_ASSESSED high
  else
    SAF_RAW=$(aix system_ftpusers cat /etc/ftpusers)
    SAF_RC=$?
    if [ "$SAF_RC" -ne 0 ]; then
      nr_warn security system_accounts_ftpusers "system accounts blocked from FTP" \
        "/etc/ftpusers (present but capture failed rc=$SAF_RC)" "cat /etc/ftpusers" \
        "cis-l1" NOT_ASSESSED high
    elif [ -z "$SAF_RAW" ]; then
      add security system_accounts_ftpusers "system accounts blocked from FTP" FAIL high \
      ...
      SAF_PARSED=$(printf '%s\n' "$SAF_RAW" | awk '
        BEGIN {
          split("root adm bin daemon guest lpd nobody nuucp sys uucp", required, " ")
        }
        /^[ \t]*($|#)/ { next }
        {
          if (NF != 1 || $1 !~ /^[A-Za-z0-9_.-]+$/) bad=1
          seen[$1]=1
        }
        END {
          if (bad) { print "bad|"; exit }
          for (i=1; i<=10; i++) if (!(required[i] in seen)) {
            missing=missing (missing ? ", " : "") required[i]
            count++
          }
          if (count) print "missing|" missing
          else print "complete|all 10 required accounts listed"
        }
      ')
      SAF_STATE=${SAF_PARSED%%\|*}
      SAF_DETAIL=${SAF_PARSED#*\|}
      case "$SAF_STATE" in
        complete)
          add security system_accounts_ftpusers "system accounts blocked from FTP" PASS high \
            "$SAF_DETAIL" \
            "Root and the nine named system accounts are denied FTP access by /etc/ftpusers." \
            "n/a" "cis-l1"
          ;;
        missing)
          add security system_accounts_ftpusers "system accounts blocked from FTP" FAIL high \
        ...
        *)
          nr_warn security system_accounts_ftpusers "system accounts blocked from FTP" \
            "/etc/ftpusers (present but unparseable rc=0)" "cat /etc/ftpusers" \
            "cis-l1" NOT_ASSESSED high
          ;;
      esac
    fi
