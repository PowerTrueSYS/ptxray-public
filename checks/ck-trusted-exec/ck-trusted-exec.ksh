...
AIXRAY_TOOL=ck-trusted-exec


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # trusted_exec — DISA STIG V-215335. TE must be on (deny-all, permit-by-
  # exception execution policy). trustchk is mode 500, so rc 127 proves the
  # subject is absent only when running as root; for any other user it is a
  # probe limitation and must render NOT_ASSESSED, never a verdict. Empty
  # stdout at rc 0 is determinate absence only when uid 0 is confirmed;
  # without that identity it remains a swallowed-error refusal.
  TE_OUT=$(aix trustchk_p trustchk -p TE); TE_RC=$?
  if [ "$TE_RC" -eq 127 ] && [ "${MYUID:-0}" = "0" ]; then
    add security trusted_exec "Trusted Execution policy" FAIL med \
        "TE=absent (trustchk not found)" \
        "Trusted Execution is not present, so it cannot satisfy the deny-all, permit-by-exception requirement of DISA STIG V-215335." \
        "install and enable Trusted Execution (trustchk -p TE=ON); PTxray only recommends this command." \
        "stig:V-215335"
  elif [ "$TE_RC" -ne 0 ]; then
    add security trusted_exec "Trusted Execution policy" NOT_ASSESSED med \
        "not assessed - trustchk -p TE failed (rc=$TE_RC)" \
        "PTxray could not read the Trusted Execution policy, so it cannot judge DISA STIG V-215335." \
        "rerun as root or correct the capture problem, then rerun PTxray." \
        "stig:V-215335"
  elif [ -z "$TE_OUT" ]; then
    if [ "${MYUID:-}" = "0" ]; then
      add security trusted_exec "Trusted Execution policy" FAIL med \
          "TE=absent (no policy output)" \
          "Trusted Execution policy is not configured, so it cannot satisfy the deny-all, permit-by-exception requirement of DISA STIG V-215335." \
          "install and enable Trusted Execution (trustchk -p TE=ON); PTxray only recommends this command." \
          "stig:V-215335"
    else
      add security trusted_exec "Trusted Execution policy" NOT_ASSESSED med \
          "not assessed - trustchk -p TE returned no output" \
          "An empty policy response is a swallowed error, not evidence of compliance with DISA STIG V-215335." \
          "rerun trustchk -p TE and confirm it prints TE=ON or TE=OFF." \
          "stig:V-215335"
    fi
  elif printf '%s\n' "$TE_OUT" | grep -q 'TE=ON'; then
    add security trusted_exec "Trusted Execution policy" PASS med \
        "TE=ON" \
        "Trusted Execution is enabled, so every program is integrity-checked before it runs, satisfying DISA STIG V-215335." \
        "n/a" "stig:V-215335"
  elif printf '%s\n' "$TE_OUT" | grep -q 'TE=OFF'; then
    add security trusted_exec "Trusted Execution policy" FAIL med \
        "TE=OFF" \
        "Trusted Execution is off, so the kernel loads any binary without an integrity check, violating DISA STIG V-215335." \
        "enable Trusted Execution with trustchk -p TE=ON; PTxray only recommends this command." \
        "stig:V-215335"
  else
    add security trusted_exec "Trusted Execution policy" NOT_ASSESSED med \
        "not assessed - trustchk -p TE output unparseable" \
        "PTxray could not parse the Trusted Execution policy into TE=ON or TE=OFF, so it cannot judge DISA STIG V-215335." \
        "inspect the trustchk -p TE output and correct the capture." \
        "stig:V-215335"
  fi
}
