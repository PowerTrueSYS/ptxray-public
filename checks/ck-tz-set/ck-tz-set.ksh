...
AIXRAY_TOOL=ck-tz-set


function standalone_check {
_AIXRAY_SESSION_KEYS=""
# tz_set — DISA STIG V-215255 (AIX7-00-002038).
# TZ in /etc/environment is UTC, GMT, or a POSIX offset from UTC, so a
# non-empty TZ= line passes. Unset TZ (grep rc=1) or an empty TZ= value
# is a FAIL: the timezone must be configured. A probe that cannot prove
# the subject fails closed as NOT_ASSESSED.

typeset TZ_RAW TZ_RC TZ_VAL TZ_REASON

TZ_RAW=$(aix etc_environment grep '^TZ=' /etc/environment)
TZ_RC=$?
TZ_VAL=""
TZ_REASON=""
...
if [ "$TZ_RC" -ne 0 ]; then
  if [ "$TZ_RC" -eq 1 ]; then
    if [ -n "$TZ_RAW" ]; then
      # grep reported no match yet emitted output: contradictory evidence, so
      # the absence of a TZ= line cannot be asserted.
      TZ_REASON="contradictory (rc=1 with output)"
    else
      # grep found no TZ= line: the timezone is not configured.
      TZ_REASON="subject-absent"
    fi
  else
    TZ_REASON="capture failed (rc=$TZ_RC)"
  fi
elif [ -z "$TZ_RAW" ]; then
  # rc=0 with no output is a swallowed error, not proof of absence.
  TZ_REASON="empty"
else
  TZ_VAL=$(printf '%s\n' "$TZ_RAW" | awk 'NR==1 && sub(/^TZ=/, ""){print; exit}')
  if [ -z "$TZ_VAL" ]; then
    # A non-empty capture yielding no value is either an empty TZ= entry
    # (invalid, FAIL) or a line grep could not have emitted (unparseable).
    case "$TZ_RAW" in
      TZ=*) TZ_REASON="empty-value" ;;
      *) TZ_REASON="unparseable" ;;
    esac
  fi
fi

if [ "$TZ_REASON" = "subject-absent" ]; then
  add config tz_set "Timezone (TZ)" FAIL med \
      "TZ is not set in /etc/environment" \
      "TZ is not set in /etc/environment, so audit record timestamps cannot be mapped to UTC or GMT as required by DISA STIG V-215255." \
      "run 'chtz UTC', then reboot; PTxray only recommends this command." \
      "stig:V-215255"
elif [ "$TZ_REASON" = "empty-value" ]; then
  add config tz_set "Timezone (TZ)" FAIL med \
      "TZ is set to an empty value in /etc/environment" \
      "TZ is set but empty in /etc/environment, so audit record timestamps cannot be mapped to UTC or GMT as required by DISA STIG V-215255." \
      "run 'chtz UTC', then reboot; PTxray only recommends this command." \
      "stig:V-215255"
elif [ -n "$TZ_REASON" ]; then
  add config tz_set "Timezone (TZ)" NOT_ASSESSED med \
      "not assessed - TZ evidence $TZ_REASON" \
      "PTxray did not obtain a trustworthy TZ= line from /etc/environment proving the system timezone is an offset from UTC." \
      "correct the capture or path problem, then rerun PTxray." \
      "stig:V-215255"
else
  add config tz_set "Timezone (TZ)" PASS med \
      "TZ=$TZ_VAL" \
      "TZ=$TZ_VAL is set in /etc/environment; any POSIX TZ is an offset from UTC, so audit record timestamps can be mapped to UTC or GMT as required by DISA STIG V-215255." \
      "n/a" "stig:V-215255"
fi
}
