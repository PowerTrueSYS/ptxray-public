...
_AIXRAY_SESSION_KEYS=""
  ...
  VSRAW=$(aix lsmap_vscsi ioscli lsmap -all); VSRAW_RC=$?
  if [ "$VSRAW_RC" -eq 0 ] && [ -n "$VSRAW" ] && printf '%s\n' "$VSRAW" | awk '$1 ~ /^vhost[0-9]/{f=1} END{exit f?0:1}'; then
    ...
    add storage vscsi_maps "Virtual SCSI mappings" NOT_APPLICABLE low "no vhost lines from 'ioscli lsmap -all'" \
        "not applicable: no vhost adapters — 'ioscli lsmap -all' produced no vhost lines, so there are no virtual SCSI mappings to check." "n/a"
  fi
}
