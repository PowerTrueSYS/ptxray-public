...
AIXRAY_TOOL=ck-world-writable


function standalone_check {
_AIXRAY_SESSION_KEYS=""
  # world_writable (scope bounded to /etc and /usr/local/bin on purpose)
  WW_ETC_EX=$(aix ww_root_etc_exists sh -c 'test -d /etc && echo yes || echo no'); WW_ETC_EX_RC=$?
  WW_USR_EX=$(aix ww_root_usrlocalbin_exists sh -c 'test -d /usr/local/bin && echo yes || echo no'); WW_USR_EX_RC=$?
  WW_ETC=$(aix find_ww_etc find /etc -xdev -type f -perm -002 -print); WW_ETC_RC=$?
  WW_USR=$(aix find_ww_usrlocalbin find /usr/local/bin -xdev -type f -perm -002 -print); WW_USR_RC=$?
  WW=""
  WW_SCANNED=""
  WW_SKIPPED=""
  WW_FAIL_ROOT=""
  WW_FAIL_RC=0
  if [ "$WW_ETC_EX_RC" -ne 0 ]; then
    WW_FAIL_ROOT="/etc"
    WW_FAIL_RC=$WW_ETC_EX_RC
  elif [ "$WW_ETC_EX" = no ]; then
    WW_SKIPPED="/etc"
  elif [ "$WW_ETC_EX" = yes ]; then
    if [ "$WW_ETC_RC" -ne 0 ]; then
      WW_FAIL_ROOT="/etc"
      WW_FAIL_RC=$WW_ETC_RC
    else
      WW_SCANNED="/etc"
      WW=$WW_ETC
    fi
  ...
  if [ "$WW_USR_EX_RC" -ne 0 ]; then
    if [ -z "$WW_FAIL_ROOT" ]; then
      WW_FAIL_ROOT="/usr/local/bin"
      WW_FAIL_RC=$WW_USR_EX_RC
    fi
  elif [ "$WW_USR_EX" = no ]; then
    if [ -n "$WW_SKIPPED" ]; then
      WW_SKIPPED="$WW_SKIPPED, /usr/local/bin"
    else
      WW_SKIPPED="/usr/local/bin"
    fi
  elif [ "$WW_USR_EX" = yes ]; then
    if [ "$WW_USR_RC" -ne 0 ]; then
      if [ -z "$WW_FAIL_ROOT" ]; then
        WW_FAIL_ROOT="/usr/local/bin"
        WW_FAIL_RC=$WW_USR_RC
      fi
    else
      if [ -n "$WW_SCANNED" ]; then
        WW_SCANNED="$WW_SCANNED, /usr/local/bin"
      else
        WW_SCANNED="/usr/local/bin"
      fi
      if [ -n "$WW_USR" ]; then
        if [ -n "$WW" ]; then
          WW=$(printf '%s\n%s' "$WW" "$WW_USR")
  ...
  if [ -n "$WW_FAIL_ROOT" ]; then
    add security world_writable "World-writable files" NOT_ASSESSED high \
        "not assessed — $WW_FAIL_ROOT world-writable scan failed (rc=$WW_FAIL_RC)" \
    ...
    add security world_writable "World-writable files" FAIL high "$WWN" \
    ...
    if [ -n "$WW_SKIPPED" ]; then
      if [ -n "$WW_SCANNED" ]; then
        WW_OBS="no world-writable files in $WW_SCANNED ($WW_SKIPPED not present, skipped)"
      else
        WW_OBS="$WW_SKIPPED not present, skipped"
      fi
    else
      WW_OBS="none in /etc, /usr/local/bin"
    fi
    add security world_writable "World-writable files" PASS low "$WW_OBS" \
